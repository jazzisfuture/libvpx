...
void store_packet(const vpx_codec_cx_pkt_t *pkt, vpx_codec_cx_pkt_t **altref_pkt) {
  *altref_pkt = malloc(sizeof(**altref_pkt));
  (*altref_pkt)->kind = pkt->kind;
  (*altref_pkt)->data.frame.buf = malloc(pkt->data.frame.sz);
  memcpy((*altref_pkt)->data.frame.buf, pkt->data.frame.buf, pkt->data.frame.sz);
...
void prepend_metadata(vpx_codec_cx_pkt_t *pkt, unsigned char control_byte) {
  ...
  /* data */
  memcpy((unsigned char *)data + offset, pkt->data.frame.buf, pkt->data.frame.sz);
  pkt->data.frame.sz += length + 1;
  pkt->data.frame.buf = data;
}

void pack_frames(const vpx_codec_cx_pkt_t **pkt, vpx_codec_cx_pkt_t **altref_pkt) {
  vpx_codec_cx_pkt_t *packed_pkt;
  packed_pkt = malloc(sizeof(*packed_pkt));
  packed_pkt->kind = (*pkt)->kind;
  packed_pkt->data.frame.sz = (*pkt)->data.frame.sz + (*altref_pkt)->data.frame.sz;
  ...
  memcpy((unsigned char *)packed_pkt->data.frame.buf, (*altref_pkt)->data.frame.buf, (*altref_pkt)->data.frame.sz);
  memcpy((unsigned char *)packed_pkt->data.frame.buf + (*altref_pkt)->data.frame.sz, (*pkt)->data.frame.buf, (*pkt)->data.frame.sz);
        ...
        switch (pkt->kind) {
          case VPX_CODEC_CX_FRAME_PKT:
            ...
            if (write_webm) {
              ...
              prepend_metadata(pkt, (pkt->data.frame.flags & VPX_FRAME_IS_INVISIBLE) ? 0x01 : 0x00);
              if(altref_pkt != NULL) {
