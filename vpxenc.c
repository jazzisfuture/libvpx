...
static const arg_def_t active_wq_factor = ARG_DEF(
    NULL, "active-wq-factor", 1, "Active worst quality factor");
static const arg_def_t base_err_per_mb = ARG_DEF(
    NULL, "base-err-per-mb", 1, "Base error per macroblock");
static const arg_def_t sr_default_decay_limit = ARG_DEF(
    NULL, "sr-default-decay-limit", 1, "Second reference default decay limit");
static const arg_def_t sr_diff_part = ARG_DEF(
    NULL, "sr-diff-part", 1, "Second reference diff");
static const arg_def_t kf_err_per_mb = ARG_DEF(
    NULL, "kf-err-per-mb", 1, "Keyframe error per macroblock");
static const arg_def_t kf_frame_min_boost = ARG_DEF(
    NULL, "kf-frame-min-boost", 1, "Keyframe min boost");
...
static const arg_def_t kf_max_total_boost = ARG_DEF(
    NULL, "kf-max-total-boost", 1, "Keyframe max total boost");
static const arg_def_t gf_max_total_boost = ARG_DEF(
    NULL, "gf-max-total-boost", 1, "Golden frame max total boost");
static const arg_def_t gf_frame_max_boost = ARG_DEF(
    NULL, "gf-frame-max-boost", 1, "Golden frame max boost");
static const arg_def_t zm_power_factor = ARG_DEF(
    NULL, "zm-power-factor", 1, "Zero motion power factor");
static const arg_def_t *vizier_rc_args[] = {
  &active_wq_factor, &base_err_per_mb, &sr_default_decay_limit,
  &sr_diff_part, &kf_err_per_mb, &kf_frame_min_boost,
  &kf_frame_max_boost_first, &kf_frame_max_boost_subs,
  &kf_max_total_boost, &gf_max_total_boost, &gf_frame_max_boost,
  &zm_power_factor, NULL
};
...
static int parse_stream_params(struct VpxEncoderConfig *global,
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    } else if (arg_match(&arg, &sr_diff_part, argi)) {
      config->cfg.sr_diff_part = arg_parse_rational(&arg);
