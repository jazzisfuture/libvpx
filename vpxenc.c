...
#define warn_or_exit_on_error(ctx, fatal, ...) \
  do { \
    if (fatal == TEST_DECODE_FATAL) { \
      ctx_exit_on_error(ctx, __VA_ARGS__); \
    } else { \
      warn(__VA_ARGS__); \
    } \
  } while (0)
        ...
        *got_data = 1;
        ...
        if (global->test_decode != TEST_DECODE_OFF && !stream->mismatch_seen) {
          ...
          if (stream->decoder.err) {
            warn_or_exit_on_error(&stream->decoder, global->test_decode,
...
static void test_decode(struct stream_state  *stream,
  ...
  if (!compare_img(&stream->ref_enc.img, &stream->ref_dec.img)) {
    ...
    find_mismatch(&stream->ref_enc.img, &stream->ref_dec.img,
                  y, u, v);
    warn_or_exit_on_error(&stream->decoder, fatal,
