...
void usage_exit() {
  ...
  for (i = 0; i < get_vpx_encoder_count(); ++i) {
    const VpxIface *const encoder = get_vpx_encoder(i);
    fprintf(stderr, "    %-6s - %s\n",
            encoder->name, vpx_codec_iface_name(encoder->iface()));
  }
...
static void parse_global_config(struct VpxEncoderConfig *global, char **argv) {
  ...
  global->codec = get_vpx_encoder(0);
  global->passes = 0;
  global->use_i420 = 1;
...
static void initialize_encoder(struct stream_state *stream,
  ...
  if (global->test_decode != TEST_DECODE_OFF) {
    const VpxIface *decoder = get_vpx_decoder_by_name(global->codec->name);
    vpx_codec_dec_init(&stream->decoder, decoder->iface(), NULL, 0);
  }
#endif
}
