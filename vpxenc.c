...
int main(int argc, const char **argv_)
{
    ...
    for (pass = one_pass_only ? one_pass_only - 1 : 0; pass < arg_passes; pass++)
    {
        ...
        if (!vpxio_open_src(&input_ctx, in_fn))                                             //try to open y4m or ivf file to read from
            if (!vpxio_open_i420_src(&input_ctx, argv[0], cfg.g_w, cfg.g_h))                //try yuv file to read from
            {
                printf("Failed to open input file: %s", vpxio_get_file_name(&input_ctx));   //exit if file cant be opened
                return 1;
            }

        cfg.g_w = vpxio_get_width(&input_ctx);          //set g_w to width
        cfg.g_h = vpxio_get_height(&input_ctx);         //set g_h to height
        arg_use_i420 = vpxio_get_use_i420(&input_ctx);  //check i420
...
#define SHOW(field) fprintf(stderr, "    %-28s = %d\n", #field, cfg.field)
        ...
        if (cfg.g_pass == VPX_RC_ONE_PASS || cfg.g_pass == VPX_RC_LAST_PASS)
        {
            ...
            if (write_webm)
            {
                if (!vpxio_open_webm_dst(&output_ctx, out_fn, cfg.g_w, cfg.g_h, arg_framerate.den, arg_framerate.num))
                {
            ...
            {
                if (!vpxio_open_ivf_dst(&output_ctx, out_fn, cfg.g_w, cfg.g_h, arg_framerate.den, arg_framerate.num))
                {
                    printf("Failed to open output file: %s", vpxio_get_file_name(&output_ctx));
                    break;
                }
        ...
        while (frame_avail || got_data)
        {
            ...
            {
                frame_avail = vpxio_read_img(&input_ctx, &img_raw);
            ...
            vpx_codec_encode(&encoder, frame_avail ? &img_raw : NULL, frame_start,
            ...
            while ((pkt = vpx_codec_get_cx_data(&encoder, &iter)))
            {
                ...
                case VPX_CODEC_CX_FRAME_PKT:
                    frames_out++;
                    nbytes = nbytes + vpxio_write_pkt(&output_ctx, pkt);
                    break;
