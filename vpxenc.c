...
int main(int argc, const char **argv_)
{
    ...
    for (pass = one_pass_only ? one_pass_only - 1 : 0; pass < arg_passes; pass++)
    {
        ...
        if ((input_ctx = vpxio_open_src(input_ctx, in_fn)) == NULL)
            if ((input_ctx = vpxio_open_raw_src(input_ctx, 
                                 in_fn, 
                                 cfg.g_w, 
                                 cfg.g_h, 
                                 arg_use_i420 ? VPX_IMG_FMT_YV12 : 
                                 VPX_IMG_FMT_I420 )) == NULL)
            {
...
#define SHOW(field) fprintf(stderr, "    %-28s = %d\n", #field, cfg.field)
        ...
        if (cfg.g_pass == VPX_RC_ONE_PASS || cfg.g_pass == VPX_RC_LAST_PASS)
        {
            if (write_webm)
            {
                if ((output_ctx = vpxio_open_webm_dst(output_ctx, out_fn, cfg.g_w, cfg.g_h,
                    arg_framerate.den, arg_framerate.num)) == NULL)
                {
                    printf("Failed to open output file: %s",
                        vpxio_get_file_name(output_ctx));
                    break;
                }
            }
            else
            {
                if ((output_ctx = vpxio_open_ivf_dst(output_ctx, out_fn, cfg.g_w, cfg.g_h,
                    arg_framerate.den, arg_framerate.num)) == NULL)
                {
                    printf("Failed to open output file: %s",
                        vpxio_get_file_name(output_ctx));
