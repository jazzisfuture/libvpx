...
#if CONFIG_VP9_HIGH
static void high_high_img_upshift(vpx_image_t *dst, vpx_image_t *src,
                                  int input_shift) {
...
static void high_high_img_downshift(vpx_image_t *dst, vpx_image_t *src,
                                    int input_shift) {
  int plane;
  assert(input_shift >= 0);
  for (plane = 0; plane < 3; plane++) {
...
static void high_high_img_shift(vpx_image_t *dst, vpx_image_t *src,
                                int input_shift) {
  if (dst->fmt != src->fmt)
    fatal("Unsupported image conversion");
  if (input_shift > 0) {
    high_high_img_upshift(dst, src, input_shift);
  } else {
    high_high_img_downshift(dst, src, -input_shift);
  }
}

static void low_high_img_shift(vpx_image_t *dst, vpx_image_t *src,
                               int input_shift) {
  int plane;
...
static void high_img_shift(vpx_image_t *dst, vpx_image_t *src,
                           int input_shift) {
  ...
  if (dst->fmt & VPX_IMG_FMT_HIGH) {
    high_high_img_shift(dst, src, input_shift);
  } else {
    low_high_img_shift(dst, src, input_shift);
  }
}

static void low_img_shift(vpx_image_t *dst, vpx_image_t *src,
                          int input_shift) {
  // Note the offset is 1 less than half
  const int offset = input_shift > 0 ? (1 << (input_shift - 1)) - 1 : 0;
  int plane;
...
static void img_shift(vpx_image_t *dst, vpx_image_t *src,
                      int input_shift) {
  if (src->fmt & VPX_IMG_FMT_HIGH) {
    high_img_shift(dst, src, input_shift);
  } else {
    low_img_shift(dst, src, input_shift);
  }
}
  ...
  for (pass = global.pass ? global.pass - 1 : 0; pass < global.passes; pass++) {
    ...
    while (frame_avail || got_data) {
      ...
      if (frames_in > global.skip_frames) {
        ...
        vpx_usec_timer_start(&timer);
#if CONFIG_VP9_HIGH
        if (input_shift || use_16bit_internal) {
          // Input bit depth and stream bit depth do not match, so up/down
          ...
          img_shift(&raw_shift, &raw, input_shift);
