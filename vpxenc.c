...
#if CONFIG_VP9_HIGH
static const arg_def_t bitdeptharg = ARG_DEF(NULL, "bit-depth", 1,
                                          "Bit depth used to use (0=BITS_8 for version <=1, 1=BITS_10 or 2=BITS_12 for version 2)");
#endif
...
static void encode_frame(struct stream_state *stream,
  ...
  /* Scale if necessary */
#if CONFIG_VP9_HIGH
  if (img && (img->fmt&VPX_IMG_FMT_HIGH) && (img->d_w != cfg->g_w || img->d_h != cfg->g_h)) {
    if (!stream->img)
...
#if CONFIG_VP9_HIGH
static void img_convert_8_to_16(vpx_image_t  *dst,vpx_image_t  *src, int input_shift) {
  int plane;
  if (dst->fmt != src->fmt + VPX_IMG_FMT_HIGH || dst->w != src->w || dst->h != src->h || dst->x_chroma_shift != src->x_chroma_shift || dst->y_chroma_shift != src->y_chroma_shift) {
  ...
  for(plane=0;plane<3;plane++) {
    ...
    for(y=0;y<h;y++) {
      ...
      unsigned short *p_dst = (unsigned short *)(dst->planes[plane] + y * dst->stride[plane]);
      for(x=0;x<w;x++) {
