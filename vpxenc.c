...
write_webm_block(EbmlGlobal                *glob,
  ...
  if (pkt->data.frame.flags & VPX_FRAME_IS_INVISIBLE) {
    ...
    memcpy(altref_frame.buf, (const void *)pkt->data.frame.buf
               , altref_frame.buf_size);
  } else {
    /* Write the Simple Block */
    Ebml_WriteID(glob, SimpleBlock);
    if (!altref_frame.is_packed) {
      control_byte = 0;
      ...
      control_byte = 1;
      packed_block_length = block_length + altref_frame.block_length + 8 + 2;
