...

        {
            ...
            Ebml_StartSubElement(glob, &trackStart, Tracks);
            {
                ...
                Ebml_SerializeString(glob, CodecID, "V_VP8");
                {
                    ...
                    Ebml_SerializeUnsigned(glob, HasAlpha, has_alpha);
...
static const arg_def_t has_alpha = ARG_DEF(NULL, "alpha", 0,
...
{
    &use_yv12, &use_i420, &has_alpha, &usage, &threads, &profile,
...
static void parse_global_config(struct global_config *global, char **argv)
{
    ...
    for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step)
    {
        ...
        else if (arg_match(&arg, &pass_arg, argi))
        {
            ...
            global->use_i420 = 1;
        else if (arg_match(&arg, &has_alpha, argi))
            global->has_alpha = 1;
