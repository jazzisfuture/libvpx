...
void vpx_hadamard_8x8_lsx(const int16_t *src, ptrdiff_t src_stride,
  ...
  LSX_BUTTERFLY_8_H(src0, src1, src2, src3, src7, src6, src5, src4, tmp0, tmp7,
                    tmp3, tmp4, tmp5, tmp1, tmp6, tmp2);
  __lsx_vst(tmp0, dst, 0);    // 0 + 4
  __lsx_vst(tmp1, dst, 16);   // 2 - 6
  __lsx_vst(tmp2, dst, 32);   // 0 - 4
  __lsx_vst(tmp3, dst, 48);   // 2 + 6
  __lsx_vst(tmp4, dst, 64);   // 3 + 7
  __lsx_vst(tmp5, dst, 80);   // 3 - 7
  __lsx_vst(tmp6, dst, 96);   // 1 - 5
  __lsx_vst(tmp7, dst, 112);  // 1 + 5
}
