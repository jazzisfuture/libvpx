...
void vpx_convolve8_horz_t2_vert_t2_neon(const uint8_t *src,
   ...
   * maximum buffer size to 64 * 64 + 7 (+ 1 to make it divisible by 4).
   */
  DECLARE_ALIGNED(16, uint8_t, temp[64 * 72]);
  // Account for the vertical phase needing 3 lines prior and 4 lines post
  int intermediate_height = h + 8;

  if (x_step_q4 != 16 || y_step_q4 != 16)
    return vpx_convolve8_c(src, src_stride, dst, dst_stride, filter, x0_q4,
                           x_step_q4, y0_q4, y_step_q4, w, h);
   ...
   */
  vpx_convolve8_horiz_filter_type2_neon(src - src_stride * 3, src_stride, temp,
                                        w, filter, x0_q4, x_step_q4, y0_q4,
                                        y_step_q4, w, intermediate_height);

  /* Step into the temp buffer 3 lines to get the actual frame data */
  vpx_convolve8_vert_filter_type2_neon(temp + w * 3, w, dst, dst_stride, filter,
                                       x0_q4, x_step_q4, y0_q4, y_step_q4, w,
                                       h);
}

void vpx_convolve8_avg_horz_t2_vert_t2_neon(const uint8_t *src,
                                            ptrdiff_t src_stride, uint8_t *dst,
                                            ptrdiff_t dst_stride,
  ...
  if (x_step_q4 != 16 || y_step_q4 != 16)
    return vpx_convolve8_avg_c(src, src_stride, dst, dst_stride, filter, x0_q4,
                               x_step_q4, y0_q4, y_step_q4, w, h);
  ...
  vpx_convolve8_horiz_filter_type2_neon(src - src_stride * 3, src_stride, temp,
                                        w, filter, x0_q4, x_step_q4, y0_q4,
                                        y_step_q4, w, intermediate_height);
  vpx_convolve8_avg_vert_filter_type2_neon(temp + w * 3, w, dst, dst_stride,
                                           filter, x0_q4, x_step_q4, y0_q4,
                                           y_step_q4, w, h);
}

void vpx_convolve8_horz_t2_vert_t1_neon(const uint8_t *src,
                                        ptrdiff_t src_stride, uint8_t *dst,
                                        ptrdiff_t dst_stride,
                                        const InterpKernel *filter, int x0_q4,
                                        int x_step_q4, int y0_q4, int y_step_q4,
  ...
  /* Step into the temp buffer 3 lines to get the actual frame data */
  vpx_convolve8_vert_filter_type1_neon(temp + w * 3, w, dst, dst_stride, filter,
                                       x0_q4, x_step_q4, y0_q4, y_step_q4, w,
                                       h);
}

void vpx_convolve8_avg_horz_t2_vert_t1_neon(const uint8_t *src,
                                            ptrdiff_t src_stride, uint8_t *dst,
                                            ptrdiff_t dst_stride,
                                            const InterpKernel *filter,
                                            int x0_q4, int x_step_q4, int y0_q4,
   ...
   * to average the values after both passes.
   */
  vpx_convolve8_horiz_filter_type2(src - src_stride * 3, src_stride, temp, w,
                                   filter, x0_q4, x_step_q4, y0_q4, y_step_q4,
                                   w, intermediate_height);
  vpx_convolve8_avg_vert_filter_type1_neon(temp + w * 3, w, dst, dst_stride,
                                           filter, x0_q4, x_step_q4, y0_q4,
                                           y_step_q4, w, h);
}

void vpx_convolve8_horz_t1_vert_t2_neon(const uint8_t *src,
                                        ptrdiff_t src_stride, uint8_t *dst,
                                        ptrdiff_t dst_stride,
                                        const InterpKernel *filter, int x0_q4,
                                        int x_step_q4, int y0_q4, int y_step_q4,
   ...
   * this is safe if somewhat less than ideal.
   */
  vpx_convolve8_horiz_filter_type1_neon(src - src_stride * 3, src_stride, temp,
                                        w, filter, x0_q4, x_step_q4, y0_q4,
                                        y_step_q4, w, intermediate_height);
...
void vpx_convolve8_avg_horz_t1_vert_t2_neon(const uint8_t *src,
                                            ptrdiff_t src_stride, uint8_t *dst,
                                            ptrdiff_t dst_stride,
                                            const InterpKernel *filter,
                                            int x0_q4, int x_step_q4, int y0_q4,
...
void vpx_convolve8_horz_t1_vert_t1_neon(const uint8_t *src,
                                        ptrdiff_t src_stride, uint8_t *dst,
                                        ptrdiff_t dst_stride,
                                        const InterpKernel *filter, int x0_q4,
                                        int x_step_q4, int y0_q4, int y_step_q4,
...
void vpx_convolve8_avg_horz_t1_vert_t1_neon(const uint8_t *src,
                                            ptrdiff_t src_stride, uint8_t *dst,
                                            ptrdiff_t dst_stride,
                                            const InterpKernel *filter,
                                            int x0_q4, int x_step_q4, int y0_q4,
