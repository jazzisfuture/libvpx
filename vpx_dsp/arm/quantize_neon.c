...
void vpx_quantize_b_neon(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  // Process first 8 values which include a dc component.
  {
    ...
    const int16x8_t iscan = vaddq_s16(vld1q_s16(iscan_ptr), one);

    const int16x8_t coeff = load_tran_low_to_s16q(coeff_ptr);
    const int16x8_t coeff_sign = vshrq_n_s16(coeff, 15);
    const int16x8_t coeff_abs = vabsq_s16(coeff);

    const int16x8_t zbin_mask = vcgeq_s16(coeff_abs, zbin);
    ...
    eob_max = vandq_s16(vtstq_s16(qcoeff, neg_one), iscan);
...

  {
    ...
    do {
      ...
      // Set non-zero elements to -1 and use that to extract values for eob.
      eob_max =
          vmaxq_s16(eob_max, vandq_s16(vtstq_s16(qcoeff, neg_one), iscan));
  ...
  {
    const int16x4_t eob_max_0 =
        vmax_s16(vget_low_s16(eob_max), vget_high_s16(eob_max));
    const int32x4_t eob_max_1 = vmovl_s16(eob_max_0);
    const int32x2_t eob_max_2 =
        vmax_s32(vget_low_s32(eob_max_1), vget_high_s32(eob_max_1));
    const int32x2_t eob_max_3 = vpmax_s32(eob_max_2, eob_max_2);
    vst1_lane_s16(eob_ptr, vreinterpret_s16_s32(eob_max_3), 0);
  }
}
