...
// 14  98 106 115 127
// 15 117 128
static void highbd_vpx_idct32_12_neon(const tran_low_t *const input,
                                      int32_t *output) {
...
static void highbd_vpx_idct32_16_neon(const int32_t *const input,
...
void vpx_highbd_idct32x32_135_add_neon(const tran_low_t *input, uint8_t *dest,
  ...
  if (bd == 8) {
    ...
    int32_t temp[32 * 16];
    int32_t *t = temp;
    highbd_vpx_idct32_12_neon(input, temp);
    highbd_vpx_idct32_12_neon(input + 32 * 8, temp + 8);

    for (i = 0; i < 32; i += 8) {
      highbd_vpx_idct32_16_neon(t, dst, stride, bd);
      t += (16 * 8);
      dst += 8;
    }
  }
