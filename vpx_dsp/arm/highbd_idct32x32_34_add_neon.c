...
// 6 21 27 33
// 7 24 32
static void highbd_vpx_idct32_6_neon(const tran_low_t *input, int32_t *output) {
...
static void highbd_vpx_idct32_8_neon(const int32_t *input, uint16_t *output,
...
void vpx_highbd_idct32x32_34_add_neon(const tran_low_t *input, uint8_t *dest,
  ...
  if (bd == 8) {
    ...
    highbd_vpx_idct32_6_neon(input, t);

    for (i = 0; i < 32; i += 8) {
      highbd_vpx_idct32_8_neon(t, dst, stride, bd);
      t += (8 * 8);
      dst += 8;
    }
  }
