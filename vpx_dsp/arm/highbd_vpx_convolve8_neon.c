...
static INLINE void load_8x8s(const int16_t *s, ptrdiff_t p, int16x8_t *s0,
...
static INLINE void load_8x8u(const uint16_t *s, ptrdiff_t p, uint16x8_t *s0,
                             uint16x8_t *s1, uint16x8_t *s2, uint16x8_t *s3,
...
static INLINE int32x4_t convolve8_4(int16x4_t s0, int16x4_t s1, int16x4_t s2,
...
static INLINE uint16x8_t convolve8_8(int16x8_t s0, int16x8_t s1, int16x8_t s2,
                                     int16x8_t s3, int16x8_t s4, int16x8_t s5,
                                     int16x8_t s6, int16x8_t s7,
                                     int16x8_t filters, uint16x8_t max) {
  const int16x4_t filters_lo = vget_low_s16(filters);
  const int16x4_t filters_hi = vget_high_s16(filters);
...
void vpx_highbd_convolve8_horiz_neon(const uint8_t *src8, ptrdiff_t src_stride,
  ...
  if (h == 4) {
    ...
    if (w == 4) {
      do {
        load_8x8s((const int16_t *)src, src_stride, &s0, &s1, &s2, &s3, &s4,
                  &s5, &s6, &s7);
        transpose_s16_8x8(&s0, &s1, &s2, &s3, &s4, &s5, &s6, &s7);

        load_8x8s((const int16_t *)(src + 7), src_stride, &s7, &s8, &s9, &s10,
                  &t4, &t5, &t6, &t7);
        src += 8 * src_stride;
        __builtin_prefetch(dst + 0 * dst_stride);
        __builtin_prefetch(dst + 1 * dst_stride);
      ...
      do {
        ...
        do {
          load_8x8s((const int16_t *)s, src_stride, &s7, &s8, &s9, &s10, &s11,
                    &s12, &s13, &s14);
