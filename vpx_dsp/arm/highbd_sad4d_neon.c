...
static INLINE uint32x4_t horizontal_add_4d_u32(uint32x4_t sum[4]) {
  ...
  res = vsetq_lane_u32(horizontal_add_uint32x4_t(sum[0]), res, 0);
  res = vsetq_lane_u32(horizontal_add_uint32x4_t(sum[1]), res, 1);
  res = vsetq_lane_u32(horizontal_add_uint32x4_t(sum[2]), res, 2);
  res = vsetq_lane_u32(horizontal_add_uint32x4_t(sum[3]), res, 3);
  return res;
#endif
}
