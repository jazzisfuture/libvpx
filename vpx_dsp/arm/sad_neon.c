...
uint32_t vpx_sad4x4_neon(const uint8_t *src_ptr, int src_stride,
  ...
  uint8x16_t sad_u8 = vabdq_u8(src_u8, ref_u8);
  uint32x4_t dp = vdotq_u32(vdupq_n_u32(0), sad_u8, vdupq_n_u8(1));
...
#if defined(__aarch64__) && defined(__ARM_FEATURE_DOTPROD) && \
    (__ARM_FEATURE_DOTPROD == 1)
  uint8x16_t sad_u8 = vabdq_u8(src_u8, avg);
  uint32x4_t prod = vdotq_u32(vdupq_n_u32(0), sad_u8, vdupq_n_u8(1));
...
uint32_t vpx_sad4x8_neon(const uint8_t *src_ptr, int src_stride,
  ...
  uint8x16_t sad1_u8 = vabdq_u8(src1_u8, ref1_u8);
  uint8x16_t sad2_u8 = vabdq_u8(src2_u8, ref2_u8);
...
uint32_t vpx_sad4x8_avg_neon(const uint8_t *src_ptr, int src_stride,
  ...
  uint8x16_t sad1_u8 = vabdq_u8(src1_u8, avg1);
  uint8x16_t sad2_u8 = vabdq_u8(src2_u8, avg2);
  prod = vdotq_u32(prod, sad1_u8, ones);
  prod = vdotq_u32(prod, sad2_u8, ones);
  return horizontal_add_uint32x4(prod);
#else
...
static INLINE uint32x2_t sad8x(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x8_t sad_u8 = vabd_u8(a_u8, b_u8);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
...
static INLINE uint32x2_t sad8x_avg(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x8_t sad_u8 = vabd_u8(a_u8, avg);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
    second_pred += 8;
    prod = vdot_u32(prod, sad_u8, ones);
...
static INLINE uint32x4_t sad16x_avg(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x16_t sad_u8 = vabdq_u8(a_u8, avg);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
    second_pred += 16;
    prod = vdotq_u32(prod, sad_u8, ones);
...
static INLINE uint32x4_t sad32x(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x16_t sad_lo_u8 = vabdq_u8(a_lo, b_lo);
    uint8x16_t sad_hi_u8 = vabdq_u8(a_hi, b_hi);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
...
static INLINE uint32x4_t sad32x_avg(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x16_t sad_lo_u8 = vabdq_u8(a_lo, avg_lo);
    uint8x16_t sad_hi_u8 = vabdq_u8(a_hi, avg_hi);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
    second_pred += 32;
    prod = vdotq_u32(prod, sad_lo_u8, ones);
...
static INLINE uint32x4_t sad64x(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x16_t sad_0_u8 = vabdq_u8(a_0, b_0);
    uint8x16_t sad_1_u8 = vabdq_u8(a_1, b_1);
    uint8x16_t sad_2_u8 = vabdq_u8(a_2, b_2);
    uint8x16_t sad_3_u8 = vabdq_u8(a_3, b_3);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
...
static INLINE uint32x4_t sad64x_avg(const uint8_t *src_ptr, int src_stride,
  ...
  for (i = 0; i < height; ++i) {
    ...
    uint8x16_t sad_0_u8 = vabdq_u8(a_0, avg_0);
    uint8x16_t sad_1_u8 = vabdq_u8(a_1, avg_1);
    uint8x16_t sad_2_u8 = vabdq_u8(a_2, avg_2);
    uint8x16_t sad_3_u8 = vabdq_u8(a_3, avg_3);
    src_ptr += src_stride;
    ref_ptr += ref_stride;
    second_pred += 64;
    prod = vdotq_u32(prod, sad_0_u8, ones);
