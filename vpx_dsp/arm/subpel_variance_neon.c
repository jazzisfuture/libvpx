...
static void var_filter_block2d_bil_w4(const uint8_t *src_ptr,
  ...
  for (i = 0; i < output_height; i += 2) {
    ...
    const uint32x2_t out_u32 = vreinterpret_u32_u8(out);
    vst1_lane_u32((uint32_unaligned *)output_ptr, out_u32, 0);
    vst1_lane_u32((uint32_unaligned *)(output_ptr + 4), out_u32, 1);
    // Next row...
