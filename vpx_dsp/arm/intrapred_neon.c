...
void vpx_dc_predictor_4x4_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  uint32x2_t above_and_left = vld1_lane_u32((const uint32_t *)above, zero, 0);
  uint16x4_t sum;
  uint8x8_t dc;
  above_and_left = vld1_lane_u32((const uint32_t *)left, above_and_left, 1);
  sum = vpaddl_u8(vreinterpret_u8_u32(above_and_left));
...
void vpx_v_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
                                const uint8_t *above, const uint8_t *left) {
  const uint8x16x2_t d = vld2q_u8(above);
  int i;
  (void)left;
...
void vpx_h_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  for (j = 0; j < 2; j++, left += 16) {
    ...
    for (i = 0; i < 2; i++, left_u8d = vget_high_u8(left_u8q)) {
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 0);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 1);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 2);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 3);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 4);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 5);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 6);
      vst2q_u8(dst, d);
      dst += stride;
      d.val[0] = d.val[1] = vdupq_lane_u8(left_u8d, 7);
      vst2q_u8(dst, d);
      dst += stride;
    }
  }
