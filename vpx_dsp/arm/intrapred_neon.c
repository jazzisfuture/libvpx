...
static INLINE void h_store_16x16(uint8_t **dst, const ptrdiff_t stride,
...
void vpx_h_predictor_16x16_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  h_store_16x16(&dst, stride, vget_low_u8(left_u8q));
  h_store_16x16(&dst, stride, vget_high_u8(left_u8q));
}

static INLINE void h_store_32x32(uint8_t **dst, const ptrdiff_t stride,
                                 const uint8x8_t left) {
  const uint8x16_t row_0 = vdupq_lane_u8(left, 0);
  const uint8x16_t row_1 = vdupq_lane_u8(left, 1);
  const uint8x16_t row_2 = vdupq_lane_u8(left, 2);
...
void vpx_h_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  for (i = 0; i < 2; i++, left += 16) {
    ...
    h_store_32x32(&dst, stride, vget_low_u8(left_u8));
    h_store_32x32(&dst, stride, vget_high_u8(left_u8));
  }
}
