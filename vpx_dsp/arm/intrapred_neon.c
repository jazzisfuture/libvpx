...
static INLINE uint16x8_t dc_sum32(const uint8_t *ref) {
...
void vpx_dc_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
                                 const uint8_t *above, const uint8_t *left) {
  const uint16x8_t sum_above = dc_sum32(above);
  const uint16x8_t sum_left = dc_sum32(left);
...
void vpx_dc_left_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
                                      const uint8_t *above,
                                      const uint8_t *left) {
  const uint16x8_t sum = dc_sum32(left);
...
void vpx_dc_top_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
                                     const uint8_t *above,
                                     const uint8_t *left) {
  const uint16x8_t sum = dc_sum32(above);
  const uint8x8_t dc0 = vrshrn_n_u16(sum, 5);
  (void)left;
  dc_store_32x32(dst, stride, dc0);
}
