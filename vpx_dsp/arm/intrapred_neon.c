...
void vpx_d63_predictor_16x16_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  memcpy(&a0, above + 0, 16);
  memcpy(&a1, above + 1, 16);
  memcpy(&a2, above + 2, 16);
...
void vpx_d63_predictor_32x32_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  memcpy(&a1, above + 1, 16);
  memcpy(&a2, above + 2, 16);
  memcpy(&a16, above + 16, 16);
  memcpy(&a17, above + 17, 16);
  memcpy(&a18, above + 18, 16);
...
void vpx_d117_predictor_4x4_neon(uint8_t *dst, ptrdiff_t stride,
  ...
  // The Neon vector intrinsics only expose an eight-byte vector type, so we
  // ignore the values of the top four lanes here.
  az = vdup_n_u8(0);
  a0 = vdup_n_u8(0);
  memcpy(&az, above - 1, 4);
  memcpy(&a0, above + 0, 4);
