...
static INLINE void transpose_u8_8x8d(uint8x8_t *a0, uint8x8_t *a1,
  ...
  const uint8x8x2_t b2 = vtrn_u8(*a4, *a5);
  const uint8x8x2_t b3 = vtrn_u8(*a6, *a7);
  ...
  const uint16x4x2_t c2 =
      vtrn_u16(vreinterpret_u16_u8(b2.val[0]), vreinterpret_u16_u8(b3.val[0]));
  const uint16x4x2_t c3 =
      vtrn_u16(vreinterpret_u16_u8(b2.val[1]), vreinterpret_u16_u8(b3.val[1]));
  ...
  const uint32x2x2_t d0 = vtrn_u32(vreinterpret_u32_u16(c0.val[0]),
                                   vreinterpret_u32_u16(c2.val[0]));
  const uint32x2x2_t d1 = vtrn_u32(vreinterpret_u32_u16(c1.val[0]),
                                   vreinterpret_u32_u16(c3.val[0]));
  const uint32x2x2_t d2 = vtrn_u32(vreinterpret_u32_u16(c0.val[1]),
                                   vreinterpret_u32_u16(c2.val[1]));
  const uint32x2x2_t d3 = vtrn_u32(vreinterpret_u32_u16(c1.val[1]),
                                   vreinterpret_u32_u16(c3.val[1]));

  *a0 = vreinterpret_u8_u32(d0.val[0]);
  *a1 = vreinterpret_u8_u32(d1.val[0]);
  *a2 = vreinterpret_u8_u32(d2.val[0]);
  *a3 = vreinterpret_u8_u32(d3.val[0]);
  *a4 = vreinterpret_u8_u32(d0.val[1]);
  *a5 = vreinterpret_u8_u32(d1.val[1]);
  *a6 = vreinterpret_u8_u32(d2.val[1]);
  *a7 = vreinterpret_u8_u32(d3.val[1]);
}

static INLINE void transpose_u8_8x8q(uint8x8_t *a0, uint8x8_t *a1,
                                     uint8x8_t *a2, uint8x8_t *a3,
                                     uint8x8_t *a4, uint8x8_t *a5,
                                     uint8x8_t *a6, uint8x8_t *a7) {
  // Swap 8 bit elements. Goes from:
