...
static INLINE void transpose_u8_8x16(
  ...
  uint32x4x2_t b0, b1, b2, b3;
  uint16x8x2_t c0, c1, c2, c3;
  uint8x16x2_t d0, d1, d2, d3;

  // b0: 00 10 20 30 40 50 60 70  08 18 28 38 48 58 68 78  P7 Q0
  //     04 14 24 34 44 54 64 74  0C 1C 2C 3C 4C 5C 6C 7C  P3 Q4
  // b1: 02 12 22 32 42 52 62 72  0A 1A 2A 3A 4A 5A 6A 7A  P5 Q2
  //     06 16 26 36 46 56 66 76  0E 1E 2E 3E 4E 5E 6E 7E  P1 Q6
  // b2: 01 11 21 31 41 51 61 71  09 19 29 39 49 59 69 79  P6 Q1
  //     05 15 25 35 45 55 65 75  0D 1D 2D 3D 4D 5D 6D 7D  P2 Q5
  // b3: 03 13 23 33 43 53 63 73  0B 1B 2B 3B 4B 5B 6B 7B  P4 Q3
  //     07 17 27 37 47 57 67 77  0F 1F 2F 3F 4F 5F 6F 7F  P0 Q7
  b0.val[0] = vreinterpretq_u32_u8(vcombine_u8(i0, i8));
  b0.val[1] = vreinterpretq_u32_u8(vcombine_u8(i4, i12));
  b1.val[0] = vreinterpretq_u32_u8(vcombine_u8(i2, i10));
  b1.val[1] = vreinterpretq_u32_u8(vcombine_u8(i6, i14));
  b2.val[0] = vreinterpretq_u32_u8(vcombine_u8(i1, i9));
  b2.val[1] = vreinterpretq_u32_u8(vcombine_u8(i5, i13));
  b3.val[0] = vreinterpretq_u32_u8(vcombine_u8(i3, i11));
  b3.val[1] = vreinterpretq_u32_u8(vcombine_u8(i7, i15));

  // b0: 00 10 20 30 04 14 24 34  08 18 28 38 0C 1C 2C 3C
  //     40 50 60 70 44 54 64 74  48 58 68 78 4C 5C 6C 7C
  // b1: 02 12 22 32 06 16 26 36  0A 1A 2A 3A 0E 1E 2E 3E
  //     42 52 62 72 46 56 66 76  4A 5A 6A 7A 4E 5E 6E 7E
  // b2: 01 11 21 31 05 15 25 35  09 19 29 39 0D 1D 2D 3D
  //     41 51 61 71 45 55 65 75  49 59 69 79 4D 5D 6D 7D
  // b3: 03 13 23 33 07 17 27 37  0B 1B 2B 3B 0F 1F 2F 3F
  //     43 53 63 73 47 57 67 77  4B 5B 6B 7B 4F 5F 6F 7F
  b0 = vtrnq_u32(b0.val[0], b0.val[1]);
  b1 = vtrnq_u32(b1.val[0], b1.val[1]);
  b2 = vtrnq_u32(b2.val[0], b2.val[1]);
  b3 = vtrnq_u32(b3.val[0], b3.val[1]);

  // c0: 00 10 02 12 04 14 06 16  08 18 0A 1A 0C 1C 0E 1E
  //     20 30 22 32 24 34 26 36  28 38 2A 3A 2C 3C 2E 3E
  // c1: 01 11 03 13 05 15 07 17  09 19 0B 1B 0D 1D 0F 1F
  //     21 31 23 33 25 35 27 37  29 39 2B 3B 2D 3D 2F 3F
  // c2: 40 50 42 52 44 54 46 56  48 58 4A 5A 4C 5C 4E 5E
  //     60 70 62 72 64 74 66 76  68 78 6A 7A 6C 7C 6E 7E
  // c3: 41 51 43 53 45 55 47 57  49 59 4B 5B 4D 5D 4F 5F
  //     61 71 63 73 65 75 67 77  69 79 6B 7B 6D 7D 6F 7F
  c0 = vtrnq_u16(vreinterpretq_u16_u32(b0.val[0]),
                 vreinterpretq_u16_u32(b1.val[0]));
  c1 = vtrnq_u16(vreinterpretq_u16_u32(b2.val[0]),
                 vreinterpretq_u16_u32(b3.val[0]));
  c2 = vtrnq_u16(vreinterpretq_u16_u32(b0.val[1]),
                 vreinterpretq_u16_u32(b1.val[1]));
  c3 = vtrnq_u16(vreinterpretq_u16_u32(b2.val[1]),
                 vreinterpretq_u16_u32(b3.val[1]));
  ...
  d0 = vtrnq_u8(vreinterpretq_u8_u16(c0.val[0]),
                vreinterpretq_u8_u16(c1.val[0]));
  d1 = vtrnq_u8(vreinterpretq_u8_u16(c0.val[1]),
                vreinterpretq_u8_u16(c1.val[1]));
  d2 = vtrnq_u8(vreinterpretq_u8_u16(c2.val[0]),
                vreinterpretq_u8_u16(c3.val[0]));
  d3 = vtrnq_u8(vreinterpretq_u8_u16(c2.val[1]),
                vreinterpretq_u8_u16(c3.val[1]));
