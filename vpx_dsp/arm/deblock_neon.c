...
static uint16x4_t mask_math(const int16x4_t sum, const int32x4_t sumsq,
...
static uint8x8_t calculate_filter_mask(const int16x4_t sum_low,
                                       const int16x4_t sum_high,
...
void vpx_mbpost_proc_down_neon(uint8_t *dst, int pitch, int rows, int cols,
  ...
  for (col = 0; col < cols; col += 8) {
    ...
    for (row = 0; row < rows; ++row) {
      ...
      mask = calculate_filter_mask(vget_low_s16(sum), vget_high_s16(sum),
                                   sumsq_low, sumsq_high, f);
