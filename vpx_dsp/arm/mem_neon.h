...
// Load 4 continguous bytes when alignment is not guaranteed.
...
// Load 8 continguous bytes when alignment is not guaranteed.
static INLINE uint8x8_t load_unaligned_u8_8x1(const uint8_t *buf) {
  ...
  return vreinterpret_u8_u64(a_u64);
}

// Load 16 continguous bytes when alignment is not guaranteed.
static INLINE uint8x16_t load_unaligned_u8_16x1(const uint8_t *buf) {
  ...
  memcpy(&a[1], buf + 8, 8);
  a_u64 = vdupq_n_u64(0);
  a_u64 = vsetq_lane_u64(a[0], a_u64, 0);
  a_u64 = vsetq_lane_u64(a[1], a_u64, 1);
  return vreinterpretq_u8_u64(a_u64);
}

// Store 4 continguous bytes from the low half of an 8x8 vector.
