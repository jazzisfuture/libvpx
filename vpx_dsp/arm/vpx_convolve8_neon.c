...
static INLINE void store_u8_8x8(uint8_t *s, const ptrdiff_t p,
  ...
  vst1_u8(s, s7);
    ...
    defined(__ARM_FEATURE_DOTPROD) && (__ARM_FEATURE_DOTPROD == 1)

DECLARE_ALIGNED(32, static const uint8_t, dot_prod_permute_tbl[48]) = {
  ...
  if (w == 4) {
    ...
    do {
      ...
      do {
        ...
        d0 = convolve8_8_dot(s0, filters, correction, range_limit,
                             permute_tbl);
        d1 = convolve8_8_dot(s1, filters, correction, range_limit,
                             permute_tbl);
        d2 = convolve8_8_dot(s2, filters, correction, range_limit,
                             permute_tbl);
        d3 = convolve8_8_dot(s3, filters, correction, range_limit,
                             permute_tbl);
