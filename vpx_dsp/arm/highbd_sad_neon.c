...
static VPX_FORCE_INLINE uint32_t highbd_sad8_neon(const uint8_t *src_ptr,
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j += 8) {
      ...
      sum_abs_diff = vabal_u16(sum_abs_diff, vget_low_u16(src_u16), vget_low_u16(ref_u16));
      sum_abs_diff = vabal_u16(sum_abs_diff, vget_high_u16(src_u16), vget_high_u16(ref_u16));
    }
    src16_ptr += src_stride;
    ref16_ptr += ref_stride;
  }
...
static VPX_FORCE_INLINE uint32_t highbd_sad8_avg_neon(
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j += 8) {
      ...
      sum_abs_diff = vabal_u16(sum_abs_diff, vget_low_u16(a_u16), vget_low_u16(avg));
      sum_abs_diff = vabal_u16(sum_abs_diff, vget_high_u16(a_u16), vget_high_u16(avg));
    }
    src16_ptr += src_stride;
    ref16_ptr += ref_stride;
    pred_ptr += width;
