...
// and cross them.
static INLINE void cross_input(const int16x8_t *a, int16x8_t *b,
                               const int pass) {
  if (pass == 0) {
...
static void dct_body(const int16x8_t *in, int16x8_t *out) {
  int16x8_t s[8];
  int16x8_t x[4];
  int16x8_t step[8];
...
void vpx_fdct16x16_neon(const int16_t *input, tran_low_t *output, int stride) {
  ...
  // Transpose top left and top right quarters into into one contiguous location
  // to process to the top half.
  ...
  // Transpose bottom left and bottom right quarters into into one contiguous
  // location to process to the top half.
  transpose_8x8(&temp0[8], &temp2[0]);
  transpose_8x8(&temp1[8], &temp2[8]);
  partial_round_shift(temp2);
  cross_input(temp2, temp3, 1);
  dct_body(temp3, temp2);
  transpose_s16_8x8(&temp2[0], &temp2[1], &temp2[2], &temp2[3], &temp2[4],
