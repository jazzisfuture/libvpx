...
void vpx_highbd_hadamard_32x32_neon(const int16_t *src_diff,
  ...
  do {
    ...
    store_s32q_to_tran_low((coeff + 4 * i), vhaddq_s32(b0, b2));
    store_s32q_to_tran_low((coeff + 4 * i) + 256, vhaddq_s32(b1, b3));
    store_s32q_to_tran_low((coeff + 4 * i) + 512, vhsubq_s32(b0, b2));
    store_s32q_to_tran_low((coeff + 4 * i) + 768, vhsubq_s32(b1, b3));
