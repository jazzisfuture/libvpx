...
// Load, cross, and multiply by 4. Load the first 8 and last 8, then the middle
...
static INLINE void load(const int16_t *a, int stride, int16x8_t *b) {
  ...
  // Shift the values we didn't want to muck yet.
...
static INLINE int16x8_t sub_round_shift(const int16x8_t a) {
  return vrshrq_n_s16(vsubq_s16(a, vreinterpretq_s16_u16(vshrq_n_u16(
                                       vreinterpretq_u16_s16(a), 15))),
                      2);
}
...
#define ADD_S16_S32(a, left_index, right_index, b, b_index)                 \
  ...
  b##_hi[b_index] =                                                         \
      vaddl_s16(vget_high_s16(a[left_index]), vget_high_s16(a[right_index]));

#define SUB_S16_S32(a, left_index, right_index, b, b_index)                 \
  b##_lo[b_index] =                                                         \
      vsubl_s16(vget_low_s16(a[left_index]), vget_low_s16(a[right_index])); \
  b##_hi[b_index] =                                                         \
      vsubl_s16(vget_high_s16(a[left_index]), vget_high_s16(a[right_index]));
...
static INLINE void butterfly_one_coeff_s16_s32(
    const int16x8_t a, const int16x8_t b, const tran_high_t c,
  ...
  const int32x4_t diff0 = vmlsl_n_s16(a0, vget_low_s16(b), c);
  const int32x4_t diff1 = vmlsl_n_s16(a1, vget_high_s16(b), c);
  *add_lo = vrshrq_n_s32(sum0, 14);
  *add_hi = vrshrq_n_s32(sum1, 14);
  *sub_lo = vrshrq_n_s32(diff0, 14);
  *sub_hi = vrshrq_n_s32(diff1, 14);
}

#define BUTTERFLY_ONE_S16_S32(a, left_index, right_index, constant, c, \
                              add_index, sub_index)                    \
  butterfly_one_coeff_s16_s32(a[left_index], a[right_index], constant, \
                              &c##_lo[add_index], &c##_hi[add_index],  \
                              &c##_lo[sub_index], &c##_hi[sub_index]);
...
static INLINE void butterfly_one_coeff_s32(
    ...
    const int32x4_t b_hi, const tran_high_t c, int32x4_t *add_lo,
  ...
  // int16_t. The constants fit in int16_t.
  const int32_t c_s32 = (int32_t)c;
  const int32x4_t a_lo_0 = vmulq_n_s32(a_lo, c_s32);
  const int32x4_t a_hi_0 = vmulq_n_s32(a_hi, c_s32);
  const int32x4_t sum0 = vmlaq_n_s32(a_lo_0, b_lo, c_s32);
  const int32x4_t sum1 = vmlaq_n_s32(a_hi_0, b_hi, c_s32);
  const int32x4_t diff0 = vmlsq_n_s32(a_lo_0, b_lo, c_s32);
  const int32x4_t diff1 = vmlsq_n_s32(a_hi_0, b_hi, c_s32);
  *add_lo = vrshrq_n_s32(sum0, 14);
  *add_hi = vrshrq_n_s32(sum1, 14);
  *sub_lo = vrshrq_n_s32(diff0, 14);
  *sub_hi = vrshrq_n_s32(diff1, 14);
...
#define BUTTERFLY_ONE_S32(a, left_index, right_index, constant, c, add_index, \
                          sub_index)                                          \
  butterfly_one_coeff_s32(a##_lo[left_index], a##_hi[left_index],             \
                          a##_lo[right_index], a##_hi[right_index], constant, \
                          &c##_lo[add_index], &c##_hi[add_index],             \
                          &c##_lo[sub_index], &c##_hi[sub_index]);
...
static INLINE void butterfly_two_coeff_s32(
    const int32x4_t a_lo, const int32x4_t a_hi, const int32x4_t b_lo,
    const int32x4_t b_hi, const tran_high_t c0, const tran_high_t c1,
    int32x4_t *add_lo, int32x4_t *add_hi, int32x4_t *sub_lo,
    int32x4_t *sub_hi) {
  const int32_t c0_s32 = (int32_t)c0;
  const int32_t c1_s32 = (int32_t)c1;
  const int32x4_t a0 = vmulq_n_s32(a_lo, c0_s32);
  const int32x4_t a1 = vmulq_n_s32(a_hi, c0_s32);
  const int32x4_t a2 = vmulq_n_s32(a_lo, c1_s32);
  const int32x4_t a3 = vmulq_n_s32(a_hi, c1_s32);
  const int32x4_t sum0 = vmlaq_n_s32(a2, b_lo, c0_s32);
  const int32x4_t sum1 = vmlaq_n_s32(a3, b_hi, c0_s32);
  const int32x4_t diff0 = vmlsq_n_s32(a0, b_lo, c1_s32);
  const int32x4_t diff1 = vmlsq_n_s32(a1, b_hi, c1_s32);
  *add_lo = vrshrq_n_s32(sum0, 14);
  *add_hi = vrshrq_n_s32(sum1, 14);
  *sub_lo = vrshrq_n_s32(diff0, 14);
  *sub_hi = vrshrq_n_s32(diff1, 14);
...
#define BUTTERFLY_TWO_S32(a, left_index, right_index, left_constant,          \
                          right_constant, c, add_index, sub_index)            \
  butterfly_two_coeff_s32(                                                    \
      a##_lo[left_index], a##_hi[left_index], a##_lo[right_index],            \
      a##_hi[right_index], left_constant, right_constant, &c##_lo[add_index], \
      &c##_hi[add_index], &c##_lo[sub_index], &c##_hi[sub_index]);

static INLINE int16x8_t add_round_shift_s32(const int32x4_t a_lo,
                                            const int32x4_t a_hi) {
  const int32x4_t one = vdupq_n_s32(1);
  const int16x4_t b_lo = vshrn_n_s32(
      vqaddq_s32(vqaddq_s32(a_lo, vreinterpretq_s32_u32(vshrq_n_u32(
                                      vreinterpretq_u32_s32(a_lo), 31))),
                 one),
      2);
  const int16x4_t b_hi = vshrn_n_s32(
      vqaddq_s32(vqaddq_s32(a_hi, vreinterpretq_s32_u32(vshrq_n_u32(
                                      vreinterpretq_u32_s32(a_hi), 31))),
                 one),
      2);
