...
unsigned int vpx_avg_4x4_neon(const uint8_t *s, int p) {
  ...
  return (vget_lane_s32(horizontal_add_uint16x8(v_sum), 0) + 8) >> 4;
}
...
unsigned int vpx_avg_8x8_neon(const uint8_t *s, int p) {
  ...
  return (vget_lane_s32(horizontal_add_uint16x8(v_sum), 0) + 32) >> 6;
}
...
int16_t vpx_int_pro_col_neon(uint8_t const *ref, const int width) {
  ...
  return vget_lane_s32(horizontal_add_uint16x8(vec_sum), 0);
}
