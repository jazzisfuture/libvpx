...
static void highbd_vpx_idct16x16_256_add_half1d(const int32_t *input,
...
static void highbd_vpx_idct16x16_38_add_half1d(const int32_t *input,
                                               int32_t *output, uint16_t *dest,
...
void highbd_vpx_idct16x16_10_add_half1d_pass1(const tran_low_t *input,
                                              int32_t *output) {
  const int32x4_t cospi_0_8_16_24 = vld1q_s32(kCospi32 + 0);
  const int32x4_t cospi_4_12_20N_28 = vld1q_s32(kCospi32 + 4);
  const int32x4_t cospi_2_30_10_22 = vld1q_s32(kCospi32 + 8);
...
void highbd_vpx_idct16x16_10_add_half1d_pass2(const int32_t *input,
...
void vpx_highbd_idct16x16_256_add_neon(const tran_low_t *input, uint8_t *dest8,
  ...
  if (bd == 8) {
    ...
    // pass 1
    // Parallel idct on the upper 8 rows
    highbd_vpx_idct16x16_256_add_half1d(input, row_idct_output, dest, stride,
                                        bd);

    // Parallel idct on the lower 8 rows
    highbd_vpx_idct16x16_256_add_half1d(input + 8 * 16, row_idct_output + 8,
                                        dest, stride, bd);

    // pass 2
    // Parallel idct to get the left 8 columns
    highbd_vpx_idct16x16_256_add_half1d(row_idct_output, NULL, dest, stride,
                                        bd);

    // Parallel idct to get the right 8 columns
    highbd_vpx_idct16x16_256_add_half1d(row_idct_output + 8 * 16, NULL,
                                        dest + 8, stride, bd);
  }
}
...
void vpx_highbd_idct16x16_38_add_neon(const tran_low_t *input, uint8_t *dest8,
  ...
  if (bd == 8) {
    ...
    // pass 1
    // Parallel idct on the upper 8 rows
    highbd_vpx_idct16x16_38_add_half1d(input, row_idct_output, dest, stride,
                                       bd);

    // pass 2
    // Parallel idct to get the left 8 columns
    highbd_vpx_idct16x16_38_add_half1d(row_idct_output, NULL, dest, stride, bd);

    // Parallel idct to get the right 8 columns
    highbd_vpx_idct16x16_38_add_half1d(row_idct_output + 16 * 8, NULL, dest + 8,
                                       stride, bd);
  }
}
...
void vpx_highbd_idct16x16_10_add_neon(const tran_low_t *input, uint8_t *dest8,
  ...
  if (bd == 8) {
    ...
    // pass 1
    // Parallel idct on the upper 8 rows
    highbd_vpx_idct16x16_10_add_half1d_pass1(input, row_idct_output);

    // pass 2
    // Parallel idct to get the left 8 columns
    highbd_vpx_idct16x16_10_add_half1d_pass2(row_idct_output, NULL, dest,
                                             stride, bd);

    // Parallel idct to get the right 8 columns
    highbd_vpx_idct16x16_10_add_half1d_pass2(row_idct_output + 4 * 8, NULL,
                                             dest + 8, stride, bd);
  }
}
