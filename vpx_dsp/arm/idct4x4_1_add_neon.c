...
void vpx_idct4x4_1_add_neon(const tran_low_t *input, uint8_t *dest,
  ...
  for (i = 0; i < 2; i++) {
    ...
    dest0 += dest_stride;
    a = vaddw_u8(vreinterpretq_u16_s16(dc), vreinterpret_u8_u32(d0));
    b = vqmovun_s16(vreinterpretq_s16_u16(a));
    vst1_lane_u32((uint32_t *)dest1, vreinterpret_u32_u8(b), 0);
    dest1 += dest_stride;
    vst1_lane_u32((uint32_t *)dest1, vreinterpret_u32_u8(b), 1);
    dest1 += dest_stride;
  }
}
