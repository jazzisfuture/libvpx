...
static INLINE int32_t hsum_epi32_epi32(__m128i v_d) {
  v_d = _mm_hadd_epi32(v_d, v_d);
  v_d = _mm_hadd_epi32(v_d, v_d);
  return _mm_cvtsi128_si32(v_d);
}

static INLINE int64_t hsum_epi64_epi64(__m128i v_q) {
  v_q = _mm_add_epi64(v_q, _mm_srli_si128(v_q, 8));
  return _mm_cvtsi128_si64(v_q);
}

static INLINE int64_t hsum_epi32_epi64(__m128i v_d) {
  ...
  return hsum_epi64_epi64(_mm_add_epi64(v_0_q, v_1_q));
}
...
static INLINE int calc_masked_variance(__m128i v_sum_d, __m128i v_sse_q,
  ...
  // Horizontal sum
  sum64 = hsum_epi32_epi64(v_sum_d);
  sse64 = hsum_epi64_epi64(v_sse_q);
...
static INLINE void highbd_masked_variance64_ssse3(
  ...
  // Horizontal sum
  *sum = hsum_epi32_epi64(v_sum_d);
  *sse = hsum_epi64_epi64(v_sse_q);
...
unsigned int vpx_masked_sub_pixel_variance##W##x##H##_ssse3(                   \
  ...
  if (xoffset == 0) {                                                          \
    if (yoffset == 0)                                                          \
      ...
      return vpx_masked_subpel_varWxH_xzero(src, src_stride, HALF_PIXEL_OFFSET,\
    ...
    if (xoffset == HALF_PIXEL_OFFSET)                                          \
      return vpx_masked_subpel_varWxH_yzero(src, src_stride, HALF_PIXEL_OFFSET,\
                                            dst, dst_stride, msk, msk_stride,  \
                                            sse, W, H, apply_filter_avg);      \
    else                                                                       \
...
static INLINE int highbd_10_calc_masked_variance(__m128i v_sum_d,
  ...
  // Horizontal sum
  sum64 = hsum_epi32_epi32(v_sum_d);
  sse64 = hsum_epi64_epi64(v_sse_q);
