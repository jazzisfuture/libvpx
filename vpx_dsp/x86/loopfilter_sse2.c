...
#define BUILD_MASK_QP do {                                                     \
  const __m128i fe = _mm_set1_epi8(0xfe);                                      \
  const __m128i ff = _mm_cmpeq_epi8(zero, zero);                               \
  __m128i abs_p1q1, abs_p0q0, abs_q1q0, work;                                  \
...
#define FILTER_4_QP do {                                                       \
  const __m128i t4 = _mm_set1_epi8(4);                                         \
  const __m128i t3 = _mm_set1_epi8(3);                                         \
  const __m128i t80 = _mm_set1_epi8(0x80);                                     \
  const __m128i t1 = _mm_set1_epi16(0x1);                                      \
  const __m128i ff = _mm_cmpeq_epi8(zero, zero);                               \
  const __m128i qs0 = _mm_xor_si128(p0q0, t80);                                \
  const __m128i qs1 = _mm_xor_si128(p1q1, t80);                                \
  __m128i filt;                                                                \
  __m128i work_a;                                                              \
  __m128i filter1, filter2;                                                    \
  qs1ps1 = _mm_xor_si128(q1p1, t80);                                           \
  qs0ps0 = _mm_xor_si128(q0p0, t80);                                           \
  hev = _mm_subs_epu8(max_abs_p1p0q1q0, thresh);                               \
  hev = _mm_xor_si128(_mm_cmpeq_epi8(hev, zero), ff);                          \
...
static void mb_lpf_horizontal_edge_w_sse2_8(unsigned char *s,
  ...
  {
    const __m128i p7_16 = _mm_unpacklo_epi8(q7p7, zero);
    const __m128i p6_16 = _mm_unpacklo_epi8(q6p6, zero);
    const __m128i p5_16 = _mm_unpacklo_epi8(q5p5, zero);
    const __m128i p4_16 = _mm_unpacklo_epi8(q4p4, zero);
    const __m128i p3_16 = _mm_unpacklo_epi8(q3p3, zero);
    const __m128i p2_16 = _mm_unpacklo_epi8(q2p2, zero);
    const __m128i p1_16 = _mm_unpacklo_epi8(q1p1, zero);
    const __m128i p0_16 = _mm_unpacklo_epi8(q0p0, zero);
    const __m128i q0_16 = _mm_unpackhi_epi8(q0p0, zero);
    const __m128i q1_16 = _mm_unpackhi_epi8(q1p1, zero);
    const __m128i q2_16 = _mm_unpackhi_epi8(q2p2, zero);
    const __m128i q3_16 = _mm_unpackhi_epi8(q3p3, zero);
    const __m128i q4_16 = _mm_unpackhi_epi8(q4p4, zero);
    const __m128i q5_16 = _mm_unpackhi_epi8(q5p5, zero);
    const __m128i q6_16 = _mm_unpackhi_epi8(q6p6, zero);
    const __m128i q7_16 = _mm_unpackhi_epi8(q7p7, zero);
    ...
    // wide flat calculations
    {
      ...
      __m128i pixetFilter_p2p1p0, pixetFilter_q2q1q0;
      __m128i sum_p7, sum_q7, res_p, res_q;
...
void vpx_lpf_horizontal_8_sse2(unsigned char *s, int p,
  ...
  __m128i q3p3, q2p2, q1p1, q0p0, p0q0, p1q1;

  __m128i abs_p1p0;

  __m128i max_abs_p1p0q1q0;
  __m128i flat_q2p2, flat_q1p1, flat_q0p0;
  __m128i qs1ps1, qs0ps0;
