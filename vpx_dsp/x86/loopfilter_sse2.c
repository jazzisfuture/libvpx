...
#define FILTER_8_LO do {                                            \
  ...
  f8_lo = _mm_add_epi16(_mm_add_epi16(p0_lo, q0_lo), f8_lo);        \
                                                                    \
  op2 = filter8_mask(&flat, &p2, &f8_lo, &f8_lo);                   \
                                                                    \
  f8_lo = filter_add2_sub2(&f8_lo, &q1_lo, &p1_lo, &p2_lo, &p3_lo); \
  op1 = filter8_mask(&flat, &op1, &f8_lo, &f8_lo);                  \
                                                                    \
  f8_lo = filter_add2_sub2(&f8_lo, &q2_lo, &p0_lo, &p1_lo, &p3_lo); \
  op0 = filter8_mask(&flat, &op0, &f8_lo, &f8_lo);                  \
                                                                    \
  f8_lo = filter_add2_sub2(&f8_lo, &q3_lo, &q0_lo, &p0_lo, &p3_lo); \
  oq0 = filter8_mask(&flat, &oq0, &f8_lo, &f8_lo);                  \
                                                                    \
  f8_lo = filter_add2_sub2(&f8_lo, &q3_lo, &q1_lo, &q0_lo, &p2_lo); \
  oq1 = filter8_mask(&flat, &oq1, &f8_lo, &f8_lo);                  \
                                                                    \
  f8_lo = filter_add2_sub2(&f8_lo, &q3_lo, &q2_lo, &q1_lo, &p1_lo); \
  oq2 = filter8_mask(&flat, &q2, &f8_lo, &f8_lo);                   \
} while (0);
...
void vpx_lpf_horizontal_8_sse2(unsigned char *s, int p,
  ...
  _mm_storel_epi64((__m128i *)(s - 3 * p), op2);
  _mm_storel_epi64((__m128i *)(s - 2 * p), op1);
  _mm_storel_epi64((__m128i *)(s - 1 * p), op0);
  _mm_storel_epi64((__m128i *)(s + 0 * p), oq0);
  _mm_storel_epi64((__m128i *)(s + 1 * p), oq1);
  _mm_storel_epi64((__m128i *)(s + 2 * p), oq2);
}
