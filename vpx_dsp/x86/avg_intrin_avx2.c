...
void vpx_hadamard_32x32_avx2(int16_t const *src_diff, ptrdiff_t src_stride,
                             tran_low_t *coeff) {
  int idx;
  for (idx = 0; idx < 4; ++idx) {
  ...
  for (idx = 0; idx < 256; idx += 16) {
    const __m256i coeff0 = _mm256_loadu_si256((const __m256i *)coeff);
    const __m256i coeff1 = _mm256_loadu_si256((const __m256i *)(coeff + 256));
    const __m256i coeff2 = _mm256_loadu_si256((const __m256i *)(coeff + 512));
    const __m256i coeff3 = _mm256_loadu_si256((const __m256i *)(coeff + 768));
