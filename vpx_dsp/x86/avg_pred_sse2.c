  ...
  if (width > 8) {
    ...
    for (i = 0; i < width * height; i += 16) {
      ...
      if (width == ref_stride) {
        ...
        __m128i r_0 = _mm_cvtsi32_si128(*(const uint32_t *)ref);
        __m128i r_1 = _mm_cvtsi32_si128(*(const uint32_t *)(ref + ref_stride));
        __m128i r_2 = _mm_cvtsi32_si128(*(const uint32_t *)(ref + 2 * ref_stride));
        __m128i r_3 = _mm_cvtsi32_si128(*(const uint32_t *)(ref + 3 * ref_stride));

        r_1 = _mm_slli_si128(r_1, 4);
        r_2 = _mm_slli_si128(r_2, 8);
        r_3 = _mm_slli_si128(r_3, 12);
        r = _mm_or_si128(r_0, r_1);
        r = _mm_or_si128(r, r_2);
        r = _mm_or_si128(r, r_3);
        ref += 4 * ref_stride;
      } else {
