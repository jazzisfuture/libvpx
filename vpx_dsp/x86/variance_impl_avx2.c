...
static inline void spv32_x8_y8(const uint8_t *src, int src_stride,
                               ...
                               int height, __m256i *sum_reg, __m256i *sse_reg) {
  const __m256i zero_reg = _mm256_set1_epi16(0);
  __m256i dst_reg, exp_src_lo, exp_src_hi, exp_dst_lo, exp_dst_hi;
  __m256i src_next_reg, src_avg;
  int i;
  // load source and another source starting from the next
  // following byte
  __m256i src_reg = _mm256_loadu_si256((__m256i const *)(src));
  AVG_NEXT_SRC(src_reg, 1)
  for (i = 0; i < height; i++) {
    src_avg = src_reg;
    src += src_stride;
...
static inline int sub_pix_var32xh(const uint8_t *src, int src_stride,
  ...
  const __m256i zero_reg = _mm256_set1_epi16(0);
  __m256i sum_reg = _mm256_set1_epi16(0);
  __m256i sse_reg = _mm256_set1_epi16(0);
