...
static void subtract_4x4(int16_t *diff, ptrdiff_t diff_stride,
  ...
  __m128i v0, v1, v2, v3;
  __m128i x0, x1, x2, x3;
  int32_t *store_diff = (int32_t *) (diff + 0 * diff_stride);
...
#if 1
  store_diff[0] = _mm_extract_epi32(x0, 0);
  store_diff[1] = _mm_extract_epi32(x0, 1);

  store_diff = (int32_t *) (diff + 1 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x1, 0);
  store_diff[1] = _mm_extract_epi32(x1, 1);

  store_diff = (int32_t *) (diff + 2 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x2, 0);
  store_diff[1] = _mm_extract_epi32(x2, 1);

  store_diff = (int32_t *) (diff + 3 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x3, 0);
  store_diff[1] = _mm_extract_epi32(x3, 1);
#else
  // Check high address of 4 uint16
...
static void subtract_4x8(int16_t *diff, ptrdiff_t diff_stride,
  ...
  store_diff = (int32_t *) (diff + 4 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x4, 0);
  store_diff[1] = _mm_extract_epi32(x4, 1);

  store_diff = (int32_t *) (diff + 5 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x5, 0);
  store_diff[1] = _mm_extract_epi32(x5, 1);

  store_diff = (int32_t *) (diff + 6 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x6, 0);
  store_diff[1] = _mm_extract_epi32(x6, 1);

  store_diff = (int32_t *) (diff + 7 * diff_stride);
  store_diff[0] = _mm_extract_epi32(x7, 0);
  store_diff[1] = _mm_extract_epi32(x7, 1);
#else
  // Check high address of 4 uint16
  _mm_storeu_si128((__m128i *) (diff + 0 * diff_stride), x0);
  _mm_storeu_si128((__m128i *) (diff + 1 * diff_stride), x1);
...
static void subtract_8x4(int16_t *diff, ptrdiff_t diff_stride,
  ...
  _mm_storeu_si128((__m128i *) (diff + 2 * diff_stride), x2);
  _mm_storeu_si128((__m128i *) (diff + 3 * diff_stride), x3);
  //subtract_4x4(diff, diff_stride, src, src_stride, pred, pred_stride);
}
...
static void subtract_8x8(int16_t *diff, ptrdiff_t diff_stride,
  ...
  _mm_storeu_si128((__m128i *) (diff + 6 * diff_stride), x6);
  _mm_storeu_si128((__m128i *) (diff + 7 * diff_stride), x7);
  //  subtract_4x8(diff, diff_stride, src, src_stride, pred, pred_stride);
}
...
void vpx_highbd_subtract_block_sse4_1(
    int rows, int cols,
    int16_t *diff, ptrdiff_t diff_stride,
