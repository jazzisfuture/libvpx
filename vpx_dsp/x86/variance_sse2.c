...
static INLINE int sum_final_sse2(const __m128i vsum) {
  const __m128i sum_lo = _mm_srai_epi32(_mm_unpacklo_epi16(vsum, vsum), 16);
  const __m128i sum_hi = _mm_srai_epi32(_mm_unpackhi_epi16(vsum, vsum), 16);
  const __m128i t = _mm_add_epi32(sum_lo, sum_hi);
  return add32x4_sse2(t);
}

static INLINE void variance4_sse2(const uint8_t *const src,
  ...
  *sum = _mm_setzero_si128();
  ...
  for (i = 0; i < h; i += 2) {
    const __m128i s = load4x2_sse2(src + i * src_stride, src_stride);
    const __m128i r = load4x2_sse2(ref + i * ref_stride, ref_stride);

    variance_kernel_sse2(s, r, sse, sum);
  }
}

static INLINE void variance8_sse2(const uint8_t *const src,
                                  const int src_stride,
                                  const uint8_t *const ref,
                                  const int ref_stride, const int h,
                                  __m128i *const sse, __m128i *const sum) {
  ...
  *sum = _mm_setzero_si128();
  ...
  for (i = 0; i < h; i++) {
    const __m128i s = _mm_unpacklo_epi8(
...
static INLINE void variance16_sse2(const uint8_t *const src,
                                   const int src_stride,
                                   const uint8_t *const ref,
                                   const int ref_stride, const int h,
                                   __m128i *const sse, __m128i *const sum) {
  ...
  *sum = _mm_setzero_si128();
  ...
  for (i = 0; i < h; ++i) {
    variance16_kernel_sse2(src + i * src_stride, ref + i * ref_stride, sse,
                           sum);
  }
}

static INLINE void variance32_sse2(const uint8_t *const src,
                                   const int src_stride,
                                   const uint8_t *const ref,
                                   const int ref_stride, const int h,
                                   __m128i *const sse, __m128i *const sum) {
  ...
  *sum = _mm_setzero_si128();
  ...
  for (i = 0; i < h; ++i) {
    variance16_kernel_sse2(src + i * src_stride + 0, ref + i * ref_stride + 0,
                           sse, sum);
    variance16_kernel_sse2(src + i * src_stride + 16, ref + i * ref_stride + 16,
                           sse, sum);
  }
}

static INLINE void variance64_sse2(const uint8_t *const src,
                                   const int src_stride,
                                   const uint8_t *const ref,
                                   const int ref_stride, const int h,
                                   __m128i *const sse, __m128i *const sum) {
  ...
  *sum = _mm_setzero_si128();
  ...
  for (i = 0; i < h; ++i) {
    ...
    variance16_kernel_sse2(src + i * src_stride + 16, ref + i * ref_stride + 16,
                           sse, sum);
    variance16_kernel_sse2(src + i * src_stride + 32, ref + i * ref_stride + 32,
                           sse, sum);
    variance16_kernel_sse2(src + i * src_stride + 48, ref + i * ref_stride + 48,
                           sse, sum);
  }
}
...
unsigned int vpx_variance32x64_sse2(const uint8_t *src, int src_stride,
  ...
  for (i = 0; i < 2; i++) {
    ...
    variance32_sse2(src + 32 * i * src_stride, src_stride,
                    ref + 32 * i * ref_stride, ref_stride, 32, &vsse, &vsum);
    sum += sum_final_sse2(vsum);
  }
  *sse = add32x4_sse2(vsse);
...
unsigned int vpx_variance64x32_sse2(const uint8_t *src, int src_stride,
  ...
  for (i = 0; i < 2; i++) {
    ...
    variance64_sse2(src + 16 * i * src_stride, src_stride,
                    ref + 16 * i * ref_stride, ref_stride, 16, &vsse, &vsum);
    sum += sum_final_sse2(vsum);
  }
  *sse = add32x4_sse2(vsse);
  return *sse - (unsigned int)(((int64_t)sum * sum) >> 11);
