...
static INLINE __m128i calculate_qcoeff(__m128i coeff, __m128i round,
                                       __m128i quant, __m128i shift) {
  __m128i tmp;
  coeff = _mm_adds_epi16(coeff, round);
  tmp = _mm_mulhi_epi16(coeff, quant);
  coeff = _mm_add_epi16(tmp, coeff);
  return _mm_mulhi_epi16(coeff, shift);
}
...
static INLINE __m128i scan_for_eob(__m128i coeff0, __m128i coeff1, __m128i zbin_mask0, __m128i zbin_mask1,
                                  const int16_t *scan_ptr, const int index,
                                  const __m128i zero) {
  __m128i zero_coeff0 = _mm_cmpeq_epi16(coeff0, zero);
  __m128i zero_coeff1 = _mm_cmpeq_epi16(coeff1, zero);
