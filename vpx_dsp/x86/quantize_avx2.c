...
static VPX_FORCE_INLINE __m256i quantize_b_32x32_16(
  ...
  }
  {
    ...
    const __m256i v_qcoeff = _mm256_sign_epi16(v_tmp32, v_coeff);
    const __m256i v_dq_lo = _mm256_mullo_epi16(v_qcoeff, *v_dequant);
    const __m256i v_dq_hi = _mm256_mulhi_epi16(v_qcoeff, *v_dequant);
    const __m256i v_dqcoeff_lo =
        _mm256_srai_epi32(_mm256_unpacklo_epi16(v_dq_lo, v_dq_hi), 1);
    const __m256i v_dqcoeff_hi =
        _mm256_srai_epi32(_mm256_unpackhi_epi16(v_dq_lo, v_dq_hi), 1);
    const __m256i v_nz_mask =
        _mm256_cmpgt_epi16(v_tmp32, _mm256_setzero_si256());
