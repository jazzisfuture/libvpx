...
static INLINE void highbd_idct4_large_sse2(__m128i *const io) {
  ...
  step[0] = multiplication_round_shift_sse2(temp, sign, cospi_p16_p16);
  temp[0] = _mm_sub_epi32(io[0], io[2]);  // input[0] - input[2]
  abs_extend_64bit_sse2(temp[0], temp, sign);
  step[1] = multiplication_round_shift_sse2(temp, sign, cospi_p16_p16);
  multiplication_and_add_2_sse2(io[1], io[3], &cospi_p24_p24, &cospi_p08_p08,
                                &step[2], &step[3]);
