...
static INLINE void highbd_idct4(__m128i *const io) {
  ...
  step[0] = multiplication_round_shift_sse4_1(temp, cospi_p16_p16);
  temp[0] = _mm_sub_epi32(io[0], io[2]);  // input[0] - input[2]
  extend_64bit(temp[0], temp);
  step[1] = multiplication_round_shift_sse4_1(temp, cospi_p16_p16);
  multiplication_and_add_2_sse4_1(io[1], io[3], &cospi_p24_p24, &cospi_p08_p08,
                                  &step[2], &step[3]);
