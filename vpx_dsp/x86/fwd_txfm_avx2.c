...
static INLINE int check256_epi16_overflow_x2(const __m256i *preg0,
                                             const __m256i *preg1) {
...
static INLINE int check256_epi16_overflow_x4(const __m256i *preg0,
                                             const __m256i *preg1,
                                             const __m256i *preg2,
                                             const __m256i *preg3) {
  const __m256i max_overflow = _mm256_set1_epi16(0x7fff);
  ...
  cmp0 =
      _mm256_or_si256(_mm256_or_si256(cmp0, cmp1), _mm256_or_si256(cmp2, cmp3));
  return _mm256_movemask_epi8(cmp0);
}

static INLINE int check256_epi16_overflow_x8(
  ...
  const int res0 = check256_epi16_overflow_x4(preg0, preg1, preg2, preg3);
  const int res1 = check256_epi16_overflow_x4(preg4, preg5, preg6, preg7);
  return res0 + res1;
}
