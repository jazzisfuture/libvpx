...
void vpx_get16x16var_avx2(const unsigned char *src_ptr, int source_stride,
  ...
  for (i = 0; i < 8; ++i) {
    ...
    const __m256i src0 = _mm256_cvtepu8_epi16(
  ...
  }
  {
    ...
    const __m128i sum_reg_128 = _mm_add_epi16(_mm256_castsi256_si128(sum_reg), _mm256_extractf128_si256(sum_reg, 1));
    const __m128i sse_reg_128 = _mm_add_epi32(_mm256_castsi256_si128(sse_reg), _mm256_extractf128_si256(sse_reg, 1));

    // sum upper and lower 64 bits together and convert up to 32 bit values
    const __m128i sum_reg_64 = _mm_add_epi16(sum_reg_128, _mm_srli_si128(sum_reg_128, 8));
...
static void get32x16var_avx2(const unsigned char *src_ptr, int source_stride,
  ...
  for (i = 0; i < 8; i++) {
    ...
    const __m256i src1 = _mm256_loadu_si256((__m256i const *)(src_ptr + source_stride));
    const __m256i ref0 = _mm256_loadu_si256((__m256i const *)(ref_ptr));
    const __m256i ref1 = _mm256_loadu_si256((__m256i const *)(ref_ptr + recon_stride));
