...
void vpx_get16x16var_avx2(const unsigned char *src_ptr, int source_stride,
...

  {
    ...
    const __m128i sum = _mm_add_epi32(sum_lo, sum_hi);
    ...
    const __m128i ex_sum_lo = _mm_unpacklo_epi32(sum, zero_reg);
    const __m128i ex_sum_hi = _mm_unpackhi_epi32(sum, zero_reg);
