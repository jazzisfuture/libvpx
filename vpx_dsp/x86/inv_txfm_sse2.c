...
static INLINE void idct32_34_8x32_quarter_1_2(const __m128i *const in /*in[8]*/,
...
static void idct32_34_8x32(const __m128i *const in /*in[32]*/,
                           __m128i *const out /*out[32]*/) {
  __m128i temp[32];

  transpose_16bit_8x8(in, out);
  idct32_34_8x32_quarter_1_2(out, temp);
  idct32_34_8x32_quarter_3_4(out, temp);
  // final stage
...
void vpx_idct32x32_34_add_sse2(const tran_low_t *input, uint8_t *dest,
  ...
  idct32_load_buffer_8x8(input, io);
  idct32_34_8x32(io, col);

  for (i = 0; i < 32; i += 8) {
    int j;
    idct32_34_8x32(col + i, io);
...
static void idct32_1024_8x32(const __m128i *const in /*in[32]*/,
                             __m128i *const out /*out[32]*/) {
  __m128i temp[32];
...
static INLINE void idct32_load_buffer_8x32(const tran_low_t *input,
                                           __m128i *const in) {
  int i;
  for (i = 0; i < 8; ++i) {
...
void vpx_idct32x32_1024_add_sse2(const tran_low_t *input, uint8_t *dest,
                                 int stride) {
  __m128i col[4][32], in[32];
  int i;

  // rows
  for (i = 0; i < 4; i++) {
    idct32_load_buffer_8x32(input, in);
    input += 32 << 3;
  ...
  for (i = 0; i < 32; i += 8) {
    ...
    transpose_16bit_8x8(col[2] + i, in + 16);
    transpose_16bit_8x8(col[3] + i, in + 24);

    idct32_1024_8x32(in, in);
