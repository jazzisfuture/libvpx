...
static INLINE void idct_load16x16(const tran_low_t *input, __m256i *in,
  ...
  // Load 16x16 values
  for (i = 0; i < 16; i++) {
    in[i] = _mm256_loadu_si256((const __m256i *)(input + i * stride));
  }
}
...
static INLINE void butterfly16(__m256i in0, __m256i in1, int c0, int c1,
  ...
  const __m256i cst0 = PAIR256_SET_EPI16(c0, -c1);
  const __m256i cst1 = PAIR256_SET_EPI16(c1, c0);
...
static INLINE void recon_and_store16(uint8_t *dest, __m256i in_x) {
  ...
  __m256i d0 = _mm256_castsi128_si256(_mm_loadl_epi64((__m128i *)(dest)));
  d0 = _mm256_inserti128_si256(d0, _mm_loadu_si128((const __m128i *)(dest + 8)),
                               1);
  d0 = _mm256_unpacklo_epi8(d0, zero);
  d0 = _mm256_add_epi16(in_x, d0);
  d0 = _mm256_packus_epi16(d0, d0);

  _mm_storel_epi64((__m128i *)dest, _mm256_castsi256_si128(d0));
  _mm_storel_epi64((__m128i *)(dest + 8), _mm256_extractf128_si256(d0, 1));
}
...
void vpx_idct32x32_1024_add_avx2(const tran_low_t *input, uint8_t *dest,
  ...
  for (i = 0; i < 32; i += 16) {
    ...
    for (j = 0; j < 32; ++j) {
      write_buffer_16x1(dest + j * stride, out[j]);
    }
    dest += 16;
  }
}
