...
#define FUN_CONV_2D(avg, opt)                                                  \
  void vpx_convolve8_##avg##opt(                                               \
    ...
    if (filter_x[0] | filter_x[1] | filter_x[6] | filter_x[7]) {               \
      ...
      vpx_convolve8_horiz_##opt(src - 1 * src_stride, src_stride,              \
                                       fdata2, 64, filter, x0_q4, x_step_q4,   \
                                       y0_q4, y_step_q4, w, h + 3);            \
      vpx_convolve8_##avg##vert_##opt(                                         \
          fdata2 + 64, 64, dst, dst_stride, filter, x0_q4, x_step_q4,          \
          y0_q4, y_step_q4, w, h);                                             \
    } else {                                                                   \
