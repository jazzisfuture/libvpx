...
static INLINE void transpose_16bit_8x4(const __m128i *const in,
  ...
  // a0:    00 10 01 11  02 12 03 13
  // a1:    20 30 21 31  22 32 23 33
  // a2:    04 14 05 15  06 16 07 17
  // a3:    24 34 25 35  26 36 27 37
  const __m128i a0 = _mm_unpacklo_epi16(in[0], in[1]);
  const __m128i a1 = _mm_unpacklo_epi16(in[2], in[3]);
  const __m128i a2 = _mm_unpackhi_epi16(in[0], in[1]);
  const __m128i a3 = _mm_unpackhi_epi16(in[2], in[3]);

  // Unpack 32 bit elements resulting in:
  // out[0]: 00 10 20 30  01 11 21 31
  // out[1]: 02 12 22 32  03 13 23 33
  // out[2]: 04 14 24 34  05 15 25 35
  // out[3]: 06 16 26 36  07 17 27 37
  out[0] = _mm_unpacklo_epi32(a0, a1);
  out[1] = _mm_unpackhi_epi32(a0, a1);
  out[2] = _mm_unpacklo_epi32(a2, a3);
  out[3] = _mm_unpackhi_epi32(a2, a3);
}
