...
%else
    ...
    movh                     m0, [srcq                ]     ;A
    movh                     m1, [srcq + 8               ]  ;A
    movh                     m2, [srcq + sstrideq     ]     ;B
    movh                     m3, [srcq + sstrideq + 8    ]  ;B
    lea                    srcq, [srcq + sstrideq * 2]
    punpcklbw                m0, m2                         ;A B
    punpcklbw                m1, m3                         ;A B
    movh                     m4, [srcq]                     ;C
    movh                     m5, [srcq + 8]                 ;C
    movh                     m6, [srcq + sstrideq]          ;D
    movh                     m7, [srcq + sstrideq + 8]      ;D
    lea                    srcq, [srcq + sstrideq * 2]
    punpcklbw                m2, m4                         ;A B next iter
    punpcklbw                m3, m5                         ;A B next iter
    ...
    movh                     m8, [srcq]                     ;E
    movh                     m9, [srcq + 8]                 ;E
    movh                    m10, [srcq + sstrideq]          ;F
    movh                    m11, [srcq + sstrideq + 8]      ;F
    lea                    srcq, [srcq + sstrideq * 2]
    punpcklbw                m6, m8                         ;C D next iter
    punpcklbw                m7, m9                         ;C D next iter
    punpcklbw                m8, m10                        ;E F
    punpcklbw                m9, m11                        ;E F
    movh                     m2, [srcq]                     ;G
    movh                     m3, [srcq + 8]                 ;G
    punpcklbw               m10, m2                         ;E F next iter
    punpcklbw               m11, m3                         ;E F next iter
    punpcklqdq               m2, m3
...
.loop:
    ...
    pmaddubsw                m8, k4k5
    paddsw                  m13, m8
    movh                     m3, [srcq + sstrideq]          ;H
    movhps                   m3, [srcq + sstrideq + 8]      ;H
    lea                    srcq, [srcq + sstrideq * 2]
...
%endif
    ...
    pmaddubsw               m10, k4k5
    paddsw                  m13, m10
    movh                     m2, [srcq]                     ;G next iter
    movhps                   m2, [srcq + 8]                 ;G next iter
...
%endif
    ...
    movh                     m7, [srcq + sstrideq]          ;H
    movhps                   m3, [srcq + sstrideq + 8]      ;H
    punpcklbw                m6, m2, m7                     ;G H
    punpckhbw                m2, m3                         ;G H
    pmaddubsw                m0, k0k1
    pmaddubsw                m1, k0k1
    pmaddubsw                m4, k2k3
