...
sh_k1k0:  times 8 dw 0200h
sh_k3k2:  times 8 dw 0604h
sh_k5k4:  times 8 dw 0a08h
sh_k7k6:  times 8 dw 0e0ch

SECTION .text
%if ARCH_X86_64
  %define LOCAL_VARS_SIZE 0
%else
  %define LOCAL_VARS_SIZE 16*5
%endif
...
%if ARCH_X86_64
    ...
    %define    krd  m12
    %define    tmp  m13
    mova       krd, [GLOBAL(pw_256)]
    mova      k0k1, m4
    pshufb    k0k1, [GLOBAL(sh_k1k0)]
    mova      k2k3, m4
    pshufb    k2k3, [GLOBAL(sh_k3k2)]
    mova      k4k5, m4
    pshufb    k4k5, [GLOBAL(sh_k5k4)]
    mova      k6k7, m4
    pshufb    k6k7, [GLOBAL(sh_k7k6)]
%else
    ...
    %define    tmp  [rsp + 16*4]
    %define    krd  [GLOBAL(pw_256)]
    pshufb      m0, m4, [GLOBAL(sh_k1k0)]
    pshufb      m1, m4, [GLOBAL(sh_k3k2)]
    pshufb      m2, m4, [GLOBAL(sh_k5k4)]
    pshufb      m3, m4, [GLOBAL(sh_k7k6)]
    mova      k0k1, m0
    mova      k2k3, m1
    mova      k4k5, m2
    mova      k6k7, m3
...
cglobal filter_block1d4_%1, 6, 8, 11, LOCAL_VARS_SIZE, \
...
%else
    ...
    mova                m5, [GLOBAL(pw_256)]
    mova          k0k1k4k5, m6
    mova          k2k3k6k7, m7
    mova               krd, m5
%endif
    mov           height2q, heightq
    shr            heightq, 1
.loop:
    ;Do two rows once
    movh                m0, [srcq - 3]
...
cglobal filter_block1d8_%1, 6, 8, 13, LOCAL_VARS_SIZE, \
                            src, sstride, dst, dstride, height, filter, \
                            goffset, height2
    GET_GOT        goffsetq
    mova                 m4, [filterq]
...
cglobal filter_block1d16_%1, 6, 7, 13, LOCAL_VARS_SIZE, \
                             src, sstride, dst, dstride, height, filter, goffset
    GET_GOT goffsetq
    mova          m4, [filterq]
    SETUP_LOCAL_VARS
.loop:
...
cglobal filter_block1d%2_%1, 6, 9, 14, LOCAL_VARS_SIZE, \
                             src, sstride, dst, dstride, height, filter, \
                             goffset, src1, sstride6
    GET_GOT goffsetq
    mova          m4, [filterq]
    SETUP_LOCAL_VARS
    mov       src1q, srcq
...
cglobal filter_block1d16_%1, 6, 9, 13, LOCAL_VARS_SIZE, \
                             src, sstride, dst, dstride, height, filter, \
                             goffset, src1, sstride6
    GET_GOT goffsetq
    mova          m4, [filterq]
