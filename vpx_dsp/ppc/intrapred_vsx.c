...
uint8x16_t avg16(const uint8_t *values) {
  int32x4_t sum4s = (int32x4_t)vec_sum4s(vec_vsx_ld(0, values), vec_splat_u32(0));
  uint32x4_t sum  = (uint32x4_t)vec_sums(sum4s, vec_splat_s32(8));
  uint32x4_t avg  = (uint32x4_t)vec_sr(sum, vec_splat_u32(4));

  return vec_splat(vec_pack(vec_pack(avg, vec_splat_u32(0)), vec_splat_u16(0)), 3);
}
...
static uint8x16_t avg3(const uint8x16_t a, const uint8x16_t b, const uint8x16_t c) {
  uint8x16_t ac = vec_adds(vec_and(a, c),
                           vec_sr(vec_xor(a, c), vec_splat_u8(1)));

  return vec_avg(ac, b);
}

// Workaround vec_sld/vec_xxsldi/vec_lsdoi being missing or broken.
static const uint8x16_t sl1 = { 0x1, 0x2, 0x3, 0x4,
                                0x5, 0x6, 0x7, 0x8,
                                0x9, 0xA, 0xB, 0xC,
                                0xD, 0xE, 0xF, 0x10 };
