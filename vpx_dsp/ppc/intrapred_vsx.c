...
uint8x16_t avg16(const uint8_t *values) {
  int32x4_t sum4s =
      (int32x4_t)vec_sum4s(vec_vsx_ld(0, values), vec_splat_u32(0));
  uint32x4_t sum = (uint32x4_t)vec_sums(sum4s, vec_splat_s32(8));
  uint32x4_t avg = (uint32x4_t)vec_sr(sum, vec_splat_u32(4));
...
uint8x16_t avg32(const uint8_t *values) {
  uint8x16_t v0 = vec_vsx_ld(0, values);
  uint8x16_t v1 = vec_vsx_ld(16, values);
  int32x4_t v16 = vec_sl(vec_splat_s32(1), vec_splat_u32(4));
  int32x4_t sum4s = (int32x4_t)vec_sum4s(v0, vec_sum4s(v1, vec_splat_u32(0)));
  uint32x4_t sum = (uint32x4_t)vec_sums(sum4s, v16);
  uint32x4_t avg = (uint32x4_t)vec_sr(sum, vec_splat_u32(5));
...
uint8x16_t dc_avg8(const uint8_t *above, const uint8_t *left) {
  uint8x16_t a0 = vec_vsx_ld(0, above);
  uint8x16_t l0 = vec_vsx_ld(0, left);
  int32x4_t sum4s = (int32x4_t)vec_sum4s(l0, vec_sum4s(a0, vec_splat_u32(0)));
  int32x4_t sum4s8 = xxpermdi(sum4s, vec_splat_s32(0), 1);
  uint32x4_t sum = (uint32x4_t)vec_sums(sum4s8, vec_splat_s32(8));
  uint32x4_t avg = (uint32x4_t)vec_sr(sum, vec_splat_u32(4));
...
uint8x16_t dc_avg16(const uint8_t *above, const uint8_t *left) {
  uint8x16_t a0 = vec_vsx_ld(0, above);
  uint8x16_t l0 = vec_vsx_ld(0, left);
  int32x4_t v16 = vec_sl(vec_splat_s32(1), vec_splat_u32(4));
  int32x4_t sum4s = (int32x4_t)vec_sum4s(l0, vec_sum4s(a0, vec_splat_u32(0)));
...
uint8x16_t dc_avg32(const uint8_t *above, const uint8_t *left) {
  uint8x16_t a0 = vec_vsx_ld(0, above);
  uint8x16_t a1 = vec_vsx_ld(16, above);
  uint8x16_t l0 = vec_vsx_ld(0, left);
  uint8x16_t l1 = vec_vsx_ld(16, left);
  int32x4_t v32 = vec_sl(vec_splat_s32(1), vec_splat_u32(5));
  uint32x4_t a_sum = vec_sum4s(a0, vec_sum4s(a1, vec_splat_u32(0)));
  int32x4_t sum4s = (int32x4_t)vec_sum4s(l0, vec_sum4s(l1, a_sum));
  uint32x4_t sum = (uint32x4_t)vec_sums(sum4s, v32);
  uint32x4_t avg = (uint32x4_t)vec_sr(sum, vec_splat_u32(6));
...
void vpx_d63_predictor_4x4_vsx(uint8_t *dst, ptrdiff_t stride,
                               const uint8_t *above, const uint8_t *left) {
  const uint8x16_t af = vec_vsx_ld(0, above);
  const uint8x16_t above_right = vec_splat(af, 7);
  const uint8x16_t a = vec_sel(af, above_right, (uint8x16_t)mask4);
  const uint8x16_t b = vec_perm(a, above_right, sl1);
  const uint8x16_t c = vec_perm(b, above_right, sl1);
  ...
  for (i = 0; i < 2; i++) {
    ...
    const uint8x16_t d1 = vec_vsx_ld(stride, dst);
    vec_vsx_st(vec_sel(row0, d0, (uint8x16_t)mask4), 0, dst);
    vec_vsx_st(vec_sel(row1, d1, (uint8x16_t)mask4), stride, dst);
    dst += stride * 2;
...
void vpx_d63_predictor_8x8_vsx(uint8_t *dst, ptrdiff_t stride,
  ...
  for (i = 0; i < 4; i++) {
    ...
    vec_vsx_st(xxpermdi(row1, d1, 1), stride, dst);
    dst += stride * 2;
    row0 = vec_perm(row0, above_right, sl1);
    row1 = vec_perm(row1, above_right, sl1);
  }
...
void vpx_d63_predictor_32x32_vsx(uint8_t *dst, ptrdiff_t stride,
  ...
  for (i = 0; i < 16; i++) {
    ...
    vec_vsx_st(row1_0, stride, dst);
    vec_vsx_st(row1_1, stride + 16, dst);
    dst += stride * 2;
