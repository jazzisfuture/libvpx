...
void vpx_idct8x8_64_add_vsx(const tran_low_t *input, uint8_t *dest,
  ...
  vec_vsx_st(output0, 0, tmp_dest);
  vec_vsx_st(output1, 16, tmp_dest);
  vec_vsx_st(output2, 32, tmp_dest);
  vec_vsx_st(output3, 48, tmp_dest);
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < 8; j++) dest[j * stride + i] = tmp_dest[j * 8 + i];
}
