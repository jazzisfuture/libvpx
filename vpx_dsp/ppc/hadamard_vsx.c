...
static void vpx_hadamard_s16_8x8_one_pass(vector signed short v[8]) {
...
void vpx_hadamard_16x16_vsx(const int16_t *src_diff, int src_stride,
  ...
  for (i = 0; i < 64; i += 8) {
    const vector signed short a0 = load_tran_low(0, coeff);
    const vector signed short a1 = load_tran_low(0, coeff + 64);
    const vector signed short a2 = load_tran_low(0, coeff + 128);
    const vector signed short a3 = load_tran_low(0, coeff + 192);
