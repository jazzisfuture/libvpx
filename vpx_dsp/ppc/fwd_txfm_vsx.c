...
static uint8x16_t perm_1_v = { 0x00, 0x01, 0x02, 0x03,
                               0x08, 0x09, 0x0A, 0x0B,
                               0x10, 0x11, 0x12, 0x13,
                               0x18, 0x19, 0x1A, 0x1B };
static uint8x16_t perm_2_v = { 0x06, 0x07, 0x04, 0x05,
                               0x0E, 0x0F, 0x0C, 0x0D,
                               0x16, 0x17, 0x14, 0x15,
                               0x1E, 0x1F, 0x1C, 0x1D };
static uint8x16_t perm_3_v = { 0x02, 0x03, 0x10, 0x11,
                               0x02, 0x03, 0x10, 0x11,
                               0x06, 0x07, 0x14, 0x15,
                               0x06, 0x07, 0x14, 0x15 };
static uint8x16_t perm_4_v = { 0x0A, 0x0B, 0x18, 0x19,
                               0x0A, 0x0B, 0x18, 0x19,
                               0x0E, 0x0F, 0x1C, 0x1D,
                               0x0E, 0x0F, 0x1C, 0x1D };
static uint8x16_t perm_5_v = { 0x00, 0x01, 0x12, 0x13,
                               0x00, 0x01, 0x12, 0x13,
                               0x04, 0x05, 0x16, 0x17,
                               0x04, 0x05, 0x16, 0x17 };
static uint8x16_t perm_6_v = { 0x08, 0x09, 0x1A, 0x1B,
                               0x08, 0x09, 0x1A, 0x1B,
                               0x0C, 0x0D, 0x1E, 0x1F,
                               0x0C, 0x0D, 0x1E, 0x1F };

static int16x8_t cospi_1_v = { 11585, 15137, -11585, 6270,
                               11585, 15137, -11585, 6270 };
static int16x8_t cospi_2_v = { 11585, 6270, 11585, -15137,
...
static INLINE void vpx_fdct4x4_one_pass(const int16x8_t input[2],
                                        int32x4_t output[4]) {
  int16x8_t v[2];
  int16x8_t s[2];
  int16x8_t a[2];
...
void vpx_fdct4x4_vsx(const int16_t *input, tran_low_t *output, int stride) {
  ...
  vpx_transpose_s32_4x4(v_intermediate, transposed);

#ifdef WORDS_BIGENDIAN
  v_out[0] = vec_pack(transposed[1], transposed[0]);
  v_out[1] = vec_pack(transposed[3], transposed[2]);
#else
  v_out[0] = vec_pack(transposed[0], transposed[1]);
  v_out[1] = vec_pack(transposed[2], transposed[3]);
#endif  // WORDS_BIGENDIAN

  vpx_fdct4x4_one_pass(v_out, last);

  last[0] = vec_add(last[0], vec_ones_s32);
  last[1] = vec_add(last[1], vec_ones_s32);
  last[2] = vec_add(last[2], vec_ones_s32);
  last[3] = vec_add(last[3], vec_ones_s32);

  last[0] = vec_sra(last[0], vec_twos_u32);
  last[1] = vec_sra(last[1], vec_twos_u32);
  last[2] = vec_sra(last[2], vec_twos_u32);
  last[3] = vec_sra(last[3], vec_twos_u32);
