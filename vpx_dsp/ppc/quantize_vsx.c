...
static INLINE int16x8_t nonzero_scanindex(int16x8_t qcoeff0, bool16x8_t mask0,
                                          ...
                                          const int16_t *iscan_ptr, int offset0,
                                          int offset1) {
  bool16x8_t zero_coeff0, zero_coeff1;
  int16x8_t eob0, eob1;
  int16x8_t scan0 = vec_vsx_ld(offset0, iscan_ptr);
  int16x8_t scan1 = vec_vsx_ld(offset1, iscan_ptr);
...
void vpx_quantize_b_vsx(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  int16x8_t coeff0 = vec_vsx_ld(offset0, coeff_ptr);
  int16x8_t coeff1 = vec_vsx_ld(offset1, coeff_ptr);
  ...
  while (n_coeffs > 0) {
    offset0 += 32;
    offset1 += 32;
    coeff0 = vec_vsx_ld(offset0, coeff_ptr);
    coeff1 = vec_vsx_ld(offset1, coeff_ptr);
    ...
    eob =
        vec_max(eob, nonzero_scanindex(qcoeff0, zero_mask0, qcoeff1, zero_mask1,
                                       iscan_ptr, offset0, offset1));
    n_coeffs -= 16;
  }
