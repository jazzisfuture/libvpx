...
// integers, and return the high 16 bits of the intermediate integers.
// (a * b) >> 17
static INLINE int16x8_t vec_mulhi_2(int16x8_t a, int16x8_t b) {
  // madds does ((A * B) >>15) + C, but we need >> 17.
  return vec_madds(a, b, vec_zeros_s16);
}
...
static INLINE int16x8_t quantize_coeff_32(int16x8_t coeff, int16x8_t coeff_abs,
  ...
  qcoeff = vec_mulhi(rounded, quant);
  qcoeff = vec_add(qcoeff, rounded);
  qcoeff = vec_mulhi_2(qcoeff, quant_shift);
  qcoeff = vec_sign(qcoeff, coeff);
  return vec_and(qcoeff, mask);
}
...
static INLINE int16x8_t dequantize_coeff_32(int16x8_t qcoeff,
                                            int16x8_t dequant) {
  int16x8_t dqcoeff;
  // TODO(ltrudeau) work with sign bit instead of absolute.
  int16x8_t qcoeff_abs = vec_abs(qcoeff);
  int32x4_t dqcoeff0 = vec_unpackl(qcoeff_abs);
  int32x4_t dqcoeff1 = vec_unpackh(qcoeff_abs);

  dqcoeff0 = vec_mul(dqcoeff0, vec_unpackl(dequant));
  dqcoeff1 = vec_mul(dqcoeff1, vec_unpackh(dequant));

  dqcoeff0 = vec_sra(dqcoeff0, vec_ones_u32);
  dqcoeff1 = vec_sra(dqcoeff1, vec_ones_u32);

  dqcoeff = vec_pack(dqcoeff1, dqcoeff0);
  return vec_sign(dqcoeff, qcoeff);
}
...
void vpx_quantize_b_32x32_vsx(
  ...
  // Divide zbin by 2
  zbin = vec_add(zbin, vec_ones_s16);
  zbin = vec_sra(zbin, vec_ones_u16);

  round = vec_add(round, vec_ones_s16);
  round = vec_sra(round, vec_ones_u16);
