...
void vpx_quantize_b_vsx(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (n_coeffs > 16) {
    ...
    do {
      ...
      coeff2 = vec_vsx_ld(32, coeff_ptr);
      ...
      vec_vsx_st(qcoeff0, 0, qcoeff_ptr);
      vec_vsx_st(qcoeff1, 16, qcoeff_ptr);
      vec_vsx_st(qcoeff2, 32, qcoeff_ptr);
      ...
      vec_vsx_st(dqcoeff0, 0, dqcoeff_ptr);
      vec_vsx_st(dqcoeff1, 16, dqcoeff_ptr);
      vec_vsx_st(dqcoeff2, 32, dqcoeff_ptr);

      eob = vec_max(eob, nonzero_scanindex(qcoeff0, zero_mask0, iscan_ptr, 0));
      eob2 = vec_max(nonzero_scanindex(qcoeff1, zero_mask1, iscan_ptr, 16),
                     nonzero_scanindex(qcoeff2, zero_mask2, iscan_ptr, 32));
