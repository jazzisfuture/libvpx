...
void vpx_mbpost_proc_down_vsx(uint8_t *dst, int pitch, int rows, int cols,
  ...
  for (col = 0; col < cols; col += 8) {
    ...
    for (row = 0; row < rows; row++) {
      ...
      if (row + 7 < rows) {
        window[end] = unpack_to_s16_h(vec_vsx_ld((row + 7) * pitch, dst));
      } else {
        window[end] = window[(end - 1) & 15];
      }

      // C: sum += s[7 * pitch] - s[-8 * pitch];
      sum = vec_add(sum, vec_sub(window[end], window[start]));

      // TODO(ltrudeau) Replace with squared Sliding Window
      // C: sumsq += s[7 * pitch] * s[7 * pitch] - s[-8 * pitch] * s[-8 *
      // pitch];
      d15_odd = vec_mulo(window[end], window[end]);
      d15_even = vec_mule(window[end], window[end]);
      d0_odd = vec_mulo(window[start], window[start]);
      d0_even = vec_mule(window[start], window[start]);
      ...
      filtered = vec_add(vec_add(rv, sum), window[curr]);
      ...
      masked = vec_sel(window[curr], filtered, mask);
