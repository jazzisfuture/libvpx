...
static INLINE int16x8_t slide_sum_s16(int16x8_t x) {
  ...
  //
  // 0 A B C D E F G
  static const uint8x16_t perm_1z = {
    0x10, 0x11, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
    0x06, 0x07, 0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D,
  };
  // 0 0 A B C D E F
  static const uint8x16_t perm_2z = { 0x10, 0x11, 0x12, 0x13, 0x00, 0x01,
                                      0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                      0x08, 0x09, 0x0A, 0x0B };
  // 0 0 0 A B C D E
  static const uint8x16_t perm_3z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x00, 0x01, 0x02, 0x03, 0x04, 0x05,
                                      0x06, 0x07, 0x08, 0x09 };
  // 0 0 0 0 A B C D
  static const uint8x16_t perm_4z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x16, 0x17, 0x00, 0x01, 0x02, 0x03,
                                      0x04, 0x05, 0x06, 0x07 };
  // 0 0 0 0 0 A B C
  static const uint8x16_t perm_5z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x16, 0x17, 0x18, 0x19, 0x00, 0x01,
                                      0x02, 0x03, 0x04, 0x05 };
  // 0 0 0 0 0 0 A B
  static const uint8x16_t perm_6z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
                                      0x00, 0x01, 0x02, 0x03 };
  // 0 0 0 0 0 0 0 A
  static const uint8x16_t perm_7z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x16, 0x17, 0x18, 0x19, 0x1A, 0x1B,
                                      0x1C, 0x1D, 0x00, 0x01 };

  const int16x8_t sum1 = vec_add(x, vec_perm(x, vec_zeros_s16, perm_1z));
  const int16x8_t sum2 = vec_add(vec_perm(x, vec_zeros_s16, perm_2z),
                                 vec_perm(x, vec_zeros_s16, perm_3z));
  const int16x8_t sum3 = vec_add(vec_perm(x, vec_zeros_s16, perm_4z),
                                 vec_perm(x, vec_zeros_s16, perm_5z));
  const int16x8_t sum4 = vec_add(vec_perm(x, vec_zeros_s16, perm_6z),
                                 vec_perm(x, vec_zeros_s16, perm_7z));
...
static INLINE int32x4_t slide_sumsq_s32(int32x4_t xsq_even, int32x4_t xsq_odd) {
  // 0 * * *
  static const uint8x16_t perm_1z = { 0x10, 0x11, 0x12, 0x13, 0x00, 0x01,
                                      0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                      0x08, 0x09, 0x0A, 0x0B };
  // 0 0 * *
  static const uint8x16_t perm_2z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x16, 0x17, 0x00, 0x01, 0x02, 0x03,
                                      0x04, 0x05, 0x06, 0x07 };
  // 0 0 0 *
  static const uint8x16_t perm_3z = { 0x10, 0x11, 0x12, 0x13, 0x14, 0x15,
                                      0x15, 0x17, 0x18, 0x19, 0x1A, 0x1B,
                                      0x00, 0x01, 0x02, 0x03 };
  //   0 A C E
  // + 0 B D F
  int32x4_t sumsq_1 = vec_add(vec_perm(xsq_even, vec_zeros_s32, perm_1z),
                              vec_perm(xsq_odd, vec_zeros_s32, perm_1z));

  //   0 0 A C
  // + 0 0 B D
  int32x4_t sumsq_2 = vec_add(vec_perm(xsq_even, vec_zeros_s32, perm_2z),
                              vec_perm(xsq_odd, vec_zeros_s32, perm_2z));

  //   0 0 0 A
  // + 0 0 0 B
  int32x4_t sumsq_3 = vec_add(vec_perm(xsq_even, vec_zeros_s32, perm_3z),
                              vec_perm(xsq_odd, vec_zeros_s32, perm_3z));
