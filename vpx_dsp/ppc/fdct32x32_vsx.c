...
void vpx_fdct32_vsx(const int16x8_t *in, int16x8_t *out, int pass) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    output[i + 16] = vec_sub(in[15 - i], in[i + 16]);
  }
  ...
  single_butterfly(output[27], output[20], &output[27], &output[20]);
  single_butterfly(output[26], output[21], &output[26], &output[21]);
  single_butterfly(output[25], output[22], &output[25], &output[22]);
  single_butterfly(output[24], output[23], &output[24], &output[23]);
