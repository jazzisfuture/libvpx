...
static INLINE int16x8_t single_butterfly(int16x8_t a, int16x8_t b,
  ...
  const int32x4_t rsum_o = vec_add(sum_o, vec_splats(8192));
  const int32x4_t rsum_e = vec_add(sum_e, vec_splats(8192));

  // TODO(ltrudeau) Replace splat with constant
  const int32x4_t ssum_o = vec_sra(rsum_o, vec_splats((uint32_t)14));
  const int32x4_t ssum_e = vec_sra(rsum_e, vec_splats((uint32_t)14));
...
static INLINE void load(const int16_t *a, int stride, int16x8_t *b) {
  ...
  for (int i = 0; i < 32; i++) {
    ...
    b[i] = vec_sl(b[i], vec_splats((uint16_t)2));
  }
}
...
static INLINE int16x8_t sub_round_shift(const int16x8_t a) {
  const int16x8_t sign = vec_sr(a, vec_splats((uint16_t)15));
  return vec_sra(vec_sub(vec_add(a, vec_splats((int16_t)2)), sign),
                 vec_splats((uint16_t)2));
}
...
static INLINE int16x8_t add_round_shift_s16(const int16x8_t a) {
  const int16x8_t sign = vec_sr(a, vec_splats((uint16_t)15));
  return vec_sra(vec_add(vec_add(a, vec_ones_s16), sign),
                 vec_splats((uint16_t)2));
}
...
void vpx_fdct32_vsx(const int16x8_t *in, int16x8_t *out, int pass) {
  ...
  for (int i = 0; i < 16; i++) {
    ...
    // TODO(ltrudeau) replace vec_neg with sub
    step[i + 16] = vec_add(vec_neg(in[i + 16]), in[15 - i]);
  }

  // Stage 2
  for (int i = 0; i < 8; i++) {
    output[i] = vec_add(step[i], step[15 - i]);
    output[i + 8] = vec_add(vec_neg(step[i + 8]), step[7 - i]);
  }
  ...
  step[4] = vec_add(vec_neg(output[4]), output[3]);
  step[5] = vec_add(vec_neg(output[5]), output[2]);
  step[6] = vec_add(vec_neg(output[6]), output[1]);
  step[7] = vec_add(vec_neg(output[7]), output[0]);
  step[8] = output[8];
  step[9] = output[9];
  ...
  step[20] = vec_add(vec_neg(output[20]), output[19]);
  step[21] = vec_add(vec_neg(output[21]), output[18]);
  step[22] = vec_add(vec_neg(output[22]), output[17]);
  step[23] = vec_add(vec_neg(output[23]), output[16]);
  step[24] = vec_add(vec_neg(output[24]), output[31]);
  step[25] = vec_add(vec_neg(output[25]), output[30]);
  step[26] = vec_add(vec_neg(output[26]), output[29]);
  step[27] = vec_add(vec_neg(output[27]), output[28]);
  ...
  output[2] = vec_add(vec_neg(step[2]), step[1]);
  output[3] = vec_add(vec_neg(step[3]), step[0]);
  output[4] = step[4];
  ...
  output[10] = vec_add(vec_neg(step[10]), step[9]);
  output[11] = vec_add(vec_neg(step[11]), step[8]);
  output[12] = vec_add(vec_neg(step[12]), step[15]);
  output[13] = vec_add(vec_neg(step[13]), step[14]);
  ...
  step[5] = vec_add(vec_neg(output[5]), output[4]);
  step[6] = vec_add(vec_neg(output[6]), output[7]);
  ...
  step[18] = vec_add(vec_neg(output[18]), output[17]);
  step[19] = vec_add(vec_neg(output[19]), output[16]);
  step[20] = vec_add(vec_neg(output[20]), output[23]);
  step[21] = vec_add(vec_neg(output[21]), output[22]);
  ...
  step[26] = vec_add(vec_neg(output[26]), output[25]);
  step[27] = vec_add(vec_neg(output[27]), output[24]);
  step[28] = vec_add(vec_neg(output[28]), output[31]);
  step[29] = vec_add(vec_neg(output[29]), output[30]);
  ...
  output[9] = vec_add(vec_neg(step[9]), step[8]);
  output[10] = vec_add(vec_neg(step[10]), step[11]);
  output[11] = vec_add(step[11], step[10]);
  output[12] = vec_add(step[12], step[13]);
  output[13] = vec_add(vec_neg(step[13]), step[12]);
  output[14] = vec_add(vec_neg(step[14]), step[15]);
  ...
  step[17] = vec_add(vec_neg(output[17]), output[16]);
  step[18] = vec_add(vec_neg(output[18]), output[19]);
  step[19] = vec_add(output[19], output[18]);
  step[20] = vec_add(output[20], output[21]);
  step[21] = vec_add(vec_neg(output[21]), output[20]);
  step[22] = vec_add(vec_neg(output[22]), output[23]);
  step[23] = vec_add(output[23], output[22]);
  step[24] = vec_add(output[24], output[25]);
  step[25] = vec_add(vec_neg(output[25]), output[24]);
  step[26] = vec_add(vec_neg(output[26]), output[27]);
  step[27] = vec_add(output[27], output[26]);
  step[28] = vec_add(output[28], output[29]);
  step[29] = vec_add(vec_neg(output[29]), output[28]);
  step[30] = vec_add(vec_neg(output[30]), output[31]);
