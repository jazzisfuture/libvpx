...
void vpx_fdct32_vsx(const int16x8_t *in, int16x8_t *out, int pass) {
  ...
  single_butterfly(output[0], output[1], cospi16_v, &out[0], &out[16]);
...
void vpx_fdct32x32_rd_vsx(const int16_t *input, tran_low_t *out, int stride) {
  ...
  vpx_transpose_s16_8x8(&temp5[0]);
  vpx_transpose_s16_8x8(&temp5[8]);
  vpx_transpose_s16_8x8(&temp5[16]);
  vpx_transpose_s16_8x8(&temp5[24]);
