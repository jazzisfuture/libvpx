...
static INLINE void load(const int16_t *a, int stride, int16x8_t *b) {
  ...
  // the fastest one.
  {
    ...
    const int16x8_t s0 = vec_sl(l0, vec_dct_scale_log2);
    const int16x8_t s1 = vec_sl(l1, vec_dct_scale_log2);
    const int16x8_t s2 = vec_sl(l2, vec_dct_scale_log2);
    const int16x8_t s3 = vec_sl(l3, vec_dct_scale_log2);
    const int16x8_t s4 = vec_sl(l4, vec_dct_scale_log2);
    const int16x8_t s5 = vec_sl(l5, vec_dct_scale_log2);
    const int16x8_t s6 = vec_sl(l6, vec_dct_scale_log2);
    const int16x8_t s7 = vec_sl(l7, vec_dct_scale_log2);

    const int16x8_t s24 = vec_sl(l24, vec_dct_scale_log2);
    const int16x8_t s25 = vec_sl(l25, vec_dct_scale_log2);
    const int16x8_t s26 = vec_sl(l26, vec_dct_scale_log2);
    const int16x8_t s27 = vec_sl(l27, vec_dct_scale_log2);
    const int16x8_t s28 = vec_sl(l28, vec_dct_scale_log2);
    const int16x8_t s29 = vec_sl(l29, vec_dct_scale_log2);
    const int16x8_t s30 = vec_sl(l30, vec_dct_scale_log2);
    const int16x8_t s31 = vec_sl(l31, vec_dct_scale_log2);
  ...
  }
  {
    ...
    const int16x8_t s8 = vec_sl(l8, vec_dct_scale_log2);
    const int16x8_t s9 = vec_sl(l9, vec_dct_scale_log2);
    const int16x8_t s10 = vec_sl(l10, vec_dct_scale_log2);
    const int16x8_t s11 = vec_sl(l11, vec_dct_scale_log2);
    const int16x8_t s12 = vec_sl(l12, vec_dct_scale_log2);
    const int16x8_t s13 = vec_sl(l13, vec_dct_scale_log2);
    const int16x8_t s14 = vec_sl(l14, vec_dct_scale_log2);
    const int16x8_t s15 = vec_sl(l15, vec_dct_scale_log2);

    const int16x8_t s16 = vec_sl(l16, vec_dct_scale_log2);
    const int16x8_t s17 = vec_sl(l17, vec_dct_scale_log2);
    const int16x8_t s18 = vec_sl(l18, vec_dct_scale_log2);
    const int16x8_t s19 = vec_sl(l19, vec_dct_scale_log2);
    const int16x8_t s20 = vec_sl(l20, vec_dct_scale_log2);
    const int16x8_t s21 = vec_sl(l21, vec_dct_scale_log2);
    const int16x8_t s22 = vec_sl(l22, vec_dct_scale_log2);
    const int16x8_t s23 = vec_sl(l23, vec_dct_scale_log2);

    b[8] = vec_add(s8, s23);
    b[9] = vec_add(s9, s22);
    b[10] = vec_add(s10, s21);
    b[11] = vec_add(s11, s20);
    b[12] = vec_add(s12, s19);
    b[13] = vec_add(s13, s18);
    b[14] = vec_add(s14, s17);
    b[15] = vec_add(s15, s16);

    b[16] = vec_sub(s15, s16);
    b[17] = vec_sub(s14, s17);
    b[18] = vec_sub(s13, s18);
    b[19] = vec_sub(s12, s19);
    b[20] = vec_sub(s11, s20);
    b[21] = vec_sub(s10, s21);
    b[22] = vec_sub(s9, s22);
    b[23] = vec_sub(s8, s23);
  }
}
...
void vpx_fdct32_vsx(const int16x8_t *in, int16x8_t *out, int pass) {
  ...
  // Stage 1
  if (pass) {
    step[0] = vec_add(in[0], in[31]);
    step[31] = vec_sub(in[0], in[31]);
    step[1] = vec_add(in[1], in[30]);
    step[30] = vec_sub(in[1], in[30]);
    step[2] = vec_add(in[2], in[29]);
    step[29] = vec_sub(in[2], in[29]);
    step[3] = vec_add(in[3], in[28]);
    step[28] = vec_sub(in[3], in[28]);
    step[4] = vec_add(in[4], in[27]);
    step[27] = vec_sub(in[4], in[27]);
    step[5] = vec_add(in[5], in[26]);
    step[26] = vec_sub(in[5], in[26]);
    step[6] = vec_add(in[6], in[25]);
    step[25] = vec_sub(in[6], in[25]);
    step[7] = vec_add(in[7], in[24]);
    step[24] = vec_sub(in[7], in[24]);
    step[8] = vec_add(in[8], in[23]);
    step[23] = vec_sub(in[8], in[23]);
    step[9] = vec_add(in[9], in[22]);
    step[22] = vec_sub(in[9], in[22]);
    step[10] = vec_add(in[10], in[21]);
    step[21] = vec_sub(in[10], in[21]);
    step[11] = vec_add(in[11], in[20]);
    step[20] = vec_sub(in[11], in[20]);
    step[12] = vec_add(in[12], in[19]);
    step[19] = vec_sub(in[12], in[19]);
    step[13] = vec_add(in[13], in[18]);
    step[18] = vec_sub(in[13], in[18]);
    step[14] = vec_add(in[14], in[17]);
    step[17] = vec_sub(in[14], in[17]);
    step[15] = vec_add(in[15], in[16]);
    step[16] = vec_sub(in[15], in[16]);
    ...
    // Stage 2
    for (int i = 0; i < 8; i++) {
      output[i] = vec_add(in[i], in[15 - i]);
      output[i + 8] = vec_sub(in[7 - i], in[i + 8]);
    }
    output[16] = in[16];
    output[17] = in[17];
    output[18] = in[18];
    ...
    single_butterfly(in[27], in[20], cospi16_v, &output[27], &output[20]);
    single_butterfly(in[26], in[21], cospi16_v, &output[26], &output[21]);
    single_butterfly(in[25], in[22], cospi16_v, &output[25], &output[22]);
    single_butterfly(in[24], in[23], cospi16_v, &output[24], &output[23]);
  ...
  double_butterfly(step[28], cospi8_v, step[19], cospi24_v, &output[28],
                   &output[19]);
  double_butterfly(step[27], cospi24_v, step[20], cospi8m_v, &output[27],
                   &output[20]);
  double_butterfly(step[26], cospi24_v, step[21], cospi8m_v, &output[26],
                   &output[21]);
  ...
  double_butterfly(output[14], cospi8_v, output[9], cospi24_v, &step[14],
                   &step[9]);
  double_butterfly(output[13], cospi24_v, output[10], cospi8m_v, &step[13],
                   &step[10]);
  step[11] = output[11];
  step[12] = output[12];
  step[15] = output[15];
  ...
  double_butterfly(step[30], cospi4_v, step[17], cospi28_v, &output[30],
                   &output[17]);
  double_butterfly(step[29], cospi28_v, step[18], cospi4m_v, &output[29],
                   &output[18]);
  output[19] = step[19];
  output[20] = step[20];
  double_butterfly(step[26], cospi20_v, step[21], cospi12_v, &output[26],
                   &output[21]);
  double_butterfly(step[25], cospi12_v, step[22], cospi20m_v, &output[25],
                   &output[22]);
  output[23] = step[23];
  output[24] = step[24];
  output[27] = step[27];
