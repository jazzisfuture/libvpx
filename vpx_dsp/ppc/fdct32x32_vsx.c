...
static const int32x4_t cospi16_s32 = { 11585, 11585, 11585, 11585 };
...
*/
  const int32x4_t ah = vec_unpackh(a);
  const int32x4_t al = vec_unpackl(a);
  const int32x4_t bh = vec_unpackh(b);
  const int32x4_t bl = vec_unpackl(b);

  const int32x4_t ac_h = vec_mul(ah, cospi16_s32);
  const int32x4_t ac_l = vec_mul(al, cospi16_s32);
  const int32x4_t bc_h = vec_mul(bh, cospi16_s32);
  const int32x4_t bc_l = vec_mul(bl, cospi16_s32);

  const int32x4_t sum_h = vec_add(ac_h, bc_h);
  const int32x4_t sum_l = vec_add(ac_l, bc_l);
  const int32x4_t diff_h = vec_sub(ac_h, bc_h);
  const int32x4_t diff_l = vec_sub(ac_l, bc_l);

  const int32x4_t rsum_h = vec_add(sum_h, vec_dct_const_rounding);
  const int32x4_t rsum_l = vec_add(sum_l, vec_dct_const_rounding);
  const int32x4_t rdiff_h = vec_add(diff_h, vec_dct_const_rounding);
  const int32x4_t rdiff_l = vec_add(diff_l, vec_dct_const_rounding);

  const int32x4_t ssum_h = vec_sra(rsum_h, vec_dct_const_bits);
  const int32x4_t ssum_l = vec_sra(rsum_l, vec_dct_const_bits);
  const int32x4_t sdiff_h = vec_sra(rdiff_h, vec_dct_const_bits);
  const int32x4_t sdiff_l = vec_sra(rdiff_l, vec_dct_const_bits);

  *add = vec_pack(ssum_h, ssum_l);
  *sub = vec_pack(sdiff_h, sdiff_l);
}
...
// benchmarking show that this is slower.
...
void vpx_fdct32_vsx(const int16x8_t *in, int16x8_t *out, int pass) {
  ...
  single_butterfly(step[27], step[20], &output[27], &output[20]);
  single_butterfly(step[26], step[21], &output[26], &output[21]);
  single_butterfly(step[25], step[22], &output[25], &output[22]);
  single_butterfly(step[24], step[23], &output[24], &output[23]);
  ...
  single_butterfly(output[13], output[10], &step[13], &step[10]);
  single_butterfly(output[12], output[11], &step[12], &step[11]);
  step[14] = output[14];
  step[15] = output[15];
  ...
  output[4] = step[4];
  single_butterfly(step[6], step[5], &output[6], &output[5]);
  output[7] = step[7];
  ...
  single_butterfly(output[0], output[1], &out[0], &out[16]);

  // 2 and 3 pass through to 8 and 24 at the end
  double_butterfly(output[3], cospi8_v, output[2], cospi24_v, &out[8],
                   &out[24]);
  ...
  double_butterfly(step[7], cospi4_v, step[4], cospi28_v, &out[4], &out[28]);
  // 5 and 6 pass through to 20 and 12 at the end
  double_butterfly(step[6], cospi20_v, step[5], cospi12_v, &out[20], &out[12]);
  ...
  // Stage 7
  double_butterfly(output[15], cospi2_v, output[8], cospi30_v, &out[2],
                   &out[30]);
  double_butterfly(output[14], cospi18_v, output[9], cospi14_v, &out[18],
                   &out[14]);
  double_butterfly(output[13], cospi10_v, output[10], cospi22_v, &out[10],
                   &out[22]);
  double_butterfly(output[12], cospi26_v, output[11], cospi6_v, &out[26],
                   &out[6]);
