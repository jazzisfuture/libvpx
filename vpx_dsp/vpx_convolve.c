...
static void convolve_horiz(const uint8_t *src, ptrdiff_t src_stride,
                           ...
                           const int x_filter_size, int x0_q4, int x_step_q4,
                           int w, int h, int avg) {
  int x, y;
  src -= x_filter_size / 2 - 1;
  for (y = 0; y < h; ++y) {
    int x_q4 = x0_q4;
    for (x = 0; x < w; ++x) {
      ...
      int k, sum = 0;
      for (k = 0; k < x_filter_size; ++k) sum += src_x[k] * x_filter[k];
      if (avg)
...
static void convolve_vert(const uint8_t *src, ptrdiff_t src_stride,
                          ...
                          const int y_filter_size, int y0_q4, int y_step_q4,
                          int w, int h, int avg) {
  int x, y;
  src -= src_stride * (y_filter_size / 2 - 1);
...
static void convolve(const uint8_t *src, ptrdiff_t src_stride, uint8_t *dst,
                     ...
                     const int y_filter_size, int y0_q4, int y_step_q4, int w,
                     int h) {
  ...
  int intermediate_height =
      (((h - 1) * y_step_q4 + y0_q4) >> SUBPEL_BITS) + y_filter_size;

  assert(x_filter_size <= 12);
  assert(y_filter_size <= 12);
  assert(w <= 64);
  assert(h <= 64);
  assert(y_step_q4 <= 32);
  assert(x_step_q4 <= 32);

  convolve_horiz(src - src_stride * (y_filter_size / 2 - 1), src_stride, temp,
                 64, x_filters, x_filter_size, x0_q4, x_step_q4, w,
                 intermediate_height, avg);
  convolve_vert(temp + 64 * (y_filter_size / 2 - 1), 64, dst, dst_stride,
                y_filters, y_filter_size, y0_q4, y_step_q4, w, h, avg);
}

static const InterpKernel *get_filter_base(const int16_t *filter,
                                           const int filter_size) {
  if (filter_size == 8) {
    // NOTE: This assumes that the 8-tap filter table is 256-byte aligned.
    // TODO(agrange) Modify to make independent of table alignment.
    return (const InterpKernel *)(((intptr_t)filter) & ~((intptr_t)0xFF));
  } else {
    // filter_size == 12
    // Assumes that 12-tap filter table is 512-byte alligned
    return (const InterpKernel *)(((intptr_t)filter) & ~((intptr_t)0x1FF));
  }
}
...
void vpx_convolve8_horiz_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  const int avg = 0;
  const InterpKernel *const filters_x = get_filter_base(filter_x, filter_size);
  const int x0_q4 = get_filter_offset(filter_x, filters_x);

  (void)filter_y;
  (void)y_step_q4;

  convolve_horiz(src, src_stride, dst, dst_stride, filters_x, filter_size,
                 x0_q4, x_step_q4, w, h, avg);
}
...
void vpx_convolve8_vert_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  const int filter_size = 8;
  const int avg = 0;
  const InterpKernel *const filters_y = get_filter_base(filter_y, filter_size);
  const int y0_q4 = get_filter_offset(filter_y, filters_y);

  (void)filter_x;
  (void)x_step_q4;

  convolve_vert(src, src_stride, dst, dst_stride, filters_y, filter_size, y0_q4,
                y_step_q4, w, h, avg);
}
...
void vpx_convolve8_c(const uint8_t *src, ptrdiff_t src_stride, uint8_t *dst,
  ...
  convolve(src, src_stride, dst, dst_stride, filters_x, filter_size, x0_q4,
           x_step_q4, filters_y, filter_size, y0_q4, y_step_q4, w, h);
}
...
void vpx_convolve12_avg_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  vpx_convolve12_c(src, src_stride, temp, 64, filter_x, x_step_q4, filter_y,
                   y_step_q4, w, h);
  vpx_convolve_avg_c(temp, 64, dst, dst_stride, NULL, 0, NULL, 0, w, h);
}
#endif  // CONFIG_CONVOLVE_12
...
static void highbd_convolve_horiz(const uint8_t *src8, ptrdiff_t src_stride,
                                  ...
                                  const InterpKernel *x_filters,
                                  const int x_filter_size, int x0_q4,
...
static void highbd_convolve_vert(const uint8_t *src8, ptrdiff_t src_stride,
                                 uint8_t *dst8, ptrdiff_t dst_stride,
                                 const InterpKernel *y_filters,
                                 const int y_filter_size, int y0_q4,
                                 int y_step_q4, int w, int h, int bd, int avg) {
  int x, y;
  uint16_t *src = CONVERT_TO_SHORTPTR(src8);
  uint16_t *dst = CONVERT_TO_SHORTPTR(dst8);
  src -= src_stride * (y_filter_size / 2 - 1);
...
static void highbd_convolve(const uint8_t *src, ptrdiff_t src_stride,
  ...
  highbd_convolve_horiz(src - src_stride * (y_filter_size / 2 - 1), src_stride,
                        CONVERT_TO_BYTEPTR(temp), 64, x_filters, x_filter_size,
                        x0_q4, x_step_q4, w, intermediate_height, bd, avg);
  highbd_convolve_vert(CONVERT_TO_BYTEPTR(temp) + 64 * (y_filter_size / 2 - 1),
                       64, dst, dst_stride, y_filters, y_filter_size, y0_q4,
                       y_step_q4, w, h, bd, avg);
}
...
void vpx_highbd_convolve8_horiz_c(const uint8_t *src, ptrdiff_t src_stride,
                                  ...
                                  const int16_t *filter_y, int y_step_q4, int w,
                                  int h, int bd) {
  const int x_filter_size = 8;
  const int avg = 0;
  const InterpKernel *const filters_x =
      get_filter_base(filter_x, x_filter_size);
  const int x0_q4 = get_filter_offset(filter_x, filters_x);
  (void)filter_y;
  (void)y_step_q4;

  highbd_convolve_horiz(src, src_stride, dst, dst_stride, filters_x,
                        x_filter_size, x0_q4, x_step_q4, w, h, bd, avg);
}
...
void vpx_highbd_convolve8_vert_c(const uint8_t *src, ptrdiff_t src_stride,
                                 ...
                                 const int16_t *filter_y, int y_step_q4, int w,
                                 int h, int bd) {
  const int y_filter_size = 8;
  const int avg = 0;
  const InterpKernel *const filters_y =
      get_filter_base(filter_y, y_filter_size);
  const int y0_q4 = get_filter_offset(filter_y, filters_y);
  (void)filter_x;
  (void)x_step_q4;

  highbd_convolve_vert(src, src_stride, dst, dst_stride, filters_y,
                       y_filter_size, y0_q4, y_step_q4, w, h, bd, avg);
}
...
void vpx_highbd_convolve8_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  highbd_convolve(src, src_stride, dst, dst_stride, filters_x, filter_size,
                  x0_q4, x_step_q4, filters_y, filter_size, y0_q4, y_step_q4, w,
                  h, bd);
}
...
void vpx_highbd_convolve12_avg_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  vpx_highbd_convolve12_c(src, src_stride, CONVERT_TO_BYTEPTR(temp), 64,
                          filter_x, x_step_q4, filter_y, y_step_q4, w, h, bd);
  vpx_highbd_convolve_avg_c(CONVERT_TO_BYTEPTR(temp), 64, dst, dst_stride, NULL,
                            0, NULL, 0, w, h, bd);
}
