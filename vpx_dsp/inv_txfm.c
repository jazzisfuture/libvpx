...
static void idct32_pass1_c(const tran_low_t *input, tran_low_t *step1) {
  tran_low_t step2[32];
  tran_high_t temp1, temp2;
...
static void idct32_pass2_c(const tran_low_t *input, tran_low_t *step1) {
  tran_low_t step2[32];
  tran_high_t temp1, temp2;
...
/*
 *
 * FROM idct32x32_add_neon.asm (cd)
...
static void idct32_blk_a_c(const tran_low_t *input, tran_low_t *step1) {
  tran_low_t step2[32];
  tran_high_t temp1, temp2;
...
static void idct32_blk_b_c(const tran_low_t *input, tran_low_t *step1) {
  tran_low_t step2[32];
  tran_high_t temp1, temp2;
  // stage 1
  temp1 = input[5] * cospi_27_64 - input[27] * cospi_5_64;
...
static void idct32_blk_c_c(const tran_low_t *input, tran_low_t *step1) {
  tran_low_t step2[32];
  tran_high_t temp1, temp2;
  // stage 1
  step1[8] = input[2];
...
static void idct32_blk_d_c(const tran_low_t *input, tran_low_t *step1) {
  tran_low_t step2[32];
  tran_high_t temp1, temp2;
  ...
  // stage 7 -- step1 8-15 from block c
  {
    int x;
    for (x = 8; x <= 15; ++x)
      step2[x] = step1[x];
...
void idct32_c(const tran_low_t *input, tran_low_t *output) {
  tran_low_t step1[32];
#if 0
  idct32_pass1_c(input, step1);
  idct32_pass2_c(input, step1);
#else
  idct32_blk_a_c(input, step1);
  idct32_blk_b_c(input, step1);
  idct32_blk_c_c(input, step1);
  idct32_blk_d_c(input, step1);
#endif
  // final stage ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
