...
static void highbd_variance64(const uint8_t *a8, int src_stride,
                              const uint8_t *b8, int ref_stride, int w, int h,
                              uint64_t *sse, int64_t *sum) {
  int i, j;

  uint16_t *src_ptr = CONVERT_TO_SHORTPTR(a8);
  uint16_t *ref_ptr = CONVERT_TO_SHORTPTR(b8);
  *sum = 0;
  *sse = 0;
...
static void highbd_8_variance(const uint8_t *a8, int src_stride,
                              const uint8_t *b8, int ref_stride, int w, int h,
                              uint32_t *sse, int *sum) {
  uint64_t sse_long = 0;
  int64_t sum_long = 0;
  highbd_variance64(a8, src_stride, b8, ref_stride, w, h, &sse_long, &sum_long);
  *sse = (uint32_t)sse_long;
  *sum = (int)sum_long;
}

static void highbd_10_variance(const uint8_t *a8, int src_stride,
                               const uint8_t *b8, int ref_stride, int w, int h,
                               uint32_t *sse, int *sum) {
  uint64_t sse_long = 0;
  int64_t sum_long = 0;
...
static void highbd_12_variance(const uint8_t *a8, int src_stride,
                               const uint8_t *b8, int ref_stride, int w, int h,
                               uint32_t *sse, int *sum) {
  uint64_t sse_long = 0;
  int64_t sum_long = 0;
...
#define HIGHBD_SUBPIX_VAR(W, H)                                              \
  uint32_t vpx_highbd_8_sub_pixel_variance##W##x##H##_c(                     \
    ...
    highbd_var_filter_block2d_bil_first_pass(                                \
        src, fdata3, src_stride, 1, H + 1, W, bilinear_filters[x_offset]);   \
