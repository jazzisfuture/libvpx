...
#define HOR_HALFPIX(W, H)                                                     \
  int vpx_hor_half_pixel##W##x##H##_c(const uint8_t *const a,                 \
                                      const int a_stride, uint8_t *const b) { \
    const int b_stride = (W & ~31) + 32;                                      \
    unsigned int i, j;                                                        \
                                                                              \
    for (i = 0; i < H; ++i) {                                                 \
      for (j = 0; j < W + 1; ++j) {                                           \
        b[i * b_stride + j] = ROUND_POWER_OF_TWO(                             \
            a[i * a_stride + j] + a[i * a_stride + j + 1], 1);                \
      }                                                                       \
    }                                                                         \
    return b_stride;                                                          \
  }
...
#define VER_HALFPIX(W, H)                                                     \
  int vpx_ver_half_pixel##W##x##H##_c(const uint8_t *const a,                 \
                                      const int a_stride, uint8_t *const b) { \
    const int b_stride = (W + 31) & ~31;                                      \
    unsigned int i, j;                                                        \
                                                                              \
    for (i = 0; i < H + 1; ++i) {                                             \
      for (j = 0; j < W; ++j) {                                               \
        b[i * b_stride + j] = ROUND_POWER_OF_TWO(                             \
            a[i * a_stride + j] + a[(i + 1) * a_stride + j], 1);              \
      }                                                                       \
    }                                                                         \
    return b_stride;                                                          \
  }
...
#define HALFPIX_AVG_VAR(W, H)                                             \
  uint32_t vpx_half_pixel_avg_variance##W##x##H##_c(                      \
    ...
    DECLARE_ALIGNED(16, uint8_t, temp[H * W]);                            \
                                                                          \
    vpx_comp_avg_pred_c(temp, second_pred, W, H, a, a_stride);            \
                                                                          \
    return vpx_variance##W##x##H##_c(temp, W, b, b_stride, sse);          \
  }
...
#define HIGHBD_HALFPIX_AVG_VAR(W, H)                                        \
  uint32_t vpx_highbd_8_half_pixel_avg_variance##W##x##H##_c(               \
      const uint8_t *const src, const int src_stride,                       \
      const uint8_t *const dst, const int dst_stride, uint32_t *const sse,  \
      const uint8_t *const second_pred) {                                   \
    DECLARE_ALIGNED(16, uint16_t, temp[H * W]);                             \
                                                                            \
    vpx_highbd_comp_avg_pred_c(temp, second_pred, W, H, src, src_stride);   \
                                                                            \
    return vpx_highbd_8_variance##W##x##H##_c(CONVERT_TO_BYTEPTR(temp), W,  \
                                              dst, dst_stride, sse);        \
  }                                                                         \
                                                                            \
  uint32_t vpx_highbd_10_half_pixel_avg_variance##W##x##H##_c(              \
      const uint8_t *const src, const int src_stride,                       \
      const uint8_t *const dst, const int dst_stride, uint32_t *const sse,  \
      const uint8_t *const second_pred) {                                   \
    DECLARE_ALIGNED(16, uint16_t, temp[H * W]);                             \
    ...
    vpx_highbd_comp_avg_pred_c(temp, second_pred, W, H, src, src_stride);   \
                                                                            \
    return vpx_highbd_10_variance##W##x##H##_c(CONVERT_TO_BYTEPTR(temp), W, \
                                               dst, dst_stride, sse);       \
  }                                                                         \
                                                                            \
  uint32_t vpx_highbd_12_half_pixel_avg_variance##W##x##H##_c(              \
      const uint8_t *const src, const int src_stride,                       \
      const uint8_t *const dst, const int dst_stride, uint32_t *const sse,  \
      const uint8_t *const second_pred) {                                   \
    DECLARE_ALIGNED(16, uint16_t, temp[H * W]);                             \
    ...
    vpx_highbd_comp_avg_pred_c(temp, second_pred, W, H, src, src_stride);   \
                                                                            \
    return vpx_highbd_12_variance##W##x##H##_c(CONVERT_TO_BYTEPTR(temp), W, \
                                               dst, dst_stride, sse);       \
  }
