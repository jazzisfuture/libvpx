...
if (vpx_config("CONFIG_OBMC") eq "yes") {
  foreach (@block_sizes) {
    ...
    add_proto qw/unsigned int/, "vpx_obmc_sub_pixel_variance${w}x${h}", "const uint8_t *pre_ptr, int pre_stride, int xoffset, int  yoffset, const uint8_t *wsrc_ptr, int wsrc_stride, const int *mask, int mask_stride, unsigned int *sse";
