    ...
    if (cb != NULL) {
...
#if CONFIG_VP9_HIGHBITDEPTH
      if (use_highbitdepth)
        fb->fmt |= VPX_IMG_FMT_HIGHBITDEPTH;
#endif
      fb->width = width;
      fb->height = height;
      fb->coded_width = aligned_width + border * 2;
      fb->coded_height = aligned_height + border * 2;
      fb->border = border;
      ...
      if (fb->type == VPX_CODEC_FRAME_BUFFER_TYPE_SIZE) {
        ...
        if (fb->stride[1] != fb->stride[2])
          return -3;
        // Since buffers are required to be 32-byte aligned, we don't need to
        // include align_addr_extra_size in this check.
        if (fb->size < frame_size)
          return -3;

        ybf->y_stride = fb->stride[0];
        ybf->uv_stride = fb->stride[1];

        ybf->y_buffer = fb->plane[0] + border * fb->stride[0] + border;
        ybf->u_buffer =
            fb->plane[1] + uv_border_h * fb->stride[1] + uv_border_w;
        ybf->v_buffer =
            fb->plane[2] + uv_border_h * fb->stride[2] + uv_border_w;
      }
