...
int vpx_realloc_frame_buffer(YV12_BUFFER_CONFIG *ybf,
  ...
  if (ybf) {
	...
	if (!ybf->subsampling_x) {
      ...
      planes_cb(cb_priv, aligned_width + border * 2, aligned_height + border * 2, format, fb);
      ...
      if (border & 0x1f) // Not sure why it's checked in the middle.
        return -3;
      ...
      return 0;  // Everything is good, I hope.
    }
...
int vpx_alloc_frame_buffer(YV12_BUFFER_CONFIG *ybf,
  ...
  if (ybf) {
    ...
    return vpx_realloc_frame_buffer(ybf, width, height, ss_x, ss_y,
                                    ...
                                    use_highbitdepth,
#endif
                                    border, byte_alignment, NULL, NULL, NULL, NULL);
  }
  return -2;
}
#endif
