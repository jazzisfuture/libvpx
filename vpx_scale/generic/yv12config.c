...
int vpx_realloc_frame_buffer(YV12_BUFFER_CONFIG *ybf,
  ...
  if (ybf) {
    ...
    if (cb != NULL) {
      ...
      fb->type = VPX_CODEC_FRAME_BUFFER_SIZE;
      ...
      if (fb->type == VPX_CODEC_FRAME_BUFFER_SIZE) {
...
#endif
#endif
      } else {  // VPX_CODEC_FRAME_BUFFER_PLANES
        if ((fb->plane[0] == NULL || fb->plane[1] == NULL || fb->plane[2] == NULL) ||
