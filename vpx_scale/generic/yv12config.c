...
int vp9_realloc_frame_buffer(YV12_BUFFER_CONFIG *ybf,
  ...
  if (ybf) {
    ...
    ybf->y_buffer = (uint8_t *)(((size_t)(buf + (border * y_stride) + border) +
                    byte_alignment_minus_one) & ~byte_alignment_minus_one);
    ybf->u_buffer = (uint8_t *)(((size_t)(buf + yplane_size +
                    (uv_border_h * uv_stride) + uv_border_w) +
                    byte_alignment_minus_one) & ~byte_alignment_minus_one);
    ybf->v_buffer = (uint8_t *)(((size_t)(buf + yplane_size + uvplane_size +
                    (uv_border_h * uv_stride) + uv_border_w) +
                    byte_alignment_minus_one) & ~byte_alignment_minus_one);
    ...
    ybf->alpha_buffer = (uint8_t *)(((size_t)(buf + yplane_size +
        2 * uvplane_size + (alpha_border_h * alpha_stride) + alpha_border_w) +
        byte_alignment_minus_one) & ~byte_alignment_minus_one);
#endif
