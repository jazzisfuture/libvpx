...
int vp9_update_external_frame_buffer(YV12_BUFFER_CONFIG *ybf,
  ...
  if (ybf) {
    ...
    if (frame_size + align_addr_extra_size > fb->size) {
      // Allocation to hold larger frame, or first allocation.

      if (cb(user_priv, frame_size + align_addr_extra_size, fb) < 0) {
        return -1;
      }

      if (fb->data == NULL || fb->size < frame_size + align_addr_extra_size) {
        return -1;
      }
