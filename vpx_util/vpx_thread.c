...
static THREADFN thread_loop(void *ptr) {
  ...
  for (;;) {
    ...
    if (worker->status_ == VPX_WORKER_STATUS_WORKING) {
      // When worker->status_ is VPX_WORKER_STATUS_WORKING, the main thread doesn't change
      ...
      // worker->impl_->mutex_, worker->status_ must still be VPX_WORKER_STATUS_WORKING.
