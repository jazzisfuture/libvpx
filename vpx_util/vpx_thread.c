...
static THREADFN thread_loop(void *ptr) {
  ...
  for (;;) {
    while (worker->status_ == VPX_STATUS_OK) {  // wait in idling mode
      pthread_cond_wait(&worker->impl_->condition_, &worker->impl_->mutex_);
    }
    if (worker->status_ == VPX_STATUS_WORK) {
      // When worker->status_ is VPX_STATUS_WORK, the main thread doesn't change
      // worker->status_ and will wait until the worker changes worker->status_
      // to VPX_STATUS_OK. See change_state(). So the worker can safely call
      // execute() without holding worker->impl_->mutex_. When the worker
      // reacquires worker->impl_->mutex_, worker->status_ must still be
      // VPX_STATUS_WORK.
      pthread_mutex_unlock(&worker->impl_->mutex_);
      execute(worker);
      pthread_mutex_lock(&worker->impl_->mutex_);
      assert(worker->status_ == VPX_STATUS_WORK);
      worker->status_ = VPX_STATUS_OK;
      // signal to the main thread that we're done (for sync())
      pthread_cond_signal(&worker->impl_->condition_);
    } else {
      assert(worker->status_ == VPX_STATUS_NOT_OK);  // finish the worker
      break;
    }
  }
  pthread_mutex_unlock(&worker->impl_->mutex_);
...
static void change_state(VPxWorker *const worker, VPxWorkerStatus new_status) {
  ...
  pthread_mutex_lock(&worker->impl_->mutex_);
  if (worker->status_ >= VPX_STATUS_OK) {
    // wait for the worker to finish
    while (worker->status_ != VPX_STATUS_OK) {
      pthread_cond_wait(&worker->impl_->condition_, &worker->impl_->mutex_);
    }
    // assign new status and release the working thread if needed
    if (new_status != VPX_STATUS_OK) {
...
static void init(VPxWorker *const worker) {
  ...
  worker->status_ = VPX_STATUS_NOT_OK;
}

static int sync(VPxWorker *const worker) {
#if CONFIG_MULTITHREAD
  change_state(worker, VPX_STATUS_OK);
#endif
  assert(worker->status_ <= VPX_STATUS_OK);
...
static int reset(VPxWorker *const worker) {
  ...
  if (worker->status_ < VPX_STATUS_OK) {
    ...
    if (ok) worker->status_ = VPX_STATUS_OK;
    pthread_mutex_unlock(&worker->impl_->mutex_);
    if (!ok) {
      pthread_mutex_destroy(&worker->impl_->mutex_);
    ...
    worker->status_ = VPX_STATUS_OK;
#endif
  } else if (worker->status_ > VPX_STATUS_OK) {
    ok = sync(worker);
  }
  assert(!ok || (worker->status_ == VPX_STATUS_OK));
  return ok;
}
...
#if CONFIG_MULTITHREAD
  change_state(worker, VPX_STATUS_WORK);
#else
  execute(worker);
#endif
}
...
#if CONFIG_MULTITHREAD
  if (worker->impl_ != NULL) {
    change_state(worker, VPX_STATUS_NOT_OK);
...
#endif
  assert(worker->status_ == VPX_STATUS_NOT_OK);
}
