...
namespace {
std::vector<std::pair<uint8_t, bool> > abs_encode_build_vals(int iters) {
  std::vector<std::pair<uint8_t, bool> > ret;
...
bool check_rabs(const std::vector<std::pair<uint8_t, bool> > &pv_vec,
                uint8_t *buf) {
  AnsCoder a;
  ans_write_init(&a, buf);

  std::clock_t start = std::clock();
  for (std::vector<std::pair<uint8_t, bool> >::const_reverse_iterator it =
           pv_vec.rbegin();
       it != pv_vec.rend(); ++it) {
  ...
  bool okay = true;
  AnsDecoder d;
  ans_read_init(&d, buf, offset);
  start = std::clock();
  for (std::vector<std::pair<uint8_t, bool> >::const_iterator it =
           pv_vec.begin();
       it != pv_vec.end(); ++it) {
...
bool check_rabs_asc(const std::vector<std::pair<uint8_t, bool> > &pv_vec,
                    uint8_t *buf) {
  AnsCoder a;
  ans_write_init(&a, buf);
...
bool check_uabs(const std::vector<std::pair<uint8_t, bool> > &pv_vec,
                uint8_t *buf) {
  AnsCoder a;
  ans_write_init(&a, buf);
...
bool check_vpxbool(const std::vector<std::pair<uint8_t, bool> > &pv_vec,
                   uint8_t *buf) {
  vpx_writer w;
  vpx_reader r;
...
bool check_vpxtree(const std::vector<int>& sym_vec, const rans_sym *sym_tab,
  ...
  for (std::vector<int>::const_iterator it = sym_vec.begin();
       it != sym_vec.end(); ++it) {
    if (vpx_read_tree(&r, tree, probs) != *it)
      return false;
  }
  std::clock_t dec_time = std::clock() - start;
...
class Vp10AbsTest : public ::testing::Test {
 protected:
  static void SetUpTestCase() {
    pv_vec_ = abs_encode_build_vals(kNumBools);
  }
  virtual void SetUp() {
    buf_ = new uint8_t[kNumBools / 8];
  }
  virtual void TearDown() {
    delete[] buf_;
  }
  static const int kNumBools = 100000000;
  static std::vector<std::pair<uint8_t, bool> > pv_vec_;
  uint8_t *buf_;
};
std::vector<std::pair<uint8_t, bool> > Vp10AbsTest::pv_vec_;
...
TEST_F(Vp10AbsTest, Vpxbool) {
  EXPECT_TRUE(check_vpxbool(pv_vec_, buf_));
}
TEST_F(Vp10AbsTest, Rabs) {
  EXPECT_TRUE(check_rabs(pv_vec_, buf_));
}
TEST_F(Vp10AbsTest, RabsAsc) {
  EXPECT_TRUE(check_rabs_asc(pv_vec_, buf_));
}
TEST_F(Vp10AbsTest, Uabs) {
  EXPECT_TRUE(check_uabs(pv_vec_, buf_));
}
