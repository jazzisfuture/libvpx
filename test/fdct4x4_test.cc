...
TEST(VP9Fadst4x4Test, SignBiasCheck) {
  ACMRandom rnd(ACMRandom::DeterministicSeed());
  int16_t test_input_block[16];
  int16_t test_output_block[16];
  const int pitch = 4;
  ...
  for (int i = 0; i < count_test_block; ++i) {
    ...
    // test 4x4 2D ADST transform
    vp9_short_fht4x4_c(test_input_block, test_output_block, pitch, 3);
  ...
  for (int j = 0; j < 16; ++j) {
    const bool bias_acceptable = (abs(count_sign_block[j][0] -
                                      count_sign_block[j][1]) < 10000);
    EXPECT_TRUE(bias_acceptable)
        << "Error: 4x4 FADST has a sign bias > 1%"
        << " for input range [-255, 255] at index " << j;
  }
  ...
  for (int j = 0; j < 16; ++j) {
    const bool bias_acceptable = (abs(count_sign_block[j][0] -
                                      count_sign_block[j][1]) < 100000);
    EXPECT_TRUE(bias_acceptable)
        << "Error: 4x4 FADST has a sign bias > 10%"
        << " for input range [-15, 15] at index " << j;
  }
};

TEST(VP9Fadst4x4Test, RoundTripErrorCheck) {
  ACMRandom rnd(ACMRandom::DeterministicSeed());
  int max_error = 0;
  double total_error = 0;
  const int count_test_block = 1000000;
  for (int i = 0; i < count_test_block; ++i) {
    ...
    // test 4x4 2D ADST
    const int pitch = 4;
    vp9_short_fht4x4_c(test_input_block, test_temp_block, pitch, 3);
    ...
    // Because the bitstream is not frozen yet, use the idct in the codebase.
    vp9_short_iht4x4_add_c(test_temp_block, dst, 4, 3);
  ...
  EXPECT_GE(1, max_error)
      << "Error: FADST/IADST has an individual roundtrip error > 1";

  EXPECT_GE(count_test_block, total_error)
      << "Error: FADST/IADST has average roundtrip error > 1 per block";
};
}  // namespace
