...
TEST(VP10ConvolveTest, vp10_convolve8) {
  ...
  InterpFilterParams filter_params = vp10_get_interp_filter_params(interp_filter);
  ...
  const int16_t* x_filter = vp10_get_interp_filter_kernel(filter_params, subpel_x_q4);
  const int16_t* y_filter = vp10_get_interp_filter_kernel(filter_params, subpel_y_q4);
...
TEST(VP10ConvolveTest, vp10_convolve_avg) {
  ...
  avg = 0;
  vp10_convolve(src0 + offset, src_stride, dst0, dst_stride, w, h, filter_params,
                subpel_x_q4, x_step_q4, subpel_y_q4, y_step_q4, avg);
  avg = 0;
  vp10_convolve(src1 + offset, src_stride, dst1, dst_stride, w, h, filter_params,
                subpel_x_q4, x_step_q4, subpel_y_q4, y_step_q4, avg);
...
TEST(VP10ConvolveTest, vp10_highbd_convolve_avg) {
  ...
  vp10_highbd_convolve(CONVERT_TO_BYTEPTR(src0 + offset), src_stride,
                       CONVERT_TO_BYTEPTR(dst0), dst_stride, w, h, filter_params,
                       subpel_x_q4, x_step_q4, subpel_y_q4, y_step_q4, avg, bd);
  avg = 0;
  vp10_highbd_convolve(CONVERT_TO_BYTEPTR(src1 + offset), src_stride,
                       CONVERT_TO_BYTEPTR(dst1), dst_stride, w, h, filter_params,
                       subpel_x_q4, x_step_q4, subpel_y_q4, y_step_q4, avg, bd);
