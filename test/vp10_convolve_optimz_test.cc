...
typedef tuple<BlockDimension, INTERP_FILTER, int, int> ConvolveParams;
...
class VP10ConvolveOptimzTest : public ::testing::TestWithParam<ConvolveParams> {
 public:
...
void VP10ConvolveOptimzTest::RunBitExactCheck() {
  int r, c;
  ACMRandom rnd(ACMRandom::DeterministicSeed());

  memset(src_, 0, maxBlockSize);
  memset(src_ref_, 0, maxBlockSize);
  memset(dst_, 0, maxBlockSize);
  memset(dst_ref_, 0, maxBlockSize);
  ...
  convolve_horiz_c(src_ref_, stride, dst_ref_, stride, width_, height_,
                   filter_params, subpel_, x_step_q4, avg_);

  convolve_horiz_ssse3(src_, stride, dst_, stride, width_, height_,
                       filter_params, subpel_, x_step_q4, avg_);
  ...
  // and test again.
  int intermediate_height = (((height_ - 1) * 16 + subpel_) >> SUBPEL_BITS)
                                       + filter_params.taps;

  convolve_horiz_c(src_ref_, stride, dst_ref_, stride, width_,
                   intermediate_height, filter_params, subpel_, x_step_q4,
                   avg_);

  convolve_horiz_ssse3(src_, stride, dst_, stride, width_,
                       intermediate_height, filter_params, subpel_, x_step_q4,
                       avg_);
...
TEST_P(VP10ConvolveOptimzTest, BitExactCheck) {
  RunBitExactCheck();
}
