...
double GetBitrateInKbps(size_t bit_size, int num_frames_, int frame_rate_num_,
                        int frame_rate_den_) {
  return static_cast<double>(bit_size) / num_frames_ * frame_rate_num_ /
         frame_rate_den_ / 1000.0;
}
...
class SimpleEncodeTest : public ::testing::Test {
 protected:
  ...
  const std::string infile_path_ = "bus_352x288_420_f20_b8.yuv";
};

TEST_F(SimpleEncodeTest, ComputeFirstPassStats) {
  SimpleEncode simple_encode(w_, h_, frame_rate_num_, frame_rate_den_,
...
TEST_F(SimpleEncodeTest, EncodeConsistencyTest2) {
  const int num_rows_4x4 = GetNumUnit4x4(w_);
  const int num_cols_4x4 = GetNumUnit4x4(h_);
  ...
  simple_encode.EndEncode();
  // The second encode.
  SimpleEncode simple_encode_2(w_, h_, frame_rate_num_, frame_rate_den_,
                               target_bitrate_, num_frames_,
                               infile_path_.c_str());
...
TEST_F(SimpleEncodeTest, GetFramePixelCount) {
  ...
  EXPECT_EQ(simple_encode.GetFramePixelCount(),
            static_cast<uint64_t>(w_ * h_ * 3 / 2));
}
