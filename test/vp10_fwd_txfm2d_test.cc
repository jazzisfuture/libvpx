...
class VP10FwdTxfm2d : public ::testing::TestWithParam<VP10FwdTxfm2dParam> {
 public:
  virtual void SetUp() {
    ...
    count_ = 500;
    TXFM_2D_FLIP_CFG fwd_txfm_flip_cfg = vp10_get_fwd_txfm_cfg(tx_type_, tx_size_);
    ...
    ref_input_ = new double[txfm2d_size_];
    ref_output_ = new double[txfm2d_size_];
  }
  ...
  void RunFwdAccuracyCheck() {
    ...
    for (int ci = 0; ci < count_; ci++) {
      ...
      reference_hybrid_2d(ref_input_, ref_output_, txfm1d_size_, type0_, type1_);

      for (int ni = 0; ni < txfm2d_size_; ++ni) {
        ref_output_[ni] = round(ref_output_[ni] * amplify_factor_);
        EXPECT_GE(max_error_, fabs(output_[ni] - ref_output_[ni]) / amplify_factor_);
      }
