...
unsigned int vp9_Average_8x8_c(const uint8_t* s, int p);
unsigned int vp9_Average_8x8_sse2(const uint8_t* s, int p);
...
#if CONFIG_VP9_ENCODER
TEST_P(AverageTest, MaxRef) {
  FillConstant(0);
  CheckAverages();
}

TEST_P(AverageTest, MaxSrc) {
  FillConstant(255);
  CheckAverages();
}
