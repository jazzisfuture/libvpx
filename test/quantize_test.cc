...
static const int kNumBlocks = 25;
static const int kNumBlockEntries = 16;
...
class QuantizeTestBase {
 ...
 protected:
  void SetupCompressor() {
    ...
    macroblockd_dst_ = (MACROBLOCKD *)vpx_memalign(32,
                                                   sizeof(*macroblockd_dst_));
    vpx_memcpy(macroblockd_dst_,
               &vp8_comp_->mb.e_mbd,
               sizeof(*macroblockd_dst_));
  ...
  void CheckOutput() {
    EXPECT_EQ(0, memcmp(vp8_comp_->mb.e_mbd.qcoeff,
                        macroblockd_dst_->qcoeff,
                        sizeof(*macroblockd_dst_->qcoeff)
                        * kNumBlocks * kNumBlockEntries))
        << "qcoeff mismatch";
    EXPECT_EQ(0, memcmp(vp8_comp_->mb.e_mbd.dqcoeff,
                        macroblockd_dst_->dqcoeff,
                        sizeof(*macroblockd_dst_->dqcoeff)
                        * kNumBlocks * kNumBlockEntries))
        << "dqcoeff mismatch";
    EXPECT_EQ(0, memcmp(vp8_comp_->mb.e_mbd.eobs,
                        macroblockd_dst_->eobs,
...
class QuantizeTest
 ...
 protected:
  void SetFunctionPointers() {
    SetupCompressor();
    ASMQuant_ = GET_PARAM(0);
    CQuant_ = GET_PARAM(1);
  }

  void RunComparison() {
    for (int i = 0; i < kNumBlocks; ++i) {
      ASM_REGISTER_STATE_CHECK(CQuant_(&vp8_comp_->mb.block[i],
                                       &vp8_comp_->mb.e_mbd.block[i]));
      ASM_REGISTER_STATE_CHECK(ASMQuant_(&vp8_comp_->mb.block[i],
                                         &macroblockd_dst_->block[i]));
    }
  }
 ...
 protected:
  ...
  void RunComparison() {
    ...
    for (int i = 0; i < kNumBlocks - 1; i += 2) {
      ASM_REGISTER_STATE_CHECK(CQuantPair_(&vp8_comp_->mb.block[i],
                                           &vp8_comp_->mb.block[i + 1],
                                           &vp8_comp_->mb.e_mbd.block[i],
                                           &vp8_comp_->mb.e_mbd.block[i + 1]));
      ASM_REGISTER_STATE_CHECK(ASMQuantPair_(&vp8_comp_->mb.block[i],
                                             &vp8_comp_->mb.block[i + 1],
                                             &macroblockd_dst_->block[i],
                                             &macroblockd_dst_->block[i + 1]));
    }
  }
...
TEST_P(QuantizeTestPair, SpeedTest) {
  SetFunctionPointers();
  FillCoeffRandom();
  ...
  for (int j = 0; j < 1000000; j++)
    // Skip the last, unpaired, block.
    for (int i = 0; i < kNumBlocks - 1; i += 2)
      ASMQuantPair_(&vp8_comp_->mb.block[i],
                    &vp8_comp_->mb.block[i + 1],
                    &macroblockd_dst_->block[i],
                    &macroblockd_dst_->block[i + 1]);
}
...
INSTANTIATE_TEST_CASE_P(SSSE3, QuantizeTest,
                        ::testing::Values(
                          make_tuple(vp8_fast_quantize_b_ssse3,
                                     vp8_fast_quantize_b_c)));
#endif  // HAVE_SSSE3

#if HAVE_SSE4_1
INSTANTIATE_TEST_CASE_P(SSE4_1, QuantizeTest,
                        ::testing::Values(
                          make_tuple(vp8_regular_quantize_b_sse4_1,
                                     vp8_regular_quantize_b_c)));
#endif  // HAVE_SSE4_1
...
INSTANTIATE_TEST_CASE_P(MEDIA, QuantizeTest,
                        ::testing::Values(
                          make_tuple(vp8_fast_quantize_b_armv6,
                                     vp8_fast_quantize_b_c)));
#endif  // HAVE_MEDIA
...
INSTANTIATE_TEST_CASE_P(NEON, QuantizeTest,
                        ::testing::Values(
                          make_tuple(vp8_fast_quantize_b_neon,
                                     vp8_fast_quantize_b_c)));

INSTANTIATE_TEST_CASE_P(NEON, QuantizeTestPair,
                        ::testing::Values(
                          make_tuple(vp8_fast_quantize_b_pair_neon,
