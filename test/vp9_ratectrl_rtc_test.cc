...
class RcInterfaceSvcTest : public ::libvpx_test::EncoderTest,
 ...
 private:
  ...
  void SetEncoderConfigSvc3Spatial(int number_temporal_layers) {
    ...
    cfg_.rc_target_bitrate = kSpatialLayerBitrate[cfg_.ss_number_layers - 1];
    cfg_.rc_overshoot_pct = 50;
    cfg_.rc_undershoot_pct = 50;

    for (unsigned int sl = 0; sl < cfg_.ss_number_layers; sl++) {
      int spatial_bitrate = kSpatialLayerBitrate[sl];
      for (int tl = 0; tl < number_temporal_layers; tl++) {
        int layer = sl * cfg_.ts_number_layers + tl;
        if (number_temporal_layers == 3)
  ...
  void SetRCConfigSvc3Spatial(int number_temporal_layers) {
    ...
    rc_cfg_.target_bandwidth =
        kSpatialLayerBitrate[cfg_.ss_number_layers - 1];
