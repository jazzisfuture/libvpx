...
class VP10HighbdTrans8x8HT : public ::testing::TestWithParam<HighbdHt8x8Param> {
 public:
  ...
  virtual void SetUp() {
    ...
    input_ = reinterpret_cast<int16_t *>(vpx_memalign(
        16, sizeof(int16_t) * num_coeffs_));
    output_ = reinterpret_cast<int32_t *>(vpx_memalign(
        16, sizeof(int32_t) * num_coeffs_));
    output_ref_ = reinterpret_cast<int32_t *>(vpx_memalign(
        16, sizeof(int32_t) * num_coeffs_));
  }
