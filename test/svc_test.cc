...
TEST_F(SvcTest, TwoPassEncode) {
  ...
  ASSERT_TRUE(stats_data != NULL);
  stats_buf.buf = realloc(stats_buf.buf, stats_buf.sz + stats_size);
  ASSERT_TRUE(stats_buf.buf != NULL);
  memcpy((char*)stats_buf.buf + stats_buf.sz, stats_data, stats_size);
  stats_buf.sz += stats_size;
