...
class SvcTest : public ::testing::Test {
 protected:
  ...
  void Pass1EncodeNFrames(uint8_t n, int layers, std::string *stats_buf) {
    vpx_codec_err_t res;
    size_t stats_size = 0;
  ...
  void StoreFrames(uint8_t max_frame_received, struct vpx_fixed_buf *outputs,
  ...
  void Pass2EncodeNFrames(std::string *stats_buf, uint8_t n, int layers,
                          struct vpx_fixed_buf *outputs) {
    vpx_codec_err_t res;
  ...
  void DecodeNFrames(struct vpx_fixed_buf *inputs, uint8_t n) {
    uint8_t decoded_frames = 0;
  ...
  void FreeBitstreamBuffers(struct vpx_fixed_buf *inputs, uint8_t n) {
    ASSERT_TRUE(inputs != NULL);
    ASSERT_GT(n, 0U);
