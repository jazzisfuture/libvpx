...
TEST_F(SvcTest, DISABLED_FirstFrameHasLayers) {
  ...
  vpx_codec_err_t res_dec =
      decoder_->DecodeFrame(static_cast<const uint8_t *>(svc_get_buffer(&svc_)),
...
TEST_F(SvcTest, GetLayerResolution) {
  ...
  res = svc_get_layer_resolution(&svc_, svc_.spatial_layers,  //
                                 &layer_width, &layer_height);
  EXPECT_EQ(VPX_CODEC_INVALID_PARAM, res);

  res = svc_get_layer_resolution(&svc_, 0, &layer_width, &layer_height);
  EXPECT_EQ(VPX_CODEC_OK, res);
  EXPECT_EQ(static_cast<uint32_t>(kWidth * 4 / 16), layer_width);
  EXPECT_EQ(static_cast<uint32_t>(kHeight * 4 / 16), layer_height);

  res = svc_get_layer_resolution(&svc_, 1, &layer_width, &layer_height);
  EXPECT_EQ(VPX_CODEC_OK, res);
