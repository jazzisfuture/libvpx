...
class SADTest : public PARAMS(int, int, sad_m_by_n_fn_t) {
  ...
  virtual void SetUp() {
    ...
    rnd_ = new ACMRandom(ACMRandom::DeterministicSeed());
  }
  ...
  unsigned int ReferenceSAD(int max_sad) {
    unsigned int sad = 0;
...
/* Failing. Not sure if it should be passing.
TEST_P(SADTest, UnalignedSrc) {
  int tmp_stride = source_stride_;
  source_stride_ -= 1;
  FillRandom(source_data_, source_stride_);
  FillRandom(reference_data_, reference_stride_);
