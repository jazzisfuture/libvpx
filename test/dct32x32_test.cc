...
void reference2_32x32_idct_2d(double *input, double *output) {
  double x;
  for (int l = 0; l < 32; ++l) {
    for (int k = 0; k < 32; ++k) {
      double s = 0;
      for (int i = 0; i < 32; ++i) {
        for (int j = 0; j < 32; ++j) {
          x=cos(PI*j*(l+0.5)/32.0)*cos(PI*i*(k+0.5)/32.0)*input[i*32+j]/1024;
          if (i != 0)
            x *= sqrt(2.0);
          if (j != 0)
            x *= sqrt(2.0);
...
#else // USW_DWT
  // FIXME dwt specific test goes here
#endif // USW_DWT
TEST(VP9Fdct32x32Test, AccuracyCheck) {
  ...
  for (int i = 0; i < count_test_block; ++i) {
    ...
    for (m=0;m<32;m++){
      printf("{");
      for (n=0;n<32;n++){
        printf("%c%d, ", test_input_block[m*32+n]<0?'-':' ',
               abs(test_input_block[m*32+n]));
      }
      printf("},\n");
    }
    printf("\nout:\n");
    for (m=0;m<32;m++){
      for (n=0;n<32;n++){
        printf("%c%3d%c", test_output_block[m*32+n]<0?'-':' ',
               abs(test_output_block[m*32+n]), n==31?'\n':' ');
      }
    }
#endif
    for (int j = 0; j < 1024; ++j) {
  ...
  for (int i = 0; i < count_test_block; ++i) {
    ...
    for (int j = 0; j < 1024; ++j) {
      ...
      EXPECT_GE(4*DCT_MAX_VALUE, abs(output_extreme_block[j]))
          << "Error: 32x32 FDCT extreme has coefficient larger than 4*DCT_MAX_VALUE";
    }
  }
}
}  // namespace
