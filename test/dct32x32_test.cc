...
typedef std::tuple<InvTxfmFunc, InvTxfmFunc, int, vpx_bit_depth_t, int>
    InvTrans32x32Param;
...
class InvTrans32x32Test : public ::testing::TestWithParam<InvTrans32x32Param> {
 ...
 protected:
  ...
  void RunInvTrans32x32SpeedTest() {
    ...
    for (int i = 0; i < count_test_block; ++i) {
      vpx_idct32x32_1024_add_c(coeff, ref, pitch_);
    }
    ...
    printf(
        "c_time = %I64d \t simd_time = %I64d \t Gain = %4.2f \n", c_sum_time,
        simd_sum_time,
        (static_cast<float>(c_sum_time) / static_cast<float>(simd_sum_time)));
  }

  void CompareInvReference() {
    ACMRandom rnd(ACMRandom::DeterministicSeed());
    const int count_test_block = 10000;
    const int eob = 31;
    const int16_t *scan = vp9_default_scan_orders[TX_32X32].scan;
    ...
    for (int i = 0; i < count_test_block; ++i) {
      ...
      for (int j = 0; j < kNumCoeffs; ++j) {
        ...
        const uint32_t error = diff * diff;
        EXPECT_EQ(dst[j], ref[j]) << "Error: 32X32 IDCT Comparison has error "
                                  << error << " at index " << j;
      }
    }
  }
...
TEST_P(InvTrans32x32Test, CompareReference) { CompareInvReference(); }
...
INSTANTIATE_TEST_SUITE_P(
    C, InvTrans32x32Test,
    ::testing::Values((make_tuple(&vpx_idct32x32_1024_add_c,
                                  &vpx_idct32x32_1024_add_c, 0, VPX_BITS_8,
                                  32)),
                      make_tuple(&vpx_idct32x32_135_add_c,
                                 &vpx_idct32x32_135_add_c, 0, VPX_BITS_8, 16)));
#endif  // CONFIG_VP9_HIGHBITDEPTH
