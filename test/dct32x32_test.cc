...
void reference_32x32_dct_2d(const int16_t input[NUMCOEFFS],
                            double output[NUMCOEFFS]) {
...
TEST_P(Trans32x32Test, AccuracyCheck) {
  ...
  DECLARE_ALIGNED_ARRAY(16, int16_t, test_input_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, test_temp_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, uint8_t, dst, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, uint8_t, src, NUMCOEFFS);
...
TEST_P(Trans32x32Test, CoeffCheck) {
  ...
  DECLARE_ALIGNED_ARRAY(16, int16_t, input_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, output_ref_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, output_block, NUMCOEFFS);
...
TEST_P(Trans32x32Test, MemCheck) {
  ...
  DECLARE_ALIGNED_ARRAY(16, int16_t, input_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, input_extreme_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, output_ref_block, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, output_block, NUMCOEFFS);
...
TEST_P(Trans32x32Test, InverseAccuracy) {
  ACMRandom rnd(ACMRandom::DeterministicSeed());
  const int count_test_block = 1000;
  DECLARE_ALIGNED_ARRAY(16, int16_t, in, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, int16_t, coeff, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, uint8_t, dst, NUMCOEFFS);
  DECLARE_ALIGNED_ARRAY(16, uint8_t, src, NUMCOEFFS);

  for (int i = 0; i < count_test_block; ++i) {
    double out_r[NUMCOEFFS];
...
INSTANTIATE_TEST_CASE_P(
    C, Trans32x32Test, ::testing::Values(
...
#if HAVE_SSE2
INSTANTIATE_TEST_CASE_P(
    SSE2, Trans32x32Test, ::testing::Values(
