...
public:
  ...
  void setImage(const vpx_image_t *img, bool end_of_input = false) {
    pthread_mutex_lock(&mutex_);
    /**
     * TODO(crbug.com/webm/1836): implement a buffer copy for vpx_image_t.
...
public:
  VP9M2MEncoder(unsigned int threads)
      : EncoderTest(static_cast<const libvpx_test::CodecFactory *>( \
              &libvpx_test::kVP9)), initialized_(false), min_psnr_(kMaxPsnr),
...
public:
  ...
  void RunLoopEncode() {
    RunLoop(&in_mem_source_);
  }

  void waitForEncoder() {
    in_mem_source_.waitForEncoder();
  }
...
//  The decoder and encoder run on separate threads, but the frames are synchronized
//  instead of using a buffer queue.
//  TODO: introduce some noise to the decoded frames.
