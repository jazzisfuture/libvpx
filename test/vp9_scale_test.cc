...
class ScaleTest : public VpxScaleBase,
 ...
 protected:
  ...
  void PrintDiffComponet(const uint8_t *const ref, const uint8_t *const opt,
  ...
  void PrintDiff() const {
    ...
    if (memcmp(dst_img_.buffer_alloc, ref_img_.buffer_alloc,
               ref_img_.frame_size)) {
      PrintDiffComponet(ref_img_.y_buffer, dst_img_.y_buffer, ref_img_.y_stride,
                        ref_img_.y_width, ref_img_.y_height, 0);
      PrintDiffComponet(ref_img_.u_buffer, dst_img_.u_buffer,
                        ref_img_.uv_stride, ref_img_.uv_width,
                        ref_img_.uv_height, 1);
      PrintDiffComponet(ref_img_.v_buffer, dst_img_.v_buffer,
                        ref_img_.uv_stride, ref_img_.uv_width,
