...
typedef void (*TXFM_FUNC)(const int32_t *input, int32_t *output, const int8_t *cos_bit, const int8_t *stage_range);

typedef void (*TXFM_FUNC_SSE2)(const M128I *input, M128I *output, const int8_t *cos_bit, const int8_t *stage_range);
...
TEST(Txfm1dSse2Test, Accuracy) {
  ...
  TXFM_FUNC_SSE2 txfm_func_sse2_list[] = {
    vp10_fdct4_new_sse2,
    vp10_fdct8_new_sse2,
    vp10_fdct16_new_sse2,
    vp10_fdct32_new_sse2,
    vp10_fadst4_new_sse2,
    vp10_fadst8_new_sse2,
    vp10_fadst16_new_sse2,
    vp10_fadst32_new_sse2,
  };

  int txfm_num = 8;
  int txfm_size_list[] = { 4, 8, 16, 32, 4, 8, 16, 32,};
  ...
  for(int i = 0; i < txfm_num; i++) {
    ...
    for(int row = 0; row < txfm_size; row++) {
      txfm_func(input + txfm_size*row, output + txfm_size*row, cos_bit, stage_range);
    }
    txfm_func_sse2((M128I*)input_sse2, (M128I*)output_sse2, cos_bit, stage_range);
