...
class SixtapPredictTest : public PARAMS(int, int, sixtap_predict_fn_t) {
 protected:
  ...
  static const int kExtraPixels = 5;
  static const int kAllocWidth = 16;

  virtual void SetUp() {
    sixtap_predict_ = GET_PARAM(2);
    dst_stride_ = GET_PARAM(0);
    src_stride_ = dst_stride_ + 5;
    height_ = GET_PARAM(1);
  ...
  DECLARE_ALIGNED(16, uint8_t, src_[(kAllocWidth + kExtraPixels)
                                    * (kAllocWidth + kExtraPixels) + 1]);
  DECLARE_ALIGNED(16, uint8_t, dst_[kAllocWidth * kAllocWidth]);
  DECLARE_ALIGNED(16, uint8_t, dst_c_[kAllocWidth * kAllocWidth]);
};
...
TEST_P(SixtapPredictTest, TestWithPresetData) {
  ...
  // Expected result for sixtap_predict16x16
  static const uint8_t test_output_16x16[256] = {
  ...
  // Expected result for sixtap_predict8x8
  static const uint8_t test_output_8x8[64] = {
    134, 103, 211, 99, 81, 162, 177, 111, 130, 213, 68, 111, 100, 31, 143,
    68, 225, 171, 78, 48, 47, 77, 167, 191, 99, 57, 138, 49, 230, 223, 159,
    84, 172, 133, 109, 68, 155, 35, 78, 214, 128, 168, 140, 192, 152, 99,
    170, 114, 205, 128, 98, 183, 101, 162, 89, 252, 135, 0, 168, 178, 50,
    46, 140, 194
  };

  // Expected result for sixtap_predict8x4
  static const uint8_t test_output_8x4[32] = {
    134, 103, 211, 99, 81, 162, 177, 111, 130, 213, 68, 111, 100, 31, 143,
    68, 225, 171, 78, 48, 47, 77, 167, 191, 99, 57, 138, 49, 230, 223, 159, 84
  };

  // Expected result for sixtap_predict4x4
  static const uint8_t test_output_4x4[16] = {
    184, 222, 135, 44, 82, 230, 96, 73, 159, 148, 150, 106, 127, 14, 79, 110
  };

  uint8_t *src = const_cast<uint8_t*>(test_data);
  uint8_t *expected_dst = NULL;

  sixtap_predict_(&src[src_stride_ * 2 + 2 +1], src_stride_,
                            2, 2, dst_c_, dst_stride_);

  if (dst_stride_ == 16 && height_ == 16)
    expected_dst = const_cast<uint8_t*>(test_output_16x16);
  else if (dst_stride_ == 8 && height_ == 8)
    expected_dst = const_cast<uint8_t*>(test_output_8x8);
  else if (dst_stride_ == 8 && height_ == 4)
    expected_dst = const_cast<uint8_t*>(test_output_8x4);
  else if (dst_stride_ == 4 && height_ == 4)
    expected_dst = const_cast<uint8_t*>(test_output_4x4);

  for (int i = 0; i < dst_stride_ * height_; ++i)
    ASSERT_EQ(dst_c_[i], expected_dst[i]) << "i==" << i;
}
...
TEST_P(SixtapPredictTest, TestWithRandomData) {
  ...
  for (int xoffset = 0; xoffset < 8; ++xoffset) {
    for (int yoffset = 0; yoffset < 8; ++yoffset) {
      ...
      if (dst_stride_ == 16 && height_ == 16)
        vp8_sixtap_predict16x16_c(&src_[src_stride_ * 2 + 2 + 1], src_stride_,
                                  xoffset, yoffset, dst_c_, dst_stride_);
      else if (dst_stride_ == 8 && height_ == 8)
        vp8_sixtap_predict8x8_c(&src_[src_stride_ * 2 + 2 + 1], src_stride_,
                                xoffset, yoffset, dst_c_, dst_stride_);
      else if (dst_stride_ == 8 && height_ == 4)
        vp8_sixtap_predict8x4_c(&src_[src_stride_ * 2 + 2 + 1], src_stride_,
                                xoffset, yoffset, dst_c_, dst_stride_);
      else if (dst_stride_ == 4 && height_ == 4)
        vp8_sixtap_predict4x4_c(&src_[src_stride_ * 2 + 2 + 1], src_stride_,
                                xoffset, yoffset, dst_c_, dst_stride_);

      // Run test.
      sixtap_predict_(&src_[src_stride_ * 2 + 2 + 1], src_stride_,
                      xoffset, yoffset, dst_, dst_stride_);

      for (int i = 0; i < dst_stride_ * height_; ++i)
        ASSERT_EQ(dst_c_[i], dst_[i]) << "i==" << i;
    }
  }
}
...
INSTANTIATE_TEST_CASE_P(C, SixtapPredictTest,
                        ::testing::Values(
                        std::tr1::make_tuple(16, 16, vp8_sixtap_predict16x16_c),
                        std::tr1::make_tuple(8, 8, vp8_sixtap_predict8x8_c),
                        std::tr1::make_tuple(8, 4, vp8_sixtap_predict8x4_c),
                        std::tr1::make_tuple(4, 4, vp8_sixtap_predict4x4_c)));
#if HAVE_MMX
INSTANTIATE_TEST_CASE_P(MMX, SixtapPredictTest,
                        ::testing::Values(
                        std::tr1::make_tuple(16, 16,
                                             vp8_sixtap_predict16x16_mmx),
                        std::tr1::make_tuple(8, 8, vp8_sixtap_predict8x8_mmx),
                        std::tr1::make_tuple(8, 4, vp8_sixtap_predict8x4_mmx),
                        std::tr1::make_tuple(4, 4, vp8_sixtap_predict4x4_mmx)));
#endif
#if HAVE_SSE2
INSTANTIATE_TEST_CASE_P(SSE2, SixtapPredictTest,
                        ...
                        std::tr1::make_tuple(8, 8, vp8_sixtap_predict8x8_sse2),
                        std::tr1::make_tuple(8, 4, vp8_sixtap_predict8x4_sse2)
                        ));
#endif
#if HAVE_SSSE3
INSTANTIATE_TEST_CASE_P(SSSE3, SixtapPredictTest,
                        ...
                        std::tr1::make_tuple(8, 8, vp8_sixtap_predict8x8_ssse3),
                        std::tr1::make_tuple(8, 4, vp8_sixtap_predict8x4_ssse3),
                        std::tr1::make_tuple(4, 4, vp8_sixtap_predict4x4_ssse3)
                        ));
#endif
}  // namespace
