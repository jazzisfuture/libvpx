...
TEST_P(SixtapPredictTest, TestWithPresetData) {
  ...
  for (int i = 0; i < height_; ++i)
    for (int j = 0; j < width_; ++j)
      ASSERT_EQ(expected_dst[i * kDstStride + j], dst_aligned_[i * kDstStride + j])
          << "i==" << (i * width_ + j);
}
...
TEST_P(SixtapPredictTest, TestWithRandomData) {
  ...
  for (int xoffset = 0; xoffset < 8; ++xoffset) {
    for (int yoffset = 0; yoffset < 8; ++yoffset) {
      ...
      vp8_sixtap_predict16x16_c(&src_aligned_[kSrcStride * 2 + 2 + 1], kSrcStride,
      ...
      for (int i = 0; i < height_; ++i)
        for (int j = 0; j < width_; ++j)
          ASSERT_EQ(dst_c_aligned_[i * kDstStride + j], dst_aligned_[i * kDstStride + j])
              << "i==" << (i * width_ + j);
    }
  }
}
