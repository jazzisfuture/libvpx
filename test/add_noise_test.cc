...
static double stddev4(char a, char b, char c, char d) {
  double n = (a + b + c + d) / 4.0;
...
// TODO(jimbankoski): The following 2 functions are duplicated in each version
// of libvpx. For now the vp9 one has been copied into the test as is. We
// should normalize these in vpx_dsp and not have 3 copies of these unless
// there is different noise we add for each codec.

// Test routine for the postproc function that adds noise.
...
TEST_P(AddNoiseTest, CheckNoiseAdded) {
  ...
  for (i = 0; i < image_size - 4 * width - 4; ++i) {
    double hd = stddev4(s[i] - 99, s[i + 1] - 99, s[i + 2] - 99, s[i + 3] - 99);
    double vd = stddev4(s[i] - 99, s[i + width] - 99, s[i + 2 * width] - 99,
                        s[i + 3 * width] - 99);
