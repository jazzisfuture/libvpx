...
vpxdec_verify_environment() {
  ...
  if [ -z "$(aom_tool_path vpxdec)" ]; then
...
# to vpxdec.
vpxdec_pipe() {
  local readonly decoder="$(aom_tool_path vpxdec)"
...
vpxdec_av1_webm() {
  if [ "$(vpxdec_can_decode_av1)" = "yes" ] && \
     [ "$(webm_io_available)" = "yes" ]; then
    vpxdec "${AV1_WEBM_FILE}" --summary --noblit
  fi
}

vpxdec_av1_webm_frame_parallel() {
  if [ "$(vpxdec_can_decode_av1)" = "yes" ] && \
     [ "$(webm_io_available)" = "yes" ]; then
    for threads in 2 3 4 5 6 7 8; do
      vpxdec "${AV1_FPM_WEBM_FILE}" --summary --noblit --threads=$threads \
        --frame-parallel
    done
  fi
}

vpxdec_av1_webm_less_than_50_frames() {
