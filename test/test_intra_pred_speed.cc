...
typedef IntraPredTest<uint8_t> VP9IntraPredTest;

void TestIntraPred(const char name[], VP9IntraPredTest *const intraPredTest,
                   VpxPredFunc const *pred_funcs,
                   const char *const signatures[], int block_size) {
  uint8_t *const above = intraPredTest->above_mem + 16;
  const int kNumTests = static_cast<int>(
      2.e10 / (block_size * block_size * kNumVp9IntraPredFuncs));
  intraPredTest->init(block_size, 8);

  for (int k = 0; k < kNumVp9IntraPredFuncs; ++k) {
    if (pred_funcs[k] == NULL) continue;
    memcpy(intraPredTest->src, intraPredTest->ref_src,
           sizeof(intraPredTest->src));
    vpx_usec_timer timer;
    vpx_usec_timer_start(&timer);
    for (int num_tests = 0; num_tests < kNumTests; ++num_tests) {
      pred_funcs[k](intraPredTest->src, kBPS, above, intraPredTest->left);
    }
    ...
    libvpx_test::MD5 md5;
    md5.Add(intraPredTest->src, sizeof(intraPredTest->src));
    printf("Mode %s[%12s]: %5d ms     MD5: %s\n", name, kVp9IntraPredNames[k],
...
void TestIntraPred4(VpxPredFunc const *pred_funcs) {
  ...
  VP9IntraPredTest intraPredTest;
  TestIntraPred("Intra4", &intraPredTest, pred_funcs, kSignatures, 4);
}
...
void TestIntraPred8(VpxPredFunc const *pred_funcs) {
  ...
  VP9IntraPredTest intraPredTest;
  TestIntraPred("Intra8", &intraPredTest, pred_funcs, kSignatures, 8);
}
...
void TestIntraPred16(VpxPredFunc const *pred_funcs) {
  ...
  VP9IntraPredTest intraPredTest;
  TestIntraPred("Intra16", &intraPredTest, pred_funcs, kSignatures, 16);
}
...
void TestIntraPred32(VpxPredFunc const *pred_funcs) {
  ...
  VP9IntraPredTest intraPredTest;
  TestIntraPred("Intra32", &intraPredTest, pred_funcs, kSignatures, 32);
}
...
typedef IntraPredTest<uint16_t> VP9HighbdIntraPredTest;

void TestHighbdIntraPred(const char name[],
                         VP9HighbdIntraPredTest *const intraPredTest,
                         VpxHighbdPredFunc const *pred_funcs,
                         const char *const signatures[], int block_size) {
  const int kNumTests = static_cast<int>(
      2.e10 / (block_size * block_size * kNumVp9IntraPredFuncs));
  uint16_t *const above = intraPredTest->above_mem + 16;
  intraPredTest->init(block_size, 12);
  ...
  for (int k = 0; k < kNumVp9IntraPredFuncs; ++k) {
    ...
    vpx_usec_timer_start(&timer);
    for (int num_tests = 0; num_tests < kNumTests; ++num_tests) {
      pred_funcs[k](intraPredTest->src, kBPS, above, intraPredTest->left, 12);
    }
    libvpx_test::ClearSystemState();
    vpx_usec_timer_mark(&timer);
    const int elapsed_time =
        static_cast<int>(vpx_usec_timer_elapsed(&timer) / 1000);
    libvpx_test::MD5 md5;
    md5.Add((uint8_t *)intraPredTest->src, sizeof(intraPredTest->src));
    printf("Mode %s[%12s]: %5d ms     MD5: %s\n", name, kVp9IntraPredNames[k],
           elapsed_time, md5.Get());
    EXPECT_STREQ(signatures[k], md5.Get());
  }
...
void TestHighbdIntraPred4(VpxHighbdPredFunc const *pred_funcs) {
  ...
  VP9HighbdIntraPredTest intraPredTest;
  TestHighbdIntraPred("Intra4", &intraPredTest, pred_funcs, kSignatures, 4);
}
...
void TestHighbdIntraPred8(VpxHighbdPredFunc const *pred_funcs) {
  ...
  VP9HighbdIntraPredTest intraPredTest;
  TestHighbdIntraPred("Intra8", &intraPredTest, pred_funcs, kSignatures, 8);
}
...
void TestHighbdIntraPred16(VpxHighbdPredFunc const *pred_funcs) {
  ...
  VP9HighbdIntraPredTest intraPredTest;
  TestHighbdIntraPred("Intra16", &intraPredTest, pred_funcs, kSignatures, 16);
}
...
void TestHighbdIntraPred32(VpxHighbdPredFunc const *pred_funcs) {
  ...
  VP9HighbdIntraPredTest intraPredTest;
  TestHighbdIntraPred("Intra32", &intraPredTest, pred_funcs, kSignatures, 32);
}
