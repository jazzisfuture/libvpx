...
void TestIntraPred(const char name[], VpxPredFunc const *pred_funcs,
  ...
  VP9IntraPredTestMem intraPredTestMem;
  uint8_t *const above = intraPredTestMem.above_mem + 16;
  const int kNumTests = static_cast<int>(
      2.e10 / (block_size * block_size * kNumVp9IntraPredFuncs));
  intraPredTestMem.Init(block_size, 8);

  for (int k = 0; k < kNumVp9IntraPredFuncs; ++k) {
    if (pred_funcs[k] == NULL) continue;
    memcpy(intraPredTestMem.src, intraPredTestMem.ref_src,
           sizeof(intraPredTestMem.src));
    vpx_usec_timer timer;
    vpx_usec_timer_start(&timer);
    for (int num_tests = 0; num_tests < kNumTests; ++num_tests) {
      pred_funcs[k](intraPredTestMem.src, kBPS, above, intraPredTestMem.left);
    }
    ...
    libvpx_test::MD5 md5;
    md5.Add(intraPredTestMem.src, sizeof(intraPredTestMem.src));
...
void TestHighbdIntraPred(const char name[],
                         ...
                         const char *const signatures[], int block_size) {
  VP9HighbdIntraPredTestMem intraPredTestMem;
  const int kNumTests = static_cast<int>(
      2.e10 / (block_size * block_size * kNumVp9IntraPredFuncs));
  uint16_t *const above = intraPredTestMem.above_mem + 16;
  intraPredTestMem.Init(block_size, 12);
  ...
  for (int k = 0; k < kNumVp9IntraPredFuncs; ++k) {
    ...
    const int elapsed_time =
        static_cast<int>(vpx_usec_timer_elapsed(&timer) / 1000);
    libvpx_test::MD5 md5;
    md5.Add((uint8_t *)intraPredTestMem.src, sizeof(intraPredTestMem.src));
    printf("Mode %s[%12s]: %5d ms     MD5: %s\n", name, kVp9IntraPredNames[k],
           elapsed_time, md5.Get());
    EXPECT_STREQ(signatures[k], md5.Get());
  }
