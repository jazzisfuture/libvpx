...
TEST_P(WedgeUtilsSSEOptTest, RandomValues) {
  ...
  for (int iter = 0 ; iter < kIterations && !HasFatalFailure(); ++iter) {
    ...
    const uint64_t ref_res = params_.ref_func_(r1, d, m, N);
    const uint64_t tst_res = params_.tst_func_(r1, d, m, N);
...
TEST_P(WedgeUtilsSignOptTest, RandomValues) {
  ...
  for (int iter = 0 ; iter < kIterations && !HasFatalFailure(); ++iter) {
    ...
    const int ref_res = params_.ref_func_(ds, m, N, limit);
    const int tst_res = params_.tst_func_(ds, m, N, limit);
...
TEST_P(WedgeUtilsDeltaSquaresOptTest, RandomValues) {
  ...
  for (int iter = 0 ; iter < kIterations && !HasFatalFailure(); ++iter) {
    ...
    params_.ref_func_(d_ref, a, b, N);
    params_.tst_func_(d_tst, a, b, N);
