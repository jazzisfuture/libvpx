...
void TestKeyframeMaximumInterval(bool is_vp9, unsigned long deadline,
...
TEST(KeyframeIntervalTest, KeyframeMaximumInterval) {
  std::vector<int> vpx_versions;
#if CONFIG_VP8_ENCODER
  vpx_versions.push_back(8);
#endif
#if CONFIG_VP9_ENCODER
  vpx_versions.push_back(9);
#endif
  for (int vpx_version : vpx_versions) {
    const bool is_vp9 = vpx_version == 9;
    for (unsigned long deadline :
      ...
      for (unsigned int kf_max_dist :
        ...
        TestKeyframeMaximumInterval(is_vp9, deadline, kf_max_dist);
      }
    }
  }
}
