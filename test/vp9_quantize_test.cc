...
TEST_P(VP9QuantizeTest, DISABLED_Speed) {
  ...
  for (TX_SIZE sz = 0; sz < 3; ++sz) {
    ...
    for (int i = 0; i < 3; ++i) {
      ...
      const TX_TYPE tx_type = (TX_TYPE)0;
      const scan_order *scan_order = &vp9_scan_orders[sz][tx_type];
      const int count = (4 << sz) * (4 << sz);  // 16, 64, 256
      if (i == 0) {
...
TEST_P(VP9Quantize32Test, DISABLED_Speed) {
  ACMRandom rnd(ACMRandom::DeterministicSeed());
  Buffer<tran_low_t> coeff = Buffer<tran_low_t>(32, 32, 0, 16);
  ASSERT_TRUE(coeff.Init());
  DECLARE_ALIGNED(16, int16_t, zbin_ptr[8]);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    vpx_usec_timer_start(&timer);
    for (int j = 0; j < 100000000 / count; ++j) {
      quantize_op_(coeff.TopLeftPixel(), count, skip_block, zbin_ptr, round_ptr,
                   quant_ptr, quant_shift_ptr, qcoeff.TopLeftPixel(),
                   dqcoeff.TopLeftPixel(), dequant_ptr, &eob, scan_order->scan,
                   scan_order->iscan);
    }
    vpx_usec_timer_mark(&timer);
    const int elapsed_time = static_cast<int>(vpx_usec_timer_elapsed(&timer));
...
INSTANTIATE_TEST_CASE_P(DISABLED_C, VP9Quantize32Test,
                        ::testing::Values(make_tuple(&vpx_quantize_b_32x32_c,
                                                     &vpx_quantize_b_32x32_c,
                                                     VPX_BITS_8)));
}  // namespace
