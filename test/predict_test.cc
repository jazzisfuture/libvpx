...
class PredictTestBase : public ::testing::TestWithParam<PredictParam> {
 public:
  virtual void SetUp() {
    ...
    // padded_dst_  provides a buffer of kBuffer around the destination memory
    // to facilitate detecting out of bounds writes.
    dst_stride_ = kBuffer + width_ + kBuffer;
    padded_dst_size_ = dst_stride_ * (kBuffer + height_ + kBuffer);
    padded_dst_ = new uint8_t[padded_dst_size_];
    dst_ = padded_dst_ + (kBuffer * dst_stride_) + kBuffer;
 ...
 protected:
  ...
  bool CompareBuffers(uint8_t *a, int a_stride, uint8_t *b,
                      int b_stride) const {
    bool mismatch = false;
  ...
  // match pixel value 'c'.
  bool CheckBorder(uint8_t *a, int a_size, uint8_t *b, int b_stride,
                   int b_height, int b_width, uint8_t c) const {
  ...
  void TestWithRandomData(PredictFunc reference) {
    ...
    for (int xoffset = 0; xoffset < 8; ++xoffset) {
      for (int yoffset = 0; yoffset < 8; ++yoffset) {
        ...
        ASSERT_TRUE(CheckBorder(padded_dst_, padded_dst_size_, dst_,
        ...
        for (int yoffset = 0; yoffset < 8; ++yoffset) {
          ...
          for (int i = 1; i < 4; i++) {
            ...
            ASSERT_TRUE(CheckBorder(padded_dst_, padded_dst_size_, dst_ + i,
                                    dst_stride_ + i, height_, width_, 128));
          }
        }
      }
    }
...
TEST_P(SixtapPredictTest, TestWithPresetData) {
  ...
  ASSERT_TRUE(CompareBuffers(const_cast<uint8_t *>(expected_dst),
