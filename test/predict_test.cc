...
class PredictTestBase : public ::testing::TestWithParam<PredictParam> {
 ...
 protected:
  ...
  // match pixel value 'c'.
  void CheckBorder(const uint8_t *a, int a_size, const uint8_t *b, int b_width,
  ...
  void TestWithRandomData(PredictFunc reference) {
    ...
    for (int xoffset = 0; xoffset < 8; ++xoffset) {
      for (int yoffset = 0; yoffset < 8; ++yoffset) {
        ...
        CheckBorder(padded_dst_, padded_dst_size_, dst_, width_, height_,
                    dst_stride_, 128);
        ASSERT_FALSE(HasFailure());
      }
    }
      ...
      for (int xoffset = 0; xoffset < 8; ++xoffset) {
        for (int yoffset = 0; yoffset < 8; ++yoffset) {
          ...
          for (int i = 1; i < 4; i++) {
            ...
            ASSERT_FALSE(HasFailure());
            CheckBorder(padded_dst_, padded_dst_size_, dst_ + i, width_,
                        height_, dst_stride_ + i, 128);
            ASSERT_FALSE(HasFailure());
          }
        }
      }
...
#if HAVE_SSSE3
INSTANTIATE_TEST_CASE_P(
    DISABLED_SSSE3, SixtapPredictTest,
