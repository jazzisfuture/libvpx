...
class PredictTestBase : public ::testing::TestWithParam<PredictParam> {
 ...
 protected:
  ...
  void CompareBuffers(const uint8_t *a, int a_stride, const uint8_t *b,
                      int b_stride) const {
    for (int height = 0; height < height_; ++height) {
      EXPECT_FALSE(memcmp(a + height * a_stride, b + height * b_stride,
                          sizeof(*a) * width_))
  ...
  // match pixel value 'c'.
  void CheckBorder(const uint8_t *a, int a_size, const uint8_t *b, int b_stride,
    ...
    for (int pixel = 0; pixel < b - a - left_border; ++pixel) {
      EXPECT_TRUE(c == a[pixel]) << "Mismatch at " << pixel
                                 << " in top border. Expecting " << (int)c
                                 << " but observed " << (int)a[pixel];
    }

    // Left border.
    for (int height = 0; height < b_height; ++height) {
      for (int width = left_border; width > 0; --width) {
        EXPECT_TRUE(c == b[height * b_stride - width])
            << "Mismatch at row " << height << " column " << left_border - width
            << " in left border. Expecting " << (int)c << " but observed "
            << (int)b[height * b_stride - width];
      }
    }
    ...
    for (int height = 0; height < b_height; ++height) {
      for (int width = b_width; width < b_width + right_border; ++width) {
        EXPECT_TRUE(c == b[height * b_stride + width])
            ...
            << " in right border. Expecting " << (int)c << " but observed "
            << (int)b[height * b_stride + width];
      }
    }
    ...
    for (int pixel = b - a + b_size; pixel < a_size; ++pixel) {
      EXPECT_TRUE(c == a[pixel]) << "Mismatch at " << pixel
                                 << " in bottom border. Expecting " << (int)c
                                 << " but observed " << (int)a[pixel];
    }
  }
  ...
  void TestWithRandomData(PredictFunc reference) {
    ...
    for (int xoffset = 0; xoffset < 8; ++xoffset) {
      for (int yoffset = 0; yoffset < 8; ++yoffset) {
        ...
        CompareBuffers(dst_c_, 16, dst_, dst_stride_);
        CheckBorder(padded_dst_, padded_dst_size_, dst_, dst_stride_, height_,
                    width_, 128);
      }
    }
  }
      ...
      for (int xoffset = 0; xoffset < 8; ++xoffset) {
        for (int yoffset = 0; yoffset < 8; ++yoffset) {
          ...
          for (int i = 1; i < 4; i++) {
            ...
            CompareBuffers(dst_c_, 16, dst_ + i, dst_stride_ + i);
            CheckBorder(padded_dst_, padded_dst_size_, dst_ + i,
