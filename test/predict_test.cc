...
class PredictTestBase : public ::testing::TestWithParam<PredictParam> {
 public:
  virtual void SetUp() {
    ...
    src_ = reinterpret_cast<uint8_t *>(vpx_malloc(kSrcSize));

    dst_stride_ = kBuffer + width_ + kBuffer;
    dst_buffered_size_ = dst_stride_ * (kBuffer + height_ + kBuffer);
    dst_buffered_ = reinterpret_cast<uint8_t *>(vpx_malloc(dst_buffered_size_));
    dst_ = dst_buffered_ + (kBuffer * dst_stride_) + kBuffer;

    dst_c_ = reinterpret_cast<uint8_t *>(vpx_malloc(16 * 16));

    memset(src_, 0, kSrcSize);
    memset(dst_buffered_, 128, dst_buffered_size_);
 ...
 protected:
  ...
  static PredictFunc predict_reference_;
  uint8_t *src_;
  uint8_t *dst_;
  uint8_t *dst_buffered_;
  int dst_buffered_size_;
  uint8_t *dst_c_;
  int dst_stride_;
  ...
  // excepting block 'b' described by b_stride, b_height, and b_width match
  // pixel value 'c'.
  bool check_border(uint8_t *a, int a_size, uint8_t *b, int b_stride,
                    int b_height, int b_width, uint8_t c) {
    bool mismatch = false, top = false, left = false, right = false,
         bottom = false;

    uint8_t *a_end = a + a_size;
    int b_size = (b_stride * b_height) + b_width;
    uint8_t *b_end = b + b_size;
    int left_border = (b_stride - b_width) / 2;
    int right_border = left_border + ((b_stride - b_width) % 2);

    if (b - left_border < a) {
      printf("'b' does not start within 'a'\n");
      return false;
    }

    if (b_end + right_border > a_end) {
      printf("'b' does not end within 'a'\n");
      return false;
    }
    ...
    // Bottom border
    for (int pixel = b - a + b_size; pixel < a_size; ++pixel) {
      if (c != a[pixel]) {
    ...
    if (left) {
      ...
      for (int height = 0; height < b_height; ++height) {
        for (int width = left_border; width > 0; --width) {
          uint8_t pixel = b[height * b_stride - width];
          if (c != pixel) {
            printf(" *%1d", pixel);
          } else {
            printf(" %2d", pixel);
  ...
  void test_with_random_data() {
    ...
    for (int xoffset = 0; xoffset < 8; ++xoffset) {
      for (int yoffset = 0; yoffset < 8; ++yoffset) {
        ...
        ASSERT_TRUE(compare_buffers(dst_c_, 16, dst_, dst_stride_));
        ASSERT_TRUE(check_border(dst_buffered_, dst_buffered_size_, dst_,
                                 dst_stride_, height_, width_, 128));
      }
    }
  }

  void test_with_unaligned_dst() {
    ACMRandom rnd(ACMRandom::DeterministicSeed());
      ...
      for (int xoffset = 0; xoffset < 8; ++xoffset) {
        for (int yoffset = 0; yoffset < 8; ++yoffset) {
          ...
          for (int i = 1; i < 4; i++) {
            ...
            ASSERT_TRUE(compare_buffers(dst_c_, 16, dst_ + i, dst_stride_ + i));
            ASSERT_TRUE(check_border(dst_buffered_, dst_buffered_size_,
                                     dst_ + i, dst_stride_ + i, height_, width_,
                                     128));
          }
        }
      }
...
PredictFunc PredictTestBase::predict_reference_ = NULL;
...
TEST_P(SixtapPredictTest, TestWithUnalignedDst) { test_with_unaligned_dst(); }
...
TEST_P(SixtapPredictTest, TestWithPresetData) {
  ...
  static const uint8_t expected_dst[256] = {
  ...
  ASM_REGISTER_STATE_CHECK(
      predict_(const_cast<uint8_t *>(test_data) + kSrcStride * 2 + 2,
               kSrcStride, 2, 2, dst_, dst_stride_));

  ASSERT_TRUE(compare_buffers(const_cast<uint8_t *>(expected_dst),
                              expected_dst_stride, dst_, dst_stride_));
}
...
TEST_P(BilinearPredictTest, TestWithUnalignedDst) { test_with_unaligned_dst(); }
