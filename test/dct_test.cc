...
class TransTestBase {
 ...
 protected:
  void BaseSetUp() { mask_ = (1 << bit_depth_) - 1; }
...
#if !CONFIG_VP9_HIGHBITDEPTH  // weird, fdct8x8_ssse3 is no good in hbd?
#if HAVE_SSSE3 && !CONFIG_EMULATE_HARDWARE
#if !ARCH_X86_64
// No fdct ssse3 functions here. Running the idct just because.
INSTANTIATE_TEST_CASE_P(
    SSSE3, TransDCT,
...
#if CONFIG_VP9_HIGHBITDEPTH
/* TODO:(johannkoenig) Determine why these fail AccuracyCheck
   make_tuple(&vpx_highbd_fdct16x16_neon, &idct16x16_12, 16, 0, VPX_BITS_12),
*/
INSTANTIATE_TEST_CASE_P(
    NEON, TransDCT,
    ::testing::Values(
        make_tuple(&vpx_highbd_fdct16x16_neon, &idct16x16_10, 16, 0,
                   VPX_BITS_10),
        make_tuple(&vpx_fdct16x16_neon, &vpx_idct16x16_256_add_neon, 16, 0,
                   VPX_BITS_8),
        make_tuple(&vpx_highbd_fdct8x8_neon, &idct8x8_10, 8, 0, VPX_BITS_10),
        make_tuple(&vpx_highbd_fdct8x8_neon, &idct8x8_12, 8, 0, VPX_BITS_12),
        make_tuple(&vpx_fdct8x8_neon, &vpx_idct8x8_64_add_neon, 8, 0,
                   VPX_BITS_8),
        make_tuple(&vpx_highbd_fdct4x4_neon, &idct4x4_10, 4, 0, VPX_BITS_10),
        make_tuple(&vpx_highbd_fdct4x4_neon, &idct4x4_12, 4, 0, VPX_BITS_12),
        make_tuple(&vpx_fdct4x4_neon, &vpx_idct4x4_16_add_neon, 4, 0,
                   VPX_BITS_8)));
#else
INSTANTIATE_TEST_CASE_P(
    NEON, TransDCT,
