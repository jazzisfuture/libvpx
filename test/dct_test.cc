...
class TransTestBase : public ::testing::TestWithParam<DctParam> {
 public:
  virtual void SetUp() {
    ...
    ASSERT_TRUE(src_ != nullptr);
    dst_ = reinterpret_cast<uint8_t *>(
        vpx_memalign(16, pixel_size_ * block_size_));
    ASSERT_TRUE(dst_ != nullptr);
  }
 ...
 protected:
  void RunAccuracyCheck(int limit) {
    ...
    ASSERT_TRUE(test_input_block.TopLeftPixel() != nullptr);
  ...
  void RunMemCheck() {
    ...
    for (int i = 0; i < count_test_block; ++i) {
      ...
      if (i == 0) {
        ...
        ASSERT_TRUE(input_extreme_block.TopLeftPixel() != nullptr);
        for (int h = 0; h < size_; ++h) {
          for (int w = 0; w < size_; ++w) {
            input_extreme_block
      ...
      // The minimum quant value is 4.
      EXPECT_TRUE(output_block.CheckValues(output_ref_block));
      ASSERT_TRUE(output_block.TopLeftPixel() != nullptr);
      for (int h = 0; h < size_; ++h) {
        for (int w = 0; w < size_; ++w) {
          EXPECT_GE(
  ...
  void RunInvAccuracyCheck(int limit) {
    ...
    for (int i = 0; i < count_test_block; ++i) {
      InitMem();
      ASSERT_TRUE(in.TopLeftPixel() != nullptr);
      // Initialize a test block with input range [-max_pixel_value_,
      // max_pixel_value_].
      for (int h = 0; h < size_; ++h) {
        for (int w = 0; w < size_; ++w) {
