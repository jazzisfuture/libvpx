...
class DatarateTestVP9Large
 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libvpx_test::VideoSource *video,
    ...
    if (video->frame() == 0) {
      ...
      if (force_intra_only_frame_) {
        ...
        // So set it here to avoid encoder-decoder ismatch check
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    // Verify that if force_intra_only_frame_ is enabled the stream does not
    // start with a key frame.
    if (force_intra_only_frame_ && frame_number_ == 0) {
      ASSERT_EQ(key_frame, false);
    }
...
TEST_P(DatarateTestVP9Large, StartWithIntraOnlyFrame) {
  cfg_.rc_buf_initial_sz = 500;
  cfg_.rc_buf_optimal_sz = 500;
  cfg_.rc_buf_sz = 1000;
  cfg_.rc_dropframe_thresh = 1;
