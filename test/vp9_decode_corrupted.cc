...
class DecodeCorruptedFrameTest
 ...
 protected:
  ...
  virtual const vpx_codec_cx_pkt_t *MutateEncoderOutputHook(
    ...
    memcpy(&modified_pkt_, pkt, sizeof(vpx_codec_cx_pkt_t));

    // Set size to 1 so it's corrupted to decoder.
    modified_pkt_.data.frame.sz = 1;
  ...
  virtual bool HandleDecodeResult(const vpx_codec_err_t res_dec,
    ...
    EXPECT_NE(VPX_CODEC_MEM_ERROR, res_dec) << decoder->DecodeError();
