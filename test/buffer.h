...
class Buffer {
 public:
  ...
  T *Src() { return b_ ? b_ + (p_ * stride_) + p_ : NULL; }
...
void Buffer<T>::PrintDifference(Buffer<T> a) {
  if ((a.x_ != this->x_) || (a.y_ != this->y_)) {
    printf("Unable to print buffers. Reference buffer of size %dx%d does not match this buffer which is size %dx%d\n",
...
template <typename T>
int Buffer<T>::CheckValues(int a) {
  for (int y = 0; y < y_; ++y) {
    for (int x = 0; x < x_; ++x) {
...
int Buffer<T>::CompareBuffer(Buffer<T> a) {
  if ((a.x_ != this->x_) || (a.y_ != this->y_)) {
    printf("Unable to compare buffers. Reference buffer of size %dx%d does not match this buffer which is size %dx%d\n",
           a.x_, a.y_, this->x_, this->y_);
    return 1;
  }
