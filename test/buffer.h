 ...
 public:
  Buffer(size_t width, size_t height, size_t top_padding, size_t left_padding,
      ...
      : width_(width), height_(height), top_padding_(top_padding),
        ...
        bottom_padding_(bottom_padding), alignment_(0), raw_buffer_(NULL) {}

  Buffer(size_t width, size_t height, size_t top_padding, size_t left_padding,
         size_t right_padding, size_t bottom_padding, unsigned int alignment)
      : width_(width), height_(height), top_padding_(top_padding),
        left_padding_(left_padding), right_padding_(right_padding),
  ...
  Buffer(size_t width, size_t height, size_t padding)
      : width_(width), height_(height), top_padding_(padding),
        ...
        bottom_padding_(padding), alignment_(0), raw_buffer_(NULL) {}

  Buffer(size_t width, size_t height, size_t padding, unsigned int alignment)
      : width_(width), height_(height), top_padding_(padding),
        left_padding_(padding), right_padding_(padding),
        bottom_padding_(padding), alignment_(alignment), raw_buffer_(NULL) {}
  ...
  bool Init() {
    ...
    if (alignment_) {
      ...
      EXPECT_EQ((left_padding_ * sizeof(T)) % alignment_, 0);
      // Ensure alignment of the subsequent rows will be preserved when there is
      // a stride.
      if (stride_ != width_) {
        EXPECT_EQ((stride_ * sizeof(T)) % alignment_, 0);
      }
