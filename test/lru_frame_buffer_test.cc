 ...
 protected:
  ...
  virtual void PreDecodeFrameHook(
    ...
    if (num_jitter_buffers_ > 0 && video.frame_number() == 0) {
      const int32_t max_reference_buffers = 8;
  ...
  virtual void DecompressedFrameHook(const vpx_image_t& img,
    ...
    // Check to see if any of the contents changed.
    if (jitter_buffer_md5_sums_.size() >
      ...
      ASSERT_STREQ(fb_md5.md5.c_str(), check_str.c_str());
      jitter_buffer_md5_sums_.pop();
    }
  }

  int32_t num_buffers() const { return num_buffers_; }
  void set_num_jitter_buffers(int32_t num_buffers) {
    num_jitter_buffers_ = num_buffers;
  }
  int32_t num_jitter_buffers() const { return num_jitter_buffers_; }

 private:
  // Total number of external frame buffers.
  int32_t num_buffers_;
  int32_t num_jitter_buffers_;
...
// has expired, the md5 checksum is computed again for the jitter buffer
// frame and checked against the md5 checksum after the frame was decoded.
// If md5 checksums match, then the test is passed. Otherwise, the test
// failed.
