...
class ExternalFrameBufferList {
 public:
  explicit ExternalFrameBufferList(vpx_codec_frame_buffer_type_t type)
      : num_buffers_(0), ext_fb_list_(NULL), type_(type), alignment_(32) {}
    ...
    if (type_ == VPX_CODEC_FRAME_BUFFER_TYPE_SIZE) {
      ...
      for (int plane = 0; plane < 3; ++plane) {
        ...
        if (plane > 0 && fb->fmt != VPX_IMG_FMT_I444) {
          if (fb->fmt != VPX_IMG_FMT_I440) plane_stride /= 2;
  ...
  void set_alignment(int alignment) { alignment_ = alignment; }
...
class ExternalFrameBufferTest
 ...
 protected:
  ExternalFrameBufferTest()
      : video_(NULL), decoder_(NULL), num_buffers_(0),
  ...
  void SetBufferAlignment(int alignment) { fb_list_.set_alignment(alignment); }
 ...
 private:
  void CheckDecodedFrames() {
    ...
    while ((img = dec_iter.Next()) != NULL) {
      if (frame_buffer_functions_set_) fb_list_.CheckXImageFrameBuffer(img);
    }
  }
...
TEST_P(ExternalFrameBufferTest, InvalidAlignement) {
  if (GET_PARAM(kCodedFrameBufferTypeParam) == VPX_CODEC_FRAME_BUFFER_TYPE_SIZE)
    return;
  const int num_buffers = VP9_MAXIMUM_REF_BUFFERS + VPX_MAXIMUM_WORK_BUFFERS;
  ASSERT_EQ(VPX_CODEC_OK,
            SetFrameBufferFunctions(num_buffers, get_vp9_frame_buffer,
