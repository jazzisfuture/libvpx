 ...
 protected:
  ...
  virtual void PreDecodeFrameHook(
    ...
    if (num_buffers_ > 0 && video.frame_number() == 0) {
      ...
      ASSERT_EQ(decoder->SetExternalFrameBuffers(frame_buffers_, num_buffers_,
                                       realloc_vp9_frame_buffer, NULL), VPX_CODEC_OK);
    }
  }
...
class ExternalFrameBufferTest : public ::testing::Test {
 protected:
  ...
  vpx_codec_err_t DecodeFramesLeft() {
...
TEST_F(ExternalFrameBufferTest, EightFrameBuffers) {
  ...
  ASSERT_EQ(SetExternalFrameBuffers(num_buffers, realloc_vp9_frame_buffer),
            VPX_CODEC_OK);
  ASSERT_EQ(DecodeFramesLeft(), VPX_CODEC_OK);
}
...
TEST_F(ExternalFrameBufferTest, NullFrameBufferList) {
  ...
  // one working buffer, plus four jitter buffers.
  const int num_buffers = 13;
  ASSERT_EQ(SetNullFrameBuffers(num_buffers, realloc_vp9_frame_buffer),
            VPX_CODEC_INVALID_PARAM);
}
...
TEST_F(ExternalFrameBufferTest, NullRealloc) {
  ...
  // one working buffer, plus four jitter buffers.
  const int num_buffers = 13;
  ASSERT_EQ(SetExternalFrameBuffers(num_buffers,
                                    zero_realloc_vp9_frame_buffer),
            VPX_CODEC_OK);
  ASSERT_EQ(DecodeOneFrame(), VPX_CODEC_MEM_ERROR);
}
