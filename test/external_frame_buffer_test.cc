...
class ExternalFrameBufferList {
 public:
  ...
  int ReturnFrameBuffer(vpx_codec_frame_buffer_t *fb) {
    ...
    ExternalFrameBuffer *const ext_fb =
        static_cast<ExternalFrameBuffer*>(fb->priv);
    ext_fb->in_use = 0;
    return 0;
  }
  ...
  void CheckXImageFrameBuffer(const vpx_image_t *img) {
    if (img->fb_priv != NULL) {
      const struct ExternalFrameBuffer *const ext_fb =
          static_cast<ExternalFrameBuffer*>(img->fb_priv);
 ...
 private:
  ...
  void SetFrameBuffer(int idx, vpx_codec_frame_buffer_t *fb) {
    ...
    fb->priv = static_cast<void*>(&ext_fb_list_[idx]);
  }
...
class ExternalFrameBufferTest : public ::testing::Test {
 protected:
  ...
  vpx_codec_err_t SetFrameBufferFunctions(
    ...
    return decoder_->SetFrameBufferFunctions(cb_get, cb_release,
                                             static_cast<void*>(&fb_list_));
  }
