...
# parameter one is used as the input file path and shifted away. All remaining
# parameters are passed through to vpxdec.
vpxdec_pipe() {
...
# the directory containing vpxdec. Positional parameter one is used as the input
# file path and shifted away. All remaining parameters are passed through to
# vpxdec.
vpxdec() {
  local decoder="${LIBVPX_BIN_PATH}/vpxdec${VPX_TEST_EXE_SUFFIX}"
  local input="${1}"
