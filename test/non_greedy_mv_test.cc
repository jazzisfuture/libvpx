...
static int read_in_mf(const char *filename, int *rows_ptr, int *cols_ptr,
  ...
  *buffer_ptr = (MV *)calloc((*rows_ptr) * (*cols_ptr), sizeof(MV));
...
static int read_in_local_var(const char *filename, int *rows_ptr, int *cols_ptr,
  ...
  *M_ptr = (int(*)[4])calloc((*rows_ptr) * (*cols_ptr) * 4, sizeof(int));
...
static void compare_mf(const MV *mf1, const MV *mf2, int rows, int cols,
  ...
  float *diffs = (float *)calloc(rows * cols, sizeof(float));
  int idx;
  float accu = 0.0f;
  for (idx = 0; idx < rows * cols; ++idx) {
    MV mv1 = mf1[idx];
...
TEST(non_greedy_mv, DISABLED_smooth_mf) {
  ...
  if (flag_mf == 1 or flag_local_var == 1 or flag_estimation or
      flag_ground_truth) {
    return;
  }

  // scale local var
  smooth_mf = (MV *)calloc(rows * cols, sizeof(MV));
