...
class IntraPredBase {
 protected:
  void SetupMacroblock(uint8_t *data, int blockSize, int stride, int numPlanes) {
    ...
    for (int p = 0; p < numPlanes; p++) {
      dataPtr_[p] = data + stride * (blockSize + 1) * p + stride + blockSize;

      // Fill edges with random data
      for (int x = -1 ; x <= blockSize_; x++)
        dataPtr_[p][x - stride] = rand();
  ...
  void CheckDCPrediction() {
    for (int p = 0; p < numPlanes_; p++) {
      ...
      if (mb_.up_available || mb_.left_available) {
        int sum = 0, shift = BlockSizeLog2Min1() + mb_.up_available + mb_.left_available;
        if (mb_.up_available)
      ...
      for (int y = 1; y < blockSize_; ++y)
        EXPECT_EQ(0, memcmp(dataPtr_[p], &dataPtr_[p][y * stride_], blockSize_));
  ...
  void CheckHPrediction() {
    ...
    for (int p = 0; p < numPlanes_; p++)
      for (int y = 0; y < blockSize_; y++)
        for (int x = 0; x < blockSize_; x++)
          EXPECT_EQ(dataPtr_[p][-1 + y * stride_], dataPtr_[p][x + y * stride_]);
  }
