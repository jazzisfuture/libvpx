  ...
  const void CheckDCPrediction() {
    for (int p = 0; p < num_planes_; p++) {
      ...
      for (int y = 1; y < block_size_; ++y)
        EXPECT_EQ(0, memcmp(data_ptr_[p], &data_ptr_[p][y * stride_],
                            block_size_));
