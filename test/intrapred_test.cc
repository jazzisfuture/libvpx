...
class IntraPredYTest : public IntraPredBase {
 public:
  ...
  void RunTestWithFunc(intra_pred_y_fn_t pred_fn) {
    pred_fn_ = pred_fn;
    RunTest();
  }
...
TEST(IntraPred, Y) {
  ...
  static const struct {
    ...
    { vp8_build_intra_predictors_mby_s_c, 0 },
#if HAVE_SSE2
    { vp8_build_intra_predictors_mby_s_sse2, HAS_SSE2 },
#endif
#if HAVE_SSSE3
    { vp8_build_intra_predictors_mby_s_ssse3, HAS_SSSE3 },
#endif
  };

  for (unsigned n = 0;
       n < sizeof(intra_pred_fns) / sizeof(intra_pred_fns[0]); n++) {
    if (!intra_pred_fns[n].cpu_flag
#if ARCH_X86 || ARCH_X86_64
        || x86_simd_caps() & intra_pred_fns[n].cpu_flag
#endif
        ) {
      test.RunTestWithFunc(intra_pred_fns[n].pred_fn);
    }
  }
}
...
class IntraPredUVTest : public IntraPredBase {
 public:
  ...
  void RunTestWithFunc(intra_pred_uv_fn_t pred_fn) {
    pred_fn_ = pred_fn;
    RunTest();
  }
...
TEST(IntraPred, UV) {
  IntraPredUVTest test;
  static const struct {
    intra_pred_uv_fn_t pred_fn;
    int cpu_flag;
  } intra_pred_fns[] = {
    { vp8_build_intra_predictors_mbuv_s_c, 0 },
#if HAVE_SSE2
    { vp8_build_intra_predictors_mbuv_s_sse2, HAS_SSE2 },
#endif
#if HAVE_SSSE3
    { vp8_build_intra_predictors_mbuv_s_ssse3, HAS_SSSE3 },
#endif
  };
