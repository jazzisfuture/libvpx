...
class DSPBuffer : public Buffer<T> {
 ...
 protected:
  void Init() {
    const int type_size_bits = sizeof(T) * 8;
    ...
    ASSERT_EQ((left_padding * type_size_bits) % alignment_bits_, 0);
    // Ensure alignment of the subsequent rows will be preserved.
    ASSERT_EQ((stride * type_size_bits) % alignment_bits_, 0);
