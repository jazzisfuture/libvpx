 ...
 protected:
  ...
  void RunAccuracyCheck(int limit) {
    ...
    const int count_test_block = 10000;
      ...
      int16_t *test_input_block =
          (int16_t *) vpx_memalign(16, sizeof(int16_t) * num_coeffs_);
      tran_low_t *test_temp_block =
          (tran_low_t *) vpx_memalign(16, sizeof(tran_low_t) * num_coeffs_);
      uint8_t *dst =
          (uint8_t *) vpx_memalign(16, sizeof(uint8_t) * num_coeffs_);
      uint8_t *src =
          (uint8_t *) vpx_memalign(16, sizeof(uint8_t) * num_coeffs_);
#if CONFIG_VP9_HIGHBITDEPTH
      uint16_t *dst16 =
          (uint16_t *) vpx_memalign(16, sizeof(uint16_t) * num_coeffs_);
      uint16_t *src16 =
          (uint16_t *) vpx_memalign(16, sizeof(uint16_t) * num_coeffs_);
#endif
  ...
  void RunCoeffCheck() {
    ...
    int16_t *input_block =
        (int16_t *) vpx_memalign(16, sizeof(int16_t) * num_coeffs_);
    tran_low_t *output_ref_block =
        (tran_low_t *) vpx_memalign(16, sizeof(tran_low_t) * num_coeffs_);
    tran_low_t *output_block =
        (tran_low_t *) vpx_memalign(16, sizeof(tran_low_t) * num_coeffs_);
  ...
  void RunMemCheck() {
    ...
    for (int i = 0; i < count_test_block; ++i) {
      ...
      for (int j = 0; j < num_coeffs_; ++j) {
        EXPECT_EQ(output_block[j], output_ref_block[j]);
        EXPECT_GE(row_length * DCT_MAX_VALUE << (bit_depth_ - 8),
                  abs(output_block[j]))
