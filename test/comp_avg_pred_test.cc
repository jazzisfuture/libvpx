...
void reference_pred(const DSPBuffer<uint8_t> &pred, const Buffer<uint8_t> &ref,
                    int width, int height, DSPBuffer<uint8_t> *avg) {
...
TEST_P(AvgPredTest, SizeCombinations) {
  ...
  for (int width_pow = 2; width_pow <= 6; ++width_pow) {
    for (int height_pow = width_pow - 1; height_pow <= width_pow + 1;
      ...
      // The sse2 special-cases ref when width == stride so make sure to test
      // it.
      for (int ref_padding = 0; ref_padding < 2; ref_padding++) {
        ...
        DSPBuffer<uint8_t> avg_ref = DSPBuffer<uint8_t>(width, height, 0, 16);
        DSPBuffer<uint8_t> avg_chk = DSPBuffer<uint8_t>(width, height, 0, 16);
    ...
    for (int height_pow = width_pow - 1; height_pow <= width_pow + 1;
      ...
      for (int ref_padding = 0; ref_padding < 2; ref_padding++) {
        ...
        Buffer<uint8_t> ref =
            Buffer<uint8_t>(width, height, ref_padding ? 8 : 0);
        DSPBuffer<uint8_t> pred = DSPBuffer<uint8_t>(width, height, 0, 16);
        DSPBuffer<uint8_t> avg = DSPBuffer<uint8_t>(width, height, 0, 16);
