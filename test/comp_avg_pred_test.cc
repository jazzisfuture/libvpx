...
void reference_pred(const Buffer<uint8_t> &pred, const Buffer<uint8_t> &ref,
                    int w, int h, Buffer<uint8_t> *avg) {
  for (int height = 0; height < h; ++height) {
    for (int width = 0; width < w; ++width) {
      avg->TopLeftPixel()[height * avg->stride() + width] =
          avg_with_rounding(pred.TopLeftPixel()[height * pred.stride() + width],
                            ref.TopLeftPixel()[height * ref.stride() + width]);
    }
  }
}
...
TEST_P(AvgPredTest, SizeCombinations) {
  ...
  for (int width_pow = 2; width_pow <= 6; ++width_pow) {
    for (int height_pow = width_pow - 1; height_pow <= width_pow + 1;
      ...
      avg_pred_func_(avg_chk.TopLeftPixel(), pred.TopLeftPixel(), width, height,
