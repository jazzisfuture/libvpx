 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libvpx_test::VideoSource *video,
    ...
    if (insert_layer_sync_) {
      int i;
      int svc_layer_sync[VPX_SS_MAX_LAYERS];
      for (i = 0; i < number_spatial_layers_; i++) svc_layer_sync[i] = 0;
      if (video->frame() == 150) {
        svc_layer_sync[1] = 1;
...
// one at middle layer first and then another one for top layer.
TEST_P(DatarateOnePassCbrSvcSmallKF, OnePassCbrSvc3SL3TLSyncFrames) {
  cfg_.rc_buf_initial_sz = 500;
  cfg_.rc_buf_optimal_sz = 500;
  cfg_.rc_buf_sz = 1000;
  cfg_.rc_min_quantizer = 0;
