  ...
  void set_frame_flags_bypass_mode(
    ...
    for (int sl = 0; sl < num_spatial_layers; ++sl)
      for (int ref = 0; ref < REF_FRAMES; ++ref)
        ref_frame_config->update_buffer_slot[ref][sl] = 0;
    ...
    for (int sl = 0; sl < num_spatial_layers; ++sl) {
      ...
      if (!tl) {
        if (!sl) {
          ...
          ref_frame_config
              ->update_buffer_slot[ref_frame_config->lst_fb_idx[sl]][sl] = 1;
        } else {
          if (is_key_frame) {
            ref_frame_config->reference_last[sl] = 1;
            ref_frame_config->reference_golden[sl] = 0;
            ref_frame_config->reference_alt_ref[sl] = 0;
            ref_frame_config
                ->update_buffer_slot[ref_frame_config->gld_fb_idx[sl]][sl] = 1;
          } else {
            ref_frame_config->reference_last[sl] = 1;
            ref_frame_config->reference_golden[sl] = 0;
            ref_frame_config->reference_alt_ref[sl] = 0;
        ...
        if (!sl) {
          ...
          ref_frame_config->reference_alt_ref[sl] = 0;
          ref_frame_config
              ->update_buffer_slot[ref_frame_config->alt_fb_idx[sl]][sl] = 1;
        } else {
