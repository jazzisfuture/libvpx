...
// Encode 3 spatial, 1 temporal layer but don't start decoding.
...
TEST_P(SyncFrameOnePassCbrSvc, OnePassCbrSvc3SL3TLSyncFrameIntraOnlyQVGA) {
  ...
  frame_to_sync_ = 20;
  decode_to_layer_before_sync_ = 2;
  // The superframe containing intra-only layer will have +1 frames. Thus set the
  // layer to decode after sync frame to +1 from decode_to_layer_before_sync_.
