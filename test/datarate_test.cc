...
class DatarateTest : public ::libvpx_test::EncoderTest,
 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libvpx_test::VideoSource *video,
    ...
    vpx_rational_t tb = video->timebase();
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    ::cerr << "bytes:" << bytes_in_buffer_model_;
    ...
    ::cerr << " dur:" << duration << " bytes:" << bytes_in_buffer_model_;
    ...
    // Subtract from the buffer the bytes associated with a played back frames.
    bytes_in_buffer_model_ -= pkt->data.frame.sz;

    ::cerr << " framesize:" << pkt->data.frame.sz << " bytes:"
        << bytes_in_buffer_model_ << "\n";
  ...
  virtual void EndPassHook(void) {
    if (bytes_total_) {
      double file_size_in_kb = bytes_total_ * 8 /* bits per byte */
          / 1024 /* bits per kilobit */;
...
TEST_P(DatarateTest, BasicBufferModel ) {
  ...
  /* Don't check overall bitrate because at the end of the clip its left in a
...
TEST_P(DatarateTest, ChangingDropFrameThresh ) {
  ...
  ResetModel();
  ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
  vpx_codec_pts_t thresh1_first_drop = first_drop_;

  cfg_.rc_dropframe_thresh = 50;
  ResetModel();
  ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
  vpx_codec_pts_t thresh50_first_drop = first_drop_;

  ::cerr << thresh50_first_drop << " " << thresh1_first_drop << "\n";
  ASSERT_GE(thresh1_first_drop, thresh50_first_drop)
    << " The first dropped frame for drop thresh 50 > drop thresh 1";
}
