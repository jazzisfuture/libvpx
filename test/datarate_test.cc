...
class DatarateOnePassCbrSvc
 ...
 protected:
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    for (int sl = 0; sl < number_spatial_layers_; ++sl) {
      if (pkt->data.frame.spatial_layer_encoded[sl] == 1){
    ...
    // Non-reference frames are top spatial and temporal layer frames, for TL > 0.
    if (temporal_layer_id_ == number_temporal_layers_ -1 && temporal_layer_id_ > 0 &&
        pkt->data.frame.spatial_layer_encoded[number_spatial_layers_ - 1] == 1)
      num_nonref_frames_++;
    for (int sl = 0; sl < number_spatial_layers_; ++sl) {
      sizes[sl] = sizes[sl] << 3;
...
TEST_P(DatarateOnePassCbrSvc, OnePassCbrSvc2SL1TLScreenContent1) {
  ...
  // The non-reference frames are expected to be mismatched frames as the encoder
  // will avoid loopfilter on these frames.
  EXPECT_EQ(static_cast<unsigned int>(num_nonref_frames_), GetMismatchFrames());
#endif
}
