...
class DatarateOnePassCbrSvc
 ...
 protected:
  ...
  virtual void GetLayerId(libvpx_test::Encoder *encoder) {
    vpx_svc_layer_id_t layer_id;
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    const bool key_frame =
        (pkt->data.frame.flags & VPX_FRAME_IS_KEY) ? true : false;
    vp9_parse_superframe_index(static_cast<const uint8_t *>(pkt->data.frame.buf),
                               pkt->data.frame.sz, sizes, &count, NULL, NULL);
...
static void assign_layer_bitrates(vpx_codec_enc_cfg_t *const enc_cfg,
  ...
  for (sl = 0; sl < spatial_layers; ++sl) {
    for (int tl = 0; tl < temporal_layers; ++tl) {
      ...
      layer_target_avg_bandwidth[layer] =
          enc_cfg->layer_target_bitrate[layer] * 1000 / framedec[tl];
...
static void check_layer_rate_targeting(vpx_codec_enc_cfg_t *const cfg,
                                       ...
                                       float thresh_overshoot,
                                       float thresh_undershoot){
