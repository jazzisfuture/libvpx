  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    for (int sl = 0; sl < number_spatial_layers_; ++sl) {
      ...
      for (int tl = temporal_layer_id_; tl < number_temporal_layers_; ++tl) {
        ...
        // There is an issue with the test for updating the buffer levels
        // on svc frame drops (we don't know the temporal_layer_id if whole
        // superframe gets dropped).
        // TODO(marpan): Re-enable this check once issue is resolved.
        // if (!key_frame && tl > 0) {
