...
class DatarateTestVP9 : public ::libvpx_test::EncoderTest,
 ...
 protected:
  ...
  // For two layers, pattern is:
  //   1     3
  // 0    2     .....
  // For three layers, pattern is:
  //   1      3    5      7
  //      2           6
  // 0          4            ....
  // LAST is always update on base/layer 0, GOLDEN is updated on layer 2.
  // For this 3 layer example, the 3rd enhancement layer does not update any
  // reference frames.
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    if (cfg_.ts_number_layers == 1) {
      bits_total_[0] += frame_size_in_bits;
    } else if (cfg_.ts_number_layers == 2) {
      if (layer == 0)
        bits_total_[0] += frame_size_in_bits;
      bits_total_[1] += frame_size_in_bits;
    } else if (cfg_.ts_number_layers == 3) {
      if (layer == 0)
        bits_total_[0] += frame_size_in_bits;
      if (layer <= 1)
        bits_total_[1] += frame_size_in_bits;
      bits_total_[2] += frame_size_in_bits;
    }
  ...
  virtual void EndPassHook(void) {
    ...
    printf("%d %f \n",layer, effective_datarate_[layer]);
    }
  }
...
TEST_P(DatarateTestVP9, BasicRateTargeting2TemporalLayers) {
  ...
  for (int i = 200; i < 800; i += 200) {
    ...
    ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
    ASSERT_GE(effective_datarate_[0], cfg_.ts_target_bitrate[0] * 0.85)
        << " The datarate for the file is lower than target by too much!";
    ASSERT_LE(effective_datarate_[0], cfg_.ts_target_bitrate[0] * 1.15)
        << " The datarate for the file is greater than target by too much!";
    ASSERT_GE(effective_datarate_[1], cfg_.ts_target_bitrate[1] * 0.85)
        << " The datarate for the file is lower than target by too much!";
    ASSERT_LE(effective_datarate_[1], cfg_.ts_target_bitrate[1] * 1.15)
        << " The datarate for the file is greater than target by too much!";
  }
}
...
TEST_P(DatarateTestVP9, BasicRateTargeting3TemporalLayers) {
  ...
  for (int i = 200; i < 800; i += 200) {
    ...
    ASSERT_LE(effective_datarate_[1], cfg_.ts_target_bitrate[1] * 1.15)
        << " The datarate for the file is greater than target by too much!";
    ASSERT_GE(effective_datarate_[2], cfg_.ts_target_bitrate[2] * 0.85)
        << " The datarate for the file is lower than target by too much!";
    ASSERT_LE(effective_datarate_[2], cfg_.ts_target_bitrate[2] * 1.15)
        << " The datarate for the file is greater than target by too much!";
  }
}
