...
TEST_P(DatarateOnePassCbrSvc, OnePassCbrSvc2SL1TLScreenContent1) {
  ...
  ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
  // For #temporal_layers > 1, mismatch will result since for the SVC patterns
  // in these tests the top enhancement does not update any reference buffers
  // any encoder will skip loopfilter.
...
TEST_P(DatarateOnePassCbrSvc, OnePassCbrSvc2SL3TL) {
  ...
  for (int i = 200; i <= 800; i += 200) {
    ...
    // When temporal layers > 1, half of the frames in these SVC patterns will
    // be non-reference frames and hence encoder will avoid loopfilter. This
    // will result in mismatch. Since frame dropper is off, expectation is 100
    // mismatch frames. Similar for the tests below, where #temporal_layers > 1
    // and frame dropper is off.
