...
static void assign_layer_bitrates(vpx_codec_enc_cfg_t *const enc_cfg,
  ...
  for (sl = 0; sl < spatial_layers; ++sl) {
    ...
    if (temporal_layering_mode == 3) {
      enc_cfg->layer_target_bitrate[sl * temporal_layers] =
          spatial_layer_target >> 1;
      enc_cfg->layer_target_bitrate[sl * temporal_layers + 1] =
          (spatial_layer_target >> 1) + (spatial_layer_target >> 2);
      enc_cfg->layer_target_bitrate[sl * temporal_layers + 2] =
...
TEST_P(DatarateOnePassCbrSvc, OnePassCbrSvc) {
  ...
  // No frame drop for now. we need to implement correct frame dropping for 
  // SVC.
  ...
  for (int i = 200; i <= 800; i += 200) {
    ...
    assign_layer_bitrates(&cfg_, &svc_params_, cfg_.ss_number_layers,
        cfg_.ts_number_layers, cfg_.temporal_layering_mode, cfg_.rc_target_bitrate);
    ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
    ASSERT_GE(cfg_.rc_target_bitrate, effective_datarate_ * 0.95)
            << " The datarate for the file exceeds the target!";
    ASSERT_LE(cfg_.rc_target_bitrate, file_datarate_ * 1.3)
...
VP9_INSTANTIATE_TEST_CASE(DatarateTestVP9Large,
                          ...
                          ::testing::Range(2, 7));
VP9_INSTANTIATE_TEST_CASE(DatarateOnePassCbrSvc, ONE_PASS_TEST_MODES);
}  // namespace
