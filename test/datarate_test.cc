    ...
    if (change_bitrate_) {
      ...
      if (video->frame() >= 200) {
        ...
        assign_layer_bitrates(
            &cfg_, &svc_params_, cfg_.ss_number_layers, cfg_.ts_number_layers,
            cfg_.temporal_layering_mode, layer_target_avg_bandwidth_,
...
TEST_P(DatarateOnePassCbrSvc, OnePassCbrSvc3SL3TLBitrateChangeMidday) {
  cfg_.rc_buf_initial_sz = 500;
  cfg_.rc_buf_optimal_sz = 500;
  cfg_.rc_buf_sz = 1000;
  cfg_.rc_min_quantizer = 0;
