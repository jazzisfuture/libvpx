...
class DatarateOnePassCbrSvc
 ...
 protected:
  ...
  virtual void GetLayerId(libvpx_test::Encoder *encoder) {
    ...
    // for every frame passed to the encoder.
...
static void assign_layer_bitrates(vpx_codec_enc_cfg_t *const enc_cfg,
  ...
  double framedec[VPX_MAX_LAYERS];
  ...
  for (sl = 0; sl < spatial_layers; ++sl) {
    for (int tl = 0; tl < temporal_layers; ++tl) {
      ...
      framedec[tl] = framerate;
      if (temporal_layers == 2 && tl == 0) framedec[tl] = framedec[tl] / 2;
      if (temporal_layers == 3 && tl == 0) framedec[tl] = framedec[tl] / 4;
      if (temporal_layers == 3 && tl == 1) framedec[tl] = framedec[tl] / 2;
