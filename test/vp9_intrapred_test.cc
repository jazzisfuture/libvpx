 ...
 public:
  void RunTest(Pixel *left_col, Pixel *above_data, Pixel *dst, Pixel *ref_dst) {
    ...
    for (int i = 0; i < count_test_block; ++i) {
      // Some of the optimised predictor implementations rely on the trailing
      // half of the above_row_ being a copy of the final element, however
      // relying on this in some cases can cause the MD5 tests to fail.
      // We have fixed all of these cases for Neon, so fill the whole of
      // above_row_ randomly.
#if HAVE_NEON
