...
TEST_P(TemporalFilterTest, SizeCombinations) {
  ...
  for (int width = 8; width <= 16; width += 8) {
    for (int height = 8; height <= 16; height += 8) {
      ...
      a.Set(&rnd_, &ACMRandom::Rand8LowVariance);
      b.Set(&rnd_, &ACMRandom::Rand8LowVariance);
      ...
      reference_filter(a, b, width, height, filter_strength, filter_weight,
                       &accum_ref, &count_ref);
      ASM_REGISTER_STATE_CHECK(filter_func_(a.TopLeftPixel(), a.stride(), b.TopLeftPixel(), width,
...
#if HAVE_SSSE3
INSTANTIATE_TEST_CASE_P(SSSE3, TemporalFilterTest,
                        ::testing::Values(&vp9_temporal_filter_apply_ssse3));
#endif  // HAVE_SSSE3
}  // namespace
