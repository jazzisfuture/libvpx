...
struct TestSSEFuncs {
  TestSSEFuncs(sse_func ref = nullptr, sse_func tst = nullptr, int depth = 0)
      : ref_func(ref), tst_func(tst), bit_depth(depth) {}
  sse_func ref_func;  // Pointer to reference function
  sse_func tst_func;  // Pointer to tested function
  int bit_depth;
};
