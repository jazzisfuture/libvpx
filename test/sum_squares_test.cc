...
TEST_P(SumSquaresTest, OperationCheck) {
  ...
  for (int k = 0; k < kNumIterations; k++) {
    ...
    const uint64_t res_ref = params_.ref_func_(src, stride, size);
    uint64_t res_tst;
    ASM_REGISTER_STATE_CHECK(res_tst = params_.tst_func_(src, stride, size));
...
TEST_P(SumSquares1DTest, RandomValues) {
  ...
  for (int iter = 0 ; iter < kIterations && !HasFatalFailure(); ++iter) {
    ...
    const uint64_t ref_res = params_.ref_func_(src, N);
    const uint64_t tst_res = params_.tst_func_(src, N);
