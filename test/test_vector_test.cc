...
class TestVectorTest : public libvpx_test::DecoderTest,
 ...
 protected:
  ...
  virtual void DecompressedFrameHook(const vpx_image_t *img) {
    ...
    int res = fscanf(md5_file_, "%s  %s", expected_md5, junk);
...
TEST_P(TestVectorTest, MD5Match) {
  ...
  uint8_t *const allocated_buf = new uint8_t[CODE_BUFFER_SIZE];
  ASSERT_TRUE(allocated_buf) << "Allocate frame buffer failed";

  // Loop through the test vectors.
  for (int i = 0; i < TestVectorNum; i++) {
