...
class TestVectorTest : public libvpx_test::DecoderTest, public ::testing::Test {
 protected:
  ...
  void OpenMD5File(std::string md5_file_name_) {
    std::string path_to_source = md5_file_name_;
    const char *kDataPath = getenv("LIBVPX_TEST_DATA_PATH");
    if (kDataPath) {
      path_to_source = kDataPath;
      path_to_source += "/";
      path_to_source += md5_file_name_;
    }

    md5_file_ = fopen(path_to_source.c_str(), "rb");

    ASSERT_TRUE(md5_file_) << "Input file open failed.";
  }

  virtual void MD5Hook(const vpx_image_t *img) {
    ...
    fscanf(md5_file_, "%s", expected_md5);
    fscanf(md5_file_, "%s", junk);
    ...
    int n = memcmp(expected_md5, actual_md5, 32);
    EXPECT_EQ(n, 0);
  }
...
TEST_F(TestVectorTest, MD5Match) {
  ...
  for (int i = 0; i < 56; i++) {
    ...
    libvpx_test::IVFVideoSource video(filename, 0, false);
