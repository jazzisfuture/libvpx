...
# Greps vpx_config.h in LIBVPX_BUILD_PATH for positional parameter one, which
...
# in LIBVPX_BUILD_PATH, and is executable.
vpx_tool_available() {
...
# LIBVPX_BUILD_PATH points to the directory containing vpxdec. Positional
...
run_tests() {
  ...
  for test in ${tests_to_run}; do
    test_begin $test
    $test
    [ "${VERBOSE_OUTPUT}" = "yes" ] && echo PASS $test
    test_end $test
  done
