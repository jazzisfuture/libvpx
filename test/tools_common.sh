...
test_configuration_target() {
  ...
  # Split $config_line with awk using 'target=' as field separator to produce
  # 'target-name [optional flags];"', then:
  #   1. Consume the '";' portion with tr.
  #   2. Remove anything that remains after the target name using cut, while
  #      allowing cut to write the output to stdout.
  echo ${config_line} | awk -F 'target=' -- '{ print $NF }' | \
      tr -d ';"' | cut -f 1 -d " "
}
...
config_hash() {
  ...
  git_hash=$(echo ${vpx_version_string} | awk -F - -- '{print $NF}' | \
      tr -d '"')
...
# found.
is_windows_target() {
  if test_configuration_target | sed -e s/darwin// | grep -q win \
      > /dev/null 2>&1; then
    echo yes
  fi
}
...
cat << EOF
  Usage: ${0##*/} [arguments]
    --bin_path <path to libvpx binaries directory>
    --config_path <path to libvpx config directory>
    --filter <filter>: User test filter. Only tests matching filter are run.
    --run_disabled_tests: Run disabled tests.
    --help: Display this message and exit.
    --test_data_path <path to libvpx test data directory>
    --verbose: Verbose output.

    When the --bin_path option is not specified the script attempts to use
    \$LIBVPX_BIN_PATH and then the current directory.

    When the --config_path option is not specified the script attempts to use
    \$LIBVPX_CONFIG_PATH and then the current directory.

    When the -test_data_path option is not specified the script attempts to use
...
# Allow partial specification of configuration when running the tests from a
# build directory on *nix/macosx.
...
VPX_TEST_OUTPUT_DIR=$(mktemp -d ${VPX_TEST_TEMP_ROOT}/vpx_test_XXXXXXXXX)
trap cleanup EXIT
...
if [ "${VPX_TEST_VERBOSE_OUTPUT}" = "yes" ]; then
  ...
  echo VPX_TEST_OUTPUT_DIR=$VPX_TEST_OUTPUT_DIR
