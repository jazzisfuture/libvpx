...
config_hash() {
  vpx_version_file="${LIBVPX_CONFIG_PATH}/config.mk"
  # Find VERSION_STRING line, split it with "-g" and print the last field to
  # output the git hash to stdout.
  vpx_version=$(awk -F -g '/VERSION_STRING/ {print $NF}' "${vpx_version_file}")
...
#
# TODO(tomfinegan): Use $@/shift and handle extra flags for testing args?
vpxdec() {
  input=$1
  pipe_input=$2
