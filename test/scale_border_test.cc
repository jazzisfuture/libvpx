...
class ExtendBorderTest
 ...
 protected:
  ...
  void FillPlane(uint8_t *buf, int width, int height, int stride) {
  ...
  void ReferenceExtendBorder() {
    ...
    // By calculating uv_crop_* earlier we round up at the appropriate time
  ...
  void ExtendPlane(uint8_t *buf, int crop_width, int crop_height, int width,
                   int height, int stride, int padding) {
