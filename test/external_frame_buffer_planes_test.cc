...
class ExternalFrameBufferList {
 public:
  ...
  int GetFreeFrameBuffer(
      vpx_img_fmt_t fmt, size_t width, size_t height,
      vpx_codec_frame_buffer_t *fb) {
    EXPECT_TRUE(fb != NULL);
    EXPECT_TRUE(fmt == VPX_IMG_FMT_I420 || fmt == VPX_IMG_FMT_I444 ||
                fmt == VPX_IMG_FMT_I422 || fmt == VPX_IMG_FMT_I440)
      << " fmt is: " << fmt;
    ...
    for (int plane = 0; plane < 3; ++plane) {
      ...
      plane_stride = (plane_stride + (alignment_ - plane_stride % alignment_));
...
class ExternalFrameBufferPlanesTest : public ::testing::Test {
 protected:
  ...
  vpx_codec_err_t SetFrameBufferFunctions(
    ...
    return decoder_->SetFrameBufferFunctions(
        cb_get, cb_release, &fb_list_);
  }
...
int get_vp9_frame_buffer_planes(void *priv, size_t size, vpx_codec_frame_buffer_t *fb) {
  ExternalFrameBufferList *const fb_list =
      reinterpret_cast<ExternalFrameBufferList*>(priv);
  fb->type = VPX_CODEC_FRAME_BUFFER_PLANES;
  return fb_list->GetFreeFrameBuffer(fb->fmt, fb->width, fb->height, fb);
}
...
class ExternalFramePlanesMD5Test
 ...
 protected:
  ...
  // error.
  static int GetVP9FramePlanes(void *priv, size_t size,
                               vpx_codec_frame_buffer_t *fb) {
    ExternalFramePlanesMD5Test *const md5Test =
        reinterpret_cast<ExternalFramePlanesMD5Test*>(priv);
    fb->type = VPX_CODEC_FRAME_BUFFER_PLANES;
    return md5Test->fb_list_.GetFreeFrameBuffer(fb->fmt, fb->width, fb->height, fb);
  }
  ...
  static int ReleaseVP9FramePlanes(void *priv,
                                   vpx_codec_frame_buffer_t *fb) {
    EXPECT_EQ(VPX_CODEC_FRAME_BUFFER_PLANES, fb->type);
    ExternalFramePlanesMD5Test *const md5Test =
        reinterpret_cast<ExternalFramePlanesMD5Test*>(priv);
...
TEST_F(ExternalFrameBufferPlanesTest, MinFrameBuffers) {
  ...
  ASSERT_EQ(VPX_CODEC_OK,
            SetFrameBufferFunctions(num_buffers,
...
TEST_F(ExternalFrameBufferPlanesTest, EightJitterBuffers) {
  ...
  ASSERT_EQ(VPX_CODEC_OK,
            SetFrameBufferFunctions(
                num_buffers, get_vp9_frame_buffer_planes,
                release_vp9_frame_buffer_planes));
  ASSERT_EQ(VPX_CODEC_OK, DecodeRemainingFrames());
}
...
TEST_F(ExternalFrameBufferPlanesTest, InvalidAlignement) {
  const int num_buffers =
