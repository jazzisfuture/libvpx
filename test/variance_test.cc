...
void MainTestClass<VarianceFunctionType>::ZeroTest() {
  const int shift = byte_shift();
  for (int i = 0; i <= 255; ++i) {
    if (!use_high_bit_depth()) {
      memset(src_, i, block_size());
#if CONFIG_VP9_HIGHBITDEPTH
    } else {
      vpx_memset16(CONVERT_TO_SHORTPTR(src_), i << shift, block_size());
#endif  // CONFIG_VP9_HIGHBITDEPTH
    }
    for (int j = 0; j <= 255; ++j) {
      if (!use_high_bit_depth()) {
        ...
        vpx_memset16(CONVERT_TO_SHORTPTR(ref_), j << shift, block_size());
#endif  // CONFIG_VP9_HIGHBITDEPTH
      }
...
void MainTestClass<VarianceFunctionType>::OneQuarterTest() {
  ...
  if (!use_high_bit_depth()) {
    ...
    vpx_memset16(CONVERT_TO_SHORTPTR(src_), 255 << shift, block_size());
    vpx_memset16(CONVERT_TO_SHORTPTR(ref_), 255 << shift, half);
