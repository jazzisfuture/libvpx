...
class VarianceTest
 ...
 public:
  virtual void SetUp() {
#if CONFIG_VP9_ENCODER
    const tuple<int, int, VarianceFunctionType, VarianceFunctionType,
                int>& params = this->GetParam();
#else
...
#if CONFIG_VP9_ENCODER
    comp_variance_ = get<3>(params);
    if (get<4>(params)) {
      bit_depth_ = (vpx_bit_depth_t) get<4>(params);
...
#if CONFIG_VP9_ENCODER
  VarianceFunctionType comp_variance_;
#endif
};
...
#if CONFIG_VP9_ENCODER
    const unsigned int var3 = comp_variance_(src_, width_, ref_, width_, &sse3);
    EXPECT_EQ(sse1, sse3);
    EXPECT_EQ(var1, var3);
#endif
    EXPECT_EQ(sse1, sse2);
...
#if CONFIG_VP9_HIGHBITDEPTH
template<typename VarianceFunctionType>
void VarianceTest<VarianceFunctionType>::RefComparisonTest() {
  for (int i = 0; i < 10; ++i) {
    for (int j = 0; j < block_size_; j++) {
      if (!use_high_bit_depth_) {
        src_[j] = rnd_.Rand8();
    ...
    unsigned int var1;
    ASM_REGISTER_STATE_CHECK(var1 = variance_(src_, width_,
                                              ref_, width_, &sse1));
    const unsigned int var2 = comp_variance_(src_, width_, ref_, width_, &sse2);
...
void VarianceTest<VarianceFunctionType>::RefStrideTest() {
  for (int i = 0; i < 10; ++i) {
    ...
    const unsigned int var2 = variance_ref(src_, ref_, log2width_,
                                           ...
                                           use_high_bit_depth_, bit_depth_);
#if CONFIG_VP9_ENCODER
    const unsigned int var3 = comp_variance_(src_, width_ * src_stride_coeff,
                                             ref_, width_ * ref_stride_coeff,
                                             &sse3);
    EXPECT_EQ(sse1, sse3);
    EXPECT_EQ(var1, var3);
...
class SubpelVarianceTest
 ...
 public:
  virtual void SetUp() {
    const tuple<int, int, SubpelVarianceFunctionType,
                SubpelVarianceFunctionType, int>& params =
        this->GetParam();
    log2width_  = get<0>(params);
    width_ = 1 << log2width_;
    log2height_ = get<1>(params);
    ...
    comp_subpel_variance_ = get<3>(params);
    if (get<4>(params)) {
      bit_depth_ = (vpx_bit_depth_t) get<4>(params);
      use_high_bit_depth_ = true;
    } else {
 ...
 protected:
  ...
  SubpelVarianceFunctionType comp_subpel_variance_;
};
  ...
  for (int x = 0; x < 16; ++x) {
    for (int y = 0; y < 16; ++y) {
      ...
      const unsigned int var2 = subpel_variance_ref(ref_, src_, log2width_,
                                                    ...
                                                    use_high_bit_depth_,
                                                    bit_depth_);
      const unsigned int var3 = comp_subpel_variance_(ref_, width_ + 1, x, y,
                                                      src_, width_, &sse3);
      EXPECT_EQ(sse1, sse3) << "at position " << x << ", " << y;
      EXPECT_EQ(var1, var3) << "at position " << x << ", " << y;
...
INSTANTIATE_TEST_CASE_P(
    C, VP8VarianceTest,
    ::testing::Values(make_tuple(2, 2, variance4x4_c, variance4x4_c, 0),
                      make_tuple(3, 3, variance8x8_c, variance8x8_c, 0),
                      make_tuple(3, 4, variance8x16_c, variance8x16_c, 0),
                      make_tuple(4, 3, variance16x8_c, variance16x8_c, 0),
                      make_tuple(4, 4, variance16x16_c, variance16x16_c, 0)));
...
INSTANTIATE_TEST_CASE_P(
    NEON, VP8MseTest,
    ::testing::Values(make_tuple(4, 4, mse16x16_neon, mse16x16_neon, 0)));
...
INSTANTIATE_TEST_CASE_P(
    MMX, VP8VarianceTest,
    ::testing::Values(make_tuple(2, 2, variance4x4_mmx, variance4x4_mmx, 0),
                      make_tuple(3, 3, variance8x8_mmx, variance8x8_mmx, 0),
                      make_tuple(3, 4, variance8x16_mmx, variance8x16_mmx, 0),
                      make_tuple(4, 3, variance16x8_mmx, variance16x8_mmx, 0),
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(make_tuple(2, 2, variance4x4_wmt, variance4x4_wmt, 0),
                      make_tuple(3, 3, variance8x8_wmt, variance8x8_wmt, 0),
                      make_tuple(3, 4, variance8x16_wmt, variance8x16_wmt, 0),
                      make_tuple(4, 3, variance16x8_wmt, variance16x8_wmt, 0),
...
#if CONFIG_VP9_HIGHBITDEPTH
TEST_P(VP9VarianceTest, RefComparison) { RefComparisonTest(); }
#endif
...
INSTANTIATE_TEST_CASE_P(
    C, VP9VarianceTest,
    ::testing::Values(make_tuple(2, 2, variance4x4_c, variance4x4_c, 0),
                      make_tuple(2, 3, variance4x8_c, variance4x8_c, 0),
                      make_tuple(3, 2, variance8x4_c, variance8x4_c, 0),
                      make_tuple(3, 3, variance8x8_c, variance8x8_c, 0),
                      make_tuple(3, 4, variance8x16_c, variance8x16_c, 0),
                      make_tuple(4, 3, variance16x8_c, variance16x8_c, 0),
                      make_tuple(4, 4, variance16x16_c, variance16x16_c, 0),
                      make_tuple(4, 5, variance16x32_c, variance16x32_c, 0),
                      make_tuple(5, 4, variance32x16_c, variance32x16_c, 0),
                      make_tuple(5, 5, variance32x32_c, variance32x32_c, 0),
                      make_tuple(5, 6, variance32x64_c, variance32x64_c, 0),
                      make_tuple(6, 5, variance64x32_c, variance64x32_c, 0),
                      make_tuple(6, 6, variance64x64_c, variance64x64_c, 0)));
#if CONFIG_VP9_HIGHBITDEPTH
...
INSTANTIATE_TEST_CASE_P(
    C_8, VP9VarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_variance4x4_c,
                                 highbd_variance4x4_c, 8),
                      make_tuple(2, 3, highbd_variance4x8_c,
                                 highbd_variance4x8_c, 8),
                      make_tuple(3, 2, highbd_variance8x4_c,
                                 highbd_variance8x4_c, 8),
                      make_tuple(3, 3, highbd_variance8x8_c,
                                 highbd_variance8x8_c, 8),
                      make_tuple(3, 4, highbd_variance8x16_c,
                                 highbd_variance8x16_c, 8),
                      make_tuple(4, 3, highbd_variance16x8_c,
                                 highbd_variance16x8_c, 8),
                      make_tuple(4, 4, highbd_variance16x16_c,
                                 highbd_variance16x16_c, 8),
                      make_tuple(4, 5, highbd_variance16x32_c,
                                 highbd_variance16x32_c, 8),
                      make_tuple(5, 4, highbd_variance32x16_c,
                                 highbd_variance32x16_c, 8),
                      make_tuple(5, 5, highbd_variance32x32_c,
                                 highbd_variance32x32_c, 8),
                      make_tuple(5, 6, highbd_variance32x64_c,
                                 highbd_variance32x64_c, 8),
                      make_tuple(6, 5, highbd_variance64x32_c,
                                 highbd_variance64x32_c, 8),
                      make_tuple(6, 6, highbd_variance64x64_c,
                                 highbd_variance64x64_c, 8)));
...
INSTANTIATE_TEST_CASE_P(
    C_10, VP9VarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_10_variance4x4_c,
                                 highbd_10_variance4x4_c, 10),
                      make_tuple(2, 3, highbd_10_variance4x8_c,
                                 highbd_10_variance4x8_c, 10),
                      make_tuple(3, 2, highbd_10_variance8x4_c,
                                 highbd_10_variance8x4_c, 10),
                      make_tuple(3, 3, highbd_10_variance8x8_c,
                                 highbd_10_variance8x8_c, 10),
                      make_tuple(3, 4, highbd_10_variance8x16_c,
                                 highbd_10_variance8x16_c, 10),
                      make_tuple(4, 3, highbd_10_variance16x8_c,
                                 highbd_10_variance16x8_c, 10),
                      make_tuple(4, 4, highbd_10_variance16x16_c,
                                 highbd_10_variance16x16_c, 10),
                      make_tuple(4, 5, highbd_10_variance16x32_c,
                                 highbd_10_variance16x32_c, 10),
                      make_tuple(5, 4, highbd_10_variance32x16_c,
                                 highbd_10_variance32x16_c, 10),
                      make_tuple(5, 5, highbd_10_variance32x32_c,
                                 highbd_10_variance32x32_c, 10),
                      make_tuple(5, 6, highbd_10_variance32x64_c,
                                 highbd_10_variance32x64_c, 10),
                      make_tuple(6, 5, highbd_10_variance64x32_c,
                                 highbd_10_variance64x32_c, 10),
                      make_tuple(6, 6, highbd_10_variance64x64_c,
                                 highbd_10_variance64x64_c, 10)));
...
INSTANTIATE_TEST_CASE_P(
    C_12, VP9VarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_12_variance4x4_c,
                                 highbd_12_variance4x4_c, 12),
                      make_tuple(2, 3, highbd_12_variance4x8_c,
                                 highbd_12_variance4x8_c, 12),
                      make_tuple(3, 2, highbd_12_variance8x4_c,
                                 highbd_12_variance8x4_c, 12),
                      make_tuple(3, 3, highbd_12_variance8x8_c,
                                 highbd_12_variance8x8_c, 12),
                      make_tuple(3, 4, highbd_12_variance8x16_c,
                                 highbd_12_variance8x16_c, 12),
                      make_tuple(4, 3, highbd_12_variance16x8_c,
                                 highbd_12_variance16x8_c, 12),
                      make_tuple(4, 4, highbd_12_variance16x16_c,
                                 highbd_12_variance16x16_c, 12),
                      make_tuple(4, 5, highbd_12_variance16x32_c,
                                 highbd_12_variance16x32_c, 12),
                      make_tuple(5, 4, highbd_12_variance32x16_c,
                                 highbd_12_variance32x16_c, 12),
                      make_tuple(5, 5, highbd_12_variance32x32_c,
                                 highbd_12_variance32x32_c, 12),
                      make_tuple(5, 6, highbd_12_variance32x64_c,
                                 highbd_12_variance32x64_c, 12),
                      make_tuple(6, 5, highbd_12_variance64x32_c,
                                 highbd_12_variance64x32_c, 12),
                      make_tuple(6, 6, highbd_12_variance64x64_c,
                                 highbd_12_variance64x64_c, 12)));
...
INSTANTIATE_TEST_CASE_P(
    C_8, VP9SubpelVarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_subpel_variance4x4_c,
                                 highbd_subpel_variance4x4_c, 8),
                      make_tuple(2, 3, highbd_subpel_variance4x8_c,
                                 highbd_subpel_variance4x8_c, 8),
                      make_tuple(3, 2, highbd_subpel_variance8x4_c,
                                 highbd_subpel_variance8x4_c, 8),
                      make_tuple(3, 3, highbd_subpel_variance8x8_c,
                                 highbd_subpel_variance8x8_c, 8),
                      make_tuple(3, 4, highbd_subpel_variance8x16_c,
                                 highbd_subpel_variance8x16_c, 8),
                      make_tuple(4, 3, highbd_subpel_variance16x8_c,
                                 highbd_subpel_variance16x8_c, 8),
                      make_tuple(4, 4, highbd_subpel_variance16x16_c,
                                 highbd_subpel_variance16x16_c, 8),
                      make_tuple(4, 5, highbd_subpel_variance16x32_c,
                                 highbd_subpel_variance16x32_c, 8),
                      make_tuple(5, 4, highbd_subpel_variance32x16_c,
                                 highbd_subpel_variance32x16_c, 8),
                      make_tuple(5, 5, highbd_subpel_variance32x32_c,
                                 highbd_subpel_variance32x32_c, 8),
                      make_tuple(5, 6, highbd_subpel_variance32x64_c,
                                 highbd_subpel_variance32x64_c, 8),
                      make_tuple(6, 5, highbd_subpel_variance64x32_c,
                                 highbd_subpel_variance64x32_c, 8),
                      make_tuple(6, 6, highbd_subpel_variance64x64_c,
                                 highbd_subpel_variance64x64_c, 8)));
...
INSTANTIATE_TEST_CASE_P(
    C_8, VP9SubpelAvgVarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_subpel_avg_variance4x4_c,
                                 highbd_subpel_avg_variance4x4_c, 8),
                      make_tuple(2, 3, highbd_subpel_avg_variance4x8_c,
                                 highbd_subpel_avg_variance4x8_c, 8),
                      make_tuple(3, 2, highbd_subpel_avg_variance8x4_c,
                                 highbd_subpel_avg_variance8x4_c, 8),
                      make_tuple(3, 3, highbd_subpel_avg_variance8x8_c,
                                 highbd_subpel_avg_variance8x8_c, 8),
                      make_tuple(3, 4, highbd_subpel_avg_variance8x16_c,
                                 highbd_subpel_avg_variance8x16_c, 8),
                      make_tuple(4, 3, highbd_subpel_avg_variance16x8_c,
                                 highbd_subpel_avg_variance16x8_c, 8),
                      make_tuple(4, 4, highbd_subpel_avg_variance16x16_c,
                                 highbd_subpel_avg_variance16x16_c, 8),
                      make_tuple(4, 5, highbd_subpel_avg_variance16x32_c,
                                 highbd_subpel_avg_variance16x32_c, 8),
                      make_tuple(5, 4, highbd_subpel_avg_variance32x16_c,
                                 highbd_subpel_avg_variance32x16_c, 8),
                      make_tuple(5, 5, highbd_subpel_avg_variance32x32_c,
                                 highbd_subpel_avg_variance32x32_c, 8),
                      make_tuple(5, 6, highbd_subpel_avg_variance32x64_c,
                                 highbd_subpel_avg_variance32x64_c, 8),
                      make_tuple(6, 5, highbd_subpel_avg_variance64x32_c,
                                 highbd_subpel_avg_variance64x32_c, 8),
                      make_tuple(6, 6, highbd_subpel_avg_variance64x64_c,
                                 highbd_subpel_avg_variance64x64_c, 8)));
...
INSTANTIATE_TEST_CASE_P(
    C_10, VP9SubpelVarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_10_subpel_variance4x4_c,
                                 highbd_10_subpel_variance4x4_c, 10),
                      make_tuple(2, 3, highbd_10_subpel_variance4x8_c,
                                 highbd_10_subpel_variance4x8_c, 10),
                      make_tuple(3, 2, highbd_10_subpel_variance8x4_c,
                                 highbd_10_subpel_variance8x4_c, 10),
                      make_tuple(3, 3, highbd_10_subpel_variance8x8_c,
                                 highbd_10_subpel_variance8x8_c, 10),
                      make_tuple(3, 4, highbd_10_subpel_variance8x16_c,
                                 highbd_10_subpel_variance8x16_c, 10),
                      make_tuple(4, 3, highbd_10_subpel_variance16x8_c,
                                 highbd_10_subpel_variance16x8_c, 10),
                      make_tuple(4, 4, highbd_10_subpel_variance16x16_c,
                                 highbd_10_subpel_variance16x16_c, 10),
                      make_tuple(4, 5, highbd_10_subpel_variance16x32_c,
                                 highbd_10_subpel_variance16x32_c, 10),
                      make_tuple(5, 4, highbd_10_subpel_variance32x16_c,
                                 highbd_10_subpel_variance32x16_c, 10),
                      make_tuple(5, 5, highbd_10_subpel_variance32x32_c,
                                 highbd_10_subpel_variance32x32_c, 10),
                      make_tuple(5, 6, highbd_10_subpel_variance32x64_c,
                                 highbd_10_subpel_variance32x64_c, 10),
                      make_tuple(6, 5, highbd_10_subpel_variance64x32_c,
                                 highbd_10_subpel_variance64x32_c, 10),
                      make_tuple(6, 6, highbd_10_subpel_variance64x64_c,
                                 highbd_10_subpel_variance64x64_c, 10)));
...
INSTANTIATE_TEST_CASE_P(
    C_12, VP9SubpelVarianceTest,
    ::testing::Values(make_tuple(2, 2, highbd_12_subpel_variance4x4_c,
                                 highbd_12_subpel_variance4x4_c, 12),
                      make_tuple(2, 3, highbd_12_subpel_variance4x8_c,
                                 highbd_12_subpel_variance4x8_c, 12),
                      make_tuple(3, 2, highbd_12_subpel_variance8x4_c,
                                 highbd_12_subpel_variance8x4_c, 12),
                      make_tuple(3, 3, highbd_12_subpel_variance8x8_c,
                                 highbd_12_subpel_variance8x8_c, 12),
                      make_tuple(3, 4, highbd_12_subpel_variance8x16_c,
                                 highbd_12_subpel_variance8x16_c, 12),
                      make_tuple(4, 3, highbd_12_subpel_variance16x8_c,
                                 highbd_12_subpel_variance16x8_c, 12),
                      make_tuple(4, 4, highbd_12_subpel_variance16x16_c,
                                 highbd_12_subpel_variance16x16_c, 12),
                      make_tuple(4, 5, highbd_12_subpel_variance16x32_c,
                                 highbd_12_subpel_variance16x32_c, 12),
                      make_tuple(5, 4, highbd_12_subpel_variance32x16_c,
                                 highbd_12_subpel_variance32x16_c, 12),
                      make_tuple(5, 5, highbd_12_subpel_variance32x32_c,
                                 highbd_12_subpel_variance32x32_c, 12),
                      make_tuple(5, 6, highbd_12_subpel_variance32x64_c,
                                 highbd_12_subpel_variance32x64_c, 12),
                      make_tuple(6, 5, highbd_12_subpel_variance64x32_c,
                                 highbd_12_subpel_variance64x32_c, 12),
                      make_tuple(6, 6, highbd_12_subpel_variance64x64_c,
                                 highbd_12_subpel_variance64x64_c, 12)));
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(make_tuple(2, 2, variance4x4_sse2, variance4x4_c, 0),
                      make_tuple(2, 3, variance4x8_sse2, variance4x8_c, 0),
                      make_tuple(3, 2, variance8x4_sse2, variance8x4_c, 0),
                      make_tuple(3, 3, variance8x8_sse2, variance8x8_c, 0),
                      make_tuple(3, 4, variance8x16_sse2, variance8x16_c, 0),
                      make_tuple(4, 3, variance16x8_sse2, variance16x8_c, 0),
                      make_tuple(4, 4, variance16x16_sse2, variance16x16_c, 0),
                      make_tuple(4, 5, variance16x32_sse2, variance16x32_c, 0),
                      make_tuple(5, 4, variance32x16_sse2, variance32x16_c, 0),
                      make_tuple(5, 5, variance32x32_sse2, variance32x32_c, 0),
                      make_tuple(5, 6, variance32x64_sse2, variance32x64_c, 0),
                      make_tuple(6, 5, variance64x32_sse2, variance64x32_c, 0),
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(make_tuple(2, 2, subpel_avg_variance4x4_sse,
                      ...
                      make_tuple(6, 6, subpel_avg_variance64x64_sse2,
                                 subpel_avg_variance64x64_c, 0)));
#if CONFIG_VP9_HIGHBITDEPTH
const vp9_variance_fn_t highbd_variance8x8_sse2 =
...
const vp9_variance_fn_t highbd_variance8x16_sse2 =
    vp9_highbd_variance8x16_sse2;
...
INSTANTIATE_TEST_CASE_P(
    SSE2_8, VP9VarianceTest,
    ::testing::Values(make_tuple(3, 3, highbd_variance8x8_sse2,
                                 highbd_variance8x8_c, 8),
                      make_tuple(3, 4, highbd_variance8x16_sse2,
                                 highbd_variance8x16_c, 8),
                      make_tuple(4, 3, highbd_variance16x8_sse2,
                                 highbd_variance16x8_c, 8),
                      make_tuple(4, 4, highbd_variance16x16_sse2,
                                 highbd_variance16x16_c, 8),
                      make_tuple(4, 5, highbd_variance16x32_sse2,
                                 highbd_variance16x32_c, 8),
                      make_tuple(5, 4, highbd_variance32x16_sse2,
                                 highbd_variance32x16_c, 8),
                      make_tuple(5, 5, highbd_variance32x32_sse2,
                                 highbd_variance32x32_c, 8),
                      make_tuple(5, 6, highbd_variance32x64_sse2,
                                 highbd_variance32x64_c, 8),
                      make_tuple(6, 5, highbd_variance64x32_sse2,
                                 highbd_variance64x32_c, 8),
                      make_tuple(6, 6, highbd_variance64x64_sse2,
                                 highbd_variance64x64_c, 8)));
INSTANTIATE_TEST_CASE_P(
    SSE2_10, VP9VarianceTest,
    ::testing::Values(make_tuple(3, 3, highbd_10_variance8x8_sse2,
                                 highbd_10_variance8x8_c, 10),
                      make_tuple(3, 4, highbd_10_variance8x16_sse2,
                                 highbd_10_variance8x16_c, 10),
                      make_tuple(4, 3, highbd_10_variance16x8_sse2,
                                 highbd_10_variance16x8_c, 10),
                      make_tuple(4, 4, highbd_10_variance16x16_sse2,
                                 highbd_10_variance16x16_c, 10),
                      make_tuple(4, 5, highbd_10_variance16x32_sse2,
                                 highbd_10_variance16x32_c, 10),
                      make_tuple(5, 4, highbd_10_variance32x16_sse2,
                                 highbd_10_variance32x16_c, 10),
                      make_tuple(5, 5, highbd_10_variance32x32_sse2,
                                 highbd_10_variance32x32_c, 10),
                      make_tuple(5, 6, highbd_10_variance32x64_sse2,
                                 highbd_10_variance32x64_c, 10),
                      make_tuple(6, 5, highbd_10_variance64x32_sse2,
                                 highbd_10_variance64x32_c, 10),
                      make_tuple(6, 6, highbd_10_variance64x64_sse2,
                                 highbd_10_variance64x64_c, 10)));
INSTANTIATE_TEST_CASE_P(
    SSE2_12, VP9VarianceTest,
    ::testing::Values(make_tuple(3, 3, highbd_12_variance8x8_sse2,
                                 highbd_12_variance8x8_c, 12),
                      make_tuple(3, 4, highbd_12_variance8x16_sse2,
                                 highbd_12_variance8x16_c, 12),
                      make_tuple(4, 3, highbd_12_variance16x8_sse2,
                                 highbd_12_variance16x8_c, 12),
                      make_tuple(4, 4, highbd_12_variance16x16_sse2,
                                 highbd_12_variance16x16_c, 12),
                      make_tuple(4, 5, highbd_12_variance16x32_sse2,
                                 highbd_12_variance16x32_c, 12),
                      make_tuple(5, 4, highbd_12_variance32x16_sse2,
                                 highbd_12_variance32x16_c, 12),
                      make_tuple(5, 5, highbd_12_variance32x32_sse2,
                                 highbd_12_variance32x32_c, 12),
                      make_tuple(5, 6, highbd_12_variance32x64_sse2,
                                 highbd_12_variance32x64_c, 12),
                      make_tuple(6, 5, highbd_12_variance64x32_sse2,
                                 highbd_12_variance64x32_c, 12),
                      make_tuple(6, 6, highbd_12_variance64x64_sse2,
                                 highbd_12_variance64x64_c, 12)));
...
INSTANTIATE_TEST_CASE_P(
    SSE2_8, VP9SubpelVarianceTest,
    ::testing::Values(make_tuple(3, 2, highbd_subpel_variance8x4_sse2,
                                 highbd_subpel_variance8x4_c, 8),
                      make_tuple(3, 3, highbd_subpel_variance8x8_sse2,
                                 highbd_subpel_variance8x8_c, 8),
                      make_tuple(3, 4, highbd_subpel_variance8x16_sse2,
                                 highbd_subpel_variance8x16_c, 8),
                      make_tuple(4, 3, highbd_subpel_variance16x8_sse2,
                                 highbd_subpel_variance16x8_c, 8),
                      make_tuple(4, 4, highbd_subpel_variance16x16_sse2,
                                 highbd_subpel_variance16x16_c, 8),
                      make_tuple(4, 5, highbd_subpel_variance16x32_sse2,
                                 highbd_subpel_variance16x32_c, 8),
                      make_tuple(5, 4, highbd_subpel_variance32x16_sse2,
                                 highbd_subpel_variance32x16_c, 8),
                      make_tuple(5, 5, highbd_subpel_variance32x32_sse2,
                                 highbd_subpel_variance32x32_c, 8),
                      make_tuple(5, 6, highbd_subpel_variance32x64_sse2,
                                 highbd_subpel_variance32x64_c, 8),
                      make_tuple(6, 5, highbd_subpel_variance64x32_sse2,
                                 highbd_subpel_variance64x32_c, 8),
                      make_tuple(6, 6, highbd_subpel_variance64x64_sse2,
                                 highbd_subpel_variance64x64_c, 8)));
...
INSTANTIATE_TEST_CASE_P(
    SSE2_10, VP9SubpelVarianceTest,
    ::testing::Values(make_tuple(3, 2, highbd_10_subpel_variance8x4_sse2,
                                 highbd_10_subpel_variance8x4_c, 10),
                      make_tuple(3, 3, highbd_10_subpel_variance8x8_sse2,
                                 highbd_10_subpel_variance8x8_c, 10),
                      make_tuple(3, 4, highbd_10_subpel_variance8x16_sse2,
                                 highbd_10_subpel_variance8x16_c, 10),
                      make_tuple(4, 3, highbd_10_subpel_variance16x8_sse2,
                                 highbd_10_subpel_variance16x8_c, 10),
                      make_tuple(4, 4, highbd_10_subpel_variance16x16_sse2,
                                 highbd_10_subpel_variance16x16_c, 10),
                      make_tuple(4, 5, highbd_10_subpel_variance16x32_sse2,
                                 highbd_10_subpel_variance16x32_c, 10),
                      make_tuple(5, 4, highbd_10_subpel_variance32x16_sse2,
                                 highbd_10_subpel_variance32x16_c, 10),
                      make_tuple(5, 5, highbd_10_subpel_variance32x32_sse2,
                                 highbd_10_subpel_variance32x32_c, 10),
                      make_tuple(5, 6, highbd_10_subpel_variance32x64_sse2,
                                 highbd_10_subpel_variance32x64_c, 10),
                      make_tuple(6, 5, highbd_10_subpel_variance64x32_sse2,
                                 highbd_10_subpel_variance64x32_c, 10),
                      make_tuple(6, 6, highbd_10_subpel_variance64x64_sse2,
                                 highbd_10_subpel_variance64x64_c, 10)));
...
INSTANTIATE_TEST_CASE_P(
    SSE2_12, VP9SubpelVarianceTest,
    ::testing::Values(make_tuple(3, 2, highbd_12_subpel_variance8x4_sse2,
                                 highbd_12_subpel_variance8x4_c, 12),
                      make_tuple(3, 3, highbd_12_subpel_variance8x8_sse2,
                                 highbd_12_subpel_variance8x8_c, 12),
                      make_tuple(3, 4, highbd_12_subpel_variance8x16_sse2,
                                 highbd_12_subpel_variance8x16_c, 12),
                      make_tuple(4, 3, highbd_12_subpel_variance16x8_sse2,
                                 highbd_12_subpel_variance16x8_c, 12),
                      make_tuple(4, 4, highbd_12_subpel_variance16x16_sse2,
                                 highbd_12_subpel_variance16x16_c, 12),
                      make_tuple(4, 5, highbd_12_subpel_variance16x32_sse2,
                                 highbd_12_subpel_variance16x32_c, 12),
                      make_tuple(5, 4, highbd_12_subpel_variance32x16_sse2,
                                 highbd_12_subpel_variance32x16_c, 12),
                      make_tuple(5, 5, highbd_12_subpel_variance32x32_sse2,
                                 highbd_12_subpel_variance32x32_c, 12),
                      make_tuple(5, 6, highbd_12_subpel_variance32x64_sse2,
                                 highbd_12_subpel_variance32x64_c, 12),
                      make_tuple(6, 5, highbd_12_subpel_variance64x32_sse2,
                                 highbd_12_subpel_variance64x32_c, 12),
                      make_tuple(6, 6, highbd_12_subpel_variance64x64_sse2,
                                 highbd_12_subpel_variance64x64_c, 12)));
...
INSTANTIATE_TEST_CASE_P(
    SSE2_8, VP9SubpelAvgVarianceTest,
    ::testing::Values(make_tuple(3, 2, highbd_subpel_avg_variance8x4_sse2,
                                 highbd_subpel_avg_variance8x4_c, 8),
                      make_tuple(3, 3, highbd_subpel_avg_variance8x8_sse2,
                                 highbd_subpel_avg_variance8x8_c, 8),
                      make_tuple(3, 4, highbd_subpel_avg_variance8x16_sse2,
                                 highbd_subpel_avg_variance8x16_c, 8),
                      make_tuple(4, 3, highbd_subpel_avg_variance16x8_sse2,
                                 highbd_subpel_avg_variance16x8_c, 8),
                      make_tuple(4, 4, highbd_subpel_avg_variance16x16_sse2,
                                 highbd_subpel_avg_variance16x16_c, 8),
                      make_tuple(4, 5, highbd_subpel_avg_variance16x32_sse2,
                                 highbd_subpel_avg_variance16x32_c, 8),
                      make_tuple(5, 4, highbd_subpel_avg_variance32x16_sse2,
                                 highbd_subpel_avg_variance32x16_c, 8),
                      make_tuple(5, 5, highbd_subpel_avg_variance32x32_sse2,
                                 highbd_subpel_avg_variance32x32_c, 8),
                      make_tuple(5, 6, highbd_subpel_avg_variance32x64_sse2,
                                 highbd_subpel_avg_variance32x64_c, 8),
                      make_tuple(6, 5, highbd_subpel_avg_variance64x32_sse2,
                                 highbd_subpel_avg_variance64x32_c, 8),
                      make_tuple(6, 6, highbd_subpel_avg_variance64x64_sse2,
                                 highbd_subpel_avg_variance64x64_c, 8)));
...
INSTANTIATE_TEST_CASE_P(
    SSE2_10, VP9SubpelAvgVarianceTest,
    ::testing::Values(make_tuple(3, 2, highbd_10_subpel_avg_variance8x4_sse2,
                                 highbd_10_subpel_avg_variance8x4_c, 10),
                      make_tuple(3, 3, highbd_10_subpel_avg_variance8x8_sse2,
                                 highbd_10_subpel_avg_variance8x8_c, 10),
                      make_tuple(3, 4, highbd_10_subpel_avg_variance8x16_sse2,
                                 highbd_10_subpel_avg_variance8x16_c, 10),
                      make_tuple(4, 3, highbd_10_subpel_avg_variance16x8_sse2,
                                 highbd_10_subpel_avg_variance16x8_c, 10),
                      make_tuple(4, 4, highbd_10_subpel_avg_variance16x16_sse2,
                                 highbd_10_subpel_avg_variance16x16_c, 10),
                      make_tuple(4, 5, highbd_10_subpel_avg_variance16x32_sse2,
                                 highbd_10_subpel_avg_variance16x32_c, 10),
                      make_tuple(5, 4, highbd_10_subpel_avg_variance32x16_sse2,
                                 highbd_10_subpel_avg_variance32x16_c, 10),
                      make_tuple(5, 5, highbd_10_subpel_avg_variance32x32_sse2,
                                 highbd_10_subpel_avg_variance32x32_c, 10),
                      make_tuple(5, 6, highbd_10_subpel_avg_variance32x64_sse2,
                                 highbd_10_subpel_avg_variance32x64_c, 10),
                      make_tuple(6, 5, highbd_10_subpel_avg_variance64x32_sse2,
                                 highbd_10_subpel_avg_variance64x32_c, 10),
                      make_tuple(6, 6, highbd_10_subpel_avg_variance64x64_sse2,
                                 highbd_10_subpel_avg_variance64x64_c, 10)));
...
INSTANTIATE_TEST_CASE_P(
    SSE2_12, VP9SubpelAvgVarianceTest,
    ::testing::Values(make_tuple(3, 2, highbd_12_subpel_avg_variance8x4_sse2,
                                 highbd_12_subpel_avg_variance8x4_c, 12),
                      make_tuple(3, 3, highbd_12_subpel_avg_variance8x8_sse2,
                                 highbd_12_subpel_avg_variance8x8_c, 12),
                      make_tuple(3, 4, highbd_12_subpel_avg_variance8x16_sse2,
                                 highbd_12_subpel_avg_variance8x16_c, 12),
                      make_tuple(4, 3, highbd_12_subpel_avg_variance16x8_sse2,
                                 highbd_12_subpel_avg_variance16x8_c, 12),
                      make_tuple(4, 4, highbd_12_subpel_avg_variance16x16_sse2,
                                 highbd_12_subpel_avg_variance16x16_c, 12),
                      make_tuple(4, 5, highbd_12_subpel_avg_variance16x32_sse2,
                                 highbd_12_subpel_avg_variance16x32_c, 12),
                      make_tuple(5, 4, highbd_12_subpel_avg_variance32x16_sse2,
                                 highbd_12_subpel_avg_variance32x16_c, 12),
                      make_tuple(5, 5, highbd_12_subpel_avg_variance32x32_sse2,
                                 highbd_12_subpel_avg_variance32x32_c, 12),
                      make_tuple(5, 6, highbd_12_subpel_avg_variance32x64_sse2,
                                 highbd_12_subpel_avg_variance32x64_c, 12),
                      make_tuple(6, 5, highbd_12_subpel_avg_variance64x32_sse2,
                                 highbd_12_subpel_avg_variance64x32_c, 12),
                      make_tuple(6, 6, highbd_12_subpel_avg_variance64x64_sse2,
                                 highbd_12_subpel_avg_variance64x64_c, 12)));
#endif  // CONFIG_VP9_HIGHBITDEPTH
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(make_tuple(4, 4, variance16x16_avx2, variance16x16_c, 0),
                      make_tuple(5, 4, variance32x16_avx2, variance32x16_c, 0),
                      make_tuple(5, 5, variance32x32_avx2, variance32x32_c, 0),
                      make_tuple(6, 5, variance64x32_avx2, variance64x32_c, 0),
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(make_tuple(3, 3, variance8x8_neon, variance8x8_neon, 0),
                      make_tuple(4, 4, variance16x16_neon,
                                 variance16x16_neon, 0),
                      make_tuple(5, 5, variance32x32_neon,
                                 variance32x32_neon, 0)));
