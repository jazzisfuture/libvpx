...
void VarianceTest<VarianceFunctionType>::RefTest() {
  for (int i = 0; i < 10; ++i) {
    ...
    REGISTER_STATE_CHECK(var2 = variance_ref(src_, ref_, log2width_,
                                             log2height_, &sse2));
    EXPECT_EQ(sse1, sse2);
    EXPECT_EQ(var1, var2);
  }
  ...
  for (int x = 0; x < 16; ++x) {
    for (int y = 0; y < 16; ++y) {
      ...
      REGISTER_STATE_CHECK(var1 = subpel_variance_(ref_, width_ + 1, x, y,
                                                   src_, width_, &sse1));
      REGISTER_STATE_CHECK(var2 = subpel_variance_ref(ref_, src_, log2width_,
...
void SubpelVarianceTest<vp9_subp_avg_variance_fn_t>::RefTest() {
  for (int x = 0; x < 16; ++x) {
    for (int y = 0; y < 16; ++y) {
      ...
      unsigned int sse1, sse2;
      unsigned int var1, var2;
      REGISTER_STATE_CHECK(var1 = subpel_variance_(
          ref_, width_ + 1, x, y, src_, width_, &sse1, sec_));
      REGISTER_STATE_CHECK(var2 = subpel_avg_variance_ref(
          ref_, src_, sec_, log2width_, log2height_, x, y, &sse2));
      EXPECT_EQ(sse1, sse2) << "at position " << x << ", " << y;
      EXPECT_EQ(var1, var2) << "at position " << x << ", " << y;
    }
  }
