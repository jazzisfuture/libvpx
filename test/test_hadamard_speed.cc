...
typedef void (*VpxHadamardFunc)(const int16_t *src_diff, int src_stride,
                                int16_t *coeff);
...
void TestHadamard8x8(VpxHadamardFunc const func, int times) {
  ...
  DECLARE_ALIGNED(16, int16_t, output[64]);
  memset(input, 1, sizeof(input));
  TestHadamard("Hadamard8x8", func, input, 8, output, times);
}

void TestHadamard16x16(VpxHadamardFunc const func, int times) {
  DECLARE_ALIGNED(16, int16_t, output[256]);
  DECLARE_ALIGNED(16, int16_t, input[256]);
  memset(input, 1, sizeof(input));
  TestHadamard("Hadamard16x16", func, input, 16, output, times);
}
...
#define HADAMARD_TEST(arch, test_func, hadamard)                               \
  TEST(arch, test_func) { test_func(hadamard); }
