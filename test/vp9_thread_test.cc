...
void DecodeFiles(const FileList files[], int count) {
...
TEST(VP9WorkerThreadTest, TestSerialInterface) {
  ...
  // Avoid using a file that will use the row-based thread loopfilter, it will
  // hang.
...
TEST(VP9DecodeMultiThreadedTest, Decode2) {
  ...
  DecodeFiles(files, static_cast<int>(sizeof(files) / sizeof(files[0])));
}
