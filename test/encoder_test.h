...
public:
  CxDataIterator(vpx_codec_ctx_t &encoder)
...
public:
  ...
  void append(const vpx_codec_cx_pkt_t *pkt) {
    size_t len = pkt->data.twopass_stats.sz;

    if (buf_.sz + len > buf_alloc_sz_) {
      size_t  new_sz = buf_alloc_sz_ + 64 * 1024;
      char   *new_ptr = (char *)realloc(buf_.buf, new_sz);

      ASSERT_NE((char *)NULL, new_ptr);
