...
TEST_P(ConvolveTest, DISABLED_Copy_Speed) {
  ...
  printf("convolve_copy_%dx%d_%d[%8d runs]: %d us\n", width, height,
         UUT_->use_highbd_, tests_num, elapsed_time);
}
...
TEST_P(ConvolveTest, DISABLED_Avg_Speed) {
  ...
  const int elapsed_time = static_cast<int>(vpx_usec_timer_elapsed(&timer));
  printf("convolve_avg_%dx%d_%d[%8d runs]: %d us\n", width, height,
         UUT_->use_highbd_, tests_num, elapsed_time);
}
