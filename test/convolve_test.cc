...
TEST_P(ConvolveTest, ChangeFilterWorks) {
  ...
  for (int x = 0; x < Width(); ++x) {
    ...
    const int ref_x = kPixelSelected +
        ((kInitialSubPelOffset + kFilterPeriodAdjust * kInputPixelStep) >> 4);
  ...
  for (int y = 0; y < Height(); ++y) {
    ...
    const int ref_y = kPixelSelected +
        ((kInitialSubPelOffset + kFilterPeriodAdjust * kInputPixelStep) >> 4);
  ...
  for (int y = 0; y < Height(); ++y) {
    ...
    const int ref_y = kPixelSelected +
        ((kInitialSubPelOffset + kFilterPeriodAdjustY * kInputPixelStep) >> 4);
    for (int x = 0; x < Width(); ++x) {
      const int kFilterPeriodAdjustX = (x >> 3) << 3;
      const int ref_x = kPixelSelected +
         ((kInitialSubPelOffset + kFilterPeriodAdjustX * kInputPixelStep) >> 4);
