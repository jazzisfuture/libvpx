...
static void filter_block2d_8_c(const uint8_t *src_ptr,
  ...
  // Horizontal pass (src -> transposed intermediate).
  {
    ...
    for (i = 0; i < intermediate_height; ++i) {
      for (j = 0; j < output_width; ++j) {
        // Apply filter...
        const int temp = (static_cast<int>(src_ptr[0]) * HFilter[0]) +
                         (static_cast<int>(src_ptr[1]) * HFilter[1]) +
                         (static_cast<int>(src_ptr[2]) * HFilter[2]) +
                         (static_cast<int>(src_ptr[3]) * HFilter[3]) +
                         (static_cast<int>(src_ptr[4]) * HFilter[4]) +
                         (static_cast<int>(src_ptr[5]) * HFilter[5]) +
                         (static_cast<int>(src_ptr[6]) * HFilter[6]) +
                         (static_cast<int>(src_ptr[7]) * HFilter[7]) +
  ...
  // Vertical pass (transposed intermediate -> dst).
  {
    ...
    for (i = 0; i < output_height; ++i) {
      for (j = 0; j < output_width; ++j) {
        // Apply filter...
        const int temp = (static_cast<int>(src_ptr[0]) * VFilter[0]) +
                         (static_cast<int>(src_ptr[1]) * VFilter[1]) +
                         (static_cast<int>(src_ptr[2]) * VFilter[2]) +
                         (static_cast<int>(src_ptr[3]) * VFilter[3]) +
                         (static_cast<int>(src_ptr[4]) * VFilter[4]) +
                         (static_cast<int>(src_ptr[5]) * VFilter[5]) +
                         (static_cast<int>(src_ptr[6]) * VFilter[6]) +
                         (static_cast<int>(src_ptr[7]) * VFilter[7]) +
                         (VP9_FILTER_WEIGHT >> 1);  // Rounding
