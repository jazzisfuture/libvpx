...
class TransTestBase {
  ...
  protected:
    ...
    virtual void RunFwdAccuracyCheck() {
      tran_low_t* input  = (tran_low_t*)malloc(sizeof(tran_low_t)*txfmSize);
      tran_low_t* output = (tran_low_t*)malloc(sizeof(tran_low_t)*txfmSize);
      double* refInput   = (double*)malloc(sizeof(double)*txfmSize);
      double* refOutput  = (double*)malloc(sizeof(double)*txfmSize);
      ...
      for (int ti =  0; ti < count_test_block; ti++) {
        ...
        for (int ni = 0; ni < txfmSize; ni++) {
          EXPECT_LE(abs(output[ni]-(tran_low_t)round(refOutput[ni])), max_error);
        }
      }
