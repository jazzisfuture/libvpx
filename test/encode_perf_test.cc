...
const EncodePerfParam kVP9EncodePerfTestVectors[] = {
...
const int encodePerfTestSpeeds[] = { 5, 6, 7, 12 };
...
TEST_P(VP9EncodePerfTest, PerfTest) {
  for (size_t i = 0; i < LENGTH(kVP9EncodePerfTestVectors); i++) {
    for (size_t j = 0; j < LENGTH(encodePerfTestSpeeds); j++) {
      SetUp();
      ...
      SetSpeed(encodePerfTestSpeeds[j]);
