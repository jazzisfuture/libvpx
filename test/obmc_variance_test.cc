...
TEST_P(ObmcVarianceTest, RandomValues) {
  ...
  for (int iter = 0 ; iter < kIterations && !HasFatalFailure() ; ++iter) {
    ...
    const unsigned int ref_res = params_.ref_func_(pre, pre_stride, wsrc, mask,
                                                   &ref_sse);
    const unsigned int tst_res = params_.tst_func_(pre, pre_stride, wsrc, mask,
                                                   &tst_sse);
...
TEST_P(ObmcVarianceHBDTest, RandomValues) {
  ...
  for (int iter = 0 ; iter < kIterations && !HasFatalFailure() ; ++iter) {
    ...
    unsigned int ref_sse, tst_sse;
    const unsigned int ref_res = params_.ref_func_(
        CONVERT_TO_BYTEPTR(pre), pre_stride, wsrc, mask, &ref_sse);
    const unsigned int tst_res = params_.tst_func_(
        CONVERT_TO_BYTEPTR(pre), pre_stride, wsrc, mask, &tst_sse);
