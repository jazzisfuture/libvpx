...
class VP10HighbdInvTrans4x4HT
 ...
 public:
  ...
  virtual void SetUp() {
    ...
    coeffs_ = reinterpret_cast<int32_t *>(
        vpx_memalign(16, sizeof(coeffs_[0]) * num_coeffs_));
    output_ = reinterpret_cast<uint16_t *>(
        vpx_memalign(16, sizeof(output_[0]) * num_coeffs_));
    output_ref_ = reinterpret_cast<uint16_t *>(
        vpx_memalign(16, sizeof(output_ref_[0]) * num_coeffs_));
  }
 ...
 private:
  int32_t clampCoeffs(int32_t number, int bit) {
