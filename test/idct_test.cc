...
class IDCTTest : public ::testing::TestWithParam<IdctFunc> {
 protected:
  virtual void SetUp() {
    UUT = GetParam();
    input = Buffer<int16_t>(4, 4, 0);
    output = Buffer<uint8_t>(4, 4, 3);
    predict = Buffer<uint8_t>(4, 4, 3);
  }
...
TEST_P(IDCTTest, TestAllZeros) {
  ...
  ASM_REGISTER_STATE_CHECK(UUT(input.Src(), predict.Src(), predict.Stride(), output.Src(), output.Stride()));
