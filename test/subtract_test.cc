...
TEST_P(SubtractBlockTest, SimpleSubtract) {
  ...
  const int kSrcStride = 4;

  // Allocate... align to 16 for mmx/sse tests
  uint8_t *source = reinterpret_cast<uint8_t*>(
      vpx_memalign(16, 4 * kSrcStride * sizeof(*source)));
  be.src_diff = reinterpret_cast<int16_t*>(
      vpx_memalign(16, 4 * kDiffPredStride * sizeof(be.src_diff)));
  bd.predictor = reinterpret_cast<unsigned char*>(
      vpx_memalign(16, 4 * kDiffPredStride * sizeof(bd.predictor)));

  // start at block0
  be.src = 0;
  be.base_src = &source;
  be.src_stride = kSrcStride;
