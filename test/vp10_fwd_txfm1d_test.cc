...
static TxfmFunc fwd_txfm_func_ls[2][4] = {{vp10_fdct4_new, vp10_fdct8_new, vp10_fdct16_new, vp10_fdct32_new},
...
TEST(vp10_fwd_txfm1d, cospi_arr) {
  for(int i = 0; i < 7; i++){
    for(int j = 0; j < 64; j++){
      EXPECT_EQ(cospi_arr[i][j], (int32_t)round(cos(M_PI*j/128) * (1<<(cos_bit_min+i))));
    }
  }
}
