...
class PartialIDctTest : public ::testing::TestWithParam<PartialInvTxfmParam> {
 public:
  ...
  virtual void SetUp() {
    ...
    stride_ = (rnd.Rand16() & 1023) + 1;
    if (stride_ < size_) {
      stride_ = size_;
    }
    // Align stide_ to 16 if it's bigger than 16.
    if (stride_ > 16) {
      stride_ &= ~15;
    }
  ...
  void INLINE Exec(InvTxfmFunc func, void *out) {
    func(input_block_,
#if !CONFIG_VP9_HIGHBITDEPTH
         reinterpret_cast<uint8_t *>(out), stride_
#else
         CONVERT_TO_BYTEPTR(out), stride_, bit_depth_
#endif
         );
  }
...
TEST_P(PartialIDctTest, RunQuantCheck) {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    for (int j = 0; j < count_test_block; ++j) {
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(
        ...
        make_tuple(&vpx_highbd_fdct4x4_c, &vpx_highbd_idct4x4_16_add_c,
                   &vpx_highbd_idct4x4_16_add_c, TX_4X4, 1, 8),
        make_tuple(&vpx_highbd_fdct4x4_c, &vpx_highbd_idct4x4_16_add_c,
                   &vpx_highbd_idct4x4_16_add_c, TX_4X4, 1, 10),
        make_tuple(&vpx_highbd_fdct4x4_c, &vpx_highbd_idct4x4_16_add_c,
                   &vpx_highbd_idct4x4_16_add_c, TX_4X4, 1, 12)));
#endif  // HAVE_SSE2 && !CONFIG_EMULATE_HARDWARE
