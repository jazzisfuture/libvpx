...
TEST_P(PartialIDctTest, RunQuantCheck) {
  ...
  for (int i = 0; i < count_test_block; ++i) {
    ...
    ACMRandom rnd(ACMRandom::DeterministicSeed());
    const int count_test_block = 1000;
    DECLARE_ALIGNED_ARRAY(16, int16_t, input_block, block_size);
    DECLARE_ALIGNED_ARRAY(16, int16_t, input_extreme_block, block_size);
    DECLARE_ALIGNED_ARRAY(16, int16_t, output_ref_block, block_size);
