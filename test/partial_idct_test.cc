    ...
    if (memcmp(output_block_ref_, output_block_,
      ...
      for (int y = 0; y < size_; y++) {
        for (int x = 0; x < size_; x++) {
          if (pixel_size_ == 1) {
            ref = ((uint8_t *)output_block_ref_)[y * stride_ + x];
            opt = ((uint8_t *)output_block_)[y * stride_ + x];
          } else {
            ref = ((uint16_t *)output_block_ref_)[y * stride_ + x];
            opt = ((uint16_t *)output_block_)[y * stride_ + x];
          }
          if (ref != opt) {
...
TEST_P(PartialIDctTest, RunQuantCheck) {
  const int countTestBlock = (size_ != 4) ? kCountTestBlock : 65536;
