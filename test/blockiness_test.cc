  ...
  void FillCheckerboard(uint8_t *data, int stride) {
    for (int h = 0; h < height_; h += 4) {
      for (int w = 0; w < width_; w += 4) {
        ...
        // Checkerboard value produced by xor w and h iterators
        const int checkerboard_value = (((h / 4) ^ (w / 4)) & 1) * 255;
        FillConstant(data + h * stride + w, stride, checkerboard_value, 4, 4);
      }
    }
  }
