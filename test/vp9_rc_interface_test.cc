...
class RcInterfaceTest : public ::testing::Test {
 public:
  explicit RcInterfaceTest() { rc_api_ = vp9::VP9RateControlRTC::Init(); }
 ...
 public:
  void RunOneLayer() {
    ...
    ASSERT_NE(one_layer_file.rdstate() & std::ifstream::failbit, 0);
    for (size_t i = 0; i < kNumFrame; i++) {
      ...
      rc_api_->PostEncodeUpdate(frame_info.bytes_used_);
    }
  }
  ...
  void RunSVC() {
    ...
    ASSERT_NE(svc_file.rdstate() & std::ifstream::failbit, 0);
    for (size_t i = 0; i < kNumFrame * rc_cfg_.ss_number_layers; i++) {
      svc_file >> frame_info;
      if (frame_info.frame_id_ > 0) frame_params.frame_type = INTER_FRAME;
      if (frame_info.frame_id_ == 200 * rc_cfg_.ss_number_layers) {
        for (int layer = 0;
        ...
        rc_api_->UpdateRateControl(rc_cfg_);
      } else if (frame_info.frame_id_ == 400 * rc_cfg_.ss_number_layers) {
        for (int layer = 0;
             layer < rc_cfg_.ss_number_layers * rc_cfg_.ts_number_layers;
             layer++)
