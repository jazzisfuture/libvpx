...
// This test reproduces https://crbug.com/webm/1828.
TEST(EncodeAPI, HugeDurationVp8) {
  vpx_codec_iface_t *const iface = vpx_codec_vp8_cx();
  ...
  ASSERT_EQ(
      vpx_codec_encode(&enc, image, 0, 0xbd6b566b15c7, 0, VPX_DL_BEST_QUALITY),
      VPX_CODEC_INVALID_PARAM);
#else
  ASSERT_EQ(vpx_codec_encode(&enc, image, 0, UINT_MAX, 0, VPX_DL_BEST_QUALITY),
            VPX_CODEC_OK);
#endif
