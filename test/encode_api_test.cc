...
TEST(EncodeAPI, Chromium324459561) {
  ...
  ASSERT_TRUE(encoder.Initialized());
...
TEST(EncodeAPI, Buganizer319964497) {
  VP9Encoder encoder(7);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/1, /*width=*/320, /*height=*/240, VPX_VBR, VPX_DL_REALTIME));
  ASSERT_TRUE(encoder.Initialized());
  encoder.Encode(/*key_frame=*/true);
  encoder.Encode(/*key_frame=*/true);
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/1, /*width=*/1, /*height=*/1, VPX_VBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/1, /*width=*/2, /*height=*/2, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
}
...
TEST(EncodeAPI, Buganizer329088759RowMT0) {
  ...
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/8, /*width=*/1686, /*height=*/398, VPX_VBR, VPX_DL_REALTIME));
  ASSERT_TRUE(encoder.Initialized());
  encoder.Encode(/*key_frame=*/true);
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/0, /*width=*/1686, /*height=*/1, VPX_VBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/0, /*width=*/1482, /*height=*/113, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/0, /*width=*/881, /*height=*/59, VPX_CBR, VPX_DL_REALTIME));
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(/*threads=*/13, /*width=*/1271,
                                            /*height=*/385, VPX_CBR,
                                            VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/2, /*width=*/1, /*height=*/62, VPX_VBR, VPX_DL_REALTIME));
}
...
TEST(EncodeAPI, Buganizer329088759RowMT1) {
  ...
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/2, /*width=*/1686, /*height=*/1, VPX_VBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/2, /*width=*/1482, /*height=*/113, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/2, /*width=*/881, /*height=*/59, VPX_CBR, VPX_DL_REALTIME));
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(/*threads=*/13, /*width=*/1271,
                                            /*height=*/385, VPX_CBR,
                                            VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
...
TEST(EncodeAPI, Buganizer331108922BitDepth8) {
  ...
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/1, /*width=*/1, /*height=*/1080, VPX_VBR, VPX_DL_REALTIME));
  ASSERT_TRUE(encoder.Initialized());
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(/*threads=*/0, /*width=*/1,
                                            /*height=*/1080, VPX_CBR,
                                            VPX_DL_GOOD_QUALITY));
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/16, /*width=*/1, /*height=*/394, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/16, /*width=*/1, /*height=*/798, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
}
...
TEST(EncodeAPI, Buganizer329674887RowMT0BitDepth12) {
  ...
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/2, /*width=*/1030, /*height=*/583, VPX_VBR, VPX_DL_REALTIME));
  ASSERT_TRUE(encoder.Initialized());
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/0, /*width=*/1030, /*height=*/1, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/0, /*width=*/548, /*height=*/322, VPX_VBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(/*threads=*/16, /*width=*/24,
                                            /*height=*/583, VPX_CBR,
                                            VPX_DL_GOOD_QUALITY));
}
...
TEST(EncodeAPI, Buganizer329179808RowMT0BitDepth10) {
  ...
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/16, /*width=*/1488, /*height=*/5, VPX_VBR, VPX_DL_REALTIME));
  ASSERT_TRUE(encoder.Initialized());
  encoder.Encode(/*key_frame=*/true);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/16, /*width=*/839, /*height=*/1, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
  ASSERT_NO_FATAL_FAILURE(encoder.Configure(
      /*threads=*/11, /*width=*/657, /*height=*/5, VPX_CBR, VPX_DL_REALTIME));
  encoder.Encode(/*key_frame=*/false);
}
