...
TEST(EncodeAPI, VP8MultiResEncode) {
  ...
  uint8_t *img_buf = reinterpret_cast<uint8_t *>(
      calloc(width * height * bps / 8, sizeof(*img_buf)));
  uint8_t *img_buf_downsize = reinterpret_cast<uint8_t *>(
      calloc(width_down * height_down * bps / 8, sizeof(*img_buf_downsize)));

  memset(&enc, 0, 2 * sizeof(vpx_codec_ctx_t));
  ...
  for (int i = 1; i < 2; i++) {
    ...
    cfg[i].rc_target_bitrate = 500;
    cfg[i].g_w = width_down;
    cfg[i].g_h = height_down;
  }
  ...
  vpx_img_wrap(&img[0], VPX_IMG_FMT_I420, width, height, 1, img_buf);
  vpx_img_wrap(&img[1], VPX_IMG_FMT_I420, width_down, height_down, 1,
               img_buf_downsize);
