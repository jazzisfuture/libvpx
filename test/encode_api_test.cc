...
void CodecEncodeFrame(vpx_codec_ctx_t *enc, const int width,
                      const int height, const int frame_index,
                      unsigned int deadline, const bool is_key) {
...
// Vary the deadline on the fly from good to realtime to best
// and back to realtime.
TEST(EncodeAPI, DynamicDeadlineChange) {
  ...
  struct Config config = { 0, 1, 1, VPX_VBR, 1000000 };
  ...
  // Change config,in particular change deadline to REALTIME.
  config = { 0, 1, 1, VPX_VBR, 1 };
  ...
  // Change config, in particular change deadline to BEST.
  config = { 0, 1, 1, VPX_VBR, 0 };
  deadline = config.deadline;
  ASSERT_EQ(vpx_codec_enc_config_set(&enc, &cfg), VPX_CODEC_OK)
      << vpx_codec_error_detail(&enc);

  // Encode 5th frame with new config, set delta frame.
  CodecEncodeFrame(&enc, cfg.g_w, cfg.g_h, frame_index, deadline, false);
  frame_index++;
  ...
  // Encode 6th frame with new config, set key frame.
  CodecEncodeFrame(&enc, cfg.g_w, cfg.g_h, frame_index, deadline, false);
  frame_index++;
