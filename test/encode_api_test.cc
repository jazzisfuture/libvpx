...
class EncodeApiGetTplStatsTest
 ...
 public:
  EncodeApiGetTplStatsTest()
      : EncoderTest(GetParam()), test_io_(false), temp_out_file_(nullptr) {}
 ...
 protected:
      ...
      switch (pkt->kind) {
        case VPX_CODEC_CX_FRAME_PKT: {
          ...
          if (test_io_ && tpl_stats.size > 0) {
            temp_out_file_ = new libvpx_test::TempOutFile("w+");
            ASSERT_NE(temp_out_file_->file(), nullptr);
            vpx_write_tpl_stats(temp_out_file_->file(), &tpl_stats);
            FILE *tpl_file = fopen(temp_out_file_->file_name().c_str(), "r");
            delete temp_out_file_;
            ASSERT_NE(tpl_file, nullptr);
            VpxTplGopStats gop_stats_io;
            ASSERT_EQ(vpx_alloc_and_read_tpl_stats(tpl_file, &gop_stats_io),
                      VPX_CODEC_OK);
  ...
  int height_;
  bool test_io_;
  libvpx_test::TempOutFile *temp_out_file_;
};
