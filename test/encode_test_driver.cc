...
void EncoderTest::RunLoop(VideoSource *video, int error_frame) {
#if CONFIG_VP8_DECODER || CONFIG_VP9_DECODER
  ...
  for (unsigned int pass = 0; pass < passes_; pass++) {
    ...
    bool again;
    int i;
    for (i = 0, again = true, video->Begin(); again; video->Next(), ++i) {
      ...
      // skip decoding for frame# specified in error_frame
      if (error_frame >= 0 &&
          cfg_.g_pass == VPX_RC_LAST_PASS && i == error_frame)
        continue;
