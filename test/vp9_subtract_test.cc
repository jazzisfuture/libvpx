...
TEST_P(VP9SubtractBlockTest, SimpleSubtract) {
  ...
  for (BLOCK_SIZE_TYPE bsize = BLOCK_SIZE_AB4X4; bsize < BLOCK_SIZE_TYPES;
    ...
    int16_t *diff = reinterpret_cast<int16_t *>(vpx_memalign(16,
        sizeof(*diff) * block_width * block_height * 2));
    uint8_t *pred = reinterpret_cast<uint8_t *>(vpx_memalign(16,
        block_width * block_height * 2));
    uint8_t *src  = reinterpret_cast<uint8_t *>(vpx_memalign(16,
        block_width * block_height * 2));
