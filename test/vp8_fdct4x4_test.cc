...
TEST_P(FdctTest, SignBiasCheck) {
  ...
  for (int i = 0; i < count_test_block; ++i) {
    ...
    f_func_(test_input_block, test_output_block, pitch);
...
TEST_P(FdctTest, RoundTripErrorCheck) {
  ...
  for (int i = 0; i < count_test_block; ++i) {
    ...
    const int pitch = 8;
    f_func_(test_input_block, test_temp_block, pitch);
...
#if HAVE_NEON
INSTANTIATE_TEST_CASE_P(NEON, FdctTest, ::testing::Values(vp8_short_fdct4x4_neon));
#endif  // HAVE_NEON

#if HAVE_MMX
INSTANTIATE_TEST_CASE_P(MMX, FdctTest, ::testing::Values(vp8_short_fdct4x4_mmx));
#endif  // HAVE_MMX

#if HAVE_SSE2
INSTANTIATE_TEST_CASE_P(SSE2, FdctTest, ::testing::Values(vp8_short_fdct4x4_sse2));
#endif  // HAVE_SSE2

#if HAVE_MSA
INSTANTIATE_TEST_CASE_P(MSA, FdctTest, ::testing::Values(vp8_short_fdct4x4_msa));
#endif  // HAVE_MSA
}  // namespace
