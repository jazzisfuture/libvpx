...
class IVFVideoSource : public CompressedVideoSource {
 public:
  ...
  virtual void Begin() {
    ...
    // Check file header
    ASSERT_TRUE(fread(file_hdr, 1, kIvfFileHdrSize, input_file_)
        == kIvfFileHdrSize && file_hdr[0] == 'D' && file_hdr[1] == 'K'
  ...
  void FillFrame() {
    ...
    frame_sz_ = mem_get_le32(frame_hdr);
    ASSERT_TRUE(frame_sz_ <= kCodeBufferSize)
        << "Frame is too big for allocated code buffer";
    ASSERT_TRUE(fread(compressed_frame_buf_, 1, frame_sz_, input_file_)
        == frame_sz_)
        << "Failed to read complete frame";
    }
  }

  virtual uint8_t *cxdata() { return end_of_file_ ?
      NULL : compressed_frame_buf_; }
  virtual unsigned int frame_size() const { return frame_sz_; }
