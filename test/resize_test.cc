...
class ResizeInternalRealtimeTest : public ::libvpx_test::EncoderTest,
 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(libvpx_test::VideoSource *video,
    ...
    if (change_bitrate_ && video->frame() == 100) {
      vpx_codec_err_t res;
      cfg_.rc_target_bitrate = 500;
      res = vpx_codec_enc_config_set(&encoder->encoder_, &cfg_);
      ASSERT_EQ(VPX_CODEC_OK, res) << "Encoder config error.";
    }
  }
...
// Run at fixed low bitrate, with resize_allowed = 1, and verify that we get
