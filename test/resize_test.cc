...
class ResizeTest
 ...
 protected:
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    width = pkt->data.frame.width;
    height = pkt->data.frame.height;
    encode_frame_width_.push_back(width);
    encode_frame_height_.push_back(height);
  }

  unsigned int GetFrameWidth(size_t idx) { return encode_frame_width_[idx]; }
  unsigned int GetFrameHeight(size_t idx) { return encode_frame_height_[idx]; }
