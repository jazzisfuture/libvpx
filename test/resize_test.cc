...
static const unsigned int kInitialWidth = 320;
static const unsigned int kInitialHeight = 240;
 ...
 protected:
  ...
  virtual void FramePktHook(const vpx_codec_cx_pkt_t *pkt) {
    if (pkt->data.frame.flags & VPX_FRAME_IS_KEY) {
      ...
      pts_list_.push_back(pkt->data.frame.pts);
      w_list_.push_back(w);
      h_list_.push_back(h);
    }
  }

  std::vector< vpx_codec_pts_t > pts_list_;
  std::vector< unsigned int > w_list_;
  std::vector< unsigned int > h_list_;
};
...
TEST_P(ResizeTest, TestExternalResizeWorks) {
  ...
  std::vector<vpx_codec_pts_t>::iterator pts;
  std::vector<unsigned int>::iterator w, h;
  for (pts = pts_list_.begin(), w = w_list_.begin(), h = h_list_.begin();
       pts != pts_list_.end() && w != w_list_.end() && h != h_list_.end();
       ++pts, ++w, ++h) {
    ...
    EXPECT_EQ(expected_w, *w) << "Frame " << *pts << "had unexpected width";
    EXPECT_EQ(expected_h, *h) << "Frame " << *pts << "had unexpected height";
  }
}
