...
TEST_P(VpxPostProcDownAndAcrossMbRowTest, CheckFilterOutput) {
  ...
  uint8_t *const dst_image = new uint8_t[output_size];
...
class VpxMbPostProcDownTest
 ...
 protected:
  ...
  virtual void RunComparison(unsigned char *s_c, unsigned char *s_asm, int rows,
    ...
    for (r = 0; r < rows; r++) {
      for (c = 0; c < cols; c++) {
        ASSERT_EQ(s_c[c], s_asm[c]) << "at (" << r << ", " << c << ")";
      }
      s_c += src_pitch;
      s_asm += src_pitch;
    }
...
TEST_P(VpxMbPostProcDownTest, CheckFilterOutput) {
  ...
  for (r = 0; r < rows; r++) {
    for (c = 0; c < cols; c++) {
      ASSERT_EQ(kExpectedOutput[r * rows + c], s_c[c]) << "at (" << r << ", "
                                                       << c << ")";
    }
    s_c += src_pitch;
  }
...
TEST_P(VpxMbPostProcDownTest, CheckCvsAssembly) {
  ...
  unsigned char *src_c = new unsigned char[src_size];
  unsigned char *src_asm = new unsigned char[src_size];
  memset(src_c, 10, src_size);
