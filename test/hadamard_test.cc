...
int compare_int16_t(const void *a, const void *b) {
  const int16_t a_16_t = static_cast<const int16_t *>(a);
  const int16_t b_16_t = static_cast<const int16_t *>(b);

  return *a_16_t - *b_16_t;
}
...
TEST_P(HadamardTest, CompareReferenceRandom) {
  ...
  qsort(b, 64, sizeof(b[0]), compare_int16_t);
  qsort(b_ref, 64, sizeof(b_ref[0]), compare_int16_t);
  EXPECT_EQ(0, memcmp(b, b_ref, sizeof(b[0]) * 64));
}
...
TEST_P(HadamardTest, VaryStride) {
  ...
  for (int i = 8; i < 64; i += 8) {
    ...
    qsort(b, 64, sizeof(b[0]), compare_int16_t);
    qsort(b_ref, 64, sizeof(b_ref[0]), compare_int16_t);
    EXPECT_EQ(memcmp(b, b_ref, sizeof(b[0]) * 64), 0);
  }
}
