...
class HadamardTestBase : public ::testing::TestWithParam<HadamardFunc> {
 public:
  ...
  void CompareReferenceRandom() {
    DECLARE_ALIGNED(16, int16_t, a[bwh * bwh]);
    DECLARE_ALIGNED(16, tran_low_t, b[bwh * bwh]);
    tran_low_t b_ref[bwh * bwh];
    for (int i = 0; i < bwh * bwh; ++i) {
    ...
    if (bwh == 32)
      reference_hadamard32x32(a, bwh, b_ref);
    else if (bwh == 16)
      reference_hadamard16x16(a, bwh, b_ref);
    else
      reference_hadamard8x8(a, bwh, b_ref);
    ASM_REGISTER_STATE_CHECK(h_func_(a, bwh, b));

    // The order of the output is not important. Sort before checking.
    std::sort(b, b + (bwh * bwh));
    std::sort(b_ref, b_ref + (bwh * bwh));
    EXPECT_EQ(0, memcmp(b, b_ref, sizeof(b)));
  }

  template<int bwh>
  void VaryStride() {
    DECLARE_ALIGNED(16, int16_t, a[bwh * bwh * 8]);
    DECLARE_ALIGNED(16, tran_low_t, b[bwh * bwh]);
    tran_low_t b_ref[bwh * bwh];
    for (int i = 0; i < bwh * bwh * 8; ++i) {
      a[i] = rnd_.Rand9Signed();
    }

    for (int i = 8; i < bwh * bwh; i += 8) {
      memset(b, 0, sizeof(b));
      memset(b_ref, 0, sizeof(b_ref));

      if (bwh == 32)
        reference_hadamard32x32(a, i, b_ref);
      else if (bwh == 16)
