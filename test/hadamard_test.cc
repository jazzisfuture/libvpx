...
class HadamardTestBase
    : public ::testing::TestWithParam<std::pair<HadamardFunc, int>> {
 public:
  virtual void SetUp() {
    h_func_ = GetParam().first;
    bwh_ = GetParam().second;
  ...
  void CompareReferenceRandom() {
    ...
    DECLARE_ALIGNED(sizeof(tran_low_t), tran_low_t, b[kMaxBlockSize]);
    memset(a, 0, sizeof(a));
    memset(b, 0, sizeof(b));
  ...
  void SpeedTest(int times) {
    ...
    DECLARE_ALIGNED(sizeof(tran_low_t), tran_low_t, output[kMaxBlockSize]);
...
class HadamardLowbdTest : public HadamardTestBase {
 protected:
  int16_t Rand() override { return rnd_.Rand9Signed(); }
};
...
INSTANTIATE_TEST_CASE_P(
    C, HadamardLowbdTest,
    ::testing::Values(std::make_pair(&vpx_hadamard_8x8_c, 8),
                      std::make_pair(&vpx_hadamard_16x16_c, 16),
                      std::make_pair(&vpx_hadamard_32x32_c, 32)));

#if HAVE_SSE2
INSTANTIATE_TEST_CASE_P(
    SSE2, HadamardLowbdTest,
    ::testing::Values(std::make_pair(&vpx_hadamard_8x8_sse2, 8),
                      std::make_pair(&vpx_hadamard_16x16_sse2, 16),
                      std::make_pair(&vpx_hadamard_32x32_sse2, 32)));
#endif  // HAVE_SSE2

#if HAVE_AVX2
INSTANTIATE_TEST_CASE_P(
    AVX2, HadamardLowbdTest,
    ::testing::Values(std::make_pair(&vpx_hadamard_16x16_avx2, 16),
                      std::make_pair(&vpx_hadamard_32x32_avx2, 32)));
#endif  // HAVE_AVX2

#if HAVE_SSSE3 && ARCH_X86_64
INSTANTIATE_TEST_CASE_P(
    SSSE3, HadamardLowbdTest,
    ::testing::Values(std::make_pair(&vpx_hadamard_8x8_ssse3, 8)));
#endif  // HAVE_SSSE3 && ARCH_X86_64

#if HAVE_NEON
INSTANTIATE_TEST_CASE_P(
    NEON, HadamardLowbdTest,
    ::testing::Values(std::make_pair(&vpx_hadamard_8x8_neon, 8),
                      std::make_pair(&vpx_hadamard_16x16_neon, 16)));
#endif  // HAVE_NEON
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(std::make_pair(&vpx_hadamard_8x8_msa, 8),
                      std::make_pair(&vpx_hadamard_16x16_msa, 16)));
#endif  // HAVE_MSA
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(std::make_pair(&vpx_hadamard_8x8_vsx, 8),
                      std::make_pair(&vpx_hadamard_16x16_vsx, 16)));
#endif  // HAVE_VSX
...
INSTANTIATE_TEST_CASE_P(
    ...
    ::testing::Values(std::make_pair(&vpx_highbd_hadamard_8x8_c, 8),
                      std::make_pair(&vpx_highbd_hadamard_16x16_c, 16),
                      std::make_pair(&vpx_highbd_hadamard_32x32_c, 32)));
#endif  // CONFIG_VP9_HIGHBITDEPTH
}  // namespace
