...
static TxfmFunc fwd_txfm_func_ls[2][5] = {
    {vp10_fdct4_new, vp10_fdct8_new, vp10_fdct16_new, vp10_fdct32_new, vp10_fdct64_new},
    {vp10_fadst4_new, vp10_fadst8_new, vp10_fadst16_new, vp10_fadst32_new, NULL}};

static TxfmFunc inv_txfm_func_ls[2][5] = {
    {vp10_idct4_new, vp10_idct8_new, vp10_idct16_new, vp10_idct32_new, vp10_idct64_new},
    ...
    for (int ti = 0; ti < txfm_type_num; ++ti) {
      ...
      if(fwd_txfm_func != NULL) {
        ...
        for (int ci = 0; ci < count_test_block; ++ci) {
          ...
          int stage_err = 0;
          for (int ni = 0; ni < txfm_size; ++ni) {
            int node_err = abs(input[ni] - round_shift(round_trip_output[ni],
                                                       get_max_bit(txfm_size) - 1));
            EXPECT_LE(node_err,
                      max_error);
            stage_err += node_err;
          }
          avg_stage_err += stage_err/txfm_size;
        }
        avg_stage_err /= count_test_block;
        printf("si: %d ti: %d avg_stage_err: %f\n", si, ti, avg_stage_err);
      }
    }
    delete[] input;
    delete[] output;
