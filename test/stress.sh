...
# Set sha1sum to proper sha program ( sha1sum, shasum, sha1).
...
download_and_check_file() {
  ...
  # Restore the trap to call cleanup on exit.
  trap cleanup EXIT
...
stress_verify_environment() {
  ...
  if [ ! -e "${YUV}" ] ; then
    download_and_check_file "${YUV}"
  fi
  if [ ! -e "${VP8}" ] ; then
    download_and_check_file "${VP8}"
  fi
  if [ ! -e "${VP9}" ] ; then
    download_and_check_file "${VP9}"
  fi
