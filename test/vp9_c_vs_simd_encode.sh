...
Y4M_360P_10BIT_INPUT="${LIBVPX_TEST_DATA_PATH}/crowd_run_360p_10.y4m"
...
# Echoes path to $1 when it's executable and exists in ${VPX_TEST_OUTPUT_DIR}, or an
...
# Y4M_360P_10BIT_INPUT, LIBVPX_SOURCE_DIR, LIBVPX_TEST_DATA_PATH are required.
...
# Configures for the given target in VPX_TEST_OUTPUT_DIR/build_target_${target} directory.
vp9_enc_build() {
  local target=$1
  local configure="$2"
...
compare_enc_output() {
  ...
  if [ $? -eq 1 ]; then
    elog "C vs SIMD encode mismatches for ${clip}, at ${bitrate} kbps, speed ${cpu}, ${preset} preset"
    return 1
  fi
}
...
vp9_test_generic() {
  ...
  vp9_enc_build "generic-gnu" ${configure}
  local encoder="$(vp9_enc_tool_path "generic-gnu")"
  vp9_enc_test $encoder "generic-gnu"
}
...
vp9_test_x86() {
  ...
  if [ $arch = "x86" ]; then
    ...
    elog "Available architecture is not x86 or x86_64"
    return 0
  fi
  local enable_simd_variants="avx512f avx2 sse4_1 ssse3 sse2"
  local configure="$LIBVPX_SOURCE_DIR/configure"
  ...
  for m in $enable_simd_variants; do
    has_x86_isa_extn $m
    if [ $? -eq 1 ]; then
      echo "${m} is not supported in this machine"
      continue
    fi
...
vp9_test_arm() {
  ...
  local configure="CROSS=aarch64-linux-gnu- $LIBVPX_SOURCE_DIR/configure"
