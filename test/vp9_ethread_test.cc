 ...
 protected:
  ...
  virtual void PreEncodeFrameHook(::libvpx_test::VideoSource * /*video*/,
    ...
    if (!encoder_initialized_) {
      ...
      encoder->Control(VP9E_SET_NEW_MT, new_mt_mode_);
      // While new_mt = 1, several speed features that would adaptively adjust
      // encoding parameters have to be disabled to guarantee the bit match of
      // the resulted bitstream.
      encoder->Control(VP9E_ENABLE_THREAD_BIT_MATCH, bit_match_mode_);
      encoder_initialized_ = true;
    }
  }
