...
class VPxEncoderThreadTest
 ...
 protected:
  ...
  double GetAveragePsnr() const {
    return nframes_ ? (psnr_ / nframes_) : 0.0;
  }
...
TEST_P(VPxEncoderThreadTest, EncoderResultTest) {
  std::vector<std::string> single_thr_md5, multi_thr_md5, new_mt_single_thr_md5,
      new_mt_multi_thr_md5;
  double single_thr_psnr, multi_thr_psnr;
  ...
  init_flags_ = VPX_CODEC_USE_PSNR;
  ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
  single_thr_psnr = GetAveragePsnr();

  // Encode using multiple threads.
  cfg_.g_threads = threads_;
  ASSERT_NO_FATAL_FAILURE(RunLoop(&video));
  multi_thr_psnr = GetAveragePsnr();

  EXPECT_LE(fabs(single_thr_psnr - multi_thr_psnr), 0.1);
}
