...
namespace {
// There are 23 double members followed by 1 int64_t member in FIRSTPASS_STATS.
// Need to modify the following constants whenever FIRSTPASS_STATS struct is
// changed.
const int kInt = 1;
const int kDbl = (int)((sizeof(FIRSTPASS_STATS) - kInt * sizeof(int64_t)) /
    sizeof(double));
const size_t kFirstPassStatsSz = kInt * sizeof(int64_t) + kDbl * sizeof(double);
 ...
 protected:
  VPxFirstPassEncoderThreadTest()
      : EncoderTest(GET_PARAM(0)), encoder_initialized_(false),
        tiles_(0), encoding_mode_(GET_PARAM(1)), set_cpu_used_(GET_PARAM(2)) {
  ...
  virtual void StatsPktHook(const vpx_codec_cx_pkt_t *pkt) {
    ...
    EXPECT_EQ(pkt_size, kFirstPassStatsSz)
        << "Error: First pass stats size doesn't equal sizeof(FIRSTPASS_STATS)";

    firstpass_stats_.buf = realloc(firstpass_stats_.buf,
