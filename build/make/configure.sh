...
process_common_toolchain() {
    ...
    OSX_SDK_ROOTS="$OSX_SDK_ROOTS /Applications/Xcode.app/Contents/Developer/Platforms/
MacOSX.platform/Developer/SDKs"
    OSX_SDK_VERSIONS="MacOSX10.4u.sdk MacOSX10.5.sdk MacOSX10.6.sdk"
    OSX_SDK_VERSIONS="$OSX_SDK_VERSIONS MacOSX10.7.sdk"
    for v in $OSX_SDK_VERSIONS; do
        for r in $OSX_SDK_ROOTS; do
            [ -d "${r}/${d}" ] && osx_sdk_dir="${r}/${d}"
        done
    done
    [ -d "$osx_sdk_dir" ] || die "Unable to determine path to XCode SDK"
