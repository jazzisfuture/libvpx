...
int main(int argc, char **argv) {
  ...
  // For VP9.
  const vpx_codec_iface_t *(*interface)(void) = vpx_codec_vp9_cx;
  unsigned int fourcc = 0x30395056;
  // For VP8:
  ...
  for (i = 0; i < cfg.ts_number_layers; i++) {
    ...
    write_ivf_file_header(outfile[i], &cfg, 0, fourcc);
  }
  ...
  for (i = 0; i < cfg.ts_number_layers; i++) {
    if (!fseek(outfile[i], 0, SEEK_SET))
      write_ivf_file_header(outfile[i], &cfg, frames_in_layer[i], fourcc);
    fclose(outfile[i]);
  }
  return EXIT_SUCCESS;
}
