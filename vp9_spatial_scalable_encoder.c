...
static void write_ivf_file_header(FILE *outfile,
                                  const vpx_codec_enc_cfg_t *enc_cfg,
                                  int frame_cnt) {
  char header[32];

  if (enc_cfg->g_pass != VPX_RC_ONE_PASS && enc_cfg->g_pass != VPX_RC_LAST_PASS)
    return;
  header[0] = 'D';
  header[1] = 'K';
  header[2] = 'I';
  ...
  // TODO(ivanmaltz) height and width should be the resolution of the
  // highest layer, i.e., input resolution * 1st scale factor
  mem_put_le16(header + 12, enc_cfg->g_w);            /* width */
  mem_put_le16(header + 14, enc_cfg->g_h);            /* height */
  mem_put_le32(header + 16, enc_cfg->g_timebase.den); /* rate */
  mem_put_le32(header + 20, enc_cfg->g_timebase.num); /* scale */
...
static void parse_command_line(int argc, char **argv, SvcContext *svc_ctx,
  ...
  opterr = 0;
  while ((c = getopt(argc, argv, "f:w:h:n:d:b:s:l:g:r:q:")) != -1) switch (c) {
      case 'f':
...
int main(int argc, char **argv) {
  ...
  svc_ctx.log_print = 1;
  svc_ctx.first_frame_full_size = 1;
  ...
  write_ivf_file_header(outfile, &enc_cfg, 0);
  ...
  before = clock();
  // Encode frames
  while (frame_cnt <= number_frames_to_code && read_frame(infile, &raw)) {
    ...
    if (svc_get_frame_size(&svc_ctx) > 0) {
      ...
      (void)fwrite(svc_get_buffer(&svc_ctx), 1, svc_get_frame_size(&svc_ctx), outfile);
    }
    frame_cnt++;
    pts += frame_duration;
  ...
  printf("Processed %d frames in different resolutions in %ld ms.\n\n",
         frame_cnt, (int)(after - before) / (CLOCKS_PER_SEC / 1000));
  ...
  if (!fseek(outfile, 0, SEEK_SET)) {
    write_ivf_file_header(outfile, &enc_cfg, frame_cnt);
  }
  fclose(outfile);
