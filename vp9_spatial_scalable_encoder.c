...
const uint32_t default_frames_to_code = 60 * 60;
const uint32_t default_frames_to_skip = 0;
const uint32_t default_width = 1920;
const uint32_t default_height = 1080;
const uint32_t default_timebase_num = 1;
const uint32_t default_timebase_den = 60;
const uint32_t default_bitrate = 1000;
const uint32_t default_spatial_layers = 5;
const uint32_t default_gop_size = 100;
const SVC_ENCODING_MODE default_encoding_mode = INTER_LAYER_PREDICTION_IP;
const int default_use_dummy_frame = 1;
...
static void parse_command_line(int argc, const char **argv_,
  ...
  svc_ctx->gop_size = default_gop_size;
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &encoding_mode_arg, argi)) {
      ...
      enc_cfg->g_timebase.num = arg_parse_uint(&arg);
    } else if (arg_match(&arg, &timebase_den_arg, argi)) {
      enc_cfg->g_timebase.den = arg_parse_uint(&arg);
      ...
      svc_ctx->gop_size = arg_parse_uint(&arg);
    } else if (arg_match(&arg, &scale_factors_arg, argi)) {
      svc_ctx->scale_factors = strdup(arg.val);
    } else if (arg_match(&arg, &quantizers_arg, argi)) {
      svc_ctx->quantizer_values = strdup(arg.val);
  ...
  printf(
      ...
      enc_cfg->rc_target_bitrate, svc_ctx->gop_size,
