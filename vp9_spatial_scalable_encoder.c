...
static const arg_def_t quantizers_arg =
    ARG_DEF("q", "quantizers", 1, "quantizers for non key frames, also will "
            ...
            "highest layer)");
static const arg_def_t quantizers_key_frame_arg =
    ARG_DEF("qn", "quantizers-key", 1, "quantizers for key frames (lowest to "
        "highest layer)");
...
static void parse_command_line(int argc, const char **argv_,
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    } else if (arg_match(&arg, &quantizers_key_frame_arg, argi)) {
