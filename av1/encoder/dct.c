...
static void fdct64_col(const tran_low_t *input, tran_low_t *output) {
  ...
  static const int8_t fwd_cos_bit_dct_dct_64[12] = { 15, 15, 15, 15, 15, 15,
                                                     15, 15, 15, 15, 15, 15 };
  int32_t in[64], out[64];
  for (i = 0; i < 64; ++i) in[i] = (int32_t)input[i];
  av1_fdct64_new(in, out, fwd_cos_bit_dct_dct_64, fwd_stage_range_dct_dct_64);
...
static void fdct64_row(const tran_low_t *input, tran_low_t *output) {
  ...
  static const int8_t fwd_cos_bit_dct_dct_64[12] = { 12, 12, 12, 12, 12, 12,
                                                     12, 12, 12, 12, 12, 12 };
  int32_t in[64], out[64];
  for (i = 0; i < 64; ++i) in[i] = (int32_t)input[i];
  av1_fdct64_new(in, out, fwd_cos_bit_dct_dct_64, fwd_stage_range_dct_dct_64);
