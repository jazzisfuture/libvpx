...
void av1_fht4x4_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  if (tx_type == DCT_DCT) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      for (j = 0; j < 4; ++j)
        output[j + i * 4] = ROUND_POWER_OF_TWO_SIGNED(temp_out[j], 2);
    }
  }
}
...
void av1_fht4x8_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  for (i = 0; i < n2; ++i) {
    ...
    ht.rows(temp_in, temp_out);
    for (j = 0; j < n; ++j)
      output[j + i * n] = ROUND_POWER_OF_TWO_SIGNED(temp_out[j], 2);
  }
...
void av1_fht8x8_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  if (tx_type == DCT_DCT) {
    ...
    for (i = 0; i < 8; ++i) {
      ...
      ht.rows(temp_in, temp_out);
      for (j = 0; j < 8; ++j)
        output[j + i * 8] = ROUND_POWER_OF_TWO_SIGNED(temp_out[j], 1);
    }
  }
}
...
void av1_fht16x16_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  if (tx_type == DCT_DCT) {
    ...
    for (i = 0; i < 16; ++i) {
      ...
      ht.cols(temp_in, temp_out);
      for (j = 0; j < 16; ++j)
        out[j * 16 + i] = ROUND_POWER_OF_TWO_SIGNED(temp_out[j], 2);
    }
...
void av1_fht32x32_c(const int16_t *input, tran_low_t *output, int stride,
  ...
  if (tx_type == DCT_DCT) {
    ...
    for (i = 0; i < 32; ++i) {
      ...
      ht.rows(temp_in, temp_out);
      for (j = 0; j < 32; ++j)
        output[j + i * 32] = ROUND_POWER_OF_TWO_SIGNED(temp_out[j], 2);
    }
  }
}
