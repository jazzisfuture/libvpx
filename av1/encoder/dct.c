...
static void fdct64(const tran_low_t *input, tran_low_t *output) {
  static const int8_t fwd_stage_range_dct_dct_64[12] = {
    19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19, 19
  };
  static const int8_t fwd_cos_bit_dct_dct_64[12] = {
    15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15, 15 };
  int32_t in[64], out[64];
  for (i = 0; i < 64; ++i)
    in[i] = (int32_t)input[i];
  av1_fdct64_new(in, out, fwd_cos_bit_dct_dct_64,
                 fwd_stage_range_dct_dct_64);
  for (i = 0; i < 64; ++i)
    output[i] = (tran_low_t)out[i];
}

void av1_fht64x64_c(const int16_t *input, tran_low_t *output, int stride,
                    int tx_type) {
  static const transform_2d FHT[] = {
    { fdct64, fdct64 },  // DCT_DCT
#if CONFIG_EXT_TX
    { fhalfright64, fdct64 },        // ADST_DCT
    { fdct64, fhalfright64 },        // DCT_ADST
    { fhalfright64, fhalfright64 },  // ADST_ADST
    { fhalfright64, fdct64 },        // FLIPADST_DCT
    { fdct64, fhalfright64 },        // DCT_FLIPADST
    ...
    { fdct64, fidtx64 },             // V_DCT
    { fidtx64, fdct64 },             // H_DCT
