...
static INLINE void write_buffer_16x32(tran_low_t *output, __m128i *restl,
  ...
  for (i = 0; i < 16; ++i) {
    store_output(&restl[i], output + i * stride + 0);
    store_output(&restr[i], output + i * stride + 8);
    store_output(&resbl[i], output + (i + 16) * stride + 0);
    store_output(&resbr[i], output + (i + 16) * stride + 8);
  }
}
...
void av1_fht16x32_sse2(const int16_t *input, tran_low_t *output, int stride,
  ...
  write_buffer_16x32(output, intl, intr, inbl, inbr, stride);
}
...
static INLINE void write_buffer_32x16(tran_low_t *output, __m128i *res0,
  ...
  for (i = 0; i < 16; ++i) {
    store_output(&res0[i], output + i * stride + 0);
    store_output(&res1[i], output + i * stride + 8);
    store_output(&res2[i], output + i * stride + 16);
    store_output(&res3[i], output + i * stride + 24);
  }
}
...
void av1_fht32x16_sse2(const int16_t *input, tran_low_t *output, int stride,
  ...
  write_buffer_32x16(output, in0, in1, in2, in3, 32);
}
