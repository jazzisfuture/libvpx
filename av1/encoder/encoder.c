...
static INLINE void shift_last_ref_frames(AV1_COMP *cpi) {
  ...
  for (ref_frame = LAST_REF_FRAMES - 1; ref_frame > 0; --ref_frame) {
    ...
    // reference frames start at [1]; Note that LAST_FRAME == 1
    if (!cpi->rc.is_src_frame_alt_ref) {
      memcpy(cpi->interp_filter_selected[ref_frame + 1 + LAST_FRAME],
             cpi->interp_filter_selected[ref_frame + LAST_FRAME],
             sizeof(cpi->interp_filter_selected[ref_frame + LAST_FRAME]));
    }
  }
}
#endif  // CONFIG_EXT_REFS
...
#if CONFIG_NEW_REFS
      uref_cnt_fb(cpi->upsampled_ref_bufs,
                  &cpi->upsampled_ref_idx[cpi->alt2_fb_idx], new_uidx);
#endif  // CONFIG_NEW_REFS
#endif  // CONFIG_EXT_REFS
      uref_cnt_fb(cpi->upsampled_ref_bufs,
...
// cpi->interp_filter_selected[GOLDEN_FRAME]?
    ...
    // Deal with the special case to show BWDREF_FRAME;
    // Refresh the LAST_FRAME with the BWDREF_FRAME and retire the LAST3_FRAME
    // by updating the virtual indices.
    // Note that the frame that BWDREF_FRAME points to now should be retired,
    // and it should not be used before getting refreshed.
    ...
    // TODO(zoeliu): Should we decrement the ref_count of
    // cm->ref_frame_map[cpi->bwd_fb_idx] by 1?
    ...
    assert(cpi->refresh_last_frame == 1);

    // Refresh the LAST_FRAME with the ALTREF2_FRAME and retire the LAST3_FRAME
    // by updating the virtual indices:
    // (1) show_existing_frame == 1:
    //     LAST_FRAME is direclty refreshed by ALTREF2_frame; Otherwise
    // (2) show_existing_frame == 0:
    //     LAST_FRAME is refreshed by the INTNL_OVERLAY frame corresponding to
    //     ALTREF2_FRAME.
    if (!cm->show_existing_frame) {
      // ALTREF2_FRAME has a corresponding INTNL_OVERLAY spot (when the temporal
      // filtering strength is greater than 0).
      ref_cnt_fb(pool->frame_bufs, &cm->ref_frame_map[cpi->alt2_fb_idx],
                 cm->new_fb_idx);
      if (use_upsampled_ref)
        uref_cnt_fb(cpi->upsampled_ref_bufs,
    ...
    // Deal with the special case to show the existing internal ALTREF_FRAME;
    ...
    // It is needed to modify the mapping accordingly.
