...
static int ransac(double *matched_points, int npoints, int *number_of_inliers,
  ...
  *number_of_inliers = 0;
  ...
  if (!(best_inlier_set1 && best_inlier_set2 && inlier_set1 && inlier_set2 &&
    ...
    if (best_inlier_set1) free(best_inlier_set1);
    if (best_inlier_set2) free(best_inlier_set2);
    if (inlier_set1) free(inlier_set1);
    if (inlier_set2) free(inlier_set2);
    if (corners1) free(corners1);
    if (corners1_int) free(corners1_int);
    if (corners2) free(corners2);
    if (image1_coord) free(image1_coord);
    if (inlier_mask) free(inlier_mask);
    return 1;
  }
