...
void multiply_mat(double *m1, double *m2, double *res, const int M1,
...
static int SVD(double *U, double *W, double *V, double *matx, int M, int N) {
  ...
  nrU = (double **)aom_malloc((M) * sizeof(*nrU));
  nrV = (double **)aom_malloc((N) * sizeof(*nrV));
  problem = !(nrU && nrV);
  if (!problem) {
    problem = 0;
    for (i = 0; i < M; i++) {
...
int pseudo_inverse(double *inv, double *matx, const int M, const int N) {
  double *U, *W, *V, ans;
  int i, j, k;
  U = (double *)aom_malloc(M * N * sizeof(*matx));
  W = (double *)aom_malloc(N * sizeof(*matx));
  V = (double *)aom_malloc(N * N * sizeof(*matx));
...
typedef void (*DenormalizeType)(double *params, double *T1, double *T2);
...
int ransac_(double *matched_points, int npoints, int *number_of_inliers,
            ...
            const int paramdim, IsDegenerateType is_degenerate,
            NormalizeType normalize, DenormalizeType denormalize,
            FindTransformationType find_transformation,
  ...
  *number_of_inliers = 0;
  ...
  best_inlier_set1 = (double *)aom_malloc(sizeof(*best_inlier_set1) * npoints * 2);
  best_inlier_set2 = (double *)aom_malloc(sizeof(*best_inlier_set2) * npoints * 2);
...
static void denormalize_affine(double *params, double *T1, double *T2) {
  ...
  params[4] = params_denorm[4];
  params[5] = params_denorm[3];
}
...
int ransac_homography(double *matched_points, int npoints,
                      ...
                      double *best_params) {
  const int result =
      ransac_(matched_points, npoints, number_of_inliers, best_inlier_mask,
