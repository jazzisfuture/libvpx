...
static void project_points_double_homography(double *mat, double *points,
  ...
  for (i = 0; i < n; ++i) {
    ...
    assert(fabs(Z_inv - 0) > 0.00001 && "Invalid value for Z in homography");
    Z = 1. / Z_inv;
  ...
  *number_of_inliers = 0;
  ...
  while (N > trial_count) {
    ...
    if (num_inliers >= max_inliers && num_inliers > 1) {
      ...
      assert(num_inliers > 1 && "Invalid value for num_inliers");
      ...
      if ((num_inliers > max_inliers) ||
        ...
        assert(npoints > 0 && "Zero input points for ransac\n");
        fracinliers = (double)num_inliers / (double)npoints;
        pNoOutliers = 1 - pow(fracinliers, minpts);
        pNoOutliers = fmax(EPS, pNoOutliers);
        pNoOutliers = fmin(1 - EPS, pNoOutliers);
        assert(fabs(1.0 - pNoOutliers) > 0.00001 && "pNoOutliers out of range");
...
int ransac_homography(double *matched_points, int npoints,
  ...
  if (!result) {
    ...
    assert(fabs(best_params[8] - 0) > 0.00001 && "Invalid value for best_params");
