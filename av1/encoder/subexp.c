...
static void encode_uniform(av1_writer *w, int v) {
  const int l = 8;
  const int m = (1 << l) - 190;
  if (v < m) {
...
static INLINE int write_bit_gte(av1_writer *w, int word, int test) {
  av1_write_literal(w, word >= test, 1);
  return word >= test;
}

static void encode_term_subexp(av1_writer *w, int word) {
  if (!write_bit_gte(w, word, 16)) {
    av1_write_literal(w, word, 4);
  } else if (!write_bit_gte(w, word, 32)) {
    av1_write_literal(w, word - 16, 4);
  } else if (!write_bit_gte(w, word, 64)) {
    av1_write_literal(w, word - 32, 5);
  } else {
    encode_uniform(w, word - 64);
  }
}

void av1_write_prob_diff_update(av1_writer *w, aom_prob newp, aom_prob oldp) {
...
void av1_cond_prob_diff_update(av1_writer *w, aom_prob *oldp,
...
void av1_write_primitive_symmetric(av1_writer *w, int word,
  ...
  if (word == 0) {
    ...
    av1_write_bit(w, 1);
    av1_write_bit(w, s);
    av1_write_literal(w, x - 1, abs_bits);
  }
}
