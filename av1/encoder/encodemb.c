...
// to quant_func_list, just trying to get this expr to work for now
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    switch (get_tx_scale(tx_size)) {
      ...
      default:
        highbd_quantize_nuq(
            coeff, tx_size_2d[tx_size], x->skip_block, p->quant,
...
#if CONFIG_TX64X64
    case 2:
      quantize_64x64_nuq(coeff, 1024, x->skip_block, p->quant, p->quant_shift,
                         pd->dequant,
                         (const cuml_bins_type_nuq *)p->cuml_bins_nuq[dq],
                         (const dequant_val_type_nuq *)pd->dequant_val_nuq[dq],
                         qcoeff, dqcoeff, eob, scan_order->scan, band);
...
#if CONFIG_TX64X64
    case 2:
      quantize_64x64_fp_nuq(coeff, tx_size_2d[tx_size], x->skip_block,
...
// to quant_func_list, just trying to get this expr to work for now
  ...
  switch (get_tx_scale(tx_size)) {
    case 2:
      quantize_dc_64x64_fp_nuq(coeff, tx_size_2d[tx_size], x->skip_block,
                               p->quant_fp[0], pd->dequant[0],
                               p->cuml_bins_nuq[dq][0],
                               pd->dequant_val_nuq[dq][0], qcoeff, dqcoeff,
                               eob);
      break;
    case 1:
