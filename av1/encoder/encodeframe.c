...
static void rd_use_partition(AV1_COMP *cpi, ThreadData *td,
...
#else
  save_context(x, mi_row, mi_col, &pre_rdo_buf, bsize);
#endif
...
#else
      restore_context(x, mi_row, mi_col, &pre_rdo_buf, bsize);
#endif
  ...
  if (do_partition_search && cpi->sf.adjust_partitioning_from_last_frame &&
    ...
    for (i = 0; i < 4; i++) {
      ...
      save_context(x, &x_ctx, mi_row, mi_col, bsize);
#else
      save_context(x, mi_row, mi_col, &buf, bsize);
#endif
      ...
      restore_context(x, &x_ctx, mi_row, mi_col, bsize);
#else
      restore_context(x, mi_row, mi_col, &buf, bsize);
#endif
      if (tmp_rdc.rate == INT_MAX || tmp_rdc.dist == INT64_MAX) {
        av1_rd_cost_reset(&chosen_rdc);
#if CONFIG_SUPERTX
