...
static void encode_superblock(AV1_COMP *cpi, ThreadData *td, TOKENEXTRA **t,
                              ...
                              BLOCK_SIZE bsize, PICK_MODE_CONTEXT *ctx, int *rate);
...
static void update_state_sb_supertx(AV1_COMP *cpi, ThreadData *td,
                                    const TileInfo *const tile, int mi_row,
                                    int mi_col, BLOCK_SIZE bsize, int dry_run,
                                    PC_TREE *pc_tree);
...
static void encode_b(AV1_COMP *cpi, const TileInfo *const tile, ThreadData *td,
  ...
  encode_superblock(cpi, td, tp, dry_run, mi_row, mi_col, bsize, ctx,
                    (rate ? &this_rate : NULL));
  ...
  if (!frame_is_intra_only(cm) && bsize <= MAX_SUPERTX_BLOCK_SIZE &&
    ...
    if (supertx_enabled) {
      ...
      if (!x->skip) {
        ...
        av1_tokenize_sb_supertx(cpi, td, tp, dry_run, bsize,
                                (rate ? &this_rate : NULL));
        if (rate) *rate += this_rate;
      } else {
        xd->mi[0]->mbmi.skip = 1;
...
static void rd_use_partition(AV1_COMP *cpi, ThreadData *td,
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      ...
      if (last_part_rdc.rate != INT_MAX && bsize >= BLOCK_8X8 &&
        ...
        encode_superblock(cpi, td, tp, DRY_RUN_NORMAL, mi_row, mi_col, subsize, ctx, NULL);
  ...
  if (do_recon) {
    if (bsize == cm->sb_size) {
      ...
      encode_sb(cpi, td, tile_info, tp, mi_row, mi_col, DRY_RUN_NORMAL,
                bsize, pc_tree, NULL);
    }
  }
...
static void rd_test_partition3(
  ...
  if (sum_rdc.rdcost < best_rdc->rdcost) {
    ...
    encode_superblock(cpi, td, tp, DRY_RUN_NORMAL, mi_row0, mi_col0, subsize0, ctx, NULL);
    ...
    if (sum_rdc.rdcost < best_rdc->rdcost) {
      ...
      encode_superblock(cpi, td, tp, DRY_RUN_NORMAL, mi_row1, mi_col1, subsize1, ctx, NULL);
...
static void predict_b_extend(AV1_COMP *cpi, ThreadData *td,
                             ...
                             int dry_run, int b_sub8x8, int bextend) {
  // Used in supertx
  // (mi_row_ori, mi_col_ori): location for mv
  // (mi_row_pred, mi_col_pred, bsize_pred): region to predict
...
static void extend_dir(AV1_COMP *cpi, ThreadData *td,
                       ...
                       int mi_row_top, int mi_col_top, int dry_run,
