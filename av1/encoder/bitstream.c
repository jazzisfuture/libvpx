...
static void write_inter_mode(AV1_COMMON *cm, av1_writer *w,
...
#if CONFIG_EXT_INTER
static void write_inter_compound_mode(AV1_COMMON *cm, av1_writer *w,
                                      PREDICTION_MODE mode,
                                      const int16_t mode_ctx) {
...
static void update_txfm_partition_probs(AV1_COMMON *cm, av1_writer *w,
                                        FRAME_COUNTS *counts) {
  int k;
...
#if CONFIG_REF_MV
static void update_inter_mode_probs(AV1_COMMON *cm, av1_writer *w,
                                    FRAME_COUNTS *counts) {
  int i;
...
#if CONFIG_EXT_INTER
static void update_inter_compound_mode_probs(AV1_COMMON *cm, av1_writer *w) {
...
static void update_skip_probs(AV1_COMMON *cm, av1_writer *w,
                              FRAME_COUNTS *counts) {
  int k;
...
static void update_switchable_interp_probs(AV1_COMMON *cm, av1_writer *w,
                                           FRAME_COUNTS *counts) {
  int j;
...
#if CONFIG_EXT_TX
static void update_ext_tx_probs(AV1_COMMON *cm, av1_writer *w) {
  const int savings_thresh = av1_cost_one(GROUP_DIFF_UPDATE_PROB) -
                             av1_cost_zero(GROUP_DIFF_UPDATE_PROB);
  int i, j;
  int s;
...
#if CONFIG_SUPERTX
static void update_supertx_probs(AV1_COMMON *cm, av1_writer *w) {
  const int savings_thresh = av1_cost_one(GROUP_DIFF_UPDATE_PROB) -
                             av1_cost_zero(GROUP_DIFF_UPDATE_PROB);
  int i, j;
  int savings = 0;
...
static void update_coef_probs_common(av1_writer *const bc, AV1_COMP *cpi,
                                     TX_SIZE tx_size,
...
static void update_coef_probs_subframe(
    av1_writer *const bc, AV1_COMP *cpi, TX_SIZE tx_size,
...
static void update_coef_probs(AV1_COMP *cpi, av1_writer *w) {
  const TX_MODE tx_mode = cpi->common.tx_mode;
  const TX_SIZE max_tx_size = tx_mode_to_biggest_tx_size[tx_mode];
  TX_SIZE tx_size;
#if CONFIG_ANS
...
static void update_seg_probs(AV1_COMP *cpi, av1_writer *w) {
  AV1_COMMON *cm = &cpi->common;
...
static void update_txfm_probs(AV1_COMMON *cm, av1_writer *w,
                              FRAME_COUNTS *counts) {
...
static void write_global_motion(AV1_COMP *cpi, av1_writer *w) {
  AV1_COMMON *const cm = &cpi->common;
  int frame;
