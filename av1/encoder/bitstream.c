...
static INLINE void write_uniform(av1_writer *w, int n, int v) {
  int l = get_unsigned_bits(n);
  int m = (1 << l) - n;
  if (l == 0) return;
  if (v < m) {
    av1_write_literal(w, v, l - 1);
  } else {
    av1_write_literal(w, m + ((v - m) >> 1), l - 1);
    av1_write_literal(w, (v - m) & 1, 1);
  }
}
...
static void write_intra_mode(av1_writer *w, PREDICTION_MODE mode,
                             const aom_prob *probs) {
  av1_write_token(w, av1_intra_mode_tree, probs, &intra_mode_encodings[mode]);
}

#if CONFIG_EXT_INTER
static void write_interintra_mode(av1_writer *w, INTERINTRA_MODE mode,
                                  const aom_prob *probs) {
...
static void write_inter_mode(VP10_COMMON *cm, av1_writer *w,
...
#if CONFIG_EXT_INTER
  av1_write(w, mode != NEWMV && mode != NEWFROMNEARMV, newmv_prob);

  if (!is_compound && (mode == NEWMV || mode == NEWFROMNEARMV))
    av1_write(w, mode == NEWFROMNEARMV, cm->fc->new2mv_prob);

  if (mode != NEWMV && mode != NEWFROMNEARMV) {
#else
  av1_write(w, mode != NEWMV, newmv_prob);
  ...
  if (mode != NEWMV) {
    ...
    av1_write(w, mode != ZEROMV, zeromv_prob);
    ...
    if (mode != ZEROMV) {
      ...
      av1_write(w, mode != NEARESTMV, refmv_prob);
    }
  }
#else
...
static void write_drl_idx(const VP10_COMMON *cm, const MB_MODE_INFO *mbmi,
                          const MB_MODE_INFO_EXT *mbmi_ext, av1_writer *w) {
  ...
  if (mbmi->mode == NEWMV) {
    ...
    for (idx = 0; idx < 2; ++idx) {
      if (mbmi_ext->ref_mv_count[ref_frame_type] > idx + 1) {
        ...
        av1_write(w, mbmi->ref_mv_idx != idx, drl_prob);
  ...
  if (mbmi->mode == NEARMV) {
    ...
    for (idx = 1; idx < 3; ++idx) {
      if (mbmi_ext->ref_mv_count[ref_frame_type] > idx + 1) {
        ...
        av1_write(w, mbmi->ref_mv_idx != (idx - 1), drl_prob);
...
#if CONFIG_EXT_INTER
static void write_inter_compound_mode(VP10_COMMON *cm, av1_writer *w,
                                      PREDICTION_MODE mode,
                                      const int16_t mode_ctx) {
...
static void write_tx_size_vartx(const VP10_COMMON *cm, const MACROBLOCKD *xd,
                                ...
                                int blk_row, int blk_col, av1_writer *w) {
  ...
  if (tx_size == mbmi->inter_tx_size[tx_row][tx_col]) {
    av1_write(w, 0, cm->fc->txfm_partition_prob[ctx]);
    ...
    int i;
    av1_write(w, 1, cm->fc->txfm_partition_prob[ctx]);
...
static void update_txfm_partition_probs(VP10_COMMON *cm, av1_writer *w,
                                        FRAME_COUNTS *counts) {
  int k;
...
#if CONFIG_REF_MV
static void update_inter_mode_probs(VP10_COMMON *cm, av1_writer *w,
                                    FRAME_COUNTS *counts) {
  int i;
...
#if CONFIG_EXT_INTER
static void update_inter_compound_mode_probs(VP10_COMMON *cm, av1_writer *w) {
  ...
  av1_write(w, do_update, GROUP_DIFF_UPDATE_PROB);
  if (do_update) {
    for (i = 0; i < INTER_MODE_CONTEXTS; ++i) {
      prob_diff_update(
          av1_inter_compound_mode_tree, cm->fc->inter_compound_mode_probs[i],
...
static int write_skip(const VP10_COMMON *cm, const MACROBLOCKD *xd,
                      int segment_id, const MODE_INFO *mi, av1_writer *w) {
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP)) {
    return 1;
  } else {
    const int skip = mi->mbmi.skip;
    av1_write(w, skip, av1_get_skip_prob(cm, xd));
    return skip;
  }
}

static void update_skip_probs(VP10_COMMON *cm, av1_writer *w,
                              FRAME_COUNTS *counts) {
  int k;
...
static void update_switchable_interp_probs(VP10_COMMON *cm, av1_writer *w,
                                           FRAME_COUNTS *counts) {
  int j;
...
#if CONFIG_EXT_TX
static void update_ext_tx_probs(VP10_COMMON *cm, av1_writer *w) {
  const int savings_thresh = av1_cost_one(GROUP_DIFF_UPDATE_PROB) -
                             av1_cost_zero(GROUP_DIFF_UPDATE_PROB);
  int i, j;
  int s;
...
static void pack_palette_tokens(av1_writer *w, const TOKENEXTRA **tp, int n,
                                int num) {
  int i;
...
#if CONFIG_SUPERTX
static void update_supertx_probs(VP10_COMMON *cm, av1_writer *w) {
  const int savings_thresh = av1_cost_one(GROUP_DIFF_UPDATE_PROB) -
                             av1_cost_zero(GROUP_DIFF_UPDATE_PROB);
  int i, j;
  int savings = 0;
...
#if !CONFIG_ANS
static void pack_mb_tokens(av1_writer *w, const TOKENEXTRA **tp,
  ...
  while (p < stop && p->token != EOSB_TOKEN) {
    ...
    if (p->skip_eob_node)
      ...
      av1_write(w, t != EOB_TOKEN, p->context_tree[0]);

    if (t != EOB_TOKEN) {
      av1_write(w, t != ZERO_TOKEN, p->context_tree[1]);

      if (t != ZERO_TOKEN) {
        av1_write(w, t != ONE_TOKEN, p->context_tree[2]);
...
#if CONFIG_VAR_TX
static void pack_txb_tokens(av1_writer *w, const TOKENEXTRA **tp,
...
static void write_segment_id(av1_writer *w, const struct segmentation *seg,
...
static void write_ref_frames(const VP10_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (cm->reference_mode == REFERENCE_MODE_SELECT) {
      av1_write(w, is_compound, av1_get_reference_mode_prob(cm, xd));
    } else {
    ...
    if (is_compound) {
      ...
      av1_write(w, bit, av1_get_pred_prob_comp_ref_p(cm, xd));

#if CONFIG_EXT_REFS
      if (!bit) {
        const int bit1 = mbmi->ref_frame[0] == LAST_FRAME;
        av1_write(w, bit1, av1_get_pred_prob_comp_ref_p1(cm, xd));
      } else {
        const int bit2 = mbmi->ref_frame[0] == GOLDEN_FRAME;
        av1_write(w, bit2, av1_get_pred_prob_comp_ref_p2(cm, xd));
      }
      av1_write(w, bit_bwd, av1_get_pred_prob_comp_bwdref_p(cm, xd));
#endif  // CONFIG_EXT_REFS
    } else {
#if CONFIG_EXT_REFS
      const int bit0 = (mbmi->ref_frame[0] == ALTREF_FRAME ||
                        mbmi->ref_frame[0] == BWDREF_FRAME);
      av1_write(w, bit0, av1_get_pred_prob_single_ref_p1(cm, xd));

      if (bit0) {
        const int bit1 = mbmi->ref_frame[0] == ALTREF_FRAME;
        av1_write(w, bit1, av1_get_pred_prob_single_ref_p2(cm, xd));
      } else {
        const int bit2 = (mbmi->ref_frame[0] == LAST3_FRAME ||
                          mbmi->ref_frame[0] == GOLDEN_FRAME);
        av1_write(w, bit2, av1_get_pred_prob_single_ref_p3(cm, xd));

        if (!bit2) {
          const int bit3 = mbmi->ref_frame[0] != LAST_FRAME;
          av1_write(w, bit3, av1_get_pred_prob_single_ref_p4(cm, xd));
        } else {
          const int bit4 = mbmi->ref_frame[0] != LAST3_FRAME;
          av1_write(w, bit4, av1_get_pred_prob_single_ref_p5(cm, xd));
        }
      }
#else   // CONFIG_EXT_REFS
...
static void write_ext_intra_mode_info(const VP10_COMMON *const cm,
  ...
  if (mbmi->mode == DC_PRED && mbmi->palette_mode_info.palette_size[0] == 0) {
    av1_write(w, mbmi->ext_intra_mode_info.use_ext_intra_mode[0],
  ...
  if (mbmi->uv_mode == DC_PRED &&
    ...
    av1_write(w, mbmi->ext_intra_mode_info.use_ext_intra_mode[1],
...
static void write_palette_mode_info(const VP10_COMMON *cm,
                                    const MACROBLOCKD *xd,
                                    const MODE_INFO *const mi, av1_writer *w) {
  ...
  if (mbmi->mode == DC_PRED) {
    ...
    if (n > 0) {
      ...
      for (i = 0; i < n; ++i)
        av1_write_literal(w, pmi->palette_colors[i], cm->bit_depth);
  ...
  if (mbmi->uv_mode == DC_PRED) {
    ...
    if (n > 0) {
      av1_write_token(w, av1_palette_size_tree,
                      ...
                      &palette_size_encodings[n - 2]);
      for (i = 0; i < n; ++i) {
        av1_write_literal(w, pmi->palette_colors[PALETTE_MAX_SIZE + i],
                          cm->bit_depth);
        av1_write_literal(w, pmi->palette_colors[2 * PALETTE_MAX_SIZE + i],
                          cm->bit_depth);
      }
...
static void pack_inter_mode_mvs(VP10_COMP *cpi, const MODE_INFO *mi,
  ...
  if (seg->update_map) {
    if (seg->temporal_update) {
      ...
      av1_write(w, pred_flag, pred_prob);
  ...
  if (!supertx_enabled)
    ...
    if (!segfeature_active(seg, segment_id, SEG_LVL_REF_FRAME))
      av1_write(w, is_inter, av1_get_intra_inter_prob(cm, xd));
  ...
  if (!is_inter) {
    ...
    if (cpi->common.reference_mode != COMPOUND_REFERENCE &&
      ...
      av1_write(w, interintra, cm->fc->interintra_prob[bsize_group]);
      if (interintra) {
        write_interintra_mode(w, mbmi->interintra_mode,
                              cm->fc->interintra_mode_prob[bsize_group]);
        if (is_interintra_wedge_used(bsize)) {
          av1_write(w, mbmi->use_wedge_interintra,
                    cm->fc->wedge_interintra_prob[bsize]);
          if (mbmi->use_wedge_interintra) {
            av1_write_literal(w, mbmi->interintra_wedge_index,
    ...
    if (cpi->common.reference_mode != SINGLE_REFERENCE &&
      ...
      av1_write(w, mbmi->use_wedge_interinter,
                cm->fc->wedge_interinter_prob[bsize]);
      if (mbmi->use_wedge_interinter) {
        av1_write_literal(w, mbmi->interinter_wedge_index,
                          get_wedge_bits_lookup(bsize));
        av1_write_bit(w, mbmi->interinter_wedge_sign);
      }
    }
#endif  // CONFIG_EXT_INTER
...
static void write_modes_b(VP10_COMP *cpi, const TileInfo *const tile,
                          av1_writer *w, const TOKENEXTRA **tok,
...
static void write_partition(const VP10_COMMON *const cm,
  ...
  if (has_rows && has_cols) {
    ...
    av1_write(w, p == PARTITION_SPLIT, probs[1]);
  } else if (has_rows && !has_cols) {
    assert(p == PARTITION_SPLIT || p == PARTITION_VERT);
    av1_write(w, p == PARTITION_SPLIT, probs[2]);
  } else {
...
static void write_modes_sb(VP10_COMP *const cpi, const TileInfo *const tile,
                           av1_writer *const w, const TOKENEXTRA **tok,
                           const TOKENEXTRA *const tok_end,
#if CONFIG_SUPERTX
                           int supertx_enabled,
#endif
  ...
  if (!supertx_enabled && !frame_is_intra_only(cm) &&
    ...
    av1_write(w, supertx_enabled, prob);
  }
#endif  // CONFIG_SUPERTX
...
static void update_coef_probs_common(av1_writer *const bc, VP10_COMP *cpi,
                                     TX_SIZE tx_size,
...
static void update_coef_probs_subframe(
    av1_writer *const bc, VP10_COMP *cpi, TX_SIZE tx_size,
...
static void update_coef_probs(VP10_COMP *cpi, av1_writer *w) {
  const TX_MODE tx_mode = cpi->common.tx_mode;
  const TX_SIZE max_tx_size = tx_mode_to_biggest_tx_size[tx_mode];
  TX_SIZE tx_size;
#if CONFIG_ANS
...
static void encode_restoration(VP10_COMMON *cm,
  ...
  if (rst->restoration_type != RESTORE_NONE) {
    if (rst->restoration_type == RESTORE_BILATERAL) {
      ...
      aom_wb_write_literal(wb, rst->restoration_level,
                           av1_restoration_level_bits(cm));
    } else {
      aom_wb_write_bit(wb, 0);
      aom_wb_write_literal(wb, rst->vfilter[0] - WIENER_FILT_TAP0_MINV,
                           WIENER_FILT_TAP0_BITS);
      aom_wb_write_literal(wb, rst->vfilter[1] - WIENER_FILT_TAP1_MINV,
                           WIENER_FILT_TAP1_BITS);
      aom_wb_write_literal(wb, rst->vfilter[2] - WIENER_FILT_TAP2_MINV,
                           WIENER_FILT_TAP2_BITS);
      aom_wb_write_literal(wb, rst->hfilter[0] - WIENER_FILT_TAP0_MINV,
                           WIENER_FILT_TAP0_BITS);
      aom_wb_write_literal(wb, rst->hfilter[1] - WIENER_FILT_TAP1_MINV,
                           WIENER_FILT_TAP1_BITS);
      aom_wb_write_literal(wb, rst->hfilter[2] - WIENER_FILT_TAP2_MINV,
                           WIENER_FILT_TAP2_BITS);
    }
  }
}
...
static void update_seg_probs(VP10_COMP *cpi, av1_writer *w) {
  VP10_COMMON *cm = &cpi->common;
...
static void update_txfm_probs(VP10_COMMON *cm, av1_writer *w,
                              FRAME_COUNTS *counts) {
...
static void write_global_motion(VP10_COMP *cpi, av1_writer *w) {
  VP10_COMMON *const cm = &cpi->common;
  int frame;
...
void av1_pack_bitstream(VP10_COMP *const cpi, uint8_t *dst, size_t *size) {
  uint8_t *data = dst;
