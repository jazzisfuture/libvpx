...
static int search_bilateral_level(const YV12_BUFFER_CONFIG *sd, AV1_COMP *cpi,
  ...
  RestorationInfo rsi;
  const int ntiles = av1_get_rest_ntiles(RESTORATION_TILESIZE, cm->width,
                                         cm->height, NULL, NULL, NULL, NULL);
  ...
  cost_norestore = RDCOST_DBL(
      x->rdmult, x->rddiv, (bits << (AV1_PROB_COST_SHIFT - 4)), err_norestore);
#if BILATERAL_SUBTILES == 0
  bits = 0;
#else
  bits = 1;  // #bits when a subtile is not restored
#endif
  cost_norestore_subtile = RDCOST_DBL(
      x->rdmult, x->rddiv, (bits << (AV1_PROB_COST_SHIFT - 4)), err_norestore);
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    if (best_tile_cost) {
      ...
      for (subtile_idx = 0; subtile_idx < BILATERAL_SUBTILES; ++subtile_idx) {
        ...
        bits +=
            (rsi.bilateral_level[fulltile_idx] >= 0) ? bilateral_level_bits : 0;
      ...
      best_tile_cost[tile_idx] = RDCOST_DBL(
          x->rdmult, x->rddiv, (bits << (AV1_PROB_COST_SHIFT - 4)), err);
    }
  }
    ...
    if (filt_direction <= 0 && filt_low != filt_mid) {
      ...
      if (ss_err[filt_low] < 0) {
        bilateral_success[filt_low] =
            search_bilateral_level(sd, cpi, filt_low, partial_frame, tmp_level,
    ...
    if (filt_direction >= 0 && filt_high != filt_mid) {
      if (ss_err[filt_high] < 0) {
        bilateral_success[filt_high] =
            search_bilateral_level(sd, cpi, filt_high, partial_frame, tmp_level,
                                   &ss_err[filt_high], tile_cost);
      }
...
static int search_wiener_filter(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  const int ntiles =
      av1_get_rest_ntiles(RESTORATION_TILESIZE, width, height, &tile_width,
...
static int search_switchable_restoration(
    const YV12_BUFFER_CONFIG *sd, AV1_COMP *cpi, int filter_level,
...
// searched level, filt_guess = q * 0.316206 + 3.87252
  ...
  /*
  printf("frame_restore_type %d: %f %f %f\n",
         cm->rst_info.frame_restoration_type, cost_bilateral, cost_wiener,
         cost_switchable);
         */
