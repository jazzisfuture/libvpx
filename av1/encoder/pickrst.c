...
static void get_proj_subspace(int64_t *src, int width, int height,
  ...
  xq[0] = rint(x[0] * (1 << SGRPROJ_PRJ_BITS));
  xq[1] = rint(x[1] * (1 << SGRPROJ_PRJ_BITS));
}
...
static void search_selfguided_restoration(uint8_t *dat8, int width, int height,
  ...
  int64_t srd[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  int64_t dgd[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  int i, j, ep, bestep = 0;
  ...
  for (ep = 0; ep < SGRPROJ_PARAMS; ep++) {
    ...
    av1_selfguided_restoration(flt1, width, height, width, sgr_params[ep].r1,
                               sgr_params[ep].e1);
    av1_selfguided_restoration(flt2, width, height, width, sgr_params[ep].r2,
                               sgr_params[ep].e2);
...
static double search_sgrproj(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    search_selfguided_restoration(
        dgd->y_buffer, h_end - h_start, v_end - v_start, dgd->y_stride,
...
// searched level, filt_guess = q * 0.316206 + 3.87252
  ...
  printf("Frame %d/%d frame_restore_type %d : %f %f %f %f %f\n",
         ...
         cost_restore[0], cost_restore[1], cost_restore[2], cost_restore[3], cost_restore[4]);
