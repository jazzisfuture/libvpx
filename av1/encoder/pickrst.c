...
#define JOINT_BILATERAL_LEVEL_SEARCH 0
...
static double search_bilateral_level(const YV12_BUFFER_CONFIG *src,
                                     AV1_COMP *cpi, int filter_level,
                                     int partial_frame, int *bilateral_level,
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    for (subtile_idx = 0; subtile_idx < BILATERAL_SUBTILES; ++subtile_idx) {
      ...
      for (i = 0; i < bilateral_levels; ++i) {
        ...
        rsi.bilateral_level[fulltile_idx] = -1;
      }
    }
    bits = 0;
    for (subtile_idx = 0; subtile_idx < BILATERAL_SUBTILES; ++subtile_idx) {
...
#if JOINT_BILATERAL_LEVEL_SEARCH
static double search_filter_bilateral_level(const YV12_BUFFER_CONFIG *src,
  ...
  best_err = search_bilateral_level(src, cpi, filt_mid, partial_frame,
                                    tmp_level, best_tile_cost);
  filt_best = filt_mid;
  ...
  while (filter_step > 0) {
    ...
    if (filt_direction <= 0 && filt_low != filt_mid) {
      ...
      if (ss_err[filt_low] < 0) {
        ss_err[filt_low] = search_bilateral_level(
            src, cpi, filt_low, partial_frame, tmp_level, tile_cost);
      }
    ...
    if (filt_direction >= 0 && filt_high != filt_mid) {
      if (ss_err[filt_high] < 0) {
        ss_err[filt_high] = search_bilateral_level(
            src, cpi, filt_high, partial_frame, tmp_level, tile_cost);
      }
  ...
  if (filter_best)
    *filter_best = filt_best;

  return best_err;
}
#endif // JOINT_BILATERAL_LEVEL_SEARCH
...
static double search_wiener_filter(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
                                   int filter_level, int partial_frame,
                                   int (*vfilter)[RESTORATION_WIN],
                                   int (*hfilter)[RESTORATION_WIN],
                                   int *wiener_level, double *best_tile_cost) {
  AV1_COMMON *const cm = &cpi->common;
  RestorationInfo rsi;
  int64_t err;
  int bits;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    best_tile_cost[tile_idx] = cost_norestore_tile;
...
static double
search_switchable_restoration(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
                              int filter_level, int partial_frame,
                              RestorationInfo *rsi,
                              double *tile_cost[RESTORE_TYPES - 1]) {
  AV1_COMMON *const cm = &cpi->common;
  // const int bilateral_level_bits = av1_bilateral_level_bits(&cpi->common);
  MACROBLOCK *x = &cpi->td.mb;
  // double err, cost_switchable2 = 0;
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    if (rsi->restoration_type[tile_idx] == RESTORE_WIENER) {
      bits += (WIENER_FILT_BITS << AV1_PROB_COST_SHIFT);
    } else if (rsi->restoration_type[tile_idx] == RESTORE_BILATERAL) {
      int s;
  ...
  */
  // printf("cost_switchable %f, cost_switchable2 %f\n",
  //        cost_switchable, cost_switchable2);
  aom_yv12_copy_y(&cpi->last_frame_uf, cm->frame_to_show);
...
// searched level, filt_guess = q * 0.316206 + 3.87252
    ...
    case AOM_BITS_12:
      ...
      break;
    default:
      assert(0 && "bit_depth should be AOM_BITS_8, AOM_BITS_10 "
                  "or AOM_BITS_12");
      return;
    }
#else
...
#if JOINT_BILATERAL_LEVEL_SEARCH
    cost_restore[RESTORE_BILATERAL] = search_filter_bilateral_level(
        src, cpi, method == LPF_PICK_FROM_SUBIMAGE, &lf->filter_level,
        cm->rst_info.bilateral_level, tile_cost[RESTORE_BILATERAL]);
#else
    lf->filter_level =
  ...
  cost_restore[RESTORE_BILATERAL] = search_bilateral_level(
      src, cpi, lf->filter_level, method == LPF_PICK_FROM_SUBIMAGE,
      cm->rst_info.bilateral_level, tile_cost[RESTORE_BILATERAL]);
  cost_restore[RESTORE_WIENER] = search_wiener_filter(
      src, cpi, lf->filter_level, method == LPF_PICK_FROM_SUBIMAGE,
      cm->rst_info.vfilter, cm->rst_info.hfilter, cm->rst_info.wiener_level,
      tile_cost[RESTORE_WIENER]);
  cost_restore[RESTORE_NONE] = search_norestore(
      src, cpi, lf->filter_level, method == LPF_PICK_FROM_SUBIMAGE,
      tile_cost[RESTORE_NONE]);
  cost_restore[RESTORE_SWITCHABLE] = search_switchable_restoration(
      src, cpi, lf->filter_level, method == LPF_PICK_FROM_SUBIMAGE,
      &cm->rst_info, tile_cost);
