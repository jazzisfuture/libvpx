...
static double search_wiener_filter(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    if (!wiener_decompose_sep_sym(M, H, vfilterd, hfilterd)) {
      for (i = 0; i < RESTORATION_HALFWIN; ++i) {
        wiener_info[tile_idx].vfilter[i] = wiener_info[tile_idx].hfilter[i] = 0;
        rsi.wiener_info[tile_idx].vfilter[i] =
            rsi.wiener_info[tile_idx].hfilter[i] = 0;
      }
      wiener_info[tile_idx].level = 0;
      continue;
    }
  ...
  /*
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    if (wiener_info[tile_idx].level == 0)
      continue;
    for (i = 0; i < RESTORATION_HALFWIN; ++i) {
      wiener_info[tile_idx].vfilter[i] = rsi.wiener_info[tile_idx].vfilter[i];
      wiener_info[tile_idx].hfilter[i] = rsi.wiener_info[tile_idx].hfilter[i];
...
static double
search_switchable_restoration(AV1_COMP *cpi, int filter_level,
                              int partial_frame, RestorationInfo *rsi,
...
// searched level, filt_guess = q * 0.316206 + 3.87252
  ...
  cost_restore[RESTORE_BILATERAL] = search_bilateral_level(
      ...
      cm->rst_info.bilateral_info, tile_cost[RESTORE_BILATERAL]);
  cost_restore[RESTORE_WIENER] = search_wiener_filter(
      src, cpi, lf->filter_level, method == LPF_PICK_FROM_SUBIMAGE,
      cm->rst_info.wiener_info, tile_cost[RESTORE_WIENER]);
