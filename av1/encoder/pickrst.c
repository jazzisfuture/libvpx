...
static int64_t sse_restoration_tile(const YV12_BUFFER_CONFIG *src,
                                    AV1_COMMON *const cm,
                                    int h_start, int width,
                                    int v_start, int height) {
  int64_t filt_err;
#if CONFIG_AOM_HIGHBITDEPTH
  if (cm->use_highbitdepth) {
    filt_err = aom_highbd_get_y_sse_part(src, cm->frame_to_show,
                                         h_start, width, v_start, height);
  } else {
    filt_err = aom_get_y_sse_part(src, cm->frame_to_show,
                                  h_start, width, v_start, height);
  }
#else
  filt_err = aom_get_y_sse_part(src, cm->frame_to_show,
...
static int64_t try_restoration_tile(const YV12_BUFFER_CONFIG *src,
  ...
  const int ntiles =
      av1_get_rest_ntiles(cm->width, cm->height, &tile_width,
                          &tile_height, &nhtiles, &nvtiles);
  (void) ntiles;

  av1_loop_restoration_frame(cm->frame_to_show, cm, rsi, 1, partial_frame);
  av1_get_rest_tile_limits(tile_idx, subtile_idx, subtile_bits,
                           nhtiles, nvtiles, tile_width, tile_height,
                           cm->width, cm->height, 0, 0,
                           &h_start, &h_end, &v_start, &v_end);
  filt_err = sse_restoration_tile(src, cm, h_start, h_end - h_start,
  ...
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    ...
    for (subtile_idx = 0; subtile_idx < BILATERAL_SUBTILES; ++subtile_idx) {
      ...
      av1_get_rest_tile_limits(tile_idx, subtile_idx, BILATERAL_SUBTILE_BITS,
                               nhtiles, nvtiles, tile_width,
                               tile_height, cm->width, cm->height, 0, 0,
                               &h_start, &h_end, &v_start, &v_end);
      // err = sse_restoration_tile(src, cm, 0, cm->width, 0, cm->height);
      err = sse_restoration_tile(src, cm, h_start, h_end - h_start,
                                 v_start, v_end - v_start);
      ...
      for (i = 0; i < bilateral_levels; ++i) {
        ...
        // err = try_restoration_frame(src, cpi, &rsi, partial_frame);
        err = try_restoration_tile(src, cpi, &rsi, partial_frame,
                                   tile_idx, subtile_idx,
                                   BILATERAL_SUBTILE_BITS);
    ...
    if (filt_direction >= 0 && filt_high != filt_mid) {
      if (ss_err[filt_high] < 0) {
        bilateral_success[filt_high] =
            search_bilateral_level(src, cpi, filt_high, partial_frame, tmp_level,
...
static int search_wiener_filter(const YV12_BUFFER_CONFIG *src, AV1_COMP *cpi,
  ...
  const int ntiles =
      av1_get_rest_ntiles(width, height, &tile_width,
                          &tile_height, &nhtiles, &nvtiles);
...
static int search_switchable_restoration(
  ...
  bits = frame_level_restore_bits[rsi->frame_restoration_type]
      << AV1_PROB_COST_SHIFT;
  for (tile_idx = 0; tile_idx < ntiles; ++tile_idx) {
    av1_get_rest_tile_limits(tile_idx, 0, 0,
                             nhtiles, nvtiles, tile_width,
                             tile_height, cm->width, cm->height, 0, 0,
                             &h_start, &h_end, &v_start, &v_end);
    // err = sse_restoration_tile(src, cm, 0, cm->width, 0, cm->height);
    err = sse_restoration_tile(src, cm, h_start, h_end - h_start,
                               v_start, v_end - v_start);
    bits = cpi->switchable_restore_cost[RESTORE_NONE];
    cost_norestore_tile =
        RDCOST_DBL(x->rdmult, x->rddiv, (bits >> 4), err);
