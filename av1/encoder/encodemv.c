...
static void encode_mv_component(av1_writer *w, int comp,
  ...
  if (mv_class == MV_CLASS_0) {
    ...
    for (i = 0; i < n; ++i) av1_write(w, (d >> i) & 1, mvcomp->bits[i]);
  }
  ...
  // High precision bit
  if (usehp)
    av1_write(w, hp, mv_class == MV_CLASS_0 ? mvcomp->class0_hp : mvcomp->hp);
}
...
static void update_mv(av1_writer *w, const unsigned int ct[2], aom_prob *cur_p,
                      aom_prob upd_p) {
  (void)upd_p;
...
void av1_write_nmv_probs(VP10_COMMON *cm, int usehp, av1_writer *w,
...
void av1_encode_mv(VP10_COMP *cpi, av1_writer *w, const MV *mv, const MV *ref,
#if CONFIG_REF_MV
                   int is_compound,
#endif
  ...
  if (is_compound) {
    av1_write(w, (j == MV_JOINT_ZERO), mvctx->zero_rmv);
