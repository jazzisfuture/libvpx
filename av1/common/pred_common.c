...
#define CHECK_FORWARD_REFS(ref_frame) \
...
int av1_get_reference_mode_context(const AV1_COMMON *cm,
  ...
  if (has_above && has_left) {  // both edges available
    if (!has_second_ref(above_mbmi) && !has_second_ref(left_mbmi))
      ...
      ctx = CHECK_BACKWARD_REFS(above_mbmi->ref_frame[0]) ^
            CHECK_BACKWARD_REFS(left_mbmi->ref_frame[0]);
    else if (!has_second_ref(above_mbmi))
      // one of two edges uses comp pred (2/3)
      ctx = 2 + (CHECK_BACKWARD_REFS(above_mbmi->ref_frame[0]) ||
                 !is_inter_block(above_mbmi));
    else if (!has_second_ref(left_mbmi))
      // one of two edges uses comp pred (2/3)
      ctx = 2 + (CHECK_BACKWARD_REFS(left_mbmi->ref_frame[0]) ||
    ...
    if (!has_second_ref(edge_mbmi))
      ...
      ctx = CHECK_BACKWARD_REFS(edge_mbmi->ref_frame[0]);
    else
...
#define CHECK_BWDREF_OR_ALTREF2(ref_frame) \
  ((ref_frame == BWDREF_FRAME) || (ref_frame == ALTREF2_FRAME))
...
int av1_get_pred_context_comp_bwdref_p(const AV1_COMMON *cm,
  ...
  if (above_in_image && left_in_image) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra (2)
      ...
      if (!has_second_ref(edge_mbmi))  // single pred (1/3)
        pred_context =
            1 + 2 * (edge_mbmi->ref_frame[1] != ALTREF_FRAME);
      else  // comp pred (1/3)
        pred_context = 1 + 2 *
            (edge_mbmi->ref_frame[bwd_ref_sign_idx] != ALTREF_FRAME);
    } else {  // inter/inter
      const int l_sg = !has_second_ref(left_mbmi);
      const int a_sg = !has_second_ref(above_mbmi);
      ...
      if (brfa == brfl && brfa == ALTREF_FRAME) {
        ...
        if ((CHECK_FORWARD_REFS(brfa) && CHECK_BWDREF_OR_ALTREF2(brfl)) ||
            (CHECK_FORWARD_REFS(brfl) && CHECK_BWDREF_OR_ALTREF2(brfa))) {
          pred_context = 4;
        ...
        if (CHECK_BWDREF_OR_ALTREF2(brfa) && CHECK_BWDREF_OR_ALTREF2(brfl)) {
          pred_context = 4;
        } else {
          // NOTE(zoeliu): Following assert may be removed once confirmed.
    ...
    if (!is_inter_block(edge_mbmi)) {
      ...
      if (has_second_ref(edge_mbmi))
        ...
        pred_context = 3 * (edge_mbmi->ref_frame[1] != ALTREF_FRAME);
    }
  } else {  // no edges available (2)
    pred_context = 2;
  }
...
//
// NOTE(zoeliu): The probability of ref_frame[1] is ALTREF2_FRAME,
      ...
      if (brfa == brfl && brfa == ALTREF2_FRAME)
        ...
        if (brfa == ALTREF2_FRAME || brfl == ALTREF2_FRAME)
          ...
          pred_context = 2 + (brfa != brfl);
                 ...
                 (CHECK_FORWARD_REFS(brfa) && CHECK_FORWARD_REFS(brfl)))
          pred_context = 3;
        else
          pred_context = 4;
      } else if (l_sg || a_sg) {  // single/comp
        ...
        if (brfc == ALTREF2_FRAME && rfs != ALTREF2_FRAME)
          ...
          pred_context =
              3 + (brfc == BWDREF_FRAME || rfs == ALTREF_FRAME);
      } else {  // comp/comp
        if (brfa == ALTREF2_FRAME || brfl == ALTREF2_FRAME)
          pred_context = 2;
        else
          pred_context =
              3 + (brfa == ALTREF_FRAME || brfl == ALTREF_FRAME);
      }
    }
  } else if (above_in_image || left_in_image) {  // one edge available
    const MB_MODE_INFO *edge_mbmi = above_in_image ? above_mbmi : left_mbmi;
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (!has_second_ref(edge_mbmi))  // single
        pred_context = 3 * (!CHECK_BACKWARD_REFS(edge_mbmi->ref_frame[0]));
      else  // comp
        pred_context = 2;
    } else {  // inter/inter
      ...
      if (above_has_second && left_has_second) {  // comp/comp
        ...
        pred_context = (!CHECK_BACKWARD_REFS(refs)) ? 4 : 1;
      } else {  // single/single
...
#if CONFIG_NEW_REFS
#define CHECK_BACKWARD_NON_ALTREF(ref_frame) \
    ((ref_frame) == BWDREF_FRAME || (ref_frame) == ALTREF2_FRAME)
#else
#define CHECK_BACKWARD_NON_ALTREF(ref_frame) ((ref_frame) == BWDREF_FRAME)
#endif  // CONFIG_NEW_REFS
...
int av1_get_pred_context_single_ref_p2(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (!has_second_ref(edge_mbmi)) {  // single
        if (!CHECK_BACKWARD_REFS(edge_mbmi->ref_frame[0]))
          ...
          pred_context = 4 * CHECK_BACKWARD_NON_ALTREF(edge_mbmi->ref_frame[0]);
      } else {
        pred_context = 1 +
                       2 * CHECK_BACKWARD_NON_ALTREF(edge_mbmi->ref_frame[1]);
      }
    } else {  // inter/inter
      const int above_has_second = has_second_ref(above_mbmi);
      const int left_has_second = has_second_ref(left_mbmi);
      ...
      if (above_has_second && left_has_second) {  // comp/comp
        if (above0 == left0 && above1 == left1)
          pred_context = 3 * CHECK_BACKWARD_NON_ALTREF(above1);
        else
          pred_context = 2;
      } else if (above_has_second || left_has_second) {  // comp/single
        const MV_REFERENCE_FRAME rfs = !above_has_second ? above0 : left0;
        ...
        if (CHECK_BACKWARD_NON_ALTREF(rfs))
          pred_context = 3 + CHECK_BACKWARD_NON_ALTREF(crf2);
        else if (rfs == ALTREF_FRAME)
          pred_context = CHECK_BACKWARD_NON_ALTREF(crf2);
        else
          pred_context = 1 + 2 * CHECK_BACKWARD_NON_ALTREF(crf2);
      } else {  // single/single
        if (!CHECK_BACKWARD_REFS(above0) && !CHECK_BACKWARD_REFS(left0)) {
          ...
          pred_context = 4 * CHECK_BACKWARD_NON_ALTREF(edge0);
        } else {
          pred_context =
              2 * CHECK_BACKWARD_NON_ALTREF(above0) +
              2 * CHECK_BACKWARD_NON_ALTREF(left0);
        }
      }
    }
  } else if (has_above || has_left) {  // one edge available
    ...
    if (!is_inter_block(edge_mbmi) ||
      ...
      pred_context = 3 * CHECK_BACKWARD_NON_ALTREF(edge_mbmi->ref_frame[1]);
  } else {  // no edges available (2)
    pred_context = 2;
  }
...
// BWDREF_FRAME, knowing that it shall be either of these 2 choices.
//
// NOTE(zoeliu): The probability of ref_frame[1] is ALTREF2_FRAME, conditioning
...
int av1_get_pred_context_single_ref_p6(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (!has_second_ref(edge_mbmi)) {  // single
        if (!CHECK_BACKWARD_NON_ALTREF(edge_mbmi->ref_frame[0]))
          pred_context = 3;
        else
          pred_context = 4 * (edge_mbmi->ref_frame[0] == BWDREF_FRAME);
      } else {  // comp
        pred_context = 1 +
                       2 * (edge_mbmi->ref_frame[1] == BWDREF_FRAME);
      }
    } else {  // inter/inter
      const int above_has_second = has_second_ref(above_mbmi);
      const int left_has_second = has_second_ref(left_mbmi);
      ...
      if (above_has_second && left_has_second) {  // comp/comp
        ...
        if (!CHECK_BACKWARD_NON_ALTREF(above0) &&
            !CHECK_BACKWARD_NON_ALTREF(left0)) {
          pred_context = 2 + (above0 == left0);
        } else if (!CHECK_BACKWARD_NON_ALTREF(above0) ||
                   !CHECK_BACKWARD_NON_ALTREF(left0)) {
          const MV_REFERENCE_FRAME edge0 =
              !CHECK_BACKWARD_NON_ALTREF(above0) ? left0 : above0;
    ...
    if (!is_inter_block(edge_mbmi) ||
        (!CHECK_BACKWARD_NON_ALTREF(edge_mbmi->ref_frame[0]) &&
         !has_second_ref(edge_mbmi)))
      pred_context = 2;
    else if (!has_second_ref(edge_mbmi))  // single
      pred_context = 4 * (edge_mbmi->ref_frame[0] == BWDREF_FRAME);
