  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (above_has_second && left_has_second) {
        pred_context = 1 + (!CHECK_BACKWARD_REFS(above0) ||
                            !CHECK_BACKWARD_REFS(above1) ||
                            !CHECK_BACKWARD_REFS(left0) ||
                            !CHECK_BACKWARD_REFS(left1));
        ...
        if (!CHECK_BACKWARD_REFS(rfs))
          pred_context = 3 + (!CHECK_BACKWARD_REFS(crf1) ||
                              !CHECK_BACKWARD_REFS(crf2));
        else
          pred_context =
