...
#define GM_ROTATION_PREC_BITS 16
#define GM_ROTATION_PREC_DIFF (WARPEDMODEL_PREC_BITS - GM_ROTATION_PREC_BITS)
#define GM_ROTATION_DECODE_FACTOR (1 << GM_ROTATION_PREC_DIFF)

#define GM_ZOOM_PREC_BITS 11
#define GM_ZOOM_PREC_DIFF (WARPEDMODEL_PREC_BITS - GM_ZOOM_PREC_BITS)
#define GM_ZOOM_DECODE_FACTOR (1 << GM_ZOOM_PREC_DIFF)
...
static INLINE int_mv gm_get_motion_vector(const Global_Motion_Params *gm) {
  int_mv res;
  res.as_mv.row = (int32_t)ROUND_POWER_OF_TWO_SIGNED(
      gm->motion_params.wmmat[0], WARPEDMODEL_PREC_BITS - 3);
  res.as_mv.col = (int32_t)ROUND_POWER_OF_TWO_SIGNED(
      gm->motion_params.wmmat[1], WARPEDMODEL_PREC_BITS - 3);
  return res;
}
