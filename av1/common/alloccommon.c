...
void av1_set_mb_mi(VP10_COMMON *cm, int width, int height) {
...
void av1_free_restoration_buffers(VP10_COMMON *cm) {
  aom_free_frame_buffer(&cm->tmp_loop_buf);
}
#endif  // CONFIG_LOOP_RESTORATION

void av1_free_context_buffers(VP10_COMMON *cm) {
  int i;
  cm->free_mi(cm);
  free_seg_map(cm);
...
int av1_alloc_context_buffers(VP10_COMMON *cm, int width, int height) {
  int new_mi_size;
...
void av1_remove_common(VP10_COMMON *cm) {
  av1_free_context_buffers(cm);
...
void av1_init_context_buffers(VP10_COMMON *cm) {
  cm->setup_mi(cm);
  if (cm->last_frame_seg_map && !cm->frame_parallel_decode)
    memset(cm->last_frame_seg_map, 0, cm->mi_rows * cm->mi_cols);
}

void av1_swap_current_and_last_seg_map(VP10_COMMON *cm) {
  // Swap indices.
