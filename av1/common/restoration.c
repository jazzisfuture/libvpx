...
void av1_selfguided_restoration(int64_t *dgd, int width, int height, int stride,
  ...
  j = 0;
  {
    ...
    const int nb = 3;
    const int64_t a = 3 * A[k] + 2 * A[k + 1] + 2 * A[k + width] + A[k + width + 1];
    const int64_t b = 3 * B[k] + 2 * B[k + 1] + 2 * B[k + width] + B[k + width + 1];
    const int64_t v = (((a * dgd[l] + b) << SGRPROJ_RST_BITS) + (1 << nb) / 2) >> nb;
  ...
  for (j = 1; j < width - 1; ++j) {
    ...
    const int l = i * stride + j;
    const int nb = 3;
    const int64_t a = A[k] + 2 * (A[k - 1] + A[k + 1]) + A[k + width] + A[k + width - 1] + A[k + width + 1];
    const int64_t b = B[k] + 2 * (B[k - 1] + B[k + 1]) + B[k + width] + B[k + width - 1] + B[k + width + 1];
    const int64_t v = (((a * dgd[l] + b) << SGRPROJ_RST_BITS) + (1 << nb) / 2) >> nb;
    dgd[l] = ROUND_POWER_OF_TWO(v, SGRPROJ_SGR_BITS);
  }
  i = height - 1;
  ...
  for (i = 1; i < height - 1; ++i) {
    ...
    const int l = i * stride + j;
    const int nb = 3;
    const int64_t a = A[k] + 2 * (A[k - width] + A[k + width]) + A[k + 1] + A[k - width + 1] + A[k + width + 1];
    const int64_t b = B[k] + 2 * (B[k - width] + B[k + width]) + B[k + 1] + B[k - width + 1] + B[k + width + 1];
    const int64_t v = (((a * dgd[l] + b) << SGRPROJ_RST_BITS) + (1 << nb) / 2) >> nb;
    dgd[l] = ROUND_POWER_OF_TWO(v, SGRPROJ_SGR_BITS);
  }
  j = width - 1;
  ...
  for (i = 1; i < height - 1; ++i) {
    for (j = 1; j < width - 1; ++j) {
      ...
      const int l = i * stride + j;
      const int nb = 5;
      const int64_t a = (A[k] + A[k - 1] + A[k + 1] +
                         A[k - width] + A[k + width]) * 4 +
          (A[k - 1 - width] + A[k - 1 + width] +
           A[k + 1 - width] + A[k + 1 + width]) * 3;
      const int64_t b = (B[k] + B[k - 1] + B[k + 1] +
                         B[k - width] + B[k + width]) * 4 +
          (B[k - 1 - width] + B[k - 1 + width] +
           B[k + 1 - width] + B[k + 1 + width]) * 3;
      const int64_t v = (((a * dgd[l] + b) << SGRPROJ_RST_BITS) + (1 << nb) / 2) >> nb;
      dgd[l] = ROUND_POWER_OF_TWO(v, SGRPROJ_SGR_BITS);
    }
  }
...
static void loop_wiener_filter_tile_highbd(uint16_t *data, int tile_idx,
  ...
  for (i = 0; i < (v_end - v_start); ++i) {
    for (j = 0; j < (h_end - h_start); ++j) {
      *tmpdata_p++ =
          hor_sym_filter_highbd(data_p++, rst->rsi->wiener_info[tile_idx].hfilter,
                                bit_depth);
    }
    data_p += stride - (h_end - h_start);
    tmpdata_p += tmpstride - (h_end - h_start);
