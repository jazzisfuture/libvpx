...
static void boxnum(int width, int height, int r, uint16_t *num,
                   int num_stride) {
  int i, j;
  for (i = 0; i <= r; ++i) {
    for (j = 0; j <= r; ++j) {
...
void av1_selfguided_restoration(int64_t *dgd, int width, int height, int stride,
                                int r, int eps) {
  int64_t T[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  int64_t A[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  int64_t B[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  uint16_t *num = (uint16_t *)malloc(width * height * sizeof(*num));
  int i, j;
...
static void apply_selfguided_restoration(int64_t *dat, int width, int height,
  ...
  int xq[2];
  int64_t flt1[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  int64_t flt2[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  int i, j;
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
...
static void loop_sgrproj_filter_tile(uint8_t *data, int tile_idx, int width,
  ...
  int h_start, h_end, v_start, v_end;
  uint8_t *data_p;
  int64_t dat[RESTORATION_TILESIZE_BIG * RESTORATION_TILESIZE_BIG];
  ...
  apply_selfguided_restoration(dat, h_end - h_start, v_end - v_start,
                               ...
                               rst->rsi->sgrproj_info[tile_idx].xqd);
  for (i = 0; i < (v_end - v_start); ++i) {
    for (j = 0; j < (h_end - h_start); ++j) {
