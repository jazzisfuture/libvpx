...
#define RESTORATION_TILESIZE  256
...
static INLINE int av1_get_rest_ntiles(int tilesize, int width, int height,
...
static INLINE void av1_get_rest_tile_limits(int tile_idx, int subtile_idx,
                                            int subtile_bits,
                                            int nhtiles, int nvtiles,
                                            int tile_width, int tile_height,
                                            int im_width, int im_height,
                                            int clamp_h, int clamp_v,
  ...
  *v_end = (vtile_idx < nvtiles - 1) ? *v_start + tile_height : im_height;
  if (subtile_bits) {
    ...
    *h_end = subtile_idx_h == num_subtiles_1d - 1 ?
        *h_end : *h_start + subtile_width;
    *v_end = subtile_idx_v == num_subtiles_1d - 1 ?
        *v_end : *v_start + subtile_height;
  }
  if (clamp_h) {
