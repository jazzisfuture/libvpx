...
void av1_iht16x32_512_add_sse2(const tran_low_t *input, uint8_t *dest,
  ...
  int i;
  for (i = 0; i < 16; ++i) {
    intl[i] = load_input_data(input + i * stride + 0);
    intr[i] = load_input_data(input + i * stride + 8);
    inbl[i] = load_input_data(input + (i + 16) * stride + 0);
    inbr[i] = load_input_data(input + (i + 16) * stride + 8);
  }
...
void av1_iht32x16_512_add_sse2(const tran_low_t *input, uint8_t *dest,
  ...
  for (i = 0; i < 16; ++i) {
    in0[i] = load_input_data(input + i * stride + 0);
    in1[i] = load_input_data(input + i * stride + 8);
    in2[i] = load_input_data(input + i * stride + 16);
    in3[i] = load_input_data(input + i * stride + 24);
  }
