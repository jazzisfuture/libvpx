...
if (aom_config("CONFIG_FILTER_INTRA") eq "yes") {
  ...
  add_proto qw/void av1_planar_filter_predictor/, "uint8_t *dst, ptrdiff_t stride, int bs, const uint8_t *above, const uint8_t *left";
  ...
  if (aom_config("CONFIG_AOM_HIGHBITDEPTH") eq "yes") {
    ...
    add_proto qw/void av1_highbd_planar_filter_predictor/, "uint16_t *dst, ptrdiff_t stride, int bs, const uint16_t *above, const uint16_t *left, int bd";
  }
}
