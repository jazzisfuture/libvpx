...
#if !CONFIG_ANS
static INLINE int read_coeff(const aom_prob *probs, int n, av1_reader *r) {
  int i, val = 0;
  for (i = 0; i < n; ++i) val = (val << 1) | av1_read(r, probs[i]);
  return val;
}
...
#endif
{
  ...
  while (c < max_eob) {
    ...
    if (!av1_read(r, prob[EOB_CONTEXT_NODE])) {
    ...
    while (!av1_read(r, prob[ZERO_CONTEXT_NODE])) {
    ...
    if (!av1_read(r, prob[ONE_CONTEXT_NODE])) {
      INCREMENT_COUNT(ONE_TOKEN);
      token = ONE_TOKEN;
      val = 1;
    } else {
      INCREMENT_COUNT(TWO_TOKEN);
      token = av1_read_tree(r, av1_coef_con_tree,
...
#if CONFIG_AOM_HIGHBITDEPTH
    dqcoeff[scan[c]] = highbd_check_range((av1_read_bit(r) ? -v : v), xd->bd);
#else
    dqcoeff[scan[c]] = check_range(av1_read_bit(r) ? -v : v);
#endif  // CONFIG_AOM_HIGHBITDEPTH
#else
    dqcoeff[scan[c]] = av1_read_bit(r) ? -v : v;
...
void av1_decode_palette_tokens(MACROBLOCKD *const xd, int plane,
  ...
  for (i = 0; i < rows; ++i) {
    for (j = (i == 0 ? 1 : 0); j < cols; ++j) {
      ...
      color_idx = av1_read_tree(r, av1_palette_color_tree[n - 2],
                                prob[n - 2][color_ctx]);
