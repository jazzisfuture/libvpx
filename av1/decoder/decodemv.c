...
static MOTION_VARIATION read_motion_mode_block(AV1_COMMON *const cm,
  ...
  if (is_motion_variation_allowed(&xd->mi[0]->mbmi)) {
    motvar = (MOTION_VARIATION)aom_read_tree(r, av1_motion_mode_tree,
...
#endif  // CONFIG_EXT_INTER
      mbmi->motion_mode = read_motion_mode_block(cm, xd, r);
#endif  // CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
