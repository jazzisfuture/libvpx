...
static INLINE int read_uniform(av1_reader *r, int n) {
  int l = get_unsigned_bits(n);
  int m = (1 << l) - n;
  int v = av1_read_literal(r, l - 1);
  ...
  if (v < m)
    ...
    return (v << 1) - m + av1_read_literal(r, 1);
}

static PREDICTION_MODE read_intra_mode(av1_reader *r, const aom_prob *p) {
  return (PREDICTION_MODE)av1_read_tree(r, av1_intra_mode_tree, p);
}

static PREDICTION_MODE read_intra_mode_y(VP10_COMMON *cm, MACROBLOCKD *xd,
                                         av1_reader *r, int size_group) {
...
static INTERINTRA_MODE read_interintra_mode(VP10_COMMON *cm, MACROBLOCKD *xd,
                                            av1_reader *r, int size_group) {
  const INTERINTRA_MODE ii_mode = (INTERINTRA_MODE)av1_read_tree(
...
#else
  const int mode =
      av1_read_tree(r, av1_inter_mode_tree, cm->fc->inter_mode_probs[ctx]);
  FRAME_COUNTS *counts = xd->counts;
...
static void read_drl_idx(const VP10_COMMON *cm, MACROBLOCKD *xd,
                         MB_MODE_INFO *mbmi, av1_reader *r) {
...
static PREDICTION_MODE read_inter_compound_mode(VP10_COMMON *cm,
                                                MACROBLOCKD *xd, av1_reader *r,
                                                int16_t ctx) {
  const int mode = av1_read_tree(r, av1_inter_compound_mode_tree,
...
static int read_segment_id(av1_reader *r,
                           const struct segmentation_probs *segp) {
  return av1_read_tree(r, av1_segment_tree, segp->tree_probs);
}
...
static void read_tx_size_vartx(VP10_COMMON *cm, MACROBLOCKD *xd,
  ...
  is_split = av1_read(r, cm->fc->txfm_partition_prob[ctx]);
...
static TX_SIZE read_selected_tx_size(VP10_COMMON *cm, MACROBLOCKD *xd,
                                     int tx_size_cat, av1_reader *r) {
  FRAME_COUNTS *counts = xd->counts;
  const int ctx = get_tx_size_context(xd);
  int tx_size = av1_read_tree(r, av1_tx_size_tree[tx_size_cat],
...
static int read_inter_segment_id(VP10_COMMON *const cm, MACROBLOCKD *const xd,
                                 int mi_row, int mi_col, av1_reader *r) {
  struct segmentation *const seg = &cm->seg;
  FRAME_COUNTS *counts = xd->counts;
  struct segmentation_probs *const segp = &cm->fc->seg;
  ...
  if (seg->temporal_update) {
    ...
    mbmi->seg_id_predicted = av1_read(r, pred_prob);
...
static int read_skip(VP10_COMMON *cm, const MACROBLOCKD *xd, int segment_id,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP)) {
    ...
    const int skip = av1_read(r, cm->fc->skip_probs[ctx]);
    FRAME_COUNTS *counts = xd->counts;
...
static void read_palette_mode_info(VP10_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (mbmi->mode == DC_PRED) {
    ...
    if (av1_read(
      ...
      pmi->palette_size[0] =
          av1_read_tree(r, av1_palette_size_tree,
                        av1_default_palette_y_size_prob[bsize - BLOCK_8X8]) +
          2;
      n = pmi->palette_size[0];
      for (i = 0; i < n; ++i)
        pmi->palette_colors[i] = av1_read_literal(r, cm->bit_depth);
    ...
    if (av1_read(r,
      ...
      for (i = 0; i < n; ++i) {
        pmi->palette_colors[PALETTE_MAX_SIZE + i] =
            av1_read_literal(r, cm->bit_depth);
...
static void read_ext_intra_mode_info(VP10_COMMON *const cm,
                                     MACROBLOCKD *const xd, av1_reader *r) {
  MODE_INFO *const mi = xd->mi[0];
  MB_MODE_INFO *const mbmi = &mi->mbmi;
  FRAME_COUNTS *counts = xd->counts;
  ...
  if (mbmi->mode == DC_PRED && mbmi->palette_mode_info.palette_size[0] == 0) {
    mbmi->ext_intra_mode_info.use_ext_intra_mode[0] =
        av1_read(r, cm->fc->ext_intra_probs[0]);
  ...
  if (mbmi->uv_mode == DC_PRED &&
    ...
    mbmi->ext_intra_mode_info.use_ext_intra_mode[1] =
        av1_read(r, cm->fc->ext_intra_probs[1]);
...
static void read_intra_angle_info(VP10_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (mbmi->mode != DC_PRED && mbmi->mode != TM_PRED) {
    ...
    if (av1_is_intra_filter_switchable(p_angle)) {
      FRAME_COUNTS *counts = xd->counts;
      mbmi->intra_filter = av1_read_tree(r, av1_intra_filter_tree,
...
static void read_intra_frame_mode_info(VP10_COMMON *const cm,
  ...
  if (!FIXED_TX_TYPE) {
    ...
    if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
      ...
      mbmi->tx_type =
          av1_read_tree(r, av1_ext_tx_tree,
...
static int read_mv_component(av1_reader *r, const nmv_component *mvcomp,
                             int usehp) {
  int mag, d, fr, hp;
  const int sign = av1_read(r, mvcomp->sign);
  const int mv_class = av1_read_tree(r, av1_mv_class_tree, mvcomp->classes);
  const int class0 = mv_class == MV_CLASS_0;

  // Integer part
  if (class0) {
    d = av1_read_tree(r, av1_mv_class0_tree, mvcomp->class0);
    mag = 0;
  } else {
    int i;
    ...
    d = 0;
    for (i = 0; i < n; ++i) d |= av1_read(r, mvcomp->bits[i]) << i;
    mag = CLASS0_SIZE << (mv_class + 2);
  }

  // Fractional part
  fr = av1_read_tree(r, av1_mv_fp_tree,
                     class0 ? mvcomp->class0_fp[d] : mvcomp->fp);

  // High precision part (if hp is not used, the default value of the hp is 1)
  hp = usehp ? av1_read(r, class0 ? mvcomp->class0_hp : mvcomp->hp) : 1;
...
static INLINE void read_mv(av1_reader *r, MV *mv, const MV *ref,
#if CONFIG_REF_MV
                           int is_compound,
#endif
...
#if CONFIG_REF_MV && !CONFIG_EXT_INTER
  if (is_compound) {
    int is_zero_rmv = av1_read(r, ctx->zero_rmv);
    if (is_zero_rmv) {
      joint_type = MV_JOINT_ZERO;
    } else {
      joint_type =
          (MV_JOINT_TYPE)av1_read_tree(r, av1_mv_joint_tree, ctx->joints);
    }
  } else {
    joint_type =
        (MV_JOINT_TYPE)av1_read_tree(r, av1_mv_joint_tree, ctx->joints);
...
#else
  joint_type = (MV_JOINT_TYPE)av1_read_tree(r, av1_mv_joint_tree, ctx->joints);
#endif
...
static REFERENCE_MODE read_block_reference_mode(VP10_COMMON *cm,
  ...
  if (cm->reference_mode == REFERENCE_MODE_SELECT) {
    ...
    const REFERENCE_MODE mode =
        (REFERENCE_MODE)av1_read(r, cm->fc->comp_inter_prob[ctx]);
    FRAME_COUNTS *counts = xd->counts;
...
static void read_ref_frames(VP10_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (mode == COMPOUND_REFERENCE) {
      ...
      const int bit = av1_read(r, fc->comp_ref_prob[ctx][0]);
      ...
      if (!bit) {
        ...
        const int bit1 = av1_read(r, fc->comp_ref_prob[ctx1][1]);
        ...
        const int bit2 = av1_read(r, fc->comp_ref_prob[ctx2][2]);
      ...
      // Decode backward references.
      {
        ...
        const int bit_bwd = av1_read(r, fc->comp_bwdref_prob[ctx_bwd][0]);
...
static MOTION_VARIATION read_motvar_block(VP10_COMMON *const cm,
  ...
  if (is_motvar_allowed(&xd->mi[0]->mbmi)) {
    motvar = (MOTION_VARIATION)av1_read_tree(r, av1_motvar_tree,
...
#endif
    FRAME_COUNTS *counts = xd->counts;
    const INTERP_FILTER type = (INTERP_FILTER)av1_read_tree(
...
static INLINE int assign_mv(VP10_COMMON *cm, MACROBLOCKD *xd,
                            ...
                            int is_compound, int allow_hp, av1_reader *r) {
  int i;
  int ret = 1;
#if CONFIG_REF_MV
...
static int read_is_inter_block(VP10_COMMON *const cm, MACROBLOCKD *const xd,
                               int segment_id, av1_reader *r) {
  if (segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    return get_segdata(&cm->seg, segment_id, SEG_LVL_REF_FRAME) != INTRA_FRAME;
  } else {
    const int ctx = av1_get_intra_inter_context(xd);
    const int is_inter = av1_read(r, cm->fc->intra_inter_prob[ctx]);
    FRAME_COUNTS *counts = xd->counts;
...
static void read_inter_block_mode_info(VP10Decoder *const pbi,
  ...
  if (cm->reference_mode != COMPOUND_REFERENCE &&
    ...
    const int interintra = av1_read(r, cm->fc->interintra_prob[bsize_group]);
    ...
    if (interintra) {
      ...
      if (is_interintra_wedge_used(bsize)) {
        mbmi->use_wedge_interintra =
            av1_read(r, cm->fc->wedge_interintra_prob[bsize]);
        if (xd->counts)
          xd->counts->wedge_interintra[bsize][mbmi->use_wedge_interintra]++;
        if (mbmi->use_wedge_interintra) {
          mbmi->interintra_wedge_index =
              av1_read_literal(r, get_wedge_bits_lookup(bsize));
...
static void read_inter_frame_mode_info(VP10Decoder *const pbi,
  ...
  if (!FIXED_TX_TYPE) {
    ...
    if (mbmi->tx_size < TX_32X32 && cm->base_qindex > 0 && !mbmi->skip &&
      ...
      FRAME_COUNTS *counts = xd->counts;
      if (inter_block) {
        mbmi->tx_type = av1_read_tree(r, av1_ext_tx_tree,
