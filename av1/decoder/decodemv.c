...
static MOTION_MODE read_motvar_block(AV1_COMMON *const cm,
                                     MACROBLOCKD *const xd, aom_reader *r) {
  BLOCK_SIZE bsize = xd->mi[0]->mbmi.sb_type;
  FRAME_COUNTS *counts = xd->counts;
  MOTION_MODE motvar;

  if (is_motion_variation_allowed(&xd->mi[0]->mbmi)) {
    motvar = (MOTION_MODE)aom_read_tree(r, av1_motvar_tree,
...
#endif  // CONFIG_EXT_INTER
      mbmi->motion_mode = read_motvar_block(cm, xd, r);
#endif  // CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
...
#if CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
      !(is_motvar_allowed(mbmi) && mbmi->motion_mode != SIMPLE_TRANSLATION) &&
#endif  // CONFIG_MOTION_VAR || CONFIG_WARPED_MOTION
