...
static void read_switchable_interp_probs(FRAME_CONTEXT *fc, av1_reader *r) {
  int i, j;
...
static void read_inter_mode_probs(FRAME_CONTEXT *fc, av1_reader *r) {
  int i;
#if CONFIG_REF_MV
...
#if CONFIG_EXT_INTER
static void read_inter_compound_mode_probs(FRAME_CONTEXT *fc, av1_reader *r) {
  int i, j;
  if (av1_read(r, GROUP_DIFF_UPDATE_PROB)) {
...
static void read_frame_reference_mode_probs(VP10_COMMON *cm, av1_reader *r) {
...
static void update_mv_probs(aom_prob *p, int n, av1_reader *r) {
  int i;
  for (i = 0; i < n; ++i) av1_diff_update_prob(r, &p[i]);
}

static void read_mv_probs(nmv_context *ctx, int allow_hp, av1_reader *r) {
  int i, j;
...
#if CONFIG_VAR_TX
static void decode_reconstruct_tx(MACROBLOCKD *const xd, av1_reader *r,
...
static void decode_block(VP10Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  if (supertx_enabled) {
    xd->corrupted |= av1_reader_has_error(r);
    return;
  }
#endif  // CONFIG_SUPERTX
...
#if CONFIG_EXT_PARTITION_TYPES
    if (bsize <= BLOCK_8X8)
      p = (PARTITION_TYPE)av1_read_tree(r, av1_partition_tree, probs);
    else
      p = (PARTITION_TYPE)av1_read_tree(r, av1_ext_partition_tree, probs);
#else
    p = (PARTITION_TYPE)av1_read_tree(r, av1_partition_tree, probs);
#endif  // CONFIG_EXT_PARTITION_TYPES
  else if (!has_rows && has_cols)
    p = av1_read(r, probs[1]) ? PARTITION_SPLIT : PARTITION_HORZ;
  else if (has_rows && !has_cols)
    p = av1_read(r, probs[2]) ? PARTITION_SPLIT : PARTITION_VERT;
  else
    p = PARTITION_SPLIT;
...
static void decode_partition(VP10Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  if (!frame_is_intra_only(cm) && partition != PARTITION_NONE &&
    ...
    supertx_enabled =
        av1_read(r, cm->fc->supertx_prob[supertx_context][supertx_size]);
    if (xd->counts)
  ...
  if (supertx_enabled && read_token) {
    ...
    if (skip) {
      ...
      if (get_ext_tx_types(supertx_size, bsize, 1) > 1) {
        ...
        if (eset > 0) {
          txfm = av1_read_tree(r, av1_ext_tx_inter_tree[eset],
      ...
      if (supertx_size < TX_32X32) {
        txfm = av1_read_tree(r, av1_ext_tx_tree,
...
static void setup_bool_decoder(const uint8_t *data, const uint8_t *data_end,
                               ...
                               av1_reader *r, aom_decrypt_cb decrypt_cb,
                               void *decrypt_state) {
...
static void read_coef_probs(FRAME_CONTEXT *fc, TX_MODE tx_mode, av1_reader *r) {
...
static void setup_restoration(VP10_COMMON *cm, struct aom_read_bit_buffer *rb) {
  RestorationInfo *rst = &cm->rst_info;
  if (aom_rb_read_bit(rb)) {
    if (aom_rb_read_bit(rb)) {
      rst->restoration_type = RESTORE_BILATERAL;
      rst->restoration_level =
          aom_rb_read_literal(rb, av1_restoration_level_bits(cm));
    } else {
      rst->restoration_type = RESTORE_WIENER;
      rst->vfilter[0] = aom_rb_read_literal(rb, WIENER_FILT_TAP0_BITS) +
                        WIENER_FILT_TAP0_MINV;
      rst->vfilter[1] = aom_rb_read_literal(rb, WIENER_FILT_TAP1_BITS) +
                        WIENER_FILT_TAP1_MINV;
      rst->vfilter[2] = aom_rb_read_literal(rb, WIENER_FILT_TAP2_BITS) +
                        WIENER_FILT_TAP2_MINV;
      rst->hfilter[0] = aom_rb_read_literal(rb, WIENER_FILT_TAP0_BITS) +
                        WIENER_FILT_TAP0_MINV;
      rst->hfilter[1] = aom_rb_read_literal(rb, WIENER_FILT_TAP1_BITS) +
                        WIENER_FILT_TAP1_MINV;
      rst->hfilter[2] = aom_rb_read_literal(rb, WIENER_FILT_TAP2_BITS) +
                        WIENER_FILT_TAP2_MINV;
    }
  } else {
...
static void setup_loopfilter(VP10_COMMON *cm, struct aom_read_bit_buffer *rb) {
...
#if CONFIG_EXT_TX
static void read_ext_tx_probs(FRAME_CONTEXT *fc, av1_reader *r) {
  int i, j, k;
  int s;
...
#if CONFIG_SUPERTX
static void read_supertx_probs(FRAME_CONTEXT *fc, av1_reader *r) {
  int i, j;
  if (av1_read(r, GROUP_DIFF_UPDATE_PROB)) {
...
static void read_global_motion_params(Global_Motion_Params *params,
                                      aom_prob *probs, av1_reader *r) {
  GLOBAL_MOTION_TYPE gmtype =
      av1_read_tree(r, av1_global_motion_types_tree, probs);
  ...
  switch (gmtype) {
    ...
    case GLOBAL_AFFINE:
      params->motion_params.wmmat[4] =
          (av1_read_primitive_symmetric(r, GM_ABS_ALPHA_BITS) *
    ...
    case GLOBAL_TRANSLATION:
      params->motion_params.wmmat[0] =
          av1_read_primitive_symmetric(r, GM_ABS_TRANS_BITS) *
...
static void read_global_motion(VP10_COMMON *cm, av1_reader *r) {
  int frame;
...
static struct aom_read_bit_buffer *init_read_bit_buffer(
    ...
    const uint8_t *data_end, uint8_t clear_data[MAX_AOM_HEADER_SIZE]) {
  rb->bit_offset = 0;
  rb->error_handler = error_handler;
  rb->error_handler_data = &pbi->common;
  if (pbi->decrypt_cb) {
    const int n = (int)AOMMIN(MAX_AOM_HEADER_SIZE, data_end - data);
...
void av1_decode_frame(VP10Decoder *pbi, const uint8_t *data,
  ...
  uint8_t clear_data[MAX_AOM_HEADER_SIZE];
