...
static const VpxIface vpx_encoders[] = {
...
const VpxIface *get_vpx_encoder(int i) {
  return &vpx_encoders[i];
}

const VpxIface *get_vpx_encoder_by_name(const char *name) {
  int i;

  for (i = 0; i < get_vpx_encoder_count(); ++i) {
    const VpxIface *encoder = get_vpx_encoder(i);
...
static const VpxIface vpx_decoders[] = {
...
const VpxIface *get_vpx_decoder(int i) {
  return &vpx_decoders[i];
}

const VpxIface *get_vpx_decoder_by_name(const char *name) {
  int i;

  for (i = 0; i < get_vpx_decoder_count(); ++i) {
     const VpxIface *const iface = get_vpx_decoder(i);
...
const VpxIface *get_vpx_decoder_by_fourcc(uint32_t fourcc) {
  int i;

  for (i = 0; i < get_vpx_decoder_count(); ++i) {
    const VpxIface *const iface = get_vpx_decoder(i);
    if (iface->fourcc == fourcc)
      return iface;
  }
