...
void usage_exit() {
  ...
  for (i = 0; i < get_vpx_decoder_count(); ++i) {
    const VpxInterface *const decoder = get_vpx_decoder(i);
    fprintf(stderr, "    %-6s - %s\n",
...
int main_loop(int argc, const char **argv_) {
  ...
  int                    arg_skip = 0;
  int                    ec_enabled = 0;
  const VpxInterface *iface = NULL, *fourcc_iface = NULL;
  ...
  fourcc_iface = get_vpx_decoder_by_fourcc(vpx_input_ctx.fourcc);
  ...
  if (vpx_codec_dec_init(&decoder,
                         (iface ? iface : get_vpx_decoder(0))->interface(),
                         &cfg, dec_flags)) {
