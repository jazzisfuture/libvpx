...
static const arg_def_t ext_fb_arg = ARG_DEF(NULL, "frame-buffers", 1,
                                            "Number of external frame buffers to use");
...
static const arg_def_t *all_args[] = {
  ...
  &threadsarg, &verbosearg, &scalearg, &ext_fb_arg,
#if CONFIG_MD5
  &md5arg,
#endif
  &error_concealment,
...
int realloc_vp9_frame_buffer(void *user_priv, int new_size,
  ...
  free(fb->data);
  fb->data = (unsigned char*)malloc(new_size);
  if (!fb->data) {
    fb->size = 0;
    return -2;
  }
...
int main_loop(int argc, const char **argv_) {
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &codecarg, argi)) {
      ...
      do_scale = 1;
    else if (arg_match(&arg, &ext_fb_arg, argi))
  ...
  if (num_external_frame_buffers > 0) {
    frame_buffers = (vpx_codec_frame_buffer_t*)calloc(
        sizeof(vpx_codec_frame_buffer_t), num_external_frame_buffers);
