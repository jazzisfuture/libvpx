...
void usage_exit() {
  ...
  for (i = 0; i < get_vpx_decoder_count(); ++i) {
    const VpxIface *const decoder = get_vpx_decoder(i);
    fprintf(stderr, "    %-6s - %s\n",
            decoder->name, vpx_codec_iface_name(decoder->iface()));
  }
...
int file_is_raw(struct VpxInputContext *input) {
  ...
  if (fread(buf, 1, 32, input->file) == 32) {
    ...
    if (mem_get_le32(buf) < 256 * 1024 * 1024) {
      for (i = 0; i < get_vpx_decoder_count(); ++i) {
        const VpxIface *const decoder = get_vpx_decoder(i);
        if (!vpx_codec_peek_stream_info(decoder->iface(),
                                        buf + 4, 32 - 4, &si)) {
          is_raw = 1;
...
int main_loop(int argc, const char **argv_) {
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &codecarg, argi)) {
      iface = get_vpx_decoder_by_name(arg.val);
      if (!iface)
  ...
  const VpxIface *fourcc_iface =
     get_vpx_decoder_by_fourcc(vpx_input_ctx.fourcc);
  if (iface && fourcc_iface && iface != fourcc_iface)
  ...
  if (vpx_codec_dec_init(&decoder,
                         (iface ? iface : get_vpx_decoder(0))->iface(),
                         &cfg, dec_flags)) {
