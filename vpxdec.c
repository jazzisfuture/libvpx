...
static const arg_def_t *all_args[] = { &help, &codecarg, &use_yv12, &use_i420,
    &flipuvarg, &rawvideo, &noblitarg, &progressarg, &limitarg, &skiparg,
    &postprocarg, &summaryarg, &outputfile, &threadsarg, &frameparallelarg,
    &verbosearg, &scalearg, &fb_arg, &md5arg, &error_concealment, &continuearg,
#if CONFIG_VP9_HIGHBITDEPTH
    &outbitdeptharg,
#endif
    &svcdecodingarg, &framestatsarg, &rowmtarg, &lpfoptarg,
    NULL };
...
static int main_loop(int argc, const char **argv_) {
  ...
  if (interface->fourcc == VP9_FOURCC &&
      ...
      vpx_codec_control(&decoder, VP9D_SET_LPF_OPT, enable_lpf_opt)) {
