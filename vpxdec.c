...
#if CONFIG_VP9_HIGH
static const arg_def_t outputshiftarg = ARG_DEF(NULL, "output-shift", 1,
                                          "Amount to downshift output images");
#endif
...
#if CONFIG_VP9_HIGH
static void img_convert_16_to_8(vpx_image_t  *dst,vpx_image_t  *src, int output_shift) {
  int plane;
  //int offset = 0;
  //if (output_shift>0)
  //  offset = 1<<(output_shift-1);
  if (src->fmt != dst->fmt + VPX_IMG_FMT_HIGH || dst->d_w != src->d_w || dst->d_h != src->d_h || dst->x_chroma_shift != src->x_chroma_shift || dst->y_chroma_shift != src->y_chroma_shift) {
  ...
  for(plane=0;plane<3;plane++) {
    ...
    for(y=0;y<h;y++) {
      ...
      unsigned short *p_src = (unsigned short *)(src->planes[plane] + y * src->stride[plane]);
      for(x=0;x<w;x++) {
    ...
    if (!noblit && img) {
      ...
      if (output_shift) {
        ...
        if (img->fmt & VPX_IMG_FMT_HIGH) {
          if (!img_8bit) {
            img_8bit = vpx_img_alloc(NULL, img->fmt - VPX_IMG_FMT_HIGH , img->d_w,
                                     img->d_h, 16);
          }
