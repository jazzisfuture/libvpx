...
unsigned int file_is_raw(struct VpxInputContext *input) {
...
webm_guess_framerate(struct VpxDecInputContext *input) {
  ...
  if (nestegg_track_seek(input->nestegg_ctx, input->video_track, 0)) {
    ...
    input->vpx_input_ctx->framerate.num = 30;
    input->vpx_input_ctx->framerate.den  = 1;
    return 0;
  }
  ...
  input->vpx_input_ctx->framerate.num = (i - 1) * 1000000;
  input->vpx_input_ctx->framerate.den = (int)(tstamp / 1000);
  return 0;
fail:
...
file_is_webm(struct VpxDecInputContext *input) {
  ...
  input->vpx_input_ctx->framerate.den = 0;
  input->vpx_input_ctx->framerate.num = 0;
...
int main_loop(int argc, const char **argv_) {
  ...
  if (use_y4m && !noblit) {
    ...
    snprintf(buffer, sizeof(buffer), "YUV4MPEG2 W%u H%u F%u:%u I%c ",
             ...
             vpx_input_ctx.framerate.num, vpx_input_ctx.framerate.den,
             'p');
