...
// Callback used by libvpx to request an external frame buffer. |user_priv|
// user private data passed into the set function. |min_size| is the
...
int get_vp9_frame_buffer(void *user_priv, size_t min_size,
  ...
  struct ExternalFrameBufferList *const ext_fb_list =
      (struct ExternalFrameBufferList*)user_priv;
  if (ext_fb_list == NULL || fb == NULL)
    return -1;
  ...
  fb->frame_priv = (void*)(&ext_fb_list->ext_fb[i]);
  return 0;
}

// Callback used by libvpx when there are no references to the frame buffer.
// |user_priv| user private data passed into the set function. |fb| pointer
// to the frame buffer.
int release_vp9_frame_buffer(void *user_priv,
  ...
  ext_fb = (struct ExternalFrameBuffer*)fb->frame_priv;
  ext_fb->in_use = 0;
  return 0;
}
