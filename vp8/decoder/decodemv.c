...
static int read_nmv_component_fp(vp8_reader *r,
  ...
  if (c == MV_CLASS_0) {
    f = vp8_treed_read(r, vp8_mv_fp_tree, mvcomp->class0_fp);
  } else {
...
static void read_nmv(vp8_reader *r, MV *mv, const nmv_context *mvctx) {
  ...
  if (j == MV_JOINT_HZVNZ || j == MV_JOINT_HNZVNZ) {
    mv->row = read_nmv_component(r, &mvctx->comps[0]);
  }
  if (j == MV_JOINT_HNZVZ || j == MV_JOINT_HNZVNZ) {
    mv->col = read_nmv_component(r, &mvctx->comps[1]);
  }
}

static void read_nmv_fp(vp8_reader *r, MV *mv, const nmv_context *mvctx,
                        int usehp) {
  MV_JOINT_TYPE j = vp8_get_mv_joint(*mv);
  if (j == MV_JOINT_HZVNZ || j == MV_JOINT_HNZVNZ) {
    mv->row = read_nmv_component_fp(r, mv->row, &mvctx->comps[0], usehp);
  }
  if (j == MV_JOINT_HNZVZ || j == MV_JOINT_HNZVNZ) {
    mv->col = read_nmv_component_fp(r, mv->col, &mvctx->comps[1], usehp);
  }
}
...
static void read_nmvprobs(vp8_reader *bc, nmv_context *mvctx,
  ...
  for (i = 0; i < 2; ++i) {
    for (j = 0; j < 3; ++j) {
      update_nmv(bc, &mvctx->comps[i].class0_fp[j],
                 VP8_NMV_UPDATE_PROB);
    }
    for (j = 0; j < 3; ++j) {
...
static void read_mb_modes_mv(VP8D_COMP *pbi, MODE_INFO *mi, MB_MODE_INFO *mbmi,
  ...
  if (mbmi->ref_frame) {
    ...
    switch (mbmi->mode) {
      case SPLITMV: {
        ...
        do { /* for each subset j */
          ...
          switch (blockmode) {
            case NEW4X4:
#if CONFIG_NEWMVENTROPY
              read_nmv(bc, &blockmv.as_mv, nmvc);
              read_nmv_fp(bc, &blockmv.as_mv, nmvc, xd->allow_high_precision_mv);
              ...
              if (mbmi->second_ref_frame) {
                ...
                read_nmv_fp(bc, &secondmv.as_mv, nmvc,
                            xd->allow_high_precision_mv);
      ...
      case NEWMV:
#if CONFIG_NEWMVENTROPY
        read_nmv(bc, &mv->as_mv, nmvc);
        read_nmv_fp(bc, &mv->as_mv, nmvc, xd->allow_high_precision_mv);
        ...
        if (mbmi->second_ref_frame) {
          ...
          read_nmv_fp(bc, &mbmi->mv[1].as_mv, nmvc,
                      xd->allow_high_precision_mv);
