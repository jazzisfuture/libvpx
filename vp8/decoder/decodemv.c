...
static void vp9_read_mb_segid(vp9_reader *r, MB_MODE_INFO *mi,
                              MACROBLOCKD *xd) {
...
static void kfread_modes(VP9D_COMP *pbi,
  ...
  if (pbi->mb.update_mb_segmentation_map) {
    vp9_read_mb_segid(bc, &m->mbmi, &pbi->mb);
...
#endif
  y_mode = (MB_PREDICTION_MODE) read_kf_ymode(bc,
    ...
    switch (mbmi->mode) {
      case SPLITMV: {
        ...
        do { /* for each subset j */
          ...
          mbmi->need_to_clamp_mvs |= vp9_check_mv_bounds(&blockmv,
                                                         mb_to_left_edge,
                                                         mb_to_right_edge,
                                                         mb_to_top_edge,
                                                         mb_to_bottom_edge);
          if (mbmi->second_ref_frame) {
            mbmi->need_to_clamp_mvs |= vp9_check_mv_bounds(&secondmv,
                                                           mb_to_left_edge,
                                                           mb_to_right_edge,
                                                           mb_to_top_edge,
                                                           mb_to_bottom_edge);
      ...
      case NEWMV:
         ...
         */
        mbmi->need_to_clamp_mvs = vp9_check_mv_bounds(mv,
                                                      mb_to_left_edge,
                                                      mb_to_right_edge,
                                                      mb_to_top_edge,
                                                      mb_to_bottom_edge);
