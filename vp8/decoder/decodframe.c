...
#if CONFIG_SUPERBLOCKS
        if (xd->mode_info_context->mbmi.encoded_as_sb) {
          vp8_dequant_dc_idct_add_y_block_8x8_inplace_c(xd->qcoeff, xd->block[0].dequant,
                                                        xd->dst.y_buffer + (n >> 1) * 16 * xd->dst.y_stride + (n & 1) * 16,
                                                        xd->dst.y_stride, xd->eobs,
                                                        xd->block[24].diff, xd);
          // do UV inline also
          vp8_dequant_idct_add_uv_block_8x8_inplace_c(xd->qcoeff + 16 * 16, xd->block[16].dequant,
...
decode_sb_row(VP8D_COMP *pbi, VP8_COMMON *pc, int mbrow, MACROBLOCKD *xd) {
  ...
  for (sb_col = 0; sb_col < sb_cols; sb_col++) {
    ...
    if (pbi->interleaved_decoding)
      xd->mode_info_context->mbmi.encoded_as_sb = vp8_read(&pbi->bc, pc->sb_coded);
#endif
...
#if CONFIG_SUPERBLOCKS
      if (xd->mode_info_context->mbmi.encoded_as_sb) {
        xd->mode_info_context[1] =
        xd->mode_info_context[pc->mode_info_stride] =
        xd->mode_info_context[pc->mode_info_stride + 1] = xd->mode_info_context[0];
      }
#endif
      decode_macroblock(pbi, xd, mb_col);
#if CONFIG_SUPERBLOCKS
      if (xd->mode_info_context->mbmi.encoded_as_sb) {
        xd->mode_info_context[1].mbmi.txfm_size =
        xd->mode_info_context[pc->mode_info_stride].mbmi.txfm_size =
        xd->mode_info_context[pc->mode_info_stride + 1].mbmi.txfm_size = xd->mode_info_context[0].mbmi.txfm_size;
      }
#endif
