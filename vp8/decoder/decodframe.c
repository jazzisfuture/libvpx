  ...
  for (sb_col = 0; sb_col < sb_cols; sb_col++) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (xd->mode_info_context->mbmi.ref_frame)
        ref_fb_idx = pc->active_ref_idx[xd->mode_info_context->mbmi.ref_frame - 1];
      ...
      if (xd->mode_info_context->mbmi.second_ref_frame) {
        ...
        /* Select the appropriate reference frame for this MB */
        second_ref_fb_idx =
            pc->active_ref_idx[xd->mode_info_context->mbmi.second_ref_frame - 1];
...
#endif
    // TODO(jkoleszar) What's the right thing to do here?
    vp8_update_mode_context(&pbi->common, pbi->refresh_frame_flags & 0x4);
  }
