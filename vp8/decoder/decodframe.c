                                ...
                                const unsigned char* token_part_sizes)
{
    ...
    for (fragment_idx = 1; fragment_idx < pbi->num_fragments; ++fragment_idx)
    {
        if (vp8dx_start_decode(bool_decoder,
                               pbi->fragments[fragment_idx],
                               pbi->fragment_sizes[fragment_idx]))
    ...
    if (pbi->decoding_thread_count > pbi->num_fragments - 1)
        pbi->decoding_thread_count = pbi->num_fragments - 1;
#endif
}
