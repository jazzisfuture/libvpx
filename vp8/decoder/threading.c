...
static void setup_decoding_thread_data(VP8D_COMP *pbi, MACROBLOCKD *xd,
  ...
  for (i = 0; i < pc->mb_rows; ++i) vpx_release_store_32(&pbi->mt_current_mb_col[i], -1);
}
...
static void mt_decode_mb_rows(VP8D_COMP *pbi, MACROBLOCKD *xd,
                              int start_mb_row) {
  const atomic_int *last_row_current_mb_col;
  volatile atomic_int *current_mb_col;
  int mb_row;
  ...
  if (start_mb_row) {
    sem_post(&pbi->h_event_end_decoding[start_mb_row - 1]);
  }
}
...
static THREAD_FUNCTION thread_decoding_proc(void *p_data) {
  ...
  while (1) {
    if (vpx_nobarrier_load_32(&pbi->b_multithreaded_rd) == 0) break;

    if (sem_wait(&pbi->h_event_start_decoding[ithread]) == 0) {
      if (vpx_nobarrier_load_32(&pbi->b_multithreaded_rd) == 0) {
        break;
      } else {
...
void vp8_decoder_create_threads(VP8D_COMP *pbi) {
  ...
  vpx_nobarrier_store_32(&pbi->b_multithreaded_rd, 0);
  ...
  if (core_count > 1) {
    vpx_nobarrier_store_32(&pbi->b_multithreaded_rd, 1);
    ...
    CALLOC_ARRAY(pbi->h_event_end_decoding, pbi->decoding_thread_count);
    ...
    for (ithread = 0; ithread < pbi->decoding_thread_count; ++ithread) {
      if (sem_init(&pbi->h_event_start_decoding[ithread], 0, 0) ||
          sem_init(&pbi->h_event_end_decoding[ithread], 0, 0)) {
...
void vp8_decoder_remove_threads(VP8D_COMP *pbi) {
  ...
  if (vpx_nobarrier_load_32(&pbi->b_multithreaded_rd)) {
    ...
    for (i = 0; i < pbi->allocated_decoding_thread_count; ++i) {
      ...
      sem_post(&pbi->h_event_end_decoding[i]);
      pthread_join(pbi->h_decoding_thread[i], NULL);
    }

    for (i = 0; i < pbi->allocated_decoding_thread_count; ++i) {
      sem_destroy(&pbi->h_event_start_decoding[i]);
      sem_destroy(&pbi->h_event_end_decoding[i]);
    }
    ...
    vpx_free(pbi->h_event_end_decoding);
    pbi->h_event_end_decoding = NULL;
...
void vp8mt_decode_mb_rows(VP8D_COMP *pbi, MACROBLOCKD *xd) {
  ...
  for (i = 0; i < pbi->decoding_thread_count; ++i) {
    sem_wait(&pbi->h_event_end_decoding[i]);
  }
}
