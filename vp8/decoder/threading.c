...
static void setup_decoding_thread_data(VP8D_COMP *pbi, MACROBLOCKD *xd,
  ...
  for (i = 0; i < pc->mb_rows; ++i)
    vpx_atomic_write(&pbi->mt_current_mb_col[i], -1);
}
...
static void mt_decode_mb_rows(VP8D_COMP *pbi, MACROBLOCKD *xd,
                              int start_mb_row) {
  // TODO(pbos): See if last_row_ can be made const.
  vpx_atomic_int *last_row_current_mb_col;
  vpx_atomic_int *current_mb_col;
  int mb_row;
  VP8_COMMON *pc = &pbi->common;
  const int nsync = pbi->sync_range;
  vpx_atomic_int first_row_no_sync_above;
  ...
  vpx_atomic_init(&first_row_no_sync_above, pc->mb_cols + nsync);
...
static THREAD_FUNCTION thread_decoding_proc(void *p_data) {
  ...
  while (1) {
    if (vpx_atomic_read(&pbi->b_multithreaded_rd) == 0) break;

    if (sem_wait(&pbi->h_event_start_decoding[ithread]) == 0) {
      if (vpx_atomic_read(&pbi->b_multithreaded_rd) == 0) {
        break;
      } else {
...
void vp8_decoder_remove_threads(VP8D_COMP *pbi) {
  ...
  if (vpx_atomic_read(&pbi->b_multithreaded_rd)) {
    int i;
    vpx_atomic_write(&pbi->b_multithreaded_rd, 0);
