    ...
    if (!res)
    {
        ...
        if (resolution_change)
        {
            ...
            pc->Height = ctx->si.h;
            {
                ...
                {
                    pc->get_fb_cb = vp8_get_frame_buffer;
                    pc->release_fb_cb = vp8_release_frame_buffer;

                    if (vp8_alloc_internal_frame_buffers(&pc->int_frame_buffers))
                        vpx_internal_error(&pc->error, VPX_CODEC_MEM_ERROR,
                                           "Failed to initialize internal frame buffers");
                    pc->cb_priv = &pc->int_frame_buffers;
                }
