                                ...
                                vpx_codec_priv_enc_mr_cfg_t *data)
{
    ...
    /* for now, we do not support frame threads */
    ctx->priv->alg_priv->cfg.framethreads = 0;

    if(ctx->priv->alg_priv->cfg.framethreads &&
            (ctx->priv->alg_priv->cfg.threads
                                  ...
                                  long                    deadline)
{
    ...
    if (!res && !ctx->decoder_init)
    {
        ...
        if (!res)
        {
            ...
            oxcf.max_frame_threads = ctx->cfg.framethreads;
                                                ...
                                                va_list args)
{
    ...
    if (update_info && !ctx->cfg.framethreads)
    {
                                              ...
                                              va_list args)
{
    ...
    if (ref_info && !ctx->cfg.framethreads)
    {
        VP8D_COMP *pbi = (VP8D_COMP *)ctx->yv12_frame_buffers.pbi[0];
