                                                ...
                                                va_list args)
{
    ...
    if (update_info && !ctx->yv12_frame_buffers.use_frame_threads)
    {
        ...
        *update_info = pbi->common.refresh_alt_ref_frame * (int) ALT_FLAG
            + pbi->common.refresh_golden_frame * (int) GOLD_FLAG
            + pbi->common.refresh_last_frame * (int) LAST_FLAG;
                                              ...
                                              va_list args)
{
    ...
    if (ref_info && !ctx->yv12_frame_buffers.use_frame_threads)
    {
        ...
        *ref_info =
            (vp8dx_references_buffer( oci, ALTREF_FRAME )?ALT_FLAG:0) |
            (vp8dx_references_buffer( oci, GOLDEN_FRAME )?GOLD_FLAG:0) |
            (vp8dx_references_buffer( oci, LAST_FRAME )?LAST_FLAG:0);
