...
static void vp8_init_ctx(vpx_codec_ctx_t *ctx, const vpx_codec_mmap_t *mmap)
{
    ...
    ctx->priv->alg_priv->decrypt_key = fake_decrypt_key;
                                            ...
                                            const unsigned char *decrypt_key)
{
    ...
    else
    {
        ...
        uint8_t data0 = decrypt_byte(data, data, decrypt_key);
        si->is_kf = 0;
        if (data_sz >= 10 && !(data0 & 0x01))  /* I-Frame */
        {
            uint8_t data3 = decrypt_byte(data + 3, data, decrypt_key);
            uint8_t data4 = decrypt_byte(data + 4, data, decrypt_key);
            uint8_t data5 = decrypt_byte(data + 5, data, decrypt_key);
            uint8_t data6 = decrypt_byte(data + 6, data, decrypt_key);
            uint8_t data7 = decrypt_byte(data + 7, data, decrypt_key);
            uint8_t data8 = decrypt_byte(data + 8, data, decrypt_key);
            uint8_t data9 = decrypt_byte(data + 9, data, decrypt_key);
