...
static INLINE void vp8_filter_block1dc_v6_mmi(unsigned short *src_ptr,
...
static INLINE void vp8_filter_block1d_h6_filter0_mmi(
    unsigned char *src_ptr, unsigned short *output_ptr,
...
static INLINE void vp8_filter_block1dc_v6_filter0_mmi(
    unsigned short *src_ptr, unsigned char *output_ptr,
    unsigned int output_height, int output_pitch,
    unsigned int pixels_per_line) {
...
#define sixtapNxM(n, m)                                                        \
  void vp8_sixtap_predict##n##x##m##_mmi(                                      \
    ...
    if (yoffset == 0) {                                                        \
      for (i = 0; i < loop; ++i) {                                             \
        vp8_filter_block1dc_v6_filter0_mmi(FData2 + n * 2 + i * 4,             \
                                           dst_ptr + i * 4, m,                 \
                                           dst_pitch, n * 2);                  \
      }                                                                        \
    } else {                                                                   \
