  ...
  for (mb_row = 0; mb_row < cm->mb_rows - 1; mb_row += 1) {
    ...
    for (mb_col = 0; mb_col < cm->mb_cols - 1; mb_col += 1) {
      ...
      if (mode_filter == 1) {
        ...
        uint8_t ysource2 = src_y[(8 + 1) * src_ystride + 8];
        uint8_t usource2 = src_u[(4 + 1) * src_uvstride + 4];
        uint8_t vsource2 = src_v[(4 + 1) * src_uvstride + 4];
        uint8_t ysource3 = src_y[8 * src_ystride + (8 + 1)];
        uint8_t usource3 = src_u[4 * src_uvstride + (4 + 1)];
        uint8_t vsource3 = src_v[4 * src_uvstride + (4 + 1)];
        uint8_t ysource4 = src_y[(8 + 1) * src_ystride + (8 + 1)];
        uint8_t usource4 = src_u[(4 + 1) * src_uvstride + (4 + 1)];
        uint8_t vsource4 = src_v[(4 + 1) * src_uvstride + (4 + 1)];
        ...
        int bl_index2 = bl_index + cm->mb_cols;
