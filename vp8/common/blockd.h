...
typedef struct {
  ...
  uint8_t mb_skip_coeff; /* does this mb has coefficients at all, 1=no
                            coefficients, 0=need decode tokens */
  uint8_t need_to_clamp_mvs;
  uint8_t segment_id; /* Which set of segmentation parameters should be used for
                         this MB */
} MB_MODE_INFO;
...
typedef struct macroblockd {
  ...
  vp8_prob mb_segment_tree_probs[MB_FEATURE_TREE_PROBS]; /* Probability Tree
                                                          * used to code Segment
                                                          * number
                                                            */

  signed char segment_feature_data[MB_LVL_MAX][MAX_MB_SEGMENTS]; /* Segment
                                                                    parameters
                                                                    */
  ...
  signed char last_mode_lf_deltas[MAX_MODE_LF_DELTAS]; /* 0 = BPRED, ZERO_MV,
                                                          MV, SPLIT */
  signed char
