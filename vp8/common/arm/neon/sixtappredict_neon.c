...
static INLINE uint8x8_t load_and_shift(unsigned char *a) {
  return vreinterpret_u8_u64(
      vshl_n_u64(vreinterpret_u64_u8(vld1_u8(a)), 32));
}

static INLINE void filter_add_accumulate(uint8x16_t a, uint8x16_t b,
                                         uint8x8_t filter,
...
static INLINE void filter_sub_accumulate(uint8x16_t a, uint8x16_t b,
                                         uint8x8_t filter,
                                         uint16x8_t *c, uint16x8_t *d) {
  const uint32x2x2_t a_shuf = vzip_u32(vreinterpret_u32_u8(vget_low_u8(a)),
                                       vreinterpret_u32_u8(vget_high_u8(a)));
...
void vp8_sixtap_predict4x4_neon(unsigned char *src_ptr, int src_pixels_per_line,
  ...
  if (xoffset == 0) {  // secondpass_filter4x4_only
    ...
    const uint8x8_t filter = vld1_u8(abs_filters[yoffset]);
    ...
    src_ptr -= src_pixels_per_line * 2;
    // Shift the even rows to allow using 'vext' to combine the vectors. armv8
    // has vcopy_lane which would be interesting. This started as just a horrible
    // workaround for clang adding alignment hints to 32bit loads:
    ...
    a3 = vld1_u8(src_ptr);
    src_ptr += src_pixels_per_line;
    a4 = load_and_shift(src_ptr);
    src_ptr += src_pixels_per_line;
    a5 = vld1_u8(src_ptr);
    src_ptr += src_pixels_per_line;
    a6 = load_and_shift(src_ptr);
    src_ptr += src_pixels_per_line;
    a7 = vld1_u8(src_ptr);
    src_ptr += src_pixels_per_line;
    a8 = vld1_u8(src_ptr);
    ...
    // Add more positive ones. vmlal sholud really return a signed type.
    ...
    // numbers in there.
    c0 = vreinterpretq_u16_s16(vqaddq_s16(vreinterpretq_s16_u16(c2),
                                          vreinterpretq_s16_u16(c0)));
    c1 = vreinterpretq_u16_s16(vqaddq_s16(vreinterpretq_s16_u16(c3),
    ...
    // Compared to casting to uint32_t and using vst1() this approximately
    // doubles runtime of the function. But it is still 10x faster than C.
    // Clang doesn't like this though, so only do it when dst_ptr is properly
    // aligned.
    if (!((int)dst_ptr & 0x3) && !(dst_pitch & 0x3)) {
      vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(d0), 0);
      dst_ptr += dst_pitch;
      vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(d0), 1);
      dst_ptr += dst_pitch;
      vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(d1), 0);
      dst_ptr += dst_pitch;
      vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(d1), 1);
    } else {
      dst_ptr[0] = vget_lane_u8(d0, 0);
      dst_ptr[1] = vget_lane_u8(d0, 1);
      dst_ptr[2] = vget_lane_u8(d0, 2);
      dst_ptr[3] = vget_lane_u8(d0, 3);

      dst_ptr += dst_pitch;

      dst_ptr[0] = vget_lane_u8(d0, 4);
      dst_ptr[1] = vget_lane_u8(d0, 5);
      dst_ptr[2] = vget_lane_u8(d0, 6);
      dst_ptr[3] = vget_lane_u8(d0, 7);

      dst_ptr += dst_pitch;

      dst_ptr[0] = vget_lane_u8(d1, 0);
      dst_ptr[1] = vget_lane_u8(d1, 1);
      dst_ptr[2] = vget_lane_u8(d1, 2);
      dst_ptr[3] = vget_lane_u8(d1, 3);

      dst_ptr += dst_pitch;

      dst_ptr[0] = vget_lane_u8(d1, 4);
      dst_ptr[1] = vget_lane_u8(d1, 5);
      dst_ptr[2] = vget_lane_u8(d1, 6);
      dst_ptr[3] = vget_lane_u8(d1, 7);
    }
...

  {
    ...
    uint32x2x2_t s01_f3_q, s23_f3_q,
                 s01_f5_q, s23_f5_q;
    ...
    uint8x8_t filter = vld1_u8(abs_filters[xoffset]);
    uint8x8_t filter0 = vdup_lane_u8(filter, 0);
    uint8x8_t filter1 = vdup_lane_u8(filter, 1);
    uint8x8_t filter2 = vdup_lane_u8(filter, 2);
    uint8x8_t filter3 = vdup_lane_u8(filter, 3);
    uint8x8_t filter4 = vdup_lane_u8(filter, 4);
    uint8x8_t filter5 = vdup_lane_u8(filter, 5);
    ...
    // Can't use vshr.u64 because it crosses the double word boundery.
    ...
    s01_f5_q = vzip_u32(vreinterpret_u32_u8(s0_f5),
                        vreinterpret_u32_u8(s1_f5));
    s23_f5_q = vzip_u32(vreinterpret_u32_u8(s2_f5),
                        vreinterpret_u32_u8(s3_f5));
    d0123 = vmull_u8(vreinterpret_u8_u32(s01_f5_q.val[0]), filter5);
    d4567 = vmull_u8(vreinterpret_u8_u32(s23_f5_q.val[0]), filter5);

    // Keep original src data as 64 bits to simplfying shifting and extracting.
    ...
    d0123 = vreinterpretq_u16_s16(vqaddq_s16(vreinterpretq_s16_u16(d0123),
                                             vreinterpretq_s16_u16(d0123_a)));
    d4567 = vreinterpretq_u16_s16(vqaddq_s16(vreinterpretq_s16_u16(d4567),
                                             vreinterpretq_s16_u16(d4567_a)));
    ...
    if (yoffset == 0) {  // firstpass_filter4x4_only
      if (!((int)dst_ptr & 0x3) && !(dst_pitch & 0x3)) {
        vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(b0), 0);
        dst_ptr += dst_pitch;
        vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(b0), 1);
        dst_ptr += dst_pitch;
        vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(b2), 0);
        dst_ptr += dst_pitch;
        vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(b2), 1);
      } else {
        dst_ptr[0] = vget_lane_u8(b0, 0);
        dst_ptr[1] = vget_lane_u8(b0, 1);
        dst_ptr[2] = vget_lane_u8(b0, 2);
        dst_ptr[3] = vget_lane_u8(b0, 3);

        dst_ptr += dst_pitch;

        dst_ptr[0] = vget_lane_u8(b0, 4);
        dst_ptr[1] = vget_lane_u8(b0, 5);
        dst_ptr[2] = vget_lane_u8(b0, 6);
        dst_ptr[3] = vget_lane_u8(b0, 7);

        dst_ptr += dst_pitch;

        dst_ptr[0] = vget_lane_u8(b2, 0);
        dst_ptr[1] = vget_lane_u8(b2, 1);
        dst_ptr[2] = vget_lane_u8(b2, 2);
        dst_ptr[3] = vget_lane_u8(b2, 3);

        dst_ptr += dst_pitch;

        dst_ptr[0] = vget_lane_u8(b2, 4);
        dst_ptr[1] = vget_lane_u8(b2, 5);
        dst_ptr[2] = vget_lane_u8(b2, 6);
        dst_ptr[3] = vget_lane_u8(b2, 7);
      }
      return;
    }
    ...
    d4567 = vreinterpretq_u16_s16(vqaddq_s16(vreinterpretq_s16_u16(d4567),
                                             vreinterpretq_s16_u16(d4567_a)));
    d89 = vreinterpretq_u16_s16(vqaddq_s16(vreinterpretq_s16_u16(d89),
                                           vreinterpretq_s16_u16(d89_a)));
