...
static INLINE uint8x8_t load_and_shift(unsigned char *a) {
  return vreinterpret_u8_u64(vshl_n_u64(vreinterpret_u64_u8(vld1_u8(a)), 32));
}

static INLINE void store4x4(unsigned char *dst_ptr, int dst_pitch, uint8x8_t a0,
                            uint8x8_t a1) {
  if (!((uintptr_t)dst_ptr & 0x3) && !(dst_pitch & 0x3)) {
    vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(a0), 0);
    dst_ptr += dst_pitch;
    vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(a0), 1);
    dst_ptr += dst_pitch;
    vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(a1), 0);
    dst_ptr += dst_pitch;
    vst1_lane_u32((uint32_t *)dst_ptr, vreinterpret_u32_u8(a1), 1);
  } else {
    dst_ptr[0] = vget_lane_u8(a0, 0);
    dst_ptr[1] = vget_lane_u8(a0, 1);
    dst_ptr[2] = vget_lane_u8(a0, 2);
    dst_ptr[3] = vget_lane_u8(a0, 3);

    dst_ptr += dst_pitch;

    dst_ptr[0] = vget_lane_u8(a0, 4);
    dst_ptr[1] = vget_lane_u8(a0, 5);
    dst_ptr[2] = vget_lane_u8(a0, 6);
    dst_ptr[3] = vget_lane_u8(a0, 7);

    dst_ptr += dst_pitch;

    dst_ptr[0] = vget_lane_u8(a1, 0);
    dst_ptr[1] = vget_lane_u8(a1, 1);
    dst_ptr[2] = vget_lane_u8(a1, 2);
    dst_ptr[3] = vget_lane_u8(a1, 3);

    dst_ptr += dst_pitch;

    dst_ptr[0] = vget_lane_u8(a1, 4);
    dst_ptr[1] = vget_lane_u8(a1, 5);
    dst_ptr[2] = vget_lane_u8(a1, 6);
    dst_ptr[3] = vget_lane_u8(a1, 7);
  }
}
...
void vp8_bilinear_predict4x4_neon(unsigned char *src_ptr,
  ...
  if (yoffset == 0) {  // skip_2ndpass_filter
    ...
    b0 = vmull_u8(e0, filter0);
    b1 = vmull_u8(e1, filter0);
