...
#define MV_MAX_UPDATE_FACTOR 192
...
const nmv_context vp8_default_nmv_context = {
  {32, 64, 96},
  {
    { /* vert component */
      ...
      {64, 96, 64},                                    /* fp */
      {{192, 176, 168, 160}, {176, 160, 152, 144}},    /* class0_hp bit */
      128,                                             /* hp */
    },
    { /* hor component */
      128,                                             /* sign */
      {200, 128, 176, 160, 176, 176, 192},             /* class */
      {208},                                           /* class0 */
      {136, 140, 148, 160, 176, 192, 224},             /* bits */
      {{128, 128, 64,}, {96, 112, 64}},                /* class0_fp */
      {64, 96, 64},                                    /* fp */
...
static void increment_nmv_component(int v,
  ...
  if (usehp) {
    if (c == MV_CLASS_0) {
      mvcomp->class0_hp[d][f][e]++;
    } else {
      mvcomp->hp[e]++;
    }
  } else {  /* assume the extra bit is 1 */
    if (c == MV_CLASS_0) {
      mvcomp->class0_hp[d][f][1]++;
    } else {
      mvcomp->hp[1]++;
    }
  }
  ...
  if (usehp) {
    for (i = 0; i < 2; ++i) {
      for (j = 0; j < CLASS0_SIZE; ++j) {
        for (k = 0; k < 4; ++k) {
          prob->comps[i].class0_hp[j][k] = vp8_bin_prob_from_distribution(
              NMVcount->comps[i].class0_hp[j][k]);
          branch_ct_class0_hp[i][j][k][0] =
              NMVcount->comps[i].class0_hp[j][k][0];
          branch_ct_class0_hp[i][j][k][1] =
              NMVcount->comps[i].class0_hp[j][k][1];
        }
      }
      prob->comps[i].hp =
...
void vp8_adapt_nmv_probs(VP8_COMMON *cm, int usehp) {
  ...
  if (usehp) {
    ...
    for (i = 0; i < 2; ++i) {
      for (j = 0; j < CLASS0_SIZE; ++j) {
        printf("{");
        for (k = 0; k < 4; ++k)
          printf("%d/%d ", cm->fc.NMVcount.comps[i].class0_hp[j][0],
                           cm->fc.NMVcount.comps[i].class0_hp[j][1]);
        printf("}, ");
      }
      printf("\n"); fflush(stdout);
    }
  ...
  if (usehp) {
    for (i = 0; i < 2; ++i) {
      for (j = 0; j < CLASS0_SIZE; ++j) {
        for (k = 0; k < 4; ++k) {
          adapt_prob(&cm->fc.nmvc.comps[i].class0_hp[j][k],
                     cm->fc.pre_nmvc.comps[i].class0_hp[j][k],
                     prob.comps[i].class0_hp[j][k],
                     branch_ct_class0_hp[i][j][k]);
        }
      }
