...
void vp8_counts_to_nmv_context(
  ...
  for (i = 0; i < 2; ++i) {
    vp8_tree_probs_from_distribution(4,
                                     vp8_mv_fp_encodings,
                                     vp8_mv_fp_tree,
                                     prob->comps[i].class0_fp,
                                     branch_ct_class0_fp[i],
                                     NMVcount->comps[i].class0_fp,
                                     256, 1);
    vp8_tree_probs_from_distribution(4,
                                     vp8_mv_fp_encodings,
                                     vp8_mv_fp_tree,
...
void vp8_adapt_nmv_probs(VP8_COMMON *cm, int usehp) {
  ...
  for (i = 0; i < 2; ++i) {
    for (j = 0; j < 4; ++j)
      printf("%d ", cm->fc.NMVcount.comps[i].class0_fp[j]);
    printf("\n"); fflush(stdout);
  }
  printf("fp count:\n");
  for (i = 0; i < 2; ++i) {
  ...
  for (i = 0; i < 2; ++i) {
    for (j = 0; j < 3; ++j) {
      adapt_prob(&cm->fc.nmvc.comps[i].class0_fp[j],
                 cm->fc.pre_nmvc.comps[i].class0_fp[j],
                 prob.comps[i].class0_fp[j],
                 branch_ct_class0_fp[i][j]);
    }
    for (j = 0; j < 3; ++j) {
