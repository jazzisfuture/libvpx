...
const nmv_context vp8_default_nmv_context = {
  {32, 64, 96},
  {
    { /* vert component */
      128,                                             /* sign */
      {192, 144, 192, 168, 192, 176, 192},             /* class */
      {225, 146, 192, 147, 168, 144, 144},             /* class0 */
    ...
    { /* hor component */
      128,                                             /* sign */
      {176, 128, 176, 160, 176, 176, 192},             /* class */
      {208, 170, 160, 235, 160, 144, 128},             /* class0 */
      {128, 132, 136, 140, 148, 160, 176, 192, 224},   /* bits */
      {192, 176, 168, 160, 160, 160, 160, 160},        /* class0 hp bit */
      128,                                             /* hp bit */
    }
    ...
    for (i = 0; i < 2; ++i) {
      for (j = 0; j < CLASS0_SIZE; ++j) {
        prob->comps[i].class0_hp[j] = vp8_bin_prob_from_distribution(
            NMVcount->comps[i].class0_hp[j]);
        branch_ct_class0_hp[i][j][0] = NMVcount->comps[i].class0_hp[j][0];
        branch_ct_class0_hp[i][j][1] = NMVcount->comps[i].class0_hp[j][1];
      }
      prob->comps[i].bits_hp =
          vp8_bin_prob_from_distribution(NMVcount->comps[i].bits_hp);
      branch_ct_bits_hp[i][0] = NMVcount->comps[i].bits_hp[0];
      branch_ct_bits_hp[i][1] = NMVcount->comps[i].bits_hp[1];
    }
  }
}
