...
#define VP8_NMV_UPDATE_PROB  244
...
#define CLASS0_BITS    3  /* bits at regular precision for class 0 */
#define CLASS0_SIZE    (1 << CLASS0_BITS)
#define MV_MAX         ((1 << (MV_CLASSES + CLASS0_BITS)) - 1)
#define MV_MAX_BITS    (MV_CLASSES + CLASS0_BITS)
#define MV_VALS        ((1 << (MV_CLASSES + CLASS0_BITS + 1)) - 1)
...
typedef struct {
  ...
  vp8_prob class0_hp[CLASS0_SIZE];
  vp8_prob bits_hp;
} nmv_component;
...
typedef struct {
  ...
  unsigned int class0_hp[CLASS0_SIZE][2];
  unsigned int bits_hp[2];
...
void vp8_increment_nmv(const MV *mv, nmv_context_counts *mvctx, int usehp);
...
void vp8_counts_to_nmv_context(
    ...
    unsigned int (*branch_ct_class0_hp)[CLASS0_SIZE][2],
    unsigned int (*branch_ct_bits_hp)[2]);
