...
static __inline void vp9_filter(signed char mask, uc hev, uc *op1,
                                uc *op0, uc *oq0, uc *oq1)

{
  signed char ps0, qs0;
  signed char ps1, qs1;
  signed char vp9_filter, Filter1, Filter2;
  signed char u;
  ...
  vp9_filter = signed_char_clamp(ps1 - qs1);
  vp9_filter &= hev;

  /* inner taps */
  vp9_filter = signed_char_clamp(vp9_filter + 3 * (qs0 - ps0));
  vp9_filter &= mask;
   ...
   */
  Filter1 = signed_char_clamp(vp9_filter + 4);
  Filter2 = signed_char_clamp(vp9_filter + 3);
  Filter1 >>= 3;
  Filter2 >>= 3;
  ...
  *op0 = u ^ 0x80;
  ...
  u = signed_char_clamp(qs1 - vp9_filter);
  *oq1 = u ^ 0x80;
  u = signed_char_clamp(ps1 + vp9_filter);
  *op1 = u ^ 0x80;
...
(
  ...
  do {
    ...
    vp9_filter(mask, hev, s - 2 * p, s - 1 * p, s, s + 1 * p);
...
(
  ...
  do {
    ...
    vp9_filter(mask, hev, s - 2, s - 1, s, s + 1);
