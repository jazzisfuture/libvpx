...
int vp8_alloc_internal_frame_buffers(InternalFrameBufferList *list) {
  assert(list != NULL);
  vp8_free_internal_frame_buffers(list);

  list->num_internal_frame_buffers = NUM_FRAME_BUFS;
  list->int_fb =
...
void vp8_free_internal_frame_buffers(InternalFrameBufferList *list) {
  int i;
...
int vp8_get_frame_buffer(void *cb_priv, size_t min_size,
...
int vp8_release_frame_buffer(void *cb_priv, vpx_codec_frame_buffer_t *fb) {
