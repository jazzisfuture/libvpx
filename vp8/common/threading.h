...
// TODO(pbos): Try to support const vpx_atomic_int.
static INLINE void vp8_atomic_sync_read(int mb_col,
                                        vpx_atomic_int *last_row_current_mb_col,
                                        const int nsync) {
  while (mb_col > (vpx_atomic_read(last_row_current_mb_col) - nsync)) {
    x86_pause_hint();
    thread_sleep(0);
  }
}
