...
(
 ...
 unsigned int   dst_stride
  ...
  // have enough horizontally filtered values as input fot the vertical pass.
  ...
  // Horizontal pass.
  {
    ...
    const int next_row_stride = src_stride - output_width;
    unsigned int i, j;
  ...
  // Vertical pass.
  {
    ...
    const int next_row_stride = dst_stride - output_width;
    unsigned int i, j;
    for (i = 0; i < output_height; i++) {
      for (j = 0; j < output_width; j++) {
        // Apply filter...
        int temp = ((int)src_ptr[0 * output_width] * VFilter[0]) +
                   ((int)src_ptr[1 * output_width] * VFilter[1]) +
                   ((int)src_ptr[2 * output_width] * VFilter[2]) +
                   ((int)src_ptr[3 * output_width] * VFilter[3]) +
                   ((int)src_ptr[4 * output_width] * VFilter[4]) +
                   ((int)src_ptr[5 * output_width] * VFilter[5]) +
                   ((int)src_ptr[6 * output_width] * VFilter[6]) +
                   ((int)src_ptr[7 * output_width] * VFilter[7]) +
                   (VP8_FILTER_WEIGHT >> 1); // Rounding
