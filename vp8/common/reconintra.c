...
void vp8_build_intra_predictors_mby_internal(MACROBLOCKD *x, unsigned char *src, int src_stride,
                                             unsigned char *ypred_ptr, int y_stride, int mode, int bsize) {
  ...
  switch (mode) {
    case DC_PRED: {
      ...
      if (x->up_available || x->left_available) {
        ...
        shift = log2_bsize_min1 + x->up_available + x->left_available;
...
void vp8_build_intra_predictors_mby(MACROBLOCKD *x) {
  vp8_build_intra_predictors_mby_internal(x, x->dst.y_buffer, x->dst.y_stride,
                                          x->predictor, 16,
                                          x->mode_info_context->mbmi.mode, 16);
}
...
void vp8_build_intra_predictors_mbuv_internal(MACROBLOCKD *x,
                                              ...
                                              int uv_stride,
                                              int mode, int bsize) {
  vp8_build_intra_predictors_mby_internal(x, x->dst.u_buffer, x->dst.uv_stride,
                                          upred_ptr, uv_stride, mode, bsize);
  vp8_build_intra_predictors_mby_internal(x, x->dst.v_buffer, x->dst.uv_stride,
                                          vpred_ptr, uv_stride, mode, bsize);
}

void vp8_build_intra_predictors_mbuv(MACROBLOCKD *x) {
  vp8_build_intra_predictors_mbuv_internal(x, &x->predictor[256], &x->predictor[320], 8,
                                           x->mode_info_context->mbmi.uv_mode, 8);
}

void vp8_build_intra_predictors_mbuv_s(MACROBLOCKD *x) {
  vp8_build_intra_predictors_mbuv_internal(x, x->dst.u_buffer, x->dst.v_buffer,
                                           x->dst.uv_stride,
                                           x->mode_info_context->mbmi.uv_mode, 8);
}

void vp8_build_intra_predictors_sbuv_s(MACROBLOCKD *x) {
  vp8_build_intra_predictors_mbuv_internal(x, x->dst.u_buffer, x->dst.v_buffer,
                                           x->dst.uv_stride,
                                           x->mode_info_context->mbmi.uv_mode, 16);
}
