...
)
{
    ...
    if (blksize == 16)
    {
        ...
        sad = (vp8_variance_sad16x16(y, y_stride, yd, yd_stride, INT_MAX)+128)>>8;
    }
    else if (blksize == 8)
    {
        act = (vp8_variance_var8x8(yd, yd_stride, VP8_ZEROS, 0, &sse)+32)>>6;
        sad = (vp8_variance_sad8x8(y, y_stride, yd, yd_stride, INT_MAX)+32)>>6;
    }
    else
    {
        act = (vp8_variance_var4x4(yd, yd_stride, VP8_ZEROS, 0, &sse)+8)>>4;
        sad = (vp8_variance_sad4x4(y, y_stride, yd, yd_stride, INT_MAX)+8)>>4;
    }
