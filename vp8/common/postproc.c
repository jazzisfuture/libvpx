...
void vp9_de_noise(YV12_BUFFER_CONFIG         *source,
  ...
  POSTPROC_INVOKE(rtcd, downacross)(source->y_buffer + 2 * source->y_stride + 2,
                                    ...
                                    source->y_width - 4,
                                    ppl);
  POSTPROC_INVOKE(rtcd, downacross)(source->u_buffer + 2 * source->uv_stride + 2,
                                    source->u_buffer + 2 * source->uv_stride + 2,
                                    source->uv_stride,
                                    source->uv_stride,
                                    source->uv_height - 4,
                                    source->uv_width - 4, ppl);
  POSTPROC_INVOKE(rtcd, downacross)(source->v_buffer + 2 * source->uv_stride + 2,
...
int vp9_post_proc_frame(VP9_COMMON *oci, YV12_BUFFER_CONFIG *dest,
  ...
  if (flags & VP9D_DEBUG_TXT_RATE_INFO) {
    char message[512];
    sprintf(message, "Bitrate: %10.2f frame_rate: %10.2f ",
