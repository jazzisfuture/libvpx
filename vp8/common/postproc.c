...
)
{
    ...
    for (row = 0; row < size; row++)
    {
        ...
        for (col = 0; col < cols; col++)
        {
            ...
            mask &= (abs(v - p_src[col - 2 * src_pixels_per_line]) < f[col]);
            mask &= (abs(v - p_src[col - src_pixels_per_line]) < f[col]);
            mask &= (abs(v - p_src[col + src_pixels_per_line]) < f[col]);
            mask &= (abs(v - p_src[col + 2 * src_pixels_per_line]) < f[col]);
            ...
            if (mask)
            {
                ...
                k1 = (p_src[col - 2 * src_pixels_per_line]
                    + p_src[col - src_pixels_per_line] + 1) >> 1;
                k2 = (p_src[col + 2 * src_pixels_per_line]
                    + p_src[col + src_pixels_per_line] + 1) >> 1;
                k3 = (k1 + k2 + 1) >> 1;
                v = (k3 + v + 1) >> 1;
            }
        ...
        for (col = 0; col < cols; col++)
        {
            ...
            mask &= (abs(v - p_src[col - 2]) < f[col]);
            mask &= (abs(v - p_src[col - 1]) < f[col]);
            mask &= (abs(v - p_src[col + 1]) < f[col]);
            mask &= (abs(v - p_src[col + 2]) < f[col]);
...
static void vp8_deblock_and_de_macro_block(
        VP8_COMMON                 *cm,
                  ...
                  int                         flag)
{
    ...
    vpx_memset(limits, ppl, 16 * mb_cols);
...
int vp8_post_proc_frame(VP8_COMMON *oci, YV12_BUFFER_CONFIG *dest, vp8_ppflags_t *ppflags)
{
         ...
         oci->base_qindex - oci->postproc_state.last_base_qindex >= 20)
    {
            ...
            oci->post_proc_buffer_int_used)
        {
            ...
            {
                vp8_deblock_and_de_macro_block(oci, &oci->post_proc_buffer_int, &oci->post_proc_buffer,
    ...
    {
        vp8_deblock_and_de_macro_block(oci, oci->frame_to_show, &oci->post_proc_buffer,
                                       q + (deblock_level - 5) * 10, 1, 0);
        oci->postproc_state.last_base_qindex = oci->base_qindex;
    }
    else if (flags & VP8D_DEBLOCK)
