...
int vp8_post_proc_frame(VP8_COMMON *oci, YV12_BUFFER_CONFIG *dest,
  ...
  if (flags & VP8D_ADDNOISE) {
    if (oci->postproc_state.last_q != q ||
      ...
      clamp = vpx_setup_noise(sigma, sizeof(ppstate->noise), ppstate->noise);
      for (i = 0; i < 16; i++) {
    ...
    vpx_plane_add_noise(
        oci->post_proc_buffer.y_buffer, oci->postproc_state.noise,
        oci->postproc_state.blackclamp, oci->postproc_state.whiteclamp,
        oci->postproc_state.bothclamp, oci->post_proc_buffer.y_width,
        oci->post_proc_buffer.y_height, oci->post_proc_buffer.y_stride);
  }
