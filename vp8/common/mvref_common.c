...
void scale_mv(
  ...
  if (candidate_mi->mbmi.ref_frame != here->mbmi.ref_frame) {

    // Sign inversion where appropriate.
    if (ref_sign_bias[candidate_mi->mbmi.ref_frame] !=
        ref_sign_bias[here->mbmi.ref_frame]) {
      new_mv->as_mv.row = -(candidate_mi->mbmi.mv.as_mv.row);
      new_mv->as_mv.col = -(candidate_mi->mbmi.mv.as_mv.col);
    } else {
      new_mv->as_int = candidate_mi->mbmi.mv.as_int;
    }
    ...
    if (frame_distances[here->mbmi.ref_frame] &&
        frame_distances[candidate_mi->mbmi.ref_frame]) {
      new_mv->as_mv.row =
        (short)(((int)(new_mv->as_mv.row) *
                 frame_distances[here->mbmi.ref_frame]) /
                frame_distances[candidate_mi->mbmi.ref_frame]);
...
void find_mv_refs(
  ...
  for (i = 0; i < 2; ++i)
  {
    if (((mv_ref_search[i][0] << 7) >= xd->mb_to_left_edge) &&
      ...
      if (valid_mvref(here, candidate_mi)) {
          ref_match = candidate_mi->mbmi.ref_frame == here->mbmi.ref_frame;
          scale_mv(xd, here, candidate_mi, ref_sign_bias, &new_mv );
  ...
  // Populate a list with candidate reference vectors from the
  // spatial neighbours.
  for (i = 0; i < MVREF_NEIGHBOURS - 2; ++i)
  {
    if (((mv_ref_search[i][0] << 7) >= xd->mb_to_left_edge) &&
        ((mv_ref_search[i][1] << 7) >= xd->mb_to_top_edge)) {
