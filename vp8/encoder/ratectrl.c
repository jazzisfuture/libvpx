...
int vp8_drop_encodedframe_overshoot(VP8_COMP *cpi, int Q) {
  int force_drop_overshoot = 0;
    ...
    // If the lowest stream of the multi-res encoding was dropped due to overshoot,
    // we force dropping on all upper layer streams (mr_encoder_id > 0).
  ...
  if (cpi->common.frame_type != KEY_FRAME &&
      (cpi->oxcf.screen_content_mode == 2 ||
       (cpi->drop_frames_allowed && (force_drop_overshoot ||
    ...
    if ((Q < thresh_qp && cpi->projected_frame_size > thresh_rate &&
        pred_err_mb > thresh_pred_err_mb) || force_drop_overshoot) {
