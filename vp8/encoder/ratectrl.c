...
int vp8_drop_encodedframe_overshoot(VP8_COMP *cpi, int Q) {
  ...
  // Reset counter if this frame is being re-encoded at max QP due to overshoot.
  if (cpi->force_maxqp) cpi->frames_since_last_drop_overshoot = 0;
  ...
  if (cpi->oxcf.mr_encoder_id > 0) {
    force_drop_overshoot = low_res_frame_info->is_frame_dropped;
  ...
  if (cpi->common.frame_type != KEY_FRAME &&
      (cpi->oxcf.screen_content_mode == 2 ||
       (cpi->drop_frames_allowed &&
        (force_drop_overshoot ||
         (cpi->rate_correction_factor < (3.0f * MIN_BPB_FACTOR) &&
    ...
    // Reduce the thresh_rate is pred_err_mb much larger than its threshold,
    // this is to give more weight to pred_err metric for overshoot detection.
    if (cpi->drop_frames_allowed && pred_err_mb > (thresh_pred_err_mb << 1))
      thresh_rate = (3 * thresh_rate) >> 2;
    if ((Q < thresh_qp && cpi->projected_frame_size > thresh_rate &&
      ...
      cpi->bits_off_target = cpi->oxcf.optimal_buffer_level;
      if (cpi->oxcf.number_of_layers > 1) {
        // Set max_qp for all temporal layers if overshoot is detected.
...
#if CONFIG_MULTI_RES_ENCODING
      low_res_frame_info->is_frame_dropped = 1;
#endif
      return 1;
    }
    cpi->force_maxqp = 0;
    cpi->frames_since_last_drop_overshoot++;
#if CONFIG_MULTI_RES_ENCODING
    low_res_frame_info->is_frame_dropped = 0;
#endif
    return 0;
  }
  cpi->force_maxqp = 0;
