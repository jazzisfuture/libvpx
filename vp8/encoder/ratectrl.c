...
// has large overshoot, and was not being encoded close to max QP, then
...
int vp8_drop_encodedframe_overshoot(VP8_COMP *cpi, int Q) {
  if (cpi->pass == 0 &&
    ...
    // projected_frame_size is somewhat greater than per-frame-bandwidth, but
    // add additional condition with high threshold on prediction residual.
    ...
    // Threshold for the average (over whole frame) of the sum residual error
    // of 16x16 blocks. Add QP dependence on threshold?
