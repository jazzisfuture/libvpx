...
int vp8_drop_encodedframe_overshoot(VP8_COMP *cpi, int Q) {
  if (cpi->common.frame_type != KEY_FRAME &&
      (cpi->oxcf.screen_content_mode == 2 || (cpi->drop_frames_allowed &&
       cpi->rate_correction_factor < (2.0f * MIN_BPB_FACTOR) &&
       cpi->last_drop_overshoot > 8))) {
    ...
    // Reduce the thresh_rate at low Q if prediction is very big.
    if (cpi->oxcf.screen_content_mode != 2 && Q == cpi->best_quality &&
        pred_err_mb > (thresh_pred_err_mb << 2))
      thresh_rate = thresh_rate >> 1;
