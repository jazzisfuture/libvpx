  ...
  for (mode = DC_PRED; mode <= TM_PRED; mode++) {
    ...
    for (mode2 = DC_PRED - 1; mode2 != TM_PRED + 1; mode2++) {
      ...
      if (mode == DC_PRED || mode == V_PRED ||
          mode == H_PRED || mode == TM_PRED ||
          mode == D45_PRED || mode == D135_PRED ||
          mode == D117_PRED || mode == D153_PRED ||
          mode == D27_PRED || mode == D63_PRED)
...
#if CONFIG_TX16X16
    if (xd->mode_info_context->mbmi.mode == DC_PRED ||
        xd->mode_info_context->mbmi.mode == H_PRED ||
        xd->mode_info_context->mbmi.mode == V_PRED ||
        xd->mode_info_context->mbmi.mode == TM_PRED ||
        xd->mode_info_context->mbmi.mode == D45_PRED ||
        xd->mode_info_context->mbmi.mode == D135_PRED ||
        xd->mode_info_context->mbmi.mode == D117_PRED ||
        xd->mode_info_context->mbmi.mode == D153_PRED ||
        xd->mode_info_context->mbmi.mode == D27_PRED ||
        xd->mode_info_context->mbmi.mode == D63_PRED ||
