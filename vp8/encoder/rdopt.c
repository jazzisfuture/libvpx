...
void setup_buffer_inter(VP8_COMP *cpi, MACROBLOCK *x, int idx, int frame_type,
  ...
  // possible best reference vectors.
  {
    ...
    // For now copy over th best 4 candidates
    xd->ref_mv[0].as_int = mbmi->ref_mvs[frame_type][0].as_int;
    xd->ref_mv[1].as_int = mbmi->ref_mvs[frame_type][1].as_int;
    xd->ref_mv[2].as_int = mbmi->ref_mvs[frame_type][2].as_int;
    xd->ref_mv[3].as_int = mbmi->ref_mvs[frame_type][3].as_int;
  }
#endif
