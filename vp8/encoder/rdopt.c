...
static int cost_coeffs(MACROBLOCK *mb, BLOCKD *b, int type, ENTROPY_CONTEXT *a, ENTROPY_CONTEXT *l)
{
    ...
    int active_ht = ( QIndex < ACTIVE_HT ) &&
        ( (mb->e_mbd.mode_info_context->mbmi.mode_rdopt == B_PRED   ) ||
          (mb->e_mbd.mode_info_context->mbmi.mode_rdopt == I8X8_PRED) ) ;

    int const *pt_scan ;

    if( (type == PLANE_TYPE_Y_WITH_DC) && active_ht )
    {
      switch (b->bmi.as_mode.test) {
        case B_VE_PRED :
        case B_VR_PRED :
    ...
    int *bestdistortion)
{
    ...
    for (mode = B_DC_PRED; mode <= B_HU_PRED; mode++)
    {
        ...
        if(active_ht)
        {
          ...
          vp8_fht4x4_c( be->src_diff, be->coeff, 32, mode ) ;
    ...
    if(active_ht)
    {
      vp8_iht4x4llm_c(best_dqcoeff, b->diff, 32, b->bmi.as_mode.first );
    }
    else
    {
