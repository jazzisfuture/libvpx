    ...
    case TX_4X4:
     ...
     //xd->mode_info_context->mbmi.txfm_size = tx_size;
      ...
      if (type == PLANE_TYPE_Y_WITH_DC &&
          (tx_type = get_tx_type_4x4(xd, b)) != DCT_DCT) {
        switch (tx_type) {
          case ADST_DCT:
            scan = vp8_row_scan;
            break;
...
#if CONFIG_HYBRIDTRANSFORM16X16
  if ((tx_type = get_tx_type_16x16(xd, b)) != DCT_DCT) {
...
static int64_t rd_pick_intra4x4block(VP8_COMP *cpi, MACROBLOCK *x, BLOCK *be,
  ...
  for (mode = B_DC_PRED; mode <= B_HU_PRED; mode++) {
    ...
    for (mode2 = (allow_comp ? 0 : (B_DC_PRED - 1));
      ...
      b->bmi.as_mode.first = mode;
#if CONFIG_HYBRIDTRANSFORM
      if ((tx_type = get_tx_type_4x4(xd, b)) != DCT_DCT) {
  ...
  RECON_INVOKE(IF_RTCD(&cpi->rtcd.common->recon), recon)(
      best_predictor, b->diff, *(b->base_dst) + b->dst, b->dst_stride);
...
static int64_t rd_pick_intra16x16mby_mode(VP8_COMP *cpi,
  ...
  for (mode = DC_PRED; mode <= TM_PRED; mode++) {
    ...
    for (mode2 = DC_PRED - 1; mode2 != TM_PRED + 1; mode2++) {
      ...
      rd_txtype = get_tx_type_16x16(&x->e_mbd, &x->e_mbd.block[0]);
#endif
...
#if CONFIG_HYBRIDTRANSFORM8X8
        TX_TYPE tx_type;
        if ((tx_type = get_tx_type_8x8(xd, b)) != DCT_DCT)
...
#if 0//CONFIG_HYBRIDTRANSFORM16X16
