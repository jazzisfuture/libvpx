...
static int64_t vp8_encode_inter_mb_segment(MACROBLOCK *x,
                                           int const *labels,
                                           int which_label,
                                           int *labelyrate,
                                           int *distortion,
...
static int64_t vp8_encode_inter_mb_segment_8x8(TXFM_MODE txfm_mode,
                                               MACROBLOCK *x,
                                               int const *labels,
                                               int which_label,
                                               int *labelyrate,
  ...
  for (i = 0; i < label_count; i++) {
    ...
    for (this_mode = LEFT4X4; this_mode <= NEW4X4; this_mode ++) {
      ...
      if (segmentation == BLOCK_4X4) {
        this_rd = vp8_encode_inter_mb_segment(x, labels, i, &labelyrate,
                                              &distortion,
                                              ta_s, tl_s, IF_RTCD(&cpi->rtcd));
      } else {
        this_rd = vp8_encode_inter_mb_segment_8x8(cpi->common.txfm_mode,
