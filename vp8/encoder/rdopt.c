...
void rd_update_mvcount(VP8_COMP *cpi, MACROBLOCK *x,
  ...
  if (x->e_mbd.mode_info_context->mbmi.mode == SPLITMV) {
    ...
    if (x->e_mbd.allow_high_precision_mv) {
      cpi->MVcount_hp[0][mv_max_hp + (x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.row
                                      - best_ref_mv->as_mv.row)]++;
      cpi->MVcount_hp[1][mv_max_hp + (x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.col
                                      - best_ref_mv->as_mv.col)]++;
      if (x->e_mbd.mode_info_context->mbmi.second_ref_frame) {
        cpi->MVcount_hp[0][mv_max_hp + (x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.row
                                        - second_best_ref_mv->as_mv.row)]++;
        cpi->MVcount_hp[1][mv_max_hp + (x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.col
                                        - second_best_ref_mv->as_mv.col)]++;
      }
    } else
    {
      cpi->MVcount[0][mv_max + ((x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.row
                                 - best_ref_mv->as_mv.row) >> 1)]++;
      cpi->MVcount[1][mv_max + ((x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.col
                                 - best_ref_mv->as_mv.col) >> 1)]++;
      if (x->e_mbd.mode_info_context->mbmi.second_ref_frame) {
        cpi->MVcount[0][mv_max + ((x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.row
                                   - second_best_ref_mv->as_mv.row) >> 1)]++;
        cpi->MVcount[1][mv_max + ((x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.col
                                   - second_best_ref_mv->as_mv.col) >> 1)]++;
      }
    }
  }
...
void vp8_rd_pick_inter_mode(VP8_COMP *cpi, MACROBLOCK *x, int recon_yoffset, int recon_uvoffset,
  ...
  *returnintra = INT64_MAX;
  ...
  if (best_mbmode.mode == SPLITMV) {
    ...
    x->e_mbd.mode_info_context->mbmi.mv[0].as_int =
      x->partition_info->bmi[15].mv.as_int;
    x->e_mbd.mode_info_context->mbmi.mv[1].as_int =
