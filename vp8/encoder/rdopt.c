...
static int64_t rd_pick_intra4x4block(VP8_COMP *cpi, MACROBLOCK *x, BLOCK *be,
  ...
  for (mode = B_DC_PRED; mode <= B_HU_PRED; mode++) {
    ...
    for (mode2 = (allow_comp ? 0 : (B_DC_PRED - 1));
      ...
      // TODO Temporarily ignore modes that need the above-right data for
      // blocks on the right column of a MB. For SB encoding, this data
      // is not available for the top right 4x4 block of the bottom right MB
      // only; so ideally we should need to skip these modes for only this
      // 4x4 block within the SB.
