...
typedef struct {
  ...
  int64_t segment_rd;
  SPLITMV_PARTITIONING segment_num;
  int r;
  int d;
  int segment_yrate;
...
static void rd_check_segment(VP8_COMP *cpi, MACROBLOCK *x,
                             BEST_SEG_INFO *bsi,
                             SPLITMV_PARTITIONING segmentation,
...
static int rd_pick_best_mbsegmentation(VP8_COMP *cpi, MACROBLOCK *x,
  ...
  if (cpi->compressor_speed == 0) {
    ...
    rd_check_segment(cpi, x, &bsi, PARTITIONING_16X8, seg_mvs[BLOCK_16X8]);
    rd_check_segment(cpi, x, &bsi, PARTITIONING_8X16, seg_mvs[BLOCK_8X16]);
    rd_check_segment(cpi, x, &bsi, PARTITIONING_8X8,  seg_mvs[BLOCK_8X8]);
    rd_check_segment(cpi, x, &bsi, PARTITIONING_4X4,  seg_mvs[BLOCK_4X4]);
  } else {
    int sr;
