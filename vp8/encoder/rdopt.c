...
int64_t vp8_rd_pick_inter_mode_sb(VP8_COMP *cpi, MACROBLOCK *x,
  ...
  for (mode_index = 0; mode_index < MAX_MODES; mode_index++) {    
    ...
    x->e_mbd.mode_info_context->mbmi.ref_frame = ref_frame;
    ...
    {
      {
        ...
        if (comp_pred) {
          ...
          if (!(cpi->ref_frame_flags & flag_list[second_ref]))
            continue;
          x->e_mbd.mode_info_context->mbmi.second_ref_frame = second_ref;
          ...
          x->e_mbd.mode_info_context->mbmi.second_ref_frame = INTRA_FRAME;
        ...
        if (0) printf("Counts[%d/%d/%d] (new=%d): %d, %d, %d, %d\n",
               this_mode, mode_index, ref_frame, NEWMV,
               mdcounts[0],
               mdcounts[1], mdcounts[2], mdcounts[3]);
        ...
        if (!comp_pred) {
          switch (this_mode) {
            ...
            case ZEROMV:
              ...
              vp8_build_inter32x32_predictors_sb(&x->e_mbd,
                                                 xd->dst.y_buffer,
                                                 xd->dst.u_buffer,
                                                 xd->dst.v_buffer,
                                                 xd->dst.y_stride,
              ...
              if (cpi->active_map_enabled && x->active_ptr[0] == 0) {
                ...
                var = VARIANCE_INVOKE(&cpi->rtcd.variance, var32x32)
                (*(b->base_src), b->src_stride,
              ...
              if (cpi->common.txfm_mode == ALLOW_8X8)
#endif
                rd_inter32x32_uv_8x8(cpi, x, &rate_uv,
                                     &distortion_uv,
          ...
          switch (this_mode) {
            case NEWMV:
              if (mc_search_result[ref1].as_int == INVALID_MV ||
                ...
                continue;
              x->e_mbd.mode_info_context->mbmi.mv[0].as_int = mc_search_result[ref1].as_int;
              x->e_mbd.mode_info_context->mbmi.mv[1].as_int = mc_search_result[ref2].as_int;
              ...
              break;
            case ZEROMV:
              x->e_mbd.mode_info_context->mbmi.mv[0].as_int = 0;
              x->e_mbd.mode_info_context->mbmi.mv[1].as_int = 0;
              break;
            case NEARMV:
              if (frame_near_mv[ref1].as_int == 0 || frame_near_mv[ref2].as_int == 0) {
                continue;
              }
              x->e_mbd.mode_info_context->mbmi.mv[0].as_int = frame_near_mv[ref1].as_int;
              x->e_mbd.mode_info_context->mbmi.mv[1].as_int = frame_near_mv[ref2].as_int;
              break;
            case NEARESTMV:
              if (frame_nearest_mv[ref1].as_int == 0 || frame_nearest_mv[ref2].as_int == 0) {
                continue;
              }
              x->e_mbd.mode_info_context->mbmi.mv[0].as_int = frame_nearest_mv[ref1].as_int;
              x->e_mbd.mode_info_context->mbmi.mv[1].as_int = frame_nearest_mv[ref2].as_int;
              break;
            default:
              break;
          }
          ...
          /* Add in the Mv/mode cost */
            ...
            vp8_clamp_mv2(&x->e_mbd.mode_info_context->mbmi.mv[0], xd);
            vp8_clamp_mv2(&x->e_mbd.mode_info_context->mbmi.mv[1], xd);
            if (((x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.row >> 3) < x->mv_row_min) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.row >> 3) > x->mv_row_max) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.col >> 3) < x->mv_col_min) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.col >> 3) > x->mv_col_max) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.row >> 3) < x->mv_row_min) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.row >> 3) > x->mv_row_max) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.col >> 3) < x->mv_col_min) ||
                ((x->e_mbd.mode_info_context->mbmi.mv[1].as_mv.col >> 3) > x->mv_col_max)) {
              continue;
            }
            
            /* build first and second prediction */
            vp8_build_inter32x32_predictors_sb(&x->e_mbd,
                                               xd->dst.y_buffer, xd->dst.u_buffer, xd->dst.v_buffer,
  ...
  if (!segfeature_active(xd, segment_id, SEG_LVL_REF_FRAME) &&
    ...
    store_coding_context(x, &x->sb_context[0], mode_index, NULL, /*&best_partition,*/
                         &frame_best_ref_mv[xd->mode_info_context->mbmi.ref_frame],
                         &frame_best_ref_mv[xd->mode_info_context->mbmi.second_ref_frame],
                         0, 0, 0);
    return best_rd;
