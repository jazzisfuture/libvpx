...
static void macro_block_yrd_16x16(MACROBLOCK *mb, int *Rate, int *Distortion,
  ...
  tx_type = get_tx_type_16x16(xd, b);
  if (tx_type != DCT_DCT) {
    vp9_fht_c(be->src_diff, be->coeff, 32, tx_type, 16);
  } else
...
static int64_t rd_pick_intra4x4block(VP9_COMP *cpi, MACROBLOCK *x, BLOCK *be,
  ...
  for (mode = B_DC_PRED; mode <= B_HU_PRED; mode++) {
    ...
    for (mode2 = (allow_comp ? 0 : (B_DC_PRED - 1));
      ...
      tx_type = get_tx_type_4x4(xd, b);
      if (tx_type != DCT_DCT) {
        vp9_fht_c(be->src_diff, be->coeff, 32, tx_type, 4);
  ...
  for (mode = DC_PRED; mode <= TM_PRED; mode++) {
    ...
    for (mode2 = DC_PRED - 1; mode2 != TM_PRED + 1; mode2++) {
      ...
      if (xd->mode_info_context->mbmi.txfm_size == TX_8X8) {
        ...
        if (tx_type != DCT_DCT)
          vp9_fht_c(be->src_diff, (x->block + idx)->coeff, 32, tx_type, 8);
        else
