...
static int vp8_rd_pick_best_mbsegmentation(VP8_COMP *cpi, MACROBLOCK *x,
                                           int_mv *best_ref_mv,
                                           int_mv *second_best_ref_mv,
                                           int64_t best_rd, int *mdcounts,
                                           int *returntotrate,
                                           int *returnyrate,
                                           int *returndistortion,
                                           int *skippable, int mvthresh,
                                           /* 16 = n_blocks */
                                           int_mv segmvs[BLOCK_MAX_SEGMENTS - 1]
                                                       [16][MAX_REF_FRAMES - 1],
                                           int64_t txfm_cache[NB_TXFM_MODES]) {
  int i;
  BEST_SEG_INFO bsi;
  ...
  if (cpi->compressor_speed == 0) {
    ...
    rd_check_segment(cpi, x, &bsi, BLOCK_16X8, segmvs[BLOCK_16X8], txfm_cache);
    rd_check_segment(cpi, x, &bsi, BLOCK_8X16, segmvs[BLOCK_8X16], txfm_cache);
    rd_check_segment(cpi, x, &bsi, BLOCK_8X8,  segmvs[BLOCK_8X8], txfm_cache);
    rd_check_segment(cpi, x, &bsi, BLOCK_4X4,  segmvs[BLOCK_4X4], txfm_cache);
  } else {
    int sr;
