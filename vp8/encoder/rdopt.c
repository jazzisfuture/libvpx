...
static void macro_block_yrd(VP8_COMP *cpi, MACROBLOCK *x, int *rate,
  ...
  if (0 && cpi->common.current_video_frame == 5)
    printf("4x4: r=%d/%d, d=%d, s=%d, rd=%lld/%lld, 8x8: r=%d/%d, d=%d, s=%d, rd=%lld/%lld\n",
           r4x4, r4x4s, d4x4, s4x4, rd4x4, rd4x4s, r8x8, r8x8s,
           d8x8, s8x8, rd8x8, rd8x8s);
...
void vp8_rd_pick_inter_mode(VP8_COMP *cpi, MACROBLOCK *x, int recon_yoffset, int recon_uvoffset,
  ...
  *returnintra = INT64_MAX;
    ...
    if ((cm->pred_filter_mode == 2) ||
      ...
      if (!mode_excluded && this_rd != INT64_MAX) {
        for (i = 0; i < NB_TXFM_MODES; i++) {
          ...
          if (0 && cpi->common.current_video_frame == 5) {
            printf("Testing mode %d, txfmsz=%d, skippable=%d, best_txfm_rd[%d]=%lld, this_rd=%lld+%lld-%lld\n",
                   mode_index, mbmi->txfm_size, skippable, i, best_txfm_rd[i],
                   this_rd, txfm_cache[i], txfm_cache[cm->txfm_mode]);
          }
  ...
  if (!x->skip) {
    ...
    //printf("Diffs: %"PRId64", %"PRId64", %"PRId64" (best: %"PRId64")\n",
    //       best_txfm_diff[0], best_txfm_diff[1], best_txfm_diff[2], best_rd);
...
void vp8_rd_pick_intra_mode(VP8_COMP *cpi, MACROBLOCK *x,
  ...
  if (cpi->common.mb_no_coeff_skip &&
    ...
    if (error4x4 < error16x16) {
      ...
      if (0) printf("%lld %lld %lld %lld\n",
             error16x16, txfm_cache[0], txfm_cache[1], txfm_cache[2]);
#endif
    }
    if (cpi->common.mb_no_coeff_skip)
