                                ...
                                const VP8_ENCODER_RTCD *rtcd, int *skip)
{
  ...
  for (n = 0; n < 4; n++)
  {
    ...
    // 2nd order DC at SB level
    by2->src_diff[x_idx * 2 + y_idx * 4 * 2 + 0 + 0] = x->coeff[ 0 * 16];
    by2->src_diff[x_idx * 2 + y_idx * 4 * 2 + 1 + 0] = x->coeff[ 4 * 16];
    by2->src_diff[x_idx * 2 + y_idx * 4 * 2 + 0 + 4] = x->coeff[ 8 * 16];
    by2->src_diff[x_idx * 2 + y_idx * 4 * 2 + 1 + 4] = x->coeff[12 * 16];
    ...
    r += vp8_rdcost_mby_8x8(x);
    skippable = skippable && mby_is_skippable_8x8(xd);
  }
#if 0
  // 2nd order DC (4x4 at SB level)
  x->short_walsh4x4(by2->src_diff, by2->coeff, 8);
  x->quantize_b(by2, bdy2);
  d += ENCODEMB_INVOKE(&rtcd->encodemb, berr)(by2->coeff, bdy2->dqcoeff);
  r += cost_coeffs(x, bdy2, PLANE_TYPE_Y2,
                   ta + vp8_block2above[24], tl + vp8_block2left[24]);
#endif
...
void vp8_rd_pick_inter_mode(VP8_COMP *cpi, MACROBLOCK *x, int recon_yoffset, int recon_uvoffset,
  ...
  *returnintra = INT64_MAX;
  ...
  if (!segfeature_active(xd, segment_id, SEG_LVL_REF_FRAME) &&
    ...
    store_coding_context(x, &x->mb_context[mb_index], best_mode_index, &best_partition,
  ...
  for (this_mode = NEARESTMV; this_mode <= NEWMV; this_mode++) {
    for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ref_frame++) {
      if (!(cpi->ref_frame_flags & flag_list[ref_frame]))
        continue;
      x->e_mbd.mode_info_context->mbmi.ref_frame = ref_frame;
      for (comp_pred = 0; comp_pred <= 1; comp_pred++) {
#endif
        int_mv mvp;
        int mode_excluded;
        int64_t this_rd = INT64_MAX;
        ...
        if (!disable_skip) {
          ...
          if (cpi->common.mb_no_coeff_skip) {
            ...
            if ((cpi->common.txfm_mode == ALLOW_8X8) && has_y2) {
              if (x->e_mbd.mode_info_context->mbmi.ref_frame != INTRA_FRAME)
                mb_skippable = sb_is_skippable_8x8(&x->e_mbd);
              else
                mb_skippable = uv_intra_skippable_8x8
                & mby_is_skippable_8x8(&x->e_mbd);
            } else {
              if (x->e_mbd.mode_info_context->mbmi.ref_frame != INTRA_FRAME)
                mb_skippable = sb_is_skippable(&x->e_mbd, has_y2);
              else
                mb_skippable = uv_intra_skippable
                & mby_is_skippable(&x->e_mbd, has_y2);
            }
...
void vp8cx_pick_mode_inter_macroblock
(
