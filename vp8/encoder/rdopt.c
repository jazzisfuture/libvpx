...
static void vp8_cal_sad(VP8_COMP *cpi, MACROBLOCKD *xd, MACROBLOCK *x,
                        int recon_yoffset, int near_sadidx[], int block_size) {

  int near_sad[8] = {0}; // 0-cf above, 1-cf left, 2-cf aboveleft, 3-lf current, 4-lf above, 5-lf left, 6-lf right, 7-lf below
  BLOCK *b = &x->block[0];
  unsigned char *src_y_ptr = *(b->base_src);
  int bs = block_size == BLOCK_16X16 ? 16 : 32;
  ...
  if (xd->mb_to_top_edge == 0 && xd->mb_to_left_edge == 0) {
    ...
    near_sad[2] = cpi->fn_ptr[block_size].sdf(src_y_ptr, b->src_stride,
                                              xd->dst.y_buffer - xd->dst.y_stride * bs - bs,
                                              xd->dst.y_stride, 0x7fffffff);
  }
  ...
  if (cpi->common.last_frame_type != KEY_FRAME) {
    ...
    if (near_sad[4] != INT_MAX)
      near_sad[4] = cpi->fn_ptr[block_size].sdf(src_y_ptr, b->src_stride,
                                                pre_y_buffer - pre_y_stride * bs,
                                                pre_y_stride, 0x7fffffff);
...
static int64_t handle_inter_mode(VP8_COMP *cpi, MACROBLOCK *x, int block_size,
                                 int *saddone, int near_sadidx[], int mdcounts[4],
                                 int64_t txfm_cache[],
    ...
    case NEWMV:
      if (is_comp_pred) {
        ...
        *rate2 += vp8_mv_bit_cost(&frame_mv[NEWMV][refs[1]],
        ...
        if (!*saddone) {
          vp8_cal_sad(cpi, xd, x, recon_yoffset, &near_sadidx[0], block_size);
          *saddone = 1;
        }
  ...
  *rate2 += vp8_cost_bit(cpi->common.prob_pred_filter_off,
  ...
  if (cpi->common.mcomp_filter_type == SWITCHABLE)
    *rate2 += SWITCHABLE_INTERP_RATE_FACTOR *
              x->switchable_interp_costs[get_pred_context(cm, xd, PRED_SWITCHABLE_INTERP)]
  ...
  *compmode_cost = vp8_cost_bit(get_pred_prob(cm, xd, PRED_COMP), is_comp_pred);
