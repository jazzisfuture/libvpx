...
static void encode_mb_row(VP8_COMP *cpi, VP8_COMMON *cm, int mb_row,
  ...
  if ((vpx_nobarrier_load_32(&cpi->b_multi_threaded) != 0) && (mb_row != 0)) {
...
#if CONFIG_MULTITHREAD
    if (vpx_nobarrier_load_32(&cpi->b_multi_threaded) != 0) {
      vpx_release_store_32(current_mb_col, mb_col - 1); /* set previous MB done */
...
#if CONFIG_MULTITHREAD
  if (vpx_nobarrier_load_32(&cpi->b_multi_threaded) != 0) vpx_release_store_32(current_mb_col, rightmost_col);
#endif
