...
static void encode_mb_row(VP8_COMP *cpi, VP8_COMMON *cm, int mb_row,
  ...
  vpx_atomic_init(&rightmost_col, cm->mb_cols + nsync);
  if (vpx_atomic_read(&cpi->b_multi_threaded) != 0 && mb_row != 0) {
...
#if CONFIG_MULTITHREAD
  if (vpx_atomic_read(&cpi->b_multi_threaded) != 0) {
    vpx_atomic_write(current_mb_col, vpx_atomic_read(&rightmost_col));
  }
#endif
...

  {
    ...
    if (vpx_atomic_read(&cpi->b_multi_threaded)) {
      ...
      for (i = 0; i < cm->mb_rows; ++i)
        vpx_atomic_write(&cpi->mt_current_mb_col[i], -1);
