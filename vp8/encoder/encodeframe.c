...
int vp8cx_encode_intra_macro_block(VP8_COMP *cpi, MACROBLOCK *x, TOKENEXTRA **t)
{
    ...
    else
    {
        ...
        if(cpi->oxcf.copy_flag==1)
        {
            ...
            {
                vpx_memcpy(&cpi->oxcf.reuse_info[curr_index].mbmi, &x->e_mbd.mode_info_context->mbmi, sizeof(MB_MODE_INFO));
                for (i = 0; i < 16; i++)
                    cpi->oxcf.reuse_info[curr_index].bmi[i].as_mode = x->e_mbd.block[i].bmi.as_mode;
            }
        }
        else
        {
            vpx_memcpy(&x->e_mbd.mode_info_context->mbmi, &cpi->oxcf.reuse_info[curr_index].mbmi, sizeof(MB_MODE_INFO));
            for ( i = 0; i < 16; i++)
                x->e_mbd.block[i].bmi.as_mode = cpi->oxcf.reuse_info[curr_index].bmi[i].as_mode;
        }
#else
        vp8_pick_intra_mode(cpi, x, &rate);
#endif
    ...
    else
    {
        ...
        if(cpi->oxcf.copy_flag==1)
        {
            ...
            if(cpi->oxcf.reuse_info!=NULL)
            {
                ...
                for (i = 0; i < 16; i++)
                    cpi->oxcf.reuse_info[curr_index].bmi[i].as_mode = x->e_mbd.block[i].bmi.as_mode;
                cpi->oxcf.reuse_info[curr_index].rate = rate;
                cpi->oxcf.reuse_info[curr_index].distortion = distortion;
                cpi->oxcf.reuse_info[curr_index].intra_error = intra_error;
                cpi->oxcf.reuse_info[curr_index].mbmi.ref_frame = xd->mode_info_context->mbmi.ref_frame;
            }
        }
        else
        {
            vpx_memcpy(&x->e_mbd.mode_info_context->mbmi,&cpi->oxcf.reuse_info[curr_index].mbmi, sizeof(MB_MODE_INFO));
            ref_frame = cpi->oxcf.reuse_info[curr_index].mbmi.ref_frame;
            for ( i = 0; i < 16; i++)
                x->e_mbd.block[i].bmi.as_mode = cpi->oxcf.reuse_info[curr_index].bmi[i].as_mode;
            reuse_update_mvcount(cpi, &x->e_mbd, &cpi->oxcf.reuse_info[curr_index].frame_best_ref_mv[ref_frame]);
            rate = cpi->oxcf.reuse_info[curr_index].rate;
            distortion = cpi->oxcf.reuse_info[curr_index].distortion;
            intra_error = cpi->oxcf.reuse_info[curr_index].intra_error;
        }
#else
        vp8_pick_inter_mode(cpi, x, recon_yoffset, recon_uvoffset, &rate,
                                    &distortion, &intra_error);
