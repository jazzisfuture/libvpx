...
static void pick_mb_modes(VP8_COMP *cpi,
  ...
  int ref_fb_idx = cm->active_ref_idx[LST_FB_IDX];
                           ...
                           int *totaldist)
{
  int map_index;
  int recon_yoffset, recon_uvoffset;
  int ref_fb_idx = cm->active_ref_idx[LST_FB_IDX]; // LST not special here.
  int dst_fb_idx = cm->new_fb_idx;
  int recon_y_stride = cm->yv12_fb[ref_fb_idx].y_stride;
  int recon_uv_stride = cm->yv12_fb[ref_fb_idx].uv_stride;
  ENTROPY_CONTEXT_PLANES left_context[2];
...
void init_encode_frame_mb_context(VP8_COMP *cpi) {
  ...
  x->src = * cpi->Source;
  xd->pre = cm->yv12_fb[cm->active_ref_idx[LST_FB_IDX]];
...
void vp8cx_encode_inter_macroblock (VP8_COMP *cpi, MACROBLOCK *x,
  ...
  if (mbmi->ref_frame == INTRA_FRAME) {
    ...
    if (mbmi->ref_frame == LAST_FRAME)
      ref_fb_idx = cm->active_ref_idx[LST_FB_IDX];
    else if (mbmi->ref_frame == GOLDEN_FRAME)
      ref_fb_idx = cm->active_ref_idx[GLD_FB_IDX];
    else
      ref_fb_idx = cm->active_ref_idx[ALT_FB_IDX];
    ...
    if (mbmi->second_ref_frame) {
      ...
      if (mbmi->second_ref_frame == LAST_FRAME)
        second_ref_fb_idx = cm->active_ref_idx[LST_FB_IDX];
      else if (mbmi->second_ref_frame == GOLDEN_FRAME)
        second_ref_fb_idx = cm->active_ref_idx[GLD_FB_IDX];
      else
        second_ref_fb_idx = cm->active_ref_idx[ALT_FB_IDX];
...
void vp8cx_encode_inter_superblock(VP8_COMP *cpi, MACROBLOCK *x, TOKENEXTRA **t,
  ...
  if (xd->mode_info_context->mbmi.ref_frame == INTRA_FRAME) {
    ...
    ref_fb_idx = cm->active_ref_idx[xd->mode_info_context->mbmi.ref_frame - 1];
    ...
    if (xd->mode_info_context->mbmi.second_ref_frame) {
      ...
      second_ref_fb_idx = cm->active_ref_idx[xd->mode_info_context->mbmi.second_ref_frame - 1];
