...
static void update_state(VP8_COMP *cpi, MACROBLOCK *x, PICK_MODE_CONTEXT *ctx, int r, int c) {
  int i;
  MACROBLOCKD *xd = &x->e_mbd;
...
#endif
      update_state(cpi, x, &x->mb_context[i], mb_row, mb_col);
...
void vp8_encode_frame(VP8_COMP *cpi) {
  if (cpi->sf.RD) {
    ...
    printf("Frame %d (type=%d), TXFMTYPE %d\n",
           cpi->common.current_video_frame, frame_type, txfm_type);
    cpi->common.txfm_mode = (1 || (frame_type == 2)) ? ALLOW_8X8 : txfm_type;
#if CONFIG_TX_SELECT
    ...
    if (0 && cpi->common.frame_type == KEY_FRAME)
      printf("Adj mode (%lld -> %d, %lld -> %d, %lld -> %d)\n",
             cpi->rd_tx_select_diff[ONLY_4X4],
             cpi->rd_tx_select_threshes[frame_type][ONLY_4X4],
             cpi->rd_tx_select_diff[ALLOW_8X8],
             cpi->rd_tx_select_threshes[frame_type][ALLOW_8X8],
             cpi->rd_tx_select_diff[TX_MODE_SELECT],
             cpi->rd_tx_select_threshes[frame_type][TX_MODE_SELECT]);
...
#if CONFIG_TX_SELECT
    if (!((cpi->common.mb_no_coeff_skip && mbmi->mb_skip_coeff) ||
