...
static void encode_nmv_component_fp(vp8_writer *w,
  ...
  if (c == MV_CLASS_0) {
    vp8_write_token(w, vp8_mv_fp_tree, mvcomp->class0_fp,
...
static void build_nmv_component_cost_table(int *mvcost,
  ...
  int class0_fp_cost[4], fp_cost[4];
  ...
  vp8_cost_tokens(class0_fp_cost, mvcomp->class0_fp, vp8_mv_fp_tree);
...
void print_nmvstats() {
  ...
  unsigned int branch_ct_class0_fp[2][4 - 1][2];
  ...
  for (i=0; i< 2; ++i) {
    ...
    printf("    { ");
    for (j = 0; j < 4; ++j)
      printf("%d, ", tnmvcounts.comps[i].class0_fp[j]);
    printf("},\n");
    printf("    { ");
    for (j = 0; j < 4; ++j)
  ...
  for (i=0; i< 2; ++i) {
    ...
    printf("    { ");
    for (j = 0; j < 3; ++j)
      printf("%d, ", prob.comps[i].class0_fp[j]);
    printf("},\n");
    printf("    { ");
    for (j = 0; j < 3; ++j)
...
static void add_nmvcount(nmv_context_counts *dst, nmv_context_counts *src) {
  ...
  for (i = 0; i < 2; ++i) {
    for (j = 0; j < 4; ++j) {
      dst->comps[i].class0_fp[j] += src->comps[i].class0_fp[j];
    }
    for (j = 0; j < 4; ++j) {
...
void vp8_write_nmvprobs(VP8_COMP * cpi, int usehp) {
  ...
  for (i = 0; i < 2; ++i) {
    for (j = 0; j < 3; ++j) {
      update_nmv(w, branch_ct_class0_fp[i][j],
                 &cpi->common.fc.nmvc.comps[i].class0_fp[j],
                 prob.comps[i].class0_fp[j],
                 VP8_NMV_UPDATE_PROB);
    }
    for (j = 0; j < 3; ++j) {
