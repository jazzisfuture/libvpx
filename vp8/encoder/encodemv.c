...
static void encode_nmv_component_fp(vp8_writer *w,
  ...
  if (usehp) {
    if (c == MV_CLASS_0) {
      vp8_write(w, e, mvcomp->class0_hp[d][f]);
    } else {
...
static void build_nmv_component_cost_table(int *mvcost,
  ...
  int class0_hp_cost[CLASS0_SIZE][4][2], hp_cost[2];
  ...
  for (i = 0; i < CLASS0_SIZE; ++i) {
    vp8_cost_tokens(class0_fp_cost[i], mvcomp->class0_fp[i],
                    vp8_mv_fp_tree);
  }
  ...
  if (usehp) {
    for (i = 0; i < CLASS0_SIZE; ++i) {
      int j;
      for (j = 0; j < 4; j++) {
        class0_hp_cost[i][j][0] = vp8_cost_zero(mvcomp->class0_hp[i][j]);
        class0_hp_cost[i][j][1] = vp8_cost_one(mvcomp->class0_hp[i][j]);
      }
    }
...
void print_nmvstats() {
  ...
  unsigned int branch_ct_class0_hp[2][CLASS0_SIZE][4][2];
  ...
  for (i=0; i< 2; ++i) {
    ...
    for (j = 0; j < CLASS0_SIZE; ++j) {
      ...
      for (k = 0; k < 4; ++k) {
        printf("%d/%d, ",
               tnmvcounts.comps[i].class0_hp[j][k][0],
               tnmvcounts.comps[i].class0_hp[j][k][1]);
      }
      printf("}, ");
    }
    printf("},\n");
  ...
  for (i=0; i< 2; ++i) {
    ...
    for (j = 0; j < CLASS0_SIZE; ++j) {
      printf("{ ");
      for (k = 0; k < 4; ++k) {
        printf("%d, ", prob.comps[i].class0_hp[j][k]);
      }
      printf("}, ");
    }
    printf("},\n");
...
static void add_nmvcount(nmv_context_counts *dst, nmv_context_counts *src) {
  ...
  for (i = 0; i < 2; ++i) {
    ...
    for (j = 0; j < CLASS0_SIZE; ++j) {
      for (k = 0; k < 4; ++k) {
        dst->comps[i].class0_hp[j][k][0] += src->comps[i].class0_hp[j][k][0];
        dst->comps[i].class0_hp[j][k][1] += src->comps[i].class0_hp[j][k][1];
      }
    }
...
void vp8_write_nmvprobs(VP8_COMP * cpi, int usehp) {
  ...
  if (usehp) {
    for (i = 0; i < 2; ++i) {
      for (j = 0; j < CLASS0_SIZE; ++j) {
        for (k = 0; k < 4; ++k)
          update_nmv(w, branch_ct_class0_hp[i][j][k],
                     &cpi->common.fc.nmvc.comps[i].class0_hp[j][k],
                     prob.comps[i].class0_hp[j][k],
                     VP8_NMV_UPDATE_PROB);
...
void vp8_encode_nmv(vp8_writer *w, const MV *mv,
  ...
  if (j == MV_JOINT_HZVNZ || j == MV_JOINT_HNZVNZ) {
    encode_nmv_component(w, mv->row, &mvctx->comps[0]);
  }
  if (j == MV_JOINT_HNZVZ || j == MV_JOINT_HNZVNZ) {
    encode_nmv_component(w, mv->col, &mvctx->comps[1]);
  }
}

void vp8_encode_nmv_fp(vp8_writer *w, const MV *mv,
                       const nmv_context *mvctx, int usehp) {
  MV_JOINT_TYPE j = vp8_get_mv_joint(*mv);
  if (j == MV_JOINT_HZVNZ || j == MV_JOINT_HNZVNZ) {
    encode_nmv_component_fp(w, mv->row, &mvctx->comps[0], usehp);
  }
  if (j == MV_JOINT_HNZVZ || j == MV_JOINT_HNZVNZ) {
    encode_nmv_component_fp(w, mv->col, &mvctx->comps[1], usehp);
  }
}
