...
static void build_nmv_component_cost_table(int *mvcost,
  ...
  if (usehp) {
    for (i = 0; i < CLASS0_SIZE; ++i) {
      ...
      class0_hp_cost[i][1] = vp8_cost_one(mvcomp->class0_hp[i]);
    }
...
void print_nmvstats() {
  ...
  vp8_counts_to_nmv_context(&tnmvcounts, &prob, 1,
                            ...
                            branch_ct_class0_hp, branch_ct_bits_hp);
  ...
  for (i=0; i< 2; ++i) {
    printf("  {\n");
    printf("    %d/%d,\n",
           tnmvcounts.comps[i].sign[0], tnmvcounts.comps[i].sign[1]);
    printf("    { ");
    ...
    for (j = 0; j < MV_OFFSET_BITS; ++j)
      printf("%d/%d, ",
             tnmvcounts.comps[i].bits[j][0], tnmvcounts.comps[i].bits[j][1]);
    printf("},\n");
    printf("    { ");
    for (j = 0; j < CLASS0_SIZE; ++j)
      printf("%d/%d, ",
             tnmvcounts.comps[i].class0_hp[j][0],
             tnmvcounts.comps[i].class0_hp[j][1]);
    printf("},\n");
    printf("    %d/%d,\n",
           tnmvcounts.comps[i].bits_hp[0],
           tnmvcounts.comps[i].bits_hp[1]);
    printf("  },\n");
  }
  ...
  for (i=0; i< 2; ++i) {
    ...
    printf("    { ");
    for (j = 0; j < CLASS0_SIZE; ++j)
      printf("%d, ", prob.comps[i].class0_hp[j]);
    printf("},\n");
...
static void add_nmvcount(nmv_context_counts *dst, nmv_context_counts *src) {
  ...
  for (i = 0; i < 2; ++i) {
    ...
    for (j = 0; j < CLASS0_SIZE; ++j) {
      dst->comps[i].class0_hp[j][0] += src->comps[i].class0_hp[j][0];
      dst->comps[i].class0_hp[j][1] += src->comps[i].class0_hp[j][1];
    }
    dst->comps[i].bits_hp[0] += src->comps[i].bits_hp[0];
    dst->comps[i].bits_hp[1] += src->comps[i].bits_hp[1];
  }
}
#endif
