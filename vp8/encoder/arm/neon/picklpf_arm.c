                                      ...
                                      YV12_BUFFER_CONFIG *dst_ybc)
{
    ...
    linestocopy = (src_ybc->y_height >> 4) / PARTIAL_FRAME_FRACTION;
    linestocopy = linestocopy ? linestocopy << 4 : 16;     /* 16 lines per MB */

    /* copy extra 4 so that full filter context is available if filtering done
     * on the copied partial frame and not original.
     * partial filter dose mb filtering for top row also, which can modify
     * 3 pixels above
     */
    linestocopy += 4;
