...
void vp8_fast_quantize_b_neon(
    ...
    coeff_p = b->coeff;
    q0s16 = vld1q_s16(coeff_p); coeff_p += 8;
    q1s16 = vld1q_s16(coeff_p);
    ...
    if ((z1 | z2) == 0) {  // zero_output
        *d->eob = 0;
        qcoeff_p = d->qcoeff;
        vst1q_s16(qcoeff_p, q0s16); qcoeff_p += 8;
        vst1q_s16(qcoeff_p, q1s16);
        dqcoeff_p = d->dqcoeff;
        vst1q_s16(dqcoeff_p, q0s16); dqcoeff_p += 8;
        vst1q_s16(dqcoeff_p, q1s16);
        return;
    }
    ...
    round_p = b->round;
    q14s16 = vld1q_s16(round_p); round_p += 8;
    q15s16 = vld1q_s16(round_p);
    quant_p = b->quant_fast;
    q8s16 = vld1q_s16(quant_p); quant_p += 8;
    q9s16 = vld1q_s16(quant_p);
    ...
    dequant_p = d->dequant;
    q2s16 = vld1q_s16(dequant_p); dequant_p += 8;
    q3s16 = vld1q_s16(dequant_p);

    q14u16 = vtstq_s16(q12s16, vreinterpretq_s16_u16(q8u16));
    q15u16 = vtstq_s16(q13s16, vreinterpretq_s16_u16(q8u16));

    qcoeff_p = d->qcoeff;
    vst1q_s16(qcoeff_p, q12s16); qcoeff_p += 8;
    vst1q_s16(qcoeff_p, q13s16);
    ...
    dqcoeff_p = d->dqcoeff;
    vst1q_s16(dqcoeff_p, q2s16); dqcoeff_p += 8;
    vst1q_s16(dqcoeff_p, q3s16);
...
void vp8_fast_quantize_b_pair_neon(
    ...
    q1s16 = vld1q_s16(coeff_p);
    round_p = b1->round;
    q6s16 = vld1q_s16(round_p); round_p += 8;
    q7s16 = vld1q_s16(round_p);
    ...
    q6s16 = vld1q_s16(dequant_p); dequant_p += 8;
    q7s16 = vld1q_s16(dequant_p);
    ...
    qcoeff_p = d1->qcoeff;
    vst1q_s16(qcoeff_p, q4s16); qcoeff_p += 8;
    vst1q_s16(qcoeff_p, q5s16);
    ...
    qcoeff_p = d2->qcoeff;
    vst1q_s16(qcoeff_p, q10s16); qcoeff_p += 8;
    vst1q_s16(qcoeff_p, q11s16);
    ...
    vst1q_s16(dqcoeff_p, q12s16); dqcoeff_p += 8;
    vst1q_s16(dqcoeff_p, q13s16);
