                             ...
                             int increase_denoising)
{
    ...
    for (r = 0; r < 8; ++r) {
        /* Calculate differences */
        const __m128i v_sig = _mm_loadl_epi64((__m128i *)(&sig[0]));
        const __m128i v_mc_running_avg = _mm_loadl_epi64(
                                         (__m128i *)(&mc_running_avg[0]));
        ...
        _mm_storel_epi64((__m128i *)running_avg, v_running_avg);
...

    {
        ...
        if (abs(sum_diff) > sum_diff_thresh) {
          ...
          if (delta < 4) {
            ...
            for (r = 0; r < 8; ++r) {
              __m128i v_running_avg =
                  _mm_loadl_epi64((__m128i *)(&running_avg[0]));
              // Calculate differences.
              const __m128i v_sig = _mm_loadl_epi64((__m128i *)(&sig[0]));
              const __m128i v_mc_running_avg =
                  _mm_loadl_epi64((__m128i *)(&mc_running_avg[0]));
              const __m128i pdiff = _mm_subs_epu8(v_mc_running_avg, v_sig);
              const __m128i ndiff = _mm_subs_epu8(v_sig, v_mc_running_avg);
              // Obtain the sign. FF if diff is negative.
              const __m128i diff_sign = _mm_cmpeq_epi8(pdiff, k_0);
