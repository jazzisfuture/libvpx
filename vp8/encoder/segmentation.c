      ...
      for (i = 0; i < 4; i++) {
        ...
        if (xd->mode_info_context->mbmi.encoded_as_sb) {
          if (mb_col + 1 < cm->mb_cols)
            segment_id = segment_id && xd->mode_info_context[1].mbmi.segment_id;
          if (mb_row + 1 < cm->mb_rows) {
            segment_id = segment_id && xd->mode_info_context[cm->mode_info_stride].mbmi.segment_id;
            if (mb_col + 1 < cm->mb_cols)
              segment_id = segment_id && xd->mode_info_context[cm->mode_info_stride + 1].mbmi.segment_id;
          }
        }
#endif
        ...
        if (cm->frame_type != KEY_FRAME) {
          ...
          int seg_predicted =
            (segment_id == get_pred_mb_segid(cm, xd, segmap_index));
