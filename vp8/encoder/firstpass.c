...
static void define_gf_group(VP8_COMP *cpi, FIRSTPASS_STATS *this_frame)
{
...

    {
        ...
        if (!cpi->source_alt_ref_pending && cpi->common.frame_type != KEY_FRAME)
            cpi->twopass.gf_group_error_left = (int)(gf_group_err - gf_first_frame_err);
        else
...
static void find_next_key_frame(VP8_COMP *cpi, FIRSTPASS_STATS *this_frame)
{
    ...
    if (cpi->oxcf.end_usage == USAGE_STREAM_FROM_SERVER)
    {
        ...
        if (cpi->drop_frames_allowed)
        {
            int df_buffer_level = (int)(cpi->oxcf.drop_frames_water_mark * (cpi->oxcf.optimal_buffer_level / 100));
    ...
    if (1)
    {
        ...
        cpi->target_bandwidth = (int)(cpi->twopass.kf_bits * cpi->output_frame_rate);
    }
    ...
    if (cpi->oxcf.allow_spatial_resampling)
    {
        ...
        kf_q = estimate_kf_group_q(cpi, err_per_frame, (int)bits_per_frame, group_iiratio);
        ...
        projected_buffer_level = (int)( cpi->buffer_level - (int)((projected_bits_perframe - av_bits_per_frame) * cpi->twopass.frames_to_key));
        ...
        if (resample_trigger)
        {
            while ((kf_q >= cpi->worst_quality) && (scale_val < 6))
            {
                 ...
                 * image size
                 */
                kf_q = estimate_kf_group_q(cpi, err_per_frame * effective_size_ratio, (int)bits_per_frame, group_iiratio);
