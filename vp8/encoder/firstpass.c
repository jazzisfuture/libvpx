...
void vp8_init_second_pass(VP8_COMP *cpi)
{
    ...
    *cpi->twopass.total_left_stats = *cpi->twopass.total_stats;
    ...
    // Scan first pass file to get consolidated mv stats
    {
        double mvr_ratio = 0.0;
        double mvc_ratio = 0.0;
        double tot_mvr_ratio = 0.0;
        double tot_mvc_ratio = 0.0;
        int i;
        ...
        while (input_stats(cpi, &this_frame) != EOF)
        {
            ...
            if ( this_frame.pcnt_motion > 0.05 )
            {
                mvr_ratio = fabs(this_frame.mvr_abs) /
                            DOUBLE_DIVIDE_CHECK(fabs(this_frame.MVr));

                tot_mvr_ratio +=
                    (mvr_ratio < 1.0) ? 1.0
                                      : (mvr_ratio > 20.0) ? 20 : mvr_ratio;

                mvc_ratio = fabs(this_frame.mvc_abs) /
                            DOUBLE_DIVIDE_CHECK(fabs(this_frame.MVc));

                tot_mvc_ratio +=
                    (mvc_ratio < 1.0) ? 1.0
                                      : (mvc_ratio > 20.0) ? 20 : mvc_ratio;
            }
            else
            {
                tot_mvr_ratio += 1.0;
        ...
        tot_mvr_ratio /= (double)i;
        tot_mvc_ratio /= (double)i;
