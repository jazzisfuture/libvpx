...
static void pack_mb_tokens(vp8_writer *bc,
                           TOKENEXTRA **tp,
...
#if CONFIG_SUPERBLOCKS
static void write_sb_mv_ref(vp8_writer *bc, MB_PREDICTION_MODE m, const vp8_prob *p) {
#if CONFIG_DEBUG
  assert(NEARESTMV <= m  &&  m < SPLITMV);
#endif
...
#else
                      if (xd->allow_high_precision_mv) {
                        write_mv_hp(bc, &cpi->mb.partition_info->bmi[j].second_mv.as_mv,
                                    &best_second_mv, (const MV_CONTEXT_HP *) mvc_hp);
                      } else {
                        write_mv(bc, &cpi->mb.partition_info->bmi[j].second_mv.as_mv,
