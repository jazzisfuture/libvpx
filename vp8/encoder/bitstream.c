...
void vp8_pack_bitstream(VP8_COMP *cpi, unsigned char *dest, unsigned long *size) {
  ...
  if (pc->frame_type == KEY_FRAME) {
    ...
    {
      int comp_pred_mode = cpi->common.comp_pred_mode;
      int use_compound_pred = comp_pred_mode != SINGLE_PREDICTION_ONLY;
      int use_hybrid_pred = comp_pred_mode == HYBRID_PREDICTION;
