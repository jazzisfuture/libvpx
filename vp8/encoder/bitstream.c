    ...
    for (col = 0; col < pc->mb_cols; col += 2) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (pc->txfm_mode == TX_MODE_SELECT &&
          ...
          vp8_encode_bool(w, sz != TX_4X4, pc->prob_tx[0]);
#if CONFIG_TX16X16
          vp8_encode_bool(w, sz != TX_8X8, pc->prob_tx[1]);
#endif
        }
#endif
...
#if CONFIG_TX_SELECT
        if (c->txfm_mode == TX_MODE_SELECT &&
            !((c->mb_no_coeff_skip && m->mbmi.mb_skip_coeff) ||
              (segfeature_active(xd, segment_id, SEG_LVL_EOB) &&
               get_segdata(xd, segment_id, SEG_LVL_EOB) == 0))) {
          TX_SIZE sz = m->mbmi.txfm_size;
          vp8_encode_bool(bc, sz != TX_4X4, c->prob_tx[0]);
#if CONFIG_TX16X16
          vp8_encode_bool(bc, sz != TX_8X8, c->prob_tx[1]);
#endif
        }
#endif
