    ...
    *returnintra = INT_MAX;
    ...
    if (cpi->oxcf.noise_sensitivity)
    {
        if (x->e_mbd.best_sse_inter_mode == DC_PRED)
        {
            ...
            x->e_mbd.best_reference_frame = best_mbmode.ref_frame;
        ...
        {
            int this_rd = 0;
            int this_ref_frame = x->e_mbd.best_zeromv_reference_frame;
        ...
        //restore the best info we may have over written it in the denoiser
        if (x->skip)
