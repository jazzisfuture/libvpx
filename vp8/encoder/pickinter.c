    ...
    for (mode_index = 0; mode_index < MAX_MODES; mode_index++)
    {
        ...
        case NEWMV:
        {
            ...
            if (cpi->sf.search_method == HEX)
            {
                ...
                /* TODO: In higher-res pick_inter_mode, step_param is used to modify hex search range.
                 * Here, set step_param to 0 not to change the behavior in lowest-resolution encoder.
...
void vp8_mr_pick_inter_mode(VP8_COMP *cpi, MACROBLOCK *x, int recon_yoffset,
                         int recon_uvoffset, int *returnrate,
                         int *returndistortion, int *returnintra, int mb_row, int mb_col)
{
    BLOCK *b = &x->block[0];
    BLOCKD *d = &x->e_mbd.block[0];
    MACROBLOCKD *xd = &x->e_mbd;
    ...
    if(cpi->oxcf.mr_down_sampling_factor.den == 1)
    {
        if(cpi->oxcf.mr_down_sampling_factor.num == 2)
            parent_mb_index = ((mb_row+1)>>1)*cpi->mr_low_res_mb_cols + ((mb_col+1)>>1);
        else if(cpi->oxcf.mr_down_sampling_factor.num == 4)
            parent_mb_index = ((mb_row+2)>>2)*cpi->mr_low_res_mb_cols + ((mb_col+2)>>2);
        else if(cpi->oxcf.mr_down_sampling_factor.num == 8)
            parent_mb_index = ((mb_row+4)>>3)*cpi->mr_low_res_mb_cols + ((mb_col+4)>>3);
        else
        {
            int round = cpi->oxcf.mr_down_sampling_factor.num/2;
            parent_mb_index = ((mb_row+round)/cpi->oxcf.mr_down_sampling_factor.num)
                   *cpi->mr_low_res_mb_cols
                   + ((mb_col+round)/cpi->oxcf.mr_down_sampling_factor.num);
        }
    }
    else
    {
        // TODO: Should try using a look-up table to see if it helps performance.
        int round = cpi->oxcf.mr_down_sampling_factor.num/2;
        int parent_mb_row, parent_mb_col;

        parent_mb_row = (mb_row*cpi->oxcf.mr_down_sampling_factor.den+round)/cpi->oxcf.mr_down_sampling_factor.num;
        parent_mb_col = (mb_col*cpi->oxcf.mr_down_sampling_factor.den+round)/cpi->oxcf.mr_down_sampling_factor.num;
    ...
    // For 3rd encoder, adjust dissim value. Low its quality for good performance.
    if (cpi->oxcf.mr_encoder_id ==2) dissim>>=1;
    ...
    if(parent_ref_frame != INTRA_FRAME)
    {
      ...
      if(cpi->oxcf.mr_down_sampling_factor.den == 1)
      {
          if(cpi->oxcf.mr_down_sampling_factor.num == 2)
          {
              parent_ref_mv.as_mv.row = store_mode_info[parent_mb_index].mv.as_mv.row <<1;
              parent_ref_mv.as_mv.col = store_mode_info[parent_mb_index].mv.as_mv.col <<1;
          }else if(cpi->oxcf.mr_down_sampling_factor.num == 4)
          {
              parent_ref_mv.as_mv.row = store_mode_info[parent_mb_index].mv.as_mv.row <<2;
              parent_ref_mv.as_mv.col = store_mode_info[parent_mb_index].mv.as_mv.col <<2;
          }else if(cpi->oxcf.mr_down_sampling_factor.num == 8)
          {
              parent_ref_mv.as_mv.row = store_mode_info[parent_mb_index].mv.as_mv.row <<3;
              parent_ref_mv.as_mv.col = store_mode_info[parent_mb_index].mv.as_mv.col <<3;
          }else
          {
              parent_ref_mv.as_mv.row = store_mode_info[parent_mb_index].mv.as_mv.row*cpi->oxcf.mr_down_sampling_factor.num;
              parent_ref_mv.as_mv.col = store_mode_info[parent_mb_index].mv.as_mv.col*cpi->oxcf.mr_down_sampling_factor.num;
          }
      }else
      {
          parent_ref_mv.as_mv.row = store_mode_info[parent_mb_index].mv.as_mv.row*cpi->oxcf.mr_down_sampling_factor.num/cpi->oxcf.mr_down_sampling_factor.den;
          parent_ref_mv.as_mv.col = store_mode_info[parent_mb_index].mv.as_mv.col*cpi->oxcf.mr_down_sampling_factor.num/cpi->oxcf.mr_down_sampling_factor.den;
      }
    ...
    *returnintra = INT_MAX;
    ...
    for (mode_index = 0; mode_index < MAX_MODES; mode_index++)
    {
        ...
        //If parent MB is inter, and it is unlikely there are multiple objects in parent MB, we use
        //parent ref frame as child MB's ref frame.
        if(parent_ref_frame && dissim <8 && parent_ref_frame != vp8_ref_frame_order[mode_index])
            continue;
        ...
        if(x->e_mbd.mode_info_context->mbmi.ref_frame)
        {
            ...
            {
                // set up all the refframe dependent pointers.
                //if (x->e_mbd.mode_info_context->mbmi.ref_frame == LAST_FRAME && (cpi->ref_frame_flags & VP8_LAST_FLAG))
                if (cpi->ref_frame_flags & VP8_LAST_FLAG)
                {
                    YV12_BUFFER_CONFIG *lst_yv12 = &cpi->common.yv12_fb[cpi->common.lst_fb_idx];
            ...
            {
                //if (x->e_mbd.mode_info_context->mbmi.ref_frame == GOLDEN_FRAME && (cpi->ref_frame_flags & VP8_GOLD_FLAG))
                if (cpi->ref_frame_flags & VP8_GOLD_FLAG)
                {
                    YV12_BUFFER_CONFIG *gld_yv12 = &cpi->common.yv12_fb[cpi->common.gld_fb_idx];
            ...
            {
                //if (x->e_mbd.mode_info_context->mbmi.ref_frame == ALTREF_FRAME && (cpi->ref_frame_flags & VP8_ALT_FLAG && cpi->source_alt_ref_active))
            ...
            if(vp8_mode_order[mode_index] == NEWMV && parent_mode == ZEROMV && best_ref_mv.as_int==0) //&& dissim==0
                continue;
            else if(vp8_mode_order[mode_index] == NEWMV && dissim==0 && best_ref_mv.as_int==parent_ref_mv.as_int)
                continue;
        }
        ...
        case NEWMV:
        {
            ...
            int speed_adjust = (cpi->Speed > 5) ? ((cpi->Speed >= 8)? 3 : 2) : 1;
            int diff_mv = MAX(abs(best_ref_mv.as_mv.row - parent_ref_mv.as_mv.row), abs(best_ref_mv.as_mv.col - parent_ref_mv.as_mv.col));
