...
static THREAD_FUNCTION thread_loopfilter(void *p_data) {
  ...
  while (1) {
    if (vpx_atomic_read(&cpi->b_multi_threaded) == 0) break;
    ...
    if (sem_wait(&cpi->h_event_start_encoding[ithread]) == 0) {
      ...
      for (mb_row = ithread + 1; mb_row < cm->mb_rows;
        ...
        for (mb_col = 0; mb_col < cm->mb_cols; ++mb_col) {
          if (((mb_col - 1) % nsync) == 0) {
            vpx_atomic_write(current_mb_col, mb_col - 1);
          }

          if (mb_row && !(mb_col & (nsync - 1))) {
            vp8_atomic_sync_read(mb_col, last_row_current_mb_col, nsync);
          }
        ...
        vpx_atomic_write(current_mb_col, mb_col + nsync);
...
int vp8cx_create_encoder_threads(VP8_COMP *cpi) {
  ...
  if (cm->processor_core_count > 1 && cpi->oxcf.multi_threaded > 1) {
    ...
    vpx_atomic_write(&cpi->b_multi_threaded, 1);
    ...
    */
    ...
    if (rc) {
      ...
      vpx_atomic_write(&cpi->b_multi_threaded, 0);
...
void vp8cx_remove_encoder_threads(VP8_COMP *cpi) {
  if (vpx_atomic_read(&cpi->b_multi_threaded)) {
    /* shutdown other threads */
    vpx_atomic_write(&cpi->b_multi_threaded, 0);
    {
      int i;
