...
TEST(VP8, TestBitIO)
{
    ...
    for (int n = 0; n < FLAGS_num_tests; ++n)
    {
        for (int method = 0; method <= 7; ++method)
        {   // we generate various proba
            const int bits_to_test = 20000;

            unsigned char probas[bits_to_test];
            ...
            for (int bit_method = 0; bit_method <= 3; ++bit_method)
            {
                ...
                BOOL_CODER bw;
                unsigned char *bw_buffer = new unsigned char[buffer_size];
                ...
                for (int i = 0; i < bits_to_test; ++i)
                {
                    ...
                    EXPECT_EQ(vp8dx_decode_bool(&br, probas[i]), bit) << "pos: "
