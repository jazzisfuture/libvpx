...
static void vp8_subtract_mb(const VP8_ENCODER_RTCD *rtcd, MACROBLOCK *x) {
  ...
  vp8_subtract_mby(x->src_diff, *(b->base_src), x->e_mbd.predictor, b->src_stride);
  vp8_subtract_mbuv(x->src_diff, x->src.u_buffer, x->src.v_buffer, x->e_mbd.predictor, x->src.uv_stride);
}
