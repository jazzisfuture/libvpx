...
void vp8_denoiser_set_parameters(VP8_DENOISER *denoiser)
{
  ...
  if (denoiser->aggressive_mode) {
    ...
    denoiser->denoise_pars.scale_motion_thresh = 2;
    denoiser->denoise_pars.denoise_mv_bias = 70;
    denoiser->denoise_pars.pickmode_mv_bias = 70;
    denoiser->denoise_pars.qp_thresh = 100;
    denoiser->denoise_pars.consec_zerolast = 10;
    denoiser->denoise_pars.spatial_blur = 40;
  }
}
                             ...
                             int uv_denoise)
{
    ...
    if (zero_frame)
    {
        ...
        int zero_bias = denoiser->denoise_pars.denoise_mv_bias;
    ...
    if (denoiser->aggressive_mode && motion_magnitude2 < NOISE_MOTION_THRESHOLD)
      x->increase_denoising = 1;

    sse_thresh = denoiser->denoise_pars.scale_sse_thresh * SSE_THRESHOLD;
    if (x->increase_denoising) sse_thresh =
