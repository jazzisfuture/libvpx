                             ...
                             int increase_denoising)
{
    ...
    // No denoising if color signal is close to average level.
    for (r = 0; r < 8; ++r)
    {
      for (c = 0; c < 8; ++c)
      {
                             ...
                             int block_index)
{
    ...
    // Option to denoise the UV channel.
    ...
    if (decision == FILTER_BLOCK)
    {
        ...
        if (apply_color_denoise &&
          ...
          decision_u =
              vp8_denoiser_filter_uv_c(mc_running_avg_u, mc_avg_uv_stride,
                                      running_avg_u, avg_uv_stride,
                                      x->block[16].src + *x->block[16].base_src,
                                      signal_stride, motion_magnitude2, 0);
          decision_v =
              vp8_denoiser_filter_uv_c(mc_running_avg_v, mc_avg_uv_stride,
