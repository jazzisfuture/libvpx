                                               ...
                                               int recon_uvoffset)
{
    ...
    {
        vp8_build_inter16x16_predictors_mb(&filter_xd, filter_xd.dst.y_buffer,
...
static void denoiser_filter(YV12_BUFFER_CONFIG* mc_running_avg,
                            ...
                            unsigned int motion_magnitude2, int y_offset,
                            int uv_offset)
{
    ...
    for (r = 0; r < 16; r++)
    {
        for (c = 0; c < 16; c++)
        {
            ...
            // Allow some additional filtering of static blocks, or blocks with
            // very small motion vectors.
            filter_coefficient += filter_coefficient
                    / (3 + (motion_magnitude2 >> 3));
            filter_coefficient =
                    filter_coefficient > 255 ? 255 : filter_coefficient;

            running_avg_y[c] = blend(mc_running_avg_y[c], sig[c],
                                     filter_coefficient);
            ...
            {
                // Replace the filter state with the signal since the change in
                // this pixel isn't classified as noise.
                             ...
                             int recon_yoffset, int recon_uvoffset)
{
    ...
    MV_REFERENCE_FRAME frame=0;

    if ( x->e_mbd.best_reference_frame > LAST_FRAME )
        frame = x->e_mbd.best_reference_frame - 1;

    // Motion compensate the running average.
    best_sse = denoiser_motion_compensate(&denoiser->yv12_running_avg[frame],
                                          &denoiser->yv12_mc_running_avg, x,
                                          best_sse, zero_mv_sse, recon_yoffset,
                                          recon_uvoffset);

    mv_row = x->e_mbd.best_sse_mv.as_mv.row;
    mv_col = x->e_mbd.best_sse_mv.as_mv.col;
    motion_magnitude2 = mv_row * mv_row + mv_col * mv_col;
    if (1) //best_sse > SSE_THRESHOLD || motion_magnitude2
           // > 8 * NOISE_MOTION_THRESHOLD)
    {
        ...
        vp8_copy_mem16x16(
                x->thismb, 16,
                denoiser->yv12_running_avg[frame].y_buffer + recon_yoffset,
                denoiser->yv12_running_avg[frame].y_stride);
        return;
    }
    // Filter.
    denoiser_filter(&denoiser->yv12_mc_running_avg,
                    &denoiser->yv12_running_avg[frame], x, motion_magnitude2,
