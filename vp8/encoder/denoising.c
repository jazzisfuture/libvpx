...

{
    ...
    // (2) has not been selected as ZERO_LAST mode at least 4 past frames
    // in a row.
    // TODO:  Parameter "4" should be varied with framerate. In particualar,
    //  should be reduced for temporal layers (base layer/LAST reference).
    if (x->is_skin && ((consec_zero_last < 4) || motion_magnitude2 > 0))
      decision = COPY_BLOCK;
