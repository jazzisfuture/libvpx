...
#define ROUNDING ((1 << (RIGHT_SHIFT - 1)) - 1)
...
static void dct16x16_1d(int16_t input[16], int16_t output[16],
    ...
    final_rounding = (1 << (final_shift - 1)) - 1;
    ...
    // step 4
    output[ 0] = (step[ 0] + step[ 1]) >> output_shift;
    output[ 8] = (step[ 0] - step[ 1]) >> output_shift;
    ...
    output[ 2] = (2*((step[4] + step[ 5])*C8) + final_rounding) >> final_shift;
    output[14] = (2*((step[7] - step[ 6])*C8) + final_rounding) >> final_shift;

    temp1 = step[4] - step[5];
    temp2 = step[6] + step[7];
    output[ 6] = (temp1 + temp2) >> output_shift;
    output[10] = (temp1 - temp2) >> output_shift;
    ...
    output[ 9] = (2*((step[10] + step[11])*C8) + final_rounding) >> final_shift;
    ...
    output[15] = (intermediate[11] + intermediate[12]) >> output_shift;
    output[ 1] = -(intermediate[11] - intermediate[12]) >> output_shift;
