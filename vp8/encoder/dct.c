 ...
 * Note: To test it, call vp8_fht_int_c() instead of vp8_fht_c().
 */
void vp8_fht_int_c(short *input, short *output, int pitch,
                   TX_TYPE tx_type, int tx_dim) {
  int i, j, k;
  short imbuf[256];
  ...
  for(j = 0; j < tx_dim; j++) {
    for(i = 0; i < tx_dim; i++) {
      ...
      for(k = 0; k < tx_dim; k++) {
        temp += ptvc[k] * ip[(k * (pitch >> 1))];
      }

      im[i] = (temp +rounding) >> 11;
      ip ++;
    }
    im += tx_dim;
    ptvc += tx_dim;
