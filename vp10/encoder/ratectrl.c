...
static void update_golden_frame_stats(VP10_COMP *cpi) {
  ...
  // Wei-Ting: If we use show_existing_frame for an OVERLAY frame, the only
  //           the virtual indices for the reference frame will be updated
  //           and cpi->refresh_golden_frame will still be zero.
  if (cpi->refresh_golden_frame || rc->is_src_frame_alt_ref) {
#else
    // Update the Golden frame usage counts.
