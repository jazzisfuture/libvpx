...
void vp10_update_reference_frames(VP10_COMP *cpi) {
  ...
  if (cpi->refresh_last_frame) {
    ...
    // updated and written to the bitstream, since the ref-idx for LAST_FRAME
    // and BWDREF_FRAME should be switched, i.e. cpi->lst_fb_idx and
    // cpi->bwd_fb_idx should be switched.
...
static void encode_frame_to_data_rate(VP10_COMP *cpi,
...
#if 0
    // NOTE(zoeliu): To check the packed bitstream for the frame just coded
    printf("After vp10_pack_bitstream(): "
           "Frame=%d, show_existing_frame=%d, show_frame=%d, "
           "twopass_gf_group_update_type[%d]=%d, size=%d\n",
           cm->current_video_frame, cm->show_existing_frame, cm->show_frame,
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    cpi->refresh_last_frame = 1;
#if CONFIG_BIDIR_PRED
    // TODO(zoeliu): We need to handle the forced KEY_FRAME specifically for
    //               BIDIR_PRED.
#endif  // CONFIG_BIDIR_PRED
#endif  // CONFIG_EXT_REFS
  }
  ...
  if (cpi->rc.is_last_nonref_frame) {
    ...
    //               writing a new frame next.
    cm->existing_fb_idx_to_show = cpi->bwd_fb_idx;
  }
#endif  // CONFIG_BIDIR_PRED
...
#if 1
  // zoeliu: for debug - backward prob update
...
int vp10_get_compressed_data(VP10_COMP *cpi, unsigned int *frame_flags,
  ...
  if (oxcf->pass == 2 && cm->show_existing_frame) {
    ...
    // coded already
    if (vp10_lookahead_pop(cpi->lookahead, flush) == NULL)
      return -1;
...
#if CONFIG_BIDIR_PRED
  // TODO(zoeliu): To handle BIDIR_PRED when alt-ref is disabled
  brf_src_index = get_brf_src_index(cpi);
  // TODO(zoeliu): To re-evaluate whether rc->is_bwd_ref_frame is needed
  rc->is_bwd_ref_frame = 0;
  if (brf_src_index) {
    if (brf_src_index <= rc->frames_to_key &&
        (!arf_src_index || brf_src_index < arf_src_index)) {
  ...
  if (cm->current_video_frame == 18) {
    ...
    for (ref_idx = 0; ref_idx < REF_FRAMES; ++ref_idx) {
      printf("%d(%d) ", cm->ref_frame_map[ref_idx],
             pool->frame_bufs[cm->ref_frame_map[ref_idx]].ref_count);
    }
    printf("\n");
  }
#endif  // 0
...
#if CONFIG_BIDIR_PRED
  if (cpi->b_calculate_psnr && oxcf->pass != 1 &&
      (cm->show_frame || cpi->rc.is_bwd_ref_frame))
#else
