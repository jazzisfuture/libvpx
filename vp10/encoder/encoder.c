...
#if !CONFIG_EXT_REFS && CONFIG_BIDIR_PRED
#if 0
  // zoeliu: for debug - Output the filtered reconstructed video
  {
    int h;
    const YV12_BUFFER_CONFIG *recon_buf = cm->frame_to_show;
    FILE *f_recon = fopen(
        "/usr/local/google/home/zoeliu/webm/work_logs"
        "/bidir_pred/results/recon_filter.yuv", "ab");
    assert(f_recon != NULL);
    printf("Frame=%d, update_type[%d]=%d, y_stride=%d, uv_stride=%d, "
           "width=%d, height=%d\n",
           cm->current_video_frame, cpi->twopass.gf_group.index,
           cpi->twopass.gf_group.update_type[cpi->twopass.gf_group.index],
           recon_buf->y_stride, recon_buf->uv_stride,
           cm->width, cm->height);
    // --- Y ---
