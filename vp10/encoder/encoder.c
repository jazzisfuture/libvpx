...
#else
  vpx_calc_psnr(cpi->Source, cpi->common.frame_to_show, &psnr, 8, 8);
#endif
...
#if CONFIG_VP9_HIGHBITDEPTH
      vpx_calc_psnr(orig, recon, &psnr, cpi->td.mb.e_mbd.bd, in_bit_depth);
#else
      vpx_calc_psnr(orig, recon, &psnr, 8, 8);
#endif  // CONFIG_VP9_HIGHBITDEPTH
