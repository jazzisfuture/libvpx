...
#if CONFIG_EXT_REFS
    // Since we allocate a spot in the gf group for the OVERLAY frame, we need
...
static void Pass2Encode(VP10_COMP *cpi, size_t *size,
  ...
  // a gf group. However An OVERLAY frame always have a spot in a gf group,
  // even if it is an existing frame, so we need to do post-encoding update
  // accordingly.
