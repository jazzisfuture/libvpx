...
static BLOCK_SIZE select_sb_size(const VP10_COMP *const cpi) {
...
#if CONFIG_EXT_TILE
  assert(cm->tile_width <= 32);
  assert(cm->tile_height <= 32);
  ...
  assert(cm->tile_width % MAX_MIB_SIZE == 0);
  assert(cm->tile_height % MAX_MIB_SIZE == 0);

  // TODO(any): Possibly could improve this with a heuristic.
  return BLOCK_128X128;
#else
  (void)cpi;
  return BLOCK_LARGEST;
...
static void set_tile_limits(VP10_COMP *cpi) {
  ...
  if (cpi->oxcf.superblock_size == VPX_SUPERBLOCK_SIZE_64X64) {
    cm->tile_width  =
        clamp(cpi->oxcf.tile_columns, 1, 64) << (MAX_MIB_SIZE_LOG2 - 1);
    cm->tile_height =
        clamp(cpi->oxcf.tile_rows, 1, 64) << (MAX_MIB_SIZE_LOG2 - 1);
  } else {
    cm->tile_width  =
        clamp(cpi->oxcf.tile_columns, 1, 32) << MAX_MIB_SIZE_LOG2;
    cm->tile_height =
        clamp(cpi->oxcf.tile_rows, 1, 32) << MAX_MIB_SIZE_LOG2;
  }
