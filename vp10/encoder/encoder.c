...
void vp10_update_reference_frames(VP10_COMP *cpi) {
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    if (cpi->refresh_bwd_ref_frame) {
      ref_cnt_fb(pool->frame_bufs,
                 &cm->ref_frame_map[cpi->bwd_fb_idx], cm->new_fb_idx);
      // TODO(zoeliu): To have bidir-pred work with unsampled reference frames.
...
static void dump_filtered_recon_frame(VP10_COMP *cpi) {
  VP10_COMMON *const cm = &cpi->common;
  const YV12_BUFFER_CONFIG *recon_buf = cm->frame_to_show;
  int h;

  FILE *f_recon = fopen("/tmp/enc_filtered_recon.yuv", "ab");

  assert(f_recon != NULL);
  assert(recon_buf != NULL);
  assert(cm->show_frame);

  if (cm->current_video_frame == 0)
    printf("\n");
  printf("Frame=%d, encode_update_type[%d]=%d, "
         "y_stride=%d, uv_stride=%d, width=%d, height=%d\n",
