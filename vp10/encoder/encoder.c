...
#if CONFIG_GF_FLAG
    sprintf(str, "./tmp/ref_frame%d_%d.yuv",cm->current_video_frame, i);
#else
    sprintf(str, "./tmp/b_ref_frame%d_%d.yuv",cm->current_video_frame, i);
#endif
...
static void encode_frame_to_data_rate(VP10_COMP *cpi,
  ...
  if (cm->show_existing_frame) {
    ...
    tmp_mask = get_refresh_mask(cpi);
    for (i = 0; i < REF_FRAMES; ++i) {
      fprintf(stdout, "%d ", tmp_mask & 1);
      tmp_mask >>= 1;
    }
    fprintf(stdout," ------( rl %d, rg %d, rb %d, ra %d, new %d) refresh mask %d\n",
            cpi->refresh_last_frame, cpi->refresh_golden_frame,
...
#if CONFIG_BUF_STATUS
    if(cpi->frame_check)
      vp10_write_yuv_frames_in_buffer(cpi);
    cpi->frame_check = 0;
#endif
...
#if CONFIG_GF_FLAG
    if(cpi->is_gf_group_begin){
      vp10_set_target_rate(cpi);
      //vp10_rc_existing_overlay_update(cpi, *size);
...
static void Pass2Encode(VP10_COMP *cpi, size_t *size,
  ...
  fprintf(stdout,"RC off target %ld (%d, %d)\n",cpi->rc.vbr_bits_off_target,
          cpi->rc.base_frame_target, cpi->rc.projected_frame_size);
#endif

#if CONFIG_BUF_STATUS
  fprintf(stdout,"gf_group.index is %d\n",cpi->twopass.gf_group.index);
#endif
}
...
#if CONFIG_BUF_STATUS
    if(cm->show_existing_frame){
      fprintf(stdout,"--- show existing\n");
    }
    if (oxcf->pass == 2)
      fprintf(stdout,"check point begin %d\n",cpi->twopass.gf_group.index);
#endif
...
#if CONFIG_GF_FLAG
    cpi->is_gf_group_begin = 0;
#endif

#if CONFIG_BUF_STATUS
  if (oxcf->pass == 2)
    fprintf(stdout,"check point end %d\n\n",cpi->twopass.gf_group.index);
#endif
...
#if CONFIG_GF_FLAG
    //lookahead the next frame to check if it is an overlay frame
    if (cpi->oxcf.pass == 2) {
      const GF_GROUP *const gf_group = &cpi->twopass.gf_group;
      rc->is_src_frame_alt_ref = (gf_group->update_type[gf_group->index]
                                  == OVERLAY_UPDATE);
      if (rc->is_src_frame_alt_ref) {
        // Current frame is an ARF overlay frame.
        cpi->alt_ref_source = NULL;
        if (cpi->arf_filter_off) {
          cm->show_existing_frame = 1;
          cpi->is_gf_group_begin = 1;
