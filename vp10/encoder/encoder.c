...
void vp10_update_reference_frames(VP10_COMP *cpi) {
  ...
  if (cpi->refresh_last_frame) {
    ...
    if (cpi->rc.is_last_nonref_frame) {
      ...
      // reference frame; The backward reference frame still acts as a reference
      // but as LAST instead of BWDREF, so its ref_count should stay the same;
      // In ref_cnt_fb(), the ref_count has been incremented by 1, so here we
      // decrease it by 1 to make it unchanged.
