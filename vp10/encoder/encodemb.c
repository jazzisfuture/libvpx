...
#if CONFIG_VP9_HIGHBITDEPTH
static VP10_QUANT_FACADE quant_func_ls[4][4] = { 
  {vp10_quantize_fp_facade, vp10_quantize_fp_32x32_facade, vp10_highbd_quantize_fp_facade, vp10_highbd_quantize_fp_32x32_facade},
  {vp10_quantize_b_facade,  vp10_quantize_b_32x32_facade, vp10_highbd_quantize_b_facade, vp10_highbd_quantize_b_32x32_facade},
  {vp10_quantize_dc_facade, vp10_quantize_dc_32x32_facade, vp10_highbd_quantize_dc_facade, vp10_highbd_quantize_dc_32x32_facade},
  {NULL, NULL, NULL, NULL}  // VP10_XFORM_QUANT_SKIP_QUANT doesn't need quant function
};

#else
static VP10_QUANT_FACADE quant_func_ls[4][2] = { 
  ...
  {NULL, NULL}  // VP10_XFORM_QUANT_SKIP_QUANT doesn't need quant function
};

#endif

static FWD_TXFM_OPT fwd_txfm_opt_ls[4] = { FWD_TXFM_OPT_NORMAL, FWD_TXFM_OPT_NORMAL, FWD_TXFM_OPT_DC, FWD_TXFM_OPT_NORMAL};
...
void vp10_xform_quant(MACROBLOCK *x, int plane, int block,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    if(xform_quant_idx != VP10_XFORM_QUANT_SKIP_QUANT) {
      if (x->skip_block) {
        ...
        if (tx_size == TX_32X32)
          quant_func_ls[xform_quant_idx][3](coeff, tx2d_size, p, qcoeff, pd, dqcoeff,
                           eob, scan_order);
        else
          quant_func_ls[xform_quant_idx][2](coeff, tx2d_size, p, qcoeff, pd, dqcoeff, eob,
                           scan_order);
      }
    }
    return;
  ...
  if(xform_quant_idx != VP10_XFORM_QUANT_SKIP_QUANT) {
    if (x->skip_block) {
      ...
      if (tx_size == TX_32X32)
        quant_func_ls[xform_quant_idx][1](coeff, tx2d_size, p, qcoeff, pd, dqcoeff, eob,
                         scan_order);
      else
        quant_func_ls[xform_quant_idx][0](coeff, tx2d_size, p, qcoeff, pd, dqcoeff, eob,
                         scan_order);
    }
  }
}
...
static void encode_block_pass1(int plane, int block, int blk_row, int blk_col,
  ...
  vp10_xform_quant(x, plane, block, blk_row, blk_col, plane_bsize, tx_size, VP10_XFORM_QUANT_B);
...
void vp10_encode_block_intra(int plane, int block, int blk_row, int blk_col,
  ...
  if(!x->skip_recode)
    ...
    vp10_xform_quant(x, plane, block, blk_row, blk_col,
                     plane_bsize, tx_size,  VP10_XFORM_QUANT_SKIP_QUANT);
