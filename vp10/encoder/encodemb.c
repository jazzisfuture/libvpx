...
static VP10_QUANT_FACADE quant_func_ls[4][4] = {
    ...
    {NULL, NULL, NULL, NULL}}; 
    // VP10_XFORM_QUANT_SKIP_QUANT doesn't need quant function
...
static FWD_TXFM_OPT fwd_txfm_opt_ls[4] = {FWD_TXFM_OPT_NORMAL, FWD_TXFM_OPT_NORMAL, 
                                          FWD_TXFM_OPT_DC, FWD_TXFM_OPT_NORMAL};
...
void vp10_xform_quant(MACROBLOCK *x, int plane, int block, int blk_row,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    if(xform_quant_idx != VP10_XFORM_QUANT_SKIP_QUANT) {
      if (x->skip_block) {
        ...
        if (tx_size == TX_32X32)
          quant_func_ls[xform_quant_idx][3](coeff, tx2d_size, p, qcoeff, pd,
                                            dqcoeff, eob, scan_order);
        else
          quant_func_ls[xform_quant_idx][2](coeff, tx2d_size, p, qcoeff, pd,
                                            dqcoeff, eob, scan_order);
      }
    }
    return;
  ...
  if(xform_quant_idx != VP10_XFORM_QUANT_SKIP_QUANT) {
    if (x->skip_block) {
      ...
      if (tx_size == TX_32X32)
        quant_func_ls[xform_quant_idx][1](coeff, tx2d_size, p, qcoeff, pd,
                                          dqcoeff, eob, scan_order);
      else
        quant_func_ls[xform_quant_idx][0](coeff, tx2d_size, p, qcoeff, pd,
                                          dqcoeff, eob, scan_order);
    }
  }
}
