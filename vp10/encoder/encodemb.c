...
int vp10_optimize_b(MACROBLOCK *mb, int plane, int block,
  ...
  const int default_eob = 16 << num_4x4_blocks_txsize_log2_lookup[tx_size];
...
void vp10_encode_block_intra(int plane, int block, int blk_row, int blk_col,
  ...
  const int tx1d_width = get_tx1d_width(tx_size);
  const int tx1d_height = get_tx1d_height(tx_size);
