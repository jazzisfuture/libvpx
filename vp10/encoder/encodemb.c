...
void vp10_xform_quant(MACROBLOCK *x, int plane, int block, int blk_row,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    if (xform_quant_idx != VP10_XFORM_QUANT_SKIP_QUANT) {
      if (x->skip_block) {
        ...
        if (tx_size == TX_32X32)
          quant_func_ls[xform_quant_idx][QUANT_FUNC_HIGHBD_32](coeff, tx2d_size, p, qcoeff, pd,
                                            dqcoeff, eob, scan_order);
        else
          quant_func_ls[xform_quant_idx][QUANT_FUNC_HIGHBD](coeff, tx2d_size, p, qcoeff, pd,
                                            dqcoeff, eob, scan_order);
      }
    }
    return;
  ...
  if (xform_quant_idx != VP10_XFORM_QUANT_SKIP_QUANT) {
    if (x->skip_block) {
      ...
      if (tx_size == TX_32X32)
        quant_func_ls[xform_quant_idx][QUANT_FUNC_LOWBD_32](coeff, tx2d_size, p, qcoeff, pd,
                                          dqcoeff, eob, scan_order);
      else
        quant_func_ls[xform_quant_idx][QUANT_FUNC_LOWBD](coeff, tx2d_size, p, qcoeff, pd,
                                          dqcoeff, eob, scan_order);
    }
  }
}
