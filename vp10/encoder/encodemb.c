...
// to quant_func_list, just trying to get this expr to work for now
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    switch (tx_size) {
      case TX_32X32:
        highbd_quantize_dc_32x32_nuq(coeff, x->skip_block,
        ...
        break;
      case TX_16X16:
        highbd_quantize_dc_nuq(coeff, x->skip_block,
                               p->quant[0], p->quant_shift[0],
                               pd->dequant[0],
                               p->cuml_bins_nuq[0],
                               pd->dequant_val_nuq[0],
  ...
  switch (tx_size) {
    case TX_32X32:
      quantize_dc_32x32_nuq(coeff, x->skip_block,
      ...
      break;
    case TX_16X16:
      quantize_dc_nuq(coeff, x->skip_block,
                      p->quant[0], p->quant_shift[0], pd->dequant[0],
                      p->cuml_bins_nuq[0],
                      pd->dequant_val_nuq[0],
                      qcoeff, dqcoeff, eob);
...
// to quant_func_list, just trying to get this expr to work for now
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    switch (tx_size) {
      case TX_32X32:
        highbd_quantize_dc_32x32_fp_nuq(coeff, x->skip_block,
        ...
        break;
      case TX_16X16:
        highbd_quantize_dc_fp_nuq(coeff, x->skip_block,
                                  p->quant_fp[0], pd->dequant[0],
                                  p->cuml_bins_nuq[0],
                                  pd->dequant_val_nuq[0],
                                  qcoeff, dqcoeff, eob);
  ...
  switch (tx_size) {
    case TX_32X32:
      quantize_dc_32x32_fp_nuq(coeff, x->skip_block,
                               p->quant_fp[0], pd->dequant[0],
                               p->cuml_bins_nuq[0],
                               pd->dequant_val_nuq[0],
                               qcoeff, dqcoeff, eob);
      break;
    case TX_16X16:
      quantize_dc_fp_nuq(coeff, x->skip_block,
                         p->quant_fp[0], pd->dequant[0],
                         p->cuml_bins_nuq[0],
                         pd->dequant_val_nuq[0],
                         qcoeff, dqcoeff, eob);
...
#if CONFIG_NEW_QUANT
  if (x->quant_fp) //TODO(sarahparker) is this needed?
    vp10_xform_quant_fp_nuq(x, plane, block, blk_row, blk_col, plane_bsize,
                            tx_size);
  else
    vp10_xform_quant_nuq(x, plane, block, blk_row, blk_col, plane_bsize,
...
//TODO(sarahparker)should ext intra take priority over new quant?
//TODO should we check for fp/dc here?
#if CONFIG_NEW_QUANT
  vp10_xform_quant_nuq(x, plane, block, blk_row, blk_col, plane_bsize,
                       tx_size);
#else  // CONFIG_NEW_QUANT
