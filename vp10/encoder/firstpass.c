...
void vp10_rc_get_second_pass_params(VP10_COMP *cpi) {
  ...
  printf("vp10_rc_get_second_pass_params(): "
         "Frame=%d, update_type[%d]=%d, target_rate(before)=%d\n",
         cm->current_video_frame, gf_group->index,
         gf_group->update_type[gf_group->index], target_rate);
#endif  // 1
  if (cpi->common.frame_type == KEY_FRAME)
  ...
  // NOTE(zoeliu): for debug
  printf("vp10_rc_get_second_pass_params(): "
         "Frame=%d, update_type[%d]=%d, target_rate(after)=%d\n",
         cm->current_video_frame, gf_group->index,
