    ...
    for (i = 0; i < cpi->extra_arfs; ++i) {
      ...
      gf_group->arf_src_offset[frame_index] =
        (unsigned char)(rc->baseline_gf_interval / (cpi->extra_arfs + 1) *
                        (cpi->extra_arfs - i) - 1);
      gf_group->arf_update_idx[frame_index] = i + 1;

      // TODO(weitinglin): Finalize choosing whether ALT0 or the closest ALT
      // as the ARF for this internal ARF
      gf_group->arf_ref_idx[frame_index] = i;
      ++frame_index;
    }
