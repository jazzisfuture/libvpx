...
void vp10_entropy_mv_init(void) {
  vpx_tokens_from_tree(mv_joint_encodings, vp10_mv_joint_tree);
  vpx_tokens_from_tree(mv_class_encodings, vp10_mv_class_tree);
  vpx_tokens_from_tree(mv_class0_encodings, vp10_mv_class0_tree);
  vpx_tokens_from_tree(mv_fp_encodings, vp10_mv_fp_tree);
}
...
static void encode_mv_component(vpx_writer* w, int comp,
  ...
  // Class
  vpx_write_tree(w, vp10_mv_class_tree, mvcomp->classes,
                 mv_class_encodings[mv_class].value,
                 mv_class_encodings[mv_class].len, 0);

  // Integer bits
  if (mv_class == MV_CLASS_0) {
    vpx_write_tree(w, vp10_mv_class0_tree, mvcomp->class0,
  ...
  // Fractional bits
  vpx_write_tree(w, vp10_mv_fp_tree,
                 mv_class == MV_CLASS_0 ?  mvcomp->class0_fp[d] : mvcomp->fp,
                     mv_fp_encodings[fr].value,
                     mv_fp_encodings[fr].len, 0);
...
void vp10_encode_mv(VP10_COMP* cpi, vpx_writer* w,
  ...
  vpx_write_tree(w, vp10_mv_joint_tree, mvctx->joints,
