...
void highbd_quantize_nuq_c(const tran_low_t *coeff_ptr,
                           ...
                           const uint8_t *band) {
  int eob = -1;
  vpx_memset16(qcoeff_ptr, 0, n_coeffs * sizeof(*qcoeff_ptr));
  vpx_memset16(dqcoeff_ptr, 0, n_coeffs * sizeof(*dqcoeff_ptr));
  if (!skip_block) {
    int i;
    for (i = 0; i < n_coeffs; i++) {
      const int rc = scan[i];
...
//    xd->plane[i].dequant_val_nuq[0] = cpi->uv_dequant_val_nuq[qindex];
//#endif  // CONFIG_NEW_QUANT
