  ...
  if (1) {
    ...
    if (off_edge) {
      ...
      *dist += (int64_t)tmp * 16;
      ...
      {
        const int txpx = 4<<tx_size;
        const int64_t ti = (int64_t)tmp * 16;
        ...
        for(row=0 ; row < txpx ; row++) {
          for(col=0 ; col < txpx ; col++) {
            fail |= src[row*src_stride + col] != diff[row*diff_stride+col] + dst[row*pd->dst.stride+col];
          }
         }

        dist_block(x, plane, block, tx_size, &td, &ts);

        ratio = (double)td/(double)ti;

        if (!fail && (ratio >= 1.05 || ratio <= 0.95)) {
          int row, col;
          const int txpx = 4<<tx_size;

          printf("tx_size=%dx%d, tx_type=%d, tdist=%ld, idist=%ld, tdist/ididt=%6.5f",
              txpx, txpx, tx_type, td, ti, ratio);

          printf(", source_image=[");
          for(row=0 ; row < txpx ; row++) {
            for(col=0 ; col < txpx ; col++) {
              printf("%d", src[row*src_stride+col]);
              if (!(row == txpx-1 && col == txpx-1))
                printf(",");
            }
          }
          printf("]");
          ...
          for(row=0 ; row < txpx ; row++) {
            for(col=0 ; col < txpx ; col++) {
              printf("%d", diff[row*diff_stride+col]);
              if (!(row == txpx-1 && col == txpx-1))
                printf(",");
            }
          }
          ...
          printf(", reconstructed_image=[");
          for(row=0 ; row < txpx ; row++) {
            for(col=0 ; col < txpx ; col++) {
              printf("%d", rec_buffer[row*32+col]);
              if (!(row == txpx-1 && col == txpx-1))
                printf(",");
            }
          }
