...
static int cost_mv_ref(const VP10_COMP *cpi, PREDICTION_MODE mode,
...
#if CONFIG_REF_MV
  int mode_cost = 0;
  uint8_t mode_ctx = mode_context & 0x07;
  ...
  if (mode == NEWMV) {
    ...
    mode_ctx = (mode_context >> 3) & 0x01;
    if (mode == ZEROMV) {
      mode_cost += cpi->zeromv_mode_cost[mode_ctx][0];
      return mode_cost;
    } else {
      mode_cost += cpi->zeromv_mode_cost[mode_ctx][1];
      mode_ctx = (mode_context >> 4) & 0x02;
      return mode_cost += cpi->refmv_mode_cost[mode_ctx][mode != NEARESTMV];
    }
  }
#else
  assert(is_inter_mode(mode));
