...
void vp10_build_prediction_by_above_preds(VP10_COMP *cpi,
  ...
  for (i = 0; i < MAX_MB_PLANE; ++i) {
    ...
    xd->plane[i].dst.stride = tmp_stride[i];
  }

  xd->mb_to_bottom_edge = ((cm->mi_rows - VPXMAX(xd->n8_h / 2, 1) - mi_row) *
                           MI_SIZE) * 8;
  for (i = 0; i < VPXMIN(xd->n8_w, cm->mi_cols - mi_col); i += mi_step) {
    ...
    xd->mb_to_right_edge  =
        ((cm->mi_cols - mi_step - mi_col - i) * MI_SIZE) * 8;
  ...
  xd->mb_to_right_edge  = ((cm->mi_cols - xd->n8_w - mi_col) * MI_SIZE) * 8;
  xd->mb_to_bottom_edge = ((cm->mi_rows - xd->n8_h - mi_row) * MI_SIZE) * 8;
}
...
void vp10_build_prediction_by_left_preds(VP10_COMP *cpi,
  ...
  xd->mb_to_right_edge  = ((cm->mi_cols - VPXMAX(xd->n8_w / 2, 1) - mi_col) *
                           MI_SIZE) * 8;
  for (i = 0; i < VPXMIN(xd->n8_h, cm->mi_rows - mi_row); i += mi_step) {
    ...
    xd->mb_to_bottom_edge =
        ((cm->mi_rows - mi_step - mi_row - i) * MI_SIZE) * 8;
