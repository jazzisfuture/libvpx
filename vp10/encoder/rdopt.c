...
static int do_tx_type_search(TX_TYPE tx_type,
  ...
  (void) tx_type;
  (void) prune_htx_type;
  for (prune_tx = 0; prune_tx < TX_TYPES_1D - 1; ++prune_tx) {
    if ((prune_vtx_type >> prune_tx) % 2)   // && vtx_tab[tx_type] == prune_tx)
      return 0;
  //  if ((prune_htx_type >> prune_tx) % 2 && htx_tab[tx_type] == prune_tx)
  //  return 0;
  }
  return 1;
}
