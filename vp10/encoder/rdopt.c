...
#if CONFIG_VP9_HIGHBITDEPTH
      DECLARE_ALIGNED(16, uint16_t, recon16[MAX_TX_PIXELS]);
      uint8_t *recon = (uint8_t*)recon16;
#else
      DECLARE_ALIGNED(16, uint8_t, recon[MAX_TX_PIXELS]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
...
#if CONFIG_VP9_HIGHBITDEPTH
  DECLARE_ALIGNED(16, uint16_t, rec_buffer16[MAX_TX_PIXELS]);
  uint8_t *rec_buffer;
#else
  DECLARE_ALIGNED(16, uint8_t, rec_buffer[MAX_TX_PIXELS]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
  const int diff_stride = 4 * num_4x4_blocks_wide_lookup[plane_bsize];
  const int16_t *diff = &p->src_diff[4 * (blk_row * diff_stride + blk_col)];
...
#if CONFIG_VP9_HIGHBITDEPTH
  DECLARE_ALIGNED(16, uint16_t, second_pred_alloc_16[MAX_CU_PIXELS]);
  uint8_t *second_pred;
#else
  DECLARE_ALIGNED(16, uint8_t, second_pred[MAX_CU_PIXELS]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
...
static int64_t handle_inter_mode(VP10_COMP *cpi, MACROBLOCK *x,
...
#endif  // CONFIG_EXT_INTER
#if CONFIG_VP9_HIGHBITDEPTH
  DECLARE_ALIGNED(16, uint16_t, tmp_buf16[MAX_MB_PLANE * MAX_CU_PIXELS]);
  uint8_t *tmp_buf;
#else
  DECLARE_ALIGNED(16, uint8_t, tmp_buf[MAX_MB_PLANE * MAX_CU_PIXELS]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
  ...
  int rs = 0;
  INTERP_FILTER best_filter = SWITCHABLE;
  uint8_t skip_txfm[MAX_MB_PLANE][MAX_TX_BLOCKS_IN_MAX_CU] = {{0}};
  int64_t bsse[MAX_MB_PLANE][MAX_TX_BLOCKS_IN_MAX_CU] = {{0}};
  ...
  *compmode_wedge_cost = 0;
      ...
      if (i > 0 && intpel_mv && IsInterpolatingFilter(i)) {
        ...
        if ((cm->interp_filter == SWITCHABLE &&
          ...
          for (j = 0; j < MAX_MB_PLANE; j++) {
            xd->plane[j].dst.buf = tmp_buf + j * MAX_CU_PIXELS;
            xd->plane[j].dst.stride = MAX_CU_SIZE;
          }
        }
  ...
  if (is_comp_pred && get_wedge_bits(bsize)) {
    ...
    if (have_newmv_in_inter_mode(this_mode)) {
      ...
      uint8_t pred0[2 * MAX_CU_PIXELS * 3];
      uint8_t pred1[2 * MAX_CU_PIXELS * 3];
      uint8_t *preds0[3] = {pred0,
                            pred0 + 2 * MAX_CU_PIXELS,
                            pred0 + 4 * MAX_CU_PIXELS};
      uint8_t *preds1[3] = {pred1,
                            pred1 + 2 * MAX_CU_PIXELS,
                            pred1 + 4 * MAX_CU_PIXELS};
      int strides[3] = {MAX_CU_SIZE, MAX_CU_SIZE, MAX_CU_SIZE};
  ...
  if (is_comp_interintra_pred) {
    ...
    for (interintra_mode = DC_PRED; interintra_mode <= TM_PRED;
      ...
      vp10_build_interintra_predictors(xd,
                                       tmp_buf,
                                       tmp_buf + MAX_CU_PIXELS,
                                       tmp_buf + 2 * MAX_CU_PIXELS,
                                       MAX_CU_SIZE,
                                       MAX_CU_SIZE,
                                       MAX_CU_SIZE,
                                       bsize);
  ...
  if (pred_exists) {
    if (best_needs_copy) {
      ...
      for (i = 0; i < MAX_MB_PLANE; i++) {
        xd->plane[i].dst.buf = tmp_buf + i * MAX_CU_PIXELS;
        xd->plane[i].dst.stride = MAX_CU_SIZE;
      }
    }
    rd = tmp_rd;
  } else {
...
void vp10_rd_pick_inter_mode_sb(VP10_COMP *cpi,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    dst_buf1[2] = CONVERT_TO_BYTEPTR(tmp_buf1 + 2 * MAX_CU_PIXELS * len);
    dst_buf2[0] = CONVERT_TO_BYTEPTR(tmp_buf2);
    dst_buf2[1] = CONVERT_TO_BYTEPTR(tmp_buf2 + MAX_CU_PIXELS * len);
    dst_buf2[2] = CONVERT_TO_BYTEPTR(tmp_buf2 + 2 * MAX_CU_PIXELS * len);
  } else {
#endif  // CONFIG_VP9_HIGHBITDEPTH
  dst_buf1[0] = tmp_buf1;
  dst_buf1[1] = tmp_buf1 + MAX_CU_PIXELS;
  dst_buf1[2] = tmp_buf1 + 2 * MAX_CU_PIXELS;
  dst_buf2[0] = tmp_buf2;
  dst_buf2[1] = tmp_buf2 + MAX_CU_PIXELS;
  dst_buf2[2] = tmp_buf2 + 2 * MAX_CU_PIXELS;
#if CONFIG_VP9_HIGHBITDEPTH
  }
#endif  // CONFIG_VP9_HIGHBITDEPTH
#endif  // CONFIG_OBMC
