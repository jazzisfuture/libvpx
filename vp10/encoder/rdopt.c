...
static int rd_pick_ext_intra_sby(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  //delta = EXT_INTRA_ANGLES / (4 * N);
  //delta = EXT_INTRA_ANGLES / (2 * N + 1);
  for (step = 0; step < 3; ++step) {
    ...
    for (i = -branches[step]; i <= branches[step]; ++i) {
      ...
      if (*best_rd == INT64_MAX)
        ...
        rd_thresh = VPXMIN(best_angle_rd, *best_rd * ADJUSTER);
      super_block_yrd(cpi, x, &this_rate_tokenonly, &this_distortion,
      ...
      this_rate = this_rate_tokenonly +
          vp10_cost_bit(cpi->common.fc->ext_intra_probs[0], 1) +
          //vp10_cost_bit(DR_EXT_INTRA_PROB, 1) +
    ...
    if (delta < 1)
      break;
    if (*best_rd < best_angle_rd / ADJUSTER)
      break;
  }
  //printf("%d\n", step);
#else
  ...
  for (step = 0; step < 3; ++step) {
    ...
    for (i = -branches[step]; i <= branches[step]; ++i) {
      ...
      if (*best_rd == INT64_MAX)
        ...
        rd_thresh = VPXMIN(best_angle_rd, *best_rd * ADJUSTER);
      if (!super_block_uvrd(cpi, x, &this_rate_tokenonly,
                            &this_distortion, &s, &this_sse, bsize, rd_thresh))
        continue;
      this_rate = this_rate_tokenonly +
          vp10_cost_bit(cpi->common.fc->ext_intra_probs[1], 1) +
          //vp10_cost_bit(DR_EXT_INTRA_PROB, 1) +
    ...
    if (ref_frame == INTRA_FRAME) {
      ...
      if (mbmi->mode == DC_PRED) {
        ...
        if (mbmi->ext_intra_mode_info.use_ext_intra_mode[0]) {
          ...
          //rate2 +=
            //  vp10_cost_bit(DR_EXT_INTRA_PROB, ext_intra_mode > FILTER_TM_PRED);
