...
static void get_energy_distribution(VP10_COMP *cpi,
                                    BLOCK_SIZE bsize,
...
static TX_TYPE_1D guess_tx_type_1d(double corr, double edst) {
  ...
  if (corr > 0.5 + FAST_EXT_TX_MARGIN) {
    // DCT preferred over DST
    if (edst < 0.5 - FAST_EXT_TX_MARGIN) {
    ...
    } else  if (edst > 0.5 + FAST_EXT_TX_MARGIN) {
  ...
  } else if (corr < 0.5 - FAST_EXT_TX_MARGIN) {
...
static int guess_inter_tx_type_for_sby(const VP10_COMP *cpi,
                                       BLOCK_SIZE bsize,
  ...
  hcorr = get_hor_correlation(p->src_diff, bw, bw, bh);
  vcorr = get_ver_correlation(p->src_diff, bw, bw, bh);

  hor_tx_type = guess_tx_type_1d(hcorr, hdist);
  ver_tx_type = guess_tx_type_1d(vcorr, vdist);
  if (hor_tx_type == UNKNOWN_TX_1D || ver_tx_type == UNKNOWN_TX_1D)
    return -1;
  else
...
#if FAST_EXT_TX_INTER_SEARCH
  int guessed_inter_tx_type =
      is_inter ? guess_inter_tx_type_for_sby(cpi, bs, x, xd) : DCT_DCT;
#endif  // FAST_EXT_TX_INTER_SEARCH
#endif  // CONFIG_EXT_TX
...
#if FAST_EXT_TX_INTER_SEARCH
        if (tx_type != DCT_DCT &&
            (guessed_inter_tx_type != -1 &&
             (int)tx_type != guessed_inter_tx_type) &&
            tx_type != IDTX)
          continue;
#else
        if (ext_tx_set == 1 &&
...
#if FAST_EXT_TX_INTER_SEARCH
  int guessed_inter_tx_type =
      is_inter ? guess_inter_tx_type_for_sby(cpi, bsize, x, xd) : DCT_DCT;
#endif  // FAST_EXT_TX_INTER_SEARCH
