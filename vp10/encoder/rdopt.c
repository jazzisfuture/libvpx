...
static int64_t encode_inter_mb_segment(VP10_COMP *cpi,
  ...
  for (idy = 0; idy < height / 4; ++idy) {
    for (idx = 0; idx < width / 4; ++idx) {
      ...
      if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
        vp10_highbd_fwd_txfm_4x4(src_diff, coeff, 8, DCT_DCT, xd->lossless[mi->mbmi.segment_id]);
      } else {
        vp10_fwd_txfm_4x4(src_diff, coeff, 8, DCT_DCT, xd->lossless[mi->mbmi.segment_id]);
      }
#else
      vp10_fwd_txfm_4x4(src_diff, coeff, 8, DCT_DCT, xd->lossless[mi->mbmi.segment_id]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
