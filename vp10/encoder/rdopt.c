...
#if CONFIG_EXT_TX
#define FAST_EXT_TX_INTER_SEARCH 0
...
static void choose_largest_tx_size(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  if (is_inter)
    guess_inter_tx_type_for_sby(cpi, bs, x, xd, &hor_tx_type, &ver_tx_type);
#endif  // FAST_EXT_TX_INTER_SEARCH
#endif  // CONFIG_EXT_TX
...
static void select_tx_type_yrd(const VP10_COMP *cpi, MACROBLOCK *x,
  ...
  TX_TYPE_1D hor_tx_type = DCT_1D, ver_tx_type = DCT_1D;
  if (is_inter)
    guess_inter_tx_type_for_sby(cpi, bsize, x, xd, &hor_tx_type, &ver_tx_type);
#endif  // FAST_EXT_TX_INTER_SEARCH
#endif // CONFIG_EXT_TX
