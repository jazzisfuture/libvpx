...
static int64_t rd_pick_intra_sby_mode(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  for (mode = DC_PRED; mode <= TM_PRED; mode++) {
    ...
    if (cpi->common.allow_screen_content_tools)
      this_rate += vp10_cost_bit(default_palette_y_mode_prob[bsize - BLOCK_8X8]
                                                             [palette_ctx], 0);
#endif  // CONFIG_PALETTE
  ...
  if (cpi->common.allow_screen_content_tools) {
    ...
    if (colors > 1 && colors <= 64 && cpi->common.allow_screen_content_tools) {
      ...
      for (n = colors > PALETTE_MAX_SIZE ? PALETTE_MAX_SIZE : colors;
        ...
        this_rate +=
            vp10_cost_bit(default_palette_y_mode_prob[bsize - BLOCK_8X8]
                                                      [palette_ctx], 1);
        color_map = xd->plane[0].color_index_map;
        this_rate +=  vp10_ceil_log2(k) * vp10_cost_bit(128, 0);
        for (i = 0; i < rows; ++i) {
        ...
        if (this_rd < best_rd) {
          ...
          if (best_palette_color_map == NULL)
            best_palette_color_map =
                vpx_calloc(rows * cols, sizeof(*best_palette_color_map));
  ...
  if (best_palette_color_map != NULL)
    vpx_free(best_palette_color_map);
#endif  // CONFIG_PALETTE
...
static int64_t rd_pick_intra_sbuv_mode(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  PALETTE_MODE_INFO palette_mode_info;
  uint8_t *best_palette_color_map = NULL;
  PALETTE_MODE_INFO *pmi = &xd->mi[0]->mbmi.palette_mode_info;
#endif  // CONFIG_PALETTE
  ...
  for (mode = DC_PRED; mode <= TM_PRED; ++mode) {
    ...
    if (cpi->common.allow_screen_content_tools &&
      ...
      this_rate +=
          vp10_cost_bit(default_palette_uv_mode_prob[pmi->palette_size[1] > 0],
                        0);
#endif  // CONFIG_PALETTE
    this_rd = RDCOST(x->rdmult, x->rddiv, this_rate, this_distortion);
  ...
  if (xd->mi[0]->mbmi.sb_type >= BLOCK_8X8 &&
    ...
    if (colors > 1 && colors <= 64) {
      ...
      int *indices = vpx_calloc(rows * cols, sizeof(*indices));
      double *data = vpx_calloc(2 * rows * cols, sizeof(*data));
      ...
      for (n = colors > PALETTE_MAX_SIZE ? PALETTE_MAX_SIZE : colors;
        ...
        r = vp10_k_means(data, centroids, indices, rows * cols, n, 2, max_itr);
        ...
        for (r = 0; r < rows; ++r)
          for (c = 0; c < cols; ++c)
            xd->plane[1].color_index_map[r * cols + c] = indices[r * cols + c];
        ...
        this_rate = this_rate_tokenonly +
            ...
            vp10_cost_bit(default_palette_uv_mode_prob
                          [pmi->palette_size[1] > 0], 1);
        this_rate +=  vp10_ceil_log2(n) * vp10_cost_bit(128, 0);
        for (i = 0; i < rows; ++i) {
          for (j = (i == 0 ? 1 : 0); j < cols; ++j) {
...
#if CONFIG_PALETTE
      if (cpi->common.allow_screen_content_tools && bsize >= BLOCK_8X8)
        rate2 += vp10_cost_bit(default_palette_y_mode_prob[bsize - BLOCK_8X8]
                                                           [palette_ctx], 0);
#endif  // CONFIG_PALETTE
  ...
  *mbmi = best_mbmode;
  ...
  if (bsize >= BLOCK_8X8 && cpi->common.allow_screen_content_tools &&
    ...
    if (cpi->common.use_highbitdepth)
      ...
      colors = vp10_count_colors(src, src_stride, rows, cols);
    if (colors > 1 && colors <= 64) {
      int n, r, c, i, j, k, max_itr = 50;
      int64_t this_rd, this_rd_y, best_rd_y = INT64_MAX;
      int rate2 = 0, rate_y = 0, rate_uv = 0;
      int total_rate_y, best_total_rate_y = INT_MAX;
      int best_token_rate_y = INT_MAX;
      int64_t distortion2 = 0, distortion_y = 0, distortion_uv = 0;
      int64_t best_distortion_y = INT64_MAX;
      int skippable = 0, skip_y = 0;
      int color_ctx, color_idx = 0;
      int color_order[PALETTE_MAX_SIZE];
      int *indices = vpx_calloc(rows * cols, sizeof(*indices));
      uint8_t *color_map;
      ...
      for (n = colors > PALETTE_MAX_SIZE ? PALETTE_MAX_SIZE : colors;
        ...
        total_rate_y = rate_y + cpi->mbmode_cost[mbmi->mode] +
            cpi->common.bit_depth * k * vp10_cost_bit(128, 0) +
            cpi->palette_y_size_cost[bsize - BLOCK_8X8][k - 2];
        total_rate_y +=
            vp10_cost_bit(default_palette_y_mode_prob[bsize - BLOCK_8X8]
                                                      [palette_ctx], 1);
        color_map = xd->plane[0].color_index_map;
        total_rate_y +=  vp10_ceil_log2(k) * vp10_cost_bit(128, 0);
        for (i = 0; i < rows; ++i) {
          for (j = (i == 0 ? 1 : 0); j < cols; ++j) {
            color_ctx = vp10_get_palette_color_context(color_map, cols, i, j,
                                                       k, color_order);
