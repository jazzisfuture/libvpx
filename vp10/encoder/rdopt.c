...
static int do_tx_type_search(TX_TYPE tx_type,
                             unsigned int prune) {
// TODO(sarahparker) Implement for base tx types
#if CONFIG_EXT_TX
  ...
  return !(((prune >> vtx_tab[tx_type]) % 2) |
         ((prune >> (htx_tab[tx_type] + TX_TYPES_1D)) % 2));
#else
...
static void choose_largest_tx_size(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  if (is_inter && cpi->sf.tx_type_search)
