...
static void choose_tx_size_from_rd(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  for (n = start_tx; n >= end_tx; n--) {
    ...
    if (is_inter_block(mbmi) && bs >= BLOCK_8X8 && !xd->lossless) {
      int r_tx_type[EXT_TX_TYPES][2], s_tx_type[EXT_TX_TYPES];
      int64_t d_tx_type[EXT_TX_TYPES], sse_tx_type[EXT_TX_TYPES];
      int64_t rd_tx_type[EXT_TX_TYPES][2] = {
          {INT64_MAX, INT64_MAX},
          {INT64_MAX, INT64_MAX},
          {INT64_MAX, INT64_MAX},
          {INT64_MAX, INT64_MAX}};
      ...
      for (i = NORM; i < EXT_TX_TYPES; i++) {
        if (mbmi->ext_txfrm >= GET_EXT_TX_TYPES(n)) {
          ...
          mbmi->ext_txfrm = i;
          txfm_rd_in_plane(x, &r_tx_type[i][0], &d_tx_type[i], &s_tx_type[i],
                           &sse_tx_type[i], ref_best_rd, 0, bs, n,
                           cpi->sf.use_fast_coef_costing);
          r_tx_type[i][0] += cpi->ext_tx_costs[n][i];

          r_tx_type[i][1] = r_tx_type[i][0];
          if (r_tx_type[i][0] < INT_MAX) {
            r_tx_type[i][1] += r_tx_size;
          }
          if (d_tx_type[i] == INT64_MAX || r_tx_type[i][0] == INT_MAX) {
            rd_tx_type[i][0] = rd_tx_type[i][1] = INT64_MAX;
          } else if (s_tx_type[i]) {
            rd_tx_type[i][0] = rd_tx_type[i][1] =
                RDCOST(x->rdmult, x->rddiv, s1, sse_tx_type[i]);
            r_tx_type[i][1] -= r_tx_size;
          } else {
            rd_tx_type[i][0] =
                RDCOST(x->rdmult, x->rddiv, r_tx_type[i][0] + s0, d_tx_type[i]);
            rd_tx_type[i][1] =
                RDCOST(x->rdmult, x->rddiv, r_tx_type[i][1] + s0, d_tx_type[i]);
          }

          if (rd_tx_type[i][1] < this_best_rd) {
            best_tx_type[n] = i;
            this_best_rd = rd_tx_type[i][1];
          }
        }
      }
...
#if CONFIG_EXT_TX
  mbmi->ext_txfrm = best_tx_type[mbmi->tx_size];
#endif  // CONFIG_EXT_TX
