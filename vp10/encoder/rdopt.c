...
void vp10_rd_pick_inter_mode_sb(VP10_COMP *cpi,
  ...
  for (midx = 0; midx < MAX_MODES; ++midx) {
    ...
    if ((ref_frame != INTRA_FRAME || this_mode == DC_PRED ||
      ...
      if (best_rd < (mode_threshold[mode_index] >> 1)) {
        //printf("mode %d, %6lld vs %6lld \n",
          //     mode_index, best_rd, mode_threshold[mode_index]);
        continue;
      }
    }
#else
...
#if 0
      if (ref_frame == INTRA_FRAME && best_rd < mode_threshold[mode_index]) {
        printf("mode %d, %6lld vs %6lld \n",
               mode_index, best_rd, mode_threshold[mode_index]);
      }
#endif
