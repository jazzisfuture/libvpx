...
static INTERP_FILTER predict_interp_filter(const VP10_COMP *cpi,
  ...
  if (cm->interp_filter != BILINEAR) {
    ...
    if (!vp10_is_interp_needed(xd) && cm->interp_filter == SWITCHABLE) {
      best_filter = vp10_predict_full_pixel_interp_filter(cm, xd);
    }
#endif
  }
  return best_filter;
...
#if CONFIG_EXT_INTERP
          if (!vp10_is_interp_needed(xd) && cm->interp_filter == SWITCHABLE &&
              mbmi->interp_filter != vp10_predict_full_pixel_interp_filter(cm, xd))  // invalid config
            continue;
#endif  // CONFIG_EXT_INTERP
          if (tmp_rd == INT64_MAX)
            continue;
...
#if CONFIG_EXT_INTERP
        if (!vp10_is_interp_needed(xd) && cm->interp_filter == SWITCHABLE) {
          mbmi->interp_filter = vp10_predict_full_pixel_interp_filter(cm, xd);
        }
#endif  // CONFIG_EXT_INTERP
        if (tmp_rd == INT64_MAX)
          continue;
