...
static void choose_tx_size_from_rd(VP10_COMP *cpi, MACROBLOCK *x,
  ...
  int s[TX_SIZES];
  int r[TX_SIZES][2] = {{INT_MAX, INT_MAX},
                        {INT_MAX, INT_MAX},
                        {INT_MAX, INT_MAX},
                        {INT_MAX, INT_MAX}};
  ...
  if (cm->tx_mode == TX_MODE_SELECT) {
    ...
    end_tx = 0;
#if CONFIG_EXT_TX
    if (mbmi->tx_type[0]!= DCT_DCT && start_tx > TX_16X16)
      start_tx = TX_16X16;
#endif  // CONFIG_EXT_TX
  } else {
...
#if CONFIG_EXT_TX
  xd->mi[0]->mbmi.tx_type[0] = DCT_DCT;
#endif  // CONFIG_EXT_TX
  ...
  for (midx = 0; midx < MAX_MODES; ++midx) {
    ...
    if (ref_frame == INTRA_FRAME) {
...
#if CONFIG_EXT_TX
      int this_rate_y = INT_MAX, this_rate_y_token = INT_MAX;
      ...
      for (tx_type = DCT_DCT; tx_type < TX_TYPES - 1; tx_type++) {
        ...
        super_block_yrd(cpi, x, &this_rate_y_token, &this_distortion_y,
                        &this_skippable, NULL, bsize, best_rd);
        if (this_rate_y_token == INT_MAX)
          continue;

        this_rate_y = this_rate_y_token;
...
#if CONFIG_EXT_TX
      if (mbmi->tx_size < TX_32X32)
        rate2 += cpi->tx_type_cost[mbmi->tx_size][mbmi->mode][mbmi->tx_type[0]];
#endif  // CONFIG_EXT_TX
