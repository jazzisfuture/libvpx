  ...
  switch (cpi->sf.tx_type_search) {
    ...
    case PRUNE_TWO :
      prune_two_for_sby(cpi, bsize, x, xd,
                        prune_htx_type, prune_vtx_type);
      if (prune_htx_type[0] != UNKNOWN_TX_1D && htx_tab[tx_type] == prune_htx_type[0])
        return 0;
      if (prune_vtx_type[0] != UNKNOWN_TX_1D && vtx_tab[tx_type] == prune_vtx_type[0])
        return 0;
      if (prune_htx_type[1] != UNKNOWN_TX_1D && htx_tab[tx_type] == prune_htx_type[1])
        return 0;
      if (prune_vtx_type[1] != UNKNOWN_TX_1D && vtx_tab[tx_type] == prune_vtx_type[1])
        return 0;
      return 1;
