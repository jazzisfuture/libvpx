    ...
    if (ref_frame == INTRA_FRAME) {
      ...
      if ((mbmi->mode == NEARMV &&
        ...
        if (mbmi->mode == NEARMV)
          ...
          rate2 += cpi->drl_mode_cost0[drl_ctx][0]; // vp10_cost_bit(128, 0);
        ...
        for (ref_idx = 0; ref_idx < ref_set; ++ref_idx) {
          ...
          if (this_mode == NEWMV) {
            tmp_rate += cpi->drl_mode_cost0[drl_ctx][1]; // vp10_cost_bit(128, 1);

            if (mbmi_ext->ref_mv_count[ref_frame_type] > 2) {
              uint8_t this_drl_ctx =
                  vp10_drl_ctx(mbmi_ext->ref_mv_stack[ref_frame_type], 1);
              tmp_rate += cpi->drl_mode_cost0[this_drl_ctx][ref_idx]; // vp10_cost_bit(128, ref_idx != 0);
            }

//            if (ref_idx > 0 && mbmi_ext->ref_mv_count[ref_frame_type] > 3)
//              tmp_rate += vp10_cost_bit(128, ref_idx != 1);
          }
