...
static void print_counts(vp10_coeff_count coef_counts[TX_SIZES][PLANE_TYPES],
                         VP10_COMP *cpi) {
  int i, j, k, l, m, tx_size;
  int count = 0;
        ...
        for (k = 0; k < COEF_BANDS; ++k)
          for (l = 0; l < BAND_COEFF_CONTEXTS(k); ++l)
            for (m = 0; m < ENTROPY_TOKENS; ++m) {
              printf("%4d ", coef_counts[tx_size][i][j][k][l][m]);
              if (++count % 30 == 0)
                printf("\n");
              if (count >= 120)
                return;
...
static void print_eob_counts(unsigned int eob_counts[TX_SIZES][PLANE_TYPES]
  ...
  for (tx_size = TX_4X4; tx_size <= max_tx_size; ++tx_size)
    for (i = 0; i < PLANE_TYPES; ++i)
      for (j = 0; j < REF_TYPES; ++j)
        for (k = 0; k < COEF_BANDS; ++k)
          for (l = 0; l < BAND_COEFF_CONTEXTS(k); ++l) {
            printf("%4d ", eob_counts[tx_size][i][j][k][l]);
            if (++count % 30 == 0)
              printf("\n");
            if (count >= 120)
              return;
...
static void get_coef_counts_diff(VP10_COMP *cpi, int index,
  ...
  const int max_idx = cpi->common.coef_probs_buf_idx;
  const TX_MODE tx_mode = cpi->common.tx_mode;
  const TX_SIZE max_tx_size = tx_mode_to_biggest_tx_size[tx_mode];
          ...
          for (l = 0; l < BAND_COEFF_CONTEXTS(k); ++l) {
            if (index == max_idx) {
              val = cpi->common.counts.eob_branch[tx_size][i][j][k][l] -
                  cpi->td.eob_counts_buf[max_idx][tx_size][i][j][k][l];
            } else {
              val = cpi->td.eob_counts_buf[index + 1][tx_size][i][j][k][l] -
                  cpi->td.eob_counts_buf[index][tx_size][i][j][k][l];
            }
            assert(val >= 0);
            if (val < 0) printf("\n error in get_coef_counts_diff (eob)\n");
            eob_counts[tx_size][i][j][k][l] = val;

            for (m = 0; m < ENTROPY_TOKENS; ++m) {
              if (index == max_idx) {
                val = cpi->td.rd_counts.coef_counts[tx_size][i][j][k][l][m] -
                    cpi->td.coef_counts_buf[max_idx][tx_size][i][j][k][l][m];
              } else {
                val =
                    cpi->td.coef_counts_buf[index + 1][tx_size][i][j][k][l][m] -
                    cpi->td.coef_counts_buf[index][tx_size][i][j][k][l][m];
              }
              assert(val >= 0);
              if (val < 0) printf("\n error in get_coef_counts_diff\n");
              coef_counts[tx_size][i][j][k][l][m] = val;
            }
          }
}

static void update_coef_probs_common2(vpx_writer* const bc, VP10_COMP *cpi,
                                     TX_SIZE tx_size,
                                     vp10_coeff_stats
            ...
            for (l = 0; l < BAND_COEFF_CONTEXTS(k); ++l) {
              for (t = 0; t < ENTROPY_NODES; ++t) {
                for (idx = 0; idx <= max_idx; ++idx) {
                  memcpy(this_branch_ct[t][idx], branch_ct[idx][i][j][k][l][t],
              ...
              for (t = 0; t < entropy_nodes_update; ++t) {
                ...
                if (t == PIVOT_NODE)
                  s = vp10_prob_diff_update_savings_search_model2(
                      this_branch_ct, old_coef_probs[i][j][k][l], &newp, upd,
                      stepsize, max_idx);
                else
                  s = vp10_prob_diff_update_savings_search2(this_branch_ct[t],
                                                            oldp, &newp, upd,
                                                            max_idx);
                if (s > 0 && newp != oldp)
                  u = 1;
...
static void update_coef_probs2(VP10_COMP *cpi, vpx_writer* w) {
  const TX_MODE tx_mode = cpi->common.tx_mode;
  const TX_SIZE max_tx_size = tx_mode_to_biggest_tx_size[tx_mode];
  TX_SIZE tx_size;
  int i;
  ...
  for (tx_size = TX_4X4; tx_size <= max_tx_size; ++tx_size) {
    if (cpi->td.counts->tx.tx_totals[tx_size] <= 20 ||
      ...
      unsigned int eob_counts[COEF_PROBS_BUFS][TX_SIZES][PLANE_TYPES][REF_TYPES]
                             [COEF_BANDS][COEFF_CONTEXTS];
      vp10_coeff_stats branch_ct[COEF_PROBS_BUFS][TX_SIZES][PLANE_TYPES];

      for (i = 0; i <= cpi->common.coef_probs_buf_idx; ++i) {
      ...
      update_coef_probs_common2(w, cpi, tx_size, branch_ct, frame_coef_probs);
    }
  }
#endif
...
#if 0
  {
    for (i = 0; i <= cpi->common.coef_probs_buf_idx; ++i) {
      printf("buf %d\n", i);
      print_eob_counts(cpi->td.eob_counts_buf[i], cpi);
      printf("\n");
    }
    ...
    for (i = 0; i <= cpi->common.coef_probs_buf_idx; ++i) {
      //break;
      printf("diff %d\n", i);
      print_eob_counts(eob_counts[i], cpi);
      printf("\n");
    }
  }
#endif
...
#if 0
  {
    for (i = 0; i <= cpi->common.coef_probs_buf_idx; ++i) {
      printf("buf %d\n", i);
      print_counts(cpi->td.coef_counts_buf[i], cpi);
      printf("\n");
    }
    ...
    for (i = 0; i <= cpi->common.coef_probs_buf_idx; ++i) {
      break;
      printf("diff %d\n", i);
      print_counts(coef_counts[i], cpi);
      printf("\n");
    }
  }
#endif
...
static void update_coef_probs(VP10_COMP *cpi, vpx_writer* w) {
...
#if CONFIG_SUBFRAME_STATS
  //update_coef_probs2(cpi, w);
  if (frame_is_intra_only(&cpi->common) || 1) {
    vp10_copy(cpi->common.fc->coef_probs, cpi->enc_starting_coef_probs);
  }
#endif  // CONFIG_SUBFRAME_STATS
...
#if 0
  {
    ...
    int i;
    for (i = 0; i < cm->coef_probs_buf_idx; ++i) {
  ...
  if (frame_is_intra_only(&cpi->common) || 1) {
    ...
    for (i = 1; i <= cpi->common.coef_probs_buf_idx; ++i) {
