  ...
  if (cm->use_subframe_update &&
...
#if 0
    {
      ...
      for (i1 = 0; i1 < TX_SIZES; ++i1) {
        for (i2 = 0; i2 < PLANE_TYPES; ++i2) {
          for (i3 = 0; i3 < REF_TYPES; ++i3) {
            for (i4 = 0; i4 < COEF_BANDS; ++i4) {
              for (i5 = 0; i5 < COEFF_CONTEXTS; ++i5) {
                for (i5 = 0; i5 < COEFF_CONTEXTS; ++i5) {
                  for (i6 = 0; i6 < ENTROPY_TOKENS; ++i6) {
                    //if (++idx % 10)
                    printf("%3d ", coef_counts[idx][i1][i2][i3][i4][i5][i6]);
                    num++;
                    if (num % 32 == 0) printf("\n");
                  }
                }
