...
#if 0
    // zoeliu: for debug
    if (cm->current_video_frame == 18 && cm->show_frame == 1) {
      ...
      printf("Before pack_inter_mode_mvs(): "
             ...
             mode, segment_id, bsize, b_mode,
             mv_x, mv_y, m->mbmi.ref_frame[0], m->mbmi.ref_frame[1]);
    }
#endif
...
#if CONFIG_BIDIR_PRED
  // TODO(zoeliu): To re-evaluable the following first flag
  if (cm->show_existing_frame) {
