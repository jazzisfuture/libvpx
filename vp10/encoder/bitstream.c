...
static void pack_palette_tokens(vpx_writer *w, TOKENEXTRA **tp,
  ...
  for (i = 0; i < rows * cols -1; ++i) {
    vp10_write_token(w, vp10_palette_color_tree, p->context_tree,
                     &palette_color_encodings[p->token]);
    ++p;
  }
  ...
  if (n > 0) {
    ...
    for (i = 0; i < n; ++i)
      vpx_write_literal(w, pmi->palette_colors[i],
                        cm->bit_depth);
    vpx_write_literal(w, pmi->palette_first_color[0], vp10_ceil_log2(n));
  }
}
