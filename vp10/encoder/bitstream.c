...
static void encode_restoration(VP10_COMMON *cm,
  ...
  if (rst->restoration_type != RESTORE_NONE) {
    if (rst->restoration_type == RESTORE_BILATERAL) {
      ...
      vpx_wb_write_literal(wb, rst->vfilter[0] -  3, 2);
      vpx_wb_write_literal(wb, rst->vfilter[1] + 23, 4);
      vpx_wb_write_literal(wb, rst->vfilter[2] -  5, 5);
      vpx_wb_write_literal(wb, rst->hfilter[0] -  3, 2);
      vpx_wb_write_literal(wb, rst->hfilter[1] + 23, 4);
      vpx_wb_write_literal(wb, rst->hfilter[2] -  5, 5);
    }
  }
}
