...
#if CONFIG_EXT_TX
static struct vp10_token ext_tx_intra_encodings[TX_TYPES];
#endif  // CONFIG_EXT_TX

void vp10_encode_token_init() {
#if CONFIG_EXT_TX
  vp10_tokens_from_tree(ext_tx_intra_encodings, vp10_ext_tx_intra_tree);
#endif  // CONFIG_EXT_TX
}
...
static void update_ext_tx_probs(VP10_COMMON *cm, vpx_writer *w) {
  ...
  for (i = TX_4X4; i < EXT_TX_SIZES; ++i) {
    for (j = 0; j < TX_TYPES; ++j)
      savings += prob_diff_update_savings(
          vp10_ext_tx_intra_tree, cm->fc->intra_ext_tx_prob[i][j],
  ...
  if (do_update) {
    for (i = TX_4X4; i < EXT_TX_SIZES; ++i) {
      for (j = 0; j < TX_TYPES; ++j)
        prob_diff_update(vp10_ext_tx_intra_tree,
...
#if CONFIG_EXT_TX
  if (mbmi->tx_size < TX_32X32 && !is_inter &&
    ...
    vp10_write_token(
        ...
        &ext_tx_intra_encodings[mbmi->tx_type]);
  }
#endif  // CONFIG_EXT_TX
}
