...
static void write_switchable_interp_filter(VP10_COMP *cpi,
  ...
  if (cm->interp_filter == SWITCHABLE) {
    ...
    /* This assert was wrong here since the frame level filter could be
     * forced in the fix_interp_filter() function. So removing this. */
    /*
    if (mbmi->interp_filter != cm->interp_filter)
