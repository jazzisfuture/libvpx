  ...
  switch (gmtype) {
    ...
    case GLOBAL_AFFINE:
      vp10_write_primitive_symmetric(
          w, (params->motion_params.wmmat[2].as_mv.row *
          GM_ALPHA_ENCODE_FACTOR) - (1 << GM_ALPHA_PREC_BITS),
          GM_ABS_ALPHA_BITS);
      vp10_write_primitive_symmetric(
          w, params->motion_params.wmmat[2].as_mv.col * GM_ALPHA_ENCODE_FACTOR,
          GM_ABS_ALPHA_BITS);
    // fallthrough intended
    case GLOBAL_ROTZOOM:
      vp10_write_primitive_symmetric(
          w, params->motion_params.wmmat[1].as_mv.row * GM_ALPHA_ENCODE_FACTOR,
          GM_ABS_ALPHA_BITS);
      vp10_write_primitive_symmetric(
          w, (params->motion_params.wmmat[1].as_mv.col *
          GM_ALPHA_ENCODE_FACTOR) - (1 << GM_ALPHA_PREC_BITS),
          GM_ABS_ALPHA_BITS);
    // fallthrough intended
    case GLOBAL_TRANSLATION:
      vp10_write_primitive_symmetric(
          w, params->motion_params.wmmat[0].as_mv.row * GM_TRANS_ENCODE_FACTOR,
          GM_ABS_TRANS_BITS);
      vp10_write_primitive_symmetric(
          w, params->motion_params.wmmat[0].as_mv.col * GM_TRANS_ENCODE_FACTOR,
          GM_ABS_TRANS_BITS);
      break;
    default: assert(0);
  }
