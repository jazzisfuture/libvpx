...
void vp10_highbd_fwd_txfm_4x4(const int16_t *src_diff, tran_low_t *coeff,
                              int diff_stride, TX_TYPE tx_type, int lossless, const int bd) {
  if (lossless) {
    assert(tx_type == DCT_DCT);
  ...
  switch (tx_type) {
    case DCT_DCT:
      if(bd == 10) {
        vp10_fwd_txfm2d_4x4(src_diff, coeff, diff_stride, &fwd_txfm_2d_cfg_dct_dct_4, 10);
        break;
      }
    case ADST_DCT:
    case DCT_ADST:
...
static void highbd_fwd_txfm_8x8(const int16_t *src_diff, tran_low_t *coeff,
  ...
  switch (tx_type) {
    case DCT_DCT:
      if(bd == 10) {
        vp10_fwd_txfm2d_8x8(src_diff, coeff, diff_stride, &fwd_txfm_2d_cfg_dct_dct_8, 10);
        break;
      }
    case ADST_DCT:
    case DCT_ADST:
...
static void highbd_fwd_txfm_16x16(const int16_t *src_diff, tran_low_t *coeff,
  ...
  switch (tx_type) {
    case DCT_DCT:
      if(bd == 10) {
        vp10_fwd_txfm2d_16x16(src_diff, coeff, diff_stride, &fwd_txfm_2d_cfg_dct_dct_16, 10);
        break;
      }
    case ADST_DCT:
    case DCT_ADST:
...
static void highbd_fwd_txfm_32x32(int rd_transform, const int16_t *src_diff,
                                  ...
                                  TX_TYPE tx_type, FWD_TXFM_OPT fwd_txfm_opt, const int bd) {
  (void)bd;
  switch (tx_type) {
    case DCT_DCT:
      if (fwd_txfm_opt == FWD_TXFM_OPT_NORMAL)
...
void highbd_fwd_txfm(const int16_t *src_diff, tran_low_t *coeff,
  ...
  switch (tx_size) {
    ...
    case TX_16X16:
      ...
      break;
    case TX_8X8:
      highbd_fwd_txfm_8x8(src_diff, coeff, diff_stride, tx_type, fwd_txfm_opt, bd);
      break;
    case TX_4X4:
      vp10_highbd_fwd_txfm_4x4(src_diff, coeff, diff_stride, tx_type, lossless, bd);
      break;
    default:
      assert(0);
      break;
