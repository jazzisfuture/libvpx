...
typedef struct macroblock_plane {
  DECLARE_ALIGNED(16, int16_t, src_diff[MAX_CU_PIXELS]);
  tran_low_t *qcoeff;
  tran_low_t *coeff;
  uint16_t *eobs;
  struct buf_2d src;
...
typedef struct {
  uint8_t best_palette_color_map[MAX_CU_PIXELS];
  double kmeans_data_buf[2 * MAX_CU_PIXELS];
  uint8_t kmeans_indices_buf[MAX_CU_PIXELS];
  uint8_t kmeans_pre_indices_buf[MAX_CU_PIXELS];
} PALETTE_BUFFER;
...
struct macroblock {
  ...
  uint8_t skip_txfm[MAX_MB_PLANE][MAX_TX_BLOCKS_IN_MAX_CU];
  #define SKIP_TXFM_NONE 0
  #define SKIP_TXFM_AC_DC 1
  #define SKIP_TXFM_AC_ONLY 2

  int64_t bsse[MAX_MB_PLANE][MAX_TX_BLOCKS_IN_MAX_CU];
