...
#if CONFIG_VAR_TX
void tokenize_tx(VP10_COMP *cpi, ThreadData *td, TOKENEXTRA **t,
  ...
  if (tx_size == plane_tx_size) {
    ...
    for (i = 0; i < 4; ++i) {
      const int offsetr = mi_row + ((i >> 1) << bsl);
      const int offsetc = mi_col + ((i & 0x01) << bsl);
  ...
  for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
    ...
    for (idy = 0; idy < mi_height; idy += bh) {
      for (idx = 0; idx < mi_width; idx += bh) {
        tokenize_tx(cpi, td, t, dry_run, max_tx_size,
                    mi_row + (idy >> 1), mi_col + (idx >> 1),
                    block, plane, &arg);
        block += step;
      }
    }
