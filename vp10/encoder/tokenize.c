...
static INLINE int get_tx_eob(const struct segmentation *seg, int segment_id,
                             TX_SIZE tx_size) {
  const int eob_max = num_4x4_blocks_tx_size_lookup[tx_size] << 4;
...
static void tokenize_b(int plane, int block, int blk_row, int blk_col,
  ...
  unsigned int (*const counts)[COEFF_CONTEXTS][ENTROPY_TOKENS] =
      td->rd_counts.coef_counts[get_tx_size_ctx(tx_size)][type][ref];
#if CONFIG_ENTROPY
  vpx_prob (*coef_probs)[COEFF_CONTEXTS][UNCONSTRAINED_NODES] =
      cpi->subframe_stats.coef_probs_buf[cpi->common.coef_probs_update_idx]
                                        [get_tx_size_ctx(tx_size)][type][ref];
#else
  vpx_prob (*const coef_probs)[COEFF_CONTEXTS][UNCONSTRAINED_NODES] =
      cpi->common.fc->coef_probs[get_tx_size_ctx(tx_size)][type][ref];
#endif  // CONFIG_ENTROPY
#if CONFIG_ANS
  rans_dec_lut (*const coef_cdfs)[COEFF_CONTEXTS] =
      cpi->common.fc->coef_cdfs[get_tx_size_ctx(tx_size)][type][ref];
#endif  // CONFIG_ANS
  unsigned int (*const eob_branch)[COEFF_CONTEXTS] =
      td->counts->eob_branch[get_tx_size_ctx(tx_size)][type][ref];
