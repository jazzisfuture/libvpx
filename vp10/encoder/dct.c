...
#if CONFIG_EXT_TX
void vp10_f1dt4x4_c(const int16_t *input, tran_low_t *output,
                    int stride, int tx_type) {
  tran_low_t out[4 * 4];
  int i, j;
  tran_low_t temp_in[4], temp_out[4];
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      ...
      for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j)
          temp_in[j] = out[j + i * 4];
        (i < TX2ROWS_1D ? FHT_4[tx_type] : ht).rows(temp_in, temp_out);
        for (j = 0; j < 4; ++j)
          output[j + i * 4] = (temp_out[j] + 1) >> 2;
      }
      return;
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 4; ++i) {
        ...
        ht.rows(temp_in, temp_out);
        for (j = 0; j < 4; ++j)
          out[i * 4 + j] = temp_out[j];
      }
      // Columns
      for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j)
          temp_in[j] = out[i + j * 4];
        (i < TX2ROWS_1D ? FHT_4[tx_type] : ht).cols(temp_in, temp_out);
        for (j = 0; j < 4; ++j)
          output[i + j * 4] = (temp_out[j] + 1) >> 2;
      }
      return;
    default:
      assert(0);
...
#if CONFIG_EXT_TX
void vp10_f1dt8x8_c(const int16_t *input, tran_low_t *output,
                    int stride, int tx_type) {
  tran_low_t out[64];
  int i, j;
  tran_low_t temp_in[8], temp_out[8];
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      ...
      for (i = 0; i < 8; ++i) {
        for (j = 0; j < 8; ++j)
          temp_in[j] = out[j + i * 8];
        (i < TX2ROWS_1D ? FHT_8[tx_type] : ht).rows(temp_in, temp_out);
        for (j = 0; j < 8; ++j)
          output[j + i * 8] = (temp_out[j] + (temp_out[j] < 0)) >> 1;
      }
      return;
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 8; ++i) {
        ...
        ht.rows(temp_in, temp_out);
        for (j = 0; j < 8; ++j)
          out[i * 8 + j] = temp_out[j];
      }
      // Columns
      for (i = 0; i < 8; ++i) {
        for (j = 0; j < 8; ++j)
          temp_in[j] = out[i + j * 8];
        (i < TX2ROWS_1D ? FHT_8[tx_type] : ht).cols(temp_in, temp_out);
        for (j = 0; j < 8; ++j)
          output[i + j * 8] = (temp_out[j] + (temp_out[j] < 0)) >> 1;
      }
      return;
    default:
      assert(0);
...
#if CONFIG_EXT_TX
void vp10_f1dt16x16_c(const int16_t *input, tran_low_t *output,
                      int stride, int tx_type) {
  tran_low_t out[256];
  int i, j;
  tran_low_t temp_in[16], temp_out[16];
  ...
  switch(tx_type) {
    ...
    case V_FLIPADST:
      ...
      for (i = 0; i < 16; ++i) {
        for (j = 0; j < 16; ++j)
          temp_in[j] = out[j + i * 16];
        (i < TX2ROWS_1D ? FHT_16[DCT_DCT] : ht).rows(temp_in, temp_out);
        for (j = 0; j < 16; ++j)
          output[j + i * 16] = temp_out[j];
      }
      return;
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 16; ++i) {
        ...
        ht.rows(temp_in, temp_out);
        for (j = 0; j < 16; ++j)
          out[i * 16 + j] = (temp_out[j] + 1 + (temp_out[j] < 0)) >> 2;
      }
      for (i = 0; i < 16; ++i) {
        for (j = 0; j < 16; ++j)
          temp_in[j] = out[i + j * 16];
        (i < TX2ROWS_1D ? FHT_16[DCT_DCT] : ht).cols(temp_in, temp_out);
        for (j = 0; j < 16; ++j)
          output[i + j * 16] = temp_out[j];
      }
      return;
    default:
      assert(0);
...
void vp10_highbd_f1dt4x4_c(const int16_t *input, tran_low_t *output,
                           int stride, int tx_type) {
  vp10_f1dt4x4_c(input, output, stride, tx_type);
}

void vp10_highbd_f1dt8x8_c(const int16_t *input, tran_low_t *output,
                           int stride, int tx_type) {
  vp10_f1dt8x8_c(input, output, stride, tx_type);
}

void vp10_highbd_f1dt16x16_c(const int16_t *input, tran_low_t *output,
                            int stride, int tx_type) {
  vp10_f1dt16x16_c(input, output, stride, tx_type);
}
...
void vp10_f1dt32x32_c(const int16_t *input, tran_low_t *output,
                      int stride, int tx_type) {
  tran_low_t out[1024];
  int i, j;
  tran_low_t temp_in[32], temp_out[32];
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      ...
      for (i = 0; i < 32; ++i) {
        for (j = 0; j < 32; ++j)
          temp_in[j] = out[j + i * 32];
        (i < TX2ROWS_1D ? fdct32 : ht.rows)(temp_in, temp_out);
        for (j = 0; j < 32; ++j)
          output[j + i * 32] =
              (tran_low_t)((temp_out[j] + 1 + (temp_out[j] < 0)) >> 2);
      }
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 32; ++i) {
        ...
        ht.rows(temp_in, temp_out);
        for (j = 0; j < 32; ++j)
          out[i * 32 + j] = (temp_out[j] + 1 + (temp_out[j] > 0)) >> 2;
      }
      // Columns
      for (i = 0; i < 32; ++i) {
        for (j = 0; j < 32; ++j)
          temp_in[j] = out[i + j * 32];
        (i < TX2ROWS_1D ? fdct32 : ht.cols)(temp_in, temp_out);
        for (j = 0; j < 32; ++j)
          output[i + j * 32] =
              (tran_low_t)((temp_out[j] + 1 + (temp_out[j] < 0)) >> 2);
      }
...
void vp10_highbd_f1dt32x32_c(const int16_t *input, tran_low_t *output,
                             int stride, int tx_type) {
  vp10_f1dt32x32_c(input, output, stride, tx_type);
}
#endif  // CONFIG_VP9_HIGHBITDEPTH
#endif  // CONFIG_EXT_TX
