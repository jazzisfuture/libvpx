...
#if CONFIG_EXT_TX
static void fhaar4(const tran_low_t *input, tran_low_t *output) {
  int i;
  for (i = 0; i < 2; ++i) {
    output[i] = input[2 * i] + input[2 * i + 1];
    output[i + 2] = input[2 * i] - input[2 * i + 1];
  }
}

static void fhaar8(const tran_low_t *input, tran_low_t *output) {
  int i;
  for (i = 0; i < 4; ++i) {
    output[i] = (tran_low_t)fdct_round_shift(
        (input[2 * i] + input[2 * i + 1]) * Sqrt2);
    output[i + 4] = (tran_low_t)fdct_round_shift(
        (input[2 * i] - input[2 * i + 1]) * Sqrt2);
  }
}

static void fhaar16(const tran_low_t *input, tran_low_t *output) {
  int i;
  for (i = 0; i < 8; ++i) {
    output[i] = 2 * (input[2 * i] + input[2 * i + 1]);
    output[i + 8] = 2 * (input[2 * i] - input[2 * i + 1]);
  }
}

static void fhaar32(const tran_low_t *input, tran_low_t *output) {
  int i;
  for (i = 0; i < 16; ++i) {
    output[i] = (tran_low_t)fdct_round_shift(
        (input[2 * i] + input[2 * i + 1]) * 2 * Sqrt2);
    output[i + 16] = (tran_low_t)fdct_round_shift(
        (input[2 * i] - input[2 * i + 1]) * 2 * Sqrt2);
  }
}
...
static const transform_2d FHT_4[] = {
  ...
  { fdct4,  fhaar4 },  // V_DCT
  { fhaar4, fdct4  },  // H_DCT
  { fadst4, fhaar4 },  // V_ADST
  { fhaar4, fadst4 },  // H_ADST
  { fadst4, fhaar4 },  // V_FLIPADST
  { fhaar4, fadst4 },  // H_FLIPADST
#endif  // CONFIG_EXT_TX
};
...
static const transform_2d FHT_8[] = {
  ...
  { fdct8,  fhaar8 },  // V_DCT
  { fhaar8, fdct8  },  // H_DCT
  { fadst8, fhaar8 },  // V_ADST
  { fhaar8, fadst8 },  // H_ADST
  { fadst8, fhaar8 },  // V_FLIPADST
  { fhaar8, fadst8 },  // H_FLIPADST
#endif  // CONFIG_EXT_TX
};
...
static const transform_2d FHT_16[] = {
  ...
  { fdct16,  fhaar16 },  // V_DCT
  { fhaar16, fdct16  },  // H_DCT
  { fadst16, fhaar16 },  // V_ADST
  { fhaar16, fadst16 },  // H_ADST
  { fadst16, fhaar16 },  // V_FLIPADST
  { fhaar16, fadst16 },  // H_FLIPADST
#endif  // CONFIG_EXT_TX
};
...
static const transform_2d FHT_32[] = {
  ...
  { fdct32,       fhaar32      },     // V_DCT
  { fhaar32,      fdct32       },     // H_DCT
  { fhalfright32, fhaar32      },     // V_ADST
  { fhaar32,      fhalfright32 },     // H_ADST
  { fhalfright32, fhaar32      },     // V_FLIPADST
  { fhaar32,      fhalfright32 },     // H_FLIPADST
};
#endif  // CONFIG_EXT_TX
