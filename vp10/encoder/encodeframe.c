  ...
  if (abs(model->motion_params.wmmat[0]) < MIN_TRANS_THRESH &&
    ...
    model->motion_params.wmmat[0] =
        clamp(model->motion_params.wmmat[0],
              -(1 << GM_ABS_TRANS_BITS), (1 << GM_ABS_TRANS_BITS));
...
static void convert_rotzoom_to_params(double *H, Global_Motion_Params *model) {
  ...
  model->motion_params.wmmat[2] = (int) floor(model->motion_params.wmmat[2] *
                                              (1 << GM_ALPHA_PREC_BITS) + 0.5);
  model->motion_params.wmmat[3] = (int) floor(model->motion_params.wmmat[3] *
                                              (1 << GM_ALPHA_PREC_BITS) + 0.5);
  model->motion_params.wmmat[2] =
      ROUND_POWER_OF_TWO_SIGNED(model->motion_params.wmmat[2] -
                                (1 << GM_ALPHA_PREC_BITS), GM_ALPHA_STEP_BITS);
  model->motion_params.wmmat[3] =
      ROUND_POWER_OF_TWO_SIGNED(model->motion_params.wmmat[3],
                                GM_ALPHA_STEP_BITS);

  model->motion_params.wmmat[2] = clamp(
      model->motion_params.wmmat[2], -(1 << GM_ABS_ALPHA_BITS),
      (1 << GM_ABS_ALPHA_BITS));
  model->motion_params.wmmat[3] = clamp(
      model->motion_params.wmmat[3], -(1 << GM_ABS_ALPHA_BITS),
      (1 << GM_ABS_ALPHA_BITS));
