        ...
        if (mode == NEWMV) {
          ...
          if (mbmi_ext->ref_mv_count[ref_frame_type] > 1) {
            ...
            if (mbmi->ref_mv_idx == 0)
              ++counts->drl_mode[drl_ctx][0];
            else
              ++counts->drl_mode[drl_ctx][1];
        ...
        if (mode == NEARMV) {
          ...
          if (mbmi_ext->ref_mv_count[ref_frame_type] > 2) {
            uint8_t drl0_ctx =
                vp10_drl_ctx(mbmi_ext->ref_mv_stack[ref_frame_type], 1);
            if (mbmi->ref_mv_idx == 0)
              ++counts->drl_mode[drl0_ctx][0];
            else
              ++counts->drl_mode[drl0_ctx][1];
            ...
            if (mbmi_ext->ref_mv_count[ref_frame_type] > 3 &&
              ...
              if (mbmi->ref_mv_idx == 1)
                ++counts->drl_mode[drl1_ctx][0];
              else
                ++counts->drl_mode[drl1_ctx][1];
            }
          }
        }
#if CONFIG_EXT_INTER
