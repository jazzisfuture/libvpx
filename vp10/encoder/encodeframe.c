...
static void encode_superblock(VP10_COMP *cpi, ThreadData *td, TOKENEXTRA **t,
  ...
  if (output_enabled && cm->current_video_frame == 13 && cm->show_frame &&
      mi_row == 7 && mi_col == 1)
    printf("Debug Encode: bsize %d, txsize %d, mode %d skip %d lossless %d\n",
           bsize, mbmi->tx_size, mbmi->mode, mbmi->skip,
