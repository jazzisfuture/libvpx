  ...
  if (cpi->common.frame_type == INTER_FRAME && cpi->Source) {
    ...
    for (frame = LAST_FRAME; frame <= ALTREF_FRAME; ++frame) {
      ...
      if (ref_buf) {
        ...
        double Htest[9] = {5, 9, 1, 0, 0, 0, 0, 0, 1};
        vp10_integerize_model(Htest, TRANSLATION, &wm);
