  ...
  if (cpi->sf.frame_parameter_update) {
    ...
    if (cm->tx_mode == TX_MODE_SELECT) {
      ...
      for (i = 0; i < TX_SIZE_CONTEXTS; ++i) {
        ...
        count8x8_lp += counts->tx_size[1][i][TX_8X4];
        count8x8_lp += counts->tx_size[2][i][TX_8X4];
        count8x8_lp += counts->tx_size[1][i][TX_4X8];
        count8x8_lp += counts->tx_size[2][i][TX_4X8];
        count8x8_8x8p += counts->tx_size[0][i][TX_8X4];
        count8x8_8x8p += counts->tx_size[0][i][TX_4X8];
