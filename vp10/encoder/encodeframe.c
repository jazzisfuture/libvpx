...
static void encode_sb(VP10_COMP *cpi, ThreadData *td,
  ...
  if (!frame_is_intra_only(cm) &&
    ...
    if (supertx_enabled) {
      ...
      /*
      enc_debug = 0;
      if (output_enabled && cm->current_video_frame == 14 && cm->show_frame &&
          mi_row == 20 && mi_col == 0) {
        MB_MODE_INFO *mbmi = &xd->mi[0]->mbmi;
        enc_debug = 1;
        if (enc_debug)
          printf("EFOUND[%d/%d %d %d] stx 1 sb_type %d mode %d obmc %d compound %d %d\n\n",
                 cm->current_video_frame, cm->show_frame, mi_row, mi_col,
                 mbmi->sb_type, mbmi->mode,
                 mbmi->obmc, has_second_ref(mbmi), mbmi->ref_frame[1] == INTRA_FRAME);
      }
      */
...
static void encode_superblock(VP10_COMP *cpi, ThreadData *td,
  ...
  if (output_enabled && cm->current_video_frame == 14 && cm->show_frame &&
    ...
    enc_debug = 1;
    if (enc_debug)
      printf("EFOUND[%d/%d %d %d] stx 0 sb_type %d obmc %d compound %d %d\n\n",
             cm->current_video_frame, cm->show_frame, mi_row, mi_col, mbmi->sb_type,
             mbmi->obmc, has_second_ref(mbmi), mbmi->ref_frame[1] == INTRA_FRAME);
  }
  */
