...
static int check_intra_sb(VP10_COMP *cpi, const TileInfo *const tile,
                          int mi_row, int mi_col, BLOCK_SIZE bsize,
                          PC_TREE *pc_tree);
static void predict_superblock(VP10_COMP *cpi,
                               int mi_row_pred, int mi_col_pred,
                               BLOCK_SIZE bsize_pred, int b_sub8x8, int block);
static int check_supertx_sb(BLOCK_SIZE bsize, TX_SIZE supertx_size,
                            PC_TREE *pc_tree);
static void predict_sb_complex(VP10_COMP *cpi, const TileInfo *const tile,
                               ...
                               PC_TREE *pc_tree);
static void update_state_sb_supertx(VP10_COMP *cpi, const TileInfo *const tile,
                                    int mi_row, int mi_col,
                                    BLOCK_SIZE bsize,
                                    int output_enabled, PC_TREE *pc_tree);
static void rd_supertx_sb(VP10_COMP *cpi, const TileInfo *const tile,
                          int mi_row, int mi_col, BLOCK_SIZE bsize,
...
#if CONFIG_SUPERTX
static void set_offsets_supertx(VP10_COMP *cpi, const TileInfo *const tile,
                                int mi_row, int mi_col, BLOCK_SIZE bsize) {
...
static void set_offsets_extend(VP10_COMP *cpi, const TileInfo *const tile,
                               int mi_row_pred, int mi_col_pred,
                               int mi_row_ori, int mi_col_ori,
...
#if CONFIG_SUPERTX
static void update_state_supertx(VP10_COMP *cpi, PICK_MODE_CONTEXT *ctx,
                                 int mi_row, int mi_col, BLOCK_SIZE bsize,
                                 int output_enabled) {
  int i, y, x_idx;
  VP10_COMMON *const cm = &cpi->common;
...
static void update_supertx_param(VP10_COMP *cpi, PICK_MODE_CONTEXT *ctx,
#if CONFIG_EXT_TX
                                 int best_tx,
#endif
...
static void update_supertx_param_sb(VP10_COMP *cpi, int mi_row, int mi_col,
                                    BLOCK_SIZE bsize,
#if CONFIG_EXT_TX
                                    int best_tx,
#endif
    ...
    if (supertx_enabled) {
      ...
      if (output_enabled) {
        ...
        if (supertx_size <= TX_16X16 && !xd->mi[0]->mbmi.skip)
          ++td->counts->ext_tx[xd->mi[0]->mbmi.tx_size]
  ...
  if (do_split) {
    ...
    if (bsize == BLOCK_8X8) {
      ...
      if (cm->frame_type != KEY_FRAME && sum_rdc.rdcost < INT64_MAX &&
          !xd->lossless) {
        TX_SIZE supertx_size = max_txsize_lookup[bsize];  // b_width_log2(bsize);
        ...
        if (is_inter_mode(pc_tree->leaf_split[0]->mic.mbmi.mode)) {
          ...
          tmp_dist = 0;
          restore_context(x, mi_row, mi_col, a, l, sa, sl, bsize);
          rd_supertx_sb(cpi, tile_info, mi_row, mi_col, bsize,
                        &tmp_rate, &tmp_dist,
#if CONFIG_EXT_TX
                        &best_tx,
#endif
...
static void predict_b_extend(VP10_COMP *cpi, const TileInfo *const tile,
                             int block,
                             int mi_row_ori, int mi_col_ori,
                             int mi_row_pred, int mi_col_pred,
  ...
  if (output_enabled && !bextend)
    update_stats(&cpi->common, &cpi->td);
}

static void extend_dir(VP10_COMP *cpi, const TileInfo *const tile,
...
static void extend_all(VP10_COMP *cpi, const TileInfo *const tile,
                       int block,
