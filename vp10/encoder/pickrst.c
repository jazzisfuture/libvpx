...
static void quantize_sym_filter(double *f, int *fi) {
  int i;
  fi[RESTORATION_HALFWIN] = RESTORATION_FILT_STEP;
  for (i = 0; i < RESTORATION_HALFWIN; ++i) {
    fi[i] = fi[RESTORATION_WIN - 1 - i] = RINT(f[i] * RESTORATION_FILT_STEP);
    fi[RESTORATION_HALFWIN] -= fi[i] * 2;
  }
  // Specialize for 7-tap filter
  fi[0] = fi[6] = CLIP(fi[0], 3, 6);
  fi[1] = fi[5] = CLIP(fi[1], -23, -8);
  fi[2] = fi[4] = CLIP(fi[2], 5, 36);
  fi[3] = RESTORATION_FILT_STEP - 2 * (fi[0] + fi[1] + fi[2]);
}

static void search_wiener_filter(const YV12_BUFFER_CONFIG *src,
                                 VP10_COMP *cpi,
                                 int filter_level,
                                 int partial_frame,
  ...
  bits = 22;
  if (best_cost_ret)
    *best_cost_ret = RDCOST_DBL(x->rdmult, x->rddiv, (bits << 2), err);
}
...
void vp10_pick_filter_restoration(
  ...
  double cost_bilateral, cost_wiener;
