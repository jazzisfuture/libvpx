...
static int search_bilateral_level(const YV12_BUFFER_CONFIG *sd, VP10_COMP *cpi,
  ...
  RestorationInfo rsi;
  const int ntiles = vp10_get_restoration_ntiles(BILATERAL_TILESIZE,
                                                 cm->width, cm->height);
  ...
  rsi.bilateral_level = (int *)vpx_malloc(
      sizeof(*rsi.bilateral_level) * ntiles);
...
static int search_wiener_filter(const YV12_BUFFER_CONFIG *src, VP10_COMP *cpi,
  ...
  vp10_get_restoration_tile_size(tilesize, width, height,
  ...
  rsi.wiener_level =
      (int *)vpx_malloc(sizeof(*rsi.wiener_level) * ntiles);
  ...
  err = try_restoration_frame(src, cpi, &rsi, partial_frame);
  cost_wiener =
      RDCOST_DBL(x->rdmult, x->rddiv, (bits << (VP10_PROB_COST_SHIFT - 4)), err);
...
void vp10_pick_filter_restoration(const YV12_BUFFER_CONFIG *sd, VP10_COMP *cpi,
  ...
  cm->rst_info.bilateral_level = (int *)vpx_realloc(
...
// searched level, filt_guess = q * 0.316206 + 3.87252
    ...
    wiener_success = search_wiener_filter(
        ...
        cm->rst_info.wiener_level, &cost_wiener);
