...
static double compute_error(projectPointsType projectPoints,
  ...
  if (mask) {
    for (i = 0; i < npoints; ++i, mp1 += stride1, mp2 += stride2) {
      if (mask[i]) {
        projectPoints(wm_params, mp1, pt, 1, stride1, stride2);
  ...
  while (N > trial_count) {
    ...
    if (num_inliers >= max_inliers) {
      ...
      if ((num_inliers > max_inliers) ||
        ...
        memcpy(best_inlier_set1, inlier_set1, num_inliers * 2 *
               sizeof(*best_inlier_set1));
        memcpy(best_inlier_set2, inlier_set2, num_inliers * 2 *
               sizeof(*best_inlier_set2));
        memcpy(best_inlier_mask, inlier_mask, npoints *
               sizeof(*best_inlier_mask));
...
int ransacTranslation(double *matched_points, int npoints,
  ...
  return ransac_(matched_points,
                 ...
                 findTranslation,
                 projectPointsTranslation, TRANSLATION);
}
...
int ransacHomography(double *matched_points, int npoints,
  ...
  int result =
      ransac_(matched_points,
              ...
              findHomography,
              projectPointsHomography, HOMOGRAPHY);
  if (!result) {
