...
static void improve_correspondence(unsigned char *frm, unsigned char *ref,
  ...
  for (i = 0; i < num_correspondences; ++i) {
    double template_norm = compute_variance(
        ref, ref_stride, (int)correspondences[i].rx, (int)correspondences[i].ry,
        NULL);
    int x, y, best_x = 0, best_y = 0;
    double best_match_ncc = 0.0;
    for (y = -SEARCH_SZ_BY2; y <= SEARCH_SZ_BY2; ++y)
      for (x = -SEARCH_SZ_BY2; x <= SEARCH_SZ_BY2; ++x) {
        ...
        if (!is_eligible_point((int)correspondences[i].x + x,
          ...
          continue;
        if (!is_eligible_distance(
            (int)correspondences[i].x + x, (int)correspondences[i].y + y,
            (int)correspondences[i].rx, (int)correspondences[i].ry, width, height))
          continue;
        subimage_norm = compute_variance(
            frm, frm_stride,
            (int)correspondences[i].x + x, (int)correspondences[i].y + y, NULL);
        match_ncc = compute_cross_correlation(
