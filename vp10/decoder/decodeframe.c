...
static void setup_restoration(VP10_COMMON *cm, struct vpx_read_bit_buffer *rb) {
  ...
  if (vpx_rb_read_bit(rb)) {
    if (vpx_rb_read_bit(rb)) {
      ...
      rsi->bilateral_level =
          (int *)realloc(rsi->bilateral_level,
      ...
      rsi->wiener_process_tile = (int *)realloc(
          rsi->wiener_process_tile,
          sizeof(*rsi->wiener_process_tile) * ntiles);
      assert(rsi->wiener_process_tile != NULL);
      rsi->vfilter = (int(*)[RESTORATION_HALFWIN])realloc(
          rsi->vfilter, sizeof(*rsi->vfilter) * ntiles);
      assert(rsi->vfilter != NULL);
      rsi->hfilter = (int(*)[RESTORATION_HALFWIN])realloc(
          rsi->hfilter, sizeof(*rsi->hfilter) * ntiles);
      assert(rsi->hfilter != NULL);
      for (i = 0; i < ntiles; ++i) {
        rsi->wiener_process_tile[i] = vpx_rb_read_bit(rb);
        if (rsi->wiener_process_tile[i]) {
