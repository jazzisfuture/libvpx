...
#if CONFIG_VAR_TX
static void decode_reconstruct_tx(VP10_COMMON *const cm,
  ...
  if (tx_size == plane_tx_size) {
    ...
    *eob_total += eob;
    ...
    for (i = 0; i < 4; ++i) {
      ...
      decode_reconstruct_tx(cm, xd, r, mbmi, plane, block + i * step,
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        ...
        for (row = 0; row < num_4x4_h; row += bw) {
          for (col = 0; col < num_4x4_w; col += bw) {
            decode_reconstruct_tx(cm, xd, r, mbmi, plane, block, row, col,
