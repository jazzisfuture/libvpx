...
static void setup_loopfilter(VP10_COMMON *cm,
  ...
  if (vpx_rb_read_bit(rb)) {
    int level =
        (int)vpx_rb_read_literal(rb, vp10_restoration_level_bits(cm)) - 1;
    rst->restoration_level = level + (level >= rst->last_restoration_level);
  } else {
    rst->restoration_level = rst->last_restoration_level;
  }
  if (cm->frame_type != KEY_FRAME)
    rst->last_restoration_level = rst->restoration_level;
  else
    rst->last_restoration_level = -1;
#endif  // CONFIG_LOOP_RESTORATION
}
