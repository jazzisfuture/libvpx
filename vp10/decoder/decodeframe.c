...
static void setup_frame_size(VP10_COMMON *cm, struct vpx_read_bit_buffer *rb) {
  ...
  pool->frame_bufs[cm->new_fb_idx].buf.display_width  = cm->display_width;
  pool->frame_bufs[cm->new_fb_idx].buf.display_height = cm->display_height;
}
...
static size_t read_uncompressed_header(VP10Decoder *pbi,
  ...
  get_frame_new_buffer(cm)->display_width  = cm->display_width;
  get_frame_new_buffer(cm)->display_height = cm->display_height;
  ...
  if (!cm->error_resilient_mode) {
    ...
    if (cm->refresh_frame_context == REFRESH_FRAME_CONTEXT_FORWARD) {
        cm->refresh_frame_context =
            vpx_rb_read_bit(rb) ? REFRESH_FRAME_CONTEXT_BACKWARD
