  ...
  if (cm->show_existing_frame) {
...
#if CONFIG_BIDIR_PRED
    // NOTE: The existing frame to show can be adopted as a reference frame.
    // TODO(zoeliu): To re-evaluate the bit cost on the following info and
    //               explore an alternative scheme to avoid it.
    // TODO(zoeliu): To check whether pbi->refresh_frame_flags could be
    // zero-valued, if yes, then no need to write/read the following bits.
    // Change should be made to coordinate with the changes at the encoder.
    ...
    if (pbi->refresh_frame_flags) {
      for (i = 0; i < REFS_PER_FRAME; ++i) {
        ...
        // (zoeliu) cm->ref_frame_sign_bias[LAST_FRAME + i] =
        // vpx_rb_read_bit(rb);
      }
    } else {
      for (i = 0; i < REFS_PER_FRAME; ++i) {
        RefBuffer *const ref_frame = &cm->frame_refs[i];
        RefBuffer *const prev_ref_frame = &cm->prev_frame_refs[i];
        ref_frame->idx = prev_ref_frame->idx;
        ref_frame->buf = prev_ref_frame->buf;
      }
    }
...
#if CONFIG_BIDIR_PRED
      cm->prev_frame_refs[i].idx = INVALID_IDX;
      cm->prev_frame_refs[i].buf = NULL;
#endif  // CONFIG_BIDIR_PRED
    }
