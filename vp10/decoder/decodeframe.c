...
static void extend_and_predict_highbd(const uint8_t *buf_ptr1,
  ...
  DECLARE_ALIGNED(16, uint16_t,
    mc_buf_high[(MAX_CU_SIZE + 16) * 2 * (MAX_CU_SIZE + 16) * 2]);
...
static void extend_and_predict(const uint8_t *buf_ptr1, int pre_buf_stride,
  ...
  DECLARE_ALIGNED(16, uint8_t,
    mc_buf[(MAX_CU_SIZE + 16) * 2 * (MAX_CU_SIZE + 16) * 2]);
  const uint8_t *buf_ptr;
...
static void dec_predict_sb_complex(VP10Decoder *const pbi,
  ...
  DECLARE_ALIGNED(16, uint8_t,
                  tmp_buf1[MAX_MB_PLANE * MAX_TX_PIXELS * 2]);
  DECLARE_ALIGNED(16, uint8_t,
                  tmp_buf2[MAX_MB_PLANE * MAX_TX_PIXELS * 2]);
  DECLARE_ALIGNED(16, uint8_t,
                  tmp_buf3[MAX_MB_PLANE * MAX_TX_PIXELS * 2]);
...
#if CONFIG_VP9_HIGHBITDEPTH
        DECLARE_ALIGNED(16, uint8_t,
                        tmp_buf1[2 * MAX_MB_PLANE * MAX_CU_PIXELS]);
        DECLARE_ALIGNED(16, uint8_t,
                        tmp_buf2[2 * MAX_MB_PLANE * MAX_CU_PIXELS]);
#else
        DECLARE_ALIGNED(16, uint8_t,
                        tmp_buf1[MAX_MB_PLANE * MAX_CU_PIXELS]);
        DECLARE_ALIGNED(16, uint8_t,
                        tmp_buf2[MAX_MB_PLANE * MAX_CU_PIXELS]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
      ...
      for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
        ...
        for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
          ...
          decode_partition(pbi, &tile_data->xd,
                           ...
                           &tile_data->token_ans,
#endif  // CONFIG_ANS
                           BLOCK_LARGEST, MAX_CU_SIZE_LOG2 - 2);
        }
