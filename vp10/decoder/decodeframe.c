      ...
      if (xd->lossless) {
        ...
        switch (tx_size) {
          ...
          case TX_8X8:
            ...
            break;
          case TX_16X16:
            vp10_highbd_iht16x16_add(tx_type, dqcoeff, dst,
                                     stride, eob, xd->bd);
            break;
          case TX_32X32:
            vp10_highbd_idct32x32_add(dqcoeff, dst, stride, eob, xd->bd);
...
static void predict_and_reconstruct_intra_block(MACROBLOCKD *const xd,
  ...
  if (!mbmi->skip) {
    ...
    const scan_order *sc = (plane || xd->lossless) ?
        &vp10_default_scan_orders[tx_size] :
        &vp10_scan_orders[tx_size][tx_type];
...
static void setup_segmentation_dequant(VP9_COMMON *const cm) {
  ...
  if (cm->seg.enabled) {
    ...
    cm->y_dequant[0][0] = vp10_dc_quant(qindex, cm->y_dc_delta_q,
                                        cm->bit_depth);
