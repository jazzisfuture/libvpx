...
#if 0
    fprintf(stdout, "\n\n=== ENCODER === ctrl_get_reference(): "
            "Frame=%d, frame->idx=%d, ref_frame_map=%d\n",
            ctx->cpi->common.current_video_frame, frame->idx,
            ctx->cpi->common.ref_frame_map[frame->idx]);
    {
      int ref_idx;
      fprintf(stdout, "ref_frame_map:");
      for (ref_idx = 0; ref_idx < REF_FRAMES; ++ref_idx)
        fprintf(stdout, " [%d]=%d", ref_idx,
                ctx->cpi->common.ref_frame_map[ref_idx]);
      fprintf(stdout, "\n");
    }
    fflush(stdout);
#endif  // 0
