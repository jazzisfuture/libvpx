...
void projectPointsTranslation(int16_t *mat, int *points, int *proj,
...
void projectPointsHomography(int16_t *mat, int *points, int *proj,
                             const int n, const int stride_points,
                             const int stride_proj,
                             const int subsampling_x,
                             const int subsampling_y) {
...
static double highbd_warp_erroradv(WarpedMotionParams *wm,
                                   uint8_t *ref8, int width, int height, int stride,
                                   uint8_t *dst8, int p_col, int p_row, int p_width,
  ...
  for (i = p_row; i < p_row + p_height; ++i) {
    for (j = p_col; j < p_col + p_width; ++j) {
      ...
      in[0] = j;
      in[1] = i;
      projectPoints((int16_t*)wm->wmmat, in, out, 1, 2, 2,
...
static double warp_erroradv(WarpedMotionParams *wm,
...
#if CONFIG_VP9_HIGHBITDEPTH
  if (use_hbd)
    return highbd_warp_erroradv(wm, ref, width, height, stride, dst, p_col,
