...
static void scan_blk_mbmi(const VP10_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (is_inside(tile, mi_col, mi_row, cm->mi_rows, &mi_pos) &&
    ...
    if (candidate->ref_frame[0] == ref_frame) {
      int_mv this_refmv = get_sub_block_mv(candidate_mi, 0, mi_pos.col, block);
      for (index = 0; index < *refmv_count; ++index)
        if (ref_mv_stack[index].this_mv.as_int == this_refmv.as_int)
          break;
      ...
      if (candidate_mi->mbmi.sb_type < BLOCK_8X8) {
        ...
        this_refmv = get_sub_block_mv(candidate_mi, 0, mi_pos.col, alt_block);
        for (index = 0; index < *refmv_count; ++index)
          if (ref_mv_stack[index].this_mv.as_int == this_refmv.as_int)
            break;
    ...
    if (candidate->ref_frame[1] == ref_frame) {
      int_mv this_refmv = get_sub_block_mv(candidate_mi, 1, mi_pos.col, block);
      for (index = 0; index < *refmv_count; ++index)
        if (ref_mv_stack[index].this_mv.as_int == this_refmv.as_int)
          break;
      ...
      if (candidate_mi->mbmi.sb_type < BLOCK_8X8) {
        int alt_block = 3 - block;
        this_refmv = get_sub_block_mv(candidate_mi, 1, mi_pos.col, alt_block);
        for (index = 0; index < *refmv_count; ++index)
          if (ref_mv_stack[index].this_mv.as_int == this_refmv.as_int)
            break;
