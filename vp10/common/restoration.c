...
int vp10_restoration_ntiles(const struct VP10Common *const cm, int tiletype) {
  int nhtiles, nvtiles;
  const int tile_width = (tiletype < 0) ?
      cm->width : restoration_tile_sizes[tiletype].width;
  const int tile_height = (tiletype < 0) ?
      cm->height : restoration_tile_sizes[tiletype].height;
...
static void loop_bilateral_filter(uint8_t *data, int width, int height,
  ...
  for (tile_idx = 0; tile_idx < rst->bilateral_ntiles; ++tile_idx) {
    ...
    for (i = v_start; i < v_end; ++i) {
      memcpy(data + i * stride + h_start,
             tmpdata + i * tmpstride + h_start,
