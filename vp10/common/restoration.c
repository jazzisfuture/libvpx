...
} restoration_params_type;

static restoration_params_type
...
static INLINE restoration_params_type vp10_restoration_level_to_params(
    int index, int kf) {
...
void vp10_loop_restoration_precal() {
  ...
  for (i = 0; i < RESTORATION_LEVELS_KF; i ++) {
    const restoration_params_type param =
        vp10_restoration_level_to_params(i, 1);
  ...
  for (i = 0; i < RESTORATION_LEVELS; i ++) {
    const restoration_params_type param =
        vp10_restoration_level_to_params(i, 0);
    const int sigma_x = param.sigma_x;
    const int sigma_y = param.sigma_y;
    const int sigma_r = param.sigma_r;
    const double sigma_r_d = (double)sigma_r / RESTORATION_PARAM_PRECISION;
...
static void loop_restoration_filter(uint8_t *data, int width, int height,
                                    int stride, restoration_internal_n *rst,
...
void vp10_loop_restoration_frame(YV12_BUFFER_CONFIG *frame,
  ...
  if (restoration_type != RESTORE_NONE) {
    restoration_info_n rsi;
    rsi.restoration_type = restoration_type;
    rsi.restoration_level = restoration_level;
