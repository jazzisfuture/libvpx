...
static void ihalfcenter32_c(const tran_low_t *input, tran_low_t *output) {
  ...
  idct16_c(input, output + 8);
  for (i = 8; i < 24; ++i) {
    output[i] = (tran_low_t)dct_const_round_shift(output[i] * Sqrt2);
  }
}
...
static void highbd_ihalfcenter32_c(const tran_low_t *input, tran_low_t *output,
  ...
  vpx_highbd_idct16_c(input, output + 8, bd);
  for (i = 8; i < 24; ++i) {
...
static void highbd_ihalfright32_c(const tran_low_t *input, tran_low_t *output,
  ...
  vpx_highbd_idct16_c(input, output + 16, bd);
  for (i = 16; i < 32; ++i) {
    output[i] = (tran_low_t)highbd_dct_const_round_shift(
        output[i] * Sqrt2, bd);
  }
