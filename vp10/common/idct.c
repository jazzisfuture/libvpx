...
void vp10_highbd_inv_txfm_add_4x4(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    case DCT_DCT:
      if(bd == 10)
        vp10_inv_txfm2d_add_4x4(input, CONVERT_TO_SHORTPTR(dest), stride, &inv_txfm_2d_cfg_dct_dct_4, bd);
      else
...
void vp10_highbd_inv_txfm_add_8x8(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    case DCT_DCT:
      if(bd == 10)
        vp10_inv_txfm2d_add_8x8(input, CONVERT_TO_SHORTPTR(dest), stride, &inv_txfm_2d_cfg_dct_dct_8, bd);
      else
...
void vp10_highbd_inv_txfm_add_16x16(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    case DCT_DCT:
      if(bd == 10)
        vp10_inv_txfm2d_add_16x16(input, CONVERT_TO_SHORTPTR(dest), stride, &inv_txfm_2d_cfg_dct_dct_16, bd);
      else
