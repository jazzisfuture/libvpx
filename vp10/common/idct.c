...
#if CONFIG_EXT_TX
void vp10_i1dt4x4_16_add_c(const tran_low_t *input, uint8_t *dest, int stride,
                           int tx_type) {
  int i, j;
  tran_low_t tmp;
  tran_low_t out[4][4];
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 4; ++i) {
        IHT_4[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i]);
        input  += 4;
      }
      // transpose
      for (i = 1 ; i < 4; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 4; ++i) {
        ...
        IHT_4[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 4; i++) {
        for (j = 0; j < i; j++) {
      ...
      // Rows
      for (i = 0; i < 4; ++i) {
        IHT_4[tx_type].rows(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 4; i++) {
        for (j = 0; j < i; j++) {
...
#if CONFIG_EXT_TX
void vp10_i1dt8x8_64_add_c(const tran_low_t *input, uint8_t *dest, int stride,
                           int tx_type) {
  int i, j;
  tran_low_t tmp;
  tran_low_t out[8][8];
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 8; ++i) {
        IHT_8[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i]);
        input  += 8;
      }
      // transpose
      for (i = 1 ; i < 8; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 8; ++i) {
        ...
        IHT_8[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 8; i++) {
        for (j = 0; j < i; j++) {
      ...
      // Rows
      for (i = 0; i < 8; ++i) {
        IHT_8[tx_type].rows(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 8; i++) {
        for (j = 0; j < i; j++) {
...
#if CONFIG_EXT_TX
void vp10_i1dt16x16_256_add_c(const tran_low_t *input, uint8_t *dest, int stride,
                              int tx_type) {
  int i, j;
  tran_low_t tmp;
  tran_low_t out[16][16];
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 16; ++i) {
        IHT_16[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i]);
        input  += 16;
      }
      // transpose
      for (i = 1 ; i < 16; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 16; ++i) {
        for (j = 0; j < 16; ++j) {
          out[i][j] = input[j * 16 + i];
        }
        IHT_16[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 16; i++) {
        for (j = 0; j < i; j++) {
      ...
      // Rows
      for (i = 0; i < 16; ++i) {
        IHT_16[tx_type].rows(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 16; i++) {
        for (j = 0; j < i; j++) {
...
void vp10_i1dt32x32_1024_add_c(const tran_low_t *input, uint8_t *dest,
                               int stride, int tx_type) {
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 32; ++i) {
        IHT_32[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i]);
        input  += 32;
      }
      // transpose
      for (i = 1 ; i < 32; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 32; ++i) {
        for (j = 0; j < 32; ++j) {
          out[i][j] = input[j * 32 + i];
        }
        IHT_32[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 32; i++) {
        for (j = 0; j < i; j++) {
      ...
      // Rows
      for (i = 0; i < 32; ++i) {
        IHT_32[tx_type].rows(out[i], out[i]);
      }
      // transpose
      for (i = 1 ; i < 32; i++) {
        for (j = 0; j < i; j++) {
...
void vp10_inv_txfm_add_4x4(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_ADST:
    case V_FLIPADST:
      vp10_i1dt4x4_16_add_c(input, dest, stride, tx_type);
      break;
    case H_ADST:
    case H_DCT:
    case H_FLIPADST:
...
void vp10_inv_txfm_add_8x8(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_ADST:
    case V_FLIPADST:
      vp10_i1dt8x8_64_add_c(input, dest, stride, tx_type);
      break;
    case H_DCT:
    case H_ADST:
    case H_FLIPADST:
...
void vp10_inv_txfm_add_16x16(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_ADST:
    case V_FLIPADST:
      vp10_i1dt16x16_256_add_c(input, dest, stride, tx_type);
      break;
    case H_DCT:
    case H_ADST:
    case H_FLIPADST:
...
void vp10_inv_txfm_add_32x32(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case H_ADST:
    case H_FLIPADST:
      vp10_i1dt32x32_1024_add_c(input, dest, stride, tx_type);
      break;
    case IDTX:
...
#if CONFIG_EXT_TX
void vp10_highbd_i1dt4x4_16_add_c(const tran_low_t *input, uint8_t *dest8,
                                  int stride, int tx_type, int bd) {
  uint16_t *dest = CONVERT_TO_SHORTPTR(dest8);
  int i, j;
  tran_low_t tmp;
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 4; ++i) {
        HIGH_IHT_4[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i], bd);
        input  += 4;
      }
      // transpose
      for (i = 1 ; i < 4; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 4; ++i) {
        ...
        HIGH_IHT_4[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 4; i++) {
        for (j = 0; j < i; j++) {
      ...
      // Rows
      for (i = 0; i < 4; ++i) {
        HIGH_IHT_4[tx_type].rows(out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 4; i++) {
        for (j = 0; j < i; j++) {
  ...
  for (i = 0; i < 4; ++i) {
    for (j = 0; j < 4; ++j) {
      int d = i * stride + j;
      int s = j * outstride + i;
      dest[d] = highbd_clip_pixel_add(
          dest[d], ROUND_POWER_OF_TWO(outp[s], 4), bd);
    }
  }
}
#endif  // CONFIG_EXT_TX
...
void vp10_highbd_iht8x8_64_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  for (i = 0; i < 8; ++i) {
    for (j = 0; j < 8; ++j) {
      ...
      int s = j * outstride + i;
      dest[d] = highbd_clip_pixel_add(
          dest[d], ROUND_POWER_OF_TWO(outp[s], 5), bd);
    }
  }
}

#if CONFIG_EXT_TX
void vp10_highbd_i1dt8x8_64_add_c(const tran_low_t *input, uint8_t *dest8,
                                  int stride, int tx_type, int bd) {
  uint16_t *dest = CONVERT_TO_SHORTPTR(dest8);
  int i, j;
  tran_low_t tmp;
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 8; ++i) {
        HIGH_IHT_8[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i], bd);
        input  += 8;
      }
      // transpose
      for (i = 1 ; i < 8; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 8; ++i) {
        ...
        HIGH_IHT_8[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 8; i++) {
        for (j = 0; j < i; j++) {
      ...
      // Rows
      for (i = 0; i < 8; ++i) {
        HIGH_IHT_8[tx_type].rows(out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 8; i++) {
        for (j = 0; j < i; j++) {
...
#if CONFIG_EXT_TX
void vp10_highbd_i1dt16x16_256_add_c(const tran_low_t *input, uint8_t *dest8,
                                     int stride, int tx_type, int bd) {
  uint16_t *dest = CONVERT_TO_SHORTPTR(dest8);
  int i, j;
  tran_low_t tmp;
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 16; ++i) {
        HIGH_IHT_16[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i], bd);
        input  += 16;
      }
      // transpose
      for (i = 1 ; i < 16; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 16; ++i) {
        ...
        HIGH_IHT_16[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(
            out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 16; i++) {
      ...
      // Rows
      for (i = 0; i < 16; ++i) {
        HIGH_IHT_16[tx_type].rows(out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 16; i++) {
        for (j = 0; j < i; j++) {
...
void vp10_highbd_i1dt32x32_1024_add_c(const tran_low_t *input, uint8_t *dest8,
                                      int stride, int tx_type, int bd) {
  uint16_t *dest = CONVERT_TO_SHORTPTR(dest8);
  int i, j;
  tran_low_t tmp;
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
      // inverse transform row vectors
      for (i = 0; i < 32; ++i) {
        HIGH_IHT_32[i < TX2ROWS_1D ? DCT_DCT : tx_type].rows(input, out[i], bd);
        input  += 32;
      }
      // transpose
      for (i = 1 ; i < 32; i++) {
    ...
    case H_FLIPADST:
      ...
      for (i = 0; i < 32; ++i) {
        ...
        HIGH_IHT_32[i < TX2ROWS_1D ? DCT_DCT : tx_type].cols(
            out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 32; i++) {
      ...
      // Rows
      for (i = 0; i < 32; ++i) {
        HIGH_IHT_32[tx_type].rows(out[i], out[i], bd);
      }
      // transpose
      for (i = 1 ; i < 32; i++) {
        for (j = 0; j < i; j++) {
  ...
  for (i = 0; i < 32; ++i) {
    for (j = 0; j < 32; ++j) {
      ...
      int s = j * outstride + i;
      dest[d] = highbd_clip_pixel_add(
          dest[d], ROUND_POWER_OF_TWO(outp[s], 6), bd);
    }
  }
}
#endif  // CONFIG_EXT_TX
...
void vp10_highbd_inv_txfm_add_4x4(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
    case H_FLIPADST:
      vp10_highbd_i1dt4x4_16_add_c(input, dest, stride, tx_type, bd);
      break;
    case IDTX:
...
void vp10_highbd_inv_txfm_add_8x8(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
    case H_FLIPADST:
      vp10_highbd_i1dt8x8_64_add_c(input, dest, stride, tx_type, bd);
      break;
    case IDTX:
...
void vp10_highbd_inv_txfm_add_16x16(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
    case H_FLIPADST:
      vp10_highbd_i1dt16x16_256_add_c(input, dest, stride, tx_type, bd);
      break;
    case IDTX:
...
void vp10_highbd_inv_txfm_add_32x32(const tran_low_t *input, uint8_t *dest,
  ...
  switch (tx_type) {
    ...
    case V_FLIPADST:
    case H_FLIPADST:
      vp10_highbd_i1dt32x32_1024_add_c(input, dest, stride, tx_type, bd);
      break;
    case IDTX:
