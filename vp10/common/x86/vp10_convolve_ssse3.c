...
static INLINE void store_2_pixel_only(__m128i x, uint8_t *src, uint8_t *dst) {
  uint32_t temp;
  (void)src;
  x = _mm_packus_epi16(x, x);
  temp = _mm_cvtsi128_si32(x);
  *(uint16_t *)dst = (uint16_t)temp;
}

static INLINE void accumulate_store_2_pixel(__m128i x, uint8_t *src,
                                            uint8_t *dst) {
  uint32_t temp;
  __m128i y = accumulate_store(x, src);
...
static INLINE void store_8_pixel_only(__m128i x, uint8_t *src, uint8_t *dst) {
  (void)src;
  x = _mm_packus_epi16(x, x);
  _mm_storel_epi64((__m128i *)dst, x);
}

static INLINE void accumulate_store_8_pixel(__m128i x, uint8_t *src,
                                            uint8_t *dst) {
  __m128i y = accumulate_store(x, src);
...
static INLINE __m128i filter_vert_ssse3(const uint8_t *src, int src_stride,
                                        __m128i *f) {
  const __m128i k_256 = _mm_set1_epi16(1 << 8);
  const __m128i zero = _mm_setzero_si128();
