...
void vp10_convolve(const uint8_t *src, int src_stride, uint8_t *dst,
...
#else
                   cosnt INTERP_FILTER interp_filter,
#endif
  ...
  if (ignore_horiz && ignore_vert) {
    ...
    InterpFilterParams filter_params =
        vp10_get_interp_filter_params(interp_filter[1 + 2 * avg]);
#else
    InterpFilterParams filter_params =
...
#if CONFIG_DUAL_FILTER
    InterpFilterParams filter_params =
        vp10_get_interp_filter_params(interp_filter[0 + 2 * avg]);
#else
    InterpFilterParams filter_params =
        vp10_get_interp_filter_params(interp_filter);
#endif
...
#if CONFIG_DUAL_FILTER
    filter_params = vp10_get_interp_filter_params(interp_filter[0 + 2 * avg]);
#else
