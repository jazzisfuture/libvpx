...
static void convolve_copy(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if(avg == 0) {
    ...
    for (r = 0; r < h; ++r) {
      for(c = 0; c < w; ++c) {
        dst[c] = clip_pixel(ROUND_POWER_OF_TWO(dst[c] + src[c], 2));
      }
      src += src_stride;
      dst += dst_stride;
    }
...
void vp10_convolve(const uint8_t *src, int src_stride, uint8_t *dst,
  ...
  if(ignore_horiz && ignore_vert) {
    ...
    convolve_horiz(src, src_stride, dst,
                   dst_stride, w, h, filter_params,
                   subpel_x_q4, x_step_q4, avg);
  } else if(ignore_horiz) {
    convolve_vert(src, src_stride, dst,
                  dst_stride, w, h, filter_params,
...
static void highbd_convolve_copy(const uint16_t *src, int src_stride, uint16_t *dst,
                                 int dst_stride, int w, int h, int avg, int bd) {
  if(avg == 0) {
    int r;
    for (r = 0; r < h; ++r) {
    ...
    for (r = 0; r < h; ++r) {
      for(c = 0; c < w; ++c) {
        dst[c] = clip_pixel_highbd(ROUND_POWER_OF_TWO(dst[c] + src[c], 2), bd);
      }
      src += src_stride;
      dst += dst_stride;
    }
...
void vp10_highbd_convolve(const uint8_t *src8, int src_stride, uint8_t *dst8,
  ...
  if(ignore_horiz && ignore_vert) {
    ...
    highbd_convolve_horiz(src, src_stride, dst,
                   dst_stride, w, h, filter_params,
                   subpel_x_q4, x_step_q4, avg, bd);
  } else if(ignore_horiz) {
    highbd_convolve_vert(src, src_stride, dst,
                  dst_stride, w, h, filter_params,
    ...
    highbd_convolve_horiz(src - src_stride * (filter_size / 2 - 1),
