...
static INLINE INTRA_FILTER get_ref_intra_filter(const MB_MODE_INFO *ref_mbmi) {
...
int vp10_get_pred_context_intra_interp(const MACROBLOCKD *xd) {
  ...
  if (xd->left_available)
    left_type = get_ref_intra_filter(left_mbmi);

  if (xd->up_available)
    above_type = get_ref_intra_filter(above_mbmi);
