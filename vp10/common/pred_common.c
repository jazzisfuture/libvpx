...
int vp10_get_reference_mode_context(const VP10_COMMON *cm,
  ...
  if (has_above && has_left) {  // both edges available
    if (!has_second_ref(above_mbmi) && !has_second_ref(left_mbmi))
      ...
      ctx = (above_mbmi->ref_frame[0] == cm->comp_bwd_ref[1]) ^
            (left_mbmi->ref_frame[0] == cm->comp_bwd_ref[1]);
    else if (!has_second_ref(above_mbmi))
      // one of two edges uses comp pred (2/3)
      ctx = 2 + (above_mbmi->ref_frame[0] == cm->comp_bwd_ref[1] ||
                 !is_inter_block(above_mbmi));
    else if (!has_second_ref(left_mbmi))
      // one of two edges uses comp pred (2/3)
      ctx = 2 + (left_mbmi->ref_frame[0] == cm->comp_bwd_ref[1] ||
    ...
    if (!has_second_ref(edge_mbmi))
      ...
      ctx = edge_mbmi->ref_frame[0] == cm->comp_bwd_ref[1];
    else
...
int vp10_get_pred_context_single_ref_p3(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (above_has_second && left_has_second) {
        if (above0 == left0 && above1 == left1)
          pred_context = 3 * (above0 == BWDREF_FRAME || above1 == BWDREF_FRAME ||
