...
void vp10_fdct4_new(const int32_t *input, int32_t *output,
  ...
  buf0 = output;
  buf1 = step;
  buf1[0] = round_shift(cospi[32] * buf0[0] + cospi[32] * buf0[1],
                        cos_bit[stage_idx]);
  buf1[1] = round_shift(-cospi[32] * buf0[1] + cospi[32] * buf0[0],
                        cos_bit[stage_idx]);
  buf1[2] = round_shift(cospi[48] * buf0[2] + cospi[16] * buf0[3],
                        cos_bit[stage_idx]);
  buf1[3] = round_shift(cospi[48] * buf0[3] + -cospi[16] * buf0[2],
                        cos_bit[stage_idx]);
  range_check(stage_idx, input, buf1, size, stage_range[stage_idx]);
...
void vp10_idct4_new(const int32_t *input, int32_t *output,
  ...
  buf1[0] = round_shift(cospi[32] * buf0[0] + cospi[32] * buf0[1],
                        cos_bit[stage_idx]);
  buf1[1] = round_shift(cospi[32] * buf0[0] + -cospi[32] * buf0[1],
                        cos_bit[stage_idx]);
  buf1[2] = round_shift(cospi[48] * buf0[2] + -cospi[16] * buf0[3],
                        cos_bit[stage_idx]);
  buf1[3] = round_shift(cospi[16] * buf0[2] + cospi[48] * buf0[3],
                        cos_bit[stage_idx]);
  range_check(stage_idx, input, buf1, size, stage_range[stage_idx]);
