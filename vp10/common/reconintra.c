...
#else
# error "Unsupported coding unit size"
#endif  // MAX_CU_SIZE
...
static int vp10_has_right(BLOCK_SIZE bsize, int mi_row, int mi_col,
  ...
  if (y == 0) {
    ...
    tr_order = order[((mi_row - 1) << (MI_BLOCK_SIZE_LOG2 - wl)) + mi_col + 1];
  ...
  if (x == 0) {
    ...
    if (mi_col == 0)
      return bottom_available &&
             (mi_row << (hl + !ss_y)) + y + step < (MI_BLOCK_SIZE << !ss_y);
...
static void dr_prediction_z1(uint8_t *dst, ptrdiff_t stride, int bs,
  ...
  if (filter_type != INTRA_FILTER_LINEAR) {
    ...
    int len;
    DECLARE_ALIGNED(16, uint8_t, buf[SUBPEL_SHIFTS][MAX_CU_SIZE]);
    DECLARE_ALIGNED(16, uint8_t, src[MAX_CU_SIZE + SUBPEL_TAPS]);
...
static void dr_prediction_z3(uint8_t *dst, ptrdiff_t stride, int bs,
  ...
  if (filter_type != INTRA_FILTER_LINEAR) {
    const int pad_size = SUBPEL_TAPS >> 1;
    int len, i;
    DECLARE_ALIGNED(16, uint8_t, buf[MAX_CU_SIZE][4 * SUBPEL_SHIFTS]);
    DECLARE_ALIGNED(16, uint8_t, src[(MAX_CU_SIZE + SUBPEL_TAPS) * 4]);
    uint8_t flags[SUBPEL_SHIFTS];
...
static void build_intra_predictors_high(const MACROBLOCKD *xd,
  ...
  DECLARE_ALIGNED(16, uint16_t, left_col[MAX_CU_SIZE]);
  DECLARE_ALIGNED(16, uint16_t, above_data[MAX_CU_SIZE + 16]);
...
static void build_intra_predictors(const MACROBLOCKD *xd, const uint8_t *ref,
                                   ...
                                   int plane) {
  int i;
  DECLARE_ALIGNED(16, uint8_t, left_col[MAX_CU_SIZE]);
  const uint8_t *above_ref = ref - ref_stride;
  DECLARE_ALIGNED(16, uint8_t, above_data[MAX_CU_SIZE + 16]);
