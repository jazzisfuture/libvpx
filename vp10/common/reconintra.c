...
static int vp10_has_bottom(BLOCK_SIZE bsize, int mi_row, int mi_col,
  ...
  if (x == 0) {
    ...
    if (mi_col == 0)
      return bottom_available && (mi_row << !ss_y) + y + step < (8 << !ss_y);
...
void vp10_predict_intra_block(const MACROBLOCKD *xd, int bwl_in, int bhl_in,
  ...
  const int have_right = aoff < bw - 1;
#endif
  const int x = aoff * 4;
  const int y = loff * 4;
#if CONFIG_EXT_IPRED_BLTR
  ...
  int xl = (-xd->mb_to_left_edge >> (3 + pd->subsampling_x)) + x;
  int yu = (-xd->mb_to_top_edge >> (3 + pd->subsampling_y)) + y;
  int xr = (xd->mb_to_right_edge >> (3 + pd->subsampling_x)) + (wpx - x - txpx);
  int yd = (xd->mb_to_bottom_edge >> (3 + pd->subsampling_y)) + (hpx - y - txpx);
