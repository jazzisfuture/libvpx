  ...
  for (i0 = 0; i0 < PLANE_TYPES; ++i0)
    for (i1 = 0; i1 < REF_TYPES; ++i1)
      for (i2 = 0; i2 < COEF_BANDS; ++i2)
        for (i3 = 0; i3 < BAND_COEFF_CONTEXTS(i2); ++i3)
...
void vp10_default_coef_probs(VP10_COMMON *cm) {
  ...
  const int index =
      VPXMIN(ROUND_POWER_OF_TWO(cm->base_qindex, 8 - QCTX_BIN_BITS),
             QCTX_BINS - 1);
  //index = 0;
  fill_qctx_probs(index, TX_4X4, cm->fc->coef_probs[TX_4X4]);
  fill_qctx_probs(index, TX_8X8, cm->fc->coef_probs[TX_8X8]);
  fill_qctx_probs(index, TX_16X16, cm->fc->coef_probs[TX_16X16]);
  fill_qctx_probs(index, TX_32X32, cm->fc->coef_probs[TX_32X32]);
#else
  vp10_copy(cm->fc->coef_probs[TX_4X4], default_coef_probs_4x4);
  vp10_copy(cm->fc->coef_probs[TX_8X8], default_coef_probs_8x8);
  vp10_copy(cm->fc->coef_probs[TX_16X16], default_coef_probs_16x16);
