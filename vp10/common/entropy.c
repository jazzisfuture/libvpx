...
static void adapt_coef_probs_p(VP10_COMMON *cm, TX_SIZE tx_size,
                               unsigned int count_sat,
                               unsigned int update_factor) {
  const FRAME_CONTEXT *pre_fc = &cm->starting_fc;
  vp10_coeff_probs_model *const probs = cm->fc->coef_probs[tx_size];
  const vp10_coeff_probs_model *pre_probs = pre_fc->coef_probs[tx_size];
  vp10_coeff_count_model *counts = cm->counts.coef[tx_size];
  unsigned int (*eob_counts)[REF_TYPES][COEF_BANDS][COEFF_CONTEXTS] =
      cm->counts.eob_branch[tx_size];
  int i, j, k, l, m;
...
void vp10_adapt_coef_probs_p(VP10_COMMON *cm) {
  TX_SIZE t;
  unsigned int count_sat, update_factor;
  ...
  for (t = TX_4X4; t <= TX_32X32; t++)
    adapt_coef_probs_p(cm, t, count_sat, update_factor);
}
