...
static INLINE void inter_predictor(const uint8_t *src, int src_stride,
                                   ...
                                   int xs, int ys) {
  InterpFilterSet interp_filter_set = vp10_get_interp_filter_set(interp_filter);
  if(interp_filter_set.tap == SUBPEL_TAPS) {
    const InterpKernel *kernel = (const InterpKernel*)interp_filter_set.filter_ptr;
    ...
    int avg = ref > 0;
    vp10_convolve(src, src_stride, dst, dst_stride, w, h, interp_filter_set,
...
static INLINE void high_inter_predictor(const uint8_t *src, int src_stride,
  ...
  if(interp_filter_set.tap == SUBPEL_TAPS) {
    ...
    // therefore we need to average the first and second results
    int avg = ref > 0;
    vp10_highbd_convolve(src, src_stride, dst, dst_stride, w, h, interp_filter_set,
