...
static INLINE void inter_predictor(const uint8_t *src, int src_stride,
                                   ...
                                   int xs, int ys) {
#if CONFIG_DUAL_FILTER
  const int flt_idx = ((subpel_x == 0) ? 1 : 0);
  const int flt_idy = ((subpel_y == 0) ? 1 : 0);
  ...
  InterpFilterParams interp_filter_params_y =
      vp10_get_interp_filter_params(interp_filter[flt_idy + 2 * ref]);

  if (interp_filter[flt_idx + 2 * ref] != interp_filter[flt_idy + 2 * ref])
