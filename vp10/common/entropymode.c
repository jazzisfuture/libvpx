...
default_new_uv_probs[NEW_INTRA_MODES][NEW_INTRA_MODES - 1] = {
...
#if CONFIG_EXT_INTRA
const vpx_tree_index vp10_new_intra_mode_tree[TREE_SIZE(NEW_INTRA_MODES)] = {
  -DC_PRED, 2,
  -NEW_TM_PRED, 4,
  -V_PRED, 6,
  -H_PRED, 8,
...
static void init_mode_probs(FRAME_CONTEXT *fc) {
  ...
  vp10_copy(fc->new_uv_mode_prob, default_new_uv_probs);
#endif  // CONFIG_EXT_INTRA
}
...
void vp10_adapt_inter_frame_probs(VP10_COMMON *cm) {
  ...
  for (i = 0; i < NEW_INTRA_MODES; ++i)
    vpx_tree_merge_probs(vp10_new_intra_mode_tree, pre_fc->new_uv_mode_prob[i],
                         counts->new_uv_mode[i], fc->new_uv_mode_prob[i]);
#endif  // CONFIG_EXT_INTRA
