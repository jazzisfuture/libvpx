...
static const vpx_prob default_rect_tx_prob[TX_SIZES - 1] = {192, 192, 192};
...
static void init_mode_probs(FRAME_CONTEXT *fc) {
  ...
  vp10_copy(fc->rect_tx_prob, default_rect_tx_prob);
...
void vp10_adapt_inter_frame_probs(VP10_COMMON *cm) {
  ...
  if (cm->tx_mode == TX_MODE_SELECT) {
    for (i = 0; i < TX_SIZES - 1; ++i) {
      fc->rect_tx_prob[i] =
          vp10_mode_mv_merge_probs(pre_fc->rect_tx_prob[i], counts->rect_tx[i]);
    }
  }
