...
int main(int argc, char **argv)
{
    ...
    cfg[0].g_w = width;
    cfg[0].g_h = height;
    cfg[0].g_threads = 1;                             /* Set # of threads used for highest-resolution encoder. */
    ...
    /* Choose down_sampling_factor, for example: 4/3, 2/1, 8/3, 4/1, 8/1, ... */
    cfg[0].mr_down_sampling_factor.num = 2;
    cfg[0].mr_down_sampling_factor.den = 1;

    /* Allocate memory to store lower-resolution motion result for higher-resolution encoder to use */
    {
        int mb_rows = (cfg[0].g_w >>4) + ((cfg[0].g_w & 15)?1:0);
        int mb_cols = (cfg[0].g_h >>4) + ((cfg[0].g_h & 15)?1:0);

        cfg[0].mr_low_res_mode_info = calloc(mb_rows*mb_cols, 100);  //sizeof(LOWER_RES_INFO) = 80 (on 32bit or 64 bit)
    ...
    for (i=1; i< NUM_ENCODES; i++)
    {
        ...
        cfg[i].g_threads = 1;                  /* Set # of threads used for other encoders. */
        ...
        if(cfg[0].mr_down_sampling_factor.den == 1)
        {
            if(cfg[0].mr_down_sampling_factor.num == 2)
            {
            ...
            {
                cfg[i].g_w = cfg[i-1].g_w/cfg[0].mr_down_sampling_factor.num;
                cfg[i].g_h = cfg[i-1].g_h/cfg[0].mr_down_sampling_factor.num;
                cfg[i].g_w += ((cfg[i-1].g_w - cfg[i].g_w*cfg[0].mr_down_sampling_factor.num)?1:0);
                cfg[i].g_h += ((cfg[i-1].g_h - cfg[i].g_h*cfg[0].mr_down_sampling_factor.num)?1:0);
            }
        }else
        {
            /* Arbitrary down-sampling factor */
            unsigned int iw = cfg[i-1].g_w*cfg[0].mr_down_sampling_factor.den;
            unsigned int ih = cfg[i-1].g_h*cfg[0].mr_down_sampling_factor.den;
            cfg[i].g_w = iw/cfg[0].mr_down_sampling_factor.num;
            cfg[i].g_h = ih/cfg[0].mr_down_sampling_factor.num;
            cfg[i].g_w += ((iw - cfg[i].g_w*cfg[0].mr_down_sampling_factor.num)?1:0);
            cfg[i].g_h += ((ih - cfg[i].g_h*cfg[0].mr_down_sampling_factor.num)?1:0);
        }
    }
    ...
    while(frame_avail || got_data)
    {
        ...
        for ( i=1; i<NUM_ENCODES; i++)
        {
            if(frame_avail)
            {
                ...
                //FilterMode = 0 results in worse psnr, FilterMode = 1 or 2(same) give better psnr. All modes have similar result performance wise.
        ...
        /* Encode each level in a reverse order. Namely, first encode at lowest-resolution level.
         * Then encode at next higher-resolution level. Finally, encode at the highest-resolution level.
         */
