...
static size_t read_input_file(input_file_t *in, unsigned char **y,
  ...
  switch (in->type) {
    ...
    case RAW_YUV:
      if (bd < 9) {
        r1 = fread(in->buf, in->frame_size, in->file);
        *y = in->buf;
        *u = in->buf + in->w * in->h;
...
#define psnr_and_ssim(ssim, psnr, buf0, buf1, w, h) \
  ...
  psnr = calc_plane_error(buf0, w, buf1, w, w, h);
    ...
    psnr_and_ssim(ssimu[n_frames], psnru[n_frames], u[0], u[1], (w+1) / 2, (h+1) / 2);
    psnr_and_ssim(ssimv[n_frames], psnrv[n_frames], v[0], v[1], (w+1) / 2, (h+1) / 2);
