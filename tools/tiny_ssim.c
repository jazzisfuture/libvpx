...
#define psnr_and_ssim(ssim, psnr, buf0, buf1, w, h) \
  ...
  psnr = calc_plane_error(buf0, w, buf1, w, w, h);
    ...
    psnr_and_ssim(ssimu[n_frames], psnru[n_frames], buf[0] + w * h, buf[1] + w * h, w / 2, h / 2);
    psnr_and_ssim(ssimv[n_frames], psnrv[n_frames], buf[0] + w * h * 5 / 4, buf[1] + w * h * 5 / 4,
                  w / 2, h / 2);
    n_frames++;
  }
  free(buf[0]);
  ...
  for (i = 0; i < n_frames; ++i) {
    ...
    psnruavg += vp9_mse2psnr(w * h * 4 / 2, 255.0, (double)psnru[i]);
    psnrvavg += vp9_mse2psnr(w * h * 4 / 2, 255.0, (double)psnrv[i]);
  ...
  psnruglb = vp9_mse2psnr((double)n_frames * w * h * 2 / 4, 255.0, psnruglb);
  psnrvglb = vp9_mse2psnr((double)n_frames * w * h * 2 / 4, 255.0, psnrvglb);
