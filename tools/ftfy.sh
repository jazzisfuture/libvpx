...
vpx_astyle()
{
  astyle --style=bsd --min-conditional-indent=0 --break-blocks \
         ...
         --align-pointer=name \
         --indent-preprocessor --indent-labels --suffix=none "$@"
}
...
commit() {
  LAST_CHANGEID=$(git show | grep Change-Id: | awk '{print $2}')
...
if [ $INTERSECT_RESULT -eq 0 ]; then
  ...
  if [ -n "$1" ]; then
    ...
    apply "${FINAL_DIFF}"
    if ! git diff --quiet; then
      echo "${self##*/}: Formatting changes applied, verify and commit." >&2
      git diff --stat
    fi
  fi
fi
