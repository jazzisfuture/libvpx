...
def main():
    hunks_1 = [x for x in ParseDiffHunks(open(sys.argv[1], "r"))]
    hunks_2 = [x for x in ParseDiffHunks(open(sys.argv[2], "r"))]
    out_hunks = []

    # Join the right hand side of the older diff with the left hand side of the
    # newer diff.
    for old_hunk, new_hunk in ZipHunks(hunks_1, hunks_2):
