class PointCloud {
  ...
  void interpolateDepth(int row, int col) {
    ...
    ArrayList<PVector> back = new ArrayList();
    ...
    while (queue.size() > 0) {
      ...
      {
        //fill current depth to the nodes on the path
        //from original location to current locatuon
        while(back_idx!=-1)
        {
          PVector last_pos = back.get(back_idx);
          int r = int(last_pos.x);
          int c = int(last_pos.y);
          back_idx = int(last_pos.z);
          if(depth[r*width+c]==0)
            depth[r*width+c] = depth[i*width+j];
        }
        break;
      }
      else
        ...
        for(int r=max(0,i-1);r<min(height,i+2);r++)
          for(int c=max(0,j-1);c<min(width,j+2);c++)
          {
            if(!visited[r*width+c])
            {
              ...
              queue.add(new PVector(r,c,back_idx));
            }
          }
      }
    }
