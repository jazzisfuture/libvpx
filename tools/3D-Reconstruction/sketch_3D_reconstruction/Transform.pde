class Transform
{
  ...
  float focal;
  int w,h;
  Transform(float tx,float ty,float tz,float qx,float qy,float qz,float qw,float focal,int w,int h)
  {
    //currently, we did not use the info of real camera's position and quaternion
    //maybe we will use it in the future when combine all frames
...
float[] getRotationMat3x3(float angle,float ax,float ay,float az)
{
  ...
  mat[3] = ay*ax*(1-c) + az*s; mat[4] = c + ay*ay*(1-c);    mat[5] = ay*az*(1-c) - ax*s;
  mat[6] = az*ax*(1-c) - ay*s; mat[7] = az*ay*(1-c) + ax*s; mat[8] = c + az*az*(1-c);
  return mat;
}
...
{
  float[] mat = new float[9];
  mat[0]  = 1-2*qy*qy-2*qz*qz; mat[1]  = 2*qx*qy-2*qz*qw;   mat[2] = 2*qx*qz+2*qy*qw;
  mat[3]  = 2*qx*qy+2*qz*qw;   mat[4]  = 1-2*qx*qx-2*qz*qz; mat[5] = 2*qy*qz-2*qx*qw;
  mat[6]  = 2*qx*qz-2*qy*qw;   mat[7]  = 2*qy*qz+2*qx*qw;   mat[8] = 1-2*qx*qx-2*qy*qy; 
  return mat;
}
