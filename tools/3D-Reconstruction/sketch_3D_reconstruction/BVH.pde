...
class BoundingBox {
  ...
  float compare(BoundingBox bbx, int axis) {
    if (axis == 1) {
...
class BVH {
  ...
  BVH(ArrayList<Triangle> mesh) {
    ...
    while (i < j) {
      ...
      while (i + 1 < this.mesh.size() &&
             ti.bbx.compare(overall_bbx, axis) < 0) {
        i++;
        ti = this.mesh.get(i);
      }
      while (j - 1 >= 0 && tj.bbx.compare(overall_bbx, axis) >= 0) {
        j--;
        tj = this.mesh.get(j);
      }
      if (i < j) {
    ...
    // If there will be an empty mesh, we force to assign one triangle to the
    // child with empty mesh.
    int flag;
    if (j == 0) {
      flag = 1;
    } else if (i == this.mesh.size() - 1) {
      flag = this.mesh.size() - 1;
    } else {
      flag = i + 1;
    }
  ...
  boolean intersect(Ray r, float[] param) {
    ...
    if (!overall_bbx.intersect(r)) return false;
