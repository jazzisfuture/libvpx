...
# There doesn't seem to be a portable way to prevent people from running this
# directly. Hopefully removing the executable bit is sufficient.
#[ $_ = $0 ] && echo "$0 must be sourced." && exit 1
...
if [ ! $(which clang) ]; then
  echo "clang must be in your PATH"
  return 1
fi
...
if [ "${sanitizer}" = "undefined" ] ; then
  major_version=$(clang --version | head -n 1 | grep -o -E "[[:digit:]]\.[[:digit:]]\.[[:digit:]]" | cut -f1 -d.)
...
case "${sanitizer}" in
  ...
  memory)
    ...
    ;;
  thread)
    # The thread sanitizer uses an entirely independant set of options.
    set -x
