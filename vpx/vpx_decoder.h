...
#define vpx_codec_dec_init(ctx, iface, cfg, flags) \
   ...
   *
   * Frame buffers with a size of 0 are valid.  The application must pass in
   * at least 8 external frame buffers, as VP9 can have up to 8 reference
   * frames. Registers a given function to be called when the current frame
   * to decode will be bigger than the external frame buffer size.
   *
   * \param[in] ctx          Pointer to this instance's context
