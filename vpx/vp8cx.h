...
enum vp8e_enc_control_id {
   ...
   *
   * 8For example, to allow 100% more bits, i.e, 2X, in a golden frame
   ...
   * input source. This control function provide a mean to switche encoder
   * into lossless coding mode (1) or back to normal coding mode (0) that
   * may be lossy.
   *                          0 = lossy coding mode
   ...
   * limitation(The minimum width of a tile column is 256 pixel, the maxium
   * is 4096).
   *
   ...
   *
   * In encoding and decoding, VP9 allows an input image frame be partitioned
   * into horizontally separated tile rows. Tile rows are encoded or decoded
   * sequentially. Even though encoding/decoding of later tile rows depends on
   * the earlier tile rows, this allows encoder output data packets for a tile
   * row prior completing the rest of tile rows within a frame, therefore
   * reduce the latency in processing. The parameter for this control describes
   * the number of tile rows with a valid range [0, 2], where
   *            0 = 1 tile row
   *            1 = 2 tile rows
   *            2 = 4 tile rows
   *
   ...
   *
   * VP9 has a bitstream feature to remove some decoding dependency by turning
   * off backward update of probability context used in encoding and decoding,
   * so to allow staged parallel processing of more than one video frames in
   * the decoder. This control function provide a mean to control encoder to
   * produce compressed video bitstream with suppport of this feature on/off.
   *
