...
#define CHECK_FSCANF_ERROR(expr) \
  do {                           \
    if (expr == EOF) {           \
      return VPX_CODEC_ERROR;    \
    }                            \
...
static void free_allocated_tpl_memory(VpxTplGopStats *tpl_gop_stats,
                                      const int num_frame) {
  int frame;
  for (frame = 0; frame < num_frame; frame++) {
    free(tpl_gop_stats->frame_stats_list[frame].block_stats_list);
  }
  free(tpl_gop_stats->frame_stats_list);
}
...
vpx_codec_err_t vpx_alloc_and_read_tpl_stats(FILE *tpl_file,
  ...
  for (i = 0; i < frame_list_size; i++) {
    ...
    if (frame_stats->block_stats_list == NULL) {
      free_allocated_tpl_memory(tpl_gop_stats, i);
      return VPX_CODEC_MEM_ERROR;
    }
    for (block = 0; block < num_blocks; block++) {
      VpxTplBlockStats *block_stats = &frame_stats->block_stats_list[block];
      CHECK_FSCANF_ERROR(fscanf(
          tpl_file,
          "%" SCNd64 " %" SCNd64 " %" SCNd16 " %" SCNd16 " %" SCNd64 " %" SCNd64
          " %d\n",
          &block_stats->inter_cost, &block_stats->intra_cost,
          &block_stats->mv_c, &block_stats->mv_r, &block_stats->recrf_dist,
          &block_stats->recrf_rate, &block_stats->ref_frame_index));
...
vpx_codec_err_t vpx_free_tpl_gop_stats(VpxTplGopStats *data) {
  int frame;
