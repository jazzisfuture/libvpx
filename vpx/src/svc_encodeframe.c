...
typedef struct SvcInternal {
  ...
  int viewport_id[VPX_SS_MAX_LAYERS];
  ...
  int layer;
  int is_keyframe;
  int use_multiple_frame_context;
...
static vpx_codec_err_t parse_viewport_values(SvcContext *svc_ctx,
                                              const char *bitrate_values) {
  char *input_string;
  char *token;
  const char *delim = ",";
  ...
  for (i = 0; i < svc_ctx->spatial_layers; ++i) {
    if (token != NULL) {
      viewport_id = atoi(token);
      if (viewport_id < 0 || viewport_id > 4) {
        svc_log(svc_ctx, SVC_LOG_ERROR,
                "viewports: invalid value %s\n", token);
        res = VPX_CODEC_INVALID_PARAM;
        break;
      }
      token = strtok_r(NULL, delim, &save_ptr);
...
static vpx_codec_err_t parse_options(SvcContext *svc_ctx, const char *options) {
  ...
  while (option_name != NULL) {
    ...
    if (strcmp("layers", option_name) == 0) {
      ...
      si->use_multiple_frame_context = atoi(option_value);
    } else if (strcmp("bitrates", option_name) == 0) {
      res = parse_bitrate_values(svc_ctx, option_value);
      if (res != VPX_CODEC_OK) break;
    } else if (strcmp("viewports", option_name) == 0) {
      res = parse_viewport_values(svc_ctx, option_value);
      if (res != VPX_CODEC_OK) break;
    } else {
  ...
  if (si->use_multiple_frame_context && svc_ctx->spatial_layers > 3)
    res = VPX_CODEC_INVALID_PARAM;
...
vpx_codec_err_t vpx_svc_init(SvcContext *svc_ctx, vpx_codec_ctx_t *codec_ctx,
  ...
  if (si->layers >= 1) {
    ...
    for (i = 0; i < si->layers; ++i) {
      enc_cfg->ss_viewport_id[i] = si->viewport_id[i];
    }
  }
  ...
  if (enc_cfg->g_error_resilient == 0 && si->use_multiple_frame_context == 0)
