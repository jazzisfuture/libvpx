...
typedef struct SvcInternal {
  ...
  int psnr_in_layer[VPX_SS_MAX_LAYERS];
...
static vpx_codec_err_t svc_parse_input_parameters(SvcContext* svc_ctx) {
  ...
  if (svc_ctx->quantizer_values == NULL ||
    ...
    svc_ctx->quantizer_values = strdup(DEFAULT_QUANTIZER_VALUES);
  }

  found = 0;
  token = strtok(svc_ctx->quantizer_values, delim);
  ...
  if (svc_ctx->scale_factors == NULL || strlen(svc_ctx->scale_factors) == 0) {
    svc_ctx->scale_factors = strdup(DEFAULT_SCALE_FACTORS);
  }
  found = 0;
  token = strtok(svc_ctx->scale_factors, delim);
  for (i = 0; i < si->layers; i++) {
    if (token != NULL) {
      if (sscanf(token, "%d/%d", &num, &den) != 2) {
    ...
    si->scaling_factor_num[i + VPX_SS_MAX_LAYERS - si->layers] = num;
    si->scaling_factor_den[i + VPX_SS_MAX_LAYERS - si->layers] = den;
  }
  if (found != si->layers) {
    svc_log(svc_ctx, SVC_LOG_ERROR,
...
vpx_codec_err_t vpx_svc_init(SvcContext* svc_ctx, vpx_codec_ctx_t* codec_ctx,
  ...
  ret = svc_parse_input_parameters(svc_ctx);
  if (ret != VPX_CODEC_OK) {
  ...
  // Initialize codec
  ret = vpx_codec_enc_init(codec_ctx, iface, enc_cfg, VPX_CODEC_USE_PSNR);
  if (ret != VPX_CODEC_OK) {
...
static void set_svc_parameters(SvcContext* svc_ctx,
  ...
  layer_index = si->layer + VPX_SS_MAX_LAYERS - si->layers;
  if (svc_ctx->encoding_mode == ALT_INTER_LAYER_PREDICTION_IP &&
      si->frame_within_gop == 0) {
    // layers 1 & 3 don't exist in this mode, use the higher one
    if (si->layer == 0 || si->layer == 2) {
      layer_index += 1;
    }
  }
  svc_params.width = si->width * si->scaling_factor_num[layer_index] /
                     si->scaling_factor_den[layer_index];
  svc_params.height = si->height * si->scaling_factor_num[layer_index] /
                      si->scaling_factor_den[layer_index];

  // make height and width even to make chrome player happy
  svc_params.width += svc_params.width % 2;
  svc_params.height += svc_params.height % 2;
...
vpx_codec_err_t vpx_svc_encode(SvcContext* svc_ctx, vpx_codec_ctx_t* codec_ctx,
  ...
  si->layers =
      (svc_ctx->first_frame_full_size == 1 && si->encode_frame_count == 0)
          ? 1
          : svc_ctx->spatial_layers;
  ...
  for (si->layer = 0; si->layer < si->layers; si->layer++) {
    ...
    // TODO(ivanmaltz) first frame should be scaled to size of first layer
    // scaling is necessary if 1st frame scale factor != 1/1
    if (si->layers > 1) set_svc_parameters(svc_ctx, codec_ctx);
      ...
      switch (cx_pkt->kind) {
        ...
        case VPX_CODEC_PSNR_PKT:
          svc_log(svc_ctx, SVC_LOG_DEBUG,
                  ...
                  "%2.3f  %2.3f  %2.3f  %2.3f \n",
                  svc_ctx->log_print ? colors[si->layer] : "",
                  si->encode_frame_count, si->layer, cx_pkt->data.psnr.psnr[0],
                  cx_pkt->data.psnr.psnr[1], cx_pkt->data.psnr.psnr[2],
                  cx_pkt->data.psnr.psnr[3]);
...
void svc_dump_statistics(SvcContext* svc_ctx) {
  int number_of_frames, number_of_keyframes;
  int i;
  int bytes_total = 0;
  SvcInternal* si = (SvcInternal*)svc_ctx->internal;
  ...
  number_of_keyframes = si->encode_frame_count / svc_ctx->gop_size + 1;
  for (i = 0; i < si->layers; i++) {
    number_of_frames = si->encode_frame_count;
    if (svc_ctx->encoding_mode == ALT_INTER_LAYER_PREDICTION_IP &&
        (i == 1 || i == 3)) {
