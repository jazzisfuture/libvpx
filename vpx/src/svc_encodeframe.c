    ...
    if (si->bitrates[0] != 0) {
      ...
      for (sl = 0; sl < svc_ctx->spatial_layers; ++sl) {
        if (si->svc_params.scaling_factor_den[sl] > 0) {
          alloc_ratio[sl] = (float)(si->svc_params.scaling_factor_num[sl] * 1.0 /
      ...
      for (sl = 0; sl < svc_ctx->spatial_layers; ++sl) {
        enc_cfg->ss_target_bitrate[sl] = spatial_layer_target = (unsigned int)(enc_cfg->rc_target_bitrate * alloc_ratio[sl]/total);
        if (svc_ctx->temporal_layering_mode == 3) {
          enc_cfg->layer_target_bitrate[sl * svc_ctx->temporal_layers] = spatial_layer_target>>1;
          enc_cfg->layer_target_bitrate[sl * svc_ctx->temporal_layers + 1] = (spatial_layer_target>>1) + (spatial_layer_target>>2);
          enc_cfg->layer_target_bitrate[sl * svc_ctx->temporal_layers + 2] = spatial_layer_target;
        }
        else if (svc_ctx->temporal_layering_mode == 2){
          enc_cfg->layer_target_bitrate[sl * svc_ctx->temporal_layers] = spatial_layer_target * 2/3;
          enc_cfg->layer_target_bitrate[sl * svc_ctx->temporal_layers + 1] = spatial_layer_target;
        }
        else {
...
vpx_codec_err_t vpx_svc_init(SvcContext *svc_ctx, vpx_codec_ctx_t *codec_ctx,
  ...
  if (si->svc_params.temporal_layering_mode == 3) {
    svc_ctx->temporal_layers = 3;
  }
  else if (si->svc_params.temporal_layering_mode == 2) {
  ...
  if (svc_ctx->temporal_layers * svc_ctx->spatial_layers > VPX_MAX_LAYERS) {
      svc_log(svc_ctx, SVC_LOG_ERROR, "spatial layers * temporal layers exceeds the maximum number of allowed layers of %d\n",
                  svc_ctx->spatial_layers * svc_ctx->temporal_layers, (int) VPX_MAX_LAYERS);
      return VPX_CODEC_INVALID_PARAM;
  }
  ...
  // Use 1 thread as default. TODO: change this to be configurable after bug is fixed with multithreading.
  enc_cfg->g_threads = 1;
  ...
  vpx_codec_control(codec_ctx, VP8E_SET_CPUUSED, svc_ctx->speed);
  vpx_codec_control(codec_ctx, VP9E_SET_TILE_COLUMNS, (enc_cfg->g_threads >> 1));
  return VPX_CODEC_OK;
}
