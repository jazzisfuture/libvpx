...
vpx_codec_err_t vpx_svc_init(SvcContext *svc_ctx, vpx_codec_ctx_t *codec_ctx,
  ...
  if (svc_ctx->temporal_layers * svc_ctx->spatial_layers > VPX_MAX_LAYERS) {
      svc_log(svc_ctx, SVC_LOG_ERROR, "spatial layers * temporal layers exceeds "
          "the maximum number of allowed layers of %d\n",
  ...
  // TODO(marpan): Change this to be configurable after bug is fixed with
  // multithreading.
  enc_cfg->g_threads = 1;
  ...
  vpx_codec_control(codec_ctx, VP9E_SET_TILE_COLUMNS,
                    (enc_cfg->g_threads >> 1));
  return VPX_CODEC_OK;
}
