...
static struct LayerData *ld_create(void *buf, size_t size) {
  struct LayerData *layer_data = malloc(sizeof(struct LayerData));
  if (layer_data == NULL) {
    return NULL;
  }
...
void vpx_svc_set_options(SvcContext *svc_ctx, const char *options) {
  strncpy(svc_ctx->options, options, sizeof(svc_ctx->options));
}

void vpx_svc_set_quantizers(SvcContext *svc_ctx, const char *quantizers) {
  strncpy(svc_ctx->quantizers, quantizers, sizeof(svc_ctx->quantizers));
}

void vpx_svc_set_scale_factors(SvcContext *svc_ctx, const char *scale_factors) {
  strncpy(svc_ctx->scale_factors, scale_factors,
          sizeof(svc_ctx->scale_factors));
}
...
vpx_codec_err_t vpx_svc_init(SvcContext *svc_ctx, vpx_codec_ctx_t *codec_ctx,
  ...
  if (svc_ctx->internal == NULL) {
    svc_ctx->internal = malloc(sizeof(*si));
    if (svc_ctx->internal == NULL) return VPX_CODEC_MEM_ERROR;
  }
  ...
  res = parse_quantizer_values(svc_ctx, svc_ctx->quantizers);
  if (res != VPX_CODEC_OK) return res;

  res = parse_scale_factors(svc_ctx, svc_ctx->scale_factors);
  if (res != VPX_CODEC_OK) return res;

  // parse aggregate command line options
  res = parse_options(svc_ctx, svc_ctx->options);
  if (res != VPX_CODEC_OK) return res;
...
vpx_codec_err_t vpx_svc_encode(SvcContext *svc_ctx, vpx_codec_ctx_t *codec_ctx,
  ...
  for (si->layer = 0; si->layer < si->layers; ++si->layer) {
    ...
    res = vpx_codec_encode(codec_ctx, rawimg, pts, (unsigned long)duration,
    ...
    while ((cx_pkt = vpx_codec_get_cx_data(codec_ctx, &iter))) {
      switch (cx_pkt->kind) {
        case VPX_CODEC_CX_FRAME_PKT:
          frame_pkt_size = (uint32_t)(cx_pkt->data.frame.sz);
          if (!vpx_svc_dummy_frame(svc_ctx, si)) {
          ...
          layer_data = ld_create(cx_pkt->data.frame.buf, cx_pkt->data.frame.sz);
          if (layer_data == NULL) {
