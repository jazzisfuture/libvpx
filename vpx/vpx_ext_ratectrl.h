...
typedef enum vpx_rc_type {
  /*!
   * The external rate control doesn't determines anything.
...
typedef struct vpx_rc_ref_frame {
  /*!
   * Ref frame index. Correspondent to |lst_fb_idx|, |gld_fb_idx| or
   * |alt_fb_idx| in VP9_COMP depending on the ref frame name specified below.
   */
  int index[VPX_RC_MAX_REF_FRAMES];
  /*!
   * Ref frame name. This decides whether the index above is used as
