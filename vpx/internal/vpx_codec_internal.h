...
/*!\brief Pass in external frame buffers for the decoder to use.
   ...
   * decode will be bigger than the external frame buffer size. The
   * application must pass in at least 8 external frame buffers, as VP9
   * can have up to 8 reference frames. This function must be called
   * before the first call to decode or libvpx will assume the default
   * behavior of allocating frame buffers internally.
   *
   * \param[in] ctx          Pointer to this instance's context
...
typedef vpx_codec_err_t (*vpx_codec_set_frame_buffers_fn_t)(
    vpx_codec_alg_priv_t *ctx, vpx_codec_frame_buffer_t *fb_list,
    int fb_count, vpx_realloc_frame_buffer_cb_fn_t cb,
    void *user_priv);
