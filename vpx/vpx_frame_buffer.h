...
/*!\brief The maximum number of planes that can be allocated by an external frame buffer.
 */
#define VP9_MAXIMUM_EXTERNAL_PLANES 3

/*!\brief External frame buffer allocation type.
 * This enum defines if the external frame buffer contains one allocation for all the planes
 * contained in the frame buffer, or one allocation per plane.
 */
typedef enum vpx_codec_frame_buffer_type {
  VPX_CODEC_FRAME_BUFFER_SIZE = 1,
  VPX_CODEC_FRAME_BUFFER_PLANES = 2,
...
typedef struct vpx_codec_frame_buffer {
  ...
  vpx_codec_frame_buffer_type_t type;  /**< Wether data and size, or plane and stride should be used. */

  uint8_t *data;  /**< Pointer to the data buffer */
  size_t size;  /**< Size of data in bytes */

  uint8_t *plane[VP9_MAXIMUM_EXTERNAL_PLANES];  /**< Pointers for each plane buffer */
 ...
 * The callback can allocate one buffer for all the frame buffer, or one buffer
 ...
 * If the application decides to allocate one buffer per plane, fb->type must be set
 * to VPX_CODEC_FRAME_BUFFER_PLANES, the buffer pointers must be assigned to
 * fb->plane, and their strides to fb->stride.
 * The application provided buffer and strides must be aligned to 32 bytes.
 * The callback must zero out all the data allocated.
 *
