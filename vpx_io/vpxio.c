...
int vpxio_open_img_file(struct vpxio_ctx *output_ctx)
{
    ...
    output_ctx->decompression_ctx.outfile_pattern =
        output_ctx->decompression_ctx.outfile_pattern ?
        output_ctx->decompression_ctx.outfile_pattern : "-";
    ...
    if (!output_ctx->decompression_ctx.noblit)
    {
        ...
        {
            generate_filename(output_ctx->decompression_ctx.outfile_pattern,
                              out_fn, PATH_MAX - 1, output_ctx->width,
                              output_ctx->height, 0);

            output_ctx->file_name = out_fn;

            if (output_ctx->file_name)
                output_ctx->file = out_open(output_ctx->file_name,
...
static int webm_guess_framerate(struct vpxio_ctx *input_ctx)
{
    ...
    if (nestegg_track_seek(input_ctx->decompression_ctx.webm_ctx.nestegg_ctx,
...
int vpxio_set_outfile_pattern(struct vpxio_ctx *input_ctx,
...
int vpxio_init_ctx(struct vpxio_ctx *input_ctx)
{
    ...
    memset(&input_ctx->compression_ctx.ebml, 0,
           sizeof input_ctx->compression_ctx.ebml);
...
{
    input_ctx->compression_ctx.detect.buf_read =
        fread(input_ctx->compression_ctx.detect.buf, 1, 4, input_ctx->file);
...
int file_is_webm(struct vpxio_ctx *input_ctx)
{
    ...
    if (nestegg_init(&input_ctx->decompression_ctx.webm_ctx.nestegg_ctx,
                     io, NULL))
        goto fail;
    ...
    for (i = 0; i < n; i++)
    {
        track_type = nestegg_track_type(
            input_ctx->decompression_ctx.webm_ctx.nestegg_ctx, i);
    ...
    if (nestegg_track_codec_id(input_ctx->decompression_ctx.webm_ctx.nestegg_ctx
        , i) != NESTEGG_CODEC_VP8)
    {
    ...
    if (nestegg_track_video_params(input_ctx->decompression_ctx.webm_ctx.nestegg_ctx,
        i, &params))
        goto fail;
...
int vpxio_open_src(struct vpxio_ctx *input_ctx, const char *input_file_name)
{
    ...
    input_ctx->file = strcmp(input_ctx->file_name, "-") ?
                      fopen(input_ctx->file_name, "rb") : set_binary_mode(stdin);
...
int vpxio_open_i420_src(struct vpxio_ctx *input_ctx,
...
int vpxio_open_ivf_dst(struct vpxio_ctx *output_ctx,
                       ...
                       int framerate_num)
{
    ...
    output_ctx->file = strcmp(output_ctx->file_name, "-") ?
                       fopen(output_ctx->file_name, "wb") : set_binary_mode(stdout);
...
int vpxio_open_webm_dst(struct vpxio_ctx *output_ctx,
                        const char *file_name,
                        int width,
                        int height,
                        int framerate_den,
...
int vpxio_open_i420_dst(struct vpxio_ctx *output_ctx,
                        const char *outfile_pattern,
                        int width,
                        int height)
{
...
int vpxio_open_y4m_dst(struct vpxio_ctx *output_ctx,
                       const char *outfile_pattern,
                       int width, int height,
                       int framerate_den,
                       int framerate_num)
...
int vpxio_close(struct vpxio_ctx *input_ctx)
{
    if (input_ctx->file)
    {
        ...
        else
        {
            ...
            if (input_ctx->decompression_ctx.single_file &&
                    !input_ctx->decompression_ctx.noblit)
            {
                out_close(input_ctx->file, input_ctx->file_name,
                          input_ctx->decompression_ctx.do_md5);
                return 0;
            }
        }
...
int vpxio_read_pkt(struct vpxio_ctx *input_ctx,
                   uint8_t **buf,
                   size_t *buf_sz,
                   size_t *buf_alloc_sz)
{
...
int vpxio_read_img(struct vpxio_ctx *input_ctx, vpx_image_t *img_raw)
{
    if (input_ctx->frame_cnt == 0)
    {
        ...
        else
        {
            ...
            vpx_img_alloc(img_raw, vpxio_get_use_i420(input_ctx) ?
                          VPX_IMG_FMT_I420 : VPX_IMG_FMT_YV12, input_ctx->width,
                          input_ctx->height, 1);
        }
    }
...
int vpxio_write_img(struct vpxio_ctx *output_ctx, vpx_image_t *img)
{
    ...
    if (output_ctx->decompression_ctx.do_md5 == FRAME_MD5 ||
            output_ctx->decompression_ctx.do_md5 == FRAME_FILE_MD5 ||
            output_ctx->decompression_ctx.do_md5 == FRAME_OUT_MD5)
    {
    ...
    if (!output_ctx->decompression_ctx.single_file)
        out_close(output_ctx->file, out_fn,
        output_ctx->decompression_ctx.do_md5);
