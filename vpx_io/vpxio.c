    ...
    if (!output_ctx->decompression_ctx.noblit)
    {
        if (output_ctx->decompression_ctx.single_file)
        {
            generate_filename(output_ctx->decompression_ctx.outfile_pattern,
                              out_fn, PATH_MAX - 1, output_ctx->width, output_ctx->height, 0);
...
int file_is_webm(struct vpxio_ctx *input_ctx)
{
    ...
    if (nestegg_track_codec_id(input_ctx->decompression_ctx.webm_ctx.nestegg_ctx, i)
            != NESTEGG_CODEC_VP8)
    {
    ...
    if (input_ctx->frame_cnt == 0)
    {
        ...
        else
        {
            ...
            vpx_img_alloc(img_raw, vpxio_get_use_i420(input_ctx) ?
                          VPX_IMG_FMT_I420 : VPX_IMG_FMT_YV12, input_ctx->width, input_ctx->height, 1);
        }
    }
...
int vpxio_write_img(struct vpxio_ctx *output_ctx, vpx_image_t *img)
{
    ...
    if (!output_ctx->decompression_ctx.single_file)
    {
        ...
        out_fn[len] = '\0';
        generate_filename(output_ctx->decompression_ctx.outfile_pattern,
                          out_fn, len - 1, img->d_w, img->d_h, output_ctx->frame_cnt);
