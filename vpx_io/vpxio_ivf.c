    ...
    for (plane = 0; plane < 3; plane++)
    {
        ...
        */
        ...
        for (r = 0; r < h; r++)
        {
            ...
            const size_t left = input_ctx->compression_ctx.detect.buf_read - input_ctx->compression_ctx.detect.position;

            if (left > 0)
            {
                const size_t more = (left < needed) ? left : needed;
                memcpy(ptr, input_ctx->compression_ctx.detect.buf + input_ctx->compression_ctx.detect.position, more);
                buf_position = more;
                needed -= more;
            ...
            if (needed > 0)
            {
                shortread |= (fread(ptr + buf_position, 1, needed, input_ctx->file) < needed);
            }
...
int vpxio_read_pkt_ivf(struct vpxio_ctx *input_ctx, uint8_t **buf, size_t *buf_sz, size_t *buf_alloc_sz)
{
    ...
    if (fread(raw_hdr, IVF_FRAME_HDR_SZ, 1, input_ctx->decompression_ctx.webm_ctx.infile) != 1)
    {
...
int vpxio_write_img_ivf(struct vpxio_ctx *output_ctx, vpx_image_t *img, uint8_t *buf)
{
    ...
    write_ivf_frame_header_dec((FILE *)output_ctx->file, 0, (3 * img->d_h * img->d_w) / 2);
