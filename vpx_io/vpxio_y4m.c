...
int write_y4m_file_header(struct vpxio_ctx *output_ctx)
{
    ...
    sprintf(buffer, "YUV4MPEG2 C%s W%u H%u F%u:%u I%c\n",
            "420jpeg", output_ctx->width, output_ctx->height, output_ctx->framerate.num, output_ctx->framerate.den, 'p');
    out_put(output_ctx->file, (unsigned char *)buffer, strlen(buffer), output_ctx->decompression_ctx.do_md5);
    return 0;
}
int vpxio_read_open_raw_file_y4m(struct vpxio_ctx *input_ctx)
{
    ...
    if (y4m_input_open(&input_ctx->compression_ctx.y4m_in, input_ctx->file, input_ctx->compression_ctx.detect.buf, 4) >= 0)
    {
...
{
    if (y4m_input_fetch_frame(&input_ctx->compression_ctx.y4m_in, input_ctx->file, img_raw) < 1)
        return 0;
    else
        return 1;
}
int vpxio_write_img_y4m(struct vpxio_ctx *output_ctx, vpx_image_t *img, uint8_t *buf)
{
    unsigned int y;

    out_put(output_ctx->file, (unsigned char *)"FRAME\n", 6, output_ctx->decompression_ctx.do_md5);

    buf = img->planes[VPX_PLANE_Y];

    for (y = 0; y < img->d_h; y++)
    {
        out_put(output_ctx->file, buf, img->d_w, output_ctx->decompression_ctx.do_md5);
    ...
    {
        out_put(output_ctx->file, buf, (1 + img->d_w) / 2, output_ctx->decompression_ctx.do_md5);
