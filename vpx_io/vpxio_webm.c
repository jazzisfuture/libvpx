...
int write_webm_file_header(struct vpxio_ctx *output_ctx)
{
    ...
    write_webm_file_header_intrn(&output_ctx->compression_ctx.ebml, output_ctx->width, output_ctx->height, &output_ctx->framerate, 0);
...
int vpxio_write_pkt_webm(struct vpxio_ctx *output_ctx, const vpx_codec_cx_pkt_t *pkt)
{
    if (!output_ctx->compression_ctx.ebml.debug)
        output_ctx->compression_ctx.hash = murmur(pkt->data.frame.buf, pkt->data.frame.sz, output_ctx->compression_ctx.hash);

    write_webm_block(&output_ctx->compression_ctx.ebml, output_ctx->timebase.num, output_ctx->timebase.den, pkt);

    return 0;
}
int vpxio_write_close_enc_file_webm(struct vpxio_ctx *output_ctx)
{
    write_webm_file_footer(&output_ctx->compression_ctx.ebml, output_ctx->compression_ctx.hash);
...
{
    if (input_ctx->decompression_ctx.webm_ctx.chunk >= input_ctx->decompression_ctx.webm_ctx.chunks)
    {
        unsigned int track;
        ...
        do
        {
            ...
            if (nestegg_read_packet(input_ctx->decompression_ctx.webm_ctx.nestegg_ctx, &input_ctx->decompression_ctx.webm_ctx.pkt) <= 0
                || nestegg_packet_track(input_ctx->decompression_ctx.webm_ctx.pkt, &track))
                return 1;
        }
        while (track != input_ctx->decompression_ctx.webm_ctx.video_track);

        if (nestegg_packet_count(input_ctx->decompression_ctx.webm_ctx.pkt, &input_ctx->decompression_ctx.webm_ctx.chunks))
            return 1;

        input_ctx->decompression_ctx.webm_ctx.chunk = 0;
    }

    if (nestegg_packet_data(input_ctx->decompression_ctx.webm_ctx.pkt, input_ctx->decompression_ctx.webm_ctx.chunk, buf, buf_sz))
        return 1;
