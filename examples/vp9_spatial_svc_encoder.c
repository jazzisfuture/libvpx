...
#define OUTPUT_SEPARATE_LAYERS 1
...
static void svc_output_rc_stats(
  ...
  for (sl = 0; sl < enc_cfg->ss_number_layers; ++sl) {
    ...
    const uint8_t *buf_start = (uint8_t *)cx_pkt->data.frame.buf + tot_size;
    for (sl2 = 0; sl2 < sl; ++sl2) {
      if (cx_pkt->data.frame.spatial_layer_encoded[sl2]) tot_size += sizes[sl2];
    }
    vpx_video_writer_write_frame(outfile[sl], buf_start, (size_t)(sizes[sl]),
