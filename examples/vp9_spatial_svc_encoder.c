...
static const arg_def_t temporal_layering_mode_arg =
    ARG_DEF("tlm", "temporal-layering-mode", 1, "temporal layering scheme.-a. VP9E_TEMPORAL_LAYERING_MODE");
...
static const arg_def_t *svc_args[] = {
  ...
  &max_bitrate_arg,   &temporal_layers_arg, &temporal_layering_mode_arg,  &lag_in_frame_arg,
#if OUTPUT_RC_STATS
  &output_rc_stats_arg,
#endif
...
static const uint32_t default_speed = 0;
...
static void parse_command_line(int argc, const char **argv_,
  ...
  for (argi = argj = argv; (*argj = *argi); argi += arg.argv_step) {
    ...
    if (arg_match(&arg, &frames_arg, argi)) {
      ...
      svc_ctx->temporal_layering_mode = enc_cfg->temporal_layering_mode = arg_parse_int(&arg);
...
static void set_rate_control_metrics(struct RateControlMetrics *rc,
  ...
  for (sl = 0; sl < cfg->ss_number_layers; ++sl) {
    ...
    for (tl = 0; tl < cfg->ts_number_layers; ++tl) {
      rc->layer_framerate[sl*cfg->ts_number_layers + tl] =
          framerate / cfg->ts_rate_decimator[tl];
      if (tl > 0) {
        rc->layer_pfb[sl*cfg->ts_number_layers + tl] = 1000.0 *
            (cfg->layer_target_bitrate[sl*cfg->ts_number_layers + tl] - cfg->layer_target_bitrate[sl*cfg->ts_number_layers + tl - 1]) /
            (rc->layer_framerate[sl*cfg->ts_number_layers + tl] - rc->layer_framerate[sl*cfg->ts_number_layers + tl - 1]);
      }
      else {
        rc->layer_pfb[sl*cfg->ts_number_layers] = 1000.0 * cfg->layer_target_bitrate[sl*cfg->ts_number_layers] /
            rc->layer_framerate[sl*cfg->ts_number_layers];
      }

      rc->layer_input_frames[sl*cfg->ts_number_layers + tl] = 0;
      rc->layer_enc_frames[sl*cfg->ts_number_layers + tl] = 0;
      rc->layer_tot_enc_frames[sl*cfg->ts_number_layers + tl] = 0;
      rc->layer_encoding_bitrate[sl*cfg->ts_number_layers + tl] = 0.0;
      rc->layer_avg_frame_size[sl*cfg->ts_number_layers + tl] = 0.0;
      rc->layer_avg_rate_mismatch[sl*cfg->ts_number_layers + tl] = 0.0;
    }
  }
  rc->window_count = 0;
  rc->window_size = 15;
...
static void printout_rate_control_summary(struct RateControlMetrics *rc,
  ...
  for (sl = 0; sl < cfg->ss_number_layers; ++sl) {
    for (tl = 0; tl < cfg->ts_number_layers; ++tl) {
      ...
      rc->layer_avg_rate_mismatch[layer] = 100.0 * rc->layer_avg_rate_mismatch[layer] /
          rc->layer_enc_frames[layer];
      ...
      printf("Number of input frames, encoded (non-key) frames, "
          "and perc dropped frames: %d %d %f.0 \n", rc->layer_input_frames[layer],
          rc->layer_enc_frames[layer],
  ...
  if (frame_cnt != tot_num_frames)
    die("Error: Number of input frames not equal to output! encoded frames = %d tot_num_frames = %d\n", frame_cnt, tot_num_frames);
}
...
int main(int argc, const char **argv) {
  ...
  if (svc_ctx.output_rc_stat) {
    set_rate_control_metrics(&rc, &enc_cfg);
...
#if OUTPUT_RC_STATS
  // for now, just writing temporal layer streams... TODO: do spatial by re-writing superframe.
  if (svc_ctx.output_rc_stat) {
    for (tl = 0; tl < enc_cfg.ts_number_layers; ++tl) {
      char file_name[PATH_MAX];

      snprintf(file_name, sizeof(file_name), "%s_t%d.ivf", app_input.output_filename, tl);
  ...
  while (!end_of_stream) {
    ...
    res = vpx_svc_encode(&svc_ctx, &codec, (end_of_stream ? NULL : &raw),
                         pts, frame_duration, svc_ctx.speed >=5 ? VPX_DL_REALTIME:VPX_DL_GOOD_QUALITY);
        ...
        case VPX_CODEC_CX_FRAME_PKT: {
          if (cx_pkt->data.frame.sz > 0) {
            ...
            if (svc_ctx.output_rc_stat) {
              ...
              parse_superframe_index(cx_pkt->data.frame.buf, cx_pkt->data.frame.sz, sizes, &count);
              for (sl = 0; sl < enc_cfg.ss_number_layers; ++sl) {
                ++rc.layer_input_frames[sl*enc_cfg.ts_number_layers + layer_id.temporal_layer_id];
              }
              for (tl = layer_id.temporal_layer_id; tl < enc_cfg.ts_number_layers; ++tl) {
                vpx_video_writer_write_frame(outfile[tl], cx_pkt->data.frame.buf,
              ...
              for (sl = 0; sl < enc_cfg.ss_number_layers; ++sl) {
                for (tl = layer_id.temporal_layer_id; tl < enc_cfg.ts_number_layers; ++tl) {
                  ...
                  if (tl == layer_id.temporal_layer_id && !(cx_pkt->data.frame.flags & VPX_FRAME_IS_KEY)) {
