...
static const arg_def_t temporal_layering_mode_arg =
    ARG_DEF("tlm", "temporal-layering-mode", 1, "temporal layering scheme.-a. "
    ...
    for (tl = 0; tl < cfg->ts_number_layers; ++tl) {
      ...
      if (tl > 0) {
        rc->layer_pfb[sl*cfg->ts_number_layers + tl] = 1000.0 *
            (cfg->layer_target_bitrate[sl*cfg->ts_number_layers + tl] -
                cfg->layer_target_bitrate[sl*cfg->ts_number_layers + tl - 1]) /
            (rc->layer_framerate[sl*cfg->ts_number_layers + tl] -
                rc->layer_framerate[sl*cfg->ts_number_layers + tl - 1]);
      } else {
        rc->layer_pfb[sl*cfg->ts_number_layers] = 1000.0 *
            cfg->layer_target_bitrate[sl*cfg->ts_number_layers] /
...
#if OUTPUT_RC_STATS
  // For now, just writing temporal layer streams.
