...
// Allocate image buffer
  ...
  // Initialize codec
  if (vpx_svc_init(&svc_ctx, &encoder, vpx_codec_vp9_cx(), &enc_cfg, test_rc_interface, &rc_rtc) !=
      VPX_CODEC_OK)
...
if (!test_rc_interface) {
  ...
  if (!test_rc_interface) {
    ...
    if (frame_cnt == 100) {
      for (i = 0; i < 9; i++)
        enc_cfg.layer_target_bitrate[i] = 2 * enc_cfg.layer_target_bitrate[i];
       enc_cfg.rc_target_bitrate = 2 *  enc_cfg.rc_target_bitrate;
       vpx_codec_enc_config_set(&encoder, &enc_cfg);

    } else if (frame_cnt == 300) {
       for (i = 0; i < 9; i++)
        enc_cfg.layer_target_bitrate[i] = enc_cfg.layer_target_bitrate[i] / 4;
       enc_cfg.rc_target_bitrate = enc_cfg.rc_target_bitrate / 4;
       vpx_codec_enc_config_set(&encoder, &enc_cfg); 
    }
  ...
  if (test_rc_interface) {
    ...
    int qp_internal = 0; int lf_level = 0;
     ...
     if (frame_cnt == 100) {
      for (i = 0; i < 9; i++)
        rc_rtc.layer_target_bitrate[i] = 2 * rc_rtc.layer_target_bitrate[i];
      rc_rtc.target_bandwidth = 2 *  rc_rtc.target_bandwidth;
      vpx_codec_control(&encoder, VP9E_UPDATE_RC_RTC, &rc_rtc);

     } else if (frame_cnt == 300) {
       for (i = 0; i < 9; i++)
         rc_rtc.layer_target_bitrate[i] = rc_rtc.layer_target_bitrate[i] / 4;
