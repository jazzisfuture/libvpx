...
#if CONFIG_DECODERS
    // Only look for mismatch on top spatial and top temporal layers.
    if (layer_id.temporal_layer_id == (int)enc_cfg.ts_number_layers - 1 &&
        layer_id.temporal_layer_id > 0 &&
        cx_pkt->data.frame.spatial_layer_encoded[enc_cfg.ss_number_layers - 1])
