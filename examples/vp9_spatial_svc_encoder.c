...
// Example for 2 spatial layers and 2 temporal layers: Corresponds to the
// spatial-temporal pattern VP9E_TEMPORAL_LAYERING_MODE_0101
// (temporal_layering_mode == 2) of non-flexible mode.
void set_frame_flags_bypass_mode(int sl, int tl, int num_spatial_layers,
                                 int is_key_frame,
                                 vpx_svc_frame_flags_t *frame_flags) {
  for (sl = 0; sl < num_spatial_layers; ++sl) {
    if (!tl) {
      if (!sl) {
        frame_flags->frame_flags[sl] = VP8_EFLAG_NO_REF_GF |
                                       VP8_EFLAG_NO_REF_ARF |
                                       VP8_EFLAG_NO_UPD_GF |
                                       VP8_EFLAG_NO_UPD_ARF;
      } else {
        if (is_key_frame) {
          frame_flags->frame_flags[sl] = VP8_EFLAG_NO_REF_LAST |
                                         VP8_EFLAG_NO_REF_ARF |
                                         VP8_EFLAG_NO_UPD_GF |
                                         VP8_EFLAG_NO_UPD_ARF;
        } else {
        frame_flags->frame_flags[sl] = VP8_EFLAG_NO_REF_ARF |
                                       VP8_EFLAG_NO_UPD_GF |
                                       VP8_EFLAG_NO_UPD_ARF;
        }
      }
    ...
    if (tl == 0) {
      frame_flags->lst_fb_idx[sl] = sl;
      if (sl)
        frame_flags->gld_fb_idx[sl] = sl - 1;
      else
        frame_flags->gld_fb_idx[sl] = 0;
      frame_flags->alt_fb_idx[sl] = 0;
    } else if (tl == 1) {
      frame_flags->lst_fb_idx[sl] = sl;
      frame_flags->gld_fb_idx[sl] = num_spatial_layers + sl - 1;
      frame_flags->alt_fb_idx[sl] = num_spatial_layers + sl;
    }
  }
}
...
int main(int argc, const char **argv) {
  ...
  while (!end_of_stream) {
    ...
    if (svc_ctx.temporal_layering_mode == VP9E_TEMPORAL_LAYERING_MODE_BYPASS) {
      // Example for 2 spatial and 2 temporal layers.
      if (frame_cnt % 2 == 0)
      ...
      // over and encoder all the spatial layers for the current superframe.
      vpx_codec_control(&codec, VP9E_SET_SVC_LAYER_ID, &layer_id);
      set_frame_flags_bypass_mode(sl, layer_id.temporal_layer_id,
                                  svc_ctx.spatial_layers,
                                  frame_cnt == 0,
                                  &frame_flags);
      vpx_codec_control(&codec, VP9E_SET_SVC_FRAME_FLAGS, &frame_flags);
    }
