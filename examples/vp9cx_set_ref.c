...
int main(int argc, char **argv) {
  FILE *infile = NULL;
  vpx_codec_ctx_t ecodec = {0};         // encoder
  ...
  int test_decode = 1;                  // test encoder/decoder mismatch
  vpx_codec_ctx_t dcodec;               // decoder
  ...
  // enable_auto_alt_ref = 1 should work, but is not recommended since it
  // would be hard to figure out which reference buffer is set.
  ...
  while (vpx_img_read(&raw, infile)) {
    ...
    if (update_frame_num > 1 && frame_out + 1 == update_frame_num) {
      ...
      // If the set_reference in decoder is commented out, the enc/dec mismatch
      // would be seen.
      if (test_decode) {
