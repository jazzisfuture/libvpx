...
int main(int argc, char **argv) {
  ...
  printf("Using %s\n", vpx_codec_iface_name(encoder->iface()));

  res = vpx_codec_enc_config_default(encoder->iface(), &cfg, 0);
  if (res)
  ...
  if (vpx_codec_enc_init(&codec, encoder->iface(), &cfg, 0))
