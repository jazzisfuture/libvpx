...
int main(int argc, char **argv) {
  ...
  unsigned int width;
  unsigned int height;
  unsigned int error_resilient = 0;
  int speed;
  int frame_avail;
  int got_data;
  int flags = 0;
  ...
  if (argc < min_args) {
    ...
    die("Usage: %s <infile> <outfile> <codec_type(vp8/vp9)> <width> <height> "
        "<rate_num> <rate_den> <speed> <frame_drop_threshold> <error_resilient> <threads> <mode> "
  ...
  error_resilient = (uint8_t)strtoul(argv[10], NULL, 0);
  if (error_resilient != 0 && error_resilient != 1) {
    die("Invalid value for error resilient %d", error_resilient);
  }
