...
if (test_rc_interface) {
   ...
   rc_rtc.scaling_factor_num[0] = 1;
   rc_rtc.scaling_factor_den[0] = 1;
   rc_rtc.layer_target_bitrate[0] = cfg.rc_target_bitrate;
   rc_rtc.max_quantizers[0] = cfg.rc_max_quantizer;
   rc_rtc.min_quantizers[0] = cfg.rc_min_quantizer;

   vpx_codec_control(&codec, VP9E_INIT_RC_RTC, &rc_rtc);
  ...
  while (frame_avail || got_data) {
    ...
    if (test_rc_interface) {
     ...
     //printf("%d %d %d \n", frame_cnt, qp_internal, lf_level);
      ...
      // Update bitrate
      if (frame_cnt == 100) {
        cfg.rc_target_bitrate = 2 * cfg.rc_target_bitrate;
        if (vpx_codec_enc_config_set(&codec, &cfg))
           die_codec(&codec, "Failed to change config");
      } else  if (frame_cnt == 300) {
         cfg.rc_target_bitrate = cfg.rc_target_bitrate / 4;
         if (vpx_codec_enc_config_set(&codec, &cfg))
           die_codec(&codec, "Failed to change config");
      }
