...
unsigned int file_is_ivf(struct VpxInputContext* input_ctx) {
  char raw_hdr[32];
  unsigned int is_ivf = 0;
  ...
  if (fread(raw_hdr, 1, 32, input_ctx->file) == 32) {
    if (raw_hdr[0] == 'D' && raw_hdr[1] == 'K'
       ...
       */
      if (input_ctx->framerate.num < 1000) {
        /* Correct for the factor of 2 applied to the timebase in the
         * encoder.
         */
        if (input_ctx->framerate.num & 1)
          input_ctx->framerate.den <<= 1;
        else
          input_ctx->framerate.num >>= 1;
      } else {
