...
void ARGBToARGB4444Row_C(const uint8 *src_argb, uint8 *dst_rgb, int width) {
  ...
  for (x = 0; x < width - 1; x += 2) {
    ...
    *(uint32 *)(dst_rgb) = b0 | (g0 << 4) | (r0 << 8) | (a0 << 12) |
...
void I444ToARGBRow_C(const uint8 *src_y, const uint8 *src_u, const uint8 *src_v,
                     uint8 *rgb_buf, const struct YuvConstants *yuvconstants,
                     int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
...
void I422AlphaToARGBRow_C(const uint8 *src_y, const uint8 *src_u,
...
void NV12ToARGBRow_C(const uint8 *src_y, const uint8 *src_uv, uint8 *rgb_buf,
                     const struct YuvConstants *yuvconstants, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
...
void NV12ToRGB565Row_C(const uint8 *src_y, const uint8 *src_uv,
                       uint8 *dst_rgb565,
                       const struct YuvConstants *yuvconstants, int width) {
  uint8 b0;
  uint8 g0;
...
void YUY2ToARGBRow_C(const uint8 *src_yuy2, uint8 *rgb_buf,
                     const struct YuvConstants *yuvconstants, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
...
void UYVYToARGBRow_C(const uint8 *src_uyvy, uint8 *rgb_buf,
                     const struct YuvConstants *yuvconstants, int width) {
  int x;
  for (x = 0; x < width - 1; x += 2) {
