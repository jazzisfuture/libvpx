...
static vpx_codec_err_t ctrl_init_rc_rtc(vpx_codec_alg_priv_t *ctx,
  ...
  oxcf->worst_allowed_q = data->max_quantizer;
  oxcf->best_allowed_q = data->min_quantizer;
  oxcf->target_bandwidth = data->target_bandwidth;
  ...
  rc->worst_quality = data->max_quantizer;
  rc->best_quality = data->min_quantizer;
...
static vpx_codec_err_t ctrl_update_rc_rtc(vpx_codec_alg_priv_t *ctx,
  ...
  VP9EncoderConfig *oxcf = &cpi->oxcf;
  RATE_CONTROL *const rc = &cpi->rc;
  // Keep these two features off: aq_mode=0 and frame_dropper off.
  cpi->oxcf.aq_mode = 0;
