...
static vpx_codec_err_t set_encoder_config(
  ...
  // const int lossless_update = extra_cfg->lossless &&
  // oxcf->worst_allowed_q > 0;
  int sl, tl;
...
#if 0
      if (oxcf->pass != 2 && (lossless_update || level_update)) {
        fprintf(stderr,
                "Warning: lossless mode is disabled to target for level %d\n",
                oxcf->target_level);
      }
#endif
