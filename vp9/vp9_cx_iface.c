...
static vpx_codec_err_t vp9e_set_svc(vpx_codec_alg_priv_t *ctx, int ctr_id,
  ...
  // CBR mode for SVC with temporal and spatial layers is not yet supported.
  if (data == 1 &&
...
static vpx_codec_err_t vp9e_set_svc_layer_id(vpx_codec_alg_priv_t *ctx,
  ...
  cpi->svc.layer_id = cpi->svc.temporal_layer_id * cpi->oxcf.ss_number_layers +
