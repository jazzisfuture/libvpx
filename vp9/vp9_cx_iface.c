...
static vpx_codec_err_t encoder_init(vpx_codec_ctx_t *ctx,
  ...
  if (ctx->priv == NULL) {
    int i;
    vpx_codec_alg_priv_t *alg_priv = calloc(1, sizeof(*alg_priv));

    if (alg_priv == NULL)
      return VPX_CODEC_MEM_ERROR;

    ctx->priv = &alg_priv->base;
    ...
    for (i = 0;
         extracfg_map[i].usage && extracfg_map[i].usage !=
                                  alg_priv->cfg.g_usage;
         ++i) {}

    alg_priv->extra_cfg = extracfg_map[i].cfg;
    ...
    if (res == VPX_CODEC_OK) {
      ...
      cpi = vp9_create_compressor(&alg_priv->oxcf);
      if (cpi == NULL)
