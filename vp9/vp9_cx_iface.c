...
static vpx_codec_err_t set_encoder_config(
  ...
  int sl, tl;
  int ori_target_level = oxcf->target_level;
  ...
  if (get_vp9_level_index(oxcf->target_level) != -1) {
    double max_average_bitrate;
    int max_over_shoot_pct;
    const int target_level_index = get_vp9_level_index(oxcf->target_level);
    ...
    max_average_bitrate =
        vp9_level_defs[target_level_index].average_bitrate * (double)900;
    if ((double)oxcf->target_bandwidth > max_average_bitrate) {
      if (oxcf->pass != 2 && ori_target_level != oxcf->target_level)
        warn("target-bitrate is changed from %d kb/s to %d kb/s to target for "
            "level %d", (int)(oxcf->target_bandwidth / 1000),
    ...
    if (oxcf->over_shoot_pct > max_over_shoot_pct) {
      if (oxcf->pass != 2 && ori_target_level != oxcf->target_level)
      warn("overshoot-pct is changed from %d%% to %d%% to target for "
          "level %d",
...
static vpx_codec_err_t ctrl_get_level(vpx_codec_alg_priv_t *ctx,
                                      va_list args) {
  int *const arg = va_arg(args, int *);
  if (arg == NULL)
    return VPX_CODEC_INVALID_PARAM;
  *arg = (int)get_vp9_level(&ctx->cpi->common);
  return VPX_CODEC_OK;
}
