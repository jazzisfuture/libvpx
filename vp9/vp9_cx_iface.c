...
static void config_target_level(
    ...
    const struct vp9_extracfg *extra_cfg, const int level_update) {
...
static vpx_codec_err_t set_encoder_config(
  ...
  const int level_update = oxcf->target_level != extra_cfg->target_level;
  int sl, tl;
  ...
  if (get_level_index(oxcf->target_level) != -1)
    config_target_level(oxcf, cfg, extra_cfg, level_update);
  /*
