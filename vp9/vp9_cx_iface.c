...
static vpx_codec_err_t ctrl_set_aq_mode(vpx_codec_alg_priv_t *ctx,
  ...
  if (ctx->cpi->fixed_qp_onepass_rtc) extra_cfg.aq_mode = 0;
  return update_extra_cfg(ctx, &extra_cfg);
}
...
static vpx_codec_err_t ctrl_set_quantizer_one_pass_rtc(vpx_codec_alg_priv_t *ctx,
                                                       va_list args) {
  VP9_COMP *const cpi = ctx->cpi;
...
static vpx_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { VP9E_SET_QUANTIZER_ONE_PASS_RTC, ctrl_set_quantizer_one_pass_rtc },
