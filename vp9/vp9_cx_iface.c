...
static struct vp9_extracfg default_extra_cfg = {
  ...
  0,                     // render width
  0,                     // render height
  1,                     // newMT - Note: change the default to 0 later.
};
...
static vpx_codec_err_t validate_config(vpx_codec_alg_priv_t *ctx,
  ...
  RANGE_CHECK(extra_cfg, newMT, 0, 1);
...
static vpx_codec_err_t ctrl_set_newMT(vpx_codec_alg_priv_t *ctx, va_list args) {
  struct vp9_extracfg extra_cfg = ctx->extra_cfg;
  extra_cfg.newMT = CAST(VP9E_SET_NEW_MT, args);
  return update_extra_cfg(ctx, &extra_cfg);
}
...
static vpx_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  { VP9E_SET_NEW_MT, ctrl_set_newMT},
