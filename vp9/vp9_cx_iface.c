...
static vpx_codec_err_t ctrl_set_svc_parameters(vpx_codec_alg_priv_t *ctx,
  ...
  // Number of temporal layers and number of spatial layers have to be set properly
  // before calling this control function.
  for (sl = 0; sl < cpi->svc.number_spatial_layers; ++sl) {
    for (tl = 0; tl < cpi->svc.number_temporal_layers; ++tl) {
      const int layer =
          LAYER_IDS_TO_IDX(sl, cpi->svc.number_temporal_layers, tl);
      LAYER_CONTEXT *lc =
