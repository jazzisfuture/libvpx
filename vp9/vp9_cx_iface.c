...
static vpx_codec_err_t set_encoder_config(
  ...
  oxcf->temporal_layering_mode = (enum vp9e_temporal_layering_mode) cfg->temporal_layering_mode;
  ...
  for (tl = 0; tl < oxcf->ts_number_layers; ++tl) {
    oxcf->ts_rate_decimator[tl] = cfg->ts_rate_decimator[tl] ? cfg->ts_rate_decimator[tl] : 1;
  }
...
static vpx_codec_frame_flags_t get_frame_pkt_flags(const VP9_COMP *cpi,
  ...
  if (lib_flags & FRAMEFLAGS_KEY
      || (is_two_pass_svc(cpi) && cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers].is_key_frame)
      || (is_one_pass_cbr_svc(cpi) && cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers].is_key_frame)
        )
  ...
  if (res == VPX_CODEC_OK && cpi != NULL) {
    ...
    while (cx_data_sz >= ctx->cx_data_sz / 2 &&
      ...
      if (size) {
...
#if CONFIG_SPATIAL_SVC
        if (is_two_pass_svc(cpi) || is_one_pass_cbr_svc(cpi))
          cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers].layer_size += size;
#endif

        // Pack invisible frames with the next visible frame
        if (!cpi->common.show_frame
            || ((is_two_pass_svc(cpi) || is_one_pass_cbr_svc(cpi)) &&
        ...
        cx_data_sz -= size;
#if CONFIG_SPATIAL_SVC
        if ((is_two_pass_svc(cpi) || is_one_pass_cbr_svc(cpi)) && !ctx->output_cx_pkt_cb.output_cx_pkt) {
...
static vpx_codec_err_t ctrl_set_svc_parameters(vpx_codec_alg_priv_t *ctx,
  ...
  if(params->temporal_layering_mode == 3) {
    for (i = 0; i < VPX_MAX_LAYERS; ++i) {
      int sl = i/3;
      int tl = i%3;
      LAYER_CONTEXT *lc = &cpi->svc.layer_context[sl*3 + tl];
