...
struct vpx_codec_alg_priv {
  ...
  vpx_rational_t timestamp_ratio;
...
static INLINE void reduce_ratio(vpx_rational_t* ratio) {
...
static vpx_codec_err_t encoder_init(vpx_codec_ctx_t *ctx,
  ...
  if (ctx->priv == NULL) {
    ...
    if (res == VPX_CODEC_OK) {
      memcpy(&priv->timestamp_ratio, &priv->cfg.g_timebase, sizeof(priv->timestamp_ratio));
  ...
  switch (ctx->cfg.g_pass) {
    case VPX_RC_ONE_PASS:
      if (deadline > 0) {
        ...
        _Static_assert(TICKS_PER_SEC > 1000000 && (TICKS_PER_SEC % 1000000) == 0, "Unexpected TICKS_PER_SEC");
        const uint64_t duration_us = (uint64_t)duration *
                                     (uint64_t)ctx->timestamp_ratio.num /
...
static int64_t timebase_units_to_ticks(const vpx_rational_t *timestamp_ratio,
                                       int64_t n) {
  return n * timestamp_ratio->num / timestamp_ratio->den;
}

static int64_t ticks_to_timebase_units(const vpx_rational_t *timestamp_ratio,
                                       int64_t n) {
...
static vpx_codec_err_t encoder_encode(vpx_codec_alg_priv_t *ctx,
  ...
  const vpx_rational_t *const timestamp_ratio = &ctx->timestamp_ratio;
  size_t data_sz;
      ...
      if (size || (cpi->use_svc && cpi->svc.skip_enhancement_layer)) {
        ...
        if (!cpi->common.show_frame ||
          ...
          if (ctx->output_cx_pkt_cb.output_cx_pkt) {
            ...
            pkt.data.frame.pts =
                ticks_to_timebase_units(timestamp_ratio, dst_time_stamp) + ctx->pts_offset;
