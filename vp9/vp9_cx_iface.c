...
static vpx_codec_err_t ctrl_set_svc_frame_flags(vpx_codec_alg_priv_t *ctx,
                                                va_list args) {
  VP9_COMP *const cpi = ctx->cpi;
  vpx_svc_frame_flags_t *data = va_arg(args, vpx_svc_frame_flags_t *);
  int sl;
  for (sl = 0; sl < cpi->svc.number_spatial_layers; ++sl) {
...
static vpx_codec_ctrl_fn_map_t encoder_ctrl_maps[] = {
  ...
  {VP9E_SET_SVC_FRAME_FLAGS,          ctrl_set_svc_frame_flags},
