...
static vpx_codec_err_t encoder_encode(vpx_codec_alg_priv_t *ctx,
  ...
  if (res == VPX_CODEC_OK) {
    ...
    int64_t dst_time_stamp =
        timebase_units_to_ticks(timestamp_ratio, pts, &err);
    size_t size, cx_data_sz;
    unsigned char *cx_data;

    if (err != VPX_CODEC_OK) {
      vpx_internal_error(&cpi->common.error, err, "pts is too big");
    }
    cpi->svc.timebase_fac = timebase_units_to_ticks(timestamp_ratio, 1, NULL);
    ...
    if (img != NULL) {
      ...
      dst_end_time_stamp =
          timebase_units_to_ticks(timestamp_ratio, pts + duration, &err);
      if (err != VPX_CODEC_OK) {
        vpx_internal_error(&cpi->common.error, err,
          ...
          if (!cpi->common.show_frame ||
            ...
            if (ctx->output_cx_pkt_cb.output_cx_pkt) {
              ...
              pkt.data.frame.pts =
                  ...
                  ctx->pts_offset;
              duration_tmp = ticks_to_timebase_units(
