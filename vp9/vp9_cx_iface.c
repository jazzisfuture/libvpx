...
static void config_target_level(
    VP9EncoderConfig *oxcf, const vpx_codec_enc_cfg_t *cfg,
    const struct vp9_extracfg *extra_cfg, const int show_warning) {
  ...
  // Maximum target bitrate is level_limit * 80%.
  max_average_bitrate =
      vp9_level_defs[target_level_index].average_bitrate * 800.0;
  if ((double)oxcf->target_bandwidth > max_average_bitrate) {
    if (oxcf->pass != 2 && show_warning) {
      fprintf(stderr,
              "Warning: target-bitrate is changed from %d kb/s to %d kb/s to "
              "target for level %d\n",
  ...
  if (oxcf->over_shoot_pct > max_over_shoot_pct) {
    if (oxcf->pass != 2 && show_warning) {
      fprintf(stderr,
              "Warning: overshoot-pct is changed from %d to %d to target for "
              "level %d\n",
...
static vpx_codec_err_t set_encoder_config(
  ...
  const int level_config_show_warning =
  ...
  if (get_level_index(oxcf->target_level) != -1)
    config_target_level(oxcf, cfg, extra_cfg, level_config_show_warning);
  /*
...
static vpx_codec_err_t encoder_encode(vpx_codec_alg_priv_t *ctx,
  ...
  if (cpi->oxcf.pass == 2 && cpi->level_constraint.level_index != -1 &&
