...
static int mrc_decode(mrc_dec *br, const uint16_t f[],
                      unsigned ftb) {
  mrc_window diff;
  unsigned  r;
  unsigned  d;
  ...
  if (s < 0) {
    ...
    for (s = MRC_SHIFT_BITS(c); s >= 0; s -= 8) {
      if (buf >= end) {
        c += MRC_LOTS_OF_BITS;
        break;
      }
...
static int decode_bool(BOOL_DECODER *br, int prob) {
  unsigned short split = (MRC_TREE_FT * prob + 128) >> 8;
  mrc_window diff;
  unsigned  r;
  unsigned  d;
  int       c;
