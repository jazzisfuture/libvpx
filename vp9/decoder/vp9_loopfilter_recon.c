...
void vp9_loop_filter_setup_masks(VP9_COMMON *cm, int mi_row, int mi_col) {
  int lf_idx = ((mi_row + 7) >> 3) * ((cm->mi_cols + 7) >> 3) + ((mi_col + 7) >> 3);
  MODE_INFO **mi_8x8 = cm->mi_grid_visible + mi_row * cm->mode_info_stride;
  setup_mask(cm, mi_row, mi_col, mi_8x8 + mi_col, cm->mode_info_stride,
             &cm->lfms[lf_idx]);
}
...
void vp9_loop_filter_setup_masks_rs(VP9D_COMP *pbi,
                                    ...
                                    VP9_COMMON *cm, MACROBLOCKD *xd,
                                    int start, int stop, int y_only) {
  struct lf_setup_masks_param *params[MAX_CPU];
  struct task *tsks[MAX_CPU];
  int i;
  struct device *dev;
  int cpu_count;
  ...
  for (i = 0; i < cpu_count; i++) {
    tsks[i] = task_cache_get_task(pbi->lf_tsk_cache, NULL, 0);
    assert(tsks[i]);
    params[i] = lf_setup_masks_param_get(tsks[i]);
    assert(params[i]);
    params[i]->cm = cm;
...
static void vp9_loop_filter_rows_rs(const YV12_BUFFER_CONFIG *frame_buffer,
                             VP9D_COMP *pbi, VP9_COMMON *cm, MACROBLOCKD *xd,
                             int start, int stop, int y_only) {
  const int num_planes = y_only ? 1 : MAX_MB_PLANE;

  int y_offset = frame_buffer->y_buffer - frame_buffer->buffer_alloc;
  int u_offset = frame_buffer->u_buffer - frame_buffer->buffer_alloc;
  int v_offset = frame_buffer->v_buffer - frame_buffer->buffer_alloc;
  int y_stride = frame_buffer->y_stride;
  int uv_stride = frame_buffer->uv_stride;

  vp9_loop_filter_rows_work_rs(pbi->loop_filter_rs_handle,
                               start, stop,
                               num_planes,
                               cm->mi_rows,
                               cm->mi_cols,
...
void vp9_loop_filter_frame_rs(VP9D_COMP *pbi, VP9_COMMON *cm,
  ...
  int start_mi_row, end_mi_row, mi_rows_to_filter;
#if CONFIG_NON420
  vp9_loop_filter_frame(cm, &pbi->mb, pbi->common.lf.filter_level, y_only, partial);
  return;
#endif
  ...
  vp9_loop_filter_frame_init_wpp(cm, frame_filter_level);
  vp9_loop_filter_setup_masks_rs(pbi, cm->frame_to_show, cm, xd,
                                 start_mi_row, end_mi_row, y_only);
