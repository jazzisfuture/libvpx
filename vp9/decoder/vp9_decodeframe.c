...
static void process_block_void(TileWorkerData *twd, VP9Decoder *const pbi,
                               int mi_row, int mi_col, BLOCK_SIZE bsize,
                               int bwl, int bhl) {
  (void)twd;
  (void)pbi;
...
static void process_partition(TileWorkerData *twd, VP9Decoder *const pbi,
  ...
  if (!hbs) {
    ...
    xd->bmode_blocks_wl = 1 >> !!(partition & PARTITION_VERT);
    xd->bmode_blocks_hl = 1 >> !!(partition & PARTITION_HORZ);
    pbi->row_mt_worker_data->process_block[parse_recon_flag](
        twd, pbi, mi_row, mi_col, subsize, 1, 1);
  } else {
    switch (partition) {
      case PARTITION_NONE:
        pbi->row_mt_worker_data->process_block[parse_recon_flag](
            twd, pbi, mi_row, mi_col, subsize, n4x4_l2, n4x4_l2);
        break;
      case PARTITION_HORZ:
        pbi->row_mt_worker_data->process_block[parse_recon_flag](
            twd, pbi, mi_row, mi_col, subsize, n4x4_l2, n8x8_l2);
        if (has_rows)
          pbi->row_mt_worker_data->process_block[parse_recon_flag](
              twd, pbi, mi_row + hbs, mi_col, subsize, n4x4_l2, n8x8_l2);
        break;
      case PARTITION_VERT:
        pbi->row_mt_worker_data->process_block[parse_recon_flag](
            twd, pbi, mi_row, mi_col, subsize, n8x8_l2, n4x4_l2);
        if (has_cols)
          pbi->row_mt_worker_data->process_block[parse_recon_flag](
              twd, pbi, mi_row, mi_col + hbs, subsize, n8x8_l2, n4x4_l2);
        break;
      case PARTITION_SPLIT:
...
#if CONFIG_MULTITHREAD
  volatile int8_t *map;
  map = row_mt_worker_data->recon_map + idx;
  pthread_mutex_lock(&row_mt_worker_data->map_mutex);
  while (!(*map)) {
    pthread_mutex_unlock(&row_mt_worker_data->map_mutex);
    sched_yield();
    ...
    if (job.job_type == LPF_JOB) {
      ...
      if (setjmp(tile_data_recon->error_info.jmp)) {
        ...
        for (mi_col = mi_col_start; mi_col < mi_col_end;
          ...
          map_write(row_mt_worker_data, (r * sb_cols) + c);
        }
        if (is_last_row) {
          vp9_tile_done(pbi);
        }
      ...
      for (mi_col = mi_col_start; mi_col < mi_col_end;
        ...
        // Top Dependency
        if (r) {
          map_read(row_mt_worker_data, ((r - 1) * sb_cols) + c);
        }
        ...
        for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
          ...
          tile_data_recon->xd.plane[plane].dqcoeff =
              ...
              (sb_num << DQCOEFFS_PER_SB_LOG2);
        }
        tile_data_recon->xd.partition =
            row_mt_worker_data->partition + (sb_num * PARTITIONS_PER_SB);
        process_partition(tile_data_recon, pbi, mi_row, mi_col, BLOCK_64X64, 4,
                          RECON);
        if (cm->lf.filter_level && !cm->skip_loop_filter) {
          // Queue LPF_JOB
          int cur_sb_row = mi_row >> MI_BLOCK_SIZE_LOG2;
...
void vp9_decode_frame(VP9Decoder *pbi, const uint8_t *data,
  ...
  *cm->fc = cm->frame_contexts[cm->frame_context_idx];
  ...
  if (pbi->row_mt == 1) {
    pbi->row_mt_worker_data->process_block[0] = process_block_void;
    pbi->row_mt_worker_data->process_block[PARSE] = parse_block;
    pbi->row_mt_worker_data->process_block[RECON] = recon_block;
  }
