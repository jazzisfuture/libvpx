...
static void set_offsets_extend(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  set_mi_row_col(xd, tile, mi_row_ori, bh, mi_col_ori, bw, cm->mi_rows, cm->mi_cols);
}
...
static void set_skip_topblock(VP9_COMMON *const cm,  MACROBLOCKD *const xd,
                              BLOCK_SIZE bsize,
...
static void dec_predict_b_sub8x8_extend(VP9_COMMON *const cm, MACROBLOCKD *const xd,
                                        const TileInfo *const tile,
                                        int mi_row, int mi_col,
                                        int mi_row_ori, int mi_col_ori,
                                        BLOCK_SIZE top_bsize,
...
static void decode_partition(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  subsize = get_subsize(bsize, partition);
#if CONFIG_SUPERTX
  assert(!supertx_enabled ? read_token : 1);
  ...
  if (supertx_enabled && read_token) {
    ...
    set_mi_row_col(xd, tile, mi_row, num_8x8_blocks_high_lookup[bsize],
                   mi_col, num_8x8_blocks_wide_lookup[bsize], cm->mi_rows, cm->mi_cols);
    set_skip_context(xd, mi_row, mi_col);
  ...
  if (supertx_enabled && read_token) {
    ...
    set_skip_topblock(cm, xd, bsize, mi_row, mi_col, skip);
  }
  xd->mi = cm->mi_grid_visible + mi_row * cm->mi_stride + mi_col;
  xd->mi[0] = &cm->mi[mi_row * cm->mi_stride + mi_col];
#endif
