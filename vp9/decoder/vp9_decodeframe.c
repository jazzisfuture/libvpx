...
static void decode_block(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  vp9_read_mode_info(cm, xd, counts, tile, mi_row, mi_col, r);
...
static void decode_partition(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (subsize < BLOCK_8X8) {
    decode_block(cm, xd, counts, tile, mi_row, mi_col, r, subsize);
  } else {
    switch (partition) {
      case PARTITION_NONE:
        decode_block(cm, xd, counts, tile, mi_row, mi_col, r, subsize);
      ...
      case PARTITION_HORZ:
        ...
        if (mi_row + hbs < cm->mi_rows)
          decode_block(cm, xd, counts, tile, mi_row + hbs, mi_col, r, subsize);
        break;
      case PARTITION_VERT:
        decode_block(cm, xd, counts, tile, mi_row, mi_col, r, subsize);
        if (mi_col + hbs < cm->mi_cols)
          decode_block(cm, xd, counts, tile, mi_row, mi_col + hbs, r, subsize);
        break;
      case PARTITION_SPLIT:
        decode_partition(cm, xd, counts, tile, mi_row, mi_col, r, subsize);
        decode_partition(cm, xd, counts, tile, mi_row, mi_col + hbs, r,
                         subsize);
        decode_partition(cm, xd, counts, tile, mi_row + hbs, mi_col, r,
                         subsize);
        decode_partition(cm, xd, counts, tile, mi_row + hbs, mi_col + hbs, r,
                         subsize);
        break;
      default:
    ...
    for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
      ...
      for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
        ...
        for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
          ...
          decode_partition(tile_data->cm, &tile_data->xd, &cm->counts, &tile, mi_row, mi_col,
...
static int tile_worker_hook(TileWorkerData *const tile_data,
  ...
  for (mi_row = tile->mi_row_start; mi_row < tile->mi_row_end;
    ...
    for (mi_col = tile->mi_col_start; mi_col < tile->mi_col_end;
         mi_col += MI_BLOCK_SIZE) {
      decode_partition(tile_data->cm, &tile_data->xd, &tile_data->cm->counts,
