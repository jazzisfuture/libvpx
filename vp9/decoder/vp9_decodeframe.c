...
#if CONFIG_NEW_QUANT
  xd->plane[0].dequant_val = cm->y_dequant_val[q_index];
#endif  // CONFIG_NEW_QUANT

  for (i = 1; i < MAX_MB_PLANE; i++) {
    xd->plane[i].dequant = cm->uv_dequant[q_index];
#if CONFIG_NEW_QUANT
    xd->plane[1].dequant_val = cm->uv_dequant_val[q_index];
#endif  // CONFIG_NEW_QUANT
  }
}
...
void vp9_init_dequantizer(VP9_COMMON *cm) {
  ...
  for (q = 0; q < QINDEX_RANGE; q++) {
    ...
    vp9_get_dequant_val_nuq(
        cm->y_dequant[q][0], 0, cm->y_dequant_val_nuq[q][0]);
    vp9_get_dequant_val_nuq(
        cm->uv_dequant[q][1], 1, cm->y_dequant_val_nuq[q][1]);
    vp9_get_dequant_val_nuq(
        cm->y_dequant[q][0], 0, cm->uv_dequant_val_nuq[q][0]);
    vp9_get_dequant_val_nuq(
        cm->uv_dequant[q][1], 1, cm->uv_dequant_val_nuq[q][1]);
#endif  // CONFIG_NEW_QUANT
  }
}
