...
static int tile_worker_hook(TileWorkerData *const tile_data, void *unused) {
  ...
  do {
    int mi_row, mi_col;
    TileBuffer *const buf = tile_data->buffers + n;
    vp9_zero(tile_data->dqcoeff);
    vp9_tile_init(&tile_data->xd.tile, &pbi->common, 0, buf->col);
...
static const uint8_t *decode_tiles_mt(VP9Decoder *pbi,
  ...
  worker0_data = (TileWorkerData*)pbi->tile_workers[0].data1;
  get_tile_buffers(pbi, data, data_end, tile_cols, tile_rows,
                   &worker0_data->buffers);

  // Sort the buffers based on size in descending order.
  qsort(worker0_data->buffers, tile_cols, sizeof(worker0_data->buffers[0]),
        compare_tile_buffers);

  // Interleave the tiles to distribute the load between threads, assuming a
  // larger tile implies it's more difficult to decode.
  {
    int i;
    int start = 1, end = tile_cols - 1;
    TileBuffer tmp;

    while (start < end) {
      tmp = worker0_data->buffers[start];
      worker0_data->buffers[start] = worker0_data->buffers[end];
      worker0_data->buffers[end] = tmp;
      start += 2;
      end -= 2;
    }
    ...
    for (i = 1; i < num_workers; ++i) {
      ...
      memcpy(tile_data->buffers, worker0_data->buffers,
             sizeof(tile_data->buffers));
    }
  }
