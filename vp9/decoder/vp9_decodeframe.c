...
static const uint8_t *decode_tiles(VP9Decoder *pbi,
  ...
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    ...
    for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
      ...
      // After loopfiltering, the last 7 row pixels in last superblock row may
      // still be changed by the longest loopfilter of current superblock row.
      // TODO(hkuang): only the last 7 pixels may be changed by next row's
      // loopfilter. But only 18 could pass all the unit test now.
      if (pbi->frame_parallel_decode)
        vp9_frameworker_broadcast(pbi->cur_buf, (mi_row  << 3) - 18);
    }
  }
...
void dec_build_inter_predictors(VP9Decoder * const pbi, MACROBLOCKD *xd,
  ...
  for (ref = 0; ref < 1 + is_compound; ++ref) {
    ...
    if (scaled_mv.col || scaled_mv.row ||
      ...
      if (pbi->frame_parallel_decode)
        vp9_frameworker_wait(pbi->owner_frame_worker, ref_buf, y1 - 1);
