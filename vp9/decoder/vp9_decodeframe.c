...
#if CONFIG_BDINTRA
          if (xd->force_dct)
            vp9_idst4x4_add(dqcoeff, dst, stride);
          else
#endif  // CONFIG_BDINTRA
            vp9_iht4x4_add(tx_type, dqcoeff, dst, stride, eob);
          break;
...
#if CONFIG_BDINTRA
          if (xd->force_dct)
            vp9_idst8x8_add(dqcoeff, dst, stride);
          else
#endif  // CONFIG_BDINTRA
            vp9_iht8x8_add(tx_type, dqcoeff, dst, stride, eob);
          break;
...
#if CONFIG_BDINTRA
          if (xd->force_dct)
            vp9_idst16x16_add(dqcoeff, dst, stride);
          else
#endif  // CONFIG_BDINTRA
            vp9_iht16x16_add(tx_type, dqcoeff, dst, stride, eob);
          break;
...
#if CONFIG_BDINTRA
    // Do not reset decoeff buffer, becase we are going to use them in the
    // final round of decoding.
#else
    if (eob == 1) {
...
static void predict_and_reconstruct_intra_block(int plane, int block,
  ...
  if (num_4x4_blocks_high_lookup[plane_bsize] == (1 << tx_size) + y)
    xd->use_bdi[0] = 1;

  // If the width of block is equal to transform size, we may be able to
  // use the right boundary.
  if (num_4x4_blocks_wide_lookup[plane_bsize] == (1 << tx_size) + x)
    xd->use_bdi[1] = 1;

  // For inter frame, only do intra prediction in the
  // final round (xd->bdintra_pass == 1).
  if (frame_is_intra_only(cm) || xd->bdintra_pass == 1)
#endif  // CONFIG_BDINTRA
  ...
  if (!mi->mbmi.skip) {
    const int eob = vp9_decode_block_tokens(cm, xd, plane, block,
                                            ...
                                            args->r);
#if CONFIG_BDINTRA
    // For inter frame, only do intra reconstruction in the
    // final round (xd->bdintra_pass == 1).
    if (frame_is_intra_only(cm) || xd->bdintra_pass == 1)
#endif  // CONFIG_BDINTRA
...
#if CONFIG_BDINTRA
  xd->force_dct = 0;
  // For inter frame, only do inter reconstruction in the
  // first round (xd->bdintra_pass == 0).
...
#if CONFIG_BDINTRA
  for (i = 0; i < MAX_MB_PLANE; ++i) {
    xd->plane[i].dqcoeff += 16 * num_4x4_blocks_wide_lookup[bsize] *
...
static void decode_partition(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (!frame_is_intra_only(cm)) {
    if (xd->bdintra_pass == 0) {
      ...
      xd->partition_history[xd->partition_index] = partition;
      xd->partition_index++;
    } else {
      partition = xd->partition_history[xd->partition_index];
      xd->partition_index++;
    }
  } else {
    partition = read_partition(cm, xd, hbs, mi_row, mi_col, bsize, r);
          ...
          for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
            ...
            if (!frame_is_intra_only(cm)) {
              ...
              xd->eob_index = 0;
              {
                ...
                xd->allow_bdintra = cm->base_qindex <= BDINTRA_Q_THRESH;
              }
              decode_partition(tile_data->cm, &tile_data->xd, &tile,
#if CONFIG_SUPERTX
                               0,
