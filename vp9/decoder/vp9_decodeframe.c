...
#if 0
y_dequant[0] = xd->y_dequant[segment_id][0];
y_dequant[1] = xd->y_dequant[segment_id][1];
uv_dequant[0] = xd->uv_dequant[segment_id][0];
uv_dequant[1] = xd->uv_dequant[segment_id][1];
#endif
...
static void predict_and_reconstruct_intra_block(int plane, int block,
  ...
  const PREDICTION_MODE mode = (plane == 0) ? get_y_mode(mi, block)
                                            : mi->mbmi.uv_mode;
  const int16_t *const dequant = (plane == 0) ? xd->y_dequant[args->seg_id]
                                              : xd->uv_dequant[args->seg_id];
  int x, y;
  uint8_t *dst;
...
static void decode_block(VP9Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  if (!is_inter_block(mbmi)) {
    struct intra_args arg = {cm, xd, counts, r , seg_id};
    ...
    if (!mbmi->skip) {
      int eobtotal = 0;
      struct inter_args arg = {cm, xd, r, counts, &eobtotal, seg_id};
...
static void setup_quantization(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  // When segmentation is disabled, only the first value is used.  The remaining
  // are don't cares.
  ...
  // Override first value and fill in the remaining.
  if (cm->seg.enabled) {
    int i;
    for (i = 0; i < MAX_SEGMENTS; ++i) {
      qindex = vp9_get_qindex(&cm->seg, 0, cm->base_qindex);
      xd->y_dequant[i][0] = vp9_dc_quant(qindex, cm->y_dc_delta_q,
                                         cm->bit_depth);
      xd->y_dequant[i][1] = vp9_ac_quant(qindex, 0, cm->bit_depth);
      xd->uv_dequant[i][0] = vp9_dc_quant(qindex, cm->uv_dc_delta_q,
                                          cm->bit_depth);
      xd->uv_dequant[i][1] = vp9_ac_quant(qindex, cm->uv_ac_delta_q,
                                          cm->bit_depth);
    }
  }
