...
static void setup_segmentation(struct segmentation *seg,
  ...
  if (seg->update_map) {
    for (i = 0; i < SEG_TREE_PROBS; i++)
      seg->tree_probs[i] = vp9_rb_read_bit(rb) ? vp9_rb_read_literal_fast(rb, 8)
                                               : MAX_PROB;

    seg->temporal_update = vp9_rb_read_bit(rb);
    if (seg->temporal_update) {
      for (i = 0; i < PREDICTION_PROBS; i++)
        seg->pred_probs[i] = vp9_rb_read_bit(rb) ? vp9_rb_read_literal_fast(rb, 8)
                                                 : MAX_PROB;
    } else {
      for (i = 0; i < PREDICTION_PROBS; i++)
...
static void setup_loopfilter(struct loopfilter *lf,
                             struct vp9_read_bit_buffer *rb) {
  lf->filter_level = vp9_rb_read_literal_fast(rb, 6);
...
static void setup_quantization(VP9_COMMON *const cm, MACROBLOCKD *const xd,
                               struct vp9_read_bit_buffer *rb) {
  cm->base_qindex = vp9_rb_read_literal_fast(rb, QINDEX_BITS);
...
void vp9_read_frame_size(struct vp9_read_bit_buffer *rb,
  ...
  *width = vp9_rb_read_literal_fast(rb, 16) + 1;
  *height = vp9_rb_read_literal_fast(rb, 16) + 1;
}
...
int vp9_read_sync_code(struct vp9_read_bit_buffer *const rb) {
  return vp9_rb_read_literal_fast(rb, 8) == VP9_SYNC_CODE_0 &&
         vp9_rb_read_literal_fast(rb, 8) == VP9_SYNC_CODE_1 &&
         vp9_rb_read_literal_fast(rb, 8) == VP9_SYNC_CODE_2;
}
...
static size_t read_uncompressed_header(VP9Decoder *pbi,
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    if (cm->intra_only) {
      ...
      pbi->refresh_frame_flags = vp9_rb_read_literal_fast(rb, REF_FRAMES);
      setup_frame_size(cm, rb);
      if (pbi->need_resync) {
        vpx_memset(&cm->ref_frame_map, -1, sizeof(cm->ref_frame_map));
        pbi->need_resync = 0;
  ...
  sz = vp9_rb_read_literal_fast(rb, 16);
