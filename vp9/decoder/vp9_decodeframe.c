  ...
  if (!is_inter_block(mi)) {
    ...
    if (!mi->skip) {
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        ...
        for (row = 0; row < max_blocks_high; row += step)
          for (col = 0; col < max_blocks_wide; col += step)
            eobtotal += reconstruct_inter_block_row_mt(twd, mi, plane, row, col,
                                                       tx_size);
      }
...
static void map_write(VP9Decoder *pbi, int idx, int flag) {
#if CONFIG_MULTITHREAD
  pbi->recon_map[idx] |= flag;
#else
  (void)pbi;
  (void)idx;
#endif
...
static void map_read(VP9Decoder *pbi, int idx, int flag) {
#if CONFIG_MULTITHREAD
  volatile int8_t *map;
...
static int lpf_map_write_check(VP9LfSync *lf_sync, int row, int num_tile_cols) {
  ...
  lf_sync->num_tiles_in_frame_row_done[row] += 1;
  if (num_tile_cols == lf_sync->num_tiles_in_frame_row_done[row])
    return_val = 1;
...
int row_decode_worker_hook(ThreadData *const thread_data, uint8_t **data_end) {
  ...
  while (!vp9_jobq_dequeue(&pbi->jobq, &job, sizeof(job), 1)) {
    ...
    if (job.job_type == LPF_JOB) {
      ...
      int is_last_row = ((sb_rows - 1) == (cur_sb_row)) ? 1 : 0;
      ...
      if (setjmp(tile_data_recon->error_info.jmp)) {
        for (mi_col = mi_col_start; mi_col < mi_col_end;
          ...
          map_write(pbi, (r * sb_cols) + c, RECON_DONE);
        }
      ...
      for (mi_col = mi_col_start; mi_col < mi_col_end;
        ...
        // Top Dependency
        if (r) {
          map_read(pbi, ((r - 1) * sb_cols) + c, RECON_DONE);
        }
      ...
      if (cm->lf.filter_level && !cm->skip_loop_filter) {
        ...
        /* Checks if this row has been decoded across the entire frame
         * (in all tiles)
         */
        ...
        if (is_lpf_job_ready) {
          ...
          tmp_job.job_type = LPF_JOB;
          if (cur_sb_row > 0) {
            if (cur_sb_row > 1) {
              map_read(pbi, ((cur_sb_row - 1) * sb_cols) - 1, LPF_QUEUED);
            }

            tmp_job.row_num = mi_row - MI_SIZE;
            vp9_jobq_queue(&pbi->jobq, &tmp_job, sizeof(tmp_job));
            map_write(pbi, ((cur_sb_row)*sb_cols) - 1, LPF_QUEUED);
          }

          if (is_last_row) {
            tmp_job.row_num = mi_row;
            vp9_jobq_queue(&pbi->jobq, &tmp_job, sizeof(tmp_job));
            map_write(pbi, ((cur_sb_row + 1) * sb_cols) - 1, LPF_QUEUED);
          }
        }
      }
      ...
      /* Queue in the recon_job for this row */
      {
        ...
        vp9_jobq_queue(&pbi->jobq, &recon_job, sizeof(recon_job));
      }
...
static const uint8_t *decode_tiles_row_mt(VP9Decoder *pbi, const uint8_t *data,
                                          const uint8_t *data_end) {
  VP9_COMMON *const cm = &pbi->common;
  const VPxWorkerInterface *const winterface = vpx_get_worker_interface();
  const int aligned_cols = mi_cols_aligned_to_sb(cm->mi_cols);
    ...
    for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
      ...
      for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
        ...
        for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
          ...
          int plane;
          for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
            tile_data->xd.plane[plane].eob = pbi->eob[plane];
            tile_data->xd.plane[plane].dqcoeff = pbi->dqcoeff[plane];
          }
          tile_data->xd.partition = pbi->partition;
          parse_partition(tile_data, pbi, mi_row, mi_col, BLOCK_64X64, 4);
...
const uint8_t *decode_tiles_multi_row(VP9Decoder *pbi, const uint8_t *data,
  ...
  if (pbi->thread_data == NULL) {
    const size_t thread_size = pbi->max_threads * sizeof(*pbi->thread_data);
    CHECK_MEM_ERROR(cm, pbi->thread_data, vpx_memalign(32, thread_size));
  }

  // Initialize LPF
  if (cm->lf.filter_level && !cm->skip_loop_filter) {
    vp9_lpf_mt_init(cm, lf_row_sync, cm->lf.filter_level,
                    pbi->num_tile_workers);
  }
  ...
  for (n = 0; n < num_workers; ++n) {
    VPxWorker *const worker = &pbi->tile_workers[n];
    ThreadData *const thread_data = &pbi->thread_data[n];
...
void vp9_decode_frame(VP9Decoder *pbi, const uint8_t *data,
  ...
  *cm->fc = cm->frame_contexts[cm->frame_context_idx];
  ...
  if (pbi->max_threads > 1 && tile_rows == 1 &&
    ...
    if (pbi->row_mt == 1) {
      *p_data_end =
          decode_tiles_row_mt(pbi, data + first_partition_size, data_end);
    } else {
