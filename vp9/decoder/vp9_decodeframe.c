...
static INLINE void dec_txfrm_block_to_raster_xy(int bw,
...
static void predict_and_reconstruct_intra_block(int plane, int block,
  ...
  vp9_predict_intra_block(xd, block >> (tx_size << 1),
                          dec_b_width_log2_lookup[bw], tx_size, mode,
...
static void dec_build_inter_predictors_sb(VP9Decoder *const pbi,
  ...
  for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
    ...
    const int num_4x4_w = (bw << 1) >> pd->subsampling_x;
    const int num_4x4_h = (bh << 1) >> pd->subsampling_y;
...
static INLINE TX_SIZE dec_get_uv_tx_size(const MB_MODE_INFO *mbmi,
                                     const struct macroblockd_plane *pd,
                                     int bw, int bh) {
  const int x = MIN(bw >> pd->subsampling_x,
                    bh >> pd->subsampling_y);
  if (!x) {
    return TX_4X4;
  } else {
...
static void dec_foreach_transformed_block_in_plane(
    const MACROBLOCKD *const xd, BLOCK_SIZE bsize, int bw, int bh,
    int plane,
  ...
  const TX_SIZE tx_size = plane ? dec_get_uv_tx_size(mbmi, pd, bw, bh)
                                : mbmi->tx_size;
  ...
  for (r = 0; r < max_blocks_high; r += (1 << tx_size)) {
    for (c = 0; c < num_4x4_w; c += (1 << tx_size)) {
      ...
      if (c < max_blocks_wide)
        visit(plane, i, plane_bsize, (num_4x4_w >> 1), tx_size, arg);
      i += step;
    }
  }
}
...
static void dec_foreach_transformed_block(const MACROBLOCKD* const xd,
  ...
  for (plane = 0; plane < MAX_MB_PLANE; ++plane)
    dec_foreach_transformed_block_in_plane(xd, bsize, bw, bh, plane, visit,
                                           arg);
}
...
static void decode_block(VP9Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  if (!is_inter_block(mbmi)) {
    ...
    dec_foreach_transformed_block(xd, bsize, bw, bh,
                                  predict_and_reconstruct_intra_block, &arg);
  } else {
    // Prediction
    dec_build_inter_predictors_sb(pbi, xd, mi_row, mi_col, bw, bh);
...
// temp copy
const int dec_mi_width_log2_lookup[BLOCK_SIZES] =
...
static INLINE int dec_partition_plane_context(const MACROBLOCKD *xd,
  ...
  // TODO(slavarnway): remove lookup
//  const int bsl = dec_mi_width_log2_lookup[bsize];
//  const int bsl = (bw >> 1) - ((bw & 0x8) >> 3);
