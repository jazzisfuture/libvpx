...
static const uint8_t *decode_tiles(VP9Decoder *pbi,
  ...
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    ...
    for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
      ...
      // still be changed by the longest loopfilter of the current superblock
      // row.
      // TODO(hkuang): only the last 7 pixels may be changed by next row's
      // loopfilter. But only 13 could pass all the unit test now.
      if (pbi->frame_parallel_decode)
        vp9_frameworker_broadcast(pbi->cur_buf, (mi_row  << 3) - 13);
    }
  }
