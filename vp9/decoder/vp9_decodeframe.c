  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      ...
      if (mi_row + hbs < cm->mi_rows && bsize > BLOCK_8X8) {
        dec_predict_b_simple(cm, xd, tile, mi_row + hbs,
                             mi_col, subsize, topbsize);
    ...
    case PARTITION_VERT:
      ...
      if (mi_col + hbs < cm->mi_cols && bsize > BLOCK_8X8) {
        dec_predict_b_simple(cm, xd, tile, mi_row,
                             mi_col + hbs, subsize, topbsize);
        for (i = 0; i < MAX_MB_PLANE; i++) {
          vp9_smooth_inter_predictor_simple(cm, xd, i, &xd->plane[i],
                                            mi_row, mi_col, bsize,
                                            PARTITION_VERT);
    ...
    case PARTITION_SPLIT:
      if (bsize != BLOCK_8X8) {
        ...
        if (mi_row < cm->mi_rows && mi_col + hbs < cm->mi_cols)
          dec_predict_sb_simple(cm, xd, tile, mi_row,
                                mi_col + hbs, subsize, topbsize);
        if (mi_row + hbs < cm->mi_rows && mi_col < cm->mi_cols)
          dec_predict_sb_simple(cm, xd, tile, mi_row + hbs,
                                mi_col, subsize, topbsize);
