...
void vp9_loop_filter_rows_mt(VP9D_COMP *pbi,
  ...
  int *cur_row_sb_col;
  const int nsync = pbi->lf_row_sync.sync_range;
#endif
  ...
  for (r = start; r < stop; r += pbi->num_tile_workers) {
...
#if CONFIG_MULTITHREAD
    if (r > 0)
      last_row_sb_col = &pbi->lf_row_sync.cur_sb_col[r - 1];

    cur_row_sb_col = &pbi->lf_row_sync.cur_sb_col[r];
#endif
    ...
    for (c = 0; c < sb_cols; ++c) {
      ...
      if (r && !(c & (nsync - 1))) {
        ...
        for (i = 0; i < 4000; ++i)
          if (!pthread_mutex_trylock(&(pbi->lf_row_sync.mutex_[r - 1]))) {
            locked = 1;
            break;
          }

        if (!locked)
          pthread_mutex_lock(&(pbi->lf_row_sync.mutex_[r - 1]));

        while (c > (*last_row_sb_col - nsync)) {
          pthread_cond_wait(&(pbi->lf_row_sync.cond_[r - 1]),
                            &(pbi->lf_row_sync.mutex_[r - 1]));
        }
        pthread_mutex_unlock(&(pbi->lf_row_sync.mutex_[r - 1]));
      }
#endif
...
#if CONFIG_MULTITHREAD
      {
        ...
        pthread_mutex_lock(&(pbi->lf_row_sync.mutex_[r]));
        *cur_row_sb_col = cur;
        if (sig)
          pthread_cond_signal(&(pbi->lf_row_sync.cond_[r]));
        pthread_mutex_unlock(&(pbi->lf_row_sync.mutex_[r]));
      }
#endif
    }
  }
