...
static INLINE int call_predict_recon_inter(MACROBLOCKD *xd, MODE_INFO *mi,
                                           int plane, TileWorkerData *twd,
...
static INLINE void call_predict_recon_intra(MACROBLOCKD *xd, MODE_INFO *mi,
                                            int plane, TileWorkerData *twd,
...
static void decode_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  if (!is_inter_block(mi)) {
    int plane;
    for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
      call_predict_recon_intra(xd, mi, plane, twd,
    ...
    if (!mi->skip) {
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        call_predict_recon_inter(xd, mi, plane, twd, reconstruct_inter_block);
      }
...
static void recon_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  if (!is_inter_block(mi)) {
    ...
    if (!mi->skip) {
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        call_predict_recon_inter(xd, mi, plane, twd,
...
static void parse_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  if (!is_inter_block(mi)) {
    int plane;
    for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
      call_predict_recon_intra(xd, mi, plane, twd, parse_intra_block_row_mt);
    }
  } else {
    if (!mi->skip) {
      int eobtotal = 0;
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        eobtotal += call_predict_recon_inter(xd, mi, plane, twd,
