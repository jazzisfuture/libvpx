...
static void read_bitdepth_colspace_sampling(VP9_COMMON *cm,
                                            struct vp9_read_bit_buffer *rb) {
...
static size_t read_uncompressed_header(VP9Decoder *pbi,
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    read_bitdepth_colspace_sampling(cm, rb);
    ...
    if (cm->intra_only) {
      ...
      if (cm->profile > PROFILE_0) {
        ...
        // either the color format or color sub-sampling in profile 0. VP9 specifies
        // that the default color space should be YUV 4:2:0 in this case (normative).
