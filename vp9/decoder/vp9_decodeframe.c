...
void vp9_init_dequantizer(VP9_COMMON *cm) {
  ...
  for (q = 0; q < QINDEX_RANGE; q++) {
    ...
    vp9_get_dequant_val_nuq(
        cm->y_dequant[q][0], 0, cm->y_dequant_val_nuq[q][0], NULL);
    vp9_get_dequant_val_nuq(
        cm->y_dequant[q][1], 1, cm->y_dequant_val_nuq[q][1], NULL);
    vp9_get_dequant_val_nuq(
        cm->uv_dequant[q][0], 0, cm->uv_dequant_val_nuq[q][0], NULL);
    vp9_get_dequant_val_nuq(
        cm->uv_dequant[q][1], 1, cm->uv_dequant_val_nuq[q][1], NULL);
#endif  // CONFIG_NEW_QUANT
  }
}
