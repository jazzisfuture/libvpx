...
static void setup_frame_size_with_refs(VP9_COMMON *cm,
  ...
  // dimensions.
  for (i = 0; i < REFS_PER_FRAME; ++i) {
    const YV12_BUFFER_CONFIG *const ref_buf = cm->frame_refs[i].buf;
    const int ref_width = ref_buf->y_width;
    const int ref_height = ref_buf->y_height;

    if (!valid_ref_size(width, height, ref_width, ref_height))
