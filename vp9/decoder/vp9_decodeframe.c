...
void recon_tile_row(TileWorkerData *tile_data, VP9Decoder *pbi, int mi_row,
...
void parse_tile_row(TileWorkerData *tile_data, VP9Decoder *pbi, int mi_row,
...
static int row_decode_worker_hook(ThreadData *const thread_data,
  ...
  RowMTWorkerData *const row_mt_worker_data = pbi->row_mt_worker_data;
  const int aligned_cols = mi_cols_aligned_to_sb(cm->mi_cols);
  volatile int sb_cols = aligned_cols >> MI_BLOCK_SIZE_LOG2;
  const int aligned_rows = mi_cols_aligned_to_sb(cm->mi_rows);
  const int sb_rows = aligned_rows >> MI_BLOCK_SIZE_LOG2;
  Job job;
