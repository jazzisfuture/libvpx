...
static void map_write(RowMtHandle *row_mt_handle, int idx) {
#if CONFIG_MULTITHREAD
  pthread_mutex_lock(&row_mt_handle->map_mutex);
  row_mt_handle->recon_map[idx] = 1;
  pthread_mutex_unlock(&row_mt_handle->map_mutex);
#else
  (void)row_mt_handle;
  (void)idx;
#endif
...
static void map_read(RowMtHandle *row_mt_handle, int idx) {
#if CONFIG_MULTITHREAD
  volatile int8_t *map;
  map = row_mt_handle->recon_map + idx;
  pthread_mutex_lock(&row_mt_handle->map_mutex);
  while (!(*map)) {
    pthread_mutex_unlock(&row_mt_handle->map_mutex);
    sched_yield();
...
#if CONFIG_MULTITHREAD
  int terminate;
  RowMtHandle *const row_mt_handle = pbi->row_mt_handle;
  const int all_parse_done = (1 << pbi->common.log2_tile_cols);
  pthread_mutex_lock(&row_mt_handle->recon_mutex);
  row_mt_handle->num_tiles_done++;
  terminate = all_parse_done == row_mt_handle->num_tiles_done;
  pthread_mutex_unlock(&row_mt_handle->recon_mutex);
  if (terminate) {
    vp9_jobq_terminate(&row_mt_handle->jobq);
  }
#else
  (void)pbi;
#endif
...
void vp9_jobq_alloc(VP9Decoder *pbi) {
  ...
  if (jobq_size > row_mt_handle->jobq_size) {
    vpx_free(row_mt_handle->jobq_buf);
    CHECK_MEM_ERROR(cm, row_mt_handle->jobq_buf, vpx_calloc(1, jobq_size));
    vp9_jobq_init(&row_mt_handle->jobq, row_mt_handle->jobq_buf, jobq_size);
    row_mt_handle->jobq_size = jobq_size;
  }
}

int row_decode_worker_hook(ThreadData *const thread_data, uint8_t **data_end) {
  ...
  while (!vp9_jobq_dequeue(&row_mt_handle->jobq, &job, sizeof(job), 1)) {
    int mi_col;
    ...
    if (job.job_type == LPF_JOB) {
      ...
      if (setjmp(tile_data_recon->error_info.jmp)) {
        ...
        for (mi_col = mi_col_start; mi_col < mi_col_end;
          ...
          map_write(row_mt_handle, (r * sb_cols) + c);
        }
        if (is_last_row) {
          vp9_tile_done(pbi);
        }
      ...
      for (mi_col = mi_col_start; mi_col < mi_col_end;
        ...
        // Top Dependency
        if (r) {
          map_read(row_mt_handle, ((r - 1) * sb_cols) + c);
        }

        for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
          tile_data_recon->xd.plane[plane].eob =
              row_mt_handle->eob[plane] + (sb_num << EOBS_PER_SB_LOG2);
          tile_data_recon->xd.plane[plane].dqcoeff =
              row_mt_handle->dqcoeff[plane] + (sb_num << DQCOEFFS_PER_SB_LOG2);
        }
        tile_data_recon->xd.partition =
            row_mt_handle->partition + (sb_num * PARTITIONS_PER_SB);
          ...
          if (mi_col + MI_BLOCK_SIZE >= mi_col_end) {
            ...
            if (is_lpf_job_ready) {
              ...
              if (cur_sb_row > 0) {
                ...
                vp9_jobq_queue(&row_mt_handle->jobq, &lpf_job, sizeof(lpf_job));
              }
              if (is_last_row) {
      ...
      TileInfo *tile;
      TileBuffer *const buf = &pbi->tile_buffers[job.tile_col];
      ...
      /* Queue in the recon_job for this row */
      {
        ...
        vp9_jobq_queue(&row_mt_handle->jobq, &recon_job, sizeof(recon_job));
      }
      ...
      if (mi_row + MI_BLOCK_SIZE < cm->mi_rows) {
        ...
        vp9_jobq_queue(&row_mt_handle->jobq, &parse_job, sizeof(parse_job));
      }
    }
  }
...
static const uint8_t *decode_tiles_split_parse_recon(VP9Decoder *pbi,
                                                     const uint8_t *data,
                                                     const uint8_t *data_end) {
  VP9_COMMON *const cm = &pbi->common;
  RowMtHandle *const row_mt_handle = pbi->row_mt_handle;
    ...
    for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
      ...
      for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
        ...
        for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
          ...
          int plane;
          for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
            tile_data->xd.plane[plane].eob = row_mt_handle->eob[plane];
            tile_data->xd.plane[plane].dqcoeff = row_mt_handle->dqcoeff[plane];
          }
          tile_data->xd.partition = row_mt_handle->partition;
          parse_partition(tile_data, pbi, mi_row, mi_col, BLOCK_64X64, 4);
...
const uint8_t *decode_tiles_row_wise_mt(VP9Decoder *pbi, const uint8_t *data,
  ...
  memset(row_mt_handle->recon_map, 0,
         sb_rows * sb_cols * sizeof(*row_mt_handle->recon_map));
  ...
  if (pbi->row_mt_handle->thread_data == NULL) {
    const size_t thread_size =
        pbi->max_threads * sizeof(*pbi->row_mt_handle->thread_data);
    CHECK_MEM_ERROR(cm, pbi->row_mt_handle->thread_data,
  ...
  for (n = 0; n < num_workers; ++n) {
    VPxWorker *const worker = &pbi->tile_workers[n];
    ThreadData *const thread_data = &pbi->row_mt_handle->thread_data[n];
    ...
    worker->data2 = &row_mt_handle->data_end;
  }
  ...
  vp9_jobq_reset(&row_mt_handle->jobq);
  row_mt_handle->num_tiles_done = 0;

  vp9_reset_lfm(cm);
  row_mt_handle->data_end = NULL;
...

  {
    ...
    row_mt_handle->data_end = vpx_reader_find_end(&tile_data->bit_reader);
  }
...
static size_t read_uncompressed_header(VP9Decoder *pbi,
  ...
  if (pbi->row_mt == 1) {
    ...
    if (pbi->row_mt_handle == NULL) {
      CHECK_MEM_ERROR(cm, pbi->row_mt_handle,
                      vpx_calloc(1, sizeof(*pbi->row_mt_handle)));
#if CONFIG_MULTITHREAD
      pthread_mutex_init(&pbi->row_mt_handle->recon_mutex, NULL);
      pthread_mutex_init(&pbi->row_mt_handle->map_mutex, NULL);
#endif
    }
    ...
    if (num_sbs > pbi->row_mt_handle->num_sbs) {
      vp9_dec_free_row_mt_mem(pbi->row_mt_handle);
      if (vp9_dec_alloc_row_mt_mem(pbi->row_mt_handle, cm, num_sbs)) {
        vpx_internal_error(
  ...
  if (pbi->max_threads > 1 && tile_rows == 1 &&
    ...
    if (pbi->row_mt == 1) {
      ...
      *p_data_end = decode_tiles_mt(pbi, data + first_partition_size, data_end);
      if (!pbi->lpf_mt_opt && !xd->corrupted) {
    ...
    if (pbi->row_mt == 1) {
      *p_data_end = decode_tiles_split_parse_recon(
