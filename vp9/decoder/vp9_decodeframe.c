...
static int reconstruct_inter_block(TileWorkerData *twd, MACROBLOCKD *const xd,
  ...
  if (!is_inter_block(mi)) {
    ...
    for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
      ...
      for (row = 0; row < max_blocks_high; row += step)
        for (col = 0; col < max_blocks_wide; col += step)
          predict_and_reconstruct_intra_block(twd, xd, mi, plane, row, col,
                                              tx_size);
    }
  } else {
    // Prediction
    ...
    if (!mi->skip) {
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        ...
        for (row = 0; row < max_blocks_high; row += step)
          for (col = 0; col < max_blocks_wide; col += step)
            eobtotal += reconstruct_inter_block(twd, xd, mi, plane, row, col,
                                                tx_size);
      }
