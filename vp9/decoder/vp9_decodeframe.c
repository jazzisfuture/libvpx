...
static const uint8_t *decode_tiles(VP9Decoder *pbi,
  ...
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    ...
    for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
      ...
      for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
        const int col2 = pbi->inv_tile_order ?
                        tile_cols - tile_col - 1 : tile_col;
        tile_data = pbi->tile_data + tile_cols * tile_row + col2;
        vp9_tile_set_col(&tile, tile_data->cm, col2);
        vp9_zero(tile_data->xd.left_context);
        vp9_zero(tile_data->xd.left_seg_context);
        for (mi_col = tile.mi_col_start; mi_col < tile.mi_col_end;
             mi_col += MI_BLOCK_SIZE) {
          // Wait until the mi in previous frame is ready.
          // TODO(hkuang): Delay this wait until really need the
          // MV and Seg map from previous frame.
          if (pbi->frame_parallel_decode) {
            row = ((mi_row + 1) << 3) - 1;
            col = ((mi_col + 1) << 3) - 1;
            vp9_frameworker_wait(pbi->pre_frame_worker, row, col,
                                 pbi->prev_buf);
          }
      ...
      vp9_frameworker_broadcast(pbi->owner_frame_worker,
                                (((mi_row - MI_BLOCK_SIZE) + 1) << 3) - 1,
                                col);
    }
  }
  ...
  vp9_frameworker_broadcast(pbi->owner_frame_worker, INT_MAX, INT_MAX);
...
void vp9_decode_frame(VP9Decoder *pbi,
  ...
  if (pbi->frame_parallel_decode && pbi->common.frame_parallel_decoding_mode) {
    ...
    vp9_frameworker_broadcast(pbi->owner_frame_worker, -1, -1);
  }
  ...
  // Non frame parallel update frame context.
  if (cm->refresh_frame_context)
    cm->frame_contexts[cm->frame_context_idx] = cm->fc;
}
...
void dec_build_inter_predictors(VP9Decoder * const pbi, MACROBLOCKD *xd,
  ...
  for (ref = 0; ref < 1 + is_compound; ++ref) {
    ...
    MV32 scaled_mv;
    int xs, ys, x0, y0, x0_16, y0_16, x1, y1, frame_width, frame_height,
        buf_stride, subpel_x, subpel_y;
    uint8_t *ref_frame, *buf_ptr;
    const int idx = xd->block_refs[ref]->idx;
    BufferPool *const pool = pbi->common.buffer_pool;
    RefCntBuffer *const ref_buf = &pool->frame_bufs[idx];
    ...
    if (plane == 0) {
      frame_width = ref_buf->buf.y_crop_width;
      frame_height = ref_buf->buf.y_crop_height;
      ref_frame = ref_buf->buf.y_buffer;
    } else {
      frame_width = ref_buf->buf.uv_crop_width;
      frame_height = ref_buf->buf.uv_crop_height;
      ref_frame = plane == 1 ? ref_buf->buf.u_buffer : ref_buf->buf.v_buffer;
    }
    ...
    if (scaled_mv.col || scaled_mv.row ||
      ...
      // Wait until reference block is ready.
      if (ref_worker) {
        vp9_frameworker_wait(ref_worker, y1 - 1, x1 - 1, ref_buf);
      }
