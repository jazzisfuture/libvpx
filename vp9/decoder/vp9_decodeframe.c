...
static const uint8_t *decode_tiles_mt(VP9Decoder *pbi,
  ...
  // Rearrange the tile buffers such that per-worker group the largest, and
  // presumably the most difficult, tile(s) will be decoded in the main thread.
  // This should help minimize the number of instances where the main thread is
  // waiting for a worker to complete.
  assert(num_workers > 1);
  for (n = 0; n < tile_cols / 2; n += num_workers - 1) {
    const TileBuffer largest = pbi->tile_buffers[n];
    memmove(pbi->tile_buffers + n, pbi->tile_buffers + n + 1,
            (tile_cols - n - 1) * sizeof(pbi->tile_buffers[0]));
