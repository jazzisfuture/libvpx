...
static void predict_and_reconstruct_intra_block(int plane, int block,
  ...
  const PREDICTION_MODE mode = (plane == 0) ? get_y_mode(mi, block)
                                            : mi->mbmi.uv_mode;
  int16_t *dequant = (plane == 0) ? args->y_dequant : args->uv_dequant;
  int x, y;
  uint8_t *dst;
...
static void decode_block(VP9Decoder *const pbi, MACROBLOCKD *const xd,
  ...
  int16_t y_dequant[2] = {0};
  int16_t uv_dequant[2] = {0};
  ...
  if (!is_inter_block(mbmi)) {
    ...
    if (!mbmi->skip) {
      int eobtotal = 0;
      struct inter_args arg = { cm, xd, r, counts, &eobtotal, y_dequant, uv_dequant };
...
static void setup_quantization(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  read_delta_q(rb, &cm->y_dc_delta_q);
  read_delta_q(rb, &cm->uv_dc_delta_q);
  read_delta_q(rb, &cm->uv_ac_delta_q);
