...
static void decode_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  if (!is_inter_block(mi)) {
    predict_recon_intra(xd, mi, twd, predict_and_reconstruct_intra_block);
  } else {
    // Prediction
    ...
    // Reconstruction
    if (!mi->skip) {
      int eobtotal = predict_recon_inter(xd, mi, twd, reconstruct_inter_block);
...
static void parse_block(TileWorkerData *twd, VP9Decoder *const pbi, int mi_row,
  ...
  if (!is_inter_block(mi)) {
    int plane;
    for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
      predict_recon_intra(xd, mi, plane, twd, parse_intra_block_row_mt);
    }
  } else {
    if (!mi->skip) {
      int eobtotal = 0;
      ...
      for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
        eobtotal +=
            predict_recon_inter(xd, mi, plane, twd, parse_inter_block_row_mt);
      }
