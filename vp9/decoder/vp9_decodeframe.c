...
#if CONFIG_NEW_QUANT
  xd->plane[0].dequant_off = cm->y_dequant_off[q_index];
#endif  // CONFIG_NEW_QUANT

  for (i = 1; i < MAX_MB_PLANE; i++) {
    xd->plane[i].dequant = cm->uv_dequant[q_index];
#if CONFIG_NEW_QUANT
    xd->plane[i].dequant_off = cm->uv_dequant_off[q_index];
#endif  // CONFIG_NEW_QUANT
  }
}
...
#if CONFIG_NEW_QUANT
    cm->y_dequant_off[q][0] = vp9_get_dequant_off(cm->y_dequant[q][0], 0, q);
    cm->y_dequant_off[q][1] = vp9_get_dequant_off(cm->y_dequant[q][1], 1, q);
    cm->uv_dequant_off[q][0] = vp9_get_dequant_off(cm->uv_dequant[q][0], 0, q);
    cm->uv_dequant_off[q][1] = vp9_get_dequant_off(cm->uv_dequant[q][1], 1, q);
#endif  // CONFIG_NEW_QUANT
  }
}
