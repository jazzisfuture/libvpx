...
void dec_build_inter_predictors(VP9Decoder *const pbi, MACROBLOCKD *xd,
  ...
  int xs, ys, x0, y0, x0_16, y0_16, frame_width, frame_height,
      buf_stride, subpel_x, subpel_y, need_extend;
  ...
  if (is_scaled || scaled_mv.col || scaled_mv.row ||
...
#if CONFIG_VP9_HIGHBITDEPTH
    if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
      high_inter_predictor(buf_ptr1, buf_stride, dst1, dst_buf->stride, subpel_x,
                           subpel_y, sf, w, h, ref, kernel, xs, ys, xd->bd);
    } else {
    ...
    if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
      high_inter_predictor(buf_ptr, buf_stride, dst, dst_buf->stride, subpel_x,
                           subpel_y, sf, w, h, ref, kernel, xs, ys, xd->bd);
      if (plane == PLANE_TYPE_UV) {
        high_inter_predictor(buf_ptr1, buf_stride, dst, dst_buf->stride,
