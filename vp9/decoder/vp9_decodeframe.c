...
void vp9_loop_filter_frame_mt(VP9D_COMP *pbi,
  ...
  // TODO(yunqingwang): This allocation can be moved to decoder level,
  // and only realloc here if the resolution changes.
  //  vpx_free(pbi->lfmt_current_sb_col);
  //  pbi->lfmt_current_sb_col = NULL ;
  // Allocate an int for each sb row.
  CHECK_MEM_ERROR(cm, pbi->lfmt_current_sb_col,
                  vpx_malloc(sizeof(*pbi->lfmt_current_sb_col) * sb_cols));
  vpx_memset(pbi->lfmt_current_sb_col, -1,
             sizeof(*pbi->lfmt_current_sb_col) * sb_cols);

  // Set up nsync
  if (cm->width < 640) pbi->sync_range = 1;
  else if (cm->width <= 1280) pbi->sync_range = 2;
  else if (cm->width <= 2560) pbi->sync_range =4;
  else pbi->sync_range = 8;
