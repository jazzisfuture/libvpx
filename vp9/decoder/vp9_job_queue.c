...
void vp9_jobq_init(jobq_t *jobq, uint8_t *buf, size_t buf_size) {
...
int vp9_jobq_queue(jobq_t *jobq, void *job, size_t job_size) {
  int ret = 0;
#if CONFIG_MULTITHREAD
  pthread_mutex_lock(&jobq->mutex);
#endif
...
int vp9_jobq_dequeue(jobq_t *jobq, void *job, size_t job_size, int blocking) {
  int ret = 0;
#if CONFIG_MULTITHREAD
  pthread_mutex_lock(&jobq->mutex);
#endif
