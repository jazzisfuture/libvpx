...
static void inter_pred_parameter_fri_ref_rs(VP9_DECODER_RECON *const decoder_recon,
                                            const int inter_blocks_num,
                                            const int ref_idx,
                                            const int tile_num) {
  BLOCK_SIZE bsize;
...
static void inter_transform_recon(VP9_DECODER_RECON *const decoder_recon,
  ...
  for (i = 0; i < MAX_MB_PLANE; i++) {
    ...
    xd->plane[i].dqcoeff =
        ...
        inter->offset;
    xd->plane[i].eobs =  decoder_recon->dequant_recon[inter->qcoeff_flag].eobs[i] +
        inter->offset / 16;
  }
...
static int vp9_sched_frame_entrop_dec(VP9D_COMP *pbi,
  ...
  vp9_decode_frame_head_recon(pbi, p_data_end, &first_partition_size, &data);
