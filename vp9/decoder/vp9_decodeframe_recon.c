...
static void apply_frame_size_recon(VP9D_COMP *pbi, int width, int height) {
  ...
  if (cm->fb_list != NULL) {
    ...
    if (rs_init == 0) {
      vp9_realloc_frame_buffer_rs(get_frame_new_buffer(cm),
                                  cm->width,cm->height,
                                  cm->subsampling_x, cm->subsampling_y,
                                  VP9BORDERINPIXELS, NULL, NULL, NULL,
                                  cm->new_fb_idx);
...
static void setup_frame_size_recon(VP9D_COMP *pbi,
  ...
  int width, height;
  read_frame_size(rb, &width, &height);
  apply_frame_size_recon(pbi, width, height);
  setup_display_size(&pbi->common, rb);
}
...
static void setup_frame_size_with_refs_recon(VP9D_COMP *pbi,
                                       struct vp9_read_bit_buffer *rb) {
  VP9_COMMON *const cm = &pbi->common;
...
static const uint8_t *decode_tiles_recon(VP9D_COMP *pbi, const uint8_t *data) {
  VP9_COMMON *const cm = &pbi->common;
  MACROBLOCKD *const xd = &pbi->mb;
  const int aligned_cols = mi_cols_aligned_to_sb(cm->mi_cols);
  const int tile_cols = 1 << cm->log2_tile_cols;
  ...
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
      ...
      setup_token_decoder(buf->data, data_end, buf->size, &cm->error, &r);
      setup_tile_context(pbi, xd, tile_row, col);
      decode_tile_recon(pbi, &tile, &r, tile_col);
...
static size_t read_uncompressed_header_recon(VP9D_COMP *pbi,
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    store_frame_size_recon(pbi, &width, &height);
    setup_frame_size_recon(pbi, rb);
  } else {
    cm->intra_only = cm->show_frame ? 0 : vp9_rb_read_bit(rb);
    ...
    if (cm->intra_only) {
      ...
      store_frame_size_recon(pbi, &width, &height);
      setup_frame_size_with_refs_recon(pbi, rb);
...
static void inverse_transform_block_recon(MACROBLOCKD* xd, int plane,
                int block, TX_SIZE tx_size, uint8_t *dst, int stride,
                uint8_t *token_cache) {
  struct macroblockd_plane *const pd = &xd->plane[plane];
...
static void decode_modes_b_recon(VP9_DECODER_RECON *const decoder_recon,
  ...
  if (!is_inter_block(mbmi)) {
    ...
    store_eobtotal_less8x8_recon(less8x8, eobtotal, mbmi->skip_coeff,
                                 decoder_recon);
...
static int set_ref_rs(VP9_COMMON *const cm, MACROBLOCKD *const xd,
                       int idx, int mi_row, int mi_col) {
  MB_MODE_INFO *const mbmi = &xd->mi_8x8[0]->mbmi;
  RefBuffer *ref_buffer = &cm->frame_refs[mbmi->ref_frame[idx] - LAST_FRAME];
  xd->block_refs[idx] = ref_buffer;
...
static void inter_pred_parameter_sec_ref_rs(
  ...
  ref_num = set_ref_rs(cm, xd, 1, mi_row, mi_col);
  filter_num = vp9_setup_interp_filters_rs(xd, mbmi->interp_filter, cm);
  ...
  for (i = 0; i <= luma_block_count; ++i) {
    build_inter_pred_param_sec_ref_rs(0, i, bsize, &args,
                                       cm, ref_num, filter_num, tile_num);
  }

  build_inter_pred_param_sec_ref_rs(1, 0, bsize, &args, cm,
                                     ref_num, filter_num, tile_num);

  build_inter_pred_param_sec_ref_rs(2, 0, bsize, &args, cm,
                                     ref_num, filter_num, tile_num);
}

static void inter_pred_parameter_fri_ref_rs(
                VP9_DECODER_RECON *const decoder_recon,
                const int inter_blocks_num,
                const int ref_idx,
                const int tile_num) {
  ...
  mbmi = &xd->mi_8x8[0]->mbmi;
  ref_num = set_ref_rs(cm, xd, 0, mi_row, mi_col);
  filter_num = vp9_setup_interp_filters_rs(xd, mbmi->interp_filter, cm);
  if (mbmi->sb_type < BLOCK_8X8) {
    luma_block_count = 3;
  }
  ...
  for (i = 0; i <= luma_block_count; ++i) {
    build_inter_pred_param_fri_ref_rs(0, i, bsize, &args,
                                       cm, ref_idx, ref_num,
                                       filter_num, tile_num);
  }
  build_inter_pred_param_fri_ref_rs(1, 0, bsize, &args,
                                     cm, ref_idx, ref_num,
                                     filter_num, tile_num);
  build_inter_pred_param_fri_ref_rs(2, 0, bsize, &args,
                                     cm, ref_idx, ref_num,
                                     filter_num, tile_num);
}

static void inter_pred_param_rs(VP9_DECODER_RECON *const decoder_recon,
  ...
  for (i = blocks_start; i < blocks_end; ++i) {
    ...
    inter_pred_parameter_fri_ref_rs(decoder_recon, i, ref_idx, tile_num);
    if (ref_idx) {
      inter_pred_parameter_sec_ref_rs(decoder_recon, i, tile_num);
    }
  }
}

static void inter_pred_rs(VP9_DECODER_RECON *const decoder_recon,
                          const int blocks_start,
                          const int blocks_end,
                          const int tile_num) {
  VP9_COMMON *const cm = decoder_recon->cm;
  if (blocks_start == blocks_end) return;
  inter_pred_param_rs(decoder_recon, blocks_start, blocks_end, tile_num);
  inter_pred_calcu_rs(cm, tile_num);
}
...
void decode_tile_recon_entropy(VP9D_COMP *pbi,
  ...
  MACROBLOCKD *const xd = &decoder_recon->mb;
  int mi_row = 0, mi_col = 0;
  PPAStartCpuEventFunc(vp9_tiles_entropy_time);
  ...
  PPAStopCpuEventFunc(vp9_tiles_entropy_time);
}
...
void decode_tile_recon_inter_rs(VP9D_COMP *pbi, const TileInfo *const tile,
  ...
  PPAStartCpuEventFunc(vp9_tiles_inter_pred_rs_time);
  inter_pred_rs(decoder_recon, 0,
                 decoder_recon->inter_blocks_count,
                 tile_col);
  PPAStopCpuEventFunc(vp9_tiles_inter_pred_rs_time);
}
...
void decode_tile_recon_inter(VP9D_COMP *pbi, const TileInfo *const tile,
  ...
  PPAStartCpuEventFunc(vp9_tiles_inter_pred_time);
  ...
  PPAStopCpuEventFunc(vp9_tiles_inter_pred_time);
}
...
void decode_tile_recon_inter_transform(VP9D_COMP *pbi,
  ...
  VP9_DECODER_RECON *const decoder_recon = &pbi->decoder_recon[tile_col];
  int i_inter_blocks_count = 0;
  PPAStartCpuEventFunc(vp9_tiles_inter_transform_time);
...
void decode_tile_recon_intra(VP9D_COMP *pbi, const TileInfo *const tile,
  ...
  PPAStartCpuEventFunc(vp9_tiles_intra_pred_time);
  ...
  PPAStopCpuEventFunc(vp9_tiles_intra_pred_time);
}
...
int vp9_decode_frame_recon(VP9D_COMP *pbi, const uint8_t **p_data_end) {
  ...
  const size_t first_partition_size = read_uncompressed_header_recon(pbi, &rb);
  ...
  if (pbi->oxcf.max_threads > 1 && tile_rows == 1) {
    ...
    *p_data_end = decode_tiles_mt_recon_for_mt(pbi,
                      data + first_partition_size);
  } else {
    *p_data_end = decode_tiles_recon(pbi, data + first_partition_size);
  }
...
static int vp9_single_thread_decode(VP9D_COMP *pbi,
                             const uint8_t **p_data_end,
...
static int vp9_sched_frame_entrop_dec(VP9D_COMP *pbi,
  ...
  ret = vp9_decode_frame_head_recon(pbi, p_data_end,&first_partition_size,
                                    &data);
  if (ret != 0) {
    if (ret == -2) {
      ret = 0;
