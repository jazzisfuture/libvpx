...
static void kfread_modes(VP9D_COMP *pbi,
  ...
  } else if (cm->txfm_mode >= ALLOW_32X32 && m->mbmi.sb_type >= BLOCK_SIZE_SB32X32) {
...
;
  ...
  } else if (mbmi->sb_type >= BLOCK_SIZE_SB32X32 && cm->txfm_mode >= ALLOW_32X32) {
...
void vp9_decode_mb_mode_mv(VP9D_COMP* const pbi,
  ...
  if (mbmi->sb_type) {
    const int bw = mb_width_log2(mbmi->sb_type);
    const int bh = mb_height_log2(mbmi->sb_type);
