  ...
  switch (bsize) {
    ...
    default:
      ...
      if (mbmi->palette_enabled) {
        ...
        mbmi->mode = DC_PRED;
        mbmi->palette_indexed_size  = vp9_read_literal(r, 3);
        mbmi->palette_literal_size  = vp9_read_literal(r, 3);
        ...
        if (1) {
          for (i = 0; i < m1; i++)
            mbmi->palette_indexed_colors[i] =
                vp9_read_literal(r, get_bit_depth(PALETTE_BUF_SIZE));
          ...
          for (i = 0; i < l; i += 2) {
            mbmi->palette_runs[i] = vp9_read_literal(r,8);
            mbmi->palette_runs[i + 1] = vp9_read_literal(r, 12);
          }
        }
