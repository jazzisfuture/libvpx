...
static int get_segment_id(const VP9_COMMON *cm, const uint8_t *segment_ids,
...
static int read_inter_segment_id(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  predicted_segment_id = cm->last_frame_seg_map ?
      get_segment_id(cm, cm->last_frame_seg_map, mi_offset, x_mis, y_mis) : 0;
...
static void read_inter_block_mode_info(VP9Decoder *const pbi,
  ...
  if (bsize < BLOCK_8X8) {
    ...
    // TODO(slavarnway): frame parallel decode fails when using the above two
    // lines.  For now, leave as is, until the frame parallel decode is fixed.
    //
    // The above statement is false.  jzern pointed out that the normal unit
    // tests also fail.  I have a bug with block sizes less than 8x8s.
