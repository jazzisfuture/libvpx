...
static MV_REFERENCE_FRAME read_ref_frame(VP9D_COMP *pbi,
  ...
  if (!seg_ref_active || (seg_ref_count > 1)) {
    ...
    unsigned char prediction_flag = (unsigned char)vp9_read(bc, pred_prob);
  ...
  if (xd->segmentation_enabled) {
    if (xd->update_mb_segmentation_map) {
      ...
      if (cm->temporal_update) {
        ...
        // Read the prediction status flag
        unsigned char seg_pred_flag = (unsigned char)vp9_read(bc, pred_prob);
...
static void read_mb_modes_mv(VP9D_COMP *pbi, MODE_INFO *mi, MB_MODE_INFO *mbmi,
  ...
  if (mbmi->ref_frame) {
...

    {
      ...
      int ref_fb_idx = cm->active_ref_idx[ref_frame - 1];
