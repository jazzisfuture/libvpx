...
static int read_nmv_component_fp(vp9_reader *r,
  ...
  if (usehp) {
    offset += mv_class == MV_CLASS_0 ? vp9_read(r, mvcomp->class0_hp)
...
static void read_mb_modes_mv(VP9D_COMP *pbi, MODE_INFO *mi, MB_MODE_INFO *mbmi,
  ...
  if (mbmi->ref_frame) {
    ...
    switch (mbmi->mode) {
      case SPLITMV: {
        const int s = mbmi->partitioning = treed_read(bc, vp9_mbsplit_tree,
                                                      cm->fc.mbsplit_prob);
...
;
    ...
    if (mbmi->mode == B_PRED) {
      int j = 0;
      do {
        const int m = mi->bmi[j].as_mode.first = read_bmode(bc,
