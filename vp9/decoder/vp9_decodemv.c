  ...
  if (bsize >= BLOCK_SIZE_SB8X8) {
    ...
    for (idy = 0; idy < 2; idy += bh) {
      for (idx = 0; idx < 2; idx += bw) {
        ...
        for (k = 1; k < bh; ++k)
          m->bmi[ib + k * 2].as_mode.first = b_mode;
        for (k = 1; k < bw; ++k)
          m->bmi[ib + k].as_mode.first = b_mode;
      }
    }
  ...
  if (bsize >= BLOCK_SIZE_SB8X8) {
    ...
    cm->fc.y_mode_counts[size_group][mbmi->mode]++;
     ...
     for (idy = 0; idy < 2; idy += bh) {
       for (idx = 0; idx < 2; idx += bw) {
         int ib = idy * 2 + idx, k;
         int b_mode = read_intra_mode(r, cm->fc.y_mode_prob[0]);
         mi->bmi[ib].as_mode.first = b_mode;
         cm->fc.y_mode_counts[0][b_mode]++;
         for (k = 1; k < bh; ++k)
           mi->bmi[ib + k * 2].as_mode.first = b_mode;
         for (k = 1; k < bw; ++k)
           mi->bmi[ib + k].as_mode.first = b_mode;
      }
    }
