...
static void kfread_modes(VP9D_COMP *pbi, MODE_INFO *m,
  ...
  if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    if (m->mbmi.txfm_size == TX_4X4 && is_filter_mode(m->mbmi.mode))
      m->mbmi.filterbit = vp9_read(r, FBIT0_PROB);
#endif
  } else {
    int idx, idy;
    ...
    for (idy = 0; idy < 2; idy += bh) {
      for (idx = 0; idx < 2; idx += bw) {
        ...
        if (is_filter_mode(m->bmi[ib].as_mode.first)) {
          m->bmi[ib].as_mode.filterbit = vp9_read(r, FBIT0_PROB);
          for (k = 1; k < bh; ++k)
  ...
  if (get_uv_tx_size(&(m->mbmi)) <= TX_4X4 && is_filter_mode(m->mbmi.uv_mode))
    m->mbmi.uv_filterbit = vp9_read(r, FBIT0_PROB);
#endif
}
...
static void read_mb_modes_mv(VP9D_COMP *pbi, MODE_INFO *mi, MB_MODE_INFO *mbmi,
  ...
  if (mbmi->ref_frame[0] != INTRA_FRAME) {
    ...
    if (bsize >= BLOCK_SIZE_SB8X8) {
      ...
      if (mbmi->txfm_size == TX_4X4 && is_filter_mode(mbmi->mode))
        mbmi->filterbit = vp9_read(r, FBIT0_PROB);
#endif
    } else {
      int idx, idy;
      for (idy = 0; idy < 2; idy += bh) {
...
#if CONFIG_FILTERBIT
          if (is_filter_mode(m)) {
            int fbit = vp9_read(r, FBIT0_PROB);
  ...
  if (get_uv_tx_size(mbmi) <= TX_4X4 && is_filter_mode(mbmi->uv_mode))
    mbmi->uv_filterbit = vp9_read(r, FBIT0_PROB);
#endif
  }
}
