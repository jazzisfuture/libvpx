...
//  m->mbmi.pred_filter_y = vp9_read(bc, 128);
  ...
  if (y_mode == I8X8_PRED) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      //fprintf(fp_dec, "b_mode[%d]=%d b_pf[%d]=%d ", ib, m->bmi[ib].as_mode.first, ib, m->bmi[ib].as_mode.pf_state);
    }
  } else {
...
//    m->mbmi.pred_filter_uv = vp9_read(bc, 128);
  }
...
//    mbmi->pred_filter_y = vp9_read(bc, 128);
...
//      mbmi->pred_filter_uv = vp9_read(bc, 128);
