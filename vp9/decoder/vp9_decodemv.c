...
static void read_ref_frames(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (vp9_segfeature_active(&cm->seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (mode == COMPOUND_REFERENCE) {
      ...
      const int ctx0 = vp9_get_pred_context_comp_same_ref_p(cm, xd);
      const int bit0 = vp9_read(r, fc->comp_same_ref_prob[ctx]);

      const int ctx1 = vp9_get_pred_context_comp_ref_p(cm, xd);
      const int bit1 = vp9_read(r, fc->comp_ref_prob[ctx]);
