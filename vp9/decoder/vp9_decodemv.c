...
static void read_ref_frame(VP9D_COMP *pbi, vp9_reader *r,
  ...
  if (!seg_ref_active || seg_ref_count > 1) {
    ...
    if (cm->comp_pred_mode == HYBRID_PREDICTION) {
      is_comp = vp9_read(r, vp9_get_pred_prob(cm, xd, PRED_COMP_INTER_INTER));
    } else {
    ...
    if (is_comp) {
      ...
      b = vp9_read(r, vp9_get_pred_prob(cm, xd, PRED_COMP_REF_P));
      ref_frame[!fix_ref_idx] = cm->comp_var_ref[b];
    } else {
      ref_frame[1] = NONE;
      if (vp9_read(r, vp9_get_pred_prob(cm, xd, PRED_SINGLE_REF_P1))) {
        int b2 = vp9_read(r, vp9_get_pred_prob(cm, xd, PRED_SINGLE_REF_P2));
  ...
  if (cm->frame_type != KEY_FRAME) {
    ...
    if (vp9_read_bit(r))
      for (i = 0; i < INTRA_INTER_CONTEXTS; i++)
        cm->fc.intra_inter_prob[i] = vp9_read_prob(r);

    if (cm->allow_comp_inter_inter) {
      cm->comp_pred_mode = read_comp_pred_mode(r);
      if (cm->comp_pred_mode == HYBRID_PREDICTION && vp9_read_bit(r))
        for (i = 0; i < COMP_INTER_CONTEXTS; i++)
          cm->fc.comp_inter_prob[i] = vp9_read_prob(r);
    } else {
      cm->comp_pred_mode = SINGLE_PREDICTION_ONLY;
    }

    if (cm->comp_pred_mode != COMP_PREDICTION_ONLY && vp9_read_bit(r))
      for (i = 0; i < REF_CONTEXTS; i++)
        for (j = 0; j < 2; j++)
          cm->fc.single_ref_prob[i][j] = vp9_read_prob(r);

    if (cm->comp_pred_mode != SINGLE_PREDICTION_ONLY && vp9_read_bit(r))
      for (i = 0; i < REF_CONTEXTS; i++)
        cm->fc.comp_ref_prob[i] = vp9_read_prob(r);
