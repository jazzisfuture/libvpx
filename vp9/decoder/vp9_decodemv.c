...
#if CONFIG_GLOBAL_MOTION || (CONFIG_WEDGE_PARTITION && CONFIG_WEDGE_TEST)
                            MV_REFERENCE_FRAME ref_frame[2],
#endif  // CONFIG_GLOBAL_MOTION || (CONFIG_WEDGE_PARTITION && CONFIG_WEDGE_TEST)
...
static void read_inter_block_mode_info(VP9_COMMON *const cm,
  ...
  if (mbmi->use_wedge_interinter &&
    ...
    vp9_setup_pre_planes(xd, 1, xd->block_refs[1]->buf, mi_row, mi_col,
                         &xd->block_refs[1]->sf);
  }
#endif  // CONFIG_WEDGE_PARTITION && CONFIG_WEDGE_TEST
...
#endif  // CONFIG_NEW_INTER
    xd->corrupted |= !assign_mv(cm, mbmi->mode,
#if CONFIG_GLOBAL_MOTION || (CONFIG_WEDGE_PARTITION && CONFIG_WEDGE_TEST)
                                mbmi->ref_frame,
#endif  // CONFIG_GLOBAL_MOTION || (CONFIG_WEDGE_PARTITION && CONFIG_WEDGE_TEST)
...
/*
#if CONFIG_WEDGE_PARTITION && CONFIG_WEDGE_TEST
    if (cm->current_video_frame >= 1 || cm->current_video_frame <= 3) {
      printf("\n==================DEC===========================\n");
