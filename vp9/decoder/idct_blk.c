...
void vp9_dequant_dc_idct_add_y_block_8x8_c(short *q, short *dq,
  ...
  q[64] = dc[1];
  vp9_dequant_idct_add_8x8_c(&q[64], dq, pre + 8, dst + 8, 16, stride, 1, xd->eobs[4]);
  ...
  vp9_dequant_idct_add_8x8_c(&q[192], dq, pre + 8 * 16 + 8,
                                dst + 8 * stride + 8, 16, stride, 1, xd->eobs[12]);
}
...
void vp9_dequant_dc_idct_add_y_block_8x8_inplace_c(short *q, short *dq,
  ...
  vp9_dequant_idct_add_8x8_c(&q[128], dq, dst + 8 * stride,
                                dst + 8 * stride, stride, stride, 1, xd->eobs[8]);

  q[192] = dc[8];
  vp9_dequant_idct_add_8x8_c(&q[192], dq, dst + 8 * stride + 8,
                                dst + 8 * stride + 8, stride, stride, 1, xd->eobs[12]);
}
#endif
...
void vp9_dequant_idct_add_uv_block_8x8_inplace_c(short *q, short *dq,
                                                 ...
                                                 unsigned short *eobs,
                                                 MACROBLOCKD *xd) {
  vp9_dequant_idct_add_8x8_c(q, dq, dstu, dstu, stride, stride, 0, xd->eobs[16]);

  q    += 64;

  vp9_dequant_idct_add_8x8_c(q, dq, dstv, dstv, stride, stride, 0, xd->eobs[20]);
}
#endif
