...
static int decode_coefs(VP9_COMMON *cm, int is_inter, PLANE_TYPE plane_type,
  ...
  const vp9_prob (*const coef_probs)[COEFF_CONTEXTS][UNCONSTRAINED_NODES] =
      cm->fc.coef_probs[tx_size][plane_type][is_inter];
  unsigned int (*const coef_counts)[COEFF_CONTEXTS][UNCONSTRAINED_NODES + 1] =
      cm->counts.coef[tx_size][plane_type][is_inter];
  unsigned int (*const eob_branch_count)[COEFF_CONTEXTS] =
