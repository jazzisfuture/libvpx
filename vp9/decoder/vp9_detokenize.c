...
#define INCREMENT_COUNT(token)               \
  do {                                       \
    coef_counts[type][vp9_get_coef_band(txfm_size, c)][pt][token]++;     \
...
static int decode_coefs(VP9D_COMP *dx, const MACROBLOCKD *xd,
  ...
  while (1) {
    ...
    if (c >= seg_eob) break;
    prob = coef_probs[type][vp9_get_coef_band(txfm_size, c)][pt];
...
SKIP_START:
  ...
  if (c < seg_eob)
    coef_counts[type][vp9_get_coef_band(txfm_size, c)][pt][DCT_EOB_TOKEN]++;
