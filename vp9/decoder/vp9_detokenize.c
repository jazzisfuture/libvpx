...
static int decode_coefs(VP9_COMMON *cm, const MACROBLOCKD *xd, PLANE_TYPE type,
...
#endif  // CONFIG_TX_SKIP
#if CONFIG_TWO_STAGE
    if (xd->mi[0].src_mi->mbmi.two_stage_coding[type])
      dqv = dq_stg1[0];
#endif  // CONFIG_TWO_STAGE

#if CONFIG_TWO_STAGE1
#if 1
    if (xd->mi[0].src_mi->mbmi.two_stage_coding[type] && SKIP_TEST)
      goto decoding_stg2;
#endif
#endif
  ...
  while (c < max_eob) {
    ...
    if (!type) {
      ...
      printf("qplus %d\n",
             xd->mi[0].src_mi->mbmi.qindex_plus);

      printf("stage 1\n");
      for (i = 0; i < bs; i++) {
        for (j = 0; j < bs; j++) {
          printf("%4d ", dqcoeff[i * bs + j]);
        }
        printf("\n");
      }
      printf("\n");
