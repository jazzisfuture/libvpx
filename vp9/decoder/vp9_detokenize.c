...
static int decode_coefs(VP9_COMMON *cm, const MACROBLOCKD *xd, PLANE_TYPE type,
  ...
  int o, skip_coef_val;
  vp9_zpc_probs *zpc_probs = &cm->fc.zpc_probs[tx_size][type];
  vp9_zpc_count *zpc_count = &counts->zpc[tx_size][type];
  vp9_prob *zprobs;
  ...
  while (c < max_eob) {
    ...
    zprobs = &(*zpc_probs)[ref][band][coef_to_zpc_ptok(ctx)];
    ...
    while (1) {
      ...
      if (skip_coef_val) {
        ...
        if (!is_eoo[o]) {
          int use_eoo, eoo = 0;
          use_eoo = vp9_use_eoo(c, max_eob, scan, tx_size, band,
                                is_last_zero, is_eoo);
          if (use_eoo) {
            eoo = vp9_read(r, zprobs[0]);
            (*zpc_count)[ref][band][coef_to_zpc_ptok(ctx)][eoo]++;
            if (eoo) is_eoo[o] = 1;
          }
        }
        is_last_zero[o] = 1;
