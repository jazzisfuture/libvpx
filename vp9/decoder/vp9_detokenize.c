...
SKIP_START:
...
#endif
#if USE_ZPC_EXTRA == 1
      use_ztr = vp9_use_zrn(c, seg_eob, txfm_size);
#elif USE_ZPC_EXTRA == 2
      use_ztr = vp9_use_ztr(c, seg_eob, scan, txfm_size, tx_type, pt);
#else
      use_ztr = 0;
#endif
...
#if USE_ZPC_EXTRA == 1
          vp9_mark_zrn(c, scan, txfm_size, token_cache);
#elif USE_ZPC_EXTRA == 2
          vp9_mark_ztr(c, scan, txfm_size, token_cache);
#endif
        }
      }
#endif
    ...
    if (last1_indicated) {
      // No need to decode ONE_CONTEXT_NODE
    ...
    if (!vp9_read(br, prob[ONE_CONTEXT_NODE])) {
      ...
      WRITE_COEF_ONE();
#if USE_OPC_LAST1
      use_last1 = vp9_use_last1(c, seg_eob, scan, txfm_size,
                                last1_indicated);
#else
      use_last1 = 0;
#endif
