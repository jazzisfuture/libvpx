...
static INLINE int block_idx_4x4(MACROBLOCKD* const xd, int block_size_b,
  ...
  return plane == 0 ? i :
         plane == 1 ? (1 << block_size_b) + i :
         (1 << block_size_b) * 5 / 4 + i;
}
...
static INLINE int decode_blocks_(VP9D_COMP* const pbi,
                                 MACROBLOCKD* const xd,
                                 BOOL_DECODER* const bc,
                                 int block_size,
                                 TX_SIZE tx_size,
  ...
  for (plane = 0; plane < MAX_MB_PLANE; plane++) {
    ...
    eobtotal += decode_block_plane(pbi, xd, bc, block_size, tx_size,
                                   segment_id, plane, is_split);
  }
  return eobtotal;
}
...
static INLINE int decode_blocks(VP9D_COMP* const pbi,
  ...
  if (tx_size == TX_8X8 && (mode == I8X8_PRED || mode == SPLITMV))
    return decode_blocks_(pbi, xd, bc, block_size, tx_size, 1);
  return decode_blocks_(pbi, xd, bc, block_size, tx_size, 0);
}

int vp9_decode_sb64_tokens(VP9D_COMP* const pbi,
                           MACROBLOCKD* const xd,
                           BOOL_DECODER* const bc) {
  return decode_blocks(pbi, xd, bc, 8, xd->mode_info_context->mbmi.txfm_size);
}

int vp9_decode_sb_tokens(VP9D_COMP* const pbi,
                         MACROBLOCKD* const xd,
                         BOOL_DECODER* const bc) {
  return decode_blocks(pbi, xd, bc, 6, xd->mode_info_context->mbmi.txfm_size);
}

static int vp9_decode_mb_tokens_16x16(VP9D_COMP* const pbi,
                                      MACROBLOCKD* const xd,
                                      BOOL_DECODER* const bc) {
  return decode_blocks(pbi, xd, bc, 4, xd->mode_info_context->mbmi.txfm_size);
}
...
static int vp9_decode_mb_tokens_4x4(VP9D_COMP* const pbi,
                                    MACROBLOCKD* const xd,
                                    BOOL_DECODER* const bc) {
  return decode_blocks(pbi, xd, bc, 4, xd->mode_info_context->mbmi.txfm_size);
}
