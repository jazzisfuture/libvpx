...
int is_full[BLOCK_TYPES][REF_TYPES][TX_SIZES][3][128] = {0};

void fill_table(bw_state *fs, PLANE_TYPE type, int is_inter,
  ...
  for (i = 0; i < 3; i++) {
    ...
    this_state->lowvalue = ((BD_VALUE)bw.lowvalue) <<
                           (BD_VALUE_SIZE - 8 - this_state->count);
  }
  is_full[type][is_inter][tx_size][context][range-128] = 1;
}
...
static int decode_coefs(VP9_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (!is_full[type][ref][tx_size][pt][r->range-128])
  ...
  if (r->value < fs[1].lowvalue) {
    ...
    if (r->count < fs[1].count  || r->count < fs[1].count )
      vp9_reader_fill(r);
