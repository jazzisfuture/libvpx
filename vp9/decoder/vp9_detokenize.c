...
static INLINE int read(vpx_reader *r, int prob, BD_VALUE *value, int *count,
                       unsigned int *range) {
  BD_VALUE bigsplit;
  unsigned int split = (*range * prob + (256 - prob)) >> CHAR_BIT;
...
static INLINE int read_coeff(vpx_reader *r, const vpx_prob *probs, int n,
  ...
  int i, val = 0;
  for (i = 0; i < n; ++i)
    val = (val << 1) | read(r, probs[i], value, count, range);
  return val;
}
...
static int decode_coefs(const MACROBLOCKD *xd, PLANE_TYPE type,
  ...
  while (c < max_eob) {
    ...
    if (!read(r, prob[EOB_CONTEXT_NODE], &value, &count, &range)) {
      INCREMENT_COUNT(EOB_MODEL_TOKEN);
      break;
    }

    while (!read(r, prob[ZERO_CONTEXT_NODE], &value, &count, &range)) {
    ...
    if (read(r, prob[ONE_CONTEXT_NODE], &value, &count, &range)) {
      const vpx_prob *p = vp9_pareto8_full[prob[PIVOT_NODE] - 1];
      INCREMENT_COUNT(TWO_TOKEN);
      if (read(r, p[0], &value, &count, &range)) {
        if (read(r, p[3], &value, &count, &range)) {
          token_cache[scan[c]] = 5;
          if (read(r, p[5], &value, &count, &range)) {
            if (read(r, p[7], &value, &count, &range)) {
              val = CAT6_MIN_VAL +
            ...
            if (read(r, p[6], &value, &count, &range)) {
              val = CAT4_MIN_VAL +
          ...
          if (read(r, p[4], &value, &count, &range)) {
            val = CAT2_MIN_VAL +
        ...
        if (read(r, p[1], &value, &count, &range)) {
          token_cache[scan[c]] = 3;
          if (read(r, p[2], &value, &count, &range)) {
            v = (4 * dqv) >> dq_shift;
          } else {
            v = (3 * dqv) >> dq_shift;
          }
...
#if CONFIG_VP9_HIGHBITDEPTH
    dqcoeff[scan[c]] =
        highbd_check_range(read(r, 128, &value, &count, &range) ? -v : v),
                           xd->bd);
#else
    dqcoeff[scan[c]] =
        check_range(read(r, 128, &value, &count, &range) ? -v : v);
#endif  // CONFIG_VP9_HIGHBITDEPTH
#else
    if(read(r, 128, &value, &count, &range)) {
      dqcoeff[scan[c]] = -v;
    } else {
      dqcoeff[scan[c]] = v;
    }
