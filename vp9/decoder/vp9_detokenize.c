  ...
  while (c < max_eob) {
    ...
    if (!vp9_read(r, prob[HIGH_LOW_CONTEXT_NODE])) {
      if (!vp9_read(r, prob[CAT_ONE_CONTEXT_NODE])) {
        ...
        WRITE_COEF_CONTINUE(val, DCT_CATEGORY1_TOKEN);
      }
      val = CAT2_MIN_VAL;
      ADJUST_COEF(CAT2_PROB1, 1);
      ADJUST_COEF(CAT2_PROB0, 0);
      WRITE_COEF_CONTINUE(val, DCT_CATEGORY2_TOKEN);
    }
    ...
    if (!vp9_read(r, prob[CAT_THREEFOUR_CONTEXT_NODE])) {
      if (!vp9_read(r, prob[CAT_THREE_CONTEXT_NODE])) {
        ...
        WRITE_COEF_CONTINUE(val, DCT_CATEGORY3_TOKEN);
      }
      val = CAT4_MIN_VAL;
      ...
      WRITE_COEF_CONTINUE(val, DCT_CATEGORY4_TOKEN);
    }
    ...
    if (!vp9_read(r, prob[CAT_FIVE_CONTEXT_NODE])) {
      ...
      WRITE_COEF_CONTINUE(val, DCT_CATEGORY5_TOKEN);
    }
    val = 0;
    cat6 = cat6_prob;
    while (*cat6)
    ...
    WRITE_COEF_CONTINUE(val, DCT_CATEGORY6_TOKEN);
  }
