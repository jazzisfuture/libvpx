...
#endif
  if (dx->common.current_video_frame == 0) {
    dcount++;
  }
...
SKIP_START:
    ...
    zprobs = (*zpc_probs)[type][ref][get_coef_band(scan, txfm_size, c)][pt];
#endif
#if CONFIG_CODE_NONZEROCOUNT
    ...
    if (!vp9_read(br, prob[ZERO_CONTEXT_NODE])) {
      ...
      eoo = 0;
      izr = 1;
      use_eoo = vp9_use_eoo(rc, txfm_size, is_last_zero, is_eoo);
      use_ztr = vp9_use_ztr(rc, txfm_size);
      use_izr = (use_eoo || use_ztr);
      if (use_izr) {
        izr = vp9_read(br, zprobs[0]);
        ++(*zpc_count)[type][ref][get_coef_band(scan, txfm_size, c)]
                      [pt][0][izr];
        if (!izr) {
          if (use_eoo && use_ztr) {
            eoo = vp9_read(br, zprobs[1]);
            if (!eoo) ztr = 1;
            ++(*zpc_count)[type][ref][get_coef_band(scan, txfm_size, c)]
                          [pt][1][eoo];
          } else if (use_eoo) {
        ...
        /*
        printf("use_izr %d use_ztr %d use_eoo %d, izr %d ztr %d eoo %d\n",
               use_izr, use_ztr, use_eoo, izr, ztr, eoo);
               */
        if (eoo) {
          is_eoo[o] = 1;
        } else if (ztr) {
          vp9_mark_ztr(rc, txfm_size, zero_cache);
        }
      }
#endif
  ...
  // if (dx->common.current_video_frame == 0) printf("dcount %d eob %d\n", dcount, c);
