...
bw_state full_set[TX_SIZES][BLOCK_TYPES][REF_TYPES][COEF_BANDS][PREV_COEF_CONTEXTS][2][128];
...
static inline void node_one_bit(const int prob, bw_state *fs, int node,
  ...
  fs->lowvalue[node] = BD_VALUEIZE((*lowvalue) << shift, fs->state[node+1].count);
  take_right(split, lowvalue, range, count);
}

bw_state * fill_table(PLANE_TYPE type, int is_inter, int band,
                      TX_SIZE tx_size, int context, int range,
                      int last_is_zero,
                      int prob1, int prob2, int prob3) {
  int count = 0;
  unsigned int lowvalue = 0;
  bw_state *fs = &full_set[tx_size][type][is_inter][band][context][last_is_zero][range-128];
