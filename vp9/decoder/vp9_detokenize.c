...
static int decode_coefs(VP9_COMMON *cm, const MACROBLOCKD *xd, PLANE_TYPE type,
...
#endif  // CONFIG_TX_SKIP
#if CONFIG_TWO_STAGE
    if (xd->mi[0].src_mi->mbmi.two_stage_coding[type] && USE_2STG)
      dqv = dq_stg1[0];
#endif  // CONFIG_TWO_STAGE
  ...
  if (OUTPUT && 1) {
    ...
    fp = fopen("./debug/dec.txt", "a");

    fprintf(fp, "dqcoeff %d \n", type);
    for (r = 0; r < bs; r++) {
      for (c = 0; c < bs; c++) {
        fprintf(fp, "%4d ", dqcoeff[r * bs + c]);
      }
      fprintf(fp, "\n");
    }
    fprintf(fp, "\n \n");
...
#if 0
  if (OUTPUT && xd->mi[0].src_mi->mbmi.two_stage_coding[type]) {
    FILE *fp;
    ...
    fprintf(fp, "dqcoeff_stg2 %d \n", type);
    for (r = 0; r < bs; r++) {
      for (c = 0; c < bs; c++) {
        fprintf(fp, "%4d ", dqcoeff_stg2[r * bs + c]);
      }
      fprintf(fp, "\n");
    }
    fprintf(fp, "\n \n");
