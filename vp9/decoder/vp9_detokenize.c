...
#if CONFIG_TX_SKIP1
static int decode_coefs_tx_skip(VP9_COMMON *cm, const MACROBLOCKD *xd,
  ...
  while (c < max_eob) {
    ...
    if (!vp9_read(r, prob[EOB_CONTEXT_NODE])) {
      INCREMENT_COUNT(EOB_MODEL_TOKEN);
#if 1
      cm->stats_token[tx_size][type][ref][ctx][EOB_TOKEN]++;
#endif
      break;
    }
#if CONFIG_NEW_QUANT
    ...
    while (!vp9_read(r, prob[ZERO_CONTEXT_NODE])) {
      INCREMENT_COUNT(ZERO_TOKEN);
#if 1
      cm->stats_token[tx_size][type][ref][ctx][ZERO_TOKEN]++;
#endif
      dqv = dq[1];
      token_cache[scan[c]] = 0;
      ++c;
    ...
    if (!vp9_read(r, prob[ONE_CONTEXT_NODE])) {
      INCREMENT_COUNT(ONE_TOKEN);
#if 1
      cm->stats_token[tx_size][type][ref][ctx][ONE_TOKEN]++;
#endif
      token = ONE_TOKEN;
      val = 1;
    } else {
      ...
      token = vp9_read_tree(r, coeff_subtree_high,
                            vp9_pareto8_full[prob[PIVOT_NODE] - 1]);
#if 1
      cm->stats_token[tx_size][type][ref][ctx][token]++;
#endif
      switch (token) {
        case TWO_TOKEN:
        case THREE_TOKEN:
...
int vp9_decode_block_tokens(VP9_COMMON *cm, MACROBLOCKD *xd,
  ...
  if (xd->mi[0].src_mi->mbmi.tx_skip[plane != 0])
    eob = decode_coefs_tx_skip(cm, xd, pd->plane_type,
