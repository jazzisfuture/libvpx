...
#if CONFIG_MULTITHREAD
  // Check if worker has already released the ref_buf.
...
void vp9_frameworker_copy_context(VP9Worker *const dst_worker,
  ...
  dst_cm->last_frame_type = src_cm->frame_type;
  ...
  memcpy(dst_cm->lf.last_ref_deltas, src_cm->lf.ref_deltas, MAX_REF_LF_DELTAS);
  memcpy(dst_cm->lf.ref_deltas, src_cm->lf.ref_deltas, MAX_REF_LF_DELTAS);
  memcpy(dst_cm-> lf.last_mode_deltas, src_cm->lf.mode_deltas, MAX_MODE_LF_DELTAS);
