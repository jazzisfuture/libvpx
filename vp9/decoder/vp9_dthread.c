...
void vp9_frameworker_wait(VP9Worker *const worker,
                           RefCntBuffer *const ref_buf, int row, int col) {
#if CONFIG_MULTITHREAD
  if (!ref_buf)
    return;

  // Enable the following two lines of code will get harmless tsan error but
  // will get best performance.
  // if (ref_buf->row >= row && ref_buf->col >= col)
  //  return;

  {
    // Find the worker thread that owns the reference frame. If the reference
    // frame has been fully decoded, it may not have owner.
    VP9Worker *ref_worker = ref_buf->owner_frame_worker;
    FrameWorkerData *const ref_worker_data = (FrameWorkerData *)ref_worker->data1;
...
#ifdef DEBUG_THREAD
    {
      ...
      printf("%d %p worker is waiting for %d %p worker (%d %d) ref %d %d\r\n",
             worker_data->worker_id, worker, ref_worker_data->worker_id,
             ref_buf->owner_frame_worker, row, col, ref_buf->row, ref_buf->col);
    }
#endif

    pthread_mutex_lock(&ref_worker_data->stats_mutex);
    while (!(ref_buf->row >= row && ref_buf->col >= col)
            && pbi->cur_buf == ref_buf) {
      pthread_cond_wait(&ref_worker_data->stats_cond, &ref_worker_data->stats_mutex);
    }
    pthread_mutex_unlock(&ref_worker_data->stats_mutex);
  }
#else
  (void)ref_buf;
  (void)row;
...
void vp9_frameworker_broadcast(RefCntBuffer *const buf, int row, int col) {
#if CONFIG_MULTITHREAD
  VP9Worker *worker = buf->owner_frame_worker;
  FrameWorkerData *const worker_data = (FrameWorkerData *)worker->data1;

#ifdef DEBUG_THREAD
  printf("%d %p worker decode to (%d %d) \r\n", worker_data->worker_id,
         buf->owner_frame_worker, row, col);
#endif
...
void vp9_frameworker_copy_context(VP9Worker *const dst_worker,
  ...
  memcpy(dst_cm-> frame_contexts, src_cm->frame_contexts,
         FRAME_CONTEXTS * sizeof(FRAME_CONTEXT));
#else
  (void) dst_worker;
  (void) src_worker;
#endif  // CONFIG_MULTITHREAD
