...
static int vp9_setup_masks_cpu(struct task *tsk,
                               struct task_step *step) {
  struct lf_setup_masks_param *param = tsk->priv;
  int mi_row, mi_col;
  int start_mi_row = param->wid * MI_BLOCK_SIZE + param->start;

  for (mi_row = start_mi_row;
       mi_row < param->stop;
       mi_row += param->step_length) {
    for (mi_col = 0; mi_col < param->cm->mi_cols; mi_col += MI_BLOCK_SIZE) {
      vp9_loop_filter_setup_masks(param->cm, mi_row, mi_col);
    }
  }
...
static int vp9_setup_masks(struct task *tsk,
                           struct task_step *step,
                           int dev_type) {
  /*FIXME now we only support CPU */
  assert(dev_type == DEV_CPU);

  return vp9_setup_masks_cpu(tsk, step);
}
...
/**
 * This is for loopfilter setup_masks,
 * used with the RenderScript loopfilter intrinsic
 */
static struct task_step lf_steps_setup_masks[] = {
  {
    "vp9_lf_setup_masks",         // name
    STEP_KEEP,                    // type
    DEV_CPU,                      // dev_type
    0,                            // step_nr
    0,                            // next_steps_map
...
struct task_steps_pool *lf_steps_pool_setup_masks_get(void) {
  return task_steps_pool_create(lf_steps_setup_masks,
                                ARRAY_SZ(lf_steps_setup_masks));
}
...
struct lf_setup_masks_param *lf_setup_masks_param_get(struct task *tsk) {
...
void lf_setup_masks_param_put(struct task *tsk,
                              struct lf_setup_masks_param *param) {
  vpx_free(param);
}
