...
int alloc_buffers_recon(VP9_COMMON *cm, VP9_DECODER_RECON *decoder_recon) {
  ...
  int mi_size;
  int col_offset;
  int new_mode_info_stride;
  const int tile_cols = cm->log2_tile_cols;
  col_offset = 1 << cm->log2_tile_cols;
  free_buffers_recon(decoder_recon);

  mi_size = cm->mode_info_stride * (cm->mi_rows + MI_BLOCK_SIZE);
  new_mode_info_stride = (cm->mi_cols >> 3)  + 1;
  mi64x64_size = new_mode_info_stride * ((cm->mi_rows >> 3) + 1);

  if (tile_cols > 0) {
    mi_size = mi_size * DEV_THREE / col_offset * 2;
    mi64x64_size = mi64x64_size * DEV_THREE / col_offset * 2;
  }
  ...
  if (decoder_recon->dequant_recon)
    vpx_memset(decoder_recon->dequant_recon, 0,
               mi64x64_size * sizeof(DEQUANT_RECON));
  if (!decoder_recon->dequant_recon)
    goto fail;

  decoder_recon->inter_pre_recon =
      vpx_calloc(mi_size, sizeof(INTER_PRE_RECON));
  if (!decoder_recon->inter_pre_recon)
    goto fail;

  decoder_recon->intra_pre_recon =
      vpx_calloc(mi_size, sizeof(INTRA_PRE_RECON));
  if (!decoder_recon->intra_pre_recon)
    goto fail;
