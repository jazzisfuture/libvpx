...
int vp9_decode_frame(VP9D_COMP *pbi, const unsigned char **p_data_end) {
  ...
  /* tile info */
  {
    BOOL_DECODER bc_bak;
    int log2_tile_cols;
    const unsigned char *size_ptr = data + first_partition_length_in_bytes;
    const unsigned char *data_ptr;
    int tile, mb_start, mb_end;
#define INVERSE_TILE_DEC_ORDER 1
#if INVERSE_TILE_DEC_ORDER
    ...
    data_ptr = size_ptr + 3 * (pc->tile_columns - 1);
    ...
    for (tile = 1; tile < pc->tile_columns; tile++) {
      int size = size_ptr[0] + (size_ptr[1] << 8) + (size_ptr[2] << 16);
      data_ptr += size;
      size_ptr += 3;
      data_ptr2[tile] = data_ptr2[tile - 1] + size;
    }
    for (mb_end = pc->mb_cols, tile = pc->tile_columns - 1; tile >= 0; tile--) {
#else
    for (mb_start = 0, tile = 0; tile < pc->tile_columns; tile++) {
      ...
      mb_start = ((sb_start << 2) > pc->mb_cols) ? pc->mb_cols : (sb_start << 2);
#else
