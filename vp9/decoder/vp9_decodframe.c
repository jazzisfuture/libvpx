...
static void decode_sb_4x4(MACROBLOCKD *xd, int y_size) {
  ...
  for (n = 0; n < y_count; n++) {
    ...
    const TX_TYPE tx_type = get_tx_type_4x4(xd, y_idx * (y_size*2) + x_idx);
    if (tx_type == DCT_DCT) {
    ...
    switch (xd->mode_info_context->mbmi.txfm_size) {
      ...
      case TX_16X16:
        for (n = 0; n < 16; n++) {
          ...
          if (tx_type == DCT_DCT) {
            vp9_dequant_idct_add_16x16(
                BLOCK_OFFSET(xd->plane[0].qcoeff, n, 256),
                xd->block[0].dequant,
                xd->dst.y_buffer + y_offset,
                xd->dst.y_buffer + y_offset,
        ...
        for (n = 0; n < 4; n++) {
          ...
          vp9_dequant_idct_add_16x16(BLOCK_OFFSET(xd->plane[1].qcoeff, n, 256),
              xd->block[16].dequant,
              xd->dst.u_buffer + uv_offset,
              xd->dst.u_buffer + uv_offset,
              xd->dst.uv_stride, xd->dst.uv_stride, xd->eobs[256 + n * 16]);
          vp9_dequant_idct_add_16x16(BLOCK_OFFSET(xd->plane[2].qcoeff, n, 256),
              xd->block[20].dequant,
              xd->dst.v_buffer + uv_offset,
              xd->dst.v_buffer + uv_offset,
