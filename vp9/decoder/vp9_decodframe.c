...
static void skip_recon_mb(VP9D_COMP *pbi, MACROBLOCKD *xd) {
  if (xd->mode_info_context->mbmi.ref_frame == INTRA_FRAME) {
    ...
    if (xd->mode_info_context->mbmi.encoded_as_sb == 1) {
...
static void decode_superblock32(VP9D_COMP *pbi, MACROBLOCKD *xd,
  ...
  assert(xd->mode_info_context->mbmi.encoded_as_sb == 1);
...
static void set_offsets(VP9D_COMP *pbi, int block_size,
  ...
  xd->mode_info_context->mbmi.encoded_as_sb = block_size >> 5;
...
#define min(a, b) (a) < (b) ? (a) : (b)
    const int n_mbs = 1 << mbmi->encoded_as_sb;
...
#endif  // CONFIG_SUPERBLOCKS64
    {
      ...
      for (j = 0; j < 4; j++) {
        ...
        if (vp9_read(bc, pc->sb32_coded)) {
          ...
          vp9_decode_mb_mode_mv(pbi, xd, mb_row + y_idx_sb, mb_col + x_idx_sb, bc);
          set_refs(pbi, 32, mb_row + y_idx_sb, mb_col + x_idx_sb);
          decode_superblock32(pbi, xd, mb_row + y_idx_sb, mb_col + x_idx_sb, bc);
          xd->corrupted |= bool_error(bc);
        } else
#endif  // CONFIG_SUPERBLOCKS
        {
