...
static unsigned int read_le16(const void *mem) {
  const unsigned char *m = (const unsigned char *)mem;
  return (m[1] << 8) | m[0];
}

static unsigned int read_le32(const void *mem) {
  const unsigned char *m = (const unsigned char *)mem;
...
static void init_frame(VP9D_COMP *pbi) {
  VP9_COMMON *const pc = &pbi->common;
  MACROBLOCKD *const mb = &pbi->mb;

  if (pc->frame_type == KEY_FRAME) {
    vp9_setup_past_independence(pc, mb);
  ...
  if (pc->frame_type != KEY_FRAME) {
    ...
    vp9_setup_interp_filters(mb, pc->mcomp_filter_type, pc);
  }

  mb->mode_info_context = pc->mi;
  mb->prev_mode_info_context = pc->prev_mi;
  mb->frame_type = pc->frame_type;
  mb->mode_info_context->mbmi.mode = DC_PRED;
  mb->mode_info_stride = pc->mode_info_stride;
  mb->corrupted = 0;
  mb->fullpixel_mask = pc->full_pixel ? 0xfffffff8 : 0xffffffff;
}
  ...
  /* tile info */
  {
    ...
    if (pbi->oxcf.inv_tile_order) {
      ...
      for (tile_row = 0; tile_row < pc->tile_rows; tile_row++) {
        if (tile_row) {
          int size = read_le32(data_ptr2[tile_row - 1][n_cols - 1]);
          data_ptr2[tile_row - 1][n_cols - 1] += 4;
          data_ptr2[tile_row][0] = data_ptr2[tile_row - 1][n_cols - 1] + size;
        }

        for (tile_col = 1; tile_col < n_cols; tile_col++) {
          int size = read_le32(data_ptr2[tile_row][tile_col - 1]);
