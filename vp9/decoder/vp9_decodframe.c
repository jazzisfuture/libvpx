...
static void decode_8x8(VP9D_COMP *pbi, MACROBLOCKD *xd,
  ...
  if (tx_type != DCT_DCT || xd->mode_info_context->mbmi.mode == I8X8_PRED) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (tx_type != DCT_DCT) {
        vp9_ht_dequant_idct_add_8x8_c(tx_type, q, dq, dst, dst, stride, stride,
                                      xd->plane[0].eobs[idx]);
      } else {
  ...
  if (xd->mode_info_context->mbmi.mode == I8X8_PRED) {
    ...
    for (i = 0; i < 4; i++) {
      ...
      b = &xd->block[16 + i];
      vp9_intra_uv4x4_predict(xd, b, i8x8mode, *(b->base_dst) + b->dst, b->dst_stride);
...
static void decode_4x4(VP9D_COMP *pbi, MACROBLOCKD *xd,
  ...
  if (mode == I8X8_PRED) {
    for (i = 0; i < 4; i++) {
      ...
      BLOCKD *b = &xd->block[ib];
      int i8x8mode = b->bmi.as_mode.first;
      vp9_intra8x8_predict(xd, b, i8x8mode, *(b->base_dst) + b->dst, b->dst_stride);
      for (j = 0; j < 4; j++) {
...
#endif
      vp9_intra4x4_predict(xd, b, b_mode, *(b->base_dst) + b->dst, b->dst_stride);
