...
static void set_offsets(VP9D_COMP *pbi, BLOCK_SIZE_TYPE bsize,
  ...
  const int mb_idx = mi_row * cm->mode_info_stride + mi_col;
  const YV12_BUFFER_CONFIG *dst_fb = &cm->yv12_fb[cm->new_fb_idx];
  const int recon_yoffset = (MI_SIZE * mi_row) * dst_fb->y_stride + (MI_SIZE * mi_col);
  const int recon_uvoffset = (MI_UV_SIZE * mi_row) * dst_fb->uv_stride + (MI_UV_SIZE * mi_col);
  ...
  xd->left_context = cm->left_context + ((mi_row >> CONFIG_SB8X8) % 4);
...
static void decode_modes_sb(VP9D_COMP *pbi, int mi_row, int mi_col,
  ...
  if (bsize > BLOCK_SIZE_MB16X16) {
    ...
    partition = treed_read(r, vp9_partition_tree,
                           pc->fc.partition_prob[bsl - 1 - CONFIG_SB8X8]);
    pc->fc.partition_counts[bsl - 1 - CONFIG_SB8X8][partition]++;
  }
