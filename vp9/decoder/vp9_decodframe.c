...
static void predict_and_reconstruct_intra_block(int plane, int block, BLOCK_SIZE plane_bsize,
                               TX_SIZE tx_size, void *arg) {
  struct recon_args *args = arg;
...
static void decode_modes_b(VP9_COMMON *const cm, MACROBLOCKD *const xd,
  ...
  if (!is_inter_block(mbmi)) {
    ...
    int eobtotal = 0;
    struct recon_args arg = { cm, xd, r, &eobtotal };
