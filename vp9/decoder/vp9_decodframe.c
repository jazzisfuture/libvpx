...
static void setup_key_frame_size(VP9D_COMP *pbi,
                                 struct vp9_read_bit_buffer *rb) {
  VP9_COMMON *const cm = &pbi->common;
  int width, height;
  read_frame_size(cm, rb, &width, &height);

  // printf("<= Reading (key frame):   %d x %d\n", cm->width, cm->height);
...
static void setup_inter_frame_size(VP9D_COMP *pbi,
                                   struct vp9_read_bit_buffer *rb) {
  VP9_COMMON *const cm = &pbi->common;
  ...
  // printf("<= Reading (inter frame): %d x %d\n", cm->width, cm->height);
