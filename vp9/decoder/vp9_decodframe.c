...
static void init_frame(VP9D_COMP *pbi) {
  ...
  if (pc->frame_type == KEY_FRAME) {
    printf("decode setup key\n");
    vp9_setup_common_key_frame(pc, xd);
  } else if (pc->error_resilient_mode) {
    printf("decode setup nonkey er mode\n");
    vp9_setup_past_independence(pc, xd);
  } else {
    printf("decode setup nonkey\n");
  }
...
int vp9_decode_frame(VP9D_COMP *pbi, const unsigned char **p_data_end) {
  ...
  if (data_end - data < 3) {
    ...
    printf("Partition length %d\n", first_partition_length_in_bytes); fflush(stdout);
