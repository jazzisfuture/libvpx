...
static int read_size(const uint8_t *data, const uint8_t *data_end,
                     int *width, int *height) {
    if (data + 4 < data_end) {
        *width = read_le16(data + 0);
...
static const uint8_t *setup_frame_size(VP9D_COMP *pbi, int scaling_active,
  ...
  if (scaling_active &&
      read_size(data, data_end, &display_width, &display_height))
    data += 4;

  if (read_size(data, data_end, &width, &height))
    data += 4;
