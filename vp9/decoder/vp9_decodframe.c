...
static int read_is_valid(const uint8_t *start, uint32_t len,
                         const uint8_t *end) {
...
static void decode_16x16(VP9D_COMP *pbi, MACROBLOCKD *xd,
  ...
  if (tx_type != DCT_DCT) {
    ...
    vp9_dequant_idct_add_16x16(xd->qcoeff,
                               xd->block[0].dequant,
                               xd->predictor,
                               xd->dst.y_buffer,
                               16, xd->dst.y_stride,
  ...
  vp9_dequant_idct_add_uv_block_8x8(xd->qcoeff + 16 * 16,
                                    xd->block[16].dequant,
                                    xd->predictor + 16 * 16,
...
static void setup_token_decoder(VP9D_COMP *pbi,
  ...
  const uint32_t partition_size = (uint32_t)(data_end - data);
...
int vp9_decode_frame(VP9D_COMP *pbi, const uint8_t **p_data_end) {
  ...
  uint32_t first_partition_size = 0;
