...
int vp9_decode_frame(VP9D_COMP *pbi, const uint8_t **p_data_end) {
  ...
  // new_fb->corrupted |=
  // read_compressed_header(pbi, data, first_partition_size);
  if (vp9_reader_init(&r, data, first_partition_size))
    vpx_internal_error(&pc->error, VPX_CODEC_MEM_ERROR,
                       "Failed to allocate bool decoder 0");
  pc->txfm_mode = xd->lossless ? ONLY_4X4 : read_tx_mode(&r);
  if (pc->txfm_mode == TX_MODE_SELECT)
    read_tx_probs(&pc->fc, &r);
  read_coef_probs(&pc->fc, pc->txfm_mode, &r);
  vp9_prepare_read_mode_info(pbi, &r);
  ...
  new_fb->corrupted = vp9_reader_has_error(&r) | xd->corrupted;
