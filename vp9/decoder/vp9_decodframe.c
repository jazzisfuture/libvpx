...
static void decode_block_intra_with_BM(int plane, int block, BLOCK_SIZE_TYPE bsize,
                               int ss_txfrm_size, void *arg) {
  ...
  if(b_mode == BM_PRED)
  {
    ...
    sad = template_matching_L(&(pbi -> common), xd, pd->dst.buf + jj * 4 * pd->dst.stride + 4 * ii, pd->dst.stride,
