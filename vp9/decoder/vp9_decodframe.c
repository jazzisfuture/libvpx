...
// Reads the next tile adjusting '*data' and setting '*size' accordingly based
// on 'is_last'.
static void get_tile(const uint8_t *const data_end,
                     int is_last,
                     struct vpx_internal_error_info *error_info,
  ...
  if (pbi->oxcf.inv_tile_order) {
    ...
    for (tile_row = 0; tile_row < tile_rows; tile_row++) {
      for (tile_col = 0; tile_col < tile_cols; tile_col++) {
        ...
        size_t size;
        get_tile(data_end, last_tile, &cm->error, &data, &size);
...
static const uint8_t *decode_tiles_mt(VP9D_COMP *pbi, const uint8_t *data) {
  ...
  while (tile_col < tile_cols) {
    ...
    for (i = 0; i < num_workers && tile_col < tile_cols; ++i) {
      ...
      get_tile(data_end, tile_col == tile_cols - 1,
               &cm->error, &data, &size);
...

  {
    ...
    return vp9_reader_find_end(
        &((TileWorkerData*)pbi->tile_workers[final_worker].data1)->bit_reader);
  }
}
