...
static void mb_init_dequantizer(VP9D_COMP *pbi, MACROBLOCKD *xd) {
  ...
  xd->idct_add              = vp9_dequant_idct_add;
  xd->dc_only_idct_add      = vp9_dc_only_idct_add_c;
  xd->dc_idct_add           = vp9_dequant_dc_idct_add;
  xd->dc_idct_add_y_block   = vp9_dequant_dc_idct_add_y_block;
  xd->idct_add_y_block      = vp9_dequant_idct_add_y_block;
  xd->idct_add_uv_block     = vp9_dequant_idct_add_uv_block;
#if CONFIG_LOSSLESS
  if (xd->lossless) {
    assert(QIndex == 0);
    ...
    xd->idct_add              = vp9_dequant_idct_add_lossless_c;
    xd->dc_only_idct_add      = vp9_dc_only_inv_walsh_add_c;
    xd->dc_idct_add           = vp9_dequant_dc_idct_add_lossless_c;
    xd->dc_idct_add_y_block   = vp9_dequant_dc_idct_add_y_block_lossless_c;
    xd->idct_add_y_block      = vp9_dequant_idct_add_y_block_lossless_c;
    xd->idct_add_uv_block     = vp9_dequant_idct_add_uv_block_lossless_c;
  }
#endif
