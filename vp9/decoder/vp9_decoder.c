...
int vp9_dec_alloc_row_mt_mem(RowMtHandle *row_mt_handle, VP9_COMMON *cm,
  ...
  for (plane = 0; plane < 3; ++plane) {
    CHECK_MEM_ERROR(cm, row_mt_handle->dqcoeff[plane],
                    vpx_calloc(num_sbs << DQCOEFFS_PER_SB_LOG2,
                               sizeof(*row_mt_handle->dqcoeff)));
    CHECK_MEM_ERROR(
        cm, row_mt_handle->eob[plane],
        vpx_calloc(num_sbs << EOBS_PER_SB_LOG2, sizeof(*row_mt_handle->eob)));
  }
  CHECK_MEM_ERROR(cm, row_mt_handle->partition,
                  vpx_calloc((num_sbs * PARTITIONS_PER_SB),
                             sizeof(*row_mt_handle->partition)));
  CHECK_MEM_ERROR(
      cm, row_mt_handle->recon_map,
      (int8_t *)vpx_calloc(num_sbs, sizeof(*row_mt_handle->recon_map)));
  return 0;
}
