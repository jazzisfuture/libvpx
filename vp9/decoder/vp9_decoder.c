...
static INLINE void flush_all_fb_on_key(VP9Decoder *pbi, const uint8_t *data,
  ...
  // This is only used to parse if the frame is key or not.
  ...
  if (frame_type == KEY_FRAME && pbi->common.current_video_frame > 0) {
    VP9_COMMON *const cm = &pbi->common;
    RefCntBuffer *const frame_bufs = cm->buffer_pool->frame_bufs;
    BufferPool *const pool = cm->buffer_pool;
    int i;
...
int vp9_receive_compressed_data(VP9Decoder *pbi, size_t size,
  ...
  if (pbi->need_resync) flush_all_fb_on_key(pbi, source, source + size);
