...
static void swap_frame_buffers(VP9Decoder *pbi) {
  ...
  for (mask = pbi->refresh_frame_flags; mask; mask >>= 1) {
    if (mask & 1) {
      ...
      // No need to increase current frame's reference count as it is
      // already increased in generating next_ref_frame_map.
      pool->frame_bufs[cm->new_fb_idx].ref_count += pbi->frame_parallel_decode ?
                                                    0 : 1;
      cm->ref_frame_map[ref_index] = cm->new_fb_idx;
      if (old_idx >= 0) {
