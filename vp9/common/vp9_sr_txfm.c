...
void sr_recon(int16_t *src, int src_stride, uint8_t *dst, int dst_stride,
              int w, int h) {
#endif
  // DECLARE_ALIGNED_ARRAY(16, int16_t, enh_resi, 64 * 64);
  // int enh_resi_stride = 64;
  DECLARE_ALIGNED_ARRAY(16, int16_t, us_resi, 64 * 64);
  int ii, jj, us_resi_stride = 64;

  sr_upsample(src, src_stride, us_resi, us_resi_stride, w/2, h/2);
/*
#if SR_USE_MULTI_F
  sr_post_filter(us_resi, us_resi_stride, enh_resi, enh_resi_stride, w, h,
                 f_hor, f_ver);
#else
  sr_post_filter(us_resi, us_resi_stride, enh_resi, enh_resi_stride, w, h);
#endif
*/
  for (ii = 0; ii < h; ii++) {
    for (jj = 0; jj < w; jj++) {
      dst[jj] = clip_pixel_add(dst[jj], us_resi[jj]);
    }
    dst += dst_stride;
    us_resi += us_resi_stride;
  }
}
