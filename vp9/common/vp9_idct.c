...
static void maybe_flip_strides(uint8_t **dst, int *dstride, tran_low_t **src,
                               int *sstride, int tx_type, int size) {
...
void idst8(const tran_low_t *input, tran_low_t *output) {
  ...
  static const int32_t sinvalue_lookup[] = {86559612, 162678858, 219176632,
                                            249238470};
  int64_t sum;
...
void idst16(const tran_low_t *input, tran_low_t *output) {
  ...
  static const int32_t sinvalue_lookup[] = {47852167,  94074787,  137093803,
...
void vp9_iht4x4_16_add_c(const tran_low_t *input, uint8_t *dest, int stride,
                         int tx_type) {
  // TODO(yongzhe): pull this out to vp9_idct.h
...
// TODO(yongzhe): pull this out to .h.
...
#if CONFIG_TX_SKIP
void vp9_tx_identity_add_rect(const tran_low_t *input, uint8_t *dest, int row,
                              int col, int stride_in, int stride_out,
                              int shift) {
  int r, c, temp;
  for (r = 0; r < row; r++)
    for (c = 0; c < col; c++) {
...
void vp9_tx_identity_add(const tran_low_t *input, uint8_t *dest, int stride,
                         int bs, int shift) {
...
void vp9_highbd_tx_identity_add_rect(const tran_low_t *input, uint8_t *dest8,
  ...
  for (r = 0; r < row; r++)
    for (c = 0; c < col; c++) {
      dest[r * stride_out + c] = highbd_clip_pixel_add(
          dest[r * stride_out + c], input[r * stride_in + c] >> shift, bd);
    }
}
...
void vp9_idct64x64_4096_add_c(const tran_low_t *input, uint8_t *dest,
  ...
  // vp9_clear_system_state();  // Make it simd safe : __asm emms;
  {
    ...
    for (i = 0; i < 64; ++i) {
      ...
      for (j = 0; j < 64; ++j) temp_in[j] = input[j + i * 64];
      butterfly_64_idct_1d(temp_in, temp_out, 1);
      for (j = 0; j < 64; ++j) out[j + i * 64] = temp_out[j];
    }
    // Then transform columns
    for (i = 0; i < 64; ++i) {
      double temp_in[64], temp_out[64];
      for (j = 0; j < 64; ++j) temp_in[j] = out[j * 64 + i];
      butterfly_64_idct_1d(temp_in, temp_out, 1);
      for (j = 0; j < 64; ++j) out2[j * 64 + i] = temp_out[j];
    }
...
#if CONFIG_EXT_TX
  maybe_flip_strides((uint8_t **)&dest, &stride, &outp, &outstride, tx_type,
                     4 * 2);
#endif
...
void vp9_highbd_idct64x64_4096_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  // vp9_clear_system_state();  // Make it simd safe : __asm emms;
  {
    ...
    for (j = 0; j < 64; ++j) {
      for (i = 0; i < 64; ++i)
        dest[i] =
            highbd_clip_pixel_add(dest[i], round(out2[j * 64 + i] / 128), bd);
      dest += stride;
    }
  }
  // vp9_clear_system_state();  // Make it simd safe : __asm emms;
...
void vp9_idct4x4_1_c(const tran_low_t *input, int16_t *dest, int dest_stride) {
  int i;
  tran_high_t a1;
  tran_low_t out = WRAPLOW(dct_const_round_shift(input[0] * cospi_16_64), 8);
  out = WRAPLOW(dct_const_round_shift(out * cospi_16_64), 8);
...
void vp9_idct16x16_256_c(const tran_low_t *input, int16_t *dest, int stride) {
  tran_low_t out[16 * 16];
  tran_low_t *outptr = out;
  int i, j;
  tran_low_t temp_in[16], temp_out[16];
...
void vp9_idct16x16_10_c(const tran_low_t *input, int16_t *dest, int stride) {
  tran_low_t out[16 * 16] = {0};
  tran_low_t *outptr = out;
  int i, j;
  tran_low_t temp_in[16], temp_out[16];
...
void vp9_idct16x16_1_c(const tran_low_t *input, int16_t *dest, int stride) {
  ...
  a1 = ROUND_POWER_OF_TWO(out, 6);
  for (j = 0; j < 16; ++j) {
    for (i = 0; i < 16; ++i) dest[i] = a1;
    dest += stride;
  }
}

void vp9_idct32x32_1024_c(const tran_low_t *input, int16_t *dest, int stride) {
  tran_low_t out[32 * 32];
  tran_low_t *outptr = out;
  int i, j;
  tran_low_t temp_in[32], temp_out[32];
...
void vp9_idct32x32_34_c(const tran_low_t *input, int16_t *dest, int stride) {
  tran_low_t out[32 * 32] = {0};
  tran_low_t *outptr = out;
  int i, j;
  tran_low_t temp_in[32], temp_out[32];
...
void vp9_idct32x32_1_c(const tran_low_t *input, int16_t *dest, int stride) {
  ...
  for (j = 0; j < 32; ++j) {
    for (i = 0; i < 32; ++i) dest[i] = a1;
    dest += stride;
  }
}

// idct
void vp9_idct4x4(const tran_low_t *input, int16_t *dest, int stride, int eob) {
  if (eob > 1)
...
void vp9_iwht4x4(const tran_low_t *input, int16_t *dest, int stride, int eob) {
  if (eob > 1)
...
void vp9_idct8x8(const tran_low_t *input, int16_t *dest, int stride, int eob) {
  // If dc is 1, then input[0] is the reconstructed value, do not need
  // dequantization. Also, when dc is 1, dc is counted in eobs, namely eobs >=1.
...
#if CONFIG_TX64X64
void vp9_idct64x64_4096_c(const tran_low_t *input, int16_t *dest, int stride) {
  // vp9_clear_system_state();  // Make it simd safe : __asm emms;
  {
    double out[64 * 64], out2[64 * 64];
    int i, j;
    ...
    for (j = 0; j < 64; ++j) {
      for (i = 0; i < 64; ++i) dest[i] = round(out2[j * 64 + i] / 128);
      dest += stride;
    }
  }
  // vp9_clear_system_state();  // Make it simd safe : __asm emms;
