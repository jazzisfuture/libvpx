...
extern double dstmtx4[4*4];
extern double dstmtx8[8*8];
extern double dstmtx16[16*16];
...
void vp9_idst_add(const int16_t *input, uint8_t *dest, int stride, int eob, int size) {
  ...
  for (i = 0; i < size; i++) {
    for (j = 0; j < size; j++) {
      tmp[i*size+j] = 0;
      for (k = 0; k < size; k++) {
        tmp[i*size+j] += input[i*size+k] * basis[j*size+k]; //r
      }
    }
  }
  ...
  for (i = 0; i < size; i++){
    for (j = 0; j < size; j++){
      tmp2[i*size+j] = 0;
      for (k = 0; k < size; k++){
        tmp2[i*size+j] += basis[i*size+k] * tmp[k*size+j]; //c
      }
      if (tmp2[i*size+j] >= 0)
        dest[i*stride+j] = clip_pixel((int)dest[i*stride+j] + (int)(tmp2[i*size+j] / factor + 0.5));
      else
        dest[i*stride+j] = clip_pixel((int)dest[i*stride+j] + (int)(tmp2[i*size+j] / factor - 0.5));
    }
  }
