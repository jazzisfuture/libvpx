...
void vp9_high_iwht4x4_1_add_c(const tran_low_t *in, uint8_t *dest8,
  ...
  for (i = 0; i < 4; i++) {
    e1 = ip[0] >> 1;
    a1 = ip[0] - e1;
    dest[dest_stride * 0] = clip_pixel_add_highbd(
        dest[dest_stride * 0], a1, bd);
    dest[dest_stride * 1] = clip_pixel_add_highbd(
        dest[dest_stride * 1], e1, bd);
    dest[dest_stride * 2] = clip_pixel_add_highbd(
        dest[dest_stride * 2], e1, bd);
    dest[dest_stride * 3] = clip_pixel_add_highbd(
