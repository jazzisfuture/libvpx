...
void vpx_idct16x16_add(const tran_low_t *input, uint8_t *dest, int stride,
                       int eob) {
  /* The calculation can be simplified if there are not many non-zero dct
...
void vp9_iht16x16_add(TX_TYPE tx_type, const tran_low_t *input, uint8_t *dest,
                      int stride, int eob) {
  if (tx_type == DCT_DCT) {
    vpx_idct16x16_add(input, dest, stride, eob);
  } else {
...
void vp9_highbd_iht4x4_16_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  const highbd_transform_2d IHT_4[] = {
    ...
    { highbd_iadst4_c, vpx_highbd_idct4_c },    // ADST_DCT = 1
    { vpx_highbd_idct4_c, highbd_iadst4_c },    // DCT_ADST = 2
...
static const highbd_transform_2d HIGH_IHT_8[] = {
  { vpx_highbd_idct4_c,  vpx_highbd_idct4_c  },  // DCT_DCT  = 0
  { highbd_iadst8_c, vpx_highbd_idct4_c  },  // ADST_DCT = 1
  { vpx_highbd_idct4_c,  highbd_iadst8_c },  // DCT_ADST = 2
...
static const highbd_transform_2d HIGH_IHT_16[] = {
  ...
  { highbd_iadst16_c, vpx_highbd_idct16_c  },  // ADST_DCT = 1
  { vpx_highbd_idct16_c,  highbd_iadst16_c },  // DCT_ADST = 2
...
void vpx_highbd_idct4x8_add(const tran_low_t *input, uint8_t *dest, int stride,
  ...
  // DC only DCT coefficient
  if (eob == 1) {
    vpx_highbd_idct4x8_1_add(input, dest, stride, bd);
  } else if (eob <= 10) {
    vpx_highbd_idct4x8_10_add(input, dest, stride, bd);
  } else {
    vpx_highbd_idct4x8_64_add(input, dest, stride, bd);
  }
}
...
void vp9_highbd_iht4x4_add(TX_TYPE tx_type, const tran_low_t *input,
  ...
  if (tx_type == DCT_DCT)
    vpx_highbd_idct4x4_add(input, dest, stride, eob, bd);
  else
...
void vp9_highbd_iht8x8_add(TX_TYPE tx_type, const tran_low_t *input,
                           uint8_t *dest, int stride, int eob, int bd) {
  if (tx_type == DCT_DCT) {
    vpx_highbd_idct4x8_add(input, dest, stride, eob, bd);
  } else {
...
void vp9_highbd_iht16x16_add(TX_TYPE tx_type, const tran_low_t *input,
                           uint8_t *dest, int stride, int eob, int bd) {
  if (tx_type == DCT_DCT) {
    vpx_highbd_idct16x16_add(input, dest, stride, eob, bd);
  } else {
