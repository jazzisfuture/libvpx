...
// WRAPLOW() macro below below that is identical to their intended
...
static INLINE tran_low_t clip_pixel_add_highbd(tran_high_t dest,
...
void vp9_high_iwht4x4_16_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  for (i = 0; i < 4; i++) {
    ...
    a1 -= b1;
    d1 += c1;
    dest[stride * 0] = clip_pixel_add_highbd(dest[stride * 0], a1, bd);
    dest[stride * 1] = clip_pixel_add_highbd(dest[stride * 1], b1, bd);
    dest[stride * 2] = clip_pixel_add_highbd(dest[stride * 2], c1, bd);
    dest[stride * 3] = clip_pixel_add_highbd(dest[stride * 3], d1, bd);
...
void vp9_high_iwht4x4_1_add_c(const tran_low_t *in, uint8_t *dest8,
  ...
  for (i = 0; i < 4; i++) {
    e1 = ip[0] >> 1;
    a1 = ip[0] - e1;
    dest[dest_stride * 0] = clip_pixel_add_highbd(dest[dest_stride * 0], a1, bd);
    dest[dest_stride * 1] = clip_pixel_add_highbd(dest[dest_stride * 1], e1, bd);
    dest[dest_stride * 2] = clip_pixel_add_highbd(dest[dest_stride * 2], e1, bd);
    dest[dest_stride * 3] = clip_pixel_add_highbd(dest[dest_stride * 3], e1, bd);
    ip++;
    dest++;
  }
}
...
void vp9_high_idct4x4_16_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  for (i = 0; i < 4; ++i) {
    ...
    for (j = 0; j < 4; ++j)
      dest[j * stride + i] = clip_pixel_add_highbd(
...
void vp9_high_idct4x4_1_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  for (i = 0; i < 4; i++) {
    dest[0] = clip_pixel_add_highbd(dest[0], a1, bd);
    dest[1] = clip_pixel_add_highbd(dest[1], a1, bd);
    dest[2] = clip_pixel_add_highbd(dest[2], a1, bd);
    dest[3] = clip_pixel_add_highbd(dest[3], a1, bd);
    dest += dest_stride;
  }
}
...
void vp9_high_idct8x8_1_add_c(const tran_low_t *input, uint8_t *dest8,
  ...
  for (j = 0; j < 8; ++j) {
    for (i = 0; i < 8; ++i)
      dest[i] = clip_pixel_add_highbd(dest[i], a1, bd);
    dest += stride;
  }
}
