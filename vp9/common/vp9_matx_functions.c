...
void vp9_matx_imwrite(const MATX* const image,
  ...
  switch (image->typeid) {
    case TYPE_8U:
      VP9_MATX_IMWRITE(uint8_t,  image, image_file, "%hhu ");
      break;
    case TYPE_8S:
      VP9_MATX_IMWRITE(int8_t,   image, image_file, "%hhd ");
      break;
    case TYPE_16U:
      VP9_MATX_IMWRITE(uint16_t, image, image_file,  "%hu ");
      break;
    case TYPE_16S:
      VP9_MATX_IMWRITE(int16_t,  image, image_file,  "%hd ");
      break;
    case TYPE_32U:
      VP9_MATX_IMWRITE(uint32_t, image, image_file,   "%u ");
      break;
    case TYPE_32S:
      VP9_MATX_IMWRITE(int32_t,  image, image_file,   "%d ");
      break;
    default:
      vpx_runtime_assert(0 /* matx: inapprorpiate type */);
  }
