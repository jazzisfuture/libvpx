...
void inverse_spin_order(int *order, int cols, int r_start, int c_start,
                        int h, int w, int idx) {
  int r, c;
  ...
  for (r = 0; r < h; r++)
    order[idx++] = (r + r_start) * cols + w + c_start;

  for (c = 0; c < w; c++)
    order[idx++] = (h + r_start) * cols + w - c + c_start;

  for (r = 0; r < h; r++)
    order[idx++] = (h - r + r_start) * cols + c_start;

  inverse_spin_order(order, cols, r_start + 1, c_start + 1, h - 2, w - 2, idx);
}

void inverse_spin_scan_order(int *order, int rows, int cols) {
  inverse_spin_order(order, cols, 0, 0, rows - 1, cols - 1, 0);
}
#endif
