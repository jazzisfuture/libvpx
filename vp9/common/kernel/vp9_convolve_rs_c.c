...
void build_inter_pred_calcu_rs_c(
  ...
  for (i = 0; i < fri_block_count; ++i) {
    ...
    src = ref_buffer + pred_param_fri[i].src_mv;
    dst = ref_buffer + pred_param_fri[i].dst_mv;
  ...
  for (i = 0; i < sec_block_count; ++i) {
    ...
    src = ref_buffer + pred_param_sec[i].src_mv;
    dst = ref_buffer + pred_param_sec[i].dst_mv;
