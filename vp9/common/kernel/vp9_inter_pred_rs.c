  ...
  if (scaled_mv.col || scaled_mv.row ||
    ...
    if (x0 < 0 || x0 > frame_width - 1 || x1 < 0 || x1 > frame_width ||
      ...
      uint8_t *buf_ptr1 = ref_frame + y0 * pre_buf->stride + x0;
        // Extend the border.
      build_mc_border(buf_ptr1, pre_buf->stride, inter_rs_obj.pref[tile_num],
                      x1 - x0, x0, y0, x1 - x0, y1 - y0, frame_width,
                      frame_height);
      buf_stride = x1 - x0;
      buf_ptr = inter_rs_obj.pref[tile_num] +
...
int inter_pred_calcu_rs(VP9_COMMON *const cm, const int tile_num) {
  ...
  if (inter_rs_obj.gpu_index_count[tile_num] > 0) {
    ...
    for (i = 0; i < inter_rs_obj.gpu_block_count[tile_num]; ++i) {
      const uint8_t *src = inter_rs_obj.new_buffer[tile_num] +
                           pred_param[i].dst_mv;
