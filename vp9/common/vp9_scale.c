...
static INLINE int scaled_x(int val, const void *scale) {
  const struct scale_factors_common *sfc = scale;
  return val * sfc->x_scale_fp >> REF_SCALE_SHIFT;
}

static INLINE int scaled_y(int val, const void *scale) {
  const struct scale_factors_common *sfc = scale;
  return val * sfc->y_scale_fp >> REF_SCALE_SHIFT;
}

static int unscaled_value(int val, const void *s) {
  return val;
}

static MV32 scaled_mv(const MV *mv, const void *s) {
  const struct scale_factors *scale = s;
  const MV32 res = {
...
static MV32 unscaled_mv(const MV *mv, const void *s) {
  const MV32 res = {
    mv->row,
    mv->col
  };
