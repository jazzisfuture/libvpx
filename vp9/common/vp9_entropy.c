...
static const vp9_zpc_probs default_zpc_probs_4x4[PLANE_TYPES] = {
  { /* Y-plane */
    { /* Intra */
      { /* Coeff Band 0 */
        200, 216, 232,
...
static const vp9_zpc_probs default_zpc_probs_8x8[PLANE_TYPES] = {
  { /* Y-plane */
    { /* Intra */
      { /* Coeff Band 0 */
        200, 216, 232,
...
static const vp9_zpc_probs default_zpc_probs_16x16[PLANE_TYPES] = {
  { /* Y-plane */
    { /* Intra */
      { /* Coeff Band 0 */
        200, 216, 232,
...
static const vp9_zpc_probs default_zpc_probs_32x32[PLANE_TYPES] = {
  { /* Y-plane */
    { /* Intra */
      { /* Coeff Band 0 */
        200, 216, 232,
...
#if CONFIG_TX64X64
static const vp9_zpc_probs default_zpc_probs_64x64[PLANE_TYPES] = {
  { /* Y-plane */
    { /* Intra */
      { /* Coeff Band 0 */
        200, 216, 232,
...
static int is_eoo_used(TX_SIZE tx_size, int band) {
  return (tx_size >= TX_32X32 && band > 1);
}
...
#if ONFIG_CODE_ZEROGROUP
  for (i = 0; i < PLANE_TYPES; ++i) {
    const vp9_zpc_probs *zpc_preprobs = &pre_fc->zpc_probs[tx_size][i];
    vp9_zpc_count *const zpc_count = &cm->counts.zpc[tx_size][i];
    vp9_zpc_probs *zpc_probs = &cm->fc.zpc_probs[tx_size][i];
    for (j = 0; j < REF_TYPES; ++j) {
      for (k = 0; k < COEF_BANDS; ++k) {
        if (!is_eoo_used(tx_size, k))
          continue;
        for (l = 0; l < ZPC_PTOKS; ++l) {
          (*zpc_probs)[j][k][l] = merge_probs((*zpc_preprobs)[j][k][l],
                                           (*zpc_count)[j][k][l],
                                           count_sat, update_factor);
        }
      }
