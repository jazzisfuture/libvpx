...
static const vp9_zpc_probs default_zpc_probs[ZPC_TX_SIZES][PLANE_TYPES] = {
  { /* TX_32X32 */
    { /* Y-plane */
      { /* Intra */
        { /* Coeff Band 3 */
...
static int is_eoo_used(TX_SIZE tx_size, int pt, int band) {
  return (tx_size >= ZPC_MIN_TX_SIZE &&
          pt <= ZPC_MAX_PT &&
          band >= ZPC_MIN_BAND);
}
...
#endif  // CONFIG_QCTX_TPROBS
#if CONFIG_CODE_ZEROGROUP
  vp9_copy(cm->fc.zpc_probs, default_zpc_probs);
#endif  // CONFIG_CODE_ZEROGROUP
}
...
#if CONFIG_CODE_ZEROGROUP
  if (tx_size >= ZPC_MIN_TX_SIZE) {
    for (i = 0; i < PLANE_TYPES; ++i) {
      const vp9_zpc_probs *zpc_preprobs =
          &pre_fc->zpc_probs[tx_size - ZPC_MIN_TX_SIZE][i];
      vp9_zpc_count *const zpc_count =
          &cm->counts.zpc[tx_size - ZPC_MIN_TX_SIZE][i];
      vp9_zpc_probs *zpc_probs =
          &cm->fc.zpc_probs[tx_size - ZPC_MIN_TX_SIZE][i];
      for (j = 0; j < REF_TYPES; ++j) {
