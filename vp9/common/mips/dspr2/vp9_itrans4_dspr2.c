...
#if HAVE_DSPR2
static void vp9_idct4_1d_rows_dspr2(int16_t *input, int16_t *output) {
...
void vp9_idct4_1d_columns_add_blk_dspr2(int16_t *input, uint8_t *dest, int dest_stride) {
  int16_t step_0, step_1, step_2, step_3;
  int Temp0, Temp1, Temp2, Temp3;
  const int const_2_power_13 = 8192;
  int i;
...
void vp9_short_idct4x4_add_dspr2(int16_t *input, uint8_t *dest, int dest_stride) {
  int16_t out[4 * 4];
  int16_t *outptr = out;
  uint32_t pos = 45;
...
void vp9_short_idct4x4_1_add_dspr2(int16_t *input, uint8_t *dest, int dest_stride) {
  int a1, absa1;
  int r;
  int32_t out;
  ...
  DCT_CONST_ROUND_SHIFT_TWICE(input_dc, out, DCT_CONST_ROUNDING, cospi_16_64);
  __asm__ __volatile__ (
      "addi     %[out],     %[out],    8       \n\t"
      "sra      %[a1],      %[out],    4       \n\t"
...
static void iadst4_1d_dspr2(int16_t *input, int16_t *output) {
...
void vp9_short_iht4x4_add_dspr2(int16_t *input, uint8_t *dest, int dest_stride, int tx_type) {
  int i, j;
  int16_t out[4 * 4];
  int16_t *outptr = out;
  ...
  switch(tx_type) {
    ...
    case ADST_DCT:  // ADST in vertical, DCT in horizontal
      ...
      for (i = 0; i < 4; ++i) {
        iadst4_1d_dspr2(&out[i*4], temp_out);

        for (j = 0; j < 4; ++j)
          dest[j * dest_stride + i] = clip_pixel(ROUND_POWER_OF_TWO(temp_out[j], 4) + dest[j * dest_stride + i]);
      }
      break;
