...
static inline int vp9_wide_mbfilter_neon(uint8x8_t dblimit,  // flimit
...
void vp9_lpf_horizontal_16_neon(uint8_t *src,
                                int p,
  ...
  for (; count > 0; count--, src += 8) {
    ...
    /*
     * Fix compiling error reported by '-Werror=maybe-uninitialized' only
     */
    d16u8 = d15u8;
    d18u8 = d15u8;
    d19u8 = d15u8;
    ...
    ret = vp9_wide_mbfilter_neon(dblimit, dlimit, dthresh,
                                 d0u8, d1u8, d2u8, d3u8,
                                 d4u8, d5u8, d6u8, d7u8,
                                 d8u8, d9u8, d10u8, d11u8,
                                 d12u8, d13u8, d14u8, d15u8,
                                 &d1u8, &d2u8, &d3u8, &d16u8,
                                 &d18u8, &d19u8, &d20u8,
                                 &d21u8, &d22u8, &d23u8, &d24u8,
                                 &d25u8, &d26u8, &d27u8);
...
void vp9_lpf_vertical_16_neon(uint8_t *src,
                              int p,
                              const uint8_t *blimit,
                              const uint8_t *limit,
                              const uint8_t *thresh) {
  ...
  case 1: {
    ...
    vst4_lane_u8(st, d0x4u8, 0);
    st += p;
    vst4_lane_u8(st, d0x4u8, 1);
    st += p;
    vst4_lane_u8(st, d0x4u8, 2);
    st += p;
    vst4_lane_u8(st, d0x4u8, 3);
    st += p;
    vst4_lane_u8(st, d0x4u8, 4);
    st += p;
    vst4_lane_u8(st, d0x4u8, 5);
    st += p;
    vst4_lane_u8(st, d0x4u8, 6);
    st += p;
    vst4_lane_u8(st, d0x4u8, 7);
    break;
  }
  case 2: {  // v_mbfilter
    for (st = src - 3, i = 0; i < 2; st = src, i++) {
      d0x3u8.val[0] = d18u8;
      d0x3u8.val[1] = d19u8;
      d0x3u8.val[2] = d20u8;
  ...
  default: {  // v_wide_mbfilter
    for (st = src - 8, i = 0; i < 2; i++, st = src) {
      ...
      vst1_u8(st, d0x2u8.val[0]);
      st += p;
      vst1_u8(st, d0x2u8.val[1]);
      st += p;
      vst1_u8(st, d1x2u8.val[0]);
      st += p;
      vst1_u8(st, d1x2u8.val[1]);
      st += p;
      vst1_u8(st, d2x2u8.val[0]);
      st += p;
      vst1_u8(st, d2x2u8.val[1]);
      st += p;
      vst1_u8(st, d3x2u8.val[0]);
      st += p;
      vst1_u8(st, d3x2u8.val[1]);
