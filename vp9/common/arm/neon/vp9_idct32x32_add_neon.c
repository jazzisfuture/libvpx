...
static void __STORE_COMBINE_CENTER_RESULTS(
        uint8_t *p1,
        uint8_t *p2,
        int stride,
        int16x8_t q6s16,
    ...
    d8s16  = vld1_s16((int16_t *)p1); p1 += stride;
    d11s16 = vld1_s16((int16_t *)p2); p2 -= stride;
    ...
    vst1_s16((int16_t *)p1, d9s16);  p1 -= stride;
    vst1_s16((int16_t *)p2, d10s16); p2 += stride;
...
static void __STORE_COMBINE_EXTREME_RESULTS(
        uint8_t *p1,
        uint8_t *p2,
        int stride,
        int16x8_t q4s16,
    ...
    d4s16 = vld1_s16((int16_t *)p1); p1 += stride;
    d7s16 = vld1_s16((int16_t *)p2); p2 -= stride;
    ...
    vst1_s16((int16_t *)p1, d5s16); p1 -= stride;
    vst1_s16((int16_t *)p2, d6s16); p2 += stride;
...
static void idct32_transpose_pair(
        int16_t *input,
        int16_t *t_buf) {
    int16_t *in;
    int i;
    ...
    for (i = 0; i < 4; i++, input += 8) {
        in = input;
        q8s16  = vld1q_s16(in); in += stride;
        q9s16  = vld1q_s16(in); in += stride;
        q10s16 = vld1q_s16(in); in += stride;
        q11s16 = vld1q_s16(in); in += stride;
        q12s16 = vld1q_s16(in); in += stride;
        q13s16 = vld1q_s16(in); in += stride;
        q14s16 = vld1q_s16(in); in += stride;
        q15s16 = vld1q_s16(in);
        ...
        vst1q_s16(t_buf, q0x2s16.val[0]); t_buf += 8;
        vst1q_s16(t_buf, q0x2s16.val[1]); t_buf += 8;
        vst1q_s16(t_buf, q1x2s16.val[0]); t_buf += 8;
        vst1q_s16(t_buf, q1x2s16.val[1]); t_buf += 8;
        vst1q_s16(t_buf, q2x2s16.val[0]); t_buf += 8;
        vst1q_s16(t_buf, q2x2s16.val[1]); t_buf += 8;
        vst1q_s16(t_buf, q3x2s16.val[0]); t_buf += 8;
        vst1q_s16(t_buf, q3x2s16.val[1]); t_buf += 8;
    }
    return;
}

static void idct32_bands_end_1st_pass(
        int16_t *out,
        int16x8_t q2s16,
        int16x8_t q3s16,
        int16x8_t q6s16,
...
static void idct32_bands_end_2nd_pass(
        int16_t *out,
        uint8_t *dest,
        int stride,
        int16x8_t q2s16,
    ...
    STORE_COMBINE_CENTER_RESULTS(r10, r9); r10 += str2; r9 -= str2;
    ...
    q6s16 = vsubq_s16(q3s16, q0s16);
    q7s16 = vsubq_s16(q2s16, q1s16);
    STORE_COMBINE_EXTREME_RESULTS(r7, r6); r7 += str2; r6 -= str2;
