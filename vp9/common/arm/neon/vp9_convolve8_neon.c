...
void vp9_convolve8_horiz_neon(const uint8_t *src, ptrdiff_t src_stride,
  ...
  for (i = h; i > 0; i -= 4) {
    ...
    for (j = w; j > 0; j -= 4) {
      ...
      v_u32x2_3 = vld1_dup_u32((const uint32_t *)psrc + (src_stride << 1)
                               + src_stride);
      ...
      vst1_lane_u32((uint32_t *)((uint8_t *)pdst + (dst_stride << 2)),
                    co_u32x2_1, 0);
      vst1_lane_u32((uint32_t *)((uint8_t *)pdst + ((dst_stride << 2) << 1)),
                    co_u32x2_0, 1);
      vst1_lane_u32((uint32_t *)((uint8_t *)pdst + ((dst_stride << 2) << 1))
                    + (dst_stride << 2), co_u32x2_1, 1);
