...
static INLINE int32x2x2_t vrshrn_n_s64_dual(const int64x2x2_t in,
                                            const int shift) {
  int32x2x2_t out;
  out.val[0] = vrshrn_n_s64(in.val[0], shift);
  out.val[1] = vrshrn_n_s64(in.val[1], shift);
  return out;
}
...
static INLINE int32x4x2_t highbd_add_dct_const_round_shift_low_8(
  ...
  const int32x2x2_t out_lo = vrshrn_n_s64_dual(sum_lo, DCT_CONST_BITS);
  const int32x2x2_t out_hi = vrshrn_n_s64_dual(sum_hi, DCT_CONST_BITS);
...
static INLINE int32x4x2_t highbd_sub_dct_const_round_shift_low_8(
  ...
  const int32x2x2_t out_lo = vrshrn_n_s64_dual(sub_lo, DCT_CONST_BITS);
  const int32x2x2_t out_hi = vrshrn_n_s64_dual(sub_hi, DCT_CONST_BITS);
  return vcombine_s32_dual(out_lo, out_hi);
}
