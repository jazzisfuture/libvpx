...
static void filter_by_weight32x32(uint8_t *src, int src_stride, uint8_t *dst,
...
static void filter_by_weight64x64(uint8_t *src, int src_stride, uint8_t *dst,
                                  int dst_stride, int weight) {
...
static void memcpy_block(uint8_t *y, uint8_t *u, uint8_t *v,
...
static void mfqe_block(BLOCK_SIZE bs, uint8_t *y, uint8_t *u,
  ...
  if (sad > 1 && sad < sad_thr && vdiff > sad * 3 && vdiff < 150) {
    ...
    memcpy_block(y, u, v, y_stride, uv_stride, yd, ud, vd,
...
static int mfqe_decision(MODE_INFO *mi, BLOCK_SIZE cur_bs) {
  ...
  int mv_len_square = mi->mbmi.mv[0].as_mv.row * mi->mbmi.mv[0].as_mv.row +
...
static void mfqe_partition(VP9_COMMON *cm, MODE_INFO *mi, BLOCK_SIZE bs,
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      ...
      mfqe_partition(cm, mi, subsize, y, u, v, y_stride, uv_stride,
                     yd, ud, vd, yd_stride, uvd_stride);
      mfqe_partition(cm, mi + mi_offset, subsize, y + y_offset,
...
void vp9_mfqe(VP9_COMMON *cm) {
  ...
  for (mi_row = 0; mi_row < cm->mi_rows; mi_row += MI_BLOCK_SIZE) {
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MI_BLOCK_SIZE) {
      ...
      mfqe_partition(cm, mi, BLOCK_64X64, y, u, v, y_stride, uv_stride,
                     yd, ud, vd, yd_stride, uvd_stride);
    }
  }
}
