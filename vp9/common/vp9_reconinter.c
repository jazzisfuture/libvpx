...
static int32_t scale_motion_vector_component(int16_t  mv,
                                             int      num,
                                             int      den,
                                             int      offset_q4) {
...
static void build_2x1_inter_predictor(const BLOCKD *d0, const BLOCKD *d1,
  ...
  /* Assumes predictor stride == 2 * block_size */
...
void vp9_build_inter16x16_predictors_mbuv(MACROBLOCKD *xd,
  ...
  for (which_mv = 0; which_mv < 1 + use_second_ref; ++which_mv) {
    ...
    uptr = (which_mv ? xd->second_pre.u_buffer : xd->pre.u_buffer) + offset;
    vptr = (which_mv ? xd->second_pre.v_buffer : xd->pre.v_buffer) + offset;

    xd->subpix.predict[!!(_o16x16mv.as_mv.col & 15)]
                      [!!(_o16x16mv.as_mv.row & 15)][which_mv](
