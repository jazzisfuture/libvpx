...
void vp9_setup_scale_factors_for_frame(struct scale_factors *scale,
  ...
  if (scale->x_num == scale->x_den && scale->y_num == scale->y_den) {
    scale->scaled_buffer_offset = buffer_offset_without_scaling;
    ...
    scale->scaled_buffer_offset = buffer_offset_with_scaling;
...
static int get_implicit_compoundinter_weight(MACROBLOCKD *xd,
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(
        base_pre + xd->scale_factor[1].
        scaled_buffer_offset(n, 0, pre_stride, &xd->scale_factor[1]),
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(
        base_pre + xd->scale_factor[1].
        scaled_buffer_offset(0, n, pre_stride, &xd->scale_factor[1]),
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    // predict a single row of pixels
    vp9_build_inter_predictor(
        base_pre + xd->scale_factor[0].
        scaled_buffer_offset(n, 0, pre_stride, &xd->scale_factor[0]),
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(
        base_pre + xd->scale_factor[0].
        scaled_buffer_offset(0, n, pre_stride, &xd->scale_factor[0]),
        pre_stride, tmp_y + n * tmp_ystride, tmp_ystride, &ymv,
...
static void build_inter32x32_predictors_sby_w(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    x->pre.y_buffer = y1 +
        x->scale_factor[0].scaled_buffer_offset(x_idx * 16,
                                                y_idx * 16,
                                                x->pre.y_stride,
                                                &x->scale_factor[0]);
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      x->second_pre.y_buffer = y2 +
          x->scale_factor[1].scaled_buffer_offset(x_idx * 16,
                                                  y_idx * 16,
...
static void build_inter32x32_predictors_sbuv_w(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    scaled_uv_offset =
        x->scale_factor_uv[0].scaled_buffer_offset(x_idx * 8,
                                                   y_idx * 8,
                                                   x->pre.uv_stride,
                                                   &x->scale_factor_uv[0]);
    ...
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      scaled_uv_offset =
          x->scale_factor_uv[1].scaled_buffer_offset(x_idx * 8,
                                                     y_idx * 8,
...
static void build_inter64x64_predictors_sby_w(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    x->pre.y_buffer = y1 +
        x->scale_factor[0].scaled_buffer_offset(x_idx * 32,
                                                y_idx * 32,
                                                x->pre.y_stride,
                                                &x->scale_factor[0]);
    ...
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      x->second_pre.y_buffer = y2 +
          x->scale_factor[1].scaled_buffer_offset(x_idx * 32,
                                                  y_idx * 32,
                                                  x->second_pre.y_stride,
                                                  &x->scale_factor[1]);
    }
...
void vp9_build_inter64x64_predictors_sbuv(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    scaled_uv_offset =
        x->scale_factor_uv[0].scaled_buffer_offset(x_idx * 16,
                                                   y_idx * 16,
                                                   x->pre.uv_stride,
                                                   &x->scale_factor_uv[0]);
    x->pre.u_buffer = u1 + scaled_uv_offset;
    ...
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      scaled_uv_offset =
          x->scale_factor_uv[1].scaled_buffer_offset(x_idx * 16,
                                                     y_idx * 16,
                                                     x->second_pre.uv_stride,
                                                     &x->scale_factor_uv[1]);
      x->second_pre.u_buffer = u2 + scaled_uv_offset;
