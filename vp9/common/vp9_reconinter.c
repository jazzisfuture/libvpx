...
static void dec_build_inter_predictors(MACROBLOCKD *xd, int plane, int block,
  ...
  for (ref = 0; ref < 1 + is_compound; ++ref) {
    ...
    if (vp9_is_scaled(sf)) {
      ...
      // Get the top left corner pixel. Precision
      // of x0_16 and y0_16 is 1/16th pixel.
      ...
      x_start = sf->scale_value_x(x_start, sf) + sf->scale_value_x(x, sf);
      y_start = sf->scale_value_y(y_start, sf) + sf->scale_value_y(y, sf);
    ...
    if (scaled_mv.col || scaled_mv.row ||
      ...
      if (x0 < 0 || x0 > frame_width - 1 || x1 < 0 || x1 > frame_width ||
        ...
        build_mc_border(buf_ptr1, pre_buf->stride, xd->mc_buf, x1 - x0 + 1,
                        x0, y0, x1 - x0 + 1, y1 - y0+1, frame_width, frame_height);
        buf_stride = x1 - x0 + 1;
