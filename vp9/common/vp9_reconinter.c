...
void vp9_setup_interp_filters(MACROBLOCKD *xd,
  ...
  for (i = 0; i < 3; ++i) {
    int other_w, other_h, this_w, this_h;
    ...
    other_h = cm->yv12_fb[cm->active_ref_idx[i]].y_height;
    other_w = cm->yv12_fb[cm->active_ref_idx[i]].y_width;
    this_h = cm->mb_rows * 16;
    this_w = cm->mb_cols * 16;
    cm->active_ref_scale[i].x_num = other_w;
    cm->active_ref_scale[i].x_den = this_w;
    cm->active_ref_scale[i].x_offset_q4 = 0;  // calculated per-mb
    cm->active_ref_scale[i].x_step_q4 = 16 * other_w / this_w;
    cm->active_ref_scale[i].y_num = other_h;
    cm->active_ref_scale[i].y_den = this_h;
    cm->active_ref_scale[i].y_offset_q4 = 0;  // calculated per-mb
    cm->active_ref_scale[i].y_step_q4 = 16 * other_h / this_h;
    ...
    if (cm->active_ref_scale[i].x_step_q4 == 16) {
      if (cm->active_ref_scale[i].y_step_q4 == 16) {
        // No scaling in either direction.
        cm->active_ref_scale[i].predict[0][0][0] = vp9_convolve_copy;
        cm->active_ref_scale[i].predict[0][0][1] = vp9_convolve_avg;
        cm->active_ref_scale[i].predict[0][1][0] = vp9_convolve8_vert;
        cm->active_ref_scale[i].predict[0][1][1] = vp9_convolve8_avg_vert;
        cm->active_ref_scale[i].predict[1][0][0] = vp9_convolve8_horiz;
        cm->active_ref_scale[i].predict[1][0][1] = vp9_convolve8_avg_horiz;
      } else {
        // No scaling in x direction. Must always scale in the y direction.
        cm->active_ref_scale[i].predict[0][0][0] = vp9_convolve8_vert;
        cm->active_ref_scale[i].predict[0][0][1] = vp9_convolve8_avg_vert;
        cm->active_ref_scale[i].predict[0][1][0] = vp9_convolve8_vert;
        cm->active_ref_scale[i].predict[0][1][1] = vp9_convolve8_avg_vert;
        cm->active_ref_scale[i].predict[1][0][0] = vp9_convolve8;
        cm->active_ref_scale[i].predict[1][0][1] = vp9_convolve8_avg;
      }
    } else {
      if (cm->active_ref_scale[i].y_step_q4 == 16) {
        // No scaling in the y direction. Must always scale in the x direction.
        cm->active_ref_scale[i].predict[0][0][0] = vp9_convolve8_horiz;
        cm->active_ref_scale[i].predict[0][0][1] = vp9_convolve8_avg_horiz;
        cm->active_ref_scale[i].predict[0][1][0] = vp9_convolve8;
        cm->active_ref_scale[i].predict[0][1][1] = vp9_convolve8_avg;
        cm->active_ref_scale[i].predict[1][0][0] = vp9_convolve8_horiz;
        cm->active_ref_scale[i].predict[1][0][1] = vp9_convolve8_avg_horiz;
      } else {
        // Must always scale in both directions.
        cm->active_ref_scale[i].predict[0][0][0] = vp9_convolve8;
        cm->active_ref_scale[i].predict[0][0][1] = vp9_convolve8_avg;
        cm->active_ref_scale[i].predict[0][1][0] = vp9_convolve8;
        cm->active_ref_scale[i].predict[0][1][1] = vp9_convolve8_avg;
        cm->active_ref_scale[i].predict[1][0][0] = vp9_convolve8;
        cm->active_ref_scale[i].predict[1][0][1] = vp9_convolve8_avg;
    ...
    cm->active_ref_scale[i].predict[1][1][0] = vp9_convolve8;
    cm->active_ref_scale[i].predict[1][1][1] = vp9_convolve8_avg;
  }
...
void vp9_build_scaled_inter_predictor(const uint8_t *src, int src_stride,
...
/* Like vp9_build_scaled_inter_predictor, but takes the full-pel part of the
 * mv separately, and the fractional part as a q4.
 */
void vp9_build_scaled_inter_predictor_q4(const uint8_t *src, int src_stride,
                                         uint8_t *dst, int dst_stride,
...
static void build_2x1_inter_predictor(const BLOCKD *d0, const BLOCKD *d1,
  ...
  if (d0->bmi.as_mv[which_mv].as_int == d1->bmi.as_mv[which_mv].as_int) {
    ...
    vp9_build_scaled_inter_predictor(*base_pre0 + d0->pre,
                                     d0->pre_stride,
                                     d0->predictor, stride,
                                     &d0->bmi.as_mv[which_mv],
                                     &scale[which_mv],
    ...
    set_scaled_offsets(scale, row, col + block_size);

    vp9_build_scaled_inter_predictor(*base_pre1 + d1->pre,
                                     d1->pre_stride,
                                     d1->predictor, stride,
...
void vp9_build_inter16x16_predictors_mby(MACROBLOCKD *xd,
  ...
  for (which_mv = 0; which_mv < 1 + use_second_ref; ++which_mv) {
    ...
    vp9_build_scaled_inter_predictor(base_pre, pre_stride,
                                     dst_y, dst_ystride,
...
void vp9_build_inter16x16_predictors_mbuv(MACROBLOCKD *xd,
  ...
  for (which_mv = 0; which_mv < 1 + use_second_ref; ++which_mv) {
    ...
    vp9_build_scaled_inter_predictor_q4(uptr, pre_stride,
                                        dst_u, dst_uvstride,
                                        &_16x16mv, &_o16x16mv,
                                        &xd->scale_factor_uv[which_mv],
                                        8, 8, which_mv, &xd->subpix);

    vp9_build_scaled_inter_predictor_q4(vptr, pre_stride,
                                        dst_v, dst_uvstride,
                                        &_16x16mv, &_o16x16mv,
                                        &xd->scale_factor_uv[which_mv],
                                        8, 8, which_mv, &xd->subpix);
...
void vp9_build_inter32x32_predictors_sb(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      x->pre.y_buffer = y2 + scaled_buffer_offset(x_idx * 16,
                                                  y_idx * 16,
...
void vp9_build_inter64x64_predictors_sb(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      x->pre.y_buffer = y2 + scaled_buffer_offset(x_idx * 32,
                                                  y_idx * 32,
                                                  x->second_pre.y_stride,
                                                  &x->scale_factor[1]);
      scaled_uv_offset = scaled_buffer_offset(x_idx * 16,
