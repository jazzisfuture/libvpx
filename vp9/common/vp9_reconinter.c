...
static void dec_build_inter_predictors(MACROBLOCKD *xd, int plane, int block,
  ...
  for (ref = 0; ref < 1 + is_compound; ++ref) {
    ...
    // get current block position in current frame.
    ...
    if (vp9_is_scaled(scale->sfc)) {
      ...
      xs = scale->sfc->x_step_q4;
      ys = scale->sfc->y_step_q4;
      // get current block position in the scaled reference frame
    ...
    // get reference block coordinate.
    x0 += scaled_mv.col >> SUBPEL_BITS;
    y0 += scaled_mv.row >> SUBPEL_BITS;
    x1 = x0 + ((w-1)*xs >> 4) + 1;
    y1 = y0 + ((h-1)*xs >> 4) + 1;

    // Do boarder extension if there is motion or width/height not alligned to 8
    if (scaled_mv.col || scaled_mv.row ||
        (frame_width & 0x7) || (frame_height & 0x7)) {
      const int subpel_x = scaled_mv.col & SUBPEL_MASK;
      const int subpel_y = scaled_mv.row & SUBPEL_MASK;
      ...
      // skip boarder extension if block is inside the frame.
