...
static void build_inter_predictors_singlemv(int plane, int block,
  ...
  const int x = x_idx * 4, y = y_idx * 4;
  const int use_second_ref = xd->mode_info_context->mbmi.second_ref_frame > 0;
  int which_mv = arg->singlemv_used;
...
static void build_inter_predictors_singlemv_sby(MACROBLOCKD *xd,
                                                int mi_row,
                                                int mi_col,
                                                BLOCK_SIZE_TYPE bsize,
                                                int which_mv) {
...
static void build_inter_predictors_singlemv_sbuv(MACROBLOCKD *xd,
                                                 int mi_row,
                                                 int mi_col,
                                                 BLOCK_SIZE_TYPE bsize,
                                                 int which_mv) {
...
void vp9_build_inter_predictors_singlemv_sb(MACROBLOCKD *xd,
                                            int mi_row, int mi_col,
                                            BLOCK_SIZE_TYPE bsize,
                                            int which_mv) {
  build_inter_predictors_singlemv_sby(xd, mi_row, mi_col, bsize, which_mv);
  build_inter_predictors_singlemv_sbuv(xd, mi_row, mi_col, bsize, which_mv);
}
