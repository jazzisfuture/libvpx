...
static MV mv_q3_to_q4_with_scaling(const MV *mv,
                                   const struct scale_factors *scale) {
...
static MV mv_q3_to_q4_without_scaling(const MV *mv,
                                      const struct scale_factors *scale) {
  MV res = {
     mv->row << 1,
     mv->col << 1
...
static MV mv_q4_with_scaling(const MV *mv,
                             const struct scale_factors *scale) {
  MV res = {
...
static MV mv_q4_without_scaling(const MV *mv,
                                const struct scale_factors *scale) {
  return *mv;
}
...
void vp9_build_inter_predictor(const uint8_t *src, int src_stride,
  ...
  MV mv = scale->scale_mv_q3_to_q4(&mv_q3->as_mv, scale);
  src += (mv.row >> 4) * src_stride + (mv.col >> 4);
  scale->predict[!!(mv.col & 15)][!!(mv.row & 15)][weight](
      src, src_stride, dst, dst_stride,
      subpix->filter_x[mv.col & 15], scale->x_step_q4,
      subpix->filter_y[mv.row & 15], scale->y_step_q4,
      w, h);
}
...
void vp9_build_inter_predictor_q4(const uint8_t *src, int src_stride,
                                  ...
                                  int w, int h, int weight,
                                  const struct subpix_fn_table *subpix) {
  MV mv = scale->scale_mv_q4(&mv_q4->as_mv, scale);
