...
void vp9_build_inter_predictor_q4(const uint8_t *src, int src_stride,
  ...
  const int scaled_mv_row_q4 =
      scale_motion_vector_component_q4(mv_q4->as_mv.row,
                                       scale->y_num, scale->y_den,
                                       scale->y_offset_q4);
  const int scaled_mv_col_q4 =
      scale_motion_vector_component_q4(mv_q4->as_mv.col,
...
static void build_inter_predictors(int plane, int block,
  ...
  for (which_mv = 0; which_mv < 1 + use_second_ref; ++which_mv) {
    ...
    set_scaled_offsets(&xd->scale_factor[which_mv], arg->y + y, arg->x + x);
...
void vp9_build_inter4x4_predictors_mbuv(MACROBLOCKD *xd,
                                        int mb_row, int mb_col) {
  vp9_build_inter_predictors_sbuv(xd, &xd->predictor[256], &xd->predictor[320],
                                  8, mb_row, mb_col, BLOCK_SIZE_MB16X16);
}
