...
static int scale_value_x_with_scaling(int val,
  ...
  return ROUND_POWER_OF_TWO(val * scale->x_scale_fp, VP9_REF_SCALE_SHIFT);
}

static int scale_value_y_with_scaling(int val,
                                      const struct scale_factors *scale) {
  return ROUND_POWER_OF_TWO(val * scale->y_scale_fp, VP9_REF_SCALE_SHIFT);
}
...
static int_mv32 mv_q3_to_q4_with_scaling(const int_mv *src_mv,
  ...
  result.as_mv.row =
    ROUND_POWER_OF_TWO(mv_row_q4 * scale->y_scale_fp, VP9_REF_SCALE_SHIFT)
                      + scale->y_offset_q4;
  result.as_mv.col =
    ROUND_POWER_OF_TWO(mv_col_q4 * scale->x_scale_fp, VP9_REF_SCALE_SHIFT)
                      + scale->x_offset_q4;
  return result;
}
...
static int32_t mv_component_q4_with_scaling(int mv_q4, int scale_fp,
  ...
  scaled_mv =
      ROUND_POWER_OF_TWO(mv_q4 * scale_fp, VP9_REF_SCALE_SHIFT) + offset_q4;
...
static void set_offsets_with_scaling(struct scale_factors *scale,
  ...
  scale->x_offset_q4 =
      ROUND_POWER_OF_TWO(x_q4 * scale->x_scale_fp, VP9_REF_SCALE_SHIFT) & 0xf;
  scale->y_offset_q4 =
      ROUND_POWER_OF_TWO(y_q4 * scale->y_scale_fp, VP9_REF_SCALE_SHIFT) & 0xf;
}
...
void vp9_setup_scale_factors_for_frame(struct scale_factors *scale,
  ...
  scale->x_step_q4 =
    ROUND_POWER_OF_TWO(16 * scale->x_scale_fp, VP9_REF_SCALE_SHIFT);

  scale->y_scale_fp = get_fixed_point_scale_factor(other_h, this_h);
  scale->y_offset_q4 = 0;  // calculated per-mb
  scale->y_step_q4 =
    ROUND_POWER_OF_TWO(16 * scale->y_scale_fp, VP9_REF_SCALE_SHIFT);
