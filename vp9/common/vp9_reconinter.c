...
void vp9_setup_scale_factors_for_frame(struct scale_factors *scale,
  ...
  if (scale->x_step_q4 == 16) {
    if (scale->y_step_q4 == 16) {
      ...
      scale->predict[0][0][1] = vp9_convolve_qtr;
      scale->predict[0][0][2] = vp9_convolve_avg;
      scale->predict[0][0][3] = vp9_convolve_3qtr;
      scale->predict[0][1][0] = vp9_convolve8_vert;
      scale->predict[0][1][1] = vp9_convolve8_qtr_vert;
      scale->predict[0][1][2] = vp9_convolve8_avg_vert;
      scale->predict[0][1][3] = vp9_convolve8_3qtr_vert;
      scale->predict[1][0][0] = vp9_convolve8_horiz;
      scale->predict[1][0][1] = vp9_convolve8_qtr_horiz;
      scale->predict[1][0][2] = vp9_convolve8_avg_horiz;
      scale->predict[1][0][3] = vp9_convolve8_3qtr_horiz;
    } else {
      // No scaling in x direction. Must always scale in the y direction.
      scale->predict[0][0][0] = vp9_convolve8_vert;
      scale->predict[0][0][1] = vp9_convolve8_qtr_vert;
      scale->predict[0][0][2] = vp9_convolve8_avg_vert;
      scale->predict[0][0][3] = vp9_convolve8_3qtr_vert;
      scale->predict[0][1][0] = vp9_convolve8_vert;
      scale->predict[0][1][1] = vp9_convolve8_qtr_vert;
      scale->predict[0][1][2] = vp9_convolve8_avg_vert;
      scale->predict[0][1][3] = vp9_convolve8_3qtr_vert;
      scale->predict[1][0][0] = vp9_convolve8;
      scale->predict[1][0][1] = vp9_convolve8_qtr;
      scale->predict[1][0][2] = vp9_convolve8_avg;
      scale->predict[1][0][3] = vp9_convolve8_3qtr;
    }
  } else {
    if (scale->y_step_q4 == 16) {
      // No scaling in the y direction. Must always scale in the x direction.
      scale->predict[0][0][0] = vp9_convolve8_horiz;
      scale->predict[0][0][1] = vp9_convolve8_qtr_horiz;
      scale->predict[0][0][2] = vp9_convolve8_avg_horiz;
      scale->predict[0][0][3] = vp9_convolve8_3qtr_horiz;
      scale->predict[0][1][0] = vp9_convolve8;
      scale->predict[0][1][1] = vp9_convolve8_qtr;
      scale->predict[0][1][2] = vp9_convolve8_avg;
      scale->predict[0][1][3] = vp9_convolve8_3qtr;
      scale->predict[1][0][0] = vp9_convolve8_horiz;
      scale->predict[1][0][1] = vp9_convolve8_qtr_horiz;
      scale->predict[1][0][2] = vp9_convolve8_avg_horiz;
      scale->predict[1][0][3] = vp9_convolve8_3qtr_horiz;
      ...
      scale->predict[0][0][1] = vp9_convolve8_qtr;
      scale->predict[0][0][2] = vp9_convolve8_avg;
      scale->predict[0][0][3] = vp9_convolve8_3qtr;
      scale->predict[0][1][0] = vp9_convolve8;
      scale->predict[0][1][1] = vp9_convolve8_qtr;
      scale->predict[0][1][2] = vp9_convolve8_avg;
      scale->predict[0][1][3] = vp9_convolve8_3qtr;
  ...
  scale->predict[1][1][1] = vp9_convolve8_qtr;
  scale->predict[1][1][2] = vp9_convolve8_avg;
  scale->predict[1][1][3] = vp9_convolve8_3qtr;
}
#else
  if (scale->x_step_q4 == 16) {
    if (scale->y_step_q4 == 16) {
...
static void build_2x1_inter_predictor(const BLOCKD *d0, const BLOCKD *d1,
  ...
  if (d0->bmi.as_mv[which_mv].as_int == d1->bmi.as_mv[which_mv].as_int) {
    ...
    vp9_build_inter_predictor(*base_pre + d0->pre,
                              ...
                              which_mv << CONFIG_IMPLICIT_COMPOUNDINTER_WEIGHT,
                              subpix);
...
#if CONFIG_IMPLICIT_COMPOUNDINTER_WEIGHT
static int get_weight(int metric_1, int metric_2) {
  int weight;
...
static int get_implicit_compoundinter_weight(MACROBLOCKD *xd,
                                             ...
                                             int mb_col) {
  const int use_second_ref = xd->mode_info_context->mbmi.second_ref_frame > 0;
  int metric_2 = 0, metric_1 = 0;
  int i, n, clamp_mvs, pre_stride;
  uint8_t *base_pre;
  uint8_t *rec_y;
  int rec_ystride;
  int_mv ymv;
  ...
  rec_y = xd->dst.y_buffer;
  rec_ystride = xd->dst.y_stride;
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(
        ...
        pre_stride, tmp_y + n * tmp_ystride, tmp_ystride, &ymv, &xd->scale_factor[1],
        1, 16, 0, &xd->subpix);
  }
  ...
  if (xd->up_available) {
    for (i = 0; i < block_size; ++i) {
      metric_2 += abs(*(rec_y - rec_ystride + i) -
                      *(tmp_y + i));
    }
  }
  if (xd->left_available) {
    for (i = 0; i < block_size; ++i) {
      metric_2 += abs(*(rec_y - 1 + i * rec_ystride) -
                      *(tmp_y + i * tmp_ystride));
    }
  }
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(
        ...
        pre_stride, tmp_y + n * tmp_ystride, tmp_ystride, &ymv, &xd->scale_factor[0],
        1, 16, 0, &xd->subpix);
  }
  xd->mb_to_top_edge = edge[0];
  xd->mb_to_bottom_edge = edge[1];
  if (xd->up_available) {
    for (i = 0; i < block_size; ++i) {
      metric_1 += abs(*(rec_y - rec_ystride + i) -
                      *(tmp_y + i));
    }
  }
  if (xd->left_available) {
    for (i = 0; i < block_size; ++i) {
      metric_1 += abs(*(rec_y - 1 + i * rec_ystride) -
                      *(tmp_y + i * tmp_ystride));
    }
  }
  // Choose final weight for averaging
  weight = get_weight(metric_1, metric_2);
  return weight;
}
...
void vp9_build_inter16x16_predictors_mby(MACROBLOCKD *xd,
  ...
  if (clamp_mvs)
    clamp_mv_to_umv_border(&ymv.as_mv, xd);
  set_scaled_offsets(&xd->scale_factor[0], mb_row * 16, mb_col * 16);
  // Now generate the first predictor
  vp9_build_inter_predictor(base_pre, pre_stride,
                            dst_y, dst_ystride,
                            &ymv, &xd->scale_factor[0],
                            16, 16, 0, &xd->subpix);
  ...
  if (use_implicit_weighting) {
    ...
    if (clamp_mvs)
      clamp_mv_to_umv_border(&ymv.as_mv, xd);
    set_scaled_offsets(&xd->scale_factor[1], mb_row * 16, mb_col * 16);
    // First generate the second predictor
    vp9_build_inter_predictor(base_pre, pre_stride,
                              tmp_y, tmp_ystride,
                              &ymv, &xd->scale_factor[1],
                              16, 16, 0, &xd->subpix);
    // Compute consistency metric
    if (xd->up_available) {
      for (i = 0; i < 16; ++i) {
    ...
    weight = get_weight(metric_1, metric_2);
    if (weight == 1)
      vp9_convolve_qtr(tmp_y, tmp_ystride, dst_y, dst_ystride,
                       NULL, 0, NULL, 0, 16, 16);
    else if (weight == 2)
      vp9_convolve_avg(tmp_y, tmp_ystride, dst_y, dst_ystride,
                       NULL, 0, NULL, 0, 16, 16);
    else if (weight == 3)
      vp9_convolve_3qtr(tmp_y, tmp_ystride, dst_y, dst_ystride,
                        NULL, 0, NULL, 0, 16, 16);
  } else {
    clamp_mvs = xd->mode_info_context->mbmi.need_to_clamp_secondmv;
    base_pre = xd->second_pre.y_buffer;
    ...
    // Generate final weighted average predictor
    vp9_build_inter_predictor(base_pre, pre_stride,
                              dst_y, dst_ystride,
                              &ymv, &xd->scale_factor[1],
                              16, 16, weight, &xd->subpix);
  }
}
#endif
