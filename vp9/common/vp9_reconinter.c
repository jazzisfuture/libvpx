...
static int get_implicit_compoundinter_weight(MACROBLOCKD *xd,
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(base_pre +
        scale->scaled_buffer_offset(n, 0, pre_stride, scale),
  ...
  for (n = 0; n < block_size; n += 16) {
    ...
    vp9_build_inter_predictor(base_pre +
        scale->scaled_buffer_offset(0, n, pre_stride, scale),
...
static void build_inter32x32_predictors_sby_w(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    x->pre.y_buffer = y1 +
        scale->scaled_buffer_offset(x_idx * 16, y_idx * 16,
        x->pre.y_stride, scale);
    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      scale = &x->scale_factor[1];
      x->second_pre.y_buffer = y2 +
          scale->scaled_buffer_offset(x_idx * 16, y_idx * 16,
          x->second_pre.y_stride, scale);
    }
...
static void build_inter32x32_predictors_sbuv_w(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    scale = &x->scale_factor_uv[0];
    scaled_uv_offset =
        scale->scaled_buffer_offset(x_idx * 8, y_idx * 8,
        x->pre.uv_stride, scale);
    x->pre.u_buffer = u1 + scaled_uv_offset;
    x->pre.v_buffer = v1 + scaled_uv_offset;

    if (x->mode_info_context->mbmi.second_ref_frame > 0) {
      scale = &x->scale_factor_uv[1];
      scaled_uv_offset =
          scale->scaled_buffer_offset(x_idx * 8, y_idx * 8,
          x->second_pre.uv_stride, scale);
...
static void build_inter64x64_predictors_sby_w(MACROBLOCKD *x,
  ...
  for (n = 0; n < 4; n++) {
    ...
    scale = &x->scale_factor[0];
    x->pre.y_buffer = y1 +
        scale->scaled_buffer_offset(x_idx * 32, y_idx * 32,
        x->pre.y_stride, scale);
