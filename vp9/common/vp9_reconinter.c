...
static void ondemand_boarder_extend(struct buf_2d *const pre_buf,
  ...
  if (y1 >= (frame_height - 5)) {
    uint8_t extend_bottom = y1 <= frame_height ? factor : - frame_height + factor;
    src_ptr = ref_frame +  pre_buf->stride * (frame_height - 1) + (x0 < 0 ? 0 : x0);
  ...
  if (x1 >= (frame_width-5)) {
    uint8_t extend_right = x1 <= frame_width ? factor : x1 - frame_width + factor;
...
static void build_inter_predictors(int plane, int block, BLOCK_SIZE bsize,
  ...
  //  frame_height = -xd->mb_to_top_edge / (8 << ss_x) + bh
    ...
    // get reference frame pointer
    ref_frame = pre_buf->buf - (y0 * pre_buf->stride + x0);
    reset_boarder(ref_frame, pre_buf->stride, frame_width2, frame_height2, 10, 10, 10, 10);
   ...
   //   src_ptr = ref_frame + (x0 < 0 ? 0 : x0 );
    ...
    if (y1 >= frame_height2) {
      ...
      src_ptr = ref_frame +  pre_buf->stride * (frame_height2 - 1) +(x0 < 0 ? 0 : (x0 >= frame_width2 ? frame_width2 - 1: x0));
