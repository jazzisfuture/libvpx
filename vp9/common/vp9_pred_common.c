...
unsigned char vp9_get_pred_context_tx_size(const MACROBLOCKD *xd) {
  ...
  const int left_in_image = xd->left_available && left_mi;
  const int above_in_image = xd->up_available && above_mi;
  const int max_tx_size = max_txsize_lookup[mi_8x8->mi->mbmi.sb_type];
...
void vp9_set_pred_flag_seg_id(MACROBLOCKD *xd, BLOCK_SIZE bsize,
                              uint8_t pred_flag) {
  xd->this_mi->mbmi.seg_id_predicted = pred_flag;
}

void vp9_set_pred_flag_seg_id_e(VP9_COMMON *cm, BLOCK_SIZE bsize,
                                int mi_row, int mi_col, uint8_t pred_flag) {
  const int mis = cm->mode_info_stride;
  MODE_INFO_8x8 *mi_8x8 = &cm->mi_grid_visible[mi_row * mis + mi_col];
  ...
  for (y = 0; y < ymis; y++)
    for (x = 0; x < xmis; x++)
      mi_8x8[y * mis + x].mi->mbmi.seg_id_predicted = pred_flag;
}
