...
int vp9_get_pred_context_single_ref_p2(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (!is_inter_block(edge_mbmi) ||
        (CHECK_LAST_OR_LAST2(edge_mbmi->ref_frame[0]) && !has_second_ref(edge_mbmi)))
      pred_context = 2;
...
int vp9_get_pred_context_single_ref_p3(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (above_has_second && left_has_second) {
        ...
        if (CHECK_GOLDEN_OR_ALTREF(above0) && CHECK_GOLDEN_OR_ALTREF(left0)) {
          pred_context = 2 + (above0 == left0);
        } else if (CHECK_GOLDEN_OR_ALTREF(above0) || CHECK_GOLDEN_OR_ALTREF(left0)) {
          const MV_REFERENCE_FRAME edge0 = CHECK_GOLDEN_OR_ALTREF(above0) ? left0
                                                                       : above0;
    ...
    if (!is_inter_block(edge_mbmi) ||
        (CHECK_GOLDEN_OR_ALTREF(edge_mbmi->ref_frame[0]) && !has_second_ref(edge_mbmi)))
      pred_context = 2;
    else if (!has_second_ref(edge_mbmi))
      pred_context = 4 * (edge_mbmi->ref_frame[0] == LAST_FRAME);
    else
