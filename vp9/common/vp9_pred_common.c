...
// Signal the first reference frame for a compound mode is GOLDEN.
//
// NOTE(zoeliu): The probability of ref_frame[0] is either GOLDEN_FRAME or
//               LAST3_FRAME.
int vp9_get_pred_context_comp_ref_p(const VP9_COMMON *cm,
                                    const MACROBLOCKD *xd) {
  int pred_context;
  ...
  if (above_in_image && left_in_image) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra (2)
      ...
      if (!has_second_ref(edge_mbmi))  // single pred (1/3)
        ...
        pred_context = 1 +
            2 * (!CHECK_GOLDEN_OR_LAST3(edge_mbmi->ref_frame[var_ref_idx]));
    } else {  // inter/inter
      const int l_sg = !has_second_ref(left_mbmi);
      const int a_sg = !has_second_ref(above_mbmi);
      const MV_REFERENCE_FRAME vrfa = a_sg ? above_mbmi->ref_frame[0]
      ...
      if (vrfa == vrfl && CHECK_GOLDEN_OR_LAST3(vrfa)) {
        pred_context = 0;
      } else if (l_sg && a_sg) {  // single/single
        if ((vrfa == ALTREF_FRAME && CHECK_LAST_OR_LAST2(vrfl)) ||
            (vrfl == ALTREF_FRAME && CHECK_LAST_OR_LAST2(vrfa))) {
          pred_context = 4;
                                    ...
                                    CHECK_LAST_OR_LAST2(vrfl))) {
          pred_context = 3;
        } else {  // Either vrfa or vrfl is GOLDEN_FRAME or LAST3_FRAME
          // NOTE(zoeliu): Following assert may be removed once confirmed.
          assert(CHECK_GOLDEN_OR_LAST3(vrfa) || CHECK_GOLDEN_OR_LAST3(vrfl));
          pred_context = 1;
        }
      } else if (l_sg || a_sg) {  // single/comp
        const MV_REFERENCE_FRAME vrfc = l_sg ? vrfa : vrfl;
        ...
        if (CHECK_GOLDEN_OR_LAST3(vrfc) && !CHECK_GOLDEN_OR_LAST3(rfs))
          pred_context = 1;
        else if (CHECK_GOLDEN_OR_LAST3(rfs) && !CHECK_GOLDEN_OR_LAST3(vrfc))
          pred_context = 2;
        else
          pred_context = 4;
      } else {  // comp/comp
    ...
    if (!is_inter_block(edge_mbmi)) {
      ...
      if (has_second_ref(edge_mbmi))
        pred_context =
            4 * (!CHECK_GOLDEN_OR_LAST3(edge_mbmi->ref_frame[var_ref_idx]));
      else
...
// Returns a context number for the given MB prediction signal
// Signal the first reference frame for a compound mode is LAST,
// conditioning on that it is known neighter GOLDEN nor LAST3.
//
// NOTE(zoeliu): The probability of ref_frame[0] is LAST_FRAME,
// conditioning on it is either LAST_FRAME or LAST2_FRAME.
int vp9_get_pred_context_comp_ref_p1(const VP9_COMMON *cm,
  ...
  if (above_in_image && left_in_image) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra (2)
      ...
      if (vrfa == vrfl && vrfa == LAST_FRAME)
        ...
        if (vrfa == LAST_FRAME || vrfl == LAST_FRAME)
          pred_context = 1;
        else if (CHECK_GOLDEN_OR_LAST3(vrfa) || CHECK_GOLDEN_OR_LAST3(vrfl))
          pred_context = 2 + (vrfa != vrfl);
        else if (vrfa == vrfl)
          pred_context = 3;
        else
        ...
        if (vrfc == LAST_FRAME && rfs != LAST_FRAME)
          ...
          pred_context = 3 +
              (vrfc == LAST2_FRAME || CHECK_GOLDEN_OR_LAST3(rfs));
      } else {  // comp/comp
        if (vrfa == LAST_FRAME || vrfl == LAST_FRAME)
          pred_context = 2;
        else
          pred_context = 3 +
              (CHECK_GOLDEN_OR_LAST3(vrfa) || CHECK_GOLDEN_OR_LAST3(vrfl));
      }
    }
  } else if (above_in_image || left_in_image) {  // one edge available
    const MB_MODE_INFO *edge_mbmi = above_in_image ? above_mbmi : left_mbmi;
...
// Returns a context number for the given MB prediction signal
// Signal the first reference frame for a compound mode is GOLDEN,
// conditioning on that it is known neighter LAST nor LAST2.
//
// NOTE(zoeliu): The probability of ref_frame[0] is GOLDEN_FRAME,
// conditioning on it is either GOLDEN_FRAME or LAST3_FRAME.
int vp9_get_pred_context_comp_ref_p2(const VP9_COMMON *cm,
                                     const MACROBLOCKD *xd) {
  int pred_context;
  const MB_MODE_INFO *const above_mbmi = get_mbmi(get_above_mi(xd));
    ...
    if (above_intra && left_intra) {  // intra/intra (2)
      ...
      if (vrfa == vrfl && vrfa == GOLDEN_FRAME)
        ...
        if (vrfc == GOLDEN_FRAME && rfs != GOLDEN_FRAME)
          ...
          // TODO(zoeliu): Check again whether vrfc == LAST3_FRAME makes sense.
          pred_context = 3 + (vrfc == LAST3_FRAME || CHECK_LAST_OR_LAST2(rfs));
      } else {  // comp/comp
        if (vrfa == GOLDEN_FRAME || vrfl == GOLDEN_FRAME)
          pred_context = 2;
        else
...
// LAST2_FRAME/LAST3_FRAME, or not.
//
// NOTE(zoeliu): The probability of ref_frame[0] is ALTREF_FRAME or
// GOLDEN_FRAME, instead of being LAST_FRAME/LAST2_FRAME/LAST3_FRAME.
int vp9_get_pred_context_single_ref_p1(const MACROBLOCKD *xd) {
  int pred_context;
  const MB_MODE_INFO *const above_mbmi = get_mbmi(get_above_mi(xd));
  const MB_MODE_INFO *const left_mbmi = get_mbmi(get_left_mi(xd));
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      if (!has_second_ref(edge_mbmi))
        pred_context = 4 * CHECK_LAST_LAST2_LAST3(edge_mbmi->ref_frame[0]);
      else
        pred_context = 1 + (CHECK_LAST_LAST2_LAST3(edge_mbmi->ref_frame[0]) ||
                            CHECK_LAST_LAST2_LAST3(edge_mbmi->ref_frame[1]));
    } else {  // inter/inter
      const int above_has_second = has_second_ref(above_mbmi);
      const int left_has_second  = has_second_ref(left_mbmi);
      ...
      if (above_has_second && left_has_second) {
        pred_context = 1 + (CHECK_LAST_LAST2_LAST3(above0) ||
                            CHECK_LAST_LAST2_LAST3(above1) ||
                            CHECK_LAST_LAST2_LAST3(left0) ||
                            CHECK_LAST_LAST2_LAST3(left1));
      } else if (above_has_second || left_has_second) {
        const MV_REFERENCE_FRAME rfs = !above_has_second ? above0 : left0;
        const MV_REFERENCE_FRAME crf1 = above_has_second ? above0 : left0;
        const MV_REFERENCE_FRAME crf2 = above_has_second ? above1 : left1;

        if (CHECK_LAST_LAST2_LAST3(rfs))
          pred_context = 3 + (CHECK_LAST_LAST2_LAST3(crf1) ||
                              CHECK_LAST_LAST2_LAST3(crf2));
        else
          pred_context = CHECK_LAST_LAST2_LAST3(crf1) ||
                         CHECK_LAST_LAST2_LAST3(crf2);
      } else {
        pred_context = 2 * CHECK_LAST_LAST2_LAST3(above0) +
                       2 * CHECK_LAST_LAST2_LAST3(left0);
      }
    }
  } else if (has_above || has_left) {  // one edge available
    const MB_MODE_INFO *edge_mbmi = has_above ? above_mbmi : left_mbmi;
...
int vp9_get_pred_context_single_ref_p2(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      const MB_MODE_INFO *edge_mbmi = above_intra ? left_mbmi : above_mbmi;
      if (!has_second_ref(edge_mbmi)) {
        if (CHECK_LAST_LAST2_LAST3(edge_mbmi->ref_frame[0]))
          pred_context = 3;
        else
          pred_context = 4 * (edge_mbmi->ref_frame[0] == GOLDEN_FRAME);
      } else {
      ...
      if (above_has_second && left_has_second) {
        ...
        if (CHECK_LAST_LAST2_LAST3(above0) && CHECK_LAST_LAST2_LAST3(left0)) {
          pred_context = 2 + (above0 == left0);
        } else if (CHECK_LAST_LAST2_LAST3(above0) ||
                   CHECK_LAST_LAST2_LAST3(left0)) {
          const MV_REFERENCE_FRAME edge0 =
              CHECK_LAST_LAST2_LAST3(above0) ? left0 : above0;
          pred_context = 4 * (edge0 == GOLDEN_FRAME);
        } else {
          pred_context = 2 * (above0 == GOLDEN_FRAME) +
                         2 * (left0  == GOLDEN_FRAME);
    ...
    if (!is_inter_block(edge_mbmi) ||
        (CHECK_LAST_LAST2_LAST3(edge_mbmi->ref_frame[0]) &&
         !has_second_ref(edge_mbmi)))
      pred_context = 2;
    else if (!has_second_ref(edge_mbmi))
      pred_context = 4 * (edge_mbmi->ref_frame[0] == GOLDEN_FRAME);
...
//
// NOTE(zoeliu): The probability of ref_frame[0] is LAST3_FRAME, conditioning
// on it is either LAST3_FRAME/LAST2_FRAME/LAST_FRAME.
int vp9_get_pred_context_single_ref_p3(const MACROBLOCKD *xd) {
  int pred_context;
  const MB_MODE_INFO *const above_mbmi = get_mbmi(get_above_mi(xd));
  const MB_MODE_INFO *const left_mbmi = get_mbmi(get_left_mi(xd));
...
#define CHECK_GOLDEN_ALTREF_LAST3(ref_frame) \
  ((ref_frame == GOLDEN_FRAME) || (ref_frame == ALTREF_FRAME) || \
  (ref_frame == LAST3_FRAME))
...
int vp9_get_pred_context_single_ref_p4(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      const MB_MODE_INFO *edge_mbmi = above_intra ? left_mbmi : above_mbmi;
      if (!has_second_ref(edge_mbmi)) {
        if (CHECK_GOLDEN_ALTREF_LAST3(edge_mbmi->ref_frame[0]))
          pred_context = 3;
        else
          pred_context = 4 * (edge_mbmi->ref_frame[0] == LAST_FRAME);
      } else {
      ...
      if (above_has_second && left_has_second) {
        ...
        if (CHECK_GOLDEN_ALTREF_LAST3(above0) &&
            CHECK_GOLDEN_ALTREF_LAST3(left0)) {
          pred_context = 2 + (above0 == left0);
        } else if (CHECK_GOLDEN_ALTREF_LAST3(above0) ||
                   CHECK_GOLDEN_ALTREF_LAST3(left0)) {
          const MV_REFERENCE_FRAME edge0 =
              CHECK_GOLDEN_ALTREF_LAST3(above0) ? left0 : above0;
          pred_context = 4 * (edge0 == LAST_FRAME);
        } else {
          pred_context = 2 * (above0 == LAST_FRAME) + 2 * (left0 == LAST_FRAME);
        }
    ...
    if (!is_inter_block(edge_mbmi) ||
        (CHECK_GOLDEN_ALTREF_LAST3(edge_mbmi->ref_frame[0]) &&
         !has_second_ref(edge_mbmi)))
      pred_context = 2;
    else if (!has_second_ref(edge_mbmi))
      pred_context = 4 * (edge_mbmi->ref_frame[0] == LAST_FRAME);
...
int vp9_get_pred_context_single_ref_p1(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (above_intra && left_intra) {  // intra/intra
      ...
      const MB_MODE_INFO *edge_mbmi = above_intra ? left_mbmi : above_mbmi;
      if (IS_SINGLE_REF(edge_mbmi))
        pred_context = 4 * (edge_mbmi->ref_frame[0] == LAST_FRAME);
      else
        pred_context = 1 + (edge_mbmi->ref_frame[0] == LAST_FRAME ||
                            edge_mbmi->ref_frame[1] == LAST_FRAME);
    } else {  // inter/inter
      const int above_has_second = !IS_SINGLE_REF(above_mbmi);
      const int left_has_second = !IS_SINGLE_REF(left_mbmi);
      const MV_REFERENCE_FRAME above0 = above_mbmi->ref_frame[0];
      const MV_REFERENCE_FRAME above1 = above_mbmi->ref_frame[1];
      const MV_REFERENCE_FRAME left0 = left_mbmi->ref_frame[0];
      const MV_REFERENCE_FRAME left1 = left_mbmi->ref_frame[1];
...
int vp9_get_pred_context_single_ref_p2(const MACROBLOCKD *xd) {
  ...
  if (has_above && has_left) {  // both edges available
    ...
    if (!is_inter_block(edge_mbmi) ||
        (edge_mbmi->ref_frame[0] == LAST_FRAME && IS_SINGLE_REF(edge_mbmi)))
      pred_context = 2;
    else if (IS_SINGLE_REF(edge_mbmi))
      pred_context = 4 * (edge_mbmi->ref_frame[0] == GOLDEN_FRAME);
    else
      pred_context = 3 * (edge_mbmi->ref_frame[0] == GOLDEN_FRAME ||
                          edge_mbmi->ref_frame[1] == GOLDEN_FRAME);
