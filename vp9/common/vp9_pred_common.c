...
#if CONFIG_SB8X8
  const int x_mis = sub(bw, xd->mb_to_right_edge >> 6);
  const int y_mis = sub(bh, xd->mb_to_bottom_edge >> 6);
#else
  const int x_mis = sub(bw, xd->mb_to_right_edge >> 7);
  const int y_mis = sub(bh, xd->mb_to_bottom_edge >> 7);
#endif
#undef sub
  int x, y;
