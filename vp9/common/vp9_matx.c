...
void vp9_matx_affirm(MATX_PTR _self, int rows, int cols, int stride, int cn,
                     MATX_TYPE typeid) {
  struct MATX *const self = (struct MATX *)_self;
  ...
  if (stride <= 0) self->stride = cols * cn;
  ...
  switch (self->typeid) {
    case TYPE_8U: VP9_MATX_REALLOC(MATX_ELEMTYPE_8U, self); break;
