...
static const uint8_t vp9_nuq_knotes_tiny[COEF_BANDS][NUQ_KNOTES] = {
  {84, 128, 128, 128, 128},  // dc, band 0
  {86, 128, 128, 128, 128},  // band 1
  {86, 128, 128, 128, 128},  // band 2
  {86, 128, 128, 128, 128},  // band 3
  {86, 128, 128, 128, 128},  // band 4
  {86, 128, 128, 128, 128},  // band 5
};
...
static const uint8_t vp9_nuq_knotes_huge[COEF_BANDS][NUQ_KNOTES] = {
  ...
  {86, 124, 128, 128, 128},  // band 1
  {86, 124, 128, 128, 128},  // band 2
  {86, 124, 128, 128, 128},  // band 3
  {86, 124, 128, 128, 128},  // band 4
  {86, 124, 128, 128, 128},  // band 5
};

static const uint8_t vp9_nuq_doff_tiny[COEF_BANDS] = { 10, 14, 18, 20, 22, 22 };
static const uint8_t vp9_nuq_doff_low[COEF_BANDS] =  { 10, 14, 18, 20, 22, 22 };
static const uint8_t vp9_nuq_doff_mid[COEF_BANDS] =  { 10, 14, 18, 20, 22, 22 };
static const uint8_t vp9_nuq_doff_high[COEF_BANDS] = { 10, 14, 18, 20, 22, 22 };
static const uint8_t vp9_nuq_doff_huge[COEF_BANDS] = { 10, 14, 18, 20, 22, 22 };

// Allow different quantization profiles in different q ranges,
// to enable entropy-constraints in scalar quantization.
static const uint8_t *get_nuq_knotes(int16_t quant, int band) {
  if (quant > 512)
...
static INLINE int16_t quant_to_doff_fixed(int16_t quant, int band) {
  if (quant > 512)
...
static INLINE void get_cumbins_nuq(int q, int band, tran_low_t *cumbins) {
  const uint8_t *knotes = get_nuq_knotes(q, band);
...
void vp9_get_dequant_val_nuq(int q, int band,
  ...
  get_cumbins_nuq(q, band, cumbins_ptr);
  dq[0] = 0;
  for (i = 1; i < NUQ_KNOTES; ++i) {
    const int16_t qstep = (knotes[i] * q + 64) >> 7;
    // doff = quant_to_doff(qstep, band);
    doff = quant_to_doff_fixed(qstep, band);
    doff = (2 * doff * qstep + q) / (2 * q);
    dq[i] = cumbins_ptr[i - 1] + (((knotes[i] - doff * 2) * q + 128) >> 8);
  }
  // doff = quant_to_doff(q, band);
  doff = quant_to_doff_fixed(q, band);
  dq[NUQ_KNOTES] =
