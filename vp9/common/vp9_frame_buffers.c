...
int vp9_alloc_internal_frame_buffers(InternalFrameBufferList_t* list) {
  assert(list != NULL);
...
void vp9_free_internal_frame_buffers(InternalFrameBufferList_t *list) {
  int i;
...
int vp9_get_frame_buffer(void *cb_priv, size_t min_size,
  ...
  int i;
  InternalFrameBufferList_t *const int_fb_list =
      (InternalFrameBufferList_t*)cb_priv;
  ...
  fb->priv = (void*)(&int_fb_list->int_fb[i]);
  return 0;
}

int vp9_release_frame_buffer(void *cb_priv, vpx_codec_frame_buffer_t *fb) {
  InternalFrameBuffer_t *int_fb;
  (void)cb_priv;
  if (!fb)
    return -1;

  int_fb = (InternalFrameBuffer_t*)fb->priv;
  int_fb->in_use = 0;
  return 0;
}
