...
void vp9_lpf_mt_init(VP9_COMMON *cm, VP9LfSync *lf_sync, int frame_filter_level,
  ...
  memset(lf_sync->num_tiles_in_frame_row_done, 0,
         sizeof(*lf_sync->num_tiles_in_frame_row_done) * sb_rows);
  cm->lf_row = 0;
}
...
void vp9_loop_filter_alloc(VP9LfSync *lf_sync, VP9_COMMON *cm, int rows,
  ...
  CHECK_MEM_ERROR(
      cm, lf_sync->num_tiles_in_frame_row_done,
      vpx_malloc(sizeof(*lf_sync->num_tiles_in_frame_row_done) *
...
void vp9_loop_filter_dealloc(VP9LfSync *lf_sync) {
  if (lf_sync != NULL) {
    ...
    vpx_free(lf_sync->num_tiles_in_frame_row_done);
