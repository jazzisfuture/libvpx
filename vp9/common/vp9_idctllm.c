...
static const int16_t iadst_i16[256] = {
    284, 850, 1407, 1951, 2476, 2977, 3450, 3889, 4291, 4652, 4967, 5235, 5453, 5618, 5729, 5784,
    850, 2476, 3889, 4967, 5618, 5784, 5453, 4652, 3450, 1951, 284, -1407, -2977, -4291, -5235, -5729,
    1407, 3889, 5453, 5729, 4652, 2476, -284, -2977, -4967, -5784, -5235, -3450, -850, 1951, 4291, 5618,
    1951, 4967, 5729, 3889, 284, -3450, -5618, -5235, -2476, 1407, 4652, 5784, 4291, 850, -2977, -5453,
    2476, 5618, 4652, 284, -4291, -5729, -2977, 1951, 5453, 4967, 850, -3889, -5784, -3450, 1407, 5235,
    2977, 5784, 2476, -3450, -5729, -1951, 3889, 5618, 1407, -4291, -5453, -850, 4652, 5235, 284, -4967,
    3450, 5453, -284, -5618, -2977, 3889, 5235, -850, -5729, -2476, 4291, 4967, -1407, -5784, -1951, 4652,
    3889, 4652, -2977, -5235, 1951, 5618, -850, -5784, -284, 5729, 1407, -5453, -2476, 4967, 3450, -4291,
    4291, 3450, -4967, -2476, 5453, 1407, -5729, -284, 5784, -850, -5618, 1951, 5235, -2977, -4652, 3889,
    4652, 1951, -5784, 1407, 4967, -4291, -2476, 5729, -850, -5235, 3889, 2977, -5618, 284, 5453, -3450,
    4967, 284, -5235, 4652, 850, -5453, 4291, 1407, -5618, 3889, 1951, -5729, 3450, 2476, -5784, 2977,
    5235, -1407, -3450, 5784, -3889, -850, 4967, -5453, 1951, 2977, -5729, 4291, 284, -4652, 5618, -2476,
    5453, -2977, -850, 4291, -5784, 4652, -1407, -2476, 5235, -5618, 3450, 284, -3889, 5729, -4967, 1951,
    5618, -4291, 1951, 850, -3450, 5235, -5784, 4967, -2977, 284, 2476, -4652, 5729, -5453, 3889, -1407,
    5729, -5235, 4291, -2977, 1407, 284, -1951, 3450, -4652, 5453, -5784, 5618, -4967, 3889, -2476, 850,
    5784, -5729, 5618, -5453, 5235, -4967, 4652, -4291, 3889, -3450, 2977, -2476, 1951, -1407, 850, -284
};
#else
static const int16_t iadst_i16[256] = {
...
#if CONFIG_INTHT
#define COS_1_32   1441      // cos(pi *  1 / 32) * sqrt(2) * 1024
#define COS_5_32   1277      // cos(pi *  5 / 32) * sqrt(2) * 1024
#define COS_9_32   919       // cos(pi *  9 / 32) * sqrt(2) * 1024
#define COS_13_32  420       // cos(pi * 13 / 32) * sqrt(2) * 1024
#define SIN_1_32   142       // sin(pi * 1  / 32) * sqrt(2) * 1024
#define SIN_2_32   283       // sin(pi * 2  / 32) * sqrt(2) * 1024
#define SIN_4_32   554       // sin(pi * 8  / 32) * sqrt(2) * 1024
#define SIN_5_32   683       // sin(pi * 5  / 32) * sqrt(2) * 1024
#define SIN_6_32   805       // sin(pi * 6  / 32) * sqrt(2) * 1024
#define SIN_9_32   1119      // sin(pi * 9  / 32) * sqrt(2) * 1024
#define SIN_10_32  1204      // sin(pi * 10 / 32) * sqrt(2) * 1024
#define SIN_12_32  1338      // sin(pi * 12 / 32) * sqrt(2) * 1024
#define SIN_13_32  1386      // sin(pi * 13 / 32) * sqrt(2) * 1024
#define SIN_14_32  1420      // sin(pi * 14 / 32) * sqrt(2) * 1024

#define RT_SHIFT     14
#define RD_FACTOR    ((1 << (RT_SHIFT-1)) - 1)

static void idctrow8(int *blk) {
  ...
  s0 = x0 + x1;
  s1 = x0 - x1;
  s2 = SIN_4_32 * x2  - SIN_12_32 * x3;
  s3 = SIN_12_32 * x2 + SIN_4_32  * x3;
  s4 = SIN_2_32 * x4  - SIN_14_32 * x7;
  s5 = SIN_10_32 * x5 - SIN_6_32 * x6;
  s6 = SIN_6_32 * x5  + SIN_10_32 * x6;
  s7 = SIN_14_32 * x4 + SIN_2_32 * x7;
...
static void idctcol8(int *blk) {
  ...
  if (!(x0 | x1 | x2 | x3 | x4 | x5 | x6 | x7)) {
    blk[0] = blk[(1 << 3)] = blk[(2 << 3)] = blk[(3 << 3)] = blk[(4 << 3)]
                    = blk[(5 << 3)] = blk[(6 << 3)] = blk[(7 << 3)] = 0;
    return;
  }
  ...
  // stage 3
  blk[0] = (s0 + s7 + ((1 << 18) - 1)) >> 19;
  blk[(1 << 3)] = (s1 + s6 + ((1 << 18) - 1)) >> 19;
  blk[(2 << 3)] = (s2 + s5 + ((1 << 18) - 1)) >> 19;
  blk[(3 << 3)] = (s3 + s4 + ((1 << 18) - 1)) >> 19;
  blk[(4 << 3)] = (s3 - s4 + ((1 << 18) - 1)) >> 19;
  blk[(5 << 3)] = (s2 - s5 + ((1 << 18) - 1)) >> 19;
  blk[(6 << 3)] = (s1 - s6 + ((1 << 18) - 1)) >> 19;
  blk[(7 << 3)] = (s0 - s7 + ((1 << 18) - 1)) >> 19;

  return;
}

static void iadstrow8(int *blk) {
  int x0, x1, x2, x3, x4, x5, x6, x7;
  int s0, s1, s2, s3, s4, s5, s6, s7;
  ...
  // stage 1
  s0 = COS_1_32 * x0 + SIN_1_32 * x1;
  s1 = SIN_1_32 * x0 - COS_1_32 * x1;
  s2 = COS_5_32 * x2 + SIN_5_32 * x3;
  s3 = SIN_5_32 * x2 - COS_5_32 * x3;
  s4 = COS_9_32 * x4 + SIN_9_32 * x5;
  s5 = SIN_9_32 * x4 - COS_9_32 * x5;
  s6 = COS_13_32 * x6 + SIN_13_32 * x7;
  s7 = SIN_13_32 * x6 - COS_13_32 * x7;
...
static void iadstcol8(int *blk) {
  ...
  x0 = s0 + s2;
  x1 = s1 + s3;
  x2 = (s0 - s2 + RD_FACTOR) >> RT_SHIFT;
  x3 = (s1 - s3 + RD_FACTOR) >> RT_SHIFT;
  x4 = s4 + s6;
  x5 = s5 + s7;
  x6 = (s4 - s6 + RD_FACTOR) >> RT_SHIFT;
  x7 = (s5 - s7 + RD_FACTOR) >> RT_SHIFT;
  ...
  blk[0]        =   ((s0 + ((1 << 19) - 1)) >> 20);
  blk[(1 << 3)] = - ((s4 + ((1 << 19) - 1)) >> 20);
  blk[(2 << 3)] =   ((s6 + ((1 << 13) - 1)) >> 14);
  blk[(3 << 3)] = - ((s2 + ((1 << 13) - 1)) >> 14);
  blk[(4 << 3)] =   ((s3 + ((1 << 13) - 1)) >> 14);
  blk[(5 << 3)] = - ((s7 + ((1 << 13) - 1)) >> 14);
  blk[(6 << 3)] =   ((s5 + ((1 << 19) - 1)) >> 20);
  blk[(7 << 3)] = - ((s1 + ((1 << 19) - 1)) >> 20);

  return;
}

void vp9_short_iht8x8_c(int16_t *coefs, int16_t *block,
