...
#define synthesis_row synthesis_26_row
#define synthesis_col synthesis_26_col
...
void dyadic_synthesize(int levels, int width, int height, short *c, int pitch_c,
  ...
  for (lv = levels - 1; lv >= 0; lv--) {
    ...
    for (j = 0; j < nw; j++) {
      ...
      synthesis_col(nh, buffer, buffer + hh, buffer + nh);
      for (i = 0; i < nh; i++)
        c[i * pitch_c + j] = buffer[i + nh];
    }
    for (i = 0; i < nh; i++) {
      memcpy(buffer, &c[i * pitch_c], nw * sizeof(short));
      synthesis_row(nw, buffer, buffer + hw, &c[i * pitch_c]);
    }
  }
  for (i = 0; i < height; i++)
    for (j = 0; j < width; j++)
      x[i * pitch_x + j] = (c[i * pitch_c + j] + DWT_PRECISION_RND) >>
      DWT_PRECISION_BITS;
}
...
void vp9_short_idct32x32_c(short *input, short *output, int pitch) {
  ...
  for (i = 0; i < 16; ++i) {
    vpx_memcpy(buffer2 + i * 32 + 16, buffer + i * 16, sizeof(short) * 16);
  }
