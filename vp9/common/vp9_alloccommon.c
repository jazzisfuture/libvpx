...
void lock_buffer_pool(const VP9_COMMON *cm) {
...
void unlock_buffer_pool(const VP9_COMMON *cm) {
#if CONFIG_MULTITHREAD
  if (cm->frame_parallel_decode)
...
static inline void alloc_mi_arry(VP9_COMMON *cm, int mi_size, int idx) {
  CHECK_MEM_ERROR(cm, cm->mip_array[idx],
                  vpx_calloc(mi_size, sizeof(*cm->mip)));
  CHECK_MEM_ERROR(cm, cm->mi_grid_base_array[idx],
                  vpx_calloc(mi_size, sizeof(*cm->mi_grid_base)));
}
...
void vp9_swap_mi_and_prev_mi(VP9_COMMON *cm) {
  ...
  // Update the prev_mi buffer now if needed. The worker that was accessing it
  // must already finish decoding. So it can be resized safely now.
  if (cm->frame_parallel_decode && cm->update_prev_mi) {
    ...
    alloc_mi_arry(cm, mi_size, cm->prev_mi_idx);
    cm->update_prev_mi = 0;
  }
