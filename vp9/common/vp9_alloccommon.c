...
static int alloc_mi(VP9_COMMON *cm, int mi_size) {
  ...
  for (i = 0; i < 2; i++) {
    cm->mip_array[i] = (MODE_INFO *)vpx_calloc(mi_size, sizeof(*cm->mip_array[i]));
    if (cm->mip_array[i] == NULL)
      return 1;
  }

  for (i = 0; i < 2; i++) {
    cm->mi_grid_base_array[i] =  (MODE_INFO **)vpx_calloc(mi_size, sizeof(*cm->mi_grid_base_array[i]));
    if (cm->mi_grid_base_array[i] == NULL)
      return 1;
  }

  // init ping-pong index.
  cm->mi_idx = 0;
  cm->prev_mi_idx = 1;
