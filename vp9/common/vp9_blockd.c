...
void vp9_foreach_transformed_block_in_plane(
  ...
  int max_blocks_wide = xd->mb_to_right_edge >= 0 ? num_4x4_w :
      num_4x4_w + (xd->mb_to_right_edge >> (5 + pd->subsampling_x));
  int max_blocks_high = xd->mb_to_bottom_edge >= 0 ? num_4x4_h :
      num_4x4_h + (xd->mb_to_bottom_edge >> (5 + pd->subsampling_y));
  ...
  for (r = 0; r < num_4x4_h; r += (1 << tx_size)) {
    for (c = 0; c < num_4x4_w; c += (1 << tx_size)) {
      // Skip visit the sub blocks that are wholly within the UMV.
