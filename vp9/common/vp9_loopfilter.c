    ...
    if (mask & 1) {
      ...
      if ((mask_8x8_0 | mask_8x8_1) & 1) {
        if ((mask_8x8_0 & mask_8x8_1) & 1) {
          ...
          vp9_high_lpf_vertical_8(s, pitch, lfi0->mblim, lfi0->lim, lfi0->hev_thr,
                             1, bps);
        } else {
          vp9_high_lpf_vertical_8(s + 8 * pitch, pitch, lfi1->mblim, lfi1->lim,
                             lfi1->hev_thr, 1, bps);
        }
      }
      ...
      if ((mask_4x4_0 | mask_4x4_1) & 1) {
        if ((mask_4x4_0 & mask_4x4_1) & 1) {
          ...
          vp9_high_lpf_vertical_4(s, pitch, lfi0->mblim, lfi0->lim, lfi0->hev_thr,
                             1, bps);
        } else {
      ...
      if ((mask_4x4_int_0 | mask_4x4_int_1) & 1) {
        if ((mask_4x4_int_0 & mask_4x4_int_1) & 1) {
          ...
          vp9_high_lpf_vertical_4(s + 8 * pitch + 4, pitch, lfi1->mblim, lfi1->lim,
                             lfi1->hev_thr, 1, bps);
        }
      }
    }
    ...
    if (mask & 1) {
      if (mask_16x16 & 1) {
        ...
        if ((mask_8x8 & 3) == 3) {
          ...
          vp9_high_lpf_horizontal_8(s, pitch, lfi->mblim, lfi->lim, lfi->hev_thr, 1, bps);
        ...
        if ((mask_4x4 & 3) == 3) {
          ...
          vp9_high_lpf_horizontal_4(s, pitch, lfi->mblim, lfi->lim, lfi->hev_thr, 1, bps);
...
static void high_filter_selectively_vert(uint16_t *s, int pitch,
  ...
  for (mask = mask_16x16 | mask_8x8 | mask_4x4 | mask_4x4_int;
    ...
    if (mask & 1) {
      if (mask_16x16 & 1) {
        vp9_high_lpf_vertical_16(s, pitch, lfi->mblim, lfi->lim, lfi->hev_thr, bps);
      } else if (mask_8x8 & 1) {
        vp9_high_lpf_vertical_8(s, pitch, lfi->mblim, lfi->lim, lfi->hev_thr, 1, bps);
      } else if (mask_4x4 & 1) {
        vp9_high_lpf_vertical_4(s, pitch, lfi->mblim, lfi->lim, lfi->hev_thr, 1, bps);
      }
    }
    if (mask_4x4_int & 1)
      vp9_high_lpf_vertical_4(s + 4, pitch, lfi->mblim, lfi->lim, lfi->hev_thr, 1, bps);
    s += 8;
    lfl += 1;
    mask_16x16 >>= 1;
    mask_8x8 >>= 1;
  ...
  for (r = 0; r < MI_BLOCK_SIZE && mi_row + r < cm->mi_rows; r += row_step) {
    ...
    if (cm->use_high) {
      high_filter_selectively_vert(CONVERT_TO_SHORTPTR(dst->buf), dst->stride,
                            ...
                            mask_4x4_int[r],
                            &cm->lf_info, &lfl[r << 3],cm->bit_depth*2+8);
    } else {
      ...
      // Disable filtering on the leftmost column
        ...
        if (cm->use_high) {
          high_filter_selectively_vert_row2(plane->plane_type,
                                   ...
                                   mask_4x4_l,
                                   mask_4x4_int_l,
                                   &cm->lf_info, &lfm->lfl_y[r << 3],cm->bit_depth*2+8);
        } else {
...

      {
        ...
        if (cm->use_high) {
          high_filter_selectively_vert_row2(plane->plane_type,
                                     ...
                                     mask_4x4_l,
                                     mask_4x4_int_l,
                                     &cm->lf_info, &lfm->lfl_uv[r << 1],cm->bit_depth*2+8);
        } else {
          filter_selectively_vert_row2(plane->plane_type,
                                     dst->buf, dst->stride,
                                     mask_16x16_l,
