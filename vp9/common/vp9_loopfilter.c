...
extern void vp9_lpf_vertical_4b(uint8_t *s, int p, const uint8_t *blimit,
                                const uint8_t *limit, const uint8_t *thresh);
extern void vp9_lpf_vertical_8b(uint8_t *s, int p, const uint8_t *blimit,
                                const uint8_t *limit, const uint8_t *thresh);
extern void vp9_lpf_vertical_16b(uint8_t *s, int p, const uint8_t *blimit,
                                 const uint8_t *limit, const uint8_t *thresh);

static void filter_selectively_vert_row1(PLANE_TYPE plane_type,
                                         uint8_t *s, int pitch,
  ...
  for (mask = mask_16x16_0 | mask_8x8_0 | mask_4x4_0 | mask_4x4_int_0;
    ...
    if (mask & 1) {
      if (mask_16x16_0 & 1) {
        vp9_lpf_vertical_16b(s, pitch, lfi0->mblim, lfi0->lim, lfi0->hev_thr);
      }

      if (mask_8x8_0 & 1) {
        vp9_lpf_vertical_8b(s, pitch, lfi0->mblim, lfi0->lim, lfi0->hev_thr);
      }

      if (mask_4x4_0 & 1) {
        vp9_lpf_vertical_4b(s, pitch, lfi0->mblim, lfi0->lim, lfi0->hev_thr);
      }

      if (mask_4x4_int_0 & 1) {
        vp9_lpf_vertical_4b(s + 4, pitch, lfi0->mblim, lfi0->lim,
                            lfi0->hev_thr);
      }
    }
...
static void filter_block_plane(VP9_COMMON *const cm,
  ...
  if (!plane->plane_type) {
    ...
    // Vertical pass: do 2 rows at one time
    for (r = 0; r < MI_BLOCK_SIZE && mi_row + r + 1 < cm->mi_rows; r += 4) {
    ...
    if ( mi_row + r + 1 == cm->mi_rows ) {
      ...
      filter_selectively_vert_row1(plane->plane_type,
                                   dst->buf, dst->stride,
                                   mask_16x16_l,
                                   mask_8x8_l,
                                   mask_4x4_l,
