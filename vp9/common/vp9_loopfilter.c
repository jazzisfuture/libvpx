...
static void vp9_lpf_marcoblock(VP9_COMMON *cm, const MODE_INFO *mi,
  ...
  if (filter_level) {
    ...
    if (cm->filter_type == NORMAL_LOOPFILTER) {
      ...
      lpf_mb_mbedges(y_ptr, u_ptr, v_ptr, y_stride, uv_stride, do_left_v,
                     do_above_h, tx_size, &lfi);
      if (!skip_lf)
        lpf_mb_internal(y_ptr, u_ptr, v_ptr, y_stride, uv_stride, mode, tx_size,
                        &lfi);
    } else {
...
void vp9_loop_filter_frame(VP9_COMMON *cm,
  ...
  if(extra_mb_row) {
    for (mb_col = 0; mb_col < cm->mb_cols; mb_col++) {
      ...
      int sb_type = mi->mbmi.sb_type;
      int tx_size = mi->mbmi.txfm_size;
      int do_left_v =  (mb_col > 0);
      int do_above_h = (mb_row > 0);
      vp9_lpf_marcoblock(cm, mi,
                         do_left_v, do_above_h,
                         y_ptr,
                         y_only? NULL : u_ptr,
                         y_only? NULL : v_ptr,
                         post->y_stride, post->uv_stride);
