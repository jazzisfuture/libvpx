...
const static uint64_t left_64x64_txform_mask[TX_SIZES]= {
...
const static uint64_t above_64x64_txform_mask[TX_SIZES]= {
    0xffffffffffffffff,  // TX_4X4
    0xffffffffffffffff,  // TX_8x8
...
const static uint64_t left_prediction_mask[BLOCK_SIZE_TYPES] = {
...
// 64 bit mask to shift and set for each prediction size
const static uint64_t above_prediction_mask[BLOCK_SIZE_TYPES] = {
    0x0000000000000001,  // BLOCK_4X4
    0x0000000000000001,  // BLOCK_4X8
    0x0000000000000001,  // BLOCK_8X4
    0x0000000000000001,  // BLOCK_8X8
...
// each 8x8 block that would be would be in the left most block of this size
// in the image
const static uint64_t size_mask[BLOCK_SIZE_TYPES] = {
    0x0000000000000001,  // BLOCK_4X4
    0x0000000000000001,  // BLOCK_4X8
    0x0000000000000001,  // BLOCK_8X4
    0x0000000000000001,  // BLOCK_8X8
...
const static uint64_t left_border =  0x1111111111111111;
const static uint64_t above_border = 0x000000ff000000ff;

// 16 bit masks for uv transform sizes.
const static uint16_t left_64x64_txform_mask_uv[TX_SIZES]= {
    0xffff,  // TX_4X4
    0xffff,  // TX_8x8
    0x5555,  // TX_16x16
    0x1111,  // TX_32x32
...
const static uint16_t above_64x64_txform_mask_uv[TX_SIZES]= {
    0xffff,  // TX_4X4
    0xffff,  // TX_8x8
    0x0f0f,  // TX_16x16
    0x000f,  // TX_32x32
...
// 16 bit left mask to shift and set for each prediction size.
const static uint16_t left_prediction_mask_uv[BLOCK_SIZE_TYPES] = {
    0x0001,  // BLOCK_4X4,
    0x0001,  // BLOCK_4X8,
    0x0001,  // BLOCK_8X4,
    0x0001,  // BLOCK_8X8,
...
// 16 bit above mask to shift and set for each prediction size.
const static uint16_t above_prediction_mask_uv[BLOCK_SIZE_TYPES] = {
    0x0001,  // BLOCK_4X4
    0x0001,  // BLOCK_4X8
    0x0001,  // BLOCK_8X4
    0x0001,  // BLOCK_8X8
...
// 64 bit mask to shift and set for each prediction size
const static uint16_t size_mask_uv[BLOCK_SIZE_TYPES] = {
    0x0001,  // BLOCK_4X4
    0x0001,  // BLOCK_4X8
    0x0001,  // BLOCK_8X4
    0x0001,  // BLOCK_8X8
...
const static uint16_t left_border_uv =  0x1111;
const static uint16_t above_border_uv = 0x000f;
...
static void build_masks(const loop_filter_info_n *const lfi_n,
  ...
  const int skip = mi->mbmi.mb_skip_coeff;
...
static void setup_mask(VP9_COMMON *const cm, const int mi_row, const int mi_col,
  ...
  if (mi_row + 8 > cm->mi_rows) {
    const uint64_t rows = cm->mi_rows - mi_row;

    // Each pixel inside the border gets a 1,
    const uint64_t mask_y = (((uint64_t) 1 << (rows << 3)) - 1);
    uint16_t mask_uv = (((uint16_t) 1 << (((rows+1) >> 1) << 2)) - 1);
  ...
  if(mi_col + 8 > cm->mi_cols) {
    const uint64_t columns = cm->mi_cols - mi_col;

    // Each pixel inside the border gets a 1, the multiply copies the border
    // to where we need it.
    const uint64_t mask_y  = (((1 << columns) - 1)) * 0x0101010101010101;
    uint16_t mask_uv  = ((1 << ((columns+1)>>1)) - 1) * 0x1111;

    // Internal edges are not applied on the last column of the image so
    // we mask 1 more for the internal edges
    uint16_t mask_uv_int  = ((1 << (columns>>1)) - 1) * 0x1111;
