...
static void lpf_sb32(VP9_COMMON *cm, const MODE_INFO *mode_info_context,
                     ...
                     uint8_t *y_ptr, uint8_t *u_ptr, uint8_t *v_ptr,
                     int y_stride, int uv_stride, int y_only) {
    ...
    lpf_mb(cm, mi, do_left_v, do_above_h,
      ...
      y_only ? NULL : (u_ptr + 8 * uv_stride + 8),
      y_only ? NULL : (v_ptr + 8 * uv_stride + 8),
      y_stride, uv_stride);
}
...
void vp9_loop_filter_frame(VP9_COMMON *cm,
  ...
  for (mb_row = 0; mb_row < sb64_rows * 4; mb_row += 4) {
    ...
    if (extra_sb32_col) {
      ...
      mode_info_context += 2;       // step to next SB64
      mb_col += 2;
    }
    if (extra_mb_col) {
      // process 1st MB
      ...
      lpf_mb(cm, mi, do_left_v, do_above_h,
             do_left_v_mbuv, do_above_h_mbuv,
             y_ptr + 32 * post->y_stride,
             y_only ? NULL : (u_ptr + 16 * uv_stride),
             y_only ? NULL : (v_ptr + 16 * uv_stride),
             y_stride, uv_stride);
      // process 2nd MB
      ...
      lpf_mb(cm, mi, do_left_v, do_above_h,
             do_left_v_mbuv, do_above_h_mbuv,
             y_ptr + 47 * post->y_stride,
             y_only ? NULL : (u_ptr + 24 * uv_stride),
             y_only ? NULL : (v_ptr + 24 * uv_stride),
             y_stride, uv_stride);
      y_ptr += 16;
    ...
    y_ptr += post->y_stride  * 64 - post->y_width;
    if (!y_only) {
      u_ptr += post->uv_stride *  32 - post->uv_width;
      v_ptr += post->uv_stride *  32 - post->uv_width;
    }
    mode_info_context++;          /* skip border MB */
    mode_info_context += mis * 3; /* skip to next SB64 row */
  }
  if (extra_sb32_row) {
    for (mb_col = 0; mb_col < sb64_cols * 4 + extra_sb32_col * 2; mb_col += 2) {
      lpf_sb32(cm, mode_info_context, mb_row, mb_col,
               y_ptr, u_ptr, v_ptr,
               y_stride, uv_stride, y_only);
      y_ptr += 32;
    ...
    if (!y_only) {
      u_ptr += uv_stride *  16 - post->uv_width;
      v_ptr += uv_stride *  16 - post->uv_width;
    }
    mode_info_context++;          /* skip border MB */
    mode_info_context += mis;     /* skip to next MB row if exit */
    mb_row += 2;
  }
  if (extra_mb_row) {
