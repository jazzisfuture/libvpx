...
#if CONFIG_WIDERLPF
            vp9_loop_filter_mbv_w(y_ptr, u_ptr, v_ptr, post->y_stride,
                                post->uv_stride, &lfi);
#else
...
#if CONFIG_WIDERLPF
            vp9_loop_filter_mbh_w(y_ptr, u_ptr, v_ptr, post->y_stride,
                                post->uv_stride, &lfi);
#else
...
void vp9_loop_filter_frame_yonly(VP9_COMMON *cm, MACROBLOCKD *xd,
  ...
  for (mb_row = 0; mb_row < cm->mb_rows; mb_row++) {
    for (mb_col = 0; mb_col < cm->mb_cols; mb_col++) {
      ...
      if (filter_level) {
        const int skip_lf = mb_lf_skip(&mode_info_context->mbmi);
        const int tx_type = mode_info_context->mbmi.txfm_size;
        if (cm->filter_type == NORMAL_LOOPFILTER) {
          const int hev_index = lfi_n->hev_thr_lut[frame_type][filter_level];
          lfi.mblim = lfi_n->mblim[filter_level];
          lfi.blim = lfi_n->blim[filter_level];
