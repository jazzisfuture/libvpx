...
static void filter_block_plane(VP9_COMMON *cm, MACROBLOCKD *xd,
  ...
  unsigned int mask_16x16[MAX_MI_BLOCK_SIZE] = {0};
  unsigned int mask_8x8[MAX_MI_BLOCK_SIZE] = {0};
  unsigned int mask_4x4[MAX_MI_BLOCK_SIZE] = {0};
  unsigned int mask_4x4_int[MAX_MI_BLOCK_SIZE] = {0};
  struct loop_filter_info lfi[MAX_MI_BLOCK_SIZE][MAX_MI_BLOCK_SIZE];
  int r, c;
  MODE_INFO *mi = xd->mode_info_context;
  int row_step_stride = cm->mode_info_stride * row_step;

  for (r = 0; r < MAX_MI_BLOCK_SIZE && mi_row + r < cm->mi_rows; r += row_step) {
    ...
    for (c = 0; c < MAX_MI_BLOCK_SIZE && mi_col + c < cm->mi_cols; c += col_step) {
...
void vp9_loop_filter_frame(VP9_COMMON *cm,
  ...
  for (mi_row = 0; mi_row < cm->mi_rows; mi_row += MAX_MI_BLOCK_SIZE) {
    MODE_INFO* const mi = cm->mi + mi_row * cm->mode_info_stride;

    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MAX_MI_BLOCK_SIZE) {
      int plane;
