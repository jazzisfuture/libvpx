...
static void setup_mask_non420(VP9_COMMON *const cm,
                              const int mi_row, const int mi_col,
                              const MODE_INFO *mi, LOOP_FILTER_MASK *lfm,
                              MACROBLOCKD *xd) {
  ...
  for (plane_idx = 0; plane_idx < 2; ++plane_idx) {
    struct macroblockd_plane *const plane = &xd->plane[plane_idx];
    const int ss_x = plane->subsampling_x;
    const int ss_y = plane->subsampling_y;
    const int row_step = 1 << ss_x;
    const int col_step = 1 << ss_y;
    const int row_step_stride = cm->mode_info_stride * row_step;
    uint64_t mask_16x16[MI_BLOCK_SIZE] = {0};
    uint64_t mask_8x8[MI_BLOCK_SIZE] = {0};
    uint64_t mask_4x4[MI_BLOCK_SIZE] = {0};
    uint64_t mask_4x4_int[MI_BLOCK_SIZE] = {0};
    int row_shift = 3 - ss_x;
    mi = mip;

    for (r = 0; r < MI_BLOCK_SIZE && mi_row + r < cm->mi_rows; r += row_step) {
      uint64_t mask_16x16_c = 0;
      uint64_t mask_8x8_c = 0;
      uint64_t mask_4x4_c = 0;
      uint64_t border_mask;
      int r_sampled = r >> ss_x;

      // Determine the vertical edges that need filtering
      for (c = 0; c < MI_BLOCK_SIZE && mi_col + c < cm->mi_cols;
           c += col_step) {
        const int skip_this =
            mi[c].mbmi.skip_coeff && is_inter_block(&mi[c].mbmi);
        // left edge of current unit is block/partition edge -> no skip
        const int block_edge_left =
            b_width_log2(mi[c].mbmi.sb_type)
                ? !(c & ((1 << (b_width_log2(mi[c].mbmi.sb_type) - 1)) - 1))
                : 1;
        const int skip_this_c = skip_this && !block_edge_left;
        // top edge of current unit is block/partition edge -> no skip
        const int block_edge_above =
            b_height_log2(mi[c].mbmi.sb_type)
                ? !(r & ((1 << (b_height_log2(mi[c].mbmi.sb_type) - 1)) - 1))
                : 1;
      ...
      if (plane_idx == 0) {
        lfm->left_y[TX_16X16] |= mask_16x16_c << (r_sampled << row_shift);
        lfm->left_y[TX_8X8] |= mask_8x8_c << (r_sampled << row_shift);
        lfm->left_y[TX_4X4] |= mask_4x4_c << (r_sampled << row_shift);
        lfm->above_y[TX_16X16] |= mask_16x16[r] << (r_sampled << row_shift);
        lfm->above_y[TX_8X8] |= mask_8x8[r] << (r_sampled << row_shift);
        lfm->above_y[TX_4X4] |= mask_4x4[r] << (r_sampled << row_shift);
        lfm->int_4x4_y |= mask_4x4_int[r] << (r_sampled << row_shift);
      } else {
        lfm->left_uv[TX_16X16] |= mask_16x16_c << (r_sampled << row_shift);
        lfm->left_uv[TX_8X8] |= mask_8x8_c << (r_sampled << row_shift);
        lfm->left_uv[TX_4X4] |= mask_4x4_c << (r_sampled << row_shift);
        lfm->above_uv[TX_16X16] |= mask_16x16[r] << (r_sampled << row_shift);
        lfm->above_uv[TX_8X8] |= mask_8x8[r] << (r_sampled << row_shift);
        lfm->above_uv[TX_4X4] |= mask_4x4[r] << (r_sampled << row_shift);
        lfm->int_4x4_uv |= mask_4x4_int[r] << (r_sampled << row_shift);
      }
      mi += row_step_stride;
    }
  }
...
static void build_masks(const loop_filter_info_n *const lfi_n,
  ...
  const BLOCK_SIZE_TYPE block_size = mi->mbmi.sb_type;
  const TX_SIZE tx_size_y = mi->mbmi.txfm_size;
...
void vp9_loop_filter_rows(const YV12_BUFFER_CONFIG *frame_buffer,
  ...
  for (mi_row = start; mi_row < stop; mi_row += MI_BLOCK_SIZE) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MI_BLOCK_SIZE) {
      ...
      // TODO(JBB): Make the non420 function go away by handling the
      // other colorspaces in setup_mask.
      if (!y_only && ( xd->plane[1].subsampling_y ||
          xd->plane[1].subsampling_x))
        setup_mask_non420(cm, mi_row, mi_col, mi + mi_col, &lfm, xd);
      else
