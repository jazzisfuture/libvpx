...
void vp9_adjust_loop_filter_mask(VP9_COMMON *const cm, const int mi_row,
...
static void setup_mask(VP9_COMMON *const cm, const int mi_row, const int mi_col,
  ...
  switch (mip[0]->mbmi.sb_type) {
    case BLOCK_64X64:
      vp9_build_masks(lfi_n, mip[0] , 0, 0, lfm);
      break;
    case BLOCK_64X32:
      vp9_build_masks(lfi_n, mip[0], 0, 0, lfm);
      ...
      if (4 >= max_rows)
        break;
      vp9_build_masks(lfi_n, mip2[0], 32, 8, lfm);
      break;
    case BLOCK_32X64:
      vp9_build_masks(lfi_n, mip[0], 0, 0, lfm);
      mip2 = mip + 4;
      if (4 >= max_cols)
        break;
      vp9_build_masks(lfi_n, mip2[0], 4, 2, lfm);
      break;
    default:
      for (idx_32 = 0; idx_32 < 4; mip += offset_32[idx_32], ++idx_32) {
        ...
        switch (mip[0]->mbmi.sb_type) {
          case BLOCK_32X32:
            vp9_build_masks(lfi_n, mip[0], shift_y, shift_uv, lfm);
            break;
          case BLOCK_32X16:
            vp9_build_masks(lfi_n, mip[0], shift_y, shift_uv, lfm);
            ...
            vp9_build_masks(lfi_n, mip2[0], shift_y + 16, shift_uv + 4, lfm);
            break;
          case BLOCK_16X32:
            vp9_build_masks(lfi_n, mip[0], shift_y, shift_uv, lfm);
            if (mi_32_col_offset + 2 >= max_cols)
              continue;
            mip2 = mip + 2;
            vp9_build_masks(lfi_n, mip2[0], shift_y + 2, shift_uv + 1, lfm);
            break;
          default:
            for (idx_16 = 0; idx_16 < 4; mip += offset_16[idx_16], ++idx_16) {
              ...
              switch (mip[0]->mbmi.sb_type) {
                ...
                case BLOCK_16X8:
                  ...
                  vp9_build_y_mask(lfi_n, mip2[0], shift_y+8, lfm);
                  break;
                case BLOCK_8X16:
                  vp9_build_masks(lfi_n, mip[0], shift_y, shift_uv, lfm);
                  if (mi_16_col_offset +1 >= max_cols)
                    continue;
                  mip2 = mip + 1;
                  vp9_build_y_mask(lfi_n, mip2[0], shift_y+1, lfm);
                  break;
                default: {
                  const int shift_y = shift_32_y[idx_32] +
                                      shift_16_y[idx_16] +
                  ...
                  for (idx_8 = 1; idx_8 < 4; mip += offset[idx_8], ++idx_8) {
                    ...
                    if (mi_8_col_offset >= max_cols ||
                      ...
                      continue;
                    vp9_build_y_mask(lfi_n, mip[0], shift_y, lfm);
                  }
                  break;
                }
              }
...
void vp9_loop_filter_rows(const YV12_BUFFER_CONFIG *frame_buffer,
  ...
  for (mi_row = start; mi_row < stop; mi_row += MI_BLOCK_SIZE) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MI_BLOCK_SIZE) {
      ...
      if (use_420)
#endif
        //setup_mask(cm, mi_row, mi_col, mi_8x8 + mi_col, cm->mode_info_stride, &lfm);
        lfmp = pick_lfm(cm, mi_row, mi_col);
