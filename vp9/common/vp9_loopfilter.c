...
static void lpf_mb_mbedges(uint8_t* y_ptr, uint8_t* u_ptr, uint8_t* v_ptr,
                           const int y_stride, const int uv_stride,
                           const int filter_v, const int filter_h,
                           const int tx_size,
...
static void lpf_mb_internal(uint8_t* y_ptr, uint8_t* u_ptr, uint8_t* v_ptr,
                            const int y_stride, const int uv_stride,
                            MB_PREDICTION_MODE mode, const int tx_size,
                            struct loop_filter_info *lfi) {
  ...
  if (tx_size >= TX_8X8) {
    if (tx_size == TX_8X8 && (mode == I8X8_PRED || mode == SPLITMV)) {
      ...
      vp9_loop_filter_bv8x8(y_ptr, NULL, NULL, y_stride, uv_stride, lfi);
      vp9_loop_filter_bh8x8(y_ptr, NULL, NULL, y_stride, uv_stride, lfi);
    }
  } else  if (tx_size == TX_4X4){
    vp9_loop_filter_bv(y_ptr, u_ptr, v_ptr, y_stride, uv_stride, lfi);
    vp9_loop_filter_bh(y_ptr, u_ptr, v_ptr, y_stride, uv_stride, lfi);
  }
}
...
void vp9_loop_filter_frame(VP9_COMMON *cm,
  ...
  for (mb_row = 0; mb_row < sb_rows * 2; mb_row += 2) {
    ...
    for (mb_col = 0; mb_col < sb_cols * 2; mb_col += 2) {
      ...
      mode = mode_info_context->mbmi.mode;
      mode_index = lfi_n->mode_lf_lut[mode];
      seg = mode_info_context->mbmi.segment_id;
      ref_frame = mode_info_context->mbmi.ref_frame;
      ...
      if (filter_level) {
          ...
          if (cm->filter_type == NORMAL_LOOPFILTER) {
            ...
            lpf_mb_mbedges(y_ptr,
                           y_only? NULL : u_ptr,
                           y_only? NULL : v_ptr,
                           post->y_stride, post->uv_stride,
                           do_left_v, do_above_h, tx_size,
                           &lfi);
            if (!skip_lf)
              lpf_mb_internal(y_ptr,
                              y_only? NULL : u_ptr,
                              y_only? NULL : v_ptr,
                              post->y_stride, post->uv_stride,
                              mode, tx_size, &lfi);
      ...
      if (filter_level) {
          ...
          if (cm->filter_type == NORMAL_LOOPFILTER) {
            ...
            lpf_mb_mbedges(y_ptr + 16 * post->y_stride,
                           y_only ? NULL : (u_ptr + 8 * post->uv_stride),
                           y_only ? NULL : (v_ptr + 8 * post->uv_stride),
                           post->y_stride, post->uv_stride,
                           do_left_v, do_above_h, tx_size, &lfi);
            if (!skip_lf)
              lpf_mb_internal(y_ptr + 16 * post->y_stride,
                              y_only ? NULL : (u_ptr + 8 * post->uv_stride),
                              y_only ? NULL : (v_ptr + 8 * post->uv_stride),
                              post->y_stride, post->uv_stride,
                              mode, tx_size, &lfi);
      ...
      if (filter_level) {
          ...
          if (cm->filter_type == NORMAL_LOOPFILTER) {
            ...
            lpf_mb_mbedges(y_ptr + 16 * post->y_stride + 16,
                           y_only ? NULL : (u_ptr + 8 * post->uv_stride + 8),
                           y_only ? NULL : (v_ptr + 8 * post->uv_stride + 8),
                           post->y_stride, post->uv_stride,
                           do_left_v, do_above_h, tx_size, &lfi);
            if (!skip_lf)
              lpf_mb_internal(y_ptr + 16 * post->y_stride + 16,
                              y_only ? NULL : (u_ptr + 8 * post->uv_stride + 8),
                              y_only ? NULL : (v_ptr + 8 * post->uv_stride + 8),
                              post->y_stride, post->uv_stride,
                              mode, tx_size, &lfi);
    ...
    mode_info_context += mis;    /* skip next mb row */
  }
  ...
  if(extra_mb_row) {
    for (mb_col = 0; mb_col < cm->mb_cols; mb_col++) {
      ...
      BLOCK_SIZE_TYPE sb_type = mi->mbmi.sb_type;
      int mode = mode_info_context->mbmi.mode;
      int mode_index = lfi_n->mode_lf_lut[mode];
      int seg = mode_info_context->mbmi.segment_id;
      int ref_frame = mode_info_context->mbmi.ref_frame;
      ...
      if (filter_level) {
        ...
        if (cm->filter_type == NORMAL_LOOPFILTER) {
          ...
          do_left_v = !((mb_col & 1) && sb_type && (tx_size >= TX_32X32 ||
                        sb_mb_lf_skip(mode_info_context, mi)));
          do_above_h = (mb_row > 0);
