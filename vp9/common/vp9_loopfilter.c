...
static double bilateral_filters_r_kf[BILATERAL_LEVELS_KF + 1][512];
static double bilateral_filters_r[BILATERAL_LEVELS + 1][512];
static double bilateral_filters_s_kf[BILATERAL_LEVELS_KF + 1][BILATERAL_WIN][BILATERAL_WIN];
static double bilateral_filters_s[BILATERAL_LEVELS + 1][BILATERAL_WIN][BILATERAL_WIN];
...
void vp9_loop_bilateral_precal() {
  ...
  for (i = 1; i < BILATERAL_LEVELS_KF + 1; i ++) {
    ...
    double *fr = bilateral_filters_r_kf[i] + 255;
    int j, x, y;
    for (j = 0; j < 256; j++) {
    ...
    for (y = -BILATERAL_HALFWIN; y <= BILATERAL_HALFWIN; y++) {
      for (x = -BILATERAL_HALFWIN; x <= BILATERAL_HALFWIN; x++) {
        bilateral_filters_s_kf[i][y + BILATERAL_HALFWIN][x + BILATERAL_HALFWIN] = exp(-(x * x) / (2 * sigma_x_d * sigma_x_d) - (y * y) / (2 * sigma_y_d * sigma_y_d));
      }
    }
  }
  ...
  for (i = 1; i < BILATERAL_LEVELS + 1; i ++) {
    ...
    double *fr = bilateral_filters_r[i] + 255;
    int j, x, y;
    for (j = 0; j < 256; j++) {
      fr[j] = exp(-(j * j) / (2 * sigma_r_d * sigma_r_d));
      fr[-j] = fr[j];
    ...
    for (y = -BILATERAL_HALFWIN; y <= BILATERAL_HALFWIN; y++) {
      for (x = -BILATERAL_HALFWIN; x <= BILATERAL_HALFWIN; x++) {
        bilateral_filters_s[i][y + BILATERAL_HALFWIN][x + BILATERAL_HALFWIN] = exp(-(x * x) / (2 * sigma_x_d * sigma_x_d) - (y * y) / (2 * sigma_y_d * sigma_y_d));
      }
    }
  }
}
...
void vp9_loop_bilateral_init(loop_filter_info_n *lfi, int level, int kf) {
  ...
  if (lfi->bilateral_used) {
    int i;
    lfi->wr_lut = kf ? bilateral_filters_r_kf[level] : bilateral_filters_r[level];
    for (i = 0; i < BILATERAL_WIN; i++)
      lfi->wx_lut[i] = kf ? bilateral_filters_s_kf[level][i] : bilateral_filters_s[level][i];
  }
}
      ...
      for (y = -BILATERAL_HALFWIN; y <= BILATERAL_HALFWIN; ++y) {
        for (x = -BILATERAL_HALFWIN; x <= BILATERAL_HALFWIN; ++x) {
          if (!is_in_image(j + x, i + y, width, height))
            continue;
          wt = lfi->wx_lut[y + BILATERAL_HALFWIN][x + BILATERAL_HALFWIN] * wr_lut_[data_p2[x] - data_p[j]];
          wtsum += wt;
          flsum += wt * data_p2[x];
        }
        data_p2 += stride;
...
void loop_bilateral_filter_highbd(uint8_t *data8, int width, int height,
      ...
      for (y = -BILATERAL_HALFWIN; y <= BILATERAL_HALFWIN; ++y) {
        for (x = -BILATERAL_HALFWIN; x <= BILATERAL_HALFWIN; ++x) {
          ...
          wt = lfi->wx_lut[y + BILATERAL_HALFWIN][x + BILATERAL_HALFWIN] * 
               wr_lut_[(data_p2[x] - data_p[j]) >> (bit_depth - 8)];
          wtsum += wt;
          flsum += wt * data_p2[x];
        }
        data_p2 += stride;
  ...
  uint16_t *data = CONVERT_TO_SHORTPTR(data8);
  uint16_t *tmpdata = CONVERT_TO_SHORTPTR(tmpdata8);
  fp = fopen("/usr/local/google/home/shunyaoli/Desktop/1.log","a+");
  for (i = 0; i < height; ++i) {
    for (j = 0; j < width; ++j) {
      int x, y;
      double wt;
      ...
      for (y = -BILATERAL_HALFWIN; y <= BILATERAL_HALFWIN; ++y) {
        for (x = -BILATERAL_HALFWIN; x <= BILATERAL_HALFWIN; ++x) {
          ...
          wt = wr_lut_[(d[0] - v[0]) >> (bit_depth - 8)] *
               lfi->wx_lut[y + BILATERAL_HALFWIN][x + BILATERAL_HALFWIN];
               //wx_lut_[y * BILATERAL_WIN + x];
         // fprintf(fp, "%d %d %d %d %f\n", i, j, x, y, lfi->wx_lut[y + BILATERAL_HALFWIN][x + BILATERAL_HALFWIN]);
          wtsum += wt;
          flsum += wt * d[0];
        }
        //printf("\n");
