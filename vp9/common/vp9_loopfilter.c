...
void vp9_loop_bilateral_init(loop_filter_info_n *lfi, int level) {
  if (level != lfi->bilateral_level_set) {
    const bilateral_params_t param = bilateral_level_to_params(level);
    const int T = param.bilateral_T;
    const int L = param.bilateral_L;
    ...
    for (j = 1; j < width - 1; j++, d++, e++, f++) {
      ...
      if (D > 16) {
        ...
        if (J >= K)
          ...
          x = ((I << lfi->bilateral_L) + (D * J)) >> lfi->bilateral_L;
        if (x > *d)
