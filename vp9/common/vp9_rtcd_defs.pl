...
if ((vpx_config("CONFIG_VP9_HIGHBITDEPTH") eq "yes") || (vpx_config("CONFIG_EMULATE_HARDWARE") eq "yes")) {
  ...
  # Also, we will need to change the names of these functions to separate the high bit-depth versions
  # with the non-high bitdepth ones.
...
if (vpx_config("CONFIG_VP9_HIGHBITDEPTH") eq "yes") {
  ...
  # dct
  #
  # Note as optimized versions of these functions are added we need to add a check to ensure that when
  # CONFIG_EMULATE_HARDWARE is on, it defaults to the C versions only
