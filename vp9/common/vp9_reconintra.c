...
int template_matching_L(VP9_COMMON *cm, MACROBLOCKD *xd, uint8_t *src,
                        ...
                        int l, int n, int *sad_l, int *sad_a) {
  // This function searches for a block in the reconstructed buffer
  // that tends to be similar to the target block located at "src".
  // The block size is "n". "l" rows above and "l" columns to the
  // left of the target block is used as the template of the target
  // block, and the search is done by minimizing the SAD between
  // this L-shaped template and all possible areas that has been
  // coded in the reconstructed buffer. "best_r" and "best_c" return
  // the row and column indices of the block selected in the recon buffer.
  int r_idx, c_idx;
  ...
  *best_r = *best_c = 0;
  ...
  // Must make sure that the pixels being compared with the template
  // have been already coded.
