...
#if CONFIG_INTERINTRA
    // In interintra prediction, D63_PRED will use yabove[2 * bw]
    // when bh = 2 * bw
    vpx_memset(yabove_row + 2 * bw, yabove_row[2 * bw -1], 1);
#endif
...
void vp9_build_interintra_predictors_sbuv(MACROBLOCKD *xd,
                                          ...
                                          int uvstride,
                                          BLOCK_SIZE_TYPE bsize) {
  const int bwl =b_width_log2(bsize) - xd->plane[1].subsampling_x;
  const int bhl =b_height_log2(bsize) - xd->plane[1].subsampling_y;
  int bw = 4 << bwl;
  int bh = 4 << bhl;
