...
#define intra_pred_high_sized(type, size) \
  void vp9_high_##type##_predictor_##size##x##size##_c(uint16_t *dst, \
                                                  ptrdiff_t stride, \
...
static INLINE void high_d117_predictor(uint16_t *dst, ptrdiff_t stride, int bs,
                                  const uint16_t *above, const uint16_t *left, int bps) {
  int r, c;
...
static INLINE void high_dc_128_predictor(uint16_t *dst, ptrdiff_t stride, int bs,
                                    const uint16_t *above, const uint16_t *left, int bps) {
  int r;
...
static INLINE void high_dc_left_predictor(uint16_t *dst, ptrdiff_t stride, int bs,
                                     const uint16_t *above,
                                     const uint16_t *left, int bps) {
...
static INLINE void high_dc_top_predictor(uint16_t *dst, ptrdiff_t stride, int bs,
                                    const uint16_t *above, const uint16_t *left, int bps) {
  int i, r, expected_dc, sum = 0;
...
static INLINE void high_dc_predictor(uint16_t *dst, ptrdiff_t stride, int bs,
                                const uint16_t *above, const uint16_t *left, int bps) {
  int i, r, expected_dc, sum = 0;
...
static void build_intra_predictors_high(const MACROBLOCKD *xd, const uint8_t *ref8,
  ...
  if (left_available) {
    ...
    vpx_memset16(left_col, base+1, bs); // TODO this value should probably change for high bitdepth
  }
  ...
  if (up_available) {
    ...
    if (xd->mb_to_right_edge < 0) {
      ...
      above_row[-1] = left_available ? above_ref[-1] : (base+1); // TODO this value should probably change for high bitdepth
    } else {
      /* faster path if the block does not need extension */
    ...
    above_row[-1] = base-1; // TODO this value should probably change for high bitdepth
  }

  // predict
  if (mode == DC_PRED) {
    dc_pred_high[left_available][up_available][tx_size](dst, dst_stride,
                                                   const_above_row, left_col, xd->bps);
  } else {
    pred_high[mode][tx_size](dst, dst_stride, const_above_row, left_col, xd->bps);
  }
}
#endif
...
void vp9_predict_intra_block(const MACROBLOCKD *xd, int block_idx, int bwl_in,
  ...
  if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
    build_intra_predictors_high(xd, ref, ref_stride, dst, dst_stride, mode, tx_size,
                         have_top, have_left, have_right, x, y, plane, xd->bps);
    return;
  }
#endif
