...
void vp9_idct32x32_135_add_sse2(const int16_t *input, uint8_t *dest,
                                 int stride) {
  const __m128i rounding = _mm_set1_epi32(DCT_CONST_ROUNDING);
  const __m128i final_rounding = _mm_set1_epi16(1 << 5);
  const __m128i zero = _mm_setzero_si128();
  ...
  __m128i in[32], col[128], zero_idx[8];
  __m128i stp1_0, stp1_1, stp1_2, stp1_3, stp1_4, stp1_5, stp1_6, stp1_7,
          stp1_8, stp1_9, stp1_10, stp1_11, stp1_12, stp1_13, stp1_14, stp1_15,
          stp1_16, stp1_17, stp1_18, stp1_19, stp1_20, stp1_21, stp1_22,
          stp1_23, stp1_24, stp1_25, stp1_26, stp1_27, stp1_28, stp1_29,
  ...
  // Only need to process the top left 16x16 block in the first pass.
  for (i = 0; i < 2; i++) {
    i32 = (i << 5);
    // First 1-D idct
    // Load input data.
    ...
    zero_idx[0] = _mm_or_si128(zero_idx[0], zero_idx[1]);
    zero_idx[2] = _mm_or_si128(zero_idx[2], zero_idx[3]);
    zero_idx[0] = _mm_or_si128(zero_idx[0], zero_idx[2]);

    if (_mm_movemask_epi8(_mm_cmpeq_epi32(zero_idx[0], zero)) == 0xFFFF) {
