...
static void highbd_mb_lpf_horizontal_edge_w_sse2_8(uint16_t *s,
  ...
  if (bd == 8)
    filt = _mm_and_si128(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(ps1, qs1), 8), hev);
  else if (bd == 10)
    filt = _mm_and_si128(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(ps1, qs1), 10), hev);
  else  // bd == 12
    filt = _mm_and_si128(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(ps1, qs1), 12), hev);
  ...
  if (bd == 8)
    filt = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, work_a), 8);
  else if (bd == 10)
    filt = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, work_a), 10);
  else  // bd == 12
    filt = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, work_a), 12);

  filt = _mm_and_si128(filt, mask);

  if (bd == 8) {
    filter1 = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, t4), 8);
    filter2 = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, t3), 8);
  } else if (bd == 10) {
    filter1 = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, t4), 10);
    filter2 = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, t3), 10);
  } else {  // bd == 12
    filter1 = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, t4), 12);
    filter2 = signed_char_clamp_bd_sse2(_mm_adds_epi16(filt, t3), 12);
  }
  ...
  if (bd == 8) {
    qs0 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(qs0, filter1), 8),
        t80);
    ps0 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_adds_epi16(ps0, filter2), 8),
        t80);
  } else if (bd == 10) {
    qs0 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(qs0, filter1), 10),
        t80);
    ps0 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_adds_epi16(ps0, filter2), 10),
        t80);
  } else {  // bd == 12
    qs0 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(qs0, filter1), 12),
        t80);
    ps0 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_adds_epi16(ps0, filter2), 12),
        t80);
  }
  ...
  if (bd == 8) {
    qs1 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(qs1, filt), 8),
        t80);
    ps1 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_adds_epi16(ps1, filt), 8),
        t80);
  } else if (bd == 10) {
    qs1 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(qs1, filt), 10),
        t80);
    ps1 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_adds_epi16(ps1, filt), 10),
        t80);
  } else {  // bd == 12
    qs1 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_subs_epi16(qs1, filt), 12),
        t80);
    ps1 = _mm_adds_epi16(
        signed_char_clamp_bd_sse2(_mm_adds_epi16(ps1, filt), 12),
        t80);
  }
  // end highbd_filter4
  // loopfilter done
