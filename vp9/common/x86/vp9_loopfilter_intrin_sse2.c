...
static INLINE __m128i _mm_abs_diff(__m128i a, __m128i b) {
...
static void mb_lpf_horizontal_edge_w_sse2_8(unsigned char *s,
...

  {
    ...
    abs_p1p0 = _mm_abs_diff(q1p1, q0p0);
    abs_q1q0 =  _mm_srli_si128(abs_p1p0, 8);
    fe = _mm_set1_epi8(0xfe);
    ff = _mm_cmpeq_epi8(abs_p1p0, abs_p1p0);
    abs_p0q0 = _mm_abs_diff(q0p0, p0q0);
    abs_p1q1 = _mm_abs_diff(q1p1, p1q1);
    ...
    work = _mm_max_epu8(_mm_abs_diff(q2p2, q1p1),
                        _mm_abs_diff(q3p3, q2p2));
  ...
  // lp filter
  {
    ...
    {
      __m128i work;
      flat = _mm_max_epu8(_mm_abs_diff(q2p2, q0p0),
                          _mm_abs_diff(q3p3, q0p0));
      ...
      flat2 = _mm_max_epu8(_mm_abs_diff(q4p4, q0p0),
                           _mm_abs_diff(q5p5, q0p0));

      q7p7 = _mm_loadl_epi64((__m128i *)(s - 8 * p));
      q7p7 = _mm_castps_si128(_mm_loadh_pi(_mm_castsi128_ps(q7p7),
                                           (__m64 *)(s + 7 * p)));

      work = _mm_max_epu8(_mm_abs_diff(q6p6, q0p0),
                          _mm_abs_diff(q7p7, q0p0));
...
static void mb_lpf_horizontal_edge_w_sse2_16(unsigned char *s,
  ...
  {
    const __m128i abs_p1p0 = _mm_abs_diff(p1, p0);
    const __m128i abs_q1q0 = _mm_abs_diff(q1, q0);
    const __m128i fe = _mm_set1_epi8(0xfe);
    const __m128i ff = _mm_cmpeq_epi8(zero, zero);
    __m128i abs_p0q0 = _mm_abs_diff(p0, q0);
    __m128i abs_p1q1 = _mm_abs_diff(p1, q1);
    __m128i work;
    ...
    // mask |= (abs(p1 - p0) > limit) * -1;
    // mask |= (abs(q1 - q0) > limit) * -1;
    work = _mm_max_epu8(_mm_abs_diff(p2, p1),
                        _mm_abs_diff(p3, p2));
    mask = _mm_max_epu8(work, mask);
    work = _mm_max_epu8(_mm_abs_diff(q2, q1),
                        _mm_abs_diff(q3, q2));
    mask = _mm_max_epu8(work, mask);
    mask = _mm_subs_epu8(mask, limit);
    mask = _mm_cmpeq_epi8(mask, zero);
  ...
  {
    __m128i work;
    work = _mm_max_epu8(_mm_abs_diff(p2, p0),
                        _mm_abs_diff(q2, q0));
    flat = _mm_max_epu8(work, max_abs_p1p0q1q0);
    work = _mm_max_epu8(_mm_abs_diff(p3, p0),
                        _mm_abs_diff(q3, q0));
    flat = _mm_max_epu8(work, flat);
    work = _mm_max_epu8(_mm_abs_diff(p4, p0),
                        _mm_abs_diff(q4, q0));
    flat = _mm_subs_epu8(flat, one);
    flat = _mm_cmpeq_epi8(flat, zero);
    flat = _mm_and_si128(flat, mask);

    flat2 = _mm_max_epu8(_mm_abs_diff(p5, p0),
                         _mm_abs_diff(q5, q0));
    flat2 = _mm_max_epu8(work, flat2);
    work = _mm_max_epu8(_mm_abs_diff(p6, p0),
                        _mm_abs_diff(q6, q0));
    flat2 = _mm_max_epu8(work, flat2);

    work = _mm_max_epu8(_mm_abs_diff(p7, p0),
                        _mm_abs_diff(q7, q0));
    flat2 = _mm_max_epu8(work, flat2);
    flat2 = _mm_subs_epu8(flat2, one);
    flat2 = _mm_cmpeq_epi8(flat2, zero);
