  ...
  for (i = 0; (i < MVREF_NEIGHBOURS + 1) &&
    ...
    if (proposed_ref_0 == ref_frame) {
      ...
      if (candidate->mbmi.sb_type < BLOCK_SIZE_SB8X8 && i < 2
        ...
        proposed_mv_0.as_int = candidate->bmi[b].as_mv[0].as_int;
      }

      if (!same_counter || proposed_mv_0.as_int != mv_ref_list[0].as_int) {
        mv_ref_list[same_counter++].as_int = proposed_mv_0.as_int;
      }

      proposed_mv_0.as_int = x;
    } else {
      if (proposed_ref_0 > INTRA_FRAME) {
        scale_mv(xd, ref_frame, proposed_ref_0, &proposed_mv_0, ref_sign_bias);

        mvs_from_diff_ref[diff_counter++].as_int = proposed_mv_0.as_int;
      }
    }

    if (proposed_ref_1 == ref_frame && proposed_ref_0 != ref_frame) {
      // if we are less than 8x8 and close to the current block
      if (candidate->mbmi.sb_type < BLOCK_SIZE_SB8X8 && i < 2
          && block_idx >= 0) {
        b = block_idx_to_mv_ref(mv_ref_search[i][0], block_idx);
      ...
      if (proposed_ref_1 > INTRA_FRAME && proposed_ref_1 != ref_frame &&
          proposed_mv_1.as_int != proposed_mv_0.as_int) {
        scale_mv(xd, ref_frame, proposed_ref_1, &proposed_mv_1, ref_sign_bias);
        mvs_from_diff_ref[diff_counter++].as_int = proposed_mv_1.as_int;
      }
    }
  }
