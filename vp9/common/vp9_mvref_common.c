...
static void find_mv_refs_idx(const VP9_COMMON *cm, const MACROBLOCKD *xd,
  ...
  const MODE_INFO *prev_mi = !cm->error_resilient_mode && cm->prev_mi
        ...
        : NULL;
  const MB_MODE_INFO *const prev_mbmi = prev_mi ? &prev_mi->target->mbmi : NULL;
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (is_inside(tile, mi_col, mi_row, cm->mi_rows, mv_ref)) {
      const MODE_INFO *const candidate_mi = xd->mi_test[mv_ref->col + mv_ref->row *
                                                   xd->mi_stride].target;
  ...
  for (; i < MVREF_NEIGHBOURS; ++i) {
    const POSITION *const mv_ref = &mv_ref_search[i];
    if (is_inside(tile, mi_col, mi_row, cm->mi_rows, mv_ref)) {
      const MB_MODE_INFO *const candidate = &xd->mi_test[mv_ref->col + mv_ref->row *
                                                    xd->mi_stride].target->mbmi;
      different_ref_found = 1;
