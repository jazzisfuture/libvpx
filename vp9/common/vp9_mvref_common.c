...
void vp9_find_mv_refs_idx(VP9_COMMON *cm, MACROBLOCKD *xd, MODE_INFO *here,
  ...
  for (i = 0; i < MVREF_NEIGHBOURS; ++i) {
    ...
    if (candidate->mbmi.ref_frame[0] != ref_frame) {
      which_mv = candidate->mbmi.mv[0];
      // We need to account for sign bias ( mvs from future have
      // a different sign than mvs from past).
      scale_mv(xd, ref_frame, candidate->mbmi.ref_frame[0], &which_mv,
               ref_sign_bias);
      add_mv_ref_list(which_mv);
    }
    ...
    if (candidate->mbmi.ref_frame[1] != ref_frame &&
      ...
      which_mv = candidate->mbmi.mv[1];
      // If the mv is different than the one in our first predictor
      // ( THIS CONDITION IS A BIT ODD - because the sign bias could
      // mean these are 2 different motion vectors but we ignore this
      // anyway ..
      if (candidate->mbmi.mv[1].as_int != candidate->mbmi.mv[0].as_int) {
        // We need to account for sign bias ( mvs from future have
        // a different sign than mvs from past).
        scale_mv(xd, ref_frame, candidate->mbmi.ref_frame[1], &which_mv,
                 ref_sign_bias);
        add_mv_ref_list(which_mv);
      }
    }
