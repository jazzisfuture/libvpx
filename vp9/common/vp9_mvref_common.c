...
void find_opt_ref_mvs_idx(const VP9_COMMON *cm,
  ...
  uint8_t *above_src = NULL;
  uint8_t *left_src = NULL;
  uint8_t *above_ref = NULL;
  uint8_t *left_ref = NULL;
  int is_top_most = (mi_row == 0);
  int is_left_most = (mi_col == 0);

  if (is_top_most && is_left_most) {
    for (i = 0; i < 2; ++i)
  ...
  for(i = 0; i < ref_mvs_counter[0]; ++i) {
    ...
    row_offset = (mv->row > 0) ? ((mv->row + 3) >> 3) : ((mv->row + 4) >> 3);
    col_offset = (mv->col > 0) ? ((mv->col + 3) >> 3) : ((mv->col + 4) >> 3);
    ...
    if (i == 0) {
      opt_sad[0] = sad;
      mv_ref_list[0].as_int = this_mv->as_int;
    } else {
      if (sad < opt_sad[0]) {
        opt_sad[1] = opt_sad[0];
        ...
        mv_ref_list[0].as_int = this_mv->as_int;
                 ...
                 sad < opt_sad[1]) {
        opt_sad[1] = sad;
        mv_ref_list[1].as_int = this_mv->as_int;
      }
    }
  }
}
...
void vp9_find_opt_ref_mvs(const VP9_COMMON *cm,
  ...
  if (mi_row == 0 && mi_col == 0) {
    ...
    find_opt_ref_mvs_idx(cm, xd, mi, tile, mi_row, mi_col, ref_frame,
                         yv12_mb_ref, mv_ref_list);
  }
}
