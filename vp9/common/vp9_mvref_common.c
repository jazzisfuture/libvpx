...
#define scale_and_add(CANDIDATE, MVI) \
  which_mv = CANDIDATE->mbmi.mv[MVI]; \
  scale_mv(xd, ref_frame, CANDIDATE->mbmi.ref_frame[MVI], &which_mv, \
           ref_sign_bias); \
  add_mv_ref_list(which_mv);

// This macro returns either the appropriate sub block or block's mv depending
// on if the block_size < 8x8 and if we have check_sub_blocks set.
#define sub_block_or_block_mv(candidate, RF) \
    (check_sub_blocks && candidate->mbmi.sb_type < BLOCK_SIZE_SB8X8 ? \
        candidate->bmi[block_idx_to_mv_ref(mv_ref_search[i][0], block_idx)]\
            .as_mv[RF] : \
        candidate->mbmi.mv[RF])

// Check if the candidate has a mv that comes from the same reference frame
...
void vp9_find_mv_refs_idx(VP9_COMMON *cm, MACROBLOCKD *xd, MODE_INFO *here,
  ...
  for (i = 0; i < 2 ; i++) {
    ...
    if (candidate->mbmi.ref_frame[0] == ref_frame) {
      add_mv_ref_list(sub_block_or_block_mv(candidate, 0));
    } else if (candidate->mbmi.ref_frame[1] == ref_frame) {
      // Add second motion vector if it has the same ref_frame.
      add_mv_ref_list(sub_block_or_block_mv(candidate, 1));
    }
  }
