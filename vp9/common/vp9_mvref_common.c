...
void vp9_find_mv_refs_idx(VP9_COMMON *cm, MACROBLOCKD *xd, MODE_INFO *here,
  ...
  for (i = 0; (i < MVREF_NEIGHBOURS + 1) &&
    ...
    proposed_mv_1.as_int = candidate_mi->mbmi.mv[1].as_int;
    ...
    if (proposed_ref_0 == ref_frame) {
      ...
      if (candidate_mi->mbmi.sb_type < BLOCK_SIZE_SB8X8
        ...
        if (mv_ref_search[i][0])
          ...
          b = 2 + x_idx;
        proposed_mv_0.as_int = candidate_mi->bmi[b].as_mv[0].as_int;
      }

      if (!same_counter  || (same_counter
          && proposed_mv_0.as_int
              != mv_ref_list[same_counter].as_int)) {
        mv_ref_list[0].as_int = proposed_mv_0.as_int;
        candidate_scores[same_counter++] = 16;
      }
    } else {
      ...
      if (proposed_ref_1 == ref_frame) {
        ...
        if (candidate_mi->mbmi.sb_type < BLOCK_SIZE_SB8X8
          ...
          if (mv_ref_search[i][0])
            ...
            b = 2 + x_idx;
          proposed_mv_1.as_int = candidate_mi->bmi[b].as_mv[1].as_int;
        }
        if (!same_counter || (same_counter
            && proposed_mv_1.as_int != mv_ref_list[0].as_int)) {
          mv_ref_list[same_counter].as_int = proposed_mv_1
              .as_int;
          candidate_scores[same_counter++] = 16;
        }
      } else {
  ...
  for (i=0;same_counter < MAX_MV_REF_CANDIDATES && i < diff_counter;++i) {
    if (!same_counter || (same_counter
        && nearby_mvs_with_diff_frame[i].as_int
            != mv_ref_list[0].as_int)) {
      mv_ref_list[same_counter].as_int =
          nearby_mvs_with_diff_frame[i].as_int;
      candidate_scores[same_counter] = 1;
      ++same_counter;
    }
  }
