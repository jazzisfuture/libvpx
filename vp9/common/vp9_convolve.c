...
static void high_convolve_horiz(const uint8_t *src8, ptrdiff_t src_stride,
                           ...
                           const InterpKernel *x_filters,
                           int x0_q4, int x_step_q4, int w, int h, int bps) {
  int x, y;
...
static void high_convolve_vert(const uint8_t *src8, ptrdiff_t src_stride,
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      ...
      for (k = 0; k < SUBPEL_TAPS; ++k)
        sum += src_y[k * src_stride] * y_filter[k];
      dst[y * dst_stride] = clip_pixel_bps(ROUND_POWER_OF_TWO(sum, FILTER_BITS),bps);
      y_q4 += y_step_q4;
    }
    ++src;
    ++dst;
...
static void high_convolve(const uint8_t *src, ptrdiff_t src_stride,
  ...
  high_convolve_horiz(src - src_stride * (SUBPEL_TAPS / 2 - 1), src_stride, CONVERT_TO_BYTEPTR(temp), 64,
                 x_filters, x0_q4, x_step_q4, w, intermediate_height, bps);
  high_convolve_vert(CONVERT_TO_BYTEPTR(temp) + 64 * (SUBPEL_TAPS / 2 - 1), 64, dst, dst_stride,
                y_filters, y0_q4, y_step_q4, w, h, bps);
}
...
void vp9_high_convolve8_avg_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  vp9_high_convolve_avg_c(CONVERT_TO_BYTEPTR(temp), 64, dst, dst_stride, NULL, 0, NULL, 0, w, h, bps);
}
