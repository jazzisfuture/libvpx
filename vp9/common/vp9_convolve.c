...
static void convolve_qtr_horiz_c(const uint8_t *src, int src_stride,
                                 uint8_t *dst, int dst_stride,
                                 const int16_t *filter_x0, int x_step_q4,
                                 const int16_t *filter_y, int y_step_q4,
                                 int w, int h, int taps) {
  ...
  for (y = 0; y < h; ++y) {
    ...
    for (x = 0; x < w; ++x) {
      ...
      sum += (VP9_FILTER_WEIGHT >> 1);
      dst[x] = (dst[x] + 3 * clip_pixel(sum >> VP9_FILTER_SHIFT) + 2) >> 2;
...
static void convolve_3qtr_horiz_c(const uint8_t *src, int src_stride,
                                  uint8_t *dst, int dst_stride,
                                  const int16_t *filter_x0, int x_step_q4,
                                  const int16_t *filter_y, int y_step_q4,
                                  int w, int h, int taps) {
  ...
  for (y = 0; y < h; ++y) {
    ...
    for (x = 0; x < w; ++x) {
      ...
      sum += (VP9_FILTER_WEIGHT >> 1);
      dst[x] = (3 * dst[x] + clip_pixel(sum >> VP9_FILTER_SHIFT) + 2) >> 2;
...
#if CONFIG_IMPLICIT_COMPOUNDINTER_WEIGHT
static void convolve_qtr_vert_c(const uint8_t *src, int src_stride,
                                uint8_t *dst, int dst_stride,
                                const int16_t *filter_x, int x_step_q4,
                                const int16_t *filter_y0, int y_step_q4,
                                int w, int h, int taps) {
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      ...
      sum += (VP9_FILTER_WEIGHT >> 1);
      dst[y * dst_stride] =
          (dst[y * dst_stride] + 3 * clip_pixel(sum >> VP9_FILTER_SHIFT) + 2)
          >> 2;
...
static void convolve_3qtr_vert_c(const uint8_t *src, int src_stride,
                                 uint8_t *dst, int dst_stride,
                                 const int16_t *filter_x, int x_step_q4,
                                 const int16_t *filter_y0, int y_step_q4,
                                 int w, int h, int taps) {
  ...
  for (x = 0; x < w; ++x) {
    ...
    for (y = 0; y < h; ++y) {
      ...
      sum += (VP9_FILTER_WEIGHT >> 1);
      dst[y * dst_stride] =
          (3 * dst[y * dst_stride] + clip_pixel(sum >> VP9_FILTER_SHIFT) + 2)
          >> 2;
...
#if CONFIG_IMPLICIT_COMPOUNDINTER_WEIGHT
static void convolve_qtr_c(const uint8_t *src, int src_stride,
                           uint8_t *dst, int dst_stride,
                           const int16_t *filter_x, int x_step_q4,
                           const int16_t *filter_y, int y_step_q4,
                           int w, int h, int taps) {
  ...
  convolve_horiz_c(src - src_stride * (taps / 2 - 1), src_stride,
                   ...
                   filter_x, x_step_q4, filter_y, y_step_q4,
                   w, intermediate_height, taps);
  convolve_qtr_vert_c(temp + 16 * (taps / 2 - 1), 16, dst, dst_stride,
                      filter_x, x_step_q4, filter_y, y_step_q4,
                      w, h, taps);
}

static void convolve_3qtr_c(const uint8_t *src, int src_stride,
                            uint8_t *dst, int dst_stride,
                            const int16_t *filter_x, int x_step_q4,
                            const int16_t *filter_y, int y_step_q4,
                            int w, int h, int taps) {
  ...
  convolve_horiz_c(src - src_stride * (taps / 2 - 1), src_stride,
                   ...
                   filter_x, x_step_q4, filter_y, y_step_q4,
                   w, intermediate_height, taps);
  convolve_3qtr_vert_c(temp + 16 * (taps / 2 - 1), 16, dst, dst_stride,
                       filter_x, x_step_q4, filter_y, y_step_q4,
                       w, h, taps);
}
#endif
...
void vp9_convolve8_qtr_horiz_c(const uint8_t *src, int src_stride,
                               ...
                               const int16_t *filter_y, int y_step_q4,
                               int w, int h) {
  convolve_qtr_horiz_c(src, src_stride, dst, dst_stride,
                       filter_x, x_step_q4, filter_y, y_step_q4,
                       w, h, 8);
}
...
void vp9_convolve8_3qtr_horiz_c(const uint8_t *src, int src_stride,
                                ...
                                const int16_t *filter_y, int y_step_q4,
                                int w, int h) {
  convolve_3qtr_horiz_c(src, src_stride, dst, dst_stride,
                        filter_x, x_step_q4, filter_y, y_step_q4,
                        w, h, 8);
}
#endif
...
void vp9_convolve8_qtr_vert_c(const uint8_t *src, int src_stride,
                              ...
                              const int16_t *filter_y, int y_step_q4,
                              int w, int h) {
  convolve_qtr_vert_c(src, src_stride, dst, dst_stride,
                      filter_x, x_step_q4, filter_y, y_step_q4,
                      w, h, 8);
}
...
void vp9_convolve8_3qtr_vert_c(const uint8_t *src, int src_stride,
                               ...
                               const int16_t *filter_y, int y_step_q4,
                               int w, int h) {
  convolve_3qtr_vert_c(src, src_stride, dst, dst_stride,
                       filter_x, x_step_q4, filter_y, y_step_q4,
                       w, h, 8);
}
#endif
