...
static void loop_filter_rows_mt(YV12_BUFFER_CONFIG *frame_buffer,
                                VP9_COMMON *cm,
                                struct macroblockd_plane planes[MAX_MB_PLANE],
                                int start, int stop, int y_only,
                                VP9Worker *workers, int n_workers,
                                VP9LfSync *lf_sync) {
  ...
  const int num_workers = MIN(n_workers, tile_cols);
  int i;
...
void vp9_loop_filter_frame_mt(YV12_BUFFER_CONFIG *frame,
  ...
  if (num_workers > 1)
    ...
    vp9_loop_filter_rows(frame, cm, planes, start_mi_row, end_mi_row,
                         y_only);
}
...
#endif  // CONFIG_MULTITHREAD
    if (lf_sync->lfdata != NULL)
      vpx_free(lf_sync->lfdata);
    if (lf_sync->cur_sb_col != NULL)
