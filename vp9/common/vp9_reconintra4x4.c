...
void vp9_intra4x4_predict(MACROBLOCKD *xd,
  ...
  if (have_top) {
    ...
    above[2] = above_ptr[2];
    above[3] = above_ptr[3];
    // FIXME can this be simplified? See also vp9_intra_prediction_down_copy()
    if (have_right &&
        !(block_idx == 7 || block_idx == 11 || block_idx == 15) &&
...
void vp9_intra_prediction_down_copy(MACROBLOCKD *xd) {
  ...
  return;  // FIXME(rbultje): remove this function, the intra predictors
           // now do this internally
