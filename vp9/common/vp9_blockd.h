...
static void set_contexts_on_border(MACROBLOCKD *xd,
    struct macroblockd_plane *pd, BLOCK_SIZE_TYPE plane_bsize,
    int tx_size_in_blocks, int has_eob,
    int aoff, int loff, ENTROPY_CONTEXT *A, ENTROPY_CONTEXT *L) {
...
static void set_contexts(MACROBLOCKD *xd, struct macroblockd_plane *pd,
  ...
  if (xd->mb_to_right_edge < 0 || xd->mb_to_bottom_edge < 0) {
    set_contexts_on_border(xd, pd, plane_bsize, tx_size_in_blocks, has_eob, aoff, loff, A, L);
  } else {
