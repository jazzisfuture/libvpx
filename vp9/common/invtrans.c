...
void vp9_inverse_transform_b_4x4(BLOCKD *b, int pitch) {
  if (b->eob <= 1)
    vp9_short_idct4x4llm_1(b->dqcoeff, b->diff, pitch);
  else
    vp9_short_idct4x4llm(b->dqcoeff, b->diff, pitch);
}
...
void vp9_inverse_transform_mby_4x4(MACROBLOCKD *xd) {
  ...
  for (i = 0; i < 16; i++) {
    vp9_inverse_transform_b_4x4(&blockd[i], 32);
  }
}
...
void vp9_inverse_transform_mbuv_4x4(MACROBLOCKD *xd) {
  ...
  for (i = 16; i < 24; i++) {
    vp9_inverse_transform_b_4x4(&blockd[i], 16);
  }
}
