...
static void print_mi_data(VP9_COMMON *common, FILE *file, char *descriptor,
  ...
  int mi_col;
  int mi_index = 0;
  MODE_INFO_8x8 *mi_8x8 = common->mi_grid_visible;
  ...
  for (mi_row = 0; mi_row < rows; mi_row++) {
    ...
    for (mi_col = 0; mi_col < cols; mi_col++) {
      fprintf(file, "%2d ",
              *((int*) ((char *) (&mi_8x8[mi_index].mi->mbmi) + member_offset)));
      mi_index++;
    }
    fprintf(file, "\n");
    mi_index += 8;
...
void vp9_print_modes_and_motion_vectors(VP9_COMMON *cm, char *file) {
  ...
  MODE_INFO_8x8 *mi_8x8 = cm->mi_grid_visible;
  int rows = cm->mi_rows;
  int cols = cm->mi_cols;
  ...
  for (mi_row = 0; mi_row < rows; mi_row++) {
    fprintf(mvs,"V ");
    for (mi_col = 0; mi_col < cols; mi_col++) {
      fprintf(mvs, "%4d:%4d ", mi_8x8[mi_index].mi->mbmi.mv[0].as_mv.row,
          mi_8x8[mi_index].mi->mbmi.mv[0].as_mv.col);
      mi_index++;
    }
    fprintf(mvs, "\n");
    mi_index += 8;
