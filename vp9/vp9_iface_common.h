...
static void yuvconfig2image(vpx_image_t *img, const YV12_BUFFER_CONFIG  *yv12,
  ...
  if (yv12->flags & YV12_FLAG_HIGH) {
    ...
    img->planes[VPX_PLANE_ALPHA] = (uint8_t*)CONVERT_TO_SHORTPTR(yv12->alpha_buffer);
...
static vpx_codec_err_t image2yuvconfig(const vpx_image_t *img,
  ...
  if (img->fmt & VPX_IMG_FMT_HIGH) {
    ...
    // 
    // This means that all the address calculations in the main body of code should work correctly
    // However, before we do any pixel operations we need to cast the address to a uint16 ponter and double its value
