...
void VP9RateControlRTC::ComputeQP(const FrameParamsQPRTC &frame_params) {
  ...
  cm->common.show_frame = 1;
  cm->common.frame_type = frame_params.frame_type;
  cpi_->refresh_golden_frame = (cm->common.frame_type == KEY_FRAME) ? 1 : 0;
  cpi_->sf.use_nonrd_pick_mode = 1;
...
void VP9RateControlRTC::PostEncode(const FrameParamsQPRTC &frame_params) {
  cpi_->rc.compute_frame_motion_pass0 = 0;
  vp9_rc_postencode_update(cpi_, frame_params.encoded_frame_size);
  if (cpi_->svc.number_spatial_layers > 1 ||
      cpi_->svc.number_temporal_layers > 1)
