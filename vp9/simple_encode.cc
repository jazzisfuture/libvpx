...
static bool init_encode_frame_result(EncodeFrameResult *encode_frame_result,
  ...
  const bool allocated = init_image_buffer(&encode_frame_result->coded_frame,
                                           frame_width, frame_height, img_fmt);
  if (!allocated) {
    return false;
  }
  return true;
...
void SimpleEncode::StartEncode() {
  ...
  if (out_file_) {
    ...
    ivf_write_file_header_simple(out_file_, *(const uint32_t *)fourcc,
                                 num_frames_, frame_width_, frame_height_,
                                 time_base);
  }
}
...
SimpleEncode::~SimpleEncode() {
  if (this->in_file_ != NULL) {
    fclose(this->in_file_);
  }
  if (out_file_ != NULL) {
    fclose(out_file_);
  }
