...
#if CONFIG_TX64X64
      case TX_64X64:
        tx_identity(src_diff, coeff, diff_stride, 64);
...
#endif  // CONFIG_TX64X64
      case TX_32X32:
        tx_identity(src_diff, coeff, diff_stride, 32);
        ...
        break;
      case TX_16X16:
        tx_identity(src_diff, coeff, diff_stride, 16);
        ...
        break;
      case TX_8X8:
        tx_identity(src_diff, coeff, diff_stride, 8);
        ...
        break;
      case TX_4X4:;
      tx_identity(src_diff, coeff, diff_stride, 4);
...
#if CONFIG_TX64X64
      case TX_64X64:
        tx_identity_add(dqcoeff, dst, pd->dst.stride, 64);
        break;
#endif // CONFIG_TX64X64
      case TX_32X32:
        tx_identity_add(dqcoeff, dst, pd->dst.stride, 32);
        break;
      case TX_16X16:
        tx_identity_add(dqcoeff, dst, pd->dst.stride, 16);
        break;
      case TX_8X8:
        tx_identity_add(dqcoeff, dst, pd->dst.stride, 8);
        break;
      case TX_4X4:
        tx_identity_add(dqcoeff, dst, pd->dst.stride, 4);
        break;
      default:
        assert(0 && "Invalid transform size");
        break;
...
#endif
#if CONFIG_TX_SKIP
  xd->mi[0].src_mi->mbmi.tx_skip = 0;
  xd->mi[0].src_mi->mbmi.tx_skip_uv = 0;
#endif
  txfrm_block_to_raster_xy(plane_bsize, tx_size, block, &i, &j);
  dst = &pd->dst.buf[4 * j * pd->dst.stride + 4 * i];
      ...
      if ((mbmi->tx_skip && !plane) || (mbmi->tx_skip_uv && plane)) {
        switch (tx_size) {
          ...
          case TX_64X64:
            ...
            if (!x->skip_encode && *eob)
              tx_identity_add(dqcoeff, dst, dst_stride, 64);
            break;
#endif  // CONFIG_TX64X64
          case TX_32X32:
            scan_order = &vp9_default_scan_orders[TX_32X32];
            ...
            if (!x->skip_encode && *eob) {
              tx_identity_add(dqcoeff, dst, dst_stride, 32);
            }
            break;
          case TX_16X16:
            tx_type = get_tx_type(pd->plane_type, xd);
            ...
            if (!x->skip_encode && *eob) {
              tx_identity_add(dqcoeff, dst, dst_stride, 16);
            }
            break;
          case TX_8X8:
            tx_type = get_tx_type(pd->plane_type, xd);
            ...
            if (!x->skip_encode && *eob) {
              tx_identity_add(dqcoeff, dst, dst_stride, 8);
            }
            break;
          case TX_4X4:
            tx_type = get_tx_type_4x4(pd->plane_type, xd, block);
            ...
            if (!x->skip_encode && *eob) {
              tx_identity_add(dqcoeff, dst, dst_stride, 4);
            }
            break;
          default:
            assert(0);
