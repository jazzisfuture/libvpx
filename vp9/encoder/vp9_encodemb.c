...
void vp9_xform_quant_fp(MACROBLOCK *x, int plane, int block,
  ...
  int shift = xd->lossless ? TX_SKIP_SHIFT_LOSSLESS : TX_SKIP_SHIFT_LOSSY;
#endif
  ...
  if (mbmi->tx_skip[plane != 0]) {
    switch (tx_size) {
#if CONFIG_TX64X64
      case TX_64X64:
        vp9_tx_identity(src_diff, coeff, 64, 64, diff_stride, 64, shift);
      ...
      case TX_32X32:
        vp9_tx_identity(src_diff, coeff, 32, 32, diff_stride, 32, shift);
        ...
        break;
      case TX_16X16:
        vp9_tx_identity(src_diff, coeff, 16, 16, diff_stride, 16, shift);
        ...
        break;
      case TX_8X8:
        vp9_tx_identity(src_diff, coeff, 8, 8, diff_stride, 8, shift);
        ...
        break;
      case TX_4X4:
        vp9_tx_identity(src_diff, coeff, 4, 4, diff_stride, 4, shift);
...
#if CONFIG_TX64X64
      case TX_64X64:
        vp9_tx_identity_add(dqcoeff, dst, 64, 64, 64, pd->dst.stride, shift);
        break;
#endif  // CONFIG_TX64X64
      case TX_32X32:
        vp9_tx_identity_add(dqcoeff, dst, 32, 32, 32, pd->dst.stride, shift);
        break;
      case TX_16X16:
        vp9_tx_identity_add(dqcoeff, dst, 16, 16, 16, pd->dst.stride, shift);
        break;
      case TX_8X8:
        vp9_tx_identity_add(dqcoeff, dst, 8, 8, 8, pd->dst.stride, shift);
        break;
      case TX_4X4:
        vp9_tx_identity_add(dqcoeff, dst, 4, 4, 4, pd->dst.stride, shift);
        break;
      default:
...
static int vp9_dpcm_intra(uint8_t *src, int src_stride,
                          ...
                          tran_low_t *dqcoeff, int skip_block,
                          struct macroblock_plane *p,
                          struct macroblockd_plane *pd,
                          const scan_order *scan_order,
                          PREDICTION_MODE mode, int bs,
                          int shift, int logsizeby32) {
  int i, j, eob;

  switch (mode) {
    case H_PRED:
      for (i = 0 ; i < bs; i++) {
        vp9_subtract_block_c(bs, 1, src_diff + i,
                             diff_stride,
                             src + i, src_stride,
                             dst + i, dst_stride);
        vp9_tx_identity(src_diff + i,
                        coeff + i, bs, 1,
                        diff_stride, bs, shift);
        quantize_slice_bigtx(coeff + i, bs, skip_block,
                           p->zbin, p->round, p->quant,
                           p->quant_shift, qcoeff + i,
                           dqcoeff + i, pd->dequant, p->zbin_extra,
                           logsizeby32, bs, i == 0);
        /*vp9_quantize_slice(coeff + i, bs, skip_block,
                           p->zbin, p->round, p->quant,
                           p->quant_shift, qcoeff + i,
                           dqcoeff + i, pd->dequant, p->zbin_extra,
                           bs, i == 0);*/
        vp9_tx_identity_add(dqcoeff + i,
                            dst + i, bs, 1,
                            bs, dst_stride, shift);
        if ( i < bs - 1 && 1)
          for (j = 0 ; j < bs; j++)
    ...
    case V_PRED:
      for (i = 0 ; i < bs; i++) {
        ...
        vp9_tx_identity(src_diff + diff_stride * i,
                        coeff + bs * i,
                        1, bs,
                        diff_stride, bs, shift);
        quantize_slice_bigtx(coeff + bs * i, bs, skip_block,
                           p->zbin, p->round, p->quant,
                           p->quant_shift, qcoeff + bs * i,
                           dqcoeff + bs * i, pd->dequant, p->zbin_extra,
                           logsizeby32, 1, i == 0);
        /*vp9_quantize_slice(coeff + bs * i, bs, skip_block,
                           p->zbin, p->round, p->quant,
                           p->quant_shift, qcoeff + bs * i,
                           dqcoeff + bs * i, pd->dequant, p->zbin_extra,
                           1, i == 0);*/
        vp9_tx_identity_add(dqcoeff + bs * i,
                            dst + dst_stride * i, 1, bs,
                            bs, dst_stride, shift);
        if ( i < bs - 1 && 1)
  ...
  eob = get_eob(skip_block, qcoeff, bs * bs, scan_order->scan);
  return eob;
}
#endif
        ...
        switch (tx_size) {
          ...
          case TX_64X64:
            ...
            if (!x->skip_encode && *eob)
              vp9_tx_identity_add(dqcoeff, dst, 64, 64, 64, dst_stride, shift);
            break;
#endif  // CONFIG_TX64X64
          case TX_32X32:
            ...
            if (!x->skip_recode) {
              if ((mode == V_PRED || mode == H_PRED) && 1) {
                *eob = vp9_dpcm_intra(src, src_stride, dst, dst_stride,
                                      ...
                                      coeff, qcoeff, dqcoeff, x->skip_block,
                                      p, pd, scan_order, mode, 32, shift, 0);
                break;
              }
            ...
            if (!x->skip_encode && *eob) {
              vp9_tx_identity_add(dqcoeff, dst, 32, 32, 32, dst_stride, shift);
            }
            break;
          case TX_16X16:
            ...
            if (!x->skip_recode) {
              if (mode == V_PRED || mode == H_PRED) {
                *eob = vp9_dpcm_intra(src, src_stride, dst, dst_stride,
                                      src_diff, diff_stride,
                                      coeff, qcoeff, dqcoeff, x->skip_block,
                                      p, pd, scan_order, mode, 16, shift, -1);
                break;
              }
            ...
            if (!x->skip_encode && *eob) {
              vp9_tx_identity_add(dqcoeff, dst, 16, 16, 16, dst_stride, shift);
            }
            break;
          case TX_8X8:
            tx_type = get_tx_type(pd->plane_type, xd);
            ...
            if (!x->skip_recode) {
              if (mode == V_PRED || mode == H_PRED) {
                *eob = vp9_dpcm_intra(src, src_stride, dst, dst_stride,
                                      src_diff, diff_stride,
                                      coeff, qcoeff, dqcoeff, x->skip_block,
                                      p, pd, scan_order, mode, 8, shift, -1);
                break;
              }
            ...
            if (!x->skip_encode && *eob) {
              vp9_tx_identity_add(dqcoeff, dst, 8, 8, 8, dst_stride, shift);
            }
            break;
          case TX_4X4:
            ...
            if (!x->skip_recode) {
              if ((mode == V_PRED || mode == H_PRED) && 1) {
                *eob = vp9_dpcm_intra(src, src_stride, dst, dst_stride,
                                      src_diff, diff_stride,
                                      coeff, qcoeff, dqcoeff, x->skip_block,
                                      p, pd, scan_order, mode, 4, shift, -1);
                break;
              }
            ...
            if (!x->skip_encode && *eob) {
              vp9_tx_identity_add(dqcoeff, dst, 4, 4, 4, dst_stride, shift);
            }
            break;
          default:
            assert(0);
