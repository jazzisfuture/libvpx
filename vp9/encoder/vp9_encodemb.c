...
#else
  //const int16_t *dequant_ptr = (b_sr && plane == 0) ? pd->dequant_sr : pd->dequant;
...
static void encode_block(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  if (plane == 0 && x->zcoeff_blk[xd->mi[0].src_mi->mbmi.sr ? TX_SIZES : tx_size][block]) {
#else  // CONFIG_SR_MODE
      ...
      if (!x->skip_recode) {
        ...
        if (x->quant_fp && plane == 0)
          ...
          vp9_quantize_32x32_nuq(coeff, 1024, x->skip_block,
                                 ...
                                 scan_order->scan, band);
#else
          vp9_quantize_b_32x32(coeff, 1024, x->skip_block, p->zbin_sr, p->round_sr,
      ...
      if (!x->skip_recode) {
        ...
        if (x->quant_fp)
          ...
          vp9_quantize_nuq(coeff, 64, x->skip_block,
                           ...
                           scan_order->scan, band);
#else
          vp9_quantize_b(coeff, 64, x->skip_block, p->zbin_sr, p->round_sr, p->quant_sr,
        ...
        if (mbmi->sr && plane != 0) {  // TX_8x8: only used in chroma case
          ...
          if (x->quant_fp)
            ...
            vp9_quantize_nuq(coeff, 16, x->skip_block,
                             ...
                             scan_order->scan, band);
#else
          vp9_quantize_b(coeff, 16, x->skip_block, p->zbin_sr, p->round_sr, p->quant_sr,
                         p->quant_shift_sr, qcoeff, dqcoeff,
                         pd->dequant_sr, eob, scan_order->scan,
                         scan_order->iscan);
#endif  // CONFIG_NEW_QUANT
