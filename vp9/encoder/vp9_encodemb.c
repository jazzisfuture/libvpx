...
void vp9_high_subtract_block_c(int rows, int cols,
                          ...
                          const uint8_t *pred8, ptrdiff_t pred_stride, int bps) {
  int r, c;
  int shift = bps-8;
...
static void encode_block(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
    switch (tx_size) {
      case TX_32X32:
        vp9_high_idct32x32_add(dqcoeff, dst, pd->dst.stride, p->eobs[block], xd->bps);
        break;
      case TX_16X16:
        vp9_high_idct16x16_add(dqcoeff, dst, pd->dst.stride, p->eobs[block], xd->bps);
        break;
      case TX_8X8:
        vp9_high_idct8x8_add(dqcoeff, dst, pd->dst.stride, p->eobs[block], xd->bps);
        break;
      case TX_4X4:
...
#if CONFIG_VP9_HIGH
        if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
           vp9_high_iht16x16_add(tx_type, dqcoeff, dst, dst_stride, *eob, xd->bps);
        } else {
...
#if CONFIG_VP9_HIGH
        if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
           vp9_high_iht8x8_add(tx_type, dqcoeff, dst, dst_stride, *eob, xd->bps);
        } else {
