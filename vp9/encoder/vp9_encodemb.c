...
static int optimize_b(int plane, int block, BLOCK_SIZE plane_bsize,
                       TX_SIZE tx_size, MACROBLOCK *mb, int ctx) {
...
static void encode_block(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  if (x->optimize && (!x->skip_recode || !x->skip_optimize)) {
    ...
    *a = *l = optimize_b(plane, block, plane_bsize, tx_size, x, ctx) > 0;
  } else {
    *a = *l = p->eobs[block] > 0;
  }
