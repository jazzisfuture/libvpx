...
int vp9_optimize_b(MACROBLOCK *mb, int plane, int block, TX_SIZE tx_size,
  ...
  // Initial RD cost.
  rate0 = *(token_costs + band_translate[0])[0][ctx0][EOB_TOKEN];
  ...
  for (i = 0; i < eob; i++) {
    ...
    if (x == 0) {  // No need to search
      rate0 = *(token_costs +
                band_cur)[token_tree_sel_cur][ctx_cur][tokens[i][0].token];
      accu_rate += rate0;
      x_prev = 0;
      ...
      const int diff_for_zero_raw = (-coeff[rc]) * (1 << shift);
      const int diff_for_zero =
#if CONFIG_VP9_HIGHBITDEPTH
          (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH)
              ? diff_for_zero_raw >> (xd->bd - 8)
              :
#endif
              diff_for_zero_raw;
      ...
      }
      {
        ...
        rate0 = base_bits0 +
                *(token_costs + band_cur)[token_tree_sel_cur][ctx_cur][t0];
        rate1 = base_bits1 +
                *(token_costs + band_cur)[token_tree_sel_cur][ctx_cur][t1];
      }
      {
        ...
        if (i < default_eob - 1) {
          ...
          next_bits0 = *(
              token_costs +
              band_next)[token_tree_sel_next][ctx_next][tokens[i + 1][0].token];
          next_eob_bits0 =
              *(token_costs +
                band_next)[token_tree_sel_next][ctx_next][EOB_TOKEN];
        ...
        x_prev = qcoeff[rc];  // Update based on selected qunatized value.
