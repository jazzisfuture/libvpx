...
void vp9_xform_quant(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  switch (tx_size) {
    case TX_32X32:
      so = &default_scan_orders[TX_32X32];
      block >>= 6;
      ...
      break;
    case TX_16X16:
      so = &default_scan_orders[TX_16X16];
      block >>= 4;
...
void vp9_encode_block_intra(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  switch (tx_size) {
    ...
    case TX_16X16:
      ...
      so = &scan_orders[TX_16X16][tx_type];
      mode = plane == 0 ? mbmi->mode : mbmi->uv_mode;
      block >>= 4;
