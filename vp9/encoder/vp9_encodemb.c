...
static void fdct32x32(int rd_transform,
  ...
  if (rd_transform == 2 && psse) {
    ...
    vp9_sfdct32x32(src, dst, src_stride, &tl_sse);

    if ((tmp_sse - tl_sse) > (tmp_sse >> 8) && 0) {
      *psse = INT64_MAX;
      return;
    }
...
//    if (*psse)
//      fprintf(stderr, "overall pix sse %d, return sse %d\n",
//              (*psse) << 4, tl_sse);

//    ++total_run;
//    if ((tmp_sse - tl_sse) < (tmp_sse >> 2)) {
//      ++select_count;
//      fprintf(stderr, "tmp_sse %d, tl_sse %d\n", tmp_sse, tl_sse);
//    }


//    if (total_run == 10000) {
//      fprintf(stderr, "select 32x32 dct %d\n", select_count);
//      select_count = 0;
//      total_run = 0;
//    }
//
  ...
  switch (tx_size) {
    case TX_32X32:
      fdct32x32(x->use_lp32x32fdct, src_diff, coeff, diff_stride,
                (max_txsize_lookup[plane_bsize] == tx_size) ?
                 &x->bsse[(plane << 2) + (block >> (tx_size << 1))] : NULL);
