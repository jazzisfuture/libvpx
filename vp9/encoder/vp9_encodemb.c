...
void optimize_init_b(int plane, int block, BLOCK_SIZE_TYPE bsize,
                         int ss_txfrm_size, void *arg) {
...
void vp9_optimize_sbuv(VP9_COMMON *const cm, MACROBLOCK *x,
  ...
  struct optimize_ctx ctx;
  struct optimize_block_args arg = {cm, x, &ctx};
  foreach_transformed_block_uv(&x->e_mbd, bsize, optimize_init_b, &arg);
...
void vp9_encode_sby(VP9_COMMON *cm, MACROBLOCK *x, BLOCK_SIZE_TYPE bsize) {
  ...
  vp9_subtract_sby(x, bsize);
  if (x->optimize)
    foreach_transformed_block_in_plane(xd, bsize, 0, optimize_init_b, &arg);
...
void vp9_encode_sbuv(VP9_COMMON *cm, MACROBLOCK *x, BLOCK_SIZE_TYPE bsize) {
  ...
  vp9_subtract_sbuv(x, bsize);
  if (x->optimize)
    foreach_transformed_block_uv(xd, bsize, optimize_init_b, &arg);
...
void vp9_encode_sb(VP9_COMMON *cm, MACROBLOCK *x, BLOCK_SIZE_TYPE bsize) {
  ...
  if (x->optimize)
    foreach_transformed_block(xd, bsize, optimize_init_b, &arg);
