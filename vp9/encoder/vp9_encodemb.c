...
void vp9_encode_sb(const VP9_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bsize,
  ...
  for (plane = 0; plane < MAX_MB_PLANE; ++plane) {
    ...
    if (x->optimize && (!x->skip_recode || !x->skip_optimize)) {
      ...
      vp9_get_entropy_contexts(bsize, tx_size, pd, ctx.ta[plane],
                               ctx.tl[plane]);
      arg.trellis_opt_type = ENABLE_TRELLIS_OPT;
    } else {
      arg.trellis_opt_type = DISABLE_TRELLIS_OPT;
    }
    arg.ta = ctx.ta[plane];
    arg.tl = ctx.tl[plane];
...
void vp9_encode_intra_block_plane(const VP9_COMP *cpi, MACROBLOCK *x,
