      ...
      case ALT:
        switch (tx_size) {
          case TX_32X32:
            ...
            break;
          case TX_16X16:
            vp9_iht16x16_add(ADST_ADST, dqcoeff, dst, pd->dst.stride, p->eobs[block]);
            break;
          case TX_8X8:
            vp9_iht8x8_add(ADST_ADST, dqcoeff, dst, pd->dst.stride, p->eobs[block]);
            break;
          case TX_4X4:
            // this is like vp9_short_idct4x4 but has a special case around eob<=1
            // which is significant (not just an optimization) for the lossless
            // case.
            vp9_iht4x4_add(ADST_ADST, dqcoeff, dst, pd->dst.stride, p->eobs[block]);
            break;
          default:
            assert(0 && "Invalid transform size");
        }
