...
void vp9_optimize_sby(VP9_COMMON *const cm, MACROBLOCK *x,
  ...
  foreach_transformed_block_in_plane(&x->e_mbd, bsize, 0, 0,
                                     optimize_block, &arg);
}
...
static void encode_block(int plane, int block, BLOCK_SIZE_TYPE bsize,
  ...
  vp9_quantize(x, plane, block, 16 << ss_txfrm_size, scan, qmul);
  if (x->optimize)
  ...
  switch (ss_txfrm_size / 2) {
    ...
    case TX_4X4:
      if (tx_type == DCT_DCT) {
        vp9_short_idct4x4(BLOCK_OFFSET(xd->plane[plane].dqcoeff, block, 16),
                          diff, bw * 2);
      } else {
...
void vp9_encode_sb(VP9_COMMON *const cm, MACROBLOCK *x,
  ...
  // TODO(jkoleszar): Could combine subtract, recon too.
