...
void vp9_xform_quant_fp(MACROBLOCK *x, int plane, int block,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    switch (tx_size) {
      case TX_32X32:
        ...
        vp9_highbd_quantize_fp_32x32(coeff, 1024, x->skip_block, NULL,
                                     p->round_fp, p->quant_fp, p->quant_shift,
                                     qcoeff, dqcoeff, pd->dequant,
                                     0, eob, scan_order->scan,
                                     scan_order->iscan);
        break;
      case TX_16X16:
        vp9_highbd_fdct16x16(src_diff, coeff, diff_stride);
        vp9_highbd_quantize_fp(coeff, 256, x->skip_block, NULL, p->round_fp,
      ...
      case TX_8X8:
        ...
        vp9_highbd_quantize_fp(coeff, 64, x->skip_block, NULL, p->round_fp,
                               p->quant_fp, p->quant_shift, qcoeff, dqcoeff,
                               pd->dequant, 0, eob,
                               scan_order->scan, scan_order->iscan);
        break;
      case TX_4X4:
        ...
        vp9_highbd_quantize_fp(coeff, 16, x->skip_block, NULL, p->round_fp,
                               p->quant_fp, p->quant_shift, qcoeff, dqcoeff,
                               pd->dequant, 0, eob,
                               scan_order->scan, scan_order->iscan);
        break;
  ...
  switch (tx_size) {
    case TX_32X32:
      ...
      vp9_quantize_fp_32x32(coeff, 1024, x->skip_block, NULL, p->round_fp,
                            p->quant_fp, p->quant_shift, qcoeff, dqcoeff,
                            pd->dequant, 0, eob, scan_order->scan,
                            scan_order->iscan);
      break;
    case TX_16X16:
      vp9_fdct16x16(src_diff, coeff, diff_stride);
      vp9_quantize_fp(coeff, 256, x->skip_block, NULL, p->round_fp,
                      p->quant_fp, p->quant_shift, qcoeff, dqcoeff,
                      pd->dequant, 0, eob,
                      scan_order->scan, scan_order->iscan);
      break;
    ...
    case TX_4X4:
      x->fwd_txm4x4(src_diff, coeff, diff_stride);
      vp9_quantize_fp(coeff, 16, x->skip_block, NULL, p->round_fp,
                      p->quant_fp, p->quant_shift, qcoeff, dqcoeff,
                      pd->dequant, 0, eob,
                      scan_order->scan, scan_order->iscan);
      break;
...
void vp9_xform_quant(MACROBLOCK *x, int plane, int block,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
     switch (tx_size) {
      case TX_32X32:
        highbd_fdct32x32(x->use_lp32x32fdct, src_diff, coeff, diff_stride);
        vp9_highbd_quantize_b_32x32(coeff, 1024, x->skip_block, NULL,
      ...
      case TX_16X16:
        vp9_highbd_fdct16x16(src_diff, coeff, diff_stride);
        vp9_highbd_quantize_b(coeff, 256, x->skip_block, NULL, p->round,
      ...
      case TX_8X8:
        vp9_highbd_fdct8x8(src_diff, coeff, diff_stride);
        vp9_highbd_quantize_b(coeff, 64, x->skip_block, NULL, p->round,
                              p->quant, p->quant_shift, qcoeff, dqcoeff,
                              pd->dequant, 0, eob,
                              scan_order->scan, scan_order->iscan);
        break;
      case TX_4X4:
        x->fwd_txm4x4(src_diff, coeff, diff_stride);
        vp9_highbd_quantize_b(coeff, 16, x->skip_block, NULL, p->round,
                              p->quant, p->quant_shift, qcoeff, dqcoeff,
                              pd->dequant, 0, eob,
                              scan_order->scan, scan_order->iscan);
        break;
  ...
  switch (tx_size) {
    case TX_32X32:
      fdct32x32(x->use_lp32x32fdct, src_diff, coeff, diff_stride);
      vp9_quantize_b_32x32(coeff, 1024, x->skip_block, NULL, p->round,
                           p->quant, p->quant_shift, qcoeff, dqcoeff,
                           pd->dequant, 0, eob, scan_order->scan,
                           scan_order->iscan);
      break;
    case TX_16X16:
      vp9_fdct16x16(src_diff, coeff, diff_stride);
      vp9_quantize_b(coeff, 256, x->skip_block, NULL, p->round,
                     p->quant, p->quant_shift, qcoeff, dqcoeff,
                     pd->dequant, 0, eob,
                     scan_order->scan, scan_order->iscan);
      break;
    case TX_8X8:
      ...
      vp9_quantize_b(coeff, 64, x->skip_block, NULL, p->round,
                     p->quant, p->quant_shift, qcoeff, dqcoeff,
                     pd->dequant, 0, eob,
                     scan_order->scan, scan_order->iscan);
      break;
    case TX_4X4:
      x->fwd_txm4x4(src_diff, coeff, diff_stride);
      vp9_quantize_b(coeff, 16, x->skip_block, NULL, p->round,
                     p->quant, p->quant_shift, qcoeff, dqcoeff,
                     pd->dequant, 0, eob,
                     scan_order->scan, scan_order->iscan);
      break;
  ...
  switch (tx_size) {
    ...
    case TX_8X8:
      ...
      if (!x->skip_recode) {
        ...
        vp9_quantize_b(coeff, 64, x->skip_block, NULL, p->round, p->quant,
    ...
    case TX_4X4:
      ...
      if (!x->skip_recode) {
        ...
        if (tx_type != DCT_DCT)
          ...
          x->fwd_txm4x4(src_diff, coeff, diff_stride);
        vp9_quantize_b(coeff, 16, x->skip_block, NULL, p->round, p->quant,
                       p->quant_shift, qcoeff, dqcoeff,
                       pd->dequant, 0, eob, scan_order->scan,
                       scan_order->iscan);
      }
