...
void vp9_subtract_sbuv_s_c(int16_t *diff, const uint8_t *usrc,
  ...
  const int bw = 16 << (bwl - 1), bh = 16 << (bhl - 1);
...
void vp9_transform_sby_4x4(MACROBLOCK *x, BLOCK_SIZE_TYPE bsize) {
  ...
  for (n = 0; n < bw * bh; n++) {
    ...
    if (tx_type != DCT_DCT) {
      vp9_short_fht8x8(x->src_diff + y_idx * stride * 4 + x_idx * 4,
...
void vp9_transform_sbuv_8x8(MACROBLOCK *x, BLOCK_SIZE_TYPE bsize) {
  ...
  for (n = 0; n < bw * bh; n++) {
    ...
    x->fwd_txm8x8(x->src_diff + 4096 + y_idx * stride * 8 + x_idx * 8,
                  x->coeff + uoff + n * 64, stride * 2);
    x->fwd_txm8x8(x->src_diff + 4096 + 1024 + y_idx * stride * 8 + x_idx * 8,
...
void vp9_optimize_sby_4x4(VP9_COMMON *const cm, MACROBLOCK *x,
  ...
  vpx_memcpy(ta, x->e_mbd.above_context, sizeof(ENTROPY_CONTEXT_PLANES) * bw);
  vpx_memcpy(tl, x->e_mbd.left_context, sizeof(ENTROPY_CONTEXT_PLANES) * bh);
  bw *= 4;
  bh *= 4;
  bwl += 2;
