...
#else
            vp9_fgbt(src_diff, coeff, diff_stride, dst, pd->dst.stride, 8, 8, basis);
#endif
            vp9_quantize_b(coeff, 64, x->skip_block, p->zbin, p->round,
                           p->quant, p->quant_shift, qcoeff, dqcoeff,
                           pd->dequant, p->zbin_extra, eob,
...
#else
            vp9_fgbt(src_diff, coeff, diff_stride, dst, pd->dst.stride, 4, 4, basis);
#endif
            vp9_quantize_b(coeff, 16, x->skip_block, p->zbin, p->round,
                           p->quant, p->quant_shift, qcoeff, dqcoeff,
                           pd->dequant, p->zbin_extra, eob,
        ...
        switch (tx_size) {
          ...
          case TX_4X4:
            ...
            vp9_iht4x4_add(ADST_ADST, dqcoeff, dst, pd->dst.stride,
                           p->eobs[block]);
#else
            vp9_igbt_add(dqcoeff, dst, pd->dst.stride, p->eobs[block], 4, 4, basis);
#endif
            break;
          default:
            assert(0 && "Invalid transform size");
