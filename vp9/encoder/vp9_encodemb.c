...
void vp9_encode_block_intra(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    if (!x->skip_recode) {
      vpx_highbd_subtract_block(bs, bs, src_diff, diff_stride,
        ...
        case TX_4X4:
          ...
          if (*eob) {
            if (tx_type == DCT_DCT) {
              ...
              vp9_highbd_iht4x4_16_add(dqcoeff, dst, dst_stride, tx_type,
                                       xd->bd);
            }
          }
          break;
  ...
  if (!x->skip_recode) {
    vpx_subtract_block(bs, bs, src_diff, diff_stride,
    ...
    switch (tx_size) {
      ...
      case TX_4X4:
        ...
        if (*eob) {
          if (tx_type == DCT_DCT)
            // this is like vp9_short_idct4x4 but has a special case around
            // eob <= 1 hich is significant (not just an optimization) for the
            // lossless case.
