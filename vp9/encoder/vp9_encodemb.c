...
static void encode_block_intra(int plane, int block, BLOCK_SIZE_TYPE bsize,
  ...
  int16_t* const diff =
      raster_block_offset_int16(xd, bsize, plane,
                                raster_block, xd->plane[plane].diff);
  ...
  // TODO(yaowu): work out bsize for 422?
  plane_b_size = bsize - xd->plane[plane].subsampling_x;
  vp9_predict_intra_block(xd, ib, plane_b_size, tx_size, b_mode,
...
void vp9_encode_intra4x4mbuv(VP9_COMMON *const cm, MACROBLOCK *x,
                            BLOCK_SIZE_TYPE bsize) {
  MACROBLOCKD* const xd = &x->e_mbd;
  struct optimize_ctx ctx;
  struct encode_b_args arg = {cm, x, &ctx};
