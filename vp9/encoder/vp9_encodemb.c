...
static void encode_block_intra(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  if (mbmi->tx_skip[plane != 0]) {
    ...
    if (x->output) {
      ...
      fp = fopen("./debug/data_enc1.txt", "a");

      fprintf(fp, "dst \n");
      for (r = 0; r < bs; r++) {
        for (c = 0; c < bs; c++) {
          fprintf(fp, "%4d ", dst16[r * dst_stride + c]);
        }
        fprintf(fp, "\n");
      }
      fprintf(fp, "\n \n");
    ...
    if (!x->skip_recode && tx_size <= TX_32X32 &&
        (mode == V_PRED || mode == H_PRED || mode == TM_PRED) && 1) {
#if CONFIG_VP9_HIGHBITDEPTH
      if (use_hbd)
    ...
    if (x->output) {
      ...
      fprintf(fp, " mi %2d %2d, plane %2d, mode %3d, tx_skip %3d dst \n",
              xd->mi_row, xd->mi_col, plane, mode, mbmi->tx_skip[plane != 0]);
      for (r = 0; r < bs; r++) {
        for (c = 0; c < bs; c++) {
          fprintf(fp, "%4d ", dst16[r * dst_stride + c]);
        }
