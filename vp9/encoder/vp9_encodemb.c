...
void sr_downsample(int16_t *src, int src_stride, int16_t *dst, int dst_stride,
  ...
  for (i = 0; i < h/2; i ++) {
    for (j = 0; j < w/2; j ++) {
      dst[j] = (src[j << 1] + src[(j << 1) + 1] + src[(j << 1) + src_stride] +
               src[(j << 1) + 1 + src_stride]) >> 2;
    }
    dst += dst_stride;
    src += (src_stride << 1);
  }
...
void inv_trfm_sr(MACROBLOCK * x, TX_SIZE tx_size, BLOCK_SIZE plane_bsize,
                 int plane, int block) {
  MACROBLOCKD *const xd = &x->e_mbd;
  struct macroblock_plane *const p = &x->plane[plane];
  struct macroblockd_plane *const pd = &xd->plane[plane];
...
#if SR_USE_MULTI_F
  sr_recon(tmp_buf, tmp_stride, dst, pd->dst.stride, bs, bs, f_hor, f_ver);
#else
...
static void encode_block(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  //int tmp_stride = 64, tmp2_stride = 64;
  int bs = 4 << tx_size;
#endif
#if CONFIG_EXT_TX
  TX_TYPE tx_type;
  ...
  if (!x->skip_recode) {
    ...
    if (xd->mi[0].src_mi->mbmi.sr) {
      ...
      int16_t *src_sr_diff = (int16_t *)&p->src_sr_diff[4 * (j * src_sr_diff_stride + i)];
      int16_t *src_diff = (int16_t *)&p->src_diff[4 * (j * src_sr_diff_stride + i)];

      // Downsample
      sr_downsample(src_diff, src_diff_stride, src_sr_diff, src_sr_diff_stride, bs, bs);
    ...
    case TX_64X64:
      ...
      if (!x->skip_recode) {
        ...
        if (mbmi->sr) {
          int bs = 64;
          assert(tx_size == max_txsize_lookup[plane_bsize]);
          sr_downsample(src_diff, diff_stride, src_sr_diff, src_sr_diff_stride, bs, bs);
