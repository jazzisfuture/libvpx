    ...
    case TM_PRED:
      ...
      for (i = 1 ; i < bs; i++) {
        for (j = 1 ; j < bs; j++) {
          ...
          vp9_subtract_block_c(1, 1, src_diff + diff_stride * i + j,diff_stride,
                               src + src_stride * i + j, src_stride,
                               dst + dst_stride * i + j, dst_stride);
          vp9_tx_identity_rect(src_diff + i * diff_stride + j,
                               coeff + bs * i + j, 1, 1, diff_stride,
                               bs, shift);
          vp9_quantize_rect(coeff + bs * i + j, 1, 1, p->zbin, p->round, p->quant,
                            p->quant_shift, qcoeff + bs * i + j,
