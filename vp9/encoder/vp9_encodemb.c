...
int vp9_optimize_b(MACROBLOCK *mb, int plane, int block, TX_SIZE tx_size,
  ...
  const int64_t rdmult =
    ((mb->rdmult * plane_rd_mult[ref][plane_type]) +
     2) >>
    (sharpness + (mb->adjust_rdmult_by_segment && mbmi->segment_id < 4
                      ? 7 - mbmi->segment_id
                      : 2));
  ...
  for (i = 0; i < eob; i++) {
    ...
    if (x == 0) {  // No need to search
      ...
      }
      {
        ...
        if (sharpness > 0 && abs(qcoeff[rc]) > 1)
          count_high_values_after_eob++;
        assert(accu_error >= 0);
