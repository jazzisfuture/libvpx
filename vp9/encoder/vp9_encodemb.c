...
void vp9_encode_sb(MACROBLOCK *x, BLOCK_SIZE bsize, int mi_row, int mi_col,
  ...
  int plane;
#if CONFIG_MISMATCH_DEBUG
  struct encode_b_args arg = { x,    1, NULL,   NULL,   &mi->skip,     NULL,
                               NULL, 0, mi_row, mi_col, output_enabled };
#else
  struct encode_b_args arg = { x, 1, NULL, NULL, &mi->skip, NULL, NULL, 0 };
  (void)mi_row;
  (void)mi_col;
  (void)output_enabled;
#endif
...
void vp9_encode_block_intra(int plane, int block, int row, int col,
  ...
  int enable_coeff_opt = !x->skip_recode;
  ...
  if (!x->skip_recode) {
    ...
    enable_coeff_opt =
        args->enable_coeff_opt &&
        !skip_coeff_optimize(xd, pd, src_diff, diff_stride, row, col,
                             plane_bsize, tx_size, args->skip_coeff_opt,
                             args->sse, args->sse_calc_done);
  }
