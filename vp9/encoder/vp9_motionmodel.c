...
static int do_16x16_motion_search(VP9_COMP *cpi, const MV *ref_mv, int bsize,
                                  int_mv *dst_mv, int mb_row, int mb_col, unsigned int *sse) {
  MACROBLOCK *const x = &cpi->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  tmp_err = do_16x16_motion_iteration(cpi, ref_mv, &tmp_mv, bsize, mb_row, mb_col, sse);
  if (tmp_err < err) {
    err = tmp_err;
    dst_mv->as_mv = tmp_mv;
  }
  ...
  if (ref_mv->row != 0 || ref_mv->col != 0) {
    ...
    tmp_err = do_16x16_motion_iteration(cpi, &zero_ref_mv, &tmp_mv, bsize,
                                        mb_row, mb_col, sse);
    if (tmp_err < err) {
      dst_mv->as_mv = tmp_mv;
      err = tmp_err;
...
static void update_motionfield_mb_stats
(
  VP9_COMP *cpi,
  ...
  if (ref) {
    ...
    sad = do_16x16_motion_search(cpi,
                                 prev_ref_mv,
                                 bsize,
                                 &intmv,
                                 mb_row, mb_col, &sse);
...
static void update_motionfield_frame_stats(VP9_COMP *cpi,
                                           YV12_BUFFER_CONFIG *buf,
                                           YV12_BUFFER_CONFIG *ref,
                                           int blocksize,
                                           MV *motionfield,
...
//  // the UMV borders.
  ...
  for (mb_row = 0; mb_row < cm->mb_rows; mb_row++) {
    ...
    for (mb_col = 0; mb_col < cm->mb_cols; mb_col++) {
      MV mv;
      update_motionfield_mb_stats(cpi, buf, mb_y_in_offset,
                                  ref, &ref_left_mv,
                                  blocksize,
                                  mb_row, mb_col, &mv,
...
void vp9_update_motionfield_stats(VP9_COMP *cpi, int ref, int blocksize, MV *motionfield, double *confidence) {
  YV12_BUFFER_CONFIG *ref_buf = get_ref_frame_buffer(cpi, ref);
  int i;
  struct lookahead_entry *q_cur = vp9_lookahead_peek(cpi->lookahead, 0);

  if (q_cur) {
    update_motionfield_frame_stats(cpi, &q_cur->img, ref_buf, blocksize, motionfield, confidence);
  }
}
