...
void *vp9_enc_grp_get_next_job(MultiThreadHandle *multi_thread_ctxt,
  ...
  TileWPPInfo *tile_wpp_info;
  JobQueueHandle *job_queue_hdl = NULL;
  void *next = NULL;
  pthread_mutex_t *mutex_handle = NULL;

  tile_wpp_info = (TileWPPInfo *)(&multi_thread_ctxt->tile_wpp_info[tile_id]);

  if (ENCODE_JOB == job_type) {
    job_queue_hdl = (JobQueueHandle *)&tile_wpp_info->job_que_enc_hdl;
    mutex_handle = &tile_wpp_info->tile_job_mutex;
  } else if (ARNR_JOB == job_type) {
    job_queue_hdl = (JobQueueHandle *)&tile_wpp_info->job_que_arnr_hdl;
    mutex_handle = &tile_wpp_info->arnr_job_mutex;
  }
...
void vp9_wpp_mem_alloc(VP9_COMP *cpi) {
  ...
  for (tile_col = 0; tile_col < tile_cols; tile_col++) {
    TileWPPInfo *tile_wpp_info = &multi_thread_ctxt->tile_wpp_info[tile_col];
    pthread_mutex_init(&tile_wpp_info->tile_job_mutex, NULL);
  }

  // Allocate memory for WPP
  for (tile_col = 0; tile_col < tile_cols; tile_col++) {
    TileDataEnc *this_tile = &cpi->tile_data[tile_col];
    vp9_wpp_sync_mem_alloc(&this_tile->wpp_sync, cm, num_enc_row_jobs,
                           multi_thread_ctxt->num_wpp_workers);
  ...
  for (tile_row = 1; tile_row < tile_rows; tile_row++) {
    for (tile_col = 0; tile_col < tile_cols; tile_col++) {
      ...
      this_tile->wpp_sync = this_col_tile->wpp_sync;
    }
  }
...
void vp9_wpp_mem_dealloc(VP9_COMP *cpi) {
  ...
  for (tile_col = 0; tile_col < multi_thread_ctxt->max_num_tile_cols;
       tile_col++) {
    TileWPPInfo *tile_wpp_info = &multi_thread_ctxt->tile_wpp_info[tile_col];
    pthread_mutex_destroy(&tile_wpp_info->tile_job_mutex);
  }

  // Free WPP sync memory
  for (tile_col = 0; tile_col < multi_thread_ctxt->max_num_tile_cols;
       tile_col++) {
    TileDataEnc *this_tile = &cpi->tile_data[tile_col];
    vp9_wpp_sync_mem_dealloc(&this_tile->wpp_sync);
    if (cpi->oxcf.pass == 1)
...
void vp9_multi_thread_init(VP9_COMP *cpi, YV12_BUFFER_CONFIG *Source) {
  ...
  multi_thread_ctxt->num_wpp_workers = VPXMAX(cpi->oxcf.max_threads, 1);
}
...
void vp9_multi_thread_tile_init(VP9_COMP *cpi) {
  ...
  for (i = 0; i < tile_cols; i++) {
    ...
    memset(this_tile->wpp_sync.cur_col, -1,
           sizeof(*this_tile->wpp_sync.cur_col) * num_job_rows);
...
void vp9_assign_tile_to_thread(MultiThreadHandle *multi_thread_ctxt,
  ...
  for (i = 0; i < multi_thread_ctxt->num_wpp_workers; i++) {
...
int vp9_get_job_queue_status(MultiThreadHandle *multi_thread_ctxt,
  ...
  tile_wpp_info = &multi_thread_ctxt->tile_wpp_info[cur_tile_id];

  if (ENCODE_JOB == job_type) {
    mutex = &tile_wpp_info->tile_job_mutex;
    job_que_hndl = &tile_wpp_info->job_que_enc_hdl;
  } else if (ARNR_JOB == job_type) {
    mutex = &tile_wpp_info->arnr_job_mutex;
    job_que_hndl = &tile_wpp_info->job_que_arnr_hdl;
  } else {
    job_que_hndl = NULL;
    mutex = NULL;
    assert(0);
...
void vp9_prepare_job_queue(MultiThreadHandle *multi_thread_ctxt, int tile_cols,
  ...
  int tile_col; // counter for bitrate
  ...
  for (tile_col = 0; tile_col < tile_cols; tile_col++) {
    TileWPPInfo *tile_ctxt = &multi_thread_ctxt->tile_wpp_info[tile_col];
