...
#endif
  if (next != NULL)
    return (void *)(&(((job_queue_t *)next)->job_info));
  else
    return NULL;
}
...
void vp9_row_mt_mem_alloc(VP9_COMP *cpi) {
  ...
  multi_thread_ctxt->job_queue =
      (job_queue_t *)vpx_memalign(32, total_jobs * sizeof(job_queue_t));
...
void vp9_prepare_job_queue(VP9_COMP *cpi, ENC_JOB_TYPES_T task_type) {
  VP9_COMMON *const cm = &cpi->common;
  MultiThreadHandle *multi_thread_ctxt = &cpi->multi_thread_ctxt;
  job_queue_t *job_queue = multi_thread_ctxt->job_queue;
  const int tile_cols = 1 << cm->log2_tile_cols;
  int job_num, jobs_per_tile, jobs_per_tile_col, total_jobs;
  const int sb_rows = mi_cols_aligned_to_sb(cm->mi_rows) >> MI_BLOCK_SIZE_LOG2;
  int tile_col, i;

  jobs_per_tile_col = (task_type != ENCODE_JOB) ? cm->mb_rows : sb_rows;
  total_jobs = jobs_per_tile_col * tile_cols;

  multi_thread_ctxt->jobs_per_tile_col = jobs_per_tile_col;
  // memset the entire job queue buffer to zero
  memset(job_queue, 0, total_jobs * sizeof(job_queue_t));

  // Job queue preparation
  for (tile_col = 0; tile_col < tile_cols; tile_col++) {
    RowMTInfo *tile_ctxt = &multi_thread_ctxt->row_mt_info[tile_col];
    job_queue_t *job_queue_curr, *job_queue_temp;
    int tile_row = 0;
    ...
    for (job_num = 0, jobs_per_tile = 0; job_num < jobs_per_tile_col;
         job_num++, jobs_per_tile++) {
      job_queue_curr->job_info.vert_unit_row_num = job_num;
      ...
      if (ENCODE_JOB == task_type) {
        if (jobs_per_tile >=
            multi_thread_ctxt->num_tile_vert_units[tile_row] - 1) {
          tile_row++;
          jobs_per_tile = -1;
        }
      }
