...
void vp9_high_ssim_parms_8x8_shift_c(uint16_t *s, int sp, uint16_t *r, int rp,
                                     uint32_t *sum_s, uint32_t *sum_r,
                                     uint32_t *sum_sq_s, uint32_t *sum_sq_r,
...
static double high_ssim_8x8_shift(uint16_t *s, int sp, uint16_t *r, int rp,
  ...
  const int oshift = bps - 8;
  vp9_high_ssim_parms_8x8_shift(s, sp, r, rp, &sum_s,
...
double vp9_high_calc_ssim(YV12_BUFFER_CONFIG *source, YV12_BUFFER_CONFIG *dest,
  ...
  a = vp9_high_ssim2(source->y_buffer, dest->y_buffer,
                     source->y_stride, dest->y_stride,
                     source->y_crop_width, source->y_crop_height, bps, shift);

  b = vp9_high_ssim2(source->u_buffer, dest->u_buffer,
                     source->uv_stride, dest->uv_stride,
                     source->uv_crop_width, source->uv_crop_height, bps, shift);
