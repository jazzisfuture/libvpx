...
void vp9_denoiser_update_frame_info(
  ...
  if (frame_type == KEY_FRAME || resized != 0 || denoiser->reset ||
    ...
    for (i = 1; i < denoiser->used_num_ref_frames; ++i)
...
int vp9_denoiser_realloc_svc(VP9_COMMON *cm, VP9_DENOISER *denoiser,
  ...
  int fail = 0;
  if (refresh_alt) {
    fail = vp9_denoiser_realloc_svc_helper(cm, denoiser, alt_fb_idx);
    if (fail) return 1;
  }
  if (refresh_gld) {
    fail = vp9_denoiser_realloc_svc_helper(cm, denoiser, gld_fb_idx);
    if (fail) return 1;
  }
  if (refresh_lst) {
    fail = vp9_denoiser_realloc_svc_helper(cm, denoiser, lst_fb_idx);
    if (fail) return 1;
  }
  return 0;
}
...
int vp9_denoiser_alloc(VP9_COMMON *cm, int use_svc, VP9_DENOISER *denoiser,
  ...
  denoiser->used_num_ref_frames = use_svc ? MAX_REF_FRAMES : NONSVC_REF_FRAMES;
  CHECK_MEM_ERROR(
      cm, denoiser->running_avg_y,
      vpx_calloc(denoiser->num_ref_frames, sizeof(denoiser->running_avg_y[0])));
  for (i = 0; i < denoiser->used_num_ref_frames; ++i) {
