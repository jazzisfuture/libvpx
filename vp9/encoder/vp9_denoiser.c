...
void vp9_denoiser_update_frame_info(VP9_DENOISER *denoiser,
  ...
  if (refresh_golden_frame + refresh_last_frame + refresh_alt_ref_frame > 1) {
    ...
    // For fixed pattern SVC update denoised last_frame if alt_ref is refreshed.
  ...
  // temporal layer (since alt_ref refresh will update denoised last_frame).
  // We do this by copying the current denoised last into the denoised
  // golden_frame, for temporal_layer_id = 0. For the fixed pattern SVC
  // golden is always spatial reference and is never used for denoising, so we
  // can use it to keep track of denoised last_frame.
