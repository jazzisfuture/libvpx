...
void vp9_denoiser_init_noise_estimate(VP9_DENOISER *denoiser,
  ...
  denoiser->thresh_noise_estimate = 50;
  if (width * height > 1280 * 720) {
    denoiser->thresh_noise_estimate = 200;
  } else if (width * height > 960 * 540) {
    denoiser->thresh_noise_estimate = 120;
  } else if (width * height > 640 * 360) {
    denoiser->thresh_noise_estimate = 80;
  }
}
...
void vp9_denoiser_update_noise_estimate(VP9_COMP *const cpi) {
  ...
  int min_blocks_estimate = cm->mi_rows * cm->mi_cols >> 8;
  ...
  if (cm->current_video_frame % frame_period != 0 ||
    ...
    for (mi_row = 0; mi_row < cm->mi_rows; mi_row ++) {
      for (mi_col = 0; mi_col < cm->mi_cols; mi_col ++) {
        ...
        if (mi_row % 4 == 0 && mi_col % 4 == 0) {
          ...
          // Only consider blocks that are likely steady background.
          if (cr->consec_zero_mv[bl_index] > thresh_consec_zeromv &&
            ...
            // if the sum difference of the current and previous frame
            // is small (to avoid effects from lighting change).
            if ((sse - variance) < thresh_sum_diff) {
              unsigned int sse2;
              const unsigned int spatial_activity =
                  cpi->fn_ptr[bsize].vf(src_y, src_ystride, const_source,
                                        0, &sse2);
              // TODO(marpan): Look into this norm. factor by spatial activity.
              if (spatial_activity > 1)
                avg_est += variance / sqrt(spatial_activity);
              num_samples++;
            }
          }
        }
    ...
    // duplicate frames, or contrast is all zero).
    if (num_samples > min_blocks_estimate && avg_est > 0) {
      // Normalize.
      avg_est = (avg_est << 8) / num_samples;
      // Update noise estimate.
      if (cpi->denoiser.noise_estimate_count == 0)
        cpi->denoiser.noise_estimate =  avg_est;
      else
