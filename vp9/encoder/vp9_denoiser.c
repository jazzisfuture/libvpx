...
int denoiser_filter(uint8_t *mc_avg, int mc_avg_stride, uint8_t *avg,
                       int avg_stride, uint8_t *sig, int sig_stride,
                       int increase_denoising, BLOCK_SIZE bs) {
  int r, c;
  uint8_t *mc_avg_start = mc_avg;
  uint8_t *avg_start = avg;
  uint8_t *sig_start = sig;
...
int perform_motion_compensation(VP9_DENOISER *denoiser, MACROBLOCK *mb,
                                BLOCK_SIZE bs) {
  // constants
...
void vp9_denoiser_denoise(VP9_DENOISER *denoiser, MACROBLOCK *mb,
  ...
  uint8_t *avg_start = partition_start(avg.y_buffer, avg.y_stride,
                                       mi_row, mi_col);
  uint8_t *mc_avg_start = partition_start(mc_avg.y_buffer, mc_avg.y_stride,
                                          mi_row, mi_col);
  struct buf_2d src = mb->plane[0].src;

  int make_reference = 0;
  if (make_reference) {
    copy_partition(avg_start, avg.y_stride, src.buf, src.stride, bs);
    return;
  }
  ...
  if (decision == FILTER_BLOCK) {
    decision = denoiser_filter(mc_avg_start, mc_avg.y_stride,
                               avg_start, avg.y_stride,
                               mb->plane[0].src.buf, mb->plane[0].src.stride,
                               0, bs);
  }

  if (decision == FILTER_BLOCK) {
    copy_partition(src.buf, src.stride, avg_start, avg.y_stride, bs);
  } else { // COPY_BLOCK
    copy_partition(avg_start, avg.y_stride, src.buf, src.stride, bs);
  }
}
