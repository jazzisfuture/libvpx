...
void vp9_denoiser_denoise(VP9_COMP *cpi, MACROBLOCK *mb, int mi_row, int mi_col,
  ...
  int shift = cpi->svc.number_spatial_layers - cpi->svc.spatial_layer_id == 2
                  ? denoiser->num_ref_frames
                  : 0;
  YV12_BUFFER_CONFIG avg = denoiser->running_avg_y[INTRA_FRAME + shift];
  int denoise_layer_index =
...
void vp9_denoiser_update_frame_info(
  ...
  int shift = second_spatial_layer ? denoiser->num_ref_frames : 0;
...
int vp9_denoiser_realloc_svc(VP9_COMMON *cm, VP9_DENOISER *denoiser, struct SVC *svc,
                             int refresh_alt, int refresh_gld, int refresh_lst,
                             int alt_fb_idx, int gld_fb_idx, int lst_fb_idx) {
  int fail = 0;
  const int shift = svc->number_spatial_layers - svc->spatial_layer_id == 2
                        ? denoiser->num_ref_frames
                        : 0;
  if (refresh_alt) {
    ...
    // denoiser.
    fail =
        vp9_denoiser_realloc_svc_helper(cm, denoiser, alt_fb_idx + 1 + shift);
    if (fail) return 1;
  }
  if (refresh_gld) {
    fail =
        vp9_denoiser_realloc_svc_helper(cm, denoiser, gld_fb_idx + 1 + shift);
    if (fail) return 1;
  }
  if (refresh_lst) {
    fail =
        vp9_denoiser_realloc_svc_helper(cm, denoiser, lst_fb_idx + 1 + shift);
    if (fail) return 1;
  }
  return 0;
}
...
int vp9_denoiser_alloc(VP9_COMMON *cm, struct SVC *svc, VP9_DENOISER *denoiser,
                       int use_svc, int width, int height, int ssx, int ssy,
#if CONFIG_VP9_HIGHBITDEPTH
  ...
  const int num_layers =
      use_svc ? svc->number_spatial_layers - svc->first_layer_denoise : 1;
