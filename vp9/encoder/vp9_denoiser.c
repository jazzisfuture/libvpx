...
void vp9_denoise_init_svc(VP9_COMP *cpi, int lst_fb_idx) {
  if (cpi->svc.spatial_layer_id == cpi->svc.number_spatial_layers - 1) {
    VP9_DENOISER *denoiser = &cpi->denoiser;
    if (denoiser->refresh_fb_idx[lst_fb_idx] == GOLDEN_FRAME)
      copy_frame(&denoiser->running_avg_y[LAST_FRAME],
                 &denoiser->running_avg_y[GOLDEN_FRAME]);
    else if (denoiser->refresh_fb_idx[lst_fb_idx] == ALTREF_FRAME)
      copy_frame(&denoiser->running_avg_y[LAST_FRAME],
...
void vp9_denoiser_update_frame_info(
  ...
  if (refresh_alt_ref_frame)
    denoiser->refresh_fb_idx[alt_fb_idx] = ALTREF_FRAME;
  if (refresh_golden_frame) denoiser->refresh_fb_idx[gld_fb_idx] = GOLDEN_FRAME;
  if (refresh_last_frame) denoiser->refresh_fb_idx[lst_fb_idx] = LAST_FRAME;
