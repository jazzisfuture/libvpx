...
void vp9_denoise_init_svc(VP9_COMP *cpi, int buffer_idx) {
  if (cpi->svc.spatial_layer_id == cpi->svc.number_spatial_layers - 1) {
    ...
    if (denoiser->frame_idx_denoiser[buffer_idx] == GOLDEN_FRAME)
      copy_frame(&denoiser->running_avg_y[LAST_FRAME],
                 &denoiser->running_avg_y[GOLDEN_FRAME]);
    else if (denoiser->frame_idx_denoiser[buffer_idx] == ALTREF_FRAME)
      copy_frame(&denoiser->running_avg_y[LAST_FRAME],
                 &denoiser->running_avg_y[ALTREF_FRAME]);
    }
  }
}

void vp9_denoiser_update_frame_info(
    VP9_COMP *cpi, VP9_DENOISER *denoiser, YV12_BUFFER_CONFIG src,
  ...
  if (refresh_alt_ref_frame)
    denoiser->frame_idx_denoiser[cpi->alt_fb_idx] = ALTREF_FRAME;
  if (refresh_golden_frame)
    denoiser->frame_idx_denoiser[cpi->gld_fb_idx] = GOLDEN_FRAME;
  if (refresh_last_frame)
    denoiser->frame_idx_denoiser[cpi->lst_fb_idx] = LAST_FRAME;
  ...
  // refreshed on non-zero spatial layers, so we can use it to keep track of
  // denoised last_frame.
