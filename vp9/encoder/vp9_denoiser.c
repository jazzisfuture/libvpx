...
static int sse_thresh(BLOCK_SIZE bs, int increase_denoising);
static int sse_diff_thresh(BLOCK_SIZE bs, int increase_denoising);
static int total_adj_thresh(BLOCK_SIZE bs, int increase_denoising);
...
static VP9_DENOISER_DECISION denoiser_filter(const uint8_t *sig, int sig_stride,
  ...
  if (abs(total_adj) <= total_adj_thresh(bs, increase_denoising)) {
    return FILTER_BLOCK;
  }

  // Otherwise, we try to dampen the filter if the delta is not too high.
  delta = ((abs(total_adj) - total_adj_thresh(bs, increase_denoising)) >> 8) + 1;
  ...
  return (abs(total_adj) > total_adj_thresh(bs, increase_denoising)) ? COPY_BLOCK : FILTER_BLOCK;
}
...
static VP9_DENOISER_DECISION perform_motion_compensation(VP9_DENOISER *denoiser,
  ...
  struct buf_2d saved_pre = filter_mbd->plane[0].pre[0];
  ...
  if (mv_row * mv_row + mv_col * mv_col >
    ...
    sse_diff_thresh_calc = 0;
  } else {
    sse_diff_thresh_calc = sse_diff_thresh(bs, increase_denoising);
  }
  ...
  if (frame != INTRA_FRAME && sse_diff > sse_diff_thresh_calc) {
  ...
  // struct.
  filter_mbd->plane[0].pre[0].buf = denoiser->running_avg_y[frame].y_buffer;
  filter_mbd->plane[1].pre[0].buf = denoiser->running_avg_y[frame].u_buffer;
  filter_mbd->plane[2].pre[0].buf = denoiser->running_avg_y[frame].v_buffer;
  filter_mbd->plane[0].dst.buf = denoiser->mc_running_avg_y.y_buffer;
  filter_mbd->plane[1].dst.buf = denoiser->mc_running_avg_y.u_buffer;
  filter_mbd->plane[2].dst.buf = denoiser->mc_running_avg_y.v_buffer;

  vp9_build_inter_predictors_sb(filter_mbd, mv_row, mv_col, bs);

  // Restore everything to its original state
  filter_mbd->plane[0].pre[0] = saved_pre;
...
void vp9_denoiser_denoise(VP9_DENOISER *denoiser, MACROBLOCK *mb,
  ...
  if (decision == FILTER_BLOCK) {
    decision = denoiser_filter(mb->plane[0].src.buf, mb->plane[0].src.stride,
...
void vp9_denoiser_update_frame_stats(VP9_DENOISER *denoiser, MB_MODE_INFO *mbmi,
  ...
  if (mode == ZEROMV && sse < denoiser->zero_mv_sse) {
    denoiser->zero_mv_sse = sse;
    denoiser->best_zeromv_reference_frame = mbmi->ref_frame[0];
  }
  if (mbmi->mode == NEWMV && sse < denoiser->best_sse) {
    denoiser->best_sse = sse;
    denoiser->best_sse_inter_mode = NEWMV;
...
static int sse_thresh(BLOCK_SIZE bs, int increase_denoising) {
  return 16 * 16 * (increase_denoising ? 60 : 40);
}
...
static int total_adj_thresh(BLOCK_SIZE bs, int increase_denoising) {
  return 16 * 16 * (increase_denoising ? 3 : 2);
}
