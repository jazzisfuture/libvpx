  ...
  for (i = 0; i < label_count && this_segment_rd < bsi->segment_rd; i++) {
    ...
    for (this_mode = LEFT4X4; this_mode <= NEW4X4; this_mode ++) {
      ...
      if (!mbmi->second_ref_frame && this_mode == NEW4X4) {
...

        {
          ...
          if ((cpi->compressor_speed == 0) && (bestsme >> sseshift) > 4000) {
            ...
            thissme = cpi->full_search_sad(x, c, e, &mvp_full,
                                           sadpb, 16, v_fn_ptr,
                                           x->nmvjointcost, (x->e_mbd.allow_high_precision_mv?x->nmvcost_hp:x->nmvcost), bsi->ref_mv);
        ...
        if (bestsme < INT_MAX) {
          ...
          cpi->find_fractional_mv_step(x, c, e, &mode_mv[NEW4X4],
                                       ...
                                       x->nmvjointcost, (x->e_mbd.allow_high_precision_mv?x->nmvcost_hp:x->nmvcost), &distortion, &sse);
      ...
      rate = labels2mode(x, labels, i, this_mode, &mode_mv[this_mode],
                         ...
                         (x->e_mbd.allow_high_precision_mv?x->nmvcost_hp:
                             x->nmvcost));
    ...
    labels2mode(x, labels, i, mode_selected, &mode_mv[mode_selected],
                ...
                bsi->ref_mv, bsi->second_ref_mv, x->nmvjointcost,
                (x->e_mbd.allow_high_precision_mv?x->nmvcost_hp:x->nmvcost));
