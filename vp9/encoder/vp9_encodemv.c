...
static void inc_mvs(const int_mv mv[2],
  ...
  for (i = 0; i < 1 + is_compound; ++i) {
    ...
    const MV diff = { mv[i].as_mv.row - ref->row,
                      mv[i].as_mv.col - ref->col};
    ...
    for (idy = 0; idy < 2; idy += num_4x4_h) {
      for (idx = 0; idx < 2; idx += num_4x4_w) {
        ...
        if (mi->bmi[i].as_mode == NEWMV)
          inc_mvs(mi->bmi[i].as_mv, mbmi, counts);
      }
    }
  } else if (mbmi->mode == NEWMV) {
    inc_mvs(mbmi->mv, mbmi, counts);
  }
}
