...
void vp9_update_mv_count(VP9_COMMON *cm, const MACROBLOCKD *xd
  ...
  if (mbmi->sb_type < BLOCK_8X8) {
    ...
    for (idy = 0; idy < 2; idy += num_4x4_h) {
      for (idx = 0; idx < 2; idx += num_4x4_w) {
...
#if CONFIG_NEWMVREF_SUB8X8
        {
          ...
          ref_mvs[0].as_int = nearest_sub8x8[0].as_int;
          ref_mvs[1].as_int = nearest_sub8x8[1].as_int;
          inc_mvs(mbmi, mi->bmi[i].as_mv, ref_mvs, &cm->counts.mv);
        }
#else
    ...
    {
      int_mv ref_mvs[2];
      ref_mvs[0].as_int = mbmi->ref_mvs[mbmi->ref_frame[0]][0].as_int;
      ref_mvs[1].as_int = mbmi->ref_mvs[mbmi->ref_frame[1]][0].as_int;
      inc_mvs(mbmi, mbmi->mv, ref_mvs, &cm->counts.mv);
    }
#else
