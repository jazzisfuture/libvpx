...
void vp9_update_mv_count(VP9_COMMON *cm, const MACROBLOCKD *xd) {
  ...
  if (mbmi->sb_type < BLOCK_8X8) {
    ...
    for (idy = 0; idy < 2; idy += num_4x4_h) {
      for (idx = 0; idx < 2; idx += num_4x4_w) {
        ...
        if (mi->bmi[i].as_mode == NEWMV)
          ...
          inc_compound_single_mv(mbmi, 1, mi->bmi[i].as_mv,
#if CONFIG_NEWMVREF_SUB8X8
                                 mi->bmi[i].ref_mv,
#else
                                 ref_mv,
          ...
          inc_compound_single_mv(mbmi, 0, mi->bmi[i].as_mv,
#if CONFIG_NEWMVREF_SUB8X8
                                 mi->bmi[i].ref_mv,
#else
                                 ref_mv,
    ...
    if (mbmi->mode == NEWMV)
      ...
      inc_compound_single_mv(mbmi, 1, mbmi->mv, ref_mv, &cm->counts.mv);
    else if (mbmi->mode == NEW_NEARESTMV || mbmi->mode == NEW_NEARMV)
      inc_compound_single_mv(mbmi, 0, mbmi->mv, ref_mv, &cm->counts.mv);
#endif
  }
}
