...
static void encode_mv_component(vp9_writer* w, int comp,
  ...
  if (mv_class == MV_CLASS_0) {
    ...
    int i;
    const int b = mv_class + CLASS0_BITS - 1;  // number of bits
    for (i = 0; i < b; ++i)
...
void vp9_encode_mv(vp9_writer* w, const MV* const mv,
                   const MV* const ref, const nmv_context* mvctx, int usehp) {
  const MV_JOINT_TYPE j = vp9_get_mv_joint(mv);
  usehp = usehp && vp9_use_nmv_hp(ref);

  write_token(w, vp9_mv_joint_tree, mvctx->joints, &vp9_mv_joint_encodings[j]);
  if (mv_joint_vertical(j))
    encode_mv_component(w, mv->row, &mvctx->comps[0], usehp);

  if (mv_joint_horizontal(j))
    encode_mv_component(w, mv->col, &mvctx->comps[1], usehp);
}
