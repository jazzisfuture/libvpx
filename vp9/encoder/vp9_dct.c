...
void vp9_fdct16x16_noscale_c(const int16_t *input, tran_low_t *output,
  ...
  for (pass = 0; pass < 2; ++pass) {
    ...
    for (i = 0; i < 16; i++) {
      if (0 == pass) {
        // Calculate input for the first 8 results.
        input[0] = (in_pass0[0 * stride] + in_pass0[15 * stride]);
        input[1] = (in_pass0[1 * stride] + in_pass0[14 * stride]);
        input[2] = (in_pass0[2 * stride] + in_pass0[13 * stride]);
        input[3] = (in_pass0[3 * stride] + in_pass0[12 * stride]);
        input[4] = (in_pass0[4 * stride] + in_pass0[11 * stride]);
        input[5] = (in_pass0[5 * stride] + in_pass0[10 * stride]);
        input[6] = (in_pass0[6 * stride] + in_pass0[ 9 * stride]);
        input[7] = (in_pass0[7 * stride] + in_pass0[ 8 * stride]);
        // Calculate input for the next 8 results.
        step1[0] = (in_pass0[7 * stride] - in_pass0[ 8 * stride]);
        step1[1] = (in_pass0[6 * stride] - in_pass0[ 9 * stride]);
        step1[2] = (in_pass0[5 * stride] - in_pass0[10 * stride]);
        step1[3] = (in_pass0[4 * stride] - in_pass0[11 * stride]);
        step1[4] = (in_pass0[3 * stride] - in_pass0[12 * stride]);
        step1[5] = (in_pass0[2 * stride] - in_pass0[13 * stride]);
        step1[6] = (in_pass0[1 * stride] - in_pass0[14 * stride]);
        step1[7] = (in_pass0[0 * stride] - in_pass0[15 * stride]);
      } else {
        // Calculate input for the first 8 results.
        input[0] = ((in[0 * 16] + 1) >> 2) + ((in[15 * 16] + 1) >> 2);
        input[1] = ((in[1 * 16] + 1) >> 2) + ((in[14 * 16] + 1) >> 2);
...
#if CONFIG_WAVELETS
void vp9_fdct32x32_noscale_c(const int16_t *input, tran_low_t *out, int stride) {
  int i, j;
  tran_high_t output[32 * 32];
