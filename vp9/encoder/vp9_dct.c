...
#define analysis_row analysis_26_row
#define analysis_col analysis_26_col
...
static void dyadic_analyze(int levels, int width, int height,
  ...
  for (lv = 0; lv < levels; lv++) {
    ...
    for (i = 0; i < nh; i++) {
      ...
      analysis_row(nw, buffer, &c[i * pitch_c], &c[i * pitch_c] + hw);
    }
    for (j = 0; j < nw; j++) {
      for (i = 0; i < nh; i++)
        buffer[i + nh] = c[i * pitch_c + j];
      analysis_col(nh, buffer + nh, buffer, buffer + hh);
      for (i = 0; i < nh; i++)
