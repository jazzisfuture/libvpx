...
static void set_rt_speed_feature_framesize_independent(
  ...
  if (speed <= 3 && cpi->oxcf.aq_mode == CYCLIC_REFRESH_AQ)
    cpi->oxcf.aq_mode = 0;
  // For all speeeds for rt mode: if the rc_mode changed (was good/best quality
  // on previous frame and now is realtime) set nonrd_keyframe to 1 to avoid
  // entering rd pickmode. This causes issues, such as: b/310663186.
