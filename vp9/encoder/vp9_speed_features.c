...
static void set_rt_speed_feature_framesize_independent(
  ...
  if (!sf->adaptive_rd_thresh_row_mt && cpi->row_mt && cpi->num_workers > 1) {
...
void vp9_set_speed_features_framesize_dependent(VP9_COMP *cpi) {
  ...
  // It can be used for speed >= 5 in realtime since adaptive_rd_thresh is
  // defined per-row for non-rd.
  if (!(oxcf->mode == REALTIME && oxcf->speed >= 5) && cpi->row_mt_bit_exact)
    sf->adaptive_rd_thresh = 0;
