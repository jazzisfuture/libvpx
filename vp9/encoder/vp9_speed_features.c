...
#define MESH_SEARCH_DENSITY_LEVELS 3  // Max speed setting for mesh motion method
static MESH_PATTERN
    good_quality_mesh_patterns[MESH_SEARCH_DENSITY_LEVELS][MAX_MESH_STEP] = {
      { { 64, 8 }, { 28, 4 }, { 15, 1 }, { 7, 1 } },
      { { 64, 8 }, { 14, 2 }, { 7, 1 }, { 7, 1 } },
      { { 64, 16 }, { 24, 8 }, { 12, 4 }, { 7, 1 } },
    };
...
static void set_good_speed_feature_framesize_independent(VP9_COMP *cpi,
  ...
  sf->exhaustive_searches_thresh =
      (cpi->twopass.fr_content_type == FC_GRAPHICS_ANIMATION) ? (1 << 22) :
          INT_MAX;

  if (cpi->twopass.fr_content_type == FC_GRAPHICS_ANIMATION) {
    for (i = 0; i < MAX_MESH_STEP; ++i) {
      sf->mesh_patterns[i].range = good_quality_mesh_patterns[0][i].range;
      sf->mesh_patterns[i].interval = good_quality_mesh_patterns[0][i].interval;
    }
  }
  ...
  if (speed >= 2) {
    ...
    if (cpi->twopass.fr_content_type == FC_GRAPHICS_ANIMATION) {
      for (i = 0; i < MAX_MESH_STEP; ++i) {
        sf->mesh_patterns[i].range = good_quality_mesh_patterns[1][i].range;
        sf->mesh_patterns[i].interval =
            good_quality_mesh_patterns[1][i].interval;
      }
    }
  }
  ...
  if (speed >= 3) {
    ...
    if (cpi->twopass.fr_content_type == FC_GRAPHICS_ANIMATION) {
      for (i = 0; i < MAX_MESH_STEP; ++i) {
        sf->mesh_patterns[i].range = good_quality_mesh_patterns[2][i].range;
        sf->mesh_patterns[i].interval =
            good_quality_mesh_patterns[2][i].interval;
      }
    }
  }
