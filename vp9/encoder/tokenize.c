...
static void tokenize_b(VP9_COMP *cpi,
  ...
  do {
    ...
    if (vp9_coef_encodings[t->Token].Len - t->skip_eob_node <= 0) {
      printf("mode %d tx-type %d tx-size %d plane-type %d eob %d c %d [%d %d]\n",
             xd->mode_info_context->mbmi.mode, tx_type, tx_size, type, eob, c,
             vp9_coef_encodings[t->Token].Len, t->skip_eob_node);
    }
...
void vp9_tokenize_mb(VP9_COMP *cpi,
  ...
  if (has_y2_block && get_tx_type_4x4(xd, &xd->block[0]) != DCT_DCT)
    has_y2_block = 0;
