...
#if CONFIG_SB8X8
#define BLOCK_SIZE_I4X4 BLOCK_SIZE_SB8X8
#else
#define BLOCK_SIZE_I4X4 BLOCK_SIZE_MB16X16
#endif
...
static void encode_intra4x4block(MACROBLOCK *x, int ib) {
  ...
  TX_TYPE tx_type;
  uint8_t* const src =
      raster_block_offset_uint8(xd, BLOCK_SIZE_I4X4, 0, ib,
                                x->plane[0].src.buf, x->plane[0].src.stride);
  uint8_t* const dst =
      raster_block_offset_uint8(xd, BLOCK_SIZE_I4X4, 0, ib,
                                xd->plane[0].dst.buf, xd->plane[0].dst.stride);
  int16_t* const src_diff =
      raster_block_offset_int16(xd, BLOCK_SIZE_I4X4, 0, ib,
                                x->plane[0].src_diff);
  int16_t* const diff =
      raster_block_offset_int16(xd, BLOCK_SIZE_I4X4, 0, ib,
                                xd->plane[0].diff);
