...
int vp9_one_pass_cbr_svc_start_layer(VP9_COMP *const cpi) {
  ...
  if (cpi->svc.temporal_layering_mode != VP9E_TEMPORAL_LAYERING_MODE_BYPASS &&
    ...
    // from previous superframe) was dropped, make sure the lst_fb_idx corresponds
    // to the (last) encoded TL0 frame.
    cpi->lst_fb_idx  = cpi->svc.lst_fb_idx_tl0[cpi->svc.spatial_layer_id];
  }
