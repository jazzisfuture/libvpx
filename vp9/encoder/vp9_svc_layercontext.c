...
static void set_flags_and_fb_idx_bypass_via_set_ref_frame_config(
  ...
  cpi->ext_refresh_alt_ref_frame = 0;
  // // TODO(marpan/jianj): keep this setting of the encoder
  // // refresh_last/golden/alt_frame flags for now. They shouldn't
  // // be needed for flexible mode, but there a few places in encoder
  // // that uses these flags, so we need to fix that first.
  // for (ref = 0; ref < REF_FRAMES; ++ref) {
  //   cpi->ext_refresh_last_frame =
  //       (ref == cpi->lst_fb_idx && (svc->update_buffer_slot[sl] & (1 <<
  //       ref)));
  //   cpi->ext_refresh_golden_frame =
  //       (ref == cpi->gld_fb_idx && (svc->update_buffer_slot[sl] & (1 <<
  //       ref)));
  //   cpi->ext_refresh_alt_ref_frame =
  //       (ref == cpi->alt_fb_idx && (svc->update_buffer_slot[sl] & (1 <<
  //       ref)));
  // }
...
int vp9_one_pass_cbr_svc_start_layer(VP9_COMP *const cpi) {
  ...
  if (svc->temporal_layering_mode == VP9E_TEMPORAL_LAYERING_MODE_BYPASS) {
    int ref;
    for (ref = 0; ref < REF_FRAMES; ++ref)
      if (svc->update_buffer_slot[svc->spatial_layer_id] & (1 << ref))
        svc->non_reference_frame = 0;
  }
...
void vp9_svc_update_ref_frame_bypass(VP9_COMP *const cpi) {
