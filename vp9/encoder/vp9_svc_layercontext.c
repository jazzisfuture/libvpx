...
int vp9_svc_lookahead_push(const VP9_COMP *const cpi, struct lookahead_ctx *ctx,
  ...
  for (i = 0; i < cpi->svc.number_spatial_layers; ++i)
    buf->svc_params[i] = cpi->svc.layer_context[i].params_before_encoding;

  return 0;
}

static int get_buf_layer_id(VP9_COMP *const cpi, struct lookahead_entry *buf) {
  int layer_id;
  ...
  return (layer_id == cpi->svc.number_spatial_layers) ? -1 : layer_id;
}

static int copy_svc_params(VP9_COMP *const cpi, struct lookahead_entry *buf) {
  int layer_id = get_buf_layer_id(cpi, buf);
