...
void vp9_svc_update_ref_frame_buffer_idx(VP9_COMP *const cpi) {
  ...
  if (svc->spatial_layer_id == 0) {
    if (cpi->refresh_last_frame || (cpi->ref_frame_flags & flag_list[LAST_FRAME]))
      svc->fb_idx_base[cpi->lst_fb_idx] = 1;
    if (cpi->refresh_golden_frame || (cpi->ref_frame_flags & flag_list[GOLDEN_FRAME]))
      svc->fb_idx_base[cpi->gld_fb_idx] = 1;
    if (cpi->refresh_alt_ref_frame || (cpi->ref_frame_flags & flag_list[ALTREF_FRAME]))
...
int vp9_svc_check_setting_intra_only(VP9_COMP *const cpi) {
  ...
  if (cpi->common.current_video_frame == 0 && svc->number_spatial_layers > 1 &&
    ...
    int count = 0;
    for (i = 0; i < 8; ++i) {
      printf("%d %d %d \n", i, svc->fb_idx_base[i], svc->fb_idx_nonbase[i]);
