...
void vp9_svc_constrain_inter_layer_pred(VP9_COMP *const cpi) {
  ...
  // Check for disabling inter-layer prediction if INTER_LAYER_PRED_ON_CONSTRAINED
  // is enabled. If the reference for inter-layer prediction (the reference that
  // is scaled) is not the previous spatial layer from the same superframe, then
  // we disable inter-layer prediction.
  if (cpi->svc.disable_inter_layer_pred == INTER_LAYER_PRED_ON_CONSTRAINED) {
    // We only use LAST and GOLDEN for prediction in real-time mode, so check both.    
    MV_REFERENCE_FRAME ref_frame;
    for (ref_frame = LAST_FRAME; ref_frame <= GOLDEN_FRAME; ref_frame++) {
      ...
      if (vp9_is_scaled(scale_fac)) {
        ...
        // This condition is verified by checking if the current frame buffer index
        // is equal to any of the slots for the previous spatial layer, and if so,
        // check if that slot was updated/refreshed. If that is the case, then this
        // reference is valid for inter-layer prediction undder the mode:
        // svc.disable_inter_layer_pred == INTER_LAYER_PRED_ON_CONSTRAINED.
        int fb_idx =
          ref_frame == LAST_FRAME ? cpi->lst_fb_idx : cpi->gld_fb_idx;
        int ref_flag =
          ref_frame == LAST_FRAME ? VP9_LAST_FLAG : VP9_GOLD_FLAG;
