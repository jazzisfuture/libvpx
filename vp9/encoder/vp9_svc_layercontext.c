...
void vp9_svc_check_spatial_layer_sync(VP9_COMP *const cpi) {
  ...
  // already sync frames.
  if (!svc->layer_context[svc->temporal_layer_id].is_key_frame) {
    // If any of the layers in the superframe is a sync frame, then
    // reset pattern counters and reset to base temporal layer
    // (as if it were key frame). And reset the golden update, if
    // golden is used a second temporal reference.
    int sl;
    for (sl = 1; sl < svc->number_spatial_layers; sl++) {
      if (svc->spatial_layer_sync[svc->spatial_layer_id]) {
