...
void update_svc_ref_flags(VP9_COMP *const cpi) {
  SVC *const svc = &cpi->svc;
  ...
  svc->update_last[sl] = cpi->ext_refresh_last_frame;
  svc->update_golden[sl] = cpi->ext_refresh_golden_frame;
  svc->update_altref[sl] = cpi->ext_refresh_alt_ref_frame;
  svc->reference_last[sl] = (cpi->ref_frame_flags & flag_list[LAST_FRAME]);
  svc->reference_golden[sl] = (cpi->ref_frame_flags & flag_list[GOLDEN_FRAME]);
  svc->reference_altref[sl] = (cpi->ref_frame_flags & flag_list[ALTREF_FRAME]);
}
