...
int vp9_one_pass_cbr_svc_start_layer(VP9_COMP *const cpi) {
  ...
  // at SL0 frame) check all spatial layers (starting from top) for possible drop,
  // and if so, set a flag to force drop of that layer and all its lower layers.
  if (svc->spatial_layer_to_encode == 0) {
    int sl;
    for (sl = 0; sl < svc->number_spatial_layers ; sl++)
      svc->force_drop_reverse_constrained[sl] = 0;
    if (svc->framedrop_mode == REVERSE_CONSTRAINED_DROP) {
      for (sl = svc->number_spatial_layers - 1; sl >= 0; sl--) {
        int layer = sl * svc->number_temporal_layers + svc->temporal_layer_id;
        LAYER_CONTEXT *const lc = &svc->layer_context[layer];
        cpi->rc = lc->rc;
        cpi->oxcf.target_bandwidth = lc->target_bandwidth;
        if (vp9_rc_drop_frame(cpi, 1)) {
          int sl2;
