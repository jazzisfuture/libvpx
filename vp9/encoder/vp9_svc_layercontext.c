...
void vp9_init_layer_context(VP9_COMP *const cpi) {
  ...
  for (sl = 0; sl < oxcf->ss_number_layers; ++sl) {
    ...
    LAYER_CONTEXT *const lc = &svc->layer_context[sl * oxcf->ts_number_layers + tl];
    RATE_CONTROL *const lrc = &lc->rc;
    int i;
    layer = sl * oxcf->ts_number_layers + tl;
    ...
    lrc->bits_off_target = lrc->buffer_level;
      lrc->starting_buffer_level = oxcf->starting_buffer_level_ms * lc->target_bandwidth / 1000;
      lrc->optimal_buffer_level = (oxcf->optimal_buffer_level_ms == 0) ? lc->target_bandwidth / 8
                                                  : oxcf->optimal_buffer_level_ms * lc->target_bandwidth / 1000;
      lrc->maximum_buffer_size = (oxcf->maximum_buffer_size_ms == 0) ? lc->target_bandwidth / 8
                                                 : oxcf->maximum_buffer_size_ms * lc->target_bandwidth / 1000;
    }
  }
...
void vp9_update_layer_context_change_config(VP9_COMP *const cpi,
  ...
  int sl, tl, layer, spatial_layer_target;
  float bitrate_alloc = 1.0;

  if (svc->temporal_layering_mode != VP9E_TEMPORAL_LAYERING_MODE_NOLAYERING) {
    for (sl = 0; sl < oxcf->ss_number_layers; ++sl) {
      svc->layer_context[sl * oxcf->ts_number_layers].target_bandwidth = oxcf->layer_target_bitrate[sl * oxcf->ts_number_layers];
      svc->layer_context[sl * oxcf->ts_number_layers + 1].target_bandwidth = oxcf->layer_target_bitrate[sl * oxcf->ts_number_layers + 1];
      spatial_layer_target = svc->layer_context[sl * oxcf->ts_number_layers + 2].target_bandwidth = oxcf->layer_target_bitrate[sl * oxcf->ts_number_layers + 2];
      for (tl = 0; tl < oxcf->ts_number_layers; ++tl) {
        LAYER_CONTEXT *const lc = &svc->layer_context[sl * oxcf->ts_number_layers + tl];
        RATE_CONTROL *const lrc = &lc->rc;
        layer = sl * oxcf->ts_number_layers + tl;
...
static LAYER_CONTEXT *get_layer_context(VP9_COMP *const cpi) {
  return &cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers + cpi->svc.temporal_layer_id];
}
...
void vp9_update_temporal_layer_framerate(VP9_COMP *const cpi) {
  ...
  RATE_CONTROL *const lrc = &lc->rc;
  const int st_idx = svc->spatial_layer_id * svc->number_temporal_layers + svc->temporal_layer_id; // index into spatial+temporal arrays.
  ...
  if (tl == 0) {
    ...
    const int prev_layer_target_bandwidth = oxcf->layer_target_bitrate[st_idx - 1];
    lc->avg_frame_size =
...
void vp9_inc_frame_in_layer(VP9_COMP *const cpi) {
  LAYER_CONTEXT *const lc =
      &cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers];
...
int vp9_is_upper_layer_key_frame(const VP9_COMP *const cpi) {
  return is_two_pass_svc(cpi) &&
         ...
         cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers + cpi->svc.temporal_layer_id].is_key_frame;
}
...
static void set_flags_and_fb_idx_for_temporal_mode3(VP9_COMP *const cpi) {
  ...
  frame_num_within_temporal_struct = cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers].current_video_frame_in_layer % 4;
  temporal_id = cpi->svc.temporal_layer_id = (frame_num_within_temporal_struct&1)?2:(frame_num_within_temporal_struct>>1);
  cpi->ext_refresh_last_frame = cpi->ext_refresh_golden_frame = cpi->ext_refresh_alt_ref_frame = 0;
  if (!temporal_id) {
...
static void set_flags_and_fb_idx_for_temporal_mode2(VP9_COMP *const cpi) {
  ...
  temporal_id = cpi->svc.temporal_layer_id =
      cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers].current_video_frame_in_layer & 1;
  cpi->ext_refresh_last_frame = cpi->ext_refresh_golden_frame = cpi->ext_refresh_alt_ref_frame = 0;
  if (!temporal_id) {
    cpi->ext_refresh_frame_flags_pending = 1;
    cpi->ext_refresh_last_frame = 1;
...
static void set_flags_and_fb_idx_for_temporal_mode_noLayering(VP9_COMP *const cpi) {
  int spatial_id;
...
int vp9_one_pass_cbr_svc_start_layer(VP9_COMP *const cpi) {
  ...
  if (cpi->svc.temporal_layering_mode == VP9E_TEMPORAL_LAYERING_MODE_0212) {
    ...
    lc = &cpi->svc.layer_context[cpi->svc.spatial_layer_id * cpi->svc.number_temporal_layers + cpi->svc.temporal_layer_id];
  }
  else if (cpi->svc.temporal_layering_mode == VP9E_TEMPORAL_LAYERING_MODE_NOLAYERING){
    set_flags_and_fb_idx_for_temporal_mode_noLayering(cpi);
    lc = &cpi->svc.layer_context[cpi->svc.spatial_layer_id];
  }
  else if (cpi->svc.temporal_layering_mode == VP9E_TEMPORAL_LAYERING_MODE_0101){
