...
void vp9_init_layer_context(VP9_COMP *const cpi) {
  ...
  for (sl = 0; sl < oxcf->ss_number_layers; ++sl) {
    ...
    LAYER_CONTEXT *const lc = &svc->layer_context[sl * oxcf->ts_number_layers +
                                                  tl];
    ...
    lrc->bits_off_target = lrc->buffer_level;
      lrc->starting_buffer_level = oxcf->starting_buffer_level_ms *
          lc->target_bandwidth / 1000;
      lrc->optimal_buffer_level = (oxcf->optimal_buffer_level_ms == 0) ?
          lc->target_bandwidth / 8 : oxcf->optimal_buffer_level_ms *
                                     lc->target_bandwidth / 1000;
      lrc->maximum_buffer_size = (oxcf->maximum_buffer_size_ms == 0) ?
          lc->target_bandwidth / 8 : oxcf->maximum_buffer_size_ms *
                                     lc->target_bandwidth / 1000;
    }
  }
...
void vp9_update_layer_context_change_config(VP9_COMP *const cpi,
  ...
  if (svc->temporal_layering_mode != VP9E_TEMPORAL_LAYERING_MODE_NOLAYERING) {
    for (sl = 0; sl < oxcf->ss_number_layers; ++sl) {
      svc->layer_context[sl * oxcf->ts_number_layers].target_bandwidth =
          oxcf->layer_target_bitrate[sl * oxcf->ts_number_layers];
      svc->layer_context[sl * oxcf->ts_number_layers + 1].target_bandwidth =
          oxcf->layer_target_bitrate[sl * oxcf->ts_number_layers + 1];
      spatial_layer_target =
          svc->layer_context[sl * oxcf->ts_number_layers + 2].target_bandwidth =
              oxcf->layer_target_bitrate[sl * oxcf->ts_number_layers + 2];
      for (tl = 0; tl < oxcf->ts_number_layers; ++tl) {
...
void vp9_update_temporal_layer_framerate(VP9_COMP *const cpi) {
  ...
  const int st_idx = svc->spatial_layer_id * svc->number_temporal_layers +
      svc->temporal_layer_id; // index into spatial+temporal arrays.
