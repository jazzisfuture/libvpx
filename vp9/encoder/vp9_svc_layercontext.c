...
int vp9_one_pass_cbr_svc_start_layer(VP9_COMP *const cpi) {
...

  {
    ...
    cpi->svc.lst_fb_idx[sl] = cpi->lst_fb_idx;
    cpi->svc.gld_fb_idx[sl] = cpi->gld_fb_idx;
    cpi->svc.alt_fb_idx[sl] = cpi->alt_fb_idx;

    cpi->svc.update_last[sl] = cpi->ext_refresh_last_frame;
    cpi->svc.update_golden[sl] = cpi->ext_refresh_golden_frame;
    cpi->svc.update_altref[sl] = cpi->ext_refresh_alt_ref_frame;
    cpi->svc.reference_last[sl] =
        (cpi->ref_frame_flags & flag_list[LAST_FRAME]);
    cpi->svc.reference_golden[sl] =
        (cpi->ref_frame_flags & flag_list[GOLDEN_FRAME]);
    cpi->svc.reference_altref[sl] =
