...
static void set_flags_and_fb_idx_bypass_via_set_ref_frame_config(
  ...
  for (ref = 0; ref < REF_FRAMES; ++ref) {
    if (ref == cpi->lst_fb_idx && svc->update_reference[ref][sl] == 1)
      cpi->ext_refresh_last_frame = 1;
    if (ref == cpi->gld_fb_idx && svc->update_reference[ref][sl] == 1)
      cpi->ext_refresh_golden_frame = 1;
    if (ref == cpi->alt_fb_idx && svc->update_reference[ref][sl] == 1)
      cpi->ext_refresh_alt_ref_frame = 1;
  }
    ...
    for (ref_frame = LAST_FRAME; ref_frame <= GOLDEN_FRAME; ref_frame++) {
      ...
      if (vp9_is_scaled(scale_fac)) {
        ...
        if ((fb_idx == svc->lst_fb_idx[sl - 1] &&
             svc->update_reference[fb_idx][sl - 1]) ||
