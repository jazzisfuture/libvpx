...
void vp9_svc_constrain_inter_layer_pred(VP9_COMP *const cpi) {
  ...
  // If any of the referenes are not the previous spatial layer from the same
  // superframe, then we disable inter-layer prediction. Only need to check when
  // inter_layer prediction is not set to OFF mode.
  if (svc->temporal_layering_mode != VP9E_TEMPORAL_LAYERING_MODE_BYPASS &&
      svc->disable_inter_layer_pred != INTER_LAYER_PRED_OFF) {
