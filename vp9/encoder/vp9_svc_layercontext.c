...
void vp9_copy_flags_ref_update_idx(VP9_COMP *const cpi) {
  ...
  svc->reference_last[sl] =
      (uint8_t)is_referenced(cpi->ref_frame_flags, LAST_FRAME);
  svc->reference_golden[sl] =
      (uint8_t)is_referenced(cpi->ref_frame_flags, GOLDEN_FRAME);
  svc->reference_altref[sl] =
      (uint8_t)is_referenced(cpi->ref_frame_flags, ALTREF_FRAME);
}
...
void vp9_svc_constrain_inter_layer_pred(VP9_COMP *const cpi) {
  ...
  if ((svc->disable_inter_layer_pred == INTER_LAYER_PRED_OFF_NONKEY &&
    ...
    for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
      ...
      if (yv12 != NULL && is_referenced(cpi->ref_frame_flags, ref_frame)) {
        const struct scale_factors *const scale_fac =
            &cm->frame_refs[ref_frame - 1].sf;
        if (vp9_is_scaled(scale_fac)) {
          set_ref_flags(&cpi->ref_frame_flags, ref_frame, 0);
