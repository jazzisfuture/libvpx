  ...
  if (svc->temporal_layering_mode != VP9E_TEMPORAL_LAYERING_MODE_BYPASS &&
    ...
    if (!cpi->svc.layer_context[cpi->svc.temporal_layer_id].is_key_frame) {
      ...
      if (svc->temporal_layer_id == 0)
        ...
        assert(svc->fb_idx_temporal_layer_id[cpi->lst_fb_idx] < svc->temporal_layer_id);
      if (svc->spatial_layer_id > 0) {
        // Non-base spatial only predicts from lower spatial layer with same temporal_id.
        assert(svc->fb_idx_spatial_layer_id[cpi->gld_fb_idx] == svc->spatial_layer_id  - 1);
        assert(svc->fb_idx_temporal_layer_id[cpi->gld_fb_idx] == svc->temporal_layer_id);
      }
    } else if (svc->spatial_layer_id > 0) {
      // Only 1 reference for frame whose base is key; reference may be LAST or GOLDEN,
      // so we check both.
      if (cpi->ref_frame_flags & VP9_LAST_FLAG) {
        assert(svc->fb_idx_spatial_layer_id[cpi->lst_fb_idx] == svc->spatial_layer_id  - 1);
        assert(svc->fb_idx_temporal_layer_id[cpi->lst_fb_idx] == svc->temporal_layer_id);
