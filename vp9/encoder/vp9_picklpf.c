...
static int get_max_filter_level(VP9_COMP *cpi) {
  return vp9_get_const_twopass(cpi)->section_intra_rating > 8 ?
         MAX_LOOP_FILTER * 3 / 4 : MAX_LOOP_FILTER;
}
...
static void search_filter_level(const YV12_BUFFER_CONFIG *sd, VP9_COMP *cpi,
  ...
  while (filter_step > 0) {
    ...
    if (vp9_get_const_twopass(cpi)->section_intra_rating < 20)
      bias = bias * vp9_get_const_twopass(cpi)->section_intra_rating / 20;
