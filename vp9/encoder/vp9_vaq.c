...
#define ENERGY_IN_BOUNDS(energy) assert(energy >= ENERGY_MIN && energy <= ENERGY_MAX)

double q_ratio[ENERGY_SPAN] = { 1, 1, 1, 1, 1, 1, 1 };
double rdmult_ratio[ENERGY_SPAN] = { 1, 1, 1, 1, 1, 1, 1 };
int segment_id[ENERGY_SPAN] = { 5, 3, 1, 0, 2, 4, 6 };
...
void vp9_vaq_frame_setup(VP9_COMP *cpi) {
  ...
  int base_rdmult = vp9_compute_rd_mult(cpi, cm->base_qindex + cm->y_dc_delta_q);
  int i;
  ...
  for (i = ENERGY_MIN; i <= ENERGY_MAX; i++) {
    ...
    qindex_delta = compute_qdelta(cpi, base_q, base_q * Q_RATIO(i));
...
int vp9_block_energy(VP9_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bs) {
  ...
  double energy = (log2(var + 1) - 14.427f)*3.0/5.0;
  return clamp(round(energy), -3, 3);
}
