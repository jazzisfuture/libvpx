...
void vp9_quantize_fp_vsx(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  eob = vec_max(vec_or(vec_vsx_ld(0, iscan_ptr), zero_coeff0),
                vec_or(vec_vsx_ld(16, iscan_ptr), zero_coeff1));

  if (n_coeffs > 16) {
    int16x8_t coeff2, qcoeff2, dqcoeff2, eob2;
    ...
    do {
      ...
      eob = vec_max(eob, vec_or(vec_vsx_ld(off0, iscan_ptr), zero_coeff0));
      eob2 = vec_max(vec_or(vec_vsx_ld(off1, iscan_ptr), zero_coeff1),
                     vec_or(vec_vsx_ld(off2, iscan_ptr), zero_coeff2));
