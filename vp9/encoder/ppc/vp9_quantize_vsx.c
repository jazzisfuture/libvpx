...
void vp9_quantize_fp_vsx(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  int index = 16;
  int16x8_t qcoeff0, qcoeff1, dqcoeff0, dqcoeff1, scan, eob;
  ...
  int16x8_t coeff1 = vec_vsx_ld(0, coeff_ptr + 8);
  ...
  vec_vsx_st(qcoeff1, 0, qcoeff_ptr + 8);
  dqcoeff1 = vec_mladd(qcoeff1, dequant, vec_zeros_s16);
  vec_vsx_st(dqcoeff1, 0, dqcoeff_ptr + 8);
  ...
  eob = vec_max(
      ...
      vec_andc(vec_add(vec_vsx_ld(0, iscan_ptr + 8), vec_ones_s16),
               zero_coeff1));
  ...
  while (index < n_coeffs) {
    ...
    zero_coeff0 = vec_cmpeq(qcoeff0, vec_zeros_s16);
    eob = vec_max(eob, vec_andc(vec_add(scan, vec_ones_s16), zero_coeff0));
    index += 8;
  }
