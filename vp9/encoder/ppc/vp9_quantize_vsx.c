...
void vp9_quantize_fp_vsx(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  if (n_coeffs > 16) {
    ...
    do {
      ...
      eob = vec_max(eob,
                    vec_andc(vec_add(vec_vsx_ld(off0, iscan_ptr), vec_ones_s16),
                             zero_coeff0));
      eob2 =
          vec_max(vec_andc(vec_add(vec_vsx_ld(off1, iscan_ptr), vec_ones_s16),
                           zero_coeff1),
                  vec_andc(vec_add(vec_vsx_ld(off2, iscan_ptr), vec_ones_s16),
                           zero_coeff2));
