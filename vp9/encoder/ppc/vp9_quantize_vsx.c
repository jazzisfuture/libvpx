...
void vp9_quantize_fp_vsx(const tran_low_t *coeff_ptr, intptr_t n_coeffs,
  ...
  while (index < n_coeffs) {
    int16x8_t coeff2, qcoeff2, dqcoeff2;
    bool16x8_t zero_coeff2;

    coeff0 = vec_vsx_ld(0, coeff_ptr + index);
    coeff1 = vec_vsx_ld(16, coeff_ptr + index);
    coeff2 = vec_vsx_ld(32, coeff_ptr + index);
    ...
    vec_vsx_st(qcoeff1, 16, qcoeff_ptr + index);
    vec_vsx_st(qcoeff2, 32, qcoeff_ptr + index);

    dqcoeff0 = vec_mladd(qcoeff0, dequant, vec_zeros_s16);
    vec_vsx_st(dqcoeff0, 0, dqcoeff_ptr + index);
    dqcoeff1 = vec_mladd(qcoeff1, dequant, vec_zeros_s16);
    vec_vsx_st(dqcoeff1, 16, dqcoeff_ptr + index);
    dqcoeff2 = vec_mladd(qcoeff2, dequant, vec_zeros_s16);
    vec_vsx_st(dqcoeff2, 32, dqcoeff_ptr + index);
    ...
    eob = vec_max(
        eob, vec_andc(vec_add(vec_vsx_ld(0, iscan_ptr + index), vec_ones_s16),
                      zero_coeff0));
    eob = vec_max(
        eob, vec_andc(vec_add(vec_vsx_ld(16, iscan_ptr + index), vec_ones_s16),
                      zero_coeff1));
    eob = vec_max(
        eob, vec_andc(vec_add(vec_vsx_ld(32, iscan_ptr + index), vec_ones_s16),
                      zero_coeff2));
    index += 24;
  }
