...
static void blur_img(unsigned char *img, int width, int height,
  ...
  convolve(smoothing_kernel, img, sizeof(*img), smooth_img,
           smoothing_size, smoothing_size, width, height, stride, smooth_stride);
...
static void optical_flow_per_pixel(double *dx, double *dy, double *dt,
  ...
  flow->confidence[loc_x + loc_y * width] = 1 / (det -
                                            (0.01 * (M[0] + M[3]) *
                                            (M[0] + M[3])));
...
static void iterative_refinement(unsigned char *ref, double *smooth_frm,
  ...
  for (n = 0; n < n_refinements; ++n) {
    ...
    blur_img(estimate, width, height, width, smooth_estimate, width, 11, 1.5, 1);
...
void compute_flow(unsigned char *frm, unsigned char *ref,
  ...
  if (n_levels == 1) {
    ...
    iterative_refinement(ref, smooth_frm, dx_frm, dy_frm, dt, &flow, width, height,
                         ref_stride, width, width, 3);
  } else {
    int i, j, k;
    ...
    for (i = n_levels - 1; i >= 0; --i) {
      ...
      blur_img(frm_pyramid->levels[i], w, h, s_f, smooth_frm, width, blur_size,
               blur_sig, 1);
      differentiate((unsigned char*)smooth_frm, sizeof(*smooth_frm), w, h, width,
                    dx_frm, dy_frm, width);
