...
static void differentiate(unsigned char *img, size_t elem_size, int width,
                          int height, int stride, double *dx_img,
                          double *dy_img, int deriv_stride) {
...
static double selection(double *vals, int length, int k, unsigned int seed) {
  ...
  pivot_ind = (int) ((length / ((double)RAND_MAX)) * rand_r(&seed_copy));
...
static void median_filter(unsigned char *source, double *filtered,
                          size_t elem_size, int block_size, int width,
                          int height, int source_stride, int filtered_stride) {
  ...
  unsigned int seed = 145;//(unsigned int)*source;
  ...
  for (block_y = 0; block_y < height; ++block_y)
    for (block_x = 0; block_x < width; ++block_x) {
      ...
      loc = source + ((block_x + block_y * source_stride) * elem_size);
      ...
      for (j = -pad_size; j <= pad_size; ++j)
        for (i = -pad_size; i <= pad_size; ++i) {
          ...
          loc = source + ((imx + imy * source_stride) * elem_size);
...
static inline void pixelwise_warp_mse(unsigned char *frm, unsigned char *ref,
...
static inline void pointwise_matrix_mult(unsigned char *mat1,
                                         unsigned char *mat2, size_t elem_size,
                                         double *prod, int width, int height,
  ...
  for (j = 0; j < height; ++j)
    for (i = 0; i < width; ++i) {
      ...
      prod[i + j * prodstride] = val1 * val2;
    }
}

static inline void pointwise_matrix_sub(unsigned char *mat1,
                                        unsigned char *mat2, size_t elem_size,
  ...
  for (j = 0; j < height; ++j)
    for (i = 0; i < width; ++i) {
      ...
      val1 = CHAR_TO_DOUBLE(elem_size, loc1);
      val2 = CHAR_TO_DOUBLE(elem_size, loc2);
      diff[i + j * diffstride] = val1 - val2;
    }
}

static inline void pointwise_matrix_add(unsigned char *mat1,
                                        unsigned char *mat2, size_t elem_size,
  ...
  for (j = 0; j < height; ++j)
    for (i = 0; i < width; ++i) {
      ...
      val1 = CHAR_TO_DOUBLE(elem_size, loc1);
      val2 = CHAR_TO_DOUBLE(elem_size, loc2);
      sum[i + j * sumstride] = val1 + val2;
    }
}

static inline void pointwise_matrix_div(unsigned char *mat1,
                                        unsigned char *mat2, size_t elem_size,
                                        double *quot, int width, int height,
                                        int stride1, int stride2,
                                        int quotstride) {
  int i, j;
  unsigned char *loc1, *loc2;
...
static void optical_flow_per_pixel(double *dx, double *dy, double *dt,
  ...
  if (RELATIVE_ERROR(det, 0) > MAX_ERROR) {
    ...
    if (M[0] == 0 && M[3] == 0) {
      memcpy(M_inv, M, sizeof(M_inv));
    } else {
      trace_inv = 1 / trace;
      M_inv[0] = M[0] * trace_inv;
      M_inv[1] = M[1] * trace_inv;
      M_inv[2] = M_inv[1];
      M_inv[3] = M[3] * trace_inv;
    }
  }
  // compute inverse harris corner score as confidence metric
  corner_score = 1 / (det - (0.01 * trace));
  flow->confidence[loc_x + loc_y * flow->stride] = corner_score > 2 ?
                                                   10 : corner_score;
  flow->u[loc_x + loc_y * flow->stride] = M_inv[0] * b[0] +  M_inv[1] * b[1];
  flow->v[loc_x + loc_y * flow->stride] = M_inv[2] * b[0] + M_inv[3] * b[1];
}
...
static void lucas_kanade_base(double *smooth_frm, double *smooth_ref,
  ...
  // windowing function for local neightborhood weights
  double window[] = {0.0039, 0.0156, 0.0234, 0.0156, 0.0039,
...
static void iterative_refinement(unsigned char *ref, double *smooth_frm,
  ...
  for (n = 0; n < n_refinements; ++n) {
    ...
    blur_img(estimate, width, height, width, smooth_estimate, width, 11,
             1.5, 1);
    ...
    pointwise_matrix_add((unsigned char*)flow->u, (unsigned char*) new_flow.u,
                         sizeof(*flow->u), new_flow.u, width, height,
                         flow->stride, new_flow.stride, new_flow.stride);
    pointwise_matrix_add((unsigned char*) flow->v, (unsigned char*) new_flow.v,
                         sizeof(*flow->v), new_flow.v, width, height,
                         flow->stride, new_flow.stride, new_flow.stride);
    median_filter((unsigned char*)new_flow.u, flow->u, sizeof(*new_flow.u), 5,
                   width, height, new_flow.stride, flow->stride);
    median_filter((unsigned char*)new_flow.v, flow->v, sizeof(*new_flow.v), 5,
                  width, height, new_flow.stride, flow->stride);
    median_filter((unsigned char*)new_flow.confidence, flow->confidence,
                  sizeof(*new_flow.confidence), 5, width, height,
...
void compute_flow(unsigned char *frm, unsigned char *ref,
  ...
  if (n_levels == 1) {
    ...
    blur_img(ref, width, height, ref_stride, smooth_ref, width, 11, 1.5, 1);
    blur_img(frm, width, height, frm_stride, smooth_frm, width, 11, 1.5, 1);
    differentiate((unsigned char*)smooth_frm, sizeof(*smooth_frm), width,
                  height, width, dx_frm, dy_frm, width);
    ...
    iterative_refinement(ref, smooth_frm, dx_frm, dy_frm, dt, &flow, width,
                         height, ref_stride, width, width, 1);
  } else {
    int i, j, k;
    ...
    for (i = n_levels - 1; i >= 0; --i) {
      ...
      blur_img(frm_pyramid->levels[i], w, h, s_f, smooth_frm, width, blur_size,
               blur_sig, 1);
      differentiate((unsigned char*)smooth_frm, sizeof(*smooth_frm), w, h,
                    width, dx_frm, dy_frm, width);
      ...
      */
      if (i < n_levels - 1) {
        blur_img(frm_approx, w, h, width, smooth_frm_approx, width, blur_size,
                 blur_sig, 1);
        differentiate((unsigned char*)smooth_frm_approx,
                    sizeof(*smooth_frm_approx),  w, h, width, dx_frm_approx,
                    dy_frm_approx, width);
        lucas_kanade_base(smooth_frm_approx, smooth_ref, dx_frm_approx,
                        dy_frm_approx, dt, &flow, w, h, width, width, width);
        pointwise_matrix_add((unsigned char*) flow.u,
                             (unsigned char*) u_upscale, sizeof(*u_upscale),
                             u_upscale, w, h, flow.stride, width, width);
        pointwise_matrix_add((unsigned char*) flow.v,
                             (unsigned char*) v_upscale, sizeof(*v_upscale),
                             v_upscale, w, h, flow.stride, width, width);
        median_filter((unsigned char*)u_upscale, flow.u, sizeof(*u_upscale),
                       5, w, h, width, flow.stride);
        median_filter((unsigned char*)v_upscale, flow.v, sizeof(*v_upscale),
                       5, w, h, width, flow.stride);
      } else {
      ...
      for (j = 0; j < h_upscale; ++j)
        for (k = 0; k < w_upscale; ++k) {
          u_upscale[k + j * w_upscale] = flow.u[(int)(k * 0.5) +
                                                 (int)(j * 0.5) * flow.stride];
          v_upscale[k + j * w_upscale] = flow.v[(int)(k * 0.5) +
                                                 (int)(j * 0.5) * flow.stride];
