...
void compute_flow(unsigned char *frm, unsigned char *ref,
  ...
  flow->u = (double*)malloc(width * height * sizeof(*flow->u));
  flow->v = (double*)malloc(width * height * sizeof(*flow->v));
  flow->confidence = (double*)malloc(width * height *
...
#else
  printf("coarse to fine\n");
  coarse_to_fine_flow(frm, ref, flow, width, height, frm_stride,
                      ref_stride, 3, 2);
#endif

  unsigned char* warped = (unsigned char*)malloc(width * height * sizeof(unsigned char));
  pixelwise_warp_mse(frm, ref, flow, warped, width,
                                height, ref_stride, frm_stride);
  flow->width = width;
  flow->height = height;
}
...
void lucas_kanade_base(double *smooth_frm, double *smooth_ref,
  ...
  double *dt = (double *)malloc(width * height * sizeof(*dt));
  double *window = (double *)malloc(local_neighborhood_sz *
                                    local_neighborhood_sz * sizeof(*window));
...
void iterative_refinement(unsigned char *ref, double *smooth_frm, double *dx,
  ...
  flowMV new_flow;
  double *new_u = (double*)malloc(width * height * sizeof(*new_u));
  double *new_v = (double*)malloc(width * height * sizeof(*new_v));
  double *new_c = (double*)malloc(width * height * sizeof(*new_c));
  new_flow.u = new_u;
  new_flow.v = new_v;
...
void coarse_to_fine_flow(unsigned char *frm, unsigned char *ref,
  ...
  frm_approx = (unsigned char*)malloc(w * h * sizeof(*frm_approx));
  ...
  for (i = n_levels - 1; i >= 0; --i) {
    ...
    u_level = (double*)malloc(w * h * sizeof(*u_level));
    v_level = (double*)malloc(w * h * sizeof(*v_level));
    conf_level = (double*)malloc(w * h * sizeof(*conf_level));
    ...
    // c/mpute optical flow at this level between the reference frame and
    ...
    u_upscale = (double*)malloc(w_upscale * h_upscale * sizeof(*u_upscale));
    v_upscale = (double*)malloc(w_upscale * h_upscale * sizeof(*v_upscale));
...
void image_pyramid(unsigned char *img, imPyramid *pyr, int width,
  ...
  if ((RELATIVE_ERROR(resize_factor, 1.0) <= MAX_ERROR) || (n_levels == 1)) {
    pyr->n_levels = 1;
    pyr->levels = (unsigned char**)malloc(sizeof(*pyr->levels));
    pyr->levels[0] = img;
    return;
  }
  ...
  pyr->levels = (unsigned char**)malloc(n_levels * sizeof(*pyr->levels));
  pyr->levels[0] = img;
...
void differentiate(unsigned char *img, size_t elem_size, int width, int height,
  ...
  *dx_img = (double *)malloc(width * height * sizeof(**dx_img));
  *dy_img = (double *)malloc(width * height * sizeof(**dy_img));
...
void blur_img(unsigned char *img, int width, int height,
  ...
  *smooth_img = (double *)malloc(width * height * sizeof(**smooth_img));
...
inline void pixelwise_warp_mse(unsigned char *frm, unsigned char *ref,
  ...
  mse /= ((width) * (height));
  // printf("pixelwise wearp: %f\n", mse);
}

// Prints a matrix. Mostly for debugging purposes.
static inline void print_mat(unsigned char *mat, size_t size, int width,
