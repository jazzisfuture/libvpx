...
static void optical_flow_per_pixel(double *dx, double *dy, double *dt,
  ...
  corner_score = 1/((M[0] * M[3]) - (M[1] * M[1]) -
                 (0.01 * (M[0] + M[3]) * (M[0] + M[3])));
  flow->confidence[loc_x + loc_y * width] = corner_score;
...
void compute_flow(unsigned char *frm, unsigned char *ref,
  ...
  if (n_levels ==1) {
    ...
    frm_approx = (unsigned char*)safe_malloc(s_f * h * sizeof(*frm_approx));
    memcpy(frm_approx, frm_pyramid->levels[n_levels - 1], s_f * h *
    ...
    for (i = n_levels - 1; i >= 0; --i) {
      ...
      if (i < n_levels - 1)
        blur_img(frm_approx, w, h, w, &smooth_frm_approx, blur_size,
                 blur_sig, 1);
      else
        blur_img(frm_approx, w, h, s_f, &smooth_frm_approx, blur_size,
                 blur_sig, 1);
      ...
      iterative_refinement(ref_pyramid->levels[i], smooth_frm, dx_frm, dy_frm,
                           &flow_level, w, h, s_r, w, ITERATIONS);
      ...
      for (j = 0; j < h_upscale; ++j)
        for (k = 0; k < w_upscale; ++k) {
          ...
          frm_approx[k + j * w_upscale] =
                                        interpolate(ref_pyramid->levels[i - 1],
