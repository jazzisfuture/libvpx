  ...
  for (i = n_levels - 1; i >= 0; --i){
    ...
    for (k = 0; k < w_upscale; ++k)
      for (j = 0; j < h_upscale; ++j) {
        u_upscale[k + j * w_upscale] = u_level[(int)(k / 2) + (int)(j / 2) * w];
        v_upscale[k + j * w_upscale] = v_level[(int)(k / 2) + (int)(j / 2) * w];
...
void image_pyramid(unsigned char *img, imPyramid *pyr, int width,
  ...
  // compute number of levels of the pyramid. The smallest dimension of the
...
void gaussian_kernel(double *mat, int size, double sig, double amp)
{
  ...
  for (x = (-1 * half); x<=half; x++)
    ...
    {
      filter_val = amp * exp((double)(-1.0 * ((x * x / (2.0 * sig * sig)) +
                   (y * y / (2.0 * sig * sig)))));
