...
static void iterative_refinement(unsigned char *ref, double *smooth_frm,
  ...
  for (n = 0; n < n_refinements; ++n) {
    ...
    pointwise_matrix_add(flow->v, new_flow.v, new_flow.v, width, height,
                         flow->stride, new_flow.stride, new_flow.stride);
    median_filter(new_flow.u, flow->u, 5, width, height, new_flow.stride,
                  flow->stride);
    median_filter(new_flow.v, flow->v, 5, width, height, new_flow.stride,
                  flow->stride);
    median_filter(new_flow.confidence, flow->confidence, 5, width, height,
      ...
      */
      if (i < n_levels - 1) {
        ...
        pointwise_matrix_add(flow.v, v_upscale, v_upscale, w, h, flow.stride,
                              width, width);
        median_filter(u_upscale, flow.u, 5, w, h, width, flow.stride);
        median_filter(v_upscale, flow.v, 5, w, h, width, flow.stride);
      } else {
