...
static void optical_flow_per_pixel(double *dx, double *dy, double *dt,
  ...
  if (RELATIVE_ERROR(det, 0) > MAX_ERROR) {
    det_inv = 1 / det;
    flow->u[loc_x + loc_y * flow->stride] = M[3] * det_inv * b[0] -
                                            M[1] * det_inv * b[1];
    flow->v[loc_x + loc_y * flow->stride] = -M[2] * det_inv * b[0] +
                                             M[0] * det_inv * b[1];
  } else {
    if (M[0] == 0 && M[3] == 0) {
      flow->u[loc_x + loc_y * flow->stride] = M[0] * b[0] +  M[1] * b[1];
      flow->v[loc_x + loc_y * flow->stride] = M[2] * b[0] + M[3] * b[1];
    } else {
      trace2_inv = 1 / trace2;
      flow->u[loc_x + loc_y * flow->stride] = M[0] * trace2_inv * b[0] +
                                              M[1] * trace2_inv * b[1];
      flow->v[loc_x + loc_y * flow->stride] = M[2] * trace2_inv * b[0] +
                                              M[3] * trace2_inv * b[1];
    }
  }
