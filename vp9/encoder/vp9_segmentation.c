...
static void count_segs(const VP9_COMMON *cm, MACROBLOCKD *xd,
  ...
  xd->mi_test = mi;
  segment_id = xd->mi_test[0].target->mbmi.segment_id;
  ...
  if (cm->frame_type != KEY_FRAME) {
    ...
    // as appropriate
    xd->mi_test[0].target->mbmi.seg_id_predicted = pred_flag;
...
static void count_segs_sb(const VP9_COMMON *cm, MACROBLOCKD *xd,
  ...
  bw = num_8x8_blocks_wide_lookup[mi[0].target->mbmi.sb_type];
  bh = num_8x8_blocks_high_lookup[mi[0].target->mbmi.sb_type];
