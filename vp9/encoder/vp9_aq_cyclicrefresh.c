...
void vp9_cyclic_refresh_update_segment(VP9_COMP *const cpi,
  ...
  if (refresh_this_block == 0 &&
    ...
    const uint8_t ysource = p[0].src.buf[shift * stride + shift];
    const uint8_t usource = p[1].src.buf[shiftuv * strideuv + shiftuv];
    const uint8_t vsource = p[2].src.buf[shiftuv * strideuv + shiftuv];
