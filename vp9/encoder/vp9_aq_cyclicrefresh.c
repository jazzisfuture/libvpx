...
int generate_map(VP9_COMP *cpi, int block_count, int sb_index, int start, int end,
		 int sb_cols, int consec_zero_mv_thresh, int qindex_thresh,
		 int *data, int mode) {
  VP9_COMMON *const cm = &cpi->common;
  CYCLIC_REFRESH *const cr = cpi->cyclic_refresh;
  unsigned char *const seg_map = cpi->segmentation_map;
...
static void cyclic_refresh_update_map(VP9_COMP *const cpi) {
  ...
  if (1) {
    ...
    num_blocks = generate_map(cpi, block_count, cr->sb_index, 0,
                              sbs_in_frame, sb_cols,
