...
CYCLIC_REFRESH *vp9_cyclic_refresh_alloc(int mi_rows, int mi_cols) {
  ...
  cr->dist_map = vpx_calloc(mi_rows * mi_cols, sizeof(*cr->dist_map));
  if (cr->dist_map == NULL) {
    vpx_free(cr);
    return NULL;
  }
...
void vp9_cyclic_refresh_update_segment(VP9_COMP *const cpi,
  ...
  const int refresh_this_block = candidate_refresh_aq(cr, mbmi, rate, dist,
                                                      bsize);
  const int high_low_dist = (dist > cr->thresh_dist_high) ? 1: 0;
  ...
  for (y = 0; y < ymis; y++)
    for (x = 0; x < xmis; x++) {
      cr->dist_map[block_index + y * cm->mi_cols + x] = high_low_dist;
...
void vp9_cyclic_refresh_setup(VP9_COMP *const cpi) {
  ...
  if (cm->frame_type == KEY_FRAME) vpx_memset(cr->dist_map, 0, cm->mi_rows * cm->mi_cols);
  ...
  if (!apply_cyclic_refresh ||
    ...
    cr->thresh_dist_high = ((int64_t)(q * q)) << 6;
    cr->motion_thresh = 32;
    // Set up segmentation.
