...
void vp9_cyclic_refresh_update_parameters(VP9_COMP *const cpi) {
  ...
  if (cpi->oxcf.content == VP9E_CONTENT_SCREEN &&
      cr->frames_since_slide_change_maxq < 100)
...
void vp9_cyclic_refresh_damp_factor(VP9_COMP *const cpi,
                                    double *adjustment_limit) {
  CYCLIC_REFRESH *const cr = cpi->cyclic_refresh;
  cr->frames_since_slide_change_maxq++;
  ...
  // Do this for medium-low motion content within some distance from the
  // re-encode at max QP (slide change).
  if (cpi->rc.avg_frame_low_motion > 50 && cr->percent_refresh > 0 &&
      cr->frames_since_slide_change_maxq < 1000 / cr->percent_refresh) {
