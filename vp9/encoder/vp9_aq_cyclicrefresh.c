...
void vp9_cyclic_refresh_postencode(VP9_COMP *const cpi) {
  ...
  // Check for golde frame update: only for non-SVC and non-golden boost.
  if (!cpi->use_svc && cpi->ext_refresh_frame_flags_pending == 0 &&
      !cpi->oxcf.gf_cbr_boost_pct) {
    // If the background has very high motion in current frame set this frame as
    // the golden frame (but maintain period of at least 4 frames).
    // Also, always force this frame as a golden update frame if this frame
    // changes the resolution (resize_pending != 0).
    if (cpi->resize_pending != 0 ||
        (fraction_low < 0.2 && rc->frames_since_golden > 4)) {
    ...
    if (!force_gf_refresh && cpi->refresh_golden_frame == 1) {
      ...
      if (fraction_low < 0.7 || cr->low_content_avg < 0.6)
