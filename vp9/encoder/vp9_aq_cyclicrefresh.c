...
int vp9_cyclic_refresh_rc_bits_per_mb(const VP9_COMP *cpi, int i,
  ...
  // Weight for segment prior to encoding: use actual from the previous just
  // encoded frame; use the target based on percent_refresh only if its less.
  ...
  double weight_segment =
      ...
      num8x8bl;
  if (target_refresh < cr->actual_num_seg1_blocks + cr->actual_num_seg2_blocks)
