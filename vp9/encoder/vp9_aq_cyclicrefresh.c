...
void vp9_cyclic_refresh_update_sb_postencode(VP9_COMP *const cpi,
  ...
  for (y = 0; y < ymis; y++)
    for (x = 0; x < xmis; x++) {
      ...
      if (is_inter_block(mi) && abs(mv.row) < 16 && abs(mv.col) < 16)
...
void vp9_cyclic_refresh_check_golden_update(VP9_COMP *const cpi) {
  ...
  // Also, force this frame as a golden update frame if this frame change
  // the resolution (resize_pending != 0).
  if (cpi->resize_pending != 0 ||
      (fraction_low < 0.25 && rc->frames_since_golden > 8)) {
