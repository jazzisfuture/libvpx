...
void vp9_cyclic_refresh_update_segment_postencode(VP9_COMP *const cpi,
                                                  MB_MODE_INFO *const mbmi,
                                                  int mi_row, int mi_col,
                                                  BLOCK_SIZE bsize) {
  const VP9_COMMON *const cm = &cpi->common;
  ...
  for (y = 0; y < ymis; y++)
    for (x = 0; x < xmis; x++) {
      ...
      if (is_inter_block(mbmi) && (mv.row < 8 && mv.row > -8) &&
          (mv.col < 8 || mv.col > -8)) {
  ...
  do {
    ...
    int qindex_thresh =
        cpi->oxcf.content == VP9E_CONTENT_SCREEN
            ...
            : (3 * cpi->rc.worst_quality) >> 1;
    int consec_zero_mv_thresh =
        cpi->oxcf.content == VP9E_CONTENT_SCREEN ? 256
            : 8 * (100 / cr->percent_refresh);
    // TODO(marpan): Add condition that spatial variance is very low.
