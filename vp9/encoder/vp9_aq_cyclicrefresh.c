...
// Update some encoding stats from the just encoded frame.
// Update the actual number of blocks that were applied the segment delta q.
// Check if we should refresh (or NOT refresh) golden frame: if this frame's
...
void vp9_cyclic_refresh_postencode(VP9_COMP *const cpi) {
  ...
  for (mi_row = 0; mi_row < rows; mi_row++) {
    int mapoffset = mi_row * cols;
    for (mi_col = 0; mi_col < cols; mi_col++) {
      if (cyclic_refresh_segment_id(
          seg_map[mapoffset + mi_col]) == CR_SEGMENT_ID_BOOST1)
         cr->actual_num_seg1_blocks++;
      else if (cyclic_refresh_segment_id(
          seg_map[mapoffset + mi_col]) == CR_SEGMENT_ID_BOOST2)
      ...
      if (cr->map[mapoffset + mi_col] < 1)
        low_content_frame++;
    }
    mi += 8;
  }
  // Check for updating (or not updating) the golden reference frame, only for
  // non-svc cbr mode.
  if (!cpi->use_svc &&
