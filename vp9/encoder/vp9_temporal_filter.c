...
static int temporal_filter_find_matching_mb_c(VP9_COMP *cpi,
  ...
  ref_mv = &x->e_mbd.mode_info_context->bmi[0].as_mv.mv[0];
  ...
  for (mb_row = 0; mb_row < mb_rows; mb_row++) {
    ...
    for (mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        mbd->mode_info_context->bmi[0].as_mv.mv[0].as_mv.row = 0;
        mbd->mode_info_context->bmi[0].as_mv.mv[0].as_mv.col = 0;
...
#define THRESH_HIGH  20000
        ...
        if (filter_weight != 0) {
          ...
          (mbd,
           ...
           cpi->frames[frame]->y_stride,
           mbd->mode_info_context->bmi[0].as_mv.mv[0].as_mv.row,
           mbd->mode_info_context->bmi[0].as_mv.mv[0].as_mv.col,
           predictor);
