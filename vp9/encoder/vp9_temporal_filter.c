...
static void vpx_convolve8_12_c(const uint8_t *src, ptrdiff_t src_stride,
  ...
  convolve_horiz_12(src - src_stride * (MAX_FILTER_TAP / 2 - 1), src_stride,
                    temp, 64, filter, x0_q4, x_step_q4, w, intermediate_height);
  convolve_vert_12(temp + 64 * (MAX_FILTER_TAP / 2 - 1), 64, dst, dst_stride,
...
static void highbd_convolve_12(const uint16_t *src, ptrdiff_t src_stride,
  ...
  highbd_convolve_vert_12(temp + 64 * (MAX_FILTER_TAP / 2 - 1), 64, dst,
                          dst_stride, filter, y0_q4, y_step_q4, w, h, bd);
}
