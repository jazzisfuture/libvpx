...
void vp9_highbd_apply_temporal_filter_c(
  ...
  const uint16_t *u_src_ptr = CONVERT_TO_SHORTPTR(u_src);
  const uint16_t *u_pre_ptr = CONVERT_TO_SHORTPTR(u_pre);
  const uint16_t *v_src_ptr = CONVERT_TO_SHORTPTR(v_src);
  const uint16_t *v_pre_ptr = CONVERT_TO_SHORTPTR(v_pre);
  ...
  for (int row = 0; row < (int)block_height; row++) {
    for (int col = 0; col < (int)block_width; col++) {
      ...
      const int filter_weight = get_filter_weight(row, col, block_height,
                                                  block_width, blk_fw,
                                                  use_32x32);
      ...
      y_mod += u_diff_sse[uv_row * y_diff_stride + uv_col];
      y_mod += v_diff_sse[uv_row * y_diff_stride + uv_col];
  ...
  for (int uv_row = 0; uv_row < (int)uv_block_height; uv_row++) {
    for (int uv_col = 0; uv_col < (int)uv_block_width; uv_col++) {
      ...
      const int filter_weight = get_filter_weight(uv_row, uv_col, uv_block_height,
                                                  uv_block_width, blk_fw,
                                                  use_32x32);
      ...
      if (blk_fw[0] || blk_fw[1] || blk_fw[2] || blk_fw[3]) {
        ...
        if (mbd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
          ...
          vp9_highbd_apply_temporal_filter(
              ...
              mbd->plane[1].subsampling_y, adj_strength, blk_fw, use_32x32,
