...
static int temporal_filter_find_matching_mb_c(VP9_COMP *cpi,
  ...
  ref_mv = &x->e_mbd.mi_8x8->mi->bmi[0].as_mv[0];
  ...
  for (mb_row = 0; mb_row < mb_rows; mb_row++) {
    ...
    for (mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        mbd->mi_8x8->mi->bmi[0].as_mv[0].as_mv.row = 0;
        mbd->mi_8x8->mi->bmi[0].as_mv[0].as_mv.col = 0;
...
#define THRESH_HIGH  20000
        ...
        if (filter_weight != 0) {
          ...
          (mbd,
           ...
           cpi->frames[frame]->y_stride,
           mbd->mi_8x8->mi->bmi[0].as_mv[0].as_mv.row,
           mbd->mi_8x8->mi->bmi[0].as_mv[0].as_mv.col,
           predictor);
