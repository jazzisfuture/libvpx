...
static void apply_temporal_filter(
  ...
  int y_diff_sse[256] = { 0 };
  int u_diff_sse[256] = { 0 };
  int v_diff_sse[256] = { 0 };
  int idx = 0, idy;
  ...
  for (i = 0; i < block_height; i++) {
    for (j = 0; j < block_width; j++) {
      int diff = y_frame1[i * (int)y_stride + j] - y_pred[i * (int)block_width + j];
  ...
  for (i = 0; i < uv_block_height; i++) {
    for (j = 0; j < uv_block_width; j++) {
      int diff = u_frame1[i * uv_stride + j] - u_pred[i * uv_buf_stride + j];
      u_diff_sse[idx] = diff * diff;

      diff = v_frame1[i * uv_stride + j] - v_pred[i * uv_buf_stride + j];
      v_diff_sse[idx] = diff * diff;
      idx++;
    }
  }
