...
// TODO(aconverse, debargha): This function should eventually
// replace the one above, once we have the sse2 code for this function
// implemented.
void vp9_temporal_filter_apply2_c(uint8_t *frame1,
                                  unsigned int stride,
                                  uint8_t *frame2,
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        if (filter_weight != 0) {
          ...
          if (mb_uv_height == mb_uv_width) {
            ...
            vp9_temporal_filter_apply2_c(f->y_buffer + mb_y_offset,
                                         f->y_stride, predictor, 16, 16,
                                         strength, filter_weight,
                                         accumulator, count);
            vp9_temporal_filter_apply2_c(f->u_buffer + mb_uv_offset,
                                         f->uv_stride, predictor + 256,
                                         mb_uv_width, mb_uv_height,
                                         strength, filter_weight,
                                         accumulator + 256, count + 256);
            vp9_temporal_filter_apply2_c(f->v_buffer + mb_uv_offset,
