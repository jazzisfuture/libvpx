...
static adjust_arnr_filter(VP9_COMP *cpi,
                          const unsigned int frames_to_arnr,
                          const int group_boost) {
  const int frames_after_arf =
            vp9_lookahead_depth(cpi->lookahead) - frames_to_arnr - 1;
  int max_bwd = cpi->rc.baseline_gf_interval;
...
void vp9_temporal_filter_prepare(VP9_COMP *cpi, int distance) {
  ...
  int strength;
  int max_frames;
  const int buffered_frames_backward = distance;
  const int buffered_frames_forward = vp9_lookahead_depth(cpi->lookahead) -
                                      (buffered_frames_backward + 1);
  int frames_to_blur_backward = buffered_frames_backward;
  int frames_to_blur_forward = buffered_frames_forward;
  struct scale_factors sf;

  // Apply context specific adjustments to the arnr filter parameters.
  adjust_arnr_filter(cpi, distance, rc->gfu_boost);
  max_frames = cpi->active_arnr_frames;
  strength = cpi->active_arnr_strength;

  // Define width limits for centered arnr blur function
