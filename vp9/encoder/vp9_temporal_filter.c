...
static void temporal_filter_predictors_mb_c(MACROBLOCKD *xd,
  ...
  const InterpKernel *const kernel =
    vp9_get_interp_kernel(xd->mi_test[0].target->mbmi.interp_filter);
...
static int temporal_filter_find_matching_mb_c(VP9_COMP *cpi,
  ...
  MV *ref_mv = &x->e_mbd.mi_test[0].target->bmi[0].as_mv[0].as_mv;
  ...
  for (mb_row = 0; mb_row < mb_rows; mb_row++) {
    ...
    for (mb_col = 0; mb_col < mb_cols; mb_col++) {
      ...
      for (frame = 0; frame < frame_count; frame++) {
        ...
        mbd->mi_test[0].target->bmi[0].as_mv[0].as_mv.row = 0;
        mbd->mi_test[0].target->bmi[0].as_mv[0].as_mv.col = 0;
        ...
        if (filter_weight != 0) {
          ...
          temporal_filter_predictors_mb_c(mbd,
              ...
              mbd->mi_test[0].target->bmi[0].as_mv[0].as_mv.row,
              mbd->mi_test[0].target->bmi[0].as_mv[0].as_mv.col,
              predictor, scale,
              mb_col * 16, mb_row * 16);
