...
#if CONFIG_VP9_HIGH
static double high_simple_weight(const YV12_BUFFER_CONFIG *buf,BIT_DEPTH bit_depth) {
  int i, j;
  double sum = 0.0;
  const int w = buf->y_crop_width;
  const int h = buf->y_crop_height;
  ...
  for (i = 0; i < h; ++i) {
    ...
    for (j = 0; j < w; ++j)
      sum += weight_table[*pixel++>>shift];  // TODO is this appropriate for high bitdepth images?
    row16 += buf->y_stride;
  }
...
#if CONFIG_VP9_HIGH
          if (xd->cur_buf->flags & YV12_FLAG_HIGH) {
            gf_motion_error = high_get_prediction_error(bsize, &x->plane[0].src,
                                                 &xd->plane[0].pre[0],xd->bps);
          } else {
...
#if CONFIG_VP9_HIGH
    if (cm->use_high) {
      fps.ssim_weighted_pred_err = fps.coded_error * high_simple_weight(cpi->Source,cm->bit_depth);
    } else {
