...
// Work out the frame coding order for a GF or ARF group.
                            ...
                            const int level)
{
  ...
  // ARFs may not be adjacent, they must be separated by at least one non-ARF.
  half_range = (abs_end - start) >> 1;

//  if (half_range <= MIN_GF_INTERVAL) {
...
void define_fixed_ARF_period (VP9_COMP *cpi) {
  ...
  if (cpi->twopass.frames_to_key <= 1) {
    return;
  }

  // Setup a fixed period ARF group.
  if (cpi->twopass.frames_to_key <= (FIXED_ARF_GROUP_SIZE + 8)) {
    // Don't leave a small group of frames at the end of the KF interval.
    cpi->baseline_gf_interval = cpi->twopass.frames_to_key - 1;
  } else {
  ...
  if ((FIXED_ARF_GROUP_SIZE < cpi->oxcf.lag_in_frames) &&
      (FIXED_ARF_GROUP_SIZE <=
          (cpi->twopass.frames_to_key - MIN_GF_INTERVAL))) {
...

  {
    ...
    if (cpi->baseline_gf_interval >= 3) {
      ...
      if (boost >= 150) {
        ...
        // AWG Note that this is the only place that uses
        // cpi->twopass.alt_extra_bits. It was used in VP8 to allocate
        // extra bits to every other frame in a GF group.
