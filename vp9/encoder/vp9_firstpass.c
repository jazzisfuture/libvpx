...
static void store_fp_motion_vector(VP9_COMP *cpi, const MV *mv,
  ...
  if (!is_second_mv) {
    ...
    printf("mv of (%d, %d): (%d, %d)\n", mb_row, mb_col,
           this_motion_vector_info->mv[0].as_mv.row,
           this_motion_vector_info->mv[0].as_mv.col);
    return;
  }
  ...
  this_motion_vector_info->mv[1].as_mv.col = mv->col;
    printf("mv of (%d, %d): (%d, %d)\n", mb_row, mb_col,
           this_motion_vector_info->mv[1].as_mv.row,
           this_motion_vector_info->mv[1].as_mv.col);
}
...
#if CONFIG_RATE_CTRL
        store_fp_motion_vector(cpi, &tmp_mv, mb_row, mb_col, /*is_second_mv=*/1);
#endif  // CONFIG_RAGE_CTRL
