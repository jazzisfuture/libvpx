...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
  ...
  vp9_clear_system_state();
  {
    ...
    const double min_err = 200 * pow(cm->MBs, 0.5);
...
static int get_section_worst_quality(const VP9_COMP *cpi,
  ...
  if ((section_bits <= 0) || (frame_count <= 0)) {
    return rc->worst_quality;  // Highest value allowed for illegal input
  } else {
    const double num_mbs = (double)cpi->common.MBs * frame_count;
    const double err_per_mb = section_err / DOUBLE_DIVIDE_CHECK(num_mbs);
    const double speed_term = 1.0 + 0.04 * oxcf->speed;
    const int target_norm_bits_per_mb =
      ((uint64_t)section_bits << BPER_MB_NORMBITS) /
      DOUBLE_DIVIDE_CHECK(num_mbs);
    int q;
...
static void define_gf_group(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if (cpi->oxcf.allow_spatial_resampling){
    ...
    //max_bit_target = MAX(max_bit_target, ((3 * gf_group_bits) / 2));
    max_bit_target = MAX(max_bit_target, (2 * gf_group_bits));
