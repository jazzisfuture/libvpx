...
static unsigned int get_block_variance(BLOCK_SIZE bsize,
  ...
  for (mb_row = 0; mb_row < cm->mb_rows; ++mb_row) {
    ...
    for (mb_col = 0; mb_col < cm->mb_cols; ++mb_col) {
      ...
      if (cm->current_video_frame > 0) {
        ...
        motion_error = get_block_variance(bsize, &x->plane[0].src,
                                          &xd->plane[0].pre[0]);
        ...
        if (cm->current_video_frame > 1 && gld_yv12 != NULL) {
          ...
          gf_motion_error = get_block_variance(bsize, &x->plane[0].src,
                                               &xd->plane[0].pre[0]);
