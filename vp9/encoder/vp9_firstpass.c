...
static void define_coding_order(VP9_COMP *cpi) {
  ...
  // TODO(agrange) if prev group was *not* MARF group cpi->sequence_number
  // will not be defined.
  ...
  // Adapt the minimum ARF separation to characteristics of the group of frames.
  if ((cpi->gfu_boost < 400) && (cpi->active_worst_quality > 80)) {
    cpi->min_arf_separation = 4;
  } else {
    cpi->min_arf_separation = 16;
  }
