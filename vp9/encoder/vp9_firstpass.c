...
static void define_gf_group(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  // If the image appears almost completely static we can extend beyond this.
  {
    ...
    if (cpi->multi_arf_allowed) {
      ...
      active_max_gf_interval = 12 + MIN(4, (int_lbq / 8));
...
void vbr_rate_correction(VP9_COMP *cpi,
  ...
  const int frames_left = (int)cpi->twopass.total_stats.count -
                          cpi->common.current_video_frame;
  // const int max_delta = (*this_frame_target * VBR_FRAME_PCT_ADJUSTMENT_LIMIT) / 100;
  int max_delta;
  ...
  if (cpi->twopass.total_stats.count) {
    position_factor = (double)cpi->common.current_video_frame /
...
void vp9_twopass_postencode_update(VP9_COMP *cpi) {
  ...
  if (total_bits) {
    rc->rate_error_estimate =
      ((rc->vbr_bits_off_target * 100) / total_bits);
  } else {
  ...
  if ((cpi->oxcf.rc_mode == VPX_VBR) &&
    ...
    // Undershoot.
    if (rc->rate_error_estimate > (cpi->oxcf.under_shoot_pct >> 1)) {
    ...
    // Overshoot.
    } else if (rc->rate_error_estimate < -(cpi->oxcf.over_shoot_pct >> 1)) {
