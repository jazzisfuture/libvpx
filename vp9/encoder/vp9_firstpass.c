...
#ifdef FP_ERR_PER_MB
    // const int active_mbs = VPXMAX(1, (double)num_mbs * active_pct);
    const double av_err_per_mb = section_err / active_pct;
#else
    // const int active_mbs =
    // VPXMAX(1, num_mbs - (int)(num_mbs * inactive_zone));
    // const int active_mbs = VPXMAX(1, (double)num_mbs * active_pct);
...
static double calc_frame_boost(VP9_COMP *cpi, const FIRSTPASS_STATS *this_frame,
  ...
  num_mbs = (int)VPXMAX(1, num_mbs * active_area);
...
void vp9_rc_get_second_pass_params(VP9_COMP *cpi) {
...

  {
    ...
    twopass->mb_av_energy =
        log((this_frame.intra_error * 256.0) + 1.0);
#else
    const int num_mbs = (cpi->oxcf.resize_mode != RESIZE_NONE)
                            ? cpi->initial_mbs
                            : cpi->common.MBs;
