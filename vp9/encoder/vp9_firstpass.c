...
static void first_pass_motion_search(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  // Initial step/diamond search centered on best mv.
...
// and motion vectors. Currently simplistic in its assumptions for testing.
...
static void schedule_frames(VP9_COMP *cpi, const int start, const int end,
  ...
  // ARF Group: work out the ARF schedule. Mark ARF frames as negative.
  if (end < 0) {
    // printf("start:%d end:%d\n", -end, -end);
...
static void find_next_key_frame(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if (1) {
    ...
    // Special case if the sequence appears almost totally static
