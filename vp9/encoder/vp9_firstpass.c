...
static double simple_weight(YV12_BUFFER_CONFIG *source) {
  ...
  // Loop throught the Y plane raw examining levels and creating a weight for
  // the image.
  i = source->y_height;
  do {
    j = source->y_width;
...
void vp9_first_pass(VP9_COMP *cpi) {
  ...
  for (mb_row = 0; mb_row < cm->mb_rows; mb_row++) {
    ...
    for (mb_col = 0; mb_col < cm->mb_cols; mb_col++) {
      ...
      // all inter modes carry an overhead cost estimate fot the mv.
      ...
      if (cm->current_video_frame > 0) {
        ...
        // If the current best reference mv is not centred on 0,0 then do a 0,0
      ...
      ((this_frame->pcnt_inter < 0.05) ||
       (
        ...
        ((fabs(last_frame->coded_error - this_frame->coded_error) /
              DOUBLE_DIVIDE_CHECK(this_frame->coded_error) > .40) ||
         (fabs(last_frame->intra_error - this_frame->intra_error) /
              DOUBLE_DIVIDE_CHECK(this_frame->intra_error) > .40) ||
