...
#define SECTION_NOISE_DEF   200.0
#define LOW_I_THRESH        16000
...
static scale_sse_threshold(VP9_COMMON *cm, int thresh) {
  int ret_val = thresh;
#if CONFIG_VP9_HIGHBITDEPTH
...
static uint8_t fp_dn_kernal_3[9] = {
  1, 2, 1,
  2, 4, 2,
  1, 2, 1};

// TODO(paulwilkins): #if CONFIG_VP9_HIGHBITDEPTH variants.
// The code below currently is only valid for 8bit
static int fp_estimate_point_noise(uint8_t *src_ptr, const int stride) {
  ...
  for (i = 0; i < kernal_size; ++i) {
    for (j = 0; j < kernal_size; ++j) {
      diff = abs((int)*src_ptr - (int)tmp_ptr[j]);
...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
  ...
  for (mb_row = 0; mb_row < cm->mb_rows; ++mb_row) {
    ...
    for (mb_col = 0; mb_col < cm->mb_cols; ++mb_col) {
      ...
      if (cm->current_video_frame == 0 &&
          (this_intra_error < scale_sse_threshold(cm, LOW_I_THRESH))) {
        ++noise_mb_count;
        frame_noise_energy += fp_estimate_block_noise(x, bsize);
      }
...

  {
    ...
    if (noise_mb_count > (num_mbs / 20)) {
      fps.frame_noise_energy = (double)frame_noise_energy / noise_mb_count;
    } else {
      // For complex (intra) blocks we assign a default noise level.
...
#define NOISE_FACTOR_MIN    0.75
#define NOISE_FACTOR_MAX    1.25
static int get_twopass_worst_quality(VP9_COMP *cpi,
  ...
  // Adjustment to arnr filter strength;
  if (noise_factor <= NOISE_FACTOR_MIN) {
    twopass->arnr_strength_adjustment = -1;
  } else if (noise_factor >= NOISE_FACTOR_MAX) {
    twopass->arnr_strength_adjustment = 1;
  } else {
