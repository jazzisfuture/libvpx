...
static void allocate_gf_group_bits(VP9_COMP *cpi, int64_t gf_group_bits,
  ...
  normal_frames = rc->baseline_gf_interval -
                  (key_frame || rc->source_alt_ref_pending);
  if (normal_frames > 1)
...
static void define_gf_group(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  while (i < rc->static_scene_max_gf_interval && i < rc->frames_to_key) {
    ...
    if ((twopass->kf_zeromotion_pct < 99) &&
  ...
  if ((twopass->kf_zeromotion_pct < 99) && allow_alt_ref &&
  ...
  rc->baseline_gf_interval = (twopass->kf_zeromotion_pct < 99)
      ? (i - (is_key_frame || rc->source_alt_ref_pending)) : i;
