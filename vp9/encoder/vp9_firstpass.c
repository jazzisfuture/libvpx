...
static void find_arf_order(int *stack, int *layer_depth, int *index_counter,
  ...
  find_arf_order(stack, layer_depth, index_counter, depth + 1, start, mid);
  find_arf_order(stack, layer_depth, index_counter, depth + 1, mid, end);
}
...
static void define_gf_group_structure(VP9_COMP *cpi) {
  ...
  if (rc->source_alt_ref_pending) {
    ...
    stack_push(arf_order, gf_group->arf_src_offset[1]);
    layer_depth[index_counter] = 1;
    ++index_counter;
    find_arf_order(arf_order, layer_depth, &index_counter, 2, 0,
