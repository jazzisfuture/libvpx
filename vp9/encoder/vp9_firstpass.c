...
#if !ENABLE_MT_BIT_MATCH
    fps->new_mv_count = fp_acc_data->new_mv_count;
#endif
...
void vp9_first_pass_encode_tile_mb_row(VP9_COMP *cpi, ThreadData *td,
                                       FIRSTPASS_DATA *fp_acc_data,
                                       TileDataEnc *tile_data, MV *lastmv,
  ...
  for (mb_col = tile.mi_col_start >> 1, c = 0; mb_col < (tile.mi_col_end >> 1);
    ...
    (*(cpi->wpp_sync_read_ptr))(&tile_data->wpp_sync, mb_row, c - 1);
    ...
    if ((lc == NULL && cm->current_video_frame > 0) ||
      ...
      if (mb_col == (tile.mi_col_start >> 1)) {
        tile_data->tile_start_best_ref_mv = zero_mv;
      }
      ...
      if (motion_error <= this_error) {
        ...
        *best_ref_mv = mv;
        ...
        if (mb_col == (tile.mi_col_start >> 1)) {
          tile_data->tile_start_best_ref_mv = mv;
        }
        ...
        if (!is_zero_mv(&mv)) {
          ...
          // At the start of tile column store the mv
          if (mb_col == (tile.mi_col_start >> 1)) {
            tile_data->tile_start_lastmv = mv;
          }
    ...
    (*(cpi->wpp_sync_write_ptr))(&tile_data->wpp_sync, mb_row, c, num_mb_cols);
  }
  vpx_clear_system_state();
}
...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
...

  {
    ...
    if (!cpi->newMT) {
      cm->log2_tile_cols = 0;
      cpi->wpp_sync_read_ptr = vp9_wpp_sync_read_dummy;
      cpi->wpp_sync_write_ptr = vp9_wpp_sync_write_dummy;
      first_pass_encode(cpi, fp_acc_data);
      first_pass_stat_calc(cpi, &fps, fp_acc_data);
    } else {
      cpi->wpp_sync_read_ptr = vp9_wpp_sync_read;
      cpi->wpp_sync_write_ptr = vp9_wpp_sync_write;
      cpi->td.mb.src_pred_recon = 1;
#if ENABLE_MT_BIT_MATCH
      cm->log2_tile_cols = 0;
      vp9_zero_array(cpi->twopass.fp_mb_float_stats, cm->MBs);
#endif
      vp9_encode_fp_tiles_mt_wpp(cpi);
