...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
  ...
  {
    FIRSTPASS_STATS fps;
    const double min_err = (double)(cm->MBs * 4.0);
    // double min_err_factor = 1.0;

    fps.frame = cm->current_video_frame;
    fps.spatial_layer_id = cpi->svc.spatial_layer_id;
    fps.coded_error = (double)(MAX(coded_error >> 8, min_err));
    fps.intra_error = (double)(MAX(intra_error >> 8, min_err));
    fps.sr_coded_error = (double)(MAX(sr_coded_error >> 8, min_err));
    /* fps.coded_error = (double)(coded_error >> 8);
    fps.sr_coded_error = (double)(sr_coded_error >> 8);
    fps.intra_error = (double)(intra_error >> 8);
    if ((fps.coded_error < min_err) && (fps.coded_error > 0.0)) {
      min_err_factor = min_err / DOUBLE_DIVIDE_CHECK(fps.coded_error);
      fps.coded_error = min_err;
      fps.sr_coded_error = fps.sr_coded_error * min_err_factor;
      fps.intra_error = fps.intra_error * min_err_factor;
    } */
    fps.count = 1.0;
