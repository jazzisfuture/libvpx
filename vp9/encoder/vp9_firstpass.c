...
void vp9_get_one_pass_params(VP9_COMP *cpi) {
  ...
  if (!cpi->refresh_alt_ref_frame &&
    ...
    cpi->rc.source_alt_ref_active = 0;
    cpi->rc.per_frame_bandwidth = cpi->rc.av_per_frame_bandwidth * 4;
  } else {
    cm->frame_type = INTER_FRAME;
...
void vp9_get_one_pass_cbr_params(VP9_COMP *cpi) {
  ...
  if ((cm->current_video_frame == 0 ||
    ...
    if (cpi->common.current_video_frame == 0) {
      cpi->rc.per_frame_bandwidth = cpi->oxcf.starting_buffer_level / 2;
    } else {
      ...
      cpi->rc.per_frame_bandwidth =
          ((16 + kf_boost) * cpi->rc.av_per_frame_bandwidth) >> 4;
    }
  } else {
    cm->frame_type = INTER_FRAME;
    cpi->rc->active_worst_quality =
        adjust_active_worst_quality_from_buffer_level(&cpi->oxcf, &cpi->rc);
    cpi->rc.per_frame_bandwidth =
        target_size_from_buffer_level(&cpi->oxcf, &cpi->rc);
  }
