...
void vp9_get_one_pass_params(VP9_COMP *cpi) {
  ...
  if (!cpi->refresh_alt_ref_frame &&
    ...
    if (cm->current_video_frame == 0) {
      ...
      // Choose active worst quality 50% over the last q
      cpi->rc.active_worst_quality = cpi->rc.last_q[KEY_FRAME] * 24 / 16;
    ...
    if (cm->current_video_frame == 1) {
      ...
      // Choose active worst quality 50% over the last q
      cpi->rc.active_worst_quality = cpi->rc.last_q[INTER_FRAME] * 24 / 16;
      if (cpi->rc.active_worst_quality > cpi->rc.worst_quality)
        cpi->rc.active_worst_quality = cpi->rc.worst_quality;
    }
  }
