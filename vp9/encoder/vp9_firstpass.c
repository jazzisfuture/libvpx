...
void vp9_init_subsampling(VP9_COMP *cpi) {
  ...
  // Setup multipliers for rate correction factor to account for scale.
  // Assumption is that the down-scaled frame creates twice as many
  // bits per MB as for the full-sized frame.
  // TODO(agrange) Link the 2.0 here to same factor used elsewhere to account
  // for down-scaling.
  rc->rcf_mult[0] = 1.0;
  rc->rcf_mult[1] = 2.0;
}
...
void vp9_init_second_pass(VP9_COMP *cpi) {
  ...
  *stats = *twopass->stats_in_end;
  ...
  if (oxcf->resize_mode != RESIZE_NONE) {
    ...
    cpi->rc.rcf_mult[0] = cpi->rc.rcf_mult[1] = 1.0;
  }
}
...
static void define_gf_group(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if (oxcf->resize_mode == RESIZE_DYNAMIC) {
    ...
    cpi->rc.next_frame_size_selector = 0;
  }
}
