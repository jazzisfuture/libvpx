...
void define_fixed_arf_period(VP9_COMP *cpi) {
  ...
  if (cpi->rc.fs.frames_to_key <= (FIXED_ARF_GROUP_SIZE + 8)) {
    // Setup a GF group close to the keyframe.
    cpi->rc.fs.source_alt_ref_pending = 0;
    cpi->rc.fs.baseline_gf_interval = cpi->rc.fs.frames_to_key;
    schedule_frames(cpi, 0, (cpi->rc.fs.baseline_gf_interval - 1), 2, 0, 0);
  } else {
    // Setup a fixed period ARF group.
    cpi->rc.fs.source_alt_ref_pending = 1;
    cpi->rc.fs.baseline_gf_interval = FIXED_ARF_GROUP_SIZE;
    schedule_frames(cpi, 0, -(cpi->rc.fs.baseline_gf_interval - 1), 2, 1, 0);
  }
...
static void define_gf_group(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  if (active_max_gf_interval > cpi->rc.fs.max_gf_interval)
    active_max_gf_interval = cpi->rc.fs.max_gf_interval;

  i = 0;
  while (((i < cpi->twopass.static_scene_max_gf_interval) ||
          ((cpi->rc.fs.frames_to_key - i) < MIN_GF_INTERVAL)) &&
         (i < cpi->rc.fs.frames_to_key)) {
  ...
  if ((cpi->rc.fs.frames_to_key - i) < MIN_GF_INTERVAL) {
    while (i < cpi->rc.fs.frames_to_key) {
      i++;

      if (EOF == input_stats(cpi, this_frame))
        break;

      if (i < cpi->rc.fs.frames_to_key) {
        mod_frame_err = calculate_modified_err(cpi, this_frame);
        gf_group_err += mod_frame_err;
      }
    }
  ...
  cpi->rc.fs.baseline_gf_interval = i;
  ...
  if (allow_alt_ref &&
      ...
      (i <= (cpi->rc.fs.frames_to_key - MIN_GF_INTERVAL)) &&
    ...
    if (cpi->multi_arf_enabled) {
      ...
      assert(cpi->new_frame_coding_order_period ==
             cpi->rc.fs.baseline_gf_interval);
    }
#endif
  }
  ...
  // key frame.
  if (cpi->rc.fs.frames_to_key >= (int)(cpi->twopass.total_stats.count -
  ...
  if (cpi->twopass.gf_group_bits >
      (int64_t)max_bits * cpi->rc.fs.baseline_gf_interval)
    cpi->twopass.gf_group_bits =
        (int64_t)max_bits * cpi->rc.fs.baseline_gf_interval;
  ...
  for (i = 0;
      i <= (cpi->rc.fs.source_alt_ref_pending &&
    ...
    boost = clamp(boost, 125, (cpi->rc.fs.baseline_gf_interval + 1) * 200);

    if (cpi->rc.fs.source_alt_ref_pending && i == 0)
      allocation_chunks = ((cpi->rc.fs.baseline_gf_interval + 1) * 100) + boost;
    else
      allocation_chunks = (cpi->rc.fs.baseline_gf_interval * 100) + (boost - 100);
    ...
    if (mod_frame_err <
        gf_group_err / (double)cpi->rc.fs.baseline_gf_interval) {
      double alt_gf_grp_bits =
        (double)cpi->twopass.kf_group_bits  *
        (mod_frame_err * (double)cpi->rc.fs.baseline_gf_interval) /
    ...
    if (i == 1 ||
        (!cpi->rc.fs.source_alt_ref_pending &&
         cpi->common.frame_type != KEY_FRAME)) {
...

  {
    ...
    // happened)
    if (!cpi->rc.fs.source_alt_ref_pending &&
        cpi->common.frame_type != KEY_FRAME)
    ...
    if (cpi->rc.fs.baseline_gf_interval >= 3) {
      const int boost = cpi->rc.fs.source_alt_ref_pending ?
  ...
  if (cpi->common.frame_type != KEY_FRAME) {
    ...
    for (i = 0; i < cpi->rc.fs.baseline_gf_interval; i++) {
...
void vp9_second_pass(VP9_COMP *cpi) {
  ...
  if (cpi->rc.fs.frames_till_gf_update_due == 0) {
...
static void find_next_key_frame(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  cpi->rc.fs.this_key_frame_forced = cpi->rc.fs.next_key_frame_forced;

  // Clear the alt ref active flag as this can never be active on a key frame
  cpi->rc.fs.source_alt_ref_active = 0;

  // Kf is always a gf so clear frames till next gf counter
  cpi->rc.fs.frames_till_gf_update_due = 0;

  cpi->rc.fs.frames_to_key = 1;
  ...
  while (cpi->twopass.stats_in < cpi->twopass.stats_in_end) {
    ...
    if (cpi->oxcf.auto_key
      ...
      if (cpi->rc.fs.frames_to_key >= 2 * (int)cpi->key_frame_frequency)
        break;
    } else {
      cpi->rc.fs.frames_to_key++;
    }
  ...
  if (cpi->oxcf.auto_key
      && cpi->rc.fs.frames_to_key > (int)cpi->key_frame_frequency) {
    FIRSTPASS_STATS *current_pos = cpi->twopass.stats_in;
    FIRSTPASS_STATS tmp_frame;

    cpi->rc.fs.frames_to_key /= 2;
    ...
    for (i = 0; i < cpi->rc.fs.frames_to_key; i++) {
    ...
    cpi->rc.fs.next_key_frame_forced = 1;
  } else {
    cpi->rc.fs.next_key_frame_forced = 0;
  }
  ...
  if ((cpi->twopass.bits_left > 0) &&
    ...
    max_grp_bits = (int64_t)max_bits * (int64_t)cpi->rc.fs.frames_to_key;
  ...
  for (i = 0; i < cpi->rc.fs.frames_to_key; i++) {
    ...
    if (i <= (cpi->rc.fs.max_gf_interval * 2)) {
  ...
  if (1) {
    ...
    if (kf_boost < (cpi->rc.fs.frames_to_key * 3))
      kf_boost = (cpi->rc.fs.frames_to_key * 3);
    ...
    // key frame.
    // cpi->rc.fs.frames_to_key-1 because key frame itself is taken
    // care of by kf_boost.
    if (zero_motion_accumulator >= 0.99) {
      allocation_chunks =
        ((cpi->rc.fs.frames_to_key - 1) * 10) + kf_boost;
    } else {
      allocation_chunks =
        ((cpi->rc.fs.frames_to_key - 1) * 100) + kf_boost;
    }
    ...
    if (kf_mod_err < kf_group_err / cpi->rc.fs.frames_to_key) {
      double  alt_kf_grp_bits =
        ((double)cpi->twopass.bits_left *
         (kf_mod_err * (double)cpi->rc.fs.frames_to_key) /
