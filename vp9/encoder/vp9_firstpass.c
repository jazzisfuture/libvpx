...
static void accumulate_stats(FIRSTPASS_STATS *section,
  ...
  section->dead_zone_rows += frame->dead_zone_rows;
...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
...

  {
    ...
    fps.dead_zone_rows = (double)image_data_start_row;
...
static int get_twopass_worst_quality(const VP9_COMP *cpi,
  ...
  dead_zone = (dead_zone < 0.0) ? 0.0 : (dead_zone > 1.0) ? 1.0 : dead_zone;
  ...
  if (section_target_bandwidth <= 0) {
    ...
    const int num_mbs = (cpi->oxcf.resize_mode != RESIZE_NONE)
                        ? cpi->initial_mbs : cpi->common.MBs;
    const int active_mbs = MAX(1, num_mbs - (int)(num_mbs * dead_zone));
...
static void define_gf_group(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  double gf_group_dead_zone_rows = 0.0;
  ...
  if (is_key_frame || rc->source_alt_ref_active) {
    ...
    gf_group_dead_zone_rows -= this_frame->dead_zone_rows;
  }
  ...
  if ((cpi->oxcf.rc_mode != VPX_Q) && (rc->baseline_gf_interval > 1)) {
    ...
    const double group_av_dead_zone =
      ((gf_group_dead_zone_rows * 2) /
    ...
    tmp_q =
      get_twopass_worst_quality(cpi, group_av_err,
                                (group_av_skip_pct + group_av_dead_zone),
...
void vp9_rc_get_second_pass_params(VP9_COMP *cpi) {
  ...
  if (cpi->oxcf.rc_mode == VPX_Q) {
    ...
    const double section_dead_zone =
      (twopass->total_left_stats.dead_zone_rows * 2) /
      ((double)cm->mb_rows * section_length);
    const int tmp_q =
      get_twopass_worst_quality(cpi, section_error,
                                section_intra_skip + section_dead_zone,
