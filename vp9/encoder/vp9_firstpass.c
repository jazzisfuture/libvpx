...
#define CORPUS_VBR_MIDPOINT 110.0
#endif
  ...
  // that is double that of frame B will be allocated 2x as many bits.
  {
    ...
    while (s < twopass->stats_in_end) {
      modified_score_total +=
          calculate_mod_frame_score(cpi, twopass, oxcf, s, av_err);
      ++s;
    }
...
static void find_next_key_frame(VP9_COMP *cpi, FIRSTPASS_STATS *this_frame) {
  ...
  GF_GROUP *const gf_group = &twopass->gf_group;
  const VP9EncoderConfig *const oxcf = &cpi->oxcf;
  const FIRSTPASS_STATS *const tot_stats = &twopass->total_stats;
