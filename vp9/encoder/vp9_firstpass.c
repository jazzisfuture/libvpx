...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
  ...
  for (mb_row = 0; mb_row < cm->mb_rows; ++mb_row) {
    ...
    for (mb_col = 0; mb_col < cm->mb_cols; ++mb_col) {
      ...
      x->skip_encode = 0;
      xd->mi_test[0].target->mbmi.mode = DC_PRED;
      xd->mi_test[0].target->mbmi.tx_size = use_dc_pred ?
      ...
      if ((lc == NULL && cm->current_video_frame > 0) ||
        ...
        if (motion_error <= this_error) {
          ...
          xd->mi_test[0].target->mbmi.mode = NEWMV;
          xd->mi_test[0].target->mbmi.mv[0].as_mv = mv;
          xd->mi_test[0].target->mbmi.tx_size = TX_4X4;
          xd->mi_test[0].target->mbmi.ref_frame[0] = LAST_FRAME;
          xd->mi_test[0].target->mbmi.ref_frame[1] = NONE;
