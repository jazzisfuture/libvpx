...
static void output_fpmb_stats(uint8_t *this_frame_mb_stats, VP9_COMMON *cm,
  ...
  pkt.data.firstpass_mb_stats.sz = cpi->MBs * sizeof(uint8_t);
  vpx_codec_pkt_list_add(pktlist, &pkt);
}
#endif
...
void vp9_first_pass(VP9_COMP *cpi, const struct lookahead_entry *source) {
  ...
  if (cpi->use_fp_mb_stats) {
    vp9_zero_array(cpi->twopass.frame_mb_stats_buf, cpi->MBs);
  }
#endif
  ...
  vp9_clear_system_state();
  {
    ...
    const double min_err = 200 * sqrt(cpi->common.MBs);
    ...
    fps.count = 1.0;
    fps.pcnt_inter = (double)intercount / cpi->common.MBs;
    fps.pcnt_second_ref = (double)second_ref_count / cpi->common.MBs;
    fps.pcnt_neutral = (double)neutral_count / cpi->common.MBs;
