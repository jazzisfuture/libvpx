...
static void adjust_group_arnr_filter(VP9_COMP *cpi,
  ...
  // Noise based adjustment to arnr filter strength;
  if (section_noise <= (SECTION_NOISE_DEF * 0.9)) {
    twopass->arnr_strength_adjustment -= 1;
  } else if (section_noise >= (SECTION_NOISE_DEF * 1.1)) {
    twopass->arnr_strength_adjustment += 1;
  }

  // Motion based adjustment.
  if (section_motion > 0.9)
    twopass->arnr_strength_adjustment -= 1;
  else if (section_zeromv > 0.50)
    twopass->arnr_strength_adjustment += 1;
}
