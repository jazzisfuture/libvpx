...
static void find_arf_order(GF_GROUP *gf_group, int *index_counter, int depth,
  ...
  gf_group->gfu_boost[*index_counter] =
      VPXMAX(MIN_ARF_GF_BOOST,
             calc_arf_boost(cpi, end - mid, mid - start) >> depth);
...
static void allocate_gf_group_bits(VP9_COMP *cpi, int64_t gf_group_bits,
  ...
  if (cpi->multi_layer_arf) {
    int idx;
    int64_t arf_depth_bits[10] = {0};
    int arf_depth_count[10] = {0};
    int arf_depth_boost[10] = {0};
    ...
    for (idx = 2; idx < 10; ++idx) {
      ...
      arf_depth_bits[idx] =
          calculate_boost_bits(rc->baseline_gf_interval,
      ...
      switch (gf_group->update_type[idx]) {
        case ARF_UPDATE:
          gf_group->bit_allocation[idx] =
              ...
              gf_group->gfu_boost[idx]) / arf_depth_boost[gf_group->layer_depth[idx]]);
          break;
