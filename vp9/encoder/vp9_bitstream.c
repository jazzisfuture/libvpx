...
static void write_modes_sb(VP9_COMP *cpi, MODE_INFO *m, vp9_writer *bc,
  ...
  if (cpi->dummy_packing == 0 && (mi_row >> 3) == 0 && (mi_col >> 3) == 0) {
    FILE *pf = fopen("enc_range.txt", "a");
    fprintf(pf, "position (%d, %d), partition %d, range %d\n",
            mi_row, mi_col, partition, bc->range);
    fclose(pf);
  }
  ...
  for (mi_row = c->cur_tile_mi_row_start;
    ...
    for (mi_col = c->cur_tile_mi_col_start;
      ...
      if (cpi->dummy_packing == 0) {
        FILE *pf = fopen("enc_range.txt", "a");
        fprintf(pf, "position (%d, %d), range %d\n", mi_row, mi_col, bc->range);
        fclose(pf);
      }
