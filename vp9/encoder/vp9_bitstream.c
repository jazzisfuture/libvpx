        ...
        for (idx = 0; idx < 2; idx += num_4x4_w) {
          ...
          if (b_mode == NEWMV) {
            ...
            for (ref = 0; ref < 1 + is_compound; ++ref) {
              ...
              vp9_append_sub8x8_mvs_for_idx(cm, xd, tile, j, ref,
                                            mi_row, mi_col,
                                            mv_ref_list,
                                            &nearest_sub8x8[ref],
                                            &near_sub8x8[ref]);
              ...
              if (cm->current_video_frame == 186 &&
                  mi_row == 2 && mi_col == 20 && j == 1) {
                printf("vp9_encode_mv()<sub8x8: "
                       "Frame#=%d Block(%d,%d) Seg(%d) Ref=%d "
                       "MV=(%d,%d) ref_mv=(%d,%d)\n",
                       cm->current_video_frame, mi_row, mi_col, j, ref,
                       mi->bmi[j].as_mv[ref].as_mv.row,
                       mi->bmi[j].as_mv[ref].as_mv.col,
                       ref_mv[ref].as_mv.row, ref_mv[ref].as_mv.col);
              }
