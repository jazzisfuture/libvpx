...
static void pack_inter_mode_mvs(VP9_COMP *cpi, MODE_INFO *m,
  ...
  if (rf == INTRA_FRAME) {
    ...
    if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
      ...
      int idx, idy;
      int b4x4s_wide = block_type_to_b4x4s_wide[mi->sb_type];
      int b4x4s_high = block_type_to_b4x4s_high[mi->sb_type];
...
static void write_mb_modes_kf(const VP9_COMP *cpi,
  ...
  if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    int idx, idy;
    int b4x4s_wide = block_type_to_b4x4s_wide[m->mbmi.sb_type];
    int b4x4s_high = block_type_to_b4x4s_high[m->mbmi.sb_type];
    for (idy = 0; idy < 2; idy += b4x4s_high) {
      for (idx = 0; idx < 2; idx += b4x4s_wide) {
        int i = idy * 2 + idx;
...
static void write_modes_sb(VP9_COMP *cpi, MODE_INFO *m, vp9_writer *bc,
  ...
  partition = block_type_to_partition[bsize][m->mbmi.sb_type];
