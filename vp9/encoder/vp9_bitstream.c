...
static void write_nzcs_sb64(VP9_COMMON *const cm,
                            MACROBLOCKD *xd,
                            int mb_row,
                            int mb_col,
                            vp9_writer* const bc) {
...
static void write_nzcs_sb32(VP9_COMMON *const cm,
                            MACROBLOCKD *xd,
                            int mb_row,
                            int mb_col,
                            vp9_writer* const bc) {
...
static void write_nzcs_mb16(VP9_COMMON *const cm,
                            MACROBLOCKD *xd,
                            int mb_row,
                            int mb_col,
                            vp9_writer* const bc) {
...
static void write_modes_b(VP9_COMP *cpi, MODE_INFO *m, vp9_writer *bc,
  ...
  if (m->mbmi.sb_type == BLOCK_SIZE_SB64X64)
    write_nzcs_sb64(&cpi->common, xd, mb_row, mb_col, bc);
  else if (m->mbmi.sb_type == BLOCK_SIZE_SB32X32)
    write_nzcs_sb32(&cpi->common, xd, mb_row, mb_col, bc);
  else
    write_nzcs_mb16(&cpi->common, xd, mb_row, mb_col, bc);
#endif
...
void vp9_pack_bitstream(VP9_COMP *cpi, unsigned char *dest,
  ...
  // NOTE that if the conts are reset, we also need to uncomment
