    ...
    for (j = 0; j < COEF_BANDS; ++j) {
      for (k = 0; k < PREV_COEF_CONTEXTS; ++k) {
        ...
        vp9_tree_probs_from_distribution(
          ...
          cpi->frame_coef_probs_4x4[i][j][k],
          cpi->frame_branch_ct_4x4[i][j][k],
          cpi->coef_counts_4x4[i][j][k],
          256, 1
        );
#ifdef ENTROPY_STATS
        if (!cpi->dummy_packing)
          for (t = 0; t < MAX_ENTROPY_TOKENS; ++t)
            context_counters_4x4[i][j][k][t] += cpi->coef_counts_4x4[i][j][k][t];
#endif
      }
    }
  }
  for (i = 0; i < BLOCK_TYPES_4X4; ++i) {
    for (j = 0; j < COEF_BANDS; ++j) {
      for (k = 0; k < PREV_COEF_CONTEXTS; ++k) {
        ...
        vp9_tree_probs_from_distribution(
          MAX_ENTROPY_TOKENS, vp9_coef_encodings, vp9_coef_tree,
          cpi->frame_hybrid_coef_probs_4x4[i][j][k],
          cpi->frame_hybrid_branch_ct_4x4[i][j][k],
          cpi->hybrid_coef_counts_4x4[i][j][k],
          256, 1
        );
#ifdef ENTROPY_STATS
        if (!cpi->dummy_packing)
          for (t = 0; t < MAX_ENTROPY_TOKENS; ++t)
            hybrid_context_counters_4x4[i][j][k][t] += cpi->hybrid_coef_counts_4x4[i][j][k][t];
#endif
      }
    }
  }
    ...
    for (i = 0; i < BLOCK_TYPES_32X32; ++i) {
      for (j = 0; j < COEF_BANDS; ++j) {
        for (k = 0; k < PREV_COEF_CONTEXTS; ++k) {
          ...
          if (!cpi->dummy_packing)
            for (t = 0; t < MAX_ENTROPY_TOKENS; ++t)
              context_counters_32x32[i][j][k][t] += cpi->coef_counts_32x32[i][j][k][t];
#endif
        }
      }
    }
