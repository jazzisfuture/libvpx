...
static void write_mb_modes_kf(const VP9_COMP *cpi,
  ...
  if (ym == B_PRED) {
    ...
    do {
      ...
      const B_PREDICTION_MODE L = (xd->left_available || (i & 3)) ? left_block_mode(m, i) : B_DC_PRED;
...
void vp9_pack_bitstream(VP9_COMP *cpi, unsigned char *dest,
  ...
  *size = VP9_HEADER_SIZE + extra_bytes_packed + header_bc.pos + 3 * (pc->tile_columns - 1);
...

  {
    int mb_start = 0, tile;
    int total_size = 0;
    unsigned char *size_ptr = cx_data + header_bc.pos;
    unsigned char *data_ptr = size_ptr + 3 * (pc->tile_columns -1);
    ...
    for (tile = 0; tile < pc->tile_columns; tile++) {
      ...
      if (tile < pc->tile_columns - 1) {
        /* size of this tile */
        size_ptr[0] = residual_bc.pos;
        size_ptr[1] = residual_bc.pos >> 8;
        size_ptr[2] = residual_bc.pos >> 16;
        size_ptr += 3;
      }
