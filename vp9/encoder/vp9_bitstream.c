...
static void update_mode(
  ...
  do {
    new_b += cost_branch256(bct[i], Pnew[i]);
    old_b += cost_branch256(bct[i], Pcur[i]);
  } while (++i < n);

  if (new_b + 2048 * n < old_b) {
    int i = 0;
...
static void update_mbintra_mode_probs(VP9_COMP* const cpi,
  ...
  for (i = 0; i < 4; i++)
    update_mode(bc, VP9_INTRA_MODES, vp9_intra_mode_encodings,
                ...
                cm->fc.y_mode_prob[i], bct,
                (unsigned int *)cpi->y_mode_count[i]);
}
...
void vp9_pack_bitstream(VP9_COMP *cpi, uint8_t *dest, unsigned long *size) {
  ...
  vp9_copy(cpi->common.fc.pre_partition_prob, cpi->common.fc.partition_prob[1]);
  ...
  if (pc->frame_type != KEY_FRAME) {
    ...
    for (i = 0; i < INTRA_INTER_CONTEXTS; i++) {
      pnew[i][0] = get_binary_prob(cpi->intra_inter_count[i][0],
                                   cpi->intra_inter_count[i][1]);
      cnew += cost_branch256(cpi->intra_inter_count[i], pnew[i][0]);
      cold += cost_branch256(cpi->intra_inter_count[i],
                             pc->fc.intra_inter_prob[i]);
    }
    if (cnew + INTRA_INTER_CONTEXTS * 2048 < cold) {
      vp9_write_bit(&header_bc, 1);
      for (i = 0; i < INTRA_INTER_CONTEXTS; i++) {
        vp9_write_prob(&header_bc, pnew[i][0]);
        pc->fc.intra_inter_prob[i] = pnew[i][0];
      }
    } else {
      ...
      if (use_compound_pred) {
        ...
        if (use_hybrid_pred) {
          ...
          for (i = 0; i < COMP_INTER_CONTEXTS; i++) {
            pnew[i][0] = get_binary_prob(cpi->comp_inter_count[i][0],
                                         cpi->comp_inter_count[i][1]);
            cnew += cost_branch256(cpi->comp_inter_count[i], pnew[i][0]);
            cold += cost_branch256(cpi->comp_inter_count[i],
                                   pc->fc.comp_inter_prob[i]);
          }
          if (cnew + 2048 * COMP_INTER_CONTEXTS < cold) {
            vp9_write_bit(&header_bc, 1);
            for (i = 0; i < COMP_INTER_CONTEXTS; i++) {
              vp9_write_prob(&header_bc, pnew[i][0]);
              pc->fc.comp_inter_prob[i] = pnew[i][0];
            }
          } else {
            vp9_write_bit(&header_bc, 0);
          }
    ...
    if (pc->comp_pred_mode != COMP_PREDICTION_ONLY) {
      ...
      for (i = 0; i < REF_CONTEXTS; i++) {
        pnew[i][0] = get_binary_prob(cpi->single_ref_count[i][0][0],
                                     cpi->single_ref_count[i][0][1]);
        pnew[i][1] = get_binary_prob(cpi->single_ref_count[i][1][0],
                                     cpi->single_ref_count[i][1][1]);
        cnew += cost_branch256(cpi->single_ref_count[i][0], pnew[i][0]);
        cold += cost_branch256(cpi->single_ref_count[i][0],
                               pc->fc.single_ref_prob[i][0]);
        cnew += cost_branch256(cpi->single_ref_count[i][1], pnew[i][1]);
        cold += cost_branch256(cpi->single_ref_count[i][1],
                               pc->fc.single_ref_prob[i][1]);
      }
      if (cnew + 2048 * 2 * REF_CONTEXTS < cold) {
        vp9_write_bit(&header_bc, 1);
        for (i = 0; i < REF_CONTEXTS; i++) {
          vp9_write_prob(&header_bc, pnew[i][0]);
          vp9_write_prob(&header_bc, pnew[i][1]);
          pc->fc.single_ref_prob[i][0] = pnew[i][0];
          pc->fc.single_ref_prob[i][1] = pnew[i][1];
        }
      } else {
        vp9_write_bit(&header_bc, 0);
      }
    ...
    if (pc->comp_pred_mode != SINGLE_PREDICTION_ONLY) {
      cnew = cold = 0;
      for (i = 0; i < REF_CONTEXTS; i++) {
        pnew[i][0] = get_binary_prob(cpi->comp_ref_count[i][0],
                                     cpi->comp_ref_count[i][1]);
        cnew += cost_branch256(cpi->comp_ref_count[i], pnew[i][0]);
        cold += cost_branch256(cpi->comp_ref_count[i], pc->fc.comp_ref_prob[i]);
      }
      if (cnew + 2048 * REF_CONTEXTS < cold) {
        vp9_write_bit(&header_bc, 1);
        for (i = 0; i < REF_CONTEXTS; i++) {
          vp9_write_prob(&header_bc, pnew[i][0]);
          pc->fc.comp_ref_prob[i] = pnew[i][0];
        }
      } else {
        vp9_write_bit(&header_bc, 0);
      }
