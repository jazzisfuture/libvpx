  ...
  if (rf == INTRA_FRAME) {
    ...
    if (mode == NEWMV) {
      ...
      if (mi->second_ref_frame > 0) {
        // Encode the index of the choice.
        vp9_write_mv_ref_id(bc, xd->mb_mv_ref_id_probs[mi->second_ref_frame],
...
#if CONFIG_SUPERBLOCKS
    const int nmbs = 1 << m->mbmi.encoded_as_sb;
#else
    const int nmbs = 1;
#endif
    const int xmbs = min(nmbs, mb_cols_left);
...
#if CONFIG_SUPERBLOCKS64 && CONFIG_SUPERBLOCKS64
      vp9_write(bc, m->mbmi.encoded_as_sb == 2, c->sb64_coded);
      if (m->mbmi.encoded_as_sb == 2) {
        ...
        for (j = 0; j < 4; j++) {
...
#if CONFIG_SUPERBLOCKS
          vp9_write(bc, sb_m->mbmi.encoded_as_sb, c->sb32_coded);
          if (sb_m->mbmi.encoded_as_sb) {
            assert(sb_m->mbmi.encoded_as_sb == 1);
...
#endif
          {
            ...
            for (i = 0; i < 4; i++) {
              ...
              if (mb_row + y_idx >= c->mb_rows || mb_col + x_idx >= c->mb_cols) {
                // MB lies outside frame, move on
                continue;
              }

              assert(mb_m->mbmi.encoded_as_sb == 0);
