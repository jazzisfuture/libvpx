...
static int prob_diff_update_savings_search_model(const unsigned int *ct,
  ...
  for (i = 1, old_b = 0; i < ENTROPY_NODES; ++i)
    old_b += cost_branch256(ct + 2 * i, oldp[i]);

  bestsavings = 0;
  bestnewp = oldp[1];

  step = (*bestp > oldp[1] ? -1 : 1);
  newp = *bestp;
  // newp = *bestp - step * (abs(*bestp - oldp[1]) >> 1);
  for (; newp != oldp[1]; newp += step) {
    if (newp < 1 || newp > 255) continue;
    vp9_get_model_distribution(newp, newplist, b, r);
    for (i = 1, new_b = 0; i < ENTROPY_NODES; ++i)
      new_b += cost_branch256(ct + 2 * i, newplist[i]);
    update_b = prob_diff_update_cost(newp, oldp[1]) + vp9_cost_upd256;
    savings = old_b - new_b - update_b;
    if (savings > bestsavings) {
      bestsavings = savings;
      bestnewp = newp;
