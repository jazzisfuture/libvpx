...
static void pack_inter_mode_mvs(VP9_COMP *cpi, MODE_INFO *m,
  ...
  if (rf == INTRA_FRAME) {
    ...
    if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
      ...
      int idx, idy;
      int block_width = block_type_to_width[mi->sb_type];
      int block_height = block_type_to_height[mi->sb_type];
...
static void write_mb_modes_kf(const VP9_COMP *cpi,
  ...
  if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    int idx, idy;
    int block_width = block_type_to_width[m->mbmi.sb_type];
    int block_height = block_type_to_height[m->mbmi.sb_type];
    for (idy = 0; idy < 2; idy += block_height) {
      for (idx = 0; idx < 2; idx += block_width) {
        int i = idy * 2 + idx;
