...
static void update_coef_probs(VP9_COMP* cpi, vp9_writer* w) {
  const TXFM_MODE mode = cpi->common.txfm_mode;

  vp9_clear_system_state();
  build_coeff_contexts(cpi);
  update_coef_probs_common(w, cpi, TX_4X4);

  if (mode > ONLY_4X4)
    update_coef_probs_common(w, cpi, TX_8X8);

  if (mode > ALLOW_8X8)
    update_coef_probs_common(w, cpi, TX_16X16);

  if (mode > ALLOW_16X16)
    update_coef_probs_common(w, cpi, TX_32X32);
}
...
void write_uncompressed_header(VP9_COMP *cpi,
  ...
  // When there is a key frame all reference buffers are updated using the new
  // key frame
  if (cm->frame_type != KEY_FRAME) {
    int refresh_mask, i;
  ...
  // Should the GF or ARF be updated using the transmitted frame or buffer
    ...
    if (cpi->refresh_golden_frame && !cpi->refresh_alt_ref_frame) {
      ...
      if (cpi->multi_arf_enabled) {
        const int sn = cpi->sequence_number;
