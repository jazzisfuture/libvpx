...
static void build_tree_distribution(VP9_COMP *cpi, TX_SIZE txfm_size) {
  ...
  vp9_coeff_stats_model *coef_branch_ct = cpi->frame_branch_ct[txfm_size];
#ifdef ENTROPY_STATS
...
static void build_coeff_contexts(VP9_COMP *cpi) {
  build_tree_distribution(cpi, TX_4X4);
  build_tree_distribution(cpi, TX_8X8);
  build_tree_distribution(cpi, TX_16X16);
  build_tree_distribution(cpi, TX_32X32);
}
...
static void update_coef_probs_common(vp9_writer* const bc, VP9_COMP *cpi,
  ...
  vp9_coeff_stats_model *frame_branch_ct = cpi->frame_branch_ct[tx_size];
  int i, j, k, l, t;
  int update[2] = {0, 0};
  int savings;
...
void print_tree_update_probs() {
  ...
  fwrite(tree_update_hist[TX_4X4],   sizeof(tree_update_hist[TX_4X4]),   1, f);
  fwrite(tree_update_hist[TX_8X8],   sizeof(tree_update_hist[TX_8X8]),   1, f);
  fwrite(tree_update_hist[TX_16X16], sizeof(tree_update_hist[TX_16X16]), 1, f);
  fwrite(tree_update_hist[TX_32X32], sizeof(tree_update_hist[TX_32X32]), 1, f);
  fclose(f);
}
#endif
