...
static void update_switchable_interp_probs(VP9_COMP *cpi, vp9_writer *w) {
  ...
  for (j = 0; j < SWITCHABLE_FILTER_CONTEXTS; ++j)
    prob_diff_update(vp9_switchable_interp_tree, cm->fc.switchable_interp_prob[j],
  ...
  if (!frame_is_intra_only(cm)) {
    ...
    for (i = 0; i < BLOCK_SIZE_GROUPS; ++i)
      prob_diff_update(vp9_intra_mode_tree, cm->fc.y_mode_prob[i],
                  (unsigned int *)cpi->y_mode_count[i], INTRA_MODES, &header_bc);

    for (i = 0; i < PARTITION_CONTEXTS; ++i)
      prob_diff_update(vp9_partition_tree, fc->partition_prob[i],
                  (unsigned int *)cpi->partition_count[i], PARTITION_TYPES, &header_bc);
