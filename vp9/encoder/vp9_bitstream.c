...
static void update_mbintra_mode_probs(VP9_COMP* const cpi,
  ...
  for (i = 0; i < BLOCK_SIZE_TYPES; i++)
    update_mode(bc, VP9_INTRA_MODES, vp9_intra_mode_encodings,
                ...
                cm->fc.y_mode_prob[i], bct, (unsigned int *)cpi->y_mode_count[i]);
}
...
static void pack_inter_mode_mvs(VP9_COMP *cpi, MODE_INFO *m,
  ...
  if (rf == INTRA_FRAME) {
    ...
    if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
      write_intra_mode(bc, mode, pc->fc.y_mode_prob[m->mbmi.sb_type]);
    } else {
      int idx, idy;
      ...
      for (idy = 0; idy < 2; idy += bh)
        for (idx = 0; idx < 2; idx += bw) {
          ...
          write_intra_mode(bc, bm, pc->fc.y_mode_prob[m->mbmi.sb_type]);
        }
    }
