...
static void write_selected_tx_size(const VP9_COMP *cpi, const MODE_INFO *m,
...
static int write_skip(const VP9_COMP *cpi, int segment_id, const MODE_INFO *m,
                      vp9_writer *w) {
  const MACROBLOCKD *const xd = &cpi->mb.e_mbd;
...
static void write_mb_modes_kf(const VP9_COMP *cpi, MODE_INFO **mi_8x8,
  ...
  if (bsize >= BLOCK_8X8 && cm->tx_mode == TX_MODE_SELECT)
    write_selected_tx_size(cpi, mi, mbmi->tx_size, bsize, w);
