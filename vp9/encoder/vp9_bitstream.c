...
#if CONFIG_NEW_QUANT && CONFIG_SWITCHABLE_DQ
static struct vp9_token dq_profile_encodings[QUANT_PROFILES];
#endif  // CONFIG_NEW_QUANT && CONFIG_SWITCHABLE_DQ
...
static void update_dq_profile_probs(VP9_COMMON *cm, vp9_writer *w) {
  ...
  printf("\ndq_indices: %d %d %d\n", cm->counts.dq_profile[0],
         cm->counts.dq_profile[1], cm->counts.dq_profile[2]);
}
#endif  // CONFIG_NEW_QUANT && CONFIG_SWITCHABLE_DQ
