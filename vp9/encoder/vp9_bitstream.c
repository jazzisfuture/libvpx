...
static void write_modes_mvs_inter_b(VP9_COMP *cpi,
                                    MODE_INFO *m, int mis,
                                    vp9_writer *const bc,
                                    int mb_row, int mb_col) {
  VP9_COMMON *const pc = &cpi->common;
  nmv_context *const nmvc = &pc->fc.nmvc;
  ...
  int skip_coeff = 1;
  const int mb_size = 1 << mi->encoded_as_sb;
  const int ymbs = min(pc->mb_rows - mb_row, mb_size);
  const int xmbs = min(pc->mb_cols - mb_col, mb_size);
  ...
  if (pc->mb_no_coeff_skip &&
    ...
    for (y = 0; y < ymbs; y++) {
      for (x = 0; x < xmbs; x++) {
        skip_coeff = m[y * mis + x].mbmi.mb_skip_coeff;
      }
    }

    vp9_write(bc, skip_coeff, vp9_get_pred_prob(pc, xd, PRED_MBSKIP));
  }
 ...
 if (rf == INTRA_FRAME) {
   ...
   if (!vp9_segfeature_active(xd, segment_id, SEG_LVL_MODE)) {
    ...
    if (mode == B_PRED) {
      ...
      int uses_second =
          m->bmi[0].as_mode.second != (B_PREDICTION_MODE)(B_DC_PRED - 1);
...
// #ifdef ENTROPY_STATS
    ...
    if (mode >= NEARESTMV && mode < SPLITMV) {
      if (cpi->common.pred_filter_mode == 2)
        ...
        assert(mi->pred_filter_enabled == cpi->common.pred_filter_mode);
          }
#endif
    if (mode >= NEARESTMV && mode <= SPLITMV) {
...
static void write_modes_kf_b(const VP9_COMP *cpi,
                             const MODE_INFO   *m,
                             int                mis,
                             vp9_writer *const  bc,
                             int mb_row, int mb_col) {
  ...
  int skip_coeff = 1;
  const int mb_size = 1 << m->mbmi.encoded_as_sb;
  const int ymbs = min(c->mb_rows - mb_row, mb_size);
  const int xmbs = min(c->mb_cols - mb_col, mb_size);
...
static void write_modes_b(VP9_COMP *cpi, MODE_INFO *mi,
                          int mis, vp9_writer *const bc,
  ...
  if (cm->frame_type == KEY_FRAME) {
    write_modes_kf_b(cpi, mi, mis, bc, mb_row, mb_col);
#ifdef ENTROPY_STATS
    active_section = 8;
#endif
  } else {
    ...
    x->partition_info = x->pi + idx;
    xd->prev_mode_info_context = cm->prev_mi + idx;
    write_modes_mvs_inter_b(cpi, mi, mis, bc, mb_row, mb_col);
#ifdef ENTROPY_STATS
    active_section = 1;
#endif
  }
  ...
  assert(*tok < tok_end);
  pack_mb_tokens(cm, bc, tok, tok_end);
}

static void write_modes(VP9_COMP* const cpi, vp9_writer* const bc) {
  VP9_COMMON *const cm = &cpi->common;
  const int mis = cm->mode_info_stride;
  MODE_INFO *mi, *mi_ptr = cm->mi;
  int i, j, mb_row, mb_col;
  TOKENEXTRA *tok = cpi->tok, *tok_end = tok + cpi->tok_count;

  for (mb_row = 0; mb_row < cm->mb_rows; mb_row += 4, mi_ptr += 4 * mis) {
    for (mi = mi_ptr, mb_col = 0; mb_col < cm->mb_cols; mb_col += 4, mi += 4) {
#if CONFIG_SUPERBLOCKS
      vp9_write(bc, mi->mbmi.encoded_as_sb == 2, cm->sb64_coded);
#endif
      if (mi->mbmi.encoded_as_sb == 2) {
        write_modes_b(cpi, mi, mis, bc, &tok, tok_end, mb_row, mb_col);
      } else {
        for (i = 0; i < 4; i++) {
          const int x_idx = i & 1, y_idx = i >> 1;
          MODE_INFO *sb_mi = mi + y_idx * 2 * mis + x_idx * 2;

          if (mb_row + y_idx * 2 < cm->mb_rows ||
              mb_col + x_idx * 2 < cm->mb_cols) {
            vp9_write(bc, sb_mi->mbmi.encoded_as_sb == 1, cm->sb32_coded);
            if (sb_mi->mbmi.encoded_as_sb == 1) {
              write_modes_b(cpi, sb_mi, mis, bc, &tok, tok_end,
                            mb_row + y_idx * 2, mb_col + x_idx * 2);
            } else {
              for (j = 0; j < 4; j++) {
                const int mb_x_idx = j & 1, mb_y_idx = j >> 1;
                MODE_INFO *mb_mi = sb_mi + y_idx * mis + x_idx;

                if (mb_row + y_idx * 2 + mb_y_idx < cm->mb_rows &&
                    mb_col + x_idx * 2 + mb_x_idx < cm->mb_cols) {
                  write_modes_b(cpi, mb_mi, mis, bc, &tok, tok_end,
                                mb_row + y_idx * 2 + mb_y_idx,
                                mb_col + x_idx * 2 + mb_x_idx);
                }
              }
            }
          }
