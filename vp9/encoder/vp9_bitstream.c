...
int64_t switchable_interp_stats[SWITCHABLE_CONTEXTS][SWITCHABLE_FILTERS];
...
void write_switchable_interp_stats() {
  ...
  printf(
      "vp9_default_switchable_filter_count[SWITCHABLE_CONTEXTS]"
      "[SWITCHABLE_FILTERS] = {\n");
  for (i = 0; i < SWITCHABLE_CONTEXTS; i++) {
    printf("  { ");
    for (j = 0; j < SWITCHABLE_FILTERS; j++) {
...
static void update_switchable_interp_probs(VP9_COMP *const cpi,
                                           vp9_writer* const bc) {
  VP9_COMMON *const cm = &cpi->common;
  unsigned int branch_ct[SWITCHABLE_CONTEXTS][SWITCHABLE_FILTERS - 1][2];
  vp9_prob new_prob[SWITCHABLE_CONTEXTS][SWITCHABLE_FILTERS - 1];
  int i, j;
  for (j = 0; j < SWITCHABLE_CONTEXTS; ++j) {
