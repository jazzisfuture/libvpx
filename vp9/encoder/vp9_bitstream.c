...
static int write_sr_usfilter(const VP9_COMMON *cm, const MACROBLOCKD *xd,
  ...
  vp9_write_token(w, vp9_sr_usfilter_tree, vp9_get_sr_usfilter_prob(cm, xd), &sr_usfilter_encodings[f_idx]);
...
static int write_sr(const VP9_COMMON *cm, const MACROBLOCKD *xd,
  ...
  if (vp9_segfeature_active(&cm->seg, segment_id, SEG_LVL_SKIP))
    ...
    vp9_write(w, sr, vp9_get_sr_prob(cm, xd));
#if SR_USE_MULTI_F
    if (sr) {
...
#endif
#if CONFIG_SR_MODE
  write_sr(cm, xd, segment_id, mi, w);  // if sr = ..., no need to write tx size
  if (mbmi->sr && !mbmi->skip) {
    //assert(mbmi->tx_size == TX_16X16 || mbmi->tx_size == TX_32X32 || mbmi->tx_size == TX_64X64);
...
#if CONFIG_SR_MODE
  write_sr(cm, xd, mbmi->segment_id, mi, w);  // if sr = ..., no need to write tx_size
  if (mbmi->sr && !mbmi->skip) {
