...
static void pack_inter_mode_mvs(VP9_COMP *cpi, const MODE_INFO *mi,
  ...
  if (!is_inter && bsize >= BLOCK_8X8 && cm->allow_palette_mode) {
    ...
    if (mbmi->palette_enabled[0]) {
      ...
      run_lengh_decoding(cpi->mb.e_mbd.mi->mbmi.palette_runs,
                         mbmi->palette_run_length[0],
                         cpi->mb.e_mbd.palette_map_buffer);
      palette_iscan(buffer, cpi->mb.e_mbd.palette_map_buffer, rows, cols,
                    mbmi->palette_scan_order[0],
                    cpi->mb.e_mbd.palette_scan_buffer);
      vp9_write_literal(w, buffer[0], get_bit_depth(n));
      for (i = 0; i < rows; i++) {
        for (j = (i == 0 ? 1 : 0); j < cols; j++) {
          color_ctx = get_palette_color_context(buffer, cols, i, j, n,
                                                color_order);
          for (k = 0; k < n; k++)
    ...
    if (mbmi->palette_enabled[1]) {
      ...
      if (xd->plane[1].subsampling_x && xd->plane[1].subsampling_y) {
        run_lengh_decoding(cpi->mb.e_mbd.mi->mbmi.palette_runs +
                           ...
                           cpi->mb.e_mbd.palette_map_buffer);
        palette_iscan(buffer, cpi->mb.e_mbd.palette_map_buffer, rows,
...
static void write_mb_modes_kf(const VP9_COMMON *cm,
  ...
  if (bsize >= BLOCK_8X8 && cm->allow_palette_mode) {
    ...
    if (mbmi->palette_enabled[0]) {
      ...
      run_lengh_decoding(mi_8x8->mbmi.palette_runs, mbmi->palette_run_length[0],
                         xd->palette_map_buffer);
      palette_iscan(buffer, xd->palette_map_buffer, rows, cols,
    ...
    if (mbmi->palette_enabled[1]) {
      ...
      if (xd->plane[1].subsampling_x && xd->plane[1].subsampling_y) {
        run_lengh_decoding(mi_8x8->mbmi.palette_runs + PALETTE_MAX_RUNS,
                           mbmi->palette_run_length[1],
                           xd->palette_map_buffer);
        palette_iscan(buffer, xd->palette_map_buffer, rows,
                      cols, mbmi->palette_scan_order[1],
