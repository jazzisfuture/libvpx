...
static void update_inter_mode_probs(VP9_COMMON *cm,
    vp9_prob mode_context[INTER_MODE_CONTEXTS][VP9_MVREFS - 1]) {
  int i, j;
...
static void update_inter_mode_probs(VP9_COMMON *pc, vp9_writer* const bc) {
  ...
  for (i = 0; i < INTER_MODE_CONTEXTS; i++) {
    for (j = 0; j < VP9_MVREFS - 1; j++) {
      vp9_cond_prob_diff_update(bc, &pc->fc.vp9_mode_contexts[i][j],
                                VP9_DEF_UPDATE_PROB, pc->fc.mv_ref_ct[i][j]);
      // vp9_cond_prob_update(bc, &pc->fc.vp9_mode_contexts[i][j],
      //                      VP9_DEF_UPDATE_PROB, pc->fc.mv_ref_ct[i][j]);
    }
  }
}
...
void vp9_pack_bitstream(VP9_COMP *cpi, uint8_t *dest, unsigned long *size) {
  ...
  vp9_copy(cpi->common.fc.pre_mv_ref_prob,
           cpi->common.fc.vp9_mode_contexts);
