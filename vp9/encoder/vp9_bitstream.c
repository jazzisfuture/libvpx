...
static void write_modes_sb(VP9_COMP *cpi, MODE_INFO *m, vp9_writer *bc,
  ...
  int bwl = mb_width_log2(m->mbmi.sb_type),  bw = 1 << bwl;
  int bhl = mb_height_log2(m->mbmi.sb_type), bh = 1 << bhl;
  ...
  if (bsize > BLOCK_SIZE_MB16X16) {
    ...
    // use the existing probability structures for testing purpose,
    // to be continued on enabling prob tree/model of partition types.
    write_token(bc, vp9_partition_tree, cm->fc.partition_prob[bsize],
                vp9_partition_encodings + partition);
//    vp9_write(bc, partition == PARTITION_NONE,
//              (bsize == BLOCK_SIZE_SB64X64) ? cm->prob_sb64_coded :
//                                              cm->prob_sb32_coded);
  }
...
void vp9_pack_bitstream(VP9_COMP *cpi, unsigned char *dest,
  ...
  */
  ...
  for (i = 0; i < PARTITION_PLANES; i++)
    vp9_copy(cpi->common.fc.pre_partition_prob[i],
