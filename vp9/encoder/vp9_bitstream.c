...
static void update_ext_tx_probs(VP9_COMMON *cm, vp9_writer *w) {
  ...
  for (i = TX_4X4; i <= TX_SIZES; ++i) {
    savings += prob_diff_update_savings(
        GET_EXT_TX_TREE(i), cm->fc.ext_tx_prob[i], cm->counts.ext_tx[i],
        GET_EXT_TX_TYPES(i));
#else
...
#if CONFIG_WAVELETS
    for (i = TX_4X4; i <= TX_SIZES; ++i) {
      prob_diff_update(GET_EXT_TX_TREE(i), cm->fc.ext_tx_prob[i],
                       cm->counts.ext_tx[i], GET_EXT_TX_TYPES(i), w);
    }
#else
    for (i = TX_4X4; i <= TX_16X16; ++i) {
    ...
    if (supertx_enabled) {
      ...
      if (!xd->mi[0].mbmi.skip)
        vp9_write_token(w, GET_EXT_TX_TREE(supertx_size),
                        ...
                        &GET_EXT_TX_ENCODINGS[xd->mi[0].mbmi.ext_txfrm]);
#else
