...
#if CONFIG_FADE_MODE
        if (mode == FADEMV) {
          vp9_write_token(w, vp9_fade_mode_tree, cm->fc.fade_mode_probs, &fade_mode_encodings[mbmi->fade_mode]);
        }
#endif
      }
    }
...
#if CONFIG_FADE_MODE
    prob_diff_update(vp9_fade_mode_tree, cm->fc.fade_mode_probs, cm->counts.fade_mode, FADE_MODE_COUNT, &header_bc);
#endif
