...
static void pack_inter_mode_mvs(VP9_COMP *cpi, MODE_INFO *m,
  ...
  if (rf == INTRA_FRAME) {
    ...
    if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
      ...
      int idx, idy;
      int ablocks_wide = block_type_to_ablocks_wide[mi->sb_type];
      int ablocks_high = block_type_to_ablocks_high[mi->sb_type];
...
static void write_mb_modes_kf(const VP9_COMP *cpi,
  ...
  if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    int idx, idy;
    int ablocks_wide = block_type_to_ablocks_wide[m->mbmi.sb_type];
    int ablocks_high = block_type_to_ablocks_high[m->mbmi.sb_type];
    for (idy = 0; idy < 2; idy += ablocks_high) {
      for (idx = 0; idx < 2; idx += ablocks_wide) {
        int i = idy * 2 + idx;
