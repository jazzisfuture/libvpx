...
static void write_mb_modes_kf(const VP9_COMP *cpi,
  ...
  if (m->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    write_intra_mode(bc, ym, c->fc.kf_y_mode_prob[A][L]);
  } else {
    int idx, idy;
...
#endif
        write_intra_mode(bc, bm, c->fc.kf_y_mode_prob[A][L]);
      }
    }
  }

  write_intra_mode(bc, m->mbmi.uv_mode, c->fc.kf_uv_mode_prob[ym]);
}
