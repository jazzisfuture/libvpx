...
static INLINE int quantize_coeff_bigtx_nuq(const tran_low_t coeffv,
  ...
  if (i == NUQ_KNOTES) {
    q = NUQ_KNOTES +
        ((((int64_t)tmp - (cumbins_ptr[NUQ_KNOTES - 1] >>
                           (1 + logsizeby32))) * quant) >> (15 - logsizeby32));
  }
  if (q) {
    *dqcoeff_ptr =
  ...
  assert(abs(*dqcoeff_ptr - coeff) < (dequant >> (1 + logsizeby32)));
  return (q != 0);
}
...
void vp9_init_quantizer(VP9_COMP *cpi) {
  ...
  for (q = 0; q < QINDEX_RANGE; q++) {
    ...
    for (i = 0; i < 2; ++i) {
      ...
      if (q == 100 && i == 0) {
        int j;
        printf("Dequant_vals [%d]:", quant);
        for (j = 0; j <= NUQ_KNOTES; j++)
          printf(" %d,", cm->y_dequant_val_nuq[q][i][j]);
        printf("\n");
        printf("Cumbins [%d]:", quant);
        for (j = 0; j < NUQ_KNOTES; j++)
          printf(" %d,", quants->y_cumbins_nuq[q][i][j]);
        printf("\n");
      }
      */
#endif  // CONFIG_NEW_QUANT
