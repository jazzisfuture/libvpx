...
void vp9_quantize_sby_16x16(MACROBLOCK *x) {
  ...
  for (n = 0; n < 4; n++) {
    TX_TYPE tx_type = get_tx_type_16x16(&x->e_mbd, (16 * (n & 2)) + ((n & 1) * 4));
...
void vp9_quantize_sby_8x8(MACROBLOCK *x) {
  ...
  for (n = 0; n < 16; n++) {
    TX_TYPE tx_type = get_tx_type_8x8(&x->e_mbd, (4 * (n & 12)) + ((n & 3) * 2));
...
void vp9_quantize_sb64y_16x16(MACROBLOCK *x) {
  ...
  for (n = 0; n < 16; n++) {
    TX_TYPE tx_type = get_tx_type_16x16(&x->e_mbd, (16 * (n & 12)) + ((n & 3) * 4));
    x->quantize_b_16x16(x, n * 16, tx_type);
  }
}
...
void vp9_quantize_sb64y_8x8(MACROBLOCK *x) {
  ...
  for (n = 0; n < 64; n++) {
    TX_TYPE tx_type = get_tx_type_8x8(&x->e_mbd, (4 * (n & 56)) + ((n & 7) * 2));
    x->quantize_b_8x8(x, n * 4, tx_type);
  }
}
