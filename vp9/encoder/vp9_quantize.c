...
void vp9_init_plane_quantizers(VP9_COMP *cpi, MACROBLOCK *x) {
...
#if CONFIG_NEW_QUANT
  // unnecessary to set dq to mbmi->dq_off_index for initial quantization
  dq = 0;
  x->plane[0].cumbins_nuq = quants->y_cumbins_nuq[qindex];
  xd->plane[0].dequant_val_nuq = cm->y_dequant_val_nuq[dq][qindex];
#endif  // CONFIG_NEW_QUANT
#if CONFIG_TX_SKIP
  ...
  xd->plane[0].dequant_val_nuq_pxd = cm->y_dequant_val_nuq_pxd[dq][qindex];
#endif  // CONFIG_NEW_QUANT
#endif  // CONFIG_TX_SKIP
  ...
  for (i = 1; i < 3; i++) {
    ...
    xd->plane[i].dequant_val_nuq = cm->uv_dequant_val_nuq[dq][qindex];
#endif  // CONFIG_NEW_QUANT
#if CONFIG_TX_SKIP
    ...
    xd->plane[i].dequant_val_nuq_pxd = cm->uv_dequant_val_nuq_pxd[dq][qindex];
#endif  // CONFIG_NEW_QUANT
#endif  // CONFIG_TX_SKIP
