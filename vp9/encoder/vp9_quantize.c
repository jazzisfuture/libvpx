...
void vp9_init_quantizer(VP9_COMP *cpi) {
  ...
  for (q = 0; q < QINDEX_RANGE; q++) {
    ...
    for (i = 1; i < 16; i++) {
      ...
      invert_quant(cpi->uv_quant[q] + rc,
                   cpi->uv_quant_shift[q] + rc, quant_val);
      cpi->uv_zbin[q][rc] = ROUND_POWER_OF_TWO(qzbin_factor * quant_val, 7);
      cpi->uv_round[q][rc] = (qrounding_factor * quant_val) >> 7;
