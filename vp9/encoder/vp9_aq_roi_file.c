...
enum roi_segments { no_roi, roi };

#define ROI_QINDEX_DELTA -10

#define ROI_AQ_FILE_NAME "roi.txt"

int vp9_roi_setup(VP9_COMP *const cpi) {
  ...
  // Open file and read each line of segments where roi exists and frame
  // duration
  ...
  while (fscanf(fp, "%d,%d,%d,%d,%d,%d\n", &start_x, &start_y, &end_x, &end_y,
    ...
    // see if current frame is within the set of frames that these roi
    // exist in
...
void vp9_roi_print_segmentation_map(VP9_COMP *const cpi) {
  int i, j;
