...
static INLINE void read_chroma_dist_row_16(int ss_x, const uint16_t *u_dist,
  ...
  static INLINE void add_luma_dist_to_8_chroma_mod(
      const uint16_t *y_dist, int ss_x, int ss_y, uint16x8_t *u_mod,
      uint16x8_t *v_mod) {
    uint16x8_t y_reg;
    if (!ss_x) {
  ...
  static void vp9_apply_temporal_filter_luma_16(
    ...
    for (h = 1; h < block_height - 1; ++h) {
      ...
      // Get modifier and store result
      sum_row_first = average_8(sum_row_first, &mul_first, strength, rounding,
                                &weight_first);
      sum_row_second = average_8(sum_row_second, &mul_second, strength,
    ...
    if (block_width == 16) {
      ...
      if (use_whole_blk) {
        vp9_apply_temporal_filter_luma_16(
            y_pre + blk_col, y_pre_stride, 16, block_height, ss_x, ss_y,
            strength, use_whole_blk, y_accum + blk_col, y_count + blk_col,
            y_dist + blk_col, u_dist + uv_blk_col, v_dist + uv_blk_col,
            neighbors_first, neighbors_second, top_weight, bottom_weight, NULL);
      } else {
        vp9_apply_temporal_filter_luma_16(
            y_pre + blk_col, y_pre_stride, 16, block_height, ss_x, ss_y,
            strength, use_whole_blk, y_accum + blk_col, y_count + blk_col,
            y_dist + blk_col, u_dist + uv_blk_col, v_dist + uv_blk_col,
            neighbors_first, neighbors_second, 0, 0, blk_fw);
      }
  ...
  static void vp9_apply_temporal_filter_chroma_8(
      ...
      uint16_t *v_count, const uint16_t *y_dist, const uint16_t *u_dist,
      const uint16_t *v_dist, const int16_t *const *neighbors, int top_weight,
  ...
  void vp9_apply_temporal_filter_neon(
    ...
    vp9_apply_temporal_filter_luma(y_pre, y_pre_stride, block_width,
