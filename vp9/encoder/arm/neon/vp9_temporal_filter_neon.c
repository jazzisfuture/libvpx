...
static INLINE void store_dist_8(const uint8_t *a, const uint8_t *b,
  ...
  const uint16x8_t a_first = vmovl_u8(a_reg);
  const uint16x8_t b_first = vmovl_u8(b_reg);

  uint16x8_t dist_first;
  dist_first = vsubq_u16(a_first, b_first);
...
static INLINE void store_dist_16(const uint8_t *a, const uint8_t *b,
  ...
  const uint16x8_t a_first = vmovl_u8(vget_low_u8(a_reg));
  const uint16x8_t a_second = vmovl_u8(vget_high_u8(a_reg));
  const uint16x8_t b_first = vmovl_u8(vget_low_u8(b_reg));
  const uint16x8_t b_second = vmovl_u8(vget_high_u8(b_reg));

  uint16x8_t dist_first, dist_second;
  dist_first = vsubq_u16(a_first, b_first);
  dist_second = vsubq_u16(a_second, b_second);
  dist_first = vmulq_u16(dist_first, dist_first);
...
static void accumulate_and_store_8(const uint16x8_t sum_u16,
  ...
  pred_u16 = vmulq_u16(sum_u16, pred_u16);
...
static INLINE void accumulate_and_store_16(const uint16x8_t sum_0_u16,
  ...
  uint32x4_t pred_0_u32, pred_1_u32, pred_2_u32, pred_3_u32;
  ...
  pred_0_u16 = vmulq_u16(sum_0_u16, pred_0_u16);
  pred_1_u16 = vmulq_u16(sum_1_u16, pred_1_u16);

  pred_0_u32 = vmovl_u16(vget_low_u16(pred_0_u16));
  pred_1_u32 = vmovl_u16(vget_high_u16(pred_0_u16));
  pred_2_u32 = vmovl_u16(vget_low_u16(pred_1_u16));
  pred_3_u32 = vmovl_u16(vget_high_u16(pred_1_u16));
  ...
  accum_0_u32 = vaddq_u32(pred_0_u32, accum_0_u32);
  accum_1_u32 = vaddq_u32(pred_1_u32, accum_1_u32);
  accum_2_u32 = vaddq_u32(pred_2_u32, accum_2_u32);
  accum_3_u32 = vaddq_u32(pred_3_u32, accum_3_u32);
...
static INLINE void read_chroma_dist_row_16(int ss_x, const uint16_t *u_dist,
  ...
  if (!ss_x) {
    ...
    *u_first = vzip1q_u16(u_reg, u_reg);
    *u_second = vzip2q_u16(u_reg, u_reg);

    read_dist_8(v_dist, &v_reg);

    *v_first = vzip1q_u16(v_reg, v_reg);
    *v_second = vzip2q_u16(v_reg, v_reg);
  }
}
