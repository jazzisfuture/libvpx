...
static INLINE void scale_plane_2_to_1_phase_non_0_c(
...
static INLINE void scale_plane_2_to_1_phase_non_0_neon(
    const uint8_t *const src, const int src_stride, uint8_t *dst,
    const int dst_stride, const int w, const int h, const int16_t c0,
    const int16_t c1) {
  const int max_width = (w + 7) & ~7;
...
void vp9_scale_and_extend_frame_neon(const YV12_BUFFER_CONFIG *src,
  ...
  if (2 * dst_w == src_w && 2 * dst_h == src_h) {
    if (phase_scaler == 0) {
      ...
      scale_plane_2_to_1_phase_non_0_neon(src->y_buffer, src->y_stride,
                                          dst->y_buffer, dst->y_stride, dst_w,
                                          dst_h, c0, c1);
      scale_plane_2_to_1_phase_non_0_neon(src->u_buffer, src->uv_stride,
                                          dst->u_buffer, dst->uv_stride,
                                          dst_uv_w, dst_uv_h, c0, c1);
      scale_plane_2_to_1_phase_non_0_neon(src->v_buffer, src->uv_stride,
