...
// ref, src = [0, 512) - max diff = 16-bits
// bwl = {1, 2, 3, 4}, width = {8, 16, 32, 64}
int vp9_vector_var_neon(int16_t const *ref, int16_t const *src, const int bwl) {
  int width = 4 << bwl;
  int32x4_t sse = vdupq_n_s32(0);  // max = 2^9 * 2^9 * 2^6 = 24-bits
  int16x8_t total = vdupq_n_s16(0);  // max = 2^9 (+sign) * 2^6 = 16-bits
  ...
  {
    // NB: 'total''s pairwise addition could be implemented similarly to
