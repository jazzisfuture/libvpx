...
static INLINE int sum_diff_16x1_neon(const int8x16_t v_sum_diff_total) {
  ...
  int64x1_t x = vqadd_s64(vget_high_s64(fedcba98_76543210),
...
static INLINE int8x16_t vp9_denoiser_16x1_neon(
   ...
   * for this macroblock.
   */
  const int8x16_t v_sum_diff = vqsubq_s8(vreinterpretq_s8_u8(v_pos_adjustment),
...
static INLINE int8x16_t vp9_denoiser_adj_16x1_neon(
    const uint8_t *sig, const uint8_t *mc_running_avg_y, uint8_t *running_avg_y,
  ...
  const int8x16_t v_sum_diff = vqsubq_s8(vreinterpretq_s8_u8(v_neg_adjustment),
