...
static INLINE int sum_diff_16x1_neon(const int64x2_t v_sum_diff_total) {
  int64x1_t x = vqadd_s64(vget_high_s64(v_sum_diff_total),
                          vget_low_s64(v_sum_diff_total));
  int sum_diff = vget_lane_s32(vreinterpret_s32_s64(x), 0);

  return sum_diff;
}

// Denoise a 16x1 vector.
static INLINE int64x2_t vp9_denoiser_16x1_neon(
    ...
    const uint8x16_t *v_delta_level_2_and_3, int64x2_t v_sum_diff_total) {
  /* Load inputs. */
...
static INLINE int64x2_t vp9_denoiser_adj_16x1_neon(
    const uint8_t *sig, const uint8_t *mc_running_avg_y, uint8_t *running_avg_y,
    const uint8x16_t k_delta, int64x2_t v_sum_diff_total) {
...
static int vp9_denoiser_NxM_neon_small(const uint8_t *sig, int sig_stride,
  ...
  for (r = 0; r < b_height; ++r) {
    ...
    v_sum_diff_total = vp9_denoiser_16x1_neon(
  ...
  {
    sum_diff = sum_diff_16x1_neon(v_sum_diff_total);
    ...
    if (abs(sum_diff) > sum_diff_thresh) {
      ...
      if (delta < 4) {
        ...
        for (r = 0; r < b_height; ++r) {
          v_sum_diff_total = vp9_denoiser_adj_16x1_neon(
...
static int vp9_denoiser_NxM_neon_big(const uint8_t *sig, int sig_stride,
                                     const uint8_t *mc_running_avg_y,
                                     int mc_avg_y_stride,
                                     uint8_t *running_avg_y, int avg_y_stride,
                                     int increase_denoising, BLOCK_SIZE bs,
  ...
  int64x2_t v_sum_diff_total[4][4];
  int r, c, sum_diff = 0;

  for (r = 0; r < 4; ++r) {
    for (c = 0; c < b_width_shift4; ++c) {
      v_sum_diff_total[c][r] = vdupq_n_s64(0);
    }
  }

  for (r = 0; r < b_height; ++r) {
    for (c = 0; c < b_width_shift4; ++c) {
      v_sum_diff_total[c][r >> 4] = vp9_denoiser_16x1_neon(
    ...
    if ((r & 0xf) == 0xf || (bs == BLOCK_16X8 && r == 7)) {
      for (c = 0; c < b_width_shift4; ++c) {
        sum_diff += sum_diff_16x1_neon(v_sum_diff_total[c][r >> 4]);
      }
    }
  ...
  {
    int sum_diff_thresh = total_adj_strong_thresh(bs, increase_denoising);
    if (abs(sum_diff) > sum_diff_thresh) {
      // Before returning to copy the block (i.e., apply no denoising),
      // check if we can still apply some (weaker) temporal filtering to
      // this block, that would otherwise not be denoised at all. Simplest
      ...
      if (delta < 4) {
        ...
        for (r = 0; r < b_height; ++r) {
          for (c = 0; c < b_width_shift4; ++c) {
            v_sum_diff_total[c][r >> 4] = vp9_denoiser_adj_16x1_neon(
                sig, mc_running_avg_y, running_avg_y, k_delta,
                v_sum_diff_total[c][r >> 4]);
...
int vp9_denoiser_filter_neon(const uint8_t *sig, int sig_stride,
  ...
  if (bs == BLOCK_16X16 || bs == BLOCK_32X32 || bs == BLOCK_64X64 ||
    ...
    return vp9_denoiser_NxM_neon_big(sig, sig_stride, mc_avg, mc_avg_stride,
                                     avg, avg_stride, increase_denoising, bs,
                                     motion_magnitude);
  } else if (bs == BLOCK_8X8 || bs == BLOCK_8X16) {
    return vp9_denoiser_NxM_neon_small(sig, sig_stride, mc_avg, mc_avg_stride,
                                       avg, avg_stride, increase_denoising, bs,
                                       motion_magnitude, 8);
  } else {
    return COPY_BLOCK;
