...
#include "vp9/encoder/arm/neon/vp9_temporal_filter_constants.h"
...
static INLINE void highbd_store_dist_8(const uint16_t *a, const uint16_t *b,
  ...
  const uint32x4_t a_first = vmovl_u16(vget_low_u16(a_reg));
  const uint32x4_t a_second = vmovl_u16(vget_high_u16(a_reg));
  const uint32x4_t b_first = vmovl_u16(vget_low_u16(b_reg));
  const uint32x4_t b_second = vmovl_u16(vget_high_u16(b_reg));

  uint32x4_t dist_first, dist_second;
  dist_first = vsubq_u32(a_first, b_first);
  dist_second = vsubq_u32(a_second, b_second);
  dist_first = vmulq_u32(dist_first, dist_first);
  dist_second = vmulq_u32(dist_second, dist_second);
...
static INLINE void highbd_average_4(uint32x4_t *output, const uint32x4_t sum,
                                    ...
                                    const int weight) {
  const int64x2_t strength_u64 = vdupq_n_s64(-strength - 32);
  ...
  // modifier * 3 / index;
  uint64x2_t sum_hi =
      vmull_u32(vget_low_u32(sum), vget_low_u32(*mul_constants));
  uint64x2_t sum_lo =
      vmull_u32(vget_high_u32(sum), vget_high_u32(*mul_constants));

  sum_lo = vaddq_u64(sum_lo, rounding_u64);
  sum_hi = vaddq_u64(sum_hi, rounding_u64);

  // we cannot use vshrn_n_u64 as strength is not known at compile time.
  sum_lo = vshlq_u64(sum_lo, strength_u64);
  sum_hi = vshlq_u64(sum_hi, strength_u64);

  sum2 = vcombine_u32(vmovn_u64(sum_hi), vmovn_u64(sum_lo));
...
static INLINE void highbd_accumulate_and_store_8(
  ...
  pred_0_u32 = vmulq_u32(pred_0_u32, sum_first_u32);
  pred_1_u32 = vmulq_u32(pred_1_u32, sum_second_u32);

  accum_0_u32 = vld1q_u32(accumulator);
  accum_1_u32 = vld1q_u32(accumulator + 4);

  accum_0_u32 = vaddq_u32(pred_0_u32, accum_0_u32);
  accum_1_u32 = vaddq_u32(pred_1_u32, accum_1_u32);
...
static INLINE void highbd_read_chroma_dist_row_8(
  ...
  if (!ss_x) {
    ...
    *u_first = vzip1q_u32(u_reg, u_reg);
    *u_second = vzip2q_u32(u_reg, u_reg);

    highbd_read_dist_4(v_dist, &v_reg);

    *v_first = vzip1q_u32(v_reg, v_reg);
    *v_second = vzip2q_u32(v_reg, v_reg);
  }
}
