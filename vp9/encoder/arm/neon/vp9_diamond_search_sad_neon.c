  ...
  *num00 = 0;
  ...
  for (i = 0, step = 0; step < tot_steps; step++) {
    for (j = 0; j < cfg->searches_per_step; j += 4, i += 4) {
      ...
      if (LIKELY(vmaxvq_u32(vreinterpretq_u32_s8(v_inside_d)) == 0)) {
        continue;
      }
      ...
      // Compute the SIMD pointer offsets.
      {
        ...
        int32x4_t v_bo_d = vld1q_s32((const int32x4_t *)&ss_os[i]);
        v_bo_d = vandq_s32(v_bo_d, v_inside_d);
      ...
      {
        const uint32_t local_best_sad =
            vminvq_u32(vreinterpretq_u32_s32(v_sad_d));
