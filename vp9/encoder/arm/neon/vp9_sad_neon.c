...
static INLINE const unsigned int horizontal_add_32x4(uint32x4_t vec_lo,
                                                     uint32x4_t vec_hi) {
  const uint32x4_t a = vaddq_u32(vec_lo, vec_hi);
...
static INLINE const unsigned int horizontal_add_16x8(uint16x8_t vec_lo,
                                                     uint16x8_t vec_hi) {
...
unsigned int vp9_sad64x64_neon(const uint8_t *src, int src_stride,
  ...
  for (i = 0; i < 64; ++i) {
    ...
    src += src_stride;
    ref += ref_stride;
    vec_accum_lo = vabdl_u8(vget_low_u8(vec_src_00), vget_low_u8(vec_ref_00));
    vec_accum_hi = vabdl_u8(vget_high_u8(vec_src_00), vget_high_u8(vec_ref_00));
  ...
  return horizontal_add_32x4(vec_w_accum_lo, vec_w_accum_hi);
}
