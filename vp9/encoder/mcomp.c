...
int vp9_find_best_sub_pixel_step(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
  ...
  *distortion = bestmse;
  ...
  right = thismse + mv_err_cost(&this_mv, ref_mv, mvjcost, mvcost, error_per_bit,
                                xd->allow_high_precision_mv);
...
(
  ...
  bestsad = vfp->sdf(what, what_stride, this_offset,
            ...
            + mvsad_err_cost(&this_mv, &fcenter_mv, mvjsadcost, mvsadcost, sad_per_bit);
...
int vp9_diamond_search_sad_c(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
                             ...
                             vp9_variance_fn_ptr_t *fn_ptr, int *mvjcost, int *mvcost[2],
                             int_mv *center_mv) {
  int i, j, step;
...
int vp9_full_pixel_diamond(VP9_COMP *cpi, MACROBLOCK *x, BLOCK *b,
  ...
  int bestsme = cpi->diamond_search_sad(x, b, d, mvp_full, &temp_mv,
                                        ...
                                        fn_ptr, x->nmvjointcost, (x->e_mbd.allow_high_precision_mv?x->nmvcost_hp:x->nmvcost), ref_mv);
    ...
    for (j = 0; j < 4; j++) {
      ...
      if ((this_col_offset > x->mv_col_min) && (this_col_offset < x->mv_col_max) &&
        ...
        if (thissad < bestsad) {
          this_mv.as_mv.row = this_row_offset;
          this_mv.as_mv.col = this_col_offset;
          thissad += mvsad_err_cost(&this_mv, &fcenter_mv, mvjsadcost, mvsadcost, error_per_bit);
    ...
    if (all_in) {
      ...
      for (j = 0; j < 4; j++) {
        if (sad_array[j] < bestsad) {
          ...
          sad_array[j] += mvsad_err_cost(&this_mv, &fcenter_mv, mvjsadcost, mvsadcost, error_per_bit);
