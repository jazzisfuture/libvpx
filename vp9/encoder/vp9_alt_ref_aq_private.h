 ...
 *  the rate-distortion  optimization this  specificity implies  that their
 *  introduced rate is similar to the one of common frames, but we can only
 ...
 *      Having the  current vp9  infrastructure this feature  is relatively
 ...
 *  corresponding  blocks  from the  other  lookahead buffers using  motion
 *  search.  Encoder  averages blocks  that are similar  after compensation
 *  using discrete set of weights, namely 0,1 or 2.
 *      The  described process  of  the altref  creation naturally  defines
 *  per-block prediction-power estimates. For the blocks that are an exact
 ...
 *
 *  * Disable all  intermediate segments, i.e. only keep segments with zero
