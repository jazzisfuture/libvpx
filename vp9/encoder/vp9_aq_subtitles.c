...
int subtitles_setup(VP9_COMP *const cpi) {
    int start_x=0, start_y=0, end_x=0, end_y=0, frame_start=0, frame_end=0;
    int i, j;
    ...
    // Open file and read each line of segments where subtitles exist and frame duration
    FILE *fp = fopen(FILE_NAME, "r");
    while (fscanf(fp, "%d,%d,%d,%d,%d,%d\n", &start_x, &start_y, &end_x, &end_y, &frame_start, &frame_end) == 6) {
        
        // see if current frame is within the set of frames that these subtitles exist in
