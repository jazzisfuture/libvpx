...
int ransac_(double *matched_points,
  ...
  static const double INLIER_THRESHOLD_UNNORMALIZED = 2.0;
  ...
  double points1[2*minpts];
  double points2[2*minpts];
  double corners1[npoints*2];
  double corners2[npoints*2];
  double image1_coord[npoints*2];
  double image2_coord[npoints*2];
  int inlier_mask[npoints];

  double *cnp1 = corners1, *cnp2 = corners2;
  double T1[9], T2[9];
  ...
  *number_of_inliers = max_inliers;
  // Get un-normalized inliers
  for (i = j = 0; j < npoints; ++j) {
    if (best_inlier_mask[j]) {
      best_inlier_set1[i*2] = matched_points[j*4];
      best_inlier_set1[i*2+1] = matched_points[j*4+1];
      best_inlier_set2[i*2] = matched_points[j*4+2];
      best_inlier_set2[i*2+1] = matched_points[j*4+3];
      i++;
    }
  }
  return 0;
...
int findRotZoom(const int np, double *pts1, double *pts2, double *mat) {
  const int np2 = np * 2;
  double a[np2 * 4], b[np2 * 1], temp[np2 * 4];
  int i, j;
  double sx, sy, dx, dy;
...
int findAffine(const int np, double *pts1, double *pts2, double *mat) {
  const int np2 = np * 2;
  double a[np2 * 6], b[np2 * 1], temp[np2 * 6];
  int i, j;
  double sx, sy, dx, dy;
...
int ransacRotZoom(double *matched_points, int npoints, double *best_inlier_set1,
                  double *best_inlier_set2, int *number_of_inliers,
...
int ransacAffine(double *matched_points, int npoints, double *best_inlier_set1,
                 double *best_inlier_set2, int *number_of_inliers,
                 int *best_inlier_mask, double *bestH) {
  return ransac_(matched_points,
                 npoints,
...
int ransacHomography(double *matched_points, int npoints, double *best_inlier_set1,
                     double *best_inlier_set2, int *number_of_inliers,
                     int *best_inlier_mask, double *bestH) {
  return ransac_(matched_points,
                 npoints,
