...
static void separate_arf_mbs(VP9_COMP *cpi) {
  ...
  for (offset = 0, mi_row = 0; mi_row < cm->mi_rows;
       offset += cm->mi_cols, mi_row++) {
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col++) {
      // If any of the blocks in the sequence failed then the MB
      // goes in segment 0
      if (arf_not_zz[offset/(2*cm->mi_cols) + mi_col/2]) {
        ncnt[0]++;
        cpi->segmentation_map[offset + mi_col] = 0;
      } else {
        cpi->segmentation_map[offset + mi_col] = 1;
        ncnt[1]++;
      }
    }
  }
