  ...
  for (offset = 0, mb_row = 0; mb_row < cm->mb_rows;
    ...
    for (mb_col = 0; mb_col < cm->mb_cols; mb_col++) {
      ...
      if (arf_not_zz[offset + mb_col]) {
        ...
        cpi->segmentation_map[offset * 4 + 2 * mb_col + cm->mb_cols] = 0;
        cpi->segmentation_map[offset * 4 + 2 * mb_col + cm->mb_cols + 1] = 0;
      } else {
        cpi->segmentation_map[offset * 4 + 2 * mb_col] = 1;
        cpi->segmentation_map[offset * 4 + 2 * mb_col + 1] = 1;
        cpi->segmentation_map[offset * 4 + 2 * mb_col + cm->mb_cols] = 1;
        cpi->segmentation_map[offset * 4 + 2 * mb_col + cm->mb_cols + 1] = 1;
#else
