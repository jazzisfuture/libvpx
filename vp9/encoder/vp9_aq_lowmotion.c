...
int vp9_low_motion_setup(VP9_COMP *const cpi) {
  ...
  vp9_disable_segfeature(seg, REG_MOTION_IDX, SEG_LVL_ALT_Q);
  ...
  if ((cm->base_qindex + QINDEX_DELTA) > 0) { 
    ...
    if (cm->frame_type == KEY_FRAME) {
      vp9_set_segdata(seg, LOW_MOTION_IDX, SEG_LVL_ALT_Q, 0);
      // printf("Key frame %d\n", cm->seg.feature_data[LOW_MOTION_IDX][0]);
    } else if (cpi->refresh_alt_ref_frame) {
      // seg->update_map=0;
      vp9_set_segdata(seg, LOW_MOTION_IDX, SEG_LVL_ALT_Q, -1);
      // printf("Altref %d\n", cm->seg.feature_data[LOW_MOTION_IDX][0]);
    } else {
      // seg->update_map = 0;
      vp9_set_segdata(seg, LOW_MOTION_IDX, SEG_LVL_ALT_Q, 0);
