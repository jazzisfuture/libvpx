...
// This function is designed to apply a bias or adjustment to an rd value.
// TODO (paulwilkins):
// For the moment this function is a stub / place holder and applies a
// bias based on the source variance and the mode but the intent is to test
// a bias based on the similarity of the source and reconstruction variance.
#define LOW_VAR_THRESH 16
#define LOW_VAR_DIFF_THRESH 1
void rd_variance_adjustment(VP9_COMP *cpi,
                            MACROBLOCK *x,
                            BLOCK_SIZE bsize,
                            int64_t *this_rd,
                            MV_REFERENCE_FRAME ref_frame,
  ...
  var_diff = abs(source_variance - recon_variance);

  if ((source_variance > LOW_VAR_THRESH) && (var_diff > LOW_VAR_DIFF_THRESH)) {
    unsigned int var_factor;
    if (source_variance > recon_variance) {
      var_factor = MIN(50, (var_diff * 50) / source_variance);
    } else {
      var_factor = MIN(10, (var_diff * 10) / source_variance);
    }
