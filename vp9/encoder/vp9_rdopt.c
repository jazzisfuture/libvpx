    ...
    if (col  &&  labelings[i] == labelings[i - 1])
      ...
      switch (m = this_mode) {
        case NEW4X4 :
          ...
          break;
        case LEFT4X4:
          this_mv->as_int = col ? d[-1].bmi.as_mv.first.as_int : left_block_mv(xd, mic, i);
          if (mbmi->second_ref_frame > 0)
            this_second_mv->as_int = col ? d[-1].bmi.as_mv.second.as_int : left_block_second_mv(xd, mic, i);
          break;
        case ABOVE4X4:
