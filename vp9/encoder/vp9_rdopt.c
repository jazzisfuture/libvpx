...
static int64_t rd_pick_intra4x4block(VP9_COMP *cpi, MACROBLOCK *x, int ib,
    ...
    for (idy = 0; idy < num_4x4_blocks_high; ++idy) {
      for (idx = 0; idx < num_4x4_blocks_wide; ++idx) {
        ...
        if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
          vp9_high_subtract_block(4, 4, src_diff, 8, src, src_stride, dst, dst_stride, xd->bps);
        } else {
...
#if CONFIG_VP9_HIGH
          if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
            vp9_high_iwht4x4_add(BLOCK_OFFSET(pd->dqcoeff, block), dst, dst_stride,
                          p->eobs[block], xd->bps);
          } else {
      ...
      *best_mode = mode;
      ...
      if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
        for (idy = 0; idy < num_4x4_blocks_high * 4; ++idy)
          vpx_memcpy(best_dst16 + idy * 8, CONVERT_TO_SHORTPTR(dst_init + idy * dst_stride),
  ...
  if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
    for (idy = 0; idy < num_4x4_blocks_high * 4; ++idy)
      vpx_memcpy(CONVERT_TO_SHORTPTR(dst_init + idy * dst_stride), best_dst16 + idy * 8,
               num_4x4_blocks_wide * 4 * sizeof(uint16_t) );
  } else {
    for (idy = 0; idy < num_4x4_blocks_high * 4; ++idy)
