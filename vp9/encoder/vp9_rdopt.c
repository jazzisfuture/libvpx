  ...
  for (idy = 0; idy < 2; idy += num_4x4_blocks_high) {
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      ...
      for (ref = 0; ref < 1 + has_second_rf; ++ref) {
        ...
        // When ref == 0, following code will incur a mismatch;
        // When ref == 1, following code does not incur any mismatch, so far,
        // but produces a little worse RD results.
      ...
      for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
        ...
        if (!has_second_rf && this_mode == NEWMV &&
          ...
          if (cpi->oxcf.mode != BEST) {
            ...
            if (i > 0) {
#if CONFIG_NEWMVREF_SUB8X8
              // Use the nearest mv as the mv predictor.
              bsi->mvp.as_int = frame_mv[NEARESTMV][mbmi->ref_frame[0]].as_int;
#else
