...
static int model_rd_for_sb_earlyterm(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  for (i = 0; i < MAX_MB_PLANE; ++i) {
    ...
    if (cpi->sf.simple_model_rd_from_var) {
      if (qstep < 120)
        rate = (sum_sse * (280 - qstep)) >> (16 - VP9_PROB_COST_SHIFT);
      else
        rate = 0;
...
static int64_t handle_inter_mode(
      ...
      for (i = 0; i < SWITCHABLE_FILTERS; ++i) {
        ...
        if (i > 0 && intpel_mv) {
          ...
          *mask_filter = VPXMAX(*mask_filter, rd);
          ...
          const int enable_earlyterm = (cpi->sf.interp_filt_prune_plane_rd) &&
                                       (cm->interp_filter == i ? 0 : 1);
          const int64_t filt_best_rd =
  ...
  if (pred_exists) {
    ...
    const int dummy = model_rd_for_sb_earlyterm(
