...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (this_mode == NEWMV) {
    ...
    if (is_comp_pred) {
      ...
      frame_mv[refs[1]].as_int = single_newmv[refs[1]].as_int;
      *rate2 += vp9_mv_bit_cost(&frame_mv[refs[1]].as_mv,
                                &frame_mv[refs[0]].as_mv,
                                x->nmvjointcost, x->mvcost, MV_COST_WEIGHT);
      } else if (this_mode == NEW_NEARESTMV || this_mode == NEW_NEARMV) {
      frame_mv[refs[0]].as_int = single_newmv[refs[0]].as_int;
      *rate2 += vp9_mv_bit_cost(&frame_mv[refs[0]].as_mv,
...
#if CONFIG_COMPOUND_MODES
  if (best_mbmode.mode == NEW_NEWMV || best_mbmode.mode == NEAREST_NEWMV ||
      best_mbmode.mode == NEW_NEARESTMV || best_mbmode.mode == NEW_NEARMV ||
