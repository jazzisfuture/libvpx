...
static INLINE int cost_coeffs(MACROBLOCK *mb,
  ...
  if (tx_type != DCT_DCT) {
    ...
    for (; c < eob; c++) {
      ...
      cost += token_costs[vp9_get_coef_band(tx_size, c)][pt][t];
      cost += vp9_dct_value_cost_ptr[v];
      pt = vp9_get_coef_context(&recent_energy, t);
    }
    if (c < seg_eob)
      cost += mb->hybrid_token_costs[tx_size][type]
                                    [vp9_get_coef_band(tx_size, c)]
          [pt][DCT_EOB_TOKEN];
  } else {
    int recent_energy = 0;
    for (; c < eob; c++) {
    ...
    if (c < seg_eob)
      cost += mb->token_costs[tx_size][type][vp9_get_coef_band(tx_size, c)]
          [pt][DCT_EOB_TOKEN];
  }
