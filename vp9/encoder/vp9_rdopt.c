...
#endif
#if CONFIG_NEWMVREF_SUB8X8
  int_mv nearest_sub8x8[MAX_REF_FRAMES];
#endif  // CONFIG_NEWMVREF_SUB8X8
...
#if CONFIG_NEWMVREF_SUB8X8
  nearest_sub8x8[0].as_int = best_ref_mv->as_int;
  nearest_sub8x8[1].as_int =
      second_best_ref_mv ? second_best_ref_mv->as_int : 0;
  bsi->ref_mv[0] = &nearest_sub8x8[0];
  bsi->ref_mv[1] = &nearest_sub8x8[1];
#else
    ...
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      ...
      for (ref = 0; ref < 1 + has_second_rf; ++ref) {
        ...
        vp9_find_best_ref_mvs(xd, allow_hp, mv_ref_list,
                              &nearest_sub8x8[ref], &near_sub8x8);
#endif  // CONFIG_NEWMVREF_SUB8X8
#if CONFIG_COMPOUND_MODES
