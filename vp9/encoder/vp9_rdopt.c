...
#if CONFIG_PALETTE
      || x->e_mbd.mi[0].src_mi->mbmi.palette_enabled[0]
  ...
  if (colors > 1 && colors <= 64 && cpi->common.allow_palette_mode) {
    ...
    for (n = colors > PALETTE_MAX_SIZE ? PALETTE_MAX_SIZE : colors;
      ...
      if (m1 > 0)
        for (i = 0; i < m1; i++) {
          mic->mbmi.palette_colors[i] =
              cpi->common.current_palette_colors[mic->mbmi.palette_indexed_colors[i]];
        }
      if (m2 > 0)
