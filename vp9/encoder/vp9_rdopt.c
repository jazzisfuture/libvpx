...
static int vp9_sb_uv_block_error_c(int16_t *coeff0, int16_t *coeff1,
  ...
  for (i = 0; i < block_size / 2; i++) {
    unsigned int this_diff = coeff0[i] - dqcoeff0[i];
    error += this_diff * this_diff;
  }
  for (i = 0; i < block_size / 2; i++) {
    unsigned int this_diff = coeff1[i] - dqcoeff1[i];
    error += this_diff * this_diff;
  }
  error >>= shift;
  return error > INT_MAX ? INT_MAX : (int)error;
...
static int zvp9_sb_uv_block_error_c(int16_t *coeff,
                                   int16_t *dqcoeff0, int16_t *dqcoeff1,
                                   int block_size, int shift) {
  int i;
  int64_t error = 0;
