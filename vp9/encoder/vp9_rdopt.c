...
static int64_t vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if(cpi->Speed > 0 ) {
    switch(block_size) {
      ...
      case BLOCK_32X32:
        for (i = 0; i < 4; i++ ) {
          ref_frame_mask |= (1 << x->mb_context[xd->sb_index][i].mic.mbmi.ref_frame);
  ...
  if ( cpi->Speed > 0 && ( ref_frame_mask & (1 << INTRA_FRAME ))){
  ...
  for (mode_index = 0; mode_index < MAX_MODES; ++mode_index) {
    ...
    if(cpi->Speed > 0 ) {
      ...
      if (vp9_mode_order[mode_index].second_ref_frame != NONE &&
          !(ref_frame_mask & (1 << vp9_mode_order[mode_index].second_ref_frame))){
        continue;
      }
    }
