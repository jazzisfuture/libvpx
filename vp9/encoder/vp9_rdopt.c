  ...
  if (cpi->sf.txfm_domain_distortion) {
    ...
    *out_sse = this_sse >> shift;
    ...
    if (x->skip_encode && !is_inter_block(xd->mi[0])) {
      ...
      int64_t p = (pd->dequant[1] * pd->dequant[1] * (1 << ss_txfrm_size)) >>
#if CONFIG_VP9_HIGHBITDEPTH
                  (shift + 2 + (bd - 8) * 2);
#else
                  (shift + 2);
...
static void block_rd_txfm(int plane, int block, int blk_row, int blk_col,
  ...
  int64_t dist;
  int64_t sse;
  int coeff_ctx = combine_entropy_contexts(args->t_left[blk_row],
      ...
      for (idx = 0; idx < num_4x4_blocks_wide; ++idx) {
        ...
        if (xd->lossless) {
          ...
          ratey += cost_coeffs(x, 0, block, TX_4X4, coeff_ctx,
                               so->scan, so->neighbors,
                               cpi->sf.use_fast_coef_costing);
          tempa[idx] = templ[idy] = x->plane[0].eobs[block] > 0;
          if (RDCOST(x->rdmult, x->rddiv, ratey, distortion) >= best_rd)
            goto next;
  ...
  for (idy = 0; idy < height / 4; ++idy) {
    for (idx = 0; idx < width / 4; ++idx) {
      ...
      thisrate += cost_coeffs(x, 0, k, TX_4X4, coeff_ctx,
                              so->scan, so->neighbors,
                              cpi->sf.use_fast_coef_costing);
      ta[k & 1] = tl[k >> 1] = (x->plane[0].eobs[k] > 0);
