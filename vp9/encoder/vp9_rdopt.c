...
static void model_rd_for_sb(VP9_COMP *cpi, BLOCK_SIZE bsize,
  ...
  int64_t rate_sum = 0;
  int64_t dist_sum = 0;
  const int ref = xd->mi_test[0].target->mbmi.ref_frame[0];
  unsigned int sse;
  unsigned int var = 0;
...
static INLINE int cost_coeffs(MACROBLOCK *x,
  ...
  MB_MODE_INFO *mbmi = &xd->mi_test[0].target->mbmi;
...
static void dist_block(int plane, int block, TX_SIZE tx_size,
  ...
  if (x->skip_encode && !is_inter_block(&xd->mi_test[0].target->mbmi)) {
...
static void txfm_rd_in_plane(MACROBLOCK *x,
  ...
  if (plane == 0)
    xd->mi_test[0].target->mbmi.tx_size = tx_size;
...
static void super_block_yrd(VP9_COMP *cpi, MACROBLOCK *x, int *rate,
  ...
  assert(bs == xd->mi_test[0].target->mbmi.sb_type);
...
static int64_t rd_pick_intra4x4block(VP9_COMP *cpi, MACROBLOCK *x, int ib,
  ...
  xd->mi_test[0].target->mbmi.tx_size = TX_4X4;
  ...
  for (mode = DC_PRED; mode <= TM_PRED; ++mode) {
    ...
    for (idy = 0; idy < num_4x4_blocks_high; ++idy) {
      for (idx = 0; idx < num_4x4_blocks_wide; ++idx) {
        ...
        xd->mi_test[0].target->bmi[block].as_mode = mode;
...
static int64_t rd_pick_intra_sub_8x8_y_mode(VP9_COMP *cpi, MACROBLOCK *mb,
  ...
  MODE_INFO *const mic = xd->mi_test[0].target;
  const MODE_INFO *above_mi = xd->mi_test[-xd->mi_stride].target;
  const MODE_INFO *left_mi = xd->left_available ? xd->mi_test[-1].target : NULL;
  const BLOCK_SIZE bsize = xd->mi_test[0].target->mbmi.sb_type;
  const int num_4x4_blocks_wide = num_4x4_blocks_wide_lookup[bsize];
  const int num_4x4_blocks_high = num_4x4_blocks_high_lookup[bsize];
  int idx, idy;
  int cost = 0;
...
static int64_t rd_pick_intra_sby_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  for (mode = DC_PRED; mode <= TM_PRED; mode++) {
    ...
    MODE_INFO *above_mi = xd->mi_test[-xd->mi_stride].target;
    MODE_INFO *left_mi = xd->left_available ? xd->mi_test[-1].target : NULL;
...
static int64_t rd_pick_intra_sbuv_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  for (mode = DC_PRED; mode <= TM_PRED; ++mode) {
    ...
    xd->mi_test[0].target->mbmi.uv_mode = mode;
  ...
  xd->mi_test[0].target->mbmi.uv_mode = mode_selected;
  return best_rd;
}
...
static int64_t rd_sbuv_dcpred(const VP9_COMP *cpi, MACROBLOCK *x,
  ...
  x->e_mbd.mi_test[0].target->mbmi.uv_mode = DC_PRED;
...
static void choose_intra_uv_mode(VP9_COMP *cpi, PICK_MODE_CONTEXT *ctx,
  ...
  *mode_uv = x->e_mbd.mi_test[0].target->mbmi.uv_mode;
}
...
static int64_t encode_inter_mb_segment(VP9_COMP *cpi,
  ...
  MODE_INFO *const mi = xd->mi_test[0].target;
...
static INLINE void mi_buf_shift(MACROBLOCK *x, int i) {
  MB_MODE_INFO *const mbmi = &x->e_mbd.mi_test[0].target->mbmi;
  struct macroblock_plane *const p = &x->plane[0];
...
static INLINE void mi_buf_restore(MACROBLOCK *x, struct buf_2d orig_src,
  ...
  MB_MODE_INFO *mbmi = &x->e_mbd.mi_test[0].target->mbmi;
...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (pred_filter_search) {
    ...
    if (xd->up_available)
      af = xd->mi_test[-xd->mi_stride].target->mbmi.interp_filter;
    if (xd->left_available)
      lf = xd->mi_test[-1].target->mbmi.interp_filter;
    ...
    if (is_comp_pred) {
      ...
      *rate2 += rate_mv;
      frame_mv[refs[0]].as_int =
          xd->mi_test[0].target->bmi[0].as_mv[0].as_int = tmp_mv.as_int;
...
void vp9_rd_pick_intra_mode_sb(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  x->skip_encode = 0;
  ctx->skip = 0;
  xd->mi_test[0].target->mbmi.ref_frame[0] = INTRA_FRAME;
  ...
  if (bsize >= BLOCK_8X8) {
    ...
    max_uv_tx_size = get_uv_tx_size_impl(xd->mi_test[0].target->mbmi.tx_size, bsize,
                                         pd[1].subsampling_x,
                                         pd[1].subsampling_y);
  ...
  for (mode_index = 0; mode_index < MAX_MODES; ++mode_index) {
    ...
    if (cpi->sf.motion_field_mode_search) {
      ...
      if ((mi_row - 1) >= tile->mi_row_start) {
        ref_mv = xd->mi_test[-xd->mi_stride].target->mbmi.mv[0];
        rf = xd->mi_test[-xd->mi_stride].target->mbmi.ref_frame[0];
        for (i = 0; i < mi_width; ++i) {
          ref_mbmi = &xd->mi_test[-xd->mi_stride + i].target->mbmi;
      ...
      if ((mi_col - 1) >= tile->mi_col_start) {
        if (ref_mv.as_int == INVALID_MV)
          ref_mv = xd->mi_test[-1].target->mbmi.mv[0];
        if (rf == NONE)
          rf = xd->mi_test[-1].target->mbmi.ref_frame[0];
        for (i = 0; i < mi_height; ++i) {
          ref_mbmi = &xd->mi_test[i * xd->mi_stride - 1].target->mbmi;
          const_motion &= (ref_mv.as_int == ref_mbmi->mv[0].as_int) &&
                          (ref_frame == ref_mbmi->ref_frame[0]);
          skip_ref_frame &= (rf == ref_mbmi->ref_frame[0]);
        }
...
int64_t vp9_rd_pick_inter_mode_sub8x8(VP9_COMP *cpi, MACROBLOCK *x,
    ...
    if (ref_frame == INTRA_FRAME) {
        ...
        if (x->source_variance < cpi->sf.disable_filter_search_var_thresh) {
          ...
          for (switchable_filter_index = 0;
            ...
            if ((newbest && cm->interp_filter == SWITCHABLE) ||
              ...
              for (i = 0; i < 4; i++) {
                tmp_best_bmodes[i] = xd->mi_test[0].target->bmi[i];
      ...
      if (!pred_exists) {
        ...
        *mbmi = tmp_best_mbmode;
        for (i = 0; i < 4; i++)
          xd->mi_test[0].target->bmi[i] = tmp_best_bmodes[i];
      }
    ...
    if (this_rd < best_rd || x->skip) {
      if (!mode_excluded) {
        ...
        *returndistortion = distortion2;
        ...
        for (i = 0; i < 4; i++)
          best_bmodes[i] = xd->mi_test[0].target->bmi[i];
  ...
  *mbmi = best_mbmode;
  ...
  if (!is_inter_block(&best_mbmode)) {
    for (i = 0; i < 4; i++)
      xd->mi_test[0].target->bmi[i].as_mode = best_bmodes[i].as_mode;
  } else {
    for (i = 0; i < 4; ++i)
      vpx_memcpy(&xd->mi_test[0].target->bmi[i], &best_bmodes[i], sizeof(b_mode_info));

    mbmi->mv[0].as_int = xd->mi_test[0].target->bmi[3].as_mv[0].as_int;
    mbmi->mv[1].as_int = xd->mi_test[0].target->bmi[3].as_mv[1].as_int;
  }
