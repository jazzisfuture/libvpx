...
static int64_t rd_pick_best_sub8x8_mode(
  ...
  for (i = 0; i < 2; i ++) {
    ref_mvs_sub8x8[i][0].as_int =
        mbmi->ref_mvs[refs[0]][i].as_int;
    ref_mvs_sub8x8[i][1].as_int = has_second_rf ?
        mbmi->ref_mvs[refs[1]][i].as_int : 0;
  }
  ...
  ref_mv_sub8x8[0].as_int =
      mbmi->ref_mvs[refs[0]][0].as_int;
  ref_mv_sub8x8[1].as_int = has_second_rf ?
      mbmi->ref_mvs[refs[1]][0].as_int : 0;
    ...
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
...
#if CONFIG_NEWMVREF
           this_mode <= (has_second_rf ? NEARFORNEW_NEARFORNEWMV : NEAR_FORNEWMV);
      ...
      for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
        ...
        for (ref = 0; ref < 2; ++ref) {
          ...
          bsi->ref_mv[ref] = &ref_mv_sub8x8[ref];
        }
#endif  // CONFIG_NEWMVREF
...
#if CONFIG_NEWMVREF
          seg_mv_sub8x8[refs[0]].as_int = seg_mvs[i][mv_idx[0]][refs[0]].as_int;
          seg_mv_sub8x8[refs[1]].as_int = seg_mvs[i][mv_idx[1]][refs[1]].as_int;
#endif  // CONFIG_NEWMVREF
        }
