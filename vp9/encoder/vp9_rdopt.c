...
//          printf("\n THIS HAPPENED \n");
        ...
        if(this_mode == NEW_NEWMV) {
          bsi->rdstat[i][mode_idx].brate =
            set_and_cost_bmi_mvs(cpi, xd, i, this_mode, mode_mv[this_mode],
                                 frame_mv[this_mode], newnew_seg_mvs[i], bsi->ref_mv,
                                 x->nmvjointcost, x->mvcost);
        } else {
#endif
        bsi->rdstat[i][mode_idx].brate =
      ...
      if (mode_selected == NEW_NEWMV) {
        seg_mvs[i][mbmi->ref_frame[0]].as_int = newnew_seg_mvs[i][mbmi->ref_frame[0]].as_int;
        seg_mvs[i][mbmi->ref_frame[1]].as_int = newnew_seg_mvs[i][mbmi->ref_frame[1]].as_int;
      }
#endif
      set_and_cost_bmi_mvs(cpi, xd, i, mode_selected, mode_mv[mode_selected],
                           frame_mv[mode_selected], seg_mvs[i], bsi->ref_mv,
                           x->nmvjointcost, x->mvcost);
...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (is_comp_pred) {
#if CONFIG_COMPOUND_MODES
    if (single_newmv[refs[0]].as_int == INVALID_MV ||
        single_newmv[refs[1]].as_int == INVALID_MV)
      return INT64_MAX;
    if (cpi->sf.adaptive_mode_search) {
      switch (this_mode) {
        ...
        case NEAREST_NEWMV:
          if (single_filter[NEARESTMV][refs[0]] ==
              single_filter[NEWMV][refs[1]])
            best_filter = single_filter[NEWMV][refs[1]];
          break;
        case NEAR_NEWMV:
		 if (single_filter[NEARMV][refs[0]] ==
			 single_filter[NEWMV][refs[1]])
    ...
    if (is_comp_pred) {
      ...
      if (this_mode == NEW_NEWMV) {
        ...
        *rate2 += rate_mv;
    	...
    	frame_mv[refs[1]].as_int = xd->mi[0].src_mi->bmi[0].as_mv[1].as_int = single_newmv[refs[1]].as_int;
    	*rate2 += vp9_mv_bit_cost(&frame_mv[refs[1]].as_mv, &frame_mv[refs[0]].as_mv,
    	                             x->nmvjointcost, x->mvcost, MV_COST_WEIGHT);
      } else if (this_mode == NEW_NEARESTMV || this_mode == NEW_NEARMV) {
    	frame_mv[refs[0]].as_int = xd->mi[0].src_mi->bmi[0].as_mv[0].as_int = single_newmv[refs[0]].as_int;
    	*rate2 += vp9_mv_bit_cost(&frame_mv[refs[0]].as_mv, &frame_mv[refs[1]].as_mv,
    	                             x->nmvjointcost, x->mvcost, MV_COST_WEIGHT);
      }
#else
      // Initialize mv using single prediction mode result.
  ...
  *rate2 += cost_mv_ref(cpi, this_mode, mbmi->mode_context[refs[0]]);
  ...
  if (RDCOST(x->rdmult, x->rddiv, *rate2, 0) > ref_best_rd &&
	  ...
	  mbmi->mode != NEAR_NEWMV && mbmi->mode != NEW_NEARMV)
#else
      mbmi->mode != NEARESTMV)
#endif
    return INT64_MAX;
  ...
  *returnrate_nocoef = INT_MAX;
  ...
  if (!vp9_segfeature_active(seg, segment_id, SEG_LVL_REF_FRAME)) {
    ...
    if (cpi->rc.is_src_frame_alt_ref && (cpi->oxcf.arnr_max_frames == 0)) {
      ...
      if (frame_mv[NEAREST_NEWMV][ALTREF_FRAME].as_int != 0)
        mode_skip_mask[ALTREF_FRAME] |= (1 << NEAREST_NEWMV);
      if (frame_mv[NEW_NEARESTMV][ALTREF_FRAME].as_int != 0)
        mode_skip_mask[ALTREF_FRAME] |= (1 << NEW_NEARESTMV);
      if (frame_mv[NEAR_NEWMV][ALTREF_FRAME].as_int != 0)
              mode_skip_mask[ALTREF_FRAME] |= (1 << NEAR_NEWMV);
            if (frame_mv[NEW_NEARMV][ALTREF_FRAME].as_int != 0)
              mode_skip_mask[ALTREF_FRAME] |= (1 << NEW_NEARMV);
#endif
    }
  }
...
#if CONFIG_COMPOUND_MODES
  if (best_mbmode.mode == NEW_NEWMV || best_mbmode.mode == NEAREST_NEWMV || best_mbmode.mode == NEW_NEARESTMV
	  || best_mbmode.mode == NEW_NEARMV || best_mbmode.mode == NEAR_NEWMV) {
    const MV_REFERENCE_FRAME refs[2] = {best_mbmode.ref_frame[0],
        best_mbmode.ref_frame[1]};
    int comp_pred_mode = refs[1] > INTRA_FRAME;
