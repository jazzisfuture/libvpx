  ...
  *distortion = 0;
  for (i = 0; i < 16; i++) {
    if (labels[i] == which_label) {
      ...
      vp9_build_scaled_inter_predictor(*(bd->base_pre) + bd->pre,
                                       bd->pre_stride,
                                       bd->predictor, 16,
                                       &bd->bmi.as_mv[0],
                                       &xd->scale_factor[0],
      ...
      if (xd->mode_info_context->mbmi.second_ref_frame > 0) {
        vp9_build_scaled_inter_predictor(*(bd->base_second_pre) + bd->pre,
                                         bd->pre_stride,
                                         bd->predictor, 16,
                                         &bd->bmi.as_mv[1],
                                         &xd->scale_factor[1],
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (labels[ib] == which_label) {
      ...
      for (which_mv = 0; which_mv < 1 + use_second_ref; ++which_mv) {
        ...
        vp9_build_scaled_inter_predictor(*base_pre + bd->pre,
                                         bd->pre_stride,
                                         bd->predictor, 16,
    ...
    case NEWMV:
      ...
      if (is_comp_pred) {
        ...
        *rate2 += vp9_mv_bit_cost(&frame_mv[NEWMV][refs[1]],
        ...
        YV12_BUFFER_CONFIG backup_yv12;
        int bestsme = INT_MAX;
  ...
  if (!x->skip) {
    if (block_size == BLOCK_64X64) {
      ...
      *skippable = skippable_y && skippable_uv;
      ...
      vp9_build_inter16x16_predictors_mbuv(xd,
          &xd->predictor[256], &xd->predictor[320], 8, mb_row, mb_col);
#if CONFIG_COMP_INTERINTRA_PRED
      if (is_comp_interintra_pred) {
