    ...
    case NEWMV:
      ...
      if (is_comp_pred) {
        ...
        if (bsize == 0) {
          ...
          YV12_BUFFER_CONFIG scaled_first_yv12;
          ...
          //for (ite = 0; ite < 4; ite++) {
          for (ite = 0; ite < 2; ite++) {
            YV12_BUFFER_CONFIG ref_yv12[2] = {xd->pre, xd->second_pre};
            int bestsme = INT_MAX;
            ...
            vp9_build_inter_predictor(ref_yv12[!id].y_buffer,
                                      ref_yv12[!id].y_stride,
                                      second_pred, 16,
            ...
            d->bmi.as_mv[id].as_int = tmp_mv.as_int;
            frame_mv[NEWMV][refs[id]].as_int = d->bmi.as_mv[id].as_int;
            if (id)
              xd->pre = scaled_first_yv12;
          }
...
int64_t vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  for (mode_index = 0; mode_index < MAX_MODES; ++mode_index) {
    ...
    if (comp_pred) {
      ...
      vpx_memcpy(mdcounts, frame_mdcounts[mbmi->second_ref_frame], sizeof(mdcounts));
      mode_excluded =
          mode_excluded ?
