...
static void rd_variance_adjustment(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    if (source_variance > 100) {
      rec_variance =
          vp9_high_get_sby_perpixel_variance(cpi, recon, bsize, xd->bd);
    ...
    if (source_variance > 100) {
      rec_variance = vp9_get_sby_perpixel_variance(cpi, recon, bsize);
      src_variance = source_variance;
    } else {
...
void vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi, TileDataEnc *tile_data,
  ...
  for (midx = 0; midx < MAX_MODES; ++midx) {
    ...
    if (recon) {
      ...
      rd_variance_adjustment(cpi, x, bsize, &this_rd, &xd->plane[0].dst,
