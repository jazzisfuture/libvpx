...
#if CONFIG_NEW_WEDGE && CONFIG_WEDGE_PARTITION
...
#endif  // CONFIG_NEW_WEDGE && CONFIG_WEDGE_PARTITION
    ...
    *comp_mode_p = 128;
    ...
    if (cm->reference_mode != SINGLE_REFERENCE) {
      ...
      ref_costs_comp[LAST_FRAME]   = base_cost + vp9_cost_bit(ref_comp_p, 0)
                                     + vp9_cost_bit(ref_same_comp_p, 1);
...
#if CONFIG_NEW_WEDGE && CONFIG_NEW_INTER && CONFIG_WEDGE_PARTITION
    if (refs[0] == refs[1])
      rate_mv_tmp = INT32_MAX;
    else
#endif  // CONFIG_NEW_WEDGE && CONFIG_NEW_INTER && CONFIG_WEDGE_PARTITION
    rate_mv_tmp = rate_mv;
