    ...
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      ...
      for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
        ...
        if (!has_second_rf && this_mode == NEWMV
          ...
          single_motion_search_sub8x8(cpi, x, i, bsize,
...
#if CONFIG_NEWMVREF_SUB8X8 && CONFIG_COMPOUND_MODES
        if (has_second_rf) {
          // Redo the single reference motion search as the mv ref may have
          // changed.
          // TODO (zoeliu): This may need further optimization that if the mv
          // ref has not changed, no need to do the 2nd single motion search.
          if (this_mode == NEW_NEARESTMV ||
            ...
            single_motion_search_sub8x8(cpi, x, i, bsize,
                                        ...
                                        &ref_mv_sub8x8[0],
                                        &bsi->mvp);
            if (frame_mv[this_mode][mbmi->ref_frame[0]].as_int != INVALID_MV)
              seg_mvs[i][mbmi->ref_frame[0]].as_int =
                  frame_mv[this_mode][mbmi->ref_frame[0]].as_int;
            // restore src pointers
            mi_buf_restore(x, orig_src, orig_pre);
            ...
            // adjust src pointer for this block
            mi_buf_shift(x, i);
            frame_mv[this_mode][mbmi->ref_frame[1]].as_int = INVALID_MV;
            single_motion_search_sub8x8(cpi, x, i, bsize,
                                        &frame_mv[this_mode][mbmi->ref_frame[1]].as_mv,
                                        &ref_mv_sub8x8[1],
                                        &bsi->mvp);
            if (frame_mv[this_mode][mbmi->ref_frame[1]].as_int != INVALID_MV)
              seg_mvs[i][mbmi->ref_frame[1]].as_int =
                  frame_mv[this_mode][mbmi->ref_frame[1]].as_int;
            // restore src pointers
            mi_buf_restore(x, orig_src, orig_pre);
...
#endif  // CONFIG_NEWMVREF_SUB8X8 && CONFIG_COMPOUND_MODES
#if CONFIG_COMPOUND_MODES
