  ...
  if (eob == 0) {
    ...
    for (c = 1; c < eob; c++) {
      ...
      if (use_fast_coef_costing) {
        cost += (*token_costs)[!prev_t][!prev_t][t] + vp9_get_cost(prev_t, vp9_dct_value_tokens_ptr[v].extra);//vp9_dct_value_cost_ptr[v];
      } else {
        pt = get_coef_context(nb, token_cache, c);
        cost += (*token_costs)[!prev_t][pt][t] + vp9_get_cost(prev_t, vp9_dct_value_tokens_ptr[v].extra);//vp9_dct_value_cost_ptr[v];
