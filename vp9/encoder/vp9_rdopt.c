...
int64_t vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  *returnrate = INT_MAX;
  ...
  for (ref_frame = LAST_FRAME;
    ...
    // Non NEWMV modes that use this frame as their primary ref
    static const int ref_frame_mask_nonnewmv[] = {
        0x121281, 0x24c404, 0x080102
    };
    // All modes that use this frame as any ref
    ...
    if (!(cpi->ref_frame_flags & flag_list[ref_frame])) {
      ...
      for (i = LAST_FRAME; i <= ALTREF_FRAME; ++i) {
        if ((x->pred_mv_sad[ref_frame] >> 2) > x->pred_mv_sad[i]) {
          mode_skip_mask |= ref_frame_mask_nonnewmv[ref_frame - LAST_FRAME];
          break;
        }
      }
    }
