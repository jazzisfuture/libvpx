...
static void copy_buf(int is_highbd, int w, int h, const uint8_t *src,
                     const int src_stride, uint8_t *dst, const int dst_stride) {
  int i;
#if CONFIG_VP9_HIGHBITDEPTH
  if (is_highbd) {
    ...
    for (i = 0; i < h; ++i) {
      memcpy(&dst16[i * dst_stride], &src16[i * src_stride], w * 2);
    }
  } else {
#endif
    for (i = 0; i < h; ++i) {
      memcpy(&dst[i * dst_stride], &src[i * src_stride], w);
    }
#if CONFIG_VP9_HIGHBITDEPTH
  }
#else
...
static void copy_block_visible(const MACROBLOCKD *xd,
  ...
  if (tx_bsize == BLOCK_4X4 ||
    ...
    copy_buf(is_highbd, w, h, src, src_stride, dst, dst_stride);
  } else {
    int r, c;
    ...
    for (r = 0; r < max_r; ++r) {
      ...
      for (c = 0; c < max_c; ++c) {
        ...
        copy_buf(is_highbd, 4, 4, src_ptr, src_stride, dst_ptr, dst_stride);
      }
    }
  }
}
...
static void dist_block(const VP9_COMP *cpi, MACROBLOCK *x, int plane,
  ...
  if (!out_recon && x->block_tx_domain && eob) {
    ...
    *out_sse = (int64_t)tmp * 16;
    if (out_recon) {
      copy_block_visible(xd, pd, dst, dst_stride, out_recon->buf,
    ...
    if (eob) {
      ...
      if (out_recon) {
        copy_block_visible(xd, pd, recon, 32, out_recon->buf, out_recon->stride,
...
static void txfm_rd_in_plane(const VP9_COMP *cpi, MACROBLOCK *x, int *rate,
  ...
  if (args.exit_early) {
    ...
    *skippable = 0;
    if (recon) {
#if CONFIG_VP9_HIGHBITDEPTH
      memset(recon, 0, 64 * 64 * 2);
#else
      memset(recon, 0, 64 * 64);
#endif
    }
  } else {
...
static void choose_tx_size_from_rd(VP9_COMP *cpi, MACROBLOCK *x, int *rate,
...
#if CONFIG_VP9_HIGHBITDEPTH
  DECLARE_ALIGNED(16, uint16_t, recon_buf16[64 * 64 * TX_SIZES]);
#else
  DECLARE_ALIGNED(16, uint8_t, recon_buf8[64 * 64 * TX_SIZES]);
#endif  // CONFIG_VP9_HIGHBITDEPTH
...
#if CONFIG_VP9_HIGHBITDEPTH
    this_recon.buf = (uint8_t *)&recon_buf16[64 * 64 * n];
#else
    this_recon.buf = &recon_buf8[64 * 64 * n];
#endif  // CONFIG_VP9_HIGHBITDEPTH
    this_recon.stride = recon ? recon->stride : 0;
    txfm_rd_in_plane(cpi, x, &r[n][0], &d[n], &s[n], &sse[n], best_rd, 0, bs, n,
                     cpi->sf.use_fast_coef_costing, recon ? &this_recon : 0);
    r[n][1] = r[n][0];
    if (r[n][0] < INT_MAX) {
      r[n][1] += r_tx_size;
    }
  ...
  *psse = sse[mi->tx_size];
  if (recon) {
#if CONFIG_VP9_HIGHBITDEPTH
    memcpy((uint16_t *)recon->buf, &recon_buf16[64 * 64 * mi->tx_size],
           64 * 64 * 2);
#else
    memcpy(recon->buf, &recon_buf8[64 * 64 * mi->tx_size], 64 * 64);
#endif  // CONFIG_VP9_HIGHBITDEPTH
  }
}
...
static void rd_variance_adjustment(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  struct buf_2d *this_recon = &xd->plane[0].dst;
  if (recon) this_recon = recon;
  ...
  if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    if (source_variance > 100) {
      rec_variance =
          vp9_high_get_sby_perpixel_variance(cpi, this_recon, bsize, xd->bd);
      src_variance = source_variance;
    } else {
      rec_variance = vp9_high_get_sby_variance(cpi, this_recon, bsize, xd->bd);
      src_variance =
          vp9_high_get_sby_variance(cpi, &x->plane[0].src, bsize, xd->bd);
    }
  } else {
    if (source_variance > 100) {
      rec_variance = vp9_get_sby_perpixel_variance(cpi, this_recon, bsize);
      src_variance = source_variance;
    } else {
      rec_variance = vp9_get_sby_variance(cpi, this_recon, bsize);
...
void vp9_rd_pick_inter_mode_sb(VP9_COMP *cpi, TileDataEnc *tile_data,
  ...
  memset(recon16, 0, 64 * 64 * 2);
  recon_buf.buf = (uint8_t *)recon16;
#else
  DECLARE_ALIGNED(16, uint8_t, recon8[64 * 64]);
  memset(recon8, 0, 64 * 64);
