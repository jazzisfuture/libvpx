...
static INLINE int cost_coeffs(MACROBLOCK *x,
  ...
  unsigned int (*token_skipval_costs)[COEFF_CONTEXTS] =
                   x->token_skipval_costs[tx_size][type][is_inter_block(mbmi)];
  unsigned int (*zpc_costs)[ZPC_PTOKS][ZPC_NODES + 1] =
  ...
  if (tx_skip) {
    token_costs = &x->token_costs[tx_size][type][is_inter_block(mbmi)]
                                 [TX_SKIP_COEFF_BAND];
#if CONFIG_CODE_ZEROGROUP
    token_skipval_costs = &x->token_skipval_costs[tx_size][type]
                                                 [is_inter_block(mbmi)]
                                                 [TX_SKIP_COEFF_BAND];
#endif  // CONFIG_CODE_ZEROGROUP
  }
  ...
  if (eob == 0) {
    ...
    for (c = 1; c < eob; c++) {
      ...
      if (skip_coef_val) {
        assert(t == ZERO_TOKEN);
        if (prev_t) {
          cost += (*token_skipval_costs)[pt];
        }
      } else {
        cost += (*token_costs)[!prev_t][pt][t] + vp9_dct_value_cost_ptr[v];
      }
      if (!is_eoo[o] && t == ZERO_TOKEN) {
        int use_eoo, eoo = 0;
        use_eoo = vp9_use_eoo(c, 16 << (2 * tx_size), scan,
                              tx_size, is_last_zero, is_eoo);
        if (use_eoo) {
          eoo = vp9_is_eoo(c, eob, scan, tx_size, last_nz_pos);
          cost += (*zpc_costs)[coef_to_zpc_ptok(pt)][eoo];
          if (eoo)
            is_eoo[o] = 1;
        }
      }
