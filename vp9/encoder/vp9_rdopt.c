...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  int pred_exists = 0;
  unsigned int best_var = 0, best_sse = 0;
  ...
  *rate2 += vp9_cost_mv_ref(cpi, this_mode,
  ...
  if (block_size == BLOCK_64X64) {
    ...
    for (switchable_filter_index = 0;
      ...
      rd = RDCOST(x->rdmult, x->rddiv, rs, sse);
    ...
    for (switchable_filter_index = 0;
      ...
      var = vp9_variance32x32(*(b->base_src), b->src_stride,
                              tmp_ybuf, 64, &sse);
      vp9_variance16x16(x->src.u_buffer, x->src.uv_stride,
                        tmp_ubuf, 32, &sseu);
      vp9_variance16x16(x->src.v_buffer, x->src.uv_stride,
                        tmp_vbuf, 32, &ssev);
      sse += sseu + ssev;
      if (cpi->common.mcomp_filter_type == SWITCHABLE) {
        const int c = vp9_get_pred_context(cm, xd, PRED_SWITCHABLE_INTERP);
    ...
    for (switchable_filter_index = 0;
      ...
      mbmi->interp_filter = vp9_switchable_interp[switchable_filter_index];
      vp9_setup_interp_filters(xd, mbmi->interp_filter, &cpi->common);
      vp9_build_1st_inter16x16_predictors_mby(xd, tmp_ybuf, 16, 0);
      if (is_comp_pred)
        vp9_build_2nd_inter16x16_predictors_mby(xd, tmp_ybuf, 16);
#if CONFIG_COMP_INTERINTRA_PRED
      if (is_comp_interintra_pred) {
        vp9_build_interintra_16x16_predictors_mby(xd, tmp_ybuf, 16);
      }
#endif
      vp9_build_1st_inter16x16_predictors_mbuv(xd, tmp_ubuf, tmp_vbuf, 8);
      if (is_comp_pred)
        vp9_build_2nd_inter16x16_predictors_mbuv(xd, tmp_ubuf, tmp_vbuf, 8);
#if CONFIG_COMP_INTERINTRA_PRED
      if (is_comp_interintra_pred) {
        vp9_build_interintra_16x16_predictors_mbuv(xd, tmp_ubuf, tmp_vbuf, 8);
      }
#endif
