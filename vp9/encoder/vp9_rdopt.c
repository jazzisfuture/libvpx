...
int64_t vp9_block_sse_pixel_c(const int16_t *resi, int resi_stride, int w, int h) {
  int64_t sse = 0;
  int i, j;
...
static void block_rd_txfm(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  if (!is_inter_block(mbmi)
    ...
    if (xd->mi[0].src_mi->mbmi.sr) {
      dist_block_pixel(plane_bsize, plane, block, tx_size, args, dst, dst_stride);
    } else {
#endif
#if CONFIG_VP9_HIGHBITDEPTH
    if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
    ...
    if (x->skip_txfm[(plane << 2) + (block >> (tx_size << 1))] == 0) {
      ...
      if (xd->mi[0].src_mi->mbmi.sr) {
        ...
        dist_block_pixel(plane_bsize, plane, block, tx_size, args, tmp_buf, tmp_stride);
      } else {
#endif
#if CONFIG_VP9_HIGHBITDEPTH
      if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
...
static void choose_tx_size_from_rd(VP9_COMP *cpi, MACROBLOCK *x,
...
#if CONFIG_SR_MODE
  int sr_r = INT_MAX, sr_s = INT_MAX, sr0 = INT_MAX, sr1 = INT_MAX;
  ...
  if (is_enable_srmode(bs) && max_mode_tx_size >= max_tx_size) {
    ...
    for (k = 0; k < SR_USFILTER_NUM; k++) {
      ...
      if (tmp_d == INT64_MAX)
        ...
        tmp_rd = RDCOST(x->rdmult, x->rddiv, s1, tmp_d);
      else
  ...
  if (is_enable_srmode(bs) && max_mode_tx_size >= max_tx_size) {
    ...
    if (sr_d == INT64_MAX)
      ...
      sr_rd = RDCOST(x->rdmult, x->rddiv, s1, sr_d);
    else
