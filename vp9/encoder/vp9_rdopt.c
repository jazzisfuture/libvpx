...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  int pred_exists = 0;
  unsigned int best_var = 0;
  unsigned int best_varu = 0;
  ...
  *rate2 += vp9_cost_mv_ref(cpi, this_mode,
  ...
  if (block_size == BLOCK_64X64) {
    ...
    for (switchable_filter_index = 0;
      ...
      // TODO(debargha): Spit this function into a y part and a uv part to make
      ...
      var = vp9_variance64x64(*(b->base_src), b->src_stride,
                              tmp_ybuf, 64, &sse);
      newbest = (switchable_filter_index == 0 || var < best_var);
      if (newbest) {
        best_var = var;
