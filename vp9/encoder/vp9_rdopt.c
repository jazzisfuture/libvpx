  ...
  if (bsize != BLOCK_SIZE_MB16X16) {
    ...
    for (switchable_filter_index = 0;
      ...
      if (interpolating_intpel_seen && intpel_mv &&
        ...
        var = cpi->fn_ptr[block_size].vf(x->plane[0].src.buf, x->plane[0].src.stride,
                                         xd->plane[0].dst.buf,
        ...
        var = cpi->fn_ptr[uv_block_size].vf(x->plane[1].src.buf, x->plane[1].src.stride,
                                            xd->plane[1].dst.buf,
        ...
        var = cpi->fn_ptr[uv_block_size].vf(x->plane[2].src.buf, x->plane[1].src.stride,
                                            xd->plane[2].dst.buf,
                                            xd->plane[1].dst.stride,
                                            &sse);
        model_rd_from_var_lapndz(var, 8 * bw * 8 * bh,
