...
static int64_t handle_inter_mode(
  ...
  if (this_mode == NEWMV) {
    int rate_mv;
    if (is_comp_pred) {
      int num_iters = MAX_JOINT_MV_SEARCH_ITERS;
      if (cpi->sf.comp_inter_joint_search_num_iter >= 2)
        num_iters = 0;
      else if (cpi->sf.comp_inter_joint_search_num_iter >= 1)
        num_iters =
            bsize < BLOCK_8X8
                ? 0
                : (bsize <= BLOCK_32X32 ? 2 : MAX_JOINT_MV_SEARCH_ITERS);
