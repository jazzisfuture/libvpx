...
static int rd_cost_sb64uv_32x32(VP9_COMMON *const cm, MACROBLOCK *x, int backup) {
  int b;
  int cost = 0;
  MACROBLOCKD *const xd = &x->e_mbd;
...
static void rd_inter64x64_uv_32x32(VP9_COMMON *const cm, MACROBLOCK *x, int *rate,
                                   int *distortion, int *skip,
                                   int backup) {
  MACROBLOCKD *const xd = &x->e_mbd;
    ...
    if (labels[ib] == which_label) {
      ...
      if (xd->mode_info_context->mbmi.txfm_size == TX_4X4) {
        ...
        for (j = 0; j < 4; j += 2) {
          ...
          *distortion += thisdistortion;
          *labelyrate += cost_coeffs(cm, x, ib + iblock[j], PLANE_TYPE_Y_WITH_DC,
                           ta + vp9_block2above[TX_4X4][ib + iblock[j]],
                           tl + vp9_block2left[TX_4X4][ib + iblock[j]],
                           TX_4X4);
          *labelyrate += cost_coeffs(cm, x, ib + iblock[j] + 1,
        ...
        if (otherrd) {
          for (j = 0; j < 4; j += 2) {
            ...
            otherdist += thisdistortion;
            xd->mode_info_context->mbmi.txfm_size = TX_4X4;
            othercost += cost_coeffs(cm, x, ib + iblock[j], PLANE_TYPE_Y_WITH_DC,
                           tacp + vp9_block2above[TX_4X4][ib + iblock[j]],
                           tlcp + vp9_block2left[TX_4X4][ib + iblock[j]],
                           TX_4X4);
            othercost += cost_coeffs(cm, x, ib + iblock[j] + 1,
                           PLANE_TYPE_Y_WITH_DC,
