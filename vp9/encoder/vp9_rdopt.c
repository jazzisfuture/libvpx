    ...
    if (col  &&  labelings[i] == labelings[i - 1])
      ...
      switch (m = this_mode) {
        ...
        case LEFT4X4:
          ...
          if (get_mi_row(xd) == 0 && cpi->common.current_video_frame == 1 && get_mi_col(xd) == 14 &&
              this_mv->as_mv.row == -3 && this_mv->as_mv.col == 17 && i == 0 && which_label == 0 &&
              labelings == vp9_mbsplits[0])
            printf("YES\n");
          if (mbmi->second_ref_frame > 0)
...
static void rd_check_segment_txsize(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  for (i = 0; i < label_count && this_segment_rd < bsi->segment_rd; i++) {
    ...
    labels2mode(x, labels, i, mode_selected, &mode_mv[mode_selected],
                ...
                bsi->ref_mv, bsi->second_ref_mv, x->nmvjointcost, x->mvcost, cpi);
...
static void setup_buffer_inter(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  scale[frame_type].x_offset_q4 =
      (mi_col * MI_SIZE * scale[frame_type].x_num / scale[frame_type].x_den) & 0xf;
  scale[frame_type].y_offset_q4 =
      (mi_row * MI_SIZE * scale[frame_type].y_num / scale[frame_type].y_den) & 0xf;
  ...
  if (bsize != BLOCK_SIZE_MB16X16) {
    ...
    for (switchable_filter_index = 0;
      ...
      if (interpolating_intpel_seen && intpel_mv &&
        ...
        // Hence quantizer step is also 8 times. To get effective quantizer
        // we need to divide by 8 before sending to modeling function.
        model_rd_from_var_lapndz(var, MI_SIZE * MI_SIZE, xd->block[0].dequant[1] >> 3,
                                 &tmp_rate_y, &tmp_dist_y);
        var = vp9_variance8x8(x->src.u_buffer, x->src.uv_stride,
                              xd->plane[1].dst.buf, xd->plane[1].dst.stride,
                              &sse);
        model_rd_from_var_lapndz(var, MI_UV_SIZE * MI_UV_SIZE, xd->block[16].dequant[1] >> 3,
                                 &tmp_rate_u, &tmp_dist_u);
        var = vp9_variance8x8(x->src.v_buffer, x->src.uv_stride,
                              xd->plane[2].dst.buf, xd->plane[1].dst.stride,
                              &sse);
        model_rd_from_var_lapndz(var, MI_UV_SIZE * MI_UV_SIZE, xd->block[20].dequant[1] >> 3,
                                 &tmp_rate_v, &tmp_dist_v);
        rd = RDCOST(x->rdmult, x->rddiv,
                    rs + tmp_rate_y + tmp_rate_u + tmp_rate_v,
                    tmp_dist_y + tmp_dist_u + tmp_dist_v);
  ...
  if (pred_exists) {
    ...
    for (i = 0; i < bh * MI_SIZE; ++i)
      vpx_memcpy(xd->plane[0].dst.buf + i * xd->plane[0].dst.stride,
                 tmp_ybuf + i * bw * MI_SIZE, sizeof(unsigned char) * bw * MI_SIZE);
    for (i = 0; i < bh * MI_UV_SIZE; ++i)
      vpx_memcpy(xd->plane[1].dst.buf + i * xd->plane[1].dst.stride,
                 tmp_ubuf + i * bw * MI_UV_SIZE, sizeof(unsigned char) * bw * MI_UV_SIZE);
    for (i = 0; i < bh * MI_UV_SIZE; ++i)
      vpx_memcpy(xd->plane[2].dst.buf + i * xd->plane[1].dst.stride,
                 tmp_vbuf + i * bw * MI_UV_SIZE, sizeof(unsigned char) * bw * MI_UV_SIZE);
  } else {
