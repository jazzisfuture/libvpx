...
static void tx_block_rd_b(const VP9_COMP *cpi, MACROBLOCK *x, TX_SIZE tx_size,
                          int blk_row, int blk_col, int plane, int block,
                          int plane_bsize, ENTROPY_CONTEXT *ta,
                          ENTROPY_CONTEXT *tl, int *zero_blk_rate,
                          int *rate, int64_t *dist, int64_t *bsse, int *skip) {
  MACROBLOCKD *const xd = &x->e_mbd;
  struct macroblockd_plane *const pd = &xd->plane[plane];
  const struct macroblock_plane *const p = &x->plane[plane];
  unsigned int this_sse = 0;
  unsigned int this_dist = 0;
  tran_low_t *const dqcoeff = BLOCK_OFFSET(pd->dqcoeff, block);
  ...
  switch (tx_size) {
    case TX_4X4:
      break;
    case TX_8X8:
      ta[blk_col] = !!*(const uint16_t *)&ta[blk_col];
      tl[blk_row] = !!*(const uint16_t *)&tl[blk_row];
      break;
    case TX_16X16:
      ta[blk_col] = !!*(const uint32_t *)&ta[blk_col];
      tl[blk_row] = !!*(const uint32_t *)&tl[blk_row];
      break;
    case TX_32X32:
      ta[blk_col] = !!*(const uint64_t *)&ta[blk_col];
      tl[blk_row] = !!*(const uint64_t *)&tl[blk_row];
      break;
    default:
  ...
  if (p->eobs[block] > 0) {
    ...
    cpi->fn_ptr[txm_bsize].vf(src, src_stride,
                              rec_buffer, 32, &this_dist);
  }

  *bsse += (int64_t)(this_sse) * 16;
  *dist += (int64_t)(this_dist) * 16;
...
static void select_tx_block(const VP9_COMP *cpi, MACROBLOCK *x,
  ...
  *skip = 1;
  ...
  if (cpi->common.tx_mode == TX_MODE_SELECT || tx_size == TX_4X4) {
    tx_block_rd_b(cpi, x, tx_size, blk_row, blk_col, 0,
                  block, bsize, ta[0], tl[0],
    ...
    if ((RDCOST(x->rdmult, x->rddiv, *rate, *dist) >=
      ...
      *skip = 1;
      ...
      x->blk_skip[0][blk_row * block_stride + blk_col] = 1;
    } else {
      x->blk_skip[0][blk_row * block_stride + blk_col] = 0;
    }
    ...
    if (tx_size >= TX_8X8) {
      for (i = 1; i < MAX_MB_PLANE; ++i) {
        struct macroblockd_plane *const pdc = &xd->plane[i];
        const BLOCK_SIZE plane_bsize = get_plane_block_size(bsize, pdc);
        int this_rate_uv = 0;
        int64_t this_dist_uv = 0, this_bsse_uv = 0;
        int this_skip_uv = 1;
        TX_SIZE uv_tx_size = get_uv_tx_size_impl(tx_size, bsize,
                                                 pdc->subsampling_x,
                                                 pdc->subsampling_y);
        tx_block_rd_b(cpi, x, uv_tx_size, (blk_row / 2), (blk_col / 2),
                      i, block, plane_bsize, ta[i], tl[i],
                      &zero_blk_rate, &this_rate_uv, &this_dist_uv,
                      &this_bsse_uv, &this_skip_uv);
        if ((RDCOST(x->rdmult, x->rddiv, this_rate_uv, this_dist_uv) >=
             RDCOST(x->rdmult, x->rddiv, zero_blk_rate, this_bsse_uv)) &&
            !xd->lossless) {
          int k;
          this_rate_uv = zero_blk_rate;
          this_dist_uv = this_bsse_uv;
          this_skip_uv = 1;
          for (k = 0; k < (1 << uv_tx_size); ++k) {
            ta[i][blk_col + k] = 0;
            tl[i][blk_row + k] = 0;
          }
          x->blk_skip[i][(blk_row / 2) * block_stride + (blk_col / 2)] = 1;
        } else {
          x->blk_skip[i][(blk_row / 2) * block_stride + (blk_col / 2)] = 0;
        }

        rate_uv += this_rate_uv;
        dist_uv += this_dist_uv;
        bsse_uv += this_bsse_uv;
        skip_uv[i - 1] = this_skip_uv;
      }

      *rate += rate_uv;
      *dist += dist_uv;
      *bsse += bsse_uv;
//      *skip &= (skip_uv[0] & skip_uv[1]);
    }
  ...
  if (tx_size > TX_4X4) {
    BLOCK_SIZE tx_bsize = txsize_to_bsize[tx_size];
    int bh = num_4x4_blocks_high_lookup[tx_bsize];
    ...
    for (i = 0; i < 4; ++i) {
      ...
      select_tx_block(cpi, x, blk_row + offsetr, blk_col + offsetc,
                      block + i * sub_step, tx_size - 1, bsize,
    ...
    if (this_rd < sum_rd) {
      ...
      x->blk_skip[1][(blk_row / 2) * block_stride + (blk_col / 2)] = skip_uv[0];
      x->blk_skip[2][(blk_row / 2) * block_stride + (blk_col / 2)] = skip_uv[1];
      *skip &= (skip_uv[0] & skip_uv[1]);
    } else {
      *rate = sum_rate;
      *dist = sum_dist;
  ...
  if (is_cost_valid) {
    ...
    for (idy = 0; idy < mi_height; idy += bh) {
      for (idx = 0; idx < mi_width; idx += bh) {
        select_tx_block(cpi, x, idy, idx, block, max_tx_size, bsize,
  ...
  if (tx_size == plane_tx_size) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      tx_block_rd(cpi, x, blk_row + offsetr, blk_col + offsetc, plane,
                  block + i * step, tx_size - 1, plane_bsize, ta, tl,
                  rate, dist, bsse, skip);
    }
  }
}
...
//  if (cm->current_video_frame == 11 && cm->show_frame == 0 &&
//      mi_row == 0 && mi_col == 24 &&
//      mbmi->ref_frame[0] == GOLDEN_FRAME && mbmi->mode == NEWMV) {
//    int idx, idy;
//    int src_stride = x->plane[0].src.stride;
//    for (idy = 0; idy < 64; ++idy) {
//      for (idx = 0; idx < 64; ++idx)
//        fprintf(stderr, "%d  ", x->plane[0].src.buf[idy * src_stride + idx]);
//      fprintf(stderr, "\n");
//    }
//
//    fprintf(stderr, "\n");
//    src_stride = x->plane[1].src.stride;
//    for (idy = 0; idy < 32; ++idy) {
//      for (idx = 0; idx < 32; ++idx)
//        fprintf(stderr, "%d  ", x->plane[1].src.buf[idy * src_stride + idx]);
//      fprintf(stderr, "\n");
//    }
//
//    fprintf(stderr, "\n");
//    src_stride = x->plane[2].src.stride;
//    for (idy = 0; idy < 32; ++idy) {
//      for (idx = 0; idx < 32; ++idx)
//        fprintf(stderr, "%d  ", x->plane[2].src.buf[idy * src_stride + idx]);
//      fprintf(stderr, "\n");
//    }
//
//    exit(0);
