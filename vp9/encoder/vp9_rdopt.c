...
#endif
  {
    ...
    for (c = 0; c < eob; c++) {
      ...
      if (t == ZERO_TOKEN && !skip_coef_val) {
        ...
        use_eoo = vp9_use_eoo(c, seg_eob, scan, tx_size, is_last_zero, is_eoo_list);
#else
        use_eoo = 0;
#endif
        if (use_eoo) {
          ...
          if (eoo) {
            ...
            for (c_ = c + 1; c_ < eob; ++c_) {
              if (o == vp9_get_orientation(scan[c_], tx_size)) {
                int pt_ = vp9_get_coef_context(scan, nb, pad, token_cache_full, c_, default_eob);
                int band_ = get_coef_band(scan, tx_size, c_);
                assert(qcoeff_ptr[scan[c_]] == 0);
                assert(token_cache_full[scan[c_]] == ZERO_TOKEN);
                if (!c_ || token_cache[scan[c_ - 1]])
                  savings += vp9_cost_bit(coef_probs[type][ref][band_][pt_][0], 1);
