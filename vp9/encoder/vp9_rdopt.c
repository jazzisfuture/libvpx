...
#if CONFIG_BDINTRA
// With possible additional ref data (right column/below row),
// re-do intra modes selection.
  ...
  VP9_COMMON *const cm = &cpi->common;
  PREDICTION_MODE mode, mode_uv[TX_SIZES];
  TX_SIZE uv_tx;
  ...
  for (mode = DC_PRED; mode <= TM_PRED; mode++) {
    ...
    this_rate += ref_costs_single[INTRA_FRAME];
