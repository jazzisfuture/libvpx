...
static int64_t handle_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  } else if (discount_newmv_test(cpi, this_mode, frame_mv[refs[0]],
                                 mode_mv, refs[0])) {
    ...
    if (midx == -1) {
      if (cpi->mfu_mv[LAST_FRAME].row
        ...
        for (i = 0; i < MAX_MODES ; i++)
          if (vp9_mode_order[mode_map[i]].mode == NEWMV &&
              vp9_mode_order[mode_map[i]].ref_frame[0] == LAST_FRAME) {
            mode_index = mode_map[i];
            break;
          }
        if (i == MAX_MODES)
          continue;
