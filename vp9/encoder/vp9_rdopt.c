...
static void block_rd_txfm(int plane, int block, int blk_row, int blk_col,
  ...
  int skip_coeff_opt = args->cpi->sf.quant_coeff_opt_level;
  int sse_calc_done = 0;
  ...
  if (!is_inter_block(mi)) {
    ...
    struct encode_b_args intra_arg = { x,
                                       x->block_qcoeff_opt,
                                       args->t_above,
                                       args->t_left,
                                       &mi->skip,
    ...
    struct encode_b_args intra_arg = {
      x,    x->block_qcoeff_opt, args->t_above, args->t_left, &mi->skip,
      &sse, &sse_calc_done,      skip_coeff_opt
    };
#endif
    ...
    if (skip_txfm_flag == SKIP_TXFM_NONE ||
      ...
      const int enable_coeff_opt =
          x->block_qcoeff_opt &&
          !skip_coeff_optimize(xd, pd, diff, diff_stride, blk_row, blk_col,
                               plane_bsize, tx_size, skip_coeff_opt, &sse,
                               &sse_calc_done);
      // full forward transform and quantization
      vp9_xform_quant(x, plane, block, blk_row, blk_col, plane_bsize, tx_size);
      if (enable_coeff_opt) vp9_optimize_b(x, plane, block, tx_size, coeff_ctx);
      dist_block(args->cpi, x, plane, plane_bsize, block, blk_row, blk_col,
