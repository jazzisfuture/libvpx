...
#else
#if CONFIG_COMPOUND_MODES
    int_mv seg_mvs[4][INTER_MODES + INTER_COMPOUND_MODES][MAX_REF_FRAMES],
#else
    ...
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      ...
      for (ref = 0; ref < 1 + has_second_rf; ++ref) {
        ...
        frame_mv[NEWMV][frame].as_int =
            seg_mvs[i][INTER_OFFSET(NEWMV)][frame].as_int;
#else
...
#else
          frame_mv[NEW_NEWMV][frame].as_int =
              seg_mvs[i][INTER_OFFSET(NEW_NEWMV)][frame].as_int;
#endif  // CONFIG_NEWMVREF
      ...
      for (this_mode = NEARESTMV;
        ...
        if (!has_second_rf &&
            ...
            this_mode == NEWMV
#if CONFIG_COMPOUND_MODES
            && seg_mvs[i][mode_idx][refs[0]].as_int == INVALID_MV
#else
...
#else
#if CONFIG_COMPOUND_MODES
          seg_mvs[i][mode_idx][refs[0]].as_int =
              frame_mv[this_mode][refs[0]].as_int;
#else
        ...
        if (has_second_rf) {
          ...
          if (have_newmv_in_inter_mode(this_mode) &&
              (seg_mvs[i][INTER_OFFSET(NEWMV)][refs[1]].as_int == INVALID_MV ||
               seg_mvs[i][INTER_OFFSET(NEWMV)][refs[0]].as_int == INVALID_MV))
#else
        ...
        if (has_second_rf &&
          ...
          if (cpi->sf.comp_inter_joint_search_thresh <= bsize) {
...
#if CONFIG_COMPOUND_MODES
              seg_mv_sub8x8[refs[ref]].as_int =
                  seg_mvs[i][INTER_OFFSET(NEWMV)][refs[ref]].as_int;
#else
            ...
            for (ref = 0; ref < 2; ++ref) {
#if CONFIG_COMPOUND_MODES
              seg_mvs[i][mode_idx][refs[ref]].as_int =
#else
...
#if CONFIG_COMPOUND_MODES
    int k;
    for (k = 0; k < INTER_MODES + INTER_COMPOUND_MODES; k++)
      for (j = 0; j < MAX_REF_FRAMES; j++)
        seg_mvs[i][k][j].as_int = INVALID_MV;
#else
    for (j = 0; j < MAX_REF_FRAMES; j++)
