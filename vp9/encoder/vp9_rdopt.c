...
#if CONFIG_TX_SKIP
  int tx_skipped = 0;
  int try_tx_skip = vp9_get_qindex(&cpi->common.seg,
                                   xd->mi[0].src_mi->mbmi.segment_id,
                                   cpi->common.base_qindex) <= TX_SKIP_Q_THRESH;
#endif
  bmode_costs = cpi->y_mode_costs[A][L];
...
#if CONFIG_TX_SKIP
    if (vp9_get_qindex(&cm->seg, xd->mi[0].src_mi->mbmi.segment_id,
...
#endif
#if CONFIG_TX_SKIP
      if (vp9_get_qindex(&cpi->common.seg, xd->mi[0].src_mi->mbmi.segment_id,
                         cpi->common.base_qindex) <= TX_SKIP_Q_THRESH) {
        mbmi->tx_skip[0] = 1;
