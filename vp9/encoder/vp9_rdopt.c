...
#endif
#if CONFIG_NEWMVREF_SUB8X8
  int_mv ref_mv_sub8x8[MAX_REF_FRAMES];
    ...
    for (idx = 0; idx < 2; idx += num_4x4_blocks_wide) {
      ...
      for (ref = 0; ref < 1 + has_second_rf; ++ref) {
        ...
        vp9_find_best_ref_mvs(xd, allow_hp, mv_ref_list,
                              &ref_mv_sub8x8[ref], &near_sub8x8);
        // (zoeliu)
        if (cm->current_video_frame == 186 &&
            mi_row == 2 && mi_col == 20 && i == 1) {
          printf("rd_pick_best_sub8x8_mode(): Obtaining mv ref "
                 "Frame#=%d Block(%d,%d) Seg(%d) Ref=%d "
                 "nearest_mv=(%d,%d) ref_mv=(%d,%d)\n",
                 cm->current_video_frame, mi_row, mi_col, i, ref,
                 frame_mv[NEARESTMV][frame].as_mv.row,
                 frame_mv[NEARESTMV][frame].as_mv.col,
                 bsi->ref_mv[ref]->as_mv.row, bsi->ref_mv[ref]->as_mv.col);
        }
#endif  // CONFIG_NEWMVREF_SUB8X8
#if CONFIG_COMPOUND_MODES
      ...
      for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
        ...
        if (!has_second_rf && this_mode == NEWMV &&
          ...
          if (bestsme < INT_MAX) {
            ...
            if (cm->current_video_frame == 186 &&
                mi_row == 2 && mi_col == 20 && i == 1) {
              printf("rd_pick_best_sub8x8_mode() Before fractional search: "
                     "Frame#=%d Block(%d,%d) Seg(%d) ref=0 "
                     "mvp_full=(%d,%d) ref_mv=(%d,%d), new_mv=(%d,%d)\n",
                     cm->current_video_frame, mi_row, mi_col, i,
                     mvp_full.row, mvp_full.col,
                     bsi->ref_mv[0]->as_mv.row, bsi->ref_mv[0]->as_mv.col,
                     new_mv->row, new_mv->col);
            }
            ...
            if (cm->current_video_frame == 186 &&
                mi_row == 2 && mi_col == 20 && i == 1) {
              printf("rd_pick_best_sub8x8_mode() After fractional search: "
                     "Frame#=%d Block(%d,%d) Seg(%d) ref=0 "
                     "allow_hp=%d, force_stop=%d, iters_per_step=%d, "
                     "mvp_full=(%d,%d) ref_mv=(%d,%d), new_mv=(%d,%d)\n",
                     cm->current_video_frame, mi_row, mi_col, i,
                     cm->allow_high_precision_mv,
                     cpi->sf.mv.subpel_force_stop,
        ...
        if (has_second_rf && this_mode == NEWMV &&
          ...
          if (cpi->sf.comp_inter_joint_search_thresh <= bsize) {
            ...
            int ref;
            for (ref = 0; ref < 2; ++ref) {
              ref_mv[ref].as_int = bsi->ref_mv[ref]->as_int;
            }
#endif  // CONFIG_NEWMVREF_SUB8X8
            // (zoeliu)
            if (cm->current_video_frame == 186 &&
                mi_row == 2 && mi_col == 20 && i == 1) {
              printf("rd_pick_best_sub8x8_mode() Before joint search: "
                     "Frame#=%d Block(%d,%d) Seg(%d) ref=0 "
                     "ref_mv[0]=(%d,%d), ref_mv[1]=(%d,%d), "
                     "frame_mv[NEWMV][0]=(%d,%d), frame_mv[NEWMV][1]=(%d,%d), "
                     "seg_mvs[1][0]=(%d,%d), seg_mvs[1][1]=(%d,%d)\n",
                     cm->current_video_frame, mi_row, mi_col, i,
                     ref_mv[0].as_mv.row, ref_mv[0].as_mv.col,
                     ref_mv[1].as_mv.row, ref_mv[1].as_mv.col,
                     frame_mv[this_mode][mbmi->ref_frame[0]].as_mv.row,
                     frame_mv[this_mode][mbmi->ref_frame[0]].as_mv.col,
                     frame_mv[this_mode][mbmi->ref_frame[1]].as_mv.row,
                     frame_mv[this_mode][mbmi->ref_frame[1]].as_mv.col,
                     seg_mvs[i][mbmi->ref_frame[0]].as_mv.row,
                     seg_mvs[i][mbmi->ref_frame[0]].as_mv.col,
                     seg_mvs[i][mbmi->ref_frame[1]].as_mv.row,
                     seg_mvs[i][mbmi->ref_frame[1]].as_mv.col);
            }
            ...
            if (cm->current_video_frame == 186 &&
                mi_row == 2 && mi_col == 20 && i == 1) {
              printf("rd_pick_best_sub8x8_mode() After joint search: "
                     "Frame#=%d Block(%d,%d) Seg(%d) ref=0 "
                     "ref_mv[0]=(%d,%d), ref_mv[1]=(%d,%d), "
                     "frame_mv[NEWMV][0]=(%d,%d), frame_mv[NEWMV][1]=(%d,%d), "
                     "seg_mvs[1][0]=(%d,%d), seg_mvs[1][1]=(%d,%d)\n",
      ...
      if (cm->current_video_frame == 186 &&
          mi_row == 2 && mi_col == 20 && i == 1) {
        printf("rd_pick_best_sub8x8_mode() mode selected: "
               "Frame#=%d Block(%d,%d) Seg(%d) mode_selected=%d bsize=%d "
               "mode_mv[%d][0]=(%d,%d), mode_mv[%d][1]=(%d,%d), "
               "ref_mv[0]=(%d,%d), ref_mv[1]=(%d,%d), "
               "frame_mv[%d][0]=(%d,%d), frame_mv[%d][1]=(%d,%d), "
               "seg_mvs[1][0]=(%d,%d), seg_mvs[1][1]=(%d,%d)\n",
               cm->current_video_frame, mi_row, mi_col, i,
               mode_selected, bsize,
               mode_selected,
               mode_mv[mode_selected][0].as_mv.row,
               mode_mv[mode_selected][0].as_mv.col,
               mode_selected,
               mode_mv[mode_selected][1].as_mv.row,
               mode_mv[mode_selected][1].as_mv.col,
               bsi->ref_mv[0]->as_mv.row, bsi->ref_mv[0]->as_mv.col,
               bsi->ref_mv[1]->as_mv.row, bsi->ref_mv[1]->as_mv.col,
               mode_selected,
               frame_mv[mode_selected][mbmi->ref_frame[0]].as_mv.row,
               frame_mv[mode_selected][mbmi->ref_frame[0]].as_mv.col,
               mode_selected,
               frame_mv[mode_selected][mbmi->ref_frame[1]].as_mv.row,
               frame_mv[mode_selected][mbmi->ref_frame[1]].as_mv.col,
               seg_mvs[i][mbmi->ref_frame[0]].as_mv.row,
               seg_mvs[i][mbmi->ref_frame[0]].as_mv.col,
               seg_mvs[i][mbmi->ref_frame[1]].as_mv.row,
               seg_mvs[i][mbmi->ref_frame[1]].as_mv.col);
