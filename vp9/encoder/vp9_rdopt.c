...
static void select_tx_block(const VP9_COMP *cpi, MACROBLOCK *x,
  ...
  vpx_memcpy(ctxa, ta, sizeof(ENTROPY_CONTEXT) * (16 >> pd->subsampling_x));
  vpx_memcpy(ctxl, tl, sizeof(ENTROPY_CONTEXT) * (16 >> pd->subsampling_y));
    ...
    if (this_rd < sum_rd) {
      ...
      *skip = all_skip;
      vpx_memcpy(pta, ctxa + (blk_col >> pd->subsampling_x),
                 sizeof(ENTROPY_CONTEXT) * max_blocks_wide);
      vpx_memcpy(ptl, ctxl + (blk_row >> pd->subsampling_y),
                 sizeof(ENTROPY_CONTEXT) * max_blocks_high);
    }
  }
}
