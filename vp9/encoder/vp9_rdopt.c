...
static int64_t rd_pick_intra_sby_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (colors > 1 && colors <= 64) {
    ...
    int palette_indexed_size_cost[PALETTE_SIZES];
    int palette_literal_size_cost[PALETTE_SIZES];
    ...
    vp9_cost_tokens(palette_indexed_size_cost,
                    cpi->common.fc.palette_indexed_size_prob[bsize - BLOCK_8X8],
                    vp9_palette_size_tree);
    vp9_cost_tokens(palette_literal_size_cost,
                    cpi->common.fc.palette_literal_size_prob[bsize - BLOCK_8X8],
                    vp9_palette_size_tree);
    ...
    for (n = colors > PALETTE_MAX_SIZE ? PALETTE_MAX_SIZE : colors;
      ...
      if (m1 == PALETTE_MAX_SIZE || m2 == PALETTE_MAX_SIZE)
        continue;
      ...
      for (ps = H_SCAN; ps < PALETTE_SCAN_ORDERS; ps++) {
        ...
        this_rate = this_rate_tokenonly +
            (1 + PALETTE_DELTA_BIT + get_bit_depth(palette_max_run(bsize)) +
                2 + get_bit_depth(mic->mbmi.current_palette_size) * m1 +
                mic->mbmi.palette_delta_bitdepth * m1 +
                8 * m2 + get_bit_depth(k) * (l >> 1)) *
                vp9_cost_bit(128, 0) +
                palette_indexed_size_cost[m1] + palette_literal_size_cost[m2];
        for (i = 0; i < l; i += 2) {
