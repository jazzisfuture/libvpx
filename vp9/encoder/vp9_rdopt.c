...
static void select_tx_block(const VP9_COMP *cpi, MACROBLOCK *x,
  ...
  *rate += 128;
  this_rd = MIN(RDCOST(x->rdmult, x->rddiv, *rate, *dist),
                RDCOST(x->rdmult, x->rddiv, 128, *bsse));
  ...
  if (tx_size > TX_4X4) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      select_tx_block(cpi, x, blk_row + offsetr, blk_col + offsetc,
                      ...
                      plane_bsize, &this_rate, &this_dist,
                      &this_bsse, &this_skip);
      sum_rate += this_rate;
      sum_dist += this_dist;
      sum_bsse += this_bsse;
    ...
    sum_rd = MIN(RDCOST(x->rdmult, x->rddiv, sum_rate, sum_dist),
                 RDCOST(x->rdmult, x->rddiv, 128, sum_bsse));
    if (this_rd < sum_rd) {
      int idx, idy;
