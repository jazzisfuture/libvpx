...
static void swap_block_ptr(MACROBLOCK *x, PICK_MODE_CONTEXT *ctx,
                           int m, int n, int max_plane) {
  struct macroblock_plane *const p = x->plane;
  struct macroblockd_plane *const pd = x->e_mbd.plane;
  int i;

  for (i = 0; i < max_plane; ++i) {
    p[i].coeff    = ctx->coeff_pbuf[i][m];
    p[i].qcoeff   = ctx->qcoeff_pbuf[i][m];
    pd[i].dqcoeff = ctx->dqcoeff_pbuf[i][m];
    p[i].eobs     = ctx->eobs_pbuf[i][m];
    ...
    ctx->coeff_pbuf[i][n]   = p[i].coeff;
    ctx->qcoeff_pbuf[i][n]  = p[i].qcoeff;
    ctx->dqcoeff_pbuf[i][n] = pd[i].dqcoeff;
    ctx->eobs_pbuf[i][n]    = p[i].eobs;
  }
}
    ...
    if (this_rd < best_rd) {
      ...
      *skippable      = s;
      if (!x->select_txfm_size)
        swap_block_ptr(x, ctx, 2, 0, MAX_MB_PLANE);
    }
  }
    ...
    if (this_rd < best_rd || x->skip) {
      ...
      if (!mode_excluded) {
        ...
        *returndistortion = distortion2;
        ...
        if (!x->select_txfm_size)
          swap_block_ptr(x, ctx, 1, 0, max_plane);
