  ...
  *rate2 += vp9_cost_mv_ref(cpi, this_mode,
  ...
  if (bsize == BLOCK_SIZE_SB64X64) {
    ...
    for (switchable_filter_index = 0;
      ...
      if ((cm->mcomp_filter_type == SWITCHABLE && newbest) ||
          (cm->mcomp_filter_type != SWITCHABLE &&
           cm->mcomp_filter_type == mbmi->interp_filter)) {
        vpx_memcpy(tmp_ybuf, xd->dst.y_buffer, sizeof(unsigned char) * 256);
        vpx_memcpy(tmp_ubuf, xd->dst.u_buffer, sizeof(unsigned char) * 64);
        vpx_memcpy(tmp_vbuf, xd->dst.v_buffer, sizeof(unsigned char) * 64);
        pred_exists = 1;
      }
      interpolating_intpel_seen |=
        intpel_mv && vp9_is_interpolating_filter[mbmi->interp_filter];
  ...
  if (pred_exists) {
    if (bsize == BLOCK_SIZE_SB64X64) {
      ...
      vpx_memcpy(xd->dst.y_buffer, tmp_ybuf, sizeof(unsigned char) * 256);
      vpx_memcpy(xd->dst.u_buffer, tmp_ubuf, sizeof(unsigned char) * 64);
      vpx_memcpy(xd->dst.v_buffer, tmp_vbuf, sizeof(unsigned char) * 64);
    }
  } else {
