...
static unsigned int get_source_diff_16x16(VP9_COMP *cpi,
                                                    MACROBLOCK *x,
                                                    int mi_row,
                                                    int mi_col,
                                                    int block_offset,
                                                    unsigned int *sse,
                                                    int *sum) {
  const YV12_BUFFER_CONFIG *pre_source = cpi->Last_Source;
  const int pre_stride = cpi->unscaled_last_source->y_stride;
  int offset = (mi_row * MI_SIZE) * pre_stride + (mi_col * MI_SIZE) +
               block_offset;

  vp9_get_sse_sum_16x16(x->plane[0].src.buf + block_offset,
                          x->plane[0].src.stride,
...
static void set_source_var_based_partition(VP9_COMP *cpi,
  ...
  if ((col8x8_remaining >= MI_BLOCK_SIZE) &&
    ...
    diff d32[4];
    int use16x16 = 0;
    int thr_32 = cpi->sf.source_var_thresh;
    int thr_64 = thr_32 * 2;
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        d16[j].var = get_source_diff_16x16(cpi, x, mi_row, mi_col,
                                           b_offset, &d16[j].sse, &d16[j].sum);
      ...
      if (d16[0].var < thr_32 && d16[1].var < thr_32 &&
          d16[2].var < thr_32 && d16[3].var < thr_32) {
        d32[i].sse = d16[0].sse;
        d32[i].sum = d16[0].sum;
    ...
    if (!use16x16) {
      if (d32[0].var < thr_64 && d32[1].var < thr_64 &&
          d32[2].var < thr_64 && d32[3].var < thr_64)  {
    ...
    if (cpi->sf.partition_search_type == SOURCE_VAR_BASED_PARTITION &&
      ...
      if ((cm->current_video_frame - 1) % check_freq >= 1) {
        if (cpi->use_large_partition_rate < 15)
          cpi->sf.partition_search_type = FIXED_PARTITION; //VAR_BASED_FIXED_PARTITION;
      }
    }

    //printf("  %d\n", cpi->use_large_partition_rate);
  }
