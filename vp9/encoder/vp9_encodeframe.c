...
static void choose_partitioning(VP9_COMP *cpi,
  ...
  int low_res = ((cm->width * cm->height) <= (352 * 288)) ? 1 : 0;
  int use_4x4_partition = (cm->frame_type == KEY_FRAME) ? 1 : 0;
  ...
  // Modify thresholds for key frame and for low-resolutions (choose lower
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (cm->frame_type != KEY_FRAME) {
        ...
        // and if it is large we go down for 4x4. For key frame we always go
        // down to 4x4.
        // Set to zero because of comparison below.
        vt.split[i].split[j].part_variances.none.variance = 0;
        if (low_res) {
      ...
      if ((cm->frame_type == KEY_FRAME) || (low_res &&
          vt.split[i].split[j].part_variances.none.variance > threshold)) {
        variance4x4downsample[i * 4 + j] = 1;
        for (k = 0; k < 4; k++) {
          int x8_idx = x16_idx + ((k & 1) << 3);
          int y8_idx = y16_idx + ((k >> 1) << 3);
          // Go down to 4x4.
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      if (variance4x4downsample[i * 4 + j] == 1) {
        for (m = 0; m < 4; m++) {
  ...
  // For low-resolutions, force split of 64x64 partition.
  if ( mi_col + 8 > cm->mi_cols || mi_row + 8 > cm->mi_rows ||
      !set_vt_partitioning(cpi, xd, &vt, BLOCK_64X64, mi_row, mi_col,
                           threshold_bsize_max, bsize_min)) {
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, xd, &vt.split[i], BLOCK_32X32,
        ...
        for (j = 0; j < 4; ++j) {
          ...
          // If 8x8 down-sampling is used for variance calculation, 8x8
          // partition is set if variance of the 16x16 block is very high.
          // If 4x4 down-sampling is used for variance calculation
          // (e.g., key frames and low-resolutions), 4x4 partition will be set
          // if variance of 8x8 block is very high.
          int64_t threshold_16x16 = (use_4x4_partition) ? threshold :
              threshold_bsize_min;
