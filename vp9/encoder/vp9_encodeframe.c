        ...
        for (k = 0; k < 4; k++) {
          ...
          int y8_idx = y16_idx + ((k >> 1) << 3);
            ...
            if (x8_idx < pixels_wide && y8_idx < pixels_high) {
              ...
              sum = s_avg - d_avg;
              sse = sum * sum;
              // If we have some difference in average pixel levels, check the
              // 8x8 pixel-based (temporal and spatial) variance and force
              // split to 8x8 block if these variances are large.
              //if (fabs(sum) > 2 && !force_split[5 + i2 + j] && segment_id) {
              if (fabs(sum) > 2 && !force_split[5 + i2 + j]) {
                vp9_get8x8var(s + y8_idx * sp + x8_idx, sp,
                              d + y8_idx * dp + x8_idx, dp, &sse2, &sum2);
                if ((sse2 - (((unsigned int)sum2 * sum2) >> 6)) >
                    (threshold << 2)) {
                  // Check the spatial: only force split if there is some also
                  // some spatial activity/edge in this 8x8 block.
                  vp9_get8x8var(s + y8_idx * sp + x8_idx, sp, VP9_VAR_OFFS, 8,
                                &sse2, &sum2);
                  if ((sse2 - (((unsigned int)sum2 * sum2) >> 6)) >
                      (threshold << 6)) {
                    // Force split for this 16x16 block, this also imposes
