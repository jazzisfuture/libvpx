...
static void update_stats(VP9_COMP *cpi, int mi_row, int mi_col) {
  ...
  if (cm->frame_type != KEY_FRAME) {
    ...
    seg_ref_active = vp9_segfeature_active(&xd->seg, segment_id, SEG_LVL_REF_FRAME);
...
static void reset_skip_txfm_size_b(VP9_COMP *cpi, MODE_INFO *mi, int mis,
  ...
  if (mbmi->txfm_size > txfm_max) {
    ...
    assert(
        vp9_segfeature_active(&xd->seg, segment_id, SEG_LVL_SKIP) || get_skip_flag(mi, mis, ymbs, xmbs));
