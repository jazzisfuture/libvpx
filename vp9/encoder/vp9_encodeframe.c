...
static void rd_pick_partition(VP9_COMP *cpi, ThreadData *td,
  ...
  *rd_cost = best_rdc;
  ...
  do {
    ...
    // bs, final RD(3), none RD(3), split RD(3), ac_quant, rdmult, rddiv,
    // none_sum_y_eobs, none_is_inter, none_is_compound, none_skip, block_var
    fprintf(fp, "%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,",
            ...
            x->rdmult, x->rddiv, none_sum_y_eobs,
            none_is_inter, none_is_compound, none_skip,
            (int)(vp9_log_block_var(cpi, x, bsize) * 1000));
    fprintf(fp, "\n");
    fclose(fp);
  } while (0);
#endif
