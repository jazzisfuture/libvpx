...
static int avg_source_sad(VP9_COMP *cpi, MACROBLOCK *x, int shift, int mi_col, int mi_row) {
  #if CONFIG_VP9_HIGHBITDEPTH
    ...
    tmp_sad = cpi->fn_ptr[bsize].sdf(src_y, src_ystride, last_src_y,
                                                   last_src_ystride);
    tmp_variance = vpx_variance64x64(
        src_y, src_ystride, last_src_y, last_src_ystride, &tmp_sse);
    printf("%d %d %d %d %d %d \n", cpi->common.current_video_frame, mi_col, mi_row, tmp_sad, tmp_variance, tmp_sse);
    ...
    if (tmp_sad < avg_source_sad_threshold2) {
     // Cap the increment to 255.
      if (x->content_state_sb_fd < 255)
        x->content_state_sb_fd++;
    } else {
      x->content_state_sb_fd = 0;
    }
    return 0;
}
  ...
  if (cpi->sf.use_source_sad && !is_key_frame) {
    ...
    x->skip_low_source_sad = (content_state == kLowSadLowSumdiff ||
                              content_state == kLowSadHighSumdiff)
                                 ? 1
                                 : 0;
    x->last_sb_high_content = x->content_state_sb_fd;
...
static void encode_nonrd_sb_row(VP9_COMP *cpi, ThreadData *td,
  ...
  for (mi_col = mi_col_start, sb_col_in_tile = 0; mi_col < mi_col_end;
    ...
    if (cpi->compute_source_sad_onepass_cbr) {
      ...
      avg_source_sad(cpi, x, shift, mi_col * 8, mi_row * 8);
    }
