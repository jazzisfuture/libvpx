...
static void choose_partitioning(VP9_COMP *cpi, MODE_INFO *m, int mi_row,
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      const unsigned char *st = s + y_idx * sp + x_idx;
      const unsigned char *dt = d + y_idx * dp + x_idx;
      unsigned int sse = 0;
      int sum = 0;
