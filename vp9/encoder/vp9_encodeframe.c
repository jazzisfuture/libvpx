...
static void duplicate_modeinfo_in_sb(const VP9_COMP *cpi,
...
static int set_vt_partitioning(VP9_COMP *cpi, void *data, MODE_INFO **m,
  ...
  if (mi_row + mi_size < cm->mi_rows && vt.part_variances->vert[0].variance < threshold
  ...
  if (mi_col + mi_size < cm->mi_cols && vt.part_variances->horz[0].variance < threshold
...
static void choose_partitioning(VP9_COMP *cpi, MODE_INFO **mi_8x8,
  ...
  if (cm->frame_type != KEY_FRAME) {
    setup_pre_planes(xd, 0, yv12, mi_row, mi_col, sf);
...
static void nonrd_pick_sb_modes(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  duplicate_modeinfo_in_sb(cpi, xd, mi_row,  mi_col, bsize);
}
...
static void nonrd_pick_fixed_partition(VP9_COMP *cpi,
  ...
  for (br = 0; br < rows; br += bh) {
    for (bc = 0; bc < cols; bc += bw) {
      ...
      duplicate_modeinfo_in_sb(cpi, xd, row, col, bs);
    }
  }
}
