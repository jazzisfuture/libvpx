...
static void pick_sb_modes(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  xd->mi_8x8->mi->mbmi.skip_coeff = 0;
...
static void copy_partitioning(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
  ...
  for (block_row = 0; block_row < 8; ++block_row) {
    for (block_col = 0; block_col < 8; ++block_col) {
      mi_8x8[block_row * mis + block_col].mi->mbmi.sb_type =
          prev_mi_8x8[block_row * mis + block_col].mi->mbmi.sb_type;
    }
  }
}
  ...
  if (cpi->sf.adjust_partitioning_from_last_frame) {
    ...
    if (partition == PARTITION_SPLIT && subsize > BLOCK_8X8) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (mi_8x8[jj * bss * mis + ii * bss].mi->mbmi.sb_type >= sub_subsize) {
          splits_below = 0;
        }
      }
    }
