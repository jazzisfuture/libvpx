  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (!is_key_frame) {
        ...
        // |min - max| is large for all the 8x8 blocks, so force split for
        // this 16x16 to 8x8. Don't change behavior for low-resolution for now.
        if (min_max_count == 4 && !low_res) {
