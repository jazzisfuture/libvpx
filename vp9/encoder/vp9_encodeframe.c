...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t, int output_enabled,
  ...
  if (mbmi->two_stage_coding[0])
    printf(">>>>>2stg! %d %d\n", mbmi->qindex_plus, mbmi->eob_thresh);
#endif
...
#if 0
    if (output_enabled && mbmi->two_stage_coding[0]) {
      struct macroblock_plane *const p = &x->plane[0];
      struct macroblockd_plane *const pd = &xd->plane[0];
      tran_low_t *qcoeff = BLOCK_OFFSET(p->qcoeff, 0);
      tran_low_t *qcoeff_stg2 = BLOCK_OFFSET(p->qcoeff_stg2, 0);
      int bs = 4 << mbmi->tx_size, i, j;
