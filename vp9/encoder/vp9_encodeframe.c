...
static void set_vbp_thresholds(VP9_COMP *cpi, int64_t thresholds[], int q) {
  ...
  if (is_key_frame) {
    ...
    // threshold[3] is only used for sub8x8 (i.e., if use_4x4_partition is set)
...
static int choose_partitioning(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  // TODO(marpan): Use this to enable sub8x8 partition for low resolutions.
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (is_key_frame || use_4x4_partition ||
          (low_res &&
           vt.split[i].split[j].part_variances.none.variance >
               (thresholds[1] << 1))) {
        force_split[split_index] = 0;
