...
static void encode_tiles(VP9_COMP *cpi) {
  ...
  for (tile_row = 0; tile_row < tile_rows; ++tile_row) {
    for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
      TileInfo tile;
      TOKENEXTRA *old_tok = tok[tile_row][tile_col];
      int mi_row;
      ...
      for (mi_row = tile.mi_row_start; mi_row < tile.mi_row_end;
        ...
        if (cpi->sf.use_nonrd_pick_mode && !frame_is_intra_only(cm))
          encode_nonrd_sb_row(cpi, &tile, mi_row, &tok[tile_row][tile_col]);
        else
          encode_rd_sb_row(cpi, &tile, mi_row, &tok[tile_row][tile_col]);
      }
      cpi->tok_count[tile_row][tile_col] =
          (unsigned int)(tok[tile_row][tile_col] - old_tok);
      assert(tok[tile_row][tile_col] - old_tok <= alloc_tokens_tile);
    }
  }
}
