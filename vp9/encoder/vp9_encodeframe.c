...
static int choose_partitioning(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (!is_key_frame) {
    ...
    if (cpi->oxcf.speed >= 8 && !low_res &&
      ...
      // has many blocks with non-zero motion, but is not a scene/slide change:
      // if the estimated integer motion vector has large row component and small
      // column component, then flag it as possible (vertical) scroll motion.
