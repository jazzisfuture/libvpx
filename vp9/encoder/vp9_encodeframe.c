...
static void encode_sb_row(VP9_COMP *cpi,
  ...
  for (mi_col = cm->cur_tile_mi_col_start;
    ...
    for (i = 0; i < MAX_MB_PLANE; i++) {
      memcpy(a + 16 * i, cm->above_context[i] +
                 (mi_col * 4 >> (CONFIG_SB8X8 + xd->plane[i].subsampling_x)),
             sizeof(ENTROPY_CONTEXT) * 16 >> xd->plane[i].subsampling_x);
      memcpy(l + 16 * i, cm->left_context[i],
             sizeof(ENTROPY_CONTEXT) * 16 >> xd->plane[i].subsampling_y);
    }
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (i = 0; i < MAX_MB_PLANE; i++) {
        vpx_memcpy(l2 + 8 * i,
                   ...
                   sizeof(ENTROPY_CONTEXT) * 8 >> xd->plane[i].subsampling_y);
        vpx_memcpy(a2 + 8 * i,
                   cm->above_context[i] +
                       ((mi_col + x_idx) * 4 >> (CONFIG_SB8X8 +
                                                 xd->plane[i].subsampling_x)),
                   sizeof(ENTROPY_CONTEXT) * 8 >> xd->plane[i].subsampling_x);
      }
      ...
      for (i = 0; i < MAX_MB_PLANE; i++) {
        vpx_memcpy(cm->left_context[i] +
                       (y_idx * 4 >> (CONFIG_SB8X8 +
                                      xd->plane[i].subsampling_y)),
                   l2 + 8 * i,
                   sizeof(ENTROPY_CONTEXT) * 8 >> xd->plane[i].subsampling_y);
        vpx_memcpy(cm->above_context[i] +
                       ((mi_col + x_idx) * 4 >> (CONFIG_SB8X8 +
                                                 xd->plane[i].subsampling_x)),
                   a2 + 8 * i,
                   sizeof(ENTROPY_CONTEXT) * 8 >> xd->plane[i].subsampling_x);
    ...
    for (i = 0; i < MAX_MB_PLANE; i++) {
      memcpy(cm->above_context[i] +
                 (mi_col * 4 >> (CONFIG_SB8X8 + xd->plane[i].subsampling_x)),
             a + 16 * i,
             sizeof(ENTROPY_CONTEXT) * 16 >> xd->plane[i].subsampling_x);
      memcpy(cm->left_context[i], l + 16 * i,
             sizeof(ENTROPY_CONTEXT) * 16 >> xd->plane[i].subsampling_y);
    }
