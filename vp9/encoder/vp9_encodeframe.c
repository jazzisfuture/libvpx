...
static void encode_sb(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (bsize >= BLOCK_8X8) {
    ...
    subsize = subsize_lookup[this_context->partitioning][bsize];
  } else {
    ctx = 0;
    subsize = BLOCK_4X4;
  }
    ...
    case PARTITION_VERT:
      ...
      if (mi_col + hbs < cm->mi_cols && bsize > BLOCK_8X8) {
        encode_b_rt(cpi, tile, tp, mi_row, mi_col + hbs, output_enabled,
                    subsize, &this_context->vertical[1]);
      }
      break;
    case PARTITION_HORZ:
      encode_b_rt(cpi, tile, tp, mi_row, mi_col, output_enabled, subsize,
      ...
      if (mi_row + hbs < cm->mi_rows && bsize > BLOCK_8X8) {
        encode_b_rt(cpi, tile, tp, mi_row + hbs, mi_col, output_enabled,
                    subsize, &this_context->horizontal[1]);
      }
      break;
    case PARTITION_SPLIT:
      subsize = get_subsize(bsize, PARTITION_SPLIT);
...
static void rd_use_partition(VP9_COMP *cpi,
                             ...
                             int do_recon, PC_TREE *this_context,
                             int block) {
  VP9_COMMON *const cm = &cpi->common;
  MACROBLOCK *const x = &cpi->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  PICK_MODE_CONTEXT *ctx = &this_context->none;
  ...
  this_context->partitioning = partition;
  ...
  if (cpi->sf.partition_search_type == SEARCH_PARTITION &&
    ...
    if (partition != PARTITION_NONE && !splits_below &&
      ...
      this_context->partitioning = PARTITION_NONE;
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &last_part_rate,
                       &last_part_dist, subsize, &this_context->horizontal[0],
                       INT64_MAX, 0);
      if (last_part_rate != INT_MAX &&
          bsize >= BLOCK_8X8 && mi_row + (mh >> 1) < cm->mi_rows) {
        int rt = 0;
        int64_t dt = 0;
        PICK_MODE_CONTEXT *ctx = &this_context->horizontal[0];
        ...
        rd_pick_sb_modes(cpi, tile, mi_row + (ms >> 1), mi_col, &rt, &dt,
                         subsize, &this_context->horizontal[1], INT64_MAX, 1);
    ...
    case PARTITION_VERT:
      rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &last_part_rate,
                       &last_part_dist, subsize, &this_context->vertical[0],
                       INT64_MAX, 0);
      if (last_part_rate != INT_MAX &&
          bsize >= BLOCK_8X8 && mi_col + (ms >> 1) < cm->mi_cols) {
        int rt = 0;
        int64_t dt = 0;
        PICK_MODE_CONTEXT *ctx = &this_context->vertical[0];
        update_state(cpi, ctx, mi_row, mi_col, subsize, 0);
        encode_superblock(cpi, tp, 0, mi_row, mi_col, subsize, ctx);
        rd_pick_sb_modes(cpi, tile, mi_row, mi_col + (ms >> 1), &rt, &dt,
                         subsize, &this_context->vertical[bsize > BLOCK_8X8], INT64_MAX, 1);
        if (rt == INT_MAX || dt == INT64_MAX) {
          last_part_rate = INT_MAX;
          last_part_dist = INT64_MAX;
          break;
    ...
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &last_part_rate,
                         &last_part_dist, subsize, this_context->leaf_split[0],
                         INT64_MAX, 0);
        break;
      }
      last_part_rate = 0;
  ...
  if (cpi->sf.adjust_partitioning_from_last_frame
    ...
    chosen_dist = 0;
    restore_context(cpi, mi_row, mi_col, a, l, sa, sl, bsize);
    this_context->partitioning = PARTITION_SPLIT;
    ...
    for (i = 0; i < 4; i++) {
      ...
      save_context(cpi, mi_row, mi_col, a, l, sa, sl, bsize);
      this_context->split[i]->partitioning = PARTITION_NONE;
      rd_pick_sb_modes(cpi, tile, mi_row + y_idx, mi_col + x_idx, &rt, &dt,
                       split_subsize, &this_context->split[i]->none,
                       INT64_MAX, i);
      ...
      if (i != 3)
        encode_sb(cpi, tile, tp,  mi_row + y_idx, mi_col + x_idx, 0,
                  split_subsize, this_context->split[i]);
...
static void rd_pick_partition(VP9_COMP *cpi, const TileInfo *const tile,
                              ...
                              PC_TREE *this_context, int block) {
  VP9_COMMON *const cm = &cpi->common;
  MACROBLOCK *const x = &cpi->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  printf("%d %4d,%4d %2d %4d,%4d %4d\n",
         cpi->common.current_video_frame,
         mi_row, mi_col, bsize,
         xd->mi_8x8[0]->mbmi.mv[0].as_mv.row,
         xd->mi_8x8[0]->mbmi.mv[0].as_mv.col,
         cpi->common.counts.eob_branch[0][0][1][0][0]);
  ...
  if (do_split) {
    ...
    if (bsize == BLOCK_8X8) {
      ...
      if (cpi->sf.adaptive_pred_interp_filter && partition_none_allowed)
        this_context->leaf_split[0]->pred_interp_filter =
            ctx->mic.mbmi.interp_filter;

      rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &sum_rate, &sum_dist, subsize,
                       this_context->leaf_split[0], best_rd, 0);
      ...
      for (i = 0; i < 4 && sum_rd < best_rd; ++i) {
        ...
        rd_pick_partition(cpi, tile, tp, mi_row + y_idx, mi_col + x_idx,
                          ...
                          best_rd - sum_rd, this_context->split[i], i);
  ...
  if (partition_horz_allowed && do_rect) {
    ...
    if (cpi->sf.adaptive_pred_interp_filter && bsize == BLOCK_8X8 &&
      ...
      this_context->horizontal[0].pred_interp_filter =
          ctx->mic.mbmi.interp_filter;
    rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &sum_rate, &sum_dist, subsize,
                     &this_context->horizontal[0], best_rd, 0);
    sum_rd = RDCOST(x->rdmult, x->rddiv, sum_rate, sum_dist);
    ...
    if (sum_rd < best_rd && mi_row + ms < cm->mi_rows) {
      ...
      if (cpi->sf.adaptive_pred_interp_filter && bsize == BLOCK_8X8 &&
          partition_none_allowed)
        this_context->horizontal[1].pred_interp_filter =
            ctx->mic.mbmi.interp_filter;
      rd_pick_sb_modes(cpi, tile, mi_row + ms, mi_col, &this_rate,
                       &this_dist, subsize, &this_context->horizontal[1],
                       best_rd - sum_rd, 1);
      if (this_rate == INT_MAX) {
        sum_rd = INT64_MAX;
      } else {
    ...
    if (sum_rd < best_rd) {
      ...
      if (sum_rd < best_rd) {
        ...
        best_rate = sum_rate;
        best_dist = sum_dist;
        this_context->partitioning = PARTITION_HORZ;
      }
    }
    restore_context(cpi, mi_row, mi_col, a, l, sa, sl, bsize);
  }
  ...
  if (partition_vert_allowed && do_rect) {
    ...
    if (cpi->sf.adaptive_pred_interp_filter && bsize == BLOCK_8X8 &&
        partition_none_allowed)
      this_context->vertical[0].pred_interp_filter =
          ctx->mic.mbmi.interp_filter;
    rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &sum_rate, &sum_dist, subsize,
                     &this_context->vertical[0], best_rd, 0);
    sum_rd = RDCOST(x->rdmult, x->rddiv, sum_rate, sum_dist);
    if (sum_rd < best_rd && mi_col + ms < cm->mi_cols) {
      update_state(cpi, &this_context->vertical[0], mi_row, mi_col, subsize, 0);
      encode_superblock(cpi, tp, 0, mi_row, mi_col, subsize, &this_context->vertical[0]);

      if (cpi->sf.adaptive_motion_search)
        load_pred_mv(x, ctx);
      if (cpi->sf.adaptive_pred_interp_filter && bsize == BLOCK_8X8 &&
          partition_none_allowed)
        this_context->vertical[bsize > BLOCK_8X8].pred_interp_filter =
            ctx->mic.mbmi.interp_filter;
      rd_pick_sb_modes(cpi, tile, mi_row, mi_col + ms, &this_rate,
                       &this_dist, subsize, &this_context->vertical[bsize > BLOCK_8X8],
                       best_rd - sum_rd, 1);
      if (this_rate == INT_MAX) {
        sum_rd = INT64_MAX;
      } else {
    ...
    if (sum_rd < best_rd) {
      ...
      if (sum_rd < best_rd) {
        ...
        best_dist = sum_dist;
        best_rd = sum_rd;
        this_context->partitioning = PARTITION_VERT;
        if (bsize == BLOCK_8X8) {
          printf("vertical < 8x8 chosen %d %d %d %d \n",
                 cpi->common.current_video_frame, mi_row,
                 mi_col, sum_rate);
        }
      }
    }
...
static void nonrd_use_partition(VP9_COMP *cpi,
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      ...
      nonrd_use_partition(cpi, tile, mi_8x8 + hbs, tp,
                          ...
                          &rate, &dist, this_context->split[1]);
      if (rate != INT_MAX && dist != INT64_MAX &&
          *totrate != INT_MAX && *totdist != INT64_MAX) {
        *totrate += rate;
        *totdist += dist;
      ...
      nonrd_use_partition(cpi, tile, mi_8x8 + hbs * mis, tp,
                          ...
                          &rate, &dist, this_context->split[2]);
      if (rate != INT_MAX && dist != INT64_MAX &&
          *totrate != INT_MAX && *totdist != INT64_MAX) {
        *totrate += rate;
        *totdist += dist;
      ...
      nonrd_use_partition(cpi, tile, mi_8x8 + hbs * mis + hbs, tp,
                          ...
                          &rate, &dist, this_context->split[3]);
      if (rate != INT_MAX && dist != INT64_MAX &&
          *totrate != INT_MAX && *totdist != INT64_MAX) {
        *totrate += rate;
        *totdist += dist;
  ...
  if (bsize == BLOCK_64X64 && output_enabled) {
    ...
    encode_sb_rt(cpi, tile, tp, mi_row, mi_col, 1, bsize, this_context);
  }
}
    ...
    switch (cpi->sf.partition_search_type) {
      ...
      case REFERENCE_PARTITION:
        if (cpi->sf.partition_check) {
          ...
          nonrd_use_partition(cpi, tile, mi_8x8, tp, mi_row, mi_col,
                              BLOCK_64X64, 0, &rate1, &dist1, x->pc_root);
          set_fixed_partitioning(cpi, tile, mi_8x8, mi_row, mi_col,
                                 BLOCK_16X16);
          nonrd_use_partition(cpi, tile, mi_8x8, tp, mi_row, mi_col,
                              BLOCK_64X64, 0, &rate2, &dist2, x->pc_root);
          set_fixed_partitioning(cpi, tile, mi_8x8, mi_row, mi_col,
                                 BLOCK_32X32);
          nonrd_use_partition(cpi, tile, mi_8x8, tp, mi_row, mi_col,
                              BLOCK_64X64, 0, &rate3, &dist3, x->pc_root);
