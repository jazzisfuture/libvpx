...
static void set_mode_info(MB_MODE_INFO *mbmi, BLOCK_SIZE bsize,
  ...
  if (mode < NEARESTMV) {
    ...
    mbmi->ref_frame[0] = LAST_FRAME;
    if (!mi_row && !mi_col)
      mbmi->mode_context[LAST_FRAME] = BOTH_PREDICTED;
    else if (!mi_row || !mi_col)
      mbmi->mode_context[LAST_FRAME] = ZERO_PLUS_PREDICTED;
    else
      mbmi->mode_context[LAST_FRAME] = BOTH_ZERO;
...
static void rtc_use_partition(VP9_COMP *cpi,
  ...
  MACROBLOCKD *const xd = &cpi->mb.e_mbd;
  const int mis = cm->mode_info_stride;
  const int mi_width = num_8x8_blocks_wide_lookup[cpi->sf.always_this_block_size];
  const int mi_height = num_8x8_blocks_high_lookup[cpi->sf.always_this_block_size];
  ...
  for (block_row = 0; block_row < MI_BLOCK_SIZE; block_row += mi_height) {
    for (block_col = 0; block_col < MI_BLOCK_SIZE; block_col += mi_width) {
      int index = block_row * mis + block_col;
      // Find a partition size that fits
      bsize = find_partition_size(cpi->sf.always_this_block_size,
                                  (row8x8_remaining - block_row),
      ...
      set_mode_info(&mi_8x8[index]->mbmi, bsize, mode, mi_row + block_row, mi_col + block_col);
