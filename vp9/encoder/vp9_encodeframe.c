...
void vp9_encode_frame(VP9_COMP *cpi) {
  ...
  if (cpi->sf.RD) {
    ...
    if (0) printf("Encoding frame %d:%d filter %d type=%d\n",
           cpi->common.current_video_frame,
           cpi->refresh_alt_ref_frame,
           filter_type, frame_type);
    encode_frame_internal(cpi);
    if (0) printf("Filter count: %d, %d, %d\n",
           cpi->common.fc.switchable_interp_count[0][0] +
           cpi->common.fc.switchable_interp_count[1][0] +
           cpi->common.fc.switchable_interp_count[2][0] +
           cpi->common.fc.switchable_interp_count[3][0],
           cpi->common.fc.switchable_interp_count[0][1] +
           cpi->common.fc.switchable_interp_count[1][1] +
           cpi->common.fc.switchable_interp_count[2][1] +
           cpi->common.fc.switchable_interp_count[3][1],
           cpi->common.fc.switchable_interp_count[0][2] +
           cpi->common.fc.switchable_interp_count[1][2] +
           cpi->common.fc.switchable_interp_count[2][2] +
           cpi->common.fc.switchable_interp_count[3][2]);

    for (i = 0; i < NB_PREDICTION_TYPES; ++i) {
      // FIXME(rbultje) can this overflow int64_t?
      const int diff = (int) (cpi->rd_comp_pred_diff[i] / cpi->common.MBs);
      cpi->rd_prediction_type_threshes[frame_type][i] += diff;
      // FIXME(rbultje) you probably want rounding towards zero here,
      // i.e. /2 not >>1
    ...
    for (i = 0; i <= VP9_SWITCHABLE_FILTERS; i++) {
      ...
      if (0) printf("Filter RD diff[%d]: %lld -> %lld\n",
             i, diff, cpi->rd_filter_threshes[frame_type][i]);
    }
