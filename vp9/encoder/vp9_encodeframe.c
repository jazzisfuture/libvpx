...
static void copy_partitioning(VP9_COMP *cpi, MODE_INFO **mi_8x8,
  ...
  for (block_row = 0; block_row < 8; ++block_row) {
    for (block_col = 0; block_col < 8; ++block_col) {
      ...
      mi->mbmi.sb_type = prev_mi->mbmi.sb_type;
    }
  }
}
...
static void set_block_size(VP9_COMMON * const cm, MODE_INFO **mi_8x8,
  ...
  MODE_INFO **m2 = mi_8x8 + mi_row * mis + mi_col;
  ...
  if (cpi->sf.adjust_partitioning_from_last_frame) {
    ...
    if (partition == PARTITION_SPLIT && subsize > BLOCK_8X8) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        if (mi_8x8[jj * bss * mis + ii * bss]->mbmi.sb_type >= sub_subsize)  {
          splits_below = 0;
        }
      }
    }
...
static void get_sb_partition_size_range(VP9_COMP *cpi, MODE_INFO ** mi_8x8,
  ...
  for (i = 0; i < sb_height_in_blocks; ++i) {
    for (j = 0; j < sb_width_in_blocks; ++j) {
      *min_block_size = MIN(*min_block_size, mi_8x8[index + j]->mbmi.sb_type);
      *max_block_size = MAX(*max_block_size, mi_8x8[index + j]->mbmi.sb_type);
    }
...
static void rd_auto_partition_range(VP9_COMP *cpi,
  ...
  const MB_MODE_INFO *const above_mbmi = &mi_8x8[-xd->mode_info_stride]->mbmi;
  const MB_MODE_INFO *const left_mbmi = &mi_8x8[-1]->mbmi;
  ...
  if (!left_in_image && !above_in_image) {
    ...
    *max_block_size = BLOCK_4X4;
    ...
    if (left_in_image) {
      get_sb_partition_size_range(cpi, &mi_8x8[-MI_BLOCK_SIZE],
...
static void set_txfm_flag(MODE_INFO **mi_8x8, int mis, int ymbs, int xmbs,
  ...
  for (y = 0; y < ymbs; y++) {
    for (x = 0; x < xmbs; x++)
      mi_8x8[y * mis + x]->mbmi.txfm_size = txfm_size;
  }
}
...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t, int output_enabled,
  ...
  if (!is_inter_block(mbmi)) {
    ...
    mb_skip_context += mi_8x8[-mis]->mbmi.skip_coeff;
      ...
      for (y = 0; y < mi_height; y++) {
        for (x = 0; x < mi_width; x++) {
          if (mi_col + x < cm->mi_cols && mi_row + y < cm->mi_rows) {
            mi_8x8[mis * y + x]->mbmi.txfm_size = sz;
          }
        }
      }
    }
