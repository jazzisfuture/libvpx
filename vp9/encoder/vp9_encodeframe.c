...
static void rd_pick_sb_modes(VP9_COMP *cpi, TileDataEnc *tile_data,
  ...
  if (rate_in_best_rd < INT_MAX && dist_in_best_rd < INT64_MAX) {
    int rddiv = x->rddiv;
    // Avoid shift of negtive numbers
    if (dist_in_best_rd < 0) {
      dist_in_best_rd *= (1 << rddiv);
      rddiv = 0;
    }
    if (rate_in_best_rd < 0) {
      best_rd =
          RDCOST_NEG_R(x->rdmult, rddiv, rate_in_best_rd, dist_in_best_rd);
    } else {
      best_rd = RDCOST(x->rdmult, rddiv, rate_in_best_rd, dist_in_best_rd);
    }
  }
