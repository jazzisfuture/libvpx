...
#if CONFIG_CODE_NONZEROCOUNT
static void gather_nzcs_mb16(MACROBLOCKD *xd) {
  int i;
...
static void gather_nzcs_sb32(MACROBLOCKD *xd) {
  int i;
  vpx_memset(xd->mode_info_context->mbmi.nzcs, 0,
             384 * sizeof(xd->mode_info_context->mbmi.nzcs[0]));
  for (i = 0; i < 256; ++i) {
...
static void gather_nzcs_sb64(MACROBLOCKD *xd) {
  int i;
  vpx_memset(xd->mode_info_context->mbmi.nzcs, 0,
             384 * sizeof(xd->mode_info_context->mbmi.nzcs[0]));
  switch (xd->mode_info_context->mbmi.txfm_size) {
