...
static void tile_data_init(VP9_COMP *cpi, TileDataEnc *tile_data) {
  int i, j;
  for (i = 0; i < BLOCK_SIZES; ++i) {
    for (j = 0; j < MAX_MODES; ++j) {
      tile_data->thresh_freq_fact[i][j] = cpi->rd.thresh_freq_fact[i][j];
      tile_data->mode_map[i][j] = cpi->rd.mode_map[i][j];
    }
  }
}
  ...
  if (cpi->tile_data == NULL) {
    ...
    for (tile_row = 0; tile_row < tile_rows; ++tile_row)
      for (tile_col = 0; tile_col < tile_cols; ++tile_col)
        tile_data_init(cpi, &cpi->tile_data[tile_row * tile_cols + tile_col]);
  }
