  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (!is_key_frame) {
        ...
        // |min - max| is large for most of the 8x8 blocks, so force split for
        // this 16x16 to 8x8. Also condition this split on high distortion/pred
        // error from last encoded frame. And don't change behavior for
        // low-resolution for now.
        block_index = (mi_row + (y16_idx >> 3)) * cm->mi_cols +
                      (mi_col + (x16_idx >> 3));
        if (min_max_count > 2 && !low_res  &&
           (cpi->oxcf.aq_mode == CYCLIC_REFRESH_AQ &&
            cr->dist_map[block_index] == 1)) {
