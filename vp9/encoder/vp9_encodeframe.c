...
static int ml_predict_breakout(const VP9_COMP *const cpi, BLOCK_SIZE bsize,
  ...
  const float *linear_weights = NULL;
  float linear_score;
  int i, feature_index = 0;

  switch (bsize) {
    case BLOCK_64X64:
      linear_weights = partition_breakout_weights_64;
      break;
    case BLOCK_32X32:
      linear_weights = partition_breakout_weights_32;
      break;
    case BLOCK_16X16:
      linear_weights = partition_breakout_weights_16;
      break;
    case BLOCK_8X8:
      linear_weights = partition_breakout_weights_8;
      break;
    default: assert(0 && "Unexpected block size."); return 0;
  }
  ...
  sse = sse >> num_pels_log2;

  vpx_clear_system_state();

  {
    float rate_f = (float)VPXMIN(rd_cost->rate, INT_MAX);
    const float dist_f =
        (float)(VPXMIN(rd_cost->dist, INT_MAX) >> num_pels_log2);
    rate_f =
        ((float)x->rdmult / 128.0f / 512.0f / (float)(1 << num_pels_log2)) * rate_f;
  ...
  linear_score = linear_weights[feature_index];
  for (i = 0; i < feature_index; ++i)
      ...
      if (this_rdc.rdcost < best_rdc.rdcost) {
        ...
        if (!cpi->sf.ml_partition_search_early_termination) {
          ...
          if (!x->e_mbd.lossless && ctx->skippable) {
            if (cpi->sf.use_ml_partition_search_breakout &&
                cm->base_qindex >= 200) {
