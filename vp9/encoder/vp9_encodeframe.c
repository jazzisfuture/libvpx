...
// computed for 8x8, and for some cases (if variance of 16x16 block is large),
// the variance based on 4x4 downsampled inputs is used for selection of
// block with size <= 16x16.
static void choose_partitioning(VP9_COMP *cpi,
  ...
  // If the variance of the 16x16 block (based on 8x8 down-sampling) is larger
  // than threshold16x16, then we compute variance from 16x16 down based
  // on 4x4 down-sampling.
  int64_t threshold16x16 = threshold;
  // Threshold used to select partition based on 4x4 down-sampling variance,
  // for blocks <= 16x16.
  int64_t thresholdPartSel4X4 = threshold << 5;

  // Test parameters for spatial grad.
  int spatial_avg[4];
  int spatial_act[16];
  int spatial_grad_thr = 20000;
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      // Flag this block if there is strong h/v spatial variation.
      if (abs((spatial_avg[0] + spatial_avg[1]) -
              (spatial_avg[2] + spatial_avg[3])) > spatial_grad_thr)
        spatial_act[i * 4 + j] = 1;
        else {
          if (abs((spatial_avg[0] + spatial_avg[2]) -
                  (spatial_avg[1] + spatial_avg[3])) > spatial_grad_thr)
              spatial_act[i * 4 + j] = 1;
        }
      // Get variance of the 16x16 and check against threshold. If this variance
      // above threshold, compute variance based on 2x2 to get more samples to
      // compare against 8x8, 8x16, 16x8.
      fill_variance_tree(&vt.split[i].split[j], BLOCK_16X16);
      if (vt.split[i].split[j].part_variances.none.variance > threshold16x16)
      {
        down2x2[i * 4 + j] = 1;
        for (i2 = 0; i2 < 4; i2++) {
          int x8_idx = x16_idx + ((i2 & 1) << 3);
          int y8_idx = y16_idx + ((i2 >> 1) << 3);
          v8x8 *vst2 = &vt2[i * 4 + j].split[i2];
          for (j2 = 0; j2 < 4; j2++) {
            int x4_idx = x8_idx + ((j2 & 1) << 2);
            int y4_idx = y8_idx + ((j2 >> 1) << 2);
            unsigned int sse = 0;
            int sum = 0;
            if (x4_idx < pixels_wide && y4_idx < pixels_high) {
              int s_avg = vp9_avg_4x4_c(s + y4_idx * sp + x4_idx, sp);
              int d_avg = vp9_avg_4x4_c(d + y4_idx * dp + x4_idx, dp);
              sum = s_avg - d_avg;
              sse = sum * sum;
            }
            fill_variance(sse, sum, 1, &vst2->split[j2].part_variances.none);
          }
        }
      }
    }
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      if (down2x2[i * 4 + j] == 1) {
        for (i2 = 0; i2 < 4; i2++) {
          fill_variance_tree(&vt2[i * 4 + j].split[i2], BLOCK_8X8);
        }
        fill_variance_tree(&vt2[i * 4 + j], BLOCK_16X16);
      }
    }
  ...
  if ( mi_col + 8 > cm->mi_cols || mi_row + 8 > cm->mi_rows ||
      !set_vt_partitioning(cpi, &vt, BLOCK_64X64, mi_row, mi_col, threshold,
                           0)) {
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, &vt.split[i], BLOCK_32X32,
        ...
        for (j = 0; j < 4; ++j) {
          ...
          const int block_width = num_8x8_blocks_wide_lookup[BLOCK_16X16];
          const int block_height = num_8x8_blocks_high_lookup[BLOCK_16X16];
          if (down2x2[i * 4 + j] == 1 ||
              mi_col + x32_idx + x16_idx + block_width / 2 >= cm->mi_cols ||
              mi_row + y32_idx + y16_idx + block_height / 2 >= cm->mi_rows) {
            if (!set_vt_partitioning(cpi, &vt2[i * 4 + j],
                                     BLOCK_16X16,
