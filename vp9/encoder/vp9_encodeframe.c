...
static void lower_extend(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  const int mi_width = num_8x8_blocks_wide_lookup[bsize];
  const int mi_height = num_8x8_blocks_high_lookup[bsize];
  BLOCK_SIZE extend_bsize = (mi_width == 1 || bsize < BLOCK_8X8) ? 
...
static void right_extend(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  const int mi_width = num_8x8_blocks_wide_lookup[bsize];
  const int mi_height = num_8x8_blocks_high_lookup[bsize];
  BLOCK_SIZE extend_bsize = (mi_height == 1 || bsize < BLOCK_8X8) ?
  ...
  switch (partition) {
    ...
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        ...
        for (i = 0; i < MAX_MB_PLANE; i++) {
          if (bsize == BLOCK_8X8 && i != 0)  // debugtest
            continue;
          if (mi_row < cm->mi_rows && mi_col + hbs < cm->mi_cols) {
            vp9_build_masked_inter_predictor_complex(xd,
                                                     dst_buf[i],
