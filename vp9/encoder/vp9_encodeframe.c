  ...
  for (mi_col = cm->cur_tile_mi_col_start;
    ...
    for (i = 0; i < 4; i++) {
      ...
      if (mi_col + x_idx + (2 << CONFIG_SB8X8) <= cm->mi_cols) {
        ...
        vpx_memcpy(cm->left_context.v + (y_idx * 2 >> CONFIG_SB8X8),
                   l2 + 12, sizeof(ENTROPY_CONTEXT) * 4);
        vpx_memcpy(cm->above_context[0] +
                       ((mi_col + x_idx) * 4 >> CONFIG_SB8X8),
                   a2 +  0, sizeof(ENTROPY_CONTEXT) * 8);
        vpx_memcpy(cm->above_context[1] +
                       ((mi_col + x_idx) * 2 >> CONFIG_SB8X8),
                   a2 +  8, sizeof(ENTROPY_CONTEXT) * 4);
        vpx_memcpy(cm->above_context[2] +
                       ((mi_col + x_idx) * 2 >> CONFIG_SB8X8),
                   a2 + 12, sizeof(ENTROPY_CONTEXT) * 4);
      }
...
static void init_encode_frame_mb_context(VP9_COMP *cpi) {
  ...
  vpx_memset(cm->above_context[0], 0,
             sizeof(ENTROPY_CONTEXT) * 8 * mb_cols_aligned_to_sb(cm));
