  ...
  if (cpi->sf.short_circuit_low_temp_var) {
    ...
    if (ref_frame_partition == LAST_FRAME) {
      ...
      if (vt.part_variances.none.variance < (thresholds[0] >> 1))
        x->variance_low[0] = 1;
      // 64x32
      if (vt.part_variances.horz[0].variance < (thresholds[0] >> 1))
        x->variance_low[1] = 1;
	  if (vt.part_variances.horz[1].variance < (thresholds[0] >> 1))
        x->variance_low[2] = 1;
      // 32x64
      if (vt.part_variances.vert[0].variance < (thresholds[0] >> 1))
        x->variance_low[3] = 1;
	  if (vt.part_variances.vert[1].variance < (thresholds[0] >> 1))
        x->variance_low[4] = 1;
      // 32x32
      for (i = 0; i < 4; i++) {
