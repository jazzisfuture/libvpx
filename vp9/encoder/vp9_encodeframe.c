...
static int choose_partitioning(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  // frame here, for the purpose of the superblock partitioning. This can happen
  // (LAST is NULL) in some cases where enhancement spatial layers are suddenly
  // enabled and the only reference is spatial reference.
  if (cpi->use_svc) {
