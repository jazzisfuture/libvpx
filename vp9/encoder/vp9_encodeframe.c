...
static void nonrd_pick_partition(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (partition_horz_allowed && do_rect) {
    ...
    nonrd_pick_sb_modes(cpi, tile, mi_row, mi_col,
                        &this_rate, &this_dist, subsize, &pc_tree->horizontal[0]);
    ...
    if (sum_rd < best_rd && mi_row + ms < cm->mi_rows) {
      ...
      nonrd_pick_sb_modes(cpi, tile, mi_row + ms, mi_col,
                          &this_rate, &this_dist, subsize, &pc_tree->horizontal[1]);
  ...
  if (partition_vert_allowed && do_rect) {
    ...
    nonrd_pick_sb_modes(cpi, tile, mi_row, mi_col,
                        &this_rate, &this_dist, subsize, &pc_tree->vertical[0]);
    ...
    if (sum_rd < best_rd && mi_col + ms < cm->mi_cols) {
      ...
      nonrd_pick_sb_modes(cpi, tile, mi_row, mi_col + ms,
                          &this_rate, &this_dist, subsize, &pc_tree->vertical[1]);
