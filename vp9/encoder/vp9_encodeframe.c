...
static BLOCK_SIZE get_nonrd_var_based_fixed_partition(VP9_COMP *cpi, MACROBLOCK *x,
                                                      int mi_row,
                                                      int mi_col) {
  unsigned int var = get_sby_perpixel_diff_variance(cpi, &x->plane[0].src,
                                                    mi_row, mi_col,
    ...
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        ...
        encode_sb(cpi, td, tile, tp, mi_row, mi_col, output_enabled, subsize,
                  pc_tree->split[0]);
        encode_sb(cpi, td, tile, tp, mi_row, mi_col + hbs, output_enabled, subsize,
                  pc_tree->split[1]);
        encode_sb(cpi, td, tile, tp, mi_row + hbs, mi_col, output_enabled, subsize,
                  pc_tree->split[2]);
...
static void nonrd_use_partition(VP9_COMP *cpi,
  ...
  switch (partition) {
    case PARTITION_NONE:
      ...
      pc_tree->none.skip_txfm[0] = x->skip_txfm[0];
      pc_tree->none.skip = x->skip;
      encode_b_rt(cpi, td, tile_info, tp, mi_row, mi_col, output_enabled, subsize,
                  &pc_tree->none);
      break;
    case PARTITION_VERT:
      pc_tree->vertical[0].pred_pixel_ready = 1;
    ...
    for (tile_col = 0; tile_col < tile_cols; ++tile_col) {
      ...
      for (mi_row = tile_info->mi_row_start; mi_row < tile_info->mi_row_end;
        ...
        if (cpi->sf.use_nonrd_pick_mode && !frame_is_intra_only(cm))
          encode_nonrd_sb_row(cpi, &cpi->td, this_tile, mi_row, &tok[tile_row][tile_col]);
        else
          encode_rd_sb_row(cpi, &cpi->td, this_tile, mi_row, &tok[tile_row][tile_col]);
      }
...
void vp9_encode_frame(VP9_COMP *cpi) {
  ...
  if (cpi->sf.frame_parameter_update) {
    ...
    for (i = 0; i < TX_MODES; ++i) {
      ...
      if (i == TX_MODE_SELECT)
        pd -= RDCOST(cpi->td.mb.rdmult, cpi->td.mb.rddiv, 2048 * (TX_SIZES - 1), 0);
