...
static void encode_sb_row(VP9_COMP *cpi,
  ...
  for (mb_col = cm->cur_tile_mb_col_start;
    ...
    for (i = 0; i < 4; i++) {
      ...
      mb_rate += vp9_cost_bit(cm->fc.partition_prob[1][PARTITION_NONE], 0);
      ...
      if (!sb32_skip && !(mb_col + x_idx + 1 >= cm->mb_cols ||
        ...
        sb_rate += vp9_cost_bit(cm->fc.partition_prob[1][PARTITION_NONE], 1);
      }
    ...
    sb32_rate += vp9_cost_bit(cm->fc.partition_prob[2][PARTITION_NONE], 0);

    if (!sb64_skip && !(mb_col + 3 >= cm->mb_cols ||
                        mb_row + 3 >= cm->mb_rows)) {
      pick_sb_modes(cpi, mb_row, mb_col, tp, &sb64_rate, &sb64_dist,
                    BLOCK_SIZE_SB64X64, &x->sb64_context);
      sb64_rate += vp9_cost_bit(cm->fc.partition_prob[2][PARTITION_NONE], 1);
    }
