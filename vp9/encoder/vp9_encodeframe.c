        ...
        if ((num =
          ...
          for (i = 0; i < num; i++) {
            ...
            if (get_gmtype(&cm->global_motion[frame][i]) != GLOBAL_ZERO) {
              ...
              /*
              double erroradvantage_unq =
                  vp9_warp_erroradv_unq(GLOBAL_MOTION_MODEL,
                                        global_motion,
                                        ref_buf->y_buffer,
                                        ref_buf->y_crop_width,
                                        ref_buf->y_crop_height,
                                        ...
                                        cpi->Source->y_stride,
                                        0, 0, 16, 16);
              printf("Ref %d [%d]: %d %d %d %d [Warp Error %f/%f]\n",
                     ...
                     erroradvantage_unq,
                     erroradvantage);
              printf("Ref %d [%d]: %f %f %f %f [Warp Error Unq %f]\n",
                     frame, cm->current_video_frame,
                     global_motion[i * get_numparams(GLOBAL_MOTION_MODEL)],
                     global_motion[i * get_numparams(GLOBAL_MOTION_MODEL) + 1],
                     global_motion[i * get_numparams(GLOBAL_MOTION_MODEL) + 2],
                     global_motion[i * get_numparams(GLOBAL_MOTION_MODEL) + 3],
                     erroradvantage_unq);
              */
