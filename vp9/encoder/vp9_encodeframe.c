...
static void predict_sb_complex_highbd(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  int i, ctx;
  DECLARE_ALIGNED_ARRAY(16, uint16_t, tmp_buf1,
                        MAX_MB_PLANE * MAXTXLEN * MAXTXLEN);
  DECLARE_ALIGNED_ARRAY(16, uint16_t, tmp_buf2,
                        MAX_MB_PLANE * MAXTXLEN * MAXTXLEN);
  DECLARE_ALIGNED_ARRAY(16, uint16_t, tmp_buf3,
                        MAX_MB_PLANE * MAXTXLEN * MAXTXLEN);
  uint8_t *dst_buf1[3] = {
    CONVERT_TO_BYTEPTR(tmp_buf1),
    CONVERT_TO_BYTEPTR(tmp_buf1 + 1 * MAXTXLEN * MAXTXLEN),
    CONVERT_TO_BYTEPTR(tmp_buf1 + 2 * MAXTXLEN * MAXTXLEN)};
  uint8_t *dst_buf2[3] = {
    CONVERT_TO_BYTEPTR(tmp_buf2),
    CONVERT_TO_BYTEPTR(tmp_buf2 + 1 * MAXTXLEN * MAXTXLEN),
    CONVERT_TO_BYTEPTR(tmp_buf2 + 2 * MAXTXLEN * MAXTXLEN)};
  uint8_t *dst_buf3[3] = {
    CONVERT_TO_BYTEPTR(tmp_buf3),
    CONVERT_TO_BYTEPTR(tmp_buf3 + 1 * MAXTXLEN * MAXTXLEN),
    CONVERT_TO_BYTEPTR(tmp_buf3 + 2 * MAXTXLEN * MAXTXLEN)};
  int dst_stride1[3] = {MAXTXLEN, MAXTXLEN, MAXTXLEN};
  int dst_stride2[3] = {MAXTXLEN, MAXTXLEN, MAXTXLEN};
  int dst_stride3[3] = {MAXTXLEN, MAXTXLEN, MAXTXLEN};
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      ...
      if (mi_row + hbs < cm->mi_rows && bsize > BLOCK_8X8) {
        ...
        for (i = 0; i < MAX_MB_PLANE; i++) {
          xd->plane[i].dst.buf = dst_buf[i];
          xd->plane[i].dst.stride = dst_stride[i];
          vp9_build_masked_inter_predictor_complex_highbd(
    ...
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        ...
        for (i = 0; i < MAX_MB_PLANE; i++) {
          if (mi_row < cm->mi_rows && mi_col + hbs < cm->mi_cols) {
            vp9_build_masked_inter_predictor_complex_highbd(dst_buf[i],
                                                            dst_stride[i],
                                                            dst_buf1[i],
                                                            dst_stride1[i],
                                                            &xd->plane[i],
                                                            ...
                                                            PARTITION_VERT);
            if (mi_row + hbs < cm->mi_rows) {
              vp9_build_masked_inter_predictor_complex_highbd(dst_buf2[i],
                                                              dst_stride2[i],
                                                              dst_buf3[i],
                                                              dst_stride3[i],
                                                              &xd->plane[i],
