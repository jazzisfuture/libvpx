...
static INLINE int qp2sse(int quantizer) { return quantizer * quantizer; }
...
static int get_distortion_denominator_by_segment(const VP9_COMP *const cpi,
  ...
  int sse_base;
  int sse_base_plus_delta;

  if (qdelta == 0) return 1;

  sse_base = qp2sse(qbase);

  // sse_base / 2 is for rounding to the nearest
  // integer during the final division
  sse_base_plus_delta = (qp2sse(qbase + qdelta) + sse_base / 2);
...
// return distortion denominator for rate-distortion optimization
static int get_distortion_denominator(const VP9_COMP *const cpi, int idx_y,
                                      int idx_x, BLOCK_SIZE block_size) {
