...
static void assign_partition_info(
    ...
    const int num_unit_cols, const int is_partition_vert,
    const int is_partition_horz, PARTITION_INFO *partition_info) {
  int i, j;
  for (i = 0; i < block_height_4x4; ++i) {
    for (j = 0; j < block_width_4x4; ++j) {
      const int row_4x4 =
          row_start_4x4 + i + (is_partition_horz ? block_height_4x4 : 0);
      const int col_4x4 =
          col_start_4x4 + j + (is_partition_vert ? block_width_4x4 : 0);
      ...
      partition_info[unit_index].row_start =
          (is_partition_horz ? row_start_4x4 + block_height_4x4 : row_start_4x4)
          << 2;
      partition_info[unit_index].column_start =
          (is_partition_vert ? col_start_4x4 + block_width_4x4 : col_start_4x4)
          << 2;
...
static void store_superblock_partition_info(
  ...
  if (pc_tree->partitioning == PARTITION_NONE ||
    ...
    assign_partition_info(row_start_4x4, col_start_4x4, block_width_4x4,
                          block_height_4x4, num_unit_rows, num_unit_cols, 0, 0,
                          partition_info);
    if (pc_tree->partitioning == PARTITION_VERT) {
      assign_partition_info(row_start_4x4, col_start_4x4, block_width_4x4,
                            block_height_4x4, num_unit_rows, num_unit_cols, 1,
                            0, partition_info);
    } else if (pc_tree->partitioning == PARTITION_HORZ) {
      assign_partition_info(row_start_4x4, col_start_4x4, block_width_4x4,
                            block_height_4x4, num_unit_rows, num_unit_cols, 0,
                            1, partition_info);
    }
