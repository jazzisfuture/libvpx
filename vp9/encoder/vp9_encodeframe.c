...
static INLINE void set_modeinfo_offsets(VP9_COMMON *const cm,
  ...
  xd->mi_test = cm->mi + idx_str;
  xd->mi_test[0].target = &xd->mi_test[0];
}
  ...
  for (j = 0; j < block_height; ++j)
    for (i = 0; i < block_width; ++i) {
      if (mi_row + j < cm->mi_rows && mi_col + i < cm->mi_cols)
        xd->mi_test[j * xd->mi_stride + i].target = &xd->mi_test[0];
    }
}
...
static void set_block_size(VP9_COMP * const cpi,
  ...
  if (cpi->common.mi_cols > mi_col && cpi->common.mi_rows > mi_row) {
    ...
    xd->mi_test[0].target->mbmi.sb_type = bsize;
...
static void choose_partitioning(VP9_COMP *cpi,
  ...
  if (cm->frame_type != KEY_FRAME) {
    ...
    xd->mi_test[0].target->mbmi.ref_frame[0] = LAST_FRAME;
    xd->mi_test[0].target->mbmi.sb_type = BLOCK_64X64;
    vp9_find_best_ref_mvs(xd, cm->allow_high_precision_mv,
                          xd->mi_test[0].target->mbmi.ref_mvs[LAST_FRAME],
                          &nearest_mv, &near_mv);

    xd->mi_test[0].target->mbmi.mv[0] = nearest_mv;
...
static void update_state(VP9_COMP *cpi, PICK_MODE_CONTEXT *ctx,
  ...
  MB_MODE_INFO *const mbmi = &xd->mi_test[0].target->mbmi;
  MODE_INFO *mi_addr = &xd->mi_test[0];
  const struct segmentation *const seg = &cm->seg;
  ...
  *mi_addr = *mi;
  ...
  if (seg->enabled && output_enabled) {
    ...
    if (cpi->oxcf.aq_mode == CYCLIC_REFRESH_AQ) {
      vp9_cyclic_refresh_update_segment(cpi, &xd->mi_test[0].target->mbmi,
  ...
  for (y = 0; y < mi_height; y++)
    for (x_idx = 0; x_idx < mi_width; x_idx++)
      if ((xd->mb_to_right_edge >> (3 + MI_SIZE_LOG2)) + mi_width > x_idx
        ...
        xd->mi_test[x_idx + y * mis].target = mi_addr;
      }
...
static void set_mode_info_seg_skip(MACROBLOCK *x, TX_MODE tx_mode, int *rate,
  ...
  if (xd->up_available)
    filter_ref = xd->mi_test[-xd->mi_stride].target->mbmi.interp_filter;
  else if (xd->left_available)
    filter_ref = xd->mi_test[-1].target->mbmi.interp_filter;
  else
    filter_ref = EIGHTTAP;
  ...
  xd->mi_test[0].target->bmi[0].as_mv[0].as_int = 0;
  x->skip = 1;
...
static void set_partial_b64x64_partition(MODE_INFO *mi, int mis,
  ...
  for (r = 0; r < MI_BLOCK_SIZE; r += bh) {
    ...
    for (c = 0; c < MI_BLOCK_SIZE; c += bw) {
      ...
      mi_8x8[index].target = mi + index;
      mi_8x8[index].target->mbmi.sb_type = find_partition_size(bsize,
  ...
  if ((col8x8_remaining >= MI_BLOCK_SIZE) &&
    ...
    for (block_row = 0; block_row < MI_BLOCK_SIZE; block_row += bh) {
      for (block_col = 0; block_col < MI_BLOCK_SIZE; block_col += bw) {
        ...
        mi_8x8[index].target = mi_upper_left + index;
        mi_8x8[index].target->mbmi.sb_type = bsize;
      }
    }
  } else {
...
static void copy_partitioning(VP9_COMMON *cm, MODE_INFO *mi_8x8,
  ...
  for (block_row = 0; block_row < 8; ++block_row) {
    for (block_col = 0; block_col < 8; ++block_col) {
      MODE_INFO *const prev_mi = prev_mi_8x8[block_row * mis + block_col].target;
      const BLOCK_SIZE sb_type = prev_mi ? prev_mi->mbmi.sb_type : 0;

      if (prev_mi) {
        const ptrdiff_t offset = prev_mi - cm->prev_mi;
        mi_8x8[block_row * mis + block_col].target = cm->mi + offset;
        mi_8x8[block_row * mis + block_col].target->mbmi.sb_type = sb_type;
      }
    }
  }
}
  ...
  if ((col8x8_remaining >= MI_BLOCK_SIZE) &&
    ...
    for (block_row = 0; block_row < MI_BLOCK_SIZE; block_row += bh) {
      for (block_col = 0; block_col < MI_BLOCK_SIZE; block_col += bw) {
        ...
        MODE_INFO *prev_mi = prev_mi_8x8[index].target;
        const BLOCK_SIZE sb_type = prev_mi ? prev_mi->mbmi.sb_type : 0;
        ...
        if (prev_mi && sb_type <= bsize) {
          ...
          for (block_row2 = 0; block_row2 < bh; ++block_row2) {
            for (block_col2 = 0; block_col2 < bw; ++block_col2) {
              ...
              prev_mi = prev_mi_8x8[index2].target;
              if (prev_mi) {
                const ptrdiff_t offset = prev_mi - cm->prev_mi;
                mi_8x8[index2].target = cm->mi + offset;
                mi_8x8[index2].target->mbmi.sb_type = prev_mi->mbmi.sb_type;
              }
            }
          }
        } else {
  ...
  if ((col8x8_remaining >= MI_BLOCK_SIZE) &&
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        mi_8x8[index].target = mi_upper_left + index;
        mi_8x8[index].target->mbmi.sb_type = BLOCK_16X16;
      ...
      if (is_larger_better) {
        ...
        mi_8x8[index].target = mi_upper_left + index;
        mi_8x8[index].target->mbmi.sb_type = BLOCK_32X32;
      }
    }
    ...
    if (use32x32 == 4) {
      ...
      // Use 64x64 partition
      if (is_larger_better) {
        mi_8x8[0].target = mi_upper_left;
        mi_8x8[0].target->mbmi.sb_type = BLOCK_64X64;
      }
    }
...
static int sb_has_motion(const VP9_COMMON *cm, MODE_INFO *prev_mi_8x8,
  ...
  if (cm->prev_mi) {
    for (block_row = 0; block_row < 8; ++block_row) {
      for (block_col = 0; block_col < 8; ++block_col) {
        const MODE_INFO *prev_mi = prev_mi_8x8[block_row * mis + block_col].target;
        if (prev_mi) {
...
static void encode_sb_rt(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (bsize >= BLOCK_8X8) {
    const int idx_str = xd->mi_stride * mi_row + mi_col;
    MODE_INFO *mi_8x8 = cm->mi[idx_str].target;
    ctx = partition_plane_context(xd, mi_row, mi_col, bsize);
    subsize = mi_8x8[0].target->mbmi.sb_type;
  } else {
    ctx = 0;
    subsize = BLOCK_4X4;
  }
...
static void rd_use_partition(VP9_COMP *cpi,
  ...
  int splits_below = 0;
  BLOCK_SIZE bs_type = mi_8x8[0].target->mbmi.sb_type;
  ...
  if (do_partition_search &&
    ...
    if (partition == PARTITION_SPLIT && subsize > BLOCK_8X8) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        MODE_INFO *this_mi = mi_8x8[jj * bss * mis + ii * bss].target;
    ...
    if (partition != PARTITION_NONE && !splits_below &&
      ...
      mi_8x8[0].target->mbmi.sb_type = bs_type;
      pc_tree->partitioning = partition;
    }
  }
  ...
  if (last_part_rd < chosen_rd) {
    mi_8x8[0].target->mbmi.sb_type = bsize;
    if (bsize >= BLOCK_8X8)
      pc_tree->partitioning = partition;
...
static void get_sb_partition_size_range(MACROBLOCKD *xd, MODE_INFO *mi_8x8,
  ...
  for (i = 0; i < sb_height_in_blocks; ++i) {
    for (j = 0; j < sb_width_in_blocks; ++j) {
      MODE_INFO *mi = mi_8x8[index+j].target;
...
static void rd_auto_partition_range(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  VP9_COMMON *const cm = &cpi->common;
  MACROBLOCKD *const xd = &cpi->mb.e_mbd;
  MODE_INFO *mi = xd->mi_test[0].target;
  const int left_in_image = xd->left_available && mi[-1].target;
  const int above_in_image = xd->up_available && mi[-xd->mi_stride].target;
  const int row8x8_remaining = tile->mi_row_end - mi_row;
  const int col8x8_remaining = tile->mi_col_end - mi_col;
  int bh, bw;
  ...
  if (left_in_image || above_in_image || cm->frame_type != KEY_FRAME) {
    ...
    if (cm->frame_type != KEY_FRAME) {
      MODE_INFO *const prev_mi =
          cm->prev_mi[mi_row * xd->mi_stride + mi_col].target;
      get_sb_partition_size_range(xd, prev_mi, &min_size, &max_size, bs_hist);
    }
    // Find the min and max partition sizes used in the left SB64
    if (left_in_image) {
      MODE_INFO *left_sb64_mi = mi[-MI_BLOCK_SIZE].target;
      get_sb_partition_size_range(xd, left_sb64_mi, &min_size, &max_size,
                                  bs_hist);
    }
    // Find the min and max partition sizes used in the above SB64.
    if (above_in_image) {
      MODE_INFO *above_sb64_mi = mi[-xd->mi_stride * MI_BLOCK_SIZE].target;
...
static void auto_partition_range(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  VP9_COMMON *const cm = &cpi->common;
  MACROBLOCKD *const xd = &cpi->mb.e_mbd;
  MODE_INFO *mi_8x8 = xd->mi_test;
  const int left_in_image = xd->left_available && mi_8x8[-1].target;
  const int above_in_image = xd->up_available &&
                             mi_8x8[-xd->mi_stride].target;
  ...
  if (search_range_ctrl &&
    ...
    if (left_in_image) {
      ...
      for (block = 0; block < MI_BLOCK_SIZE; ++block) {
        cur_mi = mi[block * xd->mi_stride].target;
    ...
    // Find the min and max partition sizes used in the above SB64.
    if (above_in_image) {
      mi = mi_8x8[-xd->mi_stride * MI_BLOCK_SIZE].target;
      for (block = 0; block < MI_BLOCK_SIZE; ++block) {
        sb_type = mi[block].target ? mi[block].target->mbmi.sb_type : 0;
        min_size = MIN(min_size, sb_type);
        max_size = MAX(max_size, sb_type);
      }
    }
...
static void set_partition_range(VP9_COMMON *cm, MACROBLOCKD *xd,
  ...
  MODE_INFO *mi;
  MODE_INFO *prev_mi =
      &cm->prev_mi[mi_row * cm->mi_stride + mi_col];
  ...
  if (prev_mi) {
    for (idy = 0; idy < mi_height; ++idy) {
      for (idx = 0; idx < mi_width; ++idx) {
        mi = prev_mi[idy * cm->mi_stride + idx].target;
  ...
  if (xd->left_available) {
    for (idy = 0; idy < mi_height; ++idy) {
      mi = xd->mi_test[idy * cm->mi_stride - 1].target;
      bs = mi ? mi->mbmi.sb_type : bsize;
      min_size = MIN(min_size, bs);
      max_size = MAX(max_size, bs);
    }
  ...
  if (xd->up_available) {
    for (idx = 0; idx < mi_width; ++idx) {
      mi = xd->mi_test[idx - cm->mi_stride].target;
      bs = mi ? mi->mbmi.sb_type : bsize;
      min_size = MIN(min_size, bs);
      max_size = MAX(max_size, bs);
    }
...
static void encode_rd_sb_row(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  for (mi_col = tile->mi_col_start; mi_col < tile->mi_col_end;
    ...
    if ((sf->partition_search_type == SEARCH_PARTITION &&
      ...
      MODE_INFO *prev_mi = (cm->prev_mip + cm->mi_stride + 1 + idx_str)->target;
...
static void reset_skip_tx_size(VP9_COMMON *cm, TX_SIZE max_tx_size) {
  ...
  for (mi_row = 0; mi_row < cm->mi_rows; ++mi_row, mi_ptr += mis) {
    for (mi_col = 0; mi_col < cm->mi_cols; ++mi_col) {
      if (mi_ptr[mi_col].target->mbmi.tx_size > max_tx_size)
        mi_ptr[mi_col].target->mbmi.tx_size = max_tx_size;
    }
  }
}
...
static void fill_mode_info_sb(VP9_COMMON *cm, MACROBLOCK *x,
  ...
  switch (partition) {
    case PARTITION_NONE:
      set_modeinfo_offsets(cm, xd, mi_row, mi_col);
      *(xd->mi_test[0].target) = pc_tree->none.mic;
      duplicate_mode_info_in_sb(cm, xd, mi_row, mi_col, bsize);
      break;
    case PARTITION_VERT:
      set_modeinfo_offsets(cm, xd, mi_row, mi_col);
      *(xd->mi_test[0].target) = pc_tree->vertical[0].mic;
      duplicate_mode_info_in_sb(cm, xd, mi_row, mi_col, bsize);

      if (mi_col + hbs < cm->mi_cols) {
        set_modeinfo_offsets(cm, xd, mi_row, mi_col + hbs);
        *(xd->mi_test[0].target) = pc_tree->vertical[1].mic;
        duplicate_mode_info_in_sb(cm, xd, mi_row, mi_col + hbs, bsize);
      }
      break;
    case PARTITION_HORZ:
      set_modeinfo_offsets(cm, xd, mi_row, mi_col);
      *(xd->mi_test[0].target) = pc_tree->horizontal[0].mic;
      duplicate_mode_info_in_sb(cm, xd, mi_row, mi_col, bsize);
      if (mi_row + hbs < cm->mi_rows) {
        set_modeinfo_offsets(cm, xd, mi_row + hbs, mi_col);
        *(xd->mi_test[0].target) = pc_tree->horizontal[1].mic;
...
static void nonrd_pick_partition(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (partition_none_allowed) {
    ...
    ctx->mic.mbmi = xd->mi_test[0].target->mbmi;
  ...
  if (partition_horz_allowed && do_rect) {
    ...
    pc_tree->horizontal[0].mic.mbmi = xd->mi_test[0].target->mbmi;
    ...
    if (sum_rd < best_rd && mi_row + ms < cm->mi_rows) {
      ...
      pc_tree->horizontal[1].mic.mbmi = xd->mi_test[0].target->mbmi;
  ...
  if (partition_vert_allowed && do_rect) {
    ...
    nonrd_pick_sb_modes(cpi, tile, mi_row, mi_col,
                        &this_rate, &this_dist, subsize,
                        &pc_tree->vertical[0]);
    pc_tree->vertical[0].mic.mbmi = xd->mi_test[0].target->mbmi;
    ...
    if (sum_rd < best_rd && mi_col + ms < cm->mi_cols) {
      ...
      nonrd_pick_sb_modes(cpi, tile, mi_row, mi_col + ms,
                          &this_rate, &this_dist, subsize,
                          &pc_tree->vertical[1]);
      pc_tree->vertical[1].mic.mbmi = xd->mi_test[0].target->mbmi;
...
static void nonrd_use_partition(VP9_COMP *cpi,
  ...
  subsize = (bsize >= BLOCK_8X8) ? mi[0].target->mbmi.sb_type : BLOCK_4X4;
  partition = partition_lookup[bsl][subsize];
  ...
  switch (partition) {
    case PARTITION_NONE:
      ...
      pc_tree->none.mic.mbmi = xd->mi_test[0].target->mbmi;
...
static void encode_nonrd_sb_row(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  for (mi_col = tile->mi_col_start; mi_col < tile->mi_col_end;
    ...
    const int idx_str = cm->mi_stride * mi_row + mi_col;
    MODE_INFO *mi = cm->mi[idx_str].target;
    MODE_INFO *prev_mi = (cm->prev_mip + cm->mi_stride + 1 + idx_str)->target;
    BLOCK_SIZE bsize;
    x->in_static_area = 0;
      ...
      for (y = 0; y < mi_height; y++)
        for (x = 0; x < mi_width; x++)
          if (mi_col + x < cm->mi_cols && mi_row + y < cm->mi_rows)
            mi_8x8[mis * y + x].target->mbmi.tx_size = tx_size;
    }
  }
}
