...
static int choose_partitioning(VP9_COMP *cpi,
  ...
  if (!is_key_frame) {
   ...
   //        cm->current_video_frame, mi_col, mi_row, y_sad >> 6, y_sad_g >> 6);
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (!is_key_frame) {
        ...
        if ((vt.split[i].split[j].part_variances.none.variance >
            ...
            ((max16x16 - min16x16) > 40)) {
          // 16X16 variance is above threshold for split, so force split for
          // this 16x16 block (and corresponding splits for the upper levels).
          //printf("FORCE SPLIT on 16X16 FROM 32X32 %d %d \n", min16x16, max16x16);
  ...
  for (i = 0; i < 4; i++) {
    ...
    // Check if variance of this 32x32 block is above the threshold, and if so,
    // force the block to split.
    if (!force_split[i + 1]) {
      get_variance(&vt.split[i].part_variances.none);
      if (vt.split[i].part_variances.none.variance > max32x32)
        max32x32 = vt.split[i].part_variances.none.variance;
      if (!is_key_frame &&
          vt.split[i].part_variances.none.variance > cpi->vbp_threshold) {
        //printf("FORCE SPLIT on 32X32 FROM 32X32 \n");
        force_split[i + 1] = 1;
        force_split[0] = 1;
      }
    }
  ...
  if (!force_split[0]) {
    ...
    get_variance(&vt.part_variances.none);
    if (!is_key_frame &&
        max32x32 > (3 * vt.part_variances.none.variance >> 1)) {
      //printf("FORCE SPLIT on 32X32 FROM 64X64 %d %d \n", max32x32, vt.part_variances.none.variance);
      force_split[0] = 1;
    }
  }
  ...
  //
   for (i = 0; i < 4; i++) {
     for (j = 0; j < 4; j++) {
       get_variance(&vt.split[i].split[j].part_variances.none);
       printf("16X16: %d %d %d %d \n", i, j, vt.split[i].split[j].part_variances.none.variance, cpi->vbp_threshold_16x16);
     }
     get_variance(&vt.split[i].part_variances.none);
     printf("32X32: %d %d %d \n", i, vt.split[i].part_variances.none.variance, cpi->vbp_threshold);
   }
   get_variance(&vt.part_variances.none);
   printf("64X64: %d %d \n", vt.part_variances.none.variance, cpi->vbp_threshold_bsize_max);
   //
  */
