...
void vp9_set_vbp_thresholds(VP9_COMP *cpi, int64_t thresholds[], int q) {
  ...
  if (sf->partition_search_type != VAR_BASED_PARTITION &&
    ...
    cpi->vbp_threshold_minmax = (q > 200) ? 40 : (q < 60) ? 20: 30;
  }
}
      ...
      if (!is_key_frame) {
        ...
        if (vt.split[i].split[j].part_variances.none.variance >
          ...
          if (minmax > cpi->vbp_threshold_minmax) {
            // We have some nominal amount of variance based on average 8x8, and
            // the minmax over the 8x8 sub-blocks is above threshold, so force
            // split to 8x8 block for this 16x16 block.
            force_split[split_index] = 1;
            force_split[i + 1] = 1;
            force_split[0] = 1;
          }
