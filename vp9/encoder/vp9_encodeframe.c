...
int fast_avg(unsigned char *s, int p) {
  __m128i s0,s1,s2,s3,s4,s5,s6,s7,u0;
  long long avg = 0;
  u0  = _mm_setzero_si128();
  s0 = _mm_loadl_epi64((__m128i *)(s));
  s1 = _mm_loadl_epi64((__m128i *)(s+p));
  s2 = _mm_loadl_epi64((__m128i *)(s+2*p));
  s3 = _mm_loadl_epi64((__m128i *)(s+3*p));
  s4 = _mm_loadl_epi64((__m128i *)(s+4*p));
  s5 = _mm_loadl_epi64((__m128i *)(s+5*p));
  s6 = _mm_loadl_epi64((__m128i *)(s+6*p));
  s7 = _mm_loadl_epi64((__m128i *)(s+7*p));
