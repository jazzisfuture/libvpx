...
static void choose_partitioning(VP9_COMP *cpi,
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (!is_key_frame) {
        ...
        // Compute the variance for 16x16 block, and if it is above the 16x16
        // bsize variance threshold, see the force_split.
        if (low_res)
  ...
  for (i = 0; i < 4; i++) {
    ...
    // If variance of 32x32 block is above the threshold, force this block
    // and the upper level to split.
