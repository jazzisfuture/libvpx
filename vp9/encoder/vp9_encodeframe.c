...
static int set_vt_partitioning(VP9_COMP *cpi,
  ...
  if (bsize == bsize_min) {
    // Variance already computed for 32x32/16x16 blocks to set the force_split.
    if (bsize == BLOCK_64X64 || low_res || cm->frame_type == KEY_FRAME)
      ...
      if (!is_key_frame) {
        ...
        if (vt.split[i].split[j].part_variances.none.variance >
          ...
          force_split[0] = 1;
                   ...
                   cpi->vbp_threshold) {
          // 16x16 variance is above variance for 32x32 split, so impose
          // split for 32x32 (and 64x64) parent block.
          force_split[i + 1] = 1;
          force_split[0] = 1;
        }
      }
