...
#if CONFIG_VP9_HIGH
// TODO should these value scale with bit depth?
  ...
  if (x->e_mbd.cur_buf->flags&YV12_FLAG_HIGH) {
    ...
    default:
      act = vp9_high_variance16x16(x->plane[0].src.buf,
                                             ...
                                             CONVERT_TO_BYTEPTR(VP9_HIGH_VAR_OFFS_8), 0, &sse) << 4;
      break;
    case 10:
      act = vp9_high_variance16x16(x->plane[0].src.buf,
                                             x->plane[0].src.stride,
    ...
    case 12:
      act = vp9_high_variance16x16(x->plane[0].src.buf,
                                             x->plane[0].src.stride,
                                             CONVERT_TO_BYTEPTR(VP9_HIGH_VAR_OFFS_12), 0, &sse) >> 4;
      break;
    }
  } else {
...
#if CONFIG_VP9_HIGH
  if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
    x->source_variance = high_get_sby_perpixel_variance(cpi, &x->plane[0].src, bsize, xd->bps);
  } else {
...
#if CONFIG_VP9_HIGH
    if (xd->cur_buf->flags&YV12_FLAG_HIGH) {
      source_variancey = high_get_sby_perpixel_variance(cpi, &x->plane[0].src, bsize, xd->bps);
    } else {
