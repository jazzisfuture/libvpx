...
static void rd_pick_sb_modes(VP9_COMP *cpi,
  ...
  int i, orig_rdmult;
  int num_4x4_sb_type, num_4x4_tx_size;
  ...
  if (ctx->mic.mode < INTRA_MODES) {
    ...
    num_4x4_tx_size = num_4x4_blocks_wide_lookup[ctx->mic.tx_size] *
                      num_4x4_blocks_high_lookup[ctx->mic.tx_size];

    for (i = 0; i < num_4x4_sb_type / num_4x4_tx_size; ++i)
      ctx->mic.sum_eobs += ctx->eobs[0][0][i];
  }
}
    ...
    if (this_rdc.rate != INT_MAX) {
      ...
      if (this_rdc.rdcost < best_rdc.rdcost) {
        ...
        if (VPXMIN(cm->width, cm->height) >= 480 &&
          ...
          if (mi_row != 0) {
            context_size =
                cm->mi_grid_visible[mi_col + cm->mi_stride * (mi_row - hbs)]
                    ->sb_type;
          ...
          if (mi_col != 0) {
            context_size =
                cm->mi_grid_visible[mi_col - hbs + cm->mi_stride * mi_row]
                    ->sb_type;
            if (context_size < bsize)
              left_partitioning = 2;
            else if (context_size == bsize)
          ...
          if (cm->prev_mi_grid_visible[0] != NULL) {
            context_size =
