...
#if CONFIG_SUPERTX
static int check_intra_b(VP9_COMP *cpi, const TileInfo *const tile,
                         int mi_row, int mi_col, BLOCK_SIZE bsize,
                         PICK_MODE_CONTEXT *ctx);
...
static void update_supertx_param_sb(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  switch (partition) {
    case PARTITION_NONE:
      update_supertx_param(cpi, &pc_tree->none, supertx_size);
      break;
    case PARTITION_VERT:
      update_supertx_param(cpi, &pc_tree->vertical[0], supertx_size);
      if (mi_col + hbs < cm->mi_cols && bsize > BLOCK_8X8)
        update_supertx_param(cpi, &pc_tree->vertical[1], supertx_size);
      break;
    case PARTITION_HORZ:
      update_supertx_param(cpi, &pc_tree->horizontal[0], supertx_size);
      if (mi_row + hbs < cm->mi_rows && bsize > BLOCK_8X8)
        update_supertx_param(cpi, &pc_tree->horizontal[1], supertx_size);
      break;
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        update_supertx_param(cpi, pc_tree->leaf_split[0], supertx_size);
      } else {
...
static void rd_use_partition(VP9_COMP *cpi,
  ...
  if (do_partition_search &&
    ...
    if (partition != PARTITION_NONE && !splits_below &&
      ...
      rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &none_rate,
#if CONFIG_SUPERTX
                       &none_rate_nocoef,
#endif
                       &none_dist, bsize,
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      ...
      if (last_part_rate != INT_MAX &&
        ...
        rd_pick_sb_modes(cpi, tile, mi_row + (mi_step >> 1), mi_col, &rt,
#if CONFIG_SUPERTX
                         &rt_nocoef,
#endif
                         &dt,
    ...
    case PARTITION_VERT:
      ...
      if (last_part_rate != INT_MAX &&
        ...
        update_state(cpi, ctx, mi_row, mi_col, subsize, 0);
        encode_superblock(cpi, tp, 0, mi_row, mi_col, subsize, ctx);
        rd_pick_sb_modes(cpi, tile, mi_row, mi_col + (mi_step >> 1), &rt,
#if CONFIG_SUPERTX
                         &rt_nocoef,
#endif
                         &dt,
    ...
    case PARTITION_SPLIT:
      ...
      for (i = 0; i < 4; i++) {
        ...
        rd_use_partition(cpi, tile, mi_8x8 + jj * bss * mis + ii * bss, tp,
                         mi_row + y_idx, mi_col + x_idx, subsize, &rt,
#if CONFIG_SUPERTX
                         &rt_nocoef,
#endif
                         &dt,
  ...
  if (do_partition_search
    ...
    for (i = 0; i < 4; i++) {
      ...
      rd_pick_sb_modes(cpi, tile, mi_row + y_idx, mi_col + x_idx, &rt,
#if CONFIG_SUPERTX
                       &rt_nocoef,
#endif
                       &dt,
...
static void rd_pick_partition(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (partition_none_allowed) {
    rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &this_rate,
#if CONFIG_SUPERTX
                     &this_rate_nocoef,
#endif
                     &this_dist, bsize,
  ...
  if (do_split) {
    ...
    if (bsize == BLOCK_8X8) {
      ...
      if (cm->frame_type != KEY_FRAME &&
        ...
        assert(sum_rate >= sum_rate_nocoef);
        assert(sum_rate_nocoef >= 0);
        sum_rate += vp9_cost_bit(cm->fc.supertxsplit_prob[supertx_size], 0);
        sum_rd = RDCOST(x->rdmult, x->rddiv, sum_rate, sum_dist);
  ...
  if (partition_horz_allowed && do_rect) {
    ...
    if (cm->frame_type != KEY_FRAME &&
      ...
      assert(sum_rate >= sum_rate_nocoef);
      assert(sum_rate_nocoef >= 0);
      sum_rate += vp9_cost_bit(cm->fc.supertx_prob[supertx_size], 0);
      sum_rd = RDCOST(x->rdmult, x->rddiv, sum_rate, sum_dist);
...
static int check_intra_sb(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  switch (partition) {
    case PARTITION_NONE:
      return check_intra_b(cpi, tile, mi_row, mi_col, subsize, &pc_tree->none);
      break;
    case PARTITION_VERT:
      if (check_intra_b(cpi, tile, mi_row, mi_col, subsize, &pc_tree->vertical[0]))
        return 1;
      if (mi_col + hbs < cm->mi_cols && bsize > BLOCK_8X8) {
        if (check_intra_b(cpi, tile, mi_row, mi_col + hbs, subsize, &pc_tree->vertical[1]))
          return 1;
      }
      break;
    case PARTITION_HORZ:
      if(check_intra_b(cpi, tile, mi_row, mi_col, subsize, &pc_tree->horizontal[0]))
        return 1;
      if (mi_row + hbs < cm->mi_rows && bsize > BLOCK_8X8) {
        if(check_intra_b(cpi, tile, mi_row + hbs, mi_col, subsize, &pc_tree->horizontal[1]))
          return 1;
      }
      break;
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        if (check_intra_b(cpi, tile, mi_row, mi_col, subsize, pc_tree->leaf_split[0]))
          return 1;
      } else {
...
static void predict_superblock_sub8x8_extend(VP9_COMP *cpi,
  ...
  vp9_build_inter_predictors_sby_sub8x8_extend(xd, mi_row, mi_col,
                                               ...
                                               top_bsize,
                                               partition);
  vp9_build_inter_predictors_sbuv_sub8x8_extend(xd, mi_row_ori, mi_col_ori,
                                                top_bsize);
}
...
static void predict_b_sub8x8_extend(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  set_offsets_extend(cpi, tile, mi_row, mi_col, mi_row_ori, mi_col_ori,
                     bsize, top_bsize);
  predict_superblock_sub8x8_extend(cpi, output_enabled, mi_row, mi_col,
                                   mi_row_ori, mi_col_ori,
                                   top_bsize, ctx, partition);
...
static void predict_b_extend(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  set_offsets_extend(cpi, tile, mi_row, mi_col, mi_row_ori, mi_col_ori,
                     bsize, top_bsize);
  predict_superblock(cpi, output_enabled, mi_row_ori, mi_col_ori,
                     top_bsize, ctx);
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      if (bsize > BLOCK_8X8) {
        ...
        predict_b_sub8x8_extend(cpi, tile, mi_row, mi_col,
                                ...
                                bsize, top_bsize, &pc_tree->horizontal[0],
                                PARTITION_HORZ);
    ...
    case PARTITION_VERT:
      if (bsize > BLOCK_8X8) {
        ...
        predict_b_sub8x8_extend(cpi, tile, mi_row, mi_col,
                                mi_row_ori, mi_col_ori, output_enabled,
                                bsize, top_bsize, &pc_tree->vertical[0],
                                PARTITION_VERT);
      }
      if (mi_col + hbs < cm->mi_cols && bsize > BLOCK_8X8) {
        for (i = 0; i < MAX_MB_PLANE; i++) {
        ...
        predict_b_extend(cpi, tile, mi_row, mi_col + hbs, mi_row_ori, mi_col_ori,
                         output_enabled, subsize, top_bsize,
                         &pc_tree->vertical[1]);
        for (i = 0; i < MAX_MB_PLANE; i++) {
          xd->plane[i].dst.buf = dst_buf[i];
    ...
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        predict_b_sub8x8_extend(cpi, tile, mi_row, mi_col,
                                mi_row_ori, mi_col_ori, output_enabled,
                                bsize, top_bsize, pc_tree->leaf_split[0],
                                PARTITION_SPLIT);
      } else {
        ...
        for (i = 0; i < MAX_MB_PLANE; i++) {
          if (mi_row < cm->mi_rows && mi_col + hbs < cm->mi_cols) {
            vp9_build_masked_inter_predictor_complex(dst_buf[i], dst_stride[i],
                                                   ...
                                                   PARTITION_VERT);
            if (mi_row + hbs < cm->mi_rows) {
              vp9_build_masked_inter_predictor_complex(dst_buf2[i], dst_stride2[i],
...
static void rd_supertx_sb(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  MACROBLOCK *const x = &cpi->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  int plane, pnskip, this_rate, base_rate = *tmp_rate;
  int64_t pnsse, this_dist;
  uint8_t *dst_buf[3];
  int dst_stride[3];
