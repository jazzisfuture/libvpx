...
static void rd_pick_partition(VP9_COMP *cpi, TOKENEXTRA **tp, int mi_row,
  ...
  const int partition_horz_allowed = ((bsize <= cpi->sf.max_partition_size &&
                                     ...
                                     (!cpi->sf.use_square_partition_only ||
                                       force_horz_split) &&
                                      bsize >= BLOCK_8X8 && !force_vert_split;
  const int partition_vert_allowed = ((bsize <= cpi->sf.max_partition_size &&
                                       bsize >  cpi->sf.min_partition_size) ||
                                       !cpi->sf.auto_min_max_partition_size) &&
                                     (!cpi->sf.use_square_partition_only ||
                                       force_vert_split) &&
                                     bsize >= BLOCK_8X8 && !force_horz_split;
  ...
  // FIXME(jingning): temporarily turn off disable_split_var_thresh during
  // refactoring process. will get this back after finishing the main framework
  // of partition search type.
  if (0 && cpi->sf.disable_split_var_thresh && partition_none_allowed) {
    ...
    if (sum_rd < best_rd && i == 4) {
      ...
      if (sum_rd < best_rd) {
        ...
        *(get_sb_partitioning(x, bsize)) = subsize;
        ...
        larger_is_better = partition_none_allowed;
      }
    }
