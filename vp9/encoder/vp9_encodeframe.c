...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t,
  ...
  if (cm->frame_type != KEY_FRAME && output_enabled) {
    FILE *pf = fopen("coding_modes_1.txt", "a");
    fprintf(pf, "position (%d, %d), sb type %d, mode %d\n",
            mi_row, mi_col, mi->mbmi.sb_type, mi->mbmi.mode);
    fclose(pf);
  }
