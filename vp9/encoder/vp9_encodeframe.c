    ...
    if (bsize == BLOCK_8X8) {
      ...
      for (i = 0; (i < 4) && ((sum_rdc.rdcost < best_rdc.rdcost) || must_split);
        ...
        rd_pick_partition(cpi, td, tile_data, tp, mi_row + y_idx,
                          ...
                          // A must split test here increase hte number of sub partitions
                          // but hurts metrics results quite a bit so this extra test is
                          // commented out pending further tests on whether it adds much
