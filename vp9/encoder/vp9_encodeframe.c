...
static void pick_sb_modes(VP9_COMP *cpi, int mi_row, int mi_col,
                          ...
                          BLOCK_SIZE_TYPE bsize, PICK_MODE_CONTEXT *ctx, int fast_ms) {
  VP9_COMMON * const cm = &cpi->common;
  MACROBLOCK * const x = &cpi->mb;
  MACROBLOCKD * const xd = &x->e_mbd;
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    /*
    if(bsize == 6) {
      printf("(%d,%d: %d,%d), ", x->e_mbd.mode_info_context->mbmi.mode,
             x->e_mbd.mode_info_context->mbmi.ref_frame[0],
             x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.row, x->e_mbd.mode_info_context->mbmi.mv[0].as_mv.col);
      if(x->e_mbd.mode_info_context->mbmi.ref_frame[0]==2)printf("\n");
    }
    */
