...
static void choose_partitioning(VP9_COMP *cpi,
  ...
  if (cm->frame_type != KEY_FRAME) {
    sp_prev = cpi->Last_Source->y_stride;
    sprev = cpi->Last_Source->y_buffer +
        (sp_prev * mi_row << 3) + (mi_col << 3);
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (cm->frame_type != KEY_FRAME) {
        for (k = 0; k < 4; k++) {
          ...
          if (x8_idx < pixels_wide && y8_idx < pixels_high) {
            ...
            sum = s_avg - d_avg;
            sse = sum * sum;
            // If we have some temporal difference in average pixel levels,
            // check the 8x8 pixel-based variance (between last source and
            // last reconstructed) and force split to 8x8 block if sse
            // (coding error on previous frame) is large. Do this only for
            // segment 1 to reduce cycles.
            if (segment_id && fabs(sum) > 0 && !force_split[5 + i2 + j]) {
              // Can we use/pass the block prediction residual from previous
              // encoded frame to here and avoid this computation?
              vp9_get8x8var(sprev + y8_idx * sp_prev + x8_idx, sp_prev,
                            d + y8_idx * dp + x8_idx, dp, &sse2, &sum2);
              if (sse2 > (threshold << 2)) {
                // Force split for this 16x16 block to 8x8, this also imposes
                // splits for the upper levels.
        ...
        if (!force_split[5 + i2 + j]) {
          fill_variance_tree(&vt.split[i].split[j], BLOCK_16X16);
          // For low-resolution, compute variance based on 8x8 down-sampling,
      ...
      if (cm->frame_type == KEY_FRAME || (low_res &&
          ...
          (threshold << 1)) && !force_split[5 + i2 + j])) {
  ...
  for (i = 0; i < 4; i++) {
    if (!force_split[i + 1]) {
      const int i2 = i << 2;
      for (j = 0; j < 4; j++) {
        if (variance4x4downsample[i2 + j] == 1 && !force_split[5 + i2 + j]) {
  ...
  if ( mi_col + 8 > cm->mi_cols || mi_row + 8 > cm->mi_rows ||
      !set_vt_partitioning(cpi, xd, &vt, BLOCK_64X64, mi_row, mi_col,
                           threshold_bsize_max, BLOCK_16X16, force_split[0])) {
    for (i = 0; i < 4; ++i) {
            ...
            for (k = 0; k < 4; ++k) {
              ...
              if (use_4x4_partition) {
                if (!set_vt_partitioning(cpi, xd, &vtemp->split[k],
                                         ...
                                         threshold_bsize_min, BLOCK_8X8, 0)) {
