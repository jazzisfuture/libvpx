...
static int set_vt_partitioning(VP9_COMP *cpi,
  ...
  // For 8x8 blocks sizes on non-key frames: only accept split (to 4x4 blocks)
  // if block has high spatial gradient (spatial_grad = 1). Otherwise if block
  // has high spatial gradient allow for 4x4 based on (temporal) variance below.
  if (bsize == BLOCK_8X8 && cm->frame_type != KEY_FRAME && spatial_grad == 0) {
...
static void choose_partitioning(VP9_COMP *cpi,
      ...
      if (cm->frame_type == KEY_FRAME || (low_res &&
        ...
        for (k = 0; k < 4; k++) {
          ...
          for (m = 0; m < 4; m++) {
            ...
            fill_variance(sse2, sum2, 0, &vspatial.split[m].part_variances.none);
          }
          if (cm->frame_type != KEY_FRAME) {
            fill_variance_tree(&vspatial, BLOCK_8X8);
            get_variance(&vspatial.part_variances.none);
            if (vspatial.part_variances.none.variance > (threshold << 6)) {
              printf("HERE \n");
              spatial_grad[n] = 1;
            }
          }
  ...
  for (i = 0; i < 4; i++) {
    for (j = 0; j < 4; j++) {
      if (variance4x4downsample[i * 4 + j] == 1) {
        v16x16 *vtemp = (cm->frame_type != KEY_FRAME) ? &vt2[i * 4 + j] :
            &vt.split[i].split[j];
        for (m = 0; m < 4; m++) {
  ...
  if ( mi_col + 8 > cm->mi_cols || mi_row + 8 > cm->mi_rows ||
      !set_vt_partitioning(cpi, xd, &vt, BLOCK_64X64, mi_row, mi_col,
                           threshold_bsize_max, BLOCK_16X16, 0)) {
    n = -1;
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, xd, &vt.split[i], BLOCK_32X32,
                               ...
                               threshold, BLOCK_16X16, 0)) {
        for (j = 0; j < 4; ++j) {
          const int x16_idx = ((j & 1) << 1);
          const int y16_idx = ((j >> 1) << 1);
          // TODO(marpan): Allow/check 4x4 partition for inter-frames.
          // use_4x4_partition = (variance4x4downsample[i * 4 + j] == 1);
          ...
          v16x16 vtemp = (cm->frame_type != KEY_FRAME &&
                          variance4x4downsample[i * 4 + j] == 1) ?
                          vt2[i * 4 + j] : vt.split[i].split[j];
          if (!set_vt_partitioning(cpi, xd, &vtemp,
                                   BLOCK_16X16,
                                   mi_row + y32_idx + y16_idx,
                                   mi_col + x32_idx + x16_idx,
                                   threshold_16x16, bsize_min, 0)) {
            for (k = 0; k < 4; ++k) {
