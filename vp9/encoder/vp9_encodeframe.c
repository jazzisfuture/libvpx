  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (cm->frame_type != KEY_FRAME) {
        ...
        // down to 4x4.
        // Set variance to default zero here, as its used in comparison below.
      ...
      if (cm->frame_type == KEY_FRAME ||
         vt.split[i].split[j].part_variances.none.variance > (threshold << 1)) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, xd, &vt.split[i], BLOCK_32X32,
        ...
        for (j = 0; j < 4; ++j) {
          ...
          //    variance4x4downsample[i * 4 + j] == 1) ? 1 : 0;
          // For now, use 4x4 partition for key frame only.
          int use_4x4_partition = (cm->frame_type == KEY_FRAME);
          // If 4x4 partition is not used, then 8x8 partition will be selected
          // if variance of 16x16 block is very high (> threshold_bsize_min).
