...
static void encode_b(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (output_enabled && !is_inter_block(&cpi->mb.e_mbd.mi->mbmi) &&
    ...
    if (cpi->mb.e_mbd.bdintra_pass == 0) {
      return;
    } else if (cpi->mb.e_mbd.allow_bdintra) {
      MB_MODE_INFO *mbmi = &cpi->mb.e_mbd.mi[0].src_mi->mbmi;

      if (mbmi->sb_type >= BLOCK_8X8)
        // Repick intra modes based on possible additional
        // ref data (right column or below row).
        repick_bdintra_mode(cpi, bsize, ctx, mi_row, mi_col);
    }
  }

  if (!frame_is_intra_only(&cpi->common) && cpi->mb.e_mbd.bdintra_pass == 0)
    // Do not generate tokens in the 0-th round.
    output_enabled = 0;
...
static void rd_pick_partition(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  *rate_nocoef = best_rate_nocoef;
  ...
  if (best_rdc.rate < INT_MAX && best_rdc.dist < INT64_MAX &&
    ...
    // Enable bdintra only if quantization is fine enough (so the additional
    // ref data is more likely to be useful).
    cpi->mb.e_mbd.allow_bdintra = cm->base_qindex <= BDINTRA_Q_THRESH;

    // The purpose of the 0-th round is to generate reconstructed pixels of
    // inter predicted blocks.
    cpi->mb.e_mbd.bdintra_pass = 0;
    encode_sb(cpi, tile, tp, mi_row, mi_col, output_enabled, bsize, pc_tree);

    if (!frame_is_intra_only(&cpi->common) && output_enabled) {
      restore_context(cpi, mi_row, mi_col, a, l, sa, sl, bsize);
      if ((cpi->oxcf.aq_mode == COMPLEXITY_AQ) && cm->seg.update_map)
      ...
      // The final round generates tokens for packing bitstream.
      cpi->mb.e_mbd.bdintra_pass = 1;
...
#if CONFIG_BDINTRA
    xd->sb_start_mi_row = mi_row;
    xd->sb_start_mi_col = mi_col;
#endif  // CONFIG_BDINTRA
    ...
    if (xd->allow_bdintra && xd->bdintra_pass == 1) {
      if (bsize >= BLOCK_8X8) {
        ...
        if (mi_row + bh < cm->mi_rows
            && mi_row - xd->sb_start_mi_row + bh < MI_BLOCK_SIZE
            && mi_col + bw < cm->mi_cols) {
          int i = 0;
          MODE_INFO *bottom_mi = xd->mi + bh * cm->mi_stride;
          while (i < bw * 8) {
            // If below block is inter, then below_pixels_available is 1.
        ...
        if (mi_row + bh < cm->mi_rows
            && mi_col + bw < cm->mi_cols
            && mi_col - xd->sb_start_mi_col + bw < MI_BLOCK_SIZE) {
          int i = 0;
          MODE_INFO *right_mi = xd->mi + bw;
          while (i < bh * 8) {
            // If right block is inter, then right_pixels_available is 1.
...
#if CONFIG_BDINTRA
    // Indicate if the left/above block is intra predicted.
    x->above_is_intra = 0;
    x->left_is_intra = 0;
    ...
    if (mi_row > 0) {
      ...
      while (cols > 0 && mi_col + mi_count < cm->mi_cols) {
        ...
        //if (above_mi->src_mi->mbmi.mode == H_PRED
          //  || above_mi->src_mi->mbmi.mode == V_PRED)
          x->above_is_intra = 1;
        cols -= 8;
        above_mi += 1;
        mi_count += 1;
