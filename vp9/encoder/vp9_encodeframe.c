...
static void choose_partitioning(VP9_COMP *cpi,
  ...
  int allow_4x4_partition = (cm->frame_type == KEY_FRAME) ? 1 : 0;
  BLOCK_SIZE bsize_ref = (allow_4x4_partition) ? BLOCK_8X8 : BLOCK_16X16;
  ...
  int64_t threshold_bsize_ref = threshold_base << 6;
  // Choose lower thresholds for key frame and for low-resolutions.
  if (cm->frame_type == KEY_FRAME) {
    threshold = threshold_base >> 2;
    threshold_bsize_ref = threshold_base << 2;
  } else if (low_res) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      for (k = 0; k < 4; k++) {
        ...
        if (cm->frame_type != KEY_FRAME && !low_res) {
          ...
          // For key frame, or low-resolutions, go down to 4x4.
  ...
  if ( mi_col + 8 > cm->mi_cols || mi_row + 8 > cm->mi_rows || low_res ||
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, xd, &vt.split[i], BLOCK_32X32,
        ...
        for (j = 0; j < 4; ++j) {
          ...
          // If 8x8 downsampling is used for variance calculation, 8x8 partition
          // is set if variance of the 16x16 block is very high.
          // If 4x4 downsampling is used for variance calculation
          // (e.g., key frames and low-resolutions), 4x4 partition will be set
          // if variance of 8x8 block is very high.
          int64_t threshold_16x16 = (allow_4x4_partition) ?
              threshold : threshold_bsize_ref;
          if (!set_vt_partitioning(cpi, xd, &vt.split[i].split[j],
                                   BLOCK_16X16,
                                   mi_row + y32_idx + y16_idx,
                                   mi_col + x32_idx + x16_idx,
                                   threshold_16x16, bsize_ref)) {
            for (k = 0; k < 4; ++k) {
              const int x8_idx = (k & 1);
              const int y8_idx = (k >> 1);
              if (allow_4x4_partition) {
                if (!set_vt_partitioning(cpi, xd,
                                         &vt.split[i].split[j].split[k],
                                         BLOCK_8X8,
                                         mi_row + y32_idx + y16_idx + y8_idx,
                                         mi_col + x32_idx + x16_idx + x8_idx,
                                         threshold_bsize_ref, bsize_ref)) {
