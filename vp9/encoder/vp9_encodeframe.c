...
static void set_offsets(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  // cannot be used.
  xd->last_mi = cm->prev_mi ? xd->prev_mi_8x8->mi : NULL;

#if 1 //ndef MIC_STREAM
  xd->this_mi =
  xd->mi_8x8->mi = cm->mi + idx_str;
#else
  xd->this_mi =
  xd->mi_8x8->mi = xd->mic_stream_ptr;
  xd->mic_stream_ptr++;
#endif
...
static void set_partitioning(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
                             BLOCK_SIZE_TYPE bsize) {
  ...
  for (block_row = 0; block_row < 8; ++block_row) {
    for (block_col = 0; block_col < 8; ++block_col) {
      mi_8x8[block_row * mis + block_col].mi->mbmi.sb_type = bsize;
    }
  }
}
static void copy_partitioning(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
  ...
  for (block_row = 0; block_row < 8; ++block_row) {
    for (block_col = 0; block_col < 8; ++block_col) {
      MODE_INFO * prev_mi = prev_mi_8x8[block_row * mis + block_col].mi;
      MODE_INFO * mi = mi_8x8[block_row * mis + block_col].mi;
      BLOCK_SIZE_TYPE sb_type = prev_mi ? prev_mi->mbmi.sb_type : 0;
      if(mi)
...
static void set_block_size(VP9_COMMON * const cm, MODE_INFO_8x8 *mi_8x8,
  ...
  MODE_INFO_8x8 *m2 = mi_8x8 + mi_row * mis + mi_col;

  for (row = 0; row < bs; row++) {
    for (col = 0; col < bs; col++) {
      if (mi_row + row >= cm->mi_rows || mi_col + col >= cm->mi_cols)
        continue;
      m2[row * mis + col].mi->mbmi.sb_type = bsize;
    }
  }
}
...
static int set_vt_partitioning(VP9_COMP *cpi, void *data, MODE_INFO_8x8 *m,
                               BLOCK_SIZE_TYPE block_size, int mi_row,
                               int mi_col, int mi_size) {
  VP9_COMMON * const cm = &cpi->common;
  vt_node vt;
...
static void choose_partitioning(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
...
static void rd_use_partition(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
                             TOKENEXTRA **tp, int mi_row, int mi_col,
                             BLOCK_SIZE_TYPE bsize, int *rate, int64_t *dist,
                             int do_recon) {
  VP9_COMMON * const cm = &cpi->common;
  MACROBLOCK * const x = &cpi->mb;
  MACROBLOCKD *xd = &cpi->mb.e_mbd;
  ...
  int splits_below = 0;
  BLOCK_SIZE_TYPE bs_type = mi_8x8->mi->mbmi.sb_type;
  ...
  if (cpi->sf.adjust_partitioning_from_last_frame) {
    ...
    if (partition == PARTITION_SPLIT && subsize > BLOCK_8X8) {
      ...
      for (i = 0; i < 4; i++) {
        ...
        MODE_INFO * this_mi = mi_8x8[jj * bss * mis + ii * bss].mi;
    ...
    if (partition != PARTITION_NONE && !splits_below &&
      ...
      *(get_sb_partitioning(x, bsize)) = bsize;
      ...
      mi_8x8->mi->mbmi.sb_type = bs_type;
      *(get_sb_partitioning(x, bsize)) = subsize;
    }
  }
...
static void get_sb_partition_size_range(VP9_COMP *cpi, MODE_INFO_8x8 * mi_8x8,
  ...
  for (i = 0; i < sb_height_in_blocks; ++i) {
    for (j = 0; j < sb_width_in_blocks; ++j) {
      MODE_INFO * mi = mi_8x8[index+j].mi;
      BLOCK_SIZE_TYPE sb_type = mi ? mi->mbmi.sb_type : 0;
...
static void rd_auto_partition_range(VP9_COMP *cpi,
  ...
  const int left_in_image = xd->left_available && mi_8x8[-1].mi;
  const int above_in_image = xd->up_available && mi_8x8[-xd->mode_info_stride].mi;
  MODE_INFO_8x8 * above_sb64_mi_8x8;
  MODE_INFO_8x8 * left_sb64_mi_8x8;
...
static void encode_sb_row(VP9_COMP *cpi, int mi_row, TOKENEXTRA **tp,
  ...
  for (mi_col = cm->cur_tile_mi_col_start; mi_col < cm->cur_tile_mi_col_end;
    ...
    if (cpi->sf.partition_by_variance || cpi->sf.use_lastframe_partitioning ||
      ...
      MODE_INFO_8x8 *mi_8x8 = cm->mi_grid_visible + idx_str;
      MODE_INFO_8x8 *prev_mi_8x8 = cm->prev_mi_grid_visible + idx_str;

      cpi->mb.source_variance = UINT_MAX;
      if (cpi->sf.use_one_partition_size_always) {
        set_offsets(cpi, mi_row, mi_col, BLOCK_64X64);
        set_partitioning(cpi, mi_8x8, cpi->sf.always_this_block_size);
...
static int get_skip_flag(MODE_INFO_8x8 *mi_8x8, int mis, int ymbs, int xmbs) {
  int x, y;

  for (y = 0; y < ymbs; y++) {
    for (x = 0; x < xmbs; x++) {
      if (!mi_8x8[y * mis + x].mi->mbmi.skip_coeff)
        return 0;
    }
  }
...
static void set_txfm_flag(MODE_INFO_8x8 *mi_8x8, int mis, int ymbs, int xmbs,
                          TX_SIZE txfm_size) {
  int x, y;

  for (y = 0; y < ymbs; y++) {
    for (x = 0; x < xmbs; x++)
      mi_8x8[y * mis + x].mi->mbmi.txfm_size = txfm_size;
  }
}

static void reset_skip_txfm_size_b(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8, int mis,
                                   TX_SIZE txfm_max, int bw, int bh, int mi_row,
                                   int mi_col, BLOCK_SIZE_TYPE bsize) {
  VP9_COMMON * const cm = &cpi->common;
  MB_MODE_INFO * const mbmi = &mi_8x8->mi->mbmi;
  ...
  if (mbmi->txfm_size > txfm_max) {
    ...
    set_txfm_flag(mi_8x8, mis, ymbs, xmbs, txfm_max);
  }
}

static void reset_skip_txfm_size_sb(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
  ...
  bwl = mi_width_log2(mi_8x8->mi->mbmi.sb_type);
  bhl = mi_height_log2(mi_8x8->mi->mbmi.sb_type);

  if (bwl == bsl && bhl == bsl) {
    reset_skip_txfm_size_b(cpi, mi_8x8, mis, txfm_max, 1 << bsl, 1 << bsl, mi_row,
                           mi_col, bsize);
  } else if (bwl == bsl && bhl < bsl) {
    reset_skip_txfm_size_b(cpi, mi_8x8, mis, txfm_max, 1 << bsl, bs, mi_row, mi_col,
                           bsize);
    reset_skip_txfm_size_b(cpi, mi_8x8 + bs * mis, mis, txfm_max, 1 << bsl, bs,
                           mi_row + bs, mi_col, bsize);
  } else if (bwl < bsl && bhl == bsl) {
    reset_skip_txfm_size_b(cpi, mi_8x8, mis, txfm_max, bs, 1 << bsl, mi_row, mi_col,
                           bsize);
    reset_skip_txfm_size_b(cpi, mi_8x8 + bs, mis, txfm_max, bs, 1 << bsl, mi_row,
                           mi_col + bs, bsize);
  } else {
    BLOCK_SIZE_TYPE subsize;
    int n;
    ...
    for (n = 0; n < 4; n++) {
      ...
      reset_skip_txfm_size_sb(cpi, mi_8x8 + y_idx * bs * mis + x_idx * bs,
...
//  MODE_INFO *mi, *mi_ptr = cm->mi;
  MODE_INFO_8x8 *mi_8x8, *mi_ptr = cm->mi_grid_visible;
...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t, int output_enabled,
  ...
  if (!is_inter_block(mbmi)) {
    ...
    int mb_skip_context = xd->left_available ? mi_8x8[-1].mi->mbmi.skip_coeff : 0;
    mb_skip_context += mi_8x8[-mis].mi ? mi_8x8[-mis].mi->mbmi.skip_coeff : 0;
