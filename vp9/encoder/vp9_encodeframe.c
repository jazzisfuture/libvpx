...
static void fill_16x16_variance(const unsigned char *s, int sp,
  ...
  unsigned int sse;
  int sum;
  vp9_get_sse_sum(s, sp, d, dp, &sse, &sum);
  fill_variance(&vt->split[0].none, sse, sum, 64);
  vp9_get_sse_sum(s + 8, sp, d + 8, dp, &sse, &sum);
  fill_variance(&vt->split[1].none, sse, sum, 64);
  vp9_get_sse_sum(s + 8 * sp, sp, d + 8 * dp, dp, &sse, &sum);
  fill_variance(&vt->split[2].none, sse, sum, 64);
  vp9_get_sse_sum(s + 8 * sp + 8, sp, d + 8 + 8 * dp, dp, &sse, &sum);
...
static void choose_partitioning(VP9_COMP *cpi, MODE_INFO *m, int mi_row,
  ...
  const int mis = cm->mode_info_stride;
  int64_t threshold = cpi->oxcf.encode_breakout;
  int i, j, k;
  v64x64 vt;
  unsigned char * s;
  int sp;
  ...
  // TODO(JBB): uncomment following when border conditions for 64x64 blocks
  // is fixed note the code as is now will fail on any image not a multiple of
  // 32x32.
  // if ( cm->frame_type == KEY_FRAME ) {
  d = vp9_64x64_zeros;
  dp = 64;
  ...
  for (i = 0; i < 4; ++i) {
    ...
    /*  printf("\n");
     printf("%d,%d  %x \n",mi_row,mi_col,m);
     for(i=0;i<8;i++) {
     for(j=0;j<8;j++) printf("%2d ",m[i*mis+j].mbmi.sb_type );
     printf("\n");
     }
     printf("\n");
     */
