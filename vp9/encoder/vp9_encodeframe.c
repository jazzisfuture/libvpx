  ...
  for (mi_col = cm->cur_tile_mi_col_start;
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        int r, d, r2, d2, mb16_rate = 0, mb16_dist = 0, k;
        ...
        for (k = 0; k < 4; k++) {
          ...
          pick_sb_modes(cpi, mi_row + y_idx_m + (k & 1),
                        ...
                        &x->sb8_context[xd->sb_index][xd->mb_index][xd->b_index]);
          mb16_rate += r;
          mb16_dist += d;
          update_state(cpi, &x->sb8_context[xd->sb_index][xd->mb_index][xd->b_index],
                       BLOCK_SIZE_SB8X8, 0);
        ...
        pick_sb_modes(cpi, mi_row + y_idx_m, mi_col + x_idx_m,
                      ...
                      &x->sb8x16_context[xd->sb_index][xd->mb_index][xd->b_index]);
        r2 += r;
        d2 += d;
        update_state(cpi, &x->sb8_context[xd->sb_index][xd->mb_index][xd->b_index],
                     BLOCK_SIZE_SB8X16, 0);
        ...
        pick_sb_modes(cpi, mi_row + y_idx_m, mi_col + x_idx_m,
                      ...
                      &x->sb16x8_context[xd->sb_index][xd->mb_index][xd->b_index]);
        r2 += r;
        d2 += d;
        update_state(cpi, &x->sb16x8_context[xd->sb_index][xd->mb_index][xd->b_index],
                     BLOCK_SIZE_SB16X8, 0);
        encode_superblock(cpi, tp,
                          0, mi_row + y_idx_m, mi_col + x_idx_m,
                          BLOCK_SIZE_SB16X8);
...
#endif
  if (xd->mode_info_context->mbmi.ref_frame == INTRA_FRAME) {
    // FIXME(rbultje) support I4X4_PRED and in-step reconstruction here
    ...
    // FIXME make sure this works for splitmv also
