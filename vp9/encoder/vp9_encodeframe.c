...
static void set_vbp_thresholds(VP9_COMP *cpi, int64_t thresholds[], int q,
  ...
  if (is_key_frame) {
    ...
    if (cpi->oxcf.speed >= 8 && cm->width <= 640 && cm->height <= 480)
      threshold_base = (5 * threshold_base) >> 2;
    else if (cpi->oxcf.speed == 7 && cpi->sf.use_source_sad &&
        ((content_state == kLowSadLowSumdiff) || (content_state == kHighSadLowSumdiff)))
...
static int choose_partitioning(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (cpi->sf.use_source_sad && !is_key_frame) {
    ...
    x->skip_low_source_sad = (content_state < 2) ? 1 : 0;
