...
static int choose_partitioning(VP9_COMP *cpi,
  ...
  if (!is_key_frame && !(is_one_pass_cbr_svc(cpi) && cpi->svc.layer_context[cpi->svc.temporal_layer_id].is_key_frame)) {
    // in the case of spatial/temporal scalable coding,
    // the assumption here is that the temporal reference frame will always be of type LAST_FRAME.
    // TODO: if that assumption is broken, we need to revisit this code.
    ...
    if (!(is_one_pass_cbr_svc(cpi) && cpi->svc.spatial_layer_id)) {
      // for now, GOLDEN will not be used for non-zero spatial layers
