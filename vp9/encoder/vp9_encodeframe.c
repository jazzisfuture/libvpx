...
static const double train_mean[24] = {
    ...
    162.027012,    1.478213,       0.0,           135382.260230, 912738.513263,
    28.845217,     1.515230,       544.158492,    131.807995,    1.436863,
    0.0,           43682.377587,   208131.711766, 28.084737,     1.356677,
...
static const double train_stdm[24] = {
    ...
    0.014638,      2.001898,       0.0,           208798.775332, 1812548.443284,
    0.018693,      1.838009,       396.986910,    0.015657,      1.332541,
    0.0,           55888.847031,   448587.962714, 0.017900,      1.904776,
  ...
  if (partition_none_allowed) {
    ...
    if (this_rdc.rate != INT_MAX) {
      ...
      if (this_rdc.rdcost < best_rdc.rdcost) {
        ...
        if (VPXMIN(cm->width, cm->height) < 480 && !x->e_mbd.lossless && ctx->skippable &&
        ...
        if (VPXMIN(cm->width, cm->height) >= 480 &&
          ...
          const double* mean;
          const double* sd;
          const int mag_mv = abs(ctx->mic.mv[0].as_mv.col) +
              abs(ctx->mic.mv[0].as_mv.row);

          ctx->sum_eobs = 0;
          vp9_foreach_transformed_block_in_plane(xd, bsize, 0, accumulate_eobs,
                                                 ctx);
  ...
  if (do_split) {
    ...
    if (sum_rdc.rdcost < best_rdc.rdcost && i == 4) {
      ...
      if (sum_rdc.rdcost < best_rdc.rdcost) {
        ...
        if (VPXMIN(cm->width, cm->height) < 480 && !x->e_mbd.lossless && ((best_rdc.dist < (dist_breakout_thr >> 2)) ||
                                   (best_rdc.dist < dist_breakout_thr &&
                                    best_rdc.rate < rate_breakout_thr))) {
          do_rect = 0;
        }
