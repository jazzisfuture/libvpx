...
static int set_vt_partitioning(VP9_COMP *cpi, void *data, MODE_INFO *m,
  ...
  // vertical split is available on all but the bottom border
  if (mi_row + mi_size < cm->mi_rows && vt.vt->vert[0].plane[0].variance < threshold
  ...
  // horizontal split is available on all but the right border
  if (mi_col + mi_size < cm->mi_cols && vt.vt->horz[0].plane[0].variance < threshold
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (x16_idx + 16 <= pixels_wide && y16_idx + 16 <= pixels_high) {
        ...
        vp9_set_segment_id(cm, cpi->segmentation_map, BLOCK_16X16, row, col, segment_id);
#if 0
          {
              int h;
              static int hist[7] = {0};
