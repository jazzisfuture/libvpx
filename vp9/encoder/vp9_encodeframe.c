...
void vp9_set_vbp_thresholds(VP9_COMP *cpi, int q) {
  ...
  if (sf->partition_search_type != VAR_BASED_PARTITION &&
    ...
    cpi->vbp_threshold_minmax = (q > 200) ? 50: 30;
  }
}
    ...
    for (j = 0; j < 4; j++) {
      ...
      if (!is_key_frame) {
        ...
        if (vt.split[i].split[j].part_variances.none.variance >
          ...
          // for this 16x16 block (this also forces splits for the upper levels).
          ...
          if (minmax > cpi->vbp_threshold_minmax) {
            // minmax over the 8x8 sub-blocks is above threshold, so force split
            // to 8x8 block for this 16x16 block.
            force_split[split_index] = 1;
            force_split[i + 1] = 1;
            force_split[0] = 1;
          }
