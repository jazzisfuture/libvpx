        ...
        for (k = 0; k < 4; k++) {
          ...
          int y8_idx = y16_idx + ((k >> 1) << 3);
            ...
            if (x8_idx < pixels_wide && y8_idx < pixels_high) {
              ...
              // last reconstructed) and force split to 8x8 block if this
              // variance (coding error on previous frame) is large. Do this
              // only for segment 1 to reduce cycles.
              if (fabs(sum) > 0 && !force_split[5 + i2 + j] && segment_id) {
                ...
                if ((sse2 - ((unsigned int)sum2 * sum2 >> 6))
                    > (threshold << 2)) {
