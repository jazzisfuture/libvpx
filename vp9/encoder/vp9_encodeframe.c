...
static void dump_pc_tree_data(const PC_TREE *const pc_tree, FILE *fp) {
  ...
  if (pc_tree->block_size > BLOCK_8X8 && partitioning == PARTITION_SPLIT) {
    int i;
    for (i = 0; i < 4; ++i) {
  ...
  if (best_rdc.rate < INT_MAX && best_rdc.dist < INT64_MAX &&
    ...
    if (!frame_is_intra_only(cm) && bsize == BLOCK_64X64 &&
      ...
      do {
        ...
        // RD selected partition decisions as label.
        fprintf(fp, "%d,", 123456);
        ...
        if (!frame_is_intra_only(cm)) {
          ...
          if (!frame_is_intra_only(cm) && bsize == BLOCK_64X64 &&
            ...
            do {
              ...
              // RD selected partition decisions as label.
              fprintf(fp, "%d,", 123456);
              dump_pc_tree_data(td->pc_root, fp);
