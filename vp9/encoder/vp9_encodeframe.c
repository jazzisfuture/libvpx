...
static int set_vt_partitioning(VP9_COMP *cpi,
  ...
  if (bsize == bsize_min) {
    ...
    if (mi_col + block_width / 2 < cm->mi_cols &&
        mi_row + block_height / 2 < cm->mi_rows &&
        (vt.part_variances->none.variance < threshold || force_split == -1)) {
      set_block_size(cpi, xd, mi_row, mi_col, bsize);
      return 1;
    }
...
void vp9_set_vbp_thresholds(VP9_COMP *cpi, int q) {
  ...
  if (sf->partition_search_type != VAR_BASED_PARTITION &&
    ...
    cpi->vbp_threshold_minmax = (q > 200) ? 50: (q < 60) ? 20: 30;
  }
}
...
static int choose_partitioning(VP9_COMP *cpi,
      ...
      if (!is_key_frame) {
        ...
        if ((vt.split[i].split[j].part_variances.none.variance >=
            ...
            ((vt.split[i].split[j].part_variances.none.variance >
              cpi->vbp_threshold) &&
              (minmax_max - minmax_min > cpi->vbp_threshold_minmax))) {
          // 16X16 variance is above threshold for split, or the minmax over the
          // the 16x16 block is very large, so force split to 8x8 for this
          // 16x16 block (this also forces splits for the upper levels).
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (cm->frame_type != KEY_FRAME && !force_split[i + 1]) {
  ...
  if (!force_split[0]) {
    ...
    if (cm->frame_type != KEY_FRAME &&
      ...
      // Force split of 64x64 block if variance of any of the 32x32 subblocks
      // variances are much larger than 64x64 variance.
      for (k = 0; k < 4; k++) {
        if (vt.split[k].part_variances.none.variance >
           (vt.part_variances.none.variance << 1)) {
          force_split[0] = 1;
          k = 4;
        }
      }
