...
static void restore_context(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  for (p = 0; p < MAX_MB_PLANE; p++) {
    vpx_memcpy(
        ...
        a + b4x4s_wide * p,
        sizeof(ENTROPY_CONTEXT) * b4x4s_wide >> xd->plane[p].subsampling_x);
    vpx_memcpy(
        cm->left_context[p]
            + ((mi_row & MI_MASK) * 2 >> xd->plane[p].subsampling_y),
        l + b4x4s_high * p,
        sizeof(ENTROPY_CONTEXT) * b4x4s_high >> xd->plane[p].subsampling_y);
  }
...
static void encode_sb(VP9_COMP *cpi, TOKENEXTRA **tp, int mi_row, int mi_col,
  ...
  partition = block_type_to_partition[bsize][c1];
...
static void rd_use_partition(VP9_COMP *cpi, MODE_INFO *m, TOKENEXTRA **tp,
  ...
  partition = block_type_to_partition[bsize][bs_type];
  ...
  if (cpi->sf.adjust_partitioning_from_last_frame
    ...
    // Split partition.
    for (i = 0; i < 4; i++) {
      int x_idx = (i & 1) * (b4x4s_wide >> 2);
      int y_idx = (i >> 1) * (b4x4s_wide >> 2);
      int rt = 0;
      int64_t dt = 0;
      ENTROPY_CONTEXT l[16 * MAX_MB_PLANE], a[16 * MAX_MB_PLANE];
      PARTITION_CONTEXT sl[8], sa[8];
...
static void sum_intra_stats(VP9_COMP *cpi, MACROBLOCK *x) {
  ...
  if (xd->mode_info_context->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    int idx, idy;
    int b4x4s_wide = block_type_to_b4x4s_wide[
      xd->mode_info_context->mbmi.sb_type];
    int b4x4s_high = block_type_to_b4x4s_high[
      xd->mode_info_context->mbmi.sb_type];
