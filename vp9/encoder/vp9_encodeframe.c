...
static void rd_pick_sb_modes(VP9_COMP *cpi, TileDataEnc *tile_data,
  ...
  if (rd_cost->rate == INT_MAX) {
    ...
    rd_cost->rdcost = RDCOST(x->rdmult, x->rddiv,
                             rd_cost->rate + partition_rate, rd_cost->dist);
  }
  ...
  switch (partition) {
    ...
    case PARTITION_HORZ:
      rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &last_part_rdc,
                       subsize, &pc_tree->horizontal[0], INT64_MAX, 0);
    ...
    case PARTITION_VERT:
      rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &last_part_rdc,
                       subsize, &pc_tree->vertical[0], INT64_MAX, 0);
      if (last_part_rdc.rate != INT_MAX && bsize >= BLOCK_8X8 &&
          mi_col + (mi_step >> 1) < cm->mi_cols) {
        RD_COST tmp_rdc;
    ...
    default:
      ...
      if (bsize == BLOCK_8X8) {
        rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &last_part_rdc,
                         subsize, pc_tree->leaf_split[0], INT64_MAX, 0);
        break;
      }
...
static void rd_pick_partition(VP9_COMP *cpi, ThreadData *td,
                              ...
                              PC_TREE *pc_tree, int partition_rate) {
  VP9_COMMON *const cm = &cpi->common;
  TileInfo *const tile_info = &tile_data->tile_info;
  MACROBLOCK *const x = &td->mb;
  MACROBLOCKD *const xd = &x->e_mbd;
  ...
  if (partition_none_allowed) {
    prate = partition_rate + cpi->partition_cost[pl][PARTITION_NONE];
    rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &this_rdc, bsize, ctx,
                     best_rdc.rdcost, prate);
  ...
  if (do_split) {
    ...
    if (bsize == BLOCK_8X8) {
      i = 4;
      prate = partition_rate + cpi->partition_cost[pl][PARTITION_SPLIT];
      if (cpi->sf.adaptive_pred_interp_filter && partition_none_allowed)
        pc_tree->leaf_split[0]->pred_interp_filter = pred_interp_filter;
      rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &sum_rdc, subsize,
                       pc_tree->leaf_split[0], best_rdc.rdcost, prate);
  ...
  if (partition_horz_allowed &&
    ...
    if (mi_row + mi_step < cm->mi_rows && bsize > BLOCK_8X8)
      sub_partition_count = 2;
    prate = (partition_rate + cpi->partition_cost[pl][PARTITION_HORZ]) /
            sub_partition_count;
    ...
    rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &sum_rdc, subsize,
                     &pc_tree->horizontal[0], best_rdc.rdcost, prate);
    ...
    if (sum_rdc.rdcost < best_rdc.rdcost && mi_row + mi_step < cm->mi_rows &&
      ...
      rd_pick_sb_modes(cpi, tile_data, x, mi_row + mi_step, mi_col, &this_rdc,
                       subsize, &pc_tree->horizontal[1],
                       best_rdc.rdcost - sum_rdc.rdcost, prate);
      if (this_rdc.rate == INT_MAX) {
        sum_rdc.rdcost = INT64_MAX;
      } else {
        sum_rdc.rate += this_rdc.rate;
  ...
  if (partition_vert_allowed &&
    ...
    sub_partition_count = 1;
    if (mi_col + mi_step < cm->mi_cols && bsize > BLOCK_8X8)
      sub_partition_count = 2;
    prate = (partition_rate + cpi->partition_cost[pl][PARTITION_VERT]) /
            sub_partition_count;
    ...
    rd_pick_sb_modes(cpi, tile_data, x, mi_row, mi_col, &sum_rdc, subsize,
                     &pc_tree->vertical[0], best_rdc.rdcost, prate);
  ...
  for (mi_col = mi_col_start, sb_col_in_tile = 0; mi_col < mi_col_end;
    ...
    if (sf->partition_search_type == FIXED_PARTITION || seg_skip) {
      ...
      rd_pick_partition(cpi, td, tile_data, tp, mi_row, mi_col, BLOCK_64X64,
                        &dummy_rdc, INT64_MAX, td->pc_root, 0);
    }
