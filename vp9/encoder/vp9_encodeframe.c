...
static void set_block_size(VP9_COMMON * const cm, MODE_INFO **mi_8x8,
  ...
  duplicate_modeinfo_in_sb(cm, mi2, mi_row, mi_col, bsize);
}
...
static int set_vt_partitioning(VP9_COMP *cpi, void *data, MODE_INFO **m,
  ...
  if (mi_col + mi_size < cm->mi_cols && mi_row + mi_size < cm->mi_rows
    ...
    set_block_size(cm, m, bsize, mi_row, mi_col);
    return 1;
  }
  ...
  if (mi_row + mi_size < cm->mi_rows &&
    ...
    set_block_size(cm, m, get_subsize(bsize, PARTITION_VERT), mi_row, mi_col);
    return 1;
  }
  ...
  if (mi_col + mi_size < cm->mi_cols &&
    ...
    set_block_size(cm, m, get_subsize(bsize, PARTITION_HORZ), mi_row, mi_col);
    return 1;
  }
  return 0;
}
...
static void choose_partitioning(VP9_COMP *cpi, MODE_INFO **mi_8x8,
  ...
  if (cm->frame_type != KEY_FRAME) {
    ...
    vp9_find_best_ref_mvs(xd, cm->allow_high_precision_mv,
                          mi_8x8[0]->mbmi.ref_mvs[LAST_FRAME],
                          &nearest_mv, &near_mv);
  ...
  // hit 8x8.
  if (!set_vt_partitioning(cpi, &vt, mi_8x8, BLOCK_64X64, mi_row, mi_col, 4)) {
    for (i = 0; i < 4; ++i) {
        ...
        for (j = 0; j < 4; ++j) {
          ...
          if (!set_vt_partitioning(cpi, &vt.split[i].split[j], mi_8x8,
            ...
            for (k = 0; k < 4; ++k) {
              ...
              set_block_size(cm, mi_8x8, BLOCK_8X8,
  ...
  for (mi_col = tile->mi_col_start; mi_col < tile->mi_col_end;
    ...
    if ((cpi->sf.partition_search_type == SEARCH_PARTITION &&
      ...
      if (cpi->sf.partition_search_type == FIXED_PARTITION) {
        ...
        set_offsets(cpi, tile, mi_row, mi_col, BLOCK_64X64);
        choose_partitioning(cpi, mi_8x8, mi_row, mi_col);
        rd_use_partition(cpi, tile, mi_8x8, tp, mi_row, mi_col, BLOCK_64X64,
                         &dummy_rate, &dummy_dist, 1);
      } else {
...
static void nonrd_pick_sb_modes(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  duplicate_modeinfo_in_sb(cm, xd->mi_8x8, mi_row, mi_col, bsize);
}
...
static void nonrd_pick_fixed_partition(VP9_COMP *cpi,
  ...
  for (br = 0; br < rows; br += bh) {
    for (bc = 0; bc < cols; bc += bw) {
      ...
      duplicate_modeinfo_in_sb(&cpi->common, xd->mi_8x8, row, col, bs);
    }
  }
}
