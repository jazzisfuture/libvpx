...
static void encode_sb(VP9_COMP *cpi,
  ...
  if (is_sb == BLOCK_SIZE_SB32X32) {
    ...
    if (output_enabled) {
      ...
      cpi->partition_count[partition_plane(BLOCK_SIZE_SB32X32)]
                          [PARTITION_NONE]++;
...
static void encode_sb64(VP9_COMP *cpi,
  ...
  if (is_sb[0] == BLOCK_SIZE_SB64X64) {
    ...
    (*tp)->Token = EOSB_TOKEN;
    (*tp)++;
    cpi->partition_count[partition_plane(BLOCK_SIZE_SB64X64)][PARTITION_NONE]++;
#if CONFIG_SBSEGMENT
  } else if (is_sb[0] == BLOCK_SIZE_SB32X64) {
    int i;

    cpi->partition_count[partition_plane(BLOCK_SIZE_SB64X64)][PARTITION_VERT]++;
    ...
    cpi->partition_count[partition_plane(BLOCK_SIZE_SB64X64)][PARTITION_HORZ]++;
    ...
    int i;
    cpi->partition_count[partition_plane(BLOCK_SIZE_SB64X64)]
                        [PARTITION_SPLIT]++;
    for (i = 0; i < 4; i++) {
      const int x_idx = i & 1, y_idx = i >> 1;
...
static void encode_frame_internal(VP9_COMP *cpi) {
  ...
  int totalrate;
   ...
   printf("encode_frame_internal frame %d (%d) type %d\n",
...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t,
  ...
  if (xd->mode_info_context->mbmi.ref_frame == INTRA_FRAME) {
    ...
    vp9_build_inter_predictors_sb(xd, mb_row, mb_col, bsize,
                                  cm->current_video_frame == 1 && mb_row == 2 && mb_col == 21 &&
                                  cpi->refresh_alt_ref_frame && output_enabled);
  }
  ...
  if (!x->skip) {
    ...
    if (cm->current_video_frame == 1 && mb_row == 2 && mb_col == 21 &&
        cpi->refresh_alt_ref_frame && output_enabled) {
      int xx,yy;
      printf("pred (mv=%d/%d, ref=%d/%d, filter=%d, mode=%d):\n", mi->mbmi.mv[0].as_mv.row,
    ...
    if (0 && cm->current_video_frame == 1 && mb_row == 2 && mb_col == 21 &&
        cpi->refresh_alt_ref_frame && output_enabled) {
      int xx,yy;
      printf("recon:\n");
      for (yy=0;yy<32;yy++){
