...
// the source variance for the purpose of activity masking.
//
...
static void rd_pick_sb_modes(VP9_COMP *cpi,
  ...
  if (aq_mode == VARIANCE_AQ) {
    ...
    x->rdmult = set_segment_rdmult(cpi, x, mi->segment_id);
      ...
      // this is tricky, because when splitting
      // by optimizing rate/distortion and having
      // smallest mi segment id for the macroblock,
      // it causes macroblocks not to split.
      // For now, I just have special condition
      // inside the code that defines macroblock tree
      x->rdmult = set_segment_rdmult(cpi, x, mi->segment_id);
