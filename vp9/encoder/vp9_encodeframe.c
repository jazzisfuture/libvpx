...
static void encode_sb(VP9_COMP *cpi,
  ...
  if (is_sb == BLOCK_SIZE_SB32X32) {
    ...
    for (i = 0; i < 2 && mb_col + i != cm->mb_cols; i++) {
      ...
      update_state(cpi, &x->sb16x32_context[xd->sb_index][i], BLOCK_SIZE_SB16X32, 1);
      encode_superblock(cpi, tp,
                        1, mb_row, mb_col + i, BLOCK_SIZE_SB16X32);
    ...
    for (i = 0; i < 2 && mb_row + i != cm->mb_rows; i++) {
      ...
      update_state(cpi, &x->sb32x16_context[xd->sb_index][i], BLOCK_SIZE_SB32X16, 1);
      encode_superblock(cpi, tp,
                        1, mb_row + i, mb_col, BLOCK_SIZE_SB32X16);
...
static void encode_sb_row(VP9_COMP *cpi,
  ...
  for (mb_col = cm->cur_tile_mb_col_start;
    ...
    for (i = 0; i < 4; i++) {
      ...
      sb32_rate += vp9_cost_bit(cm->prob_sb32_coded, 0);
      ...
      if (!sb32_skip && !(mb_col + x_idx + 1 >= cm->mb_cols ||
        ...
        r += vp9_cost_bit(cm->prob_sb32_coded, 1);
    ...
    sb64_rate += vp9_cost_bit(cm->prob_sb64_coded, 0);
    ...
    if (!sb64_skip && !(mb_col + 3 >= cm->mb_cols ||
      ...
      r += vp9_cost_bit(cm->prob_sb64_coded, 1);
      ...
      if (mi->mbmi.sb_type == BLOCK_SIZE_SB64X64) {
        ...
        if (mb_row + 2 != cm->mb_rows)
          reset_skip_txfm_size_sb(cpi, mi + 2 * mis, mis, txfm_max,
                                  cm->mb_rows - mb_row - 2, cm->mb_cols - mb_col,
                                  BLOCK_SIZE_SB64X32);
        ...
        if (mb_col + 2 != cm->mb_cols)
          reset_skip_txfm_size_sb(cpi, mi + 2, mis, txfm_max,
                                  cm->mb_rows - mb_row, cm->mb_cols - mb_col - 2,
                                  BLOCK_SIZE_SB32X64);
#endif
      } else {
        int i;
        ...
        for (i = 0; i < 4; i++) {
          ...
          if (sb_mi->mbmi.sb_type == BLOCK_SIZE_SB32X32) {
            reset_skip_txfm_size_sb(cpi, sb_mi, mis, txfm_max,
                                    ...
                                    BLOCK_SIZE_SB32X32);
#if CONFIG_SBSEGMENT
          } else if (mi->mbmi.sb_type == BLOCK_SIZE_SB32X16) {
            reset_skip_txfm_size_sb(cpi, mi, mis, txfm_max,
                                    cm->mb_rows - mb_row, cm->mb_cols - mb_col,
                                    BLOCK_SIZE_SB32X16);
            if (mb_row + 1 != cm->mb_rows)
              reset_skip_txfm_size_sb(cpi, mi + 1 * mis, mis, txfm_max,
                                      cm->mb_rows - mb_row - 1, cm->mb_cols - mb_col,
                                      BLOCK_SIZE_SB64X32);
          } else if (mi->mbmi.sb_type == BLOCK_SIZE_SB32X16) {
            reset_skip_txfm_size_sb(cpi, mi, mis, txfm_max,
                                    cm->mb_rows - mb_row, cm->mb_cols - mb_col,
            ...
            if (mb_col + 1 != cm->mb_cols)
              reset_skip_txfm_size_sb(cpi, mi + 1, mis, txfm_max,
                                      cm->mb_rows - mb_row, cm->mb_cols - mb_col - 1,
                                      BLOCK_SIZE_SB16X32);
#endif
          } else {
            int m;
