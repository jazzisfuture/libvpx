...
static void encode_sb_row(VP9_COMP *cpi,
  ...
  for (mb_col = cm->cur_tile_mb_col_start;
    ...
    for (i = 0; i < 4; i++) {
      ...
      memcpy(&a2, cm->above_context + mb_col + x_idx, sizeof(a2));
      memcpy(&l2, cm->left_context + y_idx, sizeof(l2));
      for (j = 0; j < 4; j++) {
      ...
      memcpy(cm->above_context + mb_col + x_idx, &a2, sizeof(a2));
      memcpy(cm->left_context + y_idx, &l2, sizeof(l2));
