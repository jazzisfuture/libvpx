...
static void gather_nzcs(MACROBLOCKD *xd) {
  ...
  switch (xd->mode_info_context->mbmi.txfm_size) {
    case TX_4X4:
      for (i = 0; i < 24; ++i) {
        xd->mode_info_context->mbmi.nzcs[i] = xd->block[i].nzc;
      }
      break;
    ...
    case TX_16X16:
      xd->mode_info_context->mbmi.nzcs[0] = xd->block[0].nzc;
      for (i = 16; i < 24; i += 4) {
        xd->mode_info_context->mbmi.nzcs[i] = xd->block[i].nzc;
      }
      break;
...
static void encode_macroblock(VP9_COMP *cpi, TOKENEXTRA **t,
  ...
  if (mbmi->ref_frame == INTRA_FRAME) {
    ...
    if (!x->skip && !cpi->common.mb_no_coeff_skip)
      mbmi->mb_skip_coeff = mbmi->sb_skip_coeff = 0;
  ...
  mbmi->sb_skip_coeff = mbmi->mb_skip_coeff;
...
static void encode_superblock32(VP9_COMP *cpi, TOKENEXTRA **t,
  ...
  if (!cm->mb_no_coeff_skip)
    mi->mbmi.sb_skip_coeff = 0;
  else if (vp9_segfeature_active(xd, segment_id, SEG_LVL_SKIP))
    mi->mbmi.sb_skip_coeff = 1;
  else
    mi->mbmi.sb_skip_coeff =
