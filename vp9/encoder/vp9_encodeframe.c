...
static void copy_partitioning_svc(VP9_COMP *cpi, MACROBLOCK *x, MACROBLOCKD *xd,
                                  ...
                                  int mi_row0, int mi_col0) {
  VP9_COMMON *const cm = &cpi->common;
  BLOCK_SIZE *prev_part = cpi->prev_partition_svc;
  int bsize0 = 0;
  int subsize0 = 0;
  int bsl0 = 0;
  int bs0 = 0;
  ...
  int start_pos = mi_row * (cpi->stride_svc1) + mi_col;
  ...
  if (mi_row0 >= cm->mi_rows || mi_col0 >= cm->mi_cols) return;
  ...
  if (bsize < BLOCK_32X32) {
    ...
    bsize0 = BLOCK_64X64;
    subsize0 = BLOCK_64X64;
  }
  bsl0 = b_width_log2_lookup[bsize0];
  bs0 = (1 << bsl0) / 4;

  if (subsize < BLOCK_8X8) {
    set_block_size(cpi, x, xd, mi_row0, mi_col0, bsize0);
  } else {
    switch (partition) {
      case PARTITION_NONE:
        set_block_size(cpi, x, xd, mi_row0, mi_col0, bsize0);
        break;
      case PARTITION_HORZ:
        set_block_size(cpi, x, xd, mi_row0, mi_col0, subsize0);
        if (subsize0 < BLOCK_64X64)
          set_block_size(cpi, x, xd, mi_row0 + bs0, mi_col, subsize0);
        break;
      case PARTITION_VERT:
        set_block_size(cpi, x, xd, mi_row0, mi_col0, subsize0);
        if (subsize0 < BLOCK_64X64)
          set_block_size(cpi, x, xd, mi_row0, mi_col0 + bs0, subsize0);
        break;
      case PARTITION_SPLIT:
        copy_partitioning_svc(cpi, x, xd, subsize, mi_rowref, mi_colref, mi_row,
                              mi_col, mi_row0, mi_col0);
        copy_partitioning_svc(cpi, x, xd, subsize, mi_rowref, mi_colref,
                              mi_row + bs, mi_col, mi_row0 + bs0, mi_col0);
        copy_partitioning_svc(cpi, x, xd, subsize, mi_rowref, mi_colref, mi_row,
                              mi_col + bs, mi_row0, mi_col0 + bs0);
        copy_partitioning_svc(cpi, x, xd, subsize, mi_rowref, mi_colref,
                              mi_row + bs, mi_col + bs, mi_row0 + bs0,
                              mi_col0 + bs0);
        break;
      default: assert(0);
    }
...
static void update_partition_svc(VP9_COMP *cpi, BLOCK_SIZE bsize, int mi_row,
  ...
  if (subsize < BLOCK_8X8) {
    ...
    switch (partition) {
      case PARTITION_NONE:
        prev_part[start_pos] = bsize;
        // Not sure we need to do this for +2 or +4?
        for (xx = 0; xx < 8; xx += 2)
          for (yy = 0; yy < 8; yy += 2) {
...
static int choose_partitioning(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  if (cpi->sf.use_source_sad && !is_key_frame) {
    ...
    if (cpi->sf.svc_use_lowres_part && cpi->svc.spatial_layer_id == 2 &&
        cpi->svc.non_reference_frame && cpi->prev_partition != NULL &&
