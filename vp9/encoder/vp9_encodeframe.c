...
static void convert_rotzoom_to_params(double *H, Global_Motion_Params *model) {
  ...
  model->mv.as_mv.col = clamp(model->mv.as_mv.col,
                              -GLOBAL_TRANSLATION_MAX, GLOBAL_TRANSLATION_MAX);
  model->mv.as_mv.row = clamp(model->mv.as_mv.row,
                              -GLOBAL_TRANSLATION_MAX, GLOBAL_TRANSLATION_MAX);
}
#endif  // CONFIG_GLOBAL_MOTION
        ...
        if ((num =
          ...
          for (i = 0; i < num; i++) {
            convert_rotzoom_to_params(
                global_motion + i * 4, &cm->global_motion[LAST_FRAME][i]);
