...
static void rd_pick_partition(VP9_COMP *cpi, TOKENEXTRA **tp, int mi_row,
  ...
  if (cm->frame_type && !cpi->is_src_frame_alt_ref &&
      ((use_8x8 && bsize == BLOCK_SIZE_MB16X16) || bsize == BLOCK_SIZE_SB32X32 ||
      ...
      if (d01 < 24 && d23 < 24 && d02 < 24 && d13 < 24) {
        ...
        if (ref0 == ref1 && ref1 == ref2 && ref2 == ref3 &&
          ...
          if (!d01 && !d23 && !d02 && !d13) {
            ...
            if (bsize == BLOCK_SIZE_MB16X16) {
              mode0 = x->sb8x8_context[xd->sb_index][xd->mb_index][0].mic.mbmi.mode;
              mode1 = x->sb8x8_context[xd->sb_index][xd->mb_index][1].mic.mbmi.mode;
              mode2 = x->sb8x8_context[xd->sb_index][xd->mb_index][2].mic.mbmi.mode;
              mode3 = x->sb8x8_context[xd->sb_index][xd->mb_index][3].mic.mbmi.mode;
            }
            else if (bsize == BLOCK_SIZE_SB32X32) {
              mode0 = x->mb_context[xd->sb_index][0].mic.mbmi.mode;
              mode1 = x->mb_context[xd->sb_index][1].mic.mbmi.mode;
              mode2 = x->mb_context[xd->sb_index][2].mic.mbmi.mode;
              mode3 = x->mb_context[xd->sb_index][3].mic.mbmi.mode;
            }
            else if (bsize == BLOCK_SIZE_SB64X64) {
              mode0 = x->sb32_context[0].mic.mbmi.mode;
              mode1 = x->sb32_context[1].mic.mbmi.mode;
              mode2 = x->sb32_context[2].mic.mbmi.mode;
              mode3 = x->sb32_context[3].mic.mbmi.mode;
            }

            if (mode0 == mode1 && mode1 == mode2 && mode2 == mode3) {
              // Set fast motion search level.
              x->fast_ms = 3;
              x->subblock_mode = mode0;
            }
          }
        }
      }
