...
static void encode_rd_sb_row(VP9_COMP *cpi, ThreadData *td,
  ...
  int mi_col;
  int sb_row = mi_row >> MI_BLOCK_SIZE_LOG2;
  int num_sb_cols = get_num_cols(tile_data->tile_info, MI_BLOCK_SIZE_LOG2);
  int c;
  ...
  for (mi_col = mi_col_start, c = 0; mi_col < mi_col_end;
       mi_col += MI_BLOCK_SIZE, c++) {
    const struct segmentation *const seg = &cm->seg;
    int dummy_rate;
    int64_t dummy_dist;
    RD_COST dummy_rdc;
    ...
    (*(cpi->row_mt_sync_read_ptr))(&tile_data->row_mt_sync, sb_row, c - 1);
    ...
    (*(cpi->row_mt_sync_write_ptr))(&tile_data->row_mt_sync, sb_row, c,
                                    num_sb_cols);
  }
}
