...
static void rd_pick_partition(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  RD_COST best_rdc = {INT_MAX, INT64_MAX, INT64_MAX};
...
//  int64_t sum_rd = 0;
  ...
  if (partition_none_allowed) {
    rd_pick_sb_modes(cpi, tile, mi_row, mi_col, &this_rdc, bsize, ctx, best_rd);
  ...
  if (do_split) {
    ...
    if (bsize == BLOCK_8X8) {
      ...
      for (i = 0; i < 4 && sum_rdc.rdcost < best_rd; ++i) {
      ...
      const int y_idx = (i >> 1) * mi_step;
        ...
        rd_pick_partition(cpi, tile, tp, mi_row + y_idx, mi_col + x_idx,
                          subsize, &this_rdc.rate, &this_rdc.dist,
                          best_rd - sum_rdc.rdcost, pc_tree->split[i]);
    ...
    if (sum_rdc.rdcost < best_rd && i == 4) {
      pl = partition_plane_context(xd, mi_row, mi_col, bsize);
      sum_rdc.rate += cpi->partition_cost[pl][PARTITION_SPLIT];
      sum_rdc.rdcost = RDCOST(x->rdmult, x->rddiv,
                              sum_rdc.rate, sum_rdc.dist);

      if (sum_rdc.rdcost < best_rd) {
        best_rdc = sum_rdc;
        pc_tree->partitioning = PARTITION_SPLIT;
      }
    } else {
  ...
  if (partition_horz_allowed && do_rect) {
    ...
    if (sum_rdc.rdcost < best_rd && mi_row + mi_step < cm->mi_rows &&
        bsize > BLOCK_8X8) {
      PICK_MODE_CONTEXT *ctx = &pc_tree->horizontal[0];
      update_state(cpi, ctx, mi_row, mi_col, subsize, 0);
      encode_superblock(cpi, tp, 0, mi_row, mi_col, subsize, ctx);
  ...
  if (partition_vert_allowed && do_rect) {
    ...
    if (sum_rdc.rdcost < best_rd && mi_col + mi_step < cm->mi_cols &&
        bsize > BLOCK_8X8) {
      ...
      rd_pick_sb_modes(cpi, tile, mi_row, mi_col + mi_step, &this_rdc, subsize,
                       &pc_tree->vertical[1], best_rd - sum_rdc.rdcost);
      if (this_rdc.rate == INT_MAX) {
        sum_rdc.rdcost = INT64_MAX;
      } else {
        sum_rdc.rate += this_rdc.rate;
