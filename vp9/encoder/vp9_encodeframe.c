...
static void update_state_supertx(VP9_COMP *cpi, PICK_MODE_CONTEXT *ctx,
  ...
  *mi_addr = *mi;
  ...
  vpx_memcpy(x->zcoeff_blk[mbmi->sr ? TX_SIZES : mbmi->tx_size], ctx->zcoeff_blk,
             sizeof(uint8_t) * ctx->num_4x4_blk);
#else
  vpx_memcpy(x->zcoeff_blk[mbmi->tx_size], ctx->zcoeff_blk,
             sizeof(uint8_t) * ctx->num_4x4_blk);
...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t, int output_enabled,
  ...
  if (output_enabled) {
    ...
    if (is_enable_srmode(bsize)) {
      cm->counts.sr[vp9_get_sr_context(xd)][mbmi->sr]++;
#if SR_USE_MULTI_F
      if (mbmi->sr)
        cm->counts.sr_usfilters[vp9_get_sr_usfilter_context(xd)][mbmi->us_filter_idx] ++;
#endif
    }
    else
      assert(!mbmi->sr);
    if (cm->tx_mode == TX_MODE_SELECT &&
      ...
      ++get_real_tx_counts(max_txsize_lookup[bsize], vp9_get_tx_size_context(xd),
