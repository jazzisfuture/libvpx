...
static void reset_skip_txfm_size_b(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
                                   int mis, TX_SIZE txfm_max, int bw, int bh,
                                   int mi_row, int mi_col,
                                   BLOCK_SIZE_TYPE bsize) {
  VP9_COMMON * const cm = &cpi->common;
...
static void reset_skip_txfm_size_sb(VP9_COMP *cpi, MODE_INFO_8x8 *mi_8x8,
  ...
  if (bwl == bsl && bhl == bsl) {
    reset_skip_txfm_size_b(cpi, mi_8x8, mis, txfm_max, 1 << bsl, 1 << bsl,
                           mi_row, mi_col, bsize);
  } else if (bwl == bsl && bhl < bsl) {
    reset_skip_txfm_size_b(cpi, mi_8x8, mis, txfm_max, 1 << bsl, bs, mi_row,
                           mi_col, bsize);
    reset_skip_txfm_size_b(cpi, mi_8x8 + bs * mis, mis, txfm_max, 1 << bsl, bs,
                           mi_row + bs, mi_col, bsize);
  } else if (bwl < bsl && bhl == bsl) {
    reset_skip_txfm_size_b(cpi, mi_8x8, mis, txfm_max, bs, 1 << bsl, mi_row,
                           mi_col, bsize);
    reset_skip_txfm_size_b(cpi, mi_8x8 + bs, mis, txfm_max, bs, 1 << bsl,
...
static void encode_superblock(VP9_COMP *cpi, TOKENEXTRA **t, int output_enabled,
  ...
  if (!is_inter_block(mbmi)) {
    ...
    int mb_skip_context = xd->left_available ? mi_8x8[-1].mi->mbmi.skip_coeff
                                             : 0;
