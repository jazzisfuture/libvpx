...
static void update_state(VP9_COMP *cpi,
  ...
  if (cpi->common.frame_type == KEY_FRAME) {
    ...
    */
    ...
    if (mbmi->mode >= NEARESTMV && mbmi->mode <= NEWMV &&
...
#if CONFIG_INTERINTRA
    if (bsize < BLOCK_SIZE_SB8X8){
      vp9_build_inter_predictors_sby(xd, mi_row, mi_col, BLOCK_SIZE_SB8X8);
      vp9_build_inter_predictors_sbuv(xd, mi_row, mi_col, BLOCK_SIZE_SB8X8);
    } else {
#endif
