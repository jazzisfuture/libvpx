...
static void predict_superblock(VP9_COMP *cpi,
  ...
  if (!b_sub8x8)
    ...
    vp9_build_inter_predictors_sb_sub8x8_extend(xd, mi_row_ori, mi_col_ori,
...
static void lower_extend(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  BLOCK_SIZE extend_bsize = (mi_width == 1 || bsize < BLOCK_8X8 || xss < yss) ?
                            BLOCK_8X8 : BLOCK_16X8;
  int unit = num_8x8_blocks_wide_lookup[extend_bsize];
  int mi_row_pred = mi_row + mi_height;
  int mi_col_pred = mi_col;
...
static void upper_extend(VP9_COMP *cpi, const TileInfo *const tile,
                         int block, BLOCK_SIZE bsize, BLOCK_SIZE top_bsize,
                         int mi_row, int mi_col,
                         int mi_row_top, int mi_col_top,
                         int output_enabled,
...
static void right_extend(VP9_COMP *cpi, const TileInfo *const tile,
                         int block, BLOCK_SIZE bsize, BLOCK_SIZE top_bsize,
                         int mi_row, int mi_col,
                         int mi_row_top, int mi_col_top,
                         int output_enabled,
  ...
  int xss = xd->plane[1].subsampling_x;
  int yss = xd->plane[1].subsampling_y;
  BLOCK_SIZE extend_bsize = (mi_height == 1 || bsize < BLOCK_8X8 || yss < xss) ?
                            BLOCK_8X8 : BLOCK_8X16;
  int unit = num_8x8_blocks_high_lookup[extend_bsize];
  int mi_row_pred = mi_row;
  int mi_col_pred = mi_col + mi_width;
  int b_sub8x8 = (bsize < BLOCK_8X8) ? 1 : 0;
...
static void left_extend(VP9_COMP *cpi, const TileInfo *const tile,
                        int block, BLOCK_SIZE bsize, BLOCK_SIZE top_bsize,
                        int mi_row, int mi_col,
                        int mi_row_top, int mi_col_top,
                        int output_enabled,
...
static void upperleft_extend(VP9_COMP *cpi, const TileInfo *const tile,
                             int block,
                             ...
                             int output_enabled,
                             uint8_t * dst_buf[3], int dst_stride[3]) {
  BLOCK_SIZE extend_bsize = BLOCK_8X8;
  int mi_row_pred = mi_row - 1;
  int mi_col_pred = mi_col - 1;
  int b_sub8x8 = (bsize < BLOCK_8X8) ? 1 : 0;
...
static void upperright_extend(VP9_COMP *cpi, const TileInfo *const tile,
                             int block,
                             BLOCK_SIZE bsize, BLOCK_SIZE top_bsize,
                             int mi_row, int mi_col,
                             int mi_row_top, int mi_col_top,
...
static void lowerleft_extend(VP9_COMP *cpi, const TileInfo *const tile,
                             int block,
                             BLOCK_SIZE bsize, BLOCK_SIZE top_bsize,
                             int mi_row, int mi_col,
                             int mi_row_top, int mi_col_top,
...
static void lowerright_extend(VP9_COMP *cpi, const TileInfo *const tile,
                             int block,
                             BLOCK_SIZE bsize, BLOCK_SIZE top_bsize,
                             int mi_row, int mi_col,
                             int mi_row_top, int mi_col_top,
...
static void extend_all(VP9_COMP *cpi, const TileInfo *const tile,
  ...
  lower_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  upper_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  left_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  right_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  upperleft_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  upperright_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  lowerleft_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
  lowerright_extend(cpi, tile, block, bsize, top_bsize, mi_row, mi_col,
               mi_row_top, mi_col_top, output_enabled, dst_buf, dst_stride);
}
  ...
  switch (partition) {
    case PARTITION_NONE:
      ...
      predict_b_extend(cpi, tile, 0, mi_row, mi_col, mi_row, mi_col,
                       mi_row_ori, mi_col_ori, dst_buf, dst_stride,
                       bsize, top_bsize, bsize, output_enabled, 0, 0);
      extend_all(cpi, tile, 0, bsize, top_bsize, mi_row, mi_col,
                 mi_row_ori, mi_col_ori, output_enabled, dst_buf, dst_stride);
      break;
    case PARTITION_HORZ:
      if (bsize == BLOCK_8X8) {
        // Fisrt half
        predict_b_extend(cpi, tile, 0, mi_row, mi_col, mi_row, mi_col,
                         mi_row_ori, mi_col_ori, dst_buf, dst_stride,
                         bsize, top_bsize, BLOCK_8X8, output_enabled, 1, 0);
        if (bsize < top_bsize)
        ...
        predict_b_extend(cpi, tile, 2, mi_row, mi_col, mi_row, mi_col,
                         mi_row_ori, mi_col_ori, dst_buf1, dst_stride1,
                         bsize, top_bsize, BLOCK_8X8, output_enabled, 1, 1);
        if (bsize < top_bsize)
        ...
        predict_b_extend(cpi, tile, 0, mi_row, mi_col, mi_row, mi_col,
                         mi_row_ori, mi_col_ori, dst_buf, dst_stride,
                         bsize, top_bsize, subsize, output_enabled, 0, 0);
        if (bsize < top_bsize)
          extend_all(cpi, tile, 0, subsize, top_bsize, mi_row, mi_col,
                     mi_row_ori, mi_col_ori, output_enabled,
                     dst_buf, dst_stride);
        else
          lower_extend(cpi, tile, 0, subsize, top_bsize, mi_row, mi_col,
                       mi_row_ori, mi_col_ori, output_enabled,
                       dst_buf, dst_stride);
        ...
        if (mi_row + hbs < cm->mi_rows) {
          ...
          predict_b_extend(cpi, tile, 0, mi_row + hbs, mi_col,
                           mi_row + hbs, mi_col, mi_row_ori, mi_col_ori,
                           dst_buf1, dst_stride1, bsize, top_bsize, subsize,
                           output_enabled, 0, 0);
          if (bsize < top_bsize)
            ...
            upper_extend(cpi, tile, 0, subsize, top_bsize, mi_row + hbs, mi_col,
                         mi_row_ori, mi_col_ori, output_enabled,
                         dst_buf1, dst_stride1);
    ...
    case PARTITION_VERT:
      if (bsize == BLOCK_8X8) {
        ...
        if (bsize < top_bsize)
          ...
          right_extend(cpi, tile, 0, subsize, top_bsize, mi_row, mi_col,
                       mi_row_ori, mi_col_ori, output_enabled,
                       dst_buf, dst_stride);
        ...
        if (mi_col + hbs < cm->mi_cols) {
          predict_b_extend(cpi, tile, 0, mi_row, mi_col + hbs,
                           mi_row, mi_col + hbs, mi_row_ori, mi_col_ori,
                           dst_buf1, dst_stride1, bsize, top_bsize, subsize,
                           output_enabled, 0, 0);
          if (bsize < top_bsize)
            ...
            left_extend(cpi, tile, 0, subsize, top_bsize, mi_row, mi_col + hbs,
                        mi_row_ori, mi_col_ori, output_enabled,
                        dst_buf1, dst_stride1);
    ...
    case PARTITION_SPLIT:
      if (bsize == BLOCK_8X8) {
        ...
        predict_b_extend(cpi, tile, 2, mi_row, mi_col, mi_row, mi_col,
                         mi_row_ori, mi_col_ori, dst_buf2, dst_stride2,
                         subsize, top_bsize, BLOCK_8X8, output_enabled, 1, 1);
        predict_b_extend(cpi, tile, 3, mi_row, mi_col, mi_row, mi_col,
                         mi_row_ori, mi_col_ori, dst_buf3, dst_stride3,
                         subsize, top_bsize, BLOCK_8X8, output_enabled, 1, 1);
    ...
    case PARTITION_HORZ_A:
      predict_b_extend(cpi, tile, 0, mi_row, mi_col, mi_row, mi_col,
                       mi_row_ori, mi_col_ori, dst_buf, dst_stride,
                       bsize, top_bsize, bsize2, output_enabled, 0, 0);
      extend_all(cpi, tile, 0, bsize2, top_bsize, mi_row, mi_col,
                 mi_row_ori, mi_col_ori, output_enabled, dst_buf, dst_stride);

      predict_b_extend(cpi, tile, 0, mi_row, mi_col + hbs,
                       mi_row, mi_col + hbs, mi_row_ori, mi_col_ori,
                       dst_buf1, dst_stride1, bsize, top_bsize, bsize2,
                       output_enabled, 0, 0);
      extend_all(cpi, tile, 0, bsize2, top_bsize, mi_row, mi_col + hbs,
                 mi_row_ori, mi_col_ori, output_enabled, dst_buf1, dst_stride1);
    ...
    case PARTITION_HORZ_B:
      ...
      predict_b_extend(cpi, tile, 0, mi_row + hbs, mi_col + hbs,
                       mi_row + hbs, mi_col + hbs, mi_row_ori, mi_col_ori,
                       dst_buf2, dst_stride2, bsize, top_bsize, bsize2,
                       output_enabled, 0, 0);
      extend_all(cpi, tile, 0, bsize2, top_bsize, mi_row + hbs, mi_col + hbs,
                 mi_row_ori, mi_col_ori, output_enabled, dst_buf2, dst_stride2);
