...
static int set_vt_partitioning(VP9_COMP *cpi,
  ...
  if (cm->frame_type == KEY_FRAME) {
    bsize_ref = BLOCK_8X8;
    // Choose lower thresholds for key frame variance to favor split, but keep t
    // threshold for splitting to 4x4 still fairly high for now.
    threshold_bsize_ref = threshold << 12;
