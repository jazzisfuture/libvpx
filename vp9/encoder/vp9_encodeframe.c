...
static void restore_context(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  vpx_memcpy(cm->above_seg_context + mi_col, sa,
             sizeof(PARTITION_CONTEXT) * bw / 2);
  vpx_memcpy(cm->left_seg_context + (mi_row & MI_MASK), sl,
             sizeof(PARTITION_CONTEXT) * bh / 2);
}
...
static void encode_sb(VP9_COMP *cpi, TOKENEXTRA **tp,
  ...
  if (level > BLOCK_SIZE_SB8X8) {
    ...
    c1 = *(get_sb_partitioning(x, level));
  }
...
static void rd_pick_partition(VP9_COMP *cpi, TOKENEXTRA **tp,
  ...
  // PARTITION_HORZ
  if ((mi_col + ms <= cm->mi_cols) && (bsize >= BLOCK_SIZE_MB16X16)) {
    int r2, d2;
  ...
  // PARTITION_VERT
  if ((mi_row + ms <= cm->mi_rows) && (bsize >= BLOCK_SIZE_MB16X16)) {
    int r2, d2;
