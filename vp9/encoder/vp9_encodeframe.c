...
#endif
#if CONFIG_SUPERBLOCKS
  assert(mi->mbmi.encoded_as_sb == (block_size >> 5));
#endif
...
#if CONFIG_SUPERBLOCKS
              assert(mb_mi->mbmi.encoded_as_sb == 0);
#endif
  ...
  if (xd->mode_info_context->mbmi.txfm_size == TX_32X32) {
    ...
    for (n = 0; n < 4; n++) {
      ...
      vp9_subtract_sby_s_c(x->sb_coeff_data.src_diff,
                           src + x_idx * 32 + y_idx * 32 * src_y_stride, src_y_stride,
                           dst + x_idx * 32 + y_idx * 32 * dst_y_stride, dst_y_stride);
      vp9_subtract_sbuv_s_c(x->sb_coeff_data.src_diff,
      ...
      vp9_recon_sby_s_c(&x->e_mbd,
                        dst + 32 * x_idx + 32 * y_idx * dst_y_stride, dst_y_stride);
