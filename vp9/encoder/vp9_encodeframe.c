...
static int fast_intra_pred(VP9_COMP *cpi, MACROBLOCK *x, BLOCK_SIZE bsize) {
  MACROBLOCKD *const xd = &x->e_mbd;
  MB_MODE_INFO *const mbmi = &xd->this_mi->mbmi;
  int bw = 4 * num_4x4_blocks_wide_lookup[bsize];
  int bh = 4 * num_4x4_blocks_high_lookup[bsize];
  uint8_t *s = x->plane[0].src.buf;
  uint8_t *d = xd->plane[0].dst.buf;
  int sp = x->plane[0].src.stride;
  int dp = xd->plane[0].dst.stride;

  int tx_size = vp9_largest_txfm_size(cpi, x, bsize);
  int mode = DC_PRED;
  int satd;

  build_intra_predictors(s, sp, d, dp, mode, tx_size,
...
static int fast_inter_pred(VP9_COMP *cpi, MACROBLOCK *x,
                            int mi_row, int mi_col, BLOCK_SIZE bsize) {
  VP9_COMMON *const cm = &cpi->common;
  MACROBLOCKD * const xd = &x->e_mbd;
  int bw = 4 * num_4x4_blocks_wide_lookup[bsize];
  ...
  return vp9_satd_c(s, sp, d, dp, bw, bh);
}
...
static int estimate_pred_cost(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    if (cm->tx_mode != TX_MODE_SELECT)
      return 0;
    satd = fast_intra_pred(cpi, x, bsize);
  } else {
    satd = fast_inter_pred(cpi, x, mi_row, mi_col, bsize);
    if (cm->tx_mode == TX_MODE_SELECT) {
      satd = MIN(satd, fast_intra_pred(cpi, x, bsize));
    }
  }
