...
// the variance based on 2x2 downsampled inputs is used for block <= 16x16.
static void choose_partitioning(VP9_COMP *cpi,
  ...
  int64_t threshold2x2 = threshold << 6;
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < 4; j++) {
      ...
      // is large, compute variance based on 2x2, since this most likely means
      // we need to consider more samples to compare against 8x8, 8x16, 16x8.
      fill_variance_tree(&vt.split[i].split[j], BLOCK_16X16);
      if (vt.split[i].split[j].part_variances.none.variance >= threshold)
      {
        down2x2[i * 4 + j] = 1;
        for (i2 = 0; i2 < 4; i2++) {
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, &vt.split[i], BLOCK_32X32,
        ...
        for (j = 0; j < 4; ++j) {
          ...
          v16x16 *vt16x16 = &vt.split[i].split[j];
          if (down2x2[i * 4 + j])
            vt16x16 = &vt2[i * 4 + j];
          if (!set_vt_partitioning(cpi, vt16x16,
                                   BLOCK_16X16,
                                   mi_row + y32_idx + y16_idx,
                                   mi_col + x32_idx + x16_idx, threshold2x2)) {
            for (k = 0; k < 4; ++k) {
