...
static void simple_motion_search(const VP9_COMP *const cpi,
...
static void ml_predict_var_rd_paritioning(const VP9_COMP *const cpi,
  ...
  // The variance of the residue will be used as input features.
  {
    ...
    if (bsize > BLOCK_8X8) {  // Save MV result as reference for the sub-blocks.
      const MV mv = x->e_mbd.mi[0]->mv[0].as_mv;
      for (i = 0; i < 4; ++i) pc_tree->split[i]->mv = mv;
    }
  }
...
static void rd_pick_partition(VP9_COMP *cpi, ThreadData *td,
  ...
  if (cpi->sf.ml_var_partition_pruning && !frame_is_intra_only(cm)) {
    ...
    if (do_ml_var_partition_pruning) {
      ...
      if (bsize > BLOCK_8X8) {
        for (i = 0; i < 4; ++i) vp9_zero(pc_tree->split[i]->mv);
      }
    }
  }
