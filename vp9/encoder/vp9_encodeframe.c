...
static void constrain_copy_partitioning(VP9_COMP *cpi,
                                        const TileInfo *const tile,
                                        MODE_INFO **mi_8x8,
  ...
  int row8x8_remaining = tile->mi_row_end - mi_row;
  int col8x8_remaining = tile->mi_col_end - mi_col;
  int block_row, block_col, block_row2, block_col2;
  MODE_INFO *mi_upper_left = cm->mi + mi_row * mis + mi_col;
  int bh = num_8x8_blocks_high_lookup[bsize];
  int bw = num_8x8_blocks_wide_lookup[bsize];
  ...
  if ((col8x8_remaining >= MI_BLOCK_SIZE) &&
    ...
    // Else this is a partial SB64, find a partition size that fits.
    for (block_row = 0; block_row < MI_BLOCK_SIZE; block_row += bh) {
      for (block_col = 0; block_col < MI_BLOCK_SIZE; block_col += bw) {
        int index = block_row * mis + block_col;
        bsize = find_partition_size(bsize,
    ...
    if ((cpi->sf.partition_search_type == SEARCH_PARTITION &&
      ...
      if (cpi->sf.partition_search_type == FIXED_PARTITION) {
        ...
        if ((cm->current_video_frame
          ...
          // TODO(marpan): sb_has_motion may have already be computed above,
          // so we should reuse and avoid re-computing.
          if (cpi->sf.constrain_copy_partition &&
              sb_has_motion(cm, prev_mi_8x8))
