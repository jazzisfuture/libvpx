...
static int set_vt_partitioning(VP9_COMP *cpi,
  ...
  // Avoid 64x64 blocks on segment 1.
  if (segment_id && bsize == BLOCK_64X64)
    return 0;
...
static void choose_partitioning(VP9_COMP *cpi,
  ...
  if ( mi_col + 8 > cm->mi_cols || mi_row + 8 > cm->mi_rows ||
      !set_vt_partitioning(cpi, xd, &vt, BLOCK_64X64, mi_row, mi_col,
                           threshold_bsize_max, BLOCK_16X16, segment_id)) {
    for (i = 0; i < 4; ++i) {
      ...
      if (!set_vt_partitioning(cpi, xd, &vt.split[i], BLOCK_32X32,
                               ...
                               threshold, BLOCK_16X16, segment_id)) {
        for (j = 0; j < 4; ++j) {
          ...
          if (!set_vt_partitioning(cpi, xd, vtemp, BLOCK_16X16,
                                   ...
                                   threshold_16x16, bsize_min, segment_id)) {
            for (k = 0; k < 4; ++k) {
