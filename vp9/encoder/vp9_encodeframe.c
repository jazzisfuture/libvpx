...
static int choose_partitioning(VP9_COMP *cpi,
  ...
  if (cpi->sf.short_circuit_low_temp_var) {
    ...
    if (ref_frame_partition == LAST_FRAME &&
        xd->mi[0]->mv[0].as_mv.col < 16 &&
        xd->mi[0]->mv[0].as_mv.col > -16 &&
        xd->mi[0]->mv[0].as_mv.row < 16 &&
        xd->mi[0]->mv[0].as_mv.row > -16) {
        ...
        for (i = 0; i < 4; i++) {
          if (!force_split[i + 1]) {
            ...
            if ((*this_mi)->sb_type == BLOCK_16X16 ||
              ...
              for (j = 0; j < 4; j++) {
                // Variance threshold can be adjusted, the higher the more
                // aggressive. For 32x16 and 16x32 blocks, the flag is set on
                // each 16x16 block inside.
                if (vt.split[i].split[j].part_variances.none.variance <
                    (thresholds[2] >> 8))
