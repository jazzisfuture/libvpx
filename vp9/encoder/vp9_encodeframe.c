  ...
  if (!frame_is_intra_only(cm)) {
    ...
    if (inter_block) {
      ...
      if (cm->reference_mode != SINGLE_REFERENCE &&
        ...
        ++cm->counts.wedge_interinter[bsize][0][mbmi->use_wedge_interinter];
      } else if (get_wedge_bits(bsize) && IS_SINGLE_REF(mbmi)) {
        ++cm->counts.wedge_interinter[bsize][1][mbmi->use_wedge_interinter];
