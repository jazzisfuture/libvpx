...
#if CONFIG_RATE_CTRL
static void store_superblock_partition_info(const PC_TREE *const pc_tree,
                                            PARTITION_INFO *partition_info,
                                            const int square_size_4x4,
  ...
  store_superblock_partition_info(pc_tree->split[0], partition_info,
                                  subblock_square_size_4x4, num_unit_cols,
                                  row_start_4x4, col_start_4x4);
  store_superblock_partition_info(
      pc_tree->split[1], partition_info, subblock_square_size_4x4,
      num_unit_cols, row_start_4x4, col_start_4x4 + subblock_square_size_4x4);
  store_superblock_partition_info(
      pc_tree->split[2], partition_info, subblock_square_size_4x4,
      num_unit_cols, row_start_4x4 + subblock_square_size_4x4, col_start_4x4);
  store_superblock_partition_info(pc_tree->split[3], partition_info,
                                  subblock_square_size_4x4, num_unit_cols,
...
static int rd_pick_partition(VP9_COMP *cpi, ThreadData *td,
  ...
  *rd_cost = best_rdc;
  ...
  if (should_encode_sb && pc_tree->index != 3) {
    ...
    if (output_enabled) {
      const int num_unit_cols = (cm->width + 3) >> 2;
      store_superblock_partition_info(pc_tree, cpi->partition_info,
                                      num_4x4_blocks_wide_lookup[BLOCK_64X64],
                                      num_unit_cols, mi_row, mi_col);
    }
#endif  // CONFIG_RATE_CTRL
  }
