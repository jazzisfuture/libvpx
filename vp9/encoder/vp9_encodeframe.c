...
static void motion_estimation(MACROBLOCK *x, BLOCK_SIZE bsize) {
...
#if GLOBAL_MOTION
    motion_estimation(x, BLOCK_64X64);
#endif
  ...
  if (cm->frame_type != KEY_FRAME && mi_col + hbs < cm->mi_cols &&
    ...
    for (idx = 0; idx < 4; ++idx) {
      ...
      if (subsize >= BLOCK_16X16) {
        motion_estimation(x, subsize);
      } else {
        int best_sad = INT_MAX;
        MV this_mv = x->pred_mv[LAST_FRAME];
    ...
    if (RDCOST(x->rdmult, x->rddiv, this_rate, this_dist * 16) <
        RDCOST(x->rdmult, x->rddiv, sum_rate + 9000, sum_dist * 16))
