...
static unsigned int get_source_diff_variance16x16(VP9_COMP *cpi,
  ...
  var = cpi->fn_ptr[BLOCK_16X16].vf(x->plane[0].src.buf + block_offset,
                                    x->plane[0].src.stride,
  ...
  if ((col8x8_remaining >= MI_BLOCK_SIZE) &&
    ...
    for (i = 0; i < 4; i++) {
      ...
      for (j = 0; j < 4; j++) {
        ...
        var16[j] = get_source_diff_variance16x16(cpi, x, mi_row, mi_col,
                                                 b_offset);
...
//        if (var16[j] > 10000) {
//          int idx = index;
//          mi_8x8[idx] = mi_upper_left + idx;
//          mi_8x8[idx]->mbmi.sb_type = BLOCK_8X8;
//
//          idx = index + 1;
//          mi_8x8[idx] = mi_upper_left + idx;
//          mi_8x8[idx]->mbmi.sb_type = BLOCK_8X8;
...
//        }
      ...
      if (var16[0] < thr_32 && var16[1] < thr_32 &&
          var16[2] < thr_32 && var16[3] < thr_32) {
        for (k = 0; k < 4; k++)
          var32[i] += var16[k];
        ...
        if (cm->current_video_frame % 50 == 1)
          cpi->use_large_partition_rate += 4;
      } else {
        use16x16 = 1;
      }
    }
    ...
    if (!use16x16) {
      if (var32[0] < thr_64 && var32[1] < thr_64 &&
          var32[2] < thr_64 && var32[3] < thr_64)  {
...
//           cm->frame_type);
  ...
  if (cpi->sf.use_nonrd_pick_mode) {
    ...
    if (cpi->sf.partition_search_type == SOURCE_VAR_BASED_PARTITION) {
      ...
      if (cm->current_video_frame % 50 == 2) {
        cpi->use_large_partition_rate = cpi->use_large_partition_rate * 100
            / cm->MBs;
      }

      if (cm->current_video_frame % 50 >= 2) {
        if (cpi->use_large_partition_rate < 30)
          cpi->sf.partition_search_type = VAR_BASED_FIXED_PARTITION;
      }
    }
  }
