...
static void save_context(VP9_COMP *cpi, int mi_row, int mi_col,
  ...
  for (p = 0; p < MAX_MB_PLANE; ++p) {
    ...
    vpx_memcpy(
        l + block_height * p,
        cm->left_context[p]
            + ((mi_row & MI_MASK)* 2 >> xd->plane[p].subsampling_y),sizeof(ENTROPY_CONTEXT) * block_height >> xd->plane[p].subsampling_y);
          }
...
static void sum_intra_stats(VP9_COMP *cpi, MACROBLOCK *x) {
  ...
  if (xd->mode_info_context->mbmi.sb_type >= BLOCK_SIZE_SB8X8) {
    ...
    int idx, idy;
    int block_width = block_type_to_width[xd->mode_info_context->mbmi.sb_type];
    int block_height = block_type_to_height[xd->mode_info_context->mbmi.sb_type];
