...
static void init_layer_context(VP9_COMP *const cpi) {
  ...
  for (temporal_layer = 0; temporal_layer < cpi->svc.number_temporal_layers;
    ...
    lrc->active_worst_quality = q_trans[cpi->oxcf.worst_allowed_q];
    lrc->avg_frame_qindex[INTER_FRAME] = q_trans[cpi->oxcf.worst_allowed_q];
    lrc->last_q[INTER_FRAME] = q_trans[cpi->oxcf.worst_allowed_q];
...
static void update_layer_context_change_config(VP9_COMP *const cpi,
  ...
  for (temporal_layer = 0; temporal_layer < cpi->svc.number_temporal_layers;
    ...
    lrc->buffer_level = MIN(lc->rc.buffer_level, lc->maximum_buffer_size);
