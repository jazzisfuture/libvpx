...
static void encode_frame_to_data_rate(VP9_COMP *cpi,
  ...
  if (cpi->multi_arf_enabled && (cm->frame_type != KEY_FRAME) &&
    ...
    q = cpi->rc.active_worst_quality + vp9_compute_qdelta(cpi, current_q, new_q);
  ...
  if (cm->frame_type != KEY_FRAME) {
    cpi->rc.rolling_target_bits =
      ((cpi->rc.rolling_target_bits * 3) + cpi->rc.this_frame_target + 2) / 4;
    cpi->rc.rolling_actual_bits =
      ((cpi->rc.rolling_actual_bits * 3) + cpi->rc.projected_frame_size + 2) / 4;
