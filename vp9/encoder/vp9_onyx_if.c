...
static void encode_frame_to_data_rate(VP9_COMP *cpi,
  ...
  if (cm->frame_type == KEY_FRAME) {
    if (cpi->oxcf.end_usage == USAGE_CONSTANT_QUALITY) {
      cpi->active_best_quality = cpi->cq_target_quality - 20;
    } else {
#if !CONFIG_MULTIPLE_ARF
    ...
    if (cpi->oxcf.end_usage == USAGE_CONSTANT_QUALITY) {
      cpi->active_best_quality = cpi->cq_target_quality - 8;
    } else {
      int high = 2000;
      int low = 400;
  ...
  do {
    ...
    if (cpi->oxcf.end_usage == USAGE_CONSTANT_QUALITY) {
      ...
      // Is the projected frame size out of range and are we allowed to attempt
      // to recode.
