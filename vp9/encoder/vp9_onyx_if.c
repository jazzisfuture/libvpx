...
// Maybe put these in-frame q adjustment functions in separate file.
...
static void update_refresh_map(VP9_COMP *const cpi,
  ...
  if (block_index < cm->mi_cols * cm->mi_rows) {
    ...
    } else if ((mi->mbmi.mode == NEARESTMV) && (mi->mbmi.mv[0].as_int == 0) &&
...
// We have encoded the frame, so update the cyclic refresh map. Update is
// based on the encoding mode, and possibly the segment rate over/undershooting.
static void postencode_cyclic_refresh_map(VP9_COMP *const cpi) {
  VP9_COMMON *const cm = &cpi->common;
...
static void setup_cyclic_background_refresh(VP9_COMP *const cpi) {
  ...
  if ((cpi->common.frame_type == KEY_FRAME) ||
    ...
    qindex_delta = compute_qdelta_by_rate(cpi,
                                          cm->base_qindex,
                                          rate_ratio_qdelta);
    // TODO: Incorporate the actual-vs-target rate over/undershoot for
    // segment 1, from previous encoded frame.
...
static void encode_frame_to_data_rate(VP9_COMP *cpi,
  ...
  if ((cpi->oxcf.aq_mode == CYCLIC_REFRESH_AQ) && cm->seg.enabled) {
    postencode_cyclic_refresh_map(cpi);
  }
