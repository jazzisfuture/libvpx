...
int vp9_compute_qdelta_by_rate(VP9_COMP *cpi,
  ...
  const int target_bits_per_mb = rate_target_ratio * base_bits_per_mb;
...
static void setup_in_frame_q_adj(VP9_COMP *cpi) {
  ...
  if (cm->frame_type == KEY_FRAME ||
    ...
    for (segment = 1; segment < 3; segment++) {
      const int qindex_delta = vp9_compute_qdelta_by_rate(cpi, cm->base_qindex,
...
static void encode_frame_to_data_rate(VP9_COMP *cpi,
  ...
  q = vp9_rc_pick_q_and_adjust_q_bounds(cpi, &bottom_index, &top_index);
