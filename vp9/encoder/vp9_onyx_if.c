...
static int afcq_low_motion_minq[QINDEX_RANGE];
static int afcq_high_motion_minq[QINDEX_RANGE];
...
static void init_minq_luts(void) {
  ...
  for (i = 0; i < QINDEX_RANGE; i++) {
    ...
    inter_minq[i] = calculate_minq_index(maxq,
                                         ...
                                         0.697,
                                         0.0);
    afcq_low_motion_minq[i] = calculate_minq_index(maxq,
                                                   0.0000015,
                                                   -0.0009,
                                                   0.33,
                                                   0.0);
    afcq_high_motion_minq[i] = calculate_minq_index(maxq,
                                                    0.0000021,
                                                    -0.00125,
                                                    0.55,
                                                    0.0);
  ...
  if (cm->frame_type == KEY_FRAME) {
    ...
    if (cpi->oxcf.end_usage == USAGE_CONSTANT_QUALITY) {
      if (!cpi->refresh_alt_ref_frame) {
        ...
        if (cpi->frames_since_key > 1) {
          if (cpi->gfu_boost > high) {
            cpi->active_best_quality = afcq_low_motion_minq[q];
          } else if (cpi->gfu_boost < low) {
            cpi->active_best_quality = afcq_high_motion_minq[q];
          } else {
            const int gap = high - low;
            const int offset = high - cpi->gfu_boost;
            const int qdiff = afcq_high_motion_minq[q] - afcq_low_motion_minq[q];
            const int adjustment = ((offset * qdiff) + (gap >> 1)) / gap;

            cpi->active_best_quality = afcq_low_motion_minq[q] + adjustment;
          }
        }
      }
    }
