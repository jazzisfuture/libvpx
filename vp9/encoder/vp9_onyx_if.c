...
static void update_layer_rate_context(VP9_COMP *cpi) {
  ...
  for (layer = 0; layer < cpi->svc.number_layers; layer++) {
    ...
    lc->target_bandwidth = cpi->oxcf.ts_target_bitrate[temporal_layer_id] *
        1000;
    // This setting of the buffer levels is basically duplicate code from
    // change_config(); replace with function in both places.
...
// Update framerate-related quantities for the current layer, prior to encoding.
