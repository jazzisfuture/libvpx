...
static void update_cyclic_refresh_map(VP9_COMP *const cpi) {
  ...
  // Offsets for the next mi in the 64x64 block. The last step brings us out
  // back to the starting position.
  const int mi_offset_32[] = {4, (mi_stride << 2) - 4, 4,
      -(mi_stride << 2) - 4};
  // Offsets for block index, for one split of 64x64.
  const int bl_offset_32[] = {0 , 4, (cm->mi_cols << 2) - 4, 4};
  int mi_row, mi_col, bl_index2, idx;
    ...
    for(mi_col = 0; mi_col < cm->mi_cols; mi_col += MI_BLOCK_SIZE) {
      ...
      switch (mip->mbmi.sb_type) {
        ...
        case BLOCK_64X32:
          ...
          bl_index2 = bl_index + cm->mi_cols * 4;
          if (4 >= max_rows)
          break;
          update_refresh_map(cpi, mip2, 8, 4, mi_row, mi_col, bl_index2);
          break;
        case BLOCK_32X64:
          update_refresh_map(cpi, mip, 4, 8, mi_row, mi_col, bl_index);
          mip2 = mip + 4;
          bl_index2 = bl_index + 4;
          if (4 >= max_cols)
            break;
          update_refresh_map(cpi, mip2, 4, 8, mi_row, mi_col, bl_index2);
          break;
        default:
          for (idx = 0; idx < 4; mip += mi_offset_32[idx], ++idx) {
            ...
            switch (mip->mbmi.sb_type) {
              ...
              case BLOCK_32X16:
                ...
                bl_index2 = bl_index + cm->mi_cols * 2;
                if (mi_32_row_offset + 2 >= max_rows)
                  continue;
                update_refresh_map(cpi, mip2, 4, 2, mi_row, mi_col, bl_index2);
                break;
              case BLOCK_16X32:
                update_refresh_map(cpi, mip, 2, 4, mi_row, mi_col, bl_index);
                mip2 = mip + 2;
                bl_index2 = bl_index + 2;
                if (mi_32_col_offset + 2 >= max_cols)
                  continue;
                update_refresh_map(cpi, mip2, 2, 4, mi_row, mi_col, bl_index2);
                break;
              default:
...
static void setup_cyclic_background_refresh(VP9_COMP *const cpi) {
  ...
  if ((cpi->common.frame_type == KEY_FRAME) ||
    ...
    // Limit on maximum amount of q delta (percentage of base q).
    int limit_qdelta = cr->max_qdelta_perc;
    int qindex_delta = 0;
    ...
    if ( (-qindex_delta) < limit_qdelta * cm->base_qindex / 100) {
      qindex_delta = -limit_qdelta * cm->base_qindex / 100;
    }
    vp9_set_segdata(seg, 1, SEG_LVL_ALT_Q, qindex_delta);
    // Set the segmentation map: based on current cyclic refresh map.
    // Cycle through the macroblocks, starting at cr->mb_index.
    i = cr->mb_index;
    ...
    do {
      // If the macroblock (16x16) is as a candidate for clean up then mark it
...
void vp9_change_config(VP9_PTR ptr, VP9_CONFIG *oxcf) {
  ...
  // FOR TESTING: These should removed here, and set via codec_control.
  //cpi->oxcf.aq_mode = 0;
  cpi->oxcf.aq_mode = CYCLIC_REFRESH_AQ;
  cpi->cyclic_refresh.max_mbs_perframe = 2;
  cpi->cyclic_refresh.max_qdelta_perc = 50;
  cpi->cyclic_refresh.min_block_size = BLOCK_16X16;
  cpi->cyclic_refresh.mb_index = 0;
  cpi->cyclic_refresh.time_for_refresh = 1;
  //
