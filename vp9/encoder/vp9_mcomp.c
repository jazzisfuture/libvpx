...
/* returns subpixel variance error function */
#define DIST(r, c) \
    vfp->msvf(PRE(r, c), y_stride, SP(c), SP(r), z, src_stride, mask, mask_stride, &sse)
...
int vp9_find_best_masked_sub_pixel_step_iteratively(
    ...
    int error_per_bit, const vp9_variance_fn_ptr_t *vfp, int *mvjcost,
...
int vp9_masked_refining_search_sad_c(MACROBLOCK *x, int mask_index,
...
int vp9_masked_diamond_search_sad_c(MACROBLOCK *x, int mask_index,
                                    int_mv *ref_mv, int_mv *best_mv,
...
int vp9_masked_full_pixel_diamond(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  bestsme = vp9_masked_diamond_search_sad_c(x, mask_index, mvp_full, &temp_mv,
  ...
  while (n < further_steps) {
    ...
    if (num00)
      ...
      thissme = vp9_masked_diamond_search_sad_c(x, mask_index,
                                                mvp_full, &temp_mv,
                                                step_param + n, sadpb,
                                                &num00,
                                                fn_ptr, x->nmvjointcost,
  ...
  if (do_refine == 1) {
    ...
    thissme = vp9_masked_refining_search_sad_c(x, mask_index,
                                               &best_mv, sadpb,
                                               search_range,
                                               fn_ptr, x->nmvjointcost,
                                               x->mvcost,
