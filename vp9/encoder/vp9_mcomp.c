...
#define MAX_HEX_PARAMS         10
#define HEX_CANDIDATES          6
#define HEX_CANDIDATES_REF      3
int vp9_hex_search(MACROBLOCK *x,
  ...
  static const MV candidates[MAX_HEX_PARAMS][HEX_CANDIDATES] = {
  ...
  static const int next_chkpts_indices[HEX_CANDIDATES][HEX_CANDIDATES_REF] = {
    {5, 0, 1},
    {0, 1, 2},
    {1, 2, 3},
    {2, 3, 4},
  ...
  if (best_init_s != -1) {
    ...
    do {
      ...
      for (j = 0; j < 8; j++) {
        ...
        if (all_in) {
          for (i = 0; i < HEX_CANDIDATES_REF; i++) {
            this_mv.as_mv.row = br + candidates[s][next_chkpts_indices[k][i]].row;
            this_mv.as_mv.col = bc + candidates[s][next_chkpts_indices[k][i]].col;
            this_offset = base_offset + (this_mv.as_mv.row * (in_what_stride)) +
                this_mv.as_mv.col;
            thissad = vfp->sdf(what, what_stride, this_offset, in_what_stride,
                               bestsad);
        ...
        if (best_site == -1) {
          ...
          k = next_chkpts_indices[k][best_site];
  ...
  return
      ...
      use_mvcost ? mv_err_cost(&this_mv, center_mv, x->nmvjointcost, x->mvcost,
                               x->errorperbit, xd->allow_high_precision_mv) : 0;
}

#define MAX_BIGDIA_PARAMS     10
#define BIGDIA_CANDIDATES      8
#define BIGDIA_CANDIDATES_REF  3
int vp9_bigdia_search(MACROBLOCK *x,
  ...
  static const MV candidates[MAX_BIGDIA_PARAMS][BIGDIA_CANDIDATES] = {
  ...
  static const int
      next_chkpts_indices[BIGDIA_CANDIDATES][BIGDIA_CANDIDATES_REF] = {
    {7, 0, 1},
    {0, 1, 2},
    {1, 2, 3},
    {2, 3, 4},
  ...
  if (best_init_s != -1) {
    ...
    do {
      ...
      for (j = 0; j < 8; j++) {
        ...
        if (all_in) {
          for (i = 0; i < BIGDIA_CANDIDATES_REF; i++) {
            this_mv.as_mv.row = br + candidates[s][next_chkpts_indices[k][i]].row;
            this_mv.as_mv.col = bc + candidates[s][next_chkpts_indices[k][i]].col;
            this_offset = base_offset + (this_mv.as_mv.row * (in_what_stride)) +
                this_mv.as_mv.col;
...
#define MAX_SQUARE_PARAMS     11
#define SQUARE_CANDIDATES      8
#define SQUARE_CANDIDATES_REF  3
int vp9_square_search(MACROBLOCK *x,
  ...
  static const MV candidates[MAX_SQUARE_PARAMS][SQUARE_CANDIDATES] = {
  ...
  static const int
      next_chkpts_indices[SQUARE_CANDIDATES][SQUARE_CANDIDATES_REF] = {
    {7, 0, 1},
    {0, 1, 2},
    {1, 2, 3},
    {2, 3, 4},
  ...
  if (best_init_s != -1) {
    ...
    do {
      ...
      for (j = 0; j < 8; j++) {
        ...
        if (all_in) {
          for (i = 0; i < SQUARE_CANDIDATES_REF; i++) {
            this_mv.as_mv.row = br + candidates[s][next_chkpts_indices[k][i]].row;
            this_mv.as_mv.col = bc + candidates[s][next_chkpts_indices[k][i]].col;
            this_offset = base_offset + (this_mv.as_mv.row * (in_what_stride)) +
                this_mv.as_mv.col;
