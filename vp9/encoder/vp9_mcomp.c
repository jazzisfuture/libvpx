...
int vp9_find_best_sub_pixel_step_iteratively(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
  ...
  uint8_t *y0 = *(d->base_pre) + d->pre + (bestmv->as_mv.row) * d->pre_stride + bestmv->as_mv.col;
  uint8_t *y;
...
#else
  uint8_t *y = *(d->base_pre) + d->pre + (bestmv->as_mv.row) * d->pre_stride + bestmv->as_mv.col;
...
int vp9_diamond_search_sad_c(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
  ...
  *num00 = 0;
  ...
  // Work out the start point for the search
  in_what = (uint8_t *)(*(d->base_pre) + d->pre + (ref_row * (d->pre_stride)) + ref_col);
