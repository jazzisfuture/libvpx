...
int vp9_find_best_sub_pixel_step_iteratively(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
  ...
  uint8_t *y = xd->pre.y_buffer + d->offset +
               (bestmv->as_mv.row) * xd->pre.y_stride + bestmv->as_mv.col;
  y_stride = xd->pre.y_stride;
...
(
  ...
  int in_what_stride = xd->pre.y_stride;
  int br, bc;
  int_mv this_mv;
  ...
  base_offset = (uint8_t *)(xd->pre.y_buffer + d->offset);
  this_offset = base_offset + (br * (xd->pre.y_stride)) + bc;
...
int vp9_diamond_search_sad_c(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
  ...
  *num00 = 0;
  ...
  // Work out the start point for the search
  in_what = (uint8_t *)(xd->pre.y_buffer + d->offset +
                        (ref_row * (xd->pre.y_stride)) + ref_col);
...
int vp9_full_search_sad_c(MACROBLOCK *x, BLOCK *b, BLOCKD *d, int_mv *ref_mv,
  ...
  in_what = xd->pre.y_buffer + d->offset;
  bestaddress = in_what + (ref_row * xd->pre.y_stride) + ref_col;
...
int vp9_refining_search_sad_c(MACROBLOCK *x, BLOCK *b, BLOCKD *d,
  ...
  int in_what_stride = xd->pre.y_stride;
  uint8_t *what = (*(b->base_src) + b->src);
  uint8_t *best_address = (uint8_t *)(xd->pre.y_buffer + d->offset +
                                      (ref_mv->as_mv.row * (xd->pre.y_stride)) +
                                      ref_mv->as_mv.col);
  uint8_t *check_here;
  unsigned int thissad;
  int_mv this_mv;
