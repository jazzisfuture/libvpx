  ...
  if (steps > 0) {
    ...
    for (s = 0; s < steps; ++s) {
      ...
      if (s == steps - 1 && proj_filteredlength == olength)
        ...
        out = (s & 1 ? otmp2 : otmp);
      if (filteredlength & 1)
        high_down2_symodd(in, filteredlength, out, bd);
      else
        high_down2_symeven(in, filteredlength, out, bd);
      filteredlength = proj_filteredlength;
    }
    if (filteredlength != olength) {
      high_interpolate(out, filteredlength, output, olength, bd);
    }
    if (tmpbuf)
      free(tmpbuf);
  } else {
    high_interpolate(input, length, output, olength, bd);
  }
}
...
void vp9_high_resize_frame420(const uint8_t *const y,
  ...
  vp9_high_resize_plane(y, height, width, y_stride,
                        oy, oheight, owidth, oy_stride, bd);
  vp9_high_resize_plane(u, height / 2, width / 2, uv_stride,
                        ou, oheight / 2, owidth / 2, ouv_stride, bd);
  vp9_high_resize_plane(v, height / 2, width / 2, uv_stride,
                        ov, oheight / 2, owidth / 2, ouv_stride, bd);
}

void vp9_high_resize_frame422(const uint8_t *const y, int y_stride,
                              const uint8_t *const u, const uint8_t *const v,
                              int uv_stride,
                              int height, int width,
                              uint8_t *oy, int oy_stride,
  ...
  vp9_high_resize_plane(y, height, width, y_stride,
                        oy, oheight, owidth, oy_stride, bd);
  vp9_high_resize_plane(u, height, width / 2, uv_stride,
                        ou, oheight, owidth / 2, ouv_stride, bd);
  vp9_high_resize_plane(v, height, width / 2, uv_stride,
                        ov, oheight, owidth / 2, ouv_stride, bd);
}

void vp9_high_resize_frame444(const uint8_t *const y, int y_stride,
                              const uint8_t *const u, const uint8_t *const v,
                              int uv_stride,
                              int height, int width,
                              uint8_t *oy, int oy_stride,
  ...
  vp9_high_resize_plane(y, height, width, y_stride,
                        oy, oheight, owidth, oy_stride, bd);
  vp9_high_resize_plane(u, height, width, uv_stride,
                        ou, oheight, owidth, ouv_stride, bd);
  vp9_high_resize_plane(v, height, width, uv_stride,
