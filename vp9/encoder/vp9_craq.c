...
void vp9_setup_cyclic_refresh_aq(VP9_COMP *const cpi) {
  ...
  if (!apply_cyclic_refresh ||
    ...
    int i, block_count, bl_index, bl_index2, sb_rows, sb_cols, sbs_in_frame;
    int sum_map, new_value, mi_row, mi_col, xmis, ymis, x, y, bw, bh, qindex2;
    ...
    i = cr->sb_index;
    bw = num_8x8_blocks_wide_lookup[BLOCK_64X64];
    bh = num_8x8_blocks_high_lookup[BLOCK_64X64];
    do {
      // Get mi_row/mi_col corresponding to superblock index i.
      int sb_row_index = (i / sb_cols);
      int sb_col_index = i - sb_row_index * sb_cols;
      mi_row = sb_row_index * MI_BLOCK_SIZE;
      mi_col = sb_col_index * MI_BLOCK_SIZE;
      bl_index = mi_row * cm->mi_cols + mi_col;
      // Loop through all 8x8 blocks in superblock and update map.
      xmis = MIN(cm->mi_cols - mi_col, bw);
      ymis = MIN(cm->mi_rows - mi_row, bh);
      sum_map = 0;
      for (y = 0; y < ymis; y++)
        for (x = 0; x < xmis; x++) {
