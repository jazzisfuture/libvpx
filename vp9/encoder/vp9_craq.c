...
static int apply_cyclic_refresh_bitrate(VP9_COMP *const cpi) {
  ...
  // large than bit cost of segmentation.
  // Worst case segment map cost would scale with number of blocks/frame, so
  // compare available bits to number of blocks.
  // Average bits available per frame = av_per_frame_bandwidth
  // Number of (8x8) blocks in frame = mi_rows * mi_cols;
  float factor  = 0.5;
  int number_blocks = cpi->common.mi_rows  * cpi->common.mi_cols;
  // This condition corresponds to turning off at target bitrates:
