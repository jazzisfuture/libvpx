...
// For inter-modes, zero-mv and/or residual error below some threhsold1.
...
static int candidate_refresh_aq(VP9_COMP *const cpi,
  ...
  if (use_rd) {
    ...
    // Otherwise, reject if the mode is non-zero mv and the projected distortion
    // is above some thresh_dist.
...
void vp9_update_segment_aq(VP9_COMP *const cpi,
  ...
  // as clean. The magnitude of the -ve influences (set to -1 here) how long
  // before we consider it for refresh again.
...
static void update_map(VP9_COMP *const cpi,
  ...
  if (mi->mbmi.segment_id == 1) {
    new_map_value = -1;
  } else if (vp9_candidate_refresh_aq(cpi, mi, bsize, 0)) {
    // Else if it is accepted as candidate for refresh, and has not already
    // been refreshed (marked as 1), then mark it as a candidate for cleanup
    // for future time (marked as 0).
...
void vp9_setup_cyclic_refresh_aq(VP9_COMP *const cpi) {
  ...
  if ((cpi->common.frame_type == KEY_FRAME) ||
    ...
    cr->num_seg_blocks = 0;
    cr->projected_rate_sb = 0;
    cr->projected_dist_sb = 0;

    // Some of these control parameters may be set via control function later.
    ...
    // How to set thresh_dist?
    cr->thresh_dist_sb = (cm->base_qindex * cm->base_qindex);
    if (cpi->sf.use_nonrd_pick_mode) {
      // Set to 0 for now since the best rate and distortion are not returned
      // in non-rd pick_mode.
      cr->thresh_rate_sb = 0;
      cr->thresh_dist_sb = 0;
    }
