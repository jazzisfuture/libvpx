...
// 1. Size of the coding block (below cyclic_refresh->min_block size rejected).
...
// For now testing with on zero-mv last-mode and block size.
...
void vp9_setup_cyclic_background_refresh(VP9_COMP *const cpi) {
  ...
  if ((cpi->common.frame_type == KEY_FRAME) ||
    ...
    int i, x, y, block_count, bl_index, mi_row, mi_col, qindex2;
    ...
    cr->mb_index = i;
    /*
    // Ensure segment 1 is set for whole super block.
    int bl_index2 = 0;
    for (mi_row = 0; mi_row <  cm->mi_rows; mi_row += MI_BLOCK_SIZE)
      for (mi_col = 0; mi_col < cm->mi_cols; mi_col += MI_BLOCK_SIZE) {
        bl_index = mi_row * cm->mi_cols + mi_col;
        if (seg_map[bl_index] == 1) {
          int xmis = num_8x8_blocks_wide_lookup[BLOCK_64X64];
          int ymis = num_8x8_blocks_high_lookup[BLOCK_64X64];
          xmis = MIN(cm->mi_cols - mi_col, xmis);
          ymis = MIN(cm->mi_rows - mi_row, ymis);
          for (x = 0; x < xmis; x++)
            for (y = 0; y < ymis; y++) {
