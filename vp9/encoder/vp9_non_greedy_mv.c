...
static Float_MV get_smooth_motion_vector(
    const Float_MV scaled_search_mv, const Float_MV *tmp_mf,
    const int (*M)[MF_LOCAL_STRUCTURE_SIZE], int rows, int cols, int row,
    int col, float alpha) {
  const Float_MV tmp_mv = tmp_mf[row * cols + col];
  int idx_row, idx_col;
  Float_MV avg_nb_mv = { 0.0f, 0.0f };
  Float_MV mv = { 0.0f, 0.0f };
  float filter[3][3] = { { 1.0f / 12.0f, 1.0f / 6.0f, 1.0f / 12.0f },
                         { 1.0f / 6.0f, 0.0f, 1.0f / 6.0f },
                         { 1.0f / 12.0f, 1.0f / 6.0f, 1.0f / 12.0f } };
  int ref_row = row + (int)scaled_search_mv.row;
  int ref_col = col + (int)scaled_search_mv.col;
  ...
  for (idx = 0; idx < num_iters; ++idx) {
    ...
    for (row = 0; row < rows; ++row) {
      for (col = 0; col < cols; ++col) {
        ...
        Float_MV scaled_search_mv = { (float)search_mv.row / bh,
