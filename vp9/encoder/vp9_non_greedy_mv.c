...
MV get_smooth_motion_vector(MV *scaled_search_mf, MV *tmp_mf, int *M, int rows,
  ...
  float M00 = M[ref_row * rows * 4 + ref_col * 4];
  float M01 = M[ref_row * rows * 4 + ref_col * 4 + 1];
  float M10 = M[ref_row * rows * 4 + ref_col * 4 + 2];
  float M11 = M[ref_row * rows * 4 + ref_col * 4 + 3];
