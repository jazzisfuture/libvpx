...
int vp9_get_skin_map(const int y, const int cb, const int cr) {
  if (y < y_low || y > y_high)
    return 0;
  else
...
void vp9_compute_skin_map(VP9_COMP *const cpi) {
  ...
  if (vp9_alloc_frame_buffer(&cpi->skinmap, cm->width, cm->height,
                             cm->subsampling_x, cm->subsampling_y,
                             VP9_ENC_BORDER_IN_PIXELS, cm->byte_alignment)) {
    vp9_free_frame_buffer(&cpi->skinmap);
    return;
  }
  // Set to gray scale.
  vpx_memset(cpi->skinmap.buffer_alloc, 128, cpi->skinmap.frame_size);
  y = cpi->skinmap.y_buffer;
  // Loop through 8x8 blocks and set skin map based on center pixel of block.
  // Set y to white for skin block, otherwise set to source.
  for(mi_row = 0; mi_row < cm->mi_rows; ++mi_row) {
    for(mi_col = 0; mi_col < cm->mi_cols; ++mi_col) {
      ...
      const int ysource = src_y[4 * src_ystride + 4];
      const int usource = src_u[2 * src_uvstride + 2];
      const int vsource = src_v[2 * src_uvstride + 2];
      int is_skin = vp9_get_skin_map(ysource, usource, vsource);
      for(i = 0; i < 8; i++) {
        for(j = 0; j < 8; j++) {
          if (is_skin)
