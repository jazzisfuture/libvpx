...
void vp9_compute_skin_sb(VP9_COMP *const cpi, BLOCK_SIZE bsize, int mi_row,
  ...
  const int mi_row_limit = VPXMIN(mi_row + 7, cm->mi_rows - 2);
  const int mi_col_limit = VPXMIN(mi_col + 7, cm->mi_cols - 2);
  src_y += y_shift;
  src_u += uv_shift;
  src_v += uv_shift;
  ...
  for (i = mi_row; i < mi_row_limit; i += fac) {
    for (j = mi_col; j < mi_col_limit; j += fac) {
      ...
      for (mi = -fac; mi <= fac && num_neighbor < 2; mi += fac) {
        for (mj = -fac; mj <= fac && num_neighbor < 2; mj += fac) {
      ...
      if (num_neighbor < 2) cpi->skin_map[bl_index] = 0;
    }
  }
}
