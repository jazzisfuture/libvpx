...
void vp9_compute_skin_map(VP9_COMP *const cpi, FILE *yuv_skinmap_file) {
  ...
  int shy = log2(y_bsize);
  int shuv = log2(uv_bsize);
  int fac = y_bsize / 8;
  ...
  y = skinmap.y_buffer;
  // Loop through y_bsizexy_bsize blocks and set skin map based on center 
  // pixel of block. Set y to white for skin block, otherwise set to source 
  // with gray scale. Ignore rightmost/bottom boundary blocks.
  for (mi_row = 0; mi_row < cm->mi_rows - 1; mi_row += fac) {
    num_bl = 0;
    for (mi_col = 0; mi_col < cm->mi_cols - 1; mi_col += fac) {
      // Use middle pixel for each block for skin detection.
      // If middle pixel is skin, assign whole block to skin.
      const uint8_t ysource = src_y[ypos * src_ystride + ypos];
      const uint8_t usource = src_u[uvpos * src_uvstride + uvpos];
      const uint8_t vsource = src_v[uvpos * src_uvstride + uvpos];
