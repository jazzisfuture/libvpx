...
static const int skin_mean[6][2] = {
    {233, 300}, {200, 320}, {220, 320}, {260, 300}, {260, 280},
    {212, 300}};                                        // q6
static const float skin_inv_cov[3] = {4, 3, 2};         // q16
static const int skin_threshold[2] = {1570, 700};       // q18
...
static int evaluate_skin_color_difference(int cb, int cr, int idx) {
  const int cb_q2 = cb << 1;
  const int cr_q2 = cr << 1;
  const int cb_diff_q2 =
      (cb_q2 - skin_mean[idx][0]) * (cb_q2 - skin_mean[idx][0]);
  const int cbcr_diff_q2 =
      (cb_q2 - skin_mean[idx][0]) * (cr_q2 - skin_mean[idx][1]);
  const int cr_diff_q2 =
      (cr_q2 - skin_mean[idx][1]) * (cr_q2 - skin_mean[idx][1]);
  const int skin_diff = skin_inv_cov[0] * cb_diff_q2 +
      skin_inv_cov[1] * cbcr_diff_q2 +
      skin_inv_cov[2] * cr_diff_q2;
  return skin_diff;
}
