...
typedef enum {
  ...
  THR_D45_PRED,
   ...
   int vp9_compute_rd_mult_based_on_qindex(const struct VP9_COMP *cpi,
                                           int qindex);
   ...
   void vp9_initialize_me_consts(struct VP9_COMP *cpi, MACROBLOCK *x,
                                 int qindex);
   ...
   static INLINE int rd_less_than_thresh(int64_t best_rd, int thresh,
     ...
     return best_rd < ((int64_t)thresh * (*thresh_fact) >> 5) ||
            thresh == INT_MAX;
   }
