...
typedef struct {
  ...
  // Its changed adaptively based on circumstances.
  ...
  // Always set to 0. If on it enables 0 cost ( except for the initial
  // transmission of backgrounds that don't change). The feature is disabled
  // because the very large blocksizes make the backgrounds almost free on
  // their own.
  ...
  // Skip rectangular partition test when none gives better rd than split.
  ...
  // Minimum and max partition size we enable (block_size) as per auto
