...
typedef struct {
  ...
  // which means we can re-encode based on bitrate constraints on any frame
  ...
  // Trellis  (dynamic programming ) optimization of coefficients +1 0
  ...
  // log search in future and past 4 times,  otherwise we just use the motion
  // vector found for each frame by itself.
