...
typedef struct {
  // Percentage of blocks (in 8x8 unit) per frame that are cyclicly refreshed.
  int max_mbs_perframe;
  // Maximum q-delta as percentage of base q.
  int max_qdelta_perc;
  // Block size below which we don't apply cyclic refresh.
  BLOCK_SIZE min_block_size;
  // Macroblock starting index for cycling through the frame.
  int mb_index;
  // Control for how long a block will need to wait to be refreshed again.
  int time_for_refresh;
  // Actual number of blocks that have been applied delta-q (segment 1).
  int num_seg_blocks;
