...
static TX_SIZE calculate_tx_size(VP9_COMP *const cpi, BLOCK_SIZE bsize,
  ...
  if (cpi->common.tx_mode == TX_MODE_SELECT) {
    ...
    // Avoid intra mode for now.
    if (cpi->oxcf.content == VP9E_CONTENT_SCREEN && !is_intra &&
        tx_size == TX_8X8 && bsize <= BLOCK_16X16 && var > (ac_thr << 6))
      tx_size = TX_4X4;
  } else {
    tx_size = VPXMIN(max_txsize_lookup[bsize],
                     tx_mode_to_biggest_tx_size[cpi->common.tx_mode]);
...
static void model_rd_for_sb_y_large(VP9_COMP *cpi, BLOCK_SIZE bsize,
  ...
  *sse_y = sse;
  ...
  tx_size = calculate_tx_size(cpi, bsize, xd, var, sse, ac_thr, 0);
...
static void model_rd_for_sb_y(VP9_COMP *cpi, BLOCK_SIZE bsize, MACROBLOCK *x,
  ...
  *sse_y = sse;
  ...
  xd->mi[0]->tx_size =
      calculate_tx_size(cpi, bsize, xd, var, sse, ac_thr, is_intra);
