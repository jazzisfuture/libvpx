...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  // If the segment reference frame feature is enabled and its set to GOLDEN
  // reference, then make sure which don't skip checking GOLDEN, this is to
  ...
  // If the segment reference frame feature is enabled and set to non-INTRA then
  // skip the intra prediction.
  if (segfeature_active(seg, mi->segment_id, SEG_LVL_REF_FRAME) &&
      get_segdata(seg, mi->segment_id, SEG_LVL_REF_FRAME) != INTRA_FRAME)
    perform_intra_pred = 0;
