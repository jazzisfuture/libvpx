...
static void intra_select_tx_size(VP9_COMP *cpi, BLOCK_SIZE bsize, MACROBLOCK *x,
  ...
  unsigned int var = 0;
  vp9_predict_intra_block(xd, b_width_log2_lookup[bsize], bsize, mode,
  ...
  if (best_rdc.rdcost == INT64_MAX ||
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (select_intra_tx_size)
        intra_select_tx_size(cpi, bsize, x, xd, this_mode, mi_row, mi_col, 0);
