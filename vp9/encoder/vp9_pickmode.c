...
static void compute_intra_yprediction(PREDICTION_MODE mode, BLOCK_SIZE bsize,
  ...
  for (row = 0; row < max_blocks_high; row += (1 << tx_size)) {
    ...
    for (col = 0; col < max_blocks_wide; col += (1 << tx_size)) {
     ...
     vp9_predict_intra_block(xd, b_width_log2_lookup[bsize], tx_size,
                             mode, x->skip_encode ? p->src.buf : pd->dst.buf,
                             x->skip_encode ? src_stride : dst_stride, pd->dst.buf,
                             dst_stride, col, row, 0);
...
static void select_intra_tx_size(VP9_COMP *cpi, BLOCK_SIZE bsize,
                                 MACROBLOCK *x, MACROBLOCKD *xd) {
  unsigned int sse;
  struct macroblock_plane *const p = &x->plane[0];
  struct macroblockd_plane *const pd = &xd->plane[0];
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (best_rdc.rdcost == INT64_MAX ||
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (select_tx_size) {
        // TODO: y prediciton is computed here, so we should avoid the same
        // computation below in vp9_foreach_transformed_block_in_plane().
        compute_intra_yprediction(this_mode, bsize, x, xd);
        select_intra_tx_size(cpi, bsize, x, xd);
      }
      else if (cpi->oxcf.content != VP9E_CONTENT_SCREEN && intra_tx_size > TX_16X16)
        mi->tx_size = TX_16X16;
