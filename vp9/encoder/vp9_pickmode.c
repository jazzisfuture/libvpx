...
static void recheck_zeromv_after_denoising(
    ...
    struct buf_2d yv12_mb[4][MAX_MB_PLANE], RD_COST *this_rdc,
    RD_COST *best_rdc, MV_REFERENCE_FRAME *best_ref_frame, BLOCK_SIZE bsize,
    int mi_row, int mi_col) {
  if (cpi->noise_estimate.enabled &&
      cpi->noise_estimate.level > kLow &&
      ctx_den->zero_last_cost_orig < (best_rdc->rdcost << 3) &&
      ((*best_ref_frame == INTRA_FRAME && decision >= FILTER_BLOCK) ||
    ...
    if (this_rdc->rdcost > best_rdc->rdcost) {
      *this_rdc = *best_rdc;
      ...
      if (*best_ref_frame == INTRA_FRAME)
        ...
        mi->mv[0].as_int =
            ctx_den->frame_mv[ctx_den->best_mode][*best_ref_frame].as_int;
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (cpi->oxcf.noise_sensitivity > 0 &&
    ...
    recheck_zeromv_after_denoising(cpi, mi, x, xd, decision, &ctx_den, yv12_mb,
                                   &this_rdc, &best_rdc, &best_ref_frame,
                                   bsize, mi_row, mi_col);
  }
#endif
