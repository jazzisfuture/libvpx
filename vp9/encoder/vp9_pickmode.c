...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (cpi->oxcf.rc_mode == VPX_CBR && !VP9E_CONTENT_SCREEN && !cpi->use_svc) {
    limit_newmv = 1;
    bias_golden = 0;
  }
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    if (bias_golden && ref_frame == GOLDEN_FRAME &&
        ...
        mode_rd_thresh = mode_rd_thresh << 5;
  ...
  if (cpi->sf.adaptive_rd_thresh) {
    ...
    if (best_ref_frame == INTRA_FRAME) {
      ...
      for (i = 0; i < intra_modes; i++) {
        update_thresh_freq_fact(cpi, tile_data, x->source_variance, bsize, INTRA_FRAME,
                              best_mode_idx, intra_mode_list[i], limit_newmv);
      }
    } else {
      for (ref_frame = LAST_FRAME; ref_frame <= GOLDEN_FRAME; ++ref_frame) {
        ...
        for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
          update_thresh_freq_fact(cpi, tile_data, x->source_variance, bsize, ref_frame,
                                  best_mode_idx, this_mode, limit_newmv);
        }
      }
    }
  }
