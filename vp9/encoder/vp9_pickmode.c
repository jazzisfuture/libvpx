...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (cpi->svc.spatial_layer_id > 0 && svc_force_zero_mode[GOLDEN_FRAME - 1] &&
  ...
  for (idx = 0; idx < num_inter_modes + comp_modes; ++idx) {
    ...
    if (flag_svc_subpel && ref_frame == GOLDEN_FRAME && (this_mode == NEWMV ||
       (this_mode == NEARESTMV &&
        frame_mv[NEARESTMV][ref_frame].as_mv.col == svc_mv_col &&
    ...
    // later.
    if (!force_gf_mv && force_skip_low_temp_var &&
        ref_frame == GOLDEN_FRAME &&
        frame_mv[this_mode][ref_frame].as_int != 0) {
      continue;
    }
