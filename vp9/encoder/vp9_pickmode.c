...
int64_t vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  MB_PREDICTION_MODE best_mode = ZEROMV;
  MV_REFERENCE_FRAME best_ref = LAST_FRAME;
  ...
  *returnrate = INT_MAX;
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    int rate_mv = 0;
    x->mode_sad_stack[ref_frame][INTER_OFFSET(NEWMV)] =
    ...
    for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
      ...
      this_rd = RDCOST(x->rdmult, x->rddiv, rate,
                       x->mode_sad_stack[ref_frame][INTER_OFFSET(this_mode)]);
