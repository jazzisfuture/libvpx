...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (best_rdc.rdcost == INT64_MAX ||
    ...
    for (i = 0; i < 4; ++i) {
      ...
      int model_rd_computed = 0;
      const int use_model_rd =
          cpi->sf.use_simple_block_yrd && cpi->common.frame_type != KEY_FRAME &&
          (bsize < BLOCK_32X32 ||
           (cpi->use_svc &&
            (bsize < BLOCK_32X32 || cpi->svc.temporal_layer_id > 0)));
      ...
      block_yrd(cpi, x, &this_rdc, &args.skippable, &this_sse, bsize,
                VPXMIN(mi->tx_size, TX_16X16), model_rd_computed, 1);
