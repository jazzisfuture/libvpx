...
static INLINE void find_predictors(
  ...
  if (is_referenced(cpi->ref_frame_flags, ref_frame) && (yv12 != NULL)) {
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (cpi->use_svc && svc->force_zero_mode_spatial_ref &&
    ...
    if (is_referenced(cpi->ref_frame_flags, LAST_FRAME)) {
    ...
    if (is_referenced(cpi->ref_frame_flags, GOLDEN_FRAME)) {
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= usable_ref_frame; ++ref_frame) {
    ...
    skip_ref_find_pred[ref_frame] =
        !is_referenced(cpi->ref_frame_flags, ref_frame);
  ...
  if (svc->spatial_layer_id > 0 && no_scaling &&
      is_referenced(cpi->ref_frame_flags, GOLDEN_FRAME) &&
  ...
  for (idx = 0; idx < num_inter_modes + comp_modes; ++idx) {
    ...
    if (!is_referenced(cpi->ref_frame_flags, ref_frame)) continue;
      ...
      if (sf->reference_masking &&
        ...
        if (usable_ref_frame < ALTREF_FRAME) {
          if (!force_skip_low_temp_var && usable_ref_frame > LAST_FRAME) {
            ...
            if (is_referenced(cpi->ref_frame_flags, i))
          ...
          if ((is_referenced(cpi->ref_frame_flags, ref1) &&
               (x->pred_mv_sad[ref_frame] > (x->pred_mv_sad[ref1] << 1))) ||
              (is_referenced(cpi->ref_frame_flags, ref2) &&
