...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  for (idx = 0; idx < num_inter_modes + scroll_mode + comp_modes; ++idx) {
    ...
    if ((cpi->sf.adaptive_rd_thresh_row_mt &&
        ...
        (!cpi->sf.adaptive_rd_thresh_row_mt &&
         rd_less_than_thresh(best_rdc.rdcost, mode_rd_thresh,
                             &rd_thresh_freq_fact[mode_index])))
      if (frame_mv[this_mode][ref_frame].as_int != 0 && !scroll_mode) continue;

    if (this_mode == NEWMV && !force_mv_inter_layer && !test_scroll_motion) {
    ...
    if (this_mode == NEWMV && ref_frame == LAST_FRAME && !test_scroll_motion)
