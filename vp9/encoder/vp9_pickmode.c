  ...
  if (best_rdc.rdcost == INT64_MAX ||
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (cpi->sf.nonrd_select_intra_tx_size) {
        // TODO(marpan/jianj): We should be able to skip the y prediction
        // in estimate_block_intra below since its done here, but result is
        // not bitexact when setting skip_ypred = 1.
        args.skip_ypred = 0;
