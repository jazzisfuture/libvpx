...
static int combined_motion_search(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  // Check if we should limit motion vector search.
...
static void bias_rdcost(const NOISE_ESTIMATE *ne, MACROBLOCKD *xd,
  ...
  if (sse_zeromv_normalized < 600 && use_source_variance &&
      ...
      (mv_row > mv_limit || mv_row < -mv_limit || mv_col > mv_limit ||
       mv_col < -mv_limit)) {
    ...
    if (row_diff > mv_limit || row_diff < -mv_limit || col_diff > mv_limit ||
        col_diff < -mv_limit) {
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (sf->short_circuit_flat_blocks || sf->limit_newmv_early_exit ||
      (cpi->oxcf.speed >= 8 && bsize >= BLOCK_32X32)) {
#if CONFIG_VP9_HIGHBITDEPTH
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    if (cpi->oxcf.rc_mode == VPX_CBR && cpi->oxcf.speed > 5 &&
        (x->lowvar_highsumdiff ||
         (sse_zeromv_normalized < thresh_sse_zeromv && use_source_variance &&
          x->source_variance < 40 && bsize >= BLOCK_32X32)))
      x->limit_mv_search = 1;
