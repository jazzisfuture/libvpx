...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (cpi->oxcf.noise_sensitivity > 0 &&
    ...
    if (best_ref_frame == INTRA_FRAME &&
      ...
      // Don't switch to ZEROMV and keep INTRA if either: 1) the rdcost for
      // ZEROMV on denoised source is higher than INTRA, or 2) the cost of
      // ZEROMV on original (un-denoised) source was much higher than INTRA.
      // Otherwise switch mode to ZEROMV.
      if (this_rdc.rdcost > best_rdc.rdcost ||
