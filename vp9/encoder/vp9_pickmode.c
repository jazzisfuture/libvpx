...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (cpi->sf.short_circuit_low_temp_var) {
    if (cpi->oxcf.content != VP9E_CONTENT_SCREEN) {
      ...
      // If force_skip_low_temp_var is set, and for short circuit mode = 1 and
      // 3, skip golden reference.
      ...
      low_temp_var_screen = get_force_skip_low_temp_var(&x->variance_low[0],
                                                        mi_row, mi_col, bsize);
    }
  }
