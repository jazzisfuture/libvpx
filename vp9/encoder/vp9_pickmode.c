...
static INLINE void update_row_mt_thresh_freq_fact(
    VP9_COMP *cpi, TileDataEnc *tile_data, int source_variance, int mi_row,
    BLOCK_SIZE bsize, MV_REFERENCE_FRAME ref_frame, THR_MODES best_mode_idx,
    PREDICTION_MODE mode) {
  int sb_row = mi_cols_aligned_to_sb(mi_row) >> MI_BLOCK_SIZE_LOG2;
  THR_MODES thr_mode_idx = mode_idx[ref_frame][mode_offset(mode)];
  int thresh_freq_fact_idx =
      sb_row * BLOCK_SIZES * MAX_MODES + bsize * MAX_MODES + thr_mode_idx;
  int *freq_fact = &tile_data->row_base_thresh_freq_fact[thresh_freq_fact_idx];
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    if (rd_less_than_thresh_row_mt(best_rdc.rdcost, mode_rd_thresh,
                            // #if CONFIG_MULTITHREAD
                            //             // Synchronization of this function
                            //             is only necessary when
                            //             // adaptive_rd_thresh is > 0.
                            //             cpi->sf.adaptive_rd_thresh ?
  ...
  if (best_rdc.rdcost == INT64_MAX ||
    ...
    for (i = 0; i < 4; ++i) {
      ...
      if (rd_less_than_thresh_row_mt(
              best_rdc.rdcost, mode_rd_thresh,
              // #if CONFIG_MULTITHREAD
              //               // Synchronization of this function is only
              //               necessary when
              //               // adaptive_rd_thresh is > 0.
              //               cpi->sf.adaptive_rd_thresh ?
              //               tile_data->enc_row_mt_mutex : NULL,
    ...
    if (best_ref_frame == INTRA_FRAME) {
      ...
      for (i = 0; i < intra_modes; i++) {
        if (cpi->row_mt && cpi->oxcf.max_threads > 1)
          update_row_mt_thresh_freq_fact(cpi, tile_data, x->source_variance,
                                         mi_row, bsize, INTRA_FRAME,
      ...
      for (ref_frame = LAST_FRAME; ref_frame <= GOLDEN_FRAME; ++ref_frame) {
        ...
        for (this_mode = NEARESTMV; this_mode <= NEWMV; ++this_mode) {
          if (cpi->row_mt && cpi->oxcf.max_threads > 1)
            ...
            update_thresh_freq_fact(cpi, tile_data, x->source_variance, bsize,
                                    INTRA_FRAME, best_mode_idx, this_mode);
        }
      }
    }
  }
