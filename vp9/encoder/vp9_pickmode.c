...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  for (idx = 0; idx < num_inter_modes + comp_modes; ++idx) {
    ...
    // For SVC, if force_skip_frame is set: only do ZEROMV-LAST.
    if (cpi->use_svc && cpi->svc.spatial_layer_id > 0 &&
        cpi->svc.force_skip_frame) {
  ...
  // For SVC: on non-base layer if force_skip_frame is set: set skip_flag and don't
  // check intra modes.
  if (cpi->use_svc && cpi->svc.spatial_layer_id > 0 && cpi->svc.force_skip_frame) {
    x->skip = 1;
    perform_intra_pred = 0;
  }
