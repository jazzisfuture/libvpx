...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  if (cpi->sf.short_circuit_low_temp_var) {
    if (bsize == BLOCK_64X64) {
      force_skip_low_temp_var = x->variance_low[0];
    } else if (bsize == BLOCK_64X32) {
      if (!(mi_col & 0x7) && !(mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[1];
      } else if (!(mi_col & 0x7) && (mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[2];
      }
    } else if (bsize == BLOCK_32X64) {
      if (!(mi_col & 0x7) && !(mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[3];
      } else if ((mi_col & 0x7) && !(mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[4];
      }
    } else if (bsize == BLOCK_32X32) {
      if (!(mi_col & 0x7) && !(mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[5];
      } else if ((mi_col & 0x7) && !(mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[6];
      } else if (!(mi_col & 0x7) && (mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[7];
      } else if ((mi_col & 0x7) && (mi_row & 0x7)) {
        force_skip_low_temp_var = x->variance_low[8];
      }
    }
  }
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    // force_skip_low_temp_var is set.
    if (force_skip_low_temp_var && ref_frame == GOLDEN_FRAME &&
