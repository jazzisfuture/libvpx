    ...
    if (this_mode == NEWMV) {
      if (ref_frame > LAST_FRAME && !cpi->use_svc &&
        ...
        if (frame_mv[NEWMV][ref_frame].as_int != INVALID_MV) {
          ...
          if (sf->base_mv_aggressive && base_mv_sse > (best_sse_sofar << 2))
            continue;
          if (base_mv_sse < ((3 * best_sse_sofar) >> 1)) {
            // Base layer mv is good.
            // Exit NEWMV search if the base_mv is (0, 0) and sse is low.
            unsigned int base_mv_sse_normalized = base_mv_sse >>
