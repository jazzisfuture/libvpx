...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (cpi->oxcf.speed == 8 && low_res && cpi->noise_estimate.enabled) {
    ...
    if (noise_level > kLow && bsize > BLOCK_8X8) force_smooth_filter = 1;
  }
  ...
  if (sf->short_circuit_flat_blocks || sf->limit_newmv_early_exit ||
    ...
    if (x->source_variance > 120) force_smooth_filter = 0;
  }
  if (force_smooth_filter) filter_ref = EIGHTTAP_SMOOTH;
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    if ((this_mode == NEWMV || filter_ref == SWITCHABLE) &&
      ...
      INTERP_FILTER filter_start =
          force_smooth_filter ? EIGHTTAP_SMOOTH : EIGHTTAP;
