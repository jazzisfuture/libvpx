...
static void vp9_NEWMV_diff_bias(const NOISE_ESTIMATE *ne, MACROBLOCKD *xd, PREDICTION_MODE this_mode,
                                    unsigned int source_variance, RD_COST *this_rdc,
                                    BLOCK_SIZE bsize, int mv_row, int mv_col,
                                    int is_last_frame) {
  // Bias against MVs associated with NEWMV mode that are very different from their neighbors
  if (this_mode == NEWMV) {
    ...
    row_diff = abs(al_mv_average_row - mv_row);
    col_diff = abs(al_mv_average_col - mv_col);

    if (row_diff > 48 || col_diff > 48)
      this_rdc->rdcost = (source_variance == 0) ? this_rdc->rdcost << 3 : this_rdc->rdcost << 1;
  }
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    // Bias against NEWMV that is very different from its neighbors & small motion in noise
    if (cpi->oxcf.rc_mode == VPX_CBR &&
        cpi->oxcf.speed >= 5 &&
        cpi->oxcf.content != VP9E_CONTENT_SCREEN) {
      vp9_NEWMV_diff_bias(&cpi->noise_estimate, xd, this_mode, x->source_variance,
                              &this_rdc, bsize,
