...
static int mv_refs_rt(VP9_COMP *cpi, const VP9_COMMON *cm, const MACROBLOCK *x,
  ...
  if (use_base_mv &&
    ...
    // Get base layer mv.
    const int index =
        (mi_col >> 1) + (mi_row >> 1) * cpi->svc.prev_frame_mi_cols;
    // mvs[] is allocated to size mi_cols * mi_rows. Even though use_base_ms is
    // only enabled for 2x2 scaling, if mi_cols/rows is odd then the previous
    // frame (previous spatial layer) will have size mi_cols/rows / 2 + 1.
    if (index < cpi->svc.prev_frame_mi_cols * cpi->svc.prev_frame_mi_rows) {
