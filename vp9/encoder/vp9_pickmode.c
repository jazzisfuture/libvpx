...
static void model_rd_for_sb_y(VP9_COMP *cpi, BLOCK_SIZE bsize,
  ...
  *sse_y = sse;
  ...
  // Evaluate if the partition block is a skippable block in Y plane.
  {
    ...
    unsigned int num_tx_blk =
        (1 << (b_width_log2_lookup[bsize] - b_width_log2_lookup[unit_size])) *
        (1 << (b_height_log2_lookup[bsize] - b_height_log2_lookup[unit_size]));
    unsigned int sse_tx = sse/num_tx_blk;
    unsigned int var_tx = var/num_tx_blk;

    x->skip_txfm[0] = 0;
    // Check if all ac coefficients can be quantized to zero.
    if (var_tx < ac_thr || var_tx == 0) {
      x->skip_txfm[0] = 2;
      // Check if dc coefficient can be quantized to zero.
      if (sse_tx - var_tx < dc_thr || sse_tx == var_tx)
        x->skip_txfm[0] = 1;
    }
  }
