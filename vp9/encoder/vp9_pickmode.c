...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
  ...
  if (!cpi->use_svc ||
    ...
    gf_temporal_ref = 1;
    // For temporal long term prediction, check that golden reference is
    // same scale as last reference..
    struct scale_factors *const sf_last = &cm->frame_refs[LAST_FRAME - 1].sf;
    struct scale_factors *const sf_golden = &cm->frame_refs[GOLDEN_FRAME - 1].sf;
    if (sf_last->x_scale_fp != sf_golden->x_scale_fp || sf_last->y_scale_fp != sf_golden->y_scale_fp)
      gf_temporal_ref = 0;
