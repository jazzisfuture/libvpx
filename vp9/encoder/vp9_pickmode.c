...
static void block_variance(const unsigned char *src, int src_stride,
...
static void model_rd_for_sb_y_large(VP9_COMP *cpi, BLOCK_SIZE bsize,
  ...
  int bw = b_width_log2_lookup[bsize];
  int bh = b_height_log2_lookup[bsize];
  ...
  var = sse - (((unsigned int)sum * sum) >> (bw + bh + 4));
  ...
  *sse_y = sse;
  ...
  // Evaluate if the partition block is a skippable block in Y plane.
  {
    ...
    if (tx_size >= TX_16X16) {
      const BLOCK_SIZE unit_size = txsize_to_bsize[TX_8X8];
      ...
      for (i = 0; i < nh; i += 2) {
        for (j = 0; j < nw; j += 2) {
          sse16x16[k] = sse8x8[i * nw + j] + sse8x8[i * nw + j + 1] +
              sse8x8[(i + 1) * nw + j] + sse8x8[(i + 1) * nw + j + 1];
          sum16x16[k] = sum8x8[i * nw + j] + sum8x8[i * nw + j + 1] +
              sum8x8[(i + 1) * nw + j] + sum8x8[(i + 1) * nw + j + 1];
          var16x16[k] = sse16x16[k] -
              (((unsigned int)sum16x16[k] * sum16x16[k]) >> 8);
          k++;
        }
      }
    }
    ...
    if (tx_size == TX_32X32) {
      const BLOCK_SIZE unit_size = txsize_to_bsize[TX_16X16];
      int nw = 1 << (bw - b_width_log2_lookup[unit_size]);
      int nh = 1 << (bh - b_height_log2_lookup[unit_size]);
      k = 0;
      ...
      for (i = 0; i < nh; i += 2) {
        for (j = 0; j < nw; j += 2) {
          sse32x32[k] = sse16x16[i * nw + j] + sse16x16[i * nw + j + 1] +
              sse16x16[(i + 1) * nw + j] + sse16x16[(i + 1) * nw + j + 1];
          sum32x32[k] = sum16x16[i * nw + j] + sum16x16[i * nw + j + 1] +
              sum16x16[(i + 1) * nw + j] + sum16x16[(i + 1) * nw + j + 1];
          var32x32[k] = sse32x32[k] -
              (((unsigned int)sum32x32[k] * sum32x32[k]) >> 10);
          k++;
        }
      }
    }

    {
      int ac_test = 1;
      int dc_test = 1;
      int num = (tx_size == TX_8X8) ? num8x8 :
          ((tx_size == TX_16X16) ? num16x16 : num32x32);
      unsigned int *sse_tx = (tx_size == TX_8X8) ? sse8x8 :
          ((tx_size == TX_16X16) ? sse16x16 : sse32x32);
      unsigned int *var_tx = (tx_size == TX_8X8) ? var8x8 :
    ...
    *out_dist_sum = sse << 4;
    ...
    if (bsize > BLOCK_32X32 && xd->mi[0].src_mi->mbmi.segment_id != 1) {
      ...
      for (i = 1; i <= 2; i++) {
        ...
        int sf = (bw - b_width_log2_lookup[unit_size]) +
