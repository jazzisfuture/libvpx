...
static TX_SIZE calculate_tx_size(VP9_COMP *cpi, BLOCK_SIZE bsize,
                                 MACROBLOCKD *xd, unsigned int var,
                                 unsigned int sse) {
  TX_SIZE tx_size;
...
static INLINE int denoise_svc_pickmode(VP9_COMP *cpi) {
  SVC *const svc = &cpi->svc;
...
static void search_inter_modes(
    VP9_COMP *cpi, MACROBLOCK *x, PICK_MODE_CONTEXT *ctx,
    int *skip_ref_find_pred, struct buf_2d yv12_mb[][MAX_MB_PLANE],
    const int *const rd_thresh_freq_fact, PRED_BUFFER *tmp, int *ref_frame_cost,
    BLOCK_SIZE bsize, int mi_row, int mi_col, int comp_modes,
    BEST_PICKMODE *best_pickmode, RD_COST *best_rdc,
    MV_REFERENCE_FRAME usable_ref_frame, int_mv frame_mv[][MAX_REF_FRAMES],
    int *svc_force_zero_mode, int *const_motion,
    MV_REFERENCE_FRAME inter_layer_ref, int force_skip_low_temp_var,
    int *ref_frame_skip_mask, int gf_temporal_ref, PRED_BUFFER *this_mode_pred
#if CONFIG_VP9_TEMPORAL_DENOISING
    ,
  ...
  const int num_inter_modes =
      (cpi->use_svc) ? RT_INTER_MODES_SVC : RT_INTER_MODES;
  ...
  for (idx = 0; idx < num_inter_modes + comp_modes; ++idx) {
    ...
    if (!(sf->inter_mode_mask[bsize] & (1 << this_mode))) continue;
...
#if CONFIG_VP9_TEMPORAL_DENOISING
    if (cpi->oxcf.noise_sensitivity > 0 && denoise_svc_pickmode(cpi) &&
...
static void adapt_thresh_freq_fact(VP9_COMP *cpi, TileDataEnc *tile_data,
...
static void init_best_pickmode(BEST_PICKMODE *bp) {
  bp->best_mode = ZEROMV;
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x, TileDataEnc *tile_data,
...
#if CONFIG_VP9_TEMPORAL_DENOISING
  if (cpi->oxcf.noise_sensitivity > 0 &&
      cpi->denoiser.denoising_level > kDenLowLow && denoise_svc_pickmode(cpi))
  ...
  search_inter_modes(cpi, x, ctx, skip_ref_find_pred, yv12_mb,
                     rd_thresh_freq_fact, tmp, ref_frame_cost, bsize, mi_row,
                     mi_col, comp_modes, &best_pickmode, &best_rdc,
                     usable_ref_frame, frame_mv, svc_force_zero_mode,
                     const_motion, inter_layer_ref, force_skip_low_temp_var,
                     &ref_frame_skip_mask, gf_temporal_ref, this_mode_pred
#if CONFIG_VP9_TEMPORAL_DENOISING
                     ,
                     &zero_last_cost_orig
#endif
  ...
  if (cpi->oxcf.noise_sensitivity > 0 && cpi->resize_pending == 0 &&
      denoise_svc_pickmode(cpi) && cpi->denoiser.denoising_level > kDenLowLow &&
  ...
  if (cpi->sf.adaptive_rd_thresh) {
    adapt_thresh_freq_fact(cpi, tile_data, x, bsize,
                           best_pickmode.best_ref_frame, thresh_freq_fact_idx);
  }
