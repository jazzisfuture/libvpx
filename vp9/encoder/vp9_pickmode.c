...
static void model_rd_for_sb_y_large(VP9_COMP *cpi, BLOCK_SIZE bsize,
  ...
  // Calculate variance for bsize, and save 8x8 blocks' variance to be
  // used in transform skipping test.
  ...
  *sse_y = sse;
  ...
  if (x->skip_txfm[0] == 1) {
    ...
    *out_dist_sum = sse << 4;
    ...
    // Evaluate uv planes for skipping
    for (i = 1; i <= 2; i++) {
    ...
    // If we can skip transform for yuv planes, we will check fewer modes and
    // won't check intra modes.
