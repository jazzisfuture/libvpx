  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    if (bsize <= BLOCK_32X32) {
      if (bsize >= BLOCK_16X16 && 0) {
        int rate_t8, rate_t16;
        int64_t dist_t8, dist_t16;
        int64_t sse_t8, sse_t16;
        int is_skip_t8, is_skip_t16;

        sse_t8 = (int64_t)sse_y;
        block_yrd(cpi, x, &rate_t8, &dist_t8, &is_skip_t8,
                  &sse_t8, 0, bsize, TX_8X8);


        sse_t16 = (int64_t)sse_y;
        block_yrd(cpi, x, &rate_t16, &dist_t16, &is_skip_t16,
                  &sse_t16, 0, bsize, TX_16X16);

        if (RDCOST(x->rdmult, x->rddiv, rate_t8, dist_t8) <=
            RDCOST(x->rdmult, x->rddiv, rate_t16, dist_t16)) {
          this_rdc.rate = rate_t8;
          this_rdc.dist = dist_t8;
          this_sse = sse_t8;
          is_skippable = is_skip_t8;
          mbmi->tx_size = TX_8X8;
        } else {
          this_rdc.rate = rate_t16;
          this_rdc.dist = dist_t16;
          this_sse = sse_t16;
          is_skippable = is_skip_t16;
