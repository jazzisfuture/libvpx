...
static void vp9_large_block_mv_bias(VP9_COMP *cpi, RD_COST *this_rdc,
                                    BLOCK_SIZE bsize, int mv_row, int mv_col,
                                    int is_skin, int is_last_frame) {
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  for (idx = 0; idx < RT_INTER_MODES; ++idx) {
    ...
    vp9_large_block_mv_bias(cpi, &this_rdc, bsize,
                            frame_mv[this_mode][ref_frame].as_mv.row,
                            frame_mv[this_mode][ref_frame].as_mv.col,
                            x->sb_is_skin, ref_frame == LAST_FRAME);
