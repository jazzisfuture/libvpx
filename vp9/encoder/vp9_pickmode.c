...
static void model_rd_for_sb_y(VP9_COMP *cpi, BLOCK_SIZE bsize,
  ...
  *sse_y = sse;
  ...
  if (cpi->common.tx_mode == TX_MODE_SELECT) {
    if (sse > (var << 2))
      xd->mi_test[0].target->mbmi.tx_size =
          MIN(max_txsize_lookup[bsize],
              tx_mode_to_biggest_tx_size[cpi->common.tx_mode]);
    else
      xd->mi_test[0].target->mbmi.tx_size = TX_8X8;
  } else {
    xd->mi_test[0].target->mbmi.tx_size =
        MIN(max_txsize_lookup[bsize],
            tx_mode_to_biggest_tx_size[cpi->common.tx_mode]);
...
void vp9_pick_inter_mode(VP9_COMP *cpi, MACROBLOCK *x,
  ...
  *returndistortion = INT64_MAX;
  ...
  for (ref_frame = LAST_FRAME; ref_frame <= ALTREF_FRAME; ++ref_frame) {
    ...
    if (cpi->ref_frame_flags & flag_list[ref_frame]) {
      ...
      if (!cm->error_resilient_mode)
        vp9_find_mv_refs(cm, xd, tile, xd->mi_test[0].target, ref_frame,
                         candidates, mi_row, mi_col);
      else
        const_motion[ref_frame] = mv_refs_rt(cm, xd, tile, xd->mi_test[0].target,
  ...
  xd->mi_test[0].target->bmi[0].as_mv[0].as_int = mbmi->mv[0].as_int;
  x->skip_txfm[0] = skip_txfm;
