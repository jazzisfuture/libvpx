...
static void analysis_53_row(int length, int16_t *x,
...
static void analysis_53_col(int length, int16_t *x,
                            tran_low_t *lowpass, tran_low_t *highpass) {
  int n;
  short r, *a, *b;
...
static void analysis_26_row(int length, int16_t *x,
                            tran_low_t *lowpass, tran_low_t *highpass) {
  int i, n;
  short r, s, *a, *b;
  a = lowpass;
...
static void analysis_26_col(int length, int16_t *x,
                            tran_low_t *lowpass, tran_low_t *highpass) {
  int i, n;
  short r, s, *a, *b;
  a = lowpass;
...
#if DWT_TYPE == 26
  dyadic_analyze_26(5, 32, 32, input, stride, output, 32, 1);
#elif DWT_TYPE == 97
  dyadic_analyze_97(5, 32, 32, input, stride, output, 32, 1);
#elif DWT_TYPE == 53
  dyadic_analyze_53(5, 32, 32, input, stride, output, 32, 1);
#endif
}
...
void vp9_fdwtdct32x32_c(tran_low_t *input, tran_low_t *output,
  ...
  int i, j;
  // Scales up by 2-bit from orthogonal
#if DWT_TYPE == 26
...
#endif
  // Scales down by 1-bit in the LL band
  for (i = 0; i < 16; ++i) {
    for (j = 0; j < 16; ++j) {
      output[i * 32 + j] = (output[i * 32 + j] + 4) >> 1;
    }
  }
  // 16x16 dct in LL band that is 3-bit scaled up from orthogonal
  vp9_fdct16x16_noscale(output, buffer, 32);
  // Note that the transform overall is 2-bit scaled up from orthogonal
  for (i = 0; i < 16; ++i) {
...
#if DWT_TYPE == 26
  dyadic_analyze_26(6, 64, 64, input, stride, output, 64, 1);
#elif DWT_TYPE == 97
  dyadic_analyze_97(6, 64, 64, input, stride, output, 64, 1);
#elif DWT_TYPE == 53
  dyadic_analyze_53(6, 64, 64, input, stride, output, 64, 1);
#endif
}
...
void vp9_fdwtdct64x64_c(tran_low_t *input, tran_low_t *output,
  ...
  int i;
  // Scales up by 1-bit from orthogonal
#if DWT_TYPE == 26
...
#endif
  // 32x32 dct in LL band that has no scaling
  vp9_fdct32x32_noscale(output, buffer, 64);
  // Note that the transform overall is 1-bit scaled up from orthogonal
  for (i = 0; i < 32; ++i) {
