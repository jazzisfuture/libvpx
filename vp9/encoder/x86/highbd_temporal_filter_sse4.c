...
// by weight.
static INLINE void highbd_average_4(__m128i *output, const __m128i *sum, const __m128i *mul_constants,
...
static INLINE void highbd_average_8(__m128i *output_0, __m128i *output_1,
                              const __m128i *sum_0_u32, const __m128i *sum_1_u32,
...
static INLINE void highbd_accumulate_and_store_8(const __m128i sum_first_u32,
                                                 ...
                                                 const uint16_t *pred, uint16_t *count,
...
static void vp9_highbd_apply_temporal_filter_luma_8(
    ...
    const uint32_t *const *neighbors_second, int top_weight, int bottom_weight) {
  ...
  assert(strength >= 4 && strength <= 14 && "invalid adjusted temporal filter strength");
  ...
  // Add chroma values
  highbd_read_chroma_dist_row_8(ss_x, u_dist, v_dist, &u_first, &u_second, &v_first,
                                 &v_second);
  ...
  highbd_average_8(&sum_row_first, &sum_row_second, &sum_row_first, &sum_row_second, &mul_first, &mul_second,
...
static void vp9_highbd_apply_temporal_filter_luma(
  ...
  vp9_highbd_apply_temporal_filter_luma_8(
      ...
      v_pre + uv_blk_col, uv_pre_stride, blk_col_step, block_height, ss_x, ss_y, strength,
      use_whole_blk, y_accum + blk_col, y_count + blk_col, y_dist + blk_col,
      u_dist + uv_blk_col, v_dist + uv_blk_col, neighbors_first,
      neighbors_second, top_weight, bottom_weight);
  ...
  for (; blk_col < mid_width;
    ...
    vp9_highbd_apply_temporal_filter_luma_8(
        ...
        u_pre + uv_blk_col, v_pre + uv_blk_col, uv_pre_stride, blk_col_step, block_height,
...
static INLINE void highbd_add_luma_dist_to_8_chroma_mod(const uint32_t *y_dist,
                                                        int ss_x, int ss_y,
                                                        __m128i *u_mod_fst,
                                                        __m128i *u_mod_snd,
                                                        __m128i *v_mod_fst,
...
static void vp9_highbd_apply_temporal_filter_chroma_8(
    ...
    int top_weight, int bottom_weight,
    const int *blk_fw) {
  const int rounding = (1 << strength) >> 1;
  int weight = top_weight;
  ...
  // Get modifier and store result
  if (blk_fw) {
    highbd_average_4(&u_sum_row_fst, &u_sum_row_fst, &mul_fst, strength, rounding,
                     blk_fw[0]);
    highbd_average_4(&u_sum_row_snd, &u_sum_row_snd, &mul_snd, strength, rounding,
                     blk_fw[1]);

    highbd_average_4(&v_sum_row_fst, &v_sum_row_fst, &mul_fst, strength, rounding,
                     blk_fw[0]);
    highbd_average_4(&v_sum_row_snd, &v_sum_row_snd, &mul_snd, strength, rounding,
                     blk_fw[1]);

  } else {
    highbd_average_8(&u_sum_row_fst, &u_sum_row_snd, &u_sum_row_fst,
                     &u_sum_row_snd, &mul_fst, &mul_snd, strength, rounding, weight);
    highbd_average_8(&v_sum_row_fst, &v_sum_row_snd, &v_sum_row_fst,
                     &v_sum_row_snd, &mul_fst, &mul_snd, strength, rounding, weight);
  }
...
void vp9_highbd_apply_temporal_filter_sse4_1(
  ...
  vp9_highbd_apply_temporal_filter_luma(
      ...
      strength, blk_fw, use_whole_blk, y_accum, y_count, y_dist_ptr,
      u_dist_ptr, v_dist_ptr);

  vp9_highbd_apply_temporal_filter_chroma(
    y_src, y_src_stride, y_pre, y_pre_stride, u_src, v_src, uv_src_stride,
    u_pre, v_pre, uv_pre_stride, block_width, block_height, ss_x, ss_y, strength,
    blk_fw, use_whole_blk, u_accum, u_count, v_accum, v_count, y_dist_ptr, u_dist_ptr,
    v_dist_ptr);
}
