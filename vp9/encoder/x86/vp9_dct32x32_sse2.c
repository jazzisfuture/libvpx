...
#if !FDCT32x32_HIGH_PRECISION
void vp9_sfdct32x32_sse2(const int16_t *input,
                         int16_t *output_org, int stride, int64_t *psse) {
  const int str1 = stride;
  const int str2 = 2 * stride;
  const int str3 = 2 * stride + str1;
  DECLARE_ALIGNED(16, int16_t, intermediate[32 * 32]);
  DECLARE_ALIGNED(16, int64_t, output_sse[2]);
  const __m128i k__cospi_p16_p16 = _mm_set1_epi16(+cospi_16_64);
  const __m128i k__cospi_p16_m16 = pair_set_epi16(+cospi_16_64, -cospi_16_64);
  const __m128i k__cospi_m08_p24 = pair_set_epi16(-cospi_8_64,   cospi_24_64);
  const __m128i k__cospi_m24_m08 = pair_set_epi16(-cospi_24_64, -cospi_8_64);
  ...
  for (pass = 0; pass < 2; ++pass) {
    ...
    for (column_start = 0; column_start < 32; column_start += 8) {
      ...
      // Transpose the results, do it as four 8x8 transposes.
      {
        ...
        for (transpose_block = 0; transpose_block < tb_num; ++transpose_block) {
          __m128i *this_out = &out[8 * transpose_block];
          // transpose 8x8 block
          ...
          if (0 == pass) {
            ...
            step1[0] = _mm_madd_epi16(tr2_0, tr2_0);
            step1[1] = _mm_madd_epi16(tr2_1, tr2_1);
            step1[2] = _mm_madd_epi16(tr2_2, tr2_2);
            step1[3] = _mm_madd_epi16(tr2_3, tr2_3);
            step1[4] = _mm_madd_epi16(tr2_4, tr2_4);
            step1[5] = _mm_madd_epi16(tr2_5, tr2_5);
            step1[6] = _mm_madd_epi16(tr2_6, tr2_6);
            step1[7] = _mm_madd_epi16(tr2_7, tr2_7);

            step2[0] = _mm_unpacklo_epi32(step1[0], kZero);
            step2[1] = _mm_unpackhi_epi32(step1[0], kZero);
            step2[2] = _mm_unpacklo_epi32(step1[1], kZero);
            step2[3] = _mm_unpackhi_epi32(step1[1], kZero);
            step2[4] = _mm_unpacklo_epi32(step1[2], kZero);
            step2[5] = _mm_unpackhi_epi32(step1[2], kZero);
            step2[6] = _mm_unpacklo_epi32(step1[3], kZero);
            step2[7] = _mm_unpackhi_epi32(step1[3], kZero);

            step1[0] = _mm_add_epi64(step2[0], step2[1]);
            step1[1] = _mm_add_epi64(step2[2], step2[3]);
            step1[2] = _mm_add_epi64(step2[4], step2[5]);
            step1[3] = _mm_add_epi64(step2[6], step2[7]);

            step2[0] = _mm_unpacklo_epi32(step1[4], kZero);
            step2[1] = _mm_unpackhi_epi32(step1[4], kZero);
            step2[2] = _mm_unpacklo_epi32(step1[5], kZero);
            step2[3] = _mm_unpackhi_epi32(step1[5], kZero);
            step2[4] = _mm_unpacklo_epi32(step1[6], kZero);
            step2[5] = _mm_unpackhi_epi32(step1[6], kZero);
            step2[6] = _mm_unpacklo_epi32(step1[7], kZero);
            step2[7] = _mm_unpackhi_epi32(step1[7], kZero);

            step1[4] = _mm_add_epi64(step2[0], step2[1]);
            step1[5] = _mm_add_epi64(step2[2], step2[3]);
            step1[6] = _mm_add_epi64(step2[4], step2[5]);
            step1[7] = _mm_add_epi64(step2[6], step2[7]);

            step1[0] = _mm_add_epi64(step1[0], step1[4]);
            step1[1] = _mm_add_epi64(step1[1], step1[5]);
            step1[2] = _mm_add_epi64(step1[2], step1[6]);
            step1[3] = _mm_add_epi64(step1[3], step1[7]);

            step1[0] = _mm_add_epi64(step1[0], step1[1]);
            step1[1] = _mm_add_epi64(step1[2], step1[3]);

            step2[0] = _mm_add_epi64(step1[0], step1[1]);

            _mm_storeu_si128((__m128i *)(output_sse), step2[0]);


            step2[1] = _mm_srli_si128(step2[0], 8);

            _mm_storeu_si128((__m128i *)(output_sse), step2[1]);

            step1[0] = _mm_add_epi64(step2[0], step2[1]);
            _mm_storeu_si128((__m128i *)(output_sse), step1[0]);
            *psse = output_sse[0];
          }
          // Note: even though all these stores are aligned, using the aligned
          //       intrinsic make the code slightly slower.
