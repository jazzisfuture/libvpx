...
static int vp9_denoiser_4_8xM_sse2(
  ...
  for (r = 0; r < ((4 << b_height_log2_lookup[bs]) >> shift); ++r) {
    ...
    vpx_memcpy(mc_running_buffer[r] + width, mc_running_avg_y +
               mc_avg_y_stride, width);
    vpx_memcpy(running_buffer[r], running_avg_y, width);
    vpx_memcpy(running_buffer[r] + width, running_avg_y +
               avg_y_stride, width);
    if (width == 4) {
      vpx_memcpy(sig_buffer[r] + width * 2, sig + sig_stride * 2, width);
      vpx_memcpy(sig_buffer[r] + width * 3, sig + sig_stride * 3, width);
      vpx_memcpy(mc_running_buffer[r] + width * 2, mc_running_avg_y +
                 mc_avg_y_stride * 2, width);
      vpx_memcpy(mc_running_buffer[r] + width * 3, mc_running_avg_y +
                 mc_avg_y_stride * 3, width);
      vpx_memcpy(running_buffer[r] + width * 2, running_avg_y +
                 avg_y_stride * 2, width);
      vpx_memcpy(running_buffer[r] + width * 3, running_avg_y +
                 avg_y_stride * 3, width);
    }
    ...
    if (abs(sum_diff) > sum_diff_thresh) {
      ...
      if (delta < 4) {
        ...
        for (r = 0; r < ((4 << b_height_log2_lookup[bs]) >> shift); ++r) {
          ...
          vpx_memcpy(running_avg_y, running_buffer[r], width);
          vpx_memcpy(running_avg_y + avg_y_stride, running_buffer[r] +
                     width, width);
          if (width == 4) {
            vpx_memcpy(running_avg_y + avg_y_stride * 2,
                       running_buffer[r] + width * 2, width);
...
int vp9_denoiser_filter_sse2(const uint8_t *sig, int sig_stride,
  ...
  if (bs == BLOCK_4X4 || bs == BLOCK_4X8) {
    return vp9_denoiser_4_8xM_sse2(sig, sig_stride,
