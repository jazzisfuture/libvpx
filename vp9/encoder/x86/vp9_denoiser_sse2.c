...

  {
    ...
    if (abs(sum_diff) > sum_diff_thresh) {
      ...
      if (delta < 4) {
        ...
        running_avg_y -= avg_y_stride * heights[bs];
        sum_diff = 0;
        for (r = 0; r < (heights[bs]>>2); ++r) {
...
static int vp9_denoiser_64_32_16xM_sse2(const uint8_t *sig, int sig_stride,
  ...
  for (r = 0; r < heights[bs]; r++) {
    ...
    // Update pointers for next iteration.
    sig = sig - 16*(widths[bs]>>4) + sig_stride;
    mc_running_avg_y = mc_running_avg_y - 16*(widths[bs]>>4) + mc_avg_y_stride;
    running_avg_y = running_avg_y - 16*(widths[bs]>>4) + avg_y_stride;
  }

  {
    ...
    if (abs(sum_diff) > sum_diff_thresh) {
      ...
      if (delta < 4) {
        const __m128i k_delta = _mm_set1_epi8(delta);
        sig -= sig_stride * heights[bs];
        mc_running_avg_y -= mc_avg_y_stride * heights[bs];
        running_avg_y -= avg_y_stride * heights[bs];
        sum_diff = 0;
        for (r = 0; r < heights[bs]; ++r) {
          for (c = 0; c < widths[bs]; c += 16) {
          ...
          sig = sig - 16*(widths[bs]>>4) + sig_stride;
          mc_running_avg_y = mc_running_avg_y - 16*(widths[bs]>>4)
                             + mc_avg_y_stride;
          running_avg_y = running_avg_y - 16*(widths[bs]>>4) + avg_y_stride;
        }
        if (abs(sum_diff) > sum_diff_thresh) {
