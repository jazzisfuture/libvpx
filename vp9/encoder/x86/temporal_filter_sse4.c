...
static void vp9_apply_temporal_filter_luma(
  ...
  unsigned int blk_col = 0, cr_blk_col = 0;
  const unsigned int blk_col_step = 16, cr_blk_col_step = 16 >> ss_x;
  ...
  if (block_width == 16) {
    ...
    if (use_whole_blk) {
      vp9_apply_temporal_filter_luma_16(
          ...
          u_src + cr_blk_col, v_src + cr_blk_col, uv_src_stride,
          u_pre + cr_blk_col, v_pre + cr_blk_col, uv_pre_stride, 16,
          block_height, ss_x, ss_y, strength, use_whole_blk, y_accum + blk_col,
          y_count + blk_col, y_dist + blk_col, u_dist + cr_blk_col,
          v_dist + cr_blk_col, neighbors_first, neighbors_second, top_weight,
          bottom_weight, NULL);
    } else {
      vp9_apply_temporal_filter_luma_16(
          y_src + blk_col, y_src_stride, y_pre + blk_col, y_pre_stride,
          ...
          block_height, ss_x, ss_y, strength, use_whole_blk, y_accum + blk_col,
          y_count + blk_col, y_dist + blk_col, u_dist + cr_blk_col,
          v_dist + cr_blk_col, neighbors_first, neighbors_second, 0, 0, blk_fw);
    }
  ...
  vp9_apply_temporal_filter_luma_16(
      y_src + blk_col, y_src_stride, y_pre + blk_col, y_pre_stride,
      u_src + cr_blk_col, v_src + cr_blk_col, uv_src_stride, u_pre + cr_blk_col,
      v_pre + cr_blk_col, uv_pre_stride, 16, block_height, ss_x, ss_y, strength,
      use_whole_blk, y_accum + blk_col, y_count + blk_col, y_dist + blk_col,
      u_dist + cr_blk_col, v_dist + cr_blk_col, neighbors_first,
      neighbors_second, top_weight, bottom_weight, NULL);

  blk_col += blk_col_step;
  cr_blk_col += cr_blk_col_step;

  // Middle First
  neighbors_first = LUMA_MIDDLE_COLUMN_NEIGHBORS;
  for (; blk_col < mid_width;
       blk_col += blk_col_step, cr_blk_col += cr_blk_col_step) {
    vp9_apply_temporal_filter_luma_16(
        y_src + blk_col, y_src_stride, y_pre + blk_col, y_pre_stride,
        u_src + cr_blk_col, v_src + cr_blk_col, uv_src_stride,
        u_pre + cr_blk_col, v_pre + cr_blk_col, uv_pre_stride, 16, block_height,
...
static void vp9_apply_temporal_filter_chroma(
  ...
  unsigned int blk_col = 0, cr_blk_col = 0;
  const unsigned int cr_blk_col_step = 8, blk_col_step = 8 << ss_x;
  const unsigned int uv_mid_width = uv_width >> 1,
                     uv_last_width = uv_width - cr_blk_col_step;
  int top_weight = blk_fw[0],
      bottom_weight = use_whole_blk ? blk_fw[0] : blk_fw[2];
  ...
  if (uv_width == 8) {
    ...
    if (use_whole_blk) {
      vp9_apply_temporal_filter_chroma_8(
          y_src + blk_col, y_src_stride, y_pre + blk_col, y_pre_stride,
          u_src + cr_blk_col, v_src + cr_blk_col, uv_src_stride,
          u_pre + cr_blk_col, v_pre + cr_blk_col, uv_pre_stride, uv_width,
          uv_height, ss_x, ss_y, strength, u_accum + cr_blk_col,
          u_count + cr_blk_col, v_accum + cr_blk_col, v_count + cr_blk_col,
          y_dist + blk_col, u_dist + cr_blk_col, v_dist + cr_blk_col, neighbors,
  ...
  vp9_apply_temporal_filter_chroma_8(
      y_src + blk_col, y_src_stride, y_pre + blk_col, y_pre_stride,
      u_src + cr_blk_col, v_src + cr_blk_col, uv_src_stride, u_pre + cr_blk_col,
      v_pre + cr_blk_col, uv_pre_stride, uv_width, uv_height, ss_x, ss_y,
      strength, u_accum + cr_blk_col, u_count + cr_blk_col,
      v_accum + cr_blk_col, v_count + cr_blk_col, y_dist + blk_col,
      u_dist + cr_blk_col, v_dist + cr_blk_col, neighbors, top_weight,
      bottom_weight, NULL);
