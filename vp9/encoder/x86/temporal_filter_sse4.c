...
static void sum_8(const uint8_t *a, const uint8_t *b, __m128i *sum) {
  ...
  const __m128i shift_right = _mm_slli_si128(diff_sq_u16, 2);
  const __m128i shift_left = _mm_srli_si128(diff_sq_u16, 2);
  ...
  // range.
  __m128i sum_u16 = _mm_adds_epu16(shift_left, diff_sq_u16);
...
void vp9_temporal_filter_apply_sse4_1(const uint8_t *a, unsigned int stride,
  ...
  // _mm_srl_epi16 uses a single 64 bit element for the shift.
  ...
  if (width == 8) {
    ...
    sum_row_c = average_8(sum_row_c, mul_constants, strength_u128, rounding_u16,
                          weight_u16);
    ...
    mul_constants = _mm_setr_epi16(NEIGHBOR_CONSTANT_4, NEIGHBOR_CONSTANT_6,
                                   ...
                                   NEIGHBOR_CONSTANT_6, NEIGHBOR_CONSTANT_4);
    sum_row_a = _mm_adds_epu16(sum_row_a, sum_row_b);
    sum_row_a = average_8(sum_row_a, mul_constants, strength_u128, rounding_u16,
                          weight_u16);
    accumulate_and_store_8(sum_row_a, b, count, accumulator);
