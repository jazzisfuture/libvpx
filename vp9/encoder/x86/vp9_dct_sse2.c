...
void fdct8_1d_sse2(int16_t *input, int16_t *output, int stride, int round) {
  // constants
  const __m128i k__cospi_p16_p16 = _mm_set1_epi16(cospi_16_64);
  const __m128i k__cospi_p16_m16 = pair_set_epi16(cospi_16_64, -cospi_16_64);
  const __m128i k__cospi_p24_p08 = pair_set_epi16(cospi_24_64, cospi_8_64);
  ...
  // stage 1
  s0 = _mm_add_epi16(in0, in7);
  s1 = _mm_add_epi16(in1, in6);
  s2 = _mm_add_epi16(in2, in5);
  s3 = _mm_add_epi16(in3, in4);
  s4 = _mm_sub_epi16(in3, in4);
  s5 = _mm_sub_epi16(in2, in5);
  s6 = _mm_sub_epi16(in1, in6);
  s7 = _mm_sub_epi16(in0, in7);
  ...
  res0 = _mm_packs_epi32(u0, u1);
  res2 = _mm_packs_epi32(u4, u5);
  res4 = _mm_packs_epi32(u2, u3);
  res6 = _mm_packs_epi32(u6, u7);
  ...
  res1 = _mm_packs_epi32(v0, v1);
  res3 = _mm_packs_epi32(v4, v5);
  res5 = _mm_packs_epi32(v2, v3);
  ...
  if (round == 1) {
    ...
    res3 = _mm_srai_epi16(res3, 1);
    res4 = _mm_srai_epi16(res4, 1);
    res5 = _mm_srai_epi16(res5, 1);
    res6 = _mm_srai_epi16(res6, 1);
    res7 = _mm_srai_epi16(res7, 1);
  }
  ...
  // transpose
  {
    ...
    // 06 16 26 36 07 17 27 37
    // 46 56 66 76 47 57 67 77
    in0 = _mm_unpacklo_epi64(tr1_0, tr1_1);
    in1 = _mm_unpackhi_epi64(tr1_0, tr1_1);
    in2 = _mm_unpacklo_epi64(tr1_2, tr1_3);
    in3 = _mm_unpackhi_epi64(tr1_2, tr1_3);
    in4 = _mm_unpacklo_epi64(tr1_4, tr1_5);
    in5 = _mm_unpackhi_epi64(tr1_4, tr1_5);
    in6 = _mm_unpacklo_epi64(tr1_6, tr1_7);
    in7 = _mm_unpackhi_epi64(tr1_6, tr1_7);
    // 00 10 20 30 40 50 60 70
    // 01 11 21 31 41 51 61 71
    // 02 12 22 32 42 52 62 72
    // 03 13 23 33 43 53 63 73
  ...
  // write into the 2-D array buffer
  _mm_storeu_si128((__m128i *)(output + 0 * 8), in0);
  _mm_storeu_si128((__m128i *)(output + 1 * 8), in1);
  _mm_storeu_si128((__m128i *)(output + 2 * 8), in2);
  _mm_storeu_si128((__m128i *)(output + 3 * 8), in3);
...
void fadst8_1d_sse2(int16_t *input, int16_t *output, int stride, int round) {
  // Constants
  ...
  // Load input and properly aligned for butterfly input
  __m128i in0  = _mm_loadu_si128((const __m128i *)(input + 7 * stride));
  __m128i in1  = _mm_loadu_si128((const __m128i *)(input + 0 * stride));
  __m128i in2  = _mm_loadu_si128((const __m128i *)(input + 5 * stride));
  __m128i in3  = _mm_loadu_si128((const __m128i *)(input + 2 * stride));
  __m128i in4  = _mm_loadu_si128((const __m128i *)(input + 3 * stride));
  __m128i in5  = _mm_loadu_si128((const __m128i *)(input + 4 * stride));
  __m128i in6  = _mm_loadu_si128((const __m128i *)(input + 1 * stride));
  __m128i in7  = _mm_loadu_si128((const __m128i *)(input + 6 * stride));
  ...
  res0 = _mm_packs_epi32(u0, u1);
  res1 = _mm_packs_epi32(u2, u3);
  res2 = _mm_packs_epi32(u4, u5);
  res3 = _mm_packs_epi32(u6, u7);
  res4 = _mm_packs_epi32(u8, u9);
  res5 = _mm_packs_epi32(u10, u11);
  res6 = _mm_packs_epi32(u12, u13);
  res7 = _mm_packs_epi32(u14, u15);

  // stage 2
  s0 = _mm_add_epi16(res0, res2);
  s1 = _mm_add_epi16(res1, res3);
  s2 = _mm_sub_epi16(res0, res2);
  s3 = _mm_sub_epi16(res1, res3);
  u0 = _mm_unpacklo_epi16(res4, res5);
  u1 = _mm_unpackhi_epi16(res4, res5);
  u2 = _mm_unpacklo_epi16(res6, res7);
  u3 = _mm_unpackhi_epi16(res6, res7);
  ...
  res0 = s0;
  res1 = _mm_sub_epi16(k__const_0, s4);
  res2 = s6;
  res3 = _mm_sub_epi16(k__const_0, s2);
  res4 = s3;
  res5 = _mm_sub_epi16(k__const_0, s7);
  res6 = s5;
...
typedef void (*fhtrans_1d)(int16_t*, int16_t*, int stride, int round);

typedef struct {
  fhtrans_1d cols, rows;  // vertical and horizontal
} fhtrans_2d;

static const fhtrans_2d FHT_8[] = {
  { fdct8_1d_sse2,  fdct8_1d_sse2  },  // DCT_DCT  = 0
  { fadst8_1d_sse2, fdct8_1d_sse2  },  // ADST_DCT = 1
  { fdct8_1d_sse2,  fadst8_1d_sse2 },  // DCT_ADST = 2
  { fadst8_1d_sse2, fadst8_1d_sse2 }   // ADST_ADST = 3
};
...
void vp9_short_fht8x8_sse2(int16_t *input, int16_t *output,
  ...
  int16_t *pbuf = &buf[0];
  int pitch = 8;
  const fhtrans_2d ht = FHT_8[tx_type];
  ht.cols(input, pbuf, stride, 0);
  ht.rows(pbuf, output, pitch, 1);
}
