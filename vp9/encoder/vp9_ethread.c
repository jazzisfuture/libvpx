...
static int first_pass_worker_hook(EncWorkerData *const thread_data,
  ...
  job_node_t *ps_job = NULL;
  FIRSTPASS_DATA fp_acc_data;
  MV zero_mv = {0, 0};
  MV best_ref_mv, lastmv;
  int mb_row;
  ...
  while (0 == end_of_frame) {
    ...
    ps_job =
        (job_node_t *)vp9_enc_grp_get_next_job(multi_thread_ctxt, cur_tile_id);
    if (NULL == ps_job) {
      ...
      tile_col = ps_job->tile_col_id;
      tile_row = ps_job->tile_row_id;

      this_tile = &cpi->tile_data[tile_row * tile_cols + tile_col];
      mb_row = ps_job->vert_unit_row_num;
...
void vp9_encode_fp_row_mt(VP9_COMP *cpi) {
  ...
  if (cpi->multi_thread_ctxt.max_num_tile_cols *
          cpi->multi_thread_ctxt.max_num_tile_rows <
      tile_cols * tile_rows)
    vp9_row_mt_mem_alloc(cpi);

  num_workers = multi_thread_ctxt->num_row_mt_workers;
  create_enc_workers(cpi, num_workers);
  ...
  launch_enc_workers(cpi, (VPxWorkerHook)first_pass_worker_hook,
                     &cpi->multi_thread_ctxt, num_workers);
