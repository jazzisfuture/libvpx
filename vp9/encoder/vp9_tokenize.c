...
static void tokenize_b(VP9_COMP *cpi,
  ...
  do {
    ...
    t->skip_coef_val1 = (token!= DCT_EOB_TOKEN && token != ZERO_TOKEN &&
                         last1_indicated);
    ...
    if (t->skip_coef_val1) {
      assert(t->Token == ONE_TOKEN);
      assert(t->skip_coef_val == 0);
    }
#endif
    if (!dry_run) {
    ...
    if (token == ZERO_TOKEN && !t->skip_coef_val) {
    ...
    } else if (token == ONE_TOKEN && !t->skip_coef_val1) {
#if USE_OPC_LAST1
      ...
      if (use_last1) {
        ...
        t++;
        t->skip_eob_node = t->skip_coef_val = t->skip_coef_val1 = 0;
        t->Token = last1 ? OPC_LAST1 : OPC_NOTLAST1;
        t->context_tree = &((*zpc_probs)[ref]
                            [coef_to_zpc_band(band)]
                            [coef_to_zpc_ptok(pt)][2]);
        if (!dry_run)
          (*zpc_count)[ref]
                      [coef_to_zpc_band(band)]
