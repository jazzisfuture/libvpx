...
static void tokenize_b(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  while (c < eob) {
    ...
    while (!v) {
      add_token_no_extra(&t, coef_probs[b][pt], ZERO_TOKEN, skip_eob,
                         counts[b][pt]);
      eob_branch[b][pt] += !skip_eob;
  ...
  if (c < seg_eob) {
    add_token_no_extra(&t, coef_probs[b][pt], EOB_TOKEN, 0,
                       counts[b][pt]);
    ++eob_branch[b][pt];
  }
