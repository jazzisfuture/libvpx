...
static void tokenize_b(int plane, int block, BLOCK_SIZE plane_bsize,
  ...
  while (c < eob) {
    ...
    while (!v) {
      add_token(&t, coef_probs[type][ref][band][pt], 0, ZERO_TOKEN, skip_eob,
                counts[type][ref][band][pt]);
    ...
    add_token(&t, coef_probs[type][ref][band][pt],
  ...
  if (c < seg_eob) {
    add_token(&t, coef_probs[type][ref][band][pt], 0, EOB_TOKEN, 0,
              counts[type][ref][band][pt]);
    ++cpi->common.counts.eob_branch[tx_size][type][ref][band][pt];
  }
