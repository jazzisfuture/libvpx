...
static void tokenize_b(int plane, int block, BLOCK_SIZE plane_bsize,
...
#if CONFIG_TWO_STAGE
  int two_stg = mbmi->two_stage_coding[plane != 0];
  int eob_stg2 = p->eobs_stg2[block];
  const tran_low_t *qcoeff_stg2 = BLOCK_OFFSET(p->qcoeff_stg2, block);
  int pt_stg2;
  const uint8_t *const band_stg2 =
      USE_SKIP_BAND ? vp9_coefband_tx_skip : get_band_translate(tx_size);
  ...
  *tp = t;
  ...
  pt = pt_stg2;
  so = &vp9_default_scan_orders_pxd[tx_size];
  scan = so->scan;
  nb = so->neighbors;
  c = 0;
  ...
  while (c < eob_stg2) {
    ...
    while (!v) {
      t->is_pxd_token = FOR_SCREEN_CONTENT;
      if (FOR_SCREEN_CONTENT) {
        add_token_no_extra(&t, coef_probs_pxd[pt], ZERO_TOKEN, skip_eob,
                           counts_pxd[pt]);
        eob_branch_pxd[pt] += !skip_eob;
      } else {
        add_token_no_extra(&t, coef_probs[band_stg2[c]][pt], ZERO_TOKEN,
                           skip_eob, counts[band_stg2[c]][pt]);
        eob_branch[band_stg2[c]][pt] += !skip_eob;
      }
      skip_eob = 1;
      token_cache[scan[c]] = 0;
      ++c;
    ...
    if (FOR_SCREEN_CONTENT) {
      ...
      add_token(&t, coef_probs[band_stg2[c]][pt],
                dct_value_tokens[v].extra,
                (uint8_t)dct_value_tokens[v].token,
                (uint8_t)skip_eob,
                counts[band_stg2[c]][pt]);
      eob_branch[band_stg2[c]][pt] += !skip_eob;
    }
    token_cache[scan[c]] = vp9_pt_energy_class[dct_value_tokens[v].token];
    ++c;
  ...
  if (c < seg_eob) {
    ...
    if (FOR_SCREEN_CONTENT) {
      ...
      add_token_no_extra(&t, coef_probs[band_stg2[c]][pt], EOB_TOKEN, 0,
                         counts[band_stg2[c]][pt]);
      ++eob_branch[band_stg2[c]][pt];
    }
  }
