...
void print_extra_bit_costs(vp9_extra_bit *p) {
  // initialize the cost for extra bits for all possible extrabits values.
  int bits = p->len;
  int extra;
  int i;
  for (extra = 0; extra < 2 << bits; extra++) {
    int cost = 0;

    if (p->base_val) {

      if (bits)
        cost += treed_cost(p->tree, p->prob, extra >> 1, bits);
...
static void tokenize_init_one(TOKENVALUE *t, const vp9_extra_bit *const e,
  ...
  int j;
  /*
  for (j=0;j<ENTROPY_TOKENS-2;j++) {
    print_extra_bit_costs(vp9_extra_bits + j);
  }
  for (j=0;j<ENTROPY_TOKENS;j++) {
    print_extra_bit_costs(vp9_extra_bits_high10 + j);
  }
  for (j=0;j<ENTROPY_TOKENS;j++) {
    print_extra_bit_costs(vp9_extra_bits_high12 + j);
  }
  */
  do {
    if (!i)
    ...
    // initialize the cost for extra bits for all possible coefficient value.
    {
      ...
      if (p->base_val) {
        ...
        cost += vp9_cost_bit(vp9_prob_half, extra & 1); /* sign */
        value_cost[i] = cost;
        if (t[i].token < CATEGORY6_TOKEN)
          if (value_cost[i]!=p->cost[extra])
            printf("oops\n");
      }
    }
  } while (++i < max_value);
