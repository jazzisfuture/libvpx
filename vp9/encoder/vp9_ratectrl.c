...
void adjust_gf_boost_lag_one_pass_vbr(VP9_COMP *cpi, uint64_t avg_sad_current,
  ...
  if (cpi->refresh_golden_frame == 1) {
    ...
    // Default values for boost and interval: adjustment below.
    rc->gfu_boost = DEFAULT_GF_BOOST;
    rc->baseline_gf_interval =
        (rc->min_gf_interval + rc->max_gf_interval) / 2;
...
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL &&
    ...
    if (cpi->oxcf.lag_in_frames > 0)
      adjust_gf_boost_lag_one_pass_vbr(cpi, avg_sad_current, flag_scene_change);
  }
}
