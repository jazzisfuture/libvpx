...
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL &&
    ...
    if (cpi->oxcf.lag_in_frames > 0 && cpi->refresh_golden_frame &&
      ...
      if (high_source_sad_lagindex  > 0 && high_source_sad_lagindex < 3) {
        cpi->refresh_golden_frame = 0;
        rc->gfu_boost = 1;
      } else {
        if (high_source_sad_lagindex == -1) {
          // No other scene change within lag adjust gf_int based on content.
          if (avg_source_sad_lag < min_thresh) {
            rc->gfu_boost = DEFAULT_GF_BOOST;
          } else if (avg_source_sad_lag > (min_thresh << 2)) {
            rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
