...
void vp9_rc_postencode_update(VP9_COMP *cpi, uint64_t bytes_used) {
  ...
  printf("postencode:%d %d %d %d %d %d %d \n", cm->frame_type, cm->current_video_frame, cm->width, cm->height, cm->base_qindex, rc->projected_frame_size,
...
void vp9_rc_get_one_pass_cbr_params(VP9_COMP *cpi) {
  ...
  vp9_rc_set_frame_target(cpi, target);
  if (0 && cpi->oxcf.resize_mode == RESIZE_DYNAMIC)
...
int vp9_overshoot_resize(VP9_COMP *cpi, int resize_down) {
  if (resize_down) {
    ...
    // least ~30 frames from slide change.
    if (cpi->rc.buffer_level > cpi->rc.optimal_buffer_level &&
