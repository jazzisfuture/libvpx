...
int vp9_resize_one_pass_cbr(VP9_COMP *cpi) {
  const VP9_COMMON *const cm = &cpi->common;
  RATE_CONTROL *const rc = &cpi->rc;
  RESIZE_DIRECT resize_dir = NO_RESIZE;
  ...
  if (resize_dir != NO_RESIZE) {
    int target_bits_per_frame;
    int active_worst_quality;
    int qindex;
    int tot_scale_change;
    // For now, resize is by 1/2 x 1/2.
    if (resize_dir == ONE2THREEFOUR || resize_dir == ONEHALF2THREEFOUR) {
      cpi->resize_scale_num = 3;
      cpi->resize_scale_den = 4;
    } else if (resize_dir == TOONEHALF) {
      cpi->resize_scale_num = 1;
      cpi->resize_scale_den = 2;
    } else { // TOONE or anything else
      cpi->resize_scale_num = 1;
      cpi->resize_scale_den = 1;
    }
    ...
    target_bits_per_frame = (resize_dir >= 0) ?
