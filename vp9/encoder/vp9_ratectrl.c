...
static int calc_iframe_target_size_one_pass_cbr(const VP9_COMP *cpi) {
  ...
  if (cpi->common.current_video_frame == 0) {
    target = ((oxcf->starting_buffer_level / 2) > INT_MAX)
      ? INT_MAX : (int)(oxcf->starting_buffer_level / 2);
  } else {
    const int initial_boost = 32;
    int kf_boost = MAX(initial_boost, (int)(2 * oxcf->framerate - 16));
    if (rc->frames_since_key < oxcf->framerate / 2) {
