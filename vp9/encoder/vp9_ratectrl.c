...
// Adjust gf interval based on up-coming key frame or up-coming scene change
// (if lag_in_frames > 0).
static void adjust_gf_key_lag(VP9_COMP *cpi, int high_source_sad_lag) {
  RATE_CONTROL *const rc = &cpi->rc;
  int interval_constraint = rc->frames_to_key;
  if (cpi->oxcf.lag_in_frames > 0 && high_source_sad_lag > 0 &&
      high_source_sad_lag < rc->frames_to_key)
    interval_constraint = high_source_sad_lag;
  rc->constrained_gf_group = 0;
  // Reset gf interval to make more equal spacing for up-coming key frame
  // or scene changes.
  if ((interval_constraint <= 7 * rc->baseline_gf_interval >> 2) &&
      (interval_constraint > rc->baseline_gf_interval)) {
    rc->baseline_gf_interval = interval_constraint >> 1;
    rc->constrained_gf_group = 1;
  } else {
    // Reset since frames_till_gf_update_due must be
    // <= frames_to_key/next_scene_cut
    if (rc->baseline_gf_interval > interval_constraint) {
      rc->baseline_gf_interval = interval_constraint;
      rc->constrained_gf_group = 1;
    }
  }
}
void vp9_rc_get_one_pass_vbr_params(VP9_COMP *cpi) {
  ...
  if (rc->frames_till_gf_update_due == 0) {
    ...
    adjust_gf_key_lag(cpi, rc->high_source_sad_lagindex - 1);
...
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL &&
    ...
    if (cpi->oxcf.lag_in_frames > 0) {
      int frames_to_buffer = (cm->current_video_frame == 1) ?
          cpi->oxcf.lag_in_frames - 2: 2;
      start_frame = cpi->oxcf.lag_in_frames - 2;
      frames_to_check = -1;
      ...
      // Shift data back one frame: the avg_sad for this current frame
      // is the value of frame#1 (first future frame) from previous frame.
      if (cpi->oxcf.lag_in_frames > 0) {
        for (frame = 0; frame < cpi->oxcf.lag_in_frames - 1; ++frame) {
          rc->avg_source_sad[frame] = rc->avg_source_sad[frame + 1];
           rc->high_source_sad[frame] = rc->high_source_sad[frame + 1];
        }
      }
    }
    for (frame = 0; frame < frames_to_check; ++frame) {
      ...
      // between current and the previous frame value(s). Use minimum threshold
      // for cases where there is small change from content that is completely
      // static.
      reference_sad = (lagframe_idx == 0) ? rc->avg_source_sad[0] :
          rc->avg_source_sad[lagframe_idx - 1];
      if (avg_sad >
          VPXMAX(min_thresh,
                 (unsigned int)(reference_sad * thresh)) &&
          rc->frames_since_key > 1)
        rc->high_source_sad[lagframe_idx] = 1;
      else
        rc->high_source_sad[lagframe_idx] = 0;
      if (lagframe_idx == 0) {
    ...
    if (cpi->oxcf.lag_in_frames > 0) {
      // Get first future frame high source sad.
      rc->high_source_sad_lagindex = -1;
      for (frame = 0; frame < cpi->oxcf.lag_in_frames; ++frame) {
       const int lagframe_idx = start_frame - frame + 1;
       if (rc->high_source_sad[lagframe_idx] == 1) {
         rc->high_source_sad_lagindex = lagframe_idx;
         frame = cpi->oxcf.lag_in_frames;
       }
      }
    }
    ...
    if (cpi->oxcf.rc_mode == VPX_VBR &&
      ...
      adjust_gf_key_lag(cpi, rc->high_source_sad_lagindex);
      rc->frames_till_gf_update_due = rc->baseline_gf_interval;
      target = calc_pframe_target_size_one_pass_vbr(cpi);
      vp9_rc_set_frame_target(cpi, target);
      rc->count_last_scene_change = 0;
