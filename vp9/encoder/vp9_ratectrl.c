...
static void calc_iframe_target_size(VP9_COMP *cpi) {
  ...
  //cpi->rc.this_frame_target = 5 * cpi->rc.per_frame_bandwidth;
...
void adjust_active_quality_level_from_buffer_level(VP9_COMP *cpi) {
  ...
  int max_adjustment_down = cpi->rc.active_worst_quality / 5;
  ...
  if (cpi->rc.buffer_level > cpi->oxcf.optimal_buffer_level) {
    ...
    if (max_adjustment_down) {
      ...
      cpi->rc.buffer_level > critical_level) {
    // Adjust up from ambient Q.
    if (critical_level) {
      buff_lvl_step = (cpi->oxcf.optimal_buffer_level - critical_level);
      if (buff_lvl_step) {
        adjustment = (cpi->rc.worst_quality - cpi->rc.avg_frame_qindex) *
            (cpi->oxcf.optimal_buffer_level - cpi->rc.buffer_level) /
            buff_lvl_step;
      }
      cpi->rc.active_worst_quality = cpi->rc.avg_frame_qindex + adjustment;
    }
  } else {
    // Set it to worst_quality below critical buffer level.
    cpi->rc.active_worst_quality = cpi->rc.worst_quality;
  }
}
...
int vp9_pick_q_and_adjust_q_bounds(VP9_COMP *cpi,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    if (cpi->oxcf.end_usage == USAGE_CONSTANT_QUALITY) {
      ...
      // 1-pass: use the average Q for the active_best, if its lower
      // than active_worst.
