...
void vp9_rc_get_svc_params(VP9_COMP *cpi) {
  ...
  if (cpi->oxcf.resize_mode == RESIZE_DYNAMIC && svc->single_layer_svc == 1 &&
    ...
    if (cpi->resize_pending) {
      ...
      for (tl  = 0; tl < svc->number_temporal_layers; tl++) {
        lc = &svc->layer_context[svc->spatial_layer_id * svc->number_temporal_layers + tl];
        lc->scaling_factor_num =
            cpi->resize_scale_num * lc->scaling_factor_num_config;
        lc->scaling_factor_den =
            cpi->resize_scale_den * lc->scaling_factor_den_config;
      }
