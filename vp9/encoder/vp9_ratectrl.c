...
void vp9_rc_get_svc_params(VP9_COMP *cpi) {
  ...
  if (svc->simulcast_mode) {
    ...
    // Set the buffer idx and refresh flag on key frames.
    // Buffer slot for long-term reference is set below (line 2255) and alt_ref
    // is used for that on key frame. So use last and golden for the other two
    // normal slots.
    if (cm->frame_type == KEY_FRAME) {
