...
static void calc_pframe_target_size(VP9_COMP *cpi) {
  ...
  if (cpi->rc.frames_till_gf_update_due == 0) {
    const int q = (cpi->oxcf.fixed_q < 0) ? cpi->rc.last_q[INTER_FRAME]
                                          : cpi->oxcf.fixed_q;
...
int vp9_pick_q_and_adjust_q_bounds(VP9_COMP *cpi,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    if (cpi->this_key_frame_forced) {
      ...
      cpi->rc.active_best_quality = get_active_quality(q, cpi->kf_boost,
                                                    low, high,
                                                    kf_low_motion_minq,
                                                    kf_high_motion_minq);
    ...
    if (cpi->oxcf.end_usage == USAGE_CONSTRAINED_QUALITY) {
      ...
      if (cpi->frames_since_key > 1) {
        cpi->rc.active_best_quality = get_active_quality(q, cpi->gfu_boost,
                                                      low, high,
                                                      afq_low_motion_minq,
