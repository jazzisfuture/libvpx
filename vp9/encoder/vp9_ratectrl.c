...
//printf("%d %d \n", cm->current_video_frame, rc->projected_frame_size);
...
void vp9_drop_encodedframe_overshoot(VP9_COMP *cpi,
                                     int frame_size) {
  VP9_COMMON * const cm = &cpi->common;
  RATE_CONTROL *const rc = &cpi->rc;
  int thresh_qp = 3 * (rc->worst_quality >> 2);
  int thresh_rate = rc->avg_frame_bandwidth << 4;
  if (cm->base_qindex < thresh_qp &&
    ...
    // Flag to indicate that we will drop this frame and encode next frame
    // at max QP.
    rc->drop_overshoot = 1;
    rc->force_max_qp = 1;
    return;
