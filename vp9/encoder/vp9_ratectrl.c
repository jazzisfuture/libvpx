...
static int calc_active_worst_quality_one_pass_cbr(const VP9_COMP *cpi) {
  ...
  int adjustment = 0;
  int active_worst_quality;
  // Note for ambient_qp we use rc->avg_frame_qindex[KEY_FRAME/INTER_FRAME],
  // which are computed as (3/4, 1/4) recursive average in postencode_update,
  // so for first frame following key, the qp of that key frame is weighted
