...
void adjust_gf_boost_lag_one_pass_vbr(VP9_COMP *cpi, uint64_t avg_sad_current,
  ...
  uint32_t min_thresh = 40000;
  uint32_t max_thresh = 100000;
  float thresh = 2.1f;
  double rate_err = 1.0;
  ...
  if (cpi->refresh_golden_frame == 1) {
    ...
    if (avg_source_sad_lag < min_thresh &&
      rc->high_source_sad_lagindex == -1) {
      // Larger boost for very low (up-coming) content, no other scene change
      // in lag.
      rc->gfu_boost = DEFAULT_GF_BOOST;
      rc->baseline_gf_interval =
          VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
    } else if (rate_err > 3.5) {
      // Lower the boost, keep high interval for high overshoot.
      rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
      rc->baseline_gf_interval =
          VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
    } else if (avg_source_sad_lag > max_thresh ||
               rc->avg_frame_low_motion < 20) {
      // Smaller boost and smaller interval for high (up-coming) content.
      rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
      rc->baseline_gf_interval = VPXMAX(5, rc->baseline_gf_interval >> 1);
    }
    ...
    rc->fac_active_best = 100;
    if (cm->current_video_frame > 30) {
      if (rate_err < 2.0 && avg_source_sad_lag < min_thresh &&
         !is_scene_change) {
        // Lower factor if we are not overshooting too much, and up-coming
        // content is low.
        rc->fac_active_worst = 120;
        rc->fac_active_best = 80;
      } else if (rate_err > 3.0 || avg_source_sad_lag > max_thresh ||
                is_scene_change) {
        // Increase factor for high overshoot or if high content.
