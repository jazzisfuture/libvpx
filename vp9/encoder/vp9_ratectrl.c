...
static void calc_pframe_target_size(VP9_COMP *cpi) {
  ...
  if (cpi->refresh_golden_frame) {
    assert(cpi->fs.frames_till_gf_update_due == cpi->fs.baseline_gf_interval);
    ...
    // Jims modified boost
    if (!cpi->fs.source_alt_ref_active) {
...
int vp9_rc_pick_q_and_adjust_q_bounds(const VP9_COMP *cpi,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    if (cpi->fs.this_key_frame_forced) {
      int delta_qindex;
...
#endif
  } else if (!cpi->fs.is_src_frame_alt_ref &&
             (cpi->refresh_golden_frame || cpi->refresh_alt_ref_frame)) {

    // Use the lower of active_worst_quality and recent
    // average Q as basis for GF/ARF best Q limit unless last frame was
    // a key frame.
    if (cpi->fs.frames_since_key > 1 &&
  ...
  *bottom_index = active_best_quality;
  ...
  if (cm->frame_type == KEY_FRAME && !cpi->fs.this_key_frame_forced) {
  ...
  } else if (cm->frame_type == KEY_FRAME && cpi->fs.this_key_frame_forced) {
  ...
  if ((cm->base_qindex < cpi->rc.last_boosted_qindex) ||
      ((cpi->static_mb_pct < 100) &&
       ((cm->frame_type == KEY_FRAME) || cpi->refresh_alt_ref_frame ||
        (cpi->refresh_golden_frame && !cpi->fs.is_src_frame_alt_ref)))) {
