  ...
  if (cpi->common.frame_type == KEY_FRAME) {
    ...
    if (!rc->is_src_frame_alt_ref &&
      ...
      active_worst_quality =  curr_frame == 1 ? rc->last_q[KEY_FRAME] * 5 >> 2 :
          VPXMIN(rc->last_q[INTER_FRAME],
            rc->avg_frame_qindex[INTER_FRAME]) * rc->fac_active_worst_gf / 100;
    } else {
...
static int calc_pframe_target_size_one_pass_vbr(const VP9_COMP *const cpi) {
  const RATE_CONTROL *const rc = &cpi->rc;
  int af_ratio = rc->af_ratio_one_pass_vbr;
  int target;
...
void adjust_gf_boost_lag_one_pass_vbr(VP9_COMP *cpi, uint64_t avg_sad_current) {
  ...
  int high_source_sad_lagindex2 = -1;
  ...
  for (frame = tot_frames; frame >= 1; --frame) {
    ...
    // high_source_sad_lagindex detects up-coming scene change.
    if (!found &&
    ...
    // high_source_sad_lagindex2 for detecting content changes, that
    // may not be as significant as scene change.
    if (rc->avg_source_sad[lagframe_idx] > VPXMAX(sad_thresh1 >> 1,
        (unsigned int)(reference_sad * 1.6)))
      high_source_sad_lagindex2 = lagframe_idx;
  ...
  if (cpi->refresh_golden_frame == 1 &&
      ...
      cpi->oxcf.lag_in_frames > 6) {
    int frame_constraint;
    // No gf boost if scene change (or key frame) is coming within a few frames.
    if ((rc->high_source_sad_lagindex > 0 &&
         rc->high_source_sad_lagindex < 3) || rc->frames_to_key < 3) {
    ...
    if (rc->rolling_target_bits > 0)
      rate_err =
              (double)rc->rolling_actual_bits / (double)rc->rolling_target_bits;
    if (avg_source_sad_lag < sad_thresh1 &&
        high_source_sad_lagindex == -1 &&
        high_source_sad_lagindex2 == -1) {
      rc->gfu_boost = DEFAULT_GF_BOOST;
      rc->baseline_gf_interval =
          VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
    } else if (avg_source_sad_lag > sad_thresh2) {
      // Smaller boost and interval for high (up-coming) content.
      rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
      rc->baseline_gf_interval = VPXMAX(5, rc->baseline_gf_interval >> 1);
    }
    ...
    // fac_active_worst/best scales the avg_frame_qindex.
    rc->fac_active_worst_inter = 150;  // corresponds to 3/2 (= 150 /100).
    rc->fac_active_worst_gf = 100;
    rc->af_ratio_one_pass_vbr = 10;
    if (rate_err < 1.5 && avg_source_sad_lag < sad_thresh2 &&
        high_source_sad_lagindex == -1 && high_source_sad_lagindex2 == -1) {
      // Lower factor if not overshooting too much, and up-coming content is
      // not high, and no big changes in content.
      rc->fac_active_worst_inter = 130;
      rc->fac_active_worst_gf = 90;
    } else if (rate_err > 2.5 && avg_source_sad_lag  > sad_thresh1) {
      // Increase factor for high overshoot and up-coming content is not low.
      rc->fac_active_worst_inter = 170;
      rc->fac_active_worst_gf = 120;
    }
    if (avg_source_sad_lag < sad_thresh1 && rc->avg_frame_low_motion > 90)
      rc->af_ratio_one_pass_vbr = 15;
    else if (rc->avg_frame_low_motion < 60)
      rc->af_ratio_one_pass_vbr = 6;
    target = calc_pframe_target_size_one_pass_vbr(cpi);
    vp9_rc_set_frame_target(cpi, target);
  }
}
