...
static void calc_iframe_target_size(VP9_COMP *cpi) {
  ...
  if (cpi->pass == 0 && oxcf->end_usage == USAGE_STREAM_FROM_SERVER) {
    if (cpi->common.current_video_frame == 0) {
      target = (int)(oxcf->starting_buffer_level / 2);
    } else {
...
void vp9_update_buffer_level(VP9_COMP *const cpi, int encoded_frame_size) {
  ...
  rc->buffer_level = (int)MIN(rc->bits_off_target, oxcf->maximum_buffer_size);
}
...
static int adjust_active_worst_quality_from_buffer_level(const VP9_CONFIG *oxcf,
  ...
  int critical_level = (int)(oxcf->optimal_buffer_level >> 2);
  int adjustment = 0;
  int buff_lvl_step = 0;
  if (rc->buffer_level > oxcf->optimal_buffer_level) {
    ...
    if (critical_level) {
      buff_lvl_step = (int)(oxcf->optimal_buffer_level - critical_level);
      if (buff_lvl_step) {
        adjustment = 
...
static int target_size_from_buffer_level(const VP9_CONFIG *oxcf,
  ...
  const int one_pct_bits = (int)(1 + oxcf->optimal_buffer_level / 100);
