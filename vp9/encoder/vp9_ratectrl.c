  ...
  *bottom_index = active_best_quality;
  ...
  // Special case code to try and match quality with forced key frames
    ...
    q = vp9_rc_regulate_q(cpi, rc->this_frame_target,
                          active_best_quality, active_worst_quality);
    /*
    printf("q/t, active_best/worst_quality = %d/%d, %d/%d\n",
           q, rc->this_frame_target, active_best_quality, active_worst_quality);
           */
    if (q > *top_index) {
  ...
  if (cpi->common.frame_type == KEY_FRAME) {
    ...
    if (!cpi->rc.is_src_frame_alt_ref &&
      ...
      if (cpi->common.current_video_frame == 1) {
        active_worst_quality = cpi->rc.last_q[KEY_FRAME] * 3 / 2;
      } else {
        // Choose active worst quality twice as large as the last q.
