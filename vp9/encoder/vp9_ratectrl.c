...
// non-rd mode (based on last reconstructed frame). Should try to reuse these
// computations.
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL) {
    ...
    // Loop over sub-sample of frame, and compute average 64x64 block sad,
    // between source and last_source.
    int avg_sad = 0;
    int num_samples = 0;
    ...
    // current and the previous frame value. Use a minimum threshold for cases
    // where there is small change from content that is completely static.
    if (avg_sad > MAX(1000, (rc->avg_source_sad << 3)) &&
        rc->frames_since_key > 1)
      rc->force_max_qp = 1;
    else
      rc->force_max_qp = 0;
    rc->avg_source_sad = avg_sad;
    // TODO(marpan): Test factoring this directly into rate control/QP setting.
    rc->norm_source_sad = MIN(10.0, (float)(avg_sad) / 4096);
  }
}
