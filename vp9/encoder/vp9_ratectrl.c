...
int vp9_resize_one_pass_cbr(VP9_COMP *cpi, int *scale_num, int *scale_den) {
  const VP9_COMMON *const cm = &cpi->common;
  RATE_CONTROL *const rc = &cpi->rc;
  int resize_now = 0;
  ...
  if (resize_now != 0) {
    ...
    int active_worst_quality;
    int qindex;
    int tot_scale_change = (*scale_den * (*scale_den)) /
        (*scale_num * (*scale_num));
