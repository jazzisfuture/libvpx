...
static void adjust_qp_lag(const VP9_COMP *cpi, int *q) {
  // Adjust qp based on content of current and future frames(s).
  // For now apply a clip to big changes in Q, on inter-frames.
  if (!(cpi->refresh_golden_frame || cpi->refresh_alt_ref_frame) &&
      cpi->rc.frames_since_key > 10) {
    int sad_thresh = 60000;  // ~16 * 64 *64
    int delta_qp_thresh_low = 10;
    int delta_qp_thresh_high = 20;
    // Take average of current and next few frames.
    int avg_source_sad = (cpi->rc.avg_source_sad[0] +
        cpi->rc.avg_source_sad[1] + cpi->rc.avg_source_sad[2] +
        cpi->rc.avg_source_sad[3]) >> 2;
    // Big increase in Q: if the content is low we should be able to afford
    // lower Q, so reduce the increase.
    if (*q - cpi->rc.last_q[INTER_FRAME] > delta_qp_thresh_low) {
      if (avg_source_sad < sad_thresh)
        *q = cpi->rc.last_q[INTER_FRAME] +
           ((*q - cpi->rc.last_q[INTER_FRAME]) >> 1);
    } else if (cpi->rc.last_q[INTER_FRAME] - *q > delta_qp_thresh_high) {
      // Big decrease in Q: if the content is somewhat high reduce the decrease
      // to avoid possible big overshoot.
      if (avg_source_sad > (sad_thresh << 1))
        *q = cpi->rc.last_q[INTER_FRAME] -
            ((cpi->rc.last_q[INTER_FRAME] - *q) >> 1);
    }
  }
}
...
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL &&
    ...
    // Use lagframe_idx = 0 for this (current frame).
    if (cpi->oxcf.rc_mode == VPX_VBR &&
        cm->frame_type != KEY_FRAME &&
        rc->high_source_sad &&
        rc->frames_to_key > 3 &&
    ...
    // Note: this will overwrite previous settings for any gf update.
    if (cpi->oxcf.lag_in_frames > 0) {
      int target;
      int found = 0;
      uint64_t avg_source_sad_lag = 0;
      ...
      for (frame = tot_frames; frame >= 1; --frame) {
        ...
        if (!found &&
            rc->avg_source_sad[lagframe_idx] > VPXMAX(min_thresh,
            (unsigned int)(reference_sad * thresh))) {
      ...
      if (cpi->refresh_golden_frame == 1 && cm->frame_type != KEY_FRAME) {
        // Default values for boost and interval: adjustment below based on
        // past/future content.
        rc->gfu_boost = DEFAULT_GF_BOOST;
        rc->baseline_gf_interval =
            (rc->min_gf_interval + rc->max_gf_interval) / 2;
        // No gf boost if scene change is coming within a few frames.
        if (rc->high_source_sad_lagindex  > 0 &&
            rc->high_source_sad_lagindex < 2) {
          cpi->refresh_golden_frame = 0;
          rc->gfu_boost = 1;
        } else  if (flag_scene_change) {
          rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
          // Scene cut: adjust gf boost/interval based on future data.
          if (avg_source_sad_lag < (min_thresh << 1) &&
              rc->high_source_sad_lagindex == -1) {
            // Larger boost for low content, no other scene change in lag.
            rc->gfu_boost = DEFAULT_GF_BOOST;
            rc->baseline_gf_interval =
                VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
          } else if (avg_source_sad_lag > (min_thresh << 1)) {
            // Smaller boost and more frequent update for higher content.
            rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
            rc->baseline_gf_interval = VPXMIN(6, rc->baseline_gf_interval >> 1);
          }
        } else if (cm->current_video_frame > 30 &&
          ...
          rc->baseline_gf_interval=
              VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
                   ...
                   rc->avg_frame_low_motion < 20) {
          // Decrease boost and gf interval for high (past) motion.
          rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
          rc->baseline_gf_interval = VPXMIN(6, rc->baseline_gf_interval >> 1);
        }
        // Check for constraining gf_interval for up-coming scene change,
        // or up-coming key frame, whichever is closer.
        if (cpi->refresh_golden_frame == 1) {
          int frame_constraint = rc->frames_to_key;
          if (rc->high_source_sad_lagindex > 0 &&
              rc->frames_to_key > rc->high_source_sad_lagindex)
