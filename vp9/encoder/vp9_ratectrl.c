...
static void calc_pframe_target_size(VP9_COMP *cpi) {
  ...
  // allowed. That is, having allowed for KF and GF penalties we have not
  // pushed the current interframe target too low. If the adjustment we apply
  // here is not capable of recovering all the extra bits we have spent in the
  // KF or GF then the remainder will have to be recovered over a longer time
  // span via other buffer / rate control mechanisms.
