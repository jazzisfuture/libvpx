...
void vp9_rc_postencode_update(VP9_COMP *cpi, uint64_t bytes_used) {
  ...
  printf("%d %d %d %d %ld %ld %d %d \n", cm->current_video_frame, svc->spatial_layer_id, 
    svc->temporal_layer_id, cm->base_qindex, oxcf->target_bandwidth, rc->buffer_level, 
    lf->filter_level, bytes_used);
  ...
  if (oxcf->pass == 0) {
    // HACK
    if (0 && cpi->rc.compute_frame_motion_pass0 &&
        !frame_is_intra_only(cm) &&
        (!cpi->use_svc ||
         (cpi->use_svc &&
