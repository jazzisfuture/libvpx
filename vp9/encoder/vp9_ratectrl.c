...
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL &&
    ...
    if (cpi->oxcf.lag_in_frames > 0 && cpi->refresh_golden_frame == 1 &&
        cm->frame_type != KEY_FRAME) {
      int target;
      int found = 0;
      ...
      for (frame = 0; frame < tot_frames; ++frame) {
        const int lagframe_idx = tot_frames - frame;
         if (!found &&
             rc->avg_source_sad[lagframe_idx] > VPXMAX(min_thresh,
             (unsigned int)(rc->avg_source_sad[lagframe_idx - 1] * thresh))) {
      ...
      if (high_source_sad_lagindex  > 0 && high_source_sad_lagindex < 2) {
      ...
      if (cpi->refresh_golden_frame == 1) {
        ...
        if (high_source_sad_lagindex > 0 &&
          ...
          frame_constraint = high_source_sad_lagindex;
