...
int vp9_rc_drop_frame(VP9_COMP *cpi) {
  ...
  if (cpi->use_svc) {
    // If we have dropped more than max_consec_drop, then we don't
    // drop this spatial layer.
    if (svc->drop_count[svc->spatial_layer_id] >
        svc->max_consec_drop[svc->spatial_layer_id]) {
      svc->drop_count[svc->spatial_layer_id] = 0;
      return 0;
    } else {
      drop_frames_water_mark =
          svc->framedrop_thresh[svc->spatial_layer_id];
    }
  }
