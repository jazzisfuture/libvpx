...
static int calc_active_worst_quality_one_pass_cbr(const VP9_COMP *cpi) {
  ...
  // For SVC: when inter-layer is off, on the second superframe (superframe = 1)
  // use the QP from the previous superframe. This needed to maintain quality
  // since we can't temporally predict from the very first superframe.
...
void vp9_rc_get_svc_params(VP9_COMP *cpi) {
  ...
  int first_frame_iskey = (cpi->svc.disable_inter_layer_pred == INTER_LAYER_PRED_OFF) ?
      (cpi->svc.current_superframe == 0) : (cm->current_video_frame == 0);
  if (first_frame_iskey || (cpi->frame_flags & FRAMEFLAGS_KEY) ||
    ...
    if (cpi->svc.disable_inter_layer_pred == INTER_LAYER_PRED_OFF &&
        cpi->svc.current_superframe == 1 && cpi->svc.number_spatial_layers > 1 &&
        cpi->svc.spatial_layer_id == 0) {
