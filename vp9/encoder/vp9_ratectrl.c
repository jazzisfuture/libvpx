...
static int rc_pick_q_and_bounds_one_pass_vbr(const VP9_COMP *cpi,
  ...
  // Temporary hack for testing.
   if (oxcf->lag_in_frames > 0 && oxcf->rc_mode == VPX_VBR &&
       rc->force_qpmin > 0 && active_best_quality < rc->force_qpmin) {
    active_best_quality = rc->force_qpmin;
   }
  */
...
void adjust_gf_boost_lag_one_pass_vbr(VP9_COMP *cpi, uint64_t avg_sad_current) {
  ...
  if (cpi->refresh_golden_frame == 1 &&
    ...
    very_low_content = high_source_sad_lagindex == -1 &&
        avg_source_sad_lag < (sad_thresh1 >> 2);
    if (low_content) {
      rc->gfu_boost = DEFAULT_GF_BOOST;
      rc->baseline_gf_interval =
          VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
    ...
    } else if (rate_err > 2.5 && high_content) {
    ...
    // Temporary hack for testing:
    /*
     if (rate_err < 1.2 && very_low_content && rc->avg_frame_low_motion > 90)
      rc->force_qpmin = 32;
    */
    target = calc_pframe_target_size_one_pass_vbr(cpi);
    vp9_rc_set_frame_target(cpi, target);
