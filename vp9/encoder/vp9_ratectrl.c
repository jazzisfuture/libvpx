...
static int calc_pframe_target_size_one_pass_cbr(const VP9_COMP *cpi) {
  ...
  if ((svc->number_temporal_layers > 1 || svc->number_spatial_layers > 1) &&
    ...
    int current_layer = svc->spatial_layer_id * svc->number_temporal_layers +
...
static int calc_iframe_target_size_one_pass_cbr(const VP9_COMP *cpi) {
  ...
  if (cpi->common.current_video_frame == 0) {
    ...
    if (svc->number_temporal_layers > 1 &&
      ...
      const LAYER_CONTEXT *lc = &svc->layer_context[svc->spatial_layer_id *
          svc->number_temporal_layers + svc->temporal_layer_id];
  ...
  if ((cm->current_video_frame == 0) ||
    ...
    if (is_two_pass_svc(cpi)) {
      cpi->svc.layer_context[cpi->svc.spatial_layer_id *
          ...
          cpi->svc.temporal_layer_id].is_key_frame = 1;
      cpi->ref_frame_flags &=
    ...
    if (is_two_pass_svc(cpi)) {
      ...
      if (cpi->svc.spatial_layer_id == 0) {
        ...
        lc->is_key_frame = cpi->svc.layer_context[cpi->svc.spatial_layer_id *
            cpi->svc.number_temporal_layers].is_key_frame;
        if (lc->is_key_frame)
