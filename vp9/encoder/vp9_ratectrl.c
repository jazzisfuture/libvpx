...
static int rc_pick_q_and_bounds_one_pass_vbr(const VP9_COMP *cpi,
  ...
  if (frame_is_intra_only(cm)) {
    ...
    if (oxcf->rc_mode == VPX_Q) {
      ...
      // Use the min of the average Q (with some increase) and active_worst_quality
      // as basis for active_best.
      if (cm->current_video_frame > 1) {
        q = VPXMIN((9 * rc->avg_frame_qindex[INTER_FRAME]) >> 3), active_worst_quality);
