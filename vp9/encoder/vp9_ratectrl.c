...
void vp9_rc_get_one_pass_vbr_params(VP9_COMP *cpi) {
  ...
  if (rc->frames_till_gf_update_due == 0) {
    ...
    if (cpi->oxcf.lag_in_frames > 0) {
      // If lag_inFrames is used, gf update should not cross scene change.
      if (rc->high_source_sad_lagindex > 0 &&
          rc->high_source_sad_lagindex < rc->frames_to_key &&
          rc->frames_till_gf_update_due > rc->high_source_sad_lagindex) {
        rc->frames_till_gf_update_due = rc->high_source_sad_lagindex;
      }
    }
  ...
  if (cpi->Last_Source != NULL &&
    ...
    if (cpi->oxcf.rc_mode == VPX_VBR &&
      ...
      if (rc->frames_till_gf_update_due > rc->frames_to_key)
        rc->frames_till_gf_update_due = rc->frames_to_key;
      if (cpi->oxcf.lag_in_frames > 0) {
        // If lag_in_frames is used, gf update should not cross scene change.
        if (rc->high_source_sad_lagindex > 0 &&
            rc->high_source_sad_lagindex < rc->frames_to_key &&
            rc->frames_till_gf_update_due > rc->high_source_sad_lagindex) {
          rc->frames_till_gf_update_due = rc->high_source_sad_lagindex;
