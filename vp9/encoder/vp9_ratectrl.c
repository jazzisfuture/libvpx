...
static void adjust_qp_one_pass_vbr_lag(const VP9_COMP *cpi, int *q) {
  ...
  if (!(cpi->refresh_golden_frame || cpi->refresh_alt_ref_frame)) {
    ...
    int avg_source_sad = (rc->avg_source_sad[0] + rc->avg_source_sad[1] +
        rc->avg_source_sad[2] + rc->avg_source_sad[3]) >> 2;
    if (*q - rc->last_q[INTER_FRAME] > delta_qp_thresh_low) {
      ...
      if (avg_source_sad < sad_thresh && cpi->rc.high_source_sad == 0) {
        *q = rc->last_q[INTER_FRAME] +
           ((*q - rc->last_q[INTER_FRAME]) >> 1);
      }
    } else if (rc->last_q[INTER_FRAME] - *q > delta_qp_thresh_high) {
      // Big decrease in Q: if the content has increased significantly from the
      // last frame, reduce the decrease to avoid possible big overshoot.
      if (rc->avg_source_sad[0] > ((5 * rc->avg_source_sad_prev) >> 2) ||
...
int vp9_rc_regulate_q(const VP9_COMP *cpi, int target_bits_per_frame,
  ...
  if (cpi->oxcf.lag_in_frames > 0 &&
      ...
      cpi->oxcf.speed >= 5)
    adjust_qp_one_pass_vbr_lag(cpi, &q);
...
void vp9_avg_source_sad(VP9_COMP *cpi) {
  ...
  if (cpi->Last_Source != NULL &&
    ...
    if (cpi->oxcf.lag_in_frames > 0) {
      ...
      rc->avg_source_sad_prev = rc->avg_source_sad[0];
      if (rc->avg_source_sad[1] > VPXMAX(min_thresh,
          (unsigned int)(rc->avg_source_sad[0] * thresh)) &&
          cm->current_video_frame > cpi->oxcf.lag_in_frames)
        rc->high_source_sad = 1;
      else
        rc->high_source_sad = 0;
    ...
    if (cpi->oxcf.lag_in_frames > 0) {
      ...
      if (cpi->refresh_golden_frame == 1) {
        ...
        if (rc->high_source_sad_lagindex  > 0 &&
            rc->high_source_sad_lagindex < 3) {
          cpi->refresh_golden_frame = 0;
          rc->gfu_boost = 1;
        } else  if (flag_scene_change) {
          rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
          // Scene cut: adjust gf boost/interval based on current/future data.
          if (avg_source_sad_lag < min_thresh &&
              avg_sad_current < (min_thresh << 1) &&
              rc->high_source_sad_lagindex == -1) {
            ...
            // Smaller boost and smaller interval update for higher content.
            rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
          ...
          rc->baseline_gf_interval =
              VPXMIN(15, (3 * rc->baseline_gf_interval) >> 1);
                   ...
                   rc->avg_frame_low_motion < 20) {
          // Decrease boost and gf interval for high motion.
          rc->gfu_boost = DEFAULT_GF_BOOST >> 1;
          rc->baseline_gf_interval = VPXMAX(5, rc->baseline_gf_interval >> 1);
        }
