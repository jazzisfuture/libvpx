...
int vp9_encodedframe_overshoot(VP9_COMP *cpi, int frame_size, int *q) {
  ...
  if ((sf->overshoot_detection_cbr_rt == FAST_DETECTION_MAXQ ||
    ...
    *q = cpi->rc.worst_quality;
    ...
    // For FULL_SUPERFRAME_DROP mode with no inter-layer prediction on delta frames:
    // reset and force max-q for all spatial layers, to avoid excessive frame drops.
    if (cpi->use_svc) {
      int tl = 0;
      int sl = 0;
      SVC *svc = &cpi->svc;
