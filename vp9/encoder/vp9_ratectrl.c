...
void vp9_rc_init_minq_luts(VP9_COMP *cpi) {
  int i;

  for (i = 0; i < QINDEX_RANGE; i++) {
    const double maxq = vp9_convert_qindex_to_q(i);

    // kf_low_motion_minq[i] = get_minq_index(maxq, 0.00, -0.0, 0.130);
    kf_low_motion_minq[i] = get_minq_index(maxq, 0.000001, -0.0004, 0.150);
    // kf_high_motion_minq[i] = get_minq_index(maxq, 0.0, -0.0, 0.375);
    kf_high_motion_minq[i] = get_minq_index(maxq, 0.0000021, -0.00125, 0.55);
    // arfgf_low_motion_minq[i] = get_minq_index(maxq, 0.0, -0.0, 0.24);
    arfgf_low_motion_minq[i] = get_minq_index(maxq, 0.0000015, -0.0009, 0.30);
    // arfgf_high_motion_minq[i] = get_minq_index(maxq, 0.0, -0.0, 0.375);
    arfgf_high_motion_minq[i] = get_minq_index(maxq, 0.0000021, -0.00125, 0.55);
    // inter_minq[i] = get_minq_index(maxq, 0.0, -0.0, 0.85);
  ...
  if (frame_is_intra_only(cm) || vp9_is_upper_layer_key_frame(cpi)) {
    ...
    if (rc->this_key_frame_forced) {
      ...
      if (cpi->twopass.last_kfgroup_zeromotion_pct >= STATIC_MOTION_THRESH) {
        ...
        last_boosted_q = vp9_convert_qindex_to_q(qindex);
