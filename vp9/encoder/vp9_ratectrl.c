...
// the temporal layers. Use the timestamp(pts) delta for the framerate when
// the set_ref_frame_config is used.
// The "sgn" parameter is to indicate whether we want to fillup(positive) or
// leak(negative). We always want to fillup for pre-encode, but there is a
// special case where we want to leak on dropped frame in SVC mode.
static void update_buffer_level_svc_preencode(VP9_COMP *cpi, int sgn) {
  ...
  for (i = svc->temporal_layer_id; i < svc->number_temporal_layers; ++i) {
    ...
    if (cpi->svc.use_set_ref_frame_config && ts_delta > 0 &&
      ...
      lrc->bits_off_target +=
          sgn * (int)(lc->target_bandwidth / lc_framerate_pts);
    } else {
      lrc->bits_off_target += sgn * (int)(lc->target_bandwidth / lc->framerate);
    }
...
void vp9_rc_postencode_update_drop_frame(VP9_COMP *cpi) {
  ...
  // has buffer underflow (below threshold). But in the pre-encode step we
  // always increment the buffer level for all layers. This can then lead to
  // increasing buffer levels/overflow for certain layers even though whole
  // superframe is dropped. This in turn may lead to more frame drops since QP
  // is adjusted downwards with buffer overflow. So here we decrement to offset
  // the increment in the pre-encode step.
  if (cpi->use_svc && cpi->svc.framedrop_mode != LAYER_DROP &&
      cpi->rc.buffer_level > cpi->rc.optimal_buffer_level)
    update_buffer_level_svc_preencode(cpi, -1);
}
...
void vp9_rc_get_svc_params(VP9_COMP *cpi) {
  ...
  vp9_rc_set_frame_target(cpi, target);
  if (cm->show_frame) update_buffer_level_svc_preencode(cpi, 1);
}
