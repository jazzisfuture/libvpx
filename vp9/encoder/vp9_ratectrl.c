...
static int rc_pick_q_and_bounds_one_pass_vbr(const VP9_COMP *cpi,
  ...
  if (oxcf->lag_in_frames > 0 && oxcf->rc_mode == VPX_VBR &&
    ...
    if (rc->force_qpmax < MAXQ && active_worst_quality > rc->force_qpmax)
      active_worst_quality = clamp(active_worst_quality,
                                   active_best_quality, rc->force_qpmax);
  }
#endif
...
void adjust_gf_boost_lag_one_pass_vbr(VP9_COMP *cpi, uint64_t avg_sad_current) {
  ...
  if (cpi->refresh_golden_frame == 1 &&
    ...
    rc->force_qpmin = 0;
    rc->force_qpmax = MAXQ;
     if (low_content && rc->avg_frame_low_motion > 90 &&
         avg_source_sad_lag < (sad_thresh1 >> 2))
      rc->force_qpmin = 40;
     if (high_content && rc->avg_frame_low_motion < 30 &&
         avg_source_sad_lag > sad_thresh2)
      rc->force_qpmax = 208;
     printf("%d %d \n", rc->force_qpmin, rc->force_qpmax);
#endif
    target = calc_pframe_target_size_one_pass_vbr(cpi);
    vp9_rc_set_frame_target(cpi, target);
  }
