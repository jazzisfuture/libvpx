...
static void update_buffer_level_per_frame_bandwidth(VP9_COMP *cpi) {
...
static void update_buffer_level_per_frame_bandwidth_svc(VP9_COMP *cpi) {
  ...
  for (i = svc->temporal_layer_id; i < svc->number_temporal_layers; ++i) {
    ...
    if (cpi->svc.use_set_ref_frame_config && ts_delta > 0 &&
      ...
      double framerate_pts = 10000000.0 / ts_delta;
      double lc_framerate_pts = framerate_pts / cpi->oxcf.ts_rate_decimator[i];
...
// temporal layer.
static void update_layer_buffer_level_encoded(SVC *svc,
...
static void update_buffer_level_encoded(VP9_COMP *cpi, 
                                        int encoded_frame_size) {
  RATE_CONTROL *const rc = &cpi->rc;
  ...
  if (is_one_pass_cbr_svc(cpi)) {
    update_layer_buffer_level_encoded(&cpi->svc, encoded_frame_size);
  }
}
...
void vp9_rc_postencode_update(VP9_COMP *cpi, uint64_t bytes_used) {
  ...
  update_buffer_level_encoded(cpi, rc->projected_frame_size);
...
void vp9_rc_get_svc_params(VP9_COMP *cpi) {
  ...
  if (cm->show_frame) update_buffer_level_per_frame_bandwidth_svc(cpi);
}
...
void vp9_rc_get_one_pass_cbr_params(VP9_COMP *cpi) {
  ...
  if (cm->show_frame) update_buffer_level_per_frame_bandwidth(cpi);
