...
static void encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  // For other cases (e.g., CBR mode) use it for speed < 8 for now (need to
  // check encoding time cost for doing this on mobile devices).
  if (cpi->compute_source_sad_onepass &&
      (cpi->oxcf.rc_mode == VPX_VBR ||
       cpi->oxcf.content == VP9E_CONTENT_SCREEN || cpi->oxcf.speed < 8))
