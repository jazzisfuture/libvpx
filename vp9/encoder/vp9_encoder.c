...
static int scale_down(VP9_COMP *cpi, int q) {
  ...
  if (rc->frame_size_selector == 0 &&
      ...
      // TODO(agrange) Re-enable this test when we don't automatically try
      // to encode a KF/GF/ARF full-size.
...
static int scale_up(VP9_COMP *cpi, int q) {
  ...
  (void)q;
  assert(frame_is_kf_gf_arf(cpi));
  if (rc->frame_size_selector == 1) {
    // TODO(agrange) Add the logic to test whether to scale back up.
  }
  return scale;
}
...
static int recode_loop_test(VP9_COMP *cpi,
  ...
  if ((cpi->sf.recode_loop == ALLOW_RECODE) ||
    ...
    if (frame_is_kfgfarf && oxcf->resize_mode == RESIZE_DYNAMIC) {
      // Check whether we need to code this group at a different resolution.
      RATE_CONTROL *const rc = &cpi->rc;
