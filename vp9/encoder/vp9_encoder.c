...
int compute_context_model_thresh(VP9_COMP *cpi) {
  VP9_COMMON *const cm = &cpi->common;
  const VP9EncoderConfig *const oxcf = &cpi->oxcf;
  const int frame_size = (cm->width * cm->height) >> 10;
  const int bitrate = (oxcf->target_bandwidth) >> 10;
  const int qindex_factor = cm->base_qindex + (MAXQ >> 1);
  int thresh = ((FRAME_SIZE_FACTOR * frame_size - FRAME_RATE_FACTOR * bitrate) *
                qindex_factor) >>
               9;
...
int compute_context_model_diff(const VP9_COMMON *const cm) {
  ...
  for (i = 0; i < BLOCK_SIZE_GROUPS; ++i) {
    for (j = 0; j < INTRA_MODES - 1; ++j) {
      diff += (int)counts->y_mode[i][j] *
              ((int)pre_fc->y_mode_prob[i][j] - (int)cur_fc->y_mode_prob[i][j]);
    }
    ...
    diff += (int)counts->y_mode[i][INTRA_MODES - 1] *
            ((int)pre_last_prob - (int)cur_last_prob);
  }

  // uv_mode_prob
  for (i = 0; i < INTRA_MODES; ++i) {
    for (j = 0; j < INTRA_MODES - 1; ++j) {
      diff += (int)counts->uv_mode[i][j] * ((int)pre_fc->uv_mode_prob[i][j] -
                                            (int)cur_fc->uv_mode_prob[i][j]);
    }
  ...
  for (i = 0; i < PARTITION_CONTEXTS; ++i) {
    for (j = 0; j < PARTITION_TYPES - 1; ++j) {
      diff +=
          (int)counts->partition[i][j] * ((int)pre_fc->partition_prob[i][j] -
                                          (int)cur_fc->partition_prob[i][j]);
    }
          ...
          for (m = 0; m < BAND_COEFF_CONTEXTS(l); ++m) {
            for (n = 0; n < UNCONSTRAINED_NODES; ++n) {
              diff += (int)counts->coef[i][j][k][l][m][n] *
                      ((int)pre_fc->coef_probs[i][j][k][l][m][n] -
                       (int)cur_fc->coef_probs[i][j][k][l][m][n]);
            }
  ...
  for (i = 0; i < SWITCHABLE_FILTER_CONTEXTS; ++i) {
    for (j = 0; j < SWITCHABLE_FILTERS - 1; ++j) {
      diff += (int)counts->switchable_interp[i][j] *
              ((int)pre_fc->switchable_interp_prob[i][j] -
               (int)cur_fc->switchable_interp_prob[i][j]);
    }
    pre_last_prob =
  ...
  for (i = 0; i < INTER_MODE_CONTEXTS; ++i) {
    for (j = 0; j < INTER_MODES - 1; ++j) {
      diff +=
          (int)counts->inter_mode[i][j] * ((int)pre_fc->inter_mode_probs[i][j] -
                                           (int)cur_fc->inter_mode_probs[i][j]);
    }
  ...
  for (i = 0; i < INTRA_INTER_CONTEXTS; ++i) {
    diff += (int)counts->intra_inter[i][0] * ((int)pre_fc->intra_inter_prob[i] -
                                              (int)cur_fc->intra_inter_prob[i]);

    pre_last_prob = MAX_PROB - pre_fc->intra_inter_prob[i];
    cur_last_prob = MAX_PROB - cur_fc->intra_inter_prob[i];

    diff += (int)counts->intra_inter[i][1] *
            ((int)pre_last_prob - (int)cur_last_prob);
  }
  ...
  for (i = 0; i < REF_CONTEXTS; ++i) {
    diff += (int)counts->comp_ref[i][0] *
            ((int)pre_fc->comp_ref_prob[i] - (int)cur_fc->comp_ref_prob[i]);

    pre_last_prob = MAX_PROB - pre_fc->comp_ref_prob[i];
    cur_last_prob = MAX_PROB - cur_fc->comp_ref_prob[i];

    diff +=
        (int)counts->comp_ref[i][1] * ((int)pre_last_prob - (int)cur_last_prob);
  }
  ...
  for (i = 0; i < TX_SIZE_CONTEXTS; ++i) {
    ...
    for (j = 0; j < TX_SIZES - 1; ++j) {
      diff +=
          (int)counts->tx.p32x32[i][j] * ((int)pre_fc->tx_probs.p32x32[i][j] -
                                          (int)cur_fc->tx_probs.p32x32[i][j]);
    }
    ...
    for (j = 0; j < TX_SIZES - 2; ++j) {
      diff +=
          (int)counts->tx.p16x16[i][j] * ((int)pre_fc->tx_probs.p16x16[i][j] -
                                          (int)cur_fc->tx_probs.p16x16[i][j]);
    }
    ...
    for (j = 0; j < TX_SIZES - 3; ++j) {
      diff += (int)counts->tx.p8x8[i][j] * ((int)pre_fc->tx_probs.p8x8[i][j] -
                                            (int)cur_fc->tx_probs.p8x8[i][j]);
    }
    pre_last_prob = MAX_PROB - pre_fc->tx_probs.p8x8[i][TX_SIZES - 4];
    cur_last_prob = MAX_PROB - cur_fc->tx_probs.p8x8[i][TX_SIZES - 4];

    diff += (int)counts->tx.p8x8[i][TX_SIZES - 3] *
            ((int)pre_last_prob - (int)cur_last_prob);
  }
  ...
  for (i = 0; i < 2; ++i) {
    ...
    // sign
    diff += (int)nmv_count->sign[0] *
            ((int)pre_nmv_prob->sign - (int)cur_nmv_prob->sign);

    pre_last_prob = MAX_PROB - pre_nmv_prob->sign;
    cur_last_prob = MAX_PROB - cur_nmv_prob->sign;

    diff += (int)nmv_count->sign[1] * ((int)pre_last_prob - (int)cur_last_prob);

    // classes
    for (j = 0; j < MV_CLASSES - 1; ++j) {
      diff += (int)nmv_count->classes[j] *
              ((int)pre_nmv_prob->classes[j] - (int)cur_nmv_prob->classes[j]);
    }
    ...
    for (j = 0; j < CLASS0_SIZE - 1; ++j) {
      diff += (int)nmv_count->class0[j] *
              ((int)pre_nmv_prob->class0[j] - (int)cur_nmv_prob->class0[j]);
    }
    ...
    for (j = 0; j < CLASS0_SIZE; ++j) {
      for (k = 0; k < MV_FP_SIZE - 1; ++k) {
        diff += (int)nmv_count->class0_fp[j][k] *
                ((int)pre_nmv_prob->class0_fp[j][k] -
                 (int)cur_nmv_prob->class0_fp[j][k]);
      }
    ...
    for (j = 0; j < MV_FP_SIZE - 1; ++j) {
      diff += (int)nmv_count->fp[j] *
              ((int)pre_nmv_prob->fp[j] - (int)cur_nmv_prob->fp[j]);
    }
    pre_last_prob = MAX_PROB - pre_nmv_prob->fp[MV_FP_SIZE - 2];
    cur_last_prob = MAX_PROB - cur_nmv_prob->fp[MV_FP_SIZE - 2];

    diff += (int)nmv_count->fp[MV_FP_SIZE - 1] *
            ((int)pre_last_prob - (int)cur_last_prob);

    // class0_hp
    diff += (int)nmv_count->class0_hp[0] *
            ((int)pre_nmv_prob->class0_hp - (int)cur_nmv_prob->class0_hp);

    pre_last_prob = MAX_PROB - pre_nmv_prob->class0_hp;
    cur_last_prob = MAX_PROB - cur_nmv_prob->class0_hp;

    diff += (int)nmv_count->class0_hp[1] *
            ((int)pre_last_prob - (int)cur_last_prob);

    // hp
    diff +=
        (int)nmv_count->hp[0] * ((int)pre_nmv_prob->hp - (int)cur_nmv_prob->hp);

    pre_last_prob = MAX_PROB - pre_nmv_prob->hp;
    cur_last_prob = MAX_PROB - cur_nmv_prob->hp;

    diff += (int)nmv_count->hp[1] * ((int)pre_last_prob - (int)cur_last_prob);
  }
