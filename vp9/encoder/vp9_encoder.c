...
static void dump_frame_buf(const YV12_BUFFER_CONFIG *frame_buf) {
  dump_buf(frame_buf->y_buffer, frame_buf->y_stride, 0, 0,
           frame_buf->y_height, frame_buf->y_width);
...
static void dump_tpl_stats(const VP9_COMP *cpi, int tpl_group_frames, const GF_PICTURE *gf_picture) {
  int frame_idx;
  const VP9_COMMON *cm = &cpi->common;
