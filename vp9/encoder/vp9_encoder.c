...
static const char *level_fail_messages[TARGET_LEVEL_FAIL_IDS] = {
  ...
  "the CPB size is too large. Try again with a smaller target bit-rate.",
  "the compression ratio is too small", "too many column tiles are used.",
  "the alt-ref distance is too small.",
  "the too many reference buffers are used."
};
  ...
  if (cm->current_video_frame > 0) {
    ...
    if (cpi->level_constraint.level_index != -1 && cpi->oxcf.pass != 1) {
      ...
      if (cpi->level_info.level_spec.average_bitrate >
        ...
        cpi->level_constraint.fail_id = BITRATE_TOO_LARGE;
...
static void update_level_info(VP9_COMP *cpi, size_t *size, int arf_src_index) {
  ...
  if (level_index != -1 && level_constraint->level_achievable) {
    ...
    cpb_data_size = 0;
    if (vp9_level_defs[level_index].max_cpb_size > cpb_data_size) {
      ...
      level_constraint->fail_id = CPB_TOO_LARGE;
      level_constraint->fail_flag |= (1 << CPB_TOO_LARGE);
      cpi->rc.max_frame_bandwidth = level_constraint->default_max_frame_size;
    }
  }
}
