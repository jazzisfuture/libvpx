...
static int encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
                                      uint8_t *dest) {
  VP9_COMMON *const cm = &cpi->common;
  int q = 0, bottom_index = 0, top_index = 0, layer_sync = 0;
  ...
  if (cpi->use_svc) {
    int i = 0;
    // Check if current of any upper layers is sync frame.
    for (i = cpi->svc.spatial_layer_id; i < cpi->svc.number_spatial_layers; i++)
      if (cpi->svc.spatial_layer_sync[i]) layer_sync = 1;
  }
  if (cpi->oxcf.pass == 0 && cpi->oxcf.rc_mode == VPX_CBR &&
      cm->frame_type != KEY_FRAME && !cpi->rc.high_source_sad &&
      !cpi->svc.high_source_sad_superframe && !layer_sync &&
      (!cpi->use_svc ||
