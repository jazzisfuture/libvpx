...
void vp9_remove_compressor(VP9_COMP *cpi) {
  ...
  for (frame = 0; frame < MAX_LAG_BUFFERS; ++frame) {
    if (cpi->tpl_stats[frame].tpl_stats_ptr)
  ...
  for (block = 0; block < 4; ++block) {
    ...
    if (grid_pos_row >= 0 && grid_pos_row < ref_tpl_frame->mi_rows * MI_SIZE &&
      ...
      ref_stats[ref_mi_row * ref_tpl_frame->stride + ref_mi_col].mc_ref_cost +=
          ((tpl_stats->intra_cost - tpl_stats->inter_cost) * overlap_area) / 1024;
  ...
  for (mi_row = 0; mi_row < cm->mi_rows; mi_row += 4) {
    ...
    for (mi_col = 0; mi_col < cm->mi_cols; mi_col += 4) {
      ...
      for (mode = DC_PRED; mode <= TM_PRED; ++mode) {
        ...
        vpx_subtract_block(32, 32, src_diff, 32, src, src_stride,
                           dst, dst_stride);
      ...
      for (rf_idx = 0; rf_idx < 3; ++rf_idx) {
        ...
        if (xd->cur_buf->flags & YV12_FLAG_HIGHBITDEPTH) {
          ...
          vpx_highbd_subtract_block(32, 32, src_diff, 32,
                                    this_frame->y_buffer + mb_y_offset,
