...
static int encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  if (cpi->last_post_encode_dropped_scene_change) {
    cpi->rc.high_source_sad = 1;
    cpi->svc.high_source_sad_superframe = 1;
    // Last source is for the dropped frame so source_sad might be low, which
    // will stop picking intra modes. Disable this speed feature to allow intra
    // modes.
    cpi->sf.use_source_sad = 0;
    cpi->last_post_encode_dropped_scene_change = 0;
  }
  // Check if this high_source_sad (scene/slide change) frame should be
  // encoded at high/max QP, and if so, set the q and adjust some rate
  // control parameters.
  if (!cpi->use_post_encode_drop &&
...
static void encode_frame_to_data_rate(VP9_COMP *cpi, size_t *size,
  ...
  if (cpi->use_post_encode_drop && cm->base_qindex < cpi->rc.worst_quality) {
    int frame_size = (int)(*size) << 3;
    ...
    if (new_buffer_level < 0) {
      *size = 0;
      ...
      if (cpi->rc.high_source_sad ||
          (cpi->use_svc && cpi->svc.high_source_sad_superframe))
        cpi->last_post_encode_dropped_scene_change = 1;
