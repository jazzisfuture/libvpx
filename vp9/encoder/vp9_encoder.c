...
static void dealloc_compressor_data(VP9_COMP *cpi) {
  ...
  vpx_free(cpi->prev_partition_svc);
  cpi->prev_partition_svc = NULL;
...
static void encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  if (cpi->sf.svc_use_lowres_part && cpi->svc.spatial_layer_id == 1) {
    cpi->stride_svc1 = cm->mi_stride;
    if (cpi->prev_partition_svc == NULL) {
      CHECK_MEM_ERROR(
          cm, cpi->prev_partition_svc,
          (BLOCK_SIZE *)vpx_calloc(cm->mi_stride * cm->mi_rows,
                                   sizeof(*cpi->prev_partition_svc)));
    }
  }
