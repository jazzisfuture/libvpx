...
static double get_bits_percent_diff(int target_bits, int actual_bits) {
  double diff =
      abs(target_bits - actual_bits) * 2. / (target_bits + actual_bits);
  diff *= 100;
  return diff;
}
...
static int rq_model_predict_q_index(const RATE_QSTEP_MODEL *rq_model,
  ...
  if (rq_history->recode_count > 0) {
    int actual_bits = rq_history->rate_history[rq_history->recode_count - 1];
    int prev_q_index =
        rq_history->q_index_history[rq_history->recode_count - 1];
    double percent_diff = get_bits_percent_diff(target_bits, actual_bits);
    if (percent_diff > 50) {
      // Binary search.
