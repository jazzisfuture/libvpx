...
static void encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  cpi->compute_source_sad_onepass_cbr =
      cpi->sf.use_source_sad || cpi->oxcf.content == VP9E_CONTENT_SCREEN ||
  ...
  if ((cpi->use_svc &&
    ...
    cpi->compute_source_sad_onepass_cbr = 0;
  ...
  // Compute source_sad if the flag compute_source_sad_onepass_cbr is set, and
  ...
  // them with condition above in compute_source_sad_onepass_cbr).
  if (cpi->oxcf.pass == 0 && cpi->oxcf.mode == REALTIME &&
      cpi->oxcf.speed >= 5 && cpi->resize_state == ORIG &&
      cpi->compute_source_sad_onepass_cbr && cm->show_frame)
