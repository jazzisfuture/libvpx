...
static void encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  cpi->compute_source_sad_onepass = cpi->oxcf.mode == REALTIME &&
      cpi->oxcf.speed >= 5 && cm->show_frame && (cpi->sf.use_source_sad ||
      cpi->oxcf.content == VP9E_CONTENT_SCREEN || cpi->oxcf.rc_mode == VPX_VBR);
  ...
  // and dynamic resize).
  // TODO(jianj): Look into removing condition on resize_size.
  if (cpi->compute_source_sad_onepass && cpi->resize_state == ORIG &&
     (cpi->oxcf.rc_mode == VPX_VBR || cpi->oxcf.content == VP9E_CONTENT_SCREEN))
