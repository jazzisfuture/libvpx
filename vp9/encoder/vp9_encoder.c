...
void predict_mv_mode(VP9_COMP *cpi, MACROBLOCK *x, GF_PICTURE *gf_picture,
  ...
  const int mi_height = num_8x8_blocks_high_lookup[bsize];
  const int mi_width = num_8x8_blocks_wide_lookup[bsize];
  int tmp_mv_mode_arr[(MV_PRECHECK_SIZE * (MV_PRECHECK_SIZE + 1)) >> 1];
  int *mv_mode_arr = tpl_frame->mv_mode_arr[rf_idx];
  int_mv *select_mv_arr = cpi->select_mv_arr;
  int_mv tmp_select_mv_arr[(MV_PRECHECK_SIZE * (MV_PRECHECK_SIZE + 1)) >> 1];
  ...
  // diagnal scan order
  tmp_idx = 0;
  for (idx = 0; idx < MV_PRECHECK_SIZE; ++idx) {
    int r;
    for (r = 0; r <= idx; ++r) {
      int c = idx - r;
  ...
  // beforehand.
  for (idx = 1; idx < MV_PRECHECK_SIZE; ++idx) {
    int r;
    for (r = 0; r <= idx; ++r) {
      int c = idx - r;
      int nb_row = mi_row + r * mi_height;
