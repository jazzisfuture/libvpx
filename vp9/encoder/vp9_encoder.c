...
static void scale_and_extend_frame(const YV12_BUFFER_CONFIG *src,
                                   YV12_BUFFER_CONFIG *dst, int bd, int shift) {
#else
static void scale_and_extend_frame(const YV12_BUFFER_CONFIG *src,
      ...
      if (ref->y_crop_width != cm->width || ref->y_crop_height != cm->height) {
        ...
        if (force_scaling ||
          ...
          vpx_realloc_frame_buffer(&new_fb_ptr->buf,
                                   ...
                                   VP9_ENC_BORDER_IN_PIXELS, cm->byte_alignment,
                                   NULL, NULL, NULL);
          scale_and_extend_frame(ref, &new_fb_ptr->buf, 0);
          cpi->scaled_ref_idx[ref_frame - 1] = new_fb;
          alloc_frame_mvs(cm, new_fb);
        }
#endif  // CONFIG_VP9_HIGHBITDEPTH
...
static void encode_without_recode_loop(VP9_COMP *cpi,
  ...
  // Change phase of filter for 1:2 dynamc resize.
  if (cpi->oxcf.pass == 0 &&
      cpi->oxcf.rc_mode == VPX_CBR &&
      !cpi->use_svc &&
...
YV12_BUFFER_CONFIG *vp9_scale_if_required(VP9_COMMON *cm,
  ...
  if (cm->mi_cols * MI_SIZE != unscaled->y_width ||
    ...
    if (use_normative_scaler &&
        unscaled->y_width <= (scaled->y_width << 1) &&
        unscaled->y_height <= (scaled->y_height << 1))
      scale_and_extend_frame(unscaled, scaled, phase_scaler);
    else
