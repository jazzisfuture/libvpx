...
static void encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  const INTERP_FILTER filter_scaler = (is_one_pass_cbr_svc(cpi)) ?
      cpi->svc.downsample_filter_type[cpi->svc.spatial_layer_id] : EIGHTTAP;
  const int phase_scaler = (is_one_pass_cbr_svc(cpi)) ?
  ...
  if (is_one_pass_cbr_svc(cpi) &&
    ...
    int filter_scaler2 = cpi->svc.downsample_filter_type[1];
    int phase_scaler2 = cpi->svc.downsample_filter_phase[1];
    ...
    cpi->Source =
        vp9_scale_if_required(cm, cpi->un_scaled_source, &cpi->scaled_source,
            (cpi->oxcf.pass == 0), filter_scaler, phase_scaler);
  }
...
YV12_BUFFER_CONFIG *vp9_svc_twostage_scale(
  ...
  if (cm->mi_cols * MI_SIZE != unscaled->y_width ||
    ...
    if (cm->bit_depth == VPX_BITS_8) {
      vp9_scale_and_extend_frame(unscaled, scaled_temp, filter_type,
                                 phase_scaler2);
      vp9_scale_and_extend_frame(scaled_temp, scaled, filter_type,
                                 phase_scaler);
    } else {
      scale_and_extend_frame(unscaled, scaled_temp, (int)cm->bit_depth,
                             filter_type, phase_scaler2);
