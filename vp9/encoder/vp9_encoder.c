...
VP9_COMP *vp9_create_compressor(VP9EncoderConfig *oxcf) {
  ...
  vp9_set_speed_features_framesize_dependent(cpi);  // AWG
...
static void encode_with_recode_loop(VP9_COMP *cpi,
  ...
  do {
    ...
    if (loop_count == 0) {
      ...
      // rc->this_frame_target to setup. This is done in set_target_rate.
      set_size_independent_vars(cpi, &q, &bottom_index, &top_index);
      q_low = bottom_index;
      q_high = top_index;
    }
/*
    if (_frame_size_changed_) {
      cpi->max_mv_magnitude = k * cpi->max_mv_magnitude;
    }

    if (loop_count == 0 || _frame_size_changed_) {
...
int vp9_get_compressed_data(VP9_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  if ((*size == 0) && (frame_is_intra_only(cm) == 0)) {  // AWG
    release_scaled_references(cpi);
  }
