...
static void dealloc_compressor_data(VP9_COMP *cpi) {
  ...
  vpx_free_frame_buffer(&cpi->key_buffer);
#ifdef ENABLE_KF_DENOISE
...
static void alloc_raw_frame_buffers(VP9_COMP *cpi) {
  ...
  if (vpx_realloc_frame_buffer(&cpi->key_buffer, oxcf->width, oxcf->height,
                               cm->subsampling_x, cm->subsampling_y,
#if CONFIG_VP9_HIGHBITDEPTH
                               cm->use_highbitdepth,
#endif
...
int vp9_get_compressed_data(VP9_COMP *cpi, unsigned int *frame_flags,
  ...
  if (arf_src_index) {
    ...
    if ((source = vp9_lookahead_peek(cpi->lookahead, arf_src_index)) != NULL) {
      ...
      if ((oxcf->mode != REALTIME) && (oxcf->arnr_max_frames > 0) &&
        ...
        vp9_temporal_filter(cpi, arf_src_index, &cpi->alt_ref_buffer);
  ...
  *size = 0;
  ...
  const int is_key_temporal_filter_enabled =
      cpi->oxcf.mode != REALTIME && (oxcf->pass != 1) && !cpi->use_svc &&
  ...
  if (is_key_temporal_filter_enabled && source != NULL) {
    ...
    vp9_temporal_filter(cpi, 0, &cpi->key_buffer);
    vpx_extend_frame_borders(&cpi->key_buffer);
    force_src_buffer = &cpi->key_buffer;
    cpi->un_scaled_source = cpi->Source =
        force_src_buffer ? force_src_buffer : &source->img;
  }
#endif  // !CONFIG_REALTIME_ONLY
