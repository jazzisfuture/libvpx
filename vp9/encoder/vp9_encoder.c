...
static void write_64x64_block(const uint8_t *src, int src_stride,
                              const uint8_t *rec, int rec_stride, FILE *fptr) {
  int i = 0;
  int j = 0;
  for (i = 0; i < 64; i++) {
    for (j = 0; j < 64; j++) {
      fprintf(fptr, "%d,", *(src + i * src_stride + j));
    }
    // fprintf(fptr, "\t");
  }
  fprintf(fptr, "\t");
  ...
  for (i = 0; i < 64; i++) {
    for (j = 0; j < 64; j++) {
      fprintf(fptr, "%d,", *(rec + i * rec_stride + j));
    }
    // fprintf(fptr, "\t");
  }
  // fprintf(fptr, "%d,", 129);
...
static void write_frame(VP9_COMP *cpi, FILE *fptr) {
  int i = 0;
  int j = 0;
  ...
  for (i = 0; i < cpi->Source->y_height; i++) {
    for (j = 0; j < cpi->Source->y_width; j++) {
      fprintf(fptr, "%d,",
              *(cpi->common.frame_to_show->y_buffer +
...
static void encode_frame_to_data_rate(VP9_COMP *cpi, size_t *size,
  ...
  *size = VPXMAX(1, *size);
...

  {
    ...
    fprintf(fptr, "%d\t%d\t%d\t%d\t%d\t%d\t", cm->current_video_frame - 1,
    ...
    for (j = 0; j < num_blocks_h; ++j) {
      for (i = 0; i < num_blocks_w; ++i) {
...

        {
          ...
          uint8_t *ubuf = recon->u_buffer + j * recon->uv_stride * 32 + i * 32;
          uint8_t *vbuf = recon->v_buffer + j * recon->uv_stride * 32 + i * 32;
