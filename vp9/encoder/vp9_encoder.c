...
static void encode_frame_to_data_rate(VP9_COMP *cpi, size_t *size,
  ...
  if (oxcf->pass == 0 && oxcf->rc_mode == VPX_CBR &&
    ...
    int drop_rate_control = (vp9_rc_drop_frame(cpi)) ? 1 : 0;
    // In the contrained framedrop modes for svc, if the previous spatial layer
    // was dropped, drop the current spatial layer. For encodeskip mode, the base
    // layer must be dropped to force drop on upper layers.
    if (cpi->use_svc && cpi->svc.spatial_layer_id > 0 &&
        ((cpi->svc.framedrop_mode == CONSTRAINED_LAYER_DROP &&
         cpi->svc.drop_spatial_layer[cpi->svc.spatial_layer_id - 1]) ||
        (cpi->svc.framedrop_mode == CONSTRAINED_DROPBASE_ENCODESKIP &&
         cpi->svc.drop_spatial_layer[0])))
      svc_force_drop_from_prev_layer = 1;
    if (svc_force_drop_from_prev_layer || (drop_rate_control &&
        cpi->svc.framedrop_mode != CONSTRAINED_DROPBASE_ENCODESKIP)) {
      vp9_rc_postencode_update_drop_frame(cpi);
      cpi->common.current_video_frame++;
      cpi->ext_refresh_frame_flags_pending = 0;
      ...
      return;
    } else if (cpi->svc.framedrop_mode == CONSTRAINED_DROPBASE_ENCODESKIP &&
               drop_rate_control) {
