...
static const char *level_fail_messages[TARGET_LEVEL_FAIL_IDS] = {
  "bit-rate is too high. Try again with a smaller target bit-rate and a "
  "larger quantizer.",
  "the picture size is too large.", "the luma sample rate is too large.",
  "the CPB size is too large. Try again with a smaller target bit-rate and a"
  "larger quantizer.",
  "the compression ratio is too small", "too many column tiles are used.",
  "the alt-ref distance is too small.", "too many reference buffers are used."
};
...
static void set_level_constraint(LEVEL_CONSTRAINT *ls, int8_t level_index) {
  vpx_clear_system_state();
...
void vp9_remove_compressor(VP9_COMP *cpi) {
  ...
  if (cm->current_video_frame > 0) {
    ...
    if (cpi->level_constraint.level_index != -1 && cpi->oxcf.pass != 1) {
      const int level = vp9_get_level(&cpi->level_info.level_spec);
      const int level_index = cpi->level_constraint.level_index;

      if (cpi->level_info.level_spec.average_bitrate >
          vp9_level_defs[level_index].average_bitrate) {
        cpi->level_constraint.level_achievable = 0;
        cpi->level_constraint.fail_flag |= (1 << BITRATE_TOO_LARGE);
      }

      if (cpi->level_info.level_spec.compression_ratio <
          vp9_level_defs[level_index].compression_ratio) {
        cpi->level_constraint.level_achievable = 0;
        cpi->level_constraint.fail_flag |= (1 << COMPRESSION_RATIO_TOO_SMALL);
      }
      ...
      if (level == LEVEL_UNKNOWN || level > cpi->target_level ||
          cpi->level_constraint.level_achievable == 0) {
        for (i = 0; i < TARGET_LEVEL_FAIL_IDS; ++i) {
          if (cpi->level_constraint.fail_flag & (1 << i)) {
            fprintf(stderr,
                    "\n!!!!!!!!!\n Warning: failed to encode to the target level %d "
                    "because %s\n",
                    cpi->target_level, level_fail_messages[i]);
          }
        }
      }
    }
...
static void level_rc_framerate(VP9_COMP *cpi, int arf_src_index) {
  ...
  rc->max_frame_bandwidth =
      VPXMIN(rc->max_frame_bandwidth, ls->max_frame_size);
  if (frame_is_intra_only(cm)) {
    rc->max_frame_bandwidth =
...
static void update_level_info(VP9_COMP *cpi, size_t *size, int arf_src_index) {
  ...
  if (level_index != -1 && level_constraint->level_achievable) {
    ...
    if (level_spec->max_col_tiles > vp9_level_defs[level_index].max_col_tiles) {
      level_constraint->level_achievable = 0;
      level_constraint->fail_flag |= (1 << COLUMN_TILE_TOO_MANY);
    }
    ...
    if (level_spec->max_ref_frame_buffers >
      ...
      level_constraint->level_achievable = 0;
      level_constraint->fail_flag |= (1 << REF_BUFFER_TOO_MANY);
    }
    ...
    // Set a size upper bound for the next frame. It will be used in
...
int vp9_get_compressed_data(VP9_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  if (oxcf->pass != 1 && cpi->level_constraint.level_index != -1 &&
