...
void vp9_remove_compressor(VP9_COMP *cpi) {
  ...
  if (cm->current_video_frame > 0) {
    ...
    if (cpi->oxcf.pass != 1) {
      ...
      (void)level_stats;
#if 0
      for (i = 0; i < cpi->common.level_info.level_stats.frame_window_buffer.len;
          ++i) {
        int idx = cpi->common.level_info.level_stats.frame_window_buffer.start + i;
        idx = idx % FRAME_WINDOW_SIZE;
        printf("%lld %6d %2d\n",
               level_stats->frame_window_buffer.buf[idx].ts,
               level_stats->frame_window_buffer.buf[idx].size,
               level_stats->frame_window_buffer.buf[idx].is_altref);
      }
#endif
#if 0
      ...
      {
        FILE *fp;
        fp = fopen("./libvpx_stats.txt", "a");
        if (fp == NULL)
          printf("\n open file failed....\n");
        else {
          fprintf(fp, "mlsr:%" PRId64 " mlps:%" PRId64
                  " abr:%7.1f mcs:%g cr:%g mct:%d"
                  " mad:%d mrf:%d\n",
                  level_spec->max_luma_sample_rate,
                  level_spec->max_luma_picture_size,
                  level_spec->average_bitrate, level_spec->max_cpb_size,
                  level_spec->compression_ratio, level_spec->max_col_tiles,
                  level_spec->min_altref_distance,
                  level_spec->max_ref_frame_buffers);
          fclose(fp);
        }
...
int vp9_get_compressed_data(VP9_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
  ...
  if (cm->keep_level_stats && oxcf->pass != 1) {
    ...
    //printf("%8d %7.2f \n",
      //     level_stats->total_compressed_size, level_stats->time_encoded);
