...
#if OPTIMIZED_1TO2_SCALING
#include <tmmintrin.h>
#include "vpx_ports/mem.h"
#include "vpx_ports/emmintrin_compat.h"
static void scale_and_extend_frame_old(const YV12_BUFFER_CONFIG *src,
                                   YV12_BUFFER_CONFIG *dst) {
#else 
#if CONFIG_VP9_HIGHBITDEPTH
...
static void scale_and_extend_frame(const YV12_BUFFER_CONFIG *src,
  ...
  if(src_w < dst_w)
	  ...
	  downsample_count++;
  printf("upsample count = %d, downsample count = %d\n", upsample_count, downsample_count);
#endif
...
{
    const __m128i coeffs_ab = _mm_set_epi8(6,-1,6,-1,6,-1,6,-1,6,-1,6,-1,6,-1,6,-1);
    const __m128i coeffs_cd = _mm_set_epi8(78,-19,78,-19,78,-19,78,-19,78,-19,78,-19,78,-19,78,-19);
    ...
    for(; x<widthM8; x += 8)
    {
        ...
        const __m128i abcd_terms = _mm_add_epi16(ab_terms, cd_terms);   // can not overflow
        const __m128i fehg_terms = _mm_add_epi16(fe_terms, hg_terms);   // can not overflow
        const __m128i terms = _mm_adds_epi16(abcd_terms, fehg_terms);   // can overflow, use saturating add
    ...
    dst_h /= 2;
    {
        ...
        for(y=0; y<dst_h; y++)
        {
            ...
            for(x=0; x<widthM8; x += 8)
            {
                ...
                {
                    const __m128i a = _mm_loadl_epi64((const __m128i *)(src + x - src_stride*3));
                    const __m128i b = _mm_loadl_epi64((const __m128i *)(src + x - src_stride*2));
                    const __m128i c = _mm_loadl_epi64((const __m128i *)(src + x - src_stride*1));
                    const __m128i d = _mm_loadl_epi64((const __m128i *)(src + x + src_stride*0));
                    const __m128i e = _mm_loadl_epi64((const __m128i *)(src + x + src_stride*1));
                    const __m128i f = _mm_loadl_epi64((const __m128i *)(src + x + src_stride*2));
                    const __m128i g = _mm_loadl_epi64((const __m128i *)(src + x + src_stride*3));
                    const __m128i h = _mm_loadl_epi64((const __m128i *)(src + x + src_stride*4));
                    const __m128i ab = _mm_unpacklo_epi8(a, b);
                    const __m128i cd = _mm_unpacklo_epi8(c, d);
                    const __m128i fe = _mm_unpacklo_epi8(f, e);
                    const __m128i hg = _mm_unpacklo_epi8(h, g);
...
#if CONFIG_VP9_HIGHBITDEPTH
static void scale_and_extend_frame(const YV12_BUFFER_CONFIG *src, YV12_BUFFER_CONFIG *dst, int bd)
#else
...
#endif  // CONFIG_VP9_HIGHBITDEPTH
{
    ...
    {
        downsample_2_to_1_ssse3(src->y_buffer, src->y_stride, dst->y_buffer, dst->y_stride, dst_w, dst_h);
        downsample_2_to_1_ssse3(src->u_buffer, src->uv_stride, dst->u_buffer, dst->uv_stride, dst_w/2, dst_h/2);
        downsample_2_to_1_ssse3(src->v_buffer, src->uv_stride, dst->v_buffer, dst->uv_stride, dst_w/2, dst_h/2);
    }
    else if(dst_w == src_w*2 && dst_h == src_h*2)
    {
        upsample_1_to_2_ssse3(src->y_buffer, src->y_stride, dst->y_buffer, dst->y_stride, dst_w, dst_h);
        upsample_1_to_2_ssse3(src->u_buffer, src->uv_stride, dst->u_buffer, dst->uv_stride, dst_w/2, dst_h/2);
        upsample_1_to_2_ssse3(src->v_buffer, src->uv_stride, dst->v_buffer, dst->uv_stride, dst_w/2, dst_h/2);
    }
    else
#endif
    {
