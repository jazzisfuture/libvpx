...
void init_gop_frames(VP9_COMP *cpi, GF_PICTURE *gf_picture,
  ...
  *tpl_group_frames = 0;
  ...
  for (int i = 0; i < 3; ++i) gf_picture[0].ref_frame[i] = -1;
  gld_index = 0;
  ++*tpl_group_frames;
...
void init_tpl_stats(VP9_COMP *cpi) {
  for (int frame_idx = 0; frame_idx < MAX_LAG_BUFFERS; ++frame_idx) {
    ...
    vp9_zero(tpl_frame->tpl_stats_ptr);
...
void mc_flow_dispenser(VP9_COMP *cpi, GF_PICTURE *gf_picture, int frame_idx) {
  ...
  vp9_setup_scale_factors_for_frame(
      ...
      frames[0]->y_crop_width, frames[0]->y_crop_height, cm->use_highbitdepth);
  ...
  for (int mi_row = 0; mi_row < cm->mi_rows; ++mi_row) {
    ...
    for (int mi_col = 0; mi_col < cm->mi_cols; ++mi_col) {
      ...
      for (PREDICTION_MODE mode = DC_PRED; mode <= TM_PRED; ++mode) {
        uint8_t *src, *dst;
...
void setup_tpl_stats(VP9_COMP *cpi) {
  ...
  for (int frame_idx = tpl_group_frames - 1; frame_idx > 0; --frame_idx)
