...
#define SKIP_RECODE_THRESH 40000000000
...
int64_t compute_context_model_diff(const VP9_COMMON *const cm) {
  ...
  for (i = 0; i < INTER_MODE_CONTEXTS; ++i) {
    for (j = 0; j < INTER_MODES - 1; ++j) {
      diff += counts->inter_mode[i][j] *
          (pre_fc->inter_mode_probs[i][j]
           - cur_fc->inter_mode_probs[i][j]);
    }
  ...
  for (i = 0; i < TX_SIZE_CONTEXTS; ++i) {
    ...
    diff += counts->tx.p32x32[i][TX_SIZES - 1]
        * (pre_last_prob - cur_last_prob);
    ...
    diff += counts->tx.p16x16[i][TX_SIZES - 2]
        * (pre_last_prob - cur_last_prob);
