...
void vp9_update_reference_frames(VP9_COMP *cpi) {
  VP9_COMMON * const cm = &cpi->common;
  // printf("vp9_update_reference_frames()\n");
...
#if CONFIG_MULTI_REF
  // At the end of the encoding of the current frame, update the flag
  // that indicates whether LAST_FRAME has been refreshed.
  if (cm->frame_type == KEY_FRAME)
    cpi->prev_last_frame_refreshed = 0;
  else if (cpi->refresh_last_frame)
    cpi->prev_last_frame_refreshed = 1;
#endif  // CONFIG_MULTI_REF
}
