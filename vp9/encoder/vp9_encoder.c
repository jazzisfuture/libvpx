...
void set_frame_size(VP9_COMP *cpi) {
  ...
  {
    BufferPool *const pool = cm->buffer_pool;
    const int new_idx = cm->new_fb_idx;
    // Allocate based on the maximum frame size.
    if (pool->frame_bufs[new_idx].mvs == NULL) {
      const int aligned_width = ALIGN_POWER_OF_TWO(cpi->initial_width,
                                                   MI_SIZE_LOG2);
      const int aligned_height = ALIGN_POWER_OF_TWO(cpi->initial_height,
                                                    MI_SIZE_LOG2);
      pool->frame_bufs[new_idx].mvs = (MV_REF *)vpx_calloc(
          (aligned_width >> MI_SIZE_LOG2) * (aligned_height >> MI_SIZE_LOG2),
          sizeof(*(pool->frame_bufs[new_idx].mvs)));
//      if (pool->frame_bufs[new_idx].mvs == NULL)
//        goto fail;
    }
  }
