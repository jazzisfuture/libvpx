...
void init_gop_frames(VP9_COMP *cpi, GF_PICTURE *gf_picture,
  ...
  for (i = 0; i < REFS_PER_FRAME + 1; ++i) {
    ...
    fprintf(stderr, "frame size = (%d, %d), stride = %d\n",
            cpi->tpl_recon_frames[i]->y_height,
            cpi->tpl_recon_frames[i]->y_width,
            cpi->tpl_recon_frames[i]->y_stride);
  }
...
void tpl_model_store(TplDepStats *tpl_stats, int mi_row, int mi_col,
                     BLOCK_SIZE bsize, int stride, TplDepStats *src_stats) {
