...
#define EMPIRICAL_PARAM_1 \
  2;  // empirical parameters for context prob model threshold
#define EMPIRICAL_PARAM_2 200;
#define EMPIRICAL_PARAM_3 6;
...
int64_t compute_context_model_thresh(VP9_COMP *cpi) {
  VP9_COMMON *const cm = &cpi->common;
  const VP9EncoderConfig *const oxcf = &cpi->oxcf;
  const int frame_size = oxcf->width * oxcf->height;
  const int framerate = cpi->framerate;
  const int qindex = cm->base_qindex;
  const int empirical_param = qindex + (MAXQ >> 1);
  int64_t thresh;

  thresh = (EMPIRICAL_PARAM_1 * frame_size - EMPIRICAL_PARAM_2 * framerate) *
           empirical_param;
  thresh = thresh << EMPIRICAL_PARAM_3;
...
int64_t compute_context_model_diff(const VP9_COMMON *const cm) {
  ...
  for (i = 0; i < BLOCK_SIZE_GROUPS; ++i) {
    ...
    diff +=
        counts->y_mode[i][INTRA_MODES - 1] * (pre_last_prob - cur_last_prob);
  }

  // uv_mode_prob
  for (i = 0; i < INTRA_MODES; ++i) {
    for (j = 0; j < INTRA_MODES - 1; ++j) {
      diff += counts->uv_mode[i][j] *
    ...
    diff +=
        counts->uv_mode[i][INTRA_MODES - 1] * (pre_last_prob - cur_last_prob);
  }

  // partition_prob
  for (i = 0; i < PARTITION_CONTEXTS; ++i) {
    for (j = 0; j < PARTITION_TYPES - 1; ++j) {
      diff += counts->partition[i][j] *
    ...
    diff += counts->partition[i][PARTITION_TYPES - 1] *
      ...
      for (k = 0; k < REF_TYPES; ++k)
        for (l = 0; l < COEF_BANDS; ++l)
          for (m = 0; m < BAND_COEFF_CONTEXTS(l); ++m) {
            ...
            diff += counts->coef[i][j][k][l][m][UNCONSTRAINED_NODES] *
                    (pre_last_prob - cur_last_prob);
          }
  ...
  for (i = 0; i < SWITCHABLE_FILTER_CONTEXTS; ++i) {
    for (j = 0; j < SWITCHABLE_FILTERS - 1; ++j) {
      diff += counts->switchable_interp[i][j] *
    ...
    diff += counts->switchable_interp[i][SWITCHABLE_FILTERS - 1] *
            (pre_last_prob - cur_last_prob);
  }
  ...
  for (i = 0; i < INTER_MODE_CONTEXTS; ++i) {
    ...
    diff += counts->inter_mode[i][INTER_MODES - 1] *
            (pre_last_prob - cur_last_prob);
  }

  // intra_inter_prob
  for (i = 0; i < INTRA_INTER_CONTEXTS; ++i) {
    diff += counts->intra_inter[i][0] *
    ...
    diff += counts->intra_inter[i][1] * (pre_last_prob - cur_last_prob);
  }

  // comp_inter_prob
  for (i = 0; i < COMP_INTER_CONTEXTS; ++i) {
    diff += counts->comp_inter[i][0] *
    ...
    diff += counts->comp_inter[i][1] * (pre_last_prob - cur_last_prob);
  }

  // single_ref_prob
  for (i = 0; i < REF_CONTEXTS; ++i) {
    for (j = 0; j < 2; ++j) {
      diff += counts->single_ref[i][j][0] *
      ...
      diff += counts->single_ref[i][j][1] * (pre_last_prob - cur_last_prob);
    }
  }

  // comp_ref_prob
  for (i = 0; i < REF_CONTEXTS; ++i) {
    diff += counts->comp_ref[i][0] *
    ...
    diff += counts->comp_ref[i][1] * (pre_last_prob - cur_last_prob);
  }
  ...
  for (i = 0; i < TX_SIZE_CONTEXTS; ++i) {
    ...
    diff +=
        counts->tx.p32x32[i][TX_SIZES - 1] * (pre_last_prob - cur_last_prob);

    // p16x16
    for (j = 0; j < TX_SIZES - 2; ++j) {
      diff += counts->tx.p16x16[i][j] *
    ...
    diff +=
        counts->tx.p16x16[i][TX_SIZES - 2] * (pre_last_prob - cur_last_prob);

    // p8x8
    for (j = 0; j < TX_SIZES - 3; ++j) {
      diff += counts->tx.p8x8[i][j] *
    ...
    diff += counts->tx.p8x8[i][TX_SIZES - 3] * (pre_last_prob - cur_last_prob);
  }

  // skip_probs
  for (i = 0; i < SKIP_CONTEXTS; ++i) {
    diff +=
        counts->skip[i][0] * (pre_fc->skip_probs[i] - cur_fc->skip_probs[i]);

    pre_last_prob = MAX_PROB - pre_fc->skip_probs[i];
    cur_last_prob = MAX_PROB - cur_fc->skip_probs[i];

    diff += counts->skip[i][1] * (pre_last_prob - cur_last_prob);
  }

  // mv
  for (i = 0; i < MV_JOINTS - 1; ++i) {
    diff += counts->mv.joints[i] *
  ...
  diff += counts->mv.joints[MV_JOINTS - 1] * (pre_last_prob - cur_last_prob);
  ...
  for (i = 0; i < 2; ++i) {
    ...
    // sign
    diff += nmv_count->sign[0] * (pre_nmv_prob->sign - cur_nmv_prob->sign);

    pre_last_prob = MAX_PROB - pre_nmv_prob->sign;
    cur_last_prob = MAX_PROB - cur_nmv_prob->sign;

    diff += nmv_count->sign[1] * (pre_last_prob - cur_last_prob);

    // classes
    for (j = 0; j < MV_CLASSES - 1; ++j) {
      diff += nmv_count->classes[j] *
    ...
    diff +=
        nmv_count->classes[MV_CLASSES - 1] * (pre_last_prob - cur_last_prob);

    // class0
    for (j = 0; j < CLASS0_SIZE - 1; ++j) {
      diff += nmv_count->class0[j] *
    ...
    diff +=
        nmv_count->class0[CLASS0_SIZE - 1] * (pre_last_prob - cur_last_prob);

    // bits
    for (j = 0; j < MV_OFFSET_BITS; ++j) {
      diff += nmv_count->bits[j][0] *
      ...
      diff += nmv_count->bits[j][1] * (pre_last_prob - cur_last_prob);
    }

    // class0_fp
    for (j = 0; j < CLASS0_SIZE; ++j) {
      for (k = 0; k < MV_FP_SIZE - 1; ++k) {
        diff += nmv_count->class0_fp[j][k] *
      ...
      diff += nmv_count->class0_fp[j][MV_FP_SIZE - 1] *
              (pre_last_prob - cur_last_prob);
    }

    // fp
    for (j = 0; j < MV_FP_SIZE - 1; ++j) {
      diff += nmv_count->fp[j] * (pre_nmv_prob->fp[j] - cur_nmv_prob->fp[j]);
    }
    pre_last_prob = MAX_PROB - pre_nmv_prob->fp[MV_FP_SIZE - 2];
    cur_last_prob = MAX_PROB - cur_nmv_prob->fp[MV_FP_SIZE - 2];

    diff += nmv_count->fp[MV_FP_SIZE - 1] * (pre_last_prob - cur_last_prob);

    // class0_hp
    diff += nmv_count->class0_hp[0] *
    ...
    diff += nmv_count->class0_hp[1] * (pre_last_prob - cur_last_prob);

    // hp
    diff += nmv_count->hp[0] * (pre_nmv_prob->hp - cur_nmv_prob->hp);

    pre_last_prob = MAX_PROB - pre_nmv_prob->hp;
    cur_last_prob = MAX_PROB - cur_nmv_prob->hp;

    diff += nmv_count->hp[1] * (pre_last_prob - cur_last_prob);
  }
...
static void encode_with_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  if (enable_acl) {
    // For altref, skip recoding, if model diff is below threshold
    if (!cm->show_frame) {
      int64_t thresh = compute_context_model_thresh(cpi);
      int64_t diff = compute_context_model_diff(cm);
      if (diff < thresh) {
        vpx_clear_system_state();
        restore_coding_context(cpi);
        return;
