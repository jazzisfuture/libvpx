...
void vp9_change_config(struct VP9_COMP *cpi, const VP9EncoderConfig *oxcf) {
  ...
  if (cpi->oxcf.rc_mode == RC_MODE_VBR) {
    cpi->oxcf._starting_buffer_level   = 60000;
    cpi->oxcf._optimal_buffer_level    = 60000;
    cpi->oxcf._maximum_buffer_size     = 240000;
  }

  rc->starting_buffer_level = vp9_rescale(cpi->oxcf._starting_buffer_level,
                                          cpi->oxcf.target_bandwidth, 1000);

  // Set or reset optimal and maximum buffer levels.
  if (cpi->oxcf._optimal_buffer_level == 0)
    rc->optimal_buffer_level = cpi->oxcf.target_bandwidth / 8;
  else
    rc->optimal_buffer_level = vp9_rescale(cpi->oxcf._optimal_buffer_level,
                                           cpi->oxcf.target_bandwidth, 1000);

  if (cpi->oxcf._maximum_buffer_size == 0)
    rc->maximum_buffer_size = cpi->oxcf.target_bandwidth / 8;
  else
    rc->maximum_buffer_size = vp9_rescale(cpi->oxcf._maximum_buffer_size,
                                          cpi->oxcf.target_bandwidth, 1000);
