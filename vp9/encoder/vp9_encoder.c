...
static int get_qstep_adj(int rate_excess, int rate_limit) {
  int qstep = rate_limit ?
      ((rate_excess + rate_limit / 2) / rate_limit) : INT_MAX;
    ...
    if (cpi->oxcf.rc_mode == VPX_Q) {
      ...
      if ((cm->frame_type == KEY_FRAME) && rc->this_key_frame_forced &&
        ...
        if (rc->projected_frame_size > rc->this_frame_target) {
          ...
          qstep = get_qstep_adj(rc->projected_frame_size, rc->this_frame_target);
          ...
          // Frame is too small
          qstep = get_qstep_adj(rc->this_frame_target, rc->projected_frame_size);
