...
static int check_seg_range(int seg_data[8], int range) {
  if ((abs(seg_data[0]) > range) || (abs(seg_data[1]) > range) ||
      (abs(seg_data[2]) > range) || (abs(seg_data[3]) > range) ||
      (abs(seg_data[4]) > range) || (abs(seg_data[5]) > range) ||
      (abs(seg_data[6]) > range) || (abs(seg_data[6]) > range))
    return 0;
  else
    return 1;
}
...
int vp9_set_roi_map(VP9_COMP *cpi, unsigned char *map, unsigned int rows,
                    ...
                    uint8_t skip[8], int ref_frame[8]) {
  VP9_COMMON *cm = &cpi->common;
  vpx_roi_map_t *roi = &cpi->roi;
  const int range = 63;
  ...
  if (!map || !(delta_q[0] | delta_q[1] | delta_q[2] | delta_q[3] | delta_q[4] |
                delta_q[5] | delta_q[6] | delta_q[7] | delta_lf[0] |
                delta_lf[1] | delta_lf[2] | delta_lf[3] | delta_lf[4] |
                delta_lf[5] | delta_lf[6] | delta_lf[7] | skip[0] | skip[1] |
                skip[2] | skip[3] | skip[4] | skip[5] | skip[6] | skip[7] |
                ref_frame[0] | ref_frame[1] | ref_frame[2] | ref_frame[3] |
                ref_frame[4] | ref_frame[5] | ref_frame[6] | ref_frame[7])) {
