...
static void encode_with_recode_loop(VP9_COMP *cpi, size_t *size, uint8_t *dest
  ...
  // If not specified (== -1), we take VP9's decision for the max frame size.
  int ext_rc_max_frame_size = -1;
  ...
  do {
    ...
    if (cpi->ext_ratectrl.ready) {
      ...
      if (ext_rc_max_frame_size != -1 &&
          rc->projected_frame_size < ext_rc_max_frame_size) {
        break;
      }
      if (ext_rc_max_frame_size == -1 &&
