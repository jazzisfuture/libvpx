  ...
  if (cpi->use_svc && cpi->svc.spatial_layer_id > 0) {
    ...
    if (cpi->svc.force_rtp_pred) {
      ...
      for (ref_frame = LAST_FRAME; ref_frame <= GOLDEN_FRAME; ref_frame++) {
        ...
        if (vp9_is_scaled(scale_fac)) {
          // If this reference  was not last updated on
          // previous spatial layer of current superframe, then remove this
          // reference (disable this prediction). We check if the current
          // frame buffer index was equal to any of the slots for the previous
          // spatial layer, and if so that slot was updated/refreshed, then the
          // reference is valid.
          int fb_idx =
