...
static int encode_without_recode_loop(VP9_COMP *cpi, size_t *size,
  ...
  // Enable post encode frame dropping for screen content mode, and only
  // on base spatial layer.
...
static void encode_frame_to_data_rate(VP9_COMP *cpi, size_t *size,
  ...
  // SVC: skip encoding enhancement layer if the layer target bandwidth = 0.
