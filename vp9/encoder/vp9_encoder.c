...
void vp9_update_reference_frames(VP9_COMP *cpi) {
  ...
  printf("refresh frame: %d %d %d %d\n", cpi->refresh_alt_ref_frame,
         cpi->refresh_golden_frame, cpi->refresh_last2_frame,
...
static void encode_frame_to_data_rate(VP9_COMP *cpi,
  ...
  /*
  {
    int i;
    printf("\n==============================================================\n");
    printf("*******vp9_update_reference_frames(frame=%d): get_refresh_mask=",
           cm->current_video_frame);
    if (cm->frame_type == KEY_FRAME)
      printf("KEY_FRAME");
    else
      printf("%d", cpi->refresh_mask);
    printf(", new_fb_idx=%d, ref_frame_map=[", cm->new_fb_idx);
  ...
  /*
  printf("Frame(%d): ref_frame_flags=%d", cm->current_video_frame,
         cpi->ref_frame_flags);*/
...
int vp9_get_compressed_data(VP9_COMP *cpi, unsigned int *frame_flags,
  ...
  *size = 0;
   ...
   * held.
   */
  // TODO(zoeliu): To find out whether there is a possibility to cause memory
  // leak here, i.e. whether it is possible that after the release of the
  // reference, ref_count may end up 0.
  ...
  if (oxcf->pass == 1) {
    ...
    /*
    printf("vp9_get_compressed_data(): ref_frame_map=[%d,%d,%d,%d,%d,%d,%d,%d]\n",
           cm->ref_frame_map[0], cm->ref_frame_map[1],
           cm->ref_frame_map[2], cm->ref_frame_map[3],
           cm->ref_frame_map[4], cm->ref_frame_map[5],
           cm->ref_frame_map[6], cm->ref_frame_map[7]);
    */
