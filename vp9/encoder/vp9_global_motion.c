...
void refine_param(TransformationType type, unsigned char *frm,
  ...
  for(i = 0; i < n_refinements; i++) {
    ...
    //no improvement found-> means we're either already at a minimum or step is too wide
    step /= 4;
  }
...
int vp9_compute_global_motion_multiple_optical_flow(
    struct VP9_COMP *cpi,
    TransformationType type,
    YV12_BUFFER_CONFIG *frm,
    YV12_BUFFER_CONFIG *ref,
  ...
  (void) cpi;
  compute_flow(frm->y_buffer, ref->y_buffer, &flow,
  ...
  for (i = 10; i < frm->y_width - 10; ++i)
    for(j = 10; j < frm->y_height - 10; ++j) {
      if (flow.confidence[i + j * frm->y_width] < 0.1) {
        correspondences[num_correspondences].x = i;
        correspondences[num_correspondences].y = j;
        correspondences[num_correspondences].rx = i +
                                                  flow.u[i + j * frm->y_width];
        correspondences[num_correspondences].ry = j +
                                                  flow.v[i + j * frm->y_width];
        num_correspondences++;
      }
    }
#ifdef VERBOSE
