...
static int* get_gm_param(Global_Motion_Params *gm, int index) {
  switch (index) {
    case 0 :
      return &gm->rotation;
      break;
    case 1 :
      ...
      break;
    case 2 :
      return (int*) &gm->mv.as_mv.row;
      break;
    case 3 :
      return (int*) &gm->mv.as_mv.col;
      break;
  }
  assert(0);
  return NULL;
...
void refine_quant_param(Global_Motion_Params *gm, TransformationType type,
  ...
  double best_mse;
  double curr_mse;
  // TODO(sarahparker) this value is temporary, it could be better tuned
  int step = 1 << n_refinements;
  int *param;
  int curr_param;
  int best_param;
  projectPointsType projectPoints = get_projectPointsType(type);

  for (p = 0; i < get_numparams(type); ++i) {
    param = get_gm_param(gm, p);
    curr_param = *param;
    best_param = curr_param;
    curr_mse = compute_warp_and_error(gm, projectPoints, ref, ref_width, ref_height,
                                      ref_stride, frm, 0, 0, frm_width, frm_height,
                                      frm_stride, 0, 0, 16, 16);
    best_mse = curr_mse;
    for (i = 0; i < n_refinements; i++) {
      // look to the left
      *param = curr_param - step;
      step_mse = compute_warp_and_error(gm, projectPoints, ref, ref_width, ref_height,
                                        ref_stride, frm, 0, 0, frm_width, frm_height,
                                        frm_stride, 0, 0, 16, 16);
