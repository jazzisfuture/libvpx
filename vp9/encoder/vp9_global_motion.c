...
static double ComputeErrorScore(TransformationType type,
                                double *points1, int stride1,
                                double *points2, int stride2,
                                int npoints, double *H, int *map) {
  int i, n = 0;
  double pt[2];
  double *mp1 = points1;
  double *mp2 = points2;
  ...
  if (map) {
    for (i = 0; i < npoints; ++i, mp1+=stride1, mp2+=stride2) {
      if (map[i] != -1) {
        projectPoints(&H[map[i] * numparams], mp1, pt, 1, stride1, stride2);
...
static int ComputeTransformationSingle(TransformationType type,
                                       int *correspondences,
  ...
  if (!result) {
    for (i = 0; i < num_correspondences; ++i)
      inlier_map[i] = inlier_map[i] ? 0 : -1;
  }
  printf("Error Score (inliers) = %g\n",
         ComputeErrorScore(type, matched_points, 4, matched_points + 2, 4,
...
int ComputeGlobalTransformationSingle(TransformationType type,
  ...
  num_inliers = ComputeTransformationSingle(type, correspondences,
...
static int ComputeTransformationMultiple(TransformationType type,
                                         int *correspondences,
                                         int num_correspondences,
                                         double *H,
                                         int max_models,
  ...
  *num_models = 0;
  ...
  while ((double)num_inliers_sum / (double)num_correspondences < inlier_prob &&
    ...
    result = ransac(matched_points, num_points,
                    &num_inliers, best_inlier_mask, H);
    if (!result) {
      num_points = 0;
      for (i = 0; i < num_correspondences; ++i) {
        if (processed_mask[i] == -1) {
  ...
  if (*num_models) {
    printf("Error Score (inliers) %d models = %g\n", *num_models,
           ComputeErrorScore(type, matched_points, 4, matched_points + 2, 4,
                             num_correspondences, H, processed_mask));
  }
  free(matched_points);
...
int ComputeGlobalTransformationMultiple(TransformationType type,
                                        unsigned char *frmbuf,
                                        unsigned char *refbuf,
                                        int width,
                                        int height,
  ...
  inlier_map = (int *)malloc(num_correspondences * sizeof(int));
  printf("Computing Transformation\n");
  num_inliers = ComputeTransformationMultiple(type, correspondences,
                                              num_correspondences, H,
