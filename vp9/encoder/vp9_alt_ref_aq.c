...
static void vp9_alt_ref_aq_process_map(struct ALT_REF_AQ* const self) {
  ...
  /* vp9_matx_nth_element(&self->segmentation_map, &segm_map, 1, 3, self->nsegments); */

  /* for (i = 0; i < segm_map.rows; ++i) */
  /*   for (j = 0; j < segm_map.cols; ++j) { */
  /*     int idx = i*segm_map.stride + j; */

  /*     if (segm_map.data[idx] > self->segmentation_map.data[idx] + 2) */
  /*       self->segmentation_map.data[idx] = segm_map.data[idx]; */
  /*   } */
