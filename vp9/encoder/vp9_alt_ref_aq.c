...
void vp9_alt_ref_aq_upload_map(struct ALT_REF_AQ *const self,
  ...
  for (i = 0; i < self->segmentation_map.rows; ++i) {
    ...
    int y = i * segmentation_map->rows;
    y += self->segmentation_map.rows / 2;
    y /= self->segmentation_map.rows;
    y = VPXMIN(y, self->segmentation_map.rows);

    for (j = 0; j < self->segmentation_map.cols; ++j) {
      int x = i * segmentation_map->cols;
      x += self->segmentation_map.cols / 2;
      x /= self->segmentation_map.cols;
      x = VPXMIN(y, self->segmentation_map.cols);
