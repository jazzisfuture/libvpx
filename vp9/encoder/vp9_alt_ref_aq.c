...
static void vp9_alt_ref_aq_set_single_delta(struct ALT_REF_AQ *const self,
  ...
  float ndeltas = VPXMIN(self->nsteps, MAX_SEGMENTS - 1);
...
static void vp9_alt_ref_aq_get_overall_quality(struct ALT_REF_AQ *const self) {
  ...
  for (self->overall_quality = 0.0f, i = 1; i < self->nsegments; ++i)
    self->overall_quality += (float)i * self->segment_hist[i];

  self->overall_quality /=
      (float)(self->nsegments * self->segmentation_map.rows *
              self->segmentation_map.cols);
}
...
void vp9_alt_ref_aq_setup_mode(struct ALT_REF_AQ *const self,
  ...
  // TODO(yuryg): Otherwise there is a problem with tx_size_search_method ==
  // USE_LARGESTALL. This is a hacky workaround of course, and I need a fix.
  if (cpi->oxcf.speed > 2) return;
...
int vp9_alt_ref_aq_disable_if(const struct ALT_REF_AQ *self,
  ...
  for (i = 1, sum = 0; i < self->nsegments; ++i) sum += self->segment_hist[i];

  bytes_per_segment = segmentation_overhead / (float)sum;

  (void)bandwidth;

  return bytes_per_segment > ALT_REF_AQ_PROTECT_GAIN_THRESH;
}
