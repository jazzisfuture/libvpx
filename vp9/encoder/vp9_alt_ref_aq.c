...
static void vp9_alt_ref_aq_compress_segment_hist(struct ALT_REF_AQ *self) {
  ...
  for (i = 0, j = 0; i < self->nsegments; ++i) {
    ...
    if (self->segment_hist[i] > self->SEGMENT_MIN_AREA)
      ++j;
  }

  for (/* */;i < self->nsegments; ++i)
    self->segment_hist[i] = 0;
  ...
  self->__segment_changes[self->nsegments] = 0;
...
static void vp9_alt_ref_aq_set_single_delta(struct ALT_REF_AQ* const self,
  ...
  float ndeltas = VPXMIN(self->__nsteps, MAX_SEGMENTS - 1);
...
struct ALT_REF_AQ* vp9_alt_ref_aq_create() {
  ...
  //
  // 2) Possibly non-perfect representation of
  //    the all future frames with lookahead buffers
  //
  // 3) Block grid alignment during delta quantizer
  //    map estimation
  //
  // 1) Overhead for storing segmentation map
