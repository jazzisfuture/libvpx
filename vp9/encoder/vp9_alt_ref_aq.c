...
static void vp9_alt_ref_aq_make_map(ALT_REF_AQ* const self) {
  ...
  assert(segm_map->cn == 1);
  assert(segm_map->typeid == TYPE_8U);

  // compress segmentation map's range
  memset(segment_id_remap, 0, sizeof(segment_id_remap));

  // TODO(yuryg): logically I should assign greater not lower number
  for (i = 0, j = 0; i < self->nsegments; ++i)
    if (self->segment_hist[i] <= SEGMENT_AREA_EPS)
...
void vp9_alt_ref_aq_setup(ALT_REF_AQ* const self, VP9_COMP* const cpi) {
  ...
  // (we can avoid this copy, but this way it will be easier
  //  to apply any changes to the map before setting it up)
