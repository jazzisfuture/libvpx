...
static void fill_token_skipval_costs(vp9_coeff_skipval_cost *c,
                                     vp9_coeff_probs_model (*p)[PLANE_TYPES]) {
  int i, j, k, l;
  TX_SIZE t;
  for (t = TX_4X4; t < TX_SIZES; ++t)
    for (i = 0; i < PLANE_TYPES; ++i)
      for (j = 0; j < REF_TYPES; ++j)
        for (k = 0; k < COEF_BANDS; ++k) {
          for (l = 0; l < BAND_COEFF_CONTEXTS(k); ++l) {
            c[t][i][j][k][l] = vp9_cost_one(p[t][i][j][k][l][0]);
          }
        }
}
      ...
      for (j = 0; j < REF_TYPES; ++j)
        for (k = 0; k < COEF_BANDS; ++k) {
          for (l = 0; l < ZPC_PTOKS; ++l) {
            c[t][i][j][k][l][0] = vp9_cost_zero(p[t][i][j][k][l][0]);
            c[t][i][j][k][l][1] = vp9_cost_one(p[t][i][j][k][l][0]);
          }
        }
}
...
#endif  // CONFIG_TX_SKIP
#if CONFIG_CODE_ZEROGROUP
    fill_token_skipval_costs(x->token_skipval_costs, cm->fc.coef_probs);
