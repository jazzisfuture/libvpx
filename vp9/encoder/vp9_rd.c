...
int vp9_compute_rd_mult_based_on_qindex(const VP9_COMP *cpi, int qindex) {
  ...
  if (cpi->common.frame_type == KEY_FRAME) {
    ...
    rdmult =
        (int)((double)rdmult * def_rd_q_mult * rdc->rd_mult_key_qp_fac);
  } else if (!cpi->rc.is_src_frame_alt_ref &&
             (cpi->refresh_golden_frame || cpi->refresh_alt_ref_frame)) {
    double def_rd_q_mult = def_arf_rd_multiplier(qindex);
    rdmult =
        (int)((double)rdmult * def_rd_q_mult * rdc->rd_mult_arf_qp_fac);
  } else {
    double def_rd_q_mult = def_inter_rd_multiplier(qindex);
    rdmult =
        (int)((double)rdmult * def_rd_q_mult * rdc->rd_mult_inter_qp_fac);
  }
