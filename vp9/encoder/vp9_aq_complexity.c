...
#define AQ_C_LEVELS  3
static const double aq_c_q_adj_ratio[AQ_C_LEVELS][AQ_C_SEGMENTS] =
  {{1.0, 1.0, 1.0}, {1.0, 2.0, 1.0}, {1.0, 1.5, 2.5}};
static const int aq_c_active_segments[AQ_C_LEVELS] = {1,2,3};
static const double aq_c_transitions[AQ_C_LEVELS][AQ_C_SEGMENTS] =
...
void vp9_setup_in_frame_q_adj(VP9_COMP *cpi) {
  ...
  if (cm->frame_type == KEY_FRAME ||
    ...
    int segment;
    int aq_strength = get_aq_c_strength(cm->base_qindex);
    int active_segments = aq_c_active_segments[aq_strength];
    ...
    if (cpi->rc.sb64_target_rate < 128) {
    ...
    for (segment = 1; segment < active_segments; segment++) {
      int qindex_delta =
          vp9_compute_qdelta_by_rate(&cpi->rc, cm->frame_type, cm->base_qindex,
                                     aq_c_q_adj_ratio[aq_strength][segment]);
...
void vp9_select_in_frame_q_segment(VP9_COMP *cpi,
  ...
  if (!output_enabled) {
    ...
    while (segment > 0) {
      // Currently this code only supports segments with a reduced Q
      // i.e. aq_c_transitions[aq_strength][segment] < 1.0.
      if (projected_rate <
