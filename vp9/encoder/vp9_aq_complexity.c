...
static const double aq_c_q_adj_factor[AQ_C_SEGMENTS] =
  {2.5, 1.75, 1.25, 1.0, 0.80};
static const double aq_c_transitions[AQ_C_SEGMENTS] =
  {0.25, 0.5, 0.75, 2.00, 100.0};
static const double aq_c_var_thresholds[AQ_C_SEGMENTS] =
...
void vp9_caq_select_segment(VP9_COMP *cpi, MACROBLOCK *mb, BLOCK_SIZE bs,
  ...
  if (0) {
    ...
    for (i = 0; i < AQ_C_SEGMENTS; ++i) {
      ...
      if ((projected_rate <
          target_rate * aq_c_transitions[i]) &&
          (logvar < (low_var_thresh + aq_c_var_thresholds[i]))) {
        segment = i;
        break;
      }
    }
