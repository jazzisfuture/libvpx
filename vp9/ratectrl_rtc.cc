...
Result VP9RateControlRTC::ComputeQP(const VP9FrameParamsQpRTC &frame_params) {
  ...
  if (vp9_svc_check_skip_enhancement_layer(cpi_)) return kDrop;
  ...
  if (!frame_is_intra_only(cm) &&
    ...
    if (vp9_rc_drop_frame(cpi_)) return kDrop;
  }
