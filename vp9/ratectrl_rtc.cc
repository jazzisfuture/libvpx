...
VP9RateControlRTC::VP9RateControlRTC() {
  cpi_ = static_cast<VP9_COMP *>(vpx_memalign(32, sizeof(*cpi_)));
}
...
void VP9RateControlRTC::ComputeQP(const FrameParamsQpRTC &frame_params) {
  ...
  if (cpi_->svc.number_spatial_layers == 1 &&
    ...
    if (frame_is_intra_only(cm))
      target = calc_iframe_target_size_one_pass_cbr(cpi_);
    else
      target = calc_pframe_target_size_one_pass_cbr(cpi_);
    vp9_rc_set_frame_target(cpi_, target);
    if (cm->show_frame) update_buffer_level_preencode(cpi_);
  } else {
