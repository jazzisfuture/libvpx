...
std::unique_ptr<VP9RateControlRTC> VP9RateControlRTC::Create(
  ...
  if (rc_api->InitRateControl(cfg) != VPX_CODEC_OK) return nullptr;
  if (cfg.aq_mode) {
...
vpx_codec_err_t VP9RateControlRTC::InitRateControl(
  ...
  vpx_codec_err_t res = UpdateRateControl(rc_cfg);
...
vpx_codec_err_t VP9RateControlRTC::UpdateRateControl(
    const VP9RateControlRtcConfig &rc_cfg) {
