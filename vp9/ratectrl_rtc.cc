...
std::unique_ptr<VP9RateControlRTC> VP9RateControlRTC::Create(
  ...
  if (cfg.aq_mode) {
    rc_api->cpi_->segmentation_map = static_cast<uint8_t *>(vpx_calloc(
        rc_api->cpi_->common.mi_rows * rc_api->cpi_->common.mi_rows, 2));
    rc_api->cpi_->cyclic_refresh = vp9_cyclic_refresh_alloc(
        rc_api->cpi_->common.mi_rows, rc_api->cpi_->common.mi_cols);
    rc_api->cpi_->cyclic_refresh->content_mode = 0;
  }
  return rc_api;
}
