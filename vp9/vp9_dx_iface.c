  ...
  if (!res && !ctx->decoder_init) {
    ...
    if (!res) {
      ...
      if (!optr) {
        ...
        if (ctx->fb_list && ctx->realloc_fb_cb && ctx->fb_count > 0) {
          pbi->common.fb_list = ctx->fb_list;
          pbi->common.fb_count = ctx->fb_count;
          pbi->common.realloc_fb_cb = ctx->realloc_fb_cb;
          pbi->common.user_priv = ctx->user_priv;
        } else {
          pbi->common.fb_count = FRAME_BUFFERS;
        }
        CHECK_MEM_ERROR(cm, cm->yv12_fb,
                        vpx_calloc(cm->fb_count, sizeof(YV12_BUFFER_CONFIG)));
        CHECK_MEM_ERROR(cm, cm->fb_idx_ref_cnt,
                        vpx_calloc(cm->fb_count, sizeof(int)));
        ctx->pbi = optr;
      }
    }
...
static vpx_codec_err_t vp9_set_frame_buffers(
  ...
  if (fb_count < REF_FRAMES) {
    ...
    /* If decoder has already been initialized do not accept external
     * frame buffers. */
    ctx->fb_list = fb_list;
