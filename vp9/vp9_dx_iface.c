...
static int frame_worker_hook(void *arg1, void *arg2) {
  ...
  if (frame_worker_data->pbi->frame_parallel_decode) {
    // In frame parallel decode, a worker thread must successfully decodes all
    // the compressed data.
    if ((frame_worker_data->result != 0) ||
        (frame_worker_data->data + frame_worker_data->data_size -1 > data)) {
      VP9Worker *const worker = frame_worker_data->pbi->cur_buf->frame_worker_owner;
      // Notify all the other threads that are waiting for this frame.
...
static vpx_image_t *decoder_get_frame(vpx_codec_alg_priv_t *ctx,
  ...
  if (*iter == NULL && ctx->frame_workers != NULL) {
    do {
      ...
      if (worker->had_error) {
        // Decoding fail. Release the worker thread.
        ++ctx->available_threads;
        return img;
