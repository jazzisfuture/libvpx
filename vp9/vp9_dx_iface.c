...
struct vpx_codec_alg_priv {
  ...
  // BufferPool that holds all frames. Shared by all the FrameWorkers.
...
static vpx_codec_err_t init_decoder(vpx_codec_alg_priv_t *ctx) {
  ...
  if (ctx->buffer_pool == NULL) {
    set_error_detail(ctx, "Failed to allocate buffer pool");
    return VPX_CODEC_MEM_ERROR;
  }
...
static vpx_image_t *decoder_get_frame(vpx_codec_alg_priv_t *ctx,
  ...
  if (*iter == NULL && ctx->frame_workers != NULL) {
    ...
    if (vp9_get_raw_frame(worker_data->pbi, &sd, &flags) == 0) {
      ...
      ctx->img.fb_priv = pool->frame_bufs[cm->new_fb_idx].raw_frame_buffer.priv;
      img = &ctx->img;
      *iter = img;
      // Decrease reference count of last output frame in frame parallel mode.
      if (ctx->frame_parallel_decode && ctx->last_show_frame >= 0) {
        pthread_mutex_lock(&pool->pool_mutex);
        --pool->frame_bufs[ctx->last_show_frame].ref_count;
        if (pool->frame_bufs[ctx->last_show_frame].ref_count == 0) {
          pool->release_fb_cb(pool->cb_priv,
              &pool->frame_bufs[ctx->last_show_frame].raw_frame_buffer);
        }
