...
static vpx_codec_err_t decoder_init(vpx_codec_ctx_t *ctx,
  ...
  if (!ctx->priv) {
    ...
    ctx->priv->alg_priv->frame_parallel_decode = ((ctx->priv->alg_priv->cfg.threads > 1) &&
  ...
  if (!ctx->frame_parallel_decode) {
    ...
    *data = worker_data->data_end;
    ...
    if (worker_data->scratch_buffer_size < data_sz) {
      ...
      if (worker_data->scratch_buffer == NULL) {
        set_error_detail(ctx, "Failed to allocate scrath buffer");
        return VPX_CODEC_MEM_ERROR;
      }
...
static void release_last_output_frame(vpx_codec_alg_priv_t *ctx) {
  ...
  if (ctx->frame_parallel_decode && ctx->last_show_frame >= 0) {
    lock_buffer_pool(ctx->buffer_pool);
