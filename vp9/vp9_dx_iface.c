...
static vpx_codec_err_t init_decoder(vpx_codec_alg_priv_t *ctx) {
  ...
  if (ctx->frame_parallel_decode == 0) {
    ...
    if (ctx->frame_workers == NULL) {
      ctx->base.err_detail = "Failed to allocate frame_workers";
      return VPX_CODEC_MEM_ERROR;
    }
  ...
  for (i = 0; i < ctx->num_frame_workers; ++i) {
    ...
    if (worker->data1 == NULL) {
      ctx->base.err_detail = "Failed to allocate worker_data";
      return VPX_CODEC_MEM_ERROR;
    }
...
static vpx_codec_err_t ctrl_set_reference(vpx_codec_alg_priv_t *ctx,
  ...
  if (ctx->frame_parallel_decode == 1) {
    ctx->base.err_detail = "Not supported in frame parallel decode";
