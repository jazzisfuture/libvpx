...
libvpx_test_data_url=http://downloads.webmproject.org/test_data/libvpx/$(notdir $1)

$(LIBVPX_TEST_DATA):
	@echo "    [DOWNLOAD] $@"
	$(qexec)trap 'rm -f $@' SIGINT SIGTERM &&\
            curl -L -o $@ $(call libvpx_test_data_url,$@)

testdata:: $(LIBVPX_TEST_DATA)
	$(qexec)if [ -x "$$(which sha1sum)" ]; then\
            echo "Checking test data:";\
            cat $(SRC_PATH_BARE)/test/test-data.sha1 |\
