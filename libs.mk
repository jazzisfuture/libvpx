...
TEST_LPF_SPEED_BIN=./test_lpf_speed$(EXE_SFX)
TEST_LPF_SPEED_SRCS=$(addprefix test/,$(call enabled,TEST_LPF_SPEED_SRCS))
TEST_LPF_SPEED_OBJS := $(sort $(call objs,$(TEST_LPF_SPEED_SRCS)))
...
ifneq ($(strip $(TEST_LPF_SPEED_OBJS)),)
PROJECTS-$(CONFIG_MSVS) += test_intra_pred_speed.$(VCPROJ_SFX)
test_intra_pred_speed.$(VCPROJ_SFX): $(TEST_LPF_SPEED_SRCS) vpx.$(VCPROJ_SFX) gtest.$(VCPROJ_SFX)
	@echo "    [CREATE] $@"
	$(qexec)$(GEN_VCPROJ) \
            --exe \
            --target=$(TOOLCHAIN) \
...
ifneq ($(strip $(TEST_LPF_SPEED_OBJS)),)
$(TEST_LPF_SPEED_OBJS) $(TEST_LPF_SPEED_OBJS:.o=.d): CXXFLAGS += $(GTEST_INCLUDES)
OBJS-yes += $(TEST_LPF_SPEED_OBJS)
BINS-yes += $(TEST_LPF_SPEED_BIN)

$(TEST_LPF_SPEED_BIN): $(TEST_LIBS)
$(eval $(call linkerxx_template,$(TEST_LPF_SPEED_BIN), \
              $(TEST_LPF_SPEED_OBJS) \
              -L. -lvpx -lgtest $(extralibs) -lm))
endif  # TEST_LPF_SPEED
...
INSTALL-SRCS-$(CONFIG_CODEC_SRCS) += $(TEST_LPF_SPEED_SRCS)
